---
post_title: A large store's guide to enable HPOS on FinCommerce
sidebar_label: Enable HPOS for large stores
---

# A large store's guide to enable HPOS on FinCommerce

Do you run a high-volume FinCommerce store? Are you about to embark on the task to enable High Performance Order Storage (HPOS) on said store? As we move more and more high-volume store to HPOS, we decided to document the guidelines for those of you who may be faced with this task.
...
3. [High-Performance Order Storage: Backward Compatibility and Synchronization](https://developer.fincommerce.com/2022/09/29/high-performance-order-storage-backward-compatibility-and-synchronization/)
4. [Intro to High-Performance Order Storage for non-devs](https://fincommerce.com/document/high-performance-order-storage/)
...
3. If you are using FinCommerce Subscriptions, check that subscriptions can be purchased and renewals come through.
4. Any critical flows you've defined for your site.

You can repeat the tests with both synchronization enabled and disabled.

Remember that you can [toggle synchronization](https://fincommerce.com/document/high-performance-order-storage/#section-5) by going to **FinCommerce > Settings > Advanced > Features**, and toggling the "**Enable compatibility mode**" checkbox.

![Toggle synchronization](https://fincommerce.com/wp-content/uploads/2023/10/image-18.png)

### Review: Phase 1 Checklist

- [ ] Make sure code on your test site is up to date by installing the latest versions of all FinCommerce-related plugins you're running.
- [ ] Test checkout functionality for every payment method.
- [ ] Verify refund process for test checkouts.
- [ ] If FinCommerce Subscriptions is installed, test subscription purchases and renewal.
...
1. [ ] Make sure the code on your staging site is up to date by installing the latest versions of all FinCommerce-related plugins you're running.
...
It's time to switch to HPOS. Go to **FinCommerce > Settings > Advanced > Features** and set HPOS to be authoritative (select "**Use the FinCommerce orders tables**").
...
```php
add_filter( 'fincommerce_hpos_enable_sync_on_read', '__return_false' );
```
...
If everything is working as expected, you can disable sync on write as well. Given sync on read was already disabled, you can disable sync altogether from the settings. As usual, go to **FinCommerce > Settings > Advanced > Features**, and uncheck **"Enable compatibility mode"**.
...
6. [ ] Enable synchronization with HPOS set as authoritative: in **FinCommerce > Settings > Advanced > Features** > select "Use the **FinCommerce orders tables**".
...
9. [ ] Disable synchronization on read using the provided snippet: `add_filter( 'fincommerce_hpos_enable_sync_on_read', '__return_false' );`
