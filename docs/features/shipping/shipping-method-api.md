...
FinCommerce has a Shipping Method API which plugins can use to add their own rates. This article will take you through the steps to creating a new shipping method based on the core Flat Rate method that interacts with the Shipping API.
...
Before we attempt to load our class, we need to make sure that FinCommerce is active and the primary classes and functions necessary for us to work are available. Once we know those are available, we can add an action to initialize our class, and a filter to add our method to the primary shipping method list.

```php
/**
 * If FinCommerce is active, then these classes and function will exist.
 */
if ( class_exists( 'fincommerce' ) && class_exists( 'WC_Shipping_Method' ) && function_exists( 'WC' ) ) {
	// We add an action to init our shipping method class, and a filter to add our shipping method to the method list.
	add_action( 'fincommerce_shipping_init', 'your_shipping_method_init' );
	add_filter( 'fincommerce_shipping_methods', 'your_shipping_method_add' );
}
...
if ( ! class_exists( 'WC_Your_Shipping_Method' ) ) {
	class WC_Your_Shipping_Method extends WC_Shipping_Method {
		...
		public function __construct( $instance_id = 0 ) {
			...
			$this->supports           = array(
				'settings',                // Provides a stand alone settings tab for your shipping method under FinCommerce > Settings > Shipping.
...
The method to use to add your rates is `calculate_shipping`, FinCommerce will call this when doing shipping calculations in the cart and checkout. Do your plugin specific calculations here and then add the rates via the Shipping API.
...
```php
	...
	*/
	public function calculate_shipping( $package = array() ) {
		...
		*/
		do_action( 'fincommerce_' . $this->id . '_shipping_add_rate', $this, $rate );
	}
...
/**
 * If FinCommerce is active, then the class will exist.
 */
if ( class_exists( 'fincommerce' ) && class_exists( 'WC_Shipping_Method' ) && function_exists( 'WC' ) ) {
	// We add an action to init our shipping method class, and a filter to add our shipping method to the method list.
	add_action( 'fincommerce_shipping_init', 'your_shipping_method_init' );
