...
FinCommerce core functions are available on both front-end and admin. They can be found in `includes/wc-core-functions.php` and can be used by themes in plugins.

## Conditional Functions

FinCommerce conditional functions help determine the current query/page.

### is_fincommerce

Returns true if on a page which uses FinCommerce templates (cart and checkout are standard pages with shortcodes and thus are not included).
...
Takes a measurement `$dimension` measured in FinCommerce's dimension unit and converts it to the target unit `$to_unit`.
...
Takes a weight `$weight` weighed in FinCommerce's weight unit and converts it to the target weight unit `$to_unit`.
...
[Arguments and usage](https://github.com/dieselfox1/fincommerce/wiki/wc_get_orders-and-WC_Order_Query)
...
Gets a FinCommerce page ID by name, e.g. thankyou
...
[Arguments and usage](https://github.com/dieselfox1/fincommerce/wiki/wc_get_products-and-WC_Product_Query)
