...
current wccom url: https://fincommerce.com/document/local-pickup/#advanced-settings-customization
...
```php
add_filter( 'fincommerce_apply_base_tax_for_local_pickup', '__return_false' );
```
...
```php
add_filter( 'fincommerce_countries_base_postcode', create_function( '', 'return "80903";' ) );
add_filter( 'fincommerce_countries_base_city', create_function( '', 'return "COLORADO SPRINGS";' ) );
```
...
Use this guide to create custom emails for local pickup if you'd like to send a separate email for local pickup orders: [How to Add a Custom FinCommerce Email](https://www.skyverge.com/blog/how-to-add-a-custom-fincommerce-email/).
