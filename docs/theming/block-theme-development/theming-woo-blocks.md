...
FinCommerce comes with several [block templates](https://github.com/dieselfox1/fincommerce/tree/trunk/plugins/fincommerce/templates/templates/blockified) by default. Those are:
...
- It's possible to override the templates by creating a file with the same file name under the `/templates` folder. For example, if a block theme contains a `wp-content/themes/yourtheme/templates/single-product.html` template, it will take priority over the FinCommerce default Single Product template.
...
FinCommerce also comes with two specific [block template parts](https://github.com/dieselfox1/fincommerce/tree/trunk/plugins/fincommerce/templates/parts):
