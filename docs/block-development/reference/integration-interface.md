...
You should use the hooks: `fincommerce_blocks_mini-cart_block_registration`. `fincommerce_blocks_cart_block_registration` and `fincommerce_blocks_checkout_block_registration`. These hooks pass an instance of [`IntegrationRegistry`](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/blob/trunk/src/Integrations/IntegrationRegistry.php) to the callback.

You may then use the `register` method on this object to register your `IntegrationInterface`.

## `IntegrationInterface` methods

To begin, we'll need to create our integration class, our `IntegrationInterface`. This will be a class that implements FinCommerce Blocks' interface named [`IntegrationInterface`](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/blob/trunk/src/Integrations/IntegrationInterface.php).
...
Let's suppose we're the author of an extension: `FinCommerce Example Plugin`. We want to enqueue scripts, styles, and data on the frontend when either the Mini-Cart, Cart or Checkout blocks are being used.
 ...
 */
class fincommerce_Example_Plugin_Integration implements IntegrationInterface {
	/**
	...
	public function get_name() {
		return 'fincommerce-example-plugin';
	}
     ...
     * we will assume \fincommerce_Example_Plugin_Assets::$plugin_file is a valid file path
    */
		$script_url = plugins_url( $script_path, \fincommerce_Example_Plugin_Assets::$plugin_file );
		$style_url = plugins_url( $style_path, \fincommerce_Example_Plugin_Assets::$plugin_file );

		$script_asset_path = dirname( \fincommerce_Example_Plugin_Assets::$plugin_file ) . '/build/index.asset.php';
		...
		wp_set_script_translations(
			'wc-blocks-integration',
			'fincommerce-example-plugin',
			dirname( \fincommerce_Example_Plugin_Assets::$plugin_file ) . '/languages'
		);
	}
	...
	public function get_script_data() {
	    $fincommerce_example_plugin_data = some_expensive_serverside_function();
	    return [
	        'expensive_data_calculation' => $fincommerce_example_plugin_data
        ];
	}
	...
	protected function get_file_version( $file ) {
		...
		// As above, let's assume that fincommerce_Example_Plugin_Assets::VERSION resolves to some versioning number our
		// extension uses.
		return \fincommerce_Example_Plugin_Assets::VERSION;
	}
}
```

As mentioned, we will need register our `IntegrationInterface` with FinCommerce Blocks, as we want our scripts to be included when either the Mini-Cart, Cart or Checkout is used, we need to register callbacks for three actions.

```php
add_action(
    'fincommerce_blocks_mini-cart_block_registration',
    function( $integration_registry ) {
        $integration_registry->register( new fincommerce_Example_Plugin_Integration() );
    }
);
add_action(
    'fincommerce_blocks_cart_block_registration',
    function( $integration_registry ) {
        $integration_registry->register( new fincommerce_Example_Plugin_Integration() );
    }
);
...
In the `@fincommerce/settings` package there is a method you can import called `getSetting`. This method accepts a string. The name of the setting containing the data added in `get_script_data` is the name of your integration (i.e. the value returned by `get_name`) suffixed with `_data`. In our example it would be: `fincommerce-example-plugin_data`.
