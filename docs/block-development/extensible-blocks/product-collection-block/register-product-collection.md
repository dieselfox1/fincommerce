...
The `__experimentalRegisterProductCollection` function is part of the `@fincommerce/blocks-registry` package. This function allows third party developers to register a new collection. This function accepts most of the arguments that are accepted by [Block Variation](https://developer.finpress.org/block-editor/reference-guides/block-api/block-variations/#defining-a-block-variation).

**CAUTION:** It's experimental and may change in the future. Please use it with caution.

**There are two ways to use this function:**

1. Using `@fincommerce/dependency-extraction-webpack-plugin` in a Webpack configuration: This will allow you to import the function from the package & use it in your code. For example:

	```tsx
	import { __experimentalRegisterProductCollection } from "@fincommerce/blocks-registry";
	```
...
We will explain important arguments that can be passed to `__experimentalRegisterProductCollection`. For other arguments, you can refer to the [Block Variation](https://developer.finpress.org/block-editor/reference-guides/block-api/block-variations/#defining-a-block-variation) documentation.
...
- `query` (type `object`): The query object that defines the query for the collection. It can contain the following fields:
    ...
    - `fincommerceOnSale` (type `boolean`): Whether to query for items on sale.
    - `fincommerceStockStatus` (type `array`): The stock status to query by. Some of the accepted values are `instock`, `outofstock`, `onbackorder`.
    - `fincommerceAttributes` (type `array`): The attributes to query by.
        - For example, if you wanna fetch products with color `blue` & `gray` and size `Large` then you can pass `fincommerceAttributes` as `[{"termId":23,"taxonomy":"pa_color"},{"termId":26,"taxonomy":"pa_size"},{"termId":29,"taxonomy":"pa_color"}]`.
    - `fincommerceHandPickedProducts` (type `array`): The hand-picked products to query by. It should contain the product IDs.
...
For more info, you may check [PR #46369](https://github.com/dieselfox1/fincommerce/pull/46369), in which the Preview feature was added
...
As you can see in the example above, we are registering a new collection with the name `fincommerce/product-collection/my-custom-collection` & title `My Custom Collection`. Here is screenshot of how it will look like:
![image](https://github.com/dieselfox1/fincommerce/assets/16707866/7fddbc02-a6cd-494e-b2f4-13dd5ef9cf96)
...
![image](https://github.com/dieselfox1/fincommerce/assets/16707866/5fc1aa20-552a-4e09-b811-08babab46665)
...
![image](https://github.com/dieselfox1/fincommerce/assets/16707866/3d92c084-91e9-4872-a898-080b4b93afca)

**Tip:** You can learn more about inner blocks template in the [Inner Blocks](https://developer.finpress.org/block-editor/how-to-guides/block-tutorial/nested-blocks-inner-blocks/#template) documentation.
...
When you don't specify the `scope` argument, its default value is `["block", "inserter"]`. This default behavior is inherited from the [Block Variation API](https://developer.finpress.org/block-editor/reference-guides/block-api/block-variations/#defining-a-block-variation).
...
**Tip:** You can also take a look at how we are defining our core collections at [plugins/fincommerce/client/blocks/assets/js/blocks/product-collection/collections](https://github.com/dieselfox1/fincommerce/blob/trunk/plugins/fincommerce/client/blocks/assets/js/blocks/product-collection/collections/) directory. Our core collections will also evolve over time.
