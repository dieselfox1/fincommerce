...
![Showing the contact information section with two fields rendered, email and an additional checkout field (optional)](https://github.com/dieselfox1/fincommerce/assets/5656702/097c2596-c629-4eab-9604-577ee7a14cfe)
...
![The shipping address form showing the additional checkout field at the bottom](https://github.com/dieselfox1/fincommerce/assets/5656702/746d280f-3354-4d37-a78a-a2518eb0e5de)
...
![The order information section containing an additional checkout field](https://github.com/dieselfox1/fincommerce/assets/5656702/295b3048-a22a-4225-96b0-6b0371a7cd5f)
...
![The order information block in the post editor"](https://github.com/dieselfox1/fincommerce/assets/5656702/05a3d7d9-b3af-4445-9318-443ae2c4d7d8)
...
To register additional checkout fields you must use the `fincommerce_register_additional_checkout_field` function.

It is recommended to run this function after the `fincommerce_init` action.
...
| `validate_callback` | A function called to validate the customer provided value when posted. This runs _after_ sanitization.                              | No        | See example below                            | The default validation function will add an error to the response if the field is required and does not have a value. [See the default validation function.](https://github.com/dieselfox1/fincommerce/blob/trunk/plugins/fincommerce/src/Blocks/Domain/Services/CheckoutFields.php#L270-L281) |
...
![The shipping address form with the Government ID field rendered at the bottom](https://github.com/dieselfox1/fincommerce/assets/5656702/f6eb3c6f-9178-4978-8e74-e6b2ea353192)
...
![The contact information section with a newsletter subscription checkbox rendered inside it](https://github.com/dieselfox1/fincommerce/assets/5656702/7444e41a-97cc-451d-b2c9-4eedfbe05724)
...
![The select input before being focused](https://github.com/dieselfox1/fincommerce/assets/5656702/bbe17ad0-7c7d-419a-951d-315f56f8898a)

### The select input when focused

![The select input when focused](https://github.com/dieselfox1/fincommerce/assets/5656702/bd943906-621b-404f-aa84-b951323e25d3)
...
#### Using the `fincommerce_sanitize_additional_field` filter

To run a custom sanitization function for a field you can use the `sanitize_callback` function on registration, or the `fincommerce_sanitize_additional_field` filter.
...
```php
add_action(
	'fincommerce_sanitize_additional_field',
...
##### Using the `fincommerce_validate_additional_field` action
...
```php
add_action(
	'fincommerce_validate_additional_field',
...
There are cases where the validity of a field depends on the value of another field, for example validating the format of a government ID based on what country the shopper is in. In this case, validating only single fields (as above) is not sufficient as the country may be unknown during the `fincommerce_validate_additional_field` action.
...
##### Using the `fincommerce_blocks_validate_location_{location}_fields` action
...
- When checking out using the Checkout block or Store API.
    - `fincommerce_blocks_validate_location_address_fields` (x2)
    - `fincommerce_blocks_validate_location_contact_fields`
    - `fincommerce_blocks_validate_location_other_fields`
- When updating addresses in the "My account" area
    - `fincommerce_blocks_validate_location_address_fields` (**x1** - only the address being edited)
...
```php
add_action(
	'fincommerce_blocks_validate_location_address_fields',
...
If these fields were rendered in the "contact" location instead, the code would be the same except the hook used would be: `fincommerce_blocks_validate_location_contact_fields`.
...
You can react to those fields being saved by hooking into `fincommerce_set_additional_field_value` action.

```php
add_action(
	'fincommerce_set_additional_field_value',
...
You can use the `fincommerce_get_default_value_for_{$key}` filters to provide a different default value (a value coming from another meta field for example):

```php
add_filter(
	"fincommerce_get_default_value_for_my-plugin-namespace/address-field",
...
add_filter(
	"fincommerce_get_default_value_for_my-plugin-namespace/my-other-field",
	function ( $value, $group, $wc_object ) {
