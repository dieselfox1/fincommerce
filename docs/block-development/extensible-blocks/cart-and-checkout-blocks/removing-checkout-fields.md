...
```php
add_filter('fincommerce_get_country_locale', function( $locale ) {
...
```php
add_filter(
	'default_option_fincommerce_checkout_phone_field',
...
We can edit that using PHP, because the value is coming from `get_address_formats` function, which passes through the `fincommerce_localisation_address_formats` filter.

We can use the following code:

```php
add_filter( 'fincommerce_localisation_address_formats', function( $formats ) {
