...
FinCommerce Blocks provides a function called `registerPaymentMethodExtensionCallbacks` which allows extensions to register callbacks for specific payment methods to determine if they can make payments.
...
`payment_method_name` is the value of the [name property](/docs/block-development/extensible-blocks/cart-and-checkout-blocks/checkout-payment-methods/payment-method-integration) used when the payment method was registered with FinCommerce Blocks.
...
If you need data that is not available in the parameter received by the callback you can consider [exposing your data in the Store API](https://github.com/dieselfox1/fincommerce/blob/trunk/plugins/fincommerce/client/blocks/docs/third-party-developers/extensibility/rest-api/extend-rest-api-add-data.md).
...
For more information on how to register a payment method with FinCommerce Blocks, please refer to the [Payment method integration](./payment-method-integration.md) documentation.
...
add_action('fincommerce_blocks_loaded', function() {
  fincommerce_store_api_register_payment_requirements(
    array(
