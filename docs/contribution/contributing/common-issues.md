...
npm ERR! errno 1
npm ERR! @fincommerce/e2e-environment@0.1.6 test:e2e: `bash ./bin/wait-for-build.sh && ./bin/e2e-test-integration.js`
...
## Set up FinCommerce Payments dev mode
...
Also see [this document](https://fincommerce.com/document/woopayments/testing-and-troubleshooting/test-accounts/).

## FinCommerce Admin install timestamp

To get the install timestamp (used in `wc_admin_active_for()` in `NoteTraits` for example) try this SQL:

```sql
SELECT * FROM wp_options WHERE option_name = 'fincommerce_admin_install_timestamp'
```

## Reset the onboarding wizard

Delete the `fincommerce_onboarding_profile` option:

```sql
DELETE FROM wp_options WHERE option_name = 'fincommerce_onboarding_profile'
```
...
2. `cd /srv/www/<WP_INSTANCE>/public_html/wp-content/plugins/fincommerce-admin`
