...
```php
function fincommerce_get_current_promotions( ...$args ) {
    /* Any initial prep, then first lifecycle hook... */
    do_action( 'fincommerce_before_get_current_promotions', $args );
    /* ...Do actual work, then final lifecycle hook... */
    do_action( 'fincommerce_after_get_current_promotions', $result, $args );
    /* ...Return the result, optionally via a filter... */
    return apply_filters( 'fincommerce_get_current_promotions', $result, $args );
}
```
...
function get_product_metrics( $args ): array {
    $pre = apply_filters( 'pre_fincommerce_get_product_metrics', null, $args );
...
```php
function fincommerce_format_sale_price( ...$args ): string {
    /* Prep to fill in any missing $args values... */
    $args = (array) apply_filters( 'fincommerce_format_sale_price_args', $args );
    /* ...Actual work to determine the $price string... */
    return (string) apply_filters( 'fincommerce_format_sale_price', $price, $args );
}
```
...
function get_featured_product_for_current_customer( ) {
    ...
    return apply_filters( 
        'fincommerce_featured_product_for_current_customer', 
        $product, /* WC_Product */
        $customer 
    );
}
...
function on_ajax_order_creation() {
    /* Avoid this! */
    do_action( 'fincommerce_on_order_creation' );
}
```
...
```php
/* Avoid this */
$super_products_enabled = (bool) apply_filters( 'fincommerce_super_products_are_enabled', true );

/* Prefer this */
$super_products_enabled = get_option( 'fincommerce_super_products_are_enabled', 'no' ) === 'yes';
```
...
if ( is_wp_error( $result ) ) {
    ...
    do_action( "fincommerce_foo_bar_{$error_code}_problem", $intermediate_result );
    
    /* Prefer this */
    do_action( 'fincommerce_foo_bar_problem', $result );
}
```
