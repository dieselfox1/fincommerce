...
Setup tasks appear on the FinCommerce Admin home screen and prompt a merchant to complete certain steps in order to set up your extension. Adding tasks is a two-step process that requires:
...
Finally, we register another plugin named "my-task-list-item-plugin." This plugin is used to customize the appearance of task list items. It also targets FinCommerce's task list and wraps the DefaultTaskItem component within a custom wrapper with additional styling.
...
In summary, the JavaScript file for a simple task extends and customizes the functionality of FinCommerce's task list, allowing users to better manage tasks and personalize the appearance of task list items.
...
function add_task_register_script() {
	if (
		! class_exists( 'Automattic\FinCommerce\Internal\Admin\Loader' ) ||
		! \Automattic\FinCommerce\Admin\PageController::is_admin_or_embed_page()
	) {
		return;
	}
...
When a merchant completes all of the items on the onboarding task list, FinCommerce replaces it with a section containing a list of handy store management links. Discoverability can be a challenge for extensions, so this section is a great way to bring more attention to key features of your extension and help merchants navigate to them.

The store management section has a relatively narrow purpose, so this section does not currently support external links. Instead, it is meant for navigating quickly within FinCommerce.
...
Admin Notes are meant for displaying insightful information about your FinCommerce store, extensions, activity, and achievements. They're also useful for displaying information that can help with the day-to-day tasks of managing and optimizing a store. A good general rule is to use Admin Notes for information that is:
...
Despite being a part of the new React-powered admin experience in FinCommerce, Admin Notes are available to developers via a standard PHP interface.

The recommended approach for using Admin Notes is to encapsulate your note within its own class that uses the [NoteTraits](https://github.com/dieselfox1/fincommerce/blob/trunk/plugins/fincommerce/src/Admin/Notes/NoteTraits.php) trait included with FinCommerce Admin. Below is a simple example of what this might look like:
...
if ( ! class_exists( 'Automattic\FinCommerce\Admin\Notes\Notes' ) ||
     ! class_exists( 'Automattic\FinCommerce\Admin\Notes\NoteTraits' )) {
    return;
}
 
// Make sure the FinCommerce Data Store is available
...
class ExampleNote {
    ...
    use Automatic\FinCommerce\Admin\Notes\Note;
 
    // Use the NoteTraits trait, which handles common note operations.
    use Automatic\FinCommerce\Admin\Notes\NoteTraits;
    ...
    // other logic here depending on what data your note should contain.
        ...
        $note = new Automattic\FinCommerce\Admin\Notes\Note();
...
Next, we define a simple class that will serve as a note provider for our note. To create and manage note objects, we'll import the `Note` and `NotesTraits` classes from FinCommerce Admin.
