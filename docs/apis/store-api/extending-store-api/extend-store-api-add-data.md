...
```php
use Automattic\FinCommerce\StoreApi\Schemas\V1\CartSchema;
...
Also note that the dependency injection container is not available until after the `fincommerce_blocks_loaded` action has been fired, so you should hook your file that action:
...
-   `fincommerce_store_api_register_endpoint_data( $args )`
-   `fincommerce_store_api_register_update_callback( $args )`
-   `fincommerce_store_api_register_payment_requirements( $args )`
-   `fincommerce_store_api_get_formatter( $name )`

### Errors and fatals are silence for non-admins

If your callback functions `data_callback` and `schema_callback` throw an exception or an error, or you passed the incorrect type of parameter to `register_endpoint_data`; that error would be caught and logged into FinCommerce error logs. If the current user is a shop manager or an admin, and has WP_DEBUG enabled, the error would be surfaced to the frontend.
...
This is a complete example that shows how you can register contextual FinCommerce Subscriptions data in each cart item (simplified). This example uses [Formatters](./extend-store-api-formatters.md), utility classes that allow you to format values so that they are compatible with the StoreAPI.

```php
<?php
/**
 * FinCommerce Subscriptions Extend Store API.
 *
 * A class to extend the store public API with subscription related data
 * for each subscription item
 *
 * @package FinCommerce Subscriptions
 */
use Automattic\FinCommerce\StoreApi\StoreApi;
use Automattic\FinCommerce\StoreApi\Schemas\ExtendSchema;
use Automattic\FinCommerce\StoreApi\Schemas\V1\CartItemSchema;
	...
	public static function extend_cart_item_schema() {
		return array(
			'billing_period'      => array(
				'description' => __( 'Billing period for the subscription.', 'fincommerce-subscriptions' ),
			...
			'billing_interval'    => array(
				'description' => __( 'The number of billing periods between subscription renewals.', 'fincommerce-subscriptions' ),
				'type'        => array( 'integer', 'null' ),
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			),
			'subscription_length' => array(
				'description' => __( 'Subscription Product length.', 'fincommerce-subscriptions' ),
				'type'        => array( 'integer', 'null' ),
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			),
			'trial_period'        => array(
				'description' => __( 'Subscription Product trial period.', 'fincommerce-subscriptions' ),
				'type'        => array( 'string', 'null' ),
				'enum'        => array_keys( wcs_get_subscription_period_strings() ),
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			...
			'trial_length'        => array(
				'description' => __( 'Subscription Product trial interval.', 'fincommerce-subscriptions' ),
				'type'        => array( 'integer', 'null' ),
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			),
			'sign_up_fees'        => array(
				'description' => __( 'Subscription Product signup fees.', 'fincommerce-subscriptions' ),
				'type'        => array( 'string', 'null' ),
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			),
			'sign_up_fees_tax'    => array(
				'description' => __( 'Subscription Product signup fees taxes.', 'fincommerce-subscriptions' ),
				'type'        => array( 'string', 'null' ),
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			),
