 ...
 * External dependencies
 */
const wcAdminWebpackConfig = require( '../../plugins/fincommerce/client/admin/webpack.config.js' );
...
module.exports = ( storybookConfig ) => {
	...
	wcAdminPackages.forEach( ( name ) => {
		storybookConfig.resolve.alias[ `@fincommerce/${ name }` ] =
			path.resolve( __dirname, `../../packages/js/${ name }/src` );
	} );

	storybookConfig.resolve.alias[ '@fincommerce/settings' ] = path.resolve(
		__dirname,
		'./setting.mock.js'
	);
	...
	storybookConfig.resolve.modules = [
		path.join( __dirname, '../../plugins/fincommerce/client/admin/client' ),
		...
		new CopyWebpackPlugin( {
			patterns: [
				...
				},
				{
					from: path.resolve(
						__dirname,
						`../../plugins/fincommerce/client/admin/build/app/*.css`
					),
					to: `./app-css/[name][ext]`,
				},
			],
