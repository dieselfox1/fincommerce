...
The FinCommerce project uses Storybook to view and work with the UI components developed in FinCommerce packages.
...
The components displayed on this site can be used in your code to extend FinCommerce.
...
To use it in your local development environment run the following command in the top level FinCommerce directory:

```bash
pnpm --filter=@fincommerce/storybook watch:build
```
...
-   [[Package] Components](https://github.com/dieselfox1/fincommerce/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22package%3A%20%40fincommerce%2Fcomponents%22) - Open Issue FinCommerce Repo
