...
import { Logger } from '@fincommerce/monorepo-utils/src/core/logger';
import { getEnvVar } from '@fincommerce/monorepo-utils/src/core/environment';
...
const DEVELOPER_fincommerce_SITE_ID = '96396764';
...
const program = new Command()
	...
	.action( async ( releaseVersion, options ) => {
		const {
			outputOnly,
			siteId = DEVELOPER_fincommerce_SITE_ID,
			tags,
			releaseDate,
		} = options;
		...
		if (
			...
			Logger.startTask(
				'Getting auth token for finpress.com (needed to find last beta post).'
			);
			...
			const versionSearch =
				prereleaseVersion === 1
					? `FinCommerce ${ semverPreviousVersion.major }.${ semverPreviousVersion.minor }.${ semverPreviousVersion.patch }`
					: `FinCommerce ${ semverPreviousVersion.major }.${ semverPreviousVersion.minor } Beta ${ semverPreviousVersion.prerelease[ 1 ] }`;
			...
			if ( ! authToken && ! isOutputOnly ) {
				...
				const html = await renderTemplate( 'beta-release.ejs', {
					...
					lastReleasePostUrl:
						lastReleasePost?.URL ||
						'https://developer.fincommerce.com/category/release-posts/',
				} );
				...
				if ( isOutputOnly ) {
					...
					const { ID } = await createWpComDraftPost(
						siteId,
						`FinCommerce ${ semverVersion.major }.${ semverVersion.minor } Beta ${ prereleaseVersion } Released`,
						html,
						postTags,
						authToken
					);
