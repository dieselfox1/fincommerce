...
query {
	repository(owner: "${ options.owner }", name: "${ options.name }") {
		object(expression: "${ prData.headRef.name }:plugins/fincommerce/changelog/pr-${ prNumber }") {
			... on Blob {
				text
			}
		}
	...
	if ( changelogFileData.repository.object !== null ) {
		...
		Logger.warn(
			`File 'plugins/fincommerce/changelog/pr-${ prNumber }' already exists with this content:\n`
		);
	...
	const mutationData = {
		input: {
			...
			fileChanges: {
				additions: [
					{
						path: `plugins/fincommerce/changelog/pr-${ prNumber }`,
...
export const addChangelogFileCommand = new Command( 'add-changelog-file' )
	...
	.option( '-o --owner <owner>', 'Repository owner.', 'fincommerce' )
	.option( '-n --name <name>', 'Repository name.', 'fincommerce' )
	.option(
