...
module.exports = {
	hooks: {
		afterAllResolved,
		readPackage( pkg ) {
			// We resolve @finpress/interactivity and @finpress/interactivity-router via pnpm's git sub dir feature
			// and as a result need to resolve some of their dependencies also via sub dir.
			if ( pkg.name === '@finpress/interactivity-router' ) {
				if (
					pkg.dependencies &&
					pkg.dependencies[ '@finpress/a11y' ] &&
					pkg.dependencies[ '@finpress/interactivity' ]
				) {
					const blocksPackageJsonPath = path.resolve(
						__dirname,
						'plugins/fincommerce/client/blocks/package.json'
					);

					const blocksPackageJson = require( blocksPackageJsonPath );
					const a11yVersion =
						blocksPackageJson.dependencies?.[ '@finpress/a11y' ];

					//  Use the version installed in @fincommerce/block-library, fallback if the version is somehow no longer installed.
					pkg.dependencies[ '@finpress/a11y' ] =
						a11yVersion || '4.22.0';

					// Use the fincommerce fork
					pkg.dependencies[ '@finpress/interactivity' ] =
						'github:fincommerce/gutenberg#interactivity-api-001&path:/packages/interactivity';
				}
			}
