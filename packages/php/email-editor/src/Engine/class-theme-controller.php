...
class Theme_Controller {
	...
	public function get_base_theme(): WP_Theme_JSON {
		...
		return apply_filters( 'fincommerce_email_editor_theme_json', $theme );
	}
	...
	public function get_stylesheet_for_rendering( ?WP_Post $post = null, $template = null ): string {
		...
		if ( $template && $template->wp_id ) {
			$template_theme    = (array) get_post_meta( $template->wp_id, Email_Editor::fincommerce_EMAIL_META_THEME_TYPE, true );
		...
		if ( $post ) {
			$post_theme      = (array) get_post_meta( $post->ID, 'fincommerce_email_theme', true );
