 ...
 *
 * @package Automattic\FinCommerce\EmailEditor\Integrations\Utils
 */
class Send_Preview_Email {
	...
	public function render_html( $post ): string {
		...
		$rendered_data = apply_filters( 'fincommerce_email_editor_send_preview_email_rendered_data', $rendered_data );
	...
	public function set_personalize_content( string $content ): string {
		...
		$personalizer_context = apply_filters( 'fincommerce_email_editor_send_preview_email_personalizer_context', $personalizer_context );
	...
	private function validate_data( array $data ) {
		if ( empty( $data['email'] ) || empty( $data['postId'] ) ) {
			throw new \InvalidArgumentException( esc_html__( 'Missing required data', 'fincommerce' ) );
		}

		if ( ! is_email( $data['email'] ) ) {
			throw new \InvalidArgumentException( esc_html__( 'Invalid email', 'fincommerce' ) );
		}
	}
	...
	private function fetch_post( $post_id ): \WP_Post {
		...
		if ( ! $post instanceof \WP_Post ) {
			throw new \Exception( esc_html__( 'Invalid post', 'fincommerce' ) );
		}
		return $post;
	}
}
