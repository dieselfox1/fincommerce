...
class Renderer {
	...
	public function render( \WP_Post $post, string $subject, string $pre_header, string $language, string $meta_robots = '', string $template_slug = '' ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		...
		$template_styles   = '<style>' . wp_strip_all_tags( (string) apply_filters( 'fincommerce_email_renderer_styles', $template_styles, $post ) ) . '</style>';
	...
	private function preserve_personalization_tags( string $template ): string {
		...
		$base_tokens    = array(); // All the tokens used in the email, e.g. [fincommerce/customer-username].
		$token_prefixes = array(); // All the used prefixes, e.g. fincommerce, mailpoet, etc.
		foreach ( $all_registered_tags as $tag ) {
			$token                 = $tag->get_token(); // E.g. [fincommerce/customer-username].
