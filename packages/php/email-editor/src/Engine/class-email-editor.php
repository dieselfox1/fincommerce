 ...
 */
class Email_Editor {
	public const fincommerce_EMAIL_META_THEME_TYPE = 'fincommerce_email_theme';
	...
	public function initialize(): void {
		...
		do_action( 'fincommerce_email_editor_initialized' );
		add_filter( 'fincommerce_email_editor_rendering_theme_styles', array( $this, 'extend_email_theme_styles' ), 10, 2 );
		...
		$is_editor_page = apply_filters( 'fincommerce_is_email_editor_page', false );
		if ( $is_editor_page ) {
		...
		add_filter( 'fincommerce_email_editor_send_preview_email', array( $this->send_preview_email, 'send_preview_email' ), 11, 1 ); // allow for other filter methods to take precedent.
	 ...
	 * The post types are added via fincommerce_email_editor_post_types filter.
	 *
	 * @return void
	 */
	 ...
	 * the fincommerce_email_editor_register_personalization_tags filter.
	 *
	 * @return void
	 */
	...
	private function get_post_types(): array {
		$post_types = array();
		return apply_filters( 'fincommerce_email_editor_post_types', $post_types );
	}
	...
	private function register_email_post_sent_status(): void {
		...
		$args         = apply_filters( 'fincommerce_email_editor_post_sent_status_args', $default_args );
	...
	public function extend_email_theme_styles( WP_Theme_JSON $theme, WP_Post $post ): WP_Theme_JSON {
		$email_theme = get_post_meta( $post->ID, self::fincommerce_EMAIL_META_THEME_TYPE, true );
	...
	public function load_email_preview_template( $template ) {
		...
		add_filter(
			'fincommerce_email_editor_preview_post_template_html',
			function () use ( $post ) {
