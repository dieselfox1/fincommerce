 ...
 */
export function getNewPath(query, path = getPath(), currentQuery = getQuery(), page = 'wc-admin') {
...
export const getPersistedQuery = (query = getQuery()) => {
     ...
     */
    const params = applyFilters('fincommerce_admin_persisted_queries', [
        'period',
        'compare',
        'before',
        'after',
 ...
 */
export const getQueryExcludedScreens = () => applyFilters(TIME_EXCLUDED_SCREENS_FILTER, [
    'stock',
    'settings',
    'customers',
    'homescreen',
 ...
 * @return {Array} Array containing list of screens
 */
export const getQueryExcludedScreensUrlUpdate = () => applyFilters(NAVIGATION_UPDATE_EXCLUDED_SCREENS_FILTER, ['extensions']);
/**
 ...
 * @param {string} page         Page key (defaults to "wc-admin")
 */
export function updateQueryString(query, path = getPath(), currentQuery = getQuery(), page = 'wc-admin') {
 ...
 */
export const pathIsExcluded = (path) => getQueryExcludedScreens().includes(getScreenFromPath(path));
/**
 * Get an array of IDs from a comma-separated query parameter.
 *
 * @param {string} [queryString=''] string value extracted from URL.
