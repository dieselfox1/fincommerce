 ...
 */
const Options = ({ options = [], onChange = () => { }, onExpanderClick = lodash_1.noop, onToggleExpanded = lodash_1.noop, parent = null, }) => {
    /**
    ...
    return options.map((option) => {
        ...
        if (!isRoot && !option?.isVisible)
            return null;
        return ((0, element_1.createElement)("div", { key: `${option.key ?? option.value}`, role: hasChildren ? 'treegroup' : 'treeitem', "aria-expanded": hasChildren ? expanded : undefined, className: (0, clsx_1.default)('fincommerce-tree-select-control__node', hasChildren && 'has-children') },
            (0, element_1.createElement)(components_1.Flex, { justify: "flex-start" },
                !isRoot && ((0, element_1.createElement)("button", { className: (0, clsx_1.default)('fincommerce-tree-select-control__expander', !hasChildren && 'is-hidden'), tabIndex: "-1", onClick: (e) => {
                        e.preventDefault();
                        onExpanderClick(e);
                        onToggleExpanded(option);
                    } },
                    (0, element_1.createElement)(icons_1.Icon, { icon: expanded ? icons_1.chevronUp : icons_1.chevronDown }))),
                (0, element_1.createElement)(checkbox_1.default, { className: (0, clsx_1.default)('components-base-control', 'fincommerce-tree-select-control__option', partialChecked && 'is-partially-checked'), option: option, checked: checked, onChange: (e) => {
                        onChange(e.target.checked, option, parent);
                    }, onKeyDown: (e) => {
                        handleKeyDown(e, option);
                    } })),
            hasChildren && expanded && ((0, element_1.createElement)("div", { className: (0, clsx_1.default)('fincommerce-tree-select-control__children', isRoot && 'fincommerce-tree-select-control__main') },
                (0, element_1.createElement)(Options, { options: option.children, onChange: onChange, onExpanderClick: onExpanderClick, onToggleExpanded: onToggleExpanded, parent: option })))));
    });
};
