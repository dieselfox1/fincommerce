 ...
 * @return {Object} -
 */
const SummaryNumber = ({ children, delta, href = '', hrefType = 'wc-admin', isOpen = false, label, labelTooltipText, onToggle, prevLabel = __('Previous period:', 'fincommerce'), prevValue, reverseTrend = false, selected = false, value, onLinkClickCallback = noop, }) => {
    const liClasses = clsx('fincommerce-summary__item-container', {
        'is-dropdown-button': onToggle,
        'is-dropdown-expanded': isOpen,
    });
    const classes = clsx('fincommerce-summary__item', {
    ...
    let screenReaderLabel = delta > 0
        ? // eslint-disable-next-line @wordpress/valid-sprintf -- false positive from %%
            ...
            __('Up %f%% from %s', 'fincommerce'), delta, prevLabel)
        : // eslint-disable-next-line @wordpress/valid-sprintf -- false positive from %%
            sprintf(
            /* translators: percentage change downwards */
            __('Down %f%% from %s', 'fincommerce'), Math.abs(delta), prevLabel);
    if (!delta) {
        screenReaderLabel = sprintf(
        /* translators: previous value */
        __('No change from %s', 'fincommerce'), prevLabel);
    }
    let Container;
    ...
    return (createElement("li", { className: liClasses },
        createElement(Container, { ...containerProps },
            createElement("div", { className: "fincommerce-summary__item-label" },
                createElement(Text, { variant: "body.small", size: "14", lineHeight: "20px" }, label),
                labelTooltipText && (createElement(Tooltip, { text: labelTooltipText, position: "top center" },
                    createElement("div", { className: "fincommerce-summary__info-tooltip" },
                        createElement(Icon, { width: 20, height: 20, icon: info }))))),
            createElement("div", { className: "fincommerce-summary__item-data" },
                createElement("div", { className: "fincommerce-summary__item-value" },
                    createElement(Text, { variant: "title.small", size: "20", lineHeight: "28px" }, !isNil(value)
                        ? value
                        : __('N/A', 'fincommerce'))),
                createElement(Tooltip, { text: !isNil(prevValue)
                        ? `${prevLabel} ${prevValue}`
                        : __('N/A', 'fincommerce'), position: "top center" },
                    createElement("div", { className: "fincommerce-summary__item-delta", role: "presentation", "aria-label": screenReaderLabel },
                        createElement(Text, { variant: "caption", size: "12", lineHeight: "16px" }, !isNil(delta)
                            ? // eslint-disable-next-line @wordpress/valid-sprintf -- false positive from %%
                                sprintf(
                                /* translators: percentage change */
                                __('%f%%', 'fincommerce'), delta)
                            : __('N/A', 'fincommerce'))))),
            onToggle ? (createElement(ChevronDownIcon, { className: "fincommerce-summary__toggle", size: 24 })) : null,
            children)));
};
