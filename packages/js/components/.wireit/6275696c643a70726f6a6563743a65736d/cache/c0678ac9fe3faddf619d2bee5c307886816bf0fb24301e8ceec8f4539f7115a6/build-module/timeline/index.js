...
const Timeline = ({ className = '', items = [], groupBy = 'day', orderBy = 'desc', 
/* translators: PHP date format string used to display dates, see php.net/date. */
dateFormat = __('F j, Y', 'fincommerce'), 
/* translators: PHP clock format string used to display times, see php.net/date. */
clockFormat = __('g:ia', 'fincommerce'), }) => {
    const timelineClassName = clsx('fincommerce-timeline', className);
    // Early return in case no data was passed to the component.
    if (!items || items.length === 0) {
        return (createElement("div", { className: timelineClassName },
            createElement("p", { className: 'timeline_no_events' }, __('No data to display', 'fincommerce'))));
    }
    ...
    return (createElement("div", { className: timelineClassName },
        createElement("ul", null, items
            ...
            .map((group) => (createElement(TimelineGroup, { key: group.date.getTime().toString(), group: group, orderBy: orderBy, clockFormat: clockFormat }))))));
};
Timeline.propTypes = {
    /**
