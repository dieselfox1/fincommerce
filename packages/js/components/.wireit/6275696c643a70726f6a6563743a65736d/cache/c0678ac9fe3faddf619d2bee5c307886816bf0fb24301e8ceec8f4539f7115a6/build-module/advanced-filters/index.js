...
import { Button, Card, CardBody, CardFooter, CardHeader, Dropdown, SelectControl, } from '@wordpress/components';
import { createElement, Component, createRef } from '@wordpress/element';
import { partial, isEqual } from 'lodash';
import PropTypes from 'prop-types';
import AddOutlineIcon from 'gridicons/dist/add-outline';
import { getActiveFiltersFromQuery, getDefaultOptionValue, getNewPath, getQueryFromActiveFilters, getHistory, } from '@fincommerce/navigation';
/**
...
const matches = [
    { value: 'all', label: __('All', 'fincommerce') },
    { value: 'any', label: __('Any', 'fincommerce') },
];
/**
...
class AdvancedFilters extends Component {
    constructor({ query, config }) {
        super(...arguments);
        this.instanceCounts = {};
        const filtersFromQuery = getActiveFiltersFromQuery(query, config.filters);
    ...
    getTitle() {
        ...
        return createInterpolateElement(config.title, {
            select: (createElement(SelectControl, { className: "fincommerce-filters-advanced__title-select", options: matches, value: match, onChange: this.onMatchChange, "aria-label": __('Choose to apply any or all filters', 'fincommerce') })),
        });
    }
    getAvailableFilters() {
        const { config } = this.props;
    ...
    render() {
        ...
        return (createElement(Card, { className: "fincommerce-filters-advanced", size: "small" },
            createElement(CardHeader, { justify: "flex-start" },
                createElement(Text, { variant: "subtitle.small", as: "div", weight: "600", size: "14", lineHeight: "20px", isBlock: "false" }, this.getTitle())),
            !!activeFilters.length && (createElement(CardBody, { size: null },
                createElement("ul", { className: "fincommerce-filters-advanced__list", ref: this.filterListRef }, activeFilters
                    .sort(this.orderFilters)
                    .map((filter, idx) => {
                    const { instance, key } = filter;
                    return (createElement(AdvancedFilterItem, { key: key + (instance || ''), config: config, currency: currency, filter: filter, isEnglish: isEnglish, onFilterChange: partial(this.onFilterChange, idx), query: query, removeFilter: () => this.removeFilter(idx) }));
                })))),
            availableFilters.length > 0 && (createElement(CardBody, null,
                createElement("div", { className: "fincommerce-filters-advanced__add-filter" },
                    createElement(Dropdown, { className: "fincommerce-filters-advanced__add-filter-dropdown", popoverProps: {
                            placement: 'bottom',
                        }, renderToggle: ({ isOpen, onToggle }) => (createElement(Button, { className: "fincommerce-filters-advanced__add-button", onClick: onToggle, "aria-expanded": isOpen },
                            createElement(AddOutlineIcon, null),
                            __('Add a filter', 'fincommerce'))), renderContent: ({ onClose }) => (createElement("ul", { className: "fincommerce-filters-advanced__add-dropdown" }, availableFilters.map((filter) => (createElement("li", { key: filter.key },
                            createElement(Button, { onClick: partial(this.addFilter, filter.key, onClose) }, filter.labels.add)))))) })))),
            createElement(CardFooter, { align: "center" },
                createElement("div", { className: "fincommerce-filters-advanced__controls" },
                    updateDisabled && (createElement(Button, { isPrimary: true, disabled: true }, __('Filter', 'fincommerce'))),
                    !updateDisabled && (createElement(Link, { className: "components-button is-primary is-button", type: "wc-admin", href: updateHref, onClick: this.onFilter }, __('Filter', 'fincommerce'))),
                    activeFilters.length > 0 && (createElement(Link, { type: "wc-admin", href: this.getUpdateHref([]), onClick: this.clearFilters }, __('Clear all filters', 'fincommerce')))))));
    }
}
