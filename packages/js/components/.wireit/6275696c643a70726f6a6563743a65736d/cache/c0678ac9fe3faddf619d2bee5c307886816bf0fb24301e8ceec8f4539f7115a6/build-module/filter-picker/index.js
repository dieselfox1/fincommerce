...
import { flattenFilters, updateQueryString, getQueryFromActiveFilters, } from '@fincommerce/navigation';
/**
...
class FilterPicker extends Component {
    ...
    update(value, additionalQueries = {}) {
        ...
        let update = {
            [config.param]: (config.defaultValue || DEFAULT_FILTER) === value
                ? undefined
                : value,
            ...additionalQueries,
        };
        ...
        if (config.param === 'filter' && value !== 'advanced') {
            const resetAdvancedFilters = getQueryFromActiveFilters([], query, advancedFilters.filters || {});
            update = {
                ...update,
    ...
    renderButton(filter, onClose, config) {
        if (filter.component) {
            ...
            const selectedTag = persistedFilter.value === filter.value
                ...
                : null;
            return (createElement(Search, { autocompleter: autocompleter, className: "fincommerce-filters-filter__search", type: type, placeholder: labels.placeholder, selected: selectedTag ? [selectedTag] : [], onChange: partial(this.onTagChange, filter, onClose, config), inlineTags: true, staticResults: true }));
        }
        ...
        return (createElement(Button, { className: "fincommerce-filters-filter__button", onClick: onClick }, filter.label));
    }
    ...
    render() {
        ...
        const parentFilter = nav.length
            ...
            : false;
        const selectedFilter = this.getFilter();
        return (createElement("div", { className: "fincommerce-filters-filter" },
            config.label && (createElement("span", { className: "fincommerce-filters-label" },
                config.label,
                ":")),
            createElement(Dropdown, { contentClassName: "fincommerce-filters-filter__content", popoverProps: {
                    placement: 'bottom',
                }, expandOnMobile: true, headerTitle: __('filter report to show:', 'fincommerce'), renderToggle: ({ isOpen, onToggle }) => (createElement(DropdownButton, { onClick: onToggle, isOpen: isOpen, labels: this.getButtonLabel(selectedFilter) })), renderContent: ({ onClose }) => (createElement(AnimationSlider, { animationKey: nav, animate: animate, onExited: this.onContentMount }, () => (createElement("ul", { className: "fincommerce-filters-filter__content-list" },
                    parentFilter && (createElement("li", { className: "fincommerce-filters-filter__content-list-item" },
                        createElement(Button, { className: "fincommerce-filters-filter__button", onClick: this.goBack },
                            createElement(Icon, { icon: chevronLeft }),
                            parentFilter.label))),
                    visibleFilters.map((filter) => (createElement("li", { key: filter.value, className: clsx('fincommerce-filters-filter__content-list-item', {
