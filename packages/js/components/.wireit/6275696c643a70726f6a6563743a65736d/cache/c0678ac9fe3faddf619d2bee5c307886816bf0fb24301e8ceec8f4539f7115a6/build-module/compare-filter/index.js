...
export class CompareFilter extends Component {
    ...
    render() {
        ...
        const { selected } = this.state;
        return (createElement(Card, { className: "fincommerce-filters__compare" },
            createElement(CardHeader, null,
                createElement(Text, { variant: "subtitle.small", weight: "600", size: "14", lineHeight: "20px" }, labels.title)),
            createElement(CardBody, null,
                createElement(Search, { autocompleter: autocompleter, type: type, selected: selected, placeholder: labels.placeholder, onChange: (value) => {
                        this.setState({ selected: value });
                    } })),
            createElement(CardFooter, { justify: "flex-start" },
                createElement(CompareButton, { count: selected.length, helpText: labels.helpText, onClick: this.onButtonClicked }, labels.update),
                selected.length > 0 && (createElement(Button, { isLink: true, onClick: this.clearQuery }, __('Clear all', 'fincommerce'))))));
    }
}
