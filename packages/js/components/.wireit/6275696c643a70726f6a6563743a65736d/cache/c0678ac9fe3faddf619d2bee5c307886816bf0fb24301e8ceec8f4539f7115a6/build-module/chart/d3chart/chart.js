...
import { getOrderedKeys, getUniqueDates, getFormatter, isDataEmpty, } from './utils/index';
import { getXScale, getXGroupScale, getXLineScale, getYScale, getYScaleLimits, } from './utils/scales';
...
class D3Chart extends Component {
    ...
    getParams(uniqueDates) {
        ...
        const colorKeys = newOrderedKeys.length > selectionLimit
            ? visibleKeys
            : newOrderedKeys;
        return {
    ...
    getEmptyMessage() {
        ...
        if (emptyMessage && isDataEmpty(data, baseValue)) {
            return (createElement("div", { className: "d3-chart__empty-message" }, emptyMessage));
        }
    }
    render() {
        const { className, data, height, orderedKeys, chartType } = this.props;
        const computedWidth = this.getWidth();
        return (createElement("div", { className: clsx('d3-chart__container', className), style: { height } },
            this.getEmptyMessage(),
            createElement("div", { className: "d3-chart__tooltip", ref: this.tooltipRef }),
            createElement(D3Base, { className: clsx(className), data: data, drawChart: this.drawChart, height: height, orderedKeys: orderedKeys, tooltip: this.tooltip, chartType: chartType, width: computedWidth })));
    }
}
D3Chart.propTypes = {
    /**
