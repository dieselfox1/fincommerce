...
export function createLinkedTree(items, value) {
...
export function toggleNode(tree, number, value) {
...
export function getVisibleNodeIndex(tree, highlightedIndex, direction) {
    if (direction === 'down') {
        for (const node of tree) {
...
export function countNumberOfNodes(linkedTree) {
    let count = 0;
...
export function getNodeDataByIndex(linkedTree, index) {
    for (const node of linkedTree) {
        if (node.index === index) {
            return node.data;
        }
