...
import { backwardsCompatibleCreateInterpolateElement as createInterpolateElement, textContent, } from './utils';
class NumberFilter extends Component {
    getBetweenString() {
        return _x('<rangeStart/><span> and </span><rangeEnd/>', 'Numerical range inputs arranged on a single line', 'fincommerce');
    }
    getScreenReaderText(filter, config) {
        ...
        return textContent(createInterpolateElement(config.labels.title, {
    ...
    getFormControl({ type, value, label, onChange, currencySymbol, symbolPosition, }) {
        if (type === 'currency') {
            return symbolPosition.indexOf('right') === 0 ? (createElement(TextControlWithAffixes, { suffix: createElement("span", null, currencySymbol), className: "fincommerce-filters-advanced__input", type: "number", value: value || '', "aria-label": label, onChange: onChange })) : (createElement(TextControlWithAffixes, { prefix: createElement("span", null, currencySymbol), className: "fincommerce-filters-advanced__input", type: "number", value: value || '', "aria-label": label, onChange: onChange }));
        }
        return (createElement(TextControl, { className: "fincommerce-filters-advanced__input", type: "number", value: value || '', "aria-label": label, onChange: onChange }));
    }
    getFilterInputs() {
        ...
        if (filter.rule === 'lessthan') {
            ...
            labelFormat = _x('%(field)s maximum amount', 'maximum value input', 'fincommerce');
        }
        else {
            ...
            labelFormat = _x('%(field)s minimum amount', 'minimum value input', 'fincommerce');
        }
    ...
    getRangeInput() {
        ...
        return createInterpolateElement(this.getBetweenString(), {
            rangeStart: this.getFormControl({
                ...
                __('%(field)s range start', 'fincommerce'), { field: get(config, ['labels', 'add']) }),
            ...
            rangeEnd: this.getFormControl({
                ...
                __('%(field)s range end', 'fincommerce'), { field: get(config, ['labels', 'add']) }),
    ...
    render() {
        ...
        const children = createInterpolateElement(labels.title, {
            ...
            rule: (createElement(SelectControl, { className: clsx(className, 'fincommerce-filters-advanced__rule'), options: rules, value: rule, onChange: (value) => onFilterChange({ property: 'rule', value }), "aria-label": labels.rule })),
            filter: (createElement("div", { className: clsx(className, 'fincommerce-filters-advanced__input-range', {
                    'is-between': rule === 'between',
                }) }, this.getFilterInputs())),
        });
        const screenReaderText = this.getScreenReaderText(filter, config);
        /*eslint-disable jsx-a11y/no-noninteractive-tabindex*/
        return (createElement("fieldset", { className: "fincommerce-filters-advanced__line-item", tabIndex: "0" },
            createElement("legend", { className: "screen-reader-text" }, labels.add || ''),
            createElement("div", { className: clsx('fincommerce-filters-advanced__fieldset', {
