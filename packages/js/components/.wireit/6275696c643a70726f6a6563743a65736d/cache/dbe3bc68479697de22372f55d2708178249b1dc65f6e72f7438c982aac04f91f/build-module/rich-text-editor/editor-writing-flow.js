...
import { BlockList, ObserveTyping, 
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore No types for this exist yet.
BlockTools, store as blockEditorStore, WritingFlow, } from '@wordpress/block-editor';
export const EditorWritingFlow = ({ blocks, onChange, placeholder = '', }) => {
    ...
    const { insertBlock, selectBlock, __unstableSetEditorMode } = useDispatch(blockEditorStore);
    const { selectedBlockClientIds, editorMode } = useSelect((select) => {
        ...
        const { getSelectedBlockClientIds, __unstableGetEditorMode } = select(blockEditorStore);
        return {
    ...
    createElement("div", { className: "fincommerce-rich-text-editor__writing-flow", id: `fincommerce-rich-text-editor__writing-flow-${instanceId}`, style: {
