...
const shortDateFormat = __('MM/DD/YYYY', 'fincommerce');
/**
...
class DateRangeFilterPicker extends Component {
    ...
    getButtonLabel() {
        ...
        return [
            ...
            `${__('vs.', 'fincommerce')} ${secondaryDate.label} (${secondaryDate.range})`,
        ];
    }
    ...
    render() {
        ...
        const { isViewportMobile, focusOnMount = true, popoverProps = { inline: true }, } = this.props;
        if (!popoverProps.placement) {
            popoverProps.placement = 'bottom';
        }
        const contentClasses = clsx('fincommerce-filters-date__content', {
            'is-mobile': isViewportMobile,
        });
        return (createElement("div", { className: "fincommerce-filters-filter" },
            createElement("span", { className: "fincommerce-filters-label" },
                __('Date range', 'fincommerce'),
                ":"),
            createElement(Dropdown, { contentClassName: contentClasses, expandOnMobile: true, focusOnMount: focusOnMount, popoverProps: popoverProps, renderToggle: ({ isOpen, onToggle }) => (createElement(DropdownButton, { onClick: onToggle, isOpen: isOpen, labels: this.getButtonLabel() })), renderContent: ({ onClose }) => (createElement(DatePickerContent, { period: period, compare: compare, after: after, before: before, onUpdate: this.update, onClose: onClose, onSelect: this.onSelect, isValidSelection: this.isValidSelection, resetCustomValues: this.resetCustomValues, focusedInput: focusedInput, afterText: afterText, beforeText: beforeText, afterError: afterError, beforeError: beforeError, shortDateFormat: shortDateFormat })) })));
    }
}
