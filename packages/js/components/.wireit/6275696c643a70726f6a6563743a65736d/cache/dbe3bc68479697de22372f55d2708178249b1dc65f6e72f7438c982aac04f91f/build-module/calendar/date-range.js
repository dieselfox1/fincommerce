...
class DateRange extends Component {
    ...
    onInputChange(input, event) {
        ...
        const { date, error } = validateDateInputForRange(input, value, before, after, shortDateFormat);
        this.props.onUpdate({
            [input]: date,
            [input + 'Text']: value,
    ...
    render() {
        ...
        return (createElement("div", { className: clsx('fincommerce-calendar', {
                'is-mobile': isViewportMobile,
            }) },
            createElement("div", { className: "fincommerce-calendar__inputs" },
                createElement(DateInput, { value: afterText, onChange: partial(this.onInputChange, 'after'), dateFormat: shortDateFormat, label: __('Start Date', 'fincommerce'), error: afterError, describedBy: sprintf(
                    /* translators: %s: date format specification */
                    __("Date input describing a selected date range's start date in format %s", 'fincommerce'), shortDateFormat), onFocus: () => this.onFocusChange('startDate') }),
                createElement("div", { className: "fincommerce-calendar__inputs-to" }, __('to', 'fincommerce')),
                createElement(DateInput, { value: beforeText, onChange: partial(this.onInputChange, 'before'), dateFormat: shortDateFormat, label: __('End Date', 'fincommerce'), error: beforeError, describedBy: sprintf(
                    /* translators: %s: date format specification */
                    __("Date input describing a selected date range's end date in format %s", 'fincommerce'), shortDateFormat), onFocus: () => this.onFocusChange('endDate') })),
            createElement("div", { className: "fincommerce-calendar__react-dates", ref: this.nodeRef, onBlur: partial(this.keepFocusInside, CONTAINER_DIV), tabIndex: -1 },
                createElement(DayPickerRangeController, { onNextMonthClick: partial(this.keepFocusInside, NEXT_MONTH_CLICK), onPrevMonthClick: partial(this.keepFocusInside, PREV_MONTH_CLICK), onDatesChange: this.onDatesChange, onFocusChange: this.onFocusChange, focusedInput: focusedInput, startDate: after, endDate: before, orientation: 'horizontal', numberOfMonths: isDoubleCalendar ? 2 : 1, isOutsideRange: (date) => {
                        return (isInvalidDate && isInvalidDate(date.toDate()));
                    }, minimumNights: 0, hideKeyboardShortcutsPanel: true, noBorder: true, isRTL: isRTL(), initialVisibleMonth: this.setTnitialVisibleMonth(isDoubleCalendar, before), phrases: phrases }))));
    }
}
DateRange.propTypes = {
    /**
     ...
     * See: https://github.com/dieselfox1/fincommerce-admin/pull/2929.
     */
