...
function AnalyticsError({ className }) {
    const title = __('There was an error getting your stats. Please try again.', 'fincommerce');
    const actionLabel = __('Reload', 'fincommerce');
    const actionCallback = () => {
        // @todo Add tracking for how often an error is displayed, and the reload action is clicked.
        window.location.reload();
    };
    return (createElement(EmptyContent, { className: className, title: title, actionLabel: actionLabel, actionCallback: actionCallback }));
}
