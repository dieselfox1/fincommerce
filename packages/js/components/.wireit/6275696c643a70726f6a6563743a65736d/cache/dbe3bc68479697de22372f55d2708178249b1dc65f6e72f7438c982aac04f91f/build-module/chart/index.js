...
class Chart extends Component {
    ...
    renderIntervalSelector() {
        ...
        const intervalLabels = {
            hour: __('By hour', 'fincommerce'),
            day: __('By day', 'fincommerce'),
            week: __('By week', 'fincommerce'),
            month: __('By month', 'fincommerce'),
            quarter: __('By quarter', 'fincommerce'),
            year: __('By year', 'fincommerce'),
        };
        return (createElement("div", { className: "fincommerce-chart__interval-select" },
            createElement(SelectControl, { value: interval, options: allowedIntervals.map((allowedInterval) => ({
    ...
    render() {
        ...
        const legend = legendPosition !== 'hidden' && isRequesting ? null : (createElement(D3Legend, { colorScheme: d3InterpolateViridis, data: orderedKeys, handleLegendHover: this.handleLegendHover, handleLegendToggle: this.handleLegendToggle, interactive: interactiveLegend, legendDirection: legendDirection, legendValueFormat: tooltipValueFormat, totalLabel: totalLabel }));
        const margin = {
            bottom: 50,
            left: 80,
            right: 30,
        ...
        return (createElement("div", { className: "fincommerce-chart" },
            showHeaderControls && (createElement("div", { className: "fincommerce-chart__header" },
                createElement(H, { className: "fincommerce-chart__title" }, title),
                legendPosition === 'top' && legend,
                this.renderIntervalSelector(),
                createElement(NavigableMenu, { className: "fincommerce-chart__types", orientation: "horizontal", role: "menubar" },
                    createElement(Button, { className: clsx('fincommerce-chart__type-button', {
                            'fincommerce-chart__type-button-selected': chartType === 'line',
                        }), title: __('Line chart', 'fincommerce'), "aria-checked": chartType === 'line', role: "menuitemradio", tabIndex: chartType === 'line' ? 0 : -1, onClick: partial(this.handleTypeToggle, 'line') },
                        createElement(LineGraphIcon, null)),
                    createElement(Button, { className: clsx('fincommerce-chart__type-button', {
                            'fincommerce-chart__type-button-selected': chartType === 'bar',
                        }), title: __('Bar chart', 'fincommerce'), "aria-checked": chartType === 'bar', role: "menuitemradio", tabIndex: chartType === 'bar' ? 0 : -1, onClick: partial(this.handleTypeToggle, 'bar') },
                        createElement(StatsAltIcon, null))))),
            createElement(Section, { component: false },
                createElement("div", { className: clsx('fincommerce-chart__body', `fincommerce-chart__body-${chartDirection}`), ref: this.chartBodyRef },
                    legendPosition === 'side' && legend,
                    isRequesting && (createElement(Fragment, null,
                        createElement("span", { className: "screen-reader-text" }, __('Your requested data is loading', 'fincommerce')),
                        createElement(ChartPlaceholder, { height: chartHeight }))),
                    !isRequesting && width > 0 && (createElement(D3Chart, { baseValue: baseValue, chartType: chartType, colorScheme: d3InterpolateViridis, data: visibleData, dateParser: dateParser, height: chartHeight, emptyMessage: emptyMessage, interval: interval, margin: margin, mode: mode, orderedKeys: orderedKeys, screenReaderFormat: screenReaderFormat, tooltipLabelFormat: tooltipLabelFormat, tooltipValueFormat: tooltipValueFormat, tooltipPosition: isViewportLarge ? 'over' : 'below', tooltipTitle: tooltipTitle, valueType: valueType, width: chartDirection === 'row'
                            ? width - 320
                            : width, xFormat: xFormat, x2Format: x2Format, yBelow1Format: d3chartYBelow1Format, yFormat: d3chartYFormat }))),
                legendPosition === 'bottom' && (createElement("div", { className: "fincommerce-chart__footer" }, legend)))));
    }
}
Chart.propTypes = {
    /**
