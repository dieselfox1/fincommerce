...
import { dateValidationMessages, toMoment } from '@fincommerce/date';
/**
...
class DatePicker extends Component {
    ...
    render() {
        ...
        return (createElement(Dropdown, { focusOnMount: false, popoverProps: popoverProps, renderToggle: ({ isOpen, onToggle }) => (createElement(DateInput, { disabled: disabled, value: text, onChange: this.onInputChange, onBlur: partial(this.handleBlur, isOpen, onToggle), dateFormat: dateFormat, label: __('Choose a date', 'fincommerce'), error: error, describedBy: sprintf(
                /* translators: %s: date format specification */
                __('Date input describing a selected date in format %s', 'fincommerce'), dateFormat), onFocus: partial(this.handleFocus, isOpen, onToggle), "aria-expanded": isOpen, focusOnMount: false, errorPosition: "top center" })), renderContent: ({ onToggle }) => (createElement(Section, { component: false },
                createElement(H, { className: "fincommerce-calendar__date-picker-title" }, __('select a date', 'fincommerce')),
                createElement("div", { className: "fincommerce-calendar__react-dates is-core-datepicker" },
                    createElement(WpDatePicker, { currentDate: date instanceof moment
                            ? date.toDate()
                            : date, onChange: partial(this.onDateChange, onToggle), 
                        // onMonthPreviewed is required to prevent a React error from happening.
                        onMonthPreviewed: noop, isInvalidDate: isInvalidDate })))) }));
    }
}
