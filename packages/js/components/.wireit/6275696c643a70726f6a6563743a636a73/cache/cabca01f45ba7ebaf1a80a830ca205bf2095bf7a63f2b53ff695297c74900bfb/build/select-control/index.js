...
class SelectControl extends element_1.Component {
    ...
    selectOption(option) {
        const { multiple, selected } = this.props;
        const newSelected = multiple && (0, lodash_1.isArray)(selected)
            ? [...selected, option]
            : [option];
    ...
    decrementSelectedIndex() {
        ...
        const nextSelectedIndex = (0, lodash_1.isNumber)(selectedIndex)
    ...
    announce(searchOptions) {
        ...
        if (!!searchOptions.length) {
            debouncedSpeak((0, i18n_1.sprintf)(
            // translators: %d: number of results.
            (0, i18n_1._n)('%d result found, use up and down arrow keys to navigate.', '%d results found, use up and down arrow keys to navigate.', searchOptions.length, 'fincommerce'), searchOptions.length), 'assertive');
        }
        else {
            debouncedSpeak((0, i18n_1.__)('No results.', 'fincommerce'), 'assertive');
        }
    }
    getOptions() {
        ...
        const selectedKeys = (0, lodash_1.isArray)(selected)
    ...
    getOptionsByQuery(options, query) {
        ...
        const normalizedQuery = ignoreDiacritics
            ...
            : baseQuery;
        const expression = getSearchExpression((0, lodash_1.escapeRegExp)(normalizedQuery));
    ...
    render() {
        ...
        const { key: selectedKey = '' } = ((0, lodash_1.isNumber)(selectedIndex) && options[selectedIndex]) || {};
