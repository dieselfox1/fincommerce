...
const StepNavigation = ( {
	...
	const SkipButton = (
		<Button
			className="fincommerce-tour-kit-step-navigation__skip-btn"
			variant="tertiary"
			onClick={ onDismiss( 'skip-btn' ) }
		>
			{ skipButton.text || __( 'Skip', 'fincommerce' ) }
		</Button>
	);

	const NextButton = (
		<Button
			className="fincommerce-tour-kit-step-navigation__next-btn"
			variant="primary"
			disabled={ primaryButton.isDisabled }
			onClick={ onNextStep }
		>
			{ primaryButton.text || __( 'Next', 'fincommerce' ) }
		</Button>
	);

	const BackButton = (
		<Button
			className="fincommerce-tour-kit-step-navigation__back-btn"
			variant="secondary"
			onClick={ onPreviousStep }
		>
			{ secondaryButton.text || __( 'Back', 'fincommerce' ) }
		</Button>
	);
			...
			return (
				<div>
					...
					<Button
						variant="primary"
						disabled={ primaryButton.isDisabled }
						className="fincommerce-tour-kit-step-navigation__done-btn"
						onClick={ onDismiss( 'done-btn' ) }
					>
						{ primaryButton.text || __( 'Done', 'fincommerce' ) }
					</Button>
				</div>
			);
		}
	...
	return (
		<div className="fincommerce-tour-kit-step-navigation">
			<div className="fincommerce-tour-kit-step-navigation__step">
				{ steps.length > 1
					? sprintf(
							/* translators: current progress in tour, eg: "Step 2 of 4" */
							__( 'Step %1$d of %2$d', 'fincommerce' ),
							currentStepIndex + 1,
							steps.length
					  )
					: null }
