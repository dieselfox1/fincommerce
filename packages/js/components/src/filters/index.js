	...
	renderCard( config ) {
		...
		if ( query[ param ].indexOf( 'compare' ) === 0 ) {
			...
			return (
				<div
					key={ param }
					className="fincommerce-filters__advanced-filters"
				>
					<CompareFilter
						path={ path }
						query={ query }
		...
		return (
			<Fragment>
				<H className="screen-reader-text">
					{ __( 'Filters', 'fincommerce' ) }
				</H>
				<Section component="div" className="fincommerce-filters">
					<div className="fincommerce-filters__basic-filters">
						{ showDatePicker && (
