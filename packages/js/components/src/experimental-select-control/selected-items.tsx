...
const PrivateSelectedItems = < ItemType, >(
	...
	const classes = clsx(
		'fincommerce-experimental-select-control__selected-items',
		{
	...
	const focusSibling = ( event: React.KeyboardEvent< HTMLDivElement > ) => {
		const selectedItem = ( event.target as HTMLElement ).closest(
			'.fincommerce-experimental-select-control__selected-item'
		);
		const sibling =
			...
			{ items.map( ( item, index ) => {
				return (
					...
					<div
						...
						className="fincommerce-experimental-select-control__selected-item"
						{ ...getSelectedItemProps( {
							selectedItem: item,
							index,
						} ) }
