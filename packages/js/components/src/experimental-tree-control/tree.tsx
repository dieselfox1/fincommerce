		...
		<>
			{ items.length || isCreateButtonVisible ? (
				<ol
					...
					className={ clsx(
						treeProps.className,
						'experimental-fincommerce-tree',
						`experimental-fincommerce-tree--level-${ level }`
					) }
				>
								...
								(
									rootListRef.current
										...
										?.parentElement?.querySelector(
											'.experimental-fincommerce-tree__button'
			...
			{ isCreateButtonVisible && (
				<Button
					id={
						'fincommerce-experimental-tree-control__menu-item-' +
						numberOfItems
					}
					className={ clsx( 'experimental-fincommerce-tree__button', {
						'experimental-fincommerce-tree__button--highlighted':
							...
							if ( event.key === 'ArrowUp' ) {
								const allHeadings =
									event.nativeEvent.srcElement.previousSibling.querySelectorAll(
										'.experimental-fincommerce-tree-item > .experimental-fincommerce-tree-item__heading'
									);

								allHeadings[ allHeadings.length - 1 ]
									?.querySelector(
										'.experimental-fincommerce-tree-item__label'
									)
									?.focus();
							}
