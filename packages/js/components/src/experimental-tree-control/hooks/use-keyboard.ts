...
function getFirstChild(
	...
	const firstSubTreeItem = parentTreeItem?.querySelector(
		'.experimental-fincommerce-tree > .experimental-fincommerce-tree-item'
	);
	const label = firstSubTreeItem?.querySelector< HTMLLabelElement >(
		'.experimental-fincommerce-tree-item__heading > .experimental-fincommerce-tree-item__label'
	);
	return label ?? null;
}
...
function getAllHeadings(
	...
	const rootTree = currentHeading.closest< HTMLDivElement >(
		'.experimental-fincommerce-tree--level-1'
	);
