...
class AdvancedFilters extends Component {
	...
	getTitle() {
		...
		return createInterpolateElement( config.title, {
			select: (
				<SelectControl
					className="fincommerce-filters-advanced__title-select"
					options={ matches }
					value={ match }
	...
	render() {
		...
		return (
			<Card className="fincommerce-filters-advanced" size="small">
				...
				{ !! activeFilters.length && (
					<CardBody size={ null }>
						<ul
							className="fincommerce-filters-advanced__list"
				...
				{ availableFilters.length > 0 && (
					<CardBody>
						<div className="fincommerce-filters-advanced__add-filter">
							<Dropdown
								className="fincommerce-filters-advanced__add-filter-dropdown"
								popoverProps={ {
									placement: 'bottom',
								} }
								renderToggle={ ( { isOpen, onToggle } ) => (
									<Button
										className="fincommerce-filters-advanced__add-button"
										onClick={ onToggle }
										aria-expanded={ isOpen }
									>
										<AddOutlineIcon />
										{ __( 'Add a filter', 'fincommerce' ) }
									</Button>
								) }
								renderContent={ ( { onClose } ) => (
									<ul className="fincommerce-filters-advanced__add-dropdown">
				...
				<CardFooter align="center">
					<div className="fincommerce-filters-advanced__controls">
						{ updateDisabled && (
						...
						{ activeFilters.length > 0 && (
							...
							>
								{ __( 'Clear all filters', 'fincommerce' ) }
							</Link>
						) }
					</div>
				</CardFooter>
