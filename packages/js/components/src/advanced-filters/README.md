...
Below is a config example complete with translation strings. Advanced filters makes use of [createInterpolateElement](https://developer.finpress.org/block-editor/reference-guides/packages/packages-element/#createinterpolateelement) to organize sentence structure, resulting in a filter visually represented as a sentence fragment in any language.
	...
	filters: {
		status: {
			labels: {
				...
				// See screen shot for context: https://cloudup.com/cSsUY9VeCVJ
				title: __( 'Order Status <rule/> <filter/>', 'fincommerce' ),
...
| `currency`               | Object   | `null`    | (required) The currency instance for the site (@fincommerce/currency).             |
...
`type`: A string Autocompleter type used by the [Search Component](https://github.com/dieselfox1/fincommerce/tree/trunk/packages/js/components/src/search).
	...
	filters: {
		registered: {
			rules: [
				{
					value: 'before',
					label: __( 'Before', 'fincommerce' ),
				},
				{
					value: 'after',
					label: __( 'After', 'fincommerce' ),
				},
				{
					value: 'between',
					label: __( 'Between', 'fincommerce' ),
				},
			],
			input: {
				component: 'Date',
	...
	filters: {
		quantity: {
			rules: [
				{
					value: 'lessthan',
					label: __( 'Less Than', 'fincommerce' ),
				},
				{
					value: 'morethan',
					label: __( 'More Than', 'fincommerce' ),
				},
				{
					value: 'between',
					label: __( 'Between', 'fincommerce' ),
