		...
		if ( type === 'currency' ) {
			return symbolPosition.indexOf( 'right' ) === 0 ? (
				<TextControlWithAffixes
					...
					className="fincommerce-filters-advanced__input"
					type="number"
					value={ value || '' }
					aria-label={ label }
					onChange={ onChange }
		...
		return createInterpolateElement( this.getBetweenString(), {
			rangeStart: this.getFormControl( {
				...
				label: sprintf(
					...
					__( '%(field)s range start', 'fincommerce' ),
					{ field: get( config, [ 'labels', 'add' ] ) }
				),
			...
			rangeEnd: this.getFormControl( {
				...
				label: sprintf(
					...
					__( '%(field)s range end', 'fincommerce' ),
					{ field: get( config, [ 'labels', 'add' ] ) }
				),
	...
	render() {
		...
		const children = createInterpolateElement( labels.title, {
			...
			filter: (
				<div
					className={ clsx(
						className,
						'fincommerce-filters-advanced__input-range',
						{
		...
		return (
			...
			>
				...
				<div
					className={ clsx(
						'fincommerce-filters-advanced__fieldset',
						{
