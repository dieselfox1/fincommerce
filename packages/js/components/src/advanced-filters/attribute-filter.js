...
const AttributeFilter = ( props ) => {
	...
	return (
		<fieldset
			className="fincommerce-filters-advanced__line-item"
			tabIndex="0"
		>
			<legend className="screen-reader-text">{ labels.add || '' }</legend>
			<div
				className={ clsx( 'fincommerce-filters-advanced__fieldset', {
				...
				{ createInterpolateElement( labels.title, {
					...
					rule: (
						<Select
							className={ clsx(
								className,
								'fincommerce-filters-advanced__rule'
							) }
							options={ rules }
							value={ rule }
					...
					filter: (
						<div
							className={ clsx(
								className,
								'fincommerce-filters-advanced__attribute-fieldset'
							) }
						>
							{ ! Array.isArray( value ) ||
							! value.length ||
							selectedAttribute.length ? (
								<Search
									className="fincommerce-filters-advanced__input fincommerce-search"
									onChange={ ( [ attr ] ) => {
							...
							{ selectedAttribute.length > 0 &&
								( attributeTerms.length ? (
									<Fragment>
										<span className="fincommerce-filters-advanced__attribute-field-separator">
											=
										</span>
										<SelectControl
											className="fincommerce-filters-advanced__input fincommerce-search"
