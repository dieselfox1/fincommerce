...
function ListItem( props ) {
	...
	return (
		<InnerTag { ...innerTagProps }>
			{ before && (
				<div className="fincommerce-list__item-before">{ before }</div>
			) }
			<div className="fincommerce-list__item-text">
				<span className="fincommerce-list__item-title">{ title }</span>
				{ content && (
			...
			{ after && (
				<div className="fincommerce-list__item-after">{ after }</div>
			) }
		</InnerTag>
	);
}
