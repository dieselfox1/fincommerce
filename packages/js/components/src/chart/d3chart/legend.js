	...
	render() {
		...
		return (
			<div
				className={ clsx(
					'fincommerce-legend',
					`fincommerce-legend__direction-${ legendDirection }`,
					{
			...
			>
				<ul
					className="fincommerce-legend__list"
					ref={ this.listRef }
					onScroll={ showTotalLabel ? this.updateListScroll : null }
				>
					{ data.map( ( row ) => (
						<li
							className={ clsx( 'fincommerce-legend__item', {
								'fincommerce-legend__item-checked': row.visible,
							} ) }
							key={ row.key }
							id={ `fincommerce-legend-${ instanceId }__item__${ row.key }` }
						...
						>
							<button
								...
								id={ `fincommerce-legend-${ instanceId }__item-button__${ row.key }` }
								disabled={
									( row.visible &&
							...
							>
								<div className="fincommerce-legend__item-container">
									<span
										className={ clsx(
											'fincommerce-legend__item-checkmark',
											{
												'fincommerce-legend__item-checkmark-checked':
													row.visible,
											}
										) }
										style={
									...
									/>
									<span className="fincommerce-legend__item-title">
										{ row.label }
									</span>
									<span className="fincommerce-legend__item-total">
				...
				{ showTotalLabel && (
					<div className="fincommerce-legend__total">
						{ totalLabel }
					</div>
				) }
			</div>
