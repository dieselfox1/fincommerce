...
export const SelectTree = function SelectTree( {
	...
	const selectTreeInstanceId = useInstanceId(
		SelectTree,
		'fincommerce-experimental-select-tree-control__dropdown'
	) as string;
	const menuInstanceId = useInstanceId(
		SelectTree,
		'fincommerce-select-tree-control__menu'
	) as string;
	...
	function isEventOutside( event: React.SyntheticEvent | React.FocusEvent ) {
		...
		const isInsidePopover = document
			...
			?.closest(
				'.fincommerce-experimental-select-tree-control__popover-menu'
			)
			?.contains( target );
		const isInRemoveTag = target?.classList.contains(
			'fincommerce-tag__remove'
		);
	...
	return (
		<div
			...
			className={ `fincommerce-experimental-select-tree-control__dropdown` }
			tabIndex={ -1 }
		>
			<div
				className={ clsx( 'fincommerce-experimental-select-control', {
					...
					<>
						{ props.multiple ? (
							<ComboBox
								comboBoxProps={ {
									className:
										'fincommerce-experimental-select-control__combo-box-wrapper',
								} }
								inputProps={ inputProps }
								suffix={
									<div className="fincommerce-experimental-select-control__suffix-items">
										{ isClearingAllowed && isOpen && (
											...
											>
												<SuffixIcon
													className="fincommerce-experimental-select-control__icon-clear"
													icon={ closeSmall }
												/>
											</Button>
										) }
