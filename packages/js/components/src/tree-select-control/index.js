...
const TreeSelectControl = ( {
	id,
	label,
	selectAllLabel = __( 'All', 'fincommerce' ),
	help,
	placeholder,
	className,
	disabled,
	...
	return (
		...
		<div
			{ ...focusOutside }
			onKeyDown={ onKeyDown }
			className={ clsx( 'fincommerce-tree-select-control', className ) }
		>
			{ !! label && (
				<label
					htmlFor={ `fincommerce-tree-select-control-${ instanceId }__control-input` }
					className="fincommerce-tree-select-control__label"
				>
					{ label }
				</label>
			) }
			...
			{ showTree && (
				<div
					ref={ dropdownRef }
					className="fincommerce-tree-select-control__tree"
					role="tree"
					tabIndex="-1"
				>
					<Options
			...
			{ help && (
				<div className="fincommerce-tree-select-control__help">
					{ help }
				</div>
			) }
		</div>
