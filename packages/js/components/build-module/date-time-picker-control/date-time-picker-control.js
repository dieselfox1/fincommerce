...
import { createElement, useCallback, useState, useEffect, useMemo, useRef, forwardRef, } from '@wordpress/element';
import { Icon, calendar } from '@wordpress/icons';
import moment from 'moment';
import clsx from 'clsx';
import { sprintf, __ } from '@wordpress/i18n';
import { useDebounce, useInstanceId } from '@wordpress/compose';
import { BaseControl, DatePicker, DateTimePicker as WpDateTimePicker, Dropdown, __experimentalInputControl as InputControl, } from '@wordpress/components';
...
export const DateTimePickerControl = forwardRef(function ForwardedDateTimePickerControl({ currentDate, isDateOnlyPicker = false, is12HourPicker = true, timeForDateOnly = 'start-of-day', dateTimeFormat, disabled = false, onChange, onBlur, label, placeholder, help, className = '', onChangeDebounceWait = 500, popoverProps = {}, ...props }, ref) {
    const id = useInstanceId(DateTimePickerControl, 'inspector-date-time-picker-control', props.id);
    ...
    const maybeForceTime = useCallback((momentDate) => {
    ...
    const formatDateTimeForDisplay = useCallback((dateTime) => {
    ...
    const [inputString, setInputString] = useState(currentDateTime.isValid()
    ...
    const setInputStringAndMaybeCallOnChange = useCallback((newInputString, isUserTypedInput) => {
    ...
    return (createElement(Dropdown, { className: clsx('fincommerce-date-time-picker-control', className), focusOnMount: false, onToggle: callOnBlurIfDropdownIsNotOpening, renderToggle: ({ isOpen, onClose, onToggle }) => (createElement(BaseControl, { id: id, label: label, help: help },
            createElement(InputControl, { ...props, id: id, ref: (element) => {
                ...
                }, suffix: createElement(Icon, { icon: calendar, className: "calendar-icon fincommerce-date-time-picker-control__input-control__suffix", onClick: focusInputControl, size: 16 }), placeholder: placeholder, "aria-describedby": sprintf(
                /* translators: A datetime format */
                __('Date input describing a selected date in format %s', 'fincommerce'), dateTimeFormat), onFocus: () => {
                    if (isOpen) {
            ...
            const Picker = isDateOnlyPicker
                ? DatePicker
                : WpDateTimePicker;
            return (createElement(Picker, { currentDate: inputStringDateTime.isValid()
