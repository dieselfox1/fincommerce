...
const dateStringFormat = __('MMM D, YYYY', 'fincommerce');
const dateFormat = __('MM/DD/YYYY', 'fincommerce');
class DateFilter extends Component {
    ...
    getBetweenString() {
        return _x('<after/><span> and </span><before/>', 'Date range inputs arranged on a single line', 'fincommerce');
    }
    getScreenReaderText(filterRule, config) {
        ...
        if (rule.value === 'between') {
            filterStr = createInterpolateElement(this.getBetweenString(), {
                after: (createElement(Fragment, null, after.format(dateStringFormat))),
                before: (createElement(Fragment, null, before.format(dateStringFormat))),
    ...
    getFormControl({ date, error, onUpdate, text }) {
        return (createElement(DatePicker, { date: date, dateFormat: dateFormat, error: error, isInvalidDate: this.isFutureDate, onUpdate: onUpdate, text: text }));
    }
    getRangeInput() {
    ...
    render() {
        ...
        const children = createInterpolateElement(labels.title, {
            ...
            rule: (createElement(SelectControl, { className: clsx(className, 'fincommerce-filters-advanced__rule'), options: rules, value: rule, onChange: this.onRuleChange, "aria-label": labels.rule })),
