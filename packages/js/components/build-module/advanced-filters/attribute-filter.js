...
const getScreenReaderText = ({ attributeTerms, config, filter, selectedAttribute, selectedAttributeTerm, }) => {
    ...
    __('<attribute/> <equals/> <value/>', 'fincommerce'), {
...
const AttributeFilter = (props) => {
    ...
    return (createElement("fieldset", { className: "fincommerce-filters-advanced__line-item", tabIndex: "0" },
        ...
        createElement("div", { className: clsx('fincommerce-filters-advanced__fieldset', {
            ...
            rule: (createElement(Select, { className: clsx(className, 'fincommerce-filters-advanced__rule'), options: rules, value: rule, onChange: (selectedValue) => onFilterChange({
                    property: 'rule',
                    value: selectedValue,
                }), "aria-label": labels.rule })),
            filter: (createElement("div", { className: clsx(className, 'fincommerce-filters-advanced__attribute-fieldset') },
                !Array.isArray(value) ||
                    !value.length ||
                    selectedAttribute.length ? (createElement(Search, { className: "fincommerce-filters-advanced__input fincommerce-search", onChange: ([attr]) => {
                    ...
                    }, type: "attributes", placeholder: __('Attribute name', 'fincommerce'), multiple: false, selected: selectedAttribute, inlineTags: true, "aria-label": __('Attribute name', 'fincommerce') })) : (createElement(Spinner, null)),
                selectedAttribute.length > 0 &&
                    (attributeTerms.length ? (createElement(Fragment, null,
                        createElement("span", { className: "fincommerce-filters-advanced__attribute-field-separator" }, "="),
                        createElement(SelectControl, { className: "fincommerce-filters-advanced__input fincommerce-search", placeholder: __('Attribute value', 'fincommerce'), inlineTags: true, isSearchable: true, multiple: false, showAllOnFocus: true, options: attributeTerms, selected: selectedAttributeTerm, onChange: (term) => {
