...
const OrderStatus = ({ order: { status }, className, orderStatusMap, labelPositionToLeft = false, }) => {
    const indicatorClasses = clsx('fincommerce-order-status__indicator', {
        ['is-' + status]: true,
    });
    const label = orderStatusMap[status] || status;
    return (createElement("div", { className: clsx('fincommerce-order-status', className) }, labelPositionToLeft ? (createElement(Fragment, null,
        label,
        createElement("span", { className: indicatorClasses }))) : (createElement(Fragment, null,
        createElement("span", { className: indicatorClasses }),
        label))));
};
