 ...
 * @return array
 */
export const removeItem = (items, removeIndex) => items.filter((_, index) => index !== removeIndex);
/**
 ...
 * @return array
 */
export const replaceItem = (items, replaceIndex, newProps) => {
    const newChildren = [...items];
    newChildren.splice(replaceIndex, 1, cloneElement(items[replaceIndex], newProps));
    return newChildren;
};
