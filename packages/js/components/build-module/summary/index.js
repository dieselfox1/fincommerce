 ...
 * @return {Object} -
 */
const SummaryList = ({ children, isDropdownBreakpoint, label = __('Performance Indicators', 'fincommerce'), }) => {
    ...
    const summaryMenu = (createElement(Menu, { label: label, orientation: orientation, itemCount: itemCount, items: items }));
    ...
    return (createElement(Dropdown, { className: "fincommerce-summary", popoverProps: {
            placement: 'bottom',
        }, headerTitle: label, renderToggle: ({ isOpen, onToggle }) => cloneElement(selected, { onToggle, isOpen }), renderContent: (renderContentArgs) => (createElement(Menu, { label: label, orientation: orientation, itemCount: itemCount, items: children(renderContentArgs) })) }));
};
