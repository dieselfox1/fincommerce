...
import { useEffect, useMemo, useState, useRef, createElement, } from '@wordpress/element';
import clsx from 'clsx';
import { __experimentalUseFocusOutside as useFocusOutside, useInstanceId, } from '@wordpress/compose';
/**
 ...
 */
const TreeSelectControl = ({ id, label, selectAllLabel = __('All', 'fincommerce'), help, placeholder, className, disabled, options = [], value = [], maxVisibleTags, onChange = () => { }, onDropdownVisibilityChange = noop, onInputChange = noop, includeParent = false, individuallySelectParent = false, alwaysShowPlaceholder = false, minFilterQueryLength = 3, clearOnSelect = true, }) => {
    ...
    const filteredOptions = useMemo(() => {
        ...
        const highlightOptionLabel = (optionLabel, matchPosition) => {
            ...
            return (createElement("span", null,
                createElement("span", null, optionLabel.substring(0, matchPosition)),
                createElement("strong", null, optionLabel.substring(matchPosition, matchLength)),
                createElement("span", null, optionLabel.substring(matchLength))));
        };
        const descriptors = {
            hasChildren: {
                /**
            ...
            partialChecked: {
                ...
                get() {
                    ...
                    return (!this.checked &&
                        this.children.some((opt) => opt.checked || opt.partialChecked));
                },
            },
            isVisible: {
                /**
            ...
            expanded: {
                 ...
                 */
                get() {
                    return ((isSearching && this.isVisible) ||
    ...
    const handleSingleChange = (checked, option, parent) => {
        ...
        if (includeParent &&
            ...
            parent.children &&
            parent.children.every((child) => newValue.includes(child.value)) &&
    ...
    createElement("div", { ...focusOutside, onKeyDown: onKeyDown, className: clsx('fincommerce-tree-select-control', className) },
        !!label && (createElement("label", { htmlFor: `fincommerce-tree-select-control-${instanceId}__control-input`, className: "fincommerce-tree-select-control__label" }, label)),
        createElement(Control, { ref: controlRef, disabled: disabled, tags: tags, isExpanded: showTree, onFocus: () => {
                setTreeVisible(true);
            }, onControlClick: () => {
                setTreeVisible(true);
            }, instanceId: instanceId, placeholder: placeholder, label: label, maxVisibleTags: maxVisibleTags, value: inputControlValue, onTagsChange: handleTagsChange, onInputChange: handleOnInputChange, alwaysShowPlaceholder: alwaysShowPlaceholder }),
        showTree && (createElement("div", { ref: dropdownRef, className: "fincommerce-tree-select-control__tree", role: "tree", tabIndex: "-1" },
            createElement(Options, { options: filteredOptions, onChange: handleOptionsChange, onExpanderClick: handleExpanderClick, onToggleExpanded: handleToggleExpanded }))),
        help && (createElement("div", { className: "fincommerce-tree-select-control__help" }, help))));
};
