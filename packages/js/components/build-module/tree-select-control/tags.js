...
const Tags = ({ tags = [], disabled, maxVisibleTags = 0, onChange = () => { }, }) => {
    ...
    return (createElement("div", { className: "fincommerce-tree-select-control__tags" },
        visibleTags.map((item, i) => {
            ...
            __('%1$s (%2$d of %3$d)', 'fincommerce'), item.label, i + 1, tags.length);
            return (createElement(Tag, { key: item.id, id: item.id, label: item.label, screenReaderLabel: screenReaderLabel, remove: remove }));
        }),
        maxTags > 0 && tags.length > maxTags && (createElement(Button, { isTertiary: true, className: "fincommerce-tree-select-control__show-more", onClick: () => {
                setShowAll(!showAll);
            } }, showAll
            ? __('Show less', 'fincommerce')
            : sprintf(
            // translators: %d: The number of extra tags to show
            __('+ %d more', 'fincommerce'), tags.length - maxTags)))));
};
export default Tags;
