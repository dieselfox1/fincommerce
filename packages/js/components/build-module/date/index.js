...
const Date = ({ date, machineFormat = 'Y-m-d H:i:s', screenReaderFormat = 'F j, Y', visibleFormat = 'Y-m-d', }) => {
    return (createElement("time", { dateTime: formatDate(machineFormat, date) },
        createElement("span", { "aria-hidden": "true" }, formatDate(visibleFormat, date)),
        createElement("span", { className: "screen-reader-text" }, formatDate(screenReaderFormat, date))));
};
Date.propTypes = {
    /**
