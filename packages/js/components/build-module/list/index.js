...
function List(props) {
    ...
    const listClassName = clsx('fincommerce-list', className);
    ...
    return (createElement(TransitionGroup, { component: "ul", className: listClassName, role: "menu" }, items.map((item, index) => {
        const { className: itemClasses, href, key, onClick } = item;
        const hasAction = typeof onClick === 'function' || href;
        const itemClassName = clsx('fincommerce-list__item', itemClasses, {
            'has-action': hasAction,
        });
        return (createElement(CSSTransition, { key: key || index, timeout: 500, classNames: "fincommerce-list__item" },
            createElement("li", { className: itemClassName }, children ? (children(item, index)) : (createElement(LegacyListItem, { item: item })))));
    })));
}
List.propTypes = {
    /**
