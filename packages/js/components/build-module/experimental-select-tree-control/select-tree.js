...
import { createElement, useEffect, useState, Fragment, useRef, } from '@wordpress/element';
 ...
 */
import { toggleNode, createLinkedTree, getVisibleNodeIndex as getVisibleNodeIndex, getNodeDataByIndex, } from '../experimental-tree-control/linked-tree-utils';
...
export const SelectTree = function SelectTree({ items, treeRef: ref, isLoading, disabled, initialInputValue, onInputChange, shouldShowCreateButton, help, isClearingAllowed = false, onClear = () => { }, ...props }) {
    const [linkedTree, setLinkedTree] = useState([]);
    ...
    const selectTreeInstanceId = useInstanceId(SelectTree, 'fincommerce-experimental-select-tree-control__dropdown');
    const menuInstanceId = useInstanceId(SelectTree, 'fincommerce-select-tree-control__menu');
    const selectedItemsFocusHandle = useRef(null);
    ...
    const inputProps = {
        ...
        onFocus: (event) => {
            if (props.multiple) {
                speak(__('To select existing items, type its exact label and separate with commas or the Enter key.', 'fincommerce'));
            }
            if (!isOpen) {
                setIsOpen(true);
            }
        ...
        onKeyDown: (event) => {
            ...
            if (event.key === 'ArrowDown') {
                ...
                highlightedIndex === items.length - 1 &&
                    ...
                    const visibleNodeIndex = getVisibleNodeIndex(linkedTree, Math.min(highlightedIndex + 1, items.length), 'down');
                ...
                if (highlightedIndex > 0) {
                    const visibleNodeIndex = getVisibleNodeIndex(linkedTree, Math.max(highlightedIndex - 1, -1), 'up');
                    if (visibleNodeIndex !== undefined) {
                        setHighlightedIndex(visibleNodeIndex);
                    }
                }
                ...
                highlightedIndex !== -1) {
                    const nodeData = getNodeDataByIndex(linkedTree, highlightedIndex);
                    if (!nodeData) {
                        return;
                    }
                    ...
                    const isAlreadySelected = Array.isArray(props.selected)
                        ? Boolean(props.selected.find((i) => i.label === escapeHTML(inputValue)))
                ...
                setLinkedTree(toggleNode(linkedTree, highlightedIndex, true));
            }
            else if (event.key === 'ArrowLeft') {
                setLinkedTree(toggleNode(linkedTree, highlightedIndex, false));
            }
    ...
    return (createElement("div", { id: selectTreeInstanceId, className: `fincommerce-experimental-select-tree-control__dropdown`, tabIndex: -1 },
        createElement("div", { className: clsx('fincommerce-experimental-select-control', {
            ...
            createElement(BaseControl, { label: props.label, id: `${props.id}-input`, help: props.multiple && !help
                    ? __('Separate with commas or the Enter key.', 'fincommerce')
                    : help },
                createElement(Fragment, null,
                    props.multiple ? (createElement(ComboBox, { comboBoxProps: {
                            className: 'fincommerce-experimental-select-control__combo-box-wrapper',
                        }, inputProps: inputProps, suffix: createElement("div", { className: "fincommerce-experimental-select-control__suffix-items" },
                            isClearingAllowed && isOpen && (createElement(Button, { label: __('Remove all', 'fincommerce'), onClick: handleClear },
                                createElement(SuffixIcon, { className: "fincommerce-experimental-select-control__icon-clear", icon: closeSmall }))),
                            createElement(SuffixIcon, { icon: isOpen ? chevronUp : chevronDown })) },
                        createElement(SelectedItems, { isReadOnly: isReadOnly, ref: selectedItemsFocusHandle, items: !Array.isArray(props.selected)
                                ? [props.selected]
                            ...
                            }, onSelectedItemsEnd: focusOnInput, getSelectedItemProps: () => ({}) }))) : (createElement(TextControl, { ...inputProps, value: decodeEntities(props.createValue || ''), onChange: (value) => {
                            if (onInputChange)
                                onInputChange(value);
                    ...
                    createElement(SelectTreeMenu, { ...props, onSelect: (item) => {
                            ...
                            setLinkedTree(toggleNode(linkedTree, index, value));
