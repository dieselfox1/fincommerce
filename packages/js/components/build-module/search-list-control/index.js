...
import { Button, Spinner, TextControl, withSpokenMessages, } from '@wordpress/components';
...
const defaultMessages = {
    clear: __('Clear all selected items', 'fincommerce'),
    noItems: __('No items found.', 'fincommerce'),
    /* translators: %s: search term */
    noResults: __('No results for %s', 'fincommerce'),
    search: __('Search for items', 'fincommerce'),
    selected: (n) => sprintf(
    /* translators: Number of items selected from list. */
    _n('%d item selected', '%d items selected', n, 'fincommerce'), n),
    updated: __('Search results updated.', 'fincommerce'),
};
/**
...
export const SearchListControl = (props) => {
    ...
    const defaultRenderItem = (args) => {
        return createElement(SearchListItem, { ...args });
    };
    const renderList = (list, depth = 0) => {
        ...
        return list.map((item) => (createElement(Fragment, { key: item.id },
            createElement("li", null, renderItem({
                item,
    ...
    const renderListSection = () => {
        if (isLoading) {
            return (createElement("div", { className: "fincommerce-search-list__list is-loading" },
                createElement(Spinner, null)));
        }
        const list = getFilteredList(props.list, searchValue);
        if (!list.length) {
            return (createElement("div", { className: "fincommerce-search-list__list is-not-found" },
                createElement("span", { className: "fincommerce-search-list__not-found-icon" },
                    createElement(NoticeOutlineIcon, { role: "img", "aria-hidden": "true", focusable: "false" })),
                createElement("span", { className: "fincommerce-search-list__not-found-text" }, searchValue
                    ? // eslint-disable-next-line @wordpress/valid-sprintf
                        sprintf(messages.noResults || '', searchValue)
                    : messages.noItems)));
        }
        return (createElement("ul", { className: "fincommerce-search-list__list" }, renderList(list)));
    };
    const renderSelectedSection = () => {
        ...
        return (createElement("div", { className: "fincommerce-search-list__selected" },
            createElement("div", { className: "fincommerce-search-list__selected-header" },
                createElement("strong", null, messages.selected(selectedCount)),
                selectedCount > 0 ? (createElement(Button, { isLink: true, isDestructive: true, onClick: onChange([]), "aria-label": messages.clear }, __('Clear all', 'fincommerce'))) : null),
            selectedCount > 0 ? (createElement("ul", null, selected.map((item, i) => (createElement("li", { key: i },
                createElement(Tag, { label: item.name, id: item.id, remove: onRemove })))))) : null));
    };
    return (createElement("div", { className: clsx('fincommerce-search-list', className, {
            'is-compact': isCompact,
        }) },
        renderSelectedSection(),
        createElement("div", { className: "fincommerce-search-list__search" },
            createElement(TextControl, { label: messages.search, type: "search", value: searchValue, onChange: (value) => setSearchValue(value) })),
        renderListSection()));
};
...
export default compose([withSpokenMessages, withInstanceId])(SearchListControl);
