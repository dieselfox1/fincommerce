...
export const getColor = (orderedKeys, colorScheme) => (key) => {
    const len = orderedKeys.length > selectionLimit
        ? selectionLimit
        : orderedKeys.length;
     ...
     *
     * @filter fincommerce_admin_chart_item_color
     * @example
     * addFilter(
     * 	'fincommerce_admin_chart_item_color',
     * 	'example',
     * ( idx ) => {
     * 	const colorScales = [
     *	  "#0A2F51",
     ...
     *
     */
    const color = applyFilters('fincommerce_admin_chart_item_color', idx, key, orderedKeys);
