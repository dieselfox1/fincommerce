...
class D3Legend extends Component {
    ...
    render() {
        ...
        return (createElement("div", { className: clsx('fincommerce-legend', `fincommerce-legend__direction-${legendDirection}`, {
                'has-total': showTotalLabel,
                'is-scrollable': isScrollable,
            }, this.props.className) },
            createElement("ul", { className: "fincommerce-legend__list", ref: this.listRef, onScroll: showTotalLabel ? this.updateListScroll : null }, data.map((row) => (createElement("li", { className: clsx('fincommerce-legend__item', {
                ...
                createElement("button", { role: "checkbox", "aria-checked": row.visible ? 'true' : 'false', onClick: handleLegendToggle, id: `fincommerce-legend-${instanceId}__item-button__${row.key}`, disabled: (row.visible &&
                        numberOfRowsVisible <= 1) ||
                        (!row.visible &&
                            numberOfRowsVisible >=
                                selectionLimit) ||
                        !interactive, title: numberOfRowsVisible >= selectionLimit
                        ? sprintf(
                        /* translators: %d: number of items selected */
                        __('You may select up to %d items.', 'fincommerce'), selectionLimit)
                        : '' },
                    createElement("div", { className: "fincommerce-legend__item-container" },
                        createElement("span", { className: clsx('fincommerce-legend__item-checkmark', {
                                ...
                                : null }),
                        createElement("span", { className: "fincommerce-legend__item-title" }, row.label),
                        createElement("span", { className: "fincommerce-legend__item-total" }, getFormatter(legendValueFormat)(row.total)))))))),
            showTotalLabel && (createElement("div", { className: "fincommerce-legend__total" }, totalLabel))));
    }
}
D3Legend.propTypes = {
    /**
