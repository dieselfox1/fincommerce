...
const TimelineGroup = ({ group = { title: '', items: [] }, className = '', orderBy = 'desc', clockFormat, }) => {
    const groupClassName = clsx('fincommerce-timeline-group', className);
    const itemsToTimlineItem = (item, itemIndex) => {
        const itemKey = group.title + '-' + itemIndex;
        return (createElement(TimelineItem, { key: itemKey, item: item, clockFormat: clockFormat }));
    };
    return (createElement("li", { className: groupClassName },
        createElement("p", { className: 'fincommerce-timeline-group__title' }, group.title),
        createElement("ul", null, group.items
...
TimelineGroup.propTypes = {
    ...
    group: PropTypes.shape({
        ...
        items: PropTypes.arrayOf(PropTypes.shape({
             ...
             */
            body: PropTypes.arrayOf(PropTypes.oneOfType([
                PropTypes.element,
                PropTypes.string,
            ])),
            /**
