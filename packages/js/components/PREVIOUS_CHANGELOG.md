...
-   Made @fincommerce/components/Stepper a Typescript file. #8286
-   Added Typescript type declarations to build for @fincommerce/components #8282
...
-   Grow and center buttons in all FinCommerce ellipsis menu popover containers. #8168
...
-   Remove `fincommerce/experimental` dependency. #6986
...
-   Add `@fincommerce/experimental`, `md5` and `dompurify` as dependencies. #6804
...
-   `AdvancedFilters` and `ReportsFilters` receive `currency` as a prop, it is required and must be an instance of the new `Currency` object exported by `@fincommerce/currency`
...
-   Update `@fincommerce/navigation` to v2.0.0
-   Bug fix for `<StockReportTable />` returning N/A instead of zero.
