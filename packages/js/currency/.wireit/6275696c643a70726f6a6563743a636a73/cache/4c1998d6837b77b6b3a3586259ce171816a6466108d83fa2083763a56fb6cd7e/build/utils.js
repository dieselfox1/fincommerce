...
exports.unformatLocalisedMonetaryValue = exports.localiseMonetaryValue = exports.CurrencyFactory = void 0;
...
const number_1 = require("@fincommerce/number");
...
const CurrencyFactoryBase = function (currencySetting) {
    ...
    function setCurrency(setting) {
        ...
        currency = {
            ...
            symbol: (0, html_entities_1.decodeEntities)(config.symbol.toString()),
    ...
    function formatAmount(number, useCode = false) {
        const formattedNumber = (0, number_1.numberFormat)(currency, number);
        if (formattedNumber === '') {
            return formattedNumber;
        }
        const { priceFormat, symbol, code } = currency;
        // eslint-disable-next-line @wordpress/valid-sprintf
        return (0, i18n_1.sprintf)(priceFormat, useCode ? code : symbol, formattedNumber);
    }
    /**
     * Formats money value.
     *
    ...
    function formatCurrency(number) {
        (0, deprecated_1.default)('Currency().formatCurrency', {
            version: '5.0.0',
    ...
    function getDataForCountry(countryCode, localeInfo = {}, currencySymbols = {}) {
        ...
        return {
            ...
            symbol: (0, html_entities_1.decodeEntities)(symbol),
    ...
    return {
        ...
        render(number) {
            ...
            if (number < 0) {
                return ((0, element_1.createElement)("span", { className: "is-negative" }, formatAmount(number)));
            }
            return formatAmount(number);
        },
    };
};
exports.CurrencyFactory = CurrencyFactoryBase;
/**
...
function getCurrencyData() {
    (0, deprecated_1.default)('getCurrencyData', {
        version: '3.1.0',
    ...
    // See https://github.com/dieselfox1/fincommerce-admin/issues/3101.
    return {
        US: {
            code: 'USD',
            symbol: '$',
...
const localiseMonetaryValue = (config, number) => {
    if (typeof number === 'number') {
        return (0, number_1.numberFormat)(config, number);
    }
    if (typeof number === 'string') {
        ...
        return number.replace(regex, (n) => {
            const parsed = (0, number_1.parseNumber)(config, n);
            return (0, number_1.numberFormat)(config, parsed);
        });
    }
    return number;
};
exports.localiseMonetaryValue = localiseMonetaryValue;
const unformatLocalisedMonetaryValue = (config, inputNumber) => {
    ...
    const unformattedValue = inputNumber
        .replace(new RegExp(escapeRegExp(config.thousandSeparator), 'g'), '')
        .replace(config.decimalSeparator, '.');
    return Number(unformattedValue);
};
exports.unformatLocalisedMonetaryValue = unformatLocalisedMonetaryValue;
