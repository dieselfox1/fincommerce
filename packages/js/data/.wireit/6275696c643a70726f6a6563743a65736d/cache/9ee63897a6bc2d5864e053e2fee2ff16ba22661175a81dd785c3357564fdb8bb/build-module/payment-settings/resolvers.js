 ...
 */
import { getPaymentProvidersSuccess, getPaymentProvidersError, getPaymentProvidersRequest, setIsWooPayEligible, } from './actions';
import { WC_ADMIN_NAMESPACE } from '../constants';
import { STORE_KEY } from './constants';
const resolveSelect = controls?.resolveSelect ?? select;
export function* getPaymentProviders(businessCountry) {
    yield getPaymentProvidersRequest();
    try {
        const paymentProvidersResponse = yield apiFetch({
        ...
        yield getPaymentProvidersSuccess(paymentProvidersResponse.providers, paymentProvidersResponse.offline_payment_methods, paymentProvidersResponse.suggestions, paymentProvidersResponse.suggestion_categories);
    }
    catch (e) {
...
export function* getOfflinePaymentGateways(businessCountry) {
    yield getPaymentProvidersIfNeeded(businessCountry);
}
export function* getSuggestions(businessCountry) {
    yield getPaymentProvidersIfNeeded(businessCountry);
}
export function* getSuggestionCategories(businessCountry) {
    yield getPaymentProvidersIfNeeded(businessCountry);
}
