...
import { apiFetch as controlsApiFetch, dispatch as deprecatedDispatch, select, } from '@wordpress/data-controls';
import { controls } from '@wordpress/data';
/**
 * Internal dependencies
 */
import { STORE_NAME, WC_PRODUCT_NAMESPACE, WC_V3_ENDPOINT_SUGGESTED_PRODUCTS, } from './constants';
import { getProductError, getProductsError, getProductsSuccess, getProductsTotalCountError, getProductsTotalCountSuccess, getProductSuccess, } from './actions';
import { request } from '../utils';
import { createIdFromOptions } from './utils';
const dispatch = controls && controls.dispatch ? controls.dispatch : deprecatedDispatch;
...
export function* getProducts(query) {
    ...
    try {
        const { items, totalCount } = yield request(WC_PRODUCT_NAMESPACE, productsQuery);
...
export function* getProduct(productId) {
    try {
        const product = yield controlsApiFetch({
...
export function* getRelatedProducts(productId) {
    try {
        // Get the product.
        const product = yield resolveSelect(STORE_NAME, 'getProduct', productId);
        ...
        const relatedProducts = yield resolveSelect(STORE_NAME, 'getProducts', {
...
export function* getProductsTotalCount(query) {
    try {
        const totalsQuery = {
            ...query,
            page: 1,
        ...
        const { totalCount } = yield request(WC_PRODUCT_NAMESPACE, totalsQuery);
        yield getProductsTotalCountSuccess(query, totalCount);
        return totalCount;
    }
    catch (error) {
...
export function* getPermalinkParts(productId) {
    yield resolveSelect(STORE_NAME, 'getProduct', [productId]);
}
export const getSuggestedProducts = (options) => 
