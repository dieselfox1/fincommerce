...
import { appendTimestamp, getCurrentDates, getIntervalForQuery, } from '@fincommerce/date';
import { flattenFilters, getActiveFiltersFromQuery, getQueryFromActiveFilters, } from '@fincommerce/navigation';
...
export function getQueryFromConfig(config, advancedFilters, query) {
    ...
    if (queryValue === 'advanced') {
        ...
        const filterQuery = getQueryFromActiveFilters(activeFilters.map((filter) => timeStampFilterDates(advancedFilters, filter)), {}, advancedFilters.filters);
        return {
...
export function getSummaryNumbers(options) {
    ...
    const { getReportStats, getReportStatsError, isResolving } = select(store);
    const response = {
        isRequesting: false,
        isError: false,
        totals: {
