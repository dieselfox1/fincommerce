...
fincommercePackages.forEach((lib) => {
    Object.defineProperty(global.wc, lib, {
        get: () => require(`@fincommerce/${lib}`),
    });
});
const config = require('../../../../plugins/fincommerce/client/admin/config/development.json');
// Check if test is jsdom or node
if (global.window) {
    window.wcAdminFeatures = config && config.features ? config.features : {};
}
(0, i18n_1.setLocaleData)({ '': { domain: 'fincommerce', lang: 'en_US' } }, 'fincommerce');
// Mock core/notices store for components dispatching core notices
(0, data_1.registerStore)('core/notices', {
    reducer: () => {
        return {};
    },
    actions: {
