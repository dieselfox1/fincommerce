...
export const useItems = (exclude) => {
    return useSelect((select) => {
        ...
        const allPages = select(coreDataStore).getEntityRecords(...args) || null;
        return {
            items: allPages?.map(formatPageToItem) || [],
            isFetching: !select(coreDataStore).hasFinishedResolution('getEntityRecords', args),
        };
    }, [exclude]);
};
/**
 ...
 */
export const useSelectedItem = (value) => {
    const { selectedPage, isLoading } = useSelect((select) => {
        if (!value) {
    ...
    const selectedItem = useMemo(() => (selectedPage ? formatPageToItem(selectedPage) : null), [selectedPage]);
    return {
        selectedItem,
        isLoading,
    };
