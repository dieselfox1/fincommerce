...
export const Form = ( {
		...
		apiFetch( {
			...
			.then( ( response ) => {
				...
				if ( status === 'success' ) {
					...
					createNotice(
						'success',
						__( 'Settings saved successfully', 'fincommerce' )
					);
				} else {
					createNotice(
						'error',
						__( 'Failed to save settings', 'fincommerce' )
					);
				}
			} )
			.catch( ( error ) => {
	...
	return (
		<form ref={ formRef } id="mainform" onSubmit={ handleSubmit }>
			...
			<div className="fincommerce-settings-content">
				<DataForm
					fields={ fields }
					form={ form }
					data={ data }
			...
			<div className="fincommerce-settings-content-footer">
				<Button
					variant="primary"
					type="submit"
					isBusy={ isBusy }
