...
export const presetValues = [
    { value: 'today', label: __('Today', 'fincommerce') },
    { value: 'yesterday', label: __('Yesterday', 'fincommerce') },
    { value: 'week', label: __('Week to date', 'fincommerce') },
    { value: 'last_week', label: __('Last week', 'fincommerce') },
    { value: 'month', label: __('Month to date', 'fincommerce') },
    { value: 'last_month', label: __('Last month', 'fincommerce') },
    { value: 'quarter', label: __('Quarter to date', 'fincommerce') },
    { value: 'last_quarter', label: __('Last quarter', 'fincommerce') },
    { value: 'year', label: __('Year to date', 'fincommerce') },
    { value: 'last_year', label: __('Last year', 'fincommerce') },
    { value: 'custom', label: __('Custom', 'fincommerce') },
];
export const periods = [
    {
        value: 'previous_period',
        label: __('Previous period', 'fincommerce'),
    },
    {
        value: 'previous_year',
        label: __('Previous year', 'fincommerce'),
    },
];
const isValidMomentInput = (input) => moment(input).isValid();
/**
...
export function getRangeLabel(after, before) {
    ...
    const fullDateFormat = __('MMM D, YYYY', 'fincommerce');
    if (isSameDay) {
        ...
        const monthDayFormat = __('MMM D', 'fincommerce');
 ...
 * @return {DateValue} - DateValue data about the selected period
 */
const getDateValue = memoize((period, compare, after, before) => {
    switch (period) {
        case 'today':
 ...
 */
const getDateParamsFromQueryMemoized = memoize((period, compare, after, before, defaultDateRange) => {
    if (period && compare) {
        return {
            period,
            compare,
 ...
 */
const getCurrentDatesMemoized = memoize((period, compare, primaryStart, primaryEnd, secondaryStart, secondaryEnd) => {
    const primaryItem = find(presetValues, (item) => item.value === period);
    if (!primaryItem) {
        throw new Error(`Cannot find period: ${period}`);
    }
    const secondaryItem = find(periods, (item) => item.value === compare);
    if (!secondaryItem) {
 ...
 */
export const getPreviousDate = (date, date1, date2, compare = 'previous_year', interval) => {
 ...
 */
export function getAllowedIntervalsForQuery(query, defaultDateRange = 'period=&compare=previous_year') {
 ...
 */
export function getIntervalForQuery(query, defaultDateRange = 'period=&compare=previous_year') {
...
export function getDateFormatsForIntervalPhp(interval, ticks = 0) {
    ...
    switch (interval) {
        ...
        case 'week':
            ...
            const escapedWeekOfStr = __('Week of', 'fincommerce').replace(/(\w)/g, '\\$1');
 ...
 * @return {string} Current interval.
 */
export function getDateFormatsForInterval(interval, ticks = 0, option = { type: 'd3' }) {
    switch (option.type) {
        case 'php':
...
export function loadLocaleData({ userLocale, weekdaysShort, }) {
    ...
    if (moment.locale() !== 'en') {
        moment.updateLocale(userLocale, {
            longDateFormat: {
                L: __('MM/DD/YYYY', 'fincommerce'),
                LL: __('MMMM D, YYYY', 'fincommerce'),
                LLL: __('D MMMM YYYY LT', 'fincommerce'),
                LLLL: __('dddd, D MMMM YYYY LT', 'fincommerce'),
                LT: __('HH:mm', 'fincommerce'),
...
export const dateValidationMessages = {
    invalid: __('Invalid date', 'fincommerce'),
    future: __('Select a date in the past', 'fincommerce'),
    startAfterEnd: __('Start date must be before end date', 'fincommerce'),
    endBeforeStart: __('Start date must be before end date', 'fincommerce'),
};
/**
 ...
 */
export function containsLeapYear(startDate, endDate) {
