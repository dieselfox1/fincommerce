 ...
 */
export const findCountryOption = (countryStateOptions, location, minimumSimilarity = 0.7) => {
    if (!location) {
        return null;
    }
    let match = null;
    ...
    for (const option of countryStateOptions) {
        ...
        if (countryCode === location.country_short &&
            ...
            const similarity = Math.max(stringSimilarity.compareTwoStrings(wcRegion, location.region || ''), stringSimilarity.compareTwoStrings(wcRegion, location.city || ''));
 ...
 */
export const getCountry = (countryPossiblyWithState) => countryPossiblyWithState?.split(':')[0] ?? undefined;
