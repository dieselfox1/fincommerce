...
export const WooPaymentsMethodsLogos = ( {
	...
	const handleClick = ( event: React.MouseEvent | React.KeyboardEvent ) => {
		...
		const parentDiv = clickedElement.closest(
			'.fincommerce-woopayments-payment-methods-logos-count'
		);
	...
	return (
		<div className="fincommerce-woopayments-payment-methods-logos">
			{ visiblePaymentMethods.map( ( pm ) => pm.component ) }
			{ maxShownElements < maxSupportedPaymentMethods && (
				<div
					className="fincommerce-woopayments-payment-methods-logos-count"
					role="button"
					tabIndex={ 0 }
					ref={ buttonRef }
					onClick={ handleClick }
				...
				>
					...
					{ isPopoverVisible && (
						<Popover
							className="fincommerce-woopayments-payment-methods-logos-popover"
							placement="top-start"
							offset={ 4 }
							variant="unstyled"
							focusOnMount={ true }
