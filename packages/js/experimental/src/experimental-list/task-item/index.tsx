...
const OptionalExpansionWrapper = ( {
	...
	return (
		<VerticalCSSTransition
			timeout={ 500 }
			in={ expanded }
			classNames="fincommerce-task-list__item-expandable-content"
			defaultStyle={ {
	...
	return (
		...
		>
			<OptionalTaskTooltip level={ level } completed={ completed }>
				<div className="fincommerce-task-list__item-before">
					{ level === 1 && ! completed ? (
						...
						<div className="fincommerce-task__icon">
			...
			<div className="fincommerce-task-list__item-text">
				...
				>
					<span className="fincommerce-task-list__item-title">
						{ title }
						{ badge && (
							<span className="fincommerce-task-list__item-badge">
								{ badge }
							</span>
						) }
					</span>
					<OptionalExpansionWrapper
						...
						expanded={ isTaskExpanded }
					>
						<div className="fincommerce-task-list__item-expandable-content">
							{ content }
							{ expandable && ! completed && additionalInfo && (
								<div
									className="fincommerce-task__additional-info"
							...
							{ ! completed && showActionButton && (
								<Button
									className="fincommerce-task-list__item-action"
									isPrimary
									onClick={ (
										event:
											| React.MouseEvent
					...
					{ time && (
						<div className="fincommerce-task__estimated-time">
							{ time }
						</div>
					) }
				</Text>
				{ inProgress && inProgressLabel && (
					<div className="fincommerce-task-list__item-progress">
						{ inProgressLabel }
					</div>
				) }
			</div>
			{ showEllipsisMenu && (
				<EllipsisMenu
					label={ __( 'Task Options', 'fincommerce' ) }
					className="fincommerce-task-list__item-after"
