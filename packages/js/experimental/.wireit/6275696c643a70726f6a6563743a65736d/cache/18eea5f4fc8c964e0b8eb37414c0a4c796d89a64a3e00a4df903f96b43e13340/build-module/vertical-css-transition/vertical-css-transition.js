 ...
 */
import { createElement, useState, useCallback, useEffect, useRef, } from '@wordpress/element';
 ...
 */
export const VerticalCSSTransition = ({ children, defaultStyle, ...props }) => {
    const [containerHeight, setContainerHeight] = useState(0);
    const [transitionIn, setTransitionIn] = useState(props.in || false);
    const cssTransitionRef = useRef(null);
    const collapseContainerRef = useCallback((containerElement) => {
        if (containerElement) {
    ...
    return (createElement(CSSTransition, { ...props, in: transitionIn, ref: cssTransitionRef }, (state) => (createElement("div", { className: "vertical-css-transition-container", style: getTransitionStyle(state), ref: collapseContainerRef }, children))));
};
