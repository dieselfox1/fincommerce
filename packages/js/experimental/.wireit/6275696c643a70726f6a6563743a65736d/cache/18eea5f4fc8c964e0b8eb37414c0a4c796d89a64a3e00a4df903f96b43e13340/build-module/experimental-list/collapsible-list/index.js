...
import { createElement, useState, useCallback, useEffect, Children, useRef, isValidElement, cloneElement, } from '@wordpress/element';
...
export const ExperimentalCollapsibleList = ({ children, collapsed = true, collapseLabel, expandLabel, show = 0, onCollapse, onExpand, direction = 'up', ...listProps }) => {
    const [isCollapsed, setCollapsed] = useState(collapsed);
    const [isTransitionComponentCollapsed, setTransitionComponentCollapsed] = useState(collapsed);
    ...
    const updateChildren = () => {
        let shownChildren = [];
        const allChildren = Children.map(children, (child) => isValidElement(child) && 'key' in child ? child : null) || [];
    ...
    const listClasses = clsx(listProps.className || '', 'fincommerce-experimental-list');
    const wrapperClasses = clsx({
        'fincommerce-experimental-list-wrapper': !isCollapsed,
    });
    const hiddenChildren = displayedChildren.hidden.length > 0 ? (createElement(ExperimentalListItem, { key: "collapse-item", className: "list-item-collapse", onClick: clickHandler, animation: "none", disableGutters: true },
        createElement("p", null, isCollapsed
            ? footerLabels.expand
            : footerLabels.collapse),
        createElement(Icon, { className: "list-item-collapse__icon", size: 30, icon: isCollapsed ? chevronDown : chevronUp }))) : null;
    return (createElement(ExperimentalList, { ...listProps, className: listClasses }, [
        direction === 'down' && hiddenChildren,
        ...displayedChildren.shown,
        createElement(Transition, { key: "remaining-children", timeout: 500, in: !isTransitionComponentCollapsed, mountOnEnter: true, unmountOnExit: false }, (state) => {
            const transitionStyles = getTransitionStyle(state, isCollapsed, collapseContainerRef.current);
            return (createElement("div", { className: wrapperClasses, ref: collapseContainerRef, style: transitionStyles },
                createElement(TransitionGroup, { className: "fincommerce-experimental-list" }, Children.map(displayedChildren.hidden, (child) => {
                    const { onExited, in: inTransition, enter, exit, ...remainingProps } = child.props;
                    const animationProp = remainingProps.animation ||
                        listProps.animation;
                    return (createElement(CSSTransition, { key: child.key, timeout: 500, onExited: onExited, in: inTransition, enter: enter, exit: exit, classNames: "fincommerce-list__item" }, cloneElement(child, {
