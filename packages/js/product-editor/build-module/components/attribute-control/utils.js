 ...
 * @return string
 */
export const getAttributeId = (attribute) => `${attribute.id}-${attribute.name}`;
/**
 ...
 */
export function reorderSortableProductAttributePositions(items, attributeKeyValues) {
 ...
 */
export const hasTermsOrOptions = (attribute) => !!(attribute?.terms?.length || attribute?.options?.length);
/**
 ...
 */
export const isAttributeFilledOut = (attribute) => !!attribute?.name.length && hasTermsOrOptions(attribute);
