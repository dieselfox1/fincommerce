...
import { Button, FormTokenField as CoreFormTokenField, } from '@wordpress/components';
import { useSelect, useDispatch, select as sel, dispatch, } from '@wordpress/data';
...
const FormTokenField = CoreFormTokenField;
/**
...
export const AttributeTableRow = ({ index, attribute, attributePlaceholder, disabledAttributeMessage, isLoadingAttributes, attributes, onNewAttributeAdd, onAttributeSelect, termPlaceholder, onTermsSelect, termsAutoSelection, clearButtonDisabled, removeLabel, onRemove, }) => {
    ...
    // @ts-ignore
    (select) => {
        const { getProductAttributeTerms } = select(experimentalProductAttributeTermsStore);
        return attributeId
     ...
     */
    const [localTerms, setLocalTerms] = useState(attribute?.options?.map(stringToTokenItem) || []);
    /**
    ...
    async function addNewTerms(newTokens) {
        ...
        const promises = newTokens.map(async (token) => {
            try {
                ...
                dispatch('core/notices').createErrorNotice(sprintf(
                /* translators: %s: the attribute term */
                __('There was an error trying to create the attribute term "%s".', 'fincommerce'), token.value));
                return undefined;
            }
        });
         ...
         */
        const recentTermsList = sel(experimentalProductAttributeTermsStore).getProductAttributeTerms(selectItemsQuery);
        /*
    ...
    return (createElement("tr", { key: index, className: `fincommerce-new-attribute-modal__table-row fincommerce-new-attribute-modal__table-row-${index}` },
        createElement("td", { className: "fincommerce-new-attribute-modal__table-attribute-column" },
            createElement(AttributesComboboxControl, { instanceNumber: index, placeholder: attributePlaceholder, current: attribute, items: filteredAttributes, isLoading: isLoadingAttributes, onAddNew: (newValue) => onNewAttributeAdd?.(newValue, index), onChange: (nextAttribute) => {
        ...
        createElement("td", { className: `fincommerce-new-attribute-modal__table-attribute-value-column${hasAvailableSuggestions ? ' has-values' : ''}` },
            createElement(FormTokenField, { placeholder: termPlaceholder, disabled: !attribute, suggestions: tokenFieldSuggestions, value: tokenFieldValues, onChange: (nextTokens) => {
                     ...
                     * but different slugs.
                     */
                    const slugSuggestions = tokenFieldSuggestions.map(cleanForSlug);
                    /*
        ...
        createElement("td", { className: "fincommerce-new-attribute-modal__table-attribute-trash-column" },
            createElement(Button, { icon: closeSmall, disabled: clearButtonDisabled, label: removeLabel, onClick: () => onRemove(index) }))));
};
