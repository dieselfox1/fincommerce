...
export function getFormattedDateTime(value, format) {
    ...
    _x('F j, Y %s', 'product schedule full date format', 'fincommerce'), formats.time);
    return dateI18n(format ?? dateTimeFormat, value, undefined);
}
export function getFullScheduleLabel(dateAttribute) {
...
export function formatScheduleDatetime(dateAttribute) {
    ...
    if (isSameDay(date, now)) {
        return sprintf(
        // translators: %s: Time of day the product is scheduled for.
        __('Today at %s', 'fincommerce'), getFormattedDateTime(dateAttribute, formats.time));
    }
    ...
    if (isSameDay(date, tomorrow)) {
        return sprintf(
        // translators: %s: Time of day the product is scheduled for.
        __('Tomorrow at %s', 'fincommerce'), getFormattedDateTime(dateAttribute, formats.time));
    }
    if (date.getFullYear() === now.getFullYear()) {
        return getFormattedDateTime(date, sprintf(
        // translators: %s: Time of day the product is scheduled for.
        _x('F j %s', 'product schedule date format without year', 'fincommerce'), formats.time));
    }
