...
export function useProductAttributes({ allAttributes = [], isVariationAttributes = false, onChange, productId, }) {
    const [attributes, setAttributes] = useState(getFilteredAttributes(allAttributes, isVariationAttributes));
    const fetchTerms = useCallback((attributeId) => {
    ...
    const fetchAttributes = useCallback(() => {
        const [localAttributes, globalAttributes,] = sift(getFilteredAttributes(allAttributes, isVariationAttributes), (attr) => attr.id === 0);
