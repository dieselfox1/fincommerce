...
async function getDefaultVariationValues(productId) {
    try {
        ...
        const { attributes } = await resolveSelect('core').getEntityRecord('postType', 'product', productId);
        const alreadyHasVariableAttribute = attributes.some((attr) => attr.variation);
        if (!alreadyHasVariableAttribute) {
            return {};
        }
        const products = await resolveSelect(experimentalProductVariationsStore).getProductVariations({
...
export function useProductVariationsHelper() {
    ...
    const { editedRecord: product } = useEntityRecord('postType', 'product', productId);
    const [_isGenerating, setIsGenerating] = useState(false);
    const { isGeneratingVariations, generateError } = useSelect((select) => {
        const { isGeneratingVariations: getIsGeneratingVariations, generateProductVariationsError, } = select(experimentalProductVariationsStore);
        return {
    ...
    const isGenerating = useMemo(() => _isGenerating || Boolean(isGeneratingVariations), [_isGenerating, isGeneratingVariations]);
    const generateProductVariations = useCallback(async function onGenerate(attributes, defaultAttributes) {
        setIsGenerating(true);
        // @ts-expect-error TODO react-18-upgrade: core.getEntityRecord type is not typed yet
        const { status: lastStatus, variations } = await resolveSelect('core').getEditedEntityRecord('postType', 'product', productId);
        ...
        await dispatch(experimentalProductVariationsStore).invalidateResolutionForStore();
        /**
         ...
         */
        const meta_data = applyFilters('fincommerce.product.variations.generate.meta_data', [], product);
        return dispatch(experimentalProductVariationsStore)
            ...
            .then(async (response) => {
            // @ts-expect-error invalidateResolution is not typed correctly because we are overriding the type definition. https://github.com/dieselfox1/fincommerce/blob/eeaf58e20064d837412d6c455e69cc5a5e2678b4/packages/js/product-editor/typings/index.d.ts#L15-L35
            await dispatch(coreStore).invalidateResolution('getEntityRecord', ['postType', 'product', productId]);
            await resolveSelect(coreStore).getEntityRecord('postType', 'product', productId);
            await dispatch(experimentalProductVariationsStore).invalidateResolutionForStore();
            return response;
        })
            .finally(() => {
