...
exports.useVariations = useVariations;
/**
...
function useVariations({ productId }) {
    // Variation pagination
    const [variations, setVariations] = (0, element_1.useState)([]);
    const [totalCount, setTotalCount] = (0, element_1.useState)(0);
    const [isLoading, setIsLoading] = (0, element_1.useState)(false);
    const [getVariationsError, setGetVariationsError] = (0, element_1.useState)();
    const [filters, setFilters] = (0, element_1.useState)([]);
    const perPageRef = (0, element_1.useRef)(constants_1.DEFAULT_VARIATION_PER_PAGE_OPTION);
    async function getCurrentVariationsPage(params, invalidateResolutionBeforeRequest = false) {
        ...
        try {
            const { invalidateResolution } = (0, data_2.dispatch)(data_1.experimentalProductVariationsStore);
            ...
            const { getProductVariations, getProductVariationsTotalCount } = (0, data_2.resolveSelect)(data_1.experimentalProductVariationsStore);
            setIsLoading(true);
    ...
    // Variation selection
    const [selectedCount, setSelectedCount] = (0, element_1.useState)(0);
    const [isSelectingAll, setIsSelectingAll] = (0, element_1.useState)(false);
    const selectedVariationsRef = (0, element_1.useRef)({});
    const selected = (0, element_1.useMemo)(function getSelected() {
        return selectedCount > 0
            ? Object.values(selectedVariationsRef.current)
            : [];
    }, [selectedCount]);
    const isSelected = (0, element_1.useCallback)(function isSelected(variation) {
        return (selectedCount > 0 &&
            variation.id in selectedVariationsRef.current);
    }, [selectedCount]);
    const areAllSelected = (0, element_1.useMemo)(() => selectedCount > 0 && variations.every(isSelected), [variations, selectedCount, isSelected]);
    const areSomeSelected = (0, element_1.useMemo)(() => selectedCount > 0 && variations.some(isSelected), [variations, selectedCount, isSelected]);
    ...
    async function onSelectAll() {
        ...
        const { getProductVariations } = (0, data_2.resolveSelect)(data_1.experimentalProductVariationsStore);
        let currentPage = 1;
        let fetchedCount = 0;
    ...
    // Updating
    const [isUpdating, setIsUpdating] = (0, element_1.useState)({});
    async function onUpdate({ id: variationId, ...variation }) {
        ...
        const { updateProductVariation } = (0, data_2.dispatch)(data_1.experimentalProductVariationsStore);
        return updateProductVariation({ product_id: productId, id: variationId }, variation).then(async (response) => {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            await (0, data_2.dispatch)('core').invalidateResolution('getEntityRecord', [
                'postType',
                'product_variation',
                variationId,
            ]);
    ...
    async function onDelete(variationId) {
        if (isUpdating[variationId])
            return;
        const { deleteProductVariation, invalidateResolutionForStore } = (0, data_2.dispatch)(data_1.experimentalProductVariationsStore);
    ...
    async function onBatchUpdate(values) {
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        const { invalidateResolution: coreInvalidateResolution } = (0, data_2.dispatch)('core');
        const { batchUpdateProductVariations, invalidateResolutionForStore } = (0, data_2.dispatch)(data_1.experimentalProductVariationsStore);
        selectedVariationsRef.current = {};
        setSelectedCount(0);
        let currentPage = 1;
        const offset = 50;
    ...
    // Generation
    const { isGenerating, generateProductVariations: onGenerate, generateError, } = (0, use_product_variations_helper_1.useProductVariationsHelper)();
    const wasGenerating = (0, element_1.useRef)(false);
