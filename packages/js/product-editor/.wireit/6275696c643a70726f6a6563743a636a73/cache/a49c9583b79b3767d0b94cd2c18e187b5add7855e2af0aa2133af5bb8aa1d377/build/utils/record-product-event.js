...
exports.recordProductEvent = recordProductEvent;
const tracks_1 = require("@fincommerce/tracks");
const trackableProductValueKeyMap = {
...
function recordProductEvent(eventName, product) {
    ...
    for (const productValueKey of Object.keys(product)) {
        if (potentialTrackableProductValueKeys.includes(productValueKey)) {
            const eventPropKey = trackableProductValueKeyMap[productValueKey] ||
                productValueKey;
            if (Array.isArray(product[productValueKey]) ||
    ...
    (0, tracks_1.recordEvent)(eventName, eventProps);
}
