...
import { BaseControl, ComboboxControl as CoreComboboxControl, Spinner, } from '@wordpress/components';
 ...
 */
const ComboboxControl = CoreComboboxControl;
/**
...
function ComboboxControlOption(props) {
    const { item } = props;
    if (item.disabled) {
        return createElement("div", { className: "item-wrapper is-disabled" }, item.label);
    }
    return createElement("div", { className: "item-wrapper" }, item.label);
}
const AttributesComboboxControl = ({ label, help, current = null, items = [], instanceNumber = 0, isLoading = false, onAddNew, onChange, }) => {
    const [createNewAttributeOption, updateCreateNewAttributeOption] = useState(createNewAttributeOptionDefault);
    /**
        ...
        return [
            ...attributeOptions,
            {
                label: createNewAttributeOption.state === 'draft'
                    ...
                    __('Create "%s"', 'fincommerce'), createNewAttributeOption.label)
    ...
    if (!help) {
        help = (createElement("div", { className: "fincommerce-attributes-combobox-help" }, __('Select an attribute or type to create.', 'fincommerce')));
        if (isLoading) {
            help = (createElement("div", { className: "fincommerce-attributes-combobox-help" },
                createElement(Spinner, null),
                __('Loading…', 'fincommerce')));
        }
        else if (!items.length) {
            help = (createElement("div", { className: "fincommerce-attributes-combobox-help" }, __('No attributes yet. Type to create.', 'fincommerce')));
        }
    }
    return (createElement("div", { className: clsx('fincommerce-attributes-combobox-container', {
            'no-items': !options.length,
        }), ref: comboRef },
        createElement(BaseControl, { label: label, help: help, id: labelFor },
            createElement(ComboboxControl, { className: "fincommerce-attributes-combobox", allowReset: false, options: options, value: currentValue, onChange: (newValue) => {
                    if (!newValue) {
                        return;
                    }
