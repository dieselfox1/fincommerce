...
import { productsStore, productReadOnlyProperties, experimentalProductVariationsStore, } from '@fincommerce/data';
...
import { NUMBERS_AND_DECIMAL_SEPARATOR, ONLY_ONE_DECIMAL_SEPARATOR, TRACKS_SOURCE, } from '../constants';
...
export function useProductHelper() {
    const { createProduct, updateProduct, deleteProduct } = useDispatch(productsStore);
    const { batchUpdateProductVariations, invalidateResolutionForStoreSelector, } = useDispatch(experimentalProductVariationsStore);
     ...
     */
    const createProductWithStatus = useCallback(async (product, status, skipNotice = false) => {
        setUpdating({
            ...updating,
            [status]: true,
        });
        return createProduct({
            ...
            if (!skipNotice) {
                const noticeContent = newProduct.status === 'publish'
                    ? __('Product published.', 'fincommerce')
                    : __('Product successfully created.', 'fincommerce');
            ...
            if (!skipNotice) {
                createNotice('error', status === 'publish'
                    ? __('Failed to publish product.', 'fincommerce')
                    : __('Failed to create product.', 'fincommerce'));
            }
            setUpdating({
                ...updating,
                [status]: false,
     ...
     */
    const updateProductWithStatus = useCallback(async (productId, product, status, skipNotice = false) => {
        setUpdating({
            ...updating,
            [status]: true,
        });
        return updateProduct(productId, {
            ...
            if (!skipNotice) {
                const noticeContent = product.status === 'draft' &&
                    ...
                    ? __('Product published.', 'fincommerce')
                    : __('Product successfully updated.', 'fincommerce');
                createNotice('success', `🎉‎ ${noticeContent}`, {
            ...
            if (!skipNotice) {
                createNotice('error', __('Failed to update product.', 'fincommerce'));
            }
            setUpdating({
                ...updating,
                [status]: false,
     ...
     */
    const copyProductWithStatus = useCallback(async (product, status = 'draft') => {
     ...
     */
    const deleteProductAndRedirect = useCallback(async (id) => {
        setIsDeleting(true);
        return deleteProduct(id).then((product) => {
            const noticeContent = __('Successfully moved product to Trash.', 'fincommerce');
            createNotice('success', `🎉‎ ${noticeContent}`);
            setIsDeleting(false);
            return product;
        }, (error) => {
            createNotice('error', __('Failed to move product to Trash.', 'fincommerce'));
            setIsDeleting(false);
            return error;
        });
    }, []);
    ...
    const sanitizePrice = useCallback((price) => {
        ...
        const regex = new RegExp(NUMBERS_AND_DECIMAL_SEPARATOR.replace('%s', decimalSeparator), 'g');
        const decimalRegex = new RegExp(
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        ONLY_ONE_DECIMAL_SEPARATOR.replaceAll('%s', decimalSeparator), 'g');
