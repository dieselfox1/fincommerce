...
export function Edit({ attributes, context: { postType }, }) {
    ...
    const { error, validate, ref: inputValidatorRef, } = useValidation(property, async function validator() {
        if (!inputRef.current)
            return;
        ...
        if (input.validity.valueMissing) {
            customErrorMessage =
                typeof required === 'string'
                    ? required
                    : __('This field is required.', 'fincommerce');
        }
        ...
        if (input.validity.tooShort) {
            ...
            customErrorMessage = sprintf(minLength?.message ??
                /* translators: %d: minimum length */
                __('The minimum length of the field is %d', 'fincommerce'), minLength?.value);
        }
        if (input.validity.tooLong) {
            // eslint-disable-next-line @wordpress/valid-sprintf
            customErrorMessage = sprintf(maxLength?.message ??
                /* translators: %d: maximum length */
                __('The maximum length of the field is %d', 'fincommerce'), maxLength?.value);
        }
        if (input.validity.rangeUnderflow) {
            // eslint-disable-next-line @wordpress/valid-sprintf
            customErrorMessage = sprintf(min?.message ??
                /* translators: %d: minimum length */
                __('The minimum value of the field is %d', 'fincommerce'), min?.value);
        }
        if (input.validity.rangeOverflow) {
            // eslint-disable-next-line @wordpress/valid-sprintf
            customErrorMessage = sprintf(max?.message ??
                /* translators: %d: maximum length */
                __('The maximum value of the field is %d', 'fincommerce'), max?.value);
        }
    ...
    function getSuffix() {
        ...
        if (suffix === true && isValidUrl) {
            return (createElement(Link, { type: "external", href: value, target: "_blank", rel: "noreferrer", className: "wp-block-fincommerce-product-text-field__suffix-link" },
                createElement(Icon, { icon: external, size: 20 })));
        }
        return typeof suffix === 'string' ? suffix : undefined;
    }
    return (createElement("div", { ...blockProps },
        createElement(TextControl, { ref: useMergeRefs([inputRef, inputValidatorRef]), type: type?.value ?? 'text', value: value, disabled: disabled, label: label, onChange: setValue, onBlur: () => {
                if (hasEdit(property)) {
                    validate();
                }
