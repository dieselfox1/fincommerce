...
export const ProductMVPFeedbackModalContainer = ({ productId: _productId, }) => {
    ...
    const { _feature_nonce } = getSetting('admin', {});
    const classicEditorUrl = productId
        ? getAdminLink(`post.php?post=${productId}&action=edit&product_block_editor=0&_feature_nonce=${_feature_nonce}`)
        : getAdminLink(`post-new.php?post_type=product&product_block_editor=0&_feature_nonce=${_feature_nonce}`);
    ...
    return (createElement(ProductMVPFeedbackModal, { recordScoreCallback: recordScore, onCloseModal: onCloseModal, onSkipFeedback: onSkipFeedback }));
};
