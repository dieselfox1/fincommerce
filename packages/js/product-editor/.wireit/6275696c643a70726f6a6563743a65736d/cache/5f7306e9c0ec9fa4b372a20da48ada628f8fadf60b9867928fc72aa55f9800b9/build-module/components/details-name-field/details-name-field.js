...
export const DetailsNameField = ({}) => {
    ...
    const { getInputProps, values, touched, errors, setValue, resetForm } = useFormContext();
    const { prefix: permalinkPrefix, suffix: permalinkSuffix } = getPermalinkParts(values);
    const hasNameError = () => {
    ...
    return (createElement("div", null,
        createElement(TextControl, { label: createInterpolateElement(__('Name <required />', 'fincommerce'), {
                required: (createElement("span", { className: "fincommerce-product-form__optional-input" }, __('(required)', 'fincommerce'))),
            }), name: `${PRODUCT_DETAILS_SLUG}-name`, placeholder: __('e.g. 12 oz Coffee Mug', 'fincommerce'), ...getInputProps('name', {
                onBlur: setSkuIfEmpty,
            }) }),
        values.id && !hasNameError() && permalinkPrefix && (createElement("span", { className: "fincommerce-product-form__secondary-text product-details-section__product-link" },
            __('Product link', 'fincommerce'),
            ":\u00A0",
            createElement("a", { href: values.permalink, target: "_blank", rel: "noreferrer" },
                permalinkPrefix,
        ...
        showProductLinkEditModal && (createElement(EditProductLinkModal, { permalinkPrefix: permalinkPrefix || '', permalinkSuffix: permalinkSuffix || '', product: values, onCancel: () => setShowProductLinkEditModal(false), onSaved: () => setShowProductLinkEditModal(false), saveHandler: async (slug) => {
