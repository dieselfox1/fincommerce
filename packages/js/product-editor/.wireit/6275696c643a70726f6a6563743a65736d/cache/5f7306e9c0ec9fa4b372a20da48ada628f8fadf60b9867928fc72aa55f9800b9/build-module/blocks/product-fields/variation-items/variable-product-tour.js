...
import { experimentalProductVariationsStore, optionsStore, useUserPreferences, } from '@fincommerce/data';
...
export const VariableProductTour = () => {
    ...
    const { totalCount } = useSelect((select) => {
    ...
    const { updateUserPreferences, variable_product_block_tour_shown: hasShownTour, } = useUserPreferences();
    const config = {
        placement: 'top',
        steps: [
            {
                referenceElements: {
                    desktop: '.wp-block-fincommerce-product-variation-items-field',
                },
                focusElement: {
                    desktop: '.wp-block-fincommerce-product-variation-items-field',
                },
                meta: {
                    ...
                    heading: __('⚡️ This product now has variations', 'fincommerce'),
                    descriptions: {
                        desktop: __('From now on, you’ll manage pricing, shipping, and inventory for each variation individually—just like any other product in your store.', 'fincommerce'),
                    },
                    primaryButton: {
    ...
    const { hasShownProductEditorTour } = useSelect((select) => {
        ...
        return {
            hasShownProductEditorTour: getOption('fincommerce_block_product_tour_shown') === 'yes',
        };
    }, []);
    if (hasShownTour === 'yes' ||
        !isTourOpen ||
    ...
    return createElement(TourKit, { config: config });
};
