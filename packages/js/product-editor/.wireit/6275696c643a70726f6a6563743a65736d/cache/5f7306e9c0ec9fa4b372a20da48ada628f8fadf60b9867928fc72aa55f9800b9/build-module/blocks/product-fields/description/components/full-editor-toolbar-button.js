...
function shouldForceFullEditor() {
    return (localStorage
        .getItem('__unsupported_force_product_editor_description_full_editor')
        ?.trim()
        .toLowerCase() === 'true');
}
export default function FullEditorToolbarButton({ label = __('Edit Product description', 'fincommerce'), text = __('Full editor', 'fincommerce'), }) {
    const { openModalEditor, setModalEditorBlocks } = dispatch(wooProductEditorUiStore);
    const [description] = useEntityProp('postType', 'product', 'description');
    return (createElement(ToolbarButton, { label: label, onClick: () => {
            if (isGutenbergVersionWithCrashInFullEditor()) {
                if (shouldForceFullEditor()) {
                    // eslint-disable-next-line no-alert
                    alert(__('The version of the Gutenberg plugin installed causes a crash in the full editor. You are proceeding at your own risk and may experience crashes.', 'fincommerce'));
                }
                else {
                    // eslint-disable-next-line no-alert
                    alert(__('The version of the Gutenberg plugin installed causes a crash in the full editor. To prevent this, the full editor has been disabled.', 'fincommerce'));
                    return;
                }
            }
