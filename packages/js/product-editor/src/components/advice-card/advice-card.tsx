...
export function AdviceCard( {
	...
	return (
		<div
			role="group"
			{ ...props }
			className={ clsx( className, 'fincommerce-advice-card', {
				'is-dismissible': isDismissible,
			} ) }
		>
			{ isDismissible && (
				<div className="fincommerce-advice-card__header">
					<Button
						className="fincommerce-advice-card__dismiss-button"
						onClick={ handleDismissButtonClick }
						icon={ close }
						label={ __( 'Dismiss', 'fincommerce' ) }
						isSmall={ true }
					/>
				</div>
			) }
			<div className="fincommerce-advice-card__body">{ children }</div>
			{ tip && tip.length > 0 && (
				<div className="fincommerce-advice-card__footer">{ tip }</div>
			) }
		</div>
	);
}
