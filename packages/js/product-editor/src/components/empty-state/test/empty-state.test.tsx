...
describe( 'EmptyState', () => {
	it( 'should render empty name rows when names are empty', () => {
		...
		const rows = container.querySelectorAll(
			'.fincommerce-product-empty-state__row'
		);
		expect( rows.length ).toBe( 3 );
		rows.forEach( ( row ) => {
			expect(
				row.querySelector( '.fincommerce-product-empty-state__name' )
			).toBeInTheDocument();
			expect(
				row.querySelector( '.fincommerce-product-empty-state__value' )
			).toBeInTheDocument();
			expect(
				row.querySelector( '.fincommerce-product-empty-state__actions' )
			).toBeInTheDocument();
		} );
	} );
