...
function ComboboxControlOption( props: {
	...
	const { item } = props;
	return (
		<div className="fincommerce-product-select__menu-item">
			{ item.product && (
				<ProductImage
					product={ item.product }
					className="fincommerce-product-select__menu-item-image"
				/>
			) }
			<div className="fincommerce-product-select__menu-item-content">
				<div className="fincommerce-product-select__menu-item-title">
					{ item.label }
				</div>
				...
				{ item.product && (
					<FormattedPrice
						product={ item.product }
						className="fincommerce-product-select__menu-item-description"
					/>
				) }
			</div>
		</div>
...
export function ProductSelect( {
	...
	if ( ! help ) {
		help = (
			<div className="fincommerce-product-combobox-help">
				{ __( 'Search for products', 'fincommerce' ) }
			</div>
		);
	...
	return (
		...
		>
			<BaseControl label={ label } help={ help } id={ labelFor }>
				<ComboboxControl
					className="fincommerce-product-combobox"
					allowReset={ false }
					options={ options }
					value={ value }
					ref={ comboRef }
