...
export function VariationsTableRow( {
		...
		return (
			<ImageActionsMenu
				...
				renderToggle={ ( { isOpen, onToggle, isBusy } ) =>
					isBusy ? (
						<div className="fincommerce-product-variations__add-image-button">
							<Spinner
								aria-label={ __(
									'Loading image',
									'fincommerce'
						...
						<Button
							className={ clsx(
								variation.image
									...
									: 'fincommerce-product-variations__add-image-button'
							) }
						...
						>
							{ variation.image && (
								<div
									className="fincommerce-product-variations__image"
									style={ {
	...
	function renderPrices() {
		return (
			<>
				{ variation.on_sale && (
					<span className="fincommerce-product-variations__sale-price">
						{ formatAmount( variation.sale_price ) }
					</span>
				) }
				<span
					className={ clsx(
						'fincommerce-product-variations__regular-price',
						{
							'fincommerce-product-variations__regular-price--on-sale':
								variation.on_sale,
						}
					) }
				>
	...
	function renderPriceCellContent() {
		...
		return (
			<Dropdown
				contentClassName="fincommerce-product-variations__pricing-actions-menu"
				popoverProps={ {
					placement: 'bottom',
				} }
	...
	function renderStockStatus() {
		return (
			<>
				<span
					className={ clsx(
						'fincommerce-product-variations__status-dot',
	...
	function renderStockCellContent() {
		...
		return (
			<Dropdown
				contentClassName="fincommerce-product-variations__stock-status-actions-menu"
				popoverProps={ {
					placement: 'bottom',
				} }
				renderToggle={ ( { isOpen, onToggle } ) => (
	...
	return (
		<>
			<div
				className="fincommerce-product-variations__selection"
				role="cell"
			>
				{ matchesAny && (
					<Tooltip
						...
						helperText={ __( 'View helper text', 'fincommerce' ) }
			...
			<div
				className="fincommerce-product-variations__attributes-cell"
				role="cell"
			>
				...
				<div className="fincommerce-product-variations__attributes">
					{ tags.map( ( tagInfo ) => {
						const tag = (
							<Tag
								id={ tagInfo.id }
								className="fincommerce-product-variations__attribute"
								key={ tagInfo.id }
								label={ truncate(
									tagInfo.label,
									PRODUCT_VARIATION_TITLE_LIMIT
			...
			<div
				className={ clsx( 'fincommerce-product-variations__price', {
					'fincommerce-product-variations__price--fade':
			...
			<div
				className={ clsx( 'fincommerce-product-variations__quantity', {
					'fincommerce-product-variations__quantity--fade':
						variation.status === 'private',
				} ) }
				role="cell"
			>
			...
			<div
				className="fincommerce-product-variations__actions"
				role="cell"
			>
				{ ( variation.status === 'private' ||
					...
					>
						<div className="fincommerce-attribute-list-item__actions-icon-wrapper">
							<HiddenIcon className="fincommerce-attribute-list-item__actions-icon-wrapper-icon" />
						</div>
					</Tooltip>
				) }
