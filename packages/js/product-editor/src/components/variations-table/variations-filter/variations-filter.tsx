...
export function VariationsFilter( {
	...
	return (
		<Dropdown
			className="fincommerce-product-variations-filter"
			onClose={ handleClose }
			renderToggle={ ( { isOpen, onToggle } ) => (
				<Button
					...
					className="fincommerce-product-variations-filter__toggle"
				>
					<span>
						{ sprintf(
			...
			renderContent={ ( { onClose } ) => (
				<form
					className="fincommerce-product-variations-filter__form"
					noValidate
				...
				>
					{ attribute.options.length >
						...
						<div className="fincommerce-product-variations-filter__form-header">
							<label
					...
					<div
						className="fincommerce-product-variations-filter__form-body"
						onScroll={ handleScroll }
					>
						{ options.length > 0 ? (
							<ul className="fincommerce-product-variations-filter__form-list">
								{ options.map( ( option ) => (
									<li
										key={ option.slug }
										className="fincommerce-product-variations-filter__form-list-item"
									>
										<label
											htmlFor={ `${ optionCheckboxId }-${ option.slug }` }
											className="fincommerce-product-variations-filter__form-list-item-label"
										>
											<CheckboxControl
							...
							! isLoading && (
								<div className="fincommerce-product-variations-filter__form-list-empty">
									{ __(
						...
						{ isLoading && (
							<div className="fincommerce-product-variations-filter__loading">
								<Spinner />
							</div>
						) }
					</div>
					<div className="fincommerce-product-variations-filter__form-footer">
						<Button
							type="reset"
							variant="secondary"
							aria-disabled={ isDisabled }
						>
							{ __( 'Reset', 'fincommerce' ) }
						</Button>
