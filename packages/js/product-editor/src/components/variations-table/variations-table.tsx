...
function getSnackbarText(
	...
	if ( deletedCount > 0 ) {
		return sprintf(
			...
			__( '%s variations deleted.', 'fincommerce' ),
			deletedCount
		);
	} else if ( updatedCount > 0 ) {
		return sprintf(
			/* translators: The updated variations count */
			__( '%s variations updated.', 'fincommerce' ),
			updatedCount
		);
	}
...
>( function Table(
	...
	function handleDeleteVariationClick( variation: PartialProductVariation ) {
		onDelete( variation.id )
			...
			.catch( () => {
				createErrorNotice(
					__( 'Failed to delete variation.', 'fincommerce' )
				);
			} );
	}
	...
	function handleVariationChange(
		...
		onUpdate( variation )
			...
			.catch( () => {
				createErrorNotice(
					__( 'Failed to save variation.', 'fincommerce' )
				);
			} );
	}
	...
	function handleUpdateAll( values: PartialProductVariation[] ) {
		...
		onBatchUpdate( values )
			...
			.catch( () => {
				createErrorNotice(
					__( 'Failed to update variations.', 'fincommerce' )
				);
			} );
	}
	...
	function handleDeleteAll( values: PartialProductVariation[] ) {
		...
		onBatchDelete( values )
			...
			.catch( () => {
				createErrorNotice(
					__( 'Failed to delete variations.', 'fincommerce' )
				);
			} );
	}
	...
	function renderTableBody() {
		return totalCount > 0 ? (
			<div
				className="fincommerce-product-variations__table-body"
				role="rowgroup"
			>
				{ variations.map( ( variation ) => (
					<div
						key={ `${ variation.id }` }
						className="fincommerce-product-variations__table-row"
						role="row"
					>
						<VariationsTableRow
							variation={ variation }
			...
			<EmptyOrErrorTableState
				isError={ false }
				message={ __( 'No variations were found', 'fincommerce' ) }
				actionText={ __( 'Clear filters', 'fincommerce' ) }
	...
	return (
		<div className="fincommerce-product-variations" ref={ ref }>
			{ noticeText && (
				<Notice
					status={ noticeStatus }
					className="fincommerce-product-variations__notice"
			...
			<div className="fincommerce-product-variations__table" role="table">
				{ ( hasFilters() || totalCount > 0 ) && (
					<div
						className="fincommerce-product-variations__table-header"
						role="rowgroup"
					>
						<div
							className="fincommerce-product-variations__table-row"
							role="rowheader"
						>
							<div className="fincommerce-product-variations__filters">
								{ areSomeSelected ? (
									<>
										<span>
											{ sprintf(
						...
						{ totalCount > 0 && (
							<div
								className="fincommerce-product-variations__table-row fincommerce-product-variations__table-rowheader"
								role="rowheader"
							>
								<div
									className="fincommerce-product-variations__table-column fincommerce-product-variations__selection"
									role="columnheader"
								>
									<CheckboxControl
										value="all"
								...
								<div
									className="fincommerce-product-variations__table-column"
									role="columnheader"
								>
									{ __( 'Variation', 'fincommerce' ) }
								</div>
								<div
									className="fincommerce-product-variations__table-column fincommerce-product-variations__price"
									role="columnheader"
								>
									{ __( 'Price', 'fincommerce' ) }
								</div>
								<div
									className="fincommerce-product-variations__table-column"
									role="columnheader"
								>
									{ __( 'Stock', 'fincommerce' ) }
								</div>
							</div>
						) }
					</div>
					...
					<div
						...
						aria-label={
							isGenerating
								...
								: __( 'Loading variations…', 'fincommerce' )
						}
					>
				...
				{ totalCount > 5 && (
					<div
						className="fincommerce-product-variations__table-footer"
						role="row"
					>
						<Pagination
