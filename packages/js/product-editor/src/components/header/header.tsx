...
export function Header( {
	...
	function getVisibilityTags() {
		...
		if ( productStatus === 'future' ) {
			tags.push(
				<Tag
					key={ 'scheduled-tag' }
					label={ __( 'Scheduled', 'fincommerce' ) }
				/>
			);
		}
		if (
	...
	return (
		<div
			className="fincommerce-product-header"
			role="region"
			aria-label={ __( 'Product Editor top bar.', 'fincommerce' ) }
			tabIndex={ -1 }
		>
			<div className="fincommerce-product-header__inner">
				{ isVariation ? (
					<div className="fincommerce-product-header__back">
						<Tooltip
							// @ts-expect-error className is missing in TS, should remove this when it is included.
							className="fincommerce-product-header__back-tooltip"
							text={ RETURN_TO_MAIN_PRODUCT }
						>
							<div className="fincommerce-product-header__back-tooltip-wrapper">
								<Button
									icon={ chevronLeft }
									isTertiary={ true }
									onClick={ () => {
				...
				<div
					className={ clsx( 'fincommerce-product-header-title-bar', {
						'is-variation': isVariation,
					} ) }
				>
					<div className="fincommerce-product-header-title-bar__image">
						{ isHeaderImageVisible ? (
							<img
								...
								className="fincommerce-product-header-title-bar__product-image"
							/>
						) : (
							<Icon icon={ isVariation ? group : box } />
						) }
					</div>
					<h1 className="fincommerce-product-header__title">
						{ isVariation ? (
							<>
								{ lastPersistedProduct?.name }
								<span className="fincommerce-product-header__variable-product-id">
						...
						<div className="fincommerce-product-header__visibility-tags">
							{ getVisibilityTags() }
						</div>
					</h1>
				</div>
