...
function Edit({ attributes, clientId, context, }) {
    ...
    const regularPriceId = (0, compose_1.useInstanceId)(components_1.BaseControl, 'wp-block-fincommerce-product-regular-price-field');
    const { ref: regularPriceRef, error: regularPriceValidationError, validate: validateRegularPrice, } = (0, validation_context_1.useValidation)(`regular_price-${clientId}`, async function regularPriceValidator() {
        ...
        if (listPrice) {
            if (listPrice < 0) {
                return {
                    message: (0, i18n_1.__)('Regular price must be greater than or equals to zero.', 'fincommerce'),
                };
            }
            if (salePrice &&
                listPrice <= Number.parseFloat(salePrice)) {
                return {
                    message: (0, i18n_1.__)('Regular price must be greater than the sale price.', 'fincommerce'),
                };
            }
        }
        else if (isRequired) {
            return {
                ...
                (0, i18n_1.__)('%s is required.', 'fincommerce'), label),
            };
        }
