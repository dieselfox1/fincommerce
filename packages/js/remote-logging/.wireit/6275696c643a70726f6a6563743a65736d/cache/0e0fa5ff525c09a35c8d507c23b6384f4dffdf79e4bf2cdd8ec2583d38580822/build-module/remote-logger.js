...
export const REMOTE_LOGGING_SHOULD_SEND_ERROR_FILTER = 'fincommerce_remote_logging_should_send_error';
export const REMOTE_LOGGING_ERROR_DATA_FILTER = 'fincommerce_remote_logging_error_data';
export const REMOTE_LOGGING_LOG_ENDPOINT_FILTER = 'fincommerce_remote_logging_log_endpoint';
export const REMOTE_LOGGING_JS_ERROR_ENDPOINT_FILTER = 'fincommerce_remote_logging_js_error_endpoint';
export const REMOTE_LOGGING_REQUEST_URI_PARAMS_WHITELIST_FILTER = 'fincommerce_remote_logging_request_uri_whitelist';
...
export const sanitiseRequestUriParams = (search) => {
     ...
     */
    const whitelist = applyFilters(REMOTE_LOGGING_REQUEST_URI_PARAMS_WHITELIST_FILTER, REMPOTE_LOGGING_REQUEST_URI_PARAMS_DEFAULT_WHITELIST);
...
export class RemoteLogger {
    ...
    async log(severity, message, extraData) {
        ...
        const logData = mergeLogData(DEFAULT_LOG_DATA, {
            message,
            severity,
            ...extraData,
        });
    ...
    async error(error, extraData) {
        ...
        const errorData = {
            ...
            trace: this.getFormattedStackFrame(TraceKit.computeStackTrace(error)),
        };
    ...
    async sendLog(logData) {
        ...
        try {
             ...
             */
            const endpoint = applyFilters(REMOTE_LOGGING_LOG_ENDPOINT_FILTER, 'https://public-api.wordpress.com/rest/v1.1/logstash');
    ...
    async handleError(error) {
         ...
         */
        const filteredErrorData = applyFilters(REMOTE_LOGGING_ERROR_DATA_FILTER, errorData);
        try {
    ...
    async sendError(error) {
        ...
        try {
             ...
             */
            const endpoint = applyFilters(REMOTE_LOGGING_JS_ERROR_ENDPOINT_FILTER, 'https://public-api.wordpress.com/rest/v1.1/js-error');
    ...
    shouldHandleError(error, stackFrames) {
        const containsfincommerceFrame = stackFrames.some((frame) => frame.url && frame.url.startsWith(getSetting('wcAssetUrl')));
        /**
         ...
         *
         */
        return applyFilters(REMOTE_LOGGING_SHOULD_SEND_ERROR_FILTER, containsfincommerceFrame, error, stackFrames);
    }
    isRateLimited() {
 ...
 *
 */
export async function log(severity, message, extraData) {
    if (!canLog(logger)) {
        return false;
    }
    try {
 ...
 * @param extraData - Optional additional data to include in the log.
 */
export async function captureException(error, extraData) {
    if (!canLog(logger)) {
        return false;
    }
    try {
