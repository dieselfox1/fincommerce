...
jest.mock( 'tracekit', () => ( {
	computeStackTrace: jest.fn().mockReturnValue( {
		...
		stack: [
			{
				url: 'http://example.com/fincommerce/assets/js/admin/app.min.js',
				func: 'testFunction',
				args: [],
				line: 1,
				column: 1,
...
jest.mock( '@fincommerce/settings', () => {
	return {
		getSetting: jest.fn().mockImplementation( ( key ) => {
			if ( key === 'wcAssetUrl' ) {
				return 'http://example.com/fincommerce/assets';
			}
			return null;
		} ),
	};
...
describe( 'RemoteLogger', () => {
	...
	describe( 'error', () => {
		it( 'should send an error to the API with default data', async () => {
			...
			expect( payload[ 'trace' ] ).toContain(
				'#1 at testFunction (http://example.com/fincommerce/assets/js/admin/app.min.js:1:1)'
			);
		} );
	...
	describe( 'shouldHandleError', () => {
		it( 'should return true for FinCommerce errors', () => {
			const error = new Error( 'Test error' );
			const stackFrames = [
				{
					url: 'http://example.com/fincommerce/assets/js/admin/app.min.js',
		...
		it( 'should return false for non-FinCommerce errors', () => {
			const error = new Error( 'Test error' );
			const stackFrames = [
				{
				...
				{
					url: 'http://example.com/other/plugin/fincommerce/assets/js/app.min.js',
					func: 'testFunction',
					args: [],
					line: 1,
					column: 1,
		...
		it( 'should return false for FinCommerce errors with no stack frames', () => {
			const error = new Error( 'Test error' );
	...
	describe( 'getFormattedStackFrame', () => {
		it( 'should format stack frames correctly', () => {
			const stackTrace = {
				...
				stack: [
					{
						url: 'http://example.com/fincommerce/assets/js/admin/wc-admin.min.js',
						func: 'testFunction',
						args: [],
						line: 1,
						column: 1,
			...
			expect( result ).toContain(
				'#1 at testFunction (http://example.com/fincommerce/assets/js/admin/wc-admin.min.js:1:1)'
			);
