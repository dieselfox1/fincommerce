...
function RawSendPreviewEmail() {
	...
	const sendingMethodConfigurationLink = useMemo(
		() =>
			applyFilters(
				...
				`https://www.mailpoet.com/blog/mailpoet-smtp-plugin/?utm_source=fincommerce_email_editor&utm_medium=plugin&utm_source_platform=${ postType }`
			) as string,
		[ postType ]
	);
	...
	return (
		<Modal
			className="fincommerce-send-preview-email"
			title={ __( 'Send a test email', 'fincommerce' ) }
			onRequestClose={ closeCallback }
			focusOnMount={ false }
		>
			{ sendingPreviewStatus === SendingPreviewStatus.ERROR ? (
				<div className="fincommerce-send-preview-modal-notice-error">
					<p>
						{ __(
							...
							{ createInterpolateElement(
								...
								),
								{
									link: (
										// eslint-disable-next-line jsx-a11y/anchor-has-content, jsx-a11y/control-has-associated-label
										<a
											href={ `https://account.mailpoet.com/?s=1&g=1&utm_source=fincommerce_email_editor&utm_medium=plugin&utm_source_platform=${ postType }` }
			...
			<TextControl
				...
				className="fincommerce-send-preview-email__send-to-field"
			...
			{ sendingPreviewStatus === SendingPreviewStatus.SUCCESS ? (
				<p className="fincommerce-send-preview-modal-notice-success">
					<Icon icon={ check } style={ { fill: '#4AB866' } } />
					{ __( 'Test email sent successfully!', 'fincommerce' ) }
				</p>
			) : null }
			<div className="fincommerce-send-preview-modal-footer">
				<Button
					variant="tertiary"
					onClick={ () => {
						recordEvent(
