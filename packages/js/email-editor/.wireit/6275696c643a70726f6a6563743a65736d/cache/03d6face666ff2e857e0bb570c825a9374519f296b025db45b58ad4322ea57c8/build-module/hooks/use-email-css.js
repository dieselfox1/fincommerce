...
export function useEmailCss() {
    ...
    const { editorTheme, layout, deviceType, editorSettingsStyles } = useSelect((select) => {
        ...
        getDeviceType, } = select(editorStore);
    ...
    const mergedConfig = useMemo(() => deepmerge.all([
        {},
        editorTheme || {},
        userTheme || {},
    ...
    if (padding) {
        rootContainerStyles += `padding-left:${unwrapCompressedPresetStyleVariable(padding.left)};`;
        rootContainerStyles += `padding-right:${unwrapCompressedPresetStyleVariable(padding.right)};`;
    }
