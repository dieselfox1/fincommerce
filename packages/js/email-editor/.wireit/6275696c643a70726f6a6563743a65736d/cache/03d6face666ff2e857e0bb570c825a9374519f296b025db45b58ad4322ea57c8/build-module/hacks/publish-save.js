 ...
 */
import { useEffect, createPortal, useCallback, useRef, } from '@wordpress/element';
...
function NextPublishSlot({ children }) {
    ...
    return createPortal(_jsx(_Fragment, { children: children }), sendButtonPortalEl.current);
}
export function PublishSave() {
    const observerRef = useRef(null);
    const { hasNonPostEntityChanges, isEditedPostDirty, isEditingTemplate } = useSelect((select) => ({
        ...
        isEditingTemplate: select(editorStore).getCurrentPostType() ===
            'wp_template',
    }), []);
    ...
    const toggleElementVisible = useCallback((element, visible) => {
    ...
    return (_jsx(NextPublishSlot, { children: !displayOriginalPublishButton && _jsx(SendButton, {}) }));
}
