...
function getModalTitle(items, shouldPermanentlyDelete) {
    if (shouldPermanentlyDelete) {
        return items.length > 1
            ...
            _n('Are you sure you want to permanently delete %d item?', 'Are you sure you want to permanently delete %d items?', items.length, 'fincommerce'), items.length)
            : sprintf(
            // translators: %s: The post's title
            __('Are you sure you want to permanently delete "%s"?', 'fincommerce'), decodeEntities(getItemTitle(items[0])));
    }
    return items.length > 1
        ? sprintf(
        // translators: %d: The number of items (2 or more).
        _n('Are you sure you want to move %d item to the trash ?', 'Are you sure you want to move %d items to the trash ?', items.length, 'fincommerce'), items.length)
        : sprintf(
        // translators: %s: The item's title.
        __('Are you sure you want to move "%s" to the trash?', 'fincommerce'), getItemTitle(items[0]));
}
const getTrashEmailPostAction = () => {
    const shouldPermanentlyDelete = applyFilters('fincommerce_email_editor_trash_modal_should_permanently_delete', false);
    /**
    ...
    const trashEmailPost = {
        ...
        label: shouldPermanentlyDelete
            ? __('Permanently delete', 'fincommerce')
        ...
        RenderModal: ({ items, closeModal, onActionPerformed }) => {
            ...
            return (_jsxs(VStack, { spacing: "5", children: [_jsx(Text, { children: getModalTitle(items, shouldPermanentlyDelete) }), _jsxs(HStack, { justify: "right", children: [_jsx(Button, { variant: "tertiary", onClick: () => {
                                    closeModal?.();
                                    recordEvent('trash_modal_cancel_button_clicked');
                                }, disabled: isBusy, __next40pxDefaultSize: true, children: __('Cancel', 'fincommerce') }), _jsx(Button, { variant: "primary", onClick: async () => {
                                    ...
                                    if (promiseResult.every(({ status }) => status === 'fulfilled')) {
                                        ...
                                        if (promiseResult.length === 1) {
                                            successMessage = shouldPermanentlyDelete
                                                ...
                                                __('"%s" permanently deleted.', 'fincommerce'), getItemTitle(items[0]))
                                                : sprintf(
                                                /* translators: The item's title. */
                                                __('"%s" moved to the trash.', 'fincommerce'), getItemTitle(items[0]));
                                        }
                                        else {
                                            successMessage = shouldPermanentlyDelete
                                                ? __('The items were permanently deleted.', 'fincommerce')
                                                : sprintf(
                                                /* translators: The number of items. */
                                                _n('%s item moved to the trash.', '%s items moved to the trash.', items.length, 'fincommerce'), items.length);
                                        }
                                        ...
                                        if (promiseResult.length === 1) {
                                            ...
                                            if (typedError.reason?.message) {
                                                ...
                                                errorMessage = __('An error occurred while performing the action.', 'fincommerce');
                                            }
                                            ...
                                            if (errorMessages.size === 0) {
                                                ...
                                                __('An error occurred while performing the action: %s', 'fincommerce'), [...errorMessages][0]);
                                            }
                                            else {
                                                errorMessage = sprintf(
                                                /* translators: %s: a list of comma separated error messages */
                                                __('Some errors occurred while performing the action: %s', 'fincommerce'), [...errorMessages].join(','));
                                            }
                                        }
                                    ...
                                    ? __('Delete permanently', 'fincommerce')
                                    : __('Move to trash', 'fincommerce') })] })] }));
        },
    };
