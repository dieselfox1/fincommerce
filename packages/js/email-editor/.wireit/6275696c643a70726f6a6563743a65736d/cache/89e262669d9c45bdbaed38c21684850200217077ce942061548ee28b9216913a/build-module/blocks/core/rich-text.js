 ...
 */
import { registerFormatType, unregisterFormatType, applyFormat, insert, create, toHTMLString, } from '@wordpress/rich-text';
 ...
 */
import { getCursorPosition, replacePersonalizationTagsWithHTMLComments, } from '../../components/personalization-tags/rich-text-utils';
...
function PersonalizationTagsButton({ contentRef }) {
    ...
    const blockContent = blockAttributes?.[blockContentKey]?.originalHTML ||
        ...
        '';
    const handleInsert = useCallback((tag, linkText) => {
        ...
        if (linkText) {
            ...
            richTextValue = applyFormat(richTextValue, {
            ...
            let richTextValue = create({ html: blockContent });
            richTextValue = insert(richTextValue, create({ html: `<!--${tag}-->&nbsp;` }), // Add a non-breaking space to avoid an issue when WP renderer removes blog containing only a comment
            start, end);
            updatedContent = toHTMLString({ value: richTextValue });
        }
    ...
    return (_jsx(BlockControls, { children: _jsxs(ToolbarGroup, { children: [_jsx(ToolbarButton, { icon: "shortcode", title: __('Personalization Tags', 'fincommerce'), onClick: () => {
                        setIsModalOpened(true);
                        recordEvent('block_controls_personalization_tags_button_clicked');
                    } }), _jsx(PersonalizationTagsPopover, { contentRef: contentRef, onUpdate: (originalTag, updatedTag) => {
                    ...
                    } }), _jsx(PersonalizationTagsLinkPopover, { contentRef: contentRef, onUpdate: (htmlElement, newTag, newText) => {
                    ...
                    } }), _jsx(PersonalizationTagsModal, { isOpened: isModalOpened, onInsert: (value, linkText) => {
...
function extendRichTextFormats() {
    registerFormatType('fincommerce-email-editor/shortcode', {
        name: 'fincommerce-email-editor/shortcode',
        title: __('Personalization Tags', 'fincommerce'),
    ...
    registerFormatType('fincommerce-email-editor/link-shortcode', {
        name: 'fincommerce-email-editor/link-shortcode',
        title: __('Personalization Tags Link', 'fincommerce'),
...
const personalizationTagsLiveContentUpdate = createHigherOrderComponent((BlockEdit) => (props) => {
    ...
    const handleSetAttributes = useCallback((newAttributes) => {
        if (newAttributes.content !== undefined) {
            const replacedContent = replacePersonalizationTagsWithHTMLComments(newAttributes.content, list);
            setAttributes({
                ...newAttributes,
    ...
    if (name === 'core/paragraph' ||
        name === 'core/heading' ||
        name === 'core/list-item') {
        return (_jsx(BlockEdit, { ...props, attributes: {
                ...attributes,
...
function activatePersonalizationTagsReplacing() {
    addFilter('editor.BlockEdit', 'fincommerce-email-editor/with-live-content-update', personalizationTagsLiveContentUpdate);
}
