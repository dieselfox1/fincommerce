...
function SelectTemplateBody({ hasEmailPosts, templates, handleTemplateSelection, templateSelectMode, }) {
    const [selectedCategory, setSelectedCategory] = useState(TemplateCategories[1].name // Show the “Basic” category by default
    );
    ...
    return (_jsxs("div", { className: "block-editor-block-patterns-explorer", children: [_jsx(TemplateCategoriesListSidebar, { templateCategories: displayCategories, selectedCategory: selectedCategory, onClickCategory: handleCategorySelection }), _jsx(TemplateList, { templates: templates, onTemplateSelection: handleTemplateSelection, selectedCategory: selectedCategory })] }));
}
const MemorizedSelectTemplateBody = memo(SelectTemplateBody);
export function SelectTemplateModal({ onSelectCallback, closeCallback = null, previewContent = '', postType, }) {
    const templateSelectMode = previewContent ? 'swap' : 'new';
    recordEventOnce('template_select_modal_opened', { templateSelectMode });
    const [templates, emailPosts, hasEmailPosts] = usePreviewTemplates(previewContent);
    ...
    const handleTemplateSelection = (template) => {
        ...
        void dispatch(storeName).setTemplateToPost(templateIsPostContent ? postContent.template : template.slug);
        onSelectCallback();
    };
    const handleCloseWithoutSelection = () => {
        ...
        recordEvent('template_select_modal_handle_close_without_template_selected');
        handleTemplateSelection(template);
    };
    return (_jsxs(Modal, { title: templateSelectMode === 'new'
            ? __('Start with an email preset', 'fincommerce')
            : __('Select a template', 'fincommerce'), onRequestClose: () => {
        ...
        }, isFullScreen: true, children: [_jsx(MemorizedSelectTemplateBody, { hasEmailPosts: hasEmailPosts, templates: [...templates, ...emailPosts], handleTemplateSelection: handleTemplateSelection, templateSelectMode: templateSelectMode }), _jsx(Flex, { className: "email-editor-modal-footer", justify: "flex-end", children: _jsx(FlexItem, { children: _jsx(Button, { variant: "tertiary", className: "email-editor-start_from_scratch_button", onClick: () => {
                            recordEvent('template_select_modal_start_from_scratch_clicked');
                            return handleCloseWithoutSelection();
                        }, isBusy: !hasTemplates, children: __('Start from scratch', 'fincommerce') }) }) })] }));
}
