...
export function usePreviewTemplates(customEmailContent = '') {
    const { templates, patterns, emailPosts, hasEmailPosts } = useSelect((select) => {
    ...
    const allTemplates = useMemo(() => {
        let contentPatterns = [];
        const parsedCustomEmailContent = customEmailContent && parse(customEmailContent);
    ...
    const allEmailPosts = useMemo(() => {
        return emailPosts?.map((post) => {
            const preferredTitle = applyFilters('fincommerce_email_editor_preferred_template_title', '', post);
            ...
            if (postTemplateContent?.content?.raw) {
                parsedPostContentWithTemplate = setPostContentInnerBlocks(parse(postTemplateContent?.content?.raw), parsedPostContent);
            }
            const template = {
                ...post,
                title: {
