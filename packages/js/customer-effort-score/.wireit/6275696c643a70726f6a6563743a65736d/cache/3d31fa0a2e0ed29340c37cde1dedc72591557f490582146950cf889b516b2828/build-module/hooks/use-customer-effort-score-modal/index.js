...
export const useCustomerEffortScoreModal = () => {
    const { showCesModal: _showCesModal, showProductMVPFeedbackModal } = useDispatch(STORE_KEY);
    const { updateOptions } = useDispatch(optionsStore);
    const { wasPreviouslyShown, isLoading } = useSelect((select) => {
        const { getOption, hasFinishedResolution } = select(optionsStore);
        const shownForActionsOption = getOption(SHOWN_FOR_ACTIONS_OPTION_NAME) || [];
    ...
    const markCesAsShown = async (action) => {
        ...
        const shownForActionsOption = (await getOption(SHOWN_FOR_ACTIONS_OPTION_NAME)) || [];
        updateOptions({
            [SHOWN_FOR_ACTIONS_OPTION_NAME]: [
                action,
