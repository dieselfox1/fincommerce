...
function _CustomerEffortScoreTracksContainer({ queue, resolving, clearQueue, }) {
    ...
    return (createElement(Fragment, null, queueForPage.map((item, index) => (createElement(CustomerEffortScoreTracks, { key: index, action: item.action, description: item.description, noticeLabel: item.noticeLabel, firstQuestion: item.firstQuestion, secondQuestion: item.secondQuestion, icon: item.icon, title: item.title, onSubmitLabel: item.onsubmit_label, trackProps: item.props || {} })))));
}
/** @type {import('react').ComponentType<CustomerEffortScoreTracksContainerProps>} */
export const CustomerEffortScoreTracksContainer = compose(withSelect((select) => {
