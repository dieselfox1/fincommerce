...
describe( 'bumpStat', () => {
	...
	it( 'should bump a single stat', () => {
		...
		expect( mockImage.src ).toMatch(
			/^https?:\/\/pixel\.wp\.com\/g\.gif\?v=wpcom-no-pv&x_fincommerce-group=name&t=/
		);
	} );
	...
	it( 'should bump multiple stats', () => {
		...
		expect( window.Image ).toHaveBeenCalledTimes( 1 );
		expect( mockImage.src ).toMatch(
			/^https?:\/\/pixel\.wp\.com\/g\.gif\?v=wpcom-no-pv&x_fincommerce-stat1=value1&x_fincommerce-stat2=value2&t=/
		);
	} );
} );
