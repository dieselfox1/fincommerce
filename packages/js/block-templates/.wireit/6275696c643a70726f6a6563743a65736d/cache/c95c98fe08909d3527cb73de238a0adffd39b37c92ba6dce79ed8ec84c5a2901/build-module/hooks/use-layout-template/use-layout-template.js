...
export const useLayoutTemplate = (layoutTemplateId) => {
    const [isEntityRegistered, setIsEntityRegistered] = useState(false);
    useEffect(() => {
        if (!layoutTemplateId)
            return;
        const layoutTemplateEntity = select(coreDataStore).getEntityConfig('root', 'wcLayoutTemplate');
    ...
    const { record: layoutTemplate, isResolving } = useEntityRecord('root', 'wcLayoutTemplate', 
