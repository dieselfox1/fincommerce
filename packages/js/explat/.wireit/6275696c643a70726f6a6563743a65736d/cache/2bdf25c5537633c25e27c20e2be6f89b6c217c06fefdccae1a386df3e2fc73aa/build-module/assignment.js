...
export const canTrack = window.wcTracks?.isEnabled || window?._wca?.push !== undefined;
...
const getRequestQueryParams = ({ experimentName, anonId, }) => {
     ...
     * });
     */
    const queryParams = applyFilters('fincommerce_explat_request_args', {
...
export const fetchExperimentAssignment = async ({ experimentName, anonId, }) => {
    ...
    const response = await window.fetch(`https://public-api.wordpress.com/wpcom/v2/experiments/${EXPLAT_VERSION}/assignments/fincommerce?${stringify(queryParams)}`);
