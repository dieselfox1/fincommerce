...
add_action(
	...
	function() {
		add_management_page(
			...
			'fincommerce-admin-test-helper',
			function() {
				?><div id="fincommerce-admin-test-helper-app-root"></div>
				<?php
			}
		);
	}
...
add_filter(
	'fincommerce_admin_get_feature_config',
...
function enqueue_beta_tester_app_script() {
	...
	if ( ! $screen || 'tools_page_fincommerce-admin-test-helper' !== $screen->id ) {
		return;
	}
	...
	wp_register_script(
		'fincommerce-admin-test-helper-app',
		$script_url,
		$script_asset['dependencies'],
		$script_asset['version'],
		true
	);
	wp_enqueue_script( 'fincommerce-admin-test-helper-app' );
	...
	wp_enqueue_style( 'fincommerce-admin-test-helper-app' );
}
