...
function toggle_remote_logging( $request ) {
	...
	if ( $enable ) {
		update_option( 'fincommerce_feature_remote_logging_enabled', 'yes' );
		update_option( 'fincommerce_allow_tracking', 'yes' );
		update_option( 'fincommerce_remote_variant_assignment', 1 );
		set_site_transient( RemoteLogger::WC_NEW_VERSION_TRANSIENT, WC()->version );
	} else {
		update_option( 'fincommerce_feature_remote_logging_enabled', 'no' );
	}
