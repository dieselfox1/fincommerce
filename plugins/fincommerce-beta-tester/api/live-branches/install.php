 ...
 * Avoid using WC functions so user can call these API without FinCommerce active.
 *
...
function check_live_branches_permissions() {
	if ( ! current_user_can( 'install_plugins' ) ) {
		return new \WP_Error(
			...
			__( 'Sorry, you cannot perform this action', 'fincommerce-beta-tester' )
		);
	}
	return true;
}
	...
	array(
		...
		'args'                => array(
			'download_url' => array(
				'required'          => true,
				'type'              => 'string',
				'description'       => 'The URL to download the FinCommerce plugin zip file.',
			...
			'version'      => array(
				'required'          => true,
				'type'              => 'string',
				'description'       => 'The version identifier of FinCommerce to be installed.',
				'sanitize_callback' => 'sanitize_text_field',
			),
		),
	)
...
register_fincommerce_admin_test_helper_rest_route(
	...
	'deactivate_fincommerce',
	array(
		'methods'             => 'GET',
		'permission_callback' => 'check_live_branches_permissions',
		'description'         => 'Deactivates the currently active FinCommerce plugin.',
	)
);
	...
	array(
		...
		'args'                => array(
			'version' => array(
				'required'          => true,
				'type'              => 'string',
				'description'       => 'The version identifier of FinCommerce to activate.',
				'sanitize_callback' => 'sanitize_text_field',
			),
		),
	)
...
/**
 * Get the latest FinCommerce pre-release release from GitHub API.
 *
...
function get_latest_wc_release( $include_pre_releases = false ) {
	...
	foreach ( $body as $release ) {
		// Ensure the release is a FinCommerce release, not nightly or beta tester releases.
...
/**
 * Respond to POST request to install the latest FinCommerce version.
 *
...
/**
 * Respond to GET request to deactivate FinCommerce.
 */
function deactivate_fincommerce() {
	$installer = new WC_Beta_Tester_Live_Branches_Installer();
	$installer->deactivate_fincommerce();
