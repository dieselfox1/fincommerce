<?php
/**
 * FinCommerce Beta Tester FinCommerce.com Base URL
 *
...
/**
 * Register REST API routes for setting and getting FinCommerce.com base URL.
 */
register_fincommerce_admin_test_helper_rest_route(
	...
	array(
		...
		'args'    => array(
			'url' => array(
				'description'       => 'FinCommerce.com base URL',
				'type'              => 'string',
				'required'          => true,
...
function tools_set_wccom_base_url( $request ) {
	...
	if ( empty( $url ) ) {
		...
		return new WP_REST_Response( array( 'message' => 'FinCommerce.com base URL is reset' ), 200 );
	}
...
function tools_get_wccom_base_url() {
	if ( class_exists( 'WC_Helper' ) && method_exists( 'WC_Helper', 'get_fincommerce_com_base_url' ) ) {
		$url = WC_Helper::get_fincommerce_com_base_url();
	} else {
...
/**
 * Filter FinCommerce.com base URL.
 *
 * @param string $default_url Default FinCommerce.com base URL.
 * @return string
 */
...
/**
 * Filter HTTP request arguments to disable SSL verification for custom FinCommerce.com base URL.
 *
