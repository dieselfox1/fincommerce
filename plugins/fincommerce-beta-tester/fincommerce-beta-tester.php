<?php
/**
 * Plugin Name: FinCommerce Beta Tester
 * Plugin URI: https://github.com/dieselfox1/fincommerce-beta-tester
 * Description: Run bleeding edge versions of FinCommerce. This will replace your installed version of FinCommerce with the latest tagged release - use with caution, and not on production sites.
 * Version: 3.0.0
 * Author: FinCommerce
 ...
 * WC tested up to: 9.5
 * Text Domain: fincommerce-beta-tester
 *
...
function _wc_beta_tester_load_textdomain() {
	load_plugin_textdomain( 'fincommerce-beta-tester', false, basename( dirname( __FILE__ ) ) . '/languages' );
}
...
function _wc_beta_tester_bootstrap() {
	...
	// Check if FinCommerce is enabled.
...
function add_extension_register_script() {
	...
	wp_register_script(
		'fincommerce-admin-test-helper',
		$script_url,
		$script_asset['dependencies'],
		$script_asset['version'],
		true
	);
	wp_enqueue_script( 'fincommerce-admin-test-helper' );
	...
	if ( file_exists( $css_file ) ) {
		...
		wp_enqueue_style( 'fincommerce-admin-test-helper' );
	}
}
...
add_action(
	...
	function() {
		if ( class_exists( '\Automattic\FinCommerce\Utilities\FeaturesUtil' ) ) {
			\Automattic\FinCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', __FILE__, true );
			\Automattic\FinCommerce\Utilities\FeaturesUtil::declare_compatibility( 'product_block_editor', __FILE__, true );
		}
	}
);
...
/**
 * Simulate a FinCommerce error for remote logging testing.
 *
 * This function adds a filter to the 'fincommerce_template_path' hook
 * that throws an exception, then triggers the filter by calling WC()->template_path().
 *
 * @throws Exception A simulated FinCommerce error for testing purposes.
 */
function simulate_fincommerce_error() {
	// Return if FinCommerce is not loaded.
	if ( ! function_exists( 'WC' ) || ! class_exists( 'FinCommerce' ) ) {
		return;
	}
	...
	add_filter(
		'fincommerce_template_path',
		function() {
			throw new Exception( 'Simulated FinCommerce error for remote logging test' );
		}
	);
...
$simulate_error = get_option( 'wc_beta_tester_simulate_fincommerce_php_error', false );

if ( $simulate_error ) {
	delete_option( 'wc_beta_tester_simulate_fincommerce_php_error' );

	if ( 'core' === $simulate_error ) {
		// Hook into the plugin_loaded action to simulate the error early before WP fully initializes.
		add_action( 'plugin_loaded', 'simulate_fincommerce_error' );
	} elseif ( 'beta-tester' === $simulate_error ) {
		throw new Exception( 'Test PHP exception from FinCommerce Beta Tester' );
	}
}
