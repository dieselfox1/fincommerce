...
// @match        https://github.com/dieselfox1/fincommerce/pull/*
// ==/UserScript==
...
( function () {
	...
	// - Removing `#fincommerce-live-branches`.
	const observer = new MutationObserver( ( list ) => {
		for ( const m of list ) {
			...
			for ( const n of m.removedNodes ) {
				if (
					n.id === 'fincommerce-live-branches' ||
					( n.querySelector &&
						n.querySelector( '#fincommerce-live-branches' ) )
				) {
					doit();
					return;
				}
	...
	function doit() {
		...
		if (
			! markdownBody ||
			markdownBody.querySelector( '#fincommerce-live-branches' )
		) {
			...
			const contents = `
					<details>
						...
						${ getOptionsList(
							[
								{
									label: 'FinCommerce Smooth Generator',
						...
						${ getOptionsList(
							featureFlags.map( ( flag ) => ( {
								label: flag,
								name: 'fincommerce-beta-tester-feature-flags',
								value: flag,
							} ) ),
							50
						) }
					...
					<p>
						<a id="fincommerce-beta-branch-link" target="_blank" rel="nofollow noopener" href="#">…</a>
					</p>
					`;
			appendHtml( markdownBody, contents );
			updateLink();
		...
		function getLink() {
			const query = [
				...
				`fincommerce-beta-tester-live-branch=${ currentBranch }`,
			];

			const enabledFeatureFlags = [];

			$(
				'#fincommerce-live-branches input[type=checkbox]:checked:not([data-invert]), #fincommerce-live-branches input[type=checkbox][data-invert]:not(:checked)'
			).each( ( i, input ) => {
				if ( input.name === 'fincommerce-beta-tester-feature-flags' ) {
			...
			if ( enabledFeatureFlags.length ) {
				query.push(
					`fincommerce-beta-tester-feature-flags=${ encodeURIComponent(
		...
		function appendHtml( el, contents ) {
			const $el = $( el );
			const liveBranches = $(
				'<div id="fincommerce-live-branches" />'
			).append( `<h2>FinCommerce Live Branches</h2> ${ contents }` );
			$( '#fincommerce-live-branches' ).remove();
		 ...
		 */
		function updateLink() {
			const $link = $( '#fincommerce-beta-branch-link' );
			const url = getLink();
