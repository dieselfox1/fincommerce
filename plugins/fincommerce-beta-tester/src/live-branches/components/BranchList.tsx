...
const BranchListItem = ( {
	...
	const { isError, isInProgress, installAndActivate, activate, status } =
		useLiveBranchInstall(
			branch.download_url,
			`https://github.com/dieselfox1/fincommerce/pull/${ branch.pr }`,
			branch.version,
	...
	return (
		<Item>
			...
			<p>
				Pull Request:{ ' ' }
				<a
					href={ `https://github.com/dieselfox1/fincommerce/pull/${ branch.pr }` }
				>
					{ branch.branch }
				</a>
			</p>
...
const fincommerceVersionInfo = () => {
	const version = window?.wc?.wcSettings?.WC_VERSION || 'unknown';

	return (
		<p>
			Live branch not installed. Running FinCommerce version: { version }
		</p>
	);
};
		...
		<>
			<Card elevation={ 3 } css={ cardStyle }>
				...
				<CardBody>
					...
					{ ! activeBranch && <fincommerceVersionInfo /> }
				</CardBody>
				<CardFooter>
					<Fragment />
				</CardFooter>
