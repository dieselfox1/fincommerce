...
export function* resetOnboardingWizard() {
	yield runCommand( 'Reset Onboarding Wizard', function* () {
		...
		const defaultOptions = {
			fincommerce_allow_tracking: 'no',
			fincommerce_default_country: 'US:CA',
			fincommerce_currency: 'USD',
			fincommerce_currency_pos: 'left',
			fincommerce_price_thousand_sep: ',',
			fincommerce_price_decimal_sep: '.',
			fincommerce_price_num_decimals: '2',
			fincommerce_coming_soon: 'no',
			fincommerce_weight_unit: 'lbs',
			fincommerce_dimension_unit: 'in',
		};
...
export function* updateStoreAge() {
	yield runCommand( 'Update Installation timestamp', function* () {
		...
		if ( numberOfDays !== null ) {
			...
			const payload = {
				fincommerce_admin_install_timestamp: JSON.parse( newTimestamp ),
			};
			yield apiFetch( {
				method: 'POST',
...
export function* runDisableEmail() {
	yield runCommand( 'Disable/Enable FinCommerce emails', function* () {
...
export function* updateWccomBaseUrl( { url } ) {
	yield runCommand( 'Set FinCommerce.com Base URL', function* () {
		yield apiFetch( {
