...
function RemoteLogging() {
	...
	const simulatePhpException = async ( context: 'core' | 'beta-tester' ) => {
		try {
			await dispatch( optionsStore ).saveOption(
				'wc_beta_tester_simulate_fincommerce_php_error',
				context
			);
			setNotice( {
				status: 'success',
	...
	const simulateException = async ( context: 'core' | 'beta-tester' ) => {
		try {
			await dispatch( optionsStore ).saveOption(
				'wc_beta_tester_simulate_fincommerce_js_error',
				context
			);
			...
			if ( context === 'core' ) {
				setNotice( {
					status: 'success',
					message: `Please go to FinCommerce pages to trigger the JS exception in fincommerce context.`,
				} );
			} else {
				setNotice( {
					status: 'success',
					message:
						'Please refresh your browser to trigger the JS exception in fincommerce beta tester context.',
				} );
			}
		} catch ( error ) {
			setNotice( {
	...
	const logJsEvent = async () => {
		try {
			const result = await log(
				'info',
				'Test JS event from FinCommerce Beta Tester',
				{
					extra: {
