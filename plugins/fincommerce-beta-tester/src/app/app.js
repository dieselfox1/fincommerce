...
const tabs = applyFilters( 'fincommerce_admin_test_helper_tabs', [
	{
		name: 'options',
		title: 'Options',
		content: <Options />,
...
export function App() {
	...
	return (
		<div className="wrap">
			<h1>FinCommerce Admin Test Helper</h1>
			<TabPanel
				className="fincommerce-admin-test-helper__main-tab-panel"
			...
			>
				{ () => (
					<>
						{ activeTab.content }
						{ applyFilters(
							`fincommerce_admin_test_helper_tab_${ activeTab.name }`,
							[]
						) }
					</>
				) }
