...
class WC_Beta_Tester_Override_Coming_Soon_Options {
	...
	public function wc_beta_tester_override_coming_soon_options() {
		...
		$this->override_fincommerce_coming_soon_option( $mode );
		$this->override_fincommerce_store_pages_only_option( $mode );
	}

	/**
	 * Override the fincommerce_coming_soon option.
	 *
	 * @param string $mode The coming soon mode.
	 */
	private function override_fincommerce_coming_soon_option( $mode ) {
		add_filter(
			'option_fincommerce_coming_soon',
	...
	/**
	 * Override the fincommerce_store_pages_only option.
	 *
	 * @param string $mode The coming soon mode.
	 */
	private function override_fincommerce_store_pages_only_option( $mode ) {
		add_filter(
			'option_fincommerce_store_pages_only',
			function ( $value ) use ( $mode ) {
				if ( 'store' === $mode ) {
					return 'yes';
				}
