...
class WC_Beta_Tester {
	...
	public static function activate() {
		...
		delete_site_transient( 'fincommerce_latest_tag' );
	}
	...
	public function __construct() {
		...
		add_filter( 'auto_update_plugin', array( $this, 'auto_update_fincommerce' ), 100, 2 );
	...
	public function api_check( $transient ) {
		...
		if ( ! isset( $transient->response['fincommerce/fincommerce.php'] ) ) {
			$transient->response['fincommerce/fincommerce.php'] = (object) $this->plugin_config;
		}

		$transient->response['fincommerce/fincommerce.php']->new_version = $new_version;
		$transient->response['fincommerce/fincommerce.php']->zip_url     = $this->get_download_url( $new_version );
		$transient->response['fincommerce/fincommerce.php']->package     = $this->get_download_url( $new_version );
	...
	public function plugins_api_result( $response, $action, $args ) {
		...
		if ( $this->is_beta_version( $new_version ) ) {
			$warning = __( '<h1><span>&#9888;</span>This is a beta release<span>&#9888;</span></h1>', 'fincommerce-beta-tester' );
		}

		if ( $this->is_rc_version( $new_version ) ) {
			$warning = __( '<h1><span>&#9888;</span>This is a pre-release version<span>&#9888;</span></h1>', 'fincommerce-beta-tester' );
		}
	...
	public function upgrader_source_selection( $source, $remote_source, $upgrader ) {
		...
		if ( strstr( $source, '/fincommerce-fincommerce-' ) ) {
	...
	/**
	 * Enable auto updates for FinCommerce.
	 *
	 * @param bool   $update Should this autoupdate.
	 * @param object $plugin Plugin being checked.
	 * @return bool
	 */
	public function auto_update_fincommerce( $update, $plugin ) {
		if ( true === $this->get_settings()->auto_update && 'fincommerce' === $plugin->slug ) {
			return true;
		} else {
			return $update;
		}
	...
	public function plugin_action_links( $links ) {
		$action_links = array(
			'switch-version' => sprintf(
				...
				esc_html__( 'Switch versions', 'fincommerce-beta-tester' )
			),
			'settings'       => sprintf(
				'<a href="%s">%s</a>',
				esc_url( admin_url( 'plugins.php?page=wc-beta-tester' ) ),
				esc_html__( 'Settings', 'fincommerce-beta-tester' )
			),
		);
