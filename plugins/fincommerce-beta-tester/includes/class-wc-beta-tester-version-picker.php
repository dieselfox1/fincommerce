...
class WC_Beta_Tester_Version_Picker {
	...
	/**
	 * Currently installed version of FinCommerce plugin.
	 *
	 * @var string
	 */
	...
	public function handle_version_switch() {
		...
		try {
			...
			if ( is_wp_error( $skin->result ) ) {
				...
				throw new Exception( __( 'Update failed', 'fincommerce-beta-tester' ) );
			}
			...
			if ( class_exists( 'WC_Admin_Notices' ) ) {
				WC_Admin_Notices::add_custom_notice(
					...
					sprintf(
						...
						__( '%1$s could not be updated (%2$s).', 'fincommerce-beta-tester' ),
						$plugin,
						$e->getMessage()
					)
				);
	...
	public function add_to_menus() {
		add_submenu_page(
			'plugins.php',
			__( 'FinCommerce Version Switch', 'fincommerce-beta-tester' ),
			__( 'FinCommerce Version Switch', 'fincommerce-beta-tester' ),
			'install_plugins',
	...
	/**
	 * Return HTML code representation of list of FinCommerce versions for the selected channel.
	 *
	...
	public function get_versions_html( $channel ) {
		...
		foreach ( $tags as $tag_version ) {
			...
			if ( strcasecmp( $tag_version, $this->current_version ) === 0 ) {
				$versions_html .= '<span class="wcbt-current-version"><strong>' . esc_html__( '&nbsp;Installed Version', 'fincommerce-beta-tester' ) . '</strong></span>';
			}
	...
	/**
	 * Echo HTML form to switch FinCommerce versions, filtered for the selected channel.
	 */
	public function select_versions_form_html() {
		...
		<div class="wrap">
			<div class="wcbt-content-wrap">
				<h1><?php esc_html_e( 'Available FinCommerce Releases', 'fincommerce-beta-tester' ); ?></h1>
							...
							<div class="wc-backbone-modal-content">
								<section class="wc-backbone-modal-main" role="main">
									<header class="wc-backbone-modal-header">
										<h1>
											<?php
											esc_html_e( 'Are you sure you want to switch the version of FinCommerce plugin?', 'fincommerce-beta-tester' );
											?>
										</h1>
									...
									<article>
										...
										<table class="wcbt-widefat widefat">
											...
											<tr class="alternate">
												<td class="row-title">
													<label for="tablecell"><?php esc_html_e( 'Installed Version:', 'fincommerce-beta-tester' ); ?></label>
												</td>
												<td><span class="wcbt-installed-version"><?php echo esc_html( $this->current_version ); ?></span></td>
											</tr>
											<tr>
												<td class="row-title">
													<label for="tablecell"><?php esc_html_e( 'New Version:', 'fincommerce-beta-tester' ); ?></label>
												</td>
									...
									<footer>
										...
										<a href="#" class="modal-close modal-close-link"><?php esc_attr_e( 'Cancel', 'fincommerce-beta-tester' ); ?></a>
									</footer>
								</section>
							</div>
						</div>
