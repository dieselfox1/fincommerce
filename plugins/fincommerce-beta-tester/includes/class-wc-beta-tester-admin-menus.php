...
```
	...
	private function get_theme_ssr_info( $theme ) {
		...
		if ( version_compare( $theme['version'], $theme['version_latest'], '<' ) ) {
			...
			$version .= sprintf( __( ' - %s is available', 'fincommerce-beta-tester' ), esc_html( $theme['version_latest'] ) );
		}

		$child_theme = $theme['is_child_theme'] ? __( 'Yes', 'fincommerce-beta-tester' ) : __( 'No - If you are modifying FinCommerce on a parent theme that you did not build personally we recommend using a child theme.', 'fincommerce-beta-tester' );

		if ( $theme['is_child_theme'] ) {
			$parent_version = $theme['parent_version'];
			if ( version_compare( $theme['parent_version'], $theme['parent_version_latest'], '<' ) ) {
				/* translators: 1: latest version */
				$parent_version .= sprintf( __( ' - %s is available', 'fincommerce-beta-tester' ), esc_html( $theme['parent_version_latest'] ) );
			}
		...
		$theme_info .= sprintf(
			__( "Name: %1\$s\nVersion: %2\$s\nAuthor URL: %3\$s\nChild Theme: %4\$s\nfincommerce Support: %5\$s", 'fincommerce-beta-tester' ),
			$theme['name'],
			$version,
			esc_html( $theme['author_url'] ),
			$child_theme,
			! $theme['has_fincommerce_support'] ? __( 'Not declared', 'fincommerce-beta-tester' ) : __( 'Yes', 'fincommerce-beta-tester' )
		);
			...
			if ( ! empty( $plugin['name'] ) ) {
				...
				if ( strstr( $plugin['url'], 'woothemes.com' ) || strstr( $plugin['url'], 'woo.com' ) ) {
					if ( ! empty( $plugin['version_latest'] ) && version_compare( $plugin['version_latest'], $plugin['version'], '>' ) ) {
						...
						$version_string = sprintf( esc_html__( '%s is available', 'fincommerce-beta-tester' ), $plugin['version_latest'] );
					}

					if ( false !== (bool) $plugin['network_activated'] ) {
						$network_string = __( 'Network enabled', 'fincommerce-beta-tester' );
					}
				}
				$untested_string = '';
				if ( array_key_exists( $plugin['plugin'], $untested_plugins ) ) {
					$untested_string = __( 'Not tested with the active version of FinCommerce', 'fincommerce-beta-tester' );
				}

				$plugins_info .= sprintf( __( "%1\$s: by %2\$s - %3\$s - %4\$s\n", 'fincommerce-beta-tester' ), $plugin_name, $plugin['author_name'], $plugin['version'], $version_string . $untested_string . $network_string );
			}
		}
	...
	protected function get_github_ticket_url() {
		...
		return add_query_arg(
			array(
				...
				'title' => rawurlencode( sprintf( __( '[WC Beta Tester] Bug report for version "%s"', 'fincommerce-beta-tester' ), $version ) ),
			),
			'https://github.com/dieselfox1/fincommerce/issues/new'
		);
	}
	...
	public function admin_bar_menus( $wp_admin_bar ) {
		...
		$wp_admin_bar->add_node(
			array(
				'parent' => 0,
				'id'     => 'wc-beta-tester',
				'title'  => __( 'WC Beta Tester', 'fincommerce-beta-tester' ),
			)
		);
		...
		switch ( $settings->channel ) {
			case 'beta':
				$current_channel = __( 'Beta', 'fincommerce-beta-tester' );
				break;
			case 'rc':
				$current_channel = __( 'Release Candidate', 'fincommerce-beta-tester' );
				break;
			default:
				$current_channel = __( 'Stable', 'fincommerce-beta-tester' );
				break;
		}
		...
		$nodes = array(
			array(
				...
				'title'  => sprintf( __( 'Channel: %s', 'fincommerce-beta-tester' ), $current_channel ),
				'href'   => admin_url( 'plugins.php?page=wc-beta-tester' ),
			),
			array(
				'parent' => 'wc-beta-tester',
				'id'     => 'import-export-settings',
				'title'  => __( 'Import/Export Settings', 'fincommerce-beta-tester' ),
			...
			array(
				...
				'title'  => sprintf( __( 'Release %s information', 'fincommerce-beta-tester' ), WC_VERSION ),
				'href'   => '#',
			),
			array(
				'parent' => 'wc-beta-tester',
				'id'     => 'switch-version',
				'title'  => __( 'Switch versions', 'fincommerce-beta-tester' ),
				'href'   => admin_url( 'plugins.php?page=wc-beta-tester-version-picker' ),
			),
			array(
				'parent' => 'wc-beta-tester',
				'id'     => 'submit-gh-ticket',
				'title'  => __( 'Submit bug report', 'fincommerce-beta-tester' ),
				'href'   => '#',
				'meta'   => array(
					...
					<section class="wc-backbone-modal-main" role="main">
						<header class="wc-backbone-modal-header">
							...
							<?php
								...
								echo esc_html( sprintf( __( 'Release %s information', 'fincommerce-beta-tester' ), '{{ data.version }}' ) );
							?>
							</h1>
							<button class="modal-close modal-close-link dashicons dashicons-no-alt">
								<span class="screen-reader-text"><?php esc_html_e( 'Close modal panel', 'fincommerce-beta-tester' ); ?></span>
							</button>
						</header>
						<article>
							<?php do_action( 'fincommerce_admin_version_information_start' ); ?>
							{{ data.description }}
							<?php do_action( 'fincommerce_admin_version_information_end' ); ?>
						</article>
						<footer>
							<a target="_blank" href="https://github.com/dieselfox1/fincommerce/releases/tag/{{ data.version }}"><?php esc_html_e( 'Read more on GitHub', 'fincommerce-beta-tester' ); ?></a>
						</footer>
					</section>
				</div>
			</div>
