...
class WC_Beta_Tester_Import_Export {
	 ...
	 */
	protected const IMPORT_FILENAME = 'fincommerce-settings-json';
	...
	public function add_to_menu() {
		add_submenu_page( 'plugins.php', __( 'WC Beta Tester Import/Export', 'fincommerce-beta-tester' ), __( 'WC Import/Export', 'fincommerce-beta-tester' ), static::IMPORT_CAP, 'wc-beta-tester-settings', array( $this, 'settings_page_html' ) );
	}
	...
	public function settings_page_html() {
		...
		<div class="wrap">
			...
			<p><?php esc_html_e( 'Export your FinCommerce Settings. The export file should not contain any fields that identify your site or reveal secrets (eg. API keys).', 'fincommerce-beta-tester' ); ?></p>
			<a href="<?php echo esc_url( $export_url ); ?>" class="button-primary">
								<?php
								/* translators Export FinCommerce settings button text. */
								esc_html_e( 'Export FinCommerce Settings', 'fincommerce-beta-tester' );
								?>
			</a>
			<hr />
			<form method="POST" enctype="multipart/form-data">
				...
				<p><?php esc_html_e( 'Import FinCommerce Settings exported with this tool. Some settings like store address, payment gateways, etc. will need to be configured manually.', 'fincommerce-beta-tester' ); ?></p>
				<button type="submit" class="button-primary">
				<?php
				/* translators Import FinCommerce settings button text. */
				esc_html_e( 'Import FinCommerce Settings', 'fincommerce-beta-tester' );
				?>
				</button>
	...
	public function export_settings() {
		...
		$filename = sprintf( 'fincommerce-settings-%s.json', gmdate( 'Ymdgi' ) );
		wc_set_time_limit( 0 );
		wc_nocache_headers();
	...
	public function maybe_import_settings() {
		...
		if ( ! is_readable( $tmp_file ) ) {
			$this->add_message( __( 'File could not be read.', 'fincommerce-beta-tester' ) );
			return;
		}
		...
		if ( null !== $settings ) {
			...
			$this->add_message( __( 'Settings Imported', 'fincommerce-beta-tester' ), 'updated' );
		} else {
			$this->add_message( __( 'File did not contain well formed JSON.', 'fincommerce-beta-tester' ) );
		}
	}

	/**
	 * Get an array of the FinCommerce related settings.
	 */
	...
	/**
	 * Get the FinCommerce settings list keys.
	 */
