...
class WC_Beta_Tester_Live_Branches_Installer {
	...
	private function init_filesystem() {
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new WP_Error( 'fs_api_error', __( 'FinCommerce Beta Tester: No File System access', 'fincommerce-beta-tester' ) ); // @codingStandardsIgnoreLine.
		}
	...
	/**
	 * Get the download url of a FinCommerce plugin version from the manifest.
	 *
	 * @param string $branch The name of the branch.
	 */
	public function get_branch_info_from_manifest( $branch ) {
		$response = wp_remote_get( 'https://betadownload.jetpack.me/fincommerce-branches.json' );
	...
	/**
	 * Install a FinCommerce plugin version by download url.
	 *
	...
	public function install( $download_url, $version ) {
		...
		if ( is_wp_error( $tmp_dir ) ) {
			return new WP_Error(
				'download_error',
				sprintf( __( 'Error Downloading: <a href="%1$s">%1$s</a> - Error: %2$s', 'fincommerce-beta-tester' ), $download_url, $tmp_dir->get_error_message() ) // @codingStandardsIgnoreLine.
			);
		}
		...
		$unzip_path  = $plugin_dir . "/fincommerce-$version";
		...
		if ( is_wp_error( $unzip ) ) {
			...
			return new WP_Error( 'unzip_error', sprintf( __( 'Error Unzipping file: Error: %1$s', 'fincommerce-beta-tester' ), $unzip->get_error_message() ) );
		}

		$result = true;
		// The plugin is nested under fincommerce-dev or fincommerce, so we need to move it up one level.
		$source_paths = array(
			$unzip_path . '/fincommerce-dev',
			$unzip_path . '/fincommerce',
		);
		...
		if ( $source ) {
			...
			$result = new WP_Error( 'unzip_error', __( 'Could not find fincommerce-dev or fincommerce in the zip file', 'fincommerce-beta-tester' ) );
		}
	...
	/**
	 * Deactivate all currently active FinCommerce plugins.
	 */
	...
	/**
	 * Activate a beta tester installed FinCommerce plugin
	 *
	 * @param string $version The version of the plugin to activate.
	 */
	public function activate( $version ) {
		if ( ! is_plugin_active( LIVE_BRANCH_PLUGIN_PREFIX . "_$version/fincommerce.php" ) ) {
			activate_plugin( LIVE_BRANCH_PLUGIN_PREFIX . "_$version/fincommerce.php" );
		}
	}
	...
	public function check_install_status( $version ) {
		$plugin_path = WP_PLUGIN_DIR . '/' . LIVE_BRANCH_PLUGIN_PREFIX . "_$version/fincommerce.php";
