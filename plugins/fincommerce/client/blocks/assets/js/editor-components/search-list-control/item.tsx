...
import { getHighlightedName, getBreadcrumbsForDisplay } from '@fincommerce/block-library/assets/js/editor-components/search-list-control/utils';

const Count = ( { label }: { label: string | React.ReactNode | number } ) => {
	return (
		<span className="fincommerce-search-list__item-count">{ label }</span>
	);
};
...
const ItemLabel = ( props: { item: SearchListItemProps; search: string } ) => {
	...
	return (
		<span className="fincommerce-search-list__item-label">
			{ hasBreadcrumbs ? (
				<span className="fincommerce-search-list__item-prefix">
					{ getBreadcrumbsForDisplay( item.breadcrumbs ) }
				</span>
			) : null }
			<span className="fincommerce-search-list__item-name">
...
export const SearchListItem = < T extends object = object >( {
	...
	const classes = clsx(
		[ 'fincommerce-search-list__item', `depth-${ depth }`, className ],
		{
			...
			{ isSingle ? (
				<>
					<input
						...
						checked={ isSelected }
						className="fincommerce-search-list__item-input"
						{ ...props }
					/>
