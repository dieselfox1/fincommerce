 ...
 */
import '@fincommerce/block-library/assets/js/editor-components/default-notice/editor.scss';
...
export function DefaultNotice( { block }: { block: string } ) {
	...
	const settingName =
		block === 'checkout'
			? 'fincommerce_checkout_page_id'
			: 'fincommerce_cart_page_id';
			...
			),
			{
				a: (
					...
					<a href="#" onClick={ updatePage }>
						{ __( 'update your page settings', 'fincommerce' ) }
					</a>
				),
			}
		);
		...
		<Notice
			...
			spokenMessage={
				settingStatus === 'updated'
					? __( 'Page settings updated', 'fincommerce' )
					: noticeContent
			}
		>
			{ settingStatus === 'updated' ? (
