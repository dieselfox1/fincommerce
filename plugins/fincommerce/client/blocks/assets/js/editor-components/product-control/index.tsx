...
import { convertProductResponseItemToSearchItem } from '@fincommerce/utils';
 ...
 */
import '@fincommerce/block-library/assets/js/editor-components/product-control/style.scss';
...
const messages = {
	list: __( 'Products', 'fincommerce' ),
	noItems: __( "Your store doesn't have any products.", 'fincommerce' ),
	search: __( 'Search for a product to display', 'fincommerce' ),
	updated: __( 'Product search results updated.', 'fincommerce' ),
};
...
const ProductControl = (
	...
	const renderItemWithVariations = (
		...
		const variationsCount =
			item.details?.variations && Array.isArray( item.details.variations )
				...
				: 0;
		const classes = clsx(
			'fincommerce-search-product__item',
	...
	return (
		<SearchListControl
			className="fincommerce-products"
			list={ currentList }
			isCompact={ isCompact }
			isLoading={ isLoading }
			isSingle
