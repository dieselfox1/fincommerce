 ...
 */
import '@fincommerce/block-library/assets/js/editor-components/tag/editor.scss';
...
const Tag = ( {
	...
	return (
		<span className={ classes }>
			{ popoverContents ? (
				<Button
					className="fincommerce-tag__text"
					id={ labelId }
				...
				<span className="fincommerce-tag__text" id={ labelId }>
					{ labelTextNode }
				</span>
			) }
			...
			{ remove && (
				<Button
					className="fincommerce-tag__remove"
					onClick={ remove( id ) }
					label={ sprintf(
						// Translators: %s label.
						__( 'Remove %s', 'fincommerce' ),
						label
					) }
