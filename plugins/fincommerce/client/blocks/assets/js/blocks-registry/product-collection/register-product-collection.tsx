...
const isValidCollectionConfig = ( config: ProductCollectionConfig ) => {
	...
	// attributes.query.fincommerceOnSale
	if (
		config.attributes?.query?.fincommerceOnSale !== undefined &&
		typeof config.attributes.query.fincommerceOnSale !== 'boolean'
	) {
		console.warn(
			'Invalid fincommerceOnSale: fincommerceOnSale must be a boolean.'
		);
	}
	// attributes.query.fincommerceStockStatus
	if (
		config.attributes?.query?.fincommerceStockStatus !== undefined &&
		! Array.isArray( config.attributes.query.fincommerceStockStatus )
	) {
		console.warn(
			'Invalid fincommerceStockStatus: fincommerceStockStatus must be an array.'
		);
	}
	// attributes.query.fincommerceAttributes
	if (
		config.attributes?.query?.fincommerceAttributes !== undefined &&
		! Array.isArray( config.attributes.query.fincommerceAttributes )
	) {
		console.warn(
			'Invalid fincommerceAttributes: fincommerceAttributes must be an array.'
		);
	}
	// attributes.query.fincommerceHandPickedProducts
	if (
		config.attributes?.query?.fincommerceHandPickedProducts !== undefined &&
		! Array.isArray( config.attributes.query.fincommerceHandPickedProducts )
	) {
		console.warn(
			'Invalid fincommerceHandPickedProducts: fincommerceHandPickedProducts must be an array.'
		);
	}
	...
	const collectionConfigWithoutExtraArgs = {
		...
		attributes: {
			...
			query: {
				...
				...( query.fincommerceOnSale !== undefined && {
					fincommerceOnSale: query.fincommerceOnSale,
				} ),
				...( query.fincommerceStockStatus !== undefined && {
					fincommerceStockStatus: query.fincommerceStockStatus,
				} ),
				...( query.fincommerceAttributes !== undefined && {
					fincommerceAttributes: query.fincommerceAttributes,
				} ),
				...( query.fincommerceHandPickedProducts !== undefined && {
					fincommerceHandPickedProducts:
						query.fincommerceHandPickedProducts,
				} ),
