...
} from '@fincommerce/block-library/assets/js/blocks-registry/block-components';

describe( 'blocks registry', () => {
	const context = '@fincommerce/all-products';
	const blockName = '@fincommerce-extension/price-level';
	const component = () => {
		return null;
	};
	...
	describe( 'getRegisteredBlockComponents', () => {
		it( 'gets an empty object when context has no inner blocks', () => {
			expect(
				getRegisteredBlockComponents( '@fincommerce/all-products' )
			).toEqual( {} );
		} );
	...
	describe( 'getRegisteredInnerBlocks (deprecated)', () => {
		it( 'gets an empty object when parent has no inner blocks', () => {
			expect(
				getRegisteredInnerBlocks( '@fincommerce/test-parent' )
			).toEqual( {} );
			expect( console ).toHaveWarned();
		} );
		it( 'gets a block that was successfully registered', () => {
			registerBlockComponent( { context, blockName, component } );
			expect(
				getRegisteredInnerBlocks( '@fincommerce/all-products' )
			).toEqual( {
				[ blockName ]: component,
			} );
		} );
