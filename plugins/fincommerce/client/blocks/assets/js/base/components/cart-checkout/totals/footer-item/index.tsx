 ...
 */
import '@fincommerce/block-library/assets/js/base/components/cart-checkout/totals/footer-item/style.scss';
...
const TotalsFooterItem = ( {
	...
	const label = applyCheckoutFilter( {
		...
		defaultValue: isEstimate
			? __( 'Estimated total', 'fincommerce' )
	...
	const description =
		taxLines && taxLines.length > 0
			...
			: __( 'Including <TaxAmount/> in taxes', 'fincommerce' );

	const hasSelectedRates = hasSelectedShippingRate( cart.shippingRates );
	const cartNeedsShipping = cart.cartNeedsShipping;
	const skeleton = (
		<>
			<span>{ __( 'Including', 'fincommerce' ) }</span>
			<Skeleton
				height="1em"
				width="45px"
				tag="span"
				ariaMessage={ __( 'Loading price… ', 'fincommerce' ) }
			/>
		</>
	);
