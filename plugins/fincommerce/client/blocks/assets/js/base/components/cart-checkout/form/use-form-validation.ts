...
import {
	...
	usePrevious,
	useSchemaParser,
} from '@fincommerce/base-hooks';
import { getFieldLabel, isPostcode } from '@fincommerce/blocks-checkout';
import {
	AddressFormValues,
	ContactFormValues,
	FormFields,
 ...
 */
import { hasSchemaRules } from '@fincommerce/block-library/assets/js/base/components/cart-checkout/form/utils';
			...
			switch ( error.keyword ) {
				...
				case 'pattern':
					acc[ fieldKey ] = sprintf(
						...
						__( '%1$s must match the pattern %2$s', 'fincommerce' ),
						fieldLabel,
	...
	const customValidation = formFields
		.map( ( field ) => {
			...
			if (
				...
				return [
					field.key,
					__( 'Please enter a valid postcode', 'fincommerce' ),
				];
			}
			...
			if (
				...
				return [
					field.key,
					__( 'Please enter a valid email address', 'fincommerce' ),
				];
			}
