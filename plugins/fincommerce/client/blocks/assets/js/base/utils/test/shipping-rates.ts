...
import {
	...
	getShippingPrices,
} from '@fincommerce/block-library/assets/js/blocks/checkout/inner-blocks/checkout-shipping-method-block/shared/helpers';
import { generateShippingRate } from '@fincommerce/block-library/assets/js/mocks/shipping-package';
...
jest.mock( '@fincommerce/settings', () => {
	return {
		...
		getSetting: jest.fn().mockImplementation( ( setting: string ) => {
			...
			return jest
				.requireActual( '@fincommerce/settings' )
				.getSetting( setting );
		} ),
	};
} );
jest.mock( '@fincommerce/block-settings', () => ( {
	__esModule: true,
	...jest.requireActual( '@fincommerce/block-settings' ),
	LOCAL_PICKUP_ENABLED: true,
} ) );
const blockSettingsMock = jest.requireMock( '@fincommerce/block-settings' );
