 ...
 */
import { namespace, actionPrefix } from '@fincommerce/block-library/assets/js/extensions/google-analytics/constants';
import {
	...
	trackCheckoutStep,
	trackCheckoutOption,
} from '@fincommerce/block-library/assets/js/extensions/google-analytics/utils';
...
addAction(
	...
	( { shippingRateId }: { shippingRateId: string } ): void => {
		trackCheckoutOption( {
			step: 4,
			option: __( 'Shipping Method', 'fincommerce' ),
...
addAction(
	...
	( { paymentMethodSlug }: { paymentMethodSlug: string } ): void => {
		trackCheckoutOption( {
			step: 5,
			option: __( 'Payment Method', 'fincommerce' ),
...
addAction(
	...
	( {
		...
		trackEvent( 'add_to_cart', {
			...
			event_label: __( 'Add to Cart', 'fincommerce' ),
...
addAction(
	...
	( {
		...
		trackEvent( 'remove_from_cart', {
			event_category: 'ecommerce',
			event_label: __( 'Remove Cart Item', 'fincommerce' ),
			items: [ getProductFieldObject( product, quantity ) ],
		} );
	}
);
...
addAction(
	...
	( {
		...
		trackEvent( 'change_cart_quantity', {
			event_category: 'ecommerce',
			event_label: __( 'Change Cart Item Quantity', 'fincommerce' ),
			items: [ getProductFieldObject( product, quantity ) ],
		} );
	}
);
...
addAction(
	...
	( {
		products,
		listName = __( 'Product List', 'fincommerce' ),
	}: {
		...
		trackEvent( 'view_item_list', {
			...
			event_label: __( 'Viewing products', 'fincommerce' ),
