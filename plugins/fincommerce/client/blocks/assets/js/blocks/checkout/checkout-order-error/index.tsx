...
import { CheckoutResponse } from '@fincommerce/types';

/**
 * Internal dependencies
 */
import '@fincommerce/block-library/assets/js/blocks/checkout/checkout-order-error/style.scss';
import {
...
} from '@fincommerce/block-library/assets/js/blocks/checkout/checkout-order-error/constants';
...
const ErrorTitle = ( { errorData }: ErrorComponentProps ) => {
	let heading = __( 'Checkout error', 'fincommerce' );

	if ( cartItemErrorCodes.includes( errorData.code ) ) {
		heading = __( 'There is a problem with your cart', 'fincommerce' );
	}
...
const ErrorMessage = ( { errorData }: ErrorComponentProps ) => {
	...
	if (
		...
		message =
			message +
			' ' +
			__( 'Please edit your cart and try again.', 'fincommerce' );
	}
...
const ErrorButton = ( { errorData }: ErrorComponentProps ) => {
	let buttonText = __( 'Retry', 'fincommerce' );
	...
	if (
		...
		buttonText = __( 'Edit your cart', 'fincommerce' );
	}
