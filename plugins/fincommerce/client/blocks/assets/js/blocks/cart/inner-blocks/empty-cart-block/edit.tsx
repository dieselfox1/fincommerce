	...
	? [
			'core/paragraph',
			{
				...
				content: sprintf(
					...
					__( '<a href="%s">Browse store</a>', 'fincommerce' ),
					SHOP_URL
				),
				dropCap: false,
			},
...
const defaultTemplate = [
	[
		...
		{
			textAlign: 'center',
			content: __( 'Your cart is currently empty!', 'fincommerce' ),
			level: 2,
	...
	[
		...
		{
			textAlign: 'center',
			content: __( 'New in store', 'fincommerce' ),
			level: 2,
		},
	],
	[
		'fincommerce/product-new',
		{
			columns: 4,
			rows: 1,
		},
...
export const Edit = ( { clientId }: { clientId: string } ): JSX.Element => {
	...
	return (
		<div
			{ ...blockProps }
			hidden={ currentView !== 'fincommerce/empty-cart-block' }
		>
			<InnerBlocks
