 ...
 */
import setStyles from '@fincommerce/block-library/assets/js/blocks/mini-cart/utils/set-styles';
import {
	formatPriceWithCurrency,
	normalizeCurrencyResponse,
} from '@fincommerce/block-library/packages/prices/utils/currency';
import { CartItem, Currency } from '@fincommerce/block-library/assets/js/types';

const universalLock =
	'I acknowledge that using a private store means my plugin will inevitably break on the next store release.';

const { currency, placeholderImgSrc } = getConfig( 'fincommerce' );
const {
	addToCartBehaviour,
...
} = getConfig( 'fincommerce/mini-cart' );
const {
	reduceQuantityLabel,
	...
	removeFromCartLabel,
	lowInStockLabel,
} = getConfig( 'fincommerce/mini-cart-products-table-block' );
...
const { state: fincommerceState, actions } = store< FinCommerce >(
	'fincommerce',
	{},
	{ lock: universalLock }
);
	...
	'fincommerce/mini-cart',
	{
		state: {
			get totalItemsInCart() {
				return fincommerceState.cart.items.reduce< number >(
			...
			get formattedSubtotal(): string {
				const subtotal = displayCartPriceIncludingTax
					? parseInt( fincommerceState.cart.totals.total_items, 10 ) +
					  parseInt(
							fincommerceState.cart.totals.total_items_tax,
							10
					  )
					: parseInt( fincommerceState.cart.totals.total_items, 10 );

				const normalizedCurrency = normalizeCurrencyResponse(
					fincommerceState.cart.totals,
					currency
				);
	...
	'fincommerce/mini-cart-products-table-block',
	{
		state: {
			...
			get cartItem() {
				const {
					cartItem: { id },
				} = getContext< CartItemContext >( 'fincommerce' );

				return fincommerceState.cart.items.find(
					( item ) => item.id === id
				) as CartItem;
			},
