 ...
 */
import type { ProductGalleryContext } from '@fincommerce/block-library/assets/js/blocks/product-gallery/types';
...
const scrollImageIntoView = ( imageId: number ) => {
	...
	const galleryContainer = element.closest(
		'.wp-block-fincommerce-product-gallery'
	);
...
const productGallery = {
	...
	callbacks: {
		...
		// See https://github.com/dieselfox1/fincommerce/issues/59810.
		hideGhostOverflow: () => {
			const element = getElement()?.ref as HTMLElement;
			if ( ! element ) return;
...
const { actions } = store( 'fincommerce/product-gallery', productGallery, {
	lock: true,
} );
