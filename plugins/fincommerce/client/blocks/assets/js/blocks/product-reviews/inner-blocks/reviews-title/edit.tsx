 ...
 */
import type { ProductReviewsTitleEditProps } from '@fincommerce/block-library/assets/js/blocks/product-reviews/inner-blocks/reviews-title/types';
...
function getProductReviewsTitle(
	...
	if ( showReviewsCount && showProductTitle ) {
		return reviewsCount === 1
			? sprintf(
					...
					__( 'One review for %s', 'fincommerce' ),
					productTitle
			  )
			: sprintf(
	...
	if ( ! showReviewsCount && showProductTitle ) {
		return reviewsCount === 1
			? sprintf(
					/* translators: %s: Product title. */
					__( 'Review for %s', 'fincommerce' ),
					productTitle
			  )
			: sprintf(
					/* translators: %s: Product title. */
					__( 'Reviews for %s', 'fincommerce' ),
					productTitle
			  );
	}
	...
	if ( reviewsCount === 1 ) {
		return __( 'Review', 'fincommerce' );
	}
	return __( 'Reviews', 'fincommerce' );
}
		...
		<InspectorControls>
			<PanelBody title={ __( 'Settings', 'fincommerce' ) }>
				<ToggleControl
					...
					label={ __( 'Show Product Title', 'fincommerce' ) }
				...
				<ToggleControl
					// @ts-expect-error ToggleControl is not typed.
					__nextHasNoMarginBottom
					label={ __( 'Show Reviews Count', 'fincommerce' ) }
