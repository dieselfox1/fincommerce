 ...
 */
import { QueryControlProps, CoreFilterNames } from '@fincommerce/block-library/assets/js/blocks/product-collection/types';
...
export const HandPickedProductsControlField = ( {
	...
	const selectedProductIds = query.fincommerceHandPickedProducts;
	...
	useEffect( () => {
		if ( validSelectedProductIds.length !== selectedProductIds.length ) {
			setQueryAttribute( {
				fincommerceHandPickedProducts: validSelectedProductIds,
			} );
		}
	...
	const onTokenChange = useCallback(
		( values: string[] ) => {
			...
			setQueryAttribute( {
				fincommerceHandPickedProducts: Array.from(
	...
	return (
		<FormTokenField
			...
			label={ __( 'Hand-Picked', 'fincommerce' ) }
	...
	const deselectCallback = () => {
		setQueryAttribute( {
			fincommerceHandPickedProducts:
				DEFAULT_FILTERS.fincommerceHandPickedProducts,
		} );
		trackInteraction( CoreFilterNames.HAND_PICKED );
	};
