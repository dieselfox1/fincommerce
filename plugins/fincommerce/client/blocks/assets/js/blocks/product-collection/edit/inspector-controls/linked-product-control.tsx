 ...
 */
import { useGetProduct } from '@fincommerce/block-library/assets/js/blocks/product-collection/utils';
import type {
...
const getFromCurrentProductRadioLabel = (
	...
	if ( currentLocation === REFERENCE_TYPE_CART && hasCartReference ) {
		return __( 'From products in the cart', 'fincommerce' );
	}

	if ( currentLocation === REFERENCE_TYPE_ORDER && hasOrderReference ) {
		return __( 'From products in the order', 'fincommerce' );
	}

	return __( 'From the current product', 'fincommerce' );
};
...
const LinkedProductControl = ( {
	...
	return (
		<PanelBody title={ __( 'Linked Product', 'fincommerce' ) }>
			{ showRadioControl && (
				<PanelRow>
					<RadioControl
						className="wc-block-product-collection-product-reference-radio"
						label={ __( 'Products to show', 'fincommerce' ) }
