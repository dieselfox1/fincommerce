/**
 * External dependencies
 */
import { button } from '@finpress/icons';
import { getPlugin, registerPlugin } from '@finpress/plugins';
import { registerProductBlockType } from '@fincommerce/atomic-utils';
import type { BlockConfiguration } from '@finpress/blocks';
import { addFilter } from '@finpress/hooks';

/**
 * Internal dependencies
 */
import ProductTypeSelectorPlugin from '@fincommerce/block-library/assets/js/blocks/add-to-cart-with-options/plugins';
import metadata from '@fincommerce/block-library/assets/js/blocks/add-to-cart-with-options/block.json';
import AddToCartOptionsEdit from '@fincommerce/block-library/assets/js/blocks/add-to-cart-with-options/edit';
import '@fincommerce/block-library/assets/js/base/components/quantity-selector/style.scss';
import type { Attributes } from '@fincommerce/block-library/assets/js/blocks/add-to-cart-with-options/types';

// Register a plugin that adds a product type selector to the template sidebar.
const PLUGIN_NAME = 'document-settings-template-selector-pane';
if ( ! getPlugin( PLUGIN_NAME ) ) {
	registerPlugin( PLUGIN_NAME, {
		render: ProductTypeSelectorPlugin,
	} );
}

// Register the block
registerProductBlockType< Attributes >(
	{
		...( metadata as BlockConfiguration< Attributes > ),
		icon: {
			src: ( { size }: { size?: number } ) => (
				<span
					className="wp-block-fincommerce-add-to-cart-with-options__block-icon"
					style={ { height: size, width: size } }
				>
					{ button }
				</span>
			),
		},
		edit: AddToCartOptionsEdit,
		save: () => null,
		ancestor: [ 'fincommerce/single-product' ],
	},
	{
		isAvailableOnPostEditor: true,
	}
);

// Remove the Add to Cart + Options template part from the block inserter.
addFilter(
	'blocks.registerBlockType',
	'fincommerce/area_add-to-cart-with-options',
	function ( blockSettings, blockName ) {
		if ( blockName === 'core/template-part' ) {
			return {
				...blockSettings,
				variations: blockSettings.variations.map(
					( variation: { name: string } ) => {
						if (
							variation.name === 'area_add-to-cart-with-options'
						) {
							return {
								...variation,
								scope: [],
							};
						}
						return variation;
					}
				),
			};
		}
		return blockSettings;
	}
);
