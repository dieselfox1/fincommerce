 ...
 */
import { getAttributeFromID } from '@fincommerce/block-library/assets/js/utils/attributes';
import { updateAttributeFilter } from '@fincommerce/block-library/assets/js/utils/attributes-query';
import { previewAttributeObject, previewOptions } from '@fincommerce/block-library/assets/js/blocks/attribute-filter/preview';
import '@fincommerce/block-library/assets/js/blocks/attribute-filter/style.scss';
import {
	formatParams,
	getActiveFilters,
	...
	formatSlug,
} from '@fincommerce/block-library/assets/js/blocks/attribute-filter/utils';
import { BlockAttributes, DisplayOption, GetNotice } from '@fincommerce/block-library/assets/js/blocks/attribute-filter/types';
import CheckboxFilter from '@fincommerce/block-library/assets/js/blocks/attribute-filter/checkbox-filter';
import { useSetWrapperVisibility } from '@fincommerce/block-library/assets/js/blocks/filter-wrapper/context';
...
const AttributeFilterBlock = ( {
	...
	/*
		...
		More info: https://github.com/dieselfox1/fincommerce-blocks/pull/6920#issuecomment-1222402482
	 */
						...
						<FormTokenField
							...
							messages={ {
								added: sprintf(
									...
									__( '%s filter added.', 'fincommerce' ),
									attributeObject.label
								),
								removed: sprintf(
									/* translators: %s is the attribute label. */
									__( '%s filter removed.', 'fincommerce' ),
									attributeObject.label
								),
								remove: sprintf(
									/* translators: %s is the attribute label. */
									__( 'Remove %s filter.', 'fincommerce' ),
									attributeObject.label.toLocaleLowerCase()
								),
								__experimentalInvalid: sprintf(
									/* translators: %s is the attribute label. */
									__( 'Invalid %s filter.', 'fincommerce' ),
									attributeObject.label.toLocaleLowerCase()
								),
							} }
						/>
				...
				{ blockAttributes.showFilterButton && (
					<FilterSubmitButton
						...
						screenReaderLabel={ sprintf(
							/* translators: %s is the attribute label */
							__( 'Apply attribute filter: %s', 'fincommerce' ),
							attributeObject.label
						) }
					/>
				) }
