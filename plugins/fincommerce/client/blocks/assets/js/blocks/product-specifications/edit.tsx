 ...
 */
import { ProductSpecificationsEditProps } from '@fincommerce/block-library/assets/js/blocks/product-specifications/types';
...
const Edit = ( {
	...
	const { dimensionUnit, weightUnit, isLoadingUnits } = useSelect(
		( select ) => {
			...
			return {
				...
				weightUnit: getOption( 'fincommerce_weight_unit' ) as string,
				isLoadingUnits:
	...
	const { hasInvalidContext, warningElement } =
		useQueryLoopProductContextValidation( {
			clientId,
			postType,
			blockName: __( 'Product Specifications', 'fincommerce' ),
		} );
	...
	if ( postId && ! product ) {
		return (
			<div { ...blockProps }>
				<p>{ __( 'No product found', 'fincommerce' ) }</p>
			</div>
		);
	}
	...
	if ( showAttributes ) {
		if ( isSpecificProductContext ) {
			...
			productData.test_attribute = {
				label: __( 'Test Attribute', 'fincommerce' ),
				value: __( 'First, Second, Third', 'fincommerce' ),
			};
		}
	}
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Display Settings', 'fincommerce' ) }>
					<ToggleControl
						label={ __( 'Show Weight', 'fincommerce' ) }
						checked={ showWeight }
						onChange={ () =>
					...
					/>
					<ToggleControl
						label={ __( 'Show Dimensions', 'fincommerce' ) }
					...
					/>
					<ToggleControl
						label={ __( 'Show Attributes', 'fincommerce' ) }
			...
			<figure { ...blockProps }>
				<table>
					<thead className="screen-reader-text">
						<tr>
							<th>{ __( 'Attributes', 'fincommerce' ) }</th>
							<th>{ __( 'Value', 'fincommerce' ) }</th>
						</tr>
					</thead>
					<tbody>
