 ...
 */
import { getCanMakePaymentArg } from '@fincommerce/block-library/assets/js/data/payment/utils/check-payment-methods';
import { CustomerPaymentMethodConfiguration } from '@fincommerce/block-library/assets/js/data/payment/types';
...
const getCcOrEcheckLabel = ( {
	...
	return sprintf(
		...
		__( '%1$s ending in %2$s (expires %3$s)', 'fincommerce' ),
...
const getDefaultLabel = ( {
	...
	if ( method.brand && method.last4 ) {
		return sprintf(
			...
			__( '%1$s ending in %2$s', 'fincommerce' ),
			method.brand,
			method.last4
		);
	}
	...
	return sprintf(
		...
		__( 'Saved token for %s', 'fincommerce' ),
		method.gateway
	);
};
