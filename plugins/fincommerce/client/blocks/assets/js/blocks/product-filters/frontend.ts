 ...
 */
import { decodeHtmlEntities } from '@fincommerce/block-library/assets/js/utils/html-entities';

const { getContext, store, getServerContext, getConfig } = iAPI;

const BLOCK_NAME = 'fincommerce/product-filters';
...
const productFiltersStore = {
	state: {
		get params() {
			...
			activeFilters.forEach( ( filter ) => {
				// todo: refactor this to use params data from Automattic\FinCommerce\Internal\ProductFilters\Params.
	...
	actions: {
		...
		// TODO: Remove the hardcoded type once https://github.com/dieselfox1/gutenberg/pull/8 is merged.
		*navigate(): Generator {
			...
			const sharedSettings = getConfig( 'fincommerce' );
