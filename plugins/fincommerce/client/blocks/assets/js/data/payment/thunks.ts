...
import {
	emitEventWithAbort,
	noticeContexts,
} from '@fincommerce/block-library/assets/js/base/context/event-emit';
import { EMIT_TYPES } from '@fincommerce/block-library/assets/js/base/context/providers/cart-checkout/payment-events/event-emit';
import type { emitProcessingEventType } from '@fincommerce/block-library/assets/js/data/payment/types';
import { store as cartStore } from '@fincommerce/block-library/assets/js/data/cart';
import {
	isBillingAddress,
	isShippingAddress,
} from '@fincommerce/block-library/assets/js/types/type-guards/address';
import { isObserverResponse } from '@fincommerce/block-library/assets/js/types/type-guards/observers';
import { isValidValidationErrorsObject } from '@fincommerce/block-library/assets/js/types/type-guards/validation';
		...
		return emitEventWithAbort(
			...
			observerResponses.forEach( ( response ) => {
				...
				if ( billingDataFromResponse ) {
					...
					deprecated(
						'returning billingData from an onPaymentProcessing observer in FinCommerce Blocks',
						{
							version: '9.5.0',
							alternative: 'billingAddress',
							link: 'https://github.com/dieselfox1/fincommerce-blocks/pull/6369',
						}
					);
				}
				...
				if (
					...
					deprecated(
						'returning shippingData from an onPaymentProcessing observer in FinCommerce Blocks',
						{
							version: '9.5.0',
							alternative: 'shippingAddress',
							link: 'https://github.com/dieselfox1/fincommerce-blocks/pull/8163',
						}
					);
				}
			} );
