...
import Summary from '@fincommerce/base-components/summary';
import { blocksConfig } from '@fincommerce/block-settings';
import { isEmpty, ProductResponseItem } from '@fincommerce/types';
 ...
 */
import '@fincommerce/block-library/assets/js/atomic/blocks/product-elements/summary/style.scss';
import type { BlockProps } from '@fincommerce/block-library/assets/js/atomic/blocks/product-elements/summary/types';
...
const Block = ( props: BlockProps ): JSX.Element | null => {
	...
	if ( ! source ) {
		return isDescendantOfAllProducts ? null : (
			<div className={ summaryClassName }>
				<p>{ __( 'No product summary to show.', 'fincommerce' ) }</p>
			</div>
		);
	}
