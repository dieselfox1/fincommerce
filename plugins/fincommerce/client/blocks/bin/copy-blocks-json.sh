...
# This script mimics the CopyWebpackPlugin behavior from the FinCommerce Blocks webpack configuration
# to make block.json files available for unit testing without requiring a full build.
# Ensure that the logic of this script is kept in sync with the logic of the CopyWebpackPlugin in the FinCommerce Blocks webpack configuration:
# https://github.com/dieselfox1/fincommerce/blob/84d1da7be3cbd3d8f40b17ad58729f668fd82b6a/plugins/fincommerce/client/blocks/bin/webpack-configs.js#L229-L256

# Move to the project root
while [ ! -d "plugins/fincommerce" ] || [ ! -f "pnpm-workspace.yaml" ]; do
    if [ "$PWD" = "/" ]; then
...
TARGET_DIR="plugins/fincommerce/assets/client/blocks"
...
find plugins/fincommerce/client/blocks/assets/js -name "block.json" | while read file; do
