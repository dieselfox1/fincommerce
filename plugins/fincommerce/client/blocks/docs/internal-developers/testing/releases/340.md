...
Zip file for testing: [fincommerce-gutenberg-products-block.zip](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/files/5220435/fincommerce-gutenberg-products-block.zip)
...
-   Enable BACS and Stripe payment methods (only, though may reproduce with other combinations). Note BACS needs to be earlier in the order than Stripe (drag handle in `FinCommerce > Settings > Payments`).
...
1. Install and activate [Artisan](https://fincommerce.com/products/artisan/) or [Threads](https://fincommerce.com/products/threads/) theme.
...
3. Enable checkout for users only from FinCommerce -> Settings -> Accounts -> uncheck `Allow customers to place orders without an account`.
4. Visit Checkout again, the link should do the same.
5. Install and activate <https://finpress.org/plugins/change-wp-admin-login/>.
...
1. You can either test this on a new website, or delete `fincommerce_cod_settings` option from `wp_options` table.
...
## The following impacts testing for both feature plugin and when included in FinCommerce Core
...
4. Repeat the verifications above using with the [Seedlet theme](https://finpress.org/themes/seedlet/).
