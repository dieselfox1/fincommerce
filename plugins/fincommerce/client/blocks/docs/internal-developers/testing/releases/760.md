...
Zip file for testing: [fincommerce-gutenberg-products-block.zip](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/files/8651068/fincommerce-gutenberg-products-block.zip)
...
-   Use the latest finpress version (we want to use the site editor / Global Styles in testing).
-   Install & use a block theme e.g.[ Twenty Twenty-Two](https://pcm.finpress.org/themes/twentytwentytwo/).
-   **Deactivate** the Gutenberg plugin for testing as we're currently observing [this issue](https://github.com/finpress/gutenberg/issues/40808).
...
2. Make sure FinCommerce is setup (WC pages are installed).
...
3. Above the FinCommerce Classic Template block, add the Filter Products by Attribute block (making sure you only have one Filter Products by Attribute block) and set its _Filter button_ attribute to true.
