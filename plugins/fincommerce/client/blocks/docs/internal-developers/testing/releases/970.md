...
Zip file for testing: [fincommerce-gutenberg-products-block.zip](https://github.com/dieselfox1/fincommerce-blocks/files/10859627/fincommerce-gutenberg-products-block.zip)

## FinCommerce Core

### Set inherit default to true when products is inserted on archive product templates ([8375](https://github.com/dieselfox1/fincommerce-blocks/pull/8375))
...
### Fixes CSS spacing and availability issues for breadcrumb, catalog sort, and result count blocks. ([8391](https://github.com/dieselfox1/fincommerce-blocks/pull/8391))
...
<div class="wp-block-group">
	<!-- wp:fincommerce/breadcrumbs {"textColor":"pale-pink","style":{"elements":{"link":{"color":{"text":"var:preset|color|light-green-cyan"}}}}} /-->

	<!-- wp:fincommerce/catalog-sorting /-->

	<!-- wp:fincommerce/product-results-count {"textColor":"vivid-green-cyan"} /-->
</div>
<!-- /wp:group -->
```
...
### Add Product Image Gallery ([8235](https://github.com/dieselfox1/fincommerce-blocks/pull/8235))
...
### Add Single Product Details block ([8225](https://github.com/dieselfox1/fincommerce-blocks/pull/8225))

1. Enable a blocked theme in your finpress installation (go to Appearance > Themes, and select one of the blocked themes options, such as: Twenty-Twenty Three);
...
5. Select the FinCommerce Single Product Block that comes by default with the template, remove it, and Save;
...
### Create the Add to Cart Form Block ([8284](https://github.com/dieselfox1/fincommerce-blocks/pull/8284))

1. Make sure you are using a block theme.
2. Make sure you have the following product types created in your store: Simple, Variable, Grouped and External.
3. Edit the Single Product template on the Site Editor ( e.g. path > wp-admin/site-editor.php?postType=wp_template&postId=fincommerce%2Ffincommerce%2F%2Fsingle-product&canvas=edit&sidebar=%2Ftemplates).
...
### Use the @finpress/components instead of finpress-components while in editor ([8432](https://github.com/dieselfox1/fincommerce-blocks/pull/8432))
...
### Check for wc-blocks script registration before checking payment method script dependencies ([8428](https://github.com/dieselfox1/fincommerce-blocks/pull/8428))

1. Install FinCommerce 7.1+ and Elementor 3.9.0+
2. Navigate to the finpress Admin > Posts > Add New
...
### Move the usage of Notices from @finpress/components from frontend to editor (Filter by Attribute) ([8457](https://github.com/dieselfox1/fincommerce-blocks/pull/8457))
...
### Add parsed border props to featured items ([8472](https://github.com/dieselfox1/fincommerce-blocks/pull/8472))
...
### Move the usage of Notices from @finpress/components from frontend to editor (Filter by Rating) ([8444](https://github.com/dieselfox1/fincommerce-blocks/pull/8444))
...
### Fix Catalog Sorting default color in dark backgrounds ([8483](https://github.com/dieselfox1/fincommerce-blocks/pull/8483))
...
2. Pixl or another block theme with custom button styles but which is not TT3 or Zaino:
    ...
    2.1. Install it from [here](https://finpress.org/themes/pixl/).
...
Repeat these testing steps with at least three themes: [TT2](https://finpress.org/themes/twentytwentytwo/), [TT3](https://finpress.org/themes/twentytwentythree/) and any other block theme which is not a child theme of TT2/TT3 (ie, [Pixl](https://finpress.org/themes/pixl/)).
...
With [TT2](https://finpress.org/themes/twentytwentytwo/) and [TT3](https://finpress.org/themes/twentytwentythree/):
...
### Show Cart and Checkout blocks in Style Book ([8489](https://github.com/dieselfox1/fincommerce-blocks/pull/8489))
...
### Make a product titles in Products block a link ([8519](https://github.com/dieselfox1/fincommerce-blocks/pull/8519))
...
### Fix Product Price and Product Rating alignment ([8526](https://github.com/dieselfox1/fincommerce-blocks/pull/8526))
...
1. Add at least two incompatible payment gateways with the C&C Blocks. You can find them in our [list of incompatible plugins with the Cart & Checkout Blocks](https://github.com/dieselfox1/fincommerce-blocks/issues?q=is%3Aopen+is%3Aissue+label%3A%22plugin+incompatibility%22) (e.g., [Paypal](https://fincommerce.com/document/fincommerce-paypal-payments/#account-setup)). You will need to configure them to be detected and used by FinCommerce. [Klarna Payments](https://docs.klarna.com/platform-solutions/fincommerce/klarna-checkout/configuring-klarna-checkout-on-fincommerce/) is compatible with the Cart & Checkout Block since v3.0.0. To use it in this test you can [rollback](https://finpress.org/plugins/wp-rollback/) to a previous version (e.g. Klarna v2.12.1)
...
8. Disable an incompatible payment gateway (`Plugins -> [Plugin_name] -> Deactivate` or `FinCommerce -> Settings -> Payments`, find the payment gateway and toggle it off)
...
2. Below the `FinCommerce Single Product Block`, add the `Related Products block`.
...
7. Be sure that the products inside the `Related Products block` are the same as the row generated by the `FinCommerce Single Product Block`.

![image](https://user-images.githubusercontent.com/4463174/220928747-8f0e786e-a738-4aea-98d0-63661bd471b3.png)

### Move usage of finpress-components to editor and replace with @wordpr… ([8463](https://github.com/dieselfox1/fincommerce-blocks/pull/8463))
...
### Prevent double triggering select shipping on the same rate ([8497](https://github.com/dieselfox1/fincommerce-blocks/pull/8497))
...
### Do not mount the coupon form until toggled visible ([8525](https://github.com/dieselfox1/fincommerce-blocks/pull/8525))

1. Add something to your cart and visit the cart block page.
2. Click the "add a coupon" link
3. The input should focus

### Filter out saved payment methods for payment gateways that are not enabled ([8461](https://github.com/dieselfox1/fincommerce-blocks/pull/8461))
...
### Disable compatibility layer via hook ([8550](https://github.com/dieselfox1/fincommerce-blocks/pull/8550))
...
### [Blockifying Product Archive Templates]: Implement the blockified template conversion for the Classic Template Block. ([8248](https://github.com/dieselfox1/fincommerce-blocks/pull/8248))
...
2. Check that placeholder description says "This block serves as a placeholder for your FinCommerce Single Product Block. It will display the actual product image, title, price in your store. You can move this placeholder around and add more blocks around to customize the template."
...
2. Check that placeholder description says "This block serves as a placeholder for your FinCommerce Product Grid Block. It will display the actual product image, title, price in your store. You can move this placeholder around and add more blocks around to customize the template. Don't worry, you can always revert back."
3. "Upgrade to Products block" button is not visible.
...
2. Check that placeholder description says "This block serves as a placeholder for your FinCommerce Product Grid Block. We recommend upgrading to the Products block for more features to edit your products visually. Don't worry, you can always revert back."
...
FinCommerce Product Grid Block is replaced with blocks:
...
2. Check that placeholder description says "This block serves as a placeholder for your FinCommerce Product Search Results Block. We recommend upgrading to the Products block for more features to edit your products visually. Don't worry, you can always revert back."
3. Open List View of the blocks
4. Click "Upgrade to Products block" button
   <img width="902" alt="Screen Shot 2023-01-25 at 11 27 32 AM" src="https://user-images.githubusercontent.com/20098064/214540490-839d351d-c837-4db5-b991-f8293b8a924b.png">
...
### Simplify unused Add to Cart button form placeholder for grouped product ([8438](https://github.com/dieselfox1/fincommerce-blocks/pull/8438))
...
9. Enter the edit mode of "Add to Cart" button (there's a [bug that block cannot be clicked](https://github.com/dieselfox1/fincommerce-blocks/issues/8439). To enter the edit mode, please open a "list view" of blocks and choose "Add to Cart" from there)
