...
Zip file for testing: [fincommerce-gutenberg-products-block.zip](https://github.com/dieselfox1/fincommerce-blocks/files/11325682/fincommerce-gutenberg-products-block.zip)

## FinCommerce Core

### [Patterns]: Add new Featured Products 2 columns pattern ([#9072](https://github.com/dieselfox1/fincommerce-blocks/pull/9072))

1. Create a new page or post
2. Make sure the `Featured products 2 columns` pattern appears under the FinCommerce category dropdown.
3. Insert in and make sure it shows as expected on the design.

### [Patterns] Add new Testimonials: 3 columns pattern ([#9159](https://github.com/dieselfox1/fincommerce-blocks/pull/9159))

1. Create a new page or post
2. Make sure the `Testimonials 3 columns` pattern appears under the FinCommerce category dropdown.
3. Insert in and make sure it shows as expected on the design.

### Reinstate "Hide shipping costs until an address is entered" but disable it when using local pickup ([#8964](https://github.com/dieselfox1/fincommerce-blocks/pull/8964))

1. Ensure you have some shipping rates set up in your store.
2. Disable WC Blocks local pickup (FinCommerce -> Shipping -> Local pickup).
3. Go to FinCommerce -> Shipping -> Shipping options, ensure you see the option "Hide shipping costs until an address is entered" with no additional text.
4. Check this box.
...
8. Enable WC Blocks local pickup and add a location (FinCommerce -> Shipping -> Local pickup)
9. Go to FinCommerce -> Shipping -> Shipping options, ensure you see the option "Hide shipping costs until an address is entered (Not available when using FinCommerce Blocks Local Pickup)" exactly as written.
...
1. Install and activate a block theme, e.g. [Twenty Twenty-Three](https://finpress.org/themes/twentytwentythree/).
...
### Fix filter by stock status dropdown ordering ([#9127](https://github.com/dieselfox1/fincommerce-blocks/pull/9127))
...
### fix/8129: remove rounding the caused issue with UI ([#9118](https://github.com/dieselfox1/fincommerce-blocks/pull/9118))
...
### add cursor:pointer to checkbox and radio fields ([#9102](https://github.com/dieselfox1/fincommerce-blocks/pull/9102))
...
### fix/8651: Use body background color as the Cart block sticky footer background color ([#9103](https://github.com/dieselfox1/fincommerce-blocks/pull/9103))
...
### decodeEntities for display in checkout TextInput ([#9117](https://github.com/dieselfox1/fincommerce-blocks/pull/9117))
...
### fix/8946: replace ToggleGroupControl with ToggleControl ([#9098](https://github.com/dieselfox1/fincommerce-blocks/pull/9098))
...
### fix/8363: Match editor "Place Order" button UI with frontend ([#9094](https://github.com/dieselfox1/fincommerce-blocks/pull/9094))
...
1. Install the FinCommerce Block plugin on a finpress site.
2. Create a product and add it to the site's shopping cart.
3. Proceed to checkout using the "Checkout" Block, which can be added to a page using the "FinCommerce" block category.
...
### Get product by ID if there's more products returned from useStoreProducts ([#9079](https://github.com/dieselfox1/fincommerce-blocks/pull/9079))
...
### Product SKU: fix product SKU when added in the product meta block ([#8966](https://github.com/dieselfox1/fincommerce-blocks/pull/8966))
...
### Pass proper data to On Sale badge when used in the scope of Product Image ([#9021](https://github.com/dieselfox1/fincommerce-blocks/pull/9021))
...
### Use consistent and semantically correct HTML elements in the Cart and Checkout blocks ([#9065](https://github.com/dieselfox1/fincommerce-blocks/pull/9065))
...
### Mini Cart: Remove excessive margin under cart items ([#9051](https://github.com/dieselfox1/fincommerce-blocks/pull/9051))
...
### Fix Mini Cart drawer not opening in RTL locales ([#9066](https://github.com/dieselfox1/fincommerce-blocks/pull/9066))
...
### Mini Cart template part: give user customized template priority over theme template ([#9005](https://github.com/dieselfox1/fincommerce-blocks/pull/9005))

0. With [TT3](https://github.com/finpress/twentytwentythree/), add the Mini Cart block to your store header via Appearance > Editor.
...
3. Now, let's imagine TT3 releases an update which introduces a custom Mini Cart template part. To do so, install this ZIP as a theme (it's TT3 with a `parts/mini-cart.html` file): [twentytwentythree.zip](https://github.com/dieselfox1/fincommerce-blocks/files/11200877/twentytwentythree.zip).
...
### Display Mini Cart overlay in template part editor ([#9014](https://github.com/dieselfox1/fincommerce-blocks/pull/9014))
...
2. Enter the `Mini Cart` template (wp-admin/site-editor.php?postId=fincommerce%2Ffincommerce%2F%2Fmini-cart&postType=wp_template_part&canvas=edit).
...
### Customer Account block: Fix padding for the block in the Editor ([#9009](https://github.com/dieselfox1/fincommerce-blocks/pull/9009))
...
### Filter data count mismatch > Create the get_attribute_and_meta_counts method ([#8599](https://github.com/dieselfox1/fincommerce-blocks/pull/8599))
...
### Create specific stylesheet for CartLineItemsTable component ([#8967](https://github.com/dieselfox1/fincommerce-blocks/pull/8967))
...
### Mini cart: allow changing the drawer width ([#8930](https://github.com/dieselfox1/fincommerce-blocks/pull/8930))
