...
Zip file for testing: [fincommerce-gutenberg-products-block.zip](https://github.com/dieselfox1/fincommerce-blocks/files/10557868/fincommerce-gutenberg-products-block.zip)

## FinCommerce Core

### Style Book: Fix Featured Product and Featured Category Blocks preview ([8313](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8313))
...
### Style Book: Hide Filter blocks and Product Search block ([8309](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8309))
...
### Style Book: Fix useForcedLayout to prevent breaking style book ([8243](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8243))
...
### Add border attributes to featured items block to show border styles in editor ([8304](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8304))
...
### Fix for wrong shipping schema in checkout ([8291](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8291))

1. Create and place an order with shipping address different from billing.
2. Verify on checkout that both display correctly.

### Adjust return to cart arrow ([8289](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8289))

1. Select a dark theme, e.g. the [Twenty Twenty-Three](https://finpress.org/themes/twentytwentythree/) and use a dark style.
...
### Add reset button in the editor for product attributer filter ([8285](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8285))
...
### Make `Notice` component legible in dark themes ([8278](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8278))
...
### Set inherit setting to true when is inserted in the archive product template ([8251](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8251))

1. Make sure you are using a block theme.
2. Go to the `Product Catalog` template on the `Site Editor` (`wp-admin/site-editor.php?postType=wp_template&postId=fincommerce%2Ffincommerce%2F%2Farchive-product`).
3. Insert the `Products (Beta)` block.
4. Make sure the `Inherit query from template` is enabled by default.

### Avoid changing the markup of all ratings when the Product Rating block is present in the page ([8247](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8247))

1. Make sure a product has an average rating of 4 or less.
2. With the [Twenty Twenty-Three](https://finpress.org/themes/twentytwentythree/) theme installed, go to `Appearance » Editor`.
3. Edit the Product Catalog template, adding the Products block above the FinCommerce Product Grid Block:
...
5. Go to the frontend and verify the rating markup of the FinCommerce Product Grid block is not filtered. To verify it, scroll down to the FinCommerce Product Grid block and check that the "empty" star in the rating is outlined instead of dimmed:
...
> Note: The _Products_ block is expected to have dimmed stars instead of outlined ones. This testing steps refers to the FinCommerce Product Grid block.

### Update Local Pickup price display when price is 0 and multiple packages are used ([8241](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8241))

1. Enable the [Multiple Packages for FinCommerce](https://finpress.org/plugins/multiple-packages-for-fincommerce/) extension and set a price for local pickup.
...
### Product Price: Transition from using CSS margin to Global Styles ([8269](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8269))
...
### Product Button: Transition from using CSS margin to Global Styles ([8239](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8239))
...
### Product Rating: Transition from using CSS margin to Global Styles ([8202](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8202))
...
### Add Store Breadcrumbs block ([8222](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8222))

1. Make sure you are using a block theme.
2. Edit any template on the Site Editor e.g. the `Single Product Template` (`wp-admin/site-editor.php?postType=wp_template&postId=fincommerce%2Ffincommerce%2F%2Fsingle-product&canvas=edit&sidebar=%2Ftemplates`).
...
### Add Results Count block ([8078](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8078))
...
### Add coupon link instead of dropdown in Cart and Checkout blocks ([7993](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/7993))
...
### Checkout/Store API: Push valid changes selectively ([8030](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8030))
...
### Refresh the cart when using the browser back button ([8236](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/8236))
