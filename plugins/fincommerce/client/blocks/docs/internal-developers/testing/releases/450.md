...
Zip file for testing: [fincommerce-gutenberg-products-block.zip](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/files/5988965/fincommerce-gutenberg-products-block.zip)
...
1. Ensure you have the "My Account" page setup in FinCommerce
...
2. Install <https://wordpress.org/plugins/portugal-states-distritos-for-fincommerce/> Confirm states appear for portugal
...
1. Add this code snippet to any PHP file (ie: `/fincommerce-gutenberg-products-block.php`):
...
### Show cart item total price including taxes when DISPLAY_CART_PRICES_INCLUDING_TAX is true ([3851](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/3851))

In wp-admin:

1. Go to FinCommerce > Settings and check the option `Enable tax rates and calculations`.
2. Go to FinCommerce > Settings > Tax and set `Prices entered with tax` to `No, I will enter prices exclusive of tax` and `Display prices during cart and checkout` to `Including tax`.
3. Go to FinCommerce > Settings > Tax > Standard Rates and create a tax rate for a specific country.
...
### Fix product price not displaying properly when product is on sale ([3853](https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/3853))
