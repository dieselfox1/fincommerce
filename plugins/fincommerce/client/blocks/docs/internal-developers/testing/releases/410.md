...
## Compatibility with finpress 5.6 (both feature plugin and what is included with package inclusion in core)
...
```sh
POST https://one.finpress.test/wp-json/wc/store/cart/add-item
```
...
```sh
POST https://one.finpress.test/wp-json/wc/store/checkout
```
