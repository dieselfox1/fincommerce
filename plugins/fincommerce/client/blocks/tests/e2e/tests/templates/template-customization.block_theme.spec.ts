...
test.describe( 'Template customization', () => {
	CUSTOMIZABLE_WC_TEMPLATES.forEach( ( testData ) => {
		...
		test( `"${ testData.templateName }" template can be modified and reverted`, async ( {
			...
			// See: https://github.com/dieselfox1/fincommerce/issues/42221
			await expect(
	...
	for ( const testData of testToRun ) {
		const userText = `Hello World in the ${ testData.templateName } template`;
		const fincommerceTemplateUserText = `Hello World in the FinCommerce ${ testData.templateName } template`;

		test( `user-modified "${ testData.templateName }" template based on the theme template has priority over the user-modified template based on the default FinCommerce template`, async ( {
			page,
			admin,
			editor,
			requestUtils,
			...
			await editor.insertBlock( {
				name: 'core/paragraph',
				attributes: { content: fincommerceTemplateUserText },
			} );
			await editor.saveSiteEditorEntities( {
				isOnlyCurrentEntityDirty: true,
			} );
			...
			await expect( page.getByText( userText ).first() ).toBeVisible();
			await expect(
				page.getByText( fincommerceTemplateUserText )
			).toBeHidden();
			...
			await expect(
				page.getByText( fincommerceTemplateUserText ).first()
			).toBeVisible();
			await expect( page.getByText( userText ) ).toBeHidden();
		} );
	}
