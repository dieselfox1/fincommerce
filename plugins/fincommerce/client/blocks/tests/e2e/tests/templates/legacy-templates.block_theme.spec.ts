 ...
 */
import { test, expect, wpCLI } from '@fincommerce/e2e-utils';

test.describe( 'Legacy templates', () => {
	test( 'fincommerce//* slug is supported', async ( {
		admin,
		page,
		editor,
	} ) => {
		...
		await test.step( 'Update created term to legacy format in the DB', async () => {
			await wpCLI(
				`term update wp_theme fincommerce-fincommerce \
					--by="slug" \
					--name="fincommerce" \
					--slug="fincommerce"`
			);
		...
		await test.step( 'Verify the template can be edited via a legacy ID ', async () => {
			await admin.visitSiteEditor( {
				postId: `fincommerce//${ template.id }`,
				postType: 'wp_template',
				canvas: 'edit',
			} );
