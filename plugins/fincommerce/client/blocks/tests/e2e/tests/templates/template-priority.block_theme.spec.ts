...
test.describe( 'Template priority', () => {
	...
	// 2. Template from the database with the FinCommerce slug.
	// 3. Fallback template from the database with the theme slug.
	// 4. Fallback template from the database with the FinCommerce slug.
	// 5. Template from the theme.
	// 6. Fallback template from the theme.
	// 7. Template from FinCommerce.
	...
	templatesToTest.forEach( ( testData ) => {
		test( `priorities are applied correctly in the ${ testData.templateName } template`, async ( {
			...
			await test.step( 'FinCommerce template', async () => {
			...
			await test.step( 'custom template with FinCommerce slug', async () => {
				...
				await expect(
					page.getByText( 'Custom template with FinCommerce slug' )
				).toBeVisible();
			} );
