 ...
 */
import { expect, test } from '@fincommerce/e2e-utils';
...
test.describe( `${ blockData.name } Block`, () => {
	...
	test( 'the filter `fincommerce_product_add_to_cart_text` should be applied', async ( {
		requestUtils,
		frontendUtils,
	} ) => {
		await requestUtils.activatePlugin(
			'fincommerce-blocks-test-custom-add-to-cart-button-text'
		);
		await frontendUtils.goToShop();
		const blocks = await frontendUtils.getBlockByName( blockData.slug );
