...
async function getInlineStyles( page: Page ) {
	const styleLocators = page.locator(
		'style[id^="fincommerce-"][id$="-style-inline-css"]'
	);
...
test.describe( 'Block Style Loading in Classic Themes', () => {
	...
	test( 'should not load unnecessary block styles on pages without FinCommerce blocks', async ( {
		page,
		admin,
		editor,
	} ) => {
		...
		await editor.insertBlock( {
			name: 'core/paragraph',
			attributes: {
				content: 'This is a regular page with no FinCommerce blocks.',
			},
		} );
	...
	test( 'should load base FinCommerce styles when blocks are present', async ( {
		page,
		admin,
		editor,
	} ) => {
		...
		await editor.canvas
			.getByRole( 'textbox', { name: 'Add title' } )
			.fill( 'Test Page With FinCommerce Block' );
