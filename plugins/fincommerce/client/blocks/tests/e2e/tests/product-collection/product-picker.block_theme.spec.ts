 ...
 */
import { test as base, expect, BLOCK_THEME_SLUG } from '@fincommerce/e2e-utils';
...
test.describe( 'Product Collection: Product Picker', () => {
	const CUSTOM_COLLECTIONS = [
		{
			...
			collection:
				'fincommerce/product-collection/my-custom-collection-product-context',
		},
		{
			...
			collection:
				'fincommerce/product-collection/my-custom-collection-multiple-contexts',
		},
	];
	...
	test.beforeEach( async ( { requestUtils } ) => {
		await requestUtils.activatePlugin(
			'fincommerce-blocks-test-register-product-collection'
		);
	} );
