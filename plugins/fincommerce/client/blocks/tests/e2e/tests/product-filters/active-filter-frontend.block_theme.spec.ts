 ...
 */
import { TemplateCompiler, test as base, expect } from '@fincommerce/e2e-utils';
...
test.describe( 'fincommerce/product-filter-active - Frontend', () => {
	...
	test( 'Without any filters selected, active block should not be rendered', async ( {
		...
		const chips = page.locator(
			'.wp-block-fincommerce-product-filter-active'
		);
