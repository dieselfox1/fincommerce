...
const blockData = {
	...
	placeholderUrl: `${ BASE_URL }/wp-content/plugins/fincommerce/assets/images/placeholder.webp`,
};
...
test.describe( `${ blockData.name } Block - editor side`, () => {
	...
	test( "should allow changing the block's title", async ( { editor } ) => {
		const textSelector =
			'.wp-block-fincommerce-filter-wrapper .wp-block-heading';
...
test.describe( `${ blockData.name } Block - with All products Block`, () => {
	test.beforeEach( async ( { admin, page, editor } ) => {
		await admin.createNewPost();
		await editor.insertBlock( { name: 'fincommerce/all-products' } );
		await editor.insertBlock( {
			name: 'fincommerce/filter-wrapper',
			attributes: {
				filterType: 'price-filter',
...
test.describe( `${ blockData.name } Block - with Product Collection`, () => {
	test( 'should show all products', async ( { page, templateCompiler } ) => {
		...
		await page.goto( '/shop' );
		const products = page
			.locator( '.wp-block-fincommerce-product-template' )
			.getByRole( 'listitem' );
