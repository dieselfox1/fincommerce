...
test.describe( 'Shopper (logged-in) → Order Confirmation', () => {
	...
	test( 'Place order', async ( {
		...
		const DownloadSection = page.locator(
			'[data-block-name="fincommerce/order-confirmation-downloads"]'
		);
		...
		await test.step( 'Logout the user and revisit the order received page to verify that details are displayed when fincommerce_order_received_verify_known_shoppers is disabled', async () => {
			await requestUtils.activatePlugin(
				'fincommerce-blocks-test-order-confirmation-filters'
			);
			await page.goto( '/my-account' );
			await page
				.locator(
					'li.fincommerce-MyAccount-navigation-link--customer-logout a'
				)
				.click();
			await expect(
...
test.describe( 'Shopper (guest) → Order Confirmation → Create Account', () => {
	...
	test( 'Delayed account creation flows', async ( {
		...
		await requestUtils.rest( {
			method: 'PUT',
			path: 'wc/v3/settings/account/fincommerce_enable_delayed_account_creation',
			data: { value: 'no' },
		} );
		await page.reload();
