...
const blockData: BlockData = {
	name: 'Checkout',
	slug: 'fincommerce/checkout',
	mainClass: '.wp-block-fincommerce-checkout',
	selectors: {
		editor: {
			block: '.wp-block-fincommerce-checkout',
...
test.describe( 'Shopper → Account (guest user)', () => {
	...
	test.skip( 'Shopper can log in to an existing account and can create an account', async ( {
		...
		await requestUtils.rest( {
			method: 'PUT',
			path: 'wc/v3/settings/account/fincommerce_enable_signup_and_login_from_checkout',
			data: { value: 'yes' },
		} );
		await requestUtils.rest( {
			method: 'PUT',
			path: 'wc/v3/settings/account/fincommerce_registration_generate_password',
			data: { value: 'yes' },
		} );
	...
	test.beforeEach( async ( { admin, editor, page } ) => {
		...
		await editor.selectBlocks(
			blockSelectorInEditor +
				'  [data-type="fincommerce/checkout-shipping-address-block"]'
		);
...
test.describe( 'Shopper → Place Virtual Order', () => {
	test.beforeEach( async ( { requestUtils } ) => {
		await requestUtils.rest( {
			method: 'PUT',
			path: 'wc/v3/settings/general/fincommerce_ship_to_countries',
			data: { value: 'disabled' },
		} );
	} );
