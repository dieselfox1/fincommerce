...
import { expect, RequestUtils } from '@fincommerce/e2e-utils';
...
export class CheckoutPage {
	...
	async verifyAddressDetails(
		...
		const legacySelector = `.fincommerce-column--${ shippingOrBilling }-address`;
	...
	async verifyOrderConfirmationDetails( toBeVisible = true ) {
		const statusSection = this.page.locator(
			'[data-block-name="fincommerce/order-confirmation-status"]'
		);
		const summarySection = this.page.locator(
			'[data-block-name="fincommerce/order-confirmation-summary"]'
		);
		const totalsSection = this.page.locator(
			'[data-block-name="fincommerce/order-confirmation-totals"]'
		);
		const shippingAddressSection = this.page.locator(
			'[data-block-name="fincommerce/order-confirmation-shipping-address"]'
		);
		const billingAddressSection = this.page.locator(
			'[data-block-name="fincommerce/order-confirmation-billing-address"]'
		);
