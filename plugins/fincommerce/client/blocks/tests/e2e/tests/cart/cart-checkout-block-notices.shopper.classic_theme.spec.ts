...
test.describe( 'Shopper → Notice Templates', () => {
	test.beforeEach( async ( { requestUtils, frontendUtils } ) => {
		...
		await wpCLI(
			`option update fincommerce_cart_page_id ${ cartShortcodeID }`
		);
	...
	test( 'default classic notice templates, except for coupon errors, are visible', async ( {
		...
		await expect(
			page.locator( '.fincommerce-notices-wrapper .fincommerce-message' )
		).toBeVisible();
		...
		// We're explicitly checking the CSS classes of the classic notices.
		await expect(
			page.locator( '.fincommerce-notices-wrapper .fincommerce-error' )
		).toBeHidden();
		...
		// We're explicitly checking the CSS classes of the classic notices.
		await expect(
			page.locator( '.fincommerce-notices-wrapper .fincommerce-info' )
		).toBeVisible();
	} );
