 ...
 */
import { expect, test as base } from '@fincommerce/e2e-utils';
...
test.describe( 'Shopper → Taxes', () => {
	test( 'Tax visibility on Cart/Checkout/OrderSummary blocks depends on "Enable tax rate calculations" option in WC settings -> general', async ( {
		...
		await requestUtils.rest( {
			method: 'PUT',
			path: 'wc/v3/settings/general/fincommerce_calc_taxes',
			data: { value: 'no' },
		} );
		...
		let cartSidebar = page.locator(
			'.wp-block-fincommerce-cart-totals-block'
		);
		...
		let checkoutSidebar = page.locator(
			'.wp-block-fincommerce-checkout-totals-block'
		);
		...
		cartSidebar = page.locator( '.wp-block-fincommerce-cart-totals-block' );
