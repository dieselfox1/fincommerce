	...
	test( 'renders as expected', async ( { page, editor } ) => {
		await test.step( 'in editor', async () => {
			const productGalleryBlock = editor.canvas.locator(
				'[data-type="fincommerce/product-gallery"]'
			);

			await expect(
				productGalleryBlock.locator(
					'[data-type="fincommerce/product-gallery-thumbnails"]'
				)
			).toBeVisible();

			await expect(
				productGalleryBlock.locator(
					`[data-type="fincommerce/product-gallery-thumbnails"]:left-of(
						[data-type="fincommerce/product-gallery-large-image"]
					)`
				)
			).toBeVisible();
		} );
		...
		await test.step( 'in frontend', async () => {
			...
			const productGalleryBlock = page.locator(
				'[data-block-name="fincommerce/product-gallery"]'
			);

			const thumbnailsContainer = productGalleryBlock.locator(
				'[data-block-name="fincommerce/product-gallery-thumbnails"]'
			);
			...
			await expect(
				productGalleryBlock.locator(
					`[data-block-name="fincommerce/product-gallery-thumbnails"]:left-of(
						[data-block-name="fincommerce/product-gallery-large-image"]
					)`
				)
			).toBeVisible();
