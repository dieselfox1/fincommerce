...
import { Editor, FrontendUtils } from '@fincommerce/e2e-utils';
...
export class ProductGalleryPage {
	...
	async getMainImageBlock( { page }: { page: 'frontend' | 'editor' } ) {
		const blockName = 'fincommerce/product-gallery-large-image';
		if ( page === 'frontend' ) {
			return (
	...
	async getThumbnailsBlock( { page }: { page: 'frontend' | 'editor' } ) {
		const blockName = 'fincommerce/product-gallery-thumbnails';
		if ( page === 'frontend' ) {
			return (
				await this.frontendUtils.getBlockByName( blockName )
			 ).filter( {
	...
	async getBlock( { page }: { page: 'frontend' | 'editor' } ) {
		const blockName = 'fincommerce/product-gallery';
		if ( page === 'frontend' ) {
			return (
				await this.frontendUtils.getBlockByName( blockName )
			 ).filter( {
	...
	async getAddToCartWithOptionsBlock( {
		...
		const blockName = 'fincommerce/add-to-cart-form';
		if ( page === 'frontend' ) {
			return (
				await this.frontendUtils.getBlockByName( blockName )
			 ).filter( {
