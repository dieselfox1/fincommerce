 ...
 */
import { expect, test as base, wpCLI } from '@fincommerce/e2e-utils';
...
test.describe( 'Shopper → Notices', () => {
	test( 'Shopper can add item to cart, and will not see a notice in the mini cart', async ( {
		...
		const miniCart = await editor.getBlockByName( 'fincommerce/mini-cart' );
...
test.describe( 'Shopper → Tax', () => {
	test.beforeEach( async () => {
		await wpCLI( 'option set fincommerce_prices_include_tax no' );
		await wpCLI( 'option set fincommerce_tax_display_cart incl' );
	} );
	...
	test( 'User can see tax label and price including tax', async ( {
		...
		// see https://github.com/dieselfox1/fincommerce/issues/43691
		await page
		...
		await wpCLI( 'option set fincommerce_prices_include_tax yes' );
		await wpCLI( 'option set fincommerce_tax_display_cart excl' );
		await page.reload();
