...
Right now, installing and activating FinCommerce and a few extensions quickly results in a cascade of notices vying for the store administrator’s attention (and pushing whatever wp-admin content they were trying to get to on a page down, way down.)

As part of the Dashboard and Analytics improvements coming to FinCommerce, we’re going to do something a bit dramatic to try and clean up this notice-palooza.
...
This section is dedicated to informational content coming from multiple sources such as FinCommerce core, FinCommerce.com Subscription management, extensions activity and store achievements. This section was also envisioned to display more insightful content in the future, e.g. content that could help with the day to day tasks of managing and optimizing a store.
...
1. Notes can have 0, 1, or 2 action buttons. Action buttons have URLs. They can either be complete URLs (like in the case of an external link) or partial URLs (like in the case of an admin page on the site) - in the case of partial URLs you’ll want to give a string that is the same as what you’d supply to finpress core’s admin_url function. You can even include query parameters in that string.
1. You should store your extension’s name (slug) in the source property of the note. FinCommerce will store ‘fincommerce’ there. For example, the “Panda Payments” extension could use a name like “panda-payments” - this makes it easier to use built in functions to get (or delete) all notes with a certain name.
...
<?php
/**
 * Plugin Name: FinCommerce Activity Panel Inbox Example Plugin One
...
use Automattic\FinCommerce\Admin\Notes\Notes as Notes;
use Automattic\FinCommerce\Admin\Notes\Note as Note;

class fincommerce_Activity_Panel_Inbox_Example_Plugin_One {
	...
	public static function add_activity_panel_inbox_welcome_note() {
		...
		$note->add_action(
			...
			'https://github.com/dieselfox1/fincommerce-admin/tree/main/docs'
		);
		$note->save();
	}
...
function wapi_example_one_activate() {
	fincommerce_Activity_Panel_Inbox_Example_Plugin_One::add_activity_panel_inbox_welcome_note();
}
register_activation_hook( __FILE__, 'wapi_example_one_activate' );

function wapi_example_one_deactivate() {
	fincommerce_Activity_Panel_Inbox_Example_Plugin_One::remove_activity_panel_inbox_notes();
}
...
<?php
/**
 * Plugin Name: FinCommerce Activity Panel Inbox Example Plugin Two
 * Plugin URI: https://fincommerce.com/
 * Description: An example plugin.
 * Author: Automattic
 * Author URI: https://fincommerce.com/
...
class fincommerce_Activity_Panel_Inbox_Example_Plugin_Two {
...
function admin_init() {
	fincommerce_Activity_Panel_Inbox_Example_Plugin_Two::add_or_update_activity_panel_inbox_note();
}
add_action( 'admin_init', 'admin_init' );

function wapi_example_two_deactivate() {
	fincommerce_Activity_Panel_Inbox_Example_Plugin_Two::remove_activity_panel_inbox_notes();
}
...
This is just the tip of the iceberg for possibilities for your own extensions to FinCommerce. Check
out the new [sales record notes](https://github.com/dieselfox1/fincommerce-admin/blob/main/includes/notes/class-wc-admin-notes-new-sales-record.php) and the [settings notes](https://github.com/dieselfox1/fincommerce-admin/blob/main/includes/notes/class-wc-admin-notes-settings-notes.php) in the fincommerce-admin code itself for other examples of working with this fun new feature.
