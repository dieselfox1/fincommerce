...
jest.mock( '@fincommerce/data', () => {
	const originalModule = jest.requireActual( '@fincommerce/data' );
	return {
		...originalModule,
...
jest.mock( '@fincommerce/components', () => ( {
	...jest.requireActual( '@fincommerce/components' ),
...
jest.mock( '~/utils/admin-settings', () => {
	...
	return {
		...
		getAdminSetting: jest.fn().mockImplementation( ( name, ...args ) => {
			if ( name === 'fincommerceTranslation' ) {
				return 'FinCommerce';
			}
...
jest.mock( '@fincommerce/navigation', () => ( {
	...jest.requireActual( '@fincommerce/navigation' ),
	getHistory: jest.fn(),
} ) );
