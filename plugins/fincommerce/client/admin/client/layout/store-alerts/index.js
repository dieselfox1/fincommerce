...
export const StoreAlerts = () => {
	...
	const {
		...
		return {
			...
			defaultHomescreenLayout:
				getOption( 'fincommerce_default_homepage_layout' ) ||
				'single_column',
		};
	} );
	...
	function renderActions( alert ) {
		...
		const snoozeOptions = [
			{
				...
				label: __( 'Later Today', 'fincommerce' ),
			},
			{
				value: moment()
					.add( 1, 'day' )
			...
			},
			{
				value: moment()
					...
					.unix()
					.toString(),
				label: __( 'Next Week', 'fincommerce' ),
			},
			{
				value: moment()
					.add( 1, 'month' )
					...
					.unix()
					.toString(),
				label: __( 'Next Month', 'fincommerce' ),
			},
		];
		...
		const snooze = alert.is_snoozable && (
			<SelectControl
				className="fincommerce-store-alerts__snooze"
				options={ [
					{
						label: __( 'Remind Me Later', 'fincommerce' ),
						value: '0',
					},
					...snoozeOptions,
				] }
		...
		if ( actions || snooze ) {
			return (
				<div className="fincommerce-store-alerts__actions">
					{ actions }
					{ snooze }
				</div>
			);
	...
	const className = clsx( 'fincommerce-store-alerts', {
	...
	const onDismiss = async ( note ) => {
		...
		try {
			...
			createNotice( 'success', __( 'Message dismissed', 'fincommerce' ) );
		} catch ( e ) {
			createNotice(
				'error',
				_n(
	...
	return (
		<Card className={ className } size={ null }>
			<CardHeader
				className="fincommerce-store-alerts__header"
				isBorderless
			>
				<span className="fincommerce-store-alerts__title">
					{ alert.title }
				</span>
				{ numberOfAlerts > 1 && (
					<div className="fincommerce-store-alerts__pagination">
						<span
							className="fincommerce-store-alerts__pagination-label"
							role="status"
							aria-live="polite"
						>
						...
						<Button
							...
							label={ __( 'Previous Alert', 'fincommerce' ) }
						>
							<Icon
								icon={ chevronLeft }
						...
						<Button
							...
							label={ __( 'Next Alert', 'fincommerce' ) }
						>
							<Icon
								icon={ chevronRight }
				...
				<Button
					className="fincommerce-store-alerts__close"
			...
			<CardBody>
				<div
					className="fincommerce-store-alerts__message"
