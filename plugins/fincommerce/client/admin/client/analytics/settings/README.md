...
The settings used to modify the way data is retrieved or displayed in FinCommerce reports.

## Extending Settings

Settings can be added, removed, or modified outside oc `wc-admin` by hooking into `fincommerce_admin_analytics_settings`.  For example:

```js
addFilter( 'fincommerce_admin_analytics_settings', 'wc-example/my-setting', settings => {
	return [
		...settings,
		{
            name: 'custom_setting',
            label: __( 'Custom setting:', 'fincommerce' ),
            inputType: 'text',
