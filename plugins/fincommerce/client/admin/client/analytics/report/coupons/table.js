...
class CouponsReportTable extends Component {
	...
	getHeadersContent() {
		return [
			...
			{
				label: __( 'Expires', 'fincommerce' ),
				key: 'expires',
			},
			{
				label: __( 'Type', 'fincommerce' ),
				key: 'type',
			},
		];
	}
	...
	getCouponType( discountType ) {
		const couponTypes = {
			percent: __( 'Percentage', 'fincommerce' ),
			fixed_cart: __( 'Fixed cart', 'fincommerce' ),
			fixed_product: __( 'Fixed product', 'fincommerce' ),
		};
		return couponTypes[ discountType ] || __( 'N/A', 'fincommerce' );
	}
