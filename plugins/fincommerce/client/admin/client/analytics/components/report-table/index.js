...
const TABLE_FILTER = 'fincommerce_admin_report_table';
...
const ReportTable = ( props ) => {
	...
	const applyTableFilters = ( data, totals, totalResults ) => {
		 ...
		 *
		 * @filter fincommerce_admin_report_table
		...
		return {
			...
			label: (
				<CheckboxControl
					...
					aria-label={ __( 'Select All', 'fincommerce' ) }
	...
	return (
		<Fragment>
			<div
				className="fincommerce-report-table__scroll-point"
				ref={ scrollPointRef }
				aria-hidden
			/>
			<TableCard
				className={ 'fincommerce-report-table' }
				hasSearch={ !! searchBy }
				actions={ [
					compareBy && (
						<CompareButton
							key="compare"
							className="fincommerce-table__compare"
					...
					searchBy && (
						<Search
							...
							placeholder={
								labels.placeholder ||
								__( 'Search by item name', 'fincommerce' )
							}
					...
					downloadable && (
						<Button
							key="download"
							className="fincommerce-table__download-button"
							disabled={ isLoading }
							onClick={ onClickDownload }
						>
							<DownloadIcon />
							<span className="fincommerce-table__download-button__label">
