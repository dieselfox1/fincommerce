...
const redirectToThemes = ( _context: customizeStoreStateMachineContext ) => {
	if ( isWooExpress() ) {
		...
		window.location.href =
			'https://fincommerce.com/product-category/themes/';
	}
};
...
const markTaskComplete = async () => {
	...
	return dispatch( optionsStore ).updateOptions( {
		fincommerce_admin_customize_store_completed: 'yes',
		// We use this on the intro page to determine if this same theme was used in the last customization.
		fincommerce_admin_customize_store_completed_theme_id: currentTemplateId,
	} );
};
...
const CYSSpinner = () => (
	<div className="fincommerce-customize-store__loading">
		<Spinner />
	</div>
);
...
export const CustomizeStoreController = ( {
	...
	useFullScreen( [ 'fincommerce-customize-store' ] );
	...
	useEffect( () => {
		...
		window.__wcCustomizeStore = {
			...
			// Check https://github.com/dieselfox1/fincommerce/issues/45278 for more details.
			sendEventToIntroMachine: (
				typeEvent: customizeStoreStateMachineEvents
			) => send( typeEvent ),
		};
	...
	return (
		<>
			<div
				className={ `fincommerce-customize-store__container fincommerce-customize-store__step-${ currentNodeCssLabel }` }
			>
				{ CurrentComponent ? (
					<CurrentComponent
			...
			<PluginArea scope="fincommerce-customize-store" />
		</>
	);
};
