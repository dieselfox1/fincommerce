...
export const SidebarNavigationScreenTypography = ( {
	...
	const title = __( 'Choose fonts', 'fincommerce' );
	const label = __(
	...
	const trackingAllowed = useSelect(
		( select ) =>
			select( optionsStore ).getOption( 'fincommerce_allow_tracking' ),
		[]
	);
	...
	return (
		<SidebarNavigationScreen
			...
			description={ label }
			content={
				<div className="fincommerce-customize-store_sidebar-typography-content">
					{ isFontLibraryAvailable && <FontPairing /> }
					{ upgradeNotice && (
						<div className="fincommerce-customize-store_sidebar-typography-upgrade-notice">
							<h4>
								{ __(
								...
								>
									<CheckboxControl
										...
										label={ interpolateComponents( {
											mixedString: __(
												'More fonts are available! Opt in to connect your store and access the full font library, plus get more relevant content and a tailored store setup experience. Opting in will enable {{link}}usage tracking{{/link}}, which you can opt out of at any time via FinCommerce settings.',
												'fincommerce'
											),
											components: {
												link: (
