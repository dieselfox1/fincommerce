...
export const SaveHub = () => {
	...
	const onDone = async () => {
		...
		try {
			...
			createErrorNotice(
				`${ __( 'Saving failed.', 'fincommerce' ) } ${ error }`
			);
	...
	if ( isMainScreen ) {
		return (
			<HStack
				className="fincommerce-edit-site-save-hub"
				alignment="right"
				spacing={ 4 }
			>
				<Button
					variant="primary"
					onClick={ onDone }
					className="fincommerce-edit-site-save-hub__button"
					disabled={
						isResolving ||
						isEditorLoading ||
				...
				>
					{ isResolving ? (
						...
						__( 'Finish customizing', 'fincommerce' )
					) }
				</Button>
			</HStack>
		);
