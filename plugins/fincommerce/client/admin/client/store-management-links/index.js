...
export function getItemsByCategory( shopUrl ) {
	return [
		{
			title: __( 'Marketing & Merchandising', 'fincommerce' ),
			items: [
				{
					title: __( 'Marketing', 'fincommerce' ),
				...
				{
					title: __( 'Add products', 'fincommerce' ),
					link: getLinkTypeAndHref( {
						type: 'wp-admin',
				...
				{
					title: __( 'Personalize my store', 'fincommerce' ),
					link: getLinkTypeAndHref( {
						type: 'wp-admin',
						path: 'customize.php',
					} ),
				...
				{
					title: __( 'View my store', 'fincommerce' ),
					link: getLinkTypeAndHref( {
						type: 'external',
						href: shopUrl,
					} ),
...
export const StoreManagementLinks = () => {
	...
	const extensionQuickLinks = generateExtensionLinks(
		 ...
		 *
		 * @filter fincommerce_admin_homescreen_quicklinks
		 * @param {Array.<link>} links Array of links.
		 */
		applyFilters( 'fincommerce_admin_homescreen_quicklinks', [] )
	);
		...
		<Card size="medium">
			<CardHeader size="medium">
				<Text variant="title.small" size="20" lineHeight="28px">
					{ __( 'Store management', 'fincommerce' ) }
				</Text>
			</CardHeader>
			<CardBody
				size="custom"
				className="fincommerce-store-management-links__card-body"
			>
