...
const PageContent = ( {
	...
	<>
		<h1 className="fincommerce-block-editor-guide__heading">
			{ page.heading }
		</h1>
		<p className="fincommerce-block-editor-guide__text">{ page.text }</p>
	</>
);
...
const PageImage = ( {
	...
	<div
		className={ `fincommerce-block-editor-guide__header fincommerce-block-editor-guide__header-${
			page.index + 1
		}` }
	></div>
);
...
const BlockEditorGuide = ( { onCloseGuide }: BlockEditorGuideProps ) => {
	const pagesConfig = [
		{
			heading: __( 'Fresh and modern interface', 'fincommerce' ),
			text: __(
		...
		{
			heading: __( 'Content-rich product descriptions', 'fincommerce' ),
			text: __(
		...
		{
			heading: __( 'Lightning fast performance ', 'fincommerce' ),
			text: __(
		...
		{
			heading: __( 'More features are on the way', 'fincommerce' ),
			text: __(
	...
	return (
		<Guide
			className="fincommerce-block-editor-guide"
			contentLabel=""
			finishButtonText={ __( 'Tell me more', 'fincommerce' ) }
			finishButtonLink="https://fincommerce.com/product-form-beta"
