	...
	for ( const tabKey in tabs ) {
		tabContent.push(
			tabs[ tabKey ]?.href ? (
				<a
					className={ clsx(
						'fincommerce-marketplace__tab-button',
						'components-button',
						`fincommerce-marketplace__tab-${ tabKey }`
					) }
				...
				>
					...
					{ tabs[ tabKey ]?.showUpdateCount && (
						<span
							className={ clsx(
								'fincommerce-marketplace__update-count',
								`fincommerce-marketplace__update-count-${ tabKey }`,
								{
									'is-active': tabKey === selectedTab,
								}
							) }
...
const Tabs = ( props: TabsProps ): JSX.Element => {
	...
	const tabs: Tabs = useMemo(
		() => ( {
			...
			themes: {
				name: 'themes',
				title: __( 'Themes', 'fincommerce' ),
				showUpdateCount: !! query.term && ! isLoading,
				updateCount: searchResultsCount.themes,
			},
			'business-services': {
				name: 'business-services',
				title: __( 'Business services', 'fincommerce' ),
				showUpdateCount: !! query.term && ! isLoading,
				updateCount: searchResultsCount[ 'business-services' ],
			},
			'my-subscriptions': {
				name: 'my-subscriptions',
				title: __( 'My subscriptions', 'fincommerce' ),
