...
export default function Content(): JSX.Element {
	...
	const searchCompleteAnnouncement = ( count: number ): void => {
		speak(
			sprintf(
				...
				__( '%d products found', 'fincommerce' ),
				count
			)
		);
	};
	...
	const loadMoreProducts = useCallback( () => {
		...
		fetchSearchResults( params, abortController.signal )
			.then( ( productList ) => {
				...
				speak( __( 'More products loaded', 'fincommerce' ) );
				setCurrentPage( ( prevPage ) => prevPage + 1 );
			} )
			.catch( () => {
				speak( __( 'Error loading more products', 'fincommerce' ) );
			} )
			.finally( () => {
	...
	return (
		<div className="fincommerce-marketplace__content">
