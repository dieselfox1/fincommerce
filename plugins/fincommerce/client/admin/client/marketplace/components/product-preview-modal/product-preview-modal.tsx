...
export default function ProductPreviewModal( {
	...
	useEffect( () => {
		...
		const contentElement = document.querySelector(
			'.fincommerce-marketplace__product-preview-modal__content'
		);
		if ( contentElement ) {
	...
	useEffect( () => {
		const loadPreview = async () => {
			try {
				...
				setError(
					__( 'Failed to load product preview.', 'fincommerce' )
				);
			} finally {
				setIsLoading( false );
			}
	...
	const productHeader = (
		<div className="fincommerce-marketplace__product-preview-modal__header">
			{ productIcon && (
				<img
					className="fincommerce-marketplace__product-preview-modal__icon"
					src={ productIcon }
					alt={ productTitle }
				/>
			) }
			<div className="fincommerce-marketplace__product-preview-modal__header-content">
				<h2>{ productTitle }</h2>
				{ productVendor && (
					<div className="fincommerce-marketplace__product-preview-modal__vendor">
						<span>{ __( 'By', 'fincommerce' ) }</span>{ ' ' }
						{ productVendor }
					</div>
				) }
			</div>
	...
	return (
		<Modal
			...
			className="fincommerce-marketplace__product-preview-modal"
			closeButtonLabel={ __( 'Close product preview', 'fincommerce' ) }
			size="large"
			focusOnMount="firstElement"
			title={ __( 'The FinCommerce Marketplace', 'fincommerce' ) }
		>
			{ productHeader }
			<div className="fincommerce-marketplace__product-preview-modal__content">
				{ isLoading && (
					<div className="fincommerce-marketplace__product-preview-modal__loading">
						<Spinner />
					</div>
				) }
				{ error && (
					<div className="fincommerce-marketplace__product-preview-modal__error">
						{ error }
					</div>
				) }
