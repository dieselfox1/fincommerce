...
function InstallNewProductModal( props: { products: Product[] } ) {
	...
	// Check if the store is connected to FinCommerce.com. This is run once, when the component is mounted.
	useEffect( () => {
	 ...
	 * display title, icon and send product ID to FinCommerce.com to create an order.
	 */
	useEffect( () => {
		setShowModal( false );
			...
			.catch( ( error ) => {
				...
				if ( error.data.redirect_location ) {
					setNotice( {
						status: 'warning',
						message: __(
							'We need your address to complete installing this product. We will redirect you to FinCommerce.com checkout. Afterwards, you will be able to install the product.',
							'fincommerce'
						),
					} );
	...
	function getTitle(): string {
		if ( installStatus === InstallFlowStatus.activated ) {
			return __( 'You are ready to go!', 'fincommerce' );
		}

		return __( 'Add to Store', 'fincommerce' );
	}
	...
	function getDescription(): string {
		...
		return __( 'Would you like to install this extension?', 'fincommerce' );
	}
		...
		if (
			...
			return (
				<Button
					...
					key={ 'plugin-list' }
				>
					{ __( 'View in Plugins', 'fincommerce' ) }
				</Button>
			);
		}
	...
	return (
		<Modal
			...
			focusOnMount={ true }
			className="fincommerce-marketplace__header-account-modal has-size-medium"
