...
function ProductCardFooter( props: { product: Product } ) {
	...
	function shouldShowAddToStore( productToCheck: Product ) {
		...
		// This value is sent from the FinCommerce.com API.
	...
	function getPriceLabel(): string {
		if ( product.price === 0 ) {
			return __( 'Free download', 'fincommerce' );
		}

		if ( product.freemium_type === 'primary' ) {
			return __( 'Free plan available', 'fincommerce' );
		}
	...
	function getPriceSuffix(): string {
		...
		if (
			...
			switch ( product.billingPeriod ) {
				case 'day':
					return __( 'daily', 'fincommerce' );
				case 'week':
					return __( 'weekly', 'fincommerce' );
				case 'month':
					return __( 'monthly', 'fincommerce' );
				case 'year':
				case '':
					return __( 'annually', 'fincommerce' );
				default:
					return '';
			}
		}
		...
		switch ( product.billingPeriod ) {
			case 'day':
				period = __( 'days', 'fincommerce' );
				break;
			case 'week':
				period = __( 'weeks', 'fincommerce' );
				break;
			case 'month':
				period = __( 'months', 'fincommerce' );
				break;
			default:
				period = __( 'years', 'fincommerce' );
		}

		return sprintf(
			// translators: %1$d: billing period interval, %2$s: billing period (e.g. days, weeks, months, years)
			__( 'every %1$d %2$s', 'fincommerce' ),
	...
	if ( isInstalled() ) {
		return (
			<span className="fincommerce-marketplace__product-card__installed-label">
				{ __( 'Installed', 'fincommerce' ) }
			</span>
		);
	}
	...
	if ( shouldShowAddToStore( product ) ) {
		return (
			<>
				<span className="fincommerce-marketplace__product-card__add-to-store">
					<Button variant="secondary" onClick={ openInstallModal }>
						{ __( 'Add to Store', 'fincommerce' ) }
					</Button>
				</span>
			</>
		);
	...
	return (
		<>
			<div className="fincommerce-marketplace__product-card__price">
				<span className="fincommerce-marketplace__product-card__price-label">
				...
				{ product.isOnSale && (
					<span
						className="fincommerce-marketplace__product-card__on-sale"
						aria-hidden
					>
						{ sprintf(
							getCurrencyFormat( product.currency ),
				...
				<span
					className="fincommerce-marketplace__product-card__price-billing"
					aria-hidden
				>
					{ getBillingText() }
				</span>
			</div>
			<div className="fincommerce-marketplace__product-card__rating">
				{ product.averageRating !== null && (
					<>
						<span className="fincommerce-marketplace__product-card__rating-icon">
							<Icon icon={ 'star-filled' } size={ 16 } />
						</span>
						<span className="fincommerce-marketplace__product-card__rating-average">
							<span aria-hidden>{ product.averageRating }</span>
							<span className="screen-reader-text">
								{ sprintf(
									// translators: %.1f: average rating
									__( '%.1f stars', 'fincommerce' ),
									product.averageRating
								) }
							</span>
						</span>
						<span className="fincommerce-marketplace__product-card__rating-count">
							<span aria-hidden>({ product.reviewsCount })</span>
							<span className="screen-reader-text">
								{ sprintf(
									// translators: %d: rating count
									__( 'from %d reviews', 'fincommerce' ),
