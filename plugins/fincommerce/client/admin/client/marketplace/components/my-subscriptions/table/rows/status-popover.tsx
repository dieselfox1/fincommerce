...
export default function StatusPopover( props: {
	...
	return (
		<button
			...
			onBlur={ endHover }
			className={ clsx(
				'fincommerce-marketplace__my-subscriptions__product-status',
				`fincommerce-marketplace__my-subscriptions__product-status--${ props.level }`
			) }
		>
			{ props.text }
			{ shouldShowExplanation() && (
				<Popover
					className="fincommerce-marketplace__my-subscriptions__popover"
					position="top center"
					focusOnMount={ false }
					onMouseOver={ startHover }
					onMouseOut={ endHover }
