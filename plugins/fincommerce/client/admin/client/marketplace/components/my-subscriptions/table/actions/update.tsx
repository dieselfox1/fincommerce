...
export default function Update( props: UpdateProps ) {
		...
		if ( ! window.wp.updates ) {
			addNotice(
				...
				sprintf(
					...
					__( '%s couldn’t be updated.', 'fincommerce' ),
			...
			.then( () => {
				loadSubscriptions( false ).then( () => {
					addNotice(
						...
						sprintf(
							// translators: %s is the product name.
							__( '%s updated successfully.', 'fincommerce' ),
							props.subscription.product_name
						),
	...
	return (
		<>
			...
			<Button
				variant="link"
				className="fincommerce-marketplace__my-subscriptions-update"
				onClick={ update }
				isBusy={ isUpdating }
				disabled={ isUpdating }
				label={ sprintf(
					// translators: %s is the product version.
					__( 'Update to %s', 'fincommerce' ),
