...
function getStatusBadge(
	...
	if ( subscription.product_key === '' ) {
		 ...
		 */
		return {
			text: __( 'No subscription', 'fincommerce' ),
	...
	if ( subscription.expiring && ! subscription.autorenew ) {
		return {
			text: __( 'Expires soon', 'fincommerce' ),
			level: StatusLevel.Error,
			explanation: createInterpolateElement(
				__(
	...
	if (
		...
		return {
			text: __( 'Not connected', 'fincommerce' ),
...
export function nameAndStatus( subscription: Subscription ): TableRow {
	...
	const displayElement = (
		<div className="fincommerce-marketplace__my-subscriptions__product">
			<a
				href={ appendUTMParams( subscription.product_url ) }
				target="_blank"
				rel="noreferrer"
			>
				<span className="fincommerce-marketplace__my-subscriptions__product-icon">
					{ iconElement }
				</span>
			</a>
			<a
				href={ appendUTMParams( subscription.product_url ) }
				className="fincommerce-marketplace__my-subscriptions__product-name"
				target="_blank"
				rel="noreferrer"
			>
				{ subscription.product_name }
			</a>
			<span className="fincommerce-marketplace__my-subscriptions__product-statuses">
				{ subscription.is_shared && (
					<StatusPopover
						text={ __( 'Shared with you', 'fincommerce' ) }
...
export function expiry( subscription: Subscription ): TableRow {
	...
	let expiryDateElement = __( 'Never expires', 'fincommerce' );
	...
	const displayElement = (
		<span className="fincommerce-marketplace__my-subscriptions__expiry-date">
			{ expiryDateElement }
		</span>
	);
...
export function subscriptionStatus(
	...
	function getStatus() {
		...
		if ( subscription.lifetime ) {
			status = __( 'Lifetime', 'fincommerce' );
		} else if ( subscription.autorenew ) {
			status = __( 'Active', 'fincommerce' );
		} else {
			status = __( 'Cancelled', 'fincommerce' );
		}
...
export function actions( subscription: Subscription ): TableRow {
	...
	return {
		display: (
			<div className="fincommerce-marketplace__my-subscriptions__actions">
				{ actionButton }
