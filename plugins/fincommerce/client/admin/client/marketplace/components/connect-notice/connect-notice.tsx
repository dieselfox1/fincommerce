...
export default function ConnectNotice(): JSX.Element | null {
	...
	const defaultStoreName = __( 'Your store', 'fincommerce' );
	...
	const noticeText = {
		long: sprintf(
			...
			__(
				'%s might be at risk because it’s running outdated FinCommerce plugins and is not yet connected to a FinCommerce.com account. Please complete the connection to get updates and streamlined support.',
				'fincommerce'
			),
			formattedStoreName
		),
		short: sprintf(
			/* translators: %s: store name set from the store settings, if not set, it will be "Your store" */
			__(
				'%s is not yet connected to a FinCommerce.com account. Please complete the connection to get updates and streamlined support.',
				'fincommerce'
			),
			formattedStoreName
		),
	...
	return (
		<Notice
			...
			isDismissible={ true }
			variant="warning"
			className="fincommerce-marketplace__connect-notice"
			onClose={ handleClose }
			onLoad={ handleLoad }
		>
			<Button
				...
				onClick={ handleClick }
			>
				{ __( 'Connect your store', 'fincommerce' ) }
			</Button>
			<Button
				href="https://fincommerce.com/document/managing-fincommerce-com-subscriptions/#connect-your-site-fincommercecom-account"
				target="_blank"
				variant="tertiary"
