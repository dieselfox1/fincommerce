...
const WcPayPromotionGateway = document.querySelector(
	'[data-gateway_id="pre_install_fincommerce_payments_promotion"]'
);
...
const PaymentRecommendations = () => {
	...
	useEffect( () => {
		if (
			...
			const eventProps = ( paymentGatewaySuggestions || [] ).reduce(
				...
				{
					fincommerce_payments_displayed: !! WcPayPromotionGateway,
				}
			);
			recordEvent(
		...
		.filter( ( plugin: Plugin ) => {
			return (
				...
				( ! window.wcAdminFeatures[ 'wc-pay-promotion' ] ||
					! plugin.id.startsWith( 'fincommerce_payments' ) )
			);
		} )
		.map( ( plugin: Plugin ) => {
			return {
	...
	return (
		<Card size="medium" className="fincommerce-recommended-payments-card">
			<CardHeader>
				<div className="fincommerce-recommended-payments-card__header">
					<Text
						variant="title.small"
						as="p"
						size="20"
						lineHeight="28px"
					>
						{ __( 'Recommended payment providers', 'fincommerce' ) }
					</Text>
					<Text
						className={
							'fincommerce-recommended-payments__header-heading'
						}
						variant="caption"
						as="p"
						size="12"
				...
				<div className="fincommerce-card__menu fincommerce-card__header-item">
					<EllipsisMenu
						...
						renderContent={ () => (
							<div className="fincommerce-review-activity-card__section-controls">
								<Button
			...
			<CardFooter>
				<TrackedLink
					message={ __(
						...
						'Visit {{Link}}the FinCommerce Marketplace{{/Link}} to find additional payment providers.',
						'fincommerce'
					) }
					...
					targetUrl={
						isFeatureEnabled( 'marketplace' )
							...
							: 'https://fincommerce.com/product-category/fincommerce-extensions/payment-gateways/'
					}
					linkType={
						isFeatureEnabled( 'marketplace' )
							? 'wc-admin'
