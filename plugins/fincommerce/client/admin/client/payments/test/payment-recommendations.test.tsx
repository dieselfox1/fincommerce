...
describe( 'Payment recommendations', () => {
	...
	it( 'should trigger event payments_recommendations_pageview, when first rendered', () => {
		...
		expect( recordEvent ).toHaveBeenCalledWith(
			...
			{
				test_displayed: true,
				fincommerce_payments_displayed: false,
			}
		);
	} );

	it( 'should set fincommerce-payments-displayed prop to true if pre install wc pay promotion gateway is displayed', () => {
		( isWCPaySupported as jest.Mock ).mockReturnValue( true );
		( useSelect as jest.Mock ).mockReturnValue( {
			installedPaymentGateways: {},
			paymentGatewaySuggestions: [
		...
		const { container } = render(
			<div>
				<div data-gateway_id="pre_install_fincommerce_payments_promotion"></div>
	...
	it( 'should trigger event settings_payment_recommendations_visit_marketplace_click when clicking the FinCommerce Marketplace link', () => {
		( isWCPaySupported as jest.Mock ).mockReturnValue( true );
		( useSelect as jest.Mock ).mockReturnValue( {
			installedPaymentGateways: {},
			paymentGatewaySuggestions: [
	...
	describe( 'interactions', () => {
		...
		it( 'should navigate to the marketplace when clicking the FinCommerce Marketplace link', async () => {
			...
			expect( container.firstChild ).not.toBeNull();
			fireEvent.click( getByText( 'the FinCommerce Marketplace' ) );
