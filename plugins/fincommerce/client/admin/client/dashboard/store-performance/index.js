...
import { getDateParamsFromQuery } from '@fincommerce/date';
...
class StorePerformance extends Component {
	...
	renderList() {
		...
		const prevLabel =
			compare === 'previous_period'
				? __( 'Previous period:', 'fincommerce' )
				: __( 'Previous year:', 'fincommerce' );
	...
	render() {
		...
		return (
			<Fragment>
				<SectionHeader
					title={ title || __( 'Store Performance', 'fincommerce' ) }
					menu={ this.renderMenu() }
				/>
				{ userIndicators.length > 0 && (
					<div className="fincommerce-dashboard__store-performance">
						{ this.renderList() }
					</div>
				) }
			</Fragment>
