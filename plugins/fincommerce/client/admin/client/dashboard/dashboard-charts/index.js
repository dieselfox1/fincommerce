...
import { getAllowedIntervalsForQuery } from '@fincommerce/date';
...
const renderIntervalSelector = ( {
	...
	return (
		<SelectControl
			className="fincommerce-chart__interval-select"
			value={ chartInterval }
...
const renderChartBlocks = ( { hiddenBlocks, path, query, filters } ) => {
	...
	return (
		<div className="fincommerce-dashboard__columns">
...
const DashboardCharts = ( props ) => {
	...
	const renderMenu = () => (
		<EllipsisMenu
			label={ __( 'Choose which charts to display', 'fincommerce' ) }
			placement={ 'bottom-end' }
			renderContent={ ( { onToggle } ) => (
				<Fragment>
					<MenuTitle>{ __( 'Charts', 'fincommerce' ) }</MenuTitle>
					{ renderChartToggles( {
						hiddenBlocks,
						onToggleHiddenBlock,
					} ) }
	...
	return (
		<div className="fincommerce-dashboard__dashboard-charts">
			<SectionHeader
				title={ title || __( 'Charts', 'fincommerce' ) }
				menu={ renderMenu() }
			...
			>
				...
				<NavigableMenu
					className="fincommerce-chart__types"
					orientation="horizontal"
					role="menubar"
				>
					<Button
						className={ clsx( 'fincommerce-chart__type-button', {
							'fincommerce-chart__type-button-selected':
								! query.chartType || query.chartType === 'line',
						} ) }
						title={ __( 'Line chart', 'fincommerce' ) }
					...
					<Button
						...
						title={ __( 'Bar chart', 'fincommerce' ) }
