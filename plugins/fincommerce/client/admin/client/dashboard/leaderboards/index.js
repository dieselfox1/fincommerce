...
const Leaderboards = ( props ) => {
		...
		<EllipsisMenu
			...
			renderContent={ ( { onToggle } ) => (
				<Fragment>
					...
					<MenuItem>
						<SelectControl
							className="fincommerce-dashboard__dashboard-leaderboards__select"
							label={ __( 'Rows per table', 'fincommerce' ) }
							value={ rowsPerTable }
							options={ Array.from(
								{ length: 20 },
								( v, key ) => ( {
	...
	return (
		<Fragment>
			<div className="fincommerce-dashboard__dashboard-leaderboards">
				<SectionHeader
					title={ title || __( 'Leaderboards', 'fincommerce' ) }
					menu={ renderMenu() }
				/>
