...
class CartModal extends Component {
	...
	onClickPurchaseNow() {
		...
		const url = getInAppPurchaseUrl(
			'https://fincommerce.com/cart?utm_medium=product',
			{
			...
			if ( themeInfo ) {
				listItems.push( {
					title: sprintf(
						...
						__( '%1$s — %2$s per year', 'fincommerce' ),
						themeInfo.title,
	...
	render() {
		...
		return (
			<Modal
				...
				className="fincommerce-cart-modal"
			>
				{ this.renderProducts() }

				<p className="fincommerce-cart-modal__help-text">
					{ __(
				...
				<div className="fincommerce-cart-modal__actions">
					<Button
						isLink
						isBusy={ purchaseLaterButtonBusy }
						onClick={ () => this.onClickPurchaseLater() }
					>
						{ __( 'I’ll do it later', 'fincommerce' ) }
					</Button>
