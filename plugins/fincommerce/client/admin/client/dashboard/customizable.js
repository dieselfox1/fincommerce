...
const DASHBOARD_FILTERS_FILTER = 'fincommerce_admin_dashboard_filters';
 ...
 *
 * @filter fincommerce_admin_dashboard_filters
...
const CustomizableDashboard = ( { defaultDateRange, path, query } ) => {
	...
	const renderAddMore = () => {
		...
		return (
			<Dropdown
				className="fincommerce-dashboard-section__add-more"
				renderToggle={ ( { onToggle, isOpen } ) => (
					<Button
						onClick={ onToggle }
						title={ __( 'Add more sections', 'fincommerce' ) }
				...
				renderContent={ ( { onToggle } ) => (
					<>
						<H>{ __( 'Dashboard Sections', 'fincommerce' ) }</H>
						<div className="fincommerce-dashboard-section__add-more-choices">
							{ hiddenSections.map( ( section ) => {
								return (
									<Button
										...
										className="fincommerce-dashboard-section__add-more-btn"
										title={ sprintf(
									...
									>
										<Icon
											...
											size={ 30 }
										/>
										<span className="fincommerce-dashboard-section__add-more-btn-title">
											{ section.title }
										</span>
									</Button>
								);
