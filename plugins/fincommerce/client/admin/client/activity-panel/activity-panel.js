	...
	const getPreviewSiteBtnTrackData = useCallback(
		( select, getOption ) => {
			...
			if ( query.page === 'wc-admin' && query.task === 'appearance' ) {
				...
				const demoNotice = getOption( 'fincommerce_demo_store_notice' );
				trackData = {
	...
	const getTabs = () => {
		const activity = {
			...
			visible:
				...
				! isProductScreen() &&
				currentUserCan( 'manage_fincommerce' ),
		};

		const feedback = {
			name: 'feedback',
			title: __( 'Feedback', 'fincommerce' ),
			icon: <FeedbackIcon />,
			onClick: () => {
		...
		const setup = {
			name: 'setup',
			title: __( 'Finish setup', 'fincommerce' ),
			icon: (
				<SetupProgress
		...
		const previewSite = {
			name: 'previewSite',
			title: __( 'Preview site', 'fincommerce' ),
		...
		<LayoutContextProvider value={ updatedLayoutContext }>
			<div>
				<H id={ headerId } className="screen-reader-text">
					{ __( 'Store Activity', 'fincommerce' ) }
				</H>
				<Section
					component="aside"
					id="fincommerce-activity-panel"
					className="fincommerce-layout__activity-panel"
