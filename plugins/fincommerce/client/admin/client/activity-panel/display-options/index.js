			...
			{ ( fills ) => {
				...
				return (
					<DropdownMenu
						...
						label={ __( 'Display options', 'fincommerce' ) }
						toggleProps={ {
							className:
								'fincommerce-layout__activity-panel-tab display-options',
							onClick: () =>
								recordEvent( 'homescreen_display_click' ),
						} }
						popoverProps={ {
							className:
								'fincommerce-layout__activity-panel-popover',
						} }
					>
						{ ( { onClose } ) => (
							<>
