...
export const PluginCard = ( {
	...
	return (
		<label
			className={ clsx( 'fincommerce-profiler-plugins-plugin-card', {
		...
		>
			...
			{ ! installed && (
				<CheckboxControl
					...
					className="fincommerce-profiler__checkbox"
					disabled={ disabled }
					checked={ checked }
					onChange={ ( event ) => {
						if ( ! disabled ) {
			...
			<div className="fincommerce-profiler-plugins-plugin-card-main">
				{ imageUrl ? (
					<img
						className="fincommerce-profiler-plugins-plugin-card-logo"
						src={ imageUrl }
						alt={ pluginKey }
					/>
				) : null }

				<div className="fincommerce-profiler-plugins-plugin-card-content">
					<div
						className={ clsx(
							'fincommerce-profiler-plugins-plugin-card-text-header',
							{
								installed,
							}
						) }
					>
						<h3 className="fincommerce-profiler-plugins-plugin-card-title">
							{ title }
						</h3>
						{ installed && (
							<span>{ __( 'Installed', 'fincommerce' ) }</span>
						) }
					</div>

					<div className="fincommerce-profiler-plugins-plugin-card-text">
						<p
