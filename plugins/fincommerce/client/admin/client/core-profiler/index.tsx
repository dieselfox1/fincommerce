...
import { CountryStateOption } from '@fincommerce/onboarding';
...
const recordUpdateTrackingOption = (
	...
	if ( prevValue !== newValue ) {
		recordEvent( 'fincommerce_allow_tracking_toggled', {
		...
		spawn(
			fromPromise( async () =>
				resolveSelect( onboardingStore ).getJetpackAuthUrl( {
					...
					from: 'fincommerce-core-profiler',
				} )
			)
		),
} );
...
export const CoreProfilerController = ( {
	...
	useFullScreen( [ 'fincommerce-profile-wizard__body' ] );
	...
	return (
		<>
			<div
				className={ `fincommerce-profile-wizard__container fincommerce-profile-wizard__step-${ currentNodeCssLabel }` }
			>
				{ CurrentComponent ? (
					<CurrentComponent
