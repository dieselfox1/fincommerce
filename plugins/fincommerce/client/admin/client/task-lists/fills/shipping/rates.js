...
const ShippingRateIcon = ( { zone } ) => (
	<div className="fincommerce-shipping-rate__icon">
		{ zone.locations ? (
...
const ShippingRateToggle = ( { zone, getInputProps } ) => (
	<label
		htmlFor={ `fincommerce-shipping-rate__toggle-${ zone.id }` }
		className="fincommerce-shipping-rate__name"
	>
		{ zone.name }
		<FormToggle
			id={ `fincommerce-shipping-rate__toggle-${ zone.id }` }
...
const ShippingRateInput = ( {
	...
	const textControlClassName = clsx(
		'muriel-input-text',
		'fincommerce-shipping-rate__control-wrapper',
		className
	);
	return (
		<Fragment>
			{ ! zone.toggleable && (
				<div className="fincommerce-shipping-rate__name">
					{ zone.name }
				</div>
			) }
			{ ( ! zone.toggleable || values[ `${ zone.id }_enabled` ] ) && (
				<TextControlWithAffixes
					...
					label={ __( 'Shipping cost', 'fincommerce' ) }
					required
...
class ShippingRates extends Component {
	...
	async updateShippingZones( values ) {
		...
		createNotice(
			'success',
			__( 'Your shipping rates have been updated', 'fincommerce' )
		);
	...
	renderInputPrefix() {
		...
		return (
			<span className="fincommerce-shipping-rate__control-prefix">
				{ symbol }
			</span>
		);
	}
	...
	renderInputSuffix( rate ) {
		...
		if ( symbolPosition.indexOf( 'right' ) === 0 ) {
			return (
				<span className="fincommerce-shipping-rate__control-suffix">
					{ symbol }
				</span>
			);
		}
		...
		return (
			...
			>
				{ ( {
					...
					return (
						<Fragment>
							<div className="fincommerce-shipping-rates">
								{ shippingZones.map( ( zone ) => (
									<div
										className="fincommerce-shipping-rate"
										key={ zone.id }
									>
										<ShippingRateIcon zone={ zone } />
										<div className="fincommerce-shipping-rate__main">
											{ zone.toggleable && (
												<ShippingRateToggle
													zone={ zone }
													getInputProps={
							...
							<Button isPrimary onClick={ handleSubmit }>
								{ buttonText || __( 'Update', 'fincommerce' ) }
							</Button>
						</Fragment>
					);
				} }
