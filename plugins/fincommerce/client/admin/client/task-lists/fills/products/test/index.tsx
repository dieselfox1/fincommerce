...
describe( 'Products', () => {
	...
	it( 'should render stacked layout', async () => {
		...
		expect(
			container.getElementsByClassName( 'fincommerce-products-stack' )
				.length
		).toBeGreaterThanOrEqual( 1 );
	} );

	it( 'should trigger event tasklist_add_product_visit_marketplace_click when clicking the FinCommerce Marketplace link', () => {
		const { getByText } = render( <Products /> );

		userEvent.click( getByText( 'the FinCommerce Marketplace' ) );
	...
	describe( 'Printful banner visibility', () => {
		...
		it( 'should hide Printful banner when plugin is installed', async () => {
			( useSelect as jest.Mock ).mockImplementation( ( callback ) =>
				callback( () => ( {
					getInstalledPlugins: () => [
						'printful-shipping-for-fincommerce',
					],
					isPluginsRequesting: () => false,
				} ) )
			);
