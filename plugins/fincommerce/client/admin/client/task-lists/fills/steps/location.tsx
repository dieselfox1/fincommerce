...
const StoreLocation = ( {
	...
	updateAndPersistSettingsForGroup,
	settings,
	buttonText = __( 'Continue', 'fincommerce' ),
	...
	const onSubmit = async ( values: FormValues ) => {
		...
		try {
			await updateAndPersistSettingsForGroup( 'general', {
				general: {
					...settings,
					fincommerce_store_address: values.addressLine1,
					fincommerce_store_address_2: values.addressLine2,
					fincommerce_default_country: values.countryState,
					fincommerce_store_city: values.city,
					fincommerce_store_postcode: values.postCode,
				},
			} );
	...
	const getInitialValues = () => {
		return {
			addressLine1: settings?.fincommerce_store_address || '',
			addressLine2: settings?.fincommerce_store_address_2 || '',
			city: settings?.fincommerce_store_city || '',
			countryState: settings?.fincommerce_default_country || '',
			postCode: settings?.fincommerce_store_postcode || '',
		};
	};
