...
export const getIsGatewayWCPay = ( gateway ) =>
	...
	gateway.plugins[ 0 ] === 'fincommerce-payments';
...
export const getEnrichedPaymentGateways = (
	...
	return paymentGatewaySuggestions.reduce( ( map, suggestion ) => {
		// A colon ':' is used sometimes to have multiple configs for the same gateway ex: fincommerce_payments:us.
			...
			( all, gateway ) => {
				...
				if ( getIsGatewayWCPay( gateway ) ) {
					if ( isWCPaySupported ) {
						...
						if ( gateway.id === 'fincommerce_payments:bnpl' ) {
