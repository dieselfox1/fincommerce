...
const paymentGatewaySuggestions = [
	{
		...
		image: 'http://localhost:8888/wp-content/plugins/fincommerce/assets/images/stripe.png',
		plugins: [ 'fincommerce-gateway-stripe' ],
		is_visible: true,
	...
	},
	{
		...
		image: 'http://localhost:8888/wp-content/plugins/fincommerce/assets/images/paypal.png',
		plugins: [ 'fincommerce-paypal-payments' ],
		is_visible: true,
		category_other: [ 'US' ],
	...
	},
	{
		...
		image: 'http://localhost:8888/wp-content/plugins/fincommerce-admin/images/onboarding/cod.svg',
		is_visible: true,
		is_offline: true,
	},
	{
		...
		image: 'http://localhost:8888/wp-content/plugins/fincommerce-admin/images/onboarding/bacs.svg',
		is_visible: true,
		is_offline: true,
	},
	{
	...
	},
	{
		...
		image: 'http://localhost:8888/wp-content/plugins/fincommerce-admin/images/onboarding/wcpay-bnpl.svg',
		plugins: [ 'fincommerce-payments' ],
		is_visible: true,
		recommendation_priority: 1,
	},
	{
		...
		title: 'Eway',
		content:
			'The Eway extension for FinCommerce allows you to take credit card payments directly on your store without redirecting your customers to a third party site to make payment.',
		image: 'http://localhost:8888/wp-content/plugins/fincommerce-admin/images/onboarding/eway.png',
		plugins: [ 'fincommerce-gateway-eway' ],
		is_visible: true,
		category_other: [ 'US' ],
		category_additional: [ 'US' ],
	},
...
const paymentGatewaySuggestionsWithoutWCPay = paymentGatewaySuggestions.filter(
	( p ) => ! p.id.startsWith( 'fincommerce_payments' )
);
...
describe( 'PaymentGatewaySuggestions', () => {
	test( 'should render all payment gateways, including WCPay', () => {
		...
		const paymentTitleElements = container.querySelectorAll(
			'.fincommerce-task-payment__title'
		);
		...
		expect(
			container
				.querySelector(
					'.fincommerce-recommended-payments-banner__footer'
				)
		...
		expect(
			container.querySelector( '.fincommerce-wcpay-bnpl-suggestion' )
		).toBeFalsy();
	} );
	...
	test( 'should render all payment gateways except WCPay', () => {
		...
		const paymentTitleElements = container.querySelectorAll(
			'.fincommerce-task-payment__title > span:first-child'
		);
	...
	test( 'should record event correctly when WCPay BNPL Get started is clicked', () => {
		...
		fireEvent.click(
			container.querySelector(
				'.fincommerce-wcpay-bnpl-suggestion__button'
			)
		);
		expect(
			recordEvent.mock.calls[ recordEvent.mock.calls.length - 1 ]
	...
	test( 'should navigate to the marketplace when clicking the FinCommerce Marketplace link', async () => {
