...
export const PaymentGatewaySuggestions = ( { onComplete, query } ) => {
	...
	const {
		...
		return {
			...
			countryCode: getCountryCode( settings.fincommerce_default_country ),
		};
	}, [] );
	...
	const currentGateway = useMemo( () => {
		...
		// Sometimes it features a : separator, e.g. 'fincommerce-payments:bnpl'.
	...
	const offlineSection = !! offlineGateways.length && (
		<List
			heading={ __( 'Offline payment methods', 'fincommerce' ) }
			// No recommendation if WooPayments is supported (and displayed).
			recommendation={ isWCPaySupported ? false : recommendation }
	...
	return (
		<div className="fincommerce-task-payments">
