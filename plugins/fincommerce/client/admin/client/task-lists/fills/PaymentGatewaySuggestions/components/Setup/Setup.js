...
export const Setup = ( { markConfigured, paymentGateway } ) => {
	...
	const slot = useSlot( `fincommerce_payment_gateway_setup_${ id }` );
	...
	const configureStep = useMemo(
		() => ( {
			...
			label: sprintf(
				/* translators: %s = title of the payment gateway to install */
				__( 'Configure your %(title)s account', 'fincommerce' ),
				{
					title,
				}
			),
	...
	return (
		<Card className="fincommerce-task-payment-method fincommerce-task-card">
			<CardBody>
				{ hasFills ? (
