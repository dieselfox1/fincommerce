...
const validateLocationForm = ( values: FormValues ) => {
	...
	if (
		document.getElementById( 'fincommerce-store-address-form-address_1' ) &&
		! values.addressLine1.trim().length
	) {
		errors.addressLine1 = __( 'Please enter an address', 'fincommerce' );
	}

	if (
		document.getElementById( 'fincommerce-store-address-form-postcode' ) &&
		! values.postCode.trim().length
	) {
		errors.postCode = __( 'Please enter a post code', 'fincommerce' );
	}

	if (
		document.getElementById( 'fincommerce-store-address-form-city' ) &&
		! values.city.trim().length
	) {
		errors.city = __( 'Please enter a city', 'fincommerce' );
	}
			...
			! hasCompleteAddress(
				...
				Boolean(
					document.getElementById(
						'fincommerce-store-address-form-postcode'
					)
				)
			)
		) {
