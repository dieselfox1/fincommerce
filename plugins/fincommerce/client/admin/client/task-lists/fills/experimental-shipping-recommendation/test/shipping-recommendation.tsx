		...
		fn( () => ( {
			...
			getOption: ( key: string ) => {
				return {
					...
					fincommerce_setup_jetpack_opted_in: 1,
				}[ key ];
			},
		} ) )
	),
...
describe( 'ShippingRecommendation', () => {
	test( 'should show plugins step when fincommerce-shipping is not installed and activated', () => {
	...
	test( 'should show connect step when FinCommerce Shipping is activated but not yet connected', () => {
	...
	test( 'should show "complete task" button when FinCommerce Shipping is activated and Jetpack is connected', () => {
		const { getByRole } = render(
			<ShippingRecommendation
	...
	test( 'should trigger event tasklist_shipping_recommendation_visit_marketplace_click when clicking the FinCommerce Marketplace link', () => {
