...
/**
 * A component for rendering the content of an ellipsis menu in the FinCommerce payment settings.
...
export const EllipsisMenuContent = ( {
	...
	const typeToDisplayName: { [ key: string ]: string } = {
		pricing: __( 'See pricing & fees', 'fincommerce' ),
		about: __( 'Learn more', 'fincommerce' ),
		terms: __( 'See Terms of Service', 'fincommerce' ),
		support: __( 'Get support', 'fincommerce' ),
		documentation: __( 'View documentation', 'fincommerce' ),
	};
	...
	const disableProvider = () => {
		const gatewayToggleNonce =
			window.fincommerce_admin.nonces?.gateway_toggle || '';
		...
		if ( ! gatewayToggleNonce ) {
			...
			createErrorNotice(
				__( 'Failed to disable the payments provider.', 'fincommerce' )
			);
			return;
		}
		setIsDisabling( true );
		togglePaymentGateway(
			provider.id,
			window.fincommerce_admin.ajax_url,
			gatewayToggleNonce
		)
			.then( () => {
				invalidateResolutionForStoreSelector( 'getPaymentProviders' );
	...
	return (
		<>
			{ contextLinks.map( ( link: PaymentsProviderLink ) => {
				...
				return displayName ? (
					<div
						className="fincommerce-ellipsis-menu__content__item"
						key={ link._type }
					>
						<Button
							target="_blank"
			...
			{ isSuggestion && (
				...
				>
					...
					>
						{ __( 'Hide suggestion', 'fincommerce' ) }
					</Button>
				</div>
			) }
			{ ( canResetAccount || canResetOnboarding ) && (
				...
				>
					...
					>
						{ canResetAccount
							? __( 'Reset account', 'fincommerce' )
							: __( 'Reset onboarding', 'fincommerce' ) }
					</Button>
				</div>
			) }
