...
import { Link, Pill } from '@fincommerce/components';
import { createInterpolateElement, useRef, useState } from '@finpress/element';
...
export const OfficialBadge = ( {
	...
	const handleClick = ( event: React.MouseEvent | React.KeyboardEvent ) => {
		...
		const parentSpan = clickedElement.closest(
			'.fincommerce-official-extension-badge__container'
		);
	...
	return (
		<Pill className={ `fincommerce-official-extension-badge` }>
			<span
				className="fincommerce-official-extension-badge__container"
				tabIndex={ 0 }
				role="button"
				ref={ buttonRef }
				onClick={ handleClick }
			...
			>
				<img
					...
					alt={ __(
						'Official FinCommerce extension badge',
						'fincommerce'
					) }
				/>
				{ variant === 'expanded' && (
					<span>{ __( 'Official', 'fincommerce' ) }</span>
				) }
				{ isPopoverVisible && (
					<Popover
						className="fincommerce-official-extension-badge-popover"
						placement="top-start"
						offset={ 4 }
						variant="unstyled"
						focusOnMount={ true }
						...
						<div className="components-popover__content-container">
							<p>
								{ createInterpolateElement(
									__(
										'This is an Official FinCommerce payment extension. <learnMoreLink />',
										'fincommerce'
									),
									{
										learnMoreLink: (
											<Link
												href="https://fincommerce.com/learn-more-about-official-partner-badging/"
												target="_blank"
												rel="noreferrer"
												type="external"
												onClick={ () => {
