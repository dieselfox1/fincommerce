...
export const PaymentExtensionSuggestionListItem = ( {
	...
	let ctaButtonLabel = __( 'Install', 'fincommerce' );
	if ( pluginInstalled ) {
		ctaButtonLabel = __( 'Enable', 'fincommerce' );
	} else if ( installingPlugin === suggestion.id ) {
		ctaButtonLabel = __( 'Installing', 'fincommerce' );
	}

	return (
		<div
			id={ suggestion.id }
			className={ `transitions-disabled fincommerce-list__item fincommerce-list__item-enter-done ${
