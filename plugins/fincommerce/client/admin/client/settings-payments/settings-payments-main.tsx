...
/**
 * A component that renders the main settings page for managing payment gateways in FinCommerce.
...
export const SettingsPaymentsMain = () => {
	...
	const [ businessCountry, setBusinessCountry ] = useState< string | null >(
		window.wcSettings?.admin?.fincommerce_payments_nox_profile
	...
	if ( incentiveProvider && incentive ) {
		if ( isSwitchIncentive( incentive ) ) {
			if (
				...
				// @see its server brother in plugins/fincommerce/src/Internal/Admin/Settings/PaymentsController::store_has_providers_with_incentive()
	...
	useEffect( () => {
		...
		const eventProps: { [ key: string ]: boolean } = {
			fincommerce_payments_displayed: providers.some( ( provider ) =>
	...
	const morePaymentOptionsLink = (
		<TrackedLink
			message={ __(
				...
				'Visit {{Link}}the FinCommerce Marketplace{{/Link}} to find additional payment options.',
				'fincommerce'
			) }
	...
	return (
		<>
			...
			<div className="settings-payments-main__container">
				...
				/>
				{
					// If no suggestions are available, only show a link to the FinCommerce.com payment marketplace page.
