...
const renderEmptyCard = () => (
	<ActivityCard
		...
		title={ __( 'Your inbox is empty', 'fincommerce' ) }
		icon={ false }
	>
		{ __(
		...
		<Card size="large">
			{ showHeader && (
				<CardHeader size="medium">
					<div className="fincommerce-inbox-card__header">
						<Text size="20" lineHeight="28px" variant="title.small">
							{ __( 'Inbox', 'fincommerce' ) }
						</Text>
						<Badge count={ unreadNotesCount } />
					</div>
					<EllipsisMenu
						label={ __( 'Inbox Notes Options', 'fincommerce' ) }
						renderContent={ ( { onToggle } ) => (
							<div className="fincommerce-inbox-card__section-controls">
								<Button
									onClick={ () => {
										onDismissAll( true );
										onToggle();
				...
				{ notesArray.map( ( note ) => {
					...
					return (
						<CSSTransition
							key={ noteId }
							timeout={ 500 }
							classNames="fincommerce-inbox-message"
						>
							<InboxNoteCard
								key={ noteId }
								note={ note }
			...
			{ allNotesFetched
				...
				: ( () => {
						...
						return (
							<CardFooter
								className="fincommerce-inbox-card__footer"
								size="medium"
							>
								<Button
									isPrimary={ true }
...
const InboxPanel = ( { showHeader = true } ) => {
	...
	if ( isError ) {
		...
		const actionLabel = __( 'Reload', 'fincommerce' );
	...
	return (
		<>
			...
			<div className="fincommerce-homepage-notes-wrapper">
