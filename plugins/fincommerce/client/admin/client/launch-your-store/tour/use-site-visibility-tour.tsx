...
export const useSiteVisibilityTour = () => {
	...
	// Tour should only be shown if the user has not seen it before and the `fincommerce_show_lys_tour` option is "yes" (for sites upgrading from a previous FinCommerce version)
	const shouldStoreShowLYSTour = useSelect(
		( select ) =>
			select( optionsStore ).getOption( 'fincommerce_show_lys_tour' ) ===
			'yes',
		[]
	);

	/**
	 * This is temporary to support sites upgrading from a previous version of FinCommerce.
	 * We used user meta to store the tour dismissal state but now we use FinCommerce meta instead.
	...
	const hasUserDismissedTourMeta = useSelect( ( select ) => {
		...
		return (
			...
			( currentUser as { meta: { [ key: string ]: string } } ).meta
				.fincommerce_launch_your_store_tour_hidden === 'yes'
		);
	}, [] );
