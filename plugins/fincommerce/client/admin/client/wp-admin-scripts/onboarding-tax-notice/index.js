...
const showTaxCompletionNotice = () => {
	const saveButton = document.querySelector( '.fincommerce-save-button' );
	...
	saveCompleted( saveButton ).then( () => {
		// Check if a row was added successfully after FinCommerce removes invalid rows.
		...
		dispatch( 'core/notices' ).createSuccessNotice(
			__( 'You’ve added your first tax rate!', 'fincommerce' ),
			{
				id: 'fincommerce_ONBOARDING_TAX_NOTICE',
				actions: [
					{
						url: getAdminLink( 'admin.php?page=wc-admin' ),
						label: __( 'Continue setup.', 'fincommerce' ),
					},
				],
			}
		);
