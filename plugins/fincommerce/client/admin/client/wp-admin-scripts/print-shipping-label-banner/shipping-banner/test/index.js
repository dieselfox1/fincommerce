...
const wcsPluginSlug = 'fincommerce-shipping';
...
describe( 'Tracking clicks in shippingBanner', () => {
	...
	it( 'should record an event when user clicks "FinCommerce Shipping"', async () => {
		...
		userEvent.click(
			getByRole( 'link', { name: /FinCommerce Shipping/ } )
		);
		...
		await waitFor( () =>
			expect( recordEvent ).toHaveBeenCalledWith(
				...
				getExpectedTrackingData(
					'shipping_banner_fincommerce_service_link',
					false
				)
			)
		);
...
describe( 'Create shipping label button', () => {
	...
	it( 'should install FinCommerce Shipping when button is clicked', async () => {
		const actionButtonLabel = 'Create shipping label';
	...
	it( 'should activate FinCommerce Shipping when installation finishes', async () => {
		const actionButtonLabel = 'Create shipping label';
		const { getByRole } = render(
			<ShippingBanner
				isJetpackConnected={ true }
	...
	it( 'should load WCS assets when a path is provided', async () => {
		...
		const { getByRole } = render(
			<Fragment>
				<div id="fincommerce-order-data" />
				<div id="fincommerce-order-actions" />
				<ShippingBanner
					isJetpackConnected={ true }
					activatePlugins={ activatePlugins }
					activePlugins={ [ wcstPluginSlug, 'jetpack' ] }
	...
	it( 'should open WCS modal', async () => {
		...
		const { getByRole } = render(
			<Fragment>
				<div id="fincommerce-order-data" />
				<div id="fincommerce-order-actions" />
				<div id="fincommerce-admin-print-label" />
				<ShippingBanner
					isJetpackConnected={ true }
					activatePlugins={ activatePlugins }
		...
		await waitFor( () => {
			expect(
				document.getElementById( 'fincommerce-admin-print-label' )
			).not.toBeVisible();
		} );
...
describe( 'In the process of installing, activating, loading assets for FinCommerce Service', () => {
...
describe( 'The message in the banner', () => {
	...
	const notActivatedMessage =
		'By clicking "Create shipping label", FinCommerce Shipping↗ will be installed and you agree to its Terms of Service↗.';
	...
	it( 'should continue to show the initial message "By clicking "Create shipping label"..." after FinCommerce Service is installed successfully.', () => {
...
describe( 'If incompatible WCS&T is active', () => {
	...
	it( 'should install and activate but show an error notice when an incompatible version of WCS&T is installed', async () => {
		const actionButtonLabel = 'Install FinCommerce Shipping';
		...
		const notice = await findByText( ( _, element ) => {
			const hasText = ( node ) =>
				node.textContent ===
				'Please update the FinCommerce Shipping & Tax plugin to the latest version to ensure compatibility with FinCommerce Shipping.';
