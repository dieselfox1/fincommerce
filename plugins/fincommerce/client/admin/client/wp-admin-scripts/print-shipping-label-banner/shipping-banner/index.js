...
const wcShippingPluginSlug = 'fincommerce-shipping';
const wcstPluginSlug = 'fincommerce-services';
...
export class ShippingBanner extends Component {
	...
	async installAndActivatePlugins( pluginSlug ) {
		...
		/**
		 * If a incompatible version of the FinCommerce Shipping plugin is installed, the necessary endpoints
	...
	fincommerceServiceLinkClicked = () => {
		this.trackElementClicked( 'shipping_banner_fincommerce_service_link' );
	};
	...
	generateMetaBoxHtml( nodeId, title, args ) {
		const togglePanelText = __( 'Toggle panel:', 'fincommerce' );
...
<div id="${ nodeId }" class="postbox">
	...
	<div class="inside">
		<div class="wcc-root fincommerce fincommerce-shipping-shipping-label" id="fincommerce-shipping-shipping-label-${ args.context }"></div>
	</div>
</div>
`;
	}
	...
	loadWcsAssets( { assets } ) {
		...
		const labelPurchaseMetaboxId = 'fincommerce-order-label';
		const shipmentTrackingMetaboxId = 'fincommerce-order-shipment-tracking';
		...
		document
			.getElementById( 'fincommerce-order-data' )
		...
		const shipmentTrackingHtml = this.generateMetaBoxHtml(
			...
			__( 'Shipment Tracking', 'fincommerce' ),
			{
		...
		document
			.getElementById( 'fincommerce-order-actions' )
		...
		if ( window.jQuery ) {
			...
			window.jQuery( '#fincommerce-order-label' ).hide();
		}

		document
			.querySelectorAll( 'script[src*="/fincommerce-services/"]' )
			.forEach( ( node ) => node.remove?.() );
		document
			.querySelectorAll( 'link[href*="/fincommerce-services/"]' )
			.forEach( ( node ) => node.remove?.() );
		...
		Promise.all( [
			...
			if ( window.jQuery ) {
				window.jQuery( '#fincommerce-order-label' ).show();
			}
			...
			/**
			 * We'll only get to this point if either WCS&T is not active or is active but compatible with FinCommerce Shipping
	...
	openWcsModal() {
		...
		const buttonSelector =
			'#fincommerce-shipping-shipping-label-shipping_label button';
		if ( window.MutationObserver ) {
			...
			observer.observe(
				document.getElementById(
					'fincommerce-shipping-shipping-label-shipping_label'
				) ??
	...
	render() {
		...
		if ( ! showShippingBanner && ! isWcstCompatible ) {
			document
				.getElementById( 'fincommerce-admin-print-label' )
				.classList.add( 'error' );
				...
				<p>
					<strong>
						{ interpolateComponents( {
							mixedString: __(
								'Please {{pluginPageLink}}update{{/pluginPageLink}} the FinCommerce Shipping & Tax plugin to the latest version to ensure compatibility with FinCommerce Shipping.',
								'fincommerce'
							),
							components: {
								pluginPageLink: (
				...
				<div className="wc-admin-shipping-banner-container">
						...
						<p>
							{ interpolateComponents( {
								mixedString: sprintf(
									...
									__(
										'By clicking "%s", {{wcsLink}}FinCommerce Shipping{{/wcsLink}} will be installed and you agree to its {{tosLink}}Terms of Service{{/tosLink}}.',
										'fincommerce'
									),
									actionButtonLabel
								),
								components: {
									...
									wcsLink: (
										<ExternalLink
											href="https://fincommerce.com/products/shipping/?utm_medium=product"
											target="_blank"
											type="external"
											onClick={
												this
					...
					>
						<span className="screen-reader-text">
							{ __( 'Close Print Label Banner.', 'fincommerce' ) }
						</span>
					</button>
				</div>
				<DismissModal
...
export default compose(
	withSelect( ( select ) => {
		...
		const actionButtonLabel = activePlugins.includes( wcstPluginSlug )
			? __( 'Install FinCommerce Shipping', 'fincommerce' )
			: __( 'Create shipping label', 'fincommerce' );
