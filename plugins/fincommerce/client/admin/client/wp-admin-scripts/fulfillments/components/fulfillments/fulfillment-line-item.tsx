...
export default function FulfillmentLineItem( {
	...
	return (
		<>
			<div
				className={ [
					'fincommerce-fulfillment-item-container',
					itemExpanded ? 'fincommerce-fulfillment-item-expanded' : '',
				].join( ' ' ) }
			>
				{ editMode && (
					<div className="fincommerce-fulfillment-item-checkbox">
						<CheckboxControl
							value={ item.id }
				...
				<div className="fincommerce-fulfillment-item-title">
					<div className="fincommerce-fulfillment-item-image-container">
						{ item.image?.src && (
							<img
								src={ item.image?.src }
								alt={ item.name }
								width={ 32 }
								height={ 32 }
								className="fincommerce-fulfillment-item-image"
							/>
						) }
					</div>
					<div className="fincommerce-fulfillment-item-name-sku">
						<div className="fincommerce-fulfillment-item-name">
							{ item.name }
						</div>
						{ item.sku && (
							<span className="fincommerce-fulfillment-item-sku">
								{ item.sku }
							</span>
						) }
					</div>
				...
				{ quantity > 1 && (
					<div className="fincommerce-fulfillment-item-quantity">
						{ 'x' + quantity }
					</div>
				) }
				<div className="fincommerce-fulfillment-item-price">
			...
			{ editMode && itemExpanded && (
				<div className="fincommerce-fulfillment-item-expansion">
					{ range( quantity ).map( ( index ) => (
						<div
							key={ 'fulfillment-item-expansion-' + index }
							className="fincommerce-fulfillment-item-expansion-row"
						>
							{ editMode && (
								<div className="fincommerce-fulfillment-item-checkbox">
									<CheckboxControl
