...
export default function ProductUsageNoticeModal( {
	...
	const renderBenefits = () => {
		const subtitle = isExpired
			...
			: __( 'Purchase a subscription to benefit from:', 'fincommerce' );

		const benefits = [
			{
				key: 'get-updates',
				icon: reusableBlock,
				title: __( 'Improvements and security updates', 'fincommerce' ),
				content: __(
			...
			},
			{
				key: 'get-supports',
				icon: commentContent,
				title: __( 'Help when you need it', 'fincommerce' ),
				content: __(
			...
			},
			{
				...
				icon: people,
				title: __( 'Supporting the ecosystem', 'fincommerce' ),
				content: __(
					'A subscription helps us to continuously improve your extensions, themes, and FinCommerce experience.',
					'fincommerce'
				),
			},
		];
		...
		return (
			<div className="fincommerce-subscription-benefits">
				<h3>{ subtitle }</h3>

				{ benefits.map( ( { key, icon, title, content } ) => (
					<div
						className="fincommerce-subscription-benefits__item"
						key={ key }
					>
						<div className="fincommerce-subscription-benefits__icon">
							<Icon icon={ icon } />
						</div>

						<div className="fincommerce-subscription-benefits__content">
	...
	const renderPrimaryCard = () => {
		const status = isExpired
			? __( 'Expired', 'fincommerce' )
			: __( 'Unregistered', 'fincommerce' );

		const title = isExpired
			? sprintf(
					/* translators: %s is product name */
					__( 'Renew %s', 'fincommerce' ),
					productName
			  )
			: sprintf(
					/* translators: %s is product name */
					__( 'Subscribe to %s', 'fincommerce' ),
					productName
			  );
		...
		const buttonLabel = isExpired
			? sprintf(
					...
					__( 'Renew for $%s', 'fincommerce' ),
					productRegularPrice
			  )
			: sprintf(
					/* translators: %s is product price */
					__( 'Subscribe for $%s', 'fincommerce' ),
					productRegularPrice
			  );
	...
	return (
		<Modal
			...
			className="fincommerce-product-usage-notice"
		>
			<Flex
				gap={ 0 }
				align={ 'stretch' }
