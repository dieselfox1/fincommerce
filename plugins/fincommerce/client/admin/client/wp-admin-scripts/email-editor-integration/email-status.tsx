...
export function EmailStatus( {
	...
	const isManual = fincommerce_email_data?.is_manual;
	let statusValue = 'enabled';
	if ( isManual ) {
		statusValue = 'manual';
	} else if ( ! fincommerce_email_data?.enabled ) {
	...
	const updateStatus = ( newValue: boolean ): void => {
		...
		// @ts-expect-error Property 'fincommerce_data' does not exist on type 'Updatable<Attachment<any>>'.
						...
						renderToggle={ ( { isOpen, onToggle } ) => (
							<Button
								...
								aria-label={ sprintf(
									...
									__( 'Change status: %s', 'fincommerce' ),
									status.label
								) }
