 ...
 */
import { addCustomerEffortScoreExitPageListener } from '@fincommerce/customer-effort-score';
...
export const getProductData = () => {
	...
	const productData = {
		...
		manage_stock: (
			...
			? 'Yes'
			: 'No',
		attributes: document.querySelectorAll( '.fincommerce_attribute' )
			.length,
...
const attachProductAttributesTracks = () => {
	...
	attachEventListenerToParentForChildren( attributesSection, [
		{
			eventName: 'click',
			childQuery: '.fincommerce_attribute_visible_on_product_page',
		...
		{
			eventName: 'click',
			childQuery: '.fincommerce_attribute_used_for_variations',
			callback: ( clickedElement ) => {
				const elementName = clickedElement.getAttribute( 'name' );
	...
	document
		...
		?.addEventListener( 'click', ( event ) => {
			...
			const localAttributesCount = document.querySelectorAll(
				'.fincommerce_attribute:not(.taxonomy)'
			).length;

			const globalAttributesCount = document.querySelectorAll(
				'.fincommerce_attribute.taxonomy'
			).length;
...
const attachProductScreenTracks = () => {
	...
	document
		.querySelector(
			'#fincommerce-product-updated-message-view-product__link'
		)
		?.addEventListener( 'click', () => {
