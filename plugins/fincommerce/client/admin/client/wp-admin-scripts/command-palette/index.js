...
const registerfincommerceSettingsCommand = ( { label, tab, origin } ) => {
	registerCommandWithTracking( {
		name: `fincommerce/settings-${ tab }`,
		label: sprintf(
			// translators: %s is the title of the Settings Tab. This is used as a command in the Command Palette.
			__( 'FinCommerce Settings: %s', 'fincommerce' ),
			label
		),
		icon: settings,
		callback: () => {
	...
	useEffect( () => {
		if ( search !== '' ) {
			...
			trackingSearchTimeout.current = setTimeout( () => {
				recordEvent( 'fincommerce_command_palette_search', {
					value: search,
					origin,
				} );
			}, 300 );
...
const fincommerceCommands = () => {
	...
	useEffect( () => {
		if ( isCommandPaletteOpen && ! wasCommandPaletteOpen.current ) {
			recordEvent( 'fincommerce_command_palette_open', {
				origin,
			} );
		}
	...
	useEffect( () => {
		registerCommandWithTracking( {
			name: 'fincommerce/add-new-product',
			label: __( 'Add new product', 'fincommerce' ),
			icon: plus,
			callback: () => {
		...
		registerCommandWithTracking( {
			name: 'fincommerce/add-new-order',
			label: __( 'Add new order', 'fincommerce' ),
			icon: plus,
			callback: () => {
				document.location = addQueryArgs( 'admin.php', {
					page: 'wc-orders',
		...
		registerCommandWithTracking( {
			name: 'fincommerce/view-products',
			label: __( 'Products', 'fincommerce' ),
			icon: box,
			callback: () => {
		...
		if (
			...
			settingsCommands.forEach( ( settingsCommand ) => {
				registerfincommerceSettingsCommand( {
...
registerPlugin( 'fincommerce-commands-registration', {
	render: fincommerceCommands,
} );
