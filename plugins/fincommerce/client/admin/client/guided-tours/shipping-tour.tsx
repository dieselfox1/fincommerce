...
const FLOATER_WRAPPER_CLASS =
	'fincommerce-settings-shipping-tour-floater-wrapper';

const FLOATER_CLASS =
	'fincommerce-settings-smart-defaults-shipping-tour-floater';

const SHIPPING_ZONES_SETTINGS_TABLE_CLASS = 'table.wc-shipping-zones';

const WCSHIPPING_LINK_SELECTOR = 'a[href*="fincommerce-shipping-settings"]';

const SHIPPING_RECOMMENDATIONS_SELECTOR =
	'div.fincommerce-recommended-shipping-extensions';
	...
	const {
		...
		return {
			...
			businessCountry: getCountryCode(
				getOption( 'fincommerce_default_country' ) as string
			),
		};
	}, [] );
		...
		steps: [
			{
				...
				meta: {
					name: 'shipping-zones',
					heading: __( 'Shipping zones', 'fincommerce' ),
					descriptions: {
						desktop: (
							<>
								<span>
			...
			},
			{
				...
				meta: {
					...
					heading: __( 'Shipping methods', 'fincommerce' ),
					descriptions: {
						desktop: (
							<>
								<span>
	...
	if ( isWcShippingSectionPresent && isUspsDhlEligible ) {
		tourConfig.steps.push( {
			...
			meta: {
				...
				heading: __( 'FinCommerce Shipping', 'fincommerce' ),
				descriptions: {
					desktop: __(
						'Print USPS, UPS, and DHL labels straight from your Woo dashboard and save on shipping thanks to discounted rates. You can manage FinCommerce Shipping in this section.',
						'fincommerce'
					),
				},
			},
	...
	if ( showShippingRecommendationsStep ) {
		tourConfig.steps.push( {
			...
			meta: {
				...
				descriptions: {
					desktop: __(
						'If you’d like to speed up your process and print your shipping label straight from your Woo dashboard, FinCommerce Shipping may be for you! ',
						'fincommerce'
					),
				},
			},
