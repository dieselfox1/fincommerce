...
render: function() {
	return (
		<Header
			sections={ [
				[ '/analytics', __( 'Analytics', 'fincommerce' ) ],
				__( 'Report Title', 'fincommerce' ),
			] }
		/>
  	);
}
...
* `sections` (required): Used to generate breadcrumbs. Accepts a single items or an array of items. To make an item a link, wrap it in an array with a relative link (example: `[ '/analytics', __( 'Analytics', 'fincommerce' ) ]` ).
