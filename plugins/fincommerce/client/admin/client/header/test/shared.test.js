...
// Mock dependencies
jest.mock( '@fincommerce/experimental', () => ( {
	useSlot: jest.fn(),
...
describe( 'getPageTitle', () => {
	...
	test( "should get page title as the second item's second element if section length is 3 or more and second item has a second element", () => {
		const sections = [
			[ 'admin.php?page=wc-admin', 'FinCommerce' ],
...
describe( 'BaseHeader', () => {
	...
	test( 'should render with default props', () => {
		...
		expect( header ).toHaveClass( 'fincommerce-layout__header-heading' );
		expect( header ).toHaveClass( 'fincommerce-layout__header-left-align' );

		// Check page title
		expect( header.textContent ).toBe( 'FinCommerce' );
	...
	test( 'should render with right alignment when leftAlign is false', () => {
		...
		expect( header ).not.toHaveClass(
			'fincommerce-layout__header-left-align'
		);
	} );
