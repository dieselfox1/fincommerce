...
const parseBlueprintSteps = async ( file: File ) => {
	...
	if ( ! Array.isArray( steps ) ) {
		throw new Error(
			__( 'Invalid JSON format: Expected an array.', 'fincommerce' )
		);
	}
...
const importBlueprint = async ( steps: BlueprintStep[] ) => {
	...
	try {
		...
		for ( const step of steps ) {
			...
			if ( step?.options?.fincommerce_coming_soon === 'no' ) {
				continue;
			}
		...
		if ( errors.length > 0 ) {
			...
			dispatch( 'core/notices' ).createSuccessNotice(
				`${ __( 'Your Blueprint has been imported!', 'fincommerce' ) }`,
				{
...
const checkImportAllowed = async (): Promise< boolean > => {
	try {
		...
		throw new Error(
			__( 'Failed to check if imports are allowed.', 'fincommerce' )
		);
	}
};
	...
	return (
		<>
						...
						>
							<div className="blueprint-upload-dropzone">
								...
								<p className="blueprint-upload-dropzone-text">
									{ __( 'Drag and drop or ', 'fincommerce' ) }
									<span>
										{ __( 'choose a file', 'fincommerce' ) }
									</span>
								</p>
			...
			{ state.matches( 'importing' ) && (
				<div className="blueprint-upload-form">
					<div className="blueprint-upload-dropzone-uploading">
						...
						<p className="blueprint-upload-dropzone-text">
							{ __( 'Importing your file…', 'fincommerce' ) }
						</p>
					</div>
				</div>
			) }
			...
			{ ( state.matches( 'success' ) ||
				state.matches( 'overrideModal' ) ) && (
				<Button
					className="fincommerce-blueprint-import-button"
					variant="primary"
