...
const WC_ANALYTICS_PLUGIN_SLUG = 'fincommerce-analytics';
...
export const OrderAttributionInstallBanner = ( {
	...
	if ( bannerType === BANNER_TYPE_HEADER ) {
		return (
			<Button
				className="fincommerce-order-attribution-install-header-banner"
				variant="secondary"
				icon={ plugins }
				size="default"
			...
			>
				{ __( 'Try Order Attribution', 'fincommerce' ) }
			</Button>
		);
	}
	...
	return (
		<Card
			size="medium"
			className={ `fincommerce-order-attribution-install-banner ${
				isSmallBanner ? 'small' : ''
			}` }
		>
			<CardBody
				className={ `fincommerce-order-attribution-install-banner__body ${
					isSmallBanner ? 'small' : ''
				}` }
			>
				<div className="fincommerce-order-attribution-install-banner__image_container">
					{ bannerImage }
				</div>
				<div
					className={ `fincommerce-order-attribution-install-banner__text_container ${
						isSmallBanner ? 'small' : ''
					}` }
				>
					{ badgeText && (
						<div className="fincommerce-order-attribution-install-banner__text-badge">
							<Text
								className="fincommerce-order-attribution-install-banner__text-description"
								as="p"
								size="12"
								align="center"
							>
					...
					{ title && (
						<Text
							className="fincommerce-order-attribution-install-banner__text-title"
							as="p"
							size="16"
						>
							{ title }
