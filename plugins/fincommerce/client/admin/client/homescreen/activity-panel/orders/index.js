...
import { ordersStore, itemsStore } from '@fincommerce/data';
import { recordEvent } from '@fincommerce/tracks';
import { CurrencyContext, CurrencyFactory } from '@fincommerce/currency';
...
const renderEmptyCard = () => {
	return (
		<>
			<ActivityCard
				className="fincommerce-empty-activity-card"
				title=""
				icon=""
			>
				<span
					className="fincommerce-order-empty__success-icon"
					role="img"
					aria-labelledby="fincommerce-order-empty-message"
				>
					🎉
				</span>
				<H id="fincommerce-order-empty-message">
					{ __( 'You’ve fulfilled all your orders', 'fincommerce' ) }
				</H>
			</ActivityCard>
			<Link
				...
				type="wp-admin"
			>
				{ __( 'Manage all orders', 'fincommerce' ) }
			</Link>
		</>
	);
};
...
function renderOrders( orders, customers, getFormattedOrderTotal ) {
	...
	orders.forEach( ( order ) => {
		...
		cards.push(
			<ActivityCard
				key={ orderId }
				className="fincommerce-order-activity-card"
	...
	return (
		<>
			...
			<Link
				href={ 'edit.php?post_type=shop_order' }
				className="fincommerce-layout__activity-panel-outbound-link"
				onClick={ () => recordOrderEvent( 'orders_manage' ) }
				type="wp-admin"
			>
				{ __( 'Manage all orders', 'fincommerce' ) }
		...
		if ( ! orderStatuses.length && window.wcAdminFeatures.analytics ) {
			return (
				<EmptyContent
					...
					actionLabel={ __( 'Settings', 'fincommerce' ) }
