...
export class ProductStockCard extends Component {
		...
		if ( success ) {
			createNotice(
				...
				sprintf(
					...
					__( '%s stock updated', 'fincommerce' ),
					product.name
				),
				{
					actions: [
			...
			createNotice(
				...
				sprintf(
					/* translators: %s = name of the product having stock updated */
					__( '%s stock could not be updated', 'fincommerce' ),
					product.name
				)
			);
		}
	...
	getBody() {
		...
		if ( editing ) {
			return (
				<Fragment>
					<BaseControl className="fincommerce-stock-activity-card__edit-quantity">
						<input
					...
					<span>{ __( 'in stock', 'fincommerce' ) }</span>
				</Fragment>
			);
		}
		...
		return (
			<span
				className={ clsx(
					'fincommerce-stock-activity-card__stock-quantity',
					{
	...
	render() {
		...
		const productImageClasses = clsx(
			'fincommerce-stock-activity-card__image-overlay__product',
			{
				'is-placeholder': ! productImage || ! productImage.src,
			}
		);
		const icon = (
			<div className="fincommerce-stock-activity-card__image-overlay">
				<div className={ productImageClasses }>
					<ProductImage product={ product } />
				</div>
			</div>
		);
		const activityCardClasses = clsx( 'fincommerce-stock-activity-card', {
