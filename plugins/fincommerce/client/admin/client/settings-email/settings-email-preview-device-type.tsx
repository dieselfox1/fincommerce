	...
	return (
		<div className="wc-settings-email-preview-device-type">
			<button
				...
				onClick={ setDesktop }
				title={ __( 'Email preview on desktop', 'fincommerce' ) }
				type="button"
			>
				<img
					src={ isDesktop ? desktopActiveIcon : desktopIcon }
					alt={ __( 'Desktop icon', 'fincommerce' ) }
				/>
			</button>
			<button
				className={ isMobile ? 'active' : '' }
				onClick={ setMobile }
				title={ __( 'Mobile preview on desktop', 'fincommerce' ) }
				type="button"
			>
				<img
					src={ isMobile ? mobileActiveIcon : mobileIcon }
					alt={ __( 'Mobile icon', 'fincommerce' ) }
				/>
			</button>
		</div>
	);
