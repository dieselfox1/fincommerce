...
export const ListView = ( { emailTypes }: { emailTypes: EmailType[] } ) => {
		...
		return [
			{
				...
				render: ( row: { item: EmailType } ) => {
					return (
						<div className="fincommerce-email-listing-title">
							{ row.item.title }
							<br />
							<span className="fincommerce-email-listing-description">
			...
			{
				id: 'recipients',
				label: __( 'Recipient(s)', 'fincommerce' ),
				enableHiding: true,
				filterBy: {
					operators: [ 'isAny' ],
				},
	...
	const actions = useMemo(
		() => [
			...
			{
				id: 'test',
				label: __( 'Send test email', 'fincommerce' ),
				disabled: true,
				supportsBulk: false,
				callback: () => {
			...
			},
			{
				...
				label: ( items: EmailType[] ) =>
					items[ 0 ].status === 'enabled'
						? __( 'Disable email', 'fincommerce' )
						: __( 'Enable email', 'fincommerce' ),
				supportsBulk: false,
			...
			},
			{
				...
				label: __( 'Recreate email post', 'fincommerce' ),
				disabled: false,
				supportsBulk: false,
				isEligible: ( item: EmailType ) => ! item?.post_id,
				callback: ( items: EmailType[] ) => {
