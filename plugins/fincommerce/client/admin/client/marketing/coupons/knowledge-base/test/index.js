...
const mockPosts = [
	{
		title: 'FinCommerce Blog Post 1',
		date: '2020-05-28T15:00:00',
		link: 'https://fincommerce.com/posts/woo-blog-post-1/',
		author_name: 'John Doe',
		author_avatar: 'https://avatar.domain/avatar1.png',
	},
	{
		title: 'FinCommerce Blog Post 2',
		date: '2020-04-29T12:00:00',
		link: 'https://fincommerce.com/posts/woo-blog-post-2/',
		author_name: 'Jane Doe',
		author_avatar: 'https://avatar.domain/avatar2.png',
	},
	{
		title: 'FinCommerce Blog Post 3',
		date: '2020-03-29T12:00:00',
		link: 'https://fincommerce.com/posts/woo-blog-post-3/',
		author_name: 'Jim Doe',
...
describe( 'Posts and not loading', () => {
	...
	it( 'should display default title and description', () => {
		...
		expect( getByText( 'FinCommerce knowledge base' ) ).toBeInTheDocument();

		expect(
			getByText(
				'Learn the ins and outs of successful marketing from the experts at FinCommerce.'
			)
		).toBeInTheDocument();
	} );
	...
	it( 'should display posts wrapper', () => {
		...
		expect(
			container.getElementsByClassName(
				'fincommerce-marketing-knowledgebase-card__posts'
			)
		).toHaveLength( 1 );
	} );
	...
	it( 'should display the slider', () => {
		const { container } = knowledgeBaseWrapper;
		expect(
			container.getElementsByClassName( 'fincommerce-marketing-slider' )
		).toHaveLength( 1 );
	} );
	...
	it( 'should display correct number of posts', () => {
		...
		expect(
			container.getElementsByClassName(
				'fincommerce-marketing-knowledgebase-card__page'
			)
		).toHaveLength( 1 );

		expect(
			container.getElementsByClassName(
				'fincommerce-marketing-knowledgebase-card__post'
			)
		).toHaveLength( 2 );
	} );
	...
	it( 'should not display the empty content component', () => {
		...
		expect( queryByText( 'the FinCommerce blog' ) ).toBeNull();
		expect(
...
describe( 'No posts and loading', () => {
	...
	it( 'should display placeholder', () => {
		...
		expect(
			container.querySelector(
				'.is-loading.fincommerce-marketing-knowledgebase-card__post'
			)
		).toBeTruthy();
	} );
	...
	it( 'should not display slider', () => {
		...
		expect(
			container.getElementsByClassName(
				'fincommerce-marketing-slider__slide'
			)
		).toHaveLength( 0 );
	} );
...
describe( 'Error and not loading', () => {
	...
	it( 'should display the error component', () => {
		...
		expect( getByText( 'the FinCommerce blog' ) ).toBeInTheDocument();
		expect(
...
describe( 'Pagination', () => {
	...
	it( 'should be able to click forward and back', async () => {
		...
		await waitFor( () =>
			expect(
				container.getElementsByClassName(
					'fincommerce-marketing-slider animate-left'
				)
			).toHaveLength( 1 )
		);
		...
		await waitFor( () =>
			expect(
				container.getElementsByClassName(
					'fincommerce-marketing-slider animate-right'
				)
			).toHaveLength( 1 )
		);
