...
const KnowledgeBase = ( {
	...
	isLoading,
	error,
	title = __( 'FinCommerce knowledge base', 'fincommerce' ),
	description = __(
	...
	const getCurrentSlide = () => {
		...
		const displayPosts = currentPosts.map( ( post, index ) => {
			return (
				<a
					className="fincommerce-marketing-knowledgebase-card__post"
					href={ post.link }
					key={ index }
					onClick={ () => {
						onPostClick( post );
				...
				>
					{ !! post.image && (
						<div className="fincommerce-marketing-knowledgebase-card__post-img">
							<img src={ post.image } alt="" />
						</div>
					) }
					<div className="fincommerce-marketing-knowledgebase-card__post-text">
						<h3>{ post.title }</h3>
						<p className="fincommerce-marketing-knowledgebase-card__post-meta">
							{ __( 'By', 'fincommerce' ) + ' ' }
							{ post.author_name }
							{ !! post.author_avatar && (
								<img
									...
									className="fincommerce-gravatar"
									alt=""
									width="16"
									height="16"
								/>
	...
	const renderEmpty = () => {
		const emptyTitle = __( 'No posts yet', 'fincommerce' );
	...
	const renderPosts = () => {
		return (
			<div className="fincommerce-marketing-knowledgebase-card__posts">
	...
	const renderPlaceholder = () => {
		return (
			<div className="fincommerce-marketing-knowledgebase-card__posts">
				<div className="fincommerce-marketing-knowledgebase-card__page">
	...
	const categoryClass = category
		? `fincommerce-marketing-knowledgebase-card__category-${ category }`
		: '';
	...
	return (
		<Card
			...
			className={ clsx(
				'fincommerce-marketing-knowledgebase-card',
				categoryClass
			) }
		>
			{ renderCardBody() }
