...
export const LearnMarketing = () => {
	...
	const renderFooter = () => {
		if ( isLoading ) {
			return (
				<div
					role="progressbar"
					className="fincommerce-marketing-learn-marketing-card__footer--placeholder"
				/>
			);
		}
	...
	const renderBody = () => {
		...
		if ( posts.length === 0 ) {
			return (
				<EmptyContent
					title={ __( 'No posts yet', 'fincommerce' ) }
					message={ <ReadBlogMessage /> }
					illustration=""
					actionLabel=""
				/>
	...
	return (
		<CollapsibleCard
			initialCollapsed
			className="fincommerce-marketing-learn-marketing-card"
			header={ __( 'Learn about marketing a store', 'fincommerce' ) }
			footer={ renderFooter() }
		>
			<div className="fincommerce-marketing-learn-marketing-card__body">
				{ renderBody() }
			</div>
		</CollapsibleCard>
	);
