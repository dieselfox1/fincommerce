...
} = require( '@fincommerce/internal-style-build' );
const fincommerceDependencyExtractionWebpackPlugin = require( '@fincommerce/dependency-extraction-webpack-plugin/src/index' );
...
// wpAdminScripts are loaded on wp-admin pages outside the context of FinCommerce Admin
...
const webpackConfig = {
	...
	output: {
		filename: ( data ) => {
			...
			// See https://github.com/dieselfox1/fincommerce-admin/pull/3061
		...
		library: {
			// Expose the exports of entry points so we can consume the libraries in window.wc.[modulename] with fincommerceDependencyExtractionWebpackPlugin.
			name: [ 'wc', '[modulename]' ],
			type: 'window',
		},
		...
		rules: [
			{
				...
				parser: {
					...
					// See https://github.com/dieselfox1/fincommerce-admin/pull/1004 and https://github.com/Automattic/fincommerce-services/pull/1522
					amd: false,
				},
				exclude: [
					/[\/\\]node_modules[\/\\]\.pnpm[\/\\]/,
						...
						presets: [
							...
							[
								'@babel/preset-env',
								{
									...
									// See https://github.com/dieselfox1/fincommerce-admin/pull/6411/
									corejs: '3',
									useBuiltIns: 'usage',
								},
							],
	...
	resolve: {
		fallback: {
			...
			// See https://github.com/dieselfox1/fincommerce-admin/pull/5768
			crypto: 'empty',
	...
	plugins: [
		...
		new CopyWebpackPlugin( {
			patterns: wcAdminPackages.map( ( packageName ) => ( {
				...
				// This is required for @fincommerce/component to use @automattic/* packages because scss styles from @automattic/* packages will be automatically generated by mini-css-extract-plugin with the same output name.
				force: true,
			} ) ),
		} ),
		...
		! process.env.STORYBOOK &&
			new fincommerceDependencyExtractionWebpackPlugin( {
				requestToExternal( request ) {
					...
					// Skip requesting to external if the import path is from the build or build-module directory for FinCommerce packages.
					// This can reduce the bundle size when we don't need to load the entire FinCommerce package.
					if (
						request.match( /^@fincommerce\/.*\/build(?:-module)?/ )
					) {
						return null;
					}
				},
	...
	optimization: {
		...
		splitChunks: {
			...
			// See https://github.com/dieselfox1/fincommerce-admin/pull/5229
			name: false,
		},
	},
};
