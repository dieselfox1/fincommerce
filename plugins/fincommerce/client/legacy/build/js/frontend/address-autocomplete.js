/**
 * Address provider registration for FinCommerce shortcode checkout
 */
	...
	function setActiveProvider( country, type ) {
		...
		for ( const serverProvider of serverProviders ) {
			...
			if ( provider && provider.canSearch( country ) ) {
				...
				// Add autocomplete-available class to parent .fincommerce-input-wrapper
				const addressInput = document.getElementById(
					`${ type }_address_1`
				);
				if ( addressInput ) {
					const wrapper = addressInput.closest(
						'.fincommerce-input-wrapper'
					);
					if ( wrapper ) {
		...
		// Remove autocomplete-available class from parent .fincommerce-input-wrapper
		...
		addressTypes.forEach( ( type ) => {
			...
			if ( addressInput ) {
				...
				if (
					...
					container.className = 'fincommerce-address-suggestions';
