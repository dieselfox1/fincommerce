...
jQuery( function( $ ) {
	...
	$( 'body' )
		// Tabs
		.on( 'init', '.wc-tabs-wrapper, .fincommerce-tabs', function() {
			$( this ).find( '.wc-tab, .fincommerce-tabs .panel:not(.panel .panel)' ).hide();
		...
		.on( 'click', '.wc-tabs li a, ul.tabs li a', function( e ) {
			e.preventDefault();
			var $tab          = $( this );
			var $tabs_wrapper = $tab.closest( '.wc-tabs-wrapper, .fincommerce-tabs' );
		...
		// Review link
		.on( 'click', 'a.fincommerce-review-link', function() {
	...
	$( '.wc-tabs-wrapper, .fincommerce-tabs, #rating' ).trigger( 'init' );
	...
	var ProductGallery = function( $target, args ) {
		this.$target = $target;
		this.$images = $( '.fincommerce-product-gallery__image', $target );
		...
		if ( this.flexslider_enabled ) {
			...
			$target.on( 'fincommerce_gallery_reset_slide_position', this.onResetSlidePosition );
		} else {
			this.$target.css( 'opacity', 1 );
		}
		...
		if ( this.zoom_enabled ) {
			this.initZoom();
			$target.on( 'fincommerce_gallery_init_zoom', this.initZoom );
		}
	...
	ProductGallery.prototype.initFlexslider = function( args ) {
		...
		var options = $.extend( {
			selector: '.fincommerce-product-gallery__wrapper > .fincommerce-product-gallery__image',
			start: function() {
		...
		$( '.fincommerce-product-gallery__wrapper .fincommerce-product-gallery__image:eq(0) .wp-post-image' ).one( 'load', function() {
			var $image = $( this );

			if ( $image ) {
				setTimeout( function() {
					var setHeight = $image.closest( '.fincommerce-product-gallery__image' ).height();
	...
	ProductGallery.prototype.initPhotoswipe = function() {
		if ( this.zoom_enabled && this.$images.length > 0 ) {
			this.$target.prepend(
				'<a href="#" role="button" class="fincommerce-product-gallery__trigger" aria-haspopup="dialog" ' +
				'aria-controls="photoswipe-fullscreen-dialog" aria-label="' +
				wc_single_product_params.i18n_product_gallery_trigger_text + '">' +
					'<span aria-hidden="true">🔍</span>' +
				'</a>'
			);
			this.$target.on( 'click', '.fincommerce-product-gallery__trigger', this.openPhotoswipe );
			this.$target.on( 'click', '.fincommerce-product-gallery__image a', function( e ) {
				e.preventDefault();
			});

			// If flexslider is disabled, gallery images also need to trigger photoswipe on click.
			if ( ! this.flexslider_enabled ) {
				this.$target.on( 'click', '.fincommerce-product-gallery__image a', this.openPhotoswipe );
			}
		} else {
			this.$target.on( 'click', '.fincommerce-product-gallery__image a', this.openPhotoswipe );
		}
	...
	ProductGallery.prototype.openPhotoswipe = function( e ) {
		...
		if ( 0 < eventTarget.closest( '.fincommerce-product-gallery__trigger' ).length ) {
			clicked = this.$target.find( '.flex-active-slide' );
		} else {
			clicked = eventTarget.closest( '.fincommerce-product-gallery__image' );
		}
	 ...
	 */
	$( '.fincommerce-product-gallery' ).each( function() {
