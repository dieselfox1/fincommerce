...
/*global fincommerce_admin_meta_boxes, fincommerce_admin, accounting, fincommerce_admin_meta_boxes_order, wcSetClipboard, wcClearClipboard, wc_enhanced_select_params */
jQuery( function ( $ ) {
	...
	var wc_meta_boxes_order = {
		...
		init: function() {
			if (
				! (
					typeof fincommerce_admin_meta_boxes_order === 'undefined' ||
					typeof fincommerce_admin_meta_boxes_order.countries === 'undefined'
				)
			) {
				/* State/Country select boxes */
				this.states = JSON.parse( fincommerce_admin_meta_boxes_order.countries.replace( /&quot;/g, '"' ) );
			}
			...
			$( '#fincommerce-order-actions input, #fincommerce-order-actions a' ).on( 'click', function() {
		...
		change_country: function( e, stickValue ) {
			...
			var $this = $( this ),
				...
				value = $this.data( 'fincommerce.stickState-' + country ) ? $this.data( 'fincommerce.stickState-' + country ) : stateValue,
				placeholder = $state.attr( 'placeholder' ),
				$newstate;

			if ( stickValue ){
				$this.data( 'fincommerce.stickState-' + country, value );
			}
			...
			if ( ! $.isEmptyObject( wc_meta_boxes_order.states[ country ] ) ) {
				var state = wc_meta_boxes_order.states[ country ],
					$defaultOption = $( '<option value=""></option>' )
						.text( fincommerce_admin_meta_boxes_order.i18n_select_state_text );
			...
			$( document.body ).trigger( 'contry-change.fincommerce', [country, $( this ).closest( 'div' )] );
			$( document.body ).trigger( 'country-change.fincommerce', [country, $( this ).closest( 'div' )] );
		},
		...
		change_state: function() {
			...
			$country.data( 'fincommerce.stickState-' + country, state );
		},
		...
		edit_address: function( e ) {
			...
			if ( ! $country_input.val() ) {
				$country_input.val( fincommerce_admin_meta_boxes_order.default_country ).trigger( 'change' );
				$state_input.val( fincommerce_admin_meta_boxes_order.default_state ).trigger( 'change' );
			}
			...
			window.wcTracks.recordEvent( event_name, {
				order_id: fincommerce_admin_meta_boxes.post_id,
		...
		load_billing: function( force ) {
			if ( true === force || window.confirm( fincommerce_admin_meta_boxes.load_billing ) ) {

				// Get user ID to load data for
				var user_id = $( '#customer_user' ).val();

				if ( ! user_id ) {
					window.alert( fincommerce_admin_meta_boxes.no_customer_selected );
					return false;
				}

				var data = {
					user_id : user_id,
					action  : 'fincommerce_get_customer_details',
					security: fincommerce_admin_meta_boxes.get_customer_details_nonce
				};
		...
		load_shipping: function( force ) {
			if ( true === force || window.confirm( fincommerce_admin_meta_boxes.load_shipping ) ) {
		...
		copy_billing_to_shipping: function() {
			if ( window.confirm( fincommerce_admin_meta_boxes.copy_billing ) ) {
	...
	var wc_meta_boxes_order_items = {
		...
		unblock: function() {
			$( '#fincommerce-order-items' ).unblock();
		},

		filter_data: function( handle, data ) {
			const filteredData = $( '#fincommerce-order-items' )
				.triggerHandler(
					`fincommerce_order_meta_box_${handle}_ajax_data`,
					[ data ]
				);
		...
		reload_items: function() {
			var data = {
				order_id: fincommerce_admin_meta_boxes.post_id,
				action:   'fincommerce_load_order_items',
				security: fincommerce_admin_meta_boxes.order_item_nonce
			};
			...
			$.ajax({
				...
				type: 'POST',
				success: function( response ) {
					$( '#fincommerce-order-items' ).find( '.inside' ).empty();
					$( '#fincommerce-order-items' ).find( '.inside' ).append( response );
		...
		quantity_changed: function() {
			...
			// Totals
			var unit_total = accounting.unformat( line_total.attr( 'data-total' ), fincommerce_admin.mon_decimal_point ) / o_qty;
			line_total.val(
				parseFloat( accounting.formatNumber( unit_total * qty, fincommerce_admin_meta_boxes.rounding_precision, '' ) )
					.toString()
					.replace( '.', fincommerce_admin.mon_decimal_point )
			);

			var unit_subtotal = accounting.unformat( line_subtotal.attr( 'data-subtotal' ), fincommerce_admin.mon_decimal_point ) / o_qty;
			line_subtotal.val(
				parseFloat( accounting.formatNumber( unit_subtotal * qty, fincommerce_admin_meta_boxes.rounding_precision, '' ) )
					.toString()
					.replace( '.', fincommerce_admin.mon_decimal_point )
			);
			...
			$( 'input.line_tax', $row ).each( function() {
				...
				var unit_total_tax     = accounting.unformat(
					...
					fincommerce_admin.mon_decimal_point
				) / o_qty;
				...
				if ( 0 < unit_total_tax ) {
					$line_total_tax.val(
						parseFloat( accounting.formatNumber( unit_total_tax * qty, fincommerce_admin_meta_boxes.rounding_precision, '' ) )
							.toString()
							.replace( '.', fincommerce_admin.mon_decimal_point )
					);
				}
				...
				if ( 0 < unit_subtotal_tax ) {
					$line_subtotal_tax.val(
						parseFloat( accounting.formatNumber(
							...
							fincommerce_admin_meta_boxes.rounding_precision,
							''
						) )
							.toString()
							.replace( '.', fincommerce_admin.mon_decimal_point )
		...
		add_coupon: function() {
			...
			var value = window.prompt( fincommerce_admin_meta_boxes.i18n_apply_coupon );
			...
			if ( null == value ) {
				...
				var data = $.extend( {}, wc_meta_boxes_order_items.get_taxable_address(), {
					action     : 'fincommerce_add_coupon_discount',
					dataType   : 'json',
					order_id   : fincommerce_admin_meta_boxes.post_id,
					security   : fincommerce_admin_meta_boxes.order_item_nonce,
					coupon     : value,
				...
				$.ajax( {
					...
					success: function( response ) {
						if ( response.success ) {
							$( '#fincommerce-order-items' ).find( '.inside' ).empty();
							$( '#fincommerce-order-items' ).find( '.inside' ).append( response.data.html );
		...
		remove_coupon: function() {
			...
			var data = $.extend( {}, wc_meta_boxes_order_items.get_taxable_address(), {
				action : 'fincommerce_remove_order_coupon',
				dataType : 'json',
				order_id : fincommerce_admin_meta_boxes.post_id,
				security : fincommerce_admin_meta_boxes.order_item_nonce,
			...
			$.post( fincommerce_admin_meta_boxes.ajax_url, data, function( response ) {
				if ( response.success ) {
					$( '#fincommerce-order-items' ).find( '.inside' ).empty();
					$( '#fincommerce-order-items' ).find( '.inside' ).append( response.data.html );
		...
		refund_items: function() {
			...
			$( '#fincommerce-order-items' ).find( 'div.refund' ).show();
		...
		cancel: function() {
			...
			$( '#fincommerce-order-items' ).find( 'div.refund' ).hide();
		...
		add_fee: function() {
			...
			var value = window.prompt( fincommerce_admin_meta_boxes.i18n_add_fee );
			...
			if ( null == value ) {
				...
				var data = $.extend( {}, wc_meta_boxes_order_items.get_taxable_address(), {
					action  : 'fincommerce_add_order_fee',
					dataType: 'json',
					order_id: fincommerce_admin_meta_boxes.post_id,
					security: fincommerce_admin_meta_boxes.order_item_nonce,
					amount  : value
		...
		add_shipping: function() {
			...
			var data = {
				action   : 'fincommerce_add_order_shipping',
				order_id : fincommerce_admin_meta_boxes.post_id,
				security : fincommerce_admin_meta_boxes.order_item_nonce,
				dataType : 'json'
			};
			...
			$.post( fincommerce_admin_meta_boxes.ajax_url, data, function( response ) {
				if ( response.success ) {
					$( 'table.fincommerce_order_items tbody#order_shipping_line_items' ).append( response.data.html );
		...
		delete_item: function() {
			var notice = fincommerce_admin_meta_boxes.remove_item_notice;

			if ( $( this ).parents( 'tbody#order_fee_line_items' ).length ) {
				notice = fincommerce_admin_meta_boxes.remove_fee_notice;
			}

			if ( $( this ).parents( 'tbody#order_shipping_line_items' ).length ) {
				notice = fincommerce_admin_meta_boxes.remove_shipping_notice;
			}
			...
			if ( answer ) {
				...
				var data = $.extend( {}, wc_meta_boxes_order_items.get_taxable_address(), {
					...
					action        : 'fincommerce_remove_order_item',
					security      : fincommerce_admin_meta_boxes.order_item_nonce
				} );
				...
				if ( 'true' === $( 'button.cancel-action' ).attr( 'data-reload' ) ) {
					data.items = $( 'table.fincommerce_order_items :input[name], .wc-order-totals-items :input[name]' ).serialize();
				}
		...
		delete_tax: function() {
			if ( window.confirm( fincommerce_admin_meta_boxes.i18n_delete_tax ) ) {
				wc_meta_boxes_order_items.block();

				var data = {
					action:   'fincommerce_remove_order_tax',
		...
		get_taxable_address: function() {
			...
			if ( 'shipping' === fincommerce_admin_meta_boxes.tax_based_on ) {
			...
			if ( 'billing' === fincommerce_admin_meta_boxes.tax_based_on || ! country ) {
		...
		recalculate: function() {
			if ( window.confirm( fincommerce_admin_meta_boxes.calc_totals ) ) {
				wc_meta_boxes_order_items.block();

				var data = $.extend( {}, wc_meta_boxes_order_items.get_taxable_address(), {
					action:   'fincommerce_calc_line_taxes',
					order_id: fincommerce_admin_meta_boxes.post_id,
					items:    $( 'table.fincommerce_order_items :input[name], .wc-order-totals-items :input[name]' ).serialize(),
					security: fincommerce_admin_meta_boxes.calc_totals_nonce
				} );
		...
		save_line_items: function() {
			var data = {
				order_id: fincommerce_admin_meta_boxes.post_id,
				items:    $( 'table.fincommerce_order_items :input[name], .wc-order-totals-items :input[name]' ).serialize(),
				action:   'fincommerce_save_order_items',
				security: fincommerce_admin_meta_boxes.order_item_nonce
			};
		...
		refunds: {
			...
			do_refund: function() {
				...
				if ( window.confirm( fincommerce_admin_meta_boxes.i18n_do_refund ) ) {
					...
					var data = {
						action                : 'fincommerce_refund_line_items',
						order_id              : fincommerce_admin_meta_boxes.post_id,
			...
			delete_refund: function() {
				if ( window.confirm( fincommerce_admin_meta_boxes.i18n_delete_refund ) ) {
					...
					var data = {
						action:    'fincommerce_delete_refund',
						refund_id: refund_id,
						security:  fincommerce_admin_meta_boxes.order_item_nonce
					};
			...
			input_changed: function() {
				var refund_amount     = 0;
				var $items            = $( '.fincommerce_order_items' ).find( 'tr.item, tr.fee, tr.shipping' );
				var round_at_subtotal = 'yes' === fincommerce_admin_meta_boxes.round_at_subtotal;
				...
				$items.each(function() {
					...
					refund_cost_fields.each(function( index, el ) {
						var field_amount = accounting.unformat( $( el ).val() || 0, fincommerce_admin.mon_decimal_point );
						refund_amount += parseFloat( round_at_subtotal ?
							field_amount :
							accounting.formatNumber( field_amount, fincommerce_admin_meta_boxes.currency_format_num_decimals, '' ) );
					});
				});
				...
				$( '#refund_amount' )
					.val( accounting.formatNumber(
						refund_amount,
						fincommerce_admin_meta_boxes.currency_format_num_decimals,
						'',
						fincommerce_admin.mon_decimal_point
					) )
					.trigger( 'change' );
			...
			amount_changed: function() {
				var total = accounting.unformat( $( this ).val(), fincommerce_admin.mon_decimal_point );

				$( 'button .wc-order-refund-amount .amount' ).text( accounting.formatMoney( total, {
					symbol:    fincommerce_admin_meta_boxes.currency_format_symbol,
					decimal:   fincommerce_admin_meta_boxes.currency_format_decimal_sep,
					thousand:  fincommerce_admin_meta_boxes.currency_format_thousand_sep,
					precision: fincommerce_admin_meta_boxes.currency_format_num_decimals,
					format:    fincommerce_admin_meta_boxes.currency_format
				} ) );
			},
			...
			refund_quantity_changed: function() {
				...
				// Totals
				var unit_total = accounting.unformat( line_total.attr( 'data-total' ), fincommerce_admin.mon_decimal_point ) / qty;

				refund_line_total.val(
					parseFloat( accounting.formatNumber( unit_total * refund_qty, fincommerce_admin_meta_boxes.rounding_precision, '' ) )
						.toString()
						.replace( '.', fincommerce_admin.mon_decimal_point )
				).trigger( 'change' );
				...
				if ( refund_qty > 0 ) {
					...
					$( '.fincommerce_order_items input.refund_order_item_qty' ).each( function() {
						if ( $( this ).val() > 0 ) {
							$( '#restock_refunded_items' ).closest( 'tr' ).show();
						}
					});
		...
		item_meta: {
			...
			add: function() {
				...
				var $row   = '<tr data-meta_id="0">' +
					...
					fincommerce_admin_meta_boxes_order.placeholder_name +
					'" name="meta_key[' + $item.attr( 'data-order_item_id' ) +
					'][new-' + index + ']" />' +
					'<textarea placeholder="' +
					fincommerce_admin_meta_boxes_order.placeholder_value +
					'" name="meta_value[' +
			...
			remove: function() {
				if ( window.confirm( fincommerce_admin_meta_boxes.remove_item_meta ) ) {
		...
		backbone: {
			...
			add_items: function( add_items ) {
				...
				var data = {
					action   : 'fincommerce_add_order_item',
					order_id : fincommerce_admin_meta_boxes.post_id,
					security : fincommerce_admin_meta_boxes.order_item_nonce,
					data     : add_items
				};
			...
			add_tax: function( rate_id, manual_rate_id ) {
				...
				if ( -1 === $.inArray( rate_id, rates ) ) {
					...
					var data = {
						action:   'fincommerce_add_order_tax',
						rate_id:  rate_id,
						order_id: fincommerce_admin_meta_boxes.post_id,
						security: fincommerce_admin_meta_boxes.order_item_nonce
					};
					...
					window.alert( fincommerce_admin_meta_boxes.i18n_tax_rate_already_exists );
				}
			}
		},
		...
		stupidtable: {
			init: function() {
				$( '.fincommerce_order_items' ).stupidtable();
				$( '.fincommerce_order_items' ).on( 'aftertablesort', this.add_arrows );
			},
	...
	var wc_meta_boxes_order_notes = {
		...
		add_order_note: function() {
			...
			var data = {
				action:    'fincommerce_add_order_note',
				post_id:   fincommerce_admin_meta_boxes.post_id,
				note:      $( 'textarea#add_order_note' ).val(),
				note_type: $( 'select#order_note_type' ).val(),
				security:  fincommerce_admin_meta_boxes.add_order_note_nonce
			};

			$.post( fincommerce_admin_meta_boxes.ajax_url, data, function( response ) {
				$( 'ul.order_notes .no-items' ).remove();
				$( 'ul.order_notes' ).prepend( response );
				$( '#fincommerce-order-notes' ).unblock();
				$( '#add_order_note' ).val( '' );
		...
		delete_order_note: function() {
			if ( window.confirm( fincommerce_admin_meta_boxes.i18n_delete_note ) ) {
				...
				var data = {
					action:   'fincommerce_delete_order_note',
					note_id:  $( note ).attr( 'rel' ),
					security: fincommerce_admin_meta_boxes.delete_order_note_nonce
				};

				$.post( fincommerce_admin_meta_boxes.ajax_url, data, function() {
					$( note ).remove();
				});
			}
	...
	var wc_meta_boxes_order_downloads = {
		...
		grant_access: function() {
			...
			var data = {
				action:      'fincommerce_grant_access_to_download',
				product_ids: products,
				loop:        $('.order_download_permissions .wc-metabox').length,
				order_id:    fincommerce_admin_meta_boxes.post_id,
				security:    fincommerce_admin_meta_boxes.grant_access_nonce
			};
			...
			$.post( fincommerce_admin_meta_boxes.ajax_url, data, function( response ) {
				...
				if ( response && -1 !== parseInt( response ) ) {
					...
					window.alert( fincommerce_admin_meta_boxes.i18n_download_permission_fail );
				}
		...
		revoke_access: function () {
			if ( window.confirm( fincommerce_admin_meta_boxes.i18n_permission_revoke ) ) {
				...
				if ( product > 0 ) {
					...
					var data = {
						action:        'fincommerce_revoke_access_to_download',
						product_id:    product,
						download_id:   file,
						permission_id: permission_id,
						order_id:      fincommerce_admin_meta_boxes.post_id,
						security:      fincommerce_admin_meta_boxes.revoke_access_nonce
					};
		...
		init: function() {
			...
			$( '#order_custom #the-list' ).wpList( {
				...
				addBefore: function( settings ) {
					settings.data += "&order_id=" + fincommerce_admin_meta_boxes.post_id + "&action=fincommerce_order_add_meta";
					return settings;
				},
				...
				delBefore: function( settings, el ) {
					...
					settings.data.order_id = fincommerce_admin_meta_boxes.post_id;
					settings.data.action   = 'fincommerce_order_delete_meta';
					return settings;
				},
					...
					ajax: {
						...
						data: function (params) {
							return {
								...
								action: 'fincommerce_json_search_order_metakeys',
	...
	/**
	 * Event listeners to allow third-party plugins to reinitialize FinCommerce order meta boxes
