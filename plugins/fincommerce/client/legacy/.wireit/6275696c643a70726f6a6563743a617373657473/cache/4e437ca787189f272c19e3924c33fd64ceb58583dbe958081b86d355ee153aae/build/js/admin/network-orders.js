/*global fincommerce_network_orders */
(function( $, _, undefined ) {

	if ( 'undefined' === typeof fincommerce_network_orders ) {
		return;
	}
	...
	var orders = [],
		...
		$loadingIndicator = $( document.getElementById( 'fincommerce-network-order-table-loading' ) ),
		$orderTable = $( document.getElementById( 'fincommerce-network-order-table' ) ),
		$noneFound = $( document.getElementById( 'fincommerce-network-orders-no-orders' ) );

	// No sites, so bail.
	if ( ! fincommerce_network_orders.sites.length ) {
	...
	$.each( fincommerce_network_orders.sites, function( index, value ) {
		promises[ index ] = $.Deferred();
		deferred.push( $.ajax( {
			url : fincommerce_network_orders.order_endpoint,
			data: {
				_wpnonce: fincommerce_network_orders.nonce,
				network_orders: true,
				blog_id: value
			},
			type: 'GET'
