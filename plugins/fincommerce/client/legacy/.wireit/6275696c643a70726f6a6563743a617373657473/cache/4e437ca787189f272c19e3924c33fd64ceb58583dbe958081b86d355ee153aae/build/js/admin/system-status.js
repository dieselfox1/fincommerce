/* global jQuery, fincommerce_admin_system_status, wcSetClipboard, wcClearClipboard */
jQuery( function ( $ ) {
	/**
	...
	var wcSystemStatus = {
		init: function () {
			$( document.body )
				.on(
					'click',
					'a.help_tip, a.fincommerce-help-tip, fincommerce-product-type-tip',
	...
	$( '.wc_status_table' ).on( 'click', '.run-tool .button', function( evt ) {
		...
		return window.confirm( fincommerce_admin_system_status.run_tool_confirmation );
	});

	$( '#log-viewer-select' ).on( 'click', 'h2 a.page-title-action', function( evt ) {
		evt.stopImmediatePropagation();
		return window.confirm( fincommerce_admin_system_status.delete_log_confirmation );
	});
});
