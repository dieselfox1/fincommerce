/*global inlineEditPost, fincommerce_admin, fincommerce_quick_edit */
jQuery(
	function( $ ) {
		$( '#the-list' ).on(
			'click',
			...
			function() {
				...
				var $wc_inline_data = $( '#fincommerce_inline_' + post_id );
				...
				var formatted_regular_price = regular_price.replace( '.', fincommerce_admin.mon_decimal_point ),
				formatted_sale_price        = sale_price.replace( '.', fincommerce_admin.mon_decimal_point );

				var cogs_data = $wc_inline_data.find( '.cogs_value ' );
				if( cogs_data.length > 0 ) {
					var formatted_cogs_value = cogs_data.text().replace( '.', fincommerce_admin.mon_decimal_point );
				...
				// Rename core strings.
				$( 'input[name="comment_status"]' ).parent().find( '.checkbox-title' ).text( fincommerce_quick_edit.strings.allow_reviews );
			}
		);
		...
		$( '#wpbody' ).on(
			...
			function() {
				...
				$( '#fincommerce-fields' ).find( 'select' ).prop( 'selectedIndex', 0 );
				$( '#fincommerce-fields-bulk' ).find( '.inline-edit-group .change-input' ).hide();
			}
		);

		$( '#wpbody' ).on(
			'change',
			'#fincommerce-fields-bulk .inline-edit-group .change_to',
			function() {
		...
		$( '#wpbody' ).on(
			...
			'.trash-product',
			function() {
				return window.confirm( fincommerce_admin.i18n_delete_product_notice );
			}
		);
	}
);
