...
jQuery( function ( $ ) {
	...
	/**
	 * Update the .fincommerce div with a string of html.
	 *
	...
	var update_wc_div = function ( html_str, preserve_notices ) {
		...
		var $new_form = $( '.fincommerce-cart-form', $html );
		var $new_totals = $( '.cart_totals', $html );
		var $notices = remove_duplicate_notices(
			$(
				'.fincommerce-error, .fincommerce-message, .fincommerce-info, .is-error, .is-info, .is-success',
				$html
			)
		);

		// No form, cannot do this.
		if ( $( '.fincommerce-cart-form' ).length === 0 ) {
		...
		if ( ! preserve_notices ) {
			$(
				'.fincommerce-error, .fincommerce-message, .fincommerce-info, .is-error, .is-info, .is-success, .coupon-error-notice'
			).remove();
		}

		if ( $new_form.length === 0 ) {
			// If the checkout is also displayed on this page, trigger reload instead.
			if ( $( '.fincommerce-checkout' ).length ) {
				window.location.reload();
				return;
			}
			...
			$( '.fincommerce-cart-form__contents' )
				.closest( '.fincommerce' )
	...
	var show_notice = function ( html_element, $target ) {
		if ( ! $target ) {
			$target =
				$( '.fincommerce-notices-wrapper:first' ) ||
				$( '.wc-empty-cart-message' ).closest( '.fincommerce' ) ||
				$( '.fincommerce-cart-form' );
		}
	...
	var cart_shipping = {
		...
		init: function ( cart ) {
			...
			$( document ).on(
				'submit',
				'form.fincommerce-shipping-calculator',
				this.shipping_calculator_submit
			);
	...
	var cart = {
		...
		init: function () {
			...
			$( document ).on(
				'click',
				'.fincommerce-cart-form :input[type=submit]',
				this.submit_click
			);
			$( document ).on(
				'keypress',
				'.fincommerce-cart-form :input[type=number]',
				this.input_keypress
			);
			$( document ).on(
				'submit',
			...
			$( document ).on(
				'click',
				'.fincommerce-cart-form .product-remove > a',
			...
			$( document ).on(
				'click',
				'.fincommerce-cart .restore-item',
				this.item_restore_clicked
			);
			$( document ).on(
				'change input',
				'.fincommerce-cart-form .cart_item :input',
				this.input_changed
			);
			$( document ).on(
				'change input',
			...
			$( '.fincommerce-cart-form :input[name="update_cart"]' ).prop(
				'disabled',
				true
			);
		},
		...
		update_cart: function ( preserve_notices ) {
			var $form = $( '.fincommerce-cart-form' );
		...
		cart_submit: function ( evt ) {
			...
			if (
				0 === $form.find( '.fincommerce-cart-form__contents' ).length
			) {
				return;
			}
		...
		apply_coupon: function ( $form ) {
			...
			$.ajax( {
				...
				success: function ( response ) {
					$(
						'.fincommerce-error, .fincommerce-message, .fincommerce-info, ' +
						'.is-error, .is-info, .is-success, .coupon-error-notice'
					).remove();

					// We only want to show coupon notices if they are not errors.
					// Coupon errors are shown under the input.
					if ( response.indexOf( 'fincommerce-error' ) === -1 && response.indexOf( 'is-error' ) === -1 ) {
		...
		item_restore_clicked: function ( evt ) {
			...
			var $a = $( evt.currentTarget );
			var $form = $( 'form.fincommerce-cart-form' );
