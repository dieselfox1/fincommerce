...
;(function ( $, window ) {
	...
	var productExportForm = function( $form ) {
		...
		// Initial state.
		this.$form.find('.fincommerce-exporter-progress').val( 0 );
		...
		$form.find( '.fincommerce-exporter-types' ).on( 'change', { productExportForm: this }, this.exportTypeFields );
	};
	...
	productExportForm.prototype.onSubmit = function( event ) {
		...
		event.data.productExportForm.$form.addClass( 'fincommerce-exporter__exporting' );
		event.data.productExportForm.$form.find('.fincommerce-exporter-progress').val( 0 );
		event.data.productExportForm.$form.find('.fincommerce-exporter-button').prop( 'disabled', true );
	...
	productExportForm.prototype.processStep = function( step, data, columns, filename ) {
		var $this              = this,
			selected_columns   = $( '.fincommerce-exporter-columns' ).val(),
			export_meta        = $( '#fincommerce-exporter-meta:checked' ).length ? 1: 0,
			export_types       = $( '.fincommerce-exporter-types' ).val(),
			export_category    = $( '.fincommerce-exporter-category' ).val(),
		...
		$.ajax( {
			...
			data: {
				form               : data,
				action             : 'fincommerce_do_ajax_product_export',
				step               : step,
				columns            : columns,
			...
			success: function( response ) {
				if ( response.success ) {
					if ( 'done' === response.data.step ) {
						$this.$form.find('.fincommerce-exporter-progress').val( response.data.percentage );
						window.location = response.data.url;
						setTimeout( function() {
							$this.$form.removeClass( 'fincommerce-exporter__exporting' );
							$this.$form.find('.fincommerce-exporter-button').prop( 'disabled', false );
						}, 2000 );
					} else {
						$this.$form.find('.fincommerce-exporter-progress').val( response.data.percentage );
	...
	productExportForm.prototype.exportTypeFields = function() {
		var exportCategory = $( '.fincommerce-exporter-category' );
	...
	$( '.fincommerce-exporter' ).wc_product_export_form();
