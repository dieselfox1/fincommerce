...
/**
 * Used by fincommerce/includes/admin/settings/views/html-settings-tax.php
 */
( function( $, data, wp, ajaxurl ) {
	$( function() {
		...
		var rowTemplate        = wp.template( 'wc-tax-table-row' ),
			...
			$submit            = $( '.fincommerce-save-button[type=submit]' ),
			WCTaxTableModelConstructor = Backbone.Model.extend({
				...
				save: function() {
					...
					Backbone.ajax({
						method: 'POST',
						dataType: 'json',
						url: ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=fincommerce_tax_rates_save_changes',
						data: {
