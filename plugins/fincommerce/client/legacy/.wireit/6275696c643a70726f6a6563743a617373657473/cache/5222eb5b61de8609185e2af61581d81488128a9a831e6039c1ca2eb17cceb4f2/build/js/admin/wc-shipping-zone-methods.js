/**
 * Number validation utilities for FinCommerce shipping forms
 */
		...
		var $table          = $( '.wc-shipping-zone-methods' ),
			...
			ShippingMethod       = Backbone.Model.extend({
				...
				save: function() {
					...
					$.post(
						ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=fincommerce_shipping_zone_methods_save_changes',
						{
			...
			ShippingMethodView = Backbone.View.extend({
					...
					if ( _.size( methods ) ) {
						...
						$.each( methods, function( id, rowData ) {
							if ( 'yes' === rowData.enabled ) {
								rowData.enabled_icon = '<span class="fincommerce-input-toggle fincommerce-input-toggle--enabled">' +
									data.strings.yes +
									'</span>';
							} else {
								rowData.enabled_icon = '<span class="fincommerce-input-toggle fincommerce-input-toggle--disabled">' +
									data.strings.no +
									'</span>';
							}
				...
				onDeleteRow: function( event ) {
					...
					if ( window.confirm( data.strings.delete_shipping_method_confirmation ) ) {
						...
						$.post( {
							url: ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?') + 'action=fincommerce_shipping_zone_remove_method',
							data: {
								wc_shipping_zones_nonce: data.wc_shipping_zones_nonce,
				...
				unformatShippingMethodNumericValues: function( data ) {
					...
					const numericValuesFields = [
						'fincommerce_free_shipping_min_amount',
						'fincommerce_flat_rate_cost',
						'fincommerce_flat_rate_no_class_cost',
					];
					const flatRateClassCostIdPrefix = 'fincommerce_flat_rate_class_cost_';
				...
				onConfigureShippingMethodSubmitted: function( event, target, posted_data ) {
					if ( 'wc-modal-shipping-method-settings' === target ) {
						...
						$.post(
							ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=fincommerce_shipping_zone_methods_save_settings',
							{
								wc_shipping_zones_nonce : data.wc_shipping_zones_nonce,
							...
							function( response, textStatus ) {
								if ( 'success' === textStatus && response.success ) {
									$( 'table.wc-shipping-zone-methods' ).parent().find( '#fincommerce_errors' ).remove();
				...
				showErrors: function( errors ) {
					var error_html = '<div id="fincommerce_errors" class="error notice is-dismissible">';
				...
				moveAdvancedCostsHelpTip: function( html ) {
					...
					const input = htmlContent.find( '#fincommerce_flat_rate_cost' );
				...
				moveHTMLHelpTips: function( html ) {
					...
					const helpTipsToRetain = [
						'fincommerce_flat_rate_cost',
						'fincommerce_flat_rate_no_class_cost',
						'fincommerce_flat_rate_class_cost_'
					];
					...
					labels.each( ( i ) => {
						...
						const helpTip = label.find( '.fincommerce-help-tip' );
						...
						if ( helpTipsToRetain.some( ( tip ) => id.includes( tip ) ) ) {
							const helpTip = htmlContent.find( `label[for=${ id }] span.fincommerce-help-tip` );
							helpTip.addClass( 'wc-shipping-visible-help-text' );
							return;
						}

						// fincommerce_free_shipping_ignore_discounts gets a helpTip appended to its label.
						// Otherwise, add the text as the last element in the fieldset.
						if ( id === 'fincommerce_free_shipping_ignore_discounts' ) {
				...
				onAddShippingMethodSubmitted: function( event, target, posted_data, closeModal ) {
					if ( 'wc-modal-add-shipping-method' === target ) {
						...
						// Add method to zone via ajax call
						$.post( ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=fincommerce_shipping_zone_add_method', {
							wc_shipping_zones_nonce : data.wc_shipping_zones_nonce,
				...
				possiblyHideFreeShippingRequirements: function( data ) {
					if ( Object.keys( data ).includes( 'fincommerce_free_shipping_requires' ) ) {
						const shouldHideRequirements = data.fincommerce_free_shipping_requires === null ||
							data.fincommerce_free_shipping_requires === '' ||
							data.fincommerce_free_shipping_requires === 'coupon';

						const select = $( '#fincommerce_free_shipping_requires' );
				...
				onModalLoaded: function( event, target ) {
					if ( target === 'wc-modal-shipping-method-settings' ) {
						const select = $( '#fincommerce_free_shipping_requires' );
						if ( select.length > 0 ) {
							event.data.view.possiblyHideFreeShippingRequirements( { fincommerce_free_shipping_requires: select.val() } );
						}
