...
jQuery( function ( $ ) {
	...
	var wc_checkout_form = {
		...
		init: function () {
			...
			if ( $( document.body ).hasClass( 'fincommerce-order-pay' ) ) {
		...
		init_payment_methods: function () {
			var $payment_methods = $( '.fincommerce-checkout' ).find(
				'input[name="payment_method"]'
			);
		...
		payment_method_selected: function ( e ) {
			...
			var selectedPaymentMethod = $(
				'.fincommerce-checkout input[name="payment_method"]:checked'
			).attr( 'id' );
		...
		validate_field: function ( e ) {
			...
			if ( 'input' === event_type ) {
				...
				$parent.removeClass(
					// eslint-disable-next-line max-len
					'fincommerce-invalid fincommerce-invalid-required-field fincommerce-invalid-email fincommerce-invalid-phone fincommerce-validated'
				);
			}
			...
			if (
				...
				if ( validate_required ) {
					if (
						...
						$parent
							.removeClass( 'fincommerce-validated' )
							.addClass(
								'fincommerce-invalid fincommerce-invalid-required-field'
							);
						validated = false;
					}
				}
					...
					if ( $this.val() ) {
						...
						if ( ! pattern.test( $this.val() ) ) {
							...
							$parent
								.removeClass( 'fincommerce-validated' )
								.addClass(
									'fincommerce-invalid fincommerce-invalid-email'
								); // eslint-disable-line max-len
							validated = false;
						}
					}
				...
				if ( validate_phone ) {
					...
					if ( 0 < $this.val().replace( pattern, '' ).length ) {
						...
						$parent
							.removeClass( 'fincommerce-validated' )
							.addClass(
								'fincommerce-invalid fincommerce-invalid-phone'
							);
						validated = false;
					}
				}
				...
				if ( validated ) {
					...
					$parent
						.removeClass(
							'fincommerce-invalid fincommerce-invalid-required-field fincommerce-invalid-email fincommerce-invalid-phone'
						)
						.addClass( 'fincommerce-validated' ); // eslint-disable-line max-len
				}
			}
		},
		...
		update_checkout_action: function ( args ) {
			...
			$(
				'.fincommerce-checkout-payment, .fincommerce-checkout-review-order-table'
			).block( {
				message: null,
				overlayCSS: {
					background: '#fff',
			...
			wc_checkout_form.xhr = $.ajax( {
				...
				success: function ( data ) {
					...
					$( '.fincommerce-NoticeGroup-updateOrderReview' ).remove();
					...
					if ( data && 'failure' === data.result ) {
						...
						$(
							'.fincommerce-error, .fincommerce-message, .is-error, .is-success'
						).remove();

						// Add new errors returned by this event
						if ( data.messages ) {
							$form.prepend(
								'<div class="fincommerce-NoticeGroup fincommerce-NoticeGroup-updateOrderReview">' +
									data.messages +
									'</div>'
							); // eslint-disable-line max-len
						} else {
						...
						try {
							...
							if ( result.messages ) {
								var $msgs = $( result.messages )
									// The error notice template (plugins/fincommerce/templates/notices/error.php)
		...
		submit_error: function ( error_message ) {
			$(
				'.fincommerce-NoticeGroup-checkout, .fincommerce-error, .fincommerce-message, .is-error, .is-success'
			).remove();
			wc_checkout_form.$checkout_form.prepend(
				'<div class="fincommerce-NoticeGroup fincommerce-NoticeGroup-checkout">' +
					error_message +
					'</div>'
			); // eslint-disable-line max-len
			...
			wc_checkout_form.$checkout_form
				.find(
					'.fincommerce-error[tabindex="-1"], .wc-block-components-notice-banner.is-error[tabindex="-1"]'
				)
				.focus();
		...
		scroll_to_notices: function () {
			var scrollElement = $(
				'.fincommerce-NoticeGroup-updateOrderReview, .fincommerce-NoticeGroup-checkout'
			);
	...
	var wc_checkout_coupons = {
		...
		submit: function ( evt ) {
			...
			$.ajax( {
				...
				success: function ( response ) {
					$(
						'.fincommerce-error, .fincommerce-message, .is-error, .is-success, .checkout-inline-error-message'
					).remove();
					...
					if ( response ) {
						...
						if (
							response.indexOf( 'fincommerce-error' ) === -1 &&
		...
		remove_coupon: function ( e ) {
			...
			var container = $( this ).parents(
					'.fincommerce-checkout-review-order'
				),
			...
			$.ajax( {
				...
				success: function ( code ) {
					...
					if ( code ) {
						$( 'form.fincommerce-checkout' ).before( code );
	...
	var wc_checkout_login_form = {
		...
		show_login_form: function () {
			var $form = $( 'form.login, form.fincommerce-form--login' );
			if ( $form.is( ':visible' ) ) {
	...
	var wc_terms_toggle = {
		init: function () {
			$( document.body ).on(
				'click',
				'a.fincommerce-terms-and-conditions-link',
				this.toggle_terms
			);
		},

		toggle_terms: function () {
			if ( $( '.fincommerce-terms-and-conditions' ).length ) {
				$( '.fincommerce-terms-and-conditions' ).slideToggle(
					function () {
						var link_toggle = $(
							'.fincommerce-terms-and-conditions-link'
						);

						if (
							$( '.fincommerce-terms-and-conditions' ).is(
								':visible'
							)
						) {
							link_toggle.addClass(
								'fincommerce-terms-and-conditions-link--open'
							);
							link_toggle.removeClass(
								'fincommerce-terms-and-conditions-link--closed'
							);
						} else {
							link_toggle.removeClass(
								'fincommerce-terms-and-conditions-link--open'
