jQuery( function ( $ ) {
	...
	jQuery.maybe_disable_save_button = function () {
		...
		var $save_button;
		if (
			$( '.fincommerce_variation_new_attribute_data' ).is( ':visible' )
		) {
			$tab = $( '.fincommerce_variation_new_attribute_data' );
	...
	$( '.save_attributes' ).tipTip( {
		content: function () {
			return $( '.save_attributes' ).hasClass( 'disabled' )
				? fincommerce_admin_meta_boxes.i18n_save_attribute_variation_tip
				: '';
		},
		fadeIn: 50,
		fadeOut: 50,
		...
		.on( 'wc-init-tabbed-panels', function () {
			...
			$( 'ul.wc-tabs a' ).on( 'click', function ( e ) {
				...
				$( $( this ).attr( 'href' ) ).show( 0, function () {
					$( this ).trigger( 'fincommerce_tab_shown' );
				} );
			} );
