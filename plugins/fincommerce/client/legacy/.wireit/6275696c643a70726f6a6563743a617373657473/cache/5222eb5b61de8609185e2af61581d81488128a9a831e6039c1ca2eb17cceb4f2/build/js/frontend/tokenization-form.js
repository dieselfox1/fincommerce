...
jQuery( function( $ ) {
	...
	var TokenizationForm = function( $target ) {
		...
		this.$target.on(
			'click change',
			':input.fincommerce-SavedPaymentMethods-tokenInput',
	...
	TokenizationForm.prototype.onDisplay = function() {
		...
		if ( 0 === $( ':input.fincommerce-SavedPaymentMethods-tokenInput:checked', this.$target ).length ) {
			$( ':input.fincommerce-SavedPaymentMethods-tokenInput:last', this.$target ).prop( 'checked', true );
		}

		// Don't show the "use new" radio button if we only have one method..
		if ( 0 === this.$target.data( 'count' ) ) {
			$( '.fincommerce-SavedPaymentMethods-new', this.$target ).remove();
		}
		...
		$( ':input.fincommerce-SavedPaymentMethods-tokenInput:checked', this.$target ).trigger( 'change' );
	};
	...
	TokenizationForm.prototype.showSaveNewCheckbox = function() {
		$( '.fincommerce-SavedPaymentMethods-saveNew', this.$formWrap ).show();
	};

	TokenizationForm.prototype.hideSaveNewCheckbox = function() {
		$( '.fincommerce-SavedPaymentMethods-saveNew', this.$formWrap ).hide();
	};
	...
	$( document.body ).on( 'updated_checkout wc-credit-card-form-init', function() {
		...
		var $saved_payment_methods = $( 'ul.fincommerce-SavedPaymentMethods' );
