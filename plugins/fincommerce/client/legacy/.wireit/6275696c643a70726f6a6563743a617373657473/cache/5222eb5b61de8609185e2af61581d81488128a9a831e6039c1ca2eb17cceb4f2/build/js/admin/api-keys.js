/*global jQuery, Backbone, _, fincommerce_admin_api_keys, wcSetClipboard, wcClearClipboard */
(function( $ ) {
	...
	var APIView = Backbone.View.extend({
			...
			$( document.body )
				.on( 'click', css_class, function( evt ) {
					...
					if ( ! document.queryCommandSupported( 'copy' ) ) {
						...
						$( '#copy-error' ).text( fincommerce_admin_api_keys.clipboard_failed );
					} else {
						$( '#copy-error' ).text( '' );
						wcClearClipboard();
		...
		saveKey: function( e ) {
			...
			Backbone.ajax({
				method:   'POST',
				dataType: 'json',
				url:      fincommerce_admin_api_keys.ajax_url,
				data:     {
					action:      'fincommerce_update_api_key',
					security:    fincommerce_admin_api_keys.update_api_nonce,
