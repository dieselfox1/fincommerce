...
;(function ( $, window, document, undefined ) {
	...
	VariationForm.prototype.onHide = function( event ) {
		...
		event.data.variationForm.$form
			.find( '.fincommerce-variation-add-to-cart' )
			.removeClass( 'fincommerce-variation-add-to-cart-enabled' )
			.addClass( 'fincommerce-variation-add-to-cart-disabled' );
	};
	...
	VariationForm.prototype.onShow = function( event, variation, purchasable ) {
		...
		if ( purchasable ) {
			...
			event.data.variationForm.$form
				.find( '.fincommerce-variation-add-to-cart' )
				.removeClass( 'fincommerce-variation-add-to-cart-disabled' )
				.addClass( 'fincommerce-variation-add-to-cart-enabled' );
		} else {
			event.data.variationForm.$form
				.find( '.single_add_to_cart_button' )
	...
	VariationForm.prototype.onResetDisplayedVariation = function( event ) {
		...
		form.$product
			.find( '.product_weight, .fincommerce-product-attributes-item--weight .fincommerce-product-attributes-item__value' )
			.wc_reset_content();
		form.$product
			.find( '.product_dimensions, .fincommerce-product-attributes-item--dimensions .fincommerce-product-attributes-item__value' )
			.wc_reset_content();
	...
	VariationForm.prototype.onFoundVariation = function( event, variation ) {
		var form           = event.data.variationForm,
			...
			$weight        = form.$product.find(
				'.product_weight, .fincommerce-product-attributes-item--weight .fincommerce-product-attributes-item__value'
			),
			$dimensions    = form.$product.find(
				'.product_dimensions, .fincommerce-product-attributes-item--dimensions .fincommerce-product-attributes-item__value'
			),
	...
	VariationForm.prototype.onChange = function( event ) {
		...
		if ( form.useAjax ) {
			...
			form.$form.trigger( 'fincommerce_variation_select_change' );
			form.$form.trigger( 'check_variations' );
		}

		// Custom event for when variation selection has been changed
		form.$form.trigger( 'fincommerce_variation_has_changed' );
	};
	...
	VariationForm.prototype.onUpdateAttributes = function( event ) {
		...
		form.$form.trigger( 'fincommerce_update_variation_values' );
	};
	...
	VariationForm.prototype.showNoMatchingVariationsMsg = function() {
		this.$form
			...
			.after(
				'<div role="alert">' +
					'<p class="wc-no-matching-variations fincommerce-info">' +
	...
	$.fn.wc_maybe_trigger_slide_position_reset = function( variation ) {
		...
		if ( reset_slide_position ) {
			$product_gallery.trigger( 'fincommerce_gallery_reset_slide_position' );
		}
	};
	...
	$.fn.wc_variations_image_update = function( variation ) {
		var $form             = this,
			...
			$product_img_wrap = $product_gallery
				.find( '.fincommerce-product-gallery__image, .fincommerce-product-gallery__image--placeholder' )
				.eq( 0 ),
		...
		if ( variation && variation.image && variation.image.src && variation.image.src.length > 1 ) {
			...
			if ( slideToImage.length > 0 ) {
				...
				window.setTimeout( function() {
					...
					$product_gallery.trigger( 'fincommerce_gallery_init_zoom' );
				}, 20 );
				return;
			}
