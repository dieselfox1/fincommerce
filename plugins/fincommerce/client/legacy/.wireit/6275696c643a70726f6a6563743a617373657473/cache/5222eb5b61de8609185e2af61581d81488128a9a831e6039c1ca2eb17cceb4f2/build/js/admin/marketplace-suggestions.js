...
( function( $, marketplace_suggestions, ajaxurl ) {
	$( function() {
		...
		// Helper function to construct admin URLs similar to getAdminLink from @fincommerce/settings
		...
		function dismissSuggestion( context, product, promoted, url, suggestionSlug ) {
			...
			jQuery.post(
				ajaxurl,
				{
					'action': 'fincommerce_add_dismissed_marketplace_suggestion',
			...
			if ( _.contains( highUseSuggestionContexts, context ) ) {
				...
				var contextSnoozeCookie = 'fincommerce_snooze_suggestions__' + context;
				Cookies.set( contextSnoozeCookie, 'true', { expires: 2 } );

				// keep track of how often this area gets dismissed in a cookie
				var contextDismissalCountCookie = 'fincommerce_dismissed_suggestions__' + context;
