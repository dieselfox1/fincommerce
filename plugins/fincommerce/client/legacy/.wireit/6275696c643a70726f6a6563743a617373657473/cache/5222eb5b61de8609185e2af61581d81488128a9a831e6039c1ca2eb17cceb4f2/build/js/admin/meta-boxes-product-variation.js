/* global wp, fincommerce_admin_meta_boxes_variations, fincommerce_admin, accounting */
jQuery( function ( $ ) {
	'use strict';
	...
	var wc_meta_boxes_product_variations_actions = {
		...
		create_variations: function ( event ) {
			...
			var new_attribute_data = $(
				'.fincommerce_variation_new_attribute_data'
			);
			...
			var data = {
				...
				action: 'fincommerce_add_attributes_and_variations',
				security:
					fincommerce_admin_meta_boxes.add_attributes_and_variations,
			};
		...
		variations_loaded: function ( event, needsUpdate ) {
			...
			var wrapper = $( '#fincommerce-product-data' );
			...
			if ( ! needsUpdate ) {
				...
				$( '.fincommerce_variation', wrapper ).each( function (
					index,
					el
				) {
					var $el = $( el ),
				...
				$(
					'.fincommerce_variations .variation-needs-update',
					wrapper
			...
			$(
				'.fincommerce_variations .tips, ' +
					'.fincommerce_variations .help_tip, ' +
					'.fincommerce_variations .fincommerce-help-tip, ' +
					'.toolbar-variations-defaults .fincommerce-help-tip',
				wrapper
			).tipTip( {
				attribute: 'data-tip',
				fadeIn: 50,
			...
			// Allow sorting
			$( '.fincommerce_variations', wrapper ).sortable( {
				items: '.fincommerce_variation',
				cursor: 'move',
				axis: 'y',
				handle: '.sort',
		...
		set_menu_order: function ( event ) {
			...
			var value = window.prompt(
				fincommerce_admin_meta_boxes_variations.i18n_enter_menu_order,
				$menu_order.val()
			);
		...
		variation_row_indexes: function () {
			var wrapper = $( '#variable_product_options' ).find(
					'.fincommerce_variations'
				),
				current_page = parseInt( wrapper.attr( 'data-page' ), 10 ),
				offset = parseInt(
					( current_page - 1 ) *
						fincommerce_admin_meta_boxes_variations.variations_per_page,
					10
				);

			$( '.fincommerce_variations .fincommerce_variation' ).each(
				function ( index, el ) {
					$( '.variation_menu_order', el )
						.val(
							parseInt(
								$( el ).index(
									'.fincommerce_variations .fincommerce_variation'
								),
								10
							) +
								1 +
		...
		add_image: function ( event ) {
			...
			if ( $button.is( '.remove' ) ) {
				...
				wc_meta_boxes_product_variations_media.setting_variation_image
					...
					.attr(
						'src',
						fincommerce_admin_meta_boxes_variations.fincommerce_placeholder_img_src
					);
				wc_meta_boxes_product_variations_media.setting_variation_image
				...
				wc_meta_boxes_product_variations_media.variable_image_frame = wp.media.frames.variable_image = wp.media(
					{
						...
						title:
							fincommerce_admin_meta_boxes_variations.i18n_choose_image,
						button: {
							text:
								fincommerce_admin_meta_boxes_variations.i18n_set_image,
						},
						states: [
	...
	var wc_meta_boxes_product_variations_ajax = {
		...
		init: function () {
			...
			$( document.body )
				.on(
					'change input',
					'#variable_product_options .fincommerce_variations :input',
					this.input_changed
				)
				.on(
					'change',
		...
		check_for_changes: function () {
			...
			if ( 0 < need_update.length ) {
				if (
					window.confirm(
						fincommerce_admin_meta_boxes_variations.i18n_edited_variations
					)
				) {
		 ...
		 */
		block: function () {
			$( '#fincommerce-product-data' ).block( {
				message: null,
				overlayCSS: {
					background: '#fff',
					opacity: 0.6,
		 ...
		 */
		unblock: function () {
			$( '#fincommerce-product-data' ).unblock();
		},
		...
		load_variations: function ( page, per_page ) {
			return new Promise( ( resolve, reject ) => {
				...
				$.ajax( {
					url: fincommerce_admin_meta_boxes_variations.ajax_url,
					data: {
						action: 'fincommerce_load_variations',
						security:
							fincommerce_admin_meta_boxes_variations.load_variations_nonce,
						product_id:
							fincommerce_admin_meta_boxes_variations.post_id,
					...
					success: function ( response ) {
						...
						$( '#fincommerce-product-data' ).trigger(
							'fincommerce_variations_loaded'
						);
		...
		save_changes: function ( callback ) {
			...
			if ( 0 < need_update.length ) {
				...
				data.action = 'fincommerce_save_variations';
				data.security =
					fincommerce_admin_meta_boxes_variations.save_variations_nonce;
				data.product_id =
					fincommerce_admin_meta_boxes_variations.post_id;
		...
		save_variations: function () {
			$( '#variable_product_options' ).trigger(
				'fincommerce_variations_save_variations_button'
			);
		...
		save_on_submit: function ( e ) {
			...
			if ( 0 < need_update.length ) {
				e.preventDefault();
				$( '#variable_product_options' ).trigger(
					'fincommerce_variations_save_variations_on_submit'
				);
				wc_meta_boxes_product_variations_ajax.save_changes(
		...
		cancel_variations: function () {
			var current = parseInt(
				$( '#variable_product_options' )
					.find( '.fincommerce_variations' )
					.attr( 'data-page' ),
				10
			);

			$( '#variable_product_options' )
				.find( '.fincommerce_variations .variation-needs-update' )
				.removeClass( 'variation-needs-update' );
			$( '.variations-defaults select' ).each( function () {
		...
		add_variation: function () {
			...
			var data = {
				action: 'fincommerce_add_variation',
				post_id: fincommerce_admin_meta_boxes_variations.post_id,
				loop: $( '.fincommerce_variation' ).length,
				security:
					fincommerce_admin_meta_boxes_variations.add_variation_nonce,
			};

			$.post(
				fincommerce_admin_meta_boxes_variations.ajax_url,
				data,
				function ( response ) {
					var variation = $( response );
					variation.addClass( 'variation-needs-update' );

					$( '.fincommerce-notice-invalid-variation' ).remove();
					$( '#variable_product_options' )
						.find( '.fincommerce_variations' )
						.prepend( variation );
					$(
					...
					$( '#variable_product_options' ).trigger( 'fincommerce_variations_added', 1 );
		...
		remove_variation: function () {
			...
			if (
				window.confirm(
					fincommerce_admin_meta_boxes_variations.i18n_remove_variation
				)
			) {
				var variation = $( this ).attr( 'rel' ),
					variation_ids = [],
					data = {
						action: 'fincommerce_remove_variations',
					};
				...
				if ( 0 < variation ) {
					...
					data.security =
						fincommerce_admin_meta_boxes_variations.delete_variations_nonce;
					...
					$.post(
						...
						function () {
							...
							$( '#fincommerce-product-data' ).trigger(
								'fincommerce_variations_removed'
							);
		...
		link_all_variations: function () {
			...
			if (
				window.confirm(
					fincommerce_admin_meta_boxes_variations.i18n_link_all_variations
				)
			) {
				wc_meta_boxes_product_variations_ajax.block();

				var data = {
					action: 'fincommerce_link_all_variations',
					post_id: fincommerce_admin_meta_boxes_variations.post_id,
					security:
						fincommerce_admin_meta_boxes_variations.link_variation_nonce,
				};
				...
				$.post(
					...
					function ( response ) {
						...
						const message =
							count === 1
								? fincommerce_admin_meta_boxes_variations.i18n_variation_added
								: fincommerce_admin_meta_boxes_variations.i18n_variations_added.replace(
										'%qty%',
										count
								  );
		...
		input_changed: function ( event ) {
			...
			// Do not trigger 'fincommerce_variations_input_changed' for 'input' events for backwards compat.
			if ( 'input' === event.type && $( this ).is( ':text' ) ) {
				return;
			}

			$( '#variable_product_options' ).trigger(
				'fincommerce_variations_input_changed'
			);
		},
		...
		defaults_changed: function () {
			$( this )
				...
				.find( '.fincommerce_variation:first' )
				.addClass( 'variation-needs-update' );
			...
			$( '#variable_product_options' ).trigger(
				'fincommerce_variations_defaults_changed'
			);
		},
		...
		do_variation_action: function () {
			...
			switch ( do_variation_action ) {
				case 'delete_all':
					if (
						window.confirm(
							fincommerce_admin_meta_boxes_variations.i18n_delete_all_variations
						)
					) {
						if (
							window.confirm(
								fincommerce_admin_meta_boxes_variations.i18n_last_warning
							)
						) {
							data.allowed = true;
							changes =
				...
				case 'variable_sale_price_decrease':
					let promptMessage =
						fincommerce_admin_meta_boxes_variations.i18n_enter_a_value_fixed_or_percent;
					...
					if (
						...
						promptMessage +=
							'\n\n' +
							fincommerce_admin_meta_boxes_variations.i18n_sale_price_warning;
					}
				...
				case 'variable_download_expiry':
					value = window.prompt(
						fincommerce_admin_meta_boxes_variations.i18n_enter_a_value
					);
				...
				case 'variable_unset_cogs_value':
					if (
						! window.confirm (
							fincommerce_admin_meta_boxes_variations.i18n_variation_cost_remove_warning
							)
					) {
						cancel = true;
					}
				...
				case 'variable_sale_schedule':
					data.date_from = window.prompt(
						fincommerce_admin_meta_boxes_variations.i18n_scheduled_sale_start
					);
					data.date_to = window.prompt(
						fincommerce_admin_meta_boxes_variations.i18n_scheduled_sale_end
					);
			...
			if ( cancel ) {
				...
				$.ajax( {
					url: fincommerce_admin_meta_boxes_variations.ajax_url,
					data: {
						action: 'fincommerce_bulk_edit_variations',
						security:
						fincommerce_admin_meta_boxes_variations.bulk_edit_variations_nonce,
						product_id: fincommerce_admin_meta_boxes_variations.post_id,
						product_type: $( '#product-type' ).val(),
	...
	var wc_meta_boxes_product_variations_pagenav = {
		...
		update_variations_count: function ( qty ) {
			...
			const message =
				total === 1
					? fincommerce_admin_meta_boxes_variations.i18n_variation_count_single
					: fincommerce_admin_meta_boxes_variations.i18n_variation_count_plural.replace(
							'%qty%',
							total
					  );
