...
jQuery( function( $ ) {
	...
	/**
	 * Obtains the current geo hash from the `fincommerce_geo_hash` cookie, if set.
	 *
	 * @returns {boolean}
	 */
	function get_geo_hash() {
		var geo_hash_cookie = Cookies.get( 'fincommerce_geo_hash' );
	...
	var $geolocation_redirect = function( hash ) {
		...
		Cookies.set( 'fincommerce_geo_hash', hash, { expires: 1 / 24 } );
	...
	$( document.body ).on( 'fincommerce_append_geo_hashes', function() {
		$append_hashes();
	});
});
