/* global fincommerce_settings_params, wp */
( function ( $, params, wp ) {
	$( function () {
		// Sell Countries
		$( 'select#fincommerce_allowed_countries' )
			.on( 'change', function () {
		...
		// Ship Countries
		$( 'select#fincommerce_ship_to_countries' )
			.on( 'change', function () {
				if ( 'specific' === $( this ).val() ) {
					$( this ).closest( 'tr' ).next( 'tr' ).show();
				} else {
		...
		// Stock management
		$( 'input#fincommerce_manage_stock' )
			.on( 'change', function () {
				if ( $( this ).is( ':checked' ) ) {
					$( this )
						.closest( 'tbody' )
		...
		function editPrompt () {
			...
			$( 'input, textarea, select, checkbox' ).on( 'change input', function (
				...
				if ( ! changed ) {
					...
					changed = true;
					$( '.fincommerce-save-button' ).removeAttr( 'disabled' );
				}
			} );
		...
		// Select all/none
		$( '.fincommerce' ).on( 'click', '.select_all', function () {
			$( this )
				.closest( 'td' )
				.find( 'select option' )
				.prop( 'selected', true );
		...
		$( '.fincommerce' ).on( 'click', '.select_none', function () {
			$( this )
				.closest( 'td' )
				.find( 'select option' )
				.prop( 'selected', false );
		...
		$( '.submit button' ).on( 'click', function () {
			if (
				$( 'select#fincommerce_allowed_countries' ).val() ===
					'specific' &&
				! $( '[name="fincommerce_specific_allowed_countries[]"]' ).val()
			) {
				if (
					window.confirm(
						fincommerce_settings_params.i18n_no_specific_countries_selected
					)
				) {
					return true;
				}
		...
		$( '.fincommerce-save-button.components-button' ).on( 'click', function ( e ) {
		 ...
		 */
		document.querySelectorAll( 'body.fincommerce_page_wc-settings #mainform .conditional.description' ).forEach( description => {
		...
		function settings_scroll_to_active_tab() {
			...
			if (
				...
				! body.classList.contains('fincommerce_page_wc-settings')
			) {
				return;
			}
...
} )( jQuery, fincommerce_settings_params, wp );
