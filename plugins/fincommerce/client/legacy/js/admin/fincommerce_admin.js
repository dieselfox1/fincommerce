/* global fincommerce_admin */
( function ( $, fincommerce_admin ) {
	$( function () {
		if ( 'undefined' === typeof fincommerce_admin ) {
			return;
		}
		...
		var $product_screen = $( '.edit-php.post-type-product' ),
			...
			$blankslate = $product_screen.find( '.fincommerce-BlankState' );

		if ( 0 === $blankslate.length ) {
			if ( fincommerce_admin.urls.add_product ) {
				$title_action
					.first()
					.attr( 'href', fincommerce_admin.urls.add_product );
			}
			if ( fincommerce_admin.urls.export_products ) {
				const exportLink = document.createElement('a');
				exportLink.href = fincommerce_admin.urls.export_products;
				exportLink.className = 'page-title-action';
				exportLink.textContent = fincommerce_admin.strings.export_products;

				$title_action.after(exportLink);
			}
			if ( fincommerce_admin.urls.import_products ) {
				const importLink = document.createElement('a');
				importLink.href = fincommerce_admin.urls.import_products;
				importLink.className = 'page-title-action';
				importLink.textContent = fincommerce_admin.strings.import_products;
		...
		$( '.fincommerce-progress-form-wrapper .button-next' ).on(
			'click',
			function () {
		...
		$( document.body )
			...
			.on(
				...
				function () {
					var regex,
						decimalRegex,
						decimailPoint = fincommerce_admin.decimal_point;
					...
					if (
						...
						decimailPoint = fincommerce_admin.mon_decimal_point;
					}
			...
			.on(
				...
				function () {
					...
					if (
						...
						regex = new RegExp(
							'[^-0-9%\\' +
								fincommerce_admin.decimal_point +
								']+',
							'gi'
						);
						decimalRegex = new RegExp(
			...
			.on( 'init_tooltips', function () {
				$( '.tips, .help_tip, .fincommerce-help-tip' ).tipTip( {
					attribute: 'data-tip',
					fadeIn: 50,
					fadeOut: 50,
					delay: 200,
			...
			.on( 'click', '.wc-confirm-delete', function ( event ) {
				if (
					! window.confirm( fincommerce_admin.i18n_confirm_delete )
				) {
		...
		$(
			'.fincommerce_page_wc-settings .shippingrows tbody tr:even, table.attributes-table tbody tr:nth-child(odd)'
		).addClass( 'alternate' );
		...
		// Reviews.
		$( 'input#fincommerce_enable_reviews' )
			.on( 'change', function () {
				if ( $( this ).is( ':checked' ) ) {
		...
		$( '.wc_gateways' ).on(
			...
			function () {
				var $link = $( this ),
					$row = $link.closest( 'tr' ),
					$toggle = $link.find( '.fincommerce-input-toggle' );

				var data = {
					action: 'fincommerce_toggle_gateway_enabled',
					security: fincommerce_admin.nonces.gateway_toggle,
					gateway_id: $row.data( 'gateway_id' ),
				};

				$toggle.addClass( 'fincommerce-input-toggle--loading' );

				$.ajax( {
					url: fincommerce_admin.ajax_url,
					data: data,
					dataType: 'json',
					type: 'POST',
					success: function ( response ) {
						if ( true === response.data ) {
							$toggle.removeClass(
								'fincommerce-input-toggle--enabled, fincommerce-input-toggle--disabled'
							);
							$toggle.addClass(
								'fincommerce-input-toggle--enabled'
							);
							$toggle.removeClass(
								'fincommerce-input-toggle--loading'
							);
							...
							$toggle.addClass(
								'fincommerce-input-toggle--disabled'
							);
							$toggle.removeClass(
								'fincommerce-input-toggle--loading'
							);
		...
		$( '#wpbody' ).on( 'click', '#doaction, #doaction2', function () {
			...
			if ( 'remove_personal_data' === action ) {
				return window.confirm(
					fincommerce_admin.i18n_remove_personal_data_notice
				);
			}
		} );
	...
	$( function() {
		...
		var wc_order_lock = {
			init: function() {
				// Order screen.
				this.$lock_dialog = $( '.fincommerce_page_wc-orders #post-lock-dialog.order-lock-dialog' );
				if ( 0 !== this.$lock_dialog.length && 'undefined' !== typeof fincommerce_admin_meta_boxes ) {
				...
				// Orders list table.
				this.$list_table = $( '.fincommerce_page_wc-orders table.wc-orders-list-table' );
			...
			refresh_order_lock: function( e, data ) {
				...
				data['wc-refresh-order-lock'] = fincommerce_admin_meta_boxes.post_id;
			},
			...
			function () {
				...
				setTimeout( function () {
					...
					if ( selectedProductIds.length > 0 ) {
						...
						url.searchParams.set( '_wpnonce', fincommerce_admin.nonces.export_selected_products_nonce);
						const newHref = url.toString();
						// Construct the text with the count of selected products.
						const count      = selectedProductIds.length;
						const buttonText = fincommerce_admin.strings.export_selected_products.replace( '%d', count );
