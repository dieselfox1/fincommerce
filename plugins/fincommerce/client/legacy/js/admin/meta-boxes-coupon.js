/* global fincommerce_admin_meta_boxes_coupon */
jQuery(function( $ ) {
	...
	var wc_meta_boxes_coupon_actions = {
		...
		insert_generate_coupon_code_button: function () {
			...
			button.textContent =
				fincommerce_admin_meta_boxes_coupon.generate_button_text;
		...
		generate_coupon_code: function( e ) {
			...
			var $coupon_code_field = $( '#title' ),
			    ...
			    $result = '';
			for ( var i = 0; i < fincommerce_admin_meta_boxes_coupon.char_length; i++ ) {
				$result += fincommerce_admin_meta_boxes_coupon.characters.charAt(
					Math.floor( Math.random() * fincommerce_admin_meta_boxes_coupon.characters.length )
				);
			}
			$result = fincommerce_admin_meta_boxes_coupon.prefix + $result + fincommerce_admin_meta_boxes_coupon.suffix;
