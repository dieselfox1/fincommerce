...
;(function ( $, window ) {
	...
	var productImportForm = function( $form ) {
		...
		// Initial state.
		this.$form.find('.fincommerce-importer-progress').val( 0 );
	...
	productImportForm.prototype.run_import = function() {
		...
		$.ajax( {
			...
			url: ajaxurl,
			data: {
				action            : 'fincommerce_do_ajax_product_import',
			...
			success: function( response ) {
				if ( response.success ) {
					...
					$this.$form.find('.fincommerce-importer-progress').val( response.data.percentage );
	...
	$( '.fincommerce-importer' ).wc_product_importer();
