 ...
 * Email Downloads.
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/emails/email-downloads.php.
 *
...
?><h2 class="fincommerce-order-downloads__title<?php echo $email_improvements_enabled ? ' email-order-detail-heading' : ''; ?>"><?php esc_html_e( 'Downloads', 'fincommerce' ); ?></h2>
		...
		<tr>
			<?php foreach ( $columns as $column_id => $column_name ) : ?>
				<td class="td <?php echo $email_improvements_enabled && array_key_last( $columns ) === $column_id ? 'text-align-right' : 'text-align-left'; ?>">
					<?php
					if ( has_action( 'fincommerce_email_downloads_column_' . $column_id ) ) {
						do_action( 'fincommerce_email_downloads_column_' . $column_id, $download, $plain_text );
					} else {
						switch ( $column_id ) {
							...
							case 'download-file':
								?>
								<a href="<?php echo esc_url( $download['download_url'] ); ?>" class="fincommerce-MyAccount-downloads-file button alt"><?php echo esc_html( $download['download_name'] ); ?></a>
								<?php
								break;
