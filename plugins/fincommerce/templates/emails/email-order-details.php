 ...
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/emails/email-order-details.php.
 *
...
if ( $email_improvements_enabled ) {
	add_filter( 'fincommerce_order_shipping_to_display_shipped_via', '__return_false' );
}
 ...
 * @since 2.5.0
 */
do_action( 'fincommerce_email_before_order_table', $order, $sent_to_admin, $plain_text, $email ); ?>

<h2 class="<?php echo esc_attr( $heading_class ); ?>">
	<?php
	if ( $email_improvements_enabled ) {
		echo wp_kses_post( __( 'Order summary', 'fincommerce' ) );
	}
	if ( $sent_to_admin ) {
...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>;">
	<table class="td font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="6" style="width: 100%;" border="1">
		...
		<thead>
			<tr>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Product', 'fincommerce' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Quantity', 'fincommerce' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Price', 'fincommerce' ); ?></th>
			</tr>
		</thead>
		<?php } ?>
		<tbody>
		...
		<tfoot>
			...
			if ( $order->get_customer_note() && ! $email_improvements_enabled ) {
				?>
				<tr>
					<th class="td text-align-left" scope="row" colspan="2"><?php esc_html_e( 'Note:', 'fincommerce' ); ?></th>
			...
			if ( $order->get_customer_note() && $email_improvements_enabled ) {
				...
				<tr class="order-customer-note">
					<td class="td text-align-left" colspan="3">
						<b><?php esc_html_e( 'Customer note', 'fincommerce' ); ?></b><br>
...
<?php
if ( $email_improvements_enabled ) {
	remove_filter( 'fincommerce_order_shipping_to_display_shipped_via', '__return_false' );
}
 ...
 * @since 2.5.0
 */
do_action( 'fincommerce_email_after_order_table', $order, $sent_to_admin, $plain_text, $email );
?>
