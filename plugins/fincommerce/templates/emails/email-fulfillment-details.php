 ...
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/emails/email-fulfillment-details.php.
 *
...
if ( null === $fulfillment->get_date_deleted() ) {
	...
	if ( ! $tracking_number && ! $tracking_url && ! $shipment_provider ) {
		echo '<p>' . esc_html__( 'No tracking information available for this fulfillment at the moment.', 'fincommerce' ) . '</p>';
	} else {
		echo '<p><strong>' . esc_html__( 'Tracking Number', 'fincommerce' ) . ':</strong> ' . esc_attr( $tracking_number ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Shipment Provider', 'fincommerce' ) . ':</strong> ' . esc_html( $shipment_provider ) . '</p>';
		echo '<p><a href="' . esc_url( $tracking_url ) . '" target="_blank">' . esc_attr__( 'Track your shipment', 'fincommerce' ) . '</a></p>';
	}
	echo '<br />';
	echo '<p>';
	echo wp_kses_post(
		sprintf(
			...
			__( 'You can access to more details of your order by visiting <a href="%s" target="_blank">My Account > Orders</a> and select the order you wish to see the latest status of the delivery.', 'fincommerce' ),
	 ...
	 * @since 2.5.0
	 */
	do_action( 'fincommerce_email_before_fulfillment_table', $order, $fulfillment, $sent_to_admin, $plain_text, $email );
?>

<h2 class="<?php echo esc_attr( $heading_class ); ?>">
	<?php
	echo wp_kses_post( __( 'Fulfillment summary', 'fincommerce' ) );
	if ( $sent_to_admin ) {
	...
	$order_number_string = __( 'Order #%s', 'fincommerce' );
 ...
 * @since 2.5.0
 */
do_action( 'fincommerce_email_after_fulfillment_table', $order, $fulfillment, $sent_to_admin, $plain_text, $email );
?>
