 ...
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/emails/customer-pos-refunded-order.php.
 *
 ...
 * @since 10.0.0
 */
do_action( 'fincommerce_pos_email_header', $email_heading, $email ); ?>
...
if ( ! empty( $order->get_billing_first_name() ) ) {
	...
	printf( esc_html__( 'Hi there,', 'fincommerce' ) );
}
?>
</p>
...
if ( $partial_refund ) {
	...
	echo sprintf( esc_html__( 'Your order from %s has been partially refunded.', 'fincommerce' ), esc_html( $pos_store_name ) ) . "\n\n";
} else {
	/* translators: %s: Site title */
	echo sprintf( esc_html__( 'Your order from %s has been refunded.', 'fincommerce' ), esc_html( $pos_store_name ) ) . "\n\n";
}
echo '</p><p>';
echo esc_html__( 'Here’s a reminder of what you’ve bought:', 'fincommerce' ) . "\n\n";
?>
</p>
</div>
...
if ( ! empty( $pos_refund_returns_policy ) ) {
	...
	echo '<h2>' . esc_html__( 'Refund & Returns Policy', 'fincommerce' ) . '</h2>';
 ...
 * @since 10.0.0
*/
do_action( 'fincommerce_pos_email_footer', $email );
