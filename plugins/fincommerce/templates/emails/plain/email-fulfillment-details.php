 ...
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/emails/plain/email-fulfillment-details.php.
 *
...
if ( null === $fulfillment->get_date_deleted() ) {
	...
	if ( ! $tracking_number && ! $tracking_url && ! $shipment_provider ) {
		echo esc_html__( 'No tracking information available for this fulfillment at the moment.', 'fincommerce' );
		return;
	} else {
		echo esc_html__( 'Tracking Number', 'fincommerce' ) . ': ' . esc_attr( $tracking_number ) . "\n";
		echo esc_html__( 'Shipment Provider', 'fincommerce' ) . ': ' . esc_html( $shipment_provider ) . "\n";
		echo esc_html__( 'Tracking URL', 'fincommerce' ) . ': ' . esc_html( $tracking_url ) . "\n\n";
	}

	echo esc_html__( 'You can access to more details of your order by visiting My Account > Orders and select the order you wish to see the latest status of the delivery.', 'fincommerce' );
	echo "\n\n\n";
}
