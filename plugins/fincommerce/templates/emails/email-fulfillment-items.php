 ...
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/emails/email-fulfillment-items.php.
 *
...
foreach ( $items as $item_id => $item ) :
	 ...
	 * @since 2.1.0
	 */
	if ( ! apply_filters( 'fincommerce_order_item_visible', true, $item->item ) ) {
		continue;
	}
	 ...
	 * @since 2.1.0
	 */
	$order_item_class = apply_filters( 'fincommerce_order_item_class', 'order_item', $item->item, $order );
	?>
	<tr class="<?php echo esc_attr( $order_item_class ); ?>">
			...
			<table class="order-item-data">
				<tr>
					...
					if ( $show_image ) {
						 ...
						 * @since 2.1.0
						 */
						echo '<td>' . wp_kses_post( apply_filters( 'fincommerce_order_item_thumbnail', $image, $item->item ) ) . '</td>';
					}
					?>
					<td>
						<?php
						 ...
						 * @since 2.1.0
						 */
						echo wp_kses_post( apply_filters( 'fincommerce_order_item_name', $item->item->get_name(), $item->item, false ) );
						 ...
						 * @since 2.3.0
						 */
						do_action( 'fincommerce_order_item_meta_start', $item_id, $item->item, $order, $plain_text );
						 ...
						 * @since 2.3.0
						 */
						do_action( 'fincommerce_order_item_meta_end', $item_id, $item->item, $order, $plain_text );
		...
		<td class="td font-family text-align-<?php echo esc_attr( $price_text_align ); ?>" style="vertical-align:middle;">
			 ...
			 * @since 2.4.0
			 */
			echo wp_kses_post( apply_filters( 'fincommerce_email_order_item_quantity', $qty_display, $item->item ) );
			?>
		</td>
		<td class="td font-family text-align-<?php echo esc_attr( $price_text_align ); ?>" style="vertical-align:middle;">
