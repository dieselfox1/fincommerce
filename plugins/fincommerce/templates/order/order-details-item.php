 ...
 * Order Item Details
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/order/order-details-item.php.
 *
...
?>
<tr class="<?php echo esc_attr( apply_filters( 'fincommerce_order_item_class', 'fincommerce-table__line-item order_item', $item, $order ) ); ?>">

	<td class="fincommerce-table__product-name product-name">
		<?php
		$is_visible        = $product && $product->is_visible();
		$product_permalink = apply_filters( 'fincommerce_order_item_permalink', $is_visible ? $product->get_permalink( $item ) : '', $item, $order );

		echo wp_kses_post( apply_filters( 'fincommerce_order_item_name', $product_permalink ? sprintf( '<a href="%s">%s</a>', $product_permalink, $item->get_name() ) : $item->get_name(), $item, $is_visible ) );
	...
	<td class="fincommerce-table__product-total product-total">
		<?php echo $order->get_formatted_line_subtotal( $item ); // phpcs:ignore finpress.Security.EscapeOutput.OutputNotEscaped ?>
	</td>
