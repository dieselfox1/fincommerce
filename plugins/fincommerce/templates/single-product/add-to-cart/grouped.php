 ...
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/single-product/add-to-cart/grouped.php.
 *
...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'fincommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="fincommerce-grouped-product-list group_table">
		<tbody>
			<?php
			$quantites_required      = false;
			$previous_post           = $post;
			$grouped_product_columns = apply_filters(
				'fincommerce_grouped_product_columns',
				array(
					'quantity',
					'label',
					'price',
			...
			do_action( 'fincommerce_grouped_product_list_before', $grouped_product_columns, $quantites_required, $product );
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				echo '<tr id="product-' . esc_attr( $grouped_product_child->get_id() ) . '" class="fincommerce-grouped-product-list-item ' . esc_attr( implode( ' ', wc_get_product_class( '', $grouped_product_child ) ) ) . '">';

				// Output columns for each product.
				foreach ( $grouped_product_columns as $column_id ) {
					do_action( 'fincommerce_grouped_product_list_before_' . $column_id, $grouped_product_child );
					...
					switch ( $column_id ) {
						case 'quantity':
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								fincommerce_template_loop_add_to_cart();
								...
								if ( $grouped_product_child->is_on_sale() ) {
									printf(
										...
										esc_html__( 'Buy one of %1$s on sale for %2$s, original price was %3$s', 'fincommerce' ),
									...
									printf(
										...
										esc_html__( 'Buy one of %1$s for %2$s', 'fincommerce' ),
										esc_html( $grouped_product_child->get_name() ),
										esc_html( wp_strip_all_tags( wc_price( $grouped_product_child->get_price() ) ) )
									);
								}
						...
						case 'label':
							...
							$value .= $grouped_product_child->is_visible() ? '<a href="' . esc_url( apply_filters( 'fincommerce_grouped_product_list_link', $grouped_product_child->get_permalink(), $grouped_product_child->get_id() ) ) . '">' . $grouped_product_child->get_name() . '</a>' : $grouped_product_child->get_name();
							$value .= '</label>';
							break;
						case 'price':
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<?php do_action( 'fincommerce_before_add_to_cart_button' ); ?>

		<button type="submit" class="single_add_to_cart_button button alt<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>

		<?php do_action( 'fincommerce_after_add_to_cart_button' ); ?>

	<?php endif; ?>
</form>

<?php do_action( 'fincommerce_after_add_to_cart_form' ); ?>
