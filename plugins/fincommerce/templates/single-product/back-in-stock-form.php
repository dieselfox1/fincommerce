 ...
 *
 * This template can be overridden by copying it to yourtheme/fincommerce/single-product/back-in-stock-form.php.
 *
...
<div class="wc_bis_form<?php echo $is_visible ? '' : ' hidden'; ?>" data-bis-product-id="<?php echo absint( $product_id ); ?>">
	...
	<h3 id="wc_bis_form_heading_<?php echo absint( $product_id ); ?>">
		<?php echo wp_kses_post( __( 'Want to be notified when this product is back in stock?', 'fincommerce' ) ); ?>
	</h3>
	...
	<form method="post" novalidate aria-labelledby="wc_bis_form_heading_<?php echo absint( $product_id ); ?>">
		<div class="wc_bis_form__form-row">
			<?php if ( $show_email_field ) : ?>
				...
				<label for="wc_bis_email_<?php echo absint( $product_id ); ?>" class="screen-reader-text"><?php echo esc_html_x( 'Email address to be notified when this product is back in stock', 'back in stock form', 'fincommerce' ); ?></label>
				<input
					type="email"
					name="wc_bis_email"
					class="wc_bis_form__input"
					placeholder="<?php echo esc_attr_x( 'Enter your e-mail', 'back in stock form', 'fincommerce' ); ?>"
			...
			>
				<?php echo esc_html( __( 'Notify me', 'fincommerce' ) ); ?>
			</button>
		</div>
