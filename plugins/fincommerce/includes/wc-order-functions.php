 ...
 *
 * Args and usage: https://github.com/dieselfox1/fincommerce/wiki/wc_get_orders-and-WC_Order_Query
 *
 * @since  2.6.0
...
function wc_get_order( $the_order = false ) {
	if ( ! did_action( 'fincommerce_after_register_post_type' ) ) {
		wc_doing_it_wrong( __FUNCTION__, 'wc_get_order should not be called before post types are registered (fincommerce_after_register_post_type action)', '2.5' );
		return false;
	}
...
function wc_get_order_statuses() {
	$order_statuses = array(
		OrderInternalStatus::PENDING    => _x( 'Pending payment', 'Order status', 'fincommerce' ),
		OrderInternalStatus::PROCESSING => _x( 'Processing', 'Order status', 'fincommerce' ),
		OrderInternalStatus::ON_HOLD    => _x( 'On hold', 'Order status', 'fincommerce' ),
		OrderInternalStatus::COMPLETED  => _x( 'Completed', 'Order status', 'fincommerce' ),
		OrderInternalStatus::CANCELLED  => _x( 'Cancelled', 'Order status', 'fincommerce' ),
		OrderInternalStatus::REFUNDED   => _x( 'Refunded', 'Order status', 'fincommerce' ),
		OrderInternalStatus::FAILED     => _x( 'Failed', 'Order status', 'fincommerce' ),
	);
...
function wc_get_is_paid_statuses() {
	 ...
	 */
	return apply_filters( 'fincommerce_order_is_paid_statuses', array( OrderStatus::PROCESSING, OrderStatus::COMPLETED ) );
}
...
function wc_get_is_pending_statuses() {
	 ...
	 * @param array $statuses List of statuses.
	 */
	return apply_filters( 'fincommerce_order_is_pending_statuses', array( OrderStatus::PENDING ) );
}
...
function wc_get_order_status_name( $status ) {
	// "Special statuses": these are in common usage across FinCommerce, but are not normally returned by
	...
	if ( ! is_string( $status ) ) {
		wc_doing_it_wrong(
			__FUNCTION__,
			__( 'An invalid order status slug was supplied.', 'fincommerce' ),
			'9.6'
		);
	}
...
function wc_generate_order_key( $key = '' ) {
	...
	return 'wc_' . apply_filters( 'fincommerce_generate_order_key', 'order_' . $key );
}
...
function wc_downloadable_file_permission( $download_id, $product, $order, $qty = 1, $item = null ) {
	...
	$download = apply_filters( 'fincommerce_downloadable_file_permission', $download, $product, $order, $qty, $item );
...
function wc_downloadable_product_permissions( $order_id, $force = false ) {
	...
	if ( $order->has_status( OrderStatus::PROCESSING ) && 'no' === get_option( 'fincommerce_downloads_grant_access_after_payment' ) ) {
		return;
	}
	...
	do_action( 'fincommerce_grant_product_download_permissions', $order_id );
}
add_action( 'fincommerce_order_status_completed', 'wc_downloadable_product_permissions' );
add_action( 'fincommerce_order_status_processing', 'wc_downloadable_product_permissions' );
...
function wc_delete_shop_order_transients( $order = 0 ) {
	...
	do_action( 'fincommerce_delete_shop_order_transients', $order_id );
}
...
function wc_ship_to_billing_address_only() {
	return 'billing_only' === get_option( 'fincommerce_ship_to_destination' );
}
...
function wc_create_refund( $args = array() ) {
	...
	try {
		...
		if ( ! $order ) {
			throw new Exception( __( 'Invalid order ID.', 'fincommerce' ) );
		}
		 ...
		 * @since 3.0.0
		 */
		do_action( 'fincommerce_create_refund', $refund, $args );
		...
		if ( $refund->save() ) {
			...
			if ( (bool) apply_filters(
				'fincommerce_order_is_partially_refunded',
				...
				do_action( 'fincommerce_order_partially_refunded', $order->get_id(), $refund->get_id() );
			} else {
				do_action( 'fincommerce_order_fully_refunded', $order->get_id(), $refund->get_id() );
				 ...
				 * @param int    $refund_id     The refund ID.
				 */
				$parent_status = apply_filters( 'fincommerce_order_fully_refunded_status', OrderStatus::REFUNDED, $order->get_id(), $refund->get_id() );
		...
		do_action( 'fincommerce_refund_created', $refund->get_id(), $args );
		do_action( 'fincommerce_order_refunded', $order->get_id(), $refund->get_id() );
...
function wc_refund_payment( $order, $amount, $reason = '' ) {
	try {
		...
		if ( ! $gateway ) {
			throw new Exception( __( 'The payment gateway for this order does not exist.', 'fincommerce' ) );
		}

		if ( ! $gateway->supports( PaymentGatewayFeature::REFUNDS ) ) {
			throw new Exception( __( 'The payment gateway for this order does not support automatic refunds.', 'fincommerce' ) );
		}

		$result = $gateway->process_refund( $order->get_id(), $amount, $reason );

		if ( ! $result ) {
			throw new Exception( __( 'An error occurred while attempting to create the refund using the payment gateway API.', 'fincommerce' ) );
		}
...
function wc_restock_refunded_items( $order, $refunded_line_items ) {
	if ( ! apply_filters( 'fincommerce_can_restock_refunded_items', true, $order, $refunded_line_items ) ) {
		return;
	}
	...
	foreach ( $line_items as $item_id => $item ) {
		...
		$restock_note = sprintf( __( 'Item #%1$s stock increased from %2$s to %3$s.', 'fincommerce' ), $product->get_id(), $old_stock, $new_stock );
		 ...
		 */
		$restock_note = apply_filters( 'fincommerce_refund_restock_note', $restock_note, $old_stock, $new_stock, $order, $product );

		$order->add_order_note( $restock_note );

		$item->save();

		do_action( 'fincommerce_restock_refunded_item', $product->get_id(), $old_stock, $new_stock, $order, $product );
	}
}
...
function wc_get_tax_class_by_tax_id( $tax_id ) {
	global $wpdb;
	return $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_class FROM {$wpdb->prefix}fincommerce_tax_rates WHERE tax_rate_id = %d", $tax_id ) );
}
...
function wc_order_fully_refunded( $order_id ) {
	...
	$order->add_order_note( __( 'Order status set to refunded. To return funds to the customer you will need to issue a refund through your payment gateway.', 'fincommerce' ) );
}
add_action( 'fincommerce_order_status_refunded', 'wc_order_fully_refunded' );
...
function wc_update_total_sales_counts( $order_id ) {
	...
	if ( ! $reflected_order && 'fincommerce_before_delete_order' === current_action() ) {
	 ...
	 * @param int $order_id order id
	 */
	do_action( 'fincommerce_recorded_sales', $order_id );
}
add_action( 'fincommerce_order_status_completed', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_order_status_processing', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_order_status_on-hold', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_order_status_completed_to_cancelled', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_order_status_processing_to_cancelled', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_order_status_on-hold_to_cancelled', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_trash_order', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_untrash_order', 'wc_update_total_sales_counts' );
add_action( 'fincommerce_before_delete_order', 'wc_update_total_sales_counts' );
...
function wc_update_coupon_usage_counts( $order_id ) {
	...
	$invalid_statuses = apply_filters(
		'fincommerce_update_coupon_usage_invalid_statuses',
		$invalid_statuses
	);
...
add_action( 'fincommerce_order_status_pending', 'wc_update_coupon_usage_counts' );
add_action( 'fincommerce_order_status_completed', 'wc_update_coupon_usage_counts' );
add_action( 'fincommerce_order_status_processing', 'wc_update_coupon_usage_counts' );
add_action( 'fincommerce_order_status_on-hold', 'wc_update_coupon_usage_counts' );
add_action( 'fincommerce_order_status_cancelled', 'wc_update_coupon_usage_counts' );
add_action( 'fincommerce_order_status_failed', 'wc_update_coupon_usage_counts' );
add_action( 'fincommerce_trash_order', 'wc_update_coupon_usage_counts' );
...
function wc_cancel_unpaid_orders() {
	...
	if ( $held_duration < 1 || 'yes' !== get_option( 'fincommerce_manage_stock' ) ) {
		return;
	}
	...
	if ( $unpaid_orders ) {
		foreach ( $unpaid_orders as $unpaid_order ) {
			...
			if ( apply_filters( 'fincommerce_cancel_unpaid_order', 'checkout' === $order->get_created_via(), $order ) ) {
				$order->update_status( OrderStatus::CANCELLED, __( 'Unpaid order cancelled - time limit reached.', 'fincommerce' ) );
			}
		}
	}
}
add_action( 'fincommerce_cancel_unpaid_orders', 'wc_cancel_unpaid_orders' );
...
add_filter( 'fincommerce_shortcode_order_tracking_order_id', 'wc_sanitize_order_id' );
...
function wc_get_order_note( $data ) {
	...
	return (object) apply_filters(
		...
		array(
			...
			'added_by'      => __( 'FinCommerce', 'fincommerce' ) === $data->comment_author ? 'system' : $data->comment_author,
...
function wc_create_order_note( $order_id, $note, $is_customer_note = false, $added_by_user = false ) {
	...
	if ( ! $order ) {
		return new WP_Error( 'invalid_order_id', __( 'Invalid order ID.', 'fincommerce' ), array( 'status' => 400 ) );
	}
...
function wc_delete_order_note( $note_id ) {
	...
	if ( $note && wp_delete_comment( $note_id, true ) ) {
		 ...
		 * @since 9.1.0
		 */
		do_action( 'fincommerce_order_note_deleted', $note_id, $note );
