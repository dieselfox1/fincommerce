<?php
/**
 * FinCommerce Customer Functions
 *
 ...
 *
 * Note: get_option( 'fincommerce_lock_down_admin', true ) is a deprecated option here for backwards compatibility. Defaults to true.
 *
...
function wc_disable_admin_bar( $show_admin_bar ) {
	/**
	 * Controls whether the FinCommerce admin bar should be disabled.
	 *
	 * @since 3.0.0
	 *
	 * @param bool $enabled
	 */
	if ( apply_filters( 'fincommerce_disable_admin_bar', true ) && ! ( current_user_can( 'edit_posts' ) || current_user_can( 'manage_fincommerce' ) ) ) {
...
if ( ! function_exists( 'wc_create_new_customer' ) ) {
	 ...
	 *
	 * @since 9.4.0 Moved fincommerce_registration_error_email_exists filter to the shortcode checkout class.
	...
	function wc_create_new_customer( $email, $username = '', $password = '', $args = array() ) {
		if ( empty( $email ) || ! is_email( $email ) ) {
			return new WP_Error( 'registration-error-invalid-email', __( 'Please provide a valid email address.', 'fincommerce' ) );
		}
		...
		if ( email_exists( $email ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'An account is already registered with %s. Please log in or use a different email address.', 'fincommerce' ),
					esc_html( $email )
				)
			);
		}
		...
		if ( empty( $username ) || ! validate_username( $username ) ) {
			return new WP_Error( 'registration-error-invalid-username', __( 'Please provide a valid account username.', 'fincommerce' ) );
		}

		if ( username_exists( $username ) ) {
			return new WP_Error( 'registration-error-username-exists', __( 'An account is already registered with that username. Please choose another.', 'fincommerce' ) );
		}
		...
		if ( empty( $password ) ) {
			return new WP_Error( 'registration-error-missing-password', __( 'Please create a password for your account.', 'fincommerce' ) );
		}
		 ...
		 */
		do_action( 'fincommerce_register_post', $username, $email, $errors );
		 ...
		 * @return \WP_Error
		 */
		$errors = apply_filters( 'fincommerce_registration_errors', $errors, $username, $email );
		 ...
		 */
		do_action( 'fincommerce_created_customer', $customer_id, $new_customer_data, $password_generated );
...
function wc_create_new_customer_username( $email, $new_user_args = array(), $suffix = '' ) {
	...
	if ( in_array( strtolower( $username ), array_map( 'strtolower', $illegal_logins ), true ) ) {
		...
		$new_args['first_name'] = apply_filters(
			'fincommerce_generated_customer_username',
	 ...
	 * @param string $suffix        Append string to username to make it unique.
	 */
	return apply_filters( 'fincommerce_new_customer_username', $username, $email, $new_user_args, $suffix );
}
...
function wc_update_new_customer_past_orders( $customer_id ) {
	...
	if ( ! empty( $customer_orders ) ) {
		foreach ( $customer_orders as $order_id ) {
			...
			do_action( 'fincommerce_update_new_customer_past_order', $order_id, $customer );
...
add_action( 'fincommerce_payment_complete', 'wc_paying_customer' );
add_action( 'fincommerce_order_status_completed', 'wc_paying_customer' );
...
function wc_customer_bought_product( $customer_email, $user_id, $product_id ) {
	...
	$result = apply_filters( 'fincommerce_pre_customer_bought_product', null, $customer_email, $user_id, $product_id );
	 ...
	 * @return bool
	 */
	$use_lookup_tables = apply_filters( 'fincommerce_customer_bought_product_use_lookup_tables', false, $customer_email, $user_id, $product_id );

	if ( $use_lookup_tables ) {
		// Lookup tables get refreshed along with the `fincommerce_reports` transient version (due to async processing).
		...
		$cache_version = WC_Cache_Helper::get_transient_version( 'fincommerce_reports' );
...
INNER JOIN {$wpdb->prefix}fincommerce_order_items AS i ON o.id = i.order_id
INNER JOIN {$wpdb->prefix}fincommerce_order_itemmeta AS im ON i.order_item_id = im.order_item_id
WHERE o.status IN ('" . implode( "','", $statuses ) . "')
...
INNER JOIN {$wpdb->postmeta} AS pm ON p.ID = pm.post_id
INNER JOIN {$wpdb->prefix}fincommerce_order_items AS i ON p.ID = i.order_id
INNER JOIN {$wpdb->prefix}fincommerce_order_itemmeta AS im ON i.order_item_id = im.order_item_id
 ...
 * access to the capabilities if FinCommerce is deactivated.
 *
 * @since 3.5.4
...
function wc_modify_editable_roles( $roles ) {
	...
	if ( ! wc_current_user_has_role( 'administrator' ) ) {
		...
		if ( wc_current_user_has_role( 'shop_manager' ) ) {
			$shop_manager_editable_roles = apply_filters( 'fincommerce_shop_manager_editable_roles', array( 'customer' ) );
		...
		case 'delete_user':
			if ( ! isset( $args[0] ) || $args[0] === $user_id ) {
				...
				if ( wc_user_has_role( $args[0], 'administrator' ) ) {
					...
					$shop_manager_editable_roles = apply_filters( 'fincommerce_shop_manager_editable_roles', array( 'customer' ) ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
...
function wc_get_customer_download_permissions( $customer_id ) {
	$data_store = WC_Data_Store::load( 'customer-download' );
	return apply_filters( 'fincommerce_permission_list', $data_store->get_downloads_for_customer( $customer_id ), $customer_id ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
}
...
function wc_get_customer_available_downloads( $customer_id ) {
	...
	if ( $results ) {
		foreach ( $results as $result ) {
			...
			$download_name = apply_filters(
				'fincommerce_downloadable_product_name',
				$download_file['name'],
				$_product,
				$result->download_id,
				$file_number
	...
	// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
	return apply_filters( 'fincommerce_customer_available_downloads', $downloads, $customer_id );
}
...
function wc_meta_update_last_update_time( $meta_id, $user_id, $meta_key, $_meta_value ) {
	$keys_to_track = apply_filters( 'fincommerce_user_last_update_fields', array( 'first_name', 'last_name' ) ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
...
function wc_get_customer_saved_methods_list( $customer_id ) {
	return apply_filters( 'fincommerce_saved_payment_methods_list', array(), $customer_id ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
}
...
function wc_maybe_store_user_agent( $user_login, $user ) {
	if ( 'yes' === get_option( 'fincommerce_allow_tracking', 'no' ) && user_can( $user, 'manage_fincommerce' ) ) {
		$admin_user_agents   = array_filter( (array) get_option( 'fincommerce_tracker_ua', array() ) );
		$admin_user_agents[] = wc_get_user_agent();
		update_option( 'fincommerce_tracker_ua', array_unique( $admin_user_agents ), false );
	}
}
...
function wc_user_logged_in( $user_login, $user ) {
	...
	update_user_meta( $user->ID, '_fincommerce_load_saved_cart_after_login', 1 );
}
...
/**
 * Translate WC roles using the fincommerce textdomain.
 *
 * @since 3.7.0
...
function wc_translate_user_roles( $translation, $text, $context, $domain ) {
	...
	if ( 'User role' === $context && 'default' === $domain && in_array( $text, array( 'Shop manager', 'Customer' ), true ) ) {
		return translate_user_role( $text, 'fincommerce' );
	}
