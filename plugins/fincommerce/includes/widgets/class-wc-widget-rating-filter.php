...
class WC_Widget_Rating_Filter extends WC_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'fincommerce widget_rating_filter';
		$this->widget_description = __( 'Display a list of star ratings to filter products in your store.', 'fincommerce' );
		$this->widget_id          = 'fincommerce_rating_filter';
		$this->widget_name        = __( 'Filter Products by Rating', 'fincommerce' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Average rating', 'fincommerce' ),
	...
	public function widget( $args, $instance ) {
		...
		for ( $rating = 5; $rating >= 1; $rating-- ) {
			...
			$link        = apply_filters( 'fincommerce_rating_filter_link', $link_ratings ? add_query_arg( 'rating_filter', $link_ratings, $link ) : remove_query_arg( 'rating_filter' ) );
			$rating_html = wc_get_star_rating_html( $rating );
			$count_html  = wp_kses(
				apply_filters( 'fincommerce_rating_filter_count', "({$count})", $count, $rating ),
				array(
					'em'     => array(),
					'span'   => array(),
					'strong' => array(),
