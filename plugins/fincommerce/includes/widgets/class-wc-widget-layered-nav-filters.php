...
class WC_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'fincommerce widget_layered_nav_filters';
		$this->widget_description = __( 'Display a list of active product filters.', 'fincommerce' );
		$this->widget_id          = 'fincommerce_layered_nav_filters';
		$this->widget_name        = __( 'Active Product Filters', 'fincommerce' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Active filters', 'fincommerce' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) ) {
			 ...
			 * @param array $instance Widget instance.
			 */
			do_action( 'fincommerce_widget_layered_nav_filters_start', $args, $instance );
			...
			if ( ! empty( $_chosen_attributes ) ) {
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					foreach ( $data['terms'] as $term_slug ) {
						 ...
						 * @since 8.8.0
						 */
						$anchor_text = apply_filters( 'fincommerce_widget_layered_nav_term_anchor_text', $term->name, $term, $taxonomy );
						echo '<li class="' . esc_attr( implode( ' ', $filter_classes ) ) . '"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'fincommerce' ) . '" href="' . esc_url( $link ) . '">' . esc_html( $anchor_text ) . '</a></li>';
					}
				}
			}
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'fincommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( 'Min %s', 'fincommerce' ), wc_price( $min_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}

			if ( $max_price ) {
				$link = remove_query_arg( 'max_price', $base_link );
				/* translators: %s: maximum price */
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'fincommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( 'Max %s', 'fincommerce' ), wc_price( $max_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}
			...
			if ( ! empty( $rating_filter ) ) {
				foreach ( $rating_filter as $rating ) {
					...
					echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'fincommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Rated %s out of 5', 'fincommerce' ), esc_html( $rating ) ) . '</a></li>';
				}
			}
			 ...
			 * @param array $instance Widget instance.
			 */
			do_action( 'fincommerce_widget_layered_nav_filters_end', $args, $instance );
