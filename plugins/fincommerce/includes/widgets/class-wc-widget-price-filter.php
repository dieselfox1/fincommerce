...
class WC_Widget_Price_Filter extends WC_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'fincommerce widget_price_filter';
		$this->widget_description = __( 'Display a slider to filter products in your store by price.', 'fincommerce' );
		$this->widget_id          = 'fincommerce_price_filter';
		$this->widget_name        = __( 'Filter Products by Price', 'fincommerce' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter by price', 'fincommerce' ),
		...
		wp_localize_script(
			'wc-price-slider',
			'fincommerce_price_slider_params',
			array(
				'currency_format_num_decimals' => 0,
				'currency_format_symbol'       => get_fincommerce_currency_symbol(),
				'currency_format_decimal_sep'  => esc_attr( wc_get_price_decimal_separator() ),
				'currency_format_thousand_sep' => esc_attr( wc_get_price_thousand_separator() ),
				'currency_format'              => esc_attr( str_replace( array( '%1$s', '%2$s' ), array( '%s', '%v' ), get_fincommerce_price_format() ) ),
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		$step = max( apply_filters( 'fincommerce_price_filter_widget_step', 10 ), 1 );
		...
		$min_price = apply_filters( 'fincommerce_price_filter_widget_min_amount', floor( $min_price / $step ) * $step );
		$max_price = apply_filters( 'fincommerce_price_filter_widget_max_amount', ceil( $max_price / $step ) * $step );
	...
	protected function get_filtered_price() {
		...
		$sql = "
			...
			WHERE product_id IN (
				...
				WHERE {$wpdb->posts}.post_type IN ('" . implode( "','", array_map( 'esc_sql', apply_filters( 'fincommerce_price_filter_post_type', array( 'product' ) ) ) ) . "')
				AND {$wpdb->posts}.post_status = 'publish'
				" . $tax_query_sql['where'] . $meta_query_sql['where'] . $search_query_sql . '
			)';

		$sql = apply_filters( 'fincommerce_price_filter_sql', $sql, $meta_query_sql, $tax_query_sql );
