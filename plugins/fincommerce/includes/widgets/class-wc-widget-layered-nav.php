...
class WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'fincommerce widget_layered_nav fincommerce-widget-layered-nav';
		$this->widget_description = __( 'Display a list of attributes to filter products in your store.', 'fincommerce' );
		$this->widget_id          = 'fincommerce_layered_nav';
		$this->widget_name        = __( 'Filter Products by Attribute', 'fincommerce' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'        => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'fincommerce' ),
				'label' => __( 'Title', 'fincommerce' ),
			),
			'attribute'    => array(
				'type'    => 'select',
				'std'     => $std_attribute,
				'label'   => __( 'Attribute', 'fincommerce' ),
				'options' => $attribute_array,
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'fincommerce' ),
				'options' => array(
					'list'     => __( 'List', 'fincommerce' ),
					'dropdown' => __( 'Dropdown', 'fincommerce' ),
				),
			),
			'query_type'   => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'fincommerce' ),
				'options' => array(
					'and' => __( 'AND', 'fincommerce' ),
					'or'  => __( 'OR', 'fincommerce' ),
				),
			),
		);
	}
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$any_label      = apply_filters( 'fincommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'fincommerce' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
			...
			echo '<form method="get" action="' . esc_url( $form_action ) . '" class="fincommerce-widget-layered-nav-dropdown">';
			echo '<select class="fincommerce-widget-layered-nav-dropdown dropdown_layered_nav_' . esc_attr( $taxonomy_filter_name ) . '"' . ( $multiple ? 'multiple="multiple"' : '' ) . '>';
			...
			if ( $multiple ) {
				echo '<button class="fincommerce-widget-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'fincommerce' ) . '">' . esc_html__( 'Apply', 'fincommerce' ) . '</button>';
			}
				...
				if ( jQuery().selectWoo ) {
					var wc_layered_nav_select = function() {
						jQuery( '.dropdown_layered_nav_" . esc_js( $taxonomy_filter_name ) . "' ).selectWoo( {
							...
							language: {
								noResults: function() {
									return '" . esc_js( _x( 'No matches found', 'enhanced select', 'fincommerce' ) ) . "';
								}
							}
						} );
					};
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		// List display.
		echo '<ul class="fincommerce-widget-layered-nav-list">';
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				$link      = apply_filters( 'fincommerce_layered_nav_link', $link, $term, $taxonomy );
