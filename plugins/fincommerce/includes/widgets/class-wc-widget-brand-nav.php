...
class WC_Widget_Brand_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'fincommerce widget_brand_nav widget_layered_nav';
		$this->widget_description = __( 'Shows brands in a widget which lets you narrow down the list of products when viewing products.', 'fincommerce' );
		$this->widget_id          = 'fincommerce_brand_nav';
		$this->widget_name        = __( 'FinCommerce Brand Layered Nav', 'fincommerce' );

		add_filter( 'fincommerce_product_subcategories_args', array( $this, 'filter_out_cats' ) );
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( empty( $new_instance['title'] ) ) {
			$new_instance['title'] = __( 'Brands', 'fincommerce' );
		}
	...
	public function form( $instance ) {
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'fincommerce' ); ?></label>
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>"><?php esc_html_e( 'Display Type:', 'fincommerce' ); ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'display_type' ) ); ?>">
			<option value="list" <?php selected( $instance['display_type'], 'list' ); ?>><?php esc_html_e( 'List', 'fincommerce' ); ?></option>
			<option value="dropdown" <?php selected( $instance['display_type'], 'dropdown' ); ?>><?php esc_html_e( 'Dropdown', 'fincommerce' ); ?></option>
		</select></p>
		<?php
	}
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $depth = 0 ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			if ( 0 === $depth ) {
				...
				echo '<option value="">' . esc_html__( 'Any Brand', 'fincommerce' ) . '</option>';
			}
	...
	protected function layered_nav_list( $terms, $taxonomy, $depth = 0 ) {
		...
		foreach ( $terms as $term ) {
			...
			echo ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'fincommerce_layered_nav_link', $link ) ) . '">' : '<span>'; // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment

			echo esc_html( $term->name );

			echo ( $count > 0 || $option_is_set ) ? '</a> ' : '</span> ';

			echo wp_kses_post( apply_filters( 'fincommerce_layered_nav_count', '<span class="count">(' . absint( $count ) . ')</span>', $count, $term ) );// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
	...
	protected function get_filtered_term_product_counts( $term_ids, $taxonomy, $query_type = 'and' ) {
		...
		$query             = apply_filters( 'fincommerce_get_filtered_term_product_counts_query', $query ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		$query             = implode( ' ', $query );

		// We have a query - let's see if cached results of this query already exist.
		$query_hash = md5( $query );

		$cache = apply_filters( 'fincommerce_layered_nav_count_maybe_cache', true ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		if ( true === $cache ) {
