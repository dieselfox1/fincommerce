...
class WC_Order_Item_Product extends WC_Order_Item {
	...
	*/
	...
	public function set_tax_class( $value ) {
		if ( $value && ! in_array( $value, WC_Tax::get_tax_class_slugs(), true ) ) {
			$this->error( 'order_item_product_invalid_tax_class', __( 'Invalid tax class', 'fincommerce' ) );
		}
	...
	public function set_product_id( $value ) {
		if ( $value > 0 && 'product' !== get_post_type( absint( $value ) ) ) {
			$this->error( 'order_item_product_invalid_product_id', __( 'Invalid product ID', 'fincommerce' ) );
		}
	...
	public function set_variation_id( $value ) {
		if ( $value > 0 && 'product_variation' !== get_post_type( $value ) ) {
			$this->error( 'order_item_product_invalid_variation_id', __( 'Invalid variation ID', 'fincommerce' ) );
		}
	...
	public function set_product( $product ) {
		if ( ! is_a( $product, 'WC_Product' ) ) {
			$this->error( 'order_item_product_invalid_product', __( 'Invalid product', 'fincommerce' ) );
		}
	...
	public function set_backorder_meta() {
		...
		if ( $product && $product->backorders_require_notification() && $product->is_on_backorder( $this->get_quantity() ) ) {
			$this->add_meta_data( apply_filters( 'fincommerce_backordered_item_meta_name', __( 'Backordered', 'fincommerce' ), $this ), $this->get_quantity() - max( 0, $product->get_stock_quantity() ), true );
		}
	}
	...
	*/
	...
	public function get_product() {
		...
		if ( has_filter( 'fincommerce_get_product_from_item' ) ) {
			$product = apply_filters( 'fincommerce_get_product_from_item', $product, $this, $this->get_order() );
		}

		return apply_filters( 'fincommerce_order_item_product', $product, $this );
	}
	...
	public function get_item_downloads() {
		...
		return apply_filters( 'fincommerce_get_item_downloads', $files, $this, $order );
	}
	...
	public function get_formatted_meta_data( $hideprefix = '_', $include_all = false ) {
		...
		if ( $order && $order->has_status( OrderStatus::COMPLETED ) ) {
			 ...
			 * @return string
			 */
			$backorder_meta_key = apply_filters( 'fincommerce_backordered_item_meta_name', __( 'Backordered', 'fincommerce' ), $this );
