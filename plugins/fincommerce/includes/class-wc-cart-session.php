...
final class WC_Cart_Session {
	...
	public function init() {
		 ...
		 * @since 6.9.0
		 */
		if ( ! apply_filters( 'fincommerce_cart_session_initialize', true, $this ) ) {
			return;
		}

		add_action( 'wp_loaded', array( $this, 'get_cart_from_session' ) );
		add_action( 'fincommerce_cart_emptied', array( $this, 'destroy_cart_session' ) );
		add_action( 'fincommerce_after_calculate_totals', array( $this, 'set_session' ), 1000 );
		add_action( 'fincommerce_removed_coupon', array( $this, 'set_session' ) );

		// Persistent cart stored to usermeta.
		add_action( 'fincommerce_add_to_cart', array( $this, 'persistent_cart_update' ) );
		add_action( 'fincommerce_cart_item_removed', array( $this, 'persistent_cart_update' ) );
		add_action( 'fincommerce_cart_item_restored', array( $this, 'persistent_cart_update' ) );
		add_action( 'fincommerce_cart_item_set_quantity', array( $this, 'persistent_cart_update' ) );

		// Cookie events - cart cookies need to be set before headers are sent.
		add_action( 'fincommerce_add_to_cart', array( $this, 'maybe_set_cart_cookies' ) );
	...
	public function get_cart_from_session() {
		 ...
		 * @since 3.2.0
		 */
		do_action( 'fincommerce_load_cart_from_session' );
		...
		$merge_saved_cart    = (bool) get_user_meta( get_current_user_id(), '_fincommerce_load_saved_cart_after_login', true );
		...
		if ( is_null( $cart ) || $merge_saved_cart ) {
			...
			delete_user_meta( get_current_user_id(), '_fincommerce_load_saved_cart_after_login' );
		}
		...
		foreach ( $cart as $key => $values ) {
			 ...
			 */
			if ( apply_filters( 'fincommerce_pre_remove_cart_item_from_session', false, $key, $values, $product ) ) {
				$update_cart_session = true;
				/**
				 ...
				 * @param WC_Product $product The product being added to the cart.
				 */
				do_action( 'fincommerce_remove_cart_item_from_session', $key, $values, $product );
				 ...
				 * @since 7.0.0
				 */
			} elseif ( ! apply_filters( 'fincommerce_cart_item_is_purchasable', $product->is_purchasable(), $key, $values, $product ) ) {
				$update_cart_session = true;
				/* translators: %s: product name */
				$message = sprintf( __( '%s has been removed from your cart because it can no longer be purchased. Please contact us if you need assistance.', 'fincommerce' ), $product->get_name() );
				/**
				 ...
				 */
				$message = apply_filters( 'fincommerce_cart_item_removed_message', $message, $product );
				 ...
				 * @param array      $values Cart item values e.g. quantity and product_id.
				 */
				do_action( 'fincommerce_remove_cart_item_from_session', $key, $values );
				...
				$message = sprintf( __( '%1$s has been removed from your cart because it has since been modified. You can add it back to your cart <a href="%2$s">here</a>.', 'fincommerce' ), $product->get_name(), $product->get_permalink() );
				 ...
				 * @param WC_Product $product Product data.
				 */
				$message = apply_filters( 'fincommerce_cart_item_removed_because_modified_message', $message, $product );
				 ...
				 */
				$cart_contents[ $key ] = apply_filters( 'fincommerce_get_cart_item_from_session', $session_data, $values, $key );
				if ( ! isset( $cart_contents[ $key ]['data'] ) || ! $cart_contents[ $key ]['data'] instanceof WC_Product ) {
					...
					wc_doing_it_wrong(
						__METHOD__,
						'When filtering cart items with fincommerce_get_cart_item_from_session, each item must have a data key containing a product object.',
						'9.8.0'
					);
				...
				// Add to cart right away so the product is visible in fincommerce_get_cart_item_from_session hook.
		...
		if ( ! empty( $cart_contents ) ) {
			 ...
			 */
			$this->cart->set_cart_contents( apply_filters( 'fincommerce_cart_contents_changed', $cart_contents ) );
		}
		 ...
		 */
		do_action( 'fincommerce_cart_loaded_from_session', $this->cart );
	...
	public function maybe_set_cart_cookies() {
		...
		} elseif ( isset( $_COOKIE['fincommerce_items_in_cart'] ) ) { // WPCS: input var ok.
	...
	private function dedupe_cookies() {
		...
		foreach ( $all_cookies as $cookie ) {
			...
			if ( stripos( $cookie_name, 'fincommerce_' ) !== false ) {
	...
	public function set_session() {
		 ...
		 * @since 3.2.0
		 */
		do_action( 'fincommerce_cart_updated' );
	}
	...
	public function persistent_cart_update() {
		 ...
		 */
		if ( get_current_user_id() && apply_filters( 'fincommerce_persistent_cart_enabled', true ) ) {
			update_user_meta(
				get_current_user_id(),
				'_fincommerce_persistent_cart_' . get_current_blog_id(),
				array(
					'cart' => $this->get_cart_for_session(),
				)
			);
	...
	public function persistent_cart_destroy() {
		 ...
		 */
		if ( get_current_user_id() && apply_filters( 'fincommerce_persistent_cart_enabled', true ) ) {
			delete_user_meta( get_current_user_id(), '_fincommerce_persistent_cart_' . get_current_blog_id() );
		}
	}
	...
	private function set_cart_cookies( $set = true ) {
		if ( $set ) {
			$setcookies = array(
				'fincommerce_items_in_cart' => '1',
				'fincommerce_cart_hash'     => WC()->cart->get_cart_hash(),
			);
			...
			$unsetcookies = array(
				'fincommerce_items_in_cart',
				'fincommerce_cart_hash',
			);
		...
		do_action( 'fincommerce_set_cart_cookies', $set );
	}
	...
	private function get_saved_cart() {
		 ...
		 * @param bool $enabled Whether the persistent cart is enabled. Default true.
		 */
		if ( apply_filters( 'fincommerce_persistent_cart_enabled', true ) ) {
			$saved_cart_meta = get_user_meta( get_current_user_id(), '_fincommerce_persistent_cart_' . get_current_blog_id(), true );
	...
	private function populate_cart_from_order( $order_id, $cart ) {
		 ...
		 */
		$valid_statuses = apply_filters( 'fincommerce_valid_order_statuses_for_order_again', array( OrderStatus::COMPLETED ) );
		if ( ! $order->get_id() || ! $order->has_status( $valid_statuses ) || ! current_user_can( 'order_again', $order->get_id() ) ) {
			return;
		}

		if ( apply_filters( 'fincommerce_empty_cart_when_order_again', true ) ) {
			$cart = array();
		}
		...
		foreach ( $order_items as $item ) {
			$product_id     = (int) apply_filters( 'fincommerce_add_to_cart_product_id', $item->get_product_id() );
			$quantity       = $item->get_quantity();
			$variation_id   = (int) $item->get_variation_id();
			$variations     = array();
			$cart_item_data = apply_filters( 'fincommerce_order_again_cart_item_data', array(), $item, $order );
			...
			if ( ! apply_filters( 'fincommerce_add_to_cart_validation', true, $product_id, $quantity, $variation_id, $variations, $cart_item_data ) ) {
				continue;
			}
			...
			$cart[ $cart_id ] = apply_filters(
				'fincommerce_add_order_again_cart_item',
				array_merge(
					$cart_item_data,
					array(
						'key'          => $cart_id,
		...
		do_action_ref_array( 'fincommerce_ordered_again', array( $order->get_id(), $order_items, &$cart ) );
		...
		if ( 0 < $num_items_added ) {
			wc_add_notice( __( 'The cart has been filled with the items from your previous order.', 'fincommerce' ) );
		}
