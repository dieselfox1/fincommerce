...
function wc_rest_allowed_image_mime_types() {
	return apply_filters(
		'fincommerce_rest_allowed_image_mime_types',
		array(
...
function wc_rest_upload_image_from_url( $image_url ) {
	...
	if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
		...
		return new WP_Error( 'fincommerce_rest_invalid_image_url', sprintf( __( 'Invalid URL %s.', 'fincommerce' ), $image_url ), array( 'status' => 400 ) );
	}
	...
	if ( is_wp_error( $file_array['tmp_name'] ) ) {
		return new WP_Error(
			'fincommerce_rest_invalid_remote_image_url',
			/* translators: %s: image URL */
			sprintf( __( 'Error getting remote image %s.', 'fincommerce' ), $image_url ) . ' '
			/* translators: %s: error message */
			. sprintf( __( 'Error: %s', 'fincommerce' ), $file_array['tmp_name']->get_error_message() ),
			array( 'status' => 400 )
		);
	}
	...
	if ( isset( $file['error'] ) ) {
		...
		/* translators: %s: error message */
		return new WP_Error( 'fincommerce_rest_invalid_image', sprintf( __( 'Invalid image: %s', 'fincommerce' ), $file['error'] ), array( 'status' => 400 ) );
	}

	do_action( 'fincommerce_rest_api_uploaded_image_from_url', $file, $image_url );
...
function wc_rest_validate_reports_request_arg( $value, $request, $param ) {
	...
	if ( 'string' === $args['type'] && ! is_string( $value ) ) {
		...
		return new WP_Error( 'fincommerce_rest_invalid_param', sprintf( __( '%1$s is not of type %2$s', 'fincommerce' ), $param, 'string' ) );
	}
	...
	if ( 'date' === $args['format'] ) {
		...
		if ( ! preg_match( $regex, $value, $matches ) ) {
			return new WP_Error( 'fincommerce_rest_invalid_date', __( 'The date you provided is invalid.', 'fincommerce' ) );
		}
	}
...
function wc_rest_check_post_permissions( $post_type, $context = 'read', $object_id = 0 ) {
	...
	return apply_filters( 'fincommerce_rest_check_permissions', $permission, $context, $object_id, $post_type );
}
...
function wc_rest_check_user_permissions( $context = 'read', $object_id = 0 ) {
	 ...
	 */
	return apply_filters( 'fincommerce_rest_check_permissions', $permission, $context, $object_id, 'user' );
}
...
function wc_rest_check_product_term_permissions( $taxonomy, $context = 'read', $object_id = 0 ) {
	...
	return apply_filters( 'fincommerce_rest_check_permissions', $permission, $context, $object_id, $taxonomy );
}
...
function wc_rest_check_manager_permissions( $object, $context = 'read' ) {
	$objects = array(
		'reports'          => 'view_fincommerce_reports',
		'settings'         => 'manage_fincommerce',
		'system_status'    => 'manage_fincommerce',
		'attributes'       => 'manage_product_terms',
		'shipping_methods' => 'manage_fincommerce',
		'payment_gateways' => 'manage_fincommerce',
		'webhooks'         => 'manage_fincommerce',
	);

	$permission = current_user_can( $objects[ $object ] );

	return apply_filters( 'fincommerce_rest_check_permissions', $permission, $context, 0, $object );
}
...
function wc_rest_check_product_reviews_permissions( $context = 'read', $object_id = 0 ) {
	...
	return apply_filters( 'fincommerce_rest_check_permissions', $permission, $context, $object_id, 'product_review' );
}
