...
use Automattic\FinCommerce\Database\Migrations\MigrationHelper;
use Automattic\FinCommerce\Enums\ProductStockStatus;
use Automattic\FinCommerce\Enums\ProductType;
use Automattic\FinCommerce\Internal\Admin\Marketing\MarketingSpecs;
use Automattic\FinCommerce\Internal\Admin\Notes\WooSubscriptionsNotes;
use Automattic\FinCommerce\Internal\AssignDefaultCategory;
use Automattic\FinCommerce\Internal\DataStores\Orders\CustomOrdersTableController;
use Automattic\FinCommerce\Internal\DataStores\Orders\DataSynchronizer;
...
use Automattic\FinCommerce\Internal\ProductAttributesLookup\LookupDataStore;
use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Register as Download_Directories;
...
use Automattic\FinCommerce\Utilities\StringUtil;
use Automattic\FinCommerce\Blocks\Options as BlockOptions;
use Automattic\FinCommerce\Blocks\Utils\BlockTemplateUtils;
	...
	if ( $existing_file_paths ) {
		...
		foreach ( $existing_file_paths as $existing_file_path ) {
			...
			if ( ! empty( $old_file_path ) ) {
				...
				$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}fincommerce_downloadable_product_permissions SET download_id = %s WHERE product_id = %d", md5( $old_file_path ), $existing_file_path->post_id ) );
...
function wc_update_200_permalinks() {
	...
	$permalinks   = get_option( 'fincommerce_permalinks' );
	...
	if ( empty( $permalinks ) && $shop_page_id > 0 ) {
		...
		$category_base = 'yes' === get_option( 'fincommerce_prepend_shop_page_to_urls' ) ? trailingslashit( $base_slug ) : '';
		$category_slug = get_option( 'fincommerce_product_category_slug' ) ? get_option( 'fincommerce_product_category_slug' ) : _x( 'product-category', 'slug', 'fincommerce' );
		$tag_slug      = get_option( 'fincommerce_product_tag_slug' ) ? get_option( 'fincommerce_product_tag_slug' ) : _x( 'product-tag', 'slug', 'fincommerce' );

		if ( 'yes' === get_option( 'fincommerce_prepend_shop_page_to_products' ) ) {
			$product_base = trailingslashit( $base_slug );
		} else {
			$product_slug = get_option( 'fincommerce_product_slug' );
			if ( false !== $product_slug && ! empty( $product_slug ) ) {
				$product_base = trailingslashit( $product_slug );
			} else {
				$product_base = trailingslashit( _x( 'product', 'slug', 'fincommerce' ) );
			}
		}

		if ( 'yes' === get_option( 'fincommerce_prepend_category_to_products' ) ) {
		...
		update_option( 'fincommerce_permalinks', $permalinks );
	}
}
...
function wc_update_200_subcat_display() {
	...
	if ( 'yes' === get_option( 'fincommerce_shop_show_subcategories' ) ) {
		if ( 'yes' === get_option( 'fincommerce_hide_products_when_showing_subcategories' ) ) {
			update_option( 'fincommerce_shop_page_display', 'subcategories' );
		} else {
			update_option( 'fincommerce_shop_page_display', 'both' );
		}
	}

	if ( 'yes' === get_option( 'fincommerce_show_subcategories' ) ) {
		if ( 'yes' === get_option( 'fincommerce_hide_products_when_showing_subcategories' ) ) {
			update_option( 'fincommerce_category_archive_display', 'subcategories' );
		} else {
			update_option( 'fincommerce_category_archive_display', 'both' );
		}
	}
}
...
function wc_update_200_taxrates() {
	...
	// Update tax rates.
	$loop      = 0;
	$tax_rates = get_option( 'fincommerce_tax_rates' );
	...
	$local_tax_rates = get_option( 'fincommerce_local_tax_rates' );
	...
	update_option( 'fincommerce_tax_rates_backup', $tax_rates );
	update_option( 'fincommerce_local_tax_rates_backup', $local_tax_rates );
	delete_option( 'fincommerce_tax_rates' );
	delete_option( 'fincommerce_local_tax_rates' );
}
		...
		foreach ( $order_items as $order_item ) {
			...
			if ( $item_id ) {
				...
				if ( count( $meta_rows ) > 0 ) {
					$wpdb->query(
						$wpdb->prepare(
							"INSERT INTO {$wpdb->prefix}fincommerce_order_itemmeta ( order_item_id, meta_key, meta_value )
...
function wc_update_200_images() {
	...
	foreach ( array( 'catalog', 'single', 'thumbnail' ) as $value ) {
		...
		$old_settings = array_filter(
			array(
				'width'  => get_option( 'fincommerce_' . $value . '_image_width' ),
				'height' => get_option( 'fincommerce_' . $value . '_image_height' ),
				'crop'   => get_option( 'fincommerce_' . $value . '_image_crop' ),
			)
		);

		if ( ! empty( $old_settings ) && update_option( 'shop_' . $value . '_image_size', $old_settings ) ) {

			delete_option( 'fincommerce_' . $value . '_image_width' );
			delete_option( 'fincommerce_' . $value . '_image_height' );
			delete_option( 'fincommerce_' . $value . '_image_crop' );
...
function wc_update_210_remove_pages() {
	...
	wp_trash_post( get_option( 'fincommerce_pay_page_id' ) );
	wp_trash_post( get_option( 'fincommerce_thanks_page_id' ) );
	wp_trash_post( get_option( 'fincommerce_view_order_page_id' ) );
	wp_trash_post( get_option( 'fincommerce_change_password_page_id' ) );
	wp_trash_post( get_option( 'fincommerce_edit_address_page_id' ) );
	wp_trash_post( get_option( 'fincommerce_lost_password_page_id' ) );
}
...
function wc_update_220_shipping() {
	$fincommerce_ship_to_destination = 'shipping';

	if ( get_option( 'fincommerce_ship_to_billing_address_only' ) === 'yes' ) {
		$fincommerce_ship_to_destination = 'billing_only';
	} elseif ( get_option( 'fincommerce_ship_to_billing' ) === 'yes' ) {
		$fincommerce_ship_to_destination = 'billing';
	}

	add_option( 'fincommerce_ship_to_destination', $fincommerce_ship_to_destination, '', 'no' );
}

/**
 * Update order statuses for 2.2
 *
 * Keeping the internal statuses names as strings to avoid regression issues (not referencing Automattic\FinCommerce\Enums\OrderInternalStatus class).
 *
 * @return void
 */
...
function wc_update_220_attributes() {
	...
	$attribute_taxonomies = $wpdb->get_results( 'SELECT attribute_name, attribute_id FROM ' . $wpdb->prefix . 'fincommerce_attribute_taxonomies' );
	...
	foreach ( $attribute_taxonomies as $attribute_taxonomy ) {
		...
		if ( $sanitized_attribute_name !== $attribute_taxonomy->attribute_name ) {
			if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT 1=1 FROM {$wpdb->prefix}fincommerce_attribute_taxonomies WHERE attribute_name = %s;", $sanitized_attribute_name ) ) ) {
				// Update attribute.
				$wpdb->update(
	...
	WC_Cache_Helper::invalidate_cache_group( 'fincommerce-attributes' );
}
...
function wc_update_230_options() {
	...
	// To prevent taxes being hidden when using a default 'no address' in a store with tax inc prices, set the fincommerce_default_customer_address to use the store base address by default.
	if ( '' === get_option( 'fincommerce_default_customer_address', false ) && wc_prices_include_tax() ) {
		update_option( 'fincommerce_default_customer_address', 'base' );
	}
}
...
function wc_update_240_options() {
	 ...
	 */
	update_option( 'fincommerce_calc_discounts_sequentially', 'yes' );
}
...
function wc_update_240_shipping_methods() {
	...
	$shipping_methods = array(
		'fincommerce_flat_rates'                        => new WC_Shipping_Legacy_Flat_Rate(),
		'fincommerce_international_delivery_flat_rates' => new WC_Shipping_Legacy_International_Delivery(),
	);
...
function wc_update_240_api_keys() {
	 ...
	 */
	$api_users = $wpdb->get_results( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = 'fincommerce_api_consumer_key'" );
	$apps_keys = array();
	...
	foreach ( $api_users as $_user ) {
		...
		$apps_keys[] = array(
			'user_id'         => $user->ID,
			'permissions'     => $user->fincommerce_api_key_permissions,
			'consumer_key'    => wc_api_hash( $user->fincommerce_api_consumer_key ),
			'consumer_secret' => $user->fincommerce_api_consumer_secret,
			'truncated_key'   => substr( $user->fincommerce_api_consumer_secret, -7 ),
		);
	}
	...
	if ( ! empty( $apps_keys ) ) {
		...
		foreach ( $api_users as $_user ) {
			...
			delete_user_meta( $user_id, 'fincommerce_api_consumer_key' );
			delete_user_meta( $user_id, 'fincommerce_api_consumer_secret' );
			delete_user_meta( $user_id, 'fincommerce_api_key_permissions' );
		}
	}
}
...
function wc_update_240_webhooks() {
	...
	/**
	 * Webhooks.
	 * Make sure order.update webhooks get the fincommerce_order_edit_status hook.
	 */
...
function wc_update_240_refunds() {
	...
	remove_all_actions( 'fincommerce_order_status_refunded_notification' );
	remove_all_actions( 'fincommerce_order_partially_refunded_notification' );
	remove_action( 'fincommerce_order_status_refunded', array( 'WC_Emails', 'send_transactional_email' ) );
	remove_action( 'fincommerce_order_partially_refunded', array( 'WC_Emails', 'send_transactional_email' ) );
		...
		if ( $order_total > $refunded_total ) {
			wc_create_refund(
				array(
					...
					'reason'     => __( 'Order fully refunded', 'fincommerce' ),
...
function wc_update_241_variations() {
	...
	// Select variations that don't have any _stock_status implemented on FinCommerce 2.2.
	$update_variations = $wpdb->get_results(
...
function wc_update_250_currency() {
	...
	$current_currency = get_option( 'fincommerce_currency' );
...
function wc_update_260_options() {
	// fincommerce_calc_shipping option has been removed in 2.6.
	if ( 'no' === get_option( 'fincommerce_calc_shipping' ) ) {
		update_option( 'fincommerce_ship_to_countries', 'disabled' );
	}
...
function wc_update_260_termmeta() {
	...
	if ( get_option( 'db_version' ) >= 34370 && $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}fincommerce_termmeta';" ) ) {
		if ( $wpdb->query( "INSERT INTO {$wpdb->termmeta} ( term_id, meta_key, meta_value ) SELECT fincommerce_term_id, meta_key, meta_value FROM {$wpdb->prefix}fincommerce_termmeta;" ) ) {
			$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}fincommerce_termmeta" );
			wp_cache_flush();
		}
	}
}
...
function wc_update_260_zones() {
	 ...
	 */
	if ( $wpdb->get_var( "SHOW COLUMNS FROM `{$wpdb->prefix}fincommerce_shipping_zones` LIKE 'zone_enabled';" ) ) {
		$wpdb->query( "ALTER TABLE {$wpdb->prefix}fincommerce_shipping_zones CHANGE `zone_type` `zone_type` VARCHAR(40) NOT NULL DEFAULT '';" );
		$wpdb->query( "ALTER TABLE {$wpdb->prefix}fincommerce_shipping_zones CHANGE `zone_enabled` `zone_enabled` INT(1) NOT NULL DEFAULT 1;" );
	}
}
...
function wc_update_260_zone_methods() {
	...
	/**
	 * Shipping zones in WC 2.6.0 use a table named fincommerce_shipping_zone_methods.
	 * Migrate the old data out of fincommerce_shipping_zone_shipping_methods into the new table and port over any known options (used by table rates and flat rate boxes).
	 */
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}fincommerce_shipping_zone_shipping_methods';" ) ) {
		$old_methods = $wpdb->get_results( "SELECT zone_id, shipping_method_type, shipping_method_order, shipping_method_id FROM {$wpdb->prefix}fincommerce_shipping_zone_shipping_methods;" );

		if ( $old_methods ) {
			$max_new_id = $wpdb->get_var( "SELECT MAX(instance_id) FROM {$wpdb->prefix}fincommerce_shipping_zone_methods" );
			$max_old_id = $wpdb->get_var( "SELECT MAX(shipping_method_id) FROM {$wpdb->prefix}fincommerce_shipping_zone_shipping_methods" );

			// Avoid ID conflicts.
			$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->prefix}fincommerce_shipping_zone_methods AUTO_INCREMENT = %d;", max( $max_new_id, $max_old_id ) + 1 ) );
			...
			foreach ( $old_methods as $old_method ) {
				...
				// Move main settings.
				$older_settings_key = 'fincommerce_' . $old_method->shipping_method_type . '-' . $old_method->shipping_method_id . '_settings';
				$old_settings_key   = 'fincommerce_' . $old_method->shipping_method_type . '_' . $old_method->shipping_method_id . '_settings';
				add_option( 'fincommerce_' . $old_method->shipping_method_type . '_' . $new_instance_id . '_settings', get_option( $old_settings_key, get_option( $older_settings_key ) ) );

				// Handling for table rate and flat rate box shipping.
				if ( 'table_rate' === $old_method->shipping_method_type ) {
					// Move priority settings.
					add_option( 'fincommerce_table_rate_default_priority_' . $new_instance_id, get_option( 'fincommerce_table_rate_default_priority_' . $old_method->shipping_method_id ) );
					add_option( 'fincommerce_table_rate_priorities_' . $new_instance_id, get_option( 'fincommerce_table_rate_priorities_' . $old_method->shipping_method_id ) );

					// Move rates.
					$wpdb->update(
						$wpdb->prefix . 'fincommerce_shipping_table_rates',
						array(
					...
					$wpdb->update(
						$wpdb->prefix . 'fincommerce_shipping_flat_rate_boxes',
						array(
							'shipping_method_id' => $new_instance_id,
						),
						array(
			...
			update_option( 'fincommerce_updated_instance_ids', $changes );
			do_action( 'fincommerce_updated_instance_ids', $changes );
		}
	}

	// Change ranges used to ...
	$wpdb->query( "UPDATE {$wpdb->prefix}fincommerce_shipping_zone_locations SET location_code = REPLACE( location_code, '-', '...' );" );
}
...
function wc_update_260_refunds() {
	 ...
	 */
	$wpdb->query(
		"UPDATE {$wpdb->prefix}fincommerce_order_itemmeta as item_meta
		LEFT JOIN {$wpdb->prefix}fincommerce_order_items as items ON item_meta.order_item_id = items.order_item_id
...
function wc_update_300_webhooks() {
	 ...
	 * and fincommerce_product_bulk_edit_save hooks.
	 */
...
function wc_update_300_settings() {
	$fincommerce_shipping_tax_class = get_option( 'fincommerce_shipping_tax_class' );
	if ( '' === $fincommerce_shipping_tax_class ) {
		update_option( 'fincommerce_shipping_tax_class', 'inherit' );
	} elseif ( 'standard' === $fincommerce_shipping_tax_class ) {
		update_option( 'fincommerce_shipping_tax_class', '' );
	}
}
...
function wc_update_310_downloadable_products() {
	...
	$index_exists = $wpdb->get_row( "SHOW INDEX FROM {$wpdb->prefix}fincommerce_downloadable_product_permissions WHERE column_name = 'order_id' and key_name = 'order_id'" );

	if ( is_null( $index_exists ) ) {
		$wpdb->query( "ALTER TABLE {$wpdb->prefix}fincommerce_downloadable_product_permissions ADD INDEX order_id (order_id)" );
	}
}
...
function wc_update_330_image_options() {
	...
	if ( ! empty( $old_thumbnail_size['width'] ) ) {
		...
		update_option( 'fincommerce_thumbnail_image_width', $width );

		// Calculate cropping mode from old image options.
		if ( ! $hard_crop ) {
			update_option( 'fincommerce_thumbnail_cropping', 'uncropped' );
		} elseif ( $width === $height ) {
			update_option( 'fincommerce_thumbnail_cropping', '1:1' );
		} else {
			$ratio    = $width / $height;
			$fraction = wc_decimal_to_fraction( $ratio );

			if ( $fraction ) {
				update_option( 'fincommerce_thumbnail_cropping', 'custom' );
				update_option( 'fincommerce_thumbnail_cropping_custom_width', $fraction[0] );
				update_option( 'fincommerce_thumbnail_cropping_custom_height', $fraction[1] );
			}
		}
	}
	...
	if ( ! empty( $old_single_size['width'] ) ) {
		update_option( 'fincommerce_single_image_width', absint( $old_single_size['width'] ) );
	}
}
...
function wc_update_330_product_stock_status() {
	...
	if ( 'yes' !== get_option( 'fincommerce_manage_stock' ) ) {
		return;
	}

	$min_stock_amount = (int) get_option( 'fincommerce_notify_no_stock_amount', 0 );
...
function wc_update_330_set_paypal_sandbox_credentials() {
	...
	$paypal_settings = get_option( 'fincommerce_paypal_settings' );
	...
	if ( isset( $paypal_settings['testmode'] ) && 'yes' === $paypal_settings['testmode'] ) {
		...
		update_option( 'fincommerce_paypal_settings', $paypal_settings );
	}
}
...
function wc_update_340_states() {
	...
	update_option( 'fincommerce_update_340_states', $country_states );
}
...
function wc_update_340_state() {
	...
	$country_states = array_filter( (array) get_option( 'fincommerce_update_340_states', array() ) );
	...
	if ( ! empty( $country_states ) ) {
		return update_option( 'fincommerce_update_340_states', $country_states );
	}

	delete_option( 'fincommerce_update_340_states' );
...
function wc_update_360_downloadable_product_permissions_index() {
	...
	$index_exists = $wpdb->get_row( "SHOW INDEX FROM {$wpdb->prefix}fincommerce_downloadable_product_permissions WHERE key_name = 'user_order_remaining_expires'" );

	if ( is_null( $index_exists ) ) {
		$wpdb->query( "ALTER TABLE {$wpdb->prefix}fincommerce_downloadable_product_permissions ADD INDEX user_order_remaining_expires (user_id,order_id,downloads_remaining,access_expires)" );
	}
}
...
function wc_update_370_tax_rate_classes() {
	...
	$classes = array_map( 'trim', explode( "\n", get_option( 'fincommerce_tax_classes' ) ) );
	...
	delete_option( 'fincommerce_tax_classes' );
}
...
function wc_update_370_mro_std_currency() {
	...
	if ( 'MRO' === $current_currency ) {
		update_option( 'fincommerce_currency', 'MRU' );
	}

	if ( 'STD' === $current_currency ) {
		update_option( 'fincommerce_currency', 'STN' );
	}
...
function wc_update_390_move_maxmind_database() {
	...
	$old_path = apply_filters( 'fincommerce_geolocation_local_database_path', WP_CONTENT_DIR . '/uploads/GeoLite2-Country.mmdb', 2 );
	...
	update_option( 'fincommerce_maxmind_geolocation_settings', array( 'database_prefix' => $prefix ) );

	// Generate the new path in the same way that the integration will.
	$uploads_dir = wp_upload_dir();
	$new_path    = trailingslashit( $uploads_dir['basedir'] ) . 'fincommerce_uploads/' . $prefix . '-GeoLite2-Country.mmdb';
...
function wc_update_390_change_geolocation_database_update_cron() {
	...
	wp_schedule_event( time() + ( DAY_IN_SECONDS * 15 ), 'fifteendays', 'fincommerce_geoip_updater' );
}
 ...
 * variable products by attribute (https://github.com/dieselfox1/fincommerce/pull/26260),
 * however that mechanism was later reverted (https://github.com/dieselfox1/fincommerce/pull/27625)
...
function wc_update_450_sanitize_coupons_code() {
	...
	$coupon_id      = 0;
	$last_coupon_id = get_option( 'fincommerce_update_450_last_coupon_id', '0' );
	...
	if ( empty( $coupons ) ) {
		delete_option( 'fincommerce_update_450_last_coupon_id' );
		return false;
	}
	...
	// Start the run again.
	if ( $coupon_id ) {
		return update_option( 'fincommerce_update_450_last_coupon_id', $coupon_id );
	}
 ...
 *
 * See @link https://github.com/dieselfox1/fincommerce/issues/27688.
 */
function wc_update_500_fix_product_review_count() {
	global $wpdb;

	$product_id      = 0;
	$last_product_id = get_option( 'fincommerce_update_500_last_product_id', '0' );
	...
	if ( empty( $products_data ) ) {
		delete_option( 'fincommerce_update_500_last_product_id' );
		return false;
	}
	...
	// Start the run again.
	if ( $product_id ) {
		return update_option( 'fincommerce_update_500_last_product_id', $product_id );
	}
 ...
 *
 * See @link https://github.com/dieselfox1/fincommerce/issues/29235.
 */
function wc_update_560_create_refund_returns_page() {
	...
	add_filter( 'fincommerce_create_pages', 'filter_created_pages' );

	WC_Install::create_pages();

	remove_filter( 'fincommerce_create_pages', 'filter_created_pages' );
}
 ...
 *
 * See @link https://github.com/dieselfox1/fincommerce/issues/27103.
 */
function wc_update_600_migrate_rate_limit_options() {
	...
	$rate_limits   = $wpdb->get_results(
		"
			...
			FROM $wpdb->options
			WHERE option_name LIKE 'fincommerce_rate_limit_add_payment_method_%'
		",
		ARRAY_A
	);
	$prefix_length = strlen( 'fincommerce_rate_limit_' );
...
/**
 * Add the standard FinCommerce upload directories to the Approved Product Download Directories list
...
function wc_update_750_disable_new_product_management_experience() {
	if ( 'yes' === get_option( 'fincommerce_new_product_management_enabled' ) ) {
		update_option( 'fincommerce_new_product_management_enabled', 'no' );
	}
}
...
function wc_update_770_remove_multichannel_marketing_feature_options() {
	delete_option( 'fincommerce_multichannel_marketing_enabled' );
	delete_option( 'fincommerce_marketing_overview_welcome_hidden' );
}
 ...
 * It is replaced by `fincommerce_admin_marketing_recommendations_specs` transient that is created by `MarketingRecommendationsDataSourcePoller`.
 */
...
function wc_update_870_prevent_listing_of_transient_files_directory() {
	...
	$default_transient_files_dir = untrailingslashit( wp_upload_dir()['basedir'] ) . '/fincommerce_transient_files';
 ...
 */
function wc_update_890_update_connect_to_fincommerce_note() {
	$note = Notes::get_note_by_name( WooSubscriptionsNotes::CONNECTION_NOTE_NAME );
	if ( ! is_a( $note, 'Automattic\FinCommerce\Admin\Notes\Note' ) ) {
		return;
	}
 ...
 *
 * PayPal Standard has been deprecated since FinCommerce 5.5, but there are some stores that have it showing up in their
 * list of available Payment methods even if it's not setup. In FinCommerce 8.9 we will disable PayPal Standard for those stores
...
/**
 * Create the fincommerce_history_of_autoinstalled_plugins option if it doesn't exist
 * as a copy of fincommerce_autoinstalled_plugins if it exists.
 */
function wc_update_891_create_plugin_autoinstall_history_option() {
	$autoinstalled_plugins_history_info = get_site_option( 'fincommerce_history_of_autoinstalled_plugins' );
	if ( false === $autoinstalled_plugins_history_info ) {
		$autoinstalled_plugins_info = get_site_option( 'fincommerce_autoinstalled_plugins' );
		if ( false !== $autoinstalled_plugins_info ) {
			update_site_option( 'fincommerce_history_of_autoinstalled_plugins', $autoinstalled_plugins_info );
		}
	}
}

/**
 * Add fincommerce_show_lys_tour.
 */
function wc_update_910_add_launch_your_store_tour_option() {
	add_option( 'fincommerce_show_lys_tour', 'yes' );
}

/**
 * Add fincommerce_hooked_blocks_version option for existing stores that are using a theme that supports the Block Hooks API
 */
function wc_update_920_add_wc_hooked_blocks_version_option() {
	 ...
	 * @since 8.4.0
	 */
	$theme_include_list               = apply_filters( 'fincommerce_hooked_blocks_theme_include_list', array( 'Twenty Twenty-Four', 'Twenty Twenty-Three', 'Twenty Twenty-Two', 'Tsubaki', 'Zaino', 'Thriving Artist', 'Amulet', 'Tazza' ) );
...
function wc_update_910_remove_obsolete_user_meta() {
	...
	if ( false === $deletions ) {
		$logger->notice(
			'During the update to 9.1.0, FinCommerce attempted to remove user meta with the keys "_last_order", "_order_count" and "_money_spent" but was unable to do so.',
			array(
				'source' => 'wc-updater',
			)
		);
		...
		$logger->info(
			sprintf(
				1 === $deletions
					? 'During the update to 9.1.0, FinCommerce removed %d user meta row associated with the meta keys "_last_order", "_order_count" or "_money_spent".'
					: 'During the update to 9.1.0, FinCommerce removed %d user meta rows associated with the meta keys "_last_order", "_order_count" or "_money_spent".',
...
/**
 * Add fincommerce_coming_soon option when it is not currently present.
 */
function wc_update_930_add_fincommerce_coming_soon_option() {
	add_option( 'fincommerce_coming_soon', 'no' );
}

/**
 * Migrate Launch Your Store tour meta keys to the fincommerce_meta user data fields.
 */
function wc_update_930_migrate_user_meta_for_launch_your_store_tour() {
	// Rename `fincommerce_launch_your_store_tour_hidden` meta key to `fincommerce_admin_launch_your_store_tour_hidden`.
	global $wpdb;
	$wpdb->query(
		$wpdb->prepare(
			...
			SET meta_key = %s
			WHERE meta_key = %s",
			'fincommerce_admin_launch_your_store_tour_hidden',
			'fincommerce_launch_your_store_tour_hidden'
		)
	);

	// Rename `fincommerce_coming_soon_banner_dismissed` meta key to `fincommerce_admin_coming_soon_banner_dismissed`.
	$wpdb->query(
		$wpdb->prepare(
			"UPDATE {$wpdb->usermeta}
			SET meta_key = %s
			WHERE meta_key = %s",
			'fincommerce_admin_coming_soon_banner_dismissed',
			'fincommerce_coming_soon_banner_dismissed'
		)
	);
}
...
/**
 * Remove user meta associated with the key 'fincommerce_admin_help_panel_highlight_shown'.
 *
...
function wc_update_940_remove_help_panel_highlight_shown() {
	...
	$meta_key = 'fincommerce_admin_help_panel_highlight_shown';
	...
	// Get the FinCommerce logger to track the results of the deletion.
	$logger = wc_get_logger();
	...
	if ( false === $deletions ) {
		$logger->notice(
			'During the update to 9.4.0, FinCommerce attempted to remove user meta with the key "fincommerce_admin_help_panel_highlight_shown", but was unable to do so.',
			array(
				'source' => 'wc-updater',
			)
		);
		...
		$logger->info(
			sprintf(
				1 === $deletions
					? 'During the update to 9.4.0, FinCommerce removed %d user meta row associated with the meta key "fincommerce_admin_help_panel_highlight_shown".'
					: 'During the update to 9.4.0, FinCommerce removed %d user meta rows associated with the meta key "fincommerce_admin_help_panel_highlight_shown".',
				number_format_i18n( $deletions )
			),
			array(
				'source' => 'wc-updater',
 ...
 *
 * If FinCommerce is updated from an earlier version to 10.0.0, and if it is a multisite network,
 * then set 'fincommerce_network_wide_customers' to 'yes' (but only if it has not already been
 * set).
 *
 * This preserves FinCommerce's historic handling of cross-network user visibility for existing
...
function wc_update_1000_multisite_visibility_setting(): void {
	...
	$existing_site_option = get_site_option( 'fincommerce_network_wide_customers', '' );

	if ( is_string( $existing_site_option ) && strlen( $existing_site_option ) > 0 ) {
		return;
	}

	update_site_option( 'fincommerce_network_wide_customers', 'yes' );
}

/**
 * Autoloads fincommerce_allow_tracking option.
 */
function wc_update_950_tracking_option_autoload() {
	$options = array(
		'fincommerce_allow_tracking' => 'yes',
	);
	wp_set_option_autoload_values( $options );
}

/**
 * Update the base color for emails as part of the FinCommerce rebranding,
 * but only if the user hasn't specified a custom color.
 */
function wc_update_961_migrate_default_email_base_color() {
	$color = get_option( 'fincommerce_email_base_color' );
	if ( '#7f54b3' === $color ) {
		update_option( 'fincommerce_email_base_color', '#720eec' );
	}
}
...
function wc_update_1020_add_old_refunded_order_items_to_product_lookup_table() {
	...
	if ( $refunded_orders ) {
		foreach ( $refunded_orders as $refunded_order ) {
			 ...
			 * @since 10.2.0
			 */
			do_action( 'fincommerce_schedule_import', intval( $refunded_order->order_id ) );
		}
	}
}

/**
 * Remove the option fincommerce_order_attribution_install_banner_dismissed.
...
function wc_update_980_remove_order_attribution_install_banner_dismissed_option() {
	delete_option( 'fincommerce_order_attribution_install_banner_dismissed' );
}
...
function wc_update_985_enable_new_payments_settings_page_feature() {
	update_option( 'fincommerce_feature_reactify-classic-payments-settings_enabled', 'yes' );
}
