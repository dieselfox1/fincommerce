<?php
/**
 * The FinCommerce customer class handles storage of the current customer's data, such as location.
 *
...
class WC_Customer extends WC_Legacy_Customer {
	...
	public function get_taxable_address() {
		...
		if ( true === apply_filters( 'fincommerce_apply_base_tax_for_local_pickup', true ) && count( array_intersect( wc_get_chosen_shipping_method_ids(), apply_filters( 'fincommerce_local_pickup_methods', array( 'legacy_local_pickup', 'local_pickup' ) ) ) ) > 0 ) {
			$tax_based_on = 'base';
		}
		 ...
		 */
		return apply_filters( 'fincommerce_customer_taxable_address', array( $country, $state, $postcode, $city ), $this );
	}
	...
	public function get_downloadable_products() {
		...
		return apply_filters( 'fincommerce_customer_get_downloadable_products', $downloads );
	}
	...
	*/
	...
	protected function get_address_prop( $prop, $address_type = 'billing', $context = 'view' ) {
		...
		if ( array_key_exists( $prop, $this->data[ $address_type ] ) ) {
			...
			if ( 'view' === $context ) {
				/**
				 * Filter: 'fincommerce_customer_get_[billing|shipping]_[prop]'
				 *
	...
	*/
	...
	public function set_email( $value ) {
		if ( $value && ! is_email( (string) $value ) ) {
			$this->error( 'customer_invalid_email', __( 'Invalid email address', 'fincommerce' ) );
		}
	...
	public function set_display_name( $display_name ) {
		...
		$this->set_prop( 'display_name', is_email( $display_name ) ? sprintf( _x( '%1$s %2$s', 'display name', 'fincommerce' ), $this->get_first_name(), $this->get_last_name() ) : $display_name );
	}
	...
	public function set_role( $role ) {
		...
		if ( $role && ! empty( $wp_roles->roles ) && ! in_array( $role, array_keys( $wp_roles->roles ), true ) ) {
			$this->error( 'customer_invalid_role', __( 'Invalid role', 'fincommerce' ) );
		}
	...
	public function set_billing_email( $value ) {
		if ( $value && ! is_email( (string) $value ) ) {
			$this->error( 'customer_invalid_billing_email', __( 'Invalid billing email address', 'fincommerce' ) );
		}
