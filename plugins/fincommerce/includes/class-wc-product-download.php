...
class WC_Product_Download implements ArrayAccess {
	...
	public function get_allowed_mime_types() {
		return apply_filters( 'fincommerce_downloadable_file_allowed_mime_types', get_allowed_mime_types() );
	}
		...
		if ( ! $this->data['enabled'] ) {
			throw new Exception(
				sprintf(
					...
					__( 'The downloadable file %s cannot be used as it has been disabled.', 'fincommerce' ),
		...
		if ( ! $this->is_allowed_filetype() ) {
			throw new Exception(
				sprintf(
					...
					__( 'The downloadable file %1$s cannot be used as it does not have an allowed file type. Allowed types include: %2$s', 'fincommerce' ),
					'<code>' . basename( $download_file ) . '</code>',
	...
	public function file_exists() {
		...
		return apply_filters( 'fincommerce_downloadable_file_exists', file_exists( $file_url ), $this->get_file() );
	}
	...
	private function approved_directory_checks( bool $auto_add_to_approved_directory_list = true ) {
		 ...
		 */
		if ( apply_filters( 'fincommerce_product_downloads_approved_directory_validation_for_shortcodes', true ) && 'shortcode' === $this->get_type_of_file_path() ) {
	...
	private function raise_invalid_file_exception( string $download_file ): void {
		throw new Exception(
			sprintf(
				...
				__( 'The downloadable file %s cannot be used as it does not exist on the server, or is not located within an approved directory. Please contact a site administrator for help. %2$sLearn more.%3$s', 'fincommerce' ),
				'<code>' . $download_file . '</code>',
				'<a href="https://fincommerce.com/document/approved-download-directories">',
				'</a>'
			)
		);
	}
