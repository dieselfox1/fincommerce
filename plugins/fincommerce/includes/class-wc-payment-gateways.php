...
use Automattic\FinCommerce\Internal\Admin\Settings\Payments as SettingsPaymentsService;
...
class WC_Payment_Gateways {
	...
	public function init() {
		...
		// Filter.
		$load_gateways = apply_filters( 'fincommerce_payment_gateways', $load_gateways );

		// Get sort order option.
		$ordering  = (array) get_option( 'fincommerce_gateway_order' );
		$order_end = 999;
...
%7$s',
		...
		return wp_mail(
			...
			sprintf(
				...
				__( '[%1$s] Payment gateway "%2$s" enabled', 'fincommerce' ),
				$site_title,
				$gateway_title
			),
			$email_text
	...
	public function get_available_payment_gateways() {
		...
		return array_filter( (array) apply_filters( 'fincommerce_available_payment_gateways', $_available_gateways ), array( $this, 'filter_valid_gateway_class' ) );
	}
