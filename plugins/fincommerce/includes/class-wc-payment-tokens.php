...
class WC_Payment_Tokens {
	...
	public static function get_customer_tokens( $customer_id, $gateway_id = '' ) {
		...
		$tokens = self::get_tokens(
			array(
				 ...
				 */
				'limit'      => apply_filters( 'fincommerce_get_customer_payment_tokens_limit', get_option( 'posts_per_page' ) ),
			)
		);

		return apply_filters( 'fincommerce_get_customer_payment_tokens', $tokens, $customer_id, $gateway_id );
	}
	...
	public static function get_order_tokens( $order_id ) {
		...
		return apply_filters( 'fincommerce_get_order_payment_tokens', $tokens, $order_id );
	}
	...
	public static function set_users_default( $user_id, $token_id ) {
		...
		foreach ( $users_tokens as $token ) {
			if ( $token_id === $token->get_id() ) {
				...
				do_action( 'fincommerce_payment_token_set_default', $token_id, $token );
			} else {
	...
	protected static function get_token_classname( $type ) {
		 ...
		 * @param string $type Token type.
		 */
		return apply_filters( 'fincommerce_payment_token_class', 'WC_Payment_Token_' . $type, $type );
	}
}
