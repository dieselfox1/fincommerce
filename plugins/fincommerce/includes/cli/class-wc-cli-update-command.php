...
class WC_CLI_Update_Command {
	...
	/**
	 * Runs all pending FinCommerce database updates.
	 */
	public static function update() {
		...
		if ( empty( $callbacks_to_run ) ) {
			...
			WC()->call_static(
				...
				sprintf( __( 'No updates required. Database version is %s', 'fincommerce' ), get_option( 'fincommerce_db_version' ) )
			);
			return;
		}
		...
		WC()->call_static(
			...
			sprintf( __( 'Found %1$d updates (%2$s)', 'fincommerce' ), count( $callbacks_to_run ), implode( ', ', $callbacks_to_run ) )
		);

		$progress = WC()->call_function(
			'WP_CLI\Utils\make_progress_bar',
			__( 'Updating database', 'fincommerce' ),
		...
		WC()->call_static(
			...
			sprintf( __( '%1$d update functions completed. Database version is %2$s', 'fincommerce' ), absint( $update_count ), get_option( 'fincommerce_db_version' ) )
		);
	}
}
