...
class WC_CLI_COM_Command {
	const APPLICATION_PASSWORD_SECTION_URL = 'https://fincommerce.com/my-account/#application-passwords';

	/**
	 * Registers a commands for managing FinCommerce.com extensions.
	 */
	...
	public static function disconnect( array $args, array $assoc_args ) {
		if ( ! WC_Helper::is_site_connected() ) {
			WP_CLI::error( __( 'Your store is not connected to FinCommerce.com. Run `wp wc com connect` command.', 'fincommerce' ) );
		}

		WP_CLI::confirm( __( 'Are you sure you want to disconnect your store from FinCommerce.com?', 'fincommerce' ), $assoc_args );
		WC_Helper::disconnect();
		WP_CLI::success( __( 'You have successfully disconnected your store from FinCommerce.com', 'fincommerce' ) );
	}

	/**
	 * Connects to FinCommerce.com with application-password.
	 *
	 * [--password]
	...
	public static function connect( array $args, array $assoc_args ) {
		...
		if ( WC_Helper::is_site_connected() ) {
			if ( $force ) {
				...
				WP_CLI::error( __( 'Your store is already connected.', 'fincommerce' ) );
		...
		if ( empty( $password ) ) {
			// translators: %s is the URL for the application-password section in FinCommerce.com.
			WP_CLI::log( sprintf( __( 'If you don\'t have an application password (not your account password), generate a password from %s', 'fincommerce' ), esc_url( self::APPLICATION_PASSWORD_SECTION_URL ) ) );
			$password = self::ask( __( 'Connection password:', 'fincommerce' ) );
		}
		$password = sanitize_text_field( $password );
		if ( empty( $password ) ) {
			// translators: %s is the URL for the application-password section in FinCommerce.com.
			WP_CLI::error( sprintf( __( 'Invalid password. Generate a new one from %s.', 'fincommerce' ), esc_url( self::APPLICATION_PASSWORD_SECTION_URL ) ) );
		}
		...
		if ( WC_Helper::is_site_connected() ) {
			WP_CLI::success( __( 'Store connected successfully.', 'fincommerce' ) );
		}
	}
