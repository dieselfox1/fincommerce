...
class WC_CLI_Runner {
	...
	public static function after_wp_load() {
		...
		foreach ( $response_data['routes'] as $route => $route_data ) {
			...
			if ( empty( $route_data['schema']['title'] ) ) {
				...
				WP_CLI::debug( sprintf( __( 'No schema title found for %s, skipping REST command registration.', 'fincommerce' ), $route ), 'wc' );
				continue;
			}
	...
	private static function register_route_commands( $rest_command, $route, $route_data, $command_args = array() ) {
		...
		$supported_ids = array(
			'product_id'   => __( 'Product ID.', 'fincommerce' ),
			'customer_id'  => __( 'Customer ID.', 'fincommerce' ),
			'order_id'     => __( 'Order ID.', 'fincommerce' ),
			'refund_id'    => __( 'Refund ID.', 'fincommerce' ),
			'attribute_id' => __( 'Attribute ID.', 'fincommerce' ),
			'zone_id'      => __( 'Zone ID.', 'fincommerce' ),
			'instance_id'  => __( 'Instance ID.', 'fincommerce' ),
			'id'           => __( 'The ID for the resource.', 'fincommerce' ),
			'slug'         => __( 'The slug for the resource.', 'fincommerce' ),
		);
		...
		foreach ( $supported_commands as $command => $endpoint_args ) {
			...
			if ( in_array( $command, array( 'list', 'get' ), true ) ) {
				$synopsis[] = array(
					'name'        => 'fields',
					'type'        => 'assoc',
					'description' => __( 'Limit response to specific fields. Defaults to all fields.', 'fincommerce' ),
					'optional'    => true,
				);
				$synopsis[] = array(
					'name'        => 'field',
					'type'        => 'assoc',
					'description' => __( 'Get the value of an individual field.', 'fincommerce' ),
					'optional'    => true,
				);
				$synopsis[] = array(
					'name'        => 'format',
					'type'        => 'assoc',
					'description' => __( 'Render response in a particular format.', 'fincommerce' ),
					'optional'    => true,
					'default'     => 'table',
					'options'     => array(
						'table',
			...
			if ( in_array( $command, array( 'create', 'update', 'delete' ), true ) ) {
				$synopsis[] = array(
					'name'        => 'porcelain',
					'type'        => 'flag',
					'description' => __( 'Output just the id when the operation is successful.', 'fincommerce' ),
					'optional'    => true,
				);
			}
