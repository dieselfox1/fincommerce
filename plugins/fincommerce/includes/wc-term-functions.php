...
function wc_change_get_terms_defaults( $defaults, $taxonomies ) {
	...
	} elseif ( in_array( $taxonomy, apply_filters( 'fincommerce_sortable_taxonomies', array( 'product_cat' ) ), true ) ) {
		$orderby = 'menu_order';
	}
...
function wc_get_product_terms( $product_id, $taxonomy, $args = array() ) {
	...
	return apply_filters( 'fincommerce_get_product_terms', _wc_get_cached_product_terms( $product_id, $taxonomy, $args ), $product_id, $taxonomy, $args );
}
...
/**
 * FinCommerce Dropdown categories.
 *
...
function wc_product_dropdown_categories( $args = array() ) {
	...
	$args = wp_parse_args(
		...
		array(
			...
			'show_option_none'   => __( 'Select a category', 'fincommerce' ),
			'option_none_value'  => '',
			'value_field'        => 'slug',
...
function wc_reorder_terms( $the_term, $next_id, $taxonomy, $index = 0, $terms = null ) {
	...
	foreach ( $terms as $term ) {
		...
		*/
		do_action( 'fincommerce_after_set_term_order', $term, $index, $taxonomy );
 ...
 *
 * The second count is based on FinCommerce-specific characteristics. In addition to the
...
function _wc_term_recount( $terms, $taxonomy, $callback = true, $terms_are_term_taxonomy_ids = true ) {
	 ...
	 * @param bool
	 */
	if ( ! apply_filters( 'fincommerce_product_recount_terms', true ) ) {
		return;
	}
	...
	if (
		'yes' === get_option( 'fincommerce_hide_out_of_stock_items' )
...
add_action( 'fincommerce_product_set_stock_status', 'wc_recount_after_stock_change' );
...
function wc_change_term_counts( $terms, $taxonomies ) {
	 ...
	 */
	$valid_taxonomies   = apply_filters( 'fincommerce_change_term_counts', array( 'product_cat', 'product_tag', 'product_brand' ) );
...
function wc_get_product_visibility_term_ids() {
	if ( ! taxonomy_exists( 'product_visibility' ) ) {
		wc_doing_it_wrong( __FUNCTION__, 'wc_get_product_visibility_term_ids should not be called before taxonomies are registered (fincommerce_after_register_post_type action).', '3.1' );
		return array();
	}
