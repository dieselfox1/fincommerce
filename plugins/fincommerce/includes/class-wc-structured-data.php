...
class WC_Structured_Data {
	...
	public function __construct() {
		...
		add_action( 'fincommerce_before_main_content', array( $this, 'generate_website_data' ), 30 );
		add_action( 'fincommerce_breadcrumb', array( $this, 'generate_breadcrumblist_data' ), 10 );
		add_action( 'fincommerce_single_product_summary', array( $this, 'generate_product_data' ), 60 );
		add_action( 'fincommerce_email_order_details', array( $this, 'generate_order_data' ), 20, 3 );

		// Output structured data.
		add_action( 'fincommerce_email_order_details', array( $this, 'output_email_structured_data' ), 30, 3 );
	...
	public function get_structured_data( $types ) {
		...
		foreach ( $data as $type => $value ) {
			...
			$data[ $type ] = apply_filters( 'fincommerce_structured_data_context', array( '@context' => 'https://schema.org/' ), $data, $type, $value ) + $data[ $type ];
		}
		...
		if ( ! empty( $data ) ) {
			if ( 1 < count( $data ) ) {
				$data = apply_filters( 'fincommerce_structured_data_context', array( '@context' => 'https://schema.org/' ), $data, '', '' ) + array( '@graph' => $data );
			} else {
				$data = $data[0];
			}
		}
	...
	protected function get_data_type_for_page() {
		...
		return array_filter( apply_filters( 'fincommerce_structured_data_type_for_page', $types ) );
	}
	 ...
	 * Hooked into `fincommerce_email_order_details` action hook.
	 */
	...
	*/
	 ...
	 *
	 * Hooked into `fincommerce_single_product_summary` action hook.
	 *
	...
	public function generate_product_data( $product = null ) {
		...
		if ( '' !== $product->get_price() ) {
			...
			if ( $product->is_type( ProductType::VARIABLE ) ) {
				...
				$tax_display_mode = get_option( 'fincommerce_tax_display_shop' );
				...
				if ( $product->is_on_sale() && $min_price !== $min_sale_price ) {
					...
					// See https://github.com/dieselfox1/fincommerce/issues/55043.
					array_unshift(
			...
			$markup['offers'] = array( apply_filters( 'fincommerce_structured_data_product_offer', $markup_offer, $product ) );
		}
		...
		$this->set_data( apply_filters( 'fincommerce_structured_data_product', $markup, $product ) );
	}

	/**
	 * Generates Review structured data.
	 *
	 * Hooked into `fincommerce_review_meta` action hook.
	 *
	...
	public function generate_review_data( $comment ) {
		...
		$this->set_data( apply_filters( 'fincommerce_structured_data_review', $markup, $comment ) );
	}

	/**
	 * Generates BreadcrumbList structured data.
	 *
	 * Hooked into `fincommerce_breadcrumb` action hook.
	 *
	...
	public function generate_breadcrumblist_data( $breadcrumbs ) {
		...
		$this->set_data( apply_filters( 'fincommerce_structured_data_breadcrumblist', $markup, $breadcrumbs ) );
	}

	/**
	 * Generates WebSite structured data.
	 *
	 * Hooked into `fincommerce_before_main_content` action hook.
	 */
	public function generate_website_data() {
		...
		$this->set_data( apply_filters( 'fincommerce_structured_data_website', $markup ) );
	}
	...
	public function generate_order_data( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		foreach ( $order->get_items() as $item ) {
			if ( ! apply_filters( 'fincommerce_order_item_visible', true, $item ) ) {
				continue;
			}
			...
			$markup_offers[] = array(
				...
				'priceSpecification' => array(
					...
					'eligibleQuantity' => array(
						...
						'value' => apply_filters( 'fincommerce_email_order_item_quantity', $item->get_quantity(), $item ),
					),
				),
				'itemOffered'        => array(
					'@type' => 'Product',
					'name'  => wp_kses_post( apply_filters( 'fincommerce_order_item_name', $item->get_name(), $item, $is_visible ) ),
		...
		$this->set_data( apply_filters( 'fincommerce_structured_data_order', $markup, $sent_to_admin, $order ), true );
	}
