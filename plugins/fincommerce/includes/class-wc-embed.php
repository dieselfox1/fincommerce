<?php
/**
 * FinCommerce product embed
 *
 * @version  2.4.11
 * @package  FinCommerce\Classes\Embed
 */
...
/**
 * Embed Class which handles any FinCommerce Products that are embedded on this site or another site.
 */
class WC_Embed {
	...
	public static function the_excerpt( $excerpt ) {
		...
		if ( self::is_embedded_product() ) {
			...
			if ( ! empty( $post->post_excerpt ) ) {
				ob_start();
				fincommerce_template_single_excerpt();
	...
	public static function product_buttons() {
		...
		if ( $_product->is_type( ProductType::SIMPLE ) && $_product->is_purchasable() && $_product->is_in_stock() ) {
			$buttons[] = sprintf( $button, esc_url( add_query_arg( 'add-to-cart', get_the_ID(), wc_get_cart_url() ) ), esc_html__( 'Buy now', 'fincommerce' ) );
		}

		$buttons[] = sprintf( $button, get_the_permalink(), esc_html__( 'Read more', 'fincommerce' ) );
			...
			<div class="wc-embed-rating">
				<?php
					printf(
						...
						esc_html__( 'Rated %s out of 5', 'fincommerce' ),
