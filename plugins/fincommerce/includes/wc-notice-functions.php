...
function wc_notice_count( $notice_type = '' ) {
	if ( ! did_action( 'fincommerce_init' ) ) {
		wc_doing_it_wrong( __FUNCTION__, __( 'This function should not be called before fincommerce_init.', 'fincommerce' ), '2.3' );
		return;
	}
...
function wc_add_notice( $message, $notice_type = 'success', $data = array() ) {
	...
	if ( 'success' === $notice_type ) {
		$message = apply_filters( 'fincommerce_add_message', $message );
	}

	$message = apply_filters( 'fincommerce_add_' . $notice_type, $message );
...
function wc_set_notices( $notices ) {
	if ( ! did_action( 'fincommerce_init' ) ) {
		wc_doing_it_wrong( __FUNCTION__, __( 'This function should not be called before fincommerce_init.', 'fincommerce' ), '2.6' );
		return;
	}
...
function wc_print_notices( $return = false ) {
	...
	$notice_types = apply_filters( 'fincommerce_notice_types', array( 'error', 'success', 'notice' ) );
...
function wc_kses_notice( $message ) {
	 ...
	 */
	return wp_kses( $message, apply_filters( 'fincommerce_kses_notice_allowed_tags', $allowed_tags ) );
}
