...
class WC_Shipping {
	...
	/**
	 * Stores methods loaded into fincommerce.
	 *
	 * @var array|null
	 */
	...
	public function init() {
		do_action( 'fincommerce_shipping_init' );
	}

	/**
	 * Shipping methods register themselves by returning their main class name through the fincommerce_shipping_methods filter.
	 *
	 * @return array
	 */
	public function get_shipping_method_class_names() {
		...
		return apply_filters( 'fincommerce_shipping_methods', $shipping_methods );
	}
	...
	public function load_shipping_methods( $package = array() ) {
		if ( ! empty( $package ) ) {
			$debug_mode             = 'yes' === get_option( 'fincommerce_shipping_debug_mode', 'no' );
			$shipping_zone          = WC_Shipping_Zones::get_zone_matching_package( $package );
			$this->shipping_methods = $shipping_zone->get_shipping_methods( true );

			// translators: %s: shipping zone name.
			$matched_zone_notice = sprintf( __( 'Customer matched zone "%s"', 'fincommerce' ), $shipping_zone->get_zone_name() );

			// Debug output.
			if ( $debug_mode && ! Constants::is_defined( 'fincommerce_CHECKOUT' ) && ! Constants::is_defined( 'WC_DOING_AJAX' ) && ! wc_has_notice( $matched_zone_notice ) ) {
		...
		do_action( 'fincommerce_load_shipping_methods', $package );
	...
	public function get_shipping_classes() {
		...
		return apply_filters( 'fincommerce_get_shipping_classes', $this->shipping_classes );
	}
	...
	public function calculate_shipping( $packages = array() ) {
		 ...
		 * but before FinCommerce does anything with them. A good example of usage is to merge the shipping methods for multiple
		 ...
		 */
		$this->packages = array_filter( (array) apply_filters( 'fincommerce_shipping_packages', $this->packages ) );
	...
	public function calculate_shipping_for_package( $package = array(), $package_key = 0 ) {
		...
		if ( ! is_array( $stored_rates ) || $package_hash !== $stored_rates['package_hash'] || 'yes' === get_option( 'fincommerce_shipping_debug_mode', 'no' ) ) {
			foreach ( $this->load_shipping_methods( $package ) as $shipping_method ) {
				...
				if ( ! $shipping_method->supports( 'shipping-zones' ) || $shipping_method->get_instance_id() ) {
					 ...
					 */
					do_action( 'fincommerce_before_get_rates_for_package', $package, $shipping_method );
					 ...
					 * @param WC_Shipping_Method $shipping_method Shipping method instance.
					 */
					do_action( 'fincommerce_after_get_rates_for_package', $package, $shipping_method );
				}
			}

			// Hide shipping rates when free shipping is available.
			if ( 'yes' === get_option( 'fincommerce_shipping_hide_rates_when_free', 'no' ) ) {
				$free_shipping = array();
				$local_pickup  = array();
			 ...
			 * @param array $package Package of cart items.
			 */
			$package['rates'] = apply_filters( 'fincommerce_package_rates', $package['rates'], $package );
