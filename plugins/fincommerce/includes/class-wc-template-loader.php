...
class WC_Template_Loader {
	...
	/**
	 * Is FinCommerce support defined?
	 *
	 * @var boolean
	 */
	...
	public static function init() {
		self::$theme_support = wc_current_theme_supports_fincommerce_or_fse();
	 ...
	 * overrides in /theme/fincommerce/ by default.
	 *
	...
	public static function template_loader( $template ) {
		...
		if ( $default_file ) {
			/**
			 * Filter hook to choose which files to find before FinCommerce does it's own logic.
			 *
			 * @since 3.0.0
			 * @var array
			 */
	...
	private static function has_block_template( $template_name ) {
		...
		$has_template = WP_Block_Templates_Registry::get_instance()->is_registered( 'fincommerce//' . $template_name );
		 ...
		 */
		return (bool) apply_filters( 'fincommerce_has_block_template', $has_template, $template_name );
	}
	...
	private static function get_template_loader_files( $default_file ) {
		$templates   = apply_filters( 'fincommerce_template_loader_files', array(), $default_file );
		$templates[] = 'fincommerce.php';
	...
	private static function unsupported_theme_product_page_init() {
		...
		add_filter( 'fincommerce_product_tabs', array( __CLASS__, 'unsupported_theme_remove_review_tab' ) );
	...
	private static function unsupported_theme_tax_archive_init() {
		...
		add_filter( 'fincommerce_shortcode_products_query', array( __CLASS__, 'unsupported_archive_layered_nav_compatibility' ) );
		$shortcode = new WC_Shortcode_Products( $shortcode_args );
		remove_filter( 'fincommerce_shortcode_products_query', array( __CLASS__, 'unsupported_archive_layered_nav_compatibility' ) );
	...
	/**
	 * Filter the title and insert FinCommerce content on the shop page.
	 *
	...
	public static function unsupported_theme_title_filter( $title, $id = null ) {
		...
		if ( is_page( self::$shop_page_id ) || ( is_home() && 'page' === get_option( 'show_on_front' ) && absint( get_option( 'page_on_front' ) ) === self::$shop_page_id ) ) {
			...
			if ( $args->page > 1 ) {
				...
				$title_suffix[] = sprintf( esc_html__( 'Page %d', 'fincommerce' ), $args->page );
			}
	...
	/**
	 * Filter the content and insert FinCommerce content on the shop page.
	 *
	 * For non-WC themes, this will setup the main shop page to be shortcode based to improve default appearance.
	 *
	 * @since 3.3.0
