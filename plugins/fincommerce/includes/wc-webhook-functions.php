...
function wc_webhook_execute_queue() {
	...
	foreach ( $wc_queued_webhooks as $data ) {
		...
		if ( apply_filters( 'fincommerce_webhook_deliver_async', true, $data['webhook'], $data['arg'] ) ) {
			...
			$next_scheduled_date = WC()->queue()->get_next( 'fincommerce_deliver_webhook_async', $queue_args, 'fincommerce-webhooks' );
			...
			if ( is_null( $next_scheduled_date ) || $next_scheduled_date->getTimestamp() >= ( 600 + gmdate( 'U' ) ) ) {
				WC()->queue()->add( 'fincommerce_deliver_webhook_async', $queue_args, 'fincommerce-webhooks' );
			}
		} else {
			// Deliver immediately.
...
add_action( 'fincommerce_webhook_process_delivery', 'wc_webhook_process_delivery', 10, 2 );

/**
 * Wrapper function to execute the `fincommerce_deliver_webhook_async` cron.
...
add_action( 'fincommerce_deliver_webhook_async', 'wc_deliver_webhook_async', 10, 2 );

/**
 * Check if the given topic is a valid webhook topic, a topic is valid if:
 *
 * + starts with `action.fincommerce_` or `action.wc_`.
...
function wc_is_webhook_valid_topic( $topic ) {
	$invalid_topics = array(
		'action.fincommerce_login_credentials',
		'action.fincommerce_product_csv_importer_check_import_file_path',
		'action.fincommerce_webhook_should_deliver',
	);
	...
	// Custom topics are prefixed with fincommerce_ or wc_ are valid.
	if ( 0 === strpos( $topic, 'action.fincommerce_' ) || 0 === strpos( $topic, 'action.wc_' ) ) {
		return true;
	}
	...
	$valid_resources = apply_filters( 'fincommerce_valid_webhook_resources', array( 'coupon', 'customer', 'order', 'product' ) );
	$valid_events    = apply_filters( 'fincommerce_valid_webhook_events', array( 'created', 'updated', 'deleted', 'restored' ) );
...
function wc_get_webhook_statuses() {
	return apply_filters(
		'fincommerce_webhook_statuses',
		array(
			'active'   => __( 'Active', 'fincommerce' ),
			'paused'   => __( 'Paused', 'fincommerce' ),
			'disabled' => __( 'Disabled', 'fincommerce' ),
		)
	);
}
