 ...
 * WC_Brands class.
 *
 * Important: For internal use only by the Automattic\FinCommerce\Internal\Brands package.
 *
 * @version 9.5.0
 */
class WC_Brands {
	...
	public function __construct() {
		$this->template_url = apply_filters( 'fincommerce_template_url', 'fincommerce/' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
	...
	public function register_hooks() {
		add_action( 'fincommerce_register_taxonomy', array( __CLASS__, 'init_taxonomy' ) );
		...
		add_action( 'fincommerce_product_meta_end', array( $this, 'show_brand' ) );
		add_filter( 'fincommerce_structured_data_product', array( $this, 'add_structured_data' ), 20 );

		// duplicate product brands.
		add_action( 'fincommerce_product_duplicate_before_save', array( $this, 'duplicate_store_temporary_brands' ), 10, 2 );
		add_action( 'fincommerce_new_product', array( $this, 'duplicate_add_product_brand_terms' ) );
		add_action( 'fincommerce_new_product', array( $this, 'invalidate_wc_layered_nav_counts_cache' ), 10, 0 );
		add_action( 'fincommerce_update_product', array( $this, 'invalidate_wc_layered_nav_counts_cache' ), 10, 0 );
		...
		if ( 'yes' === get_option( 'wc_brands_show_description' ) ) {
			add_action( 'fincommerce_archive_description', array( $this, 'brand_description' ) );
		}

		add_filter( 'fincommerce_product_query_tax_query', array( $this, 'update_product_query_tax_query' ), 10, 1 );

		// REST API.
		add_action( 'rest_api_init', array( $this, 'rest_api_register_routes' ) );
		add_action( 'fincommerce_rest_insert_product', array( $this, 'rest_api_maybe_set_brands' ), 10, 2 );
		add_filter( 'fincommerce_rest_prepare_product', array( $this, 'rest_api_prepare_brands_to_product' ), 10, 2 ); // WC 2.6.x.
		add_filter( 'fincommerce_rest_prepare_product_object', array( $this, 'rest_api_prepare_brands_to_product' ), 10, 2 ); // WC 3.x.
		add_action( 'fincommerce_rest_insert_product', array( $this, 'rest_api_add_brands_to_product' ), 10, 3 ); // WC 2.6.x.
		add_action( 'fincommerce_rest_insert_product_object', array( $this, 'rest_api_add_brands_to_product' ), 10, 3 ); // WC 3.x.
		add_filter( 'fincommerce_rest_product_object_query', array( $this, 'rest_api_filter_products_by_brand' ), 10, 2 );
		...
		add_filter( 'fincommerce_layered_nav_term_html', array( $this, 'fincommerce_brands_update_layered_nav_link' ), 10, 4 );

		// Filter the list of taxonomies overridden for the original term count.
		add_action( 'fincommerce_product_set_stock_status', array( $this, 'recount_after_stock_change' ) );
		add_action( 'fincommerce_update_options_products_inventory', array( $this, 'recount_all_brands' ) );

		// Product Editor compatibility.
		add_action( 'fincommerce_layout_template_after_instantiation', array( $this, 'wc_brands_on_block_template_register' ), 10, 3 );
	...
	public function recount_after_stock_change( $product_id ) {
		if ( 'yes' !== get_option( 'fincommerce_hide_out_of_stock_items' ) || empty( $product_id ) ) {
			return;
		}
	...
	public function post_type_link( $permalink, $post ) {
		...
		$product_brand = _x( 'uncategorized', 'slug', 'fincommerce' );
	...
	public static function init_taxonomy() {
		...
		$category_base = get_option( 'fincommerce_prepend_shop_page_to_urls' ) === 'yes' ? trailingslashit( $base_slug ) : '';

		$slug = $category_base . __( 'brand', 'fincommerce' );
		if ( '' === $category_base ) {
			$slug = get_option( 'fincommerce_brand_permalink', '' );
		}

		// Can't provide transatable string as get_option default.
		if ( '' === $slug ) {
			$slug = __( 'brand', 'fincommerce' );
		}
		...
		register_taxonomy(
			...
			apply_filters(
				...
				array(
					...
					'label'                 => __( 'Brands', 'fincommerce' ),
					'labels'                => array(
						'name'              => __( 'Brands', 'fincommerce' ),
						'singular_name'     => __( 'Brand', 'fincommerce' ),
						'template_name'     => _x( 'Products by Brand', 'Template name', 'fincommerce' ),
						'search_items'      => __( 'Search Brands', 'fincommerce' ),
						'all_items'         => __( 'All Brands', 'fincommerce' ),
						'parent_item'       => __( 'Parent Brand', 'fincommerce' ),
						'parent_item_colon' => __( 'Parent Brand:', 'fincommerce' ),
						'edit_item'         => __( 'Edit Brand', 'fincommerce' ),
						'update_item'       => __( 'Update Brand', 'fincommerce' ),
						'add_new_item'      => __( 'Add New Brand', 'fincommerce' ),
						'new_item_name'     => __( 'New Brand Name', 'fincommerce' ),
						'not_found'         => __( 'No Brands Found', 'fincommerce' ),
						'no_terms'          => __( 'No brands', 'fincommerce' ),
						'back_to_items'     => __( '&larr; Go to Brands', 'fincommerce' ),
					),
	 ...
	 *
	 * Templates are in the 'templates' folder. fincommerce looks for theme
	 * overides in /theme/fincommerce/ by default
	 *
	 * For beginners, it also looks for a fincommerce.php template first. If the user adds
	 * this to the theme (containing a fincommerce() inside) this will be used for all
	 * fincommerce templates.
	 *
	 * @param string $template Template.
	 */
	public function template_loader( $template ) {
		$find = array( 'fincommerce.php' );
		$file = '';
	...
	public function show_brand() {
		...
		if ( is_singular( 'product' ) ) {
			...
			$brand_output = wc_get_brands( $post->ID, ', ', ' <span class="posted_in">' . sprintf( _n( '%s: ', '%s: ', $brand_count, 'fincommerce' ), $labels->singular_name, $labels->name ), '</span>' );
	...
	public function output_product_brand_list( $atts ) {
		...
		$alphabet       = apply_filters( 'fincommerce_brands_list_alphabet', range( 'a', 'z' ) );
		$numbers        = apply_filters( 'fincommerce_brands_list_numbers', '0-9' );

		foreach ( $terms as $term ) {
			$term_letter = $this->get_brand_name_first_character( $term->name );

			// Allow a locale to be set for ctype_alpha().
			if ( has_filter( 'fincommerce_brands_list_locale' ) ) {
				setLocale( LC_CTYPE, apply_filters( 'fincommerce_brands_list_locale', 'en_US.UTF-8' ) );
			}
	...
	public function output_brand_products( $atts ) {
		...
		add_filter( 'fincommerce_shortcode_products_query', array( __CLASS__, 'get_brand_products_query_args' ), 10, 3 );
		...
		remove_filter( 'fincommerce_shortcode_products_query', array( __CLASS__, 'get_brand_products_query_args' ), 10 );

		return $shortcode->get_content();
	}

	/**
	 * Adds the taxonomy query to the FinCommerce products shortcode query arguments.
	 *
	...
	/**
	 * Adds the "brand" attribute to the list of FinCommerce products shortcode attributes.
	 *
	...
	public function rest_api_product_collection_params( $params, $post_type ) {
		$params['brand'] = array(
			'description'       => __( 'Limit result set to products assigned a specific brand ID.', 'fincommerce' ),
			'type'              => 'string',
	...
	public function wc_brands_on_block_template_register( $template_id, $template_area, $template ) {
		...
		if ( 'simple-product' === $template->get_id() ) {
			...
			if ( $section !== null ) {
				$section->add_block(
					array(
						'id'         => 'fincommerce-brands-select',
						'blockName'  => 'fincommerce/product-taxonomy-field',
						'order'      => 15,
						'attributes' => array(
							'label'       => __( 'Brands', 'fincommerce-brands' ),
							'createTitle' => __( 'Create new brand', 'fincommerce-brands' ),
							'slug'        => 'product_brand',
							'property'    => 'brands',
						),
					)
	...
	public function hook_product_brand_block( $hooked_block_types, $relative_position, $anchor_block_type, $context ) {
		...
		if ( 'fincommerce/product-meta' === $anchor_block_type &&
			 'last_child' === $relative_position &&
			 $context instanceof WP_Block_Template &&
			 'single-product' === $context->slug ) {

				remove_action( 'fincommerce_product_meta_end', array( $this, 'show_brand' ) );
	...
	public function configure_product_brand_block( $parsed_hooked_block, $hooked_block_type, $relative_position, $parsed_anchor_block, $context ) {
		...
		if ( 'core/post-terms' === $hooked_block_type &&
			 'last_child' === $relative_position &&
			 'fincommerce/product-meta' === $parsed_anchor_block['blockName'] &&
			 empty( $parsed_anchor_block['attrs'] ) ) {

			$parsed_hooked_block['attrs'] = array(
				'term'	 => 'product_brand',
				'prefix' => __( 'Brands: ', 'fincommerce' ),
			);
		}
