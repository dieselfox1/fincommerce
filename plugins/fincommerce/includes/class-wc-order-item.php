...
class WC_Order_Item extends WC_Data implements ArrayAccess {
	...
	*/
	...
	public function calculate_taxes( $calculate_tax_for = array() ) {
		...
		do_action( 'fincommerce_order_item_after_calculate_taxes', $this, $calculate_tax_for );
	...
	*/
	...
	/**
	 * Wrapper for get_formatted_meta_data that includes all metadata by default. See https://github.com/dieselfox1/fincommerce/pull/30948
	 *
	...
	public function get_formatted_meta_data( $hideprefix = '_', $include_all = false ) {
		...
		foreach ( $meta_data as $meta ) {
			...
			$formatted_meta[ $meta->id ] = (object) array(
				'key'           => $meta->key,
				'value'         => $meta->value,
				'display_key'   => apply_filters( 'fincommerce_order_item_display_meta_key', $display_key, $meta, $this ),
				'display_value' => wpautop( make_clickable( apply_filters( 'fincommerce_order_item_display_meta_value', $display_value, $meta, $this ) ) ),
			);
		}

		return apply_filters( 'fincommerce_order_item_get_formatted_meta_data', $formatted_meta, $this );
	}
	...
	*/
	...
	public function calculate_cogs_value(): bool {
		 ...
		 */
		$value = apply_filters( 'fincommerce_calculated_order_item_cogs_value', $value, $this );
	...
	protected function calculate_cogs_value_core(): ?float {
		...
		throw new Exception(
			sprintf(
				...
				__( 'Method %1$s is not implemented. Classes overriding has_cogs must override this method too.', 'fincommerce' ),
				__METHOD__
			)
		);
	...
	public function get_cogs_value_html(): string {
		...
		if ( ! $this->has_cogs() ) {
			 ...
			 * @since 9.9.0
			 */
			return apply_filters( 'fincommerce_order_item_no_cogs_html', "<span class='na'>&ndash;</span>", $this );
		}
		 ...
		 * @since 9.9.0
		 */
		return apply_filters( 'fincommerce_order_item_cogs_html', $cogs_value_html, $cogs_value, $this );
	}
	...
	public function get_cogs_value_per_unit_tooltip_text(): string {
		...
		if ( $quantity > 0 && $cogs_value > 0 ) {
			...
			$tooltip_text = sprintf( __( 'Cost per unit: %s', 'fincommerce' ), $formatted_cost_per_item );
		}
		 ...
		 * @since 9.9.0
		 */
		return apply_filters( 'fincommerce_order_item_cogs_per_item_tooltip', $tooltip_text, $cost_per_item, $formatted_cost_per_item, $this );
	}
}
