...
class WC_Gateway_Paypal_PDT_Handler extends WC_Gateway_Paypal_Response {
	...
	public function __construct( $sandbox = false, $identity_token = '' ) {
		add_action( 'fincommerce_thankyou_paypal', array( $this, 'check_response_for_order' ) );
	...
	protected function validate_transaction( $transaction ) {
		$pdt = array(
			...
			'timeout'     => 60,
			'httpversion' => '1.1',
			'user-agent'  => 'FinCommerce/' . Constants::get_constant( 'WC_VERSION' ),
		);
	...
	public function check_response() {
		global $wp;
		$order_id = apply_filters( 'fincommerce_thankyou_order_id', absint( $wp->query_vars['order-received'] ) );
	...
	public function check_response_for_order( $wc_order_id ) {
		...
		if ( $transaction_result ) {
			...
			if ( $wc_order->get_id() !== $order->get_id() ) {
				...
				WC_Gateway_Paypal::log( sprintf( __( 'Received PDT notification for order %1$d on endpoint for order %2$d.', 'fincommerce' ), $order->get_id(), $wc_order_id ), 'error' );
				return;
			}

			if ( 0 !== strcasecmp( trim( $transaction_result['receiver_email'] ), trim( $this->receiver_email ) ) ) {
				/* translators: 1: email address, 2: order ID . */
				WC_Gateway_Paypal::log( sprintf( __( 'Received PDT notification for another account: %1$s. Order ID: %2$d.', 'fincommerce' ), $transaction_result['receiver_email'], $order->get_id() ), 'error' );
				return;
			}
			...
			if ( OrderStatus::COMPLETED === $status ) {
				if ( number_format( $order->get_total(), 2, '.', '' ) !== number_format( $amount, 2, '.', '' ) ) {
					...
					$this->payment_on_hold( $order, sprintf( __( 'Validation error: PayPal amounts do not match (amt %s).', 'fincommerce' ), $amount ) );
				} else {
					...
					$this->payment_complete( $order, $transaction, __( 'PDT payment completed', 'fincommerce' ) );
				}
			} else {
				if ( 'authorization' === $transaction_result['pending_reason'] ) {
					...
					$this->payment_on_hold( $order, sprintf( __( 'Payment pending (%s).', 'fincommerce' ), $transaction_result['pending_reason'] ) );
				}
			}
		} else {
