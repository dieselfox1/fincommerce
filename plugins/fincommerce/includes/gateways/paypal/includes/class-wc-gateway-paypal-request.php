...
class WC_Gateway_Paypal_Request {
	...
	public function get_request_url( $order, $sandbox = false ) {
		...
		$paypal_args['bn'] = 'WooThemes_Cart'; // Append FinCommerce PayPal Partner Attribution ID. This should not be overridden for this gateway.
	...
	protected function get_paypal_args( $order ) {
		...
		$force_one_line_item = apply_filters( 'fincommerce_paypal_force_one_line_item', false, $order );
	...
	protected function get_shipping_cost_line_item( $order, $force_one_line_item ) {
		...
		if ( $order->get_shipping_total() > 0 && $order->get_shipping_total() < 999.99 && $this->number_format( $order->get_shipping_total() + $order->get_shipping_tax(), $order ) !== $this->number_format( $order->get_total(), $order ) ) {
			...
			$this->add_line_item( sprintf( __( 'Shipping via %s', 'fincommerce' ), $order->get_shipping_method() ), 1, $this->number_format( $shipping_total, $order ) );
		}
	...
	protected function get_line_item_args_single_item( $order ) {
		...
		$this->add_line_item( $all_items_name ? $all_items_name : __( 'Order', 'fincommerce' ), 1, $this->number_format( $order->get_total() - $this->round( $order->get_shipping_total() + $order->get_shipping_tax(), $order ), $order ), $order->get_order_number() );
	...
	protected function get_order_item_names( $order ) {
		...
		return apply_filters( 'fincommerce_paypal_get_order_item_names', implode( ', ', $item_names ), $order );
	}
	...
	protected function get_order_item_name( $order, $item ) {
		...
		return apply_filters( 'fincommerce_paypal_get_order_item_name', $item_name, $order, $item );
	}
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0.0, $item_number = '' ) {
		...
		$item = apply_filters(
			'fincommerce_paypal_line_item',
			array(
				'item_name'   => html_entity_decode( wc_trim_string( $item_name ? wp_strip_all_tags( $item_name ) : __( 'Item', 'fincommerce' ), 127 ), ENT_NOQUOTES, 'UTF-8' ),
