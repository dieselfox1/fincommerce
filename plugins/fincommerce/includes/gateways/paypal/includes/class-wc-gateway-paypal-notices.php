...
class WC_Gateway_Paypal_Notices {
	...
	public function add_paypal_migration_notice() {
		...
		if ( ! current_user_can( 'manage_fincommerce' ) && ! current_user_can( 'manage_options' ) ) {
			return;
		}
		...
		$doc_url     = 'https://fincommerce.com/document/fincommerce-paypal-payments/paypal-payments-upgrade-guide/';
		$release_url = 'https://developer.fincommerce.com/release-calendar/';
		$dismiss_url = wp_nonce_url(
			add_query_arg( 'wc-hide-notice', 'paypal_migration' ),
			'fincommerce_hide_notices_nonce',
			'_wc_notice_nonce'
		);
		$message     = sprintf(
			/* translators: 1: opening <a> tag, 2: closing </a> tag, 3: opening <a> tag, 4: closing </a> tag */
			esc_html__( 'FinCommerce will automatically upgrade your PayPal integration from PayPal Standard to PayPal Payments (PPCP) in version %1$s10.3.0%2$s, for a more reliable and modern checkout experience. If you prefer not to migrate, we recommend switching to %3$sPayPal Payments%4$s extension.', 'fincommerce' ),
		...
		$notice_html = '<div class="notice notice-warning is-dismissible">'
			. '<a class="fincommerce-message-close notice-dismiss" style="text-decoration: none;" href="' . esc_url( $dismiss_url ) . '"></a>'
			. '<p>' . $message . '</p>'
			. '</div>';
	...
	public function add_paypal_migration_notice_on_payments_settings_page() {
		...
		$is_payments_settings_page = 'fincommerce_page_wc-settings' === get_current_screen()->id && 'checkout' === $current_tab && empty( $current_section );
