...
class WC_Gateway_Paypal_IPN_Handler extends WC_Gateway_Paypal_Response {
	...
	public function __construct( $sandbox = false, $receiver_email = '' ) {
		add_action( 'fincommerce_api_wc_gateway_paypal', array( $this, 'check_response' ) );
	...
	public function validate_ipn() {
		...
		$params = array(
			...
			'compress'    => false,
			'decompress'  => false,
			'user-agent'  => 'FinCommerce/' . WC()->version,
		);
	...
	protected function validate_currency( $order, $currency ) {
		if ( $order->get_currency() !== $currency ) {
			...
			$order->update_status( OrderStatus::ON_HOLD, sprintf( __( 'Validation error: PayPal currencies do not match (code %s).', 'fincommerce' ), $currency ) );
			exit;
		}
	}
	...
	protected function validate_amount( $order, $amount ) {
		if ( number_format( $order->get_total(), 2, '.', '' ) !== number_format( $amount, 2, '.', '' ) ) {
			...
			$order->update_status( OrderStatus::ON_HOLD, sprintf( __( 'Validation error: PayPal amounts do not match (gross %s).', 'fincommerce' ), $amount ) );
			exit;
		}
	}
	 ...
	 * FinCommerce -> Settings -> Checkout -> PayPal, it will log an error about it.
	 *
	 * @param WC_Order $order          Order object.
	...
	protected function validate_receiver_email( $order, $receiver_email ) {
		if ( strcasecmp( trim( $receiver_email ), trim( $this->receiver_email ) ) !== 0 ) {
			...
			$order->update_status( OrderStatus::ON_HOLD, sprintf( __( 'Validation error: PayPal IPN response from a different email address (%s).', 'fincommerce' ), $receiver_email ) );
			exit;
		}
	}
	...
	protected function payment_status_completed( $order, $posted ) {
		...
		if ( OrderStatus::COMPLETED === $posted['payment_status'] ) {
			...
			$this->payment_complete( $order, ( ! empty( $posted['txn_id'] ) ? wc_clean( $posted['txn_id'] ) : '' ), __( 'IPN payment completed', 'fincommerce' ) );
		} else {
			if ( 'authorization' === $posted['pending_reason'] ) {
				$this->payment_on_hold( $order, __( 'Payment authorized. Change payment status to processing or complete to capture funds.', 'fincommerce' ) );
			} else {
				/* translators: %s: pending reason. */
				$this->payment_on_hold( $order, sprintf( __( 'Payment pending (%s).', 'fincommerce' ), $posted['pending_reason'] ) );
			}
		}
	}
	...
	protected function payment_status_failed( $order, $posted ) {
		...
		$order->update_status( OrderStatus::FAILED, sprintf( __( 'Payment %s via IPN.', 'fincommerce' ), wc_clean( $posted['payment_status'] ) ) );
	}
	...
	protected function payment_status_paid_cancelled_order( $order, $posted ) {
		$this->send_ipn_email_notification(
			...
			sprintf( __( 'Payment for cancelled order %s received', 'fincommerce' ), '<a class="link" href="' . esc_url( $order->get_edit_order_url() ) . '">' . $order->get_order_number() . '</a>' ),
			/* translators: %s: order ID. */
			sprintf( __( 'Order #%s has been marked paid by PayPal IPN, but was previously cancelled. Admin handling required.', 'fincommerce' ), $order->get_order_number() )
		);
	}
	...
	protected function payment_status_refunded( $order, $posted ) {
		...
		if ( $order->get_total() === wc_format_decimal( $posted['mc_gross'] * -1, wc_get_price_decimals() ) ) {
			...
			/* translators: %s: payment status. */
			$order->update_status( OrderStatus::REFUNDED, sprintf( __( 'Payment %s via IPN.', 'fincommerce' ), strtolower( $posted['payment_status'] ) ) );

			$this->send_ipn_email_notification(
				/* translators: %s: order link. */
				sprintf( __( 'Payment for order %s refunded', 'fincommerce' ), '<a class="link" href="' . esc_url( $order->get_edit_order_url() ) . '">' . $order->get_order_number() . '</a>' ),
				/* translators: %1$s: order ID, %2$s: reason code. */
				sprintf( __( 'Order #%1$s has been marked as refunded - PayPal reason code: %2$s', 'fincommerce' ), $order->get_order_number(), $posted['reason_code'] )
			);
		}
	}
	...
	protected function payment_status_reversed( $order, $posted ) {
		/* translators: %s: payment status. */
		$order->update_status( OrderStatus::ON_HOLD, sprintf( __( 'Payment %s via IPN.', 'fincommerce' ), wc_clean( $posted['payment_status'] ) ) );

		$this->send_ipn_email_notification(
			/* translators: %s: order link. */
			sprintf( __( 'Payment for order %s reversed', 'fincommerce' ), '<a class="link" href="' . esc_url( $order->get_edit_order_url() ) . '">' . $order->get_order_number() . '</a>' ),
			/* translators: %1$s: order ID, %2$s: reason code. */
			sprintf( __( 'Order #%1$s has been marked on-hold due to a reversal - PayPal reason code: %2$s', 'fincommerce' ), $order->get_order_number(), wc_clean( $posted['reason_code'] ) )
		);
	}
	...
	protected function payment_status_canceled_reversal( $order, $posted ) {
		$this->send_ipn_email_notification(
			/* translators: %s: order link. */
			sprintf( __( 'Reversal cancelled for order #%s', 'fincommerce' ), $order->get_order_number() ),
			/* translators: %1$s: order ID, %2$s: order link. */
			sprintf( __( 'Order #%1$s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %2$s', 'fincommerce' ), $order->get_order_number(), esc_url( $order->get_edit_order_url() ) )
		);
	}
	...
	protected function send_ipn_email_notification( $subject, $message ) {
		$new_order_settings = get_option( 'fincommerce_new_order_settings', array() );
		$mailer             = WC()->mailer();
		$message            = $mailer->wrap_message( $subject, $message );

		$fincommerce_paypal_settings = get_option( 'fincommerce_paypal_settings' );
		if ( ! empty( $fincommerce_paypal_settings['ipn_notification'] ) && 'no' === $fincommerce_paypal_settings['ipn_notification'] ) {
			return;
		}
