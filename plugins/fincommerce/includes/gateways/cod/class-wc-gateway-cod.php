...
class WC_Gateway_COD extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_action( 'fincommerce_thankyou_' . $this->id, array( $this, 'thankyou_page' ) );
		add_filter( 'fincommerce_payment_complete_order_status', array( $this, 'change_payment_complete_order_status' ), 10, 3 );
	...
	protected function setup_properties() {
		$this->id                 = self::ID;
		$this->icon               = apply_filters( 'fincommerce_cod_icon', '' );
		$this->method_title       = __( 'Cash on delivery', 'fincommerce' );
		$this->method_description = __( 'Let your shoppers pay upon delivery — by cash or other methods of payment.', 'fincommerce' );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled'            => array(
				...
				'label'       => __( 'Enable cash on delivery', 'fincommerce' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no',
			),
			'title'              => array(
				...
				'default'     => __( 'Cash on delivery', 'fincommerce' ),
				'desc_tip'    => true,
			),
			'description'        => array(
				'title'       => __( 'Description', 'fincommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Payment method description that the customer will see on your website.', 'fincommerce' ),
				'default'     => __( 'Pay with cash upon delivery.', 'fincommerce' ),
				'desc_tip'    => true,
			),
			'instructions'       => array(
				'title'       => __( 'Instructions', 'fincommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Instructions that will be added to the thank you page.', 'fincommerce' ),
				'default'     => __( 'Pay with cash upon delivery.', 'fincommerce' ),
				'desc_tip'    => true,
			),
			'enable_for_methods' => array(
				'title'             => __( 'Enable for shipping methods', 'fincommerce' ),
				'type'              => 'multiselect',
				'class'             => 'wc-enhanced-select',
				'css'               => 'width: 400px;',
				'default'           => '',
				'description'       => __( 'If COD is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'fincommerce' ),
				'options'           => $this->load_shipping_method_options(),
				'desc_tip'          => true,
				'custom_attributes' => array(
					'data-placeholder' => __( 'Select shipping methods', 'fincommerce' ),
				),
			),
			'enable_for_virtual' => array(
				'title'   => __( 'Accept for virtual orders', 'fincommerce' ),
				'label'   => __( 'Accept COD if the order is virtual', 'fincommerce' ),
				'type'    => 'checkbox',
				'default' => 'yes',
			),
		);
	...
	private function load_shipping_method_options() {
		...
		foreach ( WC()->shipping()->load_shipping_methods() as $method ) {
			...
			$options[ $method->get_method_title() ][ $method->id ] = sprintf( __( 'Any &quot;%1$s&quot; method', 'fincommerce' ), $method->get_method_title() );
			...
			foreach ( $zones as $zone ) {
				...
				foreach ( $shipping_method_instances as $shipping_method_instance_id => $shipping_method_instance ) {
					...
					$option_instance_title = sprintf( __( '%1$s (#%2$s)', 'fincommerce' ), $shipping_method_instance->get_title(), $shipping_method_instance_id );

					// Translators: %1$s zone name, %2$s shipping method instance name.
					$option_title = sprintf( __( '%1$s &ndash; %2$s', 'fincommerce' ), $zone->get_id() ? $zone->get_zone_name() : __( 'Other locations', 'fincommerce' ), $option_instance_title );
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			 ...
			 */
			$process_payment_status = apply_filters( 'fincommerce_cod_process_payment_order_status', $order->has_downloadable_item() ? OrderStatus::ON_HOLD : OrderStatus::PROCESSING, $order );
			// Mark as processing or on-hold (payment won't be taken until delivery).
			$order->update_status( $process_payment_status, __( 'Payment to be made upon delivery.', 'fincommerce' ) );
		} else {
