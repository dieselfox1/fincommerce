 ...
 *
 * @package FinCommerce\Gateways
 */

use Automattic\FinCommerce\Enums\OrderStatus;
use Automattic\FinCommerce\Internal\Admin\Settings\Utils as SettingsUtils;
...
class WC_Gateway_BACS extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->id                 = self::ID;
		$this->icon               = apply_filters( 'fincommerce_bacs_icon', '' );
		$this->has_fields         = false;
		$this->method_title       = __( 'Direct bank transfer', 'fincommerce' );
		$this->method_description = __( 'Take payments in person via BACS. More commonly known as direct bank/wire transfer.', 'fincommerce' );
		...
		add_action( 'fincommerce_update_options_payment_gateways_' . $this->id, array( $this, 'save_account_details' ) );
		add_action( 'fincommerce_thankyou_bacs', array( $this, 'thankyou_page' ) );

		// Customer Emails.
		add_action( 'fincommerce_email_before_order_table', array( $this, 'email_instructions' ), 10, 3 );
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled'         => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Enable bank transfer', 'fincommerce' ),
				'default' => 'no',
			),
			'title'           => array(
				...
				'default'     => __( 'Direct bank transfer', 'fincommerce' ),
				'desc_tip'    => true,
			),
			'description'     => array(
				'title'       => __( 'Description', 'fincommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Payment method description that the customer will see on your checkout.', 'fincommerce' ),
				'default'     => __( 'Make your payment directly into our bank account. Please use your Order ID as the payment reference. Your order will not be shipped until the funds have cleared in our account.', 'fincommerce' ),
				'desc_tip'    => true,
			),
			'instructions'    => array(
				'title'       => __( 'Instructions', 'fincommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Instructions that will be added to the thank you page and emails.', 'fincommerce' ),
				'default'     => '',
				'desc_tip'    => true,
			),
	...
	public function generate_account_details_html() {
		...
		$sortcode = isset( $locale[ $country ]['sortcode']['label'] ) ? $locale[ $country ]['sortcode']['label'] : __( 'Sort code', 'fincommerce' );

		?>
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label>
					<?php esc_html_e( 'Account details:', 'fincommerce' ); ?>
					<?php echo wp_kses_post( wc_help_tip( __( 'These account details will be displayed within the order thank you page and confirmation email.', 'fincommerce' ) ) ); ?>
				</label>
			</th>
				...
				<div class="wc_input_table_wrapper">
					<table class="widefat wc_input_table sortable" cellspacing="0">
						<thead>
							<tr>
								<th class="sort">&nbsp;</th>
								<th><?php esc_html_e( 'Account name', 'fincommerce' ); ?></th>
								<th><?php esc_html_e( 'Account number', 'fincommerce' ); ?></th>
								<th><?php esc_html_e( 'Bank name', 'fincommerce' ); ?></th>
								<th><?php echo esc_html( $sortcode ); ?></th>
								<th><?php esc_html_e( 'IBAN', 'fincommerce' ); ?></th>
								<th><?php esc_html_e( 'BIC / Swift', 'fincommerce' ); ?></th>
							</tr>
						</thead>
						...
						<tfoot>
							<tr>
								<th colspan="7"><a href="#" class="add button"><?php esc_html_e( '+ Add account', 'fincommerce' ); ?></a> <a href="#" class="remove_rows button"><?php esc_html_e( 'Remove selected account(s)', 'fincommerce' ); ?></a></th>
							</tr>
						</tfoot>
					</table>
				</div>
	...
	public function save_account_details() {
		...
		// phpcs:disable finpress.Security.NonceVerification.Missing -- Nonce verification already handled in WC_Admin_Settings::save()
		...
		do_action( 'fincommerce_update_option', array( 'id' => 'fincommerce_bacs_accounts' ) );
		update_option( 'fincommerce_bacs_accounts', $accounts );
	}
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		if ( ! $sent_to_admin && self::ID === $order->get_payment_method() ) {
			 ...
			 */
			$instructions_order_status = apply_filters( 'fincommerce_bacs_email_instructions_order_status', OrderStatus::ON_HOLD, $order );
	...
	private function bank_details( $order_id = '' ) {
		...
		$bacs_accounts = apply_filters( 'fincommerce_bacs_accounts', $this->account_details, $order_id );
		...
		if ( ! empty( $bacs_accounts ) ) {
			...
			foreach ( $bacs_accounts as $bacs_account ) {
				...
				$account_fields = apply_filters(
					'fincommerce_bacs_account_fields',
					array(
						'bank_name'      => array(
							'label' => __( 'Bank', 'fincommerce' ),
							'value' => $bacs_account->bank_name,
						),
						'account_number' => array(
							'label' => __( 'Account number', 'fincommerce' ),
						...
						'iban'           => array(
							'label' => __( 'IBAN', 'fincommerce' ),
							'value' => $bacs_account->iban,
						),
						'bic'            => array(
							'label' => __( 'BIC', 'fincommerce' ),
			...
			if ( $has_details ) {
				echo '<section class="fincommerce-bacs-bank-details"><h2 class="wc-bacs-bank-details-heading">' . esc_html__( 'Our bank details', 'fincommerce' ) . '</h2>' . wp_kses_post( PHP_EOL . $account_html ) . '</section>';
			}
		}
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			 ...
			 * @param object $order          The order object.
			 */
			$process_payment_status = apply_filters( 'fincommerce_bacs_process_payment_order_status', OrderStatus::ON_HOLD, $order );
			// Mark as on-hold (we're awaiting the payment).
			$order->update_status( $process_payment_status, __( 'Awaiting BACS payment', 'fincommerce' ) );
		} else {
	...
	public function get_country_locale() {
		...
		if ( empty( $this->locale ) ) {
			...
			$this->locale = apply_filters(
				'fincommerce_get_bacs_locale',
				array(
					'AU' => array(
						'sortcode' => array(
							'label' => __( 'BSB', 'fincommerce' ),
						),
					),
					'CA' => array(
						'sortcode' => array(
							'label' => __( 'Bank transit number', 'fincommerce' ),
						),
					),
					'IN' => array(
						'sortcode' => array(
							'label' => __( 'IFSC', 'fincommerce' ),
						),
					),
					'IT' => array(
						'sortcode' => array(
							'label' => __( 'Branch sort', 'fincommerce' ),
						),
					),
					'NZ' => array(
						'sortcode' => array(
							'label' => __( 'Bank code', 'fincommerce' ),
						),
					),
					'SE' => array(
						'sortcode' => array(
					...
					'US' => array(
						'sortcode' => array(
							'label' => __( 'Routing number', 'fincommerce' ),
						),
					),
					'ZA' => array(
						'sortcode' => array(
							'label' => __( 'Branch code', 'fincommerce' ),
						),
					),
				)
			);
