	...
	public function form() {
		...
		$default_fields = array(
			'routing-number' => '<p class="form-row form-row-first">
				<label for="' . esc_attr( $this->id ) . '-routing-number">' . esc_html__( 'Routing number', 'fincommerce' ) . '&nbsp;<span class="required">*</span></label>
			...
			'account-number' => '<p class="form-row form-row-wide">
				<label for="' . esc_attr( $this->id ) . '-account-number">' . esc_html__( 'Account number', 'fincommerce' ) . '&nbsp;<span class="required">*</span></label>
		...
		$fields = wp_parse_args( $fields, apply_filters( 'fincommerce_echeck_form_fields', $default_fields, $this->id ) );
		?>

		<fieldset id="<?php echo esc_attr( $this->id ); ?>-cc-form" class='wc-echeck-form wc-payment-form'>
			<?php do_action( 'fincommerce_echeck_form_start', $this->id ); ?>
			<?php
			foreach ( $fields as $field ) {
				echo $field; // phpcs:ignore finpress.XSS.EscapeOutput.OutputNotEscaped
			}
			?>
			<?php do_action( 'fincommerce_echeck_form_end', $this->id ); ?>
			<div class="clear"></div>
		</fieldset>
		<?php
	}
