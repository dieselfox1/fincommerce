...
class WC_Gateway_Cheque extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id                 = self::ID;
		$this->icon               = apply_filters( 'fincommerce_cheque_icon', '' );
		$this->has_fields         = false;
		$this->method_title       = _x( 'Check payments', 'Check payment method', 'fincommerce' );
		$this->method_description = __( 'Take payments in person via checks. This offline gateway can also be useful to test purchases.', 'fincommerce' );
		...
		add_action( 'fincommerce_thankyou_cheque', array( $this, 'thankyou_page' ) );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled'      => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Enable check payments', 'fincommerce' ),
				'default' => 'no',
			),
			'title'        => array(
				...
				'default'     => _x( 'Check payments', 'Check payment method', 'fincommerce' ),
				'desc_tip'    => true,
			),
			'description'  => array(
				...
				'default'     => __( 'Please send a check to Store Name, Store Street, Store Town, Store State / County, Store Postcode.', 'fincommerce' ),
				'desc_tip'    => true,
			),
			'instructions' => array(
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		if ( $this->instructions && ! $sent_to_admin && self::ID === $order->get_payment_method() ) {
			 ...
			 */
			$instructions_order_status = apply_filters( 'fincommerce_cheque_email_instructions_order_status', OrderStatus::ON_HOLD, $order );
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			 ...
			 * @param object $order  The order object.
			 */
			$process_payment_status = apply_filters( 'fincommerce_cheque_process_payment_order_status', OrderStatus::ON_HOLD, $order );
			// Mark as on-hold (we're awaiting the cheque).
			$order->update_status( $process_payment_status, _x( 'Awaiting check payment', 'Check payment method', 'fincommerce' ) );
		} else {
