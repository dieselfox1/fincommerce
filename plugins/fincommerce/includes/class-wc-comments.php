...
use Automattic\FinCommerce\Internal\Admin\ProductReviews\ReviewsUtil;
...
class WC_Comments {
	 ...
	 * @var string
	 */
	private const PRODUCT_REVIEWS_PENDING_COUNT_CACHE_KEY = 'fincommerce_product_reviews_pending_count';
	...
	public static function check_comment_rating( $comment_data ) {
		...
		if ( ! is_admin() && isset( $_POST['comment_post_ID'], $_POST['rating'], $comment_data['comment_type'] ) && 'product' === get_post_type( absint( $_POST['comment_post_ID'] ) ) && empty( $_POST['rating'] ) && self::is_default_comment_type( $comment_data['comment_type'] ) && wc_review_ratings_enabled() && wc_review_ratings_required() ) { // WPCS: input var ok, CSRF ok.
			wp_die( esc_html__( 'Please rate the product.', 'fincommerce' ) );
			exit;
		}
		return $comment_data;
	}
	...
	public static function add_review_comment_filter( array $comment_types ): array {
		$comment_types['review'] = __( 'Product Reviews', 'fincommerce' );
	...
	public static function validate_product_review_verified_owners( $comment_post_id ) {
		...
		if ( 'yes' !== get_option( 'fincommerce_review_rating_verification_required' ) ) {
			return;
		}
		...
		wp_die(
			esc_html__( 'Only logged in customers who have purchased this product may leave a review.', 'fincommerce' ),
			esc_html__( 'Reviews can only be left by "verified owners"', 'fincommerce' ),
			array(
				'code' => 403,
			)
		);
