...
abstract class WC_Data {
	...
	public function delete( $force_delete = false ) {
		 ...
		 * @since 8.1.0.
		 */
		$check = apply_filters( "fincommerce_pre_delete_$this->object_type", null, $this, $force_delete );
		if ( null !== $check ) {
			return $check;
		}
		if ( $this->data_store ) {
	...
	protected function is_internal_meta_key( $key ) {
		...
		wc_doing_it_wrong( __FUNCTION__, sprintf( __( 'Generic add/update/get meta methods should not be used for internal meta data, including "%s". Use getters and setters.', 'fincommerce' ), $key ), '3.2.0' );
	...
	protected function get_hook_prefix() {
		return 'fincommerce_' . $this->object_type . '_get_';
	}
	...
	protected function set_date_prop( $prop, $value ) {
		try {
			...
			if ( is_a( $value, 'WC_DateTime' ) ) {
				...
				$this->error( 'invalid_date', __( 'Invalid date provided.', 'fincommerce' ) );
			}
