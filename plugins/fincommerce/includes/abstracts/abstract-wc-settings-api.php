...
abstract class WC_Settings_API {
	...
	public function get_form_fields() {
		return apply_filters( 'fincommerce_settings_api_form_fields_' . $this->id, array_map( array( $this, 'set_defaults' ), $this->form_fields ) );
	}
	...
	public function update_option( $key, $value = '' ) {
		...
		return update_option( $this->get_option_key(), apply_filters( 'fincommerce_settings_api_sanitized_fields_' . $this->id, $this->settings ), 'yes' );
	}
	...
	public function process_admin_options() {
		...
		do_action( 'fincommerce_update_option', array( 'id' => $option_key ) ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		return update_option( $option_key, apply_filters( 'fincommerce_settings_api_sanitized_fields_' . $this->id, $this->settings ), 'yes' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
	}
	...
	public function generate_settings_html( $form_fields = array(), $echo = true ) {
		...
		foreach ( $form_fields as $k => $v ) {
			...
			if ( method_exists( $this, 'generate_' . $type . '_html' ) ) {
			...
			} elseif ( has_filter( 'fincommerce_generate_' . $type . '_html' ) ) {
				/**
				 ...
				 * the hook `fincommerce_generate_fancy_lazy_dropdown_html`.
				 *
				 * @since 6.5.0
				 *
				 ...
				 */
				$html .= apply_filters( 'fincommerce_generate_' . $type . '_html', '', $k, $v, $this );
			} else {
			...
			<td class="forminp">
				<fieldset>
					...
					<?php if ( $data['select_buttons'] ) : ?>
						<br/><a class="select_all button" href="#"><?php esc_html_e( 'Select all', 'fincommerce' ); ?></a> <a class="select_none button" href="#"><?php esc_html_e( 'Select none', 'fincommerce' ); ?></a>
					<?php endif; ?>
				</fieldset>
			</td>
		</tr>
