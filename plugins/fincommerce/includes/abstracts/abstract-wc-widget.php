...
abstract class WC_Widget extends WP_Widget {
	...
	public function get_cached_widget( $args ) {
		...
		if ( isset( $cache[ $this->get_widget_id_for_cache( $args['widget_id'] ) ] ) ) {
			echo $cache[ $this->get_widget_id_for_cache( $args['widget_id'] ) ]; // phpcs:ignore finpress.XSS.EscapeOutput.OutputNotEscaped
			return true;
		}
	...
	public function widget_start( $args, $instance ) {
		echo $args['before_widget']; // phpcs:ignore finpress.XSS.EscapeOutput.OutputNotEscaped

		$title = apply_filters( 'widget_title', $this->get_instance_title( $instance ), $instance, $this->id_base );

		if ( $title ) {
			echo $args['before_title'] . $title . $args['after_title']; // phpcs:ignore finpress.XSS.EscapeOutput.OutputNotEscaped
		}
	}
	...
	public function widget_end( $args ) {
		echo $args['after_widget']; // phpcs:ignore finpress.XSS.EscapeOutput.OutputNotEscaped
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			 ...
			 */
			$instance[ $key ] = apply_filters( 'fincommerce_widget_settings_sanitize_option', $instance[ $key ], $new_instance, $key, $setting );
		}
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			switch ( $setting['type'] ) {
				...
				case 'text':
					?>
					<p>
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo wp_kses_post( $setting['label'] ); ?></label><?php // phpcs:ignore finpress.XSS.EscapeOutput.OutputNotEscaped ?>
				...
				case 'number':
					?>
					<p>
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo $setting['label']; /* phpcs:ignore finpress.XSS.EscapeOutput.OutputNotEscaped */ ?></label>
				...
				default:
					do_action( 'fincommerce_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
					break;
			}
		}
	}
	...
	protected function get_current_page_url() {
		...
		// All current filters.
		if ( $_chosen_attributes = WC_Query::get_layered_nav_chosen_attributes() ) { // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.FoundInControlStructure, finpress.CodeAnalysis.AssignmentInCondition.Found
		...
		return apply_filters( 'fincommerce_widget_get_current_page_url', $link, $this );
	}
	...
	protected function get_widget_id_for_cache( $widget_id, $scheme = '' ) {
		...
		return apply_filters( 'fincommerce_cached_widget_id', $widget_id_for_cache );
	}
}
