<?php
/**
 * FinCommerce Product CSV importer
 *
...
class WC_Product_CSV_Importer extends WC_Product_Importer {
	...
	protected function read_file() {
		if ( ! WC_Product_CSV_Importer_Controller::is_file_valid_csv( $this->file ) ) {
			wp_die( esc_html__( 'Invalid file type. The importer supports CSV and TXT file formats.', 'fincommerce' ) );
		}
	...
	public function parse_images_field( $value ) {
		...
		$images    = array();
		$separator = apply_filters( 'fincommerce_product_import_image_separator', ',' );
	...
	protected function get_formatting_callback() {
		...
		return apply_filters( 'fincommerce_product_importer_formatting_callbacks', $callbacks, $this );
	}
	...
	protected function expand_data( $data ) {
		$data = apply_filters( 'fincommerce_product_importer_pre_expand_data', $data );
	...
	protected function set_parsed_data() {
		...
		foreach ( $this->raw_data as $row_index => $row ) {
			...
			do_action( 'fincommerce_product_importer_before_set_parsed_data', $row, $mapped_keys );
			 ...
			 * @since
			 */
			$this->parsed_data[] = apply_filters( 'fincommerce_product_importer_parsed_data', $this->expand_data( $data ), $this );
		}
	}
	...
	protected function get_row_id( $parsed_data ) {
		...
		if ( $id ) {
			...
			$row_data[] = sprintf( __( 'ID %d', 'fincommerce' ), $id );
		}
		if ( $sku ) {
			/* translators: %s: product SKU */
			$row_data[] = sprintf( __( 'SKU %s', 'fincommerce' ), $sku );
		}
	...
	public function import() {
		...
		foreach ( $this->parsed_data as $parsed_data_key => $parsed_data ) {
			do_action( 'fincommerce_product_import_before_import', $parsed_data );
			...
			if ( $sku_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error(
					'fincommerce_product_importer_error',
					esc_html__( 'A product with this SKU already exists.', 'fincommerce' ),
					array(
						'sku' => esc_attr( $sku ),
			...
			if ( $id_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error(
					'fincommerce_product_importer_error',
					esc_html__( 'A product with this ID already exists.', 'fincommerce' ),
					array(
						'id'  => $id,
						'row' => $this->get_row_id( $parsed_data ),
					)
			...
			if ( $update_existing && ( isset( $parsed_data['id'] ) || isset( $parsed_data['sku'] ) ) && ! $id_exists && ! $sku_exists ) {
				$data['skipped'][] = new WP_Error(
					'fincommerce_product_importer_error',
					esc_html__( 'No matching product exists to update.', 'fincommerce' ),
					array(
						'id'  => $id,
						'sku' => esc_attr( $sku ),
						'row' => $this->get_row_id( $parsed_data ),
