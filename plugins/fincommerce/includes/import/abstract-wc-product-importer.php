...
abstract class WC_Product_Importer implements WC_Importer_Interface {
	...
	public function get_parsed_data() {
		 ...
		 */
		return apply_filters( 'fincommerce_product_importer_parsed_data', $this->parsed_data, $this );
	}
	...
	protected function get_product_object( $data ) {
		...
		if ( isset( $data['type'] ) ) {
			...
			if ( ! array_key_exists( $data['type'], WC_Admin_Exporters::get_product_types() ) ) {
				return new WP_Error( 'fincommerce_product_importer_invalid_type', __( 'Invalid product type.', 'fincommerce' ), array( 'status' => 401 ) );
			}
			...
			try {
				...
				return new WP_Error( 'fincommerce_product_csv_importer_' . $e->getErrorCode(), $e->getMessage(), array( 'status' => 401 ) );
			}
			...
			if ( ! $product ) {
				return new WP_Error(
					'fincommerce_product_csv_importer_invalid_id',
					/* translators: %d: product ID */
					sprintf( __( 'Invalid product ID %d.', 'fincommerce' ), $id ),
					array(
						'id'     => $id,
						'status' => 401,
					)
		...
		return apply_filters( 'fincommerce_product_import_get_product_object', $product, $data );
	}
	...
	protected function process_item( $data ) {
		try {
			do_action( 'fincommerce_product_import_before_process_item', $data );
			$data = apply_filters( 'fincommerce_product_import_process_item_data', $data );
			...
			$object = apply_filters( 'fincommerce_product_import_pre_insert_product_object', $object, $data );
			$object->save();

			do_action( 'fincommerce_product_import_inserted_product_object', $object, $data );
			...
			return new WP_Error( 'fincommerce_product_importer_error', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	protected function set_variation_data( &$variation, $data ) {
		...
		if ( ! $parent ) {
			return new WP_Error( 'fincommerce_product_importer_missing_variation_parent_id', __( 'Variation cannot be imported: Missing parent ID or parent does not exist yet.', 'fincommerce' ), array( 'status' => 401 ) );
		}

		// Stop if parent is a product variation.
		if ( $parent->is_type( ProductType::VARIATION ) ) {
			return new WP_Error( 'fincommerce_product_importer_parent_set_as_variation', __( 'Variation cannot be imported: Parent product cannot be a product variation', 'fincommerce' ), array( 'status' => 401 ) );
		}
	...
	public function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id && stristr( $url, '://' ) ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				...
				throw new Exception( sprintf( __( 'Not able to attach "%s".', 'fincommerce' ), $url ), 400 );
			}
		...
		if ( ! $id ) {
			/* translators: %s: image URL */
			throw new Exception( sprintf( __( 'Unable to use image "%s".', 'fincommerce' ), $url ), 400 );
		}
	...
	public function get_attribute_taxonomy_id( $raw_name ) {
		...
		register_taxonomy(
			$taxonomy_name,
			apply_filters( 'fincommerce_taxonomy_objects_' . $taxonomy_name, array( 'product' ) ),
			apply_filters(
				'fincommerce_taxonomy_args_' . $taxonomy_name,
				array(
					'labels'       => array(
						'name' => $raw_name,
					),
	...
	protected function memory_exceeded() {
		...
		return apply_filters( 'fincommerce_product_importer_memory_exceeded', $return );
	}
	...
	protected function time_exceeded() {
		$finish = $this->start_time + apply_filters( 'fincommerce_product_importer_default_time_limit', 20 ); // 20 seconds
		$return = false;
		if ( time() >= $finish ) {
			$return = true;
		}
		return apply_filters( 'fincommerce_product_importer_time_exceeded', $return );
	}
