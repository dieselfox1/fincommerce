 ...
 * FinCommerce coupons.
 *
 * The FinCommerce coupons class gets coupon data from storage and checks coupon validity.
 *
...
class WC_Coupon extends WC_Legacy_Coupon {
	...
	public function __construct( $data = '' ) {
		...
		$coupon = apply_filters( 'fincommerce_get_shop_coupon_data', false, $data, $this );
	...
	protected function get_hook_prefix() {
		return 'fincommerce_coupon_get_';
	}
	...
	*/
	...
	/**
	 * If the filter is added through the fincommerce_get_shop_coupon_data filter, it's virtual and not in the DB.
	 *
	 * @since 3.2.0
	 * @param  string $context What the value is for. Valid values are 'view' and 'edit'.
	 * @return boolean
	...
	public function get_discount_amount( $discounting_amount, $cart_item = null, $single = false ) {
		...
		if ( $this->is_type( array( 'percent' ) ) ) {
			 ...
			 *
			 * Uses price inc tax if prices include tax to work around https://github.com/dieselfox1/fincommerce/issues/7669 and https://github.com/dieselfox1/fincommerce/issues/8074.
			 */
		...
		return apply_filters(
			'fincommerce_coupon_get_discount_amount',
	...
	*/
	...
	public function set_discount_type( $discount_type ) {
		...
		if ( ! in_array( $discount_type, array_keys( wc_get_coupon_types() ), true ) ) {
			$this->error( 'coupon_invalid_discount_type', __( 'Invalid discount type.', 'fincommerce' ) );
		}
	...
	public function set_amount( $amount ) {
		...
		if ( (float) $amount < 0 ) {
			$this->error( 'coupon_invalid_amount', __( 'Invalid discount amount.', 'fincommerce' ) );
		}
	...
	public function set_maximum_amount( $amount ) {
		if ( (float) $amount && (float) $this->get_minimum_amount() > (float) $amount ) {
			$this->error( 'coupon_invalid_maximum_amount', __( 'Invalid maximum spend value.', 'fincommerce' ) );
		}
	...
	public function set_email_restrictions( $emails = array() ) {
		...
		foreach ( $emails as $email ) {
			if ( ! is_email( $email ) ) {
				$this->error( 'coupon_invalid_email_address', __( 'Invalid email address restriction.', 'fincommerce' ) );
			}
		}
	...
	*/
	...
	public function is_valid_for_cart() {
		return apply_filters( 'fincommerce_coupon_is_valid_for_cart', $this->is_type( wc_get_cart_coupon_types() ), $this );
	}
	...
	public function is_valid_for_product( $product, $values = array() ) {
		if ( ! $this->is_type( wc_get_product_coupon_types() ) || ! is_a( $product, WC_Product::class ) ) {
			return apply_filters( 'fincommerce_coupon_is_valid_for_product', false, $product, $this, $values );
		}
		...
		return apply_filters( 'fincommerce_coupon_is_valid_for_product', $valid, $product, $this, $values );
	}
	...
	public function get_coupon_message( $msg_code ) {
		switch ( $msg_code ) {
			case self::WC_COUPON_SUCCESS:
				$msg = __( 'Coupon code applied successfully.', 'fincommerce' );
				break;
			case self::WC_COUPON_REMOVED:
				$msg = __( 'Coupon code removed successfully.', 'fincommerce' );
				break;
			default:
				$msg = '';
				break;
		}
		return apply_filters( 'fincommerce_coupon_message', $msg, $msg_code, $this );
	}
	...
	public function get_coupon_error( $err_code ) {
		switch ( $err_code ) {
			case self::E_WC_COUPON_INVALID_FILTERED:
				$err = sprintf(
					...
					esc_html__( 'Coupon "%s" cannot be applied because it is not valid.', 'fincommerce' ),
			...
			case self::E_WC_COUPON_NOT_EXIST:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Coupon "%s" cannot be applied because it does not exist.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			case self::E_WC_COUPON_INVALID_REMOVED:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Sorry, it seems the coupon "%s" is invalid - it has now been removed from your order.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			case self::E_WC_COUPON_NOT_YOURS_REMOVED:
				...
				if ( ! is_null( $billing_email ) ) {
					$err = sprintf(
						/* translators: %s: coupon code */
						esc_html__( 'Please enter a valid email to use coupon code "%s".', 'fincommerce' ),
						esc_html( $this->get_code() )
					);
				} else {
					$err = sprintf(
						/* translators: %s: coupon code */
						esc_html__( 'Please enter a valid email at checkout to use coupon code "%s".', 'fincommerce' ),
						esc_html( $this->get_code() )
					);
				}
				break;
			...
			case self::E_WC_COUPON_ALREADY_APPLIED_INDIV_USE_ONLY:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Sorry, coupon "%s" has already been applied and cannot be used in conjunction with other coupons.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			case self::E_WC_COUPON_USAGE_LIMIT_REACHED:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Usage limit for coupon "%s" has been reached.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			case self::E_WC_COUPON_EXPIRED:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Coupon "%s" has expired.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			case self::E_WC_COUPON_MIN_SPEND_LIMIT_NOT_MET:
				...
				$err          = sprintf(
					...
					esc_html__( 'The minimum spend for coupon "%1$s" is %2$s.', 'fincommerce' ),
					esc_html( $this->get_code() ),
			...
			case self::E_WC_COUPON_MAX_SPEND_LIMIT_MET:
				...
				$err          = sprintf(
					...
					esc_html__( 'The maximum spend for coupon "%1$s" is %2$s.', 'fincommerce' ),
					esc_html( $this->get_code() ),
			...
			case self::E_WC_COUPON_NOT_APPLICABLE:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Sorry, coupon "%s" is not applicable to your cart contents.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			case self::E_WC_COUPON_USAGE_LIMIT_COUPON_STUCK:
				if ( is_user_logged_in() && wc_get_page_id( 'myaccount' ) > 0 && ! WC()->is_store_api_request() ) {
					$err = sprintf(
						...
						wp_kses_data( __( 'Usage limit for coupon "%1$s" has been reached. If you were using this coupon just now but your order was not complete, you can retry or cancel the order by going to the <a href="%2$s">my account page</a>.', 'fincommerce' ) ),
						esc_html( $this->get_code() ),
			...
			case self::E_WC_COUPON_USAGE_LIMIT_COUPON_STUCK_GUEST:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Usage limit for coupon "%s" has been reached. Please try again after some time, or contact us for help.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			case self::E_WC_COUPON_EXCLUDED_PRODUCTS:
				...
				$err = sprintf(
					...
					esc_html__( 'Sorry, coupon "%1$s" is not applicable to the products: %2$s.', 'fincommerce' ),
					esc_html( $this->get_code() ),
			...
			case self::E_WC_COUPON_EXCLUDED_CATEGORIES:
				...
				$err = sprintf(
					...
					esc_html__( 'Sorry, coupon "%1$s" is not applicable to the categories: %2$s.', 'fincommerce' ),
					esc_html( $this->get_code() ),
			...
			case self::E_WC_COUPON_NOT_VALID_SALE_ITEMS:
				$err = sprintf(
					/* translators: %s: coupon code */
					esc_html__( 'Sorry, coupon "%s" is not valid for sale items.', 'fincommerce' ),
					esc_html( $this->get_code() )
				);
				break;
			default:
		...
		return apply_filters( 'fincommerce_coupon_error', $err, $err_code, $this );
	}
	...
	public static function get_generic_coupon_error( $err_code ) {
		switch ( $err_code ) {
			case self::E_WC_COUPON_NOT_EXIST:
				$err = __( 'Coupon does not exist.', 'fincommerce' );
				break;
			case self::E_WC_COUPON_PLEASE_ENTER:
				$err = __( 'Please enter a coupon code.', 'fincommerce' );
				break;
			default:
				$err = '';
				break;
		...
		return apply_filters( 'fincommerce_coupon_error', $err, $err_code, null );
	}
