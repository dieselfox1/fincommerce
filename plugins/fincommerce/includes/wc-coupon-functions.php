...
function wc_get_coupon_types() {
	return (array) apply_filters(
		'fincommerce_coupon_discount_types',
		array(
			'percent'       => __( 'Percentage discount', 'fincommerce' ),
			'fixed_cart'    => __( 'Fixed cart discount', 'fincommerce' ),
			'fixed_product' => __( 'Fixed product discount', 'fincommerce' ),
		)
	);
}
...
function wc_get_product_coupon_types() {
	return (array) apply_filters( 'fincommerce_product_coupon_types', array( 'fixed_product', 'percent' ) );
}
...
function wc_get_cart_coupon_types() {
	return (array) apply_filters( 'fincommerce_cart_coupon_types', array( 'fixed_cart' ) );
}
...
function wc_coupons_enabled() {
	return apply_filters( 'fincommerce_coupons_enabled', 'yes' === get_option( 'fincommerce_enable_coupons' ) );
}
...
function wc_get_coupon_id_by_code( $code, $exclude = 0 ) {
	...
	return apply_filters( 'fincommerce_get_coupon_id_from_code', absint( current( $ids ) ), $code, $exclude );
}
...
function wc_repair_zero_discount_coupons_lookup_table() {
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) !== $table_name ) {
		return array(
			'success' => false,
			'message' => __( 'Coupons lookup table does not exist.', 'fincommerce' ),
		);
	}
	...
	if ( empty( $zero_discount_entries ) ) {
		return array(
			'success' => true,
			'message' => __( 'No entries with zero discount amount found. Coupons lookup table is up to date.', 'fincommerce' ),
		);
	}
	...
	WC_Cache_Helper::get_transient_version( 'fincommerce_reports', true );

	$message = sprintf(
		/* translators: %1$d: number of entries processed, %2$d: number of errors */
		__( 'Coupons lookup table entries with zero discount amount repaired successfully. Processed %1$d entries with %2$d errors.', 'fincommerce' ),
		$processed_count,
		$error_count
	);
