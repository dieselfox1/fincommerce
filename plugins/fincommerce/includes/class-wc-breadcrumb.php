...
class WC_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'fincommerce_get_breadcrumb', $this->crumbs, $this );
	}
	...
	protected function add_crumbs_404() {
		$this->add_crumb( __( 'Error 404', 'fincommerce' ) );
	}
	...
	protected function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'product' === get_post_type( $post ) ) {
			...
			$terms = wc_get_product_terms(
				...
				'product_cat',
				apply_filters(
					'fincommerce_breadcrumb_product_terms_args',
					array(
						'orderby' => 'parent',
						'order'   => 'DESC',
					)
			...
			if ( $terms ) {
				$main_term = apply_filters( 'fincommerce_breadcrumb_main_term', $terms[0], $terms );
	...
	protected function add_crumbs_product_tag() {
		...
		$this->add_crumb( sprintf( __( 'Products tagged &ldquo;%s&rdquo;', 'fincommerce' ), $current_term->name ), get_term_link( $current_term, 'product_tag' ) );
	}
	...
	protected function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', 'fincommerce' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	protected function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( __( 'Author: %s', 'fincommerce' ), $userdata->display_name ) );
	}
	...
	protected function search_trail() {
		if ( is_search() ) {
			...
			$this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', 'fincommerce' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	protected function paged_trail() {
		if ( get_query_var( 'paged' ) && 'subcategories' !== fincommerce_get_loop_display_mode() ) {
			/* translators: %d: page number */
			$this->add_crumb( sprintf( __( 'Page %d', 'fincommerce' ), get_query_var( 'paged' ) ) );
		}
	}
}
