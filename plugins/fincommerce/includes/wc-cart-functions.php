...
function wc_protected_product_add_to_cart( $passed, $product_id ) {
	if ( post_password_required( $product_id ) ) {
		$passed = false;
		wc_add_notice( __( 'This product is protected and cannot be purchased.', 'fincommerce' ), 'error' );
	}
	return $passed;
}
add_filter( 'fincommerce_add_to_cart_validation', 'wc_protected_product_add_to_cart', 10, 2 );
...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	foreach ( $products as $product_id => $qty ) {
		 ...
		 */
		$title = apply_filters( 'fincommerce_add_to_cart_qty_html', ( 1 !== $qty ? wc_stock_amount( $qty ) . ' &times; ' : '' ), $product_id );
		 ...
		 */
		$title .= apply_filters(
			'fincommerce_add_to_cart_item_name_in_quotes',
			sprintf(
				/* translators: %s: product name */
				_x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'fincommerce' ),
	...
	/* translators: %s: product name */
	$added_text = sprintf( _n( '%s has been added to your cart.', '%s have been added to your cart.', $count, 'fincommerce' ), wc_format_list_of_items( $titles ) );

	// Output success messages.
	$wp_button_class = wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '';
	if ( 'yes' === get_option( 'fincommerce_cart_redirect_after_add' ) ) {
		$return_to = apply_filters( 'fincommerce_continue_shopping_redirect', wc_get_raw_referer() ? wp_validate_redirect( wc_get_raw_referer(), false ) : wc_get_page_permalink( 'shop' ) );
		$message   = sprintf( '%s <a href="%s" class="button wc-forward%s">%s</a>', esc_html( $added_text ), esc_url( $return_to ), esc_attr( $wp_button_class ), esc_html__( 'Continue shopping', 'fincommerce' ) );
	} elseif ( ! CartCheckoutUtils::has_cart_page() ) {
		$message = sprintf( '%s', esc_html( $added_text ) );
	} else {
		$message = sprintf( '%s <a href="%s" class="button wc-forward%s">%s</a>', esc_html( $added_text ), esc_url( wc_get_cart_url() ), esc_attr( $wp_button_class ), esc_html__( 'View cart', 'fincommerce' ) );
	}
	...
	if ( $return ) {
		...
		wc_add_notice( $message, apply_filters( 'fincommerce_add_to_cart_notice_type', 'success' ) );
	}
}
...
function wc_format_list_of_items( $items ) {
	...
	foreach ( $items as $key => $item ) {
		...
		if ( count( $items ) === $key + 2 ) {
			$item_string .= ' ' . __( 'and', 'fincommerce' ) . ' ';
...
function wc_clear_cart_after_payment() {
	 ...
	 */
	$should_clear_cart_after_payment = apply_filters( 'fincommerce_should_clear_cart_after_payment', $should_clear_cart_after_payment );
...
function wc_cart_totals_shipping_html() {
	...
	foreach ( $packages as $i => $package ) {
		...
		if ( count( $packages ) > 1 ) {
			...
			$product_names = apply_filters( 'fincommerce_shipping_package_details_array', $product_names, $package );
		}
		...
		wc_get_template(
			...
			array(
				...
				'show_shipping_calculator' => is_cart() && apply_filters( 'fincommerce_shipping_show_shipping_calculator', $first, $i, $package ),
				'package_details'          => implode( ', ', $product_names ),
				/* translators: %d: shipping package number */
				'package_name'             => apply_filters( 'fincommerce_shipping_package_name', ( ( $i + 1 ) > 1 ) ? sprintf( _x( 'Shipping %d', 'shipping packages', 'fincommerce' ), ( $i + 1 ) ) : _x( 'Shipping', 'shipping packages', 'fincommerce' ), $i, $package ),
				'index'                    => $i,
...
function wc_cart_totals_coupon_html( $coupon ) {
	...
	if ( $coupon->get_free_shipping() && empty( $amount ) ) {
		$discount_amount_html = __( 'Free shipping coupon', 'fincommerce' );
	}

	$discount_amount_html = apply_filters( 'fincommerce_coupon_discount_amount_html', $discount_amount_html, $coupon );
	// translators: %s: coupon code.
	$coupon_html = $discount_amount_html . ' <a role="button" aria-label="' . esc_attr( sprintf( __( 'Remove %s coupon', 'fincommerce' ), $coupon->get_code() ) ) . '" href="' . esc_url( add_query_arg( 'remove_coupon', rawurlencode( $coupon->get_code() ), Constants::is_defined( 'fincommerce_CHECKOUT' ) ? wc_get_checkout_url() : wc_get_cart_url() ) ) . '" class="fincommerce-remove-coupon" data-coupon="' . esc_attr( $coupon->get_code() ) . '">' . __( '[Remove]', 'fincommerce' ) . '</a>';

	echo wp_kses( apply_filters( 'fincommerce_cart_totals_coupon_html', $coupon_html, $coupon, $discount_amount_html ), array_replace_recursive( wp_kses_allowed_html( 'post' ), array( 'a' => array( 'data-coupon' => true ) ) ) ); // phpcs:ignore PHPCompatibility.PHP.NewFunctions.array_replace_recursiveFound
}
...
function wc_cart_totals_order_total_html() {
	...
	if ( wc_tax_enabled() && WC()->cart->display_prices_including_tax() ) {
		...
		if ( get_option( 'fincommerce_tax_total_display' ) === 'itemized' ) {
		...
		if ( ! empty( $tax_string_array ) ) {
			...
			if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) {
				...
				$tax_text = wp_kses_post( sprintf( __( '(includes %1$s estimated for %2$s)', 'fincommerce' ), implode( ', ', $tax_string_array ), $country ) );
			} else {
				/* translators: %s: tax amount */
				$tax_text = wp_kses_post( sprintf( __( '(includes %s)', 'fincommerce' ), implode( ', ', $tax_string_array ) ) );
			}
...
function wc_cart_totals_shipping_method_label( $method ) {
	...
	return apply_filters( 'fincommerce_cart_shipping_method_full_label', $label, $method );
}
...
function wc_get_chosen_shipping_method_for_package( $key, $package ) {
	...
	if ( ! $chosen_method || $changed || ! isset( $package['rates'][ $chosen_method ] ) || count( $package['rates'] ) !== $method_count ) {
		 ...
		 */
		do_action( 'fincommerce_shipping_method_chosen', $chosen_method );
	}
...
function wc_get_default_shipping_method_for_package( $key, $package, $chosen_method ) {
	 ...
	 */
	return (string) apply_filters( 'fincommerce_shipping_chosen_method', $default, $package['rates'], $chosen_method );
}
 ...
 *
 * The fincommerce_cart_item_data_to_validate filter can be used to add custom properties.
 *
...
function wc_get_cart_item_data_hash( $product ) {
	return md5(
		wp_json_encode(
			apply_filters(
				'fincommerce_cart_item_data_to_validate',
				array(
