...
class WC_Order_Item_Meta {
	...
	public function display( $flat = false, $return = false, $hideprefix = '_', $delimiter = ", \n" ) {
		...
		$output = apply_filters( 'fincommerce_order_items_meta_display', $output, $this, $flat );
		...
		if ( ! empty( $this->item['item_meta_array'] ) ) {
			foreach ( $this->item['item_meta_array'] as $meta_id => $meta ) {
				...
				$formatted_meta[ $meta_id ] = array(
					...
					'value' => apply_filters( 'fincommerce_order_item_display_meta_value', $meta_value, $meta, $this->item ),
				);
			}
		}

		return apply_filters( 'fincommerce_order_items_meta_get_formatted', $formatted_meta, $this );
	}
		...
		foreach ( $this->meta as $meta_key => $meta_values ) {
			...
			foreach ( (array) $meta_values as $meta_value ) {
				...
				$formatted_meta[ $formatted_meta_key ] = array(
					'key'   => $meta_key,
					'label' => wc_attribute_label( $attribute_key, $this->product ),
					'value' => apply_filters( 'fincommerce_order_item_display_meta_value', $meta_value, $this->meta, $this->item ),
				);
			}
		}
