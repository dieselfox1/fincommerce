 ...
 *
 * Args and usage: https://github.com/dieselfox1/fincommerce/wiki/wc_get_products-and-WC_Product_Query
 *
 * @since  3.0.0
...
function wc_get_product( $the_product = false, $deprecated = array() ) {
	if ( ! did_action( 'fincommerce_init' ) || ! did_action( 'fincommerce_after_register_taxonomy' ) || ! did_action( 'fincommerce_after_register_post_type' ) ) {
		/* translators: 1: wc_get_product 2: fincommerce_init 3: fincommerce_after_register_taxonomy 4: fincommerce_after_register_post_type */
		wc_doing_it_wrong( __FUNCTION__, sprintf( __( '%1$s should not be called before the %2$s, %3$s and %4$s actions have finished.', 'fincommerce' ), 'wc_get_product', 'fincommerce_init', 'fincommerce_after_register_taxonomy', 'fincommerce_after_register_post_type' ), '3.9' );
		return false;
	}
	if ( ! empty( $deprecated ) ) {
...
function wc_delete_product_transients( $post_id = 0 ) {
	...
	do_action( 'fincommerce_delete_product_transients', $post_id );
}
	...
	if ( ! empty( $terms ) ) {
		...
		if ( $category_object->parent ) {
			...
			foreach ( $ancestors as $ancestor ) {
				...
				if ( apply_filters( 'fincommerce_product_post_type_link_parent_category_only', false ) ) {
		...
		$product_cat = _x( 'uncategorized', 'slug', 'fincommerce' );
	}
 ...
 * @return string
 */
function wc_placeholder_img_src( $size = 'fincommerce_thumbnail' ) {
	...
	return apply_filters( 'fincommerce_placeholder_img_src', $src );
}
 ...
 * @return string
 */
function wc_placeholder_img( $size = 'fincommerce_thumbnail', $attr = '' ) {
	$dimensions        = wc_get_image_size( $size );
	$placeholder_image = get_option( 'fincommerce_placeholder_image', 0 );

	$default_attr = array(
		'class' => 'fincommerce-placeholder wp-post-image',
	...
	return apply_filters( 'fincommerce_placeholder_img', $image_html, $size, $dimensions );
}
...
add_action( 'fincommerce_scheduled_sales', 'wc_scheduled_sales' );
...
function wc_get_attachment_image_attributes( $attr ) {
	/*
	 * If the user can manage fincommerce, allow them to
	 ...
	 * image.
	 */
	if ( isset( $attr['src'] ) && strstr( $attr['src'], 'fincommerce_uploads/' ) ) {
...
function wc_prepare_attachment_for_js( $response ) {
	 ...
	 * image.
	 */
	if ( isset( $response['url'] ) && strstr( $response['url'], 'fincommerce_uploads/' ) ) {
...
function wc_track_product_view() {
	if ( ! is_singular( 'product' ) || ! is_active_widget( false, false, 'fincommerce_recently_viewed_products', true ) ) {
		return;
	}

	global $post;

	if ( empty( $_COOKIE['fincommerce_recently_viewed'] ) ) { // @codingStandardsIgnoreLine.
		$viewed_products = array();
	} else {
		$viewed_products = wp_parse_id_list( (array) explode( '|', wp_unslash( $_COOKIE['fincommerce_recently_viewed'] ) ) ); // @codingStandardsIgnoreLine.
	}
	...
	wc_setcookie( 'fincommerce_recently_viewed', implode( '|', $viewed_products ) );
}
...
function wc_get_product_types() {
	return (array) apply_filters(
		...
		array(
			ProductType::SIMPLE   => __( 'Simple product', 'fincommerce' ),
			ProductType::GROUPED  => __( 'Grouped product', 'fincommerce' ),
			ProductType::EXTERNAL => __( 'External/Affiliate product', 'fincommerce' ),
			ProductType::VARIABLE => __( 'Variable product', 'fincommerce' ),
		)
	);
}
...
function wc_get_product_attachment_props( $attachment_id = null, $product = false ) {
	...
	if ( $attachment && 'attachment' === $attachment->post_type ) {
		...
		$gallery_thumbnail_size           = apply_filters( 'fincommerce_gallery_thumbnail_size', array( $gallery_thumbnail['width'], $gallery_thumbnail['height'] ) );
		...
		// Thumbnail version.
		$thumbnail_size       = apply_filters( 'fincommerce_thumbnail_size', 'fincommerce_thumbnail' );
		...
		// Image source.
		$image_size      = apply_filters( 'fincommerce_gallery_image_size', 'fincommerce_single' );
...
function wc_get_product_visibility_options() {
	return apply_filters(
		'fincommerce_product_visibility_options',
		array(
			CatalogVisibility::VISIBLE => __( 'Shop and search results', 'fincommerce' ),
			CatalogVisibility::CATALOG => __( 'Shop only', 'fincommerce' ),
			CatalogVisibility::SEARCH  => __( 'Search results only', 'fincommerce' ),
			CatalogVisibility::HIDDEN  => __( 'Hidden', 'fincommerce' ),
		)
	);
}
...
function wc_get_product_tax_class_options() {
	...
	$tax_class_options[''] = __( 'Standard', 'fincommerce' );
...
function wc_get_product_stock_status_options() {
	return apply_filters(
		'fincommerce_product_stock_status_options',
		array(
			ProductStockStatus::IN_STOCK     => __( 'In stock', 'fincommerce' ),
			ProductStockStatus::OUT_OF_STOCK => __( 'Out of stock', 'fincommerce' ),
			ProductStockStatus::ON_BACKORDER => __( 'On backorder', 'fincommerce' ),
		)
	);
}
...
function wc_get_product_backorder_options() {
	return array(
		'no'     => __( 'Do not allow', 'fincommerce' ),
		'notify' => __( 'Allow, but notify customer', 'fincommerce' ),
		'yes'    => __( 'Allow', 'fincommerce' ),
	);
}
...
function wc_get_related_products( $product_id, $limit = 5, $exclude_ids = array(), $related_by = array() ) {
	...
	if ( false === $related_posts || count( $related_posts ) < $limit ) {
		...
		$cats_array = apply_filters( 'fincommerce_product_related_posts_relate_by_category', true, $product_id ) ? apply_filters( 'fincommerce_get_related_product_cat_terms', wc_get_product_term_ids( $product_id, 'product_cat' ), $product_id ) : array();
		$tags_array = apply_filters( 'fincommerce_product_related_posts_relate_by_tag', true, $product_id ) ? apply_filters( 'fincommerce_get_related_product_tag_terms', wc_get_product_term_ids( $product_id, 'product_tag' ), $product_id ) : array();

		// Don't bother if none are set, unless fincommerce_product_related_posts_force_display is set to true in which case all products are related.
		if ( empty( $cats_array ) && empty( $tags_array ) && ! apply_filters( 'fincommerce_product_related_posts_force_display', false, $product_id ) ) {
			$related_posts = array();
		} else {
			$data_store    = WC_Data_Store::load( 'product' );
	...
	if ( apply_filters( 'fincommerce_product_related_posts_shuffle', true ) ) {
...
function wc_get_price_including_tax( $product, $args = array() ) {
	...
	if ( $product->is_taxable() ) {
		if ( ! wc_prices_include_tax() ) {
			 ...
			 * Either remove the base or the user taxes depending on fincommerce_adjust_non_base_location_prices setting.
			 */
			if ( ! empty( WC()->customer ) && WC()->customer->get_is_vat_exempt() ) { // @codingStandardsIgnoreLine.
				$remove_taxes = apply_filters( 'fincommerce_adjust_non_base_location_prices', true ) ? WC_Tax::calc_tax( $line_price, $base_tax_rates, true ) : WC_Tax::calc_tax( $line_price, $tax_rates, true );
				...
				/**
			 * The fincommerce_adjust_non_base_location_prices filter can stop base taxes being taken off when dealing with out of base locations.
			 * e.g. If a product costs 10 including tax, all users will pay 10 regardless of location and taxes.
			 * This feature is experimental @since 2.4.7 and may change in the future. Use at your risk.
			 */
			} elseif ( $tax_rates !== $base_tax_rates && apply_filters( 'fincommerce_adjust_non_base_location_prices', true ) ) {
	...
	return apply_filters( 'fincommerce_get_price_including_tax', $return_price, $qty, $product );
}
...
function wc_get_price_excluding_tax( $product, $args = array() ) {
	...
	if ( $product->is_taxable() && wc_prices_include_tax() ) {
		...
		if ( apply_filters( 'fincommerce_adjust_non_base_location_prices', true ) ) {
	...
	return apply_filters( 'fincommerce_get_price_excluding_tax', $return_price, $qty, $product );
}

/**
 * Returns the price including or excluding tax.
 *
 * By default it's based on the 'fincommerce_tax_display_shop' setting.
 * Set `$arg['display_context']` to 'cart' to base on the 'fincommerce_tax_display_cart' setting instead.
 *
 * @since  3.0.0
...
function wc_get_price_to_display( $product, $args = array() ) {
	...
	$tax_display = get_option(
		'cart' === $args['display_context'] ? 'fincommerce_tax_display_cart' : 'fincommerce_tax_display_shop'
	);
...
function wc_update_product_lookup_tables() {
	...
	update_option( 'fincommerce_product_lookup_table_is_generating', true );
	...
	$columns = array(
		...
		'onsale',
		'tax_class',
		'tax_status', // When last column is updated, fincommerce_product_lookup_table_is_generating is updated.
	);
...
function wc_update_product_lookup_tables_column( $column ) {
	...
	if ( 'tax_status' === $column ) {
		delete_option( 'fincommerce_product_lookup_table_is_generating' );
	}
}
