<?php
/**
 * Contains the query functions for FinCommerce which alter the front-end post queries and loops
 *
 * @version 3.2.0
 * @package FinCommerce\Classes
 */

use Automattic\FinCommerce\Internal\ProductAttributesLookup\Filterer;
...
class WC_Query {
	...
	public function init_query_vars() {
		...
		$this->query_vars = array(
			// Checkout actions.
			'order-pay'                  => get_option( 'fincommerce_checkout_pay_endpoint', 'order-pay' ),
			'order-received'             => get_option( 'fincommerce_checkout_order_received_endpoint', 'order-received' ),
			// My account actions.
			'orders'                     => get_option( 'fincommerce_myaccount_orders_endpoint', 'orders' ),
			'view-order'                 => get_option( 'fincommerce_myaccount_view_order_endpoint', 'view-order' ),
			...
			'lost-password'              => get_option( 'fincommerce_myaccount_lost_password_endpoint', 'lost-password' ),
			'customer-logout'            => get_option( 'fincommerce_logout_endpoint', 'customer-logout' ),
			'add-payment-method'         => get_option( 'fincommerce_myaccount_add_payment_method_endpoint', 'add-payment-method' ),
			'delete-payment-method'      => get_option( 'fincommerce_myaccount_delete_payment_method_endpoint', 'delete-payment-method' ),
			'set-default-payment-method' => get_option( 'fincommerce_myaccount_set_default_payment_method_endpoint', 'set-default-payment-method' ),
		);
	}
	...
	public function get_endpoint_title( $endpoint, $action = '' ) {
		...
		switch ( $endpoint ) {
			case 'order-pay':
				$title = __( 'Pay for order', 'fincommerce' );
				break;
			case 'order-received':
			...
			case 'orders':
				if ( ! empty( $wp->query_vars['orders'] ) ) {
					/* translators: %s: page */
					$title = sprintf( __( 'Orders (page %d)', 'fincommerce' ), intval( $wp->query_vars['orders'] ) );
				} else {
			...
			case 'view-order':
				...
				$title = ( $order ) ? sprintf( __( 'Order #%s', 'fincommerce' ), $order->get_order_number() ) : '';
				break;
			case 'downloads':
			...
			case 'lost-password':
				if ( in_array( $action, array( 'rp', 'resetpass', 'newaccount' ), true ) ) {
					$title = __( 'Set password', 'fincommerce' );
				} else {
		 ...
		 * @param string $action Optional action or variation within the endpoint.
		 */
		return apply_filters( 'fincommerce_endpoint_' . $endpoint . '_title', $title, $endpoint, $action );
	}
	...
	public function get_endpoints_mask() {
		if ( 'page' === get_option( 'show_on_front' ) ) {
			...
			$myaccount_page_id = get_option( 'fincommerce_myaccount_page_id' );
			$checkout_page_id  = get_option( 'fincommerce_checkout_page_id' );
	...
	public function get_query_vars() {
		return apply_filters( 'fincommerce_get_query_vars', $this->query_vars );
	}
	...
	public function pre_get_posts( $q ) {
		...
		if ( wc_current_theme_supports_fincommerce_or_fse() && $q->is_page() && 'page' === get_option( 'show_on_front' ) && absint( $q->get( 'page_id' ) ) === wc_get_page_id( 'shop' ) ) {
	...
	/**
	 * Pre_get_posts above may adjust the main query to add FinCommerce logic. When this query is done, we need to ensure
	...
	public function product_query( $q ) {
		...
		do_action( 'fincommerce_product_query', $q, $this );
	}
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		if ( ! $orderby ) {
			...
			if ( ! $orderby_value ) {
				if ( is_search() ) {
					...
					$orderby_value = apply_filters( 'fincommerce_default_catalog_orderby', get_option( 'fincommerce_default_catalog_orderby', 'menu_order' ) );
				}
			}
		...
		return apply_filters( 'fincommerce_get_catalog_ordering_args', $args, $orderby, $order );
	}
	...
	public function price_filter_post_clauses( $args, $wp_query ) {
		 ...
		 */
		if ( wc_tax_enabled() && 'incl' === get_option( 'fincommerce_tax_display_shop' ) && ! wc_prices_include_tax() ) {
			$tax_class = apply_filters( 'fincommerce_price_filter_widget_tax_class', '' ); // Uses standard tax class.
	...
	public function get_meta_query( $meta_query = array(), $main_query = false ) {
		...
		return array_filter( apply_filters( 'fincommerce_product_query_meta_query', $meta_query, $this ) );
	}
	...
	public function get_tax_query( $tax_query = array(), $main_query = false ) {
		...
		if ( 'yes' === get_option( 'fincommerce_hide_out_of_stock_items' ) ) {
		...
		return array_filter( apply_filters( 'fincommerce_product_query_tax_query', $tax_query, $this ) );
	}
			...
			if ( ! empty( $_GET ) ) {
				foreach ( $_GET as $key => $value ) {
					if ( 0 === strpos( $key, 'filter_' ) ) {
						...
						self::$chosen_attributes[ $taxonomy ]['query_type'] = $query_type ? $query_type : apply_filters( 'fincommerce_layered_nav_default_query_type', 'and' );
					}
				}
			}
		}
