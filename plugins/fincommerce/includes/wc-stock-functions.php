...
function wc_update_product_stock( $product, $stock_quantity = null, $operation = 'set', $updating = false ) {
	...
	if ( ! is_null( $stock_quantity ) && $product->managing_stock() ) {
		...
		if ( $product_with_stock->is_type( ProductType::VARIATION ) ) {
			...
			do_action( 'fincommerce_variation_before_set_stock', $product_with_stock );
		} else {
			// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingSinceComment
			/** This action is documented in includes/data-stores/class-wc-product-data-store-cpt.php */
			do_action( 'fincommerce_product_before_set_stock', $product_with_stock );
		}
		...
		if ( $product_with_stock->is_type( ProductType::VARIATION ) ) {
			// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingSinceComment
			/** This action is documented in includes/data-stores/class-wc-product-data-store-cpt.php */
			do_action( 'fincommerce_variation_set_stock', $product_with_stock );
		} else {
			// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingSinceComment
			/** This action is documented in includes/data-stores/class-wc-product-data-store-cpt.php */
			do_action( 'fincommerce_product_set_stock', $product_with_stock );
		}
...
function wc_maybe_reduce_stock_levels( $order_id ) {
	...
	$trigger_reduce = apply_filters( 'fincommerce_payment_complete_reduce_order_stock', ! $stock_reduced, $order_id );
...
add_action( 'fincommerce_payment_complete', 'wc_maybe_reduce_stock_levels' );
add_action( 'fincommerce_order_status_completed', 'wc_maybe_reduce_stock_levels' );
add_action( 'fincommerce_order_status_processing', 'wc_maybe_reduce_stock_levels' );
add_action( 'fincommerce_order_status_on-hold', 'wc_maybe_reduce_stock_levels' );
...
add_action( 'fincommerce_order_status_cancelled', 'wc_maybe_increase_stock_levels' );
add_action( 'fincommerce_order_status_pending', 'wc_maybe_increase_stock_levels' );
...
function wc_reduce_stock_levels( $order_id ) {
	...
	if ( ! $order || 'yes' !== get_option( 'fincommerce_manage_stock' ) || ! apply_filters( 'fincommerce_can_reduce_order_stock', true, $order ) ) {
		return;
	}
	...
	foreach ( $order->get_items() as $item ) {
		 ...
		 */
		$qty       = apply_filters( 'fincommerce_order_item_quantity', $item->get_quantity(), $order, $item );
		$item_name = $product->get_formatted_name();
		$new_stock = wc_update_product_stock( $product, $qty, 'decrease' );

		if ( is_wp_error( $new_stock ) ) {
			/* translators: %s item name. */
			$order->add_order_note( sprintf( __( 'Unable to reduce stock for item %s.', 'fincommerce' ), $item_name ) );
			continue;
		}
		 ...
		 * @since 7.6.0
		 */
		do_action( 'fincommerce_reduce_order_item_stock', $item, $change, $order );
	}

	wc_trigger_stock_change_notifications( $order, $changes );

	do_action( 'fincommerce_reduce_order_stock', $order );
}
...
function wc_trigger_stock_change_notifications( $order, $changes ) {
	...
	$order_notes     = array();
	$no_stock_amount = absint( get_option( 'fincommerce_notify_no_stock_amount', 0 ) );
	...
	foreach ( $changes as $change ) {
		...
		if ( $change['to'] <= $no_stock_amount ) {
			 ...
			 */
			do_action( 'fincommerce_no_stock', wc_get_product( $change['product']->get_id() ) );
			 ...
			 */
			do_action( 'fincommerce_low_stock', wc_get_product( $change['product']->get_id() ) );
		}
	...
	$order->add_order_note( __( 'Stock levels reduced:', 'fincommerce' ) . ' ' . implode( ', ', $order_notes ) );
}
...
function wc_trigger_stock_change_actions( $product ) {
	...
	if ( $stock_quantity <= $no_stock_amount ) {
		 ...
		 */
		do_action( 'fincommerce_no_stock', $product );
		 ...
		 * @param WC_Product $product The product whose stock quantity has changed.
		 */
		do_action( 'fincommerce_low_stock', $product );
	}
}
...
function wc_increase_stock_levels( $order_id ) {
	...
	// We need an order, and a store with stock management to continue.
	if ( ! $order || 'yes' !== get_option( 'fincommerce_manage_stock' ) || ! apply_filters( 'fincommerce_can_restore_order_stock', true, $order ) ) {
		return;
	}
	...
	foreach ( $order->get_items() as $item ) {
		...
		if ( is_wp_error( $new_stock ) ) {
			/* translators: %s item name. */
			$order->add_order_note( sprintf( __( 'Unable to restore stock for item %s.', 'fincommerce' ), $item_name ) );
			continue;
		}
		 ...
		 * @param WC_Order $order  Order data.
		 */
		do_action( 'fincommerce_restore_order_item_stock', $item, $new_stock, $old_stock, $order );
	}

	if ( $changes ) {
		$order->add_order_note( __( 'Stock levels increased:', 'fincommerce' ) . ' ' . implode( ', ', $changes ) );
	}

	do_action( 'fincommerce_restore_order_stock', $order );
}
...
function wc_get_held_stock_quantity( WC_Product $product, $exclude_order_id = 0 ) {
	/**
	 * Filter: fincommerce_hold_stock_for_checkout
	 * Allows enable/disable hold stock functionality on checkout.
	 *
	 * @since 4.3.0
	 * @param bool $enabled Default to true if managing stock globally.
	 */
	if ( ! apply_filters( 'fincommerce_hold_stock_for_checkout', wc_string_to_bool( get_option( 'fincommerce_manage_stock', 'yes' ) ) ) ) {
		return 0;
	}
...
add_action( 'fincommerce_checkout_order_created', 'wc_reserve_stock_for_order' );
...
add_action( 'fincommerce_checkout_order_exception', 'wc_release_stock_for_order' );
add_action( 'fincommerce_payment_complete', 'wc_release_stock_for_order', 11 );
add_action( 'fincommerce_order_status_cancelled', 'wc_release_stock_for_order', 11 );
add_action( 'fincommerce_order_status_completed', 'wc_release_stock_for_order', 11 );
add_action( 'fincommerce_order_status_processing', 'wc_release_stock_for_order', 11 );
add_action( 'fincommerce_order_status_on-hold', 'wc_release_stock_for_order', 11 );
...
function wc_get_low_stock_amount( WC_Product $product ) {
	...
	if ( '' === $low_stock_amount ) {
		$low_stock_amount = get_option( 'fincommerce_notify_low_stock_amount', 2 );
	}
