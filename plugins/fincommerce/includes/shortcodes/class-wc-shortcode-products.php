 ...
 * Products shortcode
 *
 * @package  FinCommerce\Shortcodes
 * @version  3.2.4
 */
...
class WC_Shortcode_Products {
	...
	protected function parse_query_args() {
		...
		$query_args = apply_filters( 'fincommerce_shortcode_products_query', $query_args, $this->attributes, $this->type );
	...
	protected function get_transient_name() {
		...
		if ( 'rand' === $this->query_args['orderby'] ) {
			...
			$rand_index      = wp_rand( 0, max( 1, absint( apply_filters( 'fincommerce_product_query_max_rand_cache_count', 5 ) ) ) );
	...
	protected function get_query_results() {
		 ...
		 */
		return apply_filters( 'fincommerce_shortcode_products_query_results', $results, $this );
	}
	...
	protected function product_loop() {
		...
		if ( $products && $products->ids ) {
			...
			do_action( "fincommerce_shortcode_before_{$this->type}_loop", $this->attributes );
			...
			if ( wc_get_loop_prop( 'total' ) ) {
				foreach ( $products->ids as $product_id ) {
					...
					add_action( 'fincommerce_product_is_visible', array( $this, 'set_product_as_visible' ) );

					// Render product template.
					wc_get_template_part( 'content', 'product' );

					// Restore product visibility.
					remove_action( 'fincommerce_product_is_visible', array( $this, 'set_product_as_visible' ) );
				}
			}
			...
			do_action( "fincommerce_shortcode_after_{$this->type}_loop", $this->attributes );

			wp_reset_postdata();
			wc_reset_loop();
		} else {
			do_action( "fincommerce_shortcode_{$this->type}_loop_no_results", $this->attributes );
		}
