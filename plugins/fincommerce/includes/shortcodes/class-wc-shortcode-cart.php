 ...
 *
 * @package FinCommerce\Shortcodes\Cart
 * @version 2.3.0
 */
...
class WC_Shortcode_Cart {
	...
	public static function calculate_shipping() {
		try {
			...
			$address = apply_filters( 'fincommerce_cart_calculate_shipping_address', $address );

			if ( $address['postcode'] && ! WC_Validation::is_postcode( $address['postcode'], $address['country'] ) ) {
				throw new Exception( __( 'Please enter a valid postcode / ZIP.', 'fincommerce' ) );
			}
			...
			wc_add_notice( __( 'Shipping costs updated.', 'fincommerce' ), 'notice' );

			do_action( 'fincommerce_calculated_shipping' );
	...
	public static function output( $atts ) {
		if ( ! apply_filters( 'fincommerce_output_cart_shortcode_content', true ) ) {
			return;
		}
		...
		$atts        = shortcode_atts( array(), $atts, 'fincommerce_cart' );
		$nonce_value = wc_get_var( $_REQUEST['fincommerce-shipping-calculator-nonce'], wc_get_var( $_REQUEST['_wpnonce'], '' ) ); // @codingStandardsIgnoreLine.

		// Update Shipping. Nonce check uses new value and old value (fincommerce-cart). @todo remove in 4.0.
		if ( ! empty( $_POST['calc_shipping'] ) && ( wp_verify_nonce( $nonce_value, 'fincommerce-shipping-calculator' ) || wp_verify_nonce( $nonce_value, 'fincommerce-cart' ) ) ) { // WPCS: input var ok.
