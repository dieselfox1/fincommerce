 ...
 *
 * @package FinCommerce\Shortcodes\Checkout
 * @version 2.0.0
 */
...
class WC_Shortcode_Checkout {
	...
	private static function order_pay( $order_id ) {
		...
		do_action( 'before_fincommerce_pay' );
		...
		if ( isset( $_GET['pay_for_order'], $_GET['key'] ) && $order_id ) { // WPCS: input var ok, CSRF ok.
			try {
				...
				if ( ! $order || $order->get_id() !== $order_id || ! hash_equals( $order->get_order_key(), $order_key ) ) {
					throw new Exception( __( 'Sorry, this order is invalid and cannot be paid for.', 'fincommerce' ) );
				}

				// Logged out customer does not have permission to pay for this order.
				if ( ! current_user_can( 'pay_for_order', $order_id ) && ! is_user_logged_in() ) {
					wc_print_notice( esc_html__( 'Please log in to your account below to continue to the payment form.', 'fincommerce' ), 'notice' );
				...
				if ( ! $order->get_user_id() && is_user_logged_in() ) {
					...
					if ( $order->get_billing_email() !== wp_get_current_user()->user_email ) {
						wc_print_notice( __( 'You are paying for a guest order. Please continue with payment only if you recognize this order.', 'fincommerce' ), 'error' );
					}
				}

				// Logged in customer trying to pay for someone else's order.
				if ( ! current_user_can( 'pay_for_order', $order_id ) ) {
					throw new Exception( __( 'This order cannot be paid for. Please contact us if you need assistance.', 'fincommerce' ) );
				}

				// Does not need payment.
				if ( ! $order->needs_payment() ) {
					/* translators: %s: order status */
					throw new Exception( sprintf( __( 'This order&rsquo;s status is &ldquo;%s&rdquo;&mdash;it cannot be paid for. Please contact us if you need assistance.', 'fincommerce' ), wc_get_order_status_name( $order->get_status() ) ) );
				}
						...
						foreach ( $order->get_items() as $item_key => $item ) {
							if ( $item && is_callable( array( $item, 'get_product' ) ) ) {
								...
								if ( ! apply_filters( 'fincommerce_pay_order_product_in_stock', $product->is_in_stock(), $product, $order ) ) {
									...
									throw new Exception( sprintf( __( 'Sorry, "%s" is no longer in stock so this order cannot be paid for. We apologize for any inconvenience caused.', 'fincommerce' ), $product->get_name() ) );
								}
								...
								if ( ! apply_filters( 'fincommerce_pay_order_product_has_enough_stock', ( $product->get_stock_quantity() >= ( $held_stock + $required_stock ) ), $product, $order ) ) {
									...
									throw new Exception( sprintf( __( 'Sorry, we do not have enough "%1$s" in stock to fulfill your order (%2$s available). We apologize for any inconvenience caused.', 'fincommerce' ), $product->get_name(), wc_format_stock_quantity_for_display( $product->get_stock_quantity() - $held_stock, $product ) ) );
								}
							}
						}
					}
				 ...
				 * @since 3.0.2
				 */
				$order_button_text = apply_filters( 'fincommerce_pay_order_button_text', __( 'Pay for order', 'fincommerce' ) );
				 ...
				 * @since 6.6
				 */
				do_action( 'before_fincommerce_pay_form', $order, $order_button_text, $available_gateways );
			...
			if ( $order && $order->get_id() === $order_id && hash_equals( $order->get_order_key(), $order_key ) ) {
				...
				if ( $order->needs_payment() ) {
					...
					/* translators: %s: order status */
					wc_print_notice( sprintf( __( 'This order&rsquo;s status is &ldquo;%s&rdquo;&mdash;it cannot be paid for. Please contact us if you need assistance.', 'fincommerce' ), wc_get_order_status_name( $order->get_status() ) ), 'error' );
				}
			} else {
				wc_print_notice( __( 'Sorry, this order is invalid and cannot be paid for.', 'fincommerce' ), 'error' );
			}
		} else {
			wc_print_notice( __( 'Invalid order.', 'fincommerce' ), 'error' );
		}

		do_action( 'after_fincommerce_pay' );
	}
	...
	private static function order_received( $order_id = 0 ) {
		...
		// Get the order.
		$order_id  = apply_filters( 'fincommerce_thankyou_order_id', absint( $order_id ) );
		$order_key = apply_filters( 'fincommerce_thankyou_order_key', empty( $_GET['key'] ) ? '' : wc_clean( wp_unslash( $_GET['key'] ) ) ); // WPCS: input var ok, CSRF ok.
		 ...
		 * @since 8.4.0
		 */
		$verify_known_shoppers = apply_filters( 'fincommerce_order_received_verify_known_shoppers', true );
		...
		if ( $verify_known_shoppers && $order_customer_id && get_current_user_id() !== $order_customer_id ) {
			...
			wc_print_notice( esc_html__( 'Please log in to your account to view this order.', 'fincommerce' ), 'notice' );
			fincommerce_login_form( array( 'redirect' => $order->get_checkout_order_received_url() ) );
			return;
		}
	...
	private static function checkout() {
		...
		do_action( 'fincommerce_before_checkout_form_cart_notices' );

		// Check cart has contents.
		if ( WC()->cart->is_empty() && ! is_customize_preview() && apply_filters( 'fincommerce_checkout_redirect_empty_cart', true ) ) {
			return;
		}
		...
		if ( empty( $_POST ) && wc_notice_count( 'error' ) > 0 ) { // WPCS: input var ok, CSRF ok.
			...
			$non_js_checkout = ! empty( $_POST['fincommerce_checkout_update_totals'] ); // WPCS: input var ok, CSRF ok.

			if ( wc_notice_count( 'error' ) === 0 && $non_js_checkout ) {
				wc_add_notice( __( 'The order totals have been updated. Please confirm your order by pressing the "Place order" button at the bottom of the page.', 'fincommerce' ) );
			}
