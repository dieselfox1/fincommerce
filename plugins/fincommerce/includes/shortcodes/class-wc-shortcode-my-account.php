 ...
 *
 * @package FinCommerce\Shortcodes\My_Account
 * @version 2.0.0
 */
...
class WC_Shortcode_My_Account {
	 ...
	 * logged out. See `fincommerce_my_account_message`.
	 */
	private static function my_account_add_notices() {
		...
		if ( ! is_user_logged_in() ) {
			 ...
			 * @since 2.6.0
			 */
			$message = apply_filters( 'fincommerce_my_account_message', '' );
		...
		if ( get_user_option( 'default_password_nag' ) && ( wc_is_current_account_menu_item( 'dashboard' ) || wc_is_current_account_menu_item( 'edit-account' ) ) ) {
			wc_add_notice(
				sprintf(
					...
					__( 'Your account with %s is using a temporary password. We emailed you a link to change your password.', 'fincommerce' ),
	...
	public static function view_order( $order_id ) {
		...
		if ( ! $order || ! current_user_can( 'view_order', $order_id ) ) {
			wc_print_notice(
				esc_html__( 'Invalid order.', 'fincommerce' ) . ' <a href="' . esc_url( wc_get_page_permalink( 'myaccount' ) ) . '" class="wc-forward">' . esc_html__( 'My account', 'fincommerce' ) . '</a>',
				'error'
			);
			return;
		}
	...
	public static function edit_address( $load_address = 'billing' ) {
		...
		foreach ( $address as $key => $field ) {
			...
			$address[ $key ]['value'] = apply_filters( 'fincommerce_my_account_edit_address_field_value', $value, $key, $load_address );
		}
		...
		wc_get_template(
			...
			array(
				...
				'address'      => apply_filters( 'fincommerce_address_to_edit', $address, $load_address ),
			)
		);
	}
	...
	public static function retrieve_password() {
		...
		if ( empty( $login ) ) {
			...
			wc_add_notice( __( 'Enter a username or email address.', 'fincommerce' ), 'error' );
		...
		if ( ! $user_data && is_email( $login ) && apply_filters( 'fincommerce_get_username_from_email', true ) ) {
		...
		if ( ! $user_data ) {
			wc_add_notice( __( 'Invalid username or email.', 'fincommerce' ), 'error' );
		...
		if ( ! $allow ) {
			...
			wc_add_notice( __( 'Password reset is not allowed for this user', 'fincommerce' ), 'error' );
		...
		do_action( 'fincommerce_reset_password_notification', $user_login, $key );
	...
	public static function check_password_reset_key( $key, $login ) {
		...
		if ( is_wp_error( $user ) ) {
			wc_add_notice( __( 'This key is invalid or has already been used. Please reset your password again if needed.', 'fincommerce' ), 'error' );
			return false;
		}
	...
	public static function reset_password( $user, $new_pass ) {
		...
		// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		if ( ! apply_filters( 'fincommerce_disable_password_change_notification', false ) ) {
	...
	public static function add_payment_method() {
		if ( ! is_user_logged_in() ) {
			...
			do_action( 'before_fincommerce_add_payment_method' );

			wc_get_template( 'myaccount/form-add-payment-method.php' );

			do_action( 'after_fincommerce_add_payment_method' );
		}
	}
}
