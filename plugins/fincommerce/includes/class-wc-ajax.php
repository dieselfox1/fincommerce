<?php
/**
 * FinCommerce WC_AJAX. AJAX Event Handlers.
 *
 * @class   WC_AJAX
...
use Automattic\FinCommerce\Internal\Orders\CouponsController;
use Automattic\FinCommerce\Internal\Orders\TaxesController;
use Automattic\FinCommerce\Internal\Admin\Orders\MetaBoxes\CustomMetaBox;
use Automattic\FinCommerce\Internal\Utilities\Users;
use Automattic\FinCommerce\Proxies\LegacyProxy;
use Automattic\FinCommerce\Utilities\ArrayUtil;
...
class WC_AJAX {
	...
	public static function get_endpoint( $request = '' ) {
		return esc_url_raw( apply_filters( 'fincommerce_ajax_get_endpoint', add_query_arg( 'wc-ajax', $request, remove_query_arg( array( 'remove_item', 'add-to-cart', 'added-to-cart', 'order_again', '_wpnonce' ), home_url( '/', 'relative' ) ) ), $request ) );
	}
	...
	public static function add_ajax_events() {
		...
		foreach ( $ajax_events_nopriv as $ajax_event ) {
			add_action( 'wp_ajax_fincommerce_' . $ajax_event, array( __CLASS__, $ajax_event ) );
			add_action( 'wp_ajax_nopriv_fincommerce_' . $ajax_event, array( __CLASS__, $ajax_event ) );
		...
		foreach ( $ajax_private_events as $ajax_event ) {
			add_action(
				'wp_ajax_fincommerce_' . $ajax_event,
	...
	public static function get_refreshed_fragments() {
		...
		fincommerce_mini_cart();

		$mini_cart = ob_get_clean();

		$data = array(
			'fragments' => apply_filters(
				'fincommerce_add_to_cart_fragments',
				array(
	...
	public static function remove_coupon() {
		...
		if ( StringUtil::is_null_or_whitespace( $coupon ) ) {
			...
			wc_add_notice( __( 'Coupon has been removed.', 'fincommerce' ) );
		}
	...
	public static function update_shipping_method() {
		...
		wc_maybe_define_constant( 'fincommerce_CART', true );
	...
	private static function update_order_review_expired() {
		wp_send_json(
			array(
				'fragments' => apply_filters(
					'fincommerce_update_order_review_fragments',
					array(
						'form.fincommerce-checkout' => wc_print_notice(
							esc_html__( 'Sorry, your session has expired.', 'fincommerce' ) . ' <a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="wc-backward">' . esc_html__( 'Return to shop', 'fincommerce' ) . '</a>',
							'error',
							array(),
							true
						),
	...
	public static function update_order_review() {
		...
		wc_maybe_define_constant( 'fincommerce_CHECKOUT', true );

		if ( WC()->cart->is_empty() && ! is_customize_preview() && apply_filters( 'fincommerce_checkout_update_order_review_expired', true ) ) {
		...
		ob_start();
		fincommerce_order_review();
		$fincommerce_order_review = ob_get_clean();

		// Get checkout payment fragment.
		ob_start();
		fincommerce_checkout_payment();
		$fincommerce_checkout_payment = ob_get_clean();
		...
		wp_send_json(
			array(
				...
				'fragments' => apply_filters(
					'fincommerce_update_order_review_fragments',
					array(
						'.fincommerce-checkout-review-order-table' => $fincommerce_order_review,
						'.fincommerce-checkout-payment' => $fincommerce_checkout_payment,
					)
				),
			)
		);
	...
	public static function add_to_cart() {
		...
		$product_id        = apply_filters( 'fincommerce_add_to_cart_product_id', absint( $_POST['product_id'] ) );
		...
		if ( $passed_validation && false !== WC()->cart->add_to_cart( $product_id, $quantity, $variation_id, $variation ) && ProductStatus::PUBLISH === $product_status ) {
			...
			do_action( 'fincommerce_ajax_added_to_cart', $product_id );

			if ( 'yes' === get_option( 'fincommerce_cart_redirect_after_add' ) ) {
			...
			$data = array(
				'error'       => true,
				'product_url' => apply_filters( 'fincommerce_cart_redirect_after_error', get_permalink( $product_id ), $product_id ),
			);
	...
	public static function feature_product() {
		if ( current_user_can( 'edit_products' ) && check_admin_referer( 'fincommerce-feature-product' ) && isset( $_GET['product_id'] ) ) {
	...
	public static function mark_order_status() {
		if ( current_user_can( 'edit_shop_orders' ) && check_admin_referer( 'fincommerce-mark-order-status' ) && isset( $_GET['status'], $_GET['order_id'] ) ) {
			...
			if ( wc_is_order_status( 'wc-' . $status ) && $order ) {
				...
				do_action( 'fincommerce_order_edit_status', $order->get_id(), $status );
			}
		}
	...
	public static function get_order_details() {
		check_admin_referer( 'fincommerce-preview-order', 'security' );
	...
	public static function add_attribute() {
		...
		$attribute->set_visible( apply_filters( 'fincommerce_attribute_default_visibility', 1 ) );
		$attribute->set_variation(
			apply_filters(
				'fincommerce_attribute_default_is_variation',
	...
	public static function revoke_access_to_download() {
		...
		do_action( 'fincommerce_ajax_revoke_access_to_product_download', $download_id, $product_id, $order_id, $permission_id );
				...
				foreach ( $download_data['files'] as $download_id => $file ) {
					...
					if ( $inserted_id ) {
						...
						if ( $file->get_name() ) {
							...
							$file_count = sprintf( __( 'File %d', 'fincommerce' ), $file_counter );
						}
	...
	public static function get_customer_details() {
		...
		if ( has_filter( 'fincommerce_found_customer_details' ) ) {
			wc_deprecated_function( 'The fincommerce_found_customer_details filter', '3.0', 'fincommerce_ajax_get_customer_details' );
		}
		...
		$customer_data = apply_filters( 'fincommerce_ajax_get_customer_details', $data, $customer, $user_id );
	...
	public static function add_order_item() {
		...
		if ( ! isset( $_POST['order_id'] ) ) {
			throw new Exception( __( 'Invalid order', 'fincommerce' ) );
		}
	...
	private static function maybe_add_order_item( $order_id, $items, $items_to_add ) {
		try {
			...
			foreach ( $items_to_add as $item ) {
				...
				if ( ! $product ) {
					throw new Exception( __( 'Invalid product ID', 'fincommerce' ) . ' ' . $product_id );
				}
				if ( ProductType::VARIABLE === $product->get_type() ) {
					/* translators: %s product name */
					throw new Exception( sprintf( __( '%s is a variable product parent and cannot be added.', 'fincommerce' ), $product->get_name() ) );
				}
				$validation_error = new WP_Error();
				$validation_error = apply_filters( 'fincommerce_ajax_add_order_item_validation', $validation_error, $product, $order, $qty );

				if ( $validation_error->get_error_code() ) {
					/* translators: %s: error message */
					throw new Exception( sprintf( __( 'Error: %s', 'fincommerce' ), $validation_error->get_error_message() ) );
				}
				$item_id                 = $order->add_product( $product, $qty, array( 'order' => $order ) );
				$item                    = apply_filters( 'fincommerce_ajax_order_item', $order->get_item( $item_id ), $item_id, $order, $product );
				...
				do_action( 'fincommerce_ajax_add_order_item_meta', $item_id, $item, $order );
			}

			/* translators: %s item name. */
			$order->add_order_note( sprintf( __( 'Added line items: %s', 'fincommerce' ), implode( ', ', $order_notes ) ), false, true );

			do_action( 'fincommerce_ajax_order_items_added', $added_items, $order );
	...
	public static function add_order_fee() {
		...
		try {
			...
			$fee->set_name( sprintf( __( '%s fee', 'fincommerce' ), wc_clean( $formatted_amount ) ) );
	...
	public static function add_order_tax() {
		...
		try {
			...
			if ( ! $rate_id ) {
				throw new Exception( __( 'Invalid rate', 'fincommerce' ) );
			}
	...
	public static function remove_order_item() {
		...
		try {
				...
				foreach ( $order_item_ids as $item_id ) {
					...
					if ( $item->is_type( 'line_item' ) ) {
						...
						if ( $changed_stock && ! is_wp_error( $changed_stock ) ) {
							...
							$order->add_order_note( sprintf( __( 'Deleted %1$s and adjusted stock (%2$s)', 'fincommerce' ), $item->get_name(), $changed_stock['from'] . '&rarr;' . $changed_stock['to'] ), false, true );
						} else {
							/* translators: %s item name. */
							$order->add_order_note( sprintf( __( 'Deleted %s', 'fincommerce' ), $item->get_name() ), false, true );
						}
					}
			 ...
			 */
			do_action( 'fincommerce_ajax_order_items_removed', $item_id ?? 0, $item ?? false, $changed_stock ?? false, $order );
	...
	public static function remove_order_tax() {
		...
		try {
			...
			if ( ! $order->is_editable() ) {
				throw new Exception( __( 'Order not editable', 'fincommerce' ) );
			}
	...
	public static function add_order_note() {
		...
		if ( $post_id > 0 ) {
			...
			$note_classes   = apply_filters( 'fincommerce_order_note_class', array_filter( $note_classes ), $note );
			?>
			<li rel="<?php echo absint( $note->id ); ?>" class="<?php echo esc_attr( implode( ' ', $note_classes ) ); ?>">
				...
				<p class="meta">
					<abbr class="exact-date" title="<?php echo esc_attr( $note->date_created->date( 'y-m-d h:i:s' ) ); ?>">
						...
						printf( esc_html__( 'added on %1$s at %2$s', 'fincommerce' ), esc_html( $note->date_created->date_i18n( wc_date_format() ) ), esc_html( $note->date_created->date_i18n( wc_time_format() ) ) );
						?>
					</abbr>
					<?php
					if ( 'system' !== $note->added_by ) :
						/* translators: %s: note author */
						printf( ' ' . esc_html__( 'by %s', 'fincommerce' ), esc_html( $note->added_by ) );
					endif;
					?>
					<a href="#" class="delete_note" role="button"><?php esc_html_e( 'Delete note', 'fincommerce' ); ?></a>
				</p>
			</li>
			<?php
		}
	...
	public static function json_search_products( $term = '', $include_variations = false ) {
		...
		if ( ! empty( $_GET['limit'] ) ) {
			...
			$limit = absint( apply_filters( 'fincommerce_json_search_limit', 30 ) );
		}
		...
		foreach ( $ids as $id ) {
			...
			if ( $managing_stock && ! empty( $_GET['display_stock'] ) ) {
				...
				$formatted_name .= ' &ndash; ' . sprintf( __( 'Stock: %d', 'fincommerce' ), wc_format_stock_quantity_for_display( $stock_amount, $product_object ) );
			}

			$products[ $product_object->get_id() ] = rawurldecode( wp_strip_all_tags( $formatted_name ) );
		}

		wp_send_json( apply_filters( 'fincommerce_json_search_found_products', $products ) );
	}
	...
	public static function json_search_customers() {
		...
		foreach ( $ids as $id ) {
			...
			$found_customers[ $id ] = sprintf(
				...
				esc_html__( '%1$s (#%2$s &ndash; %3$s)', 'fincommerce' ),
		...
		wp_send_json( apply_filters( 'fincommerce_json_search_found_customers', $found_customers ) );
	}
	...
	public static function json_search_categories() {
		...
		wp_send_json( apply_filters( 'fincommerce_json_search_found_categories', $found_categories ) );
	}
	...
	public static function json_search_categories_tree() {
		...
		wp_send_json( apply_filters( 'fincommerce_json_search_found_categories', $parent_terms ) );
	}
	...
	public static function json_search_taxonomy_terms() {
		 ...
		 */
		$terms = get_terms( apply_filters( 'fincommerce_product_attribute_terms', $args ) );
		 ...
		 */
		wp_send_json( apply_filters( 'fincommerce_json_search_found_product_attribute_terms', $terms, $taxonomy ) );
	}
	...
	public static function json_search_product_attributes() {
		 ...
		 */
		wp_send_json( apply_filters( 'fincommerce_json_search_found_product_categories', $found_product_categories, $search_text ) );
	}
	...
	public static function json_search_pages() {
		...
		if ( ! current_user_can( 'manage_fincommerce' ) ) {
			wp_die( -1 );
		}
		...
		wp_send_json( apply_filters( 'fincommerce_json_search_found_pages', $pages_results ) );
	}
	...
	public static function product_ordering() {
		...
		foreach ( $menu_orders as $id => $menu_order ) {
			...
			*/
			do_action( 'fincommerce_after_single_product_ordering', $id, $index );
		}
		...
		do_action( 'fincommerce_after_product_ordering', $sorting_id, $menu_orders );
	...
	public static function refund_line_items() {
		...
		try {
			...
			if ( ( ! $refund_amount && ( wc_format_decimal( 0, wc_get_price_decimals() ) !== $refund_amount ) ) || $max_refund < $refund_amount || 0 > $refund_amount ) {
				throw new Exception( __( 'Invalid refund amount', 'fincommerce' ) );
			}

			if ( wc_format_decimal( $order->get_total_refunded(), wc_get_price_decimals() ) !== $refunded_amount ) {
				throw new Exception( __( 'Error processing refund. Please try again.', 'fincommerce' ) );
			}
			...
			if ( did_action( 'fincommerce_order_fully_refunded' ) ) {
	...
	public static function delete_refund() {
		...
		foreach ( $refund_ids as $refund_id ) {
			if ( $refund_id && 'shop_order_refund' === OrderUtil::get_order_type( $refund_id ) ) {
				...
				$refund->delete( true );
				do_action( 'fincommerce_refund_deleted', $refund_id, $order_id );
			}
		}
		wp_die();
	}
	...
	public static function rated() {
		...
		update_option( 'fincommerce_admin_footer_text_rated', 1 );
		wp_die();
	}
	...
	public static function update_api_key() {
		...
		try {
			if ( empty( $_POST['description'] ) ) {
				throw new Exception( __( 'Description is missing.', 'fincommerce' ) );
			}
			if ( empty( $_POST['user'] ) ) {
				throw new Exception( __( 'User is missing.', 'fincommerce' ) );
			}
			if ( empty( $_POST['permissions'] ) ) {
				throw new Exception( __( 'Permissions is missing.', 'fincommerce' ) );
			}
			...
			if ( $user_id && ! current_user_can( 'edit_user', $user_id ) ) {
				if ( get_current_user_id() !== $user_id ) {
					throw new Exception( __( 'You do not have permission to assign API Keys to the selected user.', 'fincommerce' ) );
				}
			}
			...
			if ( 0 < $key_id ) {
				...
				$response['message']         = __( 'API Key updated successfully.', 'fincommerce' );
			} else {
				...
				if ( 0 === $wpdb->insert_id ) {
					throw new Exception( __( 'There was an error generating your API Key.', 'fincommerce' ) );
				}
				...
				$response['message']         = __( 'API Key generated successfully. Make sure to copy your new keys now as the secret key will be hidden once you leave this page.', 'fincommerce' );
				$response['revoke_url']      = '<a style="color: #a00; text-decoration: none;" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'revoke-key' => $key_id ), admin_url( 'admin.php?page=wc-settings&tab=advanced&section=keys' ) ), 'revoke' ) ) . '">' . __( 'Revoke key', 'fincommerce' ) . '</a>';
			}
		} catch ( Exception $e ) {
	...
	public static function save_variations() {
		...
		do_action( 'fincommerce_ajax_save_product_variations', $product_id );
		...
		if ( $errors ) {
			...
			echo '<button type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice.', 'fincommerce' ) . '</span></button>';
			echo '</div>';
	...
	public static function bulk_edit_variations() {
		...
		if ( apply_filters( 'fincommerce_bulk_edit_variations_need_children', true ) ) {
		...
		if ( method_exists( __CLASS__, "variation_bulk_action_$bulk_action" ) ) {
			...
			do_action( 'fincommerce_bulk_edit_variations_default', $bulk_action, $data, $product_id, $variations );
		}

		do_action( 'fincommerce_bulk_edit_variations', $bulk_action, $data, $product_id, $variations );
	...
	public static function shipping_zones_save_changes() {
		...
		foreach ( $changes as $zone_id => $data ) {
			if ( isset( $data['deleted'] ) ) {
				 ...
				 */
				do_action(
					'fincommerce_update_non_option_setting',
					array(
						'id'     => 'shipping_zone',
						'action' => 'delete',
					)
	...
	public static function shipping_zone_methods_save_changes() {
		...
		if ( isset( $changes['zone_name'] ) ) {
			 ...
			 * @since 7.8.0
			 */
			do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'zone_name' ) );
		...
		if ( isset( $changes['zone_locations'] ) ) {
			 ...
			 * @since 7.8.0
			 */
			do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'zone_locations' ) );
		...
		if ( isset( $changes['zone_postcodes'] ) ) {
			 ...
			 * @since 7.8.0
			 */
			do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'zone_postcodes' ) );
		...
		if ( isset( $changes['methods'] ) ) {
			foreach ( $changes['methods'] as $instance_id => $data ) {
				$method_id = $wpdb->get_var( $wpdb->prepare( "SELECT method_id FROM {$wpdb->prefix}fincommerce_shipping_zone_methods WHERE instance_id = %d", $instance_id ) );

				if ( isset( $data['deleted'] ) ) {
					$shipping_method = WC_Shipping_Zones::get_shipping_method( $instance_id );
					$option_key      = $shipping_method->get_instance_option_key();
					if ( $wpdb->delete( "{$wpdb->prefix}fincommerce_shipping_zone_methods", array( 'instance_id' => $instance_id ) ) ) {
						...
						do_action( 'fincommerce_shipping_zone_method_deleted', $instance_id, $method_id, $zone_id );
					}
					continue;
				}
				...
				if ( isset( $method_data['method_order'] ) ) {
					 ...
					 * @since 7.8.0
					 */
					do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'zone_methods_order' ) );
					$wpdb->update( "{$wpdb->prefix}fincommerce_shipping_zone_methods", array( 'method_order' => absint( $method_data['method_order'] ) ), array( 'instance_id' => absint( $instance_id ) ) );
				}
				...
				if ( isset( $method_data['enabled'] ) ) {
					 ...
					 * @since 7.8.0
					 */
					do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'zone_methods_enabled' ) );
					$is_enabled = absint( 'yes' === $method_data['enabled'] );
					if ( $wpdb->update( "{$wpdb->prefix}fincommerce_shipping_zone_methods", array( 'is_enabled' => $is_enabled ), array( 'instance_id' => absint( $instance_id ) ) ) ) {
						do_action( 'fincommerce_shipping_zone_method_status_toggled', $instance_id, $method_id, $zone_id, $is_enabled );
					}
				}
			}
		}
	...
	public static function shipping_classes_save_changes() {
		...
		foreach ( $changes as $term_id => $data ) {
			...
			if ( isset( $data['name'] ) ) {
				 ...
				 * @since 7.8.0
				 */
				do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'shipping_class_name' ) );
			...
			if ( isset( $data['slug'] ) ) {
				 ...
				 * @since 7.8.0
				 */
				do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'shipping_class_slug' ) );
			...
			if ( isset( $data['description'] ) ) {
				 ...
				 * @since 7.8.0
				 */
				do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'shipping_class_description' ) );
			...
			if ( isset( $data['newRow'] ) ) {
				 ...
				 * @since 7.8.0
				 */
				do_action( 'fincommerce_update_non_option_setting', array( 'id' => 'shipping_class' ) );
				wp_update_term( $term_id, 'product_shipping_class', $update_args );
			}

			do_action( 'fincommerce_shipping_classes_save_class', $term_id, $data );
		}
	...
	public static function toggle_gateway_enabled() {
		if ( current_user_can( 'manage_fincommerce' ) && check_ajax_referer( 'fincommerce-toggle-payment-gateway-enabled', 'security' ) && isset( $_POST['gateway_id'] ) ) {
			// Set current tab.
			...
			foreach ( $payment_gateways as $gateway ) {
				...
				if ( ! wc_string_to_bool( $enabled ) ) {
					if ( $gateway->needs_setup() ) {
						...
						do_action( 'fincommerce_update_option', $option );
	...
	public static function load_status_widget() {
		...
		if ( ! current_user_can( 'manage_fincommerce' ) || ! current_user_can( 'view_fincommerce_reports' ) || ! current_user_can( 'publish_shop_orders' ) ) {
	...
	/**
	 * Hooked into `wp_ajax_fincommerce_json_search_order_metakeys` to return the list of unique meta keys for the
	...
	private static function order_refresh_lock( $response, $data ) {
		return wc_get_container()->get( Automattic\FinCommerce\Internal\Admin\Orders\EditLock::class )->refresh_lock_ajax( $response, $data );
	}
	...
	private static function check_locked_orders( $response, $data ) {
		return wc_get_container()->get( Automattic\FinCommerce\Internal\Admin\Orders\EditLock::class )->check_locked_orders_ajax( $response, $data );
	}
