...
class WC_Shipping_Zone extends WC_Legacy_Shipping_Zone {
	...
	public function get_formatted_location( $max = 10, $context = 'view' ) {
		...
		if ( count( $location_parts ) > $max ) {
			...
			return sprintf( _n( '%s and %d other region', '%s and %d other regions', $remaining, 'fincommerce' ), implode( ', ', array_splice( $location_parts, 0, $max ) ), $remaining );
			// @codingStandardsIgnoreEnd
		} elseif ( ! empty( $location_parts ) ) {
			return implode( ', ', $location_parts );
		} else {
			return __( 'Everywhere', 'fincommerce' );
		}
	}
	...
	public function get_shipping_methods( $enabled_only = false, $context = 'admin' ) {
		...
		return apply_filters( 'fincommerce_shipping_zone_shipping_methods', $methods, $raw_methods, $allowed_classes, $this );
	}
	...
	public function is_valid_location_type( $type ) {
		return in_array( $type, apply_filters( 'fincommerce_valid_location_types', array( 'postcode', 'state', 'country', 'continent' ) ), true );
	}
	...
	public function add_shipping_method( $type ) {
		...
		if ( $instance_id ) {
			do_action( 'fincommerce_shipping_zone_method_added', $instance_id, $type, $this->get_id() );
		}
	...
	public function delete_shipping_method( $instance_id ) {
		...
		if ( $method ) {
			...
			do_action( 'fincommerce_shipping_zone_method_deleted', $instance_id, $method->method_id, $this->get_id() );
		}
