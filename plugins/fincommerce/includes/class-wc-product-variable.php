...
class WC_Product_Variable extends WC_Product {
	...
	*/
	...
	public function add_to_cart_aria_describedby() {
		 ...
		 * @since 7.8.0
		 */
		return apply_filters( 'fincommerce_product_add_to_cart_aria_describedby', $this->is_purchasable() ? __( 'This product has multiple variants. The options may be chosen on the product page', 'fincommerce' ) : '', $this );
	}
	...
	public function add_to_cart_text() {
		return apply_filters( 'fincommerce_product_add_to_cart_text', $this->is_purchasable() ? __( 'Select options', 'fincommerce' ) : __( 'Read more', 'fincommerce' ), $this );
	}
	...
	public function add_to_cart_description() {
		...
		return apply_filters( 'fincommerce_product_add_to_cart_description', sprintf( __( 'Select options for &ldquo;%s&rdquo;', 'fincommerce' ), $this->get_name() ), $this );
	}

	/**
	 * Get an array of all sale and regular prices from all variations. This is used for example when displaying the price range at variable product level or seeing if the variable product is on sale.
	 *
	 * @param  bool $for_display If true, prices will be adapted for display based on the `fincommerce_tax_display_shop` setting (including or excluding taxes).
	 ...
	 * @param  boolean $for_display If true, prices will be adapted for display based on the `fincommerce_tax_display_shop` setting (including or excluding taxes).
	 * @return string
	 */
	public function get_variation_regular_price( $min_or_max = 'min', $for_display = false ) {
		...
		return apply_filters( 'fincommerce_get_variation_regular_price', $price, $this, $min_or_max, $for_display );
	}
	...
	public function get_variation_sale_price( $min_or_max = 'min', $for_display = false ) {
		...
		return apply_filters( 'fincommerce_get_variation_sale_price', $price, $this, $min_or_max, $for_display );
	}
	...
	public function get_variation_price( $min_or_max = 'min', $for_display = false ) {
		...
		return apply_filters( 'fincommerce_get_variation_price', $price, $this, $min_or_max, $for_display );
	}
	...
	public function get_price_html( $price = '' ) {
		...
		if ( empty( $prices['price'] ) ) {
			$price = apply_filters( 'fincommerce_variable_empty_price_html', '', $this );
		} else {
			...
			$price = apply_filters( 'fincommerce_variable_price_html', $price . $this->get_price_suffix(), $this );
		}
	...
	public function get_price_suffix( $price = '', $qty = 1 ) {
		...
		if ( strstr( $suffix, '{' ) ) {
			return apply_filters( 'fincommerce_get_price_suffix', '', $this, $price, $qty );
		} else {
	...
	public function get_children( $visible_only = '' ) {
		...
		return apply_filters( 'fincommerce_get_children', $this->children, $this, false );
	}
	...
	public function get_visible_children() {
		...
		return apply_filters( 'fincommerce_get_children', $this->visible_children, $this, true );
	}
	...
	public function get_available_variations( $return = 'array' ) {
		...
		$hide_out_of_stock_items = ( 'yes' === get_option( 'fincommerce_hide_out_of_stock_items' ) );
		...
		foreach ( $variation_ids as $variation_id ) {
			...
			/**
			 * Filter 'fincommerce_hide_invisible_variations' to optionally hide invisible variations (disabled variations and variations with empty price).
			 *
			 * @since 2.6.8
			 *
			 ...
			 */
			if ( apply_filters( 'fincommerce_hide_invisible_variations', true, $this->get_id(), $variation ) && ! $variation->variation_is_visible() ) {
				continue;
			}
	...
	public function get_available_variation( $variation ) {
		...
		$show_variation_price = apply_filters( 'fincommerce_show_variation_price', $variation->get_price() === '' || $this->get_variation_sale_price( 'min' ) !== $this->get_variation_sale_price( 'max' ) || $this->get_variation_regular_price( 'min' ) !== $this->get_variation_regular_price( 'max' ), $this, $variation );

		return apply_filters(
			'fincommerce_available_variation',
			array(
	...
	*/
	...
	public function has_options() {
		return apply_filters( 'fincommerce_product_has_options', true, $this );
	}
	...
	*/
	...
	public static function sync( $product, $save = true ) {
		...
		if ( is_a( $product, 'WC_Product_Variable' ) ) {
			...
			do_action( 'fincommerce_variable_product_sync_data', $product );
			...
			wc_do_deprecated_action(
				'fincommerce_variable_product_sync',
				array(
					$product->get_id(),
					$product->get_visible_children(),
				),
				'3.0',
				'fincommerce_variable_product_sync_data, fincommerce_new_product or fincommerce_update_product'
			);
		}
