...
class WC_Logger implements WC_Logger_Interface {
	 ...
	 *
	 * @param array  $handlers Optional. Array of log handlers. If $handlers is not provided, the filter 'fincommerce_register_log_handlers' will be used to define the handlers. If $handlers is provided, the filter will not be applied and the handlers will be used directly.
	...
	protected function get_handlers() {
		if ( ! is_null( $this->handlers ) ) {
			 ...
			 * @since 3.0.0
			 */
			$handlers = apply_filters( 'fincommerce_register_log_handlers', array( $handler_instance ) );
		}
				...
				if ( $handler instanceof WC_Log_Handler_Interface ) {
					...
					wc_doing_it_wrong(
						...
						sprintf(
							...
							__( 'The provided handler %1$s does not implement %2$s.', 'fincommerce' ),
	...
	public function add( $handle, $message, $level = WC_Log_Levels::NOTICE ) {
		$message = apply_filters( 'fincommerce_logger_add_message', $message, $handle );
		$this->log(
			$level,
			$message,
			array(
		...
		wc_do_deprecated_action( 'fincommerce_log_add', array( $handle, $message ), '3.0', 'This action has been deprecated with no alternative.' );
		return true;
	}
	...
	public function log( $level, $message, $context = array() ) {
		if ( ! WC_Log_Levels::is_valid_level( $level ) ) {
			...
			wc_doing_it_wrong( __METHOD__, sprintf( __( '%1$s was called with an invalid level "%2$s".', 'fincommerce' ), '<code>WC_Logger::log</code>', $level ), '3.0' );
		}
		...
		if ( $this->should_handle( $level ) ) {
			...
			foreach ( $this->get_handlers() as $handler ) {
				 ...
				 */
				$filtered_message = apply_filters( 'fincommerce_logger_log_message', $message, $level, $context, $handler );
