 ...
 * Order Class.
 *
 * These are regular FinCommerce orders, which extend the abstract order class.
 */
class WC_Order extends WC_Abstract_Order {
	...
	public function payment_complete( $transaction_id = '' ) {
		...
		try {
			do_action( 'fincommerce_pre_payment_complete', $this->get_id(), $transaction_id );
			 ...
			 * @since 2.7.0
			 */
			$valid_completed_statuses = apply_filters( 'fincommerce_valid_order_statuses_for_payment_complete', OrderStatus::PAYMENT_COMPLETE_STATUSES, $this );
			if ( $this->has_status( $valid_completed_statuses ) ) {
				 ...
				 * @since 2.7.0
				 */
				$this->set_status( apply_filters( 'fincommerce_payment_complete_order_status', $this->needs_processing() ? OrderStatus::PROCESSING : OrderStatus::COMPLETED, $this->get_id(), $this ) );
				$this->save();

				do_action( 'fincommerce_payment_complete', $this->get_id(), $transaction_id );
			} else {
				do_action( 'fincommerce_payment_complete_order_status_' . $this->get_status(), $this->get_id(), $transaction_id );
			}
		} catch ( Exception $e ) {
			/**
			...
			$this->add_order_note( __( 'Payment complete event failed.', 'fincommerce' ) . ' ' . $e->getMessage() );
			return false;
		}
		return true;
	}
	...
	public function get_formatted_order_total( $tax_display = '', $display_refunded = true ) {
		...
		if ( wc_tax_enabled() && 'incl' === $tax_display ) {
			...
			if ( ! empty( $tax_string_array ) ) {
				/* translators: %s: taxes */
				$tax_string = ' <small class="includes_tax">' . sprintf( __( '(includes %s)', 'fincommerce' ), implode( ', ', $tax_string_array ) ) . '</small>';
			}
		}
		...
		/**
		 * Filter FinCommerce formatted order total.
		 *
		 ...
		 * @param bool     $display_refunded If should include refunded value.
		 */
		return apply_filters( 'fincommerce_get_formatted_order_total', $formatted_total, $this, $tax_display, $display_refunded );
	}
	...
	*/
	...
	public function set_status( $new_status, $note = '', $manual_update = false ) {
		...
		if ( true === $this->object_read && ! empty( $result['from'] ) && $result['from'] !== $result['to'] ) {
			...
			if ( $manual_update ) {
				do_action( 'fincommerce_order_edit_status', $this->get_id(), $result['to'] );
			}
	...
	public function maybe_set_date_paid() {
		...
		if ( ! $this->get_date_paid( 'edit' ) ) {
			 ...
			 * @since 2.7.0
			 */
			$payment_completed_status = apply_filters( 'fincommerce_payment_complete_order_status', $this->needs_processing() ? OrderStatus::PROCESSING : OrderStatus::COMPLETED, $this->get_id(), $this );
	...
	public function update_status( $new_status, $note = '', $manual = false ) {
		...
		try {
			...
			$this->add_order_note( __( 'Update status event failed.', 'fincommerce' ) . ' ' . $e->getMessage() );
			return false;
		}
		return true;
	}
	...
	protected function status_transition() {
		...
		if ( $status_transition && $order_persisted && $order_items_persisted ) {
			try {
				 ...
				 * }
				 */
				do_action( 'fincommerce_order_status_' . $status_transition['to'], $this->get_id(), $this, $status_transition );

				if ( ! empty( $status_transition['from'] ) ) {
					/* translators: 1: old order status 2: new order status */
					$transition_note = sprintf( __( 'Order status changed from %1$s to %2$s.', 'fincommerce' ), wc_get_order_status_name( $status_transition['from'] ), wc_get_order_status_name( $status_transition['to'] ) );

					// Note the transition occurred.
					$this->add_status_transition_note( $transition_note, $status_transition );

					do_action( 'fincommerce_order_status_' . $status_transition['from'] . '_to_' . $status_transition['to'], $this->get_id(), $this );
					do_action( 'fincommerce_order_status_changed', $this->get_id(), $status_transition['from'], $status_transition['to'], $this );
					...
					if (
						 ...
						 * @since 4.0.0
						 */
						in_array( $status_transition['from'], apply_filters( 'fincommerce_valid_order_statuses_for_payment', array( OrderStatus::PENDING, OrderStatus::FAILED ), $this ), true )
						 ...
						 */
						do_action( 'fincommerce_order_payment_status_changed', $this->get_id(), $this );
					}
				} else {
					/* translators: %s: new order status */
					$transition_note = sprintf( __( 'Order status set to %s.', 'fincommerce' ), wc_get_order_status_name( $status_transition['to'] ) );
				...
				$this->add_order_note( __( 'Error during status transition.', 'fincommerce' ) . ' ' . $e->getMessage() );
			}
		}
	}
	...
	*/
	...
	public function get_order_number() {
		return (string) apply_filters( 'fincommerce_order_number', $this->get_id(), $this );
	}
	...
	protected function get_address_prop( $prop, $address_type = 'billing', $context = 'view' ) {
		...
		if ( array_key_exists( $prop, $this->data[ $address_type ] ) ) {
			...
			if ( 'view' === $context ) {
				/**
				 * Filter: 'fincommerce_order_get_[billing|shipping]_[prop]'
				 *
	...
	public function get_date_paid( $context = 'view' ) {
		...
		if ( 'view' === $context && ! $date_paid && version_compare( $this->get_version( 'edit' ), '3.0', '<' )
			 ...
			 * @since 3.0.0
			 */
			&& $this->has_status( apply_filters( 'fincommerce_payment_complete_order_status', $this->needs_processing() ? OrderStatus::PROCESSING : OrderStatus::COMPLETED, $this->get_id(), $this ) ) ) {
	...
	public function get_address( $address_type = 'billing' ) {
		/**
		 * Filter: 'fincommerce_get_order_address'
		 *
		 ...
		 * @param string $address_type Type of address; 'billing' or 'shipping'.
		 */
		return apply_filters( 'fincommerce_get_order_address', array_merge( $this->data[ $address_type ], $this->get_prop( $address_type, 'view' ) ), $address_type, $this );
	}
	...
	public function get_shipping_address_map_url() {
		...
		$address = apply_filters( 'fincommerce_shipping_address_map_url_parts', $address, $this );

		return apply_filters( 'fincommerce_shipping_address_map_url', 'https://maps.google.com/maps?&q=' . rawurlencode( implode( ', ', $address ) ) . '&z=16', $this );
	}
	...
	public function get_formatted_billing_full_name() {
		...
		return sprintf( _x( '%1$s %2$s', 'full name', 'fincommerce' ), $this->get_billing_first_name(), $this->get_billing_last_name() );
	}
	...
	public function get_formatted_shipping_full_name() {
		/* translators: 1: first name 2: last name */
		return sprintf( _x( '%1$s %2$s', 'full name', 'fincommerce' ), $this->get_shipping_first_name(), $this->get_shipping_last_name() );
	}
	...
	public function get_formatted_billing_address( $empty_content = '' ) {
		$raw_address = apply_filters( 'fincommerce_order_formatted_billing_address', $this->get_address( 'billing' ), $this );
		 ...
		 */
		return apply_filters( 'fincommerce_order_get_formatted_billing_address', $address ? $address : $empty_content, $raw_address, $this );
	}
	...
	public function get_formatted_shipping_address( $empty_content = '' ) {
		...
		if ( $this->has_shipping_address() ) {
			$raw_address = apply_filters( 'fincommerce_order_formatted_shipping_address', $raw_address, $this );
			$address     = WC()->countries->get_formatted_address( $raw_address );
		}
		 ...
		 * @param WC_Order $order       Order data. @since 3.9.0
		 */
		return apply_filters( 'fincommerce_order_get_formatted_shipping_address', $address ? $address : $empty_content, $raw_address, $this );
	}
	...
	*/
	...
	public function set_billing_email( $value ) {
		if ( $value && ! is_email( $value ) ) {
			$this->error( 'order_invalid_billing_email', __( 'Invalid billing email address', 'fincommerce' ) );
		}
	...
	*/
	...
	public function is_paid() {
		return apply_filters( 'fincommerce_order_is_paid', $this->has_status( wc_get_is_paid_statuses() ), $this );
	}
	...
	public function is_download_permitted() {
		 ...
		 * @since 2.7.0
		 */
		return apply_filters( 'fincommerce_order_is_download_permitted', $this->has_status( OrderStatus::COMPLETED ) || ( 'yes' === get_option( 'fincommerce_downloads_grant_access_after_payment' ) && $this->has_status( OrderStatus::PROCESSING ) ), $this );
	}
	...
	public function needs_shipping_address() {
		 ...
		 * @since 2.7.0
		 */
		$hide          = apply_filters( 'fincommerce_order_hide_shipping_address', LocalPickupUtils::get_local_pickup_method_ids(), $this );
		$needs_address = false;
		...
		return apply_filters( 'fincommerce_order_needs_shipping_address', $needs_address, $hide, $this );
	}
	...
	public function get_downloadable_items() {
		...
		return apply_filters( 'fincommerce_order_get_downloadable_items', $downloads, $this );
	}
	...
	public function needs_payment() {
		 ...
		 * @since 2.7.0
		 */
		$valid_order_statuses = apply_filters( 'fincommerce_valid_order_statuses_for_payment', array( OrderStatus::PENDING, OrderStatus::FAILED ), $this );
		return apply_filters( 'fincommerce_order_needs_payment', ( $this->has_status( $valid_order_statuses ) && $this->get_total() > 0 ), $this, $valid_order_statuses );
	}
			...
			if ( count( $this->get_items() ) > 0 ) {
				foreach ( $this->get_items() as $item ) {
					if ( $item->is_type( 'line_item' ) ) {
						...
						if ( apply_filters( 'fincommerce_order_item_needs_processing', ! $virtual_downloadable_item, $product, $this->get_id() ) ) {
							$needs_processing = 1;
							break;
						}
					}
	...
	*/
	...
	public function get_checkout_payment_url( $on_checkout = false ) {
		...
		return apply_filters( 'fincommerce_get_checkout_payment_url', $pay_url, $this );
	}
	...
	public function get_checkout_order_received_url() {
		...
		return apply_filters( 'fincommerce_get_checkout_order_received_url', $order_received_url, $this );
	}
	...
	public function get_cancel_order_url( $redirect = '' ) {
		...
		return apply_filters(
			'fincommerce_get_cancel_order_url',
			wp_nonce_url(
				add_query_arg(
					array(
						'cancel_order' => 'true',
	...
	public function get_cancel_order_url_raw( $redirect = '' ) {
		 ...
		 */
		return apply_filters(
			'fincommerce_get_cancel_order_url_raw',
			add_query_arg(
				array(
					'cancel_order' => 'true',
					'order'        => $this->get_order_key(),
					'order_id'     => $this->get_id(),
					'redirect'     => $redirect,
					'_wpnonce'     => wp_create_nonce( 'fincommerce-cancel_order' ),
				),
				$this->get_cancel_endpoint()
			),
			$this,
	...
	public function get_view_order_url() {
		return apply_filters( 'fincommerce_get_view_order_url', wc_get_endpoint_url( 'view-order', $this->get_id(), wc_get_page_permalink( 'myaccount' ) ), $this );
	}
	...
	public function get_edit_order_url() {
		$edit_url = \Automattic\FinCommerce\Utilities\OrderUtil::get_order_admin_edit_url( $this->get_id() );
		/**
		 * Filter the URL to edit the order in the backend.
		 *
		 * @since 3.3.0
		 */
		return apply_filters( 'fincommerce_get_edit_order_url', $edit_url, $this );
	}
	...
	*/
	...
	public function add_order_note( $note, $is_customer_note = 0, $added_by_user = false ) {
		...
		if ( is_user_logged_in() && current_user_can( 'edit_shop_orders', $this->get_id() ) && $added_by_user ) {
			...
			$comment_author        = __( 'FinCommerce', 'fincommerce' );
			$comment_author_email  = strtolower( __( 'FinCommerce', 'fincommerce' ) ) . '@';
		...
		$commentdata = apply_filters(
			'fincommerce_new_order_note_data',
			array(
		 ...
		 * @since 4.4.0
		 */
		do_action( 'fincommerce_order_note_added', $comment_id, $this );
	...
	*/
	...
	public function get_item_count_refunded( $item_type = '' ) {
		...
		return apply_filters( 'fincommerce_get_item_count_refunded', $count, $item_type, $this );
	}
	...
	protected function add_order_item_totals_payment_method_row( &$total_rows, $tax_display ) {
		if ( $this->get_total() > 0 && $this->get_payment_method_title() && 'other' !== $this->get_payment_method() ) {
			...
			$total_rows['payment_method'] = array(
				'type'  => 'payment_method',
				'label' => __( 'Payment method:', 'fincommerce' ),
				'value' => $value,
			);
		}
	}
	...
	public function get_order_item_totals( $tax_display = '' ) {
		$tax_display = $tax_display ? $tax_display : get_option( 'fincommerce_tax_display_cart' );
		$total_rows  = array();
		...
		return apply_filters( 'fincommerce_get_order_item_totals', $total_rows, $this, $tax_display );
	}
	...
	public function is_created_via( $modus ) {
		return apply_filters( 'fincommerce_order_is_created_via', $modus === $this->get_created_via(), $this, $modus );
	}
