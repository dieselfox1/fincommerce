...
class WC_Integration_MaxMind_Database_Service {
	...
	public function get_database_path() {
		...
		$database_path = trailingslashit( $uploads_dir['basedir'] ) . 'fincommerce_uploads/';
		...
		$database_path = apply_filters_deprecated(
			'fincommerce_geolocation_local_database_path',
			array( $database_path, 2 ),
			'3.9.0',
			'fincommerce_maxmind_geolocation_database_path'
		);
		 ...
		 * @param string $database_path The path to the database.
		 */
		return apply_filters( 'fincommerce_maxmind_geolocation_database_path', $database_path );
	}
	...
	public function download_database( $license_key ) {
		...
		if ( is_wp_error( $tmp_archive_path ) ) {
			...
			if ( isset( $error_data['code'] ) ) {
				switch ( $error_data['code'] ) {
					case 401:
						return new WP_Error(
							'fincommerce_maxmind_geolocation_database_license_key',
							__( 'The MaxMind license key is invalid. If you have recently created this key, you may need to wait for it to become active.', 'fincommerce' )
						);
				}
			}

			return new WP_Error( 'fincommerce_maxmind_geolocation_database_download', __( 'Failed to download the MaxMind database.', 'fincommerce' ) );
		}
		...
		try {
			...
			return new WP_Error( 'fincommerce_maxmind_geolocation_database_archive', $exception->getMessage() );
		} finally {
	...
	public function get_iso_country_code_for_ip( $ip_address ) {
		...
		if ( ! class_exists( 'MaxMind\Db\Reader' ) ) {
			wc_get_logger()->notice( __( 'Missing MaxMind Reader library!', 'fincommerce' ), array( 'source' => 'maxmind-geolocation' ) );
			return $country_code;
		}
