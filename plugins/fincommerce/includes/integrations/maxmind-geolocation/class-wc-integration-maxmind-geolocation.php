 ...
 *
 * @version 3.9.0
 * @package FinCommerce\Integrations
 */
...
class WC_Integration_MaxMind_Geolocation extends WC_Integration {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'MaxMind Geolocation', 'fincommerce' );
		$this->method_description = __( 'An integration for utilizing MaxMind to do Geolocation lookups. Please note that this integration will only do country lookups.', 'fincommerce' );
		 ...
		 */
		$this->database_service = apply_filters( 'fincommerce_maxmind_geolocation_database_service', null );
		...
		add_action( 'update_option_fincommerce_default_customer_address', array( $this, 'display_missing_license_key_notice' ), 1000, 2 );
		...
		$bind_updater = apply_filters_deprecated(
			'fincommerce_geolocation_update_database_periodically',
			array( true ),
			'3.9.0',
			'fincommerce_maxmind_geolocation_update_database_periodically'
		);
		 ...
		 */
		$bind_updater = apply_filters( 'fincommerce_maxmind_geolocation_update_database_periodically', $bind_updater );

		// Bind to the scheduled updater action.
		if ( $bind_updater ) {
			add_action( 'fincommerce_geoip_updater', array( $this, 'update_database' ) );
		}

		// Bind to the geolocation filter for MaxMind database lookups.
		add_filter( 'fincommerce_get_geolocation', array( $this, 'get_geolocation' ), 10, 2 );
	}
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'license_key' => array(
				'title'       => __( 'MaxMind License Key', 'fincommerce' ),
				'type'        => 'password',
				'description' => sprintf(
					...
					'https://fincommerce.com/document/maxmind-geolocation-integration/'
				),
				'desc_tip'    => false,
				'default'     => '',
			),
	...
	public function update_database( $new_database_path = null ) {
		...
		if ( ! WP_Filesystem() ) {
			wc_get_logger()->warning( __( 'Failed to initialise WC_Filesystem API while trying to update the MaxMind Geolocation database.', 'fincommerce' ) );
			return;
		}
	...
	public function get_geolocation( $data, $ip_address ) {
		// FinCommerce look for headers first, and at this moment could be just enough.
	...
	public function display_missing_license_key_notice( $old_value, $new_value ) {
		if ( ! apply_filters( 'fincommerce_maxmind_geolocation_display_notices', true ) ) {
			return;
		}
