...
class WC_Privacy_Erasers {
	...
	public static function customer_data_eraser( $email_address, $page ) {
		...
		$props_to_erase = apply_filters(
			'fincommerce_privacy_erase_customer_personal_data_props',
			array(
		...
		foreach ( $props_to_erase as $prop => $label ) {
			...
			$erased = apply_filters( 'fincommerce_privacy_erase_customer_personal_data_prop', $erased, $prop, $customer );

			if ( $erased ) {
				/* Translators: %s Prop name. */
				$response['messages'][]    = sprintf( __( 'Removed customer "%s"', 'fincommerce' ), $label );
		 ...
		 */
		return apply_filters( 'fincommerce_privacy_erase_personal_data_customer', $response, $customer );
	}

	/**
	 * Finds and erases data which could be used to identify a person from FinCommerce data associated with an email address.
	 *
	...
	public static function order_data_eraser( $email_address, $page ) {
		$page            = (int) $page;
		$user            = get_user_by( 'email', $email_address ); // Check if user has an ID in the DB to load stored personal data.
		$erasure_enabled = wc_string_to_bool( get_option( 'fincommerce_erasure_request_removes_order_data', 'no' ) );
		$response        = array(
			'items_removed'  => false,
			'items_retained' => false,
			'messages'       => array(),
		...
		if ( 0 < count( $orders ) ) {
			foreach ( $orders as $order ) {
				if ( apply_filters( 'fincommerce_privacy_erase_order_personal_data', $erasure_enabled, $order ) ) {
					self::remove_order_personal_data( $order );

					/* Translators: %s Order number. */
					$response['messages'][]    = sprintf( __( 'Removed personal data from order %s.', 'fincommerce' ), $order->get_order_number() );
					$response['items_removed'] = true;
				} else {
					/* Translators: %s Order number. */
					$response['messages'][]     = sprintf( __( 'Personal data within order %s has been retained.', 'fincommerce' ), $order->get_order_number() );
	...
	public static function download_data_eraser( $email_address, $page ) {
		$page            = (int) $page;
		$user            = get_user_by( 'email', $email_address ); // Check if user has an ID in the DB to load stored personal data.
		$erasure_enabled = wc_string_to_bool( get_option( 'fincommerce_erasure_request_removes_download_data', 'no' ) );
		$response        = array(
			'items_removed'  => false,
			'items_retained' => false,
			'messages'       => array(),
		...
		if ( apply_filters( 'fincommerce_privacy_erase_download_personal_data', $erasure_enabled, $email_address ) ) {
			if ( $user instanceof WP_User ) {
			...
			if ( $result ) {
				$response['messages'][]    = __( 'Removed access to downloadable files.', 'fincommerce' );
				$response['items_removed'] = true;
			}
		} else {
			$response['messages'][]     = __( 'Customer download permissions have been retained.', 'fincommerce' );
			$response['items_retained'] = true;
		}
	...
	/**
	 * Remove personal data specific to FinCommerce from an order object.
	 *
	...
	public static function remove_order_personal_data( $order ) {
		 ...
		 */
		do_action( 'fincommerce_privacy_before_remove_order_personal_data', $order );
		...
		$props_to_remove = apply_filters(
			'fincommerce_privacy_remove_order_personal_data_props',
			array(
		...
		if ( ! empty( $props_to_remove ) && is_array( $props_to_remove ) ) {
			foreach ( $props_to_remove as $prop => $data_type ) {
				 ...
				 */
				$anonymized_data[ $prop ] = apply_filters( 'fincommerce_privacy_remove_order_personal_data_prop_value', $anon_value, $prop, $value, $data_type, $order );
			}
		}
		...
		$meta_to_remove = apply_filters(
			'fincommerce_privacy_remove_order_personal_data_meta',
			array(
		...
		if ( ! empty( $meta_to_remove ) && is_array( $meta_to_remove ) ) {
			foreach ( $meta_to_remove as $meta_key => $data_type ) {
				 ...
				 * @param WC_Order $order An order object.
				 */
				$anon_value = apply_filters( 'fincommerce_privacy_remove_order_personal_data_meta_value', $anon_value, $meta_key, $value, $data_type, $order );
		...
		$order->add_order_note( __( 'Personal data removed.', 'fincommerce' ) );
		 ...
		 */
		do_action( 'fincommerce_privacy_remove_order_personal_data', $order );
	}
	...
	public static function customer_tokens_eraser( $email_address, $page ) {
		...
		foreach ( $tokens as $token ) {
			...
			/* Translators: %s Prop name. */
			$response['messages'][]    = sprintf( __( 'Removed payment token "%d"', 'fincommerce' ), $token->get_id() );
			$response['items_removed'] = true;
		}
		 ...
		 */
		return apply_filters( 'fincommerce_privacy_erase_personal_data_tokens', $response, $tokens );
	}
}
