...
class WC_Order_Data_Store_CPT extends Abstract_WC_Order_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Order_Data_Store_Interface {
	...
	public function create( &$order ) {
		...
		// Do not fire 'fincommerce_new_order' for draft statuses.
	...
	public function update( &$order ) {
		...
		if ( ! $order->get_date_paid( 'edit' ) && version_compare( $order->get_version( 'edit' ), '3.0', '<' ) ) {
			 ...
			 */
			$payment_complete_status = apply_filters( 'fincommerce_payment_complete_order_status', $order->needs_processing() ? OrderStatus::PROCESSING : OrderStatus::COMPLETED, $order->get_id(), $order );
	...
	public function get_total_shipping_refunded( $order ) {
		...
		$total = $wpdb->get_var(
			$wpdb->prepare(
				...
				FROM {$wpdb->prefix}fincommerce_order_itemmeta AS order_itemmeta
				INNER JOIN $wpdb->posts AS posts ON ( posts.post_type = 'shop_order_refund' AND posts.post_parent = %d )
				INNER JOIN {$wpdb->prefix}fincommerce_order_items AS order_items ON ( order_items.order_id = posts.ID AND order_items.order_item_type = 'shipping' )
				WHERE order_itemmeta.order_item_id = order_items.order_item_id
	...
	private function get_orders_generate_customer_meta_query( $values, $relation = 'or' ) {
		...
		foreach ( $values as $value ) {
			if ( is_array( $value ) ) {
				...
				return new WP_Error( 'fincommerce_query_invalid', __( 'Invalid customer query.', 'fincommerce' ), $values );
			}
		}
	...
	public function search_orders( $term ) {
		...
		$search_fields = array_map(
			'wc_clean',
			apply_filters(
				'fincommerce_shop_order_search_fields',
				array(
					'_billing_address_index',
					'_shipping_address_index',
					'_billing_last_name',
			...
			$order_ids = array_unique(
				array_merge(
					...
					$wpdb->get_col(
						$wpdb->prepare(
							"SELECT order_id
							FROM {$wpdb->prefix}fincommerce_order_items as order_items
		 ...
		 * @return array
		 */
		$order_ids = apply_filters( 'fincommerce_shop_order_search_results', $order_ids, $term, $search_fields );
	...
	protected function get_wp_query_args( $query_vars ) {
		...
		return apply_filters( 'fincommerce_order_data_store_cpt_get_orders_query', $wp_query_args, $query_vars, $this );
	}
	...
	public function query( $query_vars ) {
		...
		$unsupported_args = (array) apply_filters(
			'fincommerce_order_data_store_cpt_query_unsupported_args',
