	...
	*/
	...
	public function read( &$order ) {
		...
		if ( ! $order->get_id() || ! $post_object || ! in_array( $post_object->post_type, wc_get_order_types(), true ) ) {
			throw new Exception( esc_html__( 'Invalid order.', 'fincommerce' ) );
		}
	...
	public function delete( &$order, $args = array() ) {
		...
		if ( $args['force_delete'] ) {
			if ( $do_filters ) {
				 ...
				 */
				do_action( 'fincommerce_before_delete_order', $id, $order );
			}
			...
			if ( $do_filters ) {
				 ...
				 */
				do_action( 'fincommerce_delete_order', $id );
			}
		} else {
			if ( $do_filters ) {
				/**
				 ...
				 */
				do_action( 'fincommerce_before_trash_order', $id, $order );
			}
			...
			if ( $do_filters ) {
				 ...
				 */
				do_action( 'fincommerce_trash_order', $id );
			}
		}
	}
	...
	*/
	...
	protected function get_post_status( $order ) {
		...
		if ( ! $order_status ) {
			 ...
			 * @since 3.7.0
			 */
			$order_status = apply_filters( 'fincommerce_default_order_status', OrderStatus::PENDING );
		}
	...
	protected function get_post_title() {
		...
		return sprintf( __( 'Order &ndash; %s', 'fincommerce' ), ( new DateTime( 'now' ) )->format( _x( 'M d, Y @ h:i A', 'Order date parsed by DateTime::format', 'fincommerce' ) ) );
	...
	protected function read_order_data( &$order, $post_object ) {
		...
		$this->set_order_props(
			...
			array(
				...
				'prices_include_tax' => metadata_exists( 'post', $id, '_prices_include_tax' ) ? 'yes' === get_post_meta( $id, '_prices_include_tax', true ) : 'yes' === get_option( 'fincommerce_prices_include_tax' ),
			)
		);
	...
	protected function update_post_meta( &$order ) {
		 ...
		 * @since 2.7.0
		 */
		do_action( 'fincommerce_order_object_updated_props', $order, $updated_props );
	}
	...
	public function read_items( $order, $type ) {
		...
		if ( false === $items ) {
			$items = $wpdb->get_results(
				$wpdb->prepare( "SELECT order_item_type, order_item_id, order_id, order_item_name FROM {$wpdb->prefix}fincommerce_order_items WHERE order_id = %d ORDER BY order_item_id;", $order->get_id() )
			);
	...
	public function get_order_item_type( $order, $order_item_id ) {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT DISTINCT order_item_type FROM {$wpdb->prefix}fincommerce_order_items WHERE order_id = %d and order_item_id = %d;", $order->get_id(), $order_item_id ) );
	}
	...
	public function delete_items( $order, $type = null ) {
		...
		if ( ! empty( $type ) ) {
			$wpdb->query( $wpdb->prepare( "DELETE itemmeta FROM {$wpdb->prefix}fincommerce_order_itemmeta as itemmeta INNER JOIN {$wpdb->prefix}fincommerce_order_items as items WHERE itemmeta.order_item_id = items.order_item_id AND items.order_id = %d AND items.order_item_type = %s", $order->get_id(), $type ) );
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}fincommerce_order_items WHERE order_id = %d AND order_item_type = %s", $order->get_id(), $type ) );
		} else {
			$wpdb->query( $wpdb->prepare( "DELETE itemmeta FROM {$wpdb->prefix}fincommerce_order_itemmeta as itemmeta INNER JOIN {$wpdb->prefix}fincommerce_order_items as items WHERE itemmeta.order_item_id = items.order_item_id and items.order_id = %d", $order->get_id() ) );
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}fincommerce_order_items WHERE order_id = %d", $order->get_id() ) );
		}
	...
	public function get_total_shipping_tax_refunded( $order ) {
		...
		$total = $wpdb->get_var(
			$wpdb->prepare(
				...
				INNER JOIN {$wpdb->prefix}fincommerce_order_items AS order_items ON ( order_items.order_id = posts.ID AND order_items.order_item_type = 'tax' )
