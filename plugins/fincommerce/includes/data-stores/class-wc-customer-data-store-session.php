...
class WC_Customer_Data_Store_Session extends WC_Data_Store_WP implements WC_Customer_Data_Store_Interface, WC_Object_Data_Store_Interface {
	...
	public function save_to_session( $customer ) {
		if ( ! WC()->session ) {
			wc_doing_it_wrong(
				__METHOD__,
				__( 'WC_Session is not available, customer data cannot be saved to session.', 'fincommerce' ),
				'9.8.0'
			);
			return;
		}
		...
		foreach ( $this->session_keys as $session_key ) {
			...
			if ( 'meta_data' === $session_key ) {
				 ...
				 *
				 * If the customer object contains any meta data with these keys, it will be stored within the FinCommerce session.
				 *
				 * @since 8.7.0
				 * @param array $allowed_keys The allowed meta data keys.
				 * @param WC_Customer $customer The customer object.
				 */
				$allowed_keys = apply_filters( 'fincommerce_customer_allowed_session_meta_keys', array(), $customer );
