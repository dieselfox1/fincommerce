...
class WC_Customer_Download_Log_Data_Store implements WC_Customer_Download_Log_Data_Store_Interface {
	...
	public function create( WC_Customer_Download_Log &$download_log ) {
		...
		$result = $wpdb->insert(
			...
			apply_filters( 'fincommerce_downloadable_product_download_log_insert_data', $data ),
			apply_filters( 'fincommerce_downloadable_product_download_log_insert_format', $format, $data )
		);

		do_action( 'fincommerce_downloadable_product_download_log_insert', $data );

		if ( $result ) {
			$download_log->set_id( $wpdb->insert_id );
			$download_log->apply_changes();
		} else {
			wp_die( esc_html__( 'Unable to insert download log entry in database.', 'fincommerce' ) );
		}
	}
	...
	public function read( &$download_log ) {
		...
		if ( ! $download_log->get_id() ) {
			throw new Exception( __( 'Invalid download log: no ID.', 'fincommerce' ) );
		}
		...
		if ( ! $raw_download_log ) {
			throw new Exception( __( 'Invalid download log: not found.', 'fincommerce' ) );
		}
	...
	public function delete_by_permission_id( $id ) {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}fincommerce_downloadable_product_permissions WHERE permission_id = %d", $id ) );
