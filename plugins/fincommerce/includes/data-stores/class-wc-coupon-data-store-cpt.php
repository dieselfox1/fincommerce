...
class WC_Coupon_Data_Store_CPT extends WC_Data_Store_WP implements WC_Coupon_Data_Store_Interface, WC_Object_Data_Store_Interface {
	...
	public function create( &$coupon ) {
		...
		if ( $coupon_id ) {
			...
			do_action( 'fincommerce_new_coupon', $coupon_id, $coupon );
		}
	}
	 ...
	 *
	 * FinCommerce always stores the coupon product ids as a comma-separated string, but it seems that
	...
	public function update( &$coupon ) {
		...
		do_action( 'fincommerce_update_coupon', $coupon->get_id(), $coupon );
	}
	...
	private function update_post_meta( &$coupon ) {
		...
		do_action( 'fincommerce_coupon_object_updated_props', $coupon, $this->updated_props );
	}
	...
	public function increase_usage_count( &$coupon, $used_by = '', $order = null ) {
		...
		do_action( 'fincommerce_increase_coupon_usage_count', $coupon, $new_count, $used_by );
	...
	public function decrease_usage_count( &$coupon, $used_by = '' ) {
		...
		do_action( 'fincommerce_decrease_coupon_usage_count', $coupon, $new_count, $used_by );
	 ...
	 * It will use `fincommerce_coupon_hold_minutes` filter to get the value, defaulting to `fincommerce_hold_stock_minutes` option if set, with a 1-minute minimum if set to 0.
	 * Note that the filter `fincommerce_coupon_hold_minutes` only support minutes because it's getting used elsewhere as well, however this function returns in seconds.
	 *
	 * @return int
	 */
	private function get_tentative_held_time() {
		$default_hold_time_minutes = (int) get_option( 'fincommerce_hold_stock_minutes', 1 );

		if ( 0 >= $default_hold_time_minutes ) {
			// Held time is at least 1 minute if `fincommerce_hold_stock_minutes` is set to 0.
		 ...
		 */
		return (int) apply_filters( 'fincommerce_coupon_hold_minutes', $default_hold_time_minutes ) * 60;
	}
