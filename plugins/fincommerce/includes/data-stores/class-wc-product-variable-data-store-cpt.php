...
class WC_Product_Variable_Data_Store_CPT extends WC_Product_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Product_Variable_Data_Store_Interface {
	...
	public function read_children( &$product, $force_read = false ) {
		...
		if ( ! isset( $children['all'] ) || ! isset( $children['visible'] ) || $force_read ) {
			...
			$children['all']     = get_posts( apply_filters( 'fincommerce_variable_children_args', $all_args, $product, false ) );
			$children['visible'] = get_posts( apply_filters( 'fincommerce_variable_children_args', $visible_only_args, $product, true ) );
	 ...
	 *
	 * Can be filtered by plugins which modify costs, but otherwise will include the raw meta costs unlike get_price() which runs costs through the fincommerce_get_price filter.
		...
		if ( empty( $this->prices_array[ $price_hash ] ) ) {
			...
			if ( empty( $transient_cached_prices_array[ $price_hash ] ) ) {
				...
				foreach ( $variation_ids as $variation_id ) {
					...
					if ( $variation ) {
						$price         = apply_filters( 'fincommerce_variation_prices_price', $variation->get_price( 'edit' ), $variation, $product );
						$regular_price = apply_filters( 'fincommerce_variation_prices_regular_price', $variation->get_regular_price( 'edit' ), $variation, $product );
						$sale_price    = apply_filters( 'fincommerce_variation_prices_sale_price', $variation->get_sale_price( 'edit' ), $variation, $product );
						...
						$prices_array = apply_filters( 'fincommerce_variation_prices_array', $prices_array, $variation, $for_display );
					}
				}
			 ...
			 */
			$this->prices_array[ $price_hash ] = apply_filters( 'fincommerce_variation_prices', $transient_cached_prices_array[ $price_hash ], $product, $for_display );
		}
	...
	protected function get_price_hash( &$product, $for_display = false ) {
		...
		if ( $for_display && wc_tax_enabled() ) {
			$price_hash = array(
				get_option( 'fincommerce_tax_display_shop', 'excl' ),
				WC_Tax::get_rates(),
				empty( WC()->customer ) ? false : WC()->customer->is_vat_exempt(),
			);
		}

		$filter_names = array( 'fincommerce_variation_prices_price', 'fincommerce_variation_prices_regular_price', 'fincommerce_variation_prices_sale_price' );
		...
		return md5( wp_json_encode( apply_filters( 'fincommerce_get_variation_prices_hash', $price_hash, $product, $for_display ) ) );
	}
	...
	public function sync_price( &$product ) {
		 ...
		 */
		do_action( 'fincommerce_updated_product_price', $product->get_id() );
	}
	...
	public function delete_variations( $product_id, $force_delete = false ) {
		...
		if ( ! empty( $variation_ids ) ) {
			foreach ( $variation_ids as $variation_id ) {
				if ( $force_delete ) {
					do_action( 'fincommerce_before_delete_product_variation', $variation_id );
					wp_delete_post( $variation_id, true );
					do_action( 'fincommerce_delete_product_variation', $variation_id );
				} else {
					wp_trash_post( $variation_id );
					do_action( 'fincommerce_trash_product_variation', $variation_id );
				}
			}
		}
