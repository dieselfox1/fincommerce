...
class WC_Customer_Download_Data_Store implements WC_Customer_Download_Data_Store_Interface {
	...
	public function create_from_data( $data ) {
		...
		do_action( 'fincommerce_grant_product_download_access', $data );
	...
	private function insert_new_download_permission( $data ) {
		...
		$result = $wpdb->insert(
			...
			apply_filters( 'fincommerce_downloadable_file_permission_data', $data ),
			apply_filters( 'fincommerce_downloadable_file_permission_format', $format, $data )
		);
	...
	private function adjust_date_for_db( $date ) {
		...
		$msg = sprintf( __( "I don't know how to get a date from a %s", 'fincommerce' ), is_object( $date ) ? get_class( $date ) : gettype( $date ) );
	...
	public function read( &$download ) {
		...
		if ( ! $download->get_id() ) {
			throw new Exception( __( 'Invalid download.', 'fincommerce' ) );
		}

		$download->set_defaults();
		$raw_download = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}fincommerce_downloadable_product_permissions WHERE permission_id = %d",
				$download->get_id()
			)
		);
	...
	private function delete_download_log_by_field_value( $field, $value ) {
		...
		if ( is_int( $value ) ) {
			...
			wc_doing_it_wrong( __METHOD__, __( 'Unsupported argument type provided as value.', 'fincommerce' ), '7.0' );
		...
		$query = "DELETE FROM {$wpdb->prefix}wc_download_log
					WHERE permission_id IN (
					    ...
					    FROM {$wpdb->prefix}fincommerce_downloadable_product_permissions
	...
	public function get_downloads( $args = array() ) {
		...
		$query   = array();
		$query[] = "SELECT {$fields} FROM {$wpdb->prefix}fincommerce_downloadable_product_permissions WHERE 1=1";
	...
	public function get_downloads_for_customer( $customer_id ) {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}fincommerce_downloadable_product_permissions as permissions
				WHERE user_id = %d
