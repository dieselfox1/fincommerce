...
class WC_Customer_Data_Store extends WC_Data_Store_WP implements WC_Customer_Data_Store_Interface, WC_Object_Data_Store_Interface {
	...
	protected function exclude_internal_meta_keys( $meta ) {
		...
		return ! in_array( $meta->meta_key, $this->internal_meta_keys, true )
			&& 0 !== strpos( $meta->meta_key, '_fincommerce_persistent_cart' )
	...
	public function create( &$customer ) {
		...
		wp_update_user(
			apply_filters(
				'fincommerce_update_customer_args',
				array(
					'ID'           => $customer->get_id(),
		...
		do_action( 'fincommerce_new_customer', $customer->get_id(), $customer );
	}
	...
	public function read( &$customer ) {
		...
		if ( ! $user_object || empty( $user_object->ID ) ) {
			throw new Exception( __( 'Invalid customer.', 'fincommerce' ) );
		}
		...
		do_action( 'fincommerce_customer_loaded', $customer );
	}
	...
	public function update( &$customer ) {
		...
		$customer->save_meta_data();
		$customer->apply_changes();
		do_action( 'fincommerce_update_customer', $customer->get_id(), $customer );
	}
	...
	public function delete( &$customer, $args = array() ) {
		...
		do_action( 'fincommerce_delete_customer', $id );
	}
	...
	private function update_user_meta( $customer ) {
		...
		do_action( 'fincommerce_customer_object_updated_props', $customer, $updated_props );
	}
	...
	public function get_last_order( &$customer ) {
		...
		$last_order_id = apply_filters(
			'fincommerce_customer_get_last_order',
			$last_order_id,
			$customer
		);
	...
	public function get_order_count( &$customer ) {
		$count = apply_filters(
			'fincommerce_customer_get_order_count',
	...
	public function get_total_spent( &$customer ) {
		$spent = apply_filters(
			'fincommerce_customer_get_total_spent',
		...
		if ( '' === $spent ) {
			 ...
			 */
			$sql = apply_filters( 'fincommerce_customer_get_total_spent_query', $sql, $customer );
			//phpcs:enable FinCommerce.Commenting.CommentHooks.MissingSinceComment
	...
	public function search_customers( $term, $limit = '' ) {
		$results = apply_filters( 'fincommerce_customer_pre_search_customers', false, $term, $limit );
		if ( is_array( $results ) ) {
			return $results;
		}
		...
		$query = new WP_User_Query(
			apply_filters(
				'fincommerce_customer_search_customers',
				array(
