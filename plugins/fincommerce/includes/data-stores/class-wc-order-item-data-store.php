...
class WC_Order_Item_Data_Store implements WC_Order_Item_Data_Store_Interface {
	...
	public function update_order_item( $item_id, $item ) {
		global $wpdb;
		$updated = $wpdb->update( $wpdb->prefix . 'fincommerce_order_items', $item, array( 'order_item_id' => $item_id ) );
	...
	public function delete_order_item( $item_id ) {
		...
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}fincommerce_order_items WHERE order_item_id = %d", $item_id ) );
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}fincommerce_order_itemmeta WHERE order_item_id = %d", $item_id ) );
	...
	public function get_order_id_by_order_item_id( $item_id ) {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT order_id FROM {$wpdb->prefix}fincommerce_order_items WHERE order_item_id = %d",
				$item_id
			)
		);
	}
	...
	public function get_order_item_type( $item_id ) {
		...
		$order_item_type = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT order_item_type FROM {$wpdb->prefix}fincommerce_order_items WHERE order_item_id = %d LIMIT 1;",
				$item_id
			)
		);
