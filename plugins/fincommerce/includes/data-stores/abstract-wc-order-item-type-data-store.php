...
abstract class Abstract_WC_Order_Item_Type_Data_Store extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface {
	...
	public function create( &$item ) {
		...
		do_action( 'fincommerce_new_order_item', $item->get_id(), $item, $item->get_order_id() );
	}
	...
	public function update( &$item ) {
		...
		do_action( 'fincommerce_update_order_item', $item->get_id(), $item, $item->get_order_id() );
	}
	...
	public function delete( &$item, $args = array() ) {
		if ( $item->get_id() ) {
			global $wpdb;
			do_action( 'fincommerce_before_delete_order_item', $item->get_id() );
			$wpdb->delete( $wpdb->prefix . 'fincommerce_order_items', array( 'order_item_id' => $item->get_id() ) );
			$wpdb->delete( $wpdb->prefix . 'fincommerce_order_itemmeta', array( 'order_item_id' => $item->get_id() ) );
			do_action( 'fincommerce_delete_order_item', $item->get_id() );
			$this->clear_cache( $item );
		}
	}
	...
	public function read( &$item ) {
		...
		if ( false === $data ) {
			$data = $wpdb->get_row( $wpdb->prepare( "SELECT order_id, order_item_name FROM {$wpdb->prefix}fincommerce_order_items WHERE order_item_id = %d LIMIT 1;", $item->get_id() ) );
			wp_cache_set( 'item-' . $item->get_id(), $data, 'order-items' );
		}

		if ( ! $data ) {
			throw new Exception( __( 'Invalid order item.', 'fincommerce' ) );
		}
		...
		if ( $this->cogs_is_enabled && $item->has_cogs() ) {
			 ...
			 */
			$cogs_value = apply_filters( 'fincommerce_load_order_item_cogs_value', $cogs_value, $item );
	...
	private function save_cogs_data( WC_Order_Item $item ) {
		 ...
		 */
		$cogs_value = apply_filters( 'fincommerce_save_order_item_cogs_value', $cogs_value, $item );
