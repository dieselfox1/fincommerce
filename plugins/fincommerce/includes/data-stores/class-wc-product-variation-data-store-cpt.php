...
class WC_Product_Variation_Data_Store_CPT extends WC_Product_Data_Store_CPT implements WC_Object_Data_Store_Interface {
	...
	*/
	...
	public function read( &$product ) {
		...
		if ( 'product_variation' !== $post_object->post_type ) {
			throw new WC_Data_Exception( 'variation_invalid_id', __( 'Invalid product type: passed ID does not correspond to a product variation.', 'fincommerce' ) );
		}
	...
	public function create( &$product ) {
		...
		$id = wp_insert_post(
			apply_filters(
				'fincommerce_new_product_variation_data',
				array(
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			do_action( 'fincommerce_new_product_variation', $id, $product );
		}
	}
	...
	public function update( &$product ) {
		...
		do_action( 'fincommerce_update_product_variation', $product->get_id(), $product );
	}
	...
	*/
	...
	protected function generate_product_title( $product ) {
		...
		$should_include_attributes = apply_filters( 'fincommerce_product_variation_title_include_attributes', $should_include_attributes, $product );
		$separator                 = apply_filters( 'fincommerce_product_variation_title_attributes_separator', ' - ', $product );
		...
		return apply_filters( 'fincommerce_product_variation_title', $title_suffix ? $title_base . $separator . $title_suffix : $title_base, $product, $title_base, $title_suffix );
	}
	...
	protected function load_cogs_data( $product ) {
		 ...
		 */
		$cogs_value_is_additive = apply_filters( 'fincommerce_load_product_cogs_is_additive_flag', $cogs_value_is_additive, $product );
	...
	protected function update_attributes( &$product, $force = false ) {
		...
		if ( $force || array_key_exists( 'attributes', $changes ) ) {
			...
			$delete_attribute_keys = $wpdb->get_col(
				$wpdb->prepare(
					// phpcs:ignore finpress.DB.PreparedSQL.NotPrepared, finpress.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholderGeneration
	...
	public function update_post_meta( &$product, $force = false ) {
		...
		if ( $this->cogs_feature_is_enabled() ) {
			 ...
			 */
			$cogs_value_is_additive = apply_filters( 'fincommerce_save_product_cogs_is_additive_flag', $cogs_value_is_additive, $product );
