 ...
 *
 * @version  3.3.0
 * @package  FinCommerce\Classes\Data_Store
 */
...
class WC_Webhook_Data_Store implements WC_Webhook_Data_Store_Interface {
	...
	public function create( &$webhook ) {
		...
		do_action( 'fincommerce_new_webhook', $webhook_id, $webhook );
	}
	...
	public function read( &$webhook ) {
		...
		if ( is_array( $data ) ) {
			...
			do_action( 'fincommerce_webhook_loaded', $webhook );
		} else {
			throw new Exception( __( 'Invalid webhook.', 'fincommerce' ) );
		}
	}
	...
	public function update( &$webhook ) {
		...
		do_action( 'fincommerce_webhook_updated', $webhook->get_id() );
	}
	...
	public function delete( &$webhook ) {
		...
		wp_cache_delete( $webhook->get_id(), 'webhooks' );
		WC_Cache_Helper::invalidate_cache_group( 'webhooks' );
		do_action( 'fincommerce_webhook_deleted', $webhook->get_id(), $webhook );
	}
	...
	private function get_transient_key( $status = '' ) {
		return empty( $status ) ? 'fincommerce_webhook_ids' : sprintf( 'fincommerce_webhook_ids_status_%s', $status );
	}
