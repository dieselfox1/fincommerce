...
class WC_Payment_Token_Data_Store extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Payment_Token_Data_Store_Interface {
	...
	public function create( &$token ) {
		if ( false === $token->validate() ) {
			throw new Exception( __( 'Invalid or missing payment token fields.', 'fincommerce' ) );
		}
		...
		$wpdb->insert( $wpdb->prefix . 'fincommerce_payment_tokens', $payment_token_data );
		...
		do_action( 'fincommerce_new_payment_token', $token_id, $token );
	}
	...
	public function update( &$token ) {
		...
		do_action( 'fincommerce_payment_token_object_updated_props', $token, $updated_props );
		do_action( 'fincommerce_payment_token_updated', $token->get_id() );
	}
	...
	public function delete( &$token, $force_delete = false ) {
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . 'fincommerce_payment_tokens', array( 'token_id' => $token->get_id() ), array( '%d' ) );
		$wpdb->delete( $wpdb->prefix . 'fincommerce_payment_tokenmeta', array( 'payment_token_id' => $token->get_id() ), array( '%d' ) );
		do_action( 'fincommerce_payment_token_deleted', $token->get_id(), $token );
	}
	...
	public function read( &$token ) {
		...
		$data = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT token, user_id, gateway_id, is_default FROM {$wpdb->prefix}fincommerce_payment_tokens WHERE token_id = %d LIMIT 1",
				$token->get_id()
			)
		);
		...
		if ( $data ) {
			...
			do_action( 'fincommerce_payment_token_loaded', $token );
		} else {
			throw new Exception( __( 'Invalid payment token.', 'fincommerce' ) );
		}
	}
	...
	public function get_tokens( $args ) {
		...
		$sql   = "SELECT * FROM {$wpdb->prefix}fincommerce_payment_tokens";
		$where = array( '1=1' );
	...
	public function get_users_default_token( $user_id ) {
		...
		return $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}fincommerce_payment_tokens WHERE user_id = %d AND is_default = 1",
				$user_id
			)
		);
	}
	...
	public function get_token_by_id( $token_id ) {
		...
		return $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}fincommerce_payment_tokens WHERE token_id = %d",
				$token_id
			)
		);
	}
	...
	public function get_token_type_by_id( $token_id ) {
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				"SELECT type FROM {$wpdb->prefix}fincommerce_payment_tokens WHERE token_id = %d",
				$token_id
			)
		);
	}
