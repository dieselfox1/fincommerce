...
class WC_Product_Data_Store_CPT extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface {
	...
	*/
	...
	public function create( &$product ) {
		...
		$id = wp_insert_post(
			apply_filters(
				'fincommerce_new_product_data',
				array(
					'post_type'      => 'product',
					'post_status'    => $product->get_status() ? $product->get_status() : ProductStatus::PUBLISH,
					'post_author'    => get_current_user_id(),
					'post_title'     => $product->get_name() ? $product->get_name() : __( 'Product', 'fincommerce' ),
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			if ( ! empty( $sku ) && WC()->is_rest_api_request() && ! $this->obtain_lock_on_sku_for_concurrent_requests( $product ) ) {
				$product->delete( true );
				// translators: 1: SKU.
				throw new Exception( esc_html( sprintf( __( 'The product with SKU (%1$s) you are trying to insert is already present in the lookup table', 'fincommerce' ), $sku ) ) );
			}

			// get the post object so that we can set the status
			// to the correct value; it is possible that the status was
			// changed by the fincommerce_new_product_data filter above.
			...
			do_action( 'fincommerce_new_product', $id, $product );
		}
	}
	...
	public function read( &$product ) {
		...
		if ( ! $product->get_id() || ! $post_object || 'product' !== $post_object->post_type ) {
			throw new Exception( __( 'Invalid product.', 'fincommerce' ) );
		}
		 ...
		 */
		do_action( 'fincommerce_product_read', $product->get_id(), $product );
	}
	...
	public function update( &$product ) {
		...
		do_action( 'fincommerce_update_product', $product->get_id(), $product );
	}
	...
	public function delete( &$product, $args = array() ) {
		...
		if ( $args['force_delete'] ) {
			do_action( 'fincommerce_before_delete_' . $post_type, $id );
			wp_delete_post( $id );
			$product->set_id( 0 );
			do_action( 'fincommerce_delete_' . $post_type, $id );
		} else {
			wp_trash_post( $id );
			$product->set_status( ProductStatus::TRASH );
			do_action( 'fincommerce_trash_' . $post_type, $id );
		}
	}
	...
	*/
	...
	protected function load_cogs_data( $product ) {
		 ...
		 */
		$cogs_value = apply_filters( 'fincommerce_load_product_cogs_value', $cogs_value, $product );
	...
	protected function read_downloads( &$product ) {
		...
		if ( $meta_values ) {
			...
			foreach ( $meta_values as $key => $value ) {
				...
				$download->set_file( apply_filters( 'fincommerce_file_download_path', $value['file'], $product, $key ) );
				$downloads[] = $download;
			}
	...
	protected function update_post_meta( &$product, $force = false ) {
			...
			switch ( $prop ) {
				...
				case 'stock_quantity':
					...
					if ( $product->is_type( ProductType::VARIATION ) ) {
						...
						*/
						do_action( 'fincommerce_variation_before_set_stock', $product );
					} else {
						/**
						...
						*/
						do_action( 'fincommerce_product_before_set_stock', $product );
					}
					break;
			}
		...
		if ( $this->cogs_feature_is_enabled() ) {
			 ...
			 */
			$cogs_value = apply_filters( 'fincommerce_save_product_cogs_value', $cogs_value, $product );
	...
	protected function handle_updated_props( &$product ) {
		...
		if ( in_array( 'stock_quantity', $this->updated_props, true ) ) {
			if ( $product->is_type( ProductType::VARIATION ) ) {
				 ...
				 */
				do_action( 'fincommerce_variation_set_stock', $product );
			} else {
				/**
				 ...
				 * @param WC_Product $product The variation whose stock has changed.
				 */
				do_action( 'fincommerce_product_set_stock', $product );
			}
		}
		...
		if ( in_array( 'stock_status', $this->updated_props, true ) ) {
			if ( $product->is_type( ProductType::VARIATION ) ) {
				 ...
				 */
				do_action( 'fincommerce_variation_set_stock_status', $product->get_id(), $product->get_stock_status(), $product );
			} else {
				/**
				 ...
				 * @param WC_Product $product      The product object.
				 */
				do_action( 'fincommerce_product_set_stock_status', $product->get_id(), $product->get_stock_status(), $product );
			}
		}
		...
		do_action( 'fincommerce_product_object_updated_props', $product, $this->updated_props );
	...
	protected function update_visibility( &$product, $force = false ) {
		...
		if ( $force || array_intersect( array( 'featured', 'stock_status', 'average_rating', 'catalog_visibility' ), array_keys( $changes ) ) ) {
			...
			if ( ! is_wp_error( wp_set_post_terms( $product->get_id(), $terms, 'product_visibility', false ) ) ) {
				do_action( 'fincommerce_product_set_visibility', $product->get_id(), $product->get_catalog_visibility() );
			}
		}
	}
	...
	protected function update_attributes( &$product, $force = false ) {
		...
		if ( $force || array_key_exists( 'attributes', $changes ) ) {
			...
			/**
			 * Fires after FinCommerce product attributes have been updated.
			 *
			 * @since 10.2.0
			 ...
			 */
			do_action( 'fincommerce_product_attributes_updated', $product, $force );
		}
	}
	...
	protected function update_downloads( &$product, $force = false ) {
		...
		if ( $force || array_key_exists( 'downloads', $changes ) ) {
			...
			if ( $product->is_type( ProductType::VARIATION ) ) {
				do_action( 'fincommerce_process_product_file_download_paths', $product->get_parent_id(), $product->get_id(), $downloads );
			} else {
				do_action( 'fincommerce_process_product_file_download_paths', $product->get_id(), 0, $downloads );
			}
	...
	protected function update_version_and_type( &$product ) {
		...
		if ( $old_type !== $new_type ) {
			...
			do_action( 'fincommerce_product_type_changed', $product, $old_type, $new_type );
		}
	}
	...
	*/
	...
	public function get_on_sale_products() {
		...
		if ( 'yes' === get_option( 'fincommerce_hide_out_of_stock_items' ) && $product_visibility_term_ids[ ProductStockStatus::OUT_OF_STOCK ] ) {
	...
	public function get_product_id_by_sku( $sku ) {
		...
		return (int) apply_filters( 'fincommerce_get_product_id_by_sku', $id, $sku );
	}
	...
	public function get_product_id_by_global_unique_id( $global_unique_id ) {
		...
		/**
		 * Hook fincommerce_get_product_id_by_global_unique_id.
		 *
		 * @since 9.1.0
		 * @param mixed $id List of post statuses.
		 * @param string $global_unique_id Unique ID.
		 */
		return (int) apply_filters( 'fincommerce_get_product_id_by_global_unique_id', $id, $global_unique_id );
	}
	...
	public function update_product_stock( $product_id_with_stock, $stock_quantity = null, $operation = 'set' ) {
		 ...
		 */
		do_action( 'fincommerce_updated_product_stock', $product_id_with_stock );
	...
	public function update_product_sales( $product_id, $quantity = null, $operation = 'set' ) {
		 ...
		 */
		do_action( 'fincommerce_updated_product_sales', $product_id );
	}
	...
	public function search_products( $term, $type = '', $include_variations = false, $all_statuses = false, $limit = null, $include = null, $exclude = null ) {
		...
		$custom_results = apply_filters( 'fincommerce_product_pre_search_products', false, $term, $type, $include_variations, $all_statuses, $limit );
		...
		/**
		 * Hook fincommerce_search_products_post_statuses.
		 *
		 * @since 3.7.0
		 * @param array $post_statuses List of post statuses.
		 */
		$post_statuses = apply_filters(
			'fincommerce_search_products_post_statuses',
	...
	protected function get_wp_query_args( $query_vars ) {
		...
		return apply_filters( 'fincommerce_product_data_store_cpt_get_products_query', $wp_query_args, $query_vars, $this );
	}
	...
	protected function get_data_for_lookup_table( $id, $table ) {
		if ( 'wc_product_meta_lookup' === $table ) {
			...
			if ( get_option( 'fincommerce_schema_version', 0 ) >= 920 ) {
