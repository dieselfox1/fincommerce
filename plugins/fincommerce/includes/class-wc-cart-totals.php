...
final class WC_Cart_Totals {
	...
	protected function get_fees_from_cart() {
		...
		foreach ( $this->cart->get_fees() as $fee_key => $fee_object ) {
			...
			$fee->taxes     = apply_filters( 'fincommerce_cart_totals_get_fees_from_cart_taxes', $fee->taxes, $fee, $this );
	...
	protected function get_shipping_from_cart() {
		...
		$this->shipping         = array_map(
			function ( $shipping_object ) use ( $default_shipping_props ) {
				...
				$shipping_line->tax_class = get_option( 'fincommerce_shipping_tax_class', 'inherit' );
	...
	protected function get_coupons_from_cart() {
		...
		foreach ( $this->coupons as $coupon ) {
			...
			$coupon->sort = apply_filters( 'fincommerce_coupon_sort', $coupon->sort, $coupon );
		}
	...
	/**
	 * Only ran if fincommerce_adjust_non_base_location_prices is true.
	 *
	...
	protected function get_item_tax_rates( $item ) {
		...
		return apply_filters( 'fincommerce_cart_totals_get_item_tax_rates', $item_tax_rates, $item, $this->cart );
	}
	...
	*/
	...
	protected function calculate_item_totals() {
		...
		foreach ( $this->items as $item_key => $item ) {
			...
			if ( has_filter( 'fincommerce_get_discounted_price' ) ) {
				$item->total = wc_add_number_precision(
					 ...
					 * @since 3.2.0
					 */
					(float) apply_filters( 'fincommerce_get_discounted_price', wc_remove_number_precision( $item->total ), $item->object, $this->cart )
				);
			}

			if ( $this->calculate_tax && $item->product->is_taxable() ) {
				$total_taxes     = apply_filters( 'fincommerce_calculate_item_totals_taxes', WC_Tax::calc_tax( $item->total, $item->tax_rates, $item->price_includes_tax ), $item, $this );
	...
	protected function calculate_item_subtotals() {
		...
		$adjust_non_base_location_prices = apply_filters( 'fincommerce_adjust_non_base_location_prices', true );
	...
	protected function calculate_totals() {
		...
		if ( has_action( 'fincommerce_calculate_totals' ) ) {
			do_action( 'fincommerce_calculate_totals', $this->cart );
		}

		// Allow plugins to filter the grand total, and sum the cart totals in case of modifications.
		$this->cart->set_total( max( 0, apply_filters( 'fincommerce_calculated_total', $this->get_total( 'total' ), $this->cart ) ) );
	}
}
