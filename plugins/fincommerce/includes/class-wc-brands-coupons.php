...
class WC_Brands_Coupons {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_coupon_is_valid', array( $this, 'is_coupon_valid' ), 10, 3 );
		add_filter( 'fincommerce_coupon_is_valid_for_product', array( $this, 'is_valid_for_product' ), 10, 3 );
		add_filter( 'fincommerce_coupon_error', array( $this, 'brand_exclusion_error' ), 10, 2 );
	}
	...
	public function is_coupon_valid( $valid, $coupon, $discounts = null ) {
		...
		if ( ! $included_brands_match && ! empty( $brand_coupon_settings['included_brands'] ) ) {
			throw new Exception( $coupon->get_coupon_error( WC_Coupon::E_WC_COUPON_NOT_APPLICABLE ), WC_Coupon::E_WC_COUPON_NOT_APPLICABLE ); // phpcs:ignore finpress.Security.EscapeOutput.ExceptionNotEscaped
		}

		// 2) All products in the cart match brand exclusion rule.
		if ( count( $items ) === $excluded_brands_matches ) {
			throw new Exception( __( 'Sorry, this coupon is not applicable to the brands of selected products.', 'fincommerce' ), self::E_WC_COUPON_EXCLUDED_BRANDS ); // phpcs:ignore finpress.Security.EscapeOutput.ExceptionNotEscaped
		}
	...
	public function brand_exclusion_error( $err, $err_code ) {
		...
		return __( 'Sorry, this coupon is not applicable to the brands of selected products.', 'fincommerce' );
	}
