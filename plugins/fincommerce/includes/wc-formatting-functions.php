...
// Once FinCommerce requires PHP 7.4, the "$x = $x ?? ''" constructs can be replaced with "$x ??= ''".
...
function wc_get_dimension( $dimension, $to_unit, $from_unit = '' ) {
	...
	if ( empty( $from_unit ) ) {
		$from_unit = strtolower( get_option( 'fincommerce_dimension_unit' ) );
	}
...
function wc_get_weight( $weight, $to_unit, $from_unit = '' ) {
	...
	if ( empty( $from_unit ) ) {
		$from_unit = strtolower( get_option( 'fincommerce_weight_unit' ) );
	}
 ...
 * @param  mixed        $dp number  Number of decimal points to use, blank to use fincommerce_price_num_decimals, or false to avoid all rounding.
...
function wc_format_localized_price( $value ) {
	return apply_filters( 'fincommerce_format_localized_price', str_replace( '.', wc_get_price_decimal_separator(), strval( $value ) ), $value );
}
...
function wc_format_localized_decimal( $value ) {
	...
	return apply_filters( 'fincommerce_format_localized_decimal', str_replace( '.', $decimal, strval( $value ) ), $value );
}
...
function wc_format_coupon_code( $value ) {
	return apply_filters( 'fincommerce_coupon_code', $value );
}
 ...
 *
 * @since  3.6.0
 * @since  10.0.0 Decode HTML entities here instead of via fincommerce_coupon_code filter.
 * @param  string $value Coupon code to format.
 * @return string
 */
...
function wc_stock_amount( $amount ) {
	 ...
	 * @return int|float
	 */
	return NumberUtil::normalize( apply_filters( 'fincommerce_stock_amount', $amount ), intval( $amount ) );
}
 ...
 * @return string
 */
function get_fincommerce_price_format() {
	$currency_pos = get_option( 'fincommerce_currency_pos' );
	$format       = '%1$s%2$s';
	...
	return apply_filters( 'fincommerce_price_format', $format, $currency_pos );
}
...
function wc_get_price_thousand_separator() {
	return stripslashes( apply_filters( 'wc_get_price_thousand_separator', get_option( 'fincommerce_price_thousand_sep' ) ) );
}
...
function wc_get_price_decimal_separator() {
	$separator = apply_filters( 'wc_get_price_decimal_separator', get_option( 'fincommerce_price_decimal_sep' ) );
...
function wc_get_price_decimals() {
	return absint( apply_filters( 'wc_get_price_decimals', get_option( 'fincommerce_price_num_decimals', 2 ) ) );
}
 ...
 *                                      Defaults to empty string (Use the result from get_fincommerce_currency()).
 ...
 *                                      Defaults the result of get_fincommerce_price_format().
...
function wc_price( $price, $args = array() ) {
	$args = apply_filters(
		...
		wp_parse_args(
			...
			array(
				...
				'price_format'       => get_fincommerce_price_format(),
				'in_span'            => true,
				'aria-hidden'        => false,
			)
		)
	 ...
	 */
	$price = apply_filters( 'raw_fincommerce_price', $negative ? $price * -1 : $price, $original_price );
	 ...
	 * @param float|string $original_price     Original price as float, or empty string. Since 5.0.0.
	 */
	$price = apply_filters( 'formatted_fincommerce_price', number_format( $price, $args['decimals'], $args['decimal_separator'], $args['thousand_separator'] ), $price, $args['decimals'], $args['decimal_separator'], $args['thousand_separator'], $original_price );

	if ( apply_filters( 'fincommerce_price_trim_zeros', false ) && $args['decimals'] > 0 ) {
		$price = wc_trim_zeros( $price );
	}

	if ( $args['in_span'] ) {
		$formatted_price = ( $negative ? '-' : '' ) . sprintf( $args['price_format'], '<span class="fincommerce-Price-currencySymbol">' . get_fincommerce_currency_symbol( $args['currency'] ) . '</span>', $price );
		$aria_hidden     = $args['aria-hidden'] ? ' aria-hidden="true"' : '';
		$return          = '<span class="fincommerce-Price-amount amount"' . $aria_hidden . '><bdi class="fincommerce-Price-bidi">' . $formatted_price . '</bdi></span>';
	} else {
		$formatted_price = ( $negative ? '-' : '' ) . sprintf( $args['price_format'], get_fincommerce_currency_symbol( $args['currency'] ), $price );
		$return          = $formatted_price;
	}

	if ( $args['ex_tax_label'] && wc_tax_enabled() ) {
		$return .= ' <small class="fincommerce-Price-taxLabel tax_label">' . WC()->countries->ex_tax_or_vat() . '</small>';
	}
...
/**
 * FinCommerce Date Format - Allows to change date format for everything FinCommerce.
 *
 * @return string
 */
function wc_date_format() {
	...
	return apply_filters( 'fincommerce_date_format', $date_format );
}

/**
 * FinCommerce Time Format - Allows to change time format for everything FinCommerce.
 *
 * @return string
 */
function wc_time_format() {
	...
	return apply_filters( 'fincommerce_time_format', $time_format );
}
...
function wc_format_postcode( $postcode, $country ) {
	...
	return apply_filters( 'fincommerce_format_postcode', trim( $postcode ), $country );
}
...
function wc_format_content( $raw_string ) {
	...
	return apply_filters( 'fincommerce_format_content', apply_filters( 'fincommerce_short_description', $raw_string ), $raw_string );
}
...
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_price_decimal_sep', 'wc_format_option_price_separators', 10, 3 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_price_thousand_sep', 'wc_format_option_price_separators', 10, 3 );
...
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_price_num_decimals', 'wc_format_option_price_num_decimals', 10, 3 );
...
function wc_format_option_hold_stock_minutes( $value, $option, $raw_value ) {
	...
	if ( function_exists( 'as_unschedule_all_actions' ) ) {
		as_unschedule_all_actions( 'fincommerce_cancel_unpaid_orders' );
	} else {
	...
	if ( '' !== $value ) {
		 ...
		 */
		$cancel_unpaid_interval = apply_filters( 'fincommerce_cancel_unpaid_orders_interval_minutes', absint( $value ) );
		...
		if ( function_exists( 'as_schedule_single_action' ) ) {
			...
			wp_schedule_single_event( time() + ( absint( $cancel_unpaid_interval ) * 60 ), 'fincommerce_cancel_unpaid_orders' );
		}
	}

	return $value;
}
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_hold_stock_minutes', 'wc_format_option_hold_stock_minutes', 10, 3 );
...
function wc_format_stock_for_display( $product ) {
	$display      = __( 'In stock', 'fincommerce' );
	$stock_amount = $product->get_stock_quantity();

	switch ( get_option( 'fincommerce_stock_format' ) ) {
		case 'low_amount':
			if ( $stock_amount <= wc_get_low_stock_amount( $product ) ) {
				/* translators: %s: stock amount */
				$display = sprintf( __( 'Only %s left in stock', 'fincommerce' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
			}
			break;
		case '':
			/* translators: %s: stock amount */
			$display = sprintf( __( '%s in stock', 'fincommerce' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
			break;
	}

	if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
		$display .= ' ' . __( '(can be backordered)', 'fincommerce' );
	}
...
function wc_format_stock_quantity_for_display( $stock_quantity, $product ) {
	return apply_filters( 'fincommerce_format_stock_quantity', $stock_quantity, $product );
}
...
function wc_format_sale_price( $regular_price, $sale_price ) {
	...
	$price .= esc_html( sprintf( __( 'Original price was: %s.', 'fincommerce' ), wp_strip_all_tags( $formatted_regular_price ) ) );
	$price .= '</span>';

	// Add the sale price.
	$price .= '<ins class="fincommerce-Price-salePrice" aria-hidden="true">' . $formatted_sale_price . '</ins>';

	// For accessibility (a11y) we'll also display that information to screen readers.
	$price .= '<span class="screen-reader-text">';
	// translators: %s is a product's current (sale) price.
	$price .= esc_html( sprintf( __( 'Current price is: %s.', 'fincommerce' ), wp_strip_all_tags( $formatted_sale_price ) ) );
	$price .= '</span>';

	return apply_filters( 'fincommerce_format_sale_price', $price, $regular_price, $sale_price );
}
...
function wc_format_price_range( $from, $to ) {
	...
	$price  = sprintf( _x( '%1$s <span aria-hidden="true">&ndash;</span> %2$s', 'Price range: from-to', 'fincommerce' ), is_numeric( $from ) ? wc_price( $from, array( 'aria-hidden' => true ) ) : $from, is_numeric( $to ) ? wc_price( $to, array( 'aria-hidden' => true ) ) : $to );
	$price .= '<span class="screen-reader-text">';
	$price .= sprintf(
		/* translators: 1: price from 2: price to */
		__( 'Price range: %1$s through %2$s', 'fincommerce' ),
	...
	$price .= '</span>';
	return apply_filters( 'fincommerce_format_price_range', $price, $from, $to );
}
...
function wc_format_weight( $weight ) {
	...
	if ( ! empty( $weight_string ) ) {
		$weight_label = I18nUtil::get_weight_unit_label( get_option( 'fincommerce_weight_unit' ) );

		$weight_string = sprintf(
			// translators: 1. A formatted number; 2. A label for a weight unit of measure. E.g. 2.72 kg.
			_x( '%1$s %2$s', 'formatted weight', 'fincommerce' ),
			$weight_string,
			$weight_label
		);
	} else {
		$weight_string = __( 'N/A', 'fincommerce' );
	}

	return apply_filters( 'fincommerce_format_weight', $weight_string, $weight );
}
...
function wc_format_dimensions( $dimensions ) {
	...
	if ( ! empty( $dimension_string ) ) {
		$dimension_label = I18nUtil::get_dimensions_unit_label( get_option( 'fincommerce_dimension_unit' ) );

		$dimension_string = sprintf(
			// translators: 1. A formatted number; 2. A label for a dimensions unit of measure. E.g. 3.14 cm.
			_x( '%1$s %2$s', 'formatted dimensions', 'fincommerce' ),
			$dimension_string,
			$dimension_label
		);
	} else {
		$dimension_string = __( 'N/A', 'fincommerce' );
	}

	return apply_filters( 'fincommerce_format_dimensions', $dimension_string, $dimensions );
}
...
function wc_parse_relative_date_option( $raw_value ) {
	$periods = array(
		'days'   => __( 'Day(s)', 'fincommerce' ),
		'weeks'  => __( 'Week(s)', 'fincommerce' ),
		'months' => __( 'Month(s)', 'fincommerce' ),
		'years'  => __( 'Year(s)', 'fincommerce' ),
	);
...
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_checkout_pay_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_checkout_order_received_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_add_payment_method_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_delete_payment_method_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_set_default_payment_method_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_orders_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_view_order_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_downloads_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_edit_account_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_edit_address_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_payment_methods_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_myaccount_lost_password_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_logout_endpoint', 'wc_sanitize_endpoint_slug', 10, 1 );
