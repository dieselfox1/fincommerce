<?php
/**
 * FinCommerce countries
 *
 * @package FinCommerce\l10n
 * @version 3.3.0
 */
...
/**
 * The FinCommerce countries class stores country/state data.
 */
class WC_Countries {
	...
	public function get_countries() {
		if ( empty( $this->geo_cache['countries'] ) ) {
			 ...
			 */
			$this->geo_cache['countries'] = apply_filters( 'fincommerce_countries', include WC()->plugin_path() . '/i18n/countries.php' );
			if ( apply_filters( 'fincommerce_sort_countries', true ) ) {
	...
	public function get_continents() {
		if ( empty( $this->geo_cache['continents'] ) ) {
			 ...
			 */
			$this->geo_cache['continents'] = apply_filters( 'fincommerce_continents', include WC()->plugin_path() . '/i18n/continents.php' );
		}
	...
	public function load_country_states() {
		 ...
		 * @param array $states
		 */
		$this->geo_cache['states'] = apply_filters( 'fincommerce_states', $states );
	}
	...
	public function get_states( $cc = null ) {
		if ( ! isset( $this->geo_cache['states'] ) ) {
			 ...
			 * @param array $states
			 */
			$this->geo_cache['states'] = apply_filters( 'fincommerce_states', include WC()->plugin_path() . '/i18n/states.php' );
		}
	...
	public function get_base_address() {
		$base_address = get_option( 'fincommerce_store_address', '' );
		return apply_filters( 'fincommerce_countries_base_address', $base_address );
	}
	...
	public function get_base_address_2() {
		$base_address_2 = get_option( 'fincommerce_store_address_2', '' );
		return apply_filters( 'fincommerce_countries_base_address_2', $base_address_2 );
	}
	...
	public function get_base_country() {
		...
		return apply_filters( 'fincommerce_countries_base_country', $default['country'] );
	}
	...
	public function get_base_state() {
		$default = wc_get_base_location();
		return apply_filters( 'fincommerce_countries_base_state', $default['state'] );
	}
	...
	public function get_base_city() {
		$base_city = get_option( 'fincommerce_store_city', '' );
		return apply_filters( 'fincommerce_countries_base_city', $base_city );
	}
	...
	public function get_base_postcode() {
		$base_postcode = get_option( 'fincommerce_store_postcode', '' );
		return apply_filters( 'fincommerce_countries_base_postcode', $base_postcode );
	}
	...
	public function get_allowed_countries() {
		...
		$allowed_countries = get_option( 'fincommerce_allowed_countries' );

		if ( 'all_except' === $allowed_countries ) {
			$except_countries = get_option( 'fincommerce_all_except_countries', array() );
			...
			$countries     = array();
			$raw_countries = get_option( 'fincommerce_specific_allowed_countries', array() );
		 ...
		 * @param array $countries
		 */
		return apply_filters( 'fincommerce_countries_allowed_countries', $countries );
	}
	...
	public function get_shipping_countries() {
		...
		if ( 'disabled' === get_option( 'fincommerce_ship_to_countries' ) ) {
			return array();
		}
		...
		if ( 'all' === get_option( 'fincommerce_ship_to_countries' ) ) {
			$countries = $this->countries;
		} elseif ( 'specific' === get_option( 'fincommerce_ship_to_countries' ) ) {
			$countries     = array();
			$raw_countries = get_option( 'fincommerce_specific_ship_to_countries', array() );
		 ...
		 * @param array $countries
		 */
		return apply_filters( 'fincommerce_countries_shipping_countries', $countries );
	}
	...
	public function get_allowed_country_states() {
		if ( get_option( 'fincommerce_allowed_countries' ) !== 'specific' ) {
			return $this->states;
		}

		$states = array();

		$raw_countries = get_option( 'fincommerce_specific_allowed_countries' );
		...
		return apply_filters( 'fincommerce_countries_allowed_country_states', $states );
	}
	...
	public function get_shipping_country_states() {
		if ( get_option( 'fincommerce_ship_to_countries' ) === '' ) {
			return $this->get_allowed_country_states();
		}

		if ( get_option( 'fincommerce_ship_to_countries' ) !== 'specific' ) {
			return $this->states;
		}

		$states = array();

		$raw_countries = get_option( 'fincommerce_specific_ship_to_countries' );
		...
		return apply_filters( 'fincommerce_countries_shipping_country_states', $states );
	}
	...
	public function get_european_union_countries( $type = '' ) {
		...
		return apply_filters( 'fincommerce_european_union_countries', $countries, $type );
	}
	...
	public function countries_using_vat() {
		...
		return apply_filters( 'fincommerce_countries_using_vat', $countries );
	}
	...
	public function get_vat_countries() {
		...
		return apply_filters( 'fincommerce_vat_countries', array_merge( $eu_countries, $vat_countries ) );
	}
	...
	public function shipping_to_prefix( $country_code = '' ) {
		...
		$return       = in_array( $country_code, $countries, true ) ? _x( 'to the', 'shipping country prefix', 'fincommerce' ) : _x( 'to', 'shipping country prefix', 'fincommerce' );

		return apply_filters( 'fincommerce_countries_shipping_to_prefix', $return, $country_code );
	}
	...
	public function estimated_for_prefix( $country_code = '' ) {
		...
		$countries    = array( 'AE', 'CZ', 'DO', 'GB', 'NL', 'PH', 'US', 'USAF' );
		$return       = in_array( $country_code, $countries, true ) ? __( 'the', 'fincommerce' ) . ' ' : '';

		return apply_filters( 'fincommerce_countries_estimated_for_prefix', $return, $country_code );
	}
	...
	public function tax_or_vat() {
		$return = in_array( $this->get_base_country(), $this->get_vat_countries(), true ) ? __( 'VAT', 'fincommerce' ) : __( 'Tax', 'fincommerce' );

		return apply_filters( 'fincommerce_countries_tax_or_vat', $return );
	}
	...
	public function inc_tax_or_vat() {
		$return = in_array( $this->get_base_country(), $this->get_vat_countries(), true ) ? __( '(incl. VAT)', 'fincommerce' ) : __( '(incl. tax)', 'fincommerce' );

		return apply_filters( 'fincommerce_countries_inc_tax_or_vat', $return );
	}
	...
	public function ex_tax_or_vat() {
		$return = in_array( $this->get_base_country(), $this->get_vat_countries(), true ) ? __( '(ex. VAT)', 'fincommerce' ) : __( '(ex. tax)', 'fincommerce' );

		return apply_filters( 'fincommerce_countries_ex_tax_or_vat', $return );
	}
	...
	public function get_address_formats() {
		if ( empty( $this->address_formats ) ) {
			$this->address_formats = apply_filters(
				'fincommerce_localisation_address_formats',
				array(
					'default' => "{name}\n{company}\n{address_1}\n{address_2}\n{city}\n{state}\n{postcode}\n{country}",
					'AT'      => "{company}\n{name}\n{address_1}\n{address_2}\n{postcode} {city}\n{country}",
					'AU'      => "{name}\n{company}\n{address_1}\n{address_2}\n{city} {state} {postcode}\n{country}",
	...
	public function get_formatted_address( $args = array(), $separator = '<br/>' ) {
		...
		if ( $country === $this->get_base_country() && ! apply_filters( 'fincommerce_formatted_address_force_country_display', false ) ) {
		...
		$replace = array_map(
			'esc_html',
			apply_filters(
				'fincommerce_formatted_address_replacements',
				array(
					'{first_name}'       => $args['first_name'],
					'{last_name}'        => $args['last_name'],
					'{name}'             => sprintf(
						/* translators: 1: first name 2: last name */
						_x( '%1$s %2$s', 'full name', 'fincommerce' ),
						$args['first_name'],
						$args['last_name']
					),
	...
	public function get_default_address_fields() {
		$address_2_label = __( 'Apartment, suite, unit, etc.', 'fincommerce' );

		// If necessary, append '(optional)' to the placeholder: we don't need to worry about the
		// label, though, as fincommerce_form_field() takes care of that.
		if ( 'optional' === CartCheckoutUtils::get_address_2_field_visibility() ) {
			$address_2_placeholder = __( 'Apartment, suite, unit, etc. (optional)', 'fincommerce' );
		} else {
		...
		$fields = array(
			'first_name' => array(
				'label'        => __( 'First name', 'fincommerce' ),
				'required'     => true,
			...
			'last_name'  => array(
				'label'        => __( 'Last name', 'fincommerce' ),
				'required'     => true,
			...
			'company'    => array(
				'label'        => __( 'Company name', 'fincommerce' ),
			...
			'address_1'  => array(
				'label'        => __( 'Street address', 'fincommerce' ),
				/* translators: use local order of street name and house number. */
				'placeholder'  => esc_attr__( 'House number and street name', 'fincommerce' ),
				'required'     => true,
			...
			'city'       => array(
				'label'        => __( 'Town / City', 'fincommerce' ),
				'required'     => true,
				'class'        => array( 'form-row-wide', 'address-field' ),
			...
			'state'      => array(
				'type'         => 'state',
				'label'        => __( 'State / County', 'fincommerce' ),
				'required'     => true,
				'class'        => array( 'form-row-wide', 'address-field' ),
			...
			'postcode'   => array(
				'label'        => __( 'Postcode / ZIP', 'fincommerce' ),
				'required'     => true,
				'class'        => array( 'form-row-wide', 'address-field' ),
		...
		$default_address_fields = apply_filters( 'fincommerce_default_address_fields', $fields );
	...
	public function get_country_locale_field_selectors() {
		...
		return apply_filters( 'fincommerce_country_locale_field_selectors', $locale_fields );
	}
	...
	public function get_country_locale() {
		if ( empty( $this->locale ) ) {
			$this->locale = apply_filters(
				'fincommerce_get_country_locale',
				array(
					'AE' => array(
						'postcode' => array(
							'required' => false,
					...
					'AU' => array(
						'city'     => array(
							'label' => __( 'Suburb', 'fincommerce' ),
						),
						'postcode' => array(
							'label' => __( 'Postcode', 'fincommerce' ),
						),
						'state'    => array(
							'label' => __( 'State', 'fincommerce' ),
						),
					),
					'AX' => array(
						'postcode' => array(
					...
					'BO' => array(
						...
						'state'    => array(
							'label' => __( 'Department', 'fincommerce' ),
						),
					),
					'BS' => array(
						'postcode' => array(
					...
					'CL' => array(
						...
						'postcode' => array(
							'required' => false,
							// Hidden for stores within Chile. @see https://github.com/dieselfox1/fincommerce/issues/36546.
					...
					'HK' => array(
						...
						'city'     => array(
							'label' => __( 'Town / District', 'fincommerce' ),
						),
						'state'    => array(
							'label' => __( 'Region', 'fincommerce' ),
						),
					...
					'JM' => array(
						'city'     => array(
							'label' => __( 'Town / City / Post Office', 'fincommerce' ),
						),
						'postcode' => array(
							'required' => false,
							'label'    => __( 'Postal Code', 'fincommerce' ),
					...
					'JP' => array(
						...
						'state'      => array(
							'label'    => __( 'Prefecture', 'fincommerce' ),
							'class'    => array( 'form-row-last', 'address-field' ),
							'priority' => 66,
						),
						'city'       => array(
					...
					'LV' => array(
						'state' => array(
							'label'    => __( 'Municipality', 'fincommerce' ),
							'required' => false,
						),
					),
					'LB' => array(
					...
					'NP' => array(
						'state'    => array(
							'label' => __( 'State / Zone', 'fincommerce' ),
						),
						'postcode' => array(
							'required' => false,
						),
					...
					'MD' => array(
						'state' => array(
							'label' => __( 'Municipality / District', 'fincommerce' ),
						),
					),
					'SE' => array(
						'postcode' => array(
					...
					'UG' => array(
						...
						'city'     => array(
							'label'    => __( 'Town / Village', 'fincommerce' ),
							'required' => true,
						),
						'state'    => array(
							'label'    => __( 'District', 'fincommerce' ),
			...
			$this->locale['default'] = apply_filters( 'fincommerce_get_country_locale_default', $this->get_default_address_fields() );
			...
			$this->locale['default']                   = apply_filters( 'fincommerce_get_country_locale_base', $this->locale['default'] );
			$this->locale[ $this->get_base_country() ] = apply_filters( 'fincommerce_get_country_locale_base', $this->locale[ $this->get_base_country() ] );
		}
	...
	public function get_address_fields( $country = '', $type = 'billing_' ) {
		...
		if ( 'billing_' === $type ) {
			...
			$address_fields['billing_email'] = array(
				'label'        => __( 'Email address', 'fincommerce' ),
				'required'     => true,
				'type'         => 'email',
				'class'        => array( 'form-row-wide' ),
		 ...
		 * the fincommerce_default_address_fields. The locales/default locales apply on top based
		 * on country selection. If you want to change things like the required status of an
		 * address field, filter fincommerce_default_address_fields instead.
		 */
		$address_fields = apply_filters( 'fincommerce_' . $type . 'fields', $address_fields, $country );
		// Sort each of the fields based on priority.
