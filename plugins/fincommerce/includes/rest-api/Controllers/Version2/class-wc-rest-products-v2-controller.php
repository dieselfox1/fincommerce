...
class WC_REST_Products_V2_Controller extends WC_REST_CRUD_Controller {
	...
	public function __construct() {
		add_action( "fincommerce_rest_insert_{$this->post_type}_object", array( $this, 'clear_transients' ) );
	}
	...
	public function prepare_object_for_response( $object, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( "fincommerce_rest_prepare_{$this->post_type}_object", $response, $object, $request );
	}
		...
		foreach ( $fields as $field ) {
			switch ( $field ) {
				...
				case 'short_description':
					$base_data['short_description'] = 'view' === $context ? apply_filters( 'fincommerce_short_description', $product->get_short_description() ) : $product->get_short_description( $context );
					break;
				case 'sku':
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		...
		if ( ProductType::VARIATION === $product->get_type() ) {
			return new WP_Error(
				"fincommerce_rest_invalid_{$this->post_type}_id",
				__( 'To manipulate product variations you should use the /products/&lt;product_id&gt;/variations/&lt;id&gt; endpoint.', 'fincommerce' ),
				array(
					'status' => 404,
				)
			);
		 ...
		 * @param bool            $creating If is creating a new object.
		 */
		return apply_filters( "fincommerce_rest_pre_insert_{$this->post_type}_object", $product, $request, $creating );
	}
	...
	public function delete_item( $request ) {
		...
		if ( ! $object || 0 === $object->get_id() ) {
			return new WP_Error(
				"fincommerce_rest_{$this->post_type}_invalid_id",
				__( 'Invalid ID.', 'fincommerce' ),
				array(
					'status' => 404,
				)
			);
		 ...
		 */
		$supports_trash = apply_filters( "fincommerce_rest_{$this->post_type}_object_trashable", $supports_trash, $object );

		if ( ! wc_rest_check_post_permissions( $this->post_type, 'delete', $object->get_id() ) ) {
			return new WP_Error(
				"fincommerce_rest_user_cannot_delete_{$this->post_type}",
				/* translators: %s: post type */
				sprintf( __( 'Sorry, you are not allowed to delete %s.', 'fincommerce' ), $this->post_type ),
				array(
		...
		if ( $force ) {
			...
			if ( ! $supports_trash ) {
				return new WP_Error(
					'fincommerce_rest_trash_not_supported',
					/* translators: %s: post type */
					sprintf( __( 'The %s does not support trashing.', 'fincommerce' ), $this->post_type ),
					array(
						'status' => 501,
					)
				);
			...
			if ( is_callable( array( $object, 'get_status' ) ) ) {
				if ( ProductStatus::TRASH === $object->get_status() ) {
					return new WP_Error(
						'fincommerce_rest_already_trashed',
						/* translators: %s: post type */
						sprintf( __( 'The %s has already been deleted.', 'fincommerce' ), $this->post_type ),
						array(
							'status' => 410,
						)
					);
		...
		if ( ! $result ) {
			return new WP_Error(
				'fincommerce_rest_cannot_delete',
				/* translators: %s: post type */
				sprintf( __( 'The %s cannot be deleted.', 'fincommerce' ), $this->post_type ),
				array(
					'status' => 500,
				)
			);
		 ...
		 */
		do_action( "fincommerce_rest_delete_{$this->post_type}_object", $object, $response, $request );
		...
		$schema               = array(
			...
			'properties' => array(
				...
				'date_created_gmt'      => array(
					'description' => __( 'The date the product was created, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				...
				'date_modified_gmt'     => array(
					'description' => __( 'The date the product was last modified, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				...
				'date_on_sale_from'     => array(
					'description' => __( "Start date of sale price, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
				),
				'date_on_sale_from_gmt' => array(
					'description' => __( 'Start date of sale price, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
				),
				'date_on_sale_to'       => array(
					'description' => __( "End date of sale price, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
				),
				'date_on_sale_to_gmt'   => array(
					'description' => __( 'End date of sale price, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
				),
				'price_html'            => array(
				...
				'upsell_ids'            => array(
					'description' => __( 'List of up-sell products IDs.', 'fincommerce' ),
					'type'        => 'array',
					'items'       => array(
						'type' => 'integer',
					),
				...
				'images'                => array(
					...
					'items'       => array(
						...
						'properties' => array(
							...
							'date_created_gmt'  => array(
								'description' => __( 'The date the image was created, as GMT.', 'fincommerce' ),
								'type'        => 'date-time',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							...
							'date_modified_gmt' => array(
								'description' => __( 'The date the image was last modified, as GMT.', 'fincommerce' ),
								'type'        => 'date-time',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
				...
				'variations'            => array(
					'description' => __( 'List of variations IDs.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type' => 'integer',
	...
	public function get_collection_params() {
		...
		$params['sku']            = array(
			'description'       => __( 'Limit result set to products with specific SKU(s). Use commas to separate.', 'fincommerce' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['featured']       = array(
			'description'       => __( 'Limit result set to featured products.', 'fincommerce' ),
			'type'              => 'boolean',
		...
		$params['attribute']      = array(
			'description'       => __( 'Limit result set to products with a specific attribute. Use the taxonomy name/attribute slug.', 'fincommerce' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		if ( wc_tax_enabled() ) {
			$params['tax_class'] = array(
				'description'       => __( 'Limit result set to products with a specific tax class.', 'fincommerce' ),
				'type'              => 'string',
		...
		$params['in_stock']     = array(
			'description'       => __( 'Limit result set to products in stock or out of stock.', 'fincommerce' ),
			'type'              => 'boolean',
			'sanitize_callback' => 'wc_string_to_bool',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['on_sale']      = array(
			'description'       => __( 'Limit result set to products on sale.', 'fincommerce' ),
			'type'              => 'boolean',
			'sanitize_callback' => 'wc_string_to_bool',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['min_price']    = array(
			'description'       => __( 'Limit result set to products based on a minimum price.', 'fincommerce' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['max_price']    = array(
			'description'       => __( 'Limit result set to products based on a maximum price.', 'fincommerce' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['include_meta'] = array(
			'default'           => array(),
			'description'       => __( 'Limit meta_data to specific keys.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
			),
		...
		$params['exclude_meta'] = array(
			'default'           => array(),
			'description'       => __( 'Ensure meta_data excludes specific keys.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
			),
