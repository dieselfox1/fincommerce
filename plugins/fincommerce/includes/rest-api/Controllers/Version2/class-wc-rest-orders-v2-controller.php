...
class WC_REST_Orders_V2_Controller extends WC_REST_CRUD_Controller {
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ( ! $object || 0 === $object->get_id() ) && ! wc_rest_check_post_permissions( $this->post_type, 'read' ) ) {
			return new WP_Error( 'fincommerce_rest_cannot_edit', __( 'Sorry, you are not allowed to edit this resource.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ( ! $object || 0 === $object->get_id() ) && ! wc_rest_check_post_permissions( $this->post_type, 'read' ) ) {
			return new WP_Error( 'fincommerce_rest_cannot_delete', __( 'Sorry, you are not allowed to delete this resource.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	protected function prepare_objects_query( $request ) {
		...
		if ( ! empty( $request['product'] ) ) {
			$order_ids = $wpdb->get_col(
				$wpdb->prepare(
					"SELECT order_id
					FROM {$wpdb->prefix}fincommerce_order_items
					WHERE order_item_id IN ( SELECT order_item_id FROM {$wpdb->prefix}fincommerce_order_itemmeta WHERE meta_key = '_product_id' AND meta_value = %d )
		 ...
		 * @since 4.5.0.
		 */
		$args = apply_filters( 'fincommerce_rest_orders_prepare_object_query', $args, $request );
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		 ...
		 * @since 4.5.0.
		 */
		return apply_filters( "fincommerce_rest_pre_insert_{$this->post_type}_object", $order, $request, $creating );
	}
	...
	protected function save_object( $request, $creating = false ) {
		try {
			...
			if ( $creating ) {
				...
				$object->set_prices_include_tax( 'yes' === get_option( 'fincommerce_prices_include_tax' ) );
				$object->save();
	...
	protected function get_product_id( $posted, $action = 'create' ) {
		if ( ! empty( $posted['sku'] ) ) {
			...
			throw new WC_REST_Exception( 'fincommerce_rest_required_product_reference', __( 'Product ID or SKU is required.', 'fincommerce' ), 400 );
		}
		return $product_id;
	}
	...
	protected function prepare_shipping_lines( $posted, $action = 'create', $item = null ) {
		...
		if ( 'create' === $action ) {
			if ( empty( $posted['method_id'] ) ) {
				throw new WC_REST_Exception( 'fincommerce_rest_invalid_shipping_item', __( 'Shipping method ID is required.', 'fincommerce' ), 400 );
			}
		}
	...
	protected function prepare_fee_lines( $posted, $action = 'create', $item = null ) {
		...
		if ( 'create' === $action ) {
			if ( empty( $posted['name'] ) ) {
				throw new WC_REST_Exception( 'fincommerce_rest_invalid_fee_item', __( 'Fee name is required.', 'fincommerce' ), 400 );
			}
		}
	...
	protected function prepare_coupon_lines( $posted, $action = 'create', $item = null ) {
		...
		if ( 'create' === $action ) {
			...
			if ( StringUtil::is_null_or_whitespace( $coupon_code ) ) {
				throw new WC_REST_Exception( 'fincommerce_rest_invalid_coupon_coupon', __( 'Coupon code is required.', 'fincommerce' ), 400 );
			}
		}
	...
	protected function set_item( $order, $item_type, $posted ) {
		...
		if ( 'update' === $action ) {
			...
			if ( ! $item ) {
				throw new WC_REST_Exception( 'fincommerce_rest_invalid_item_id', __( 'Order item ID provided is not associated with order.', 'fincommerce' ), 400 );
			}
		}
		 ...
		 * @since 4.5.0.
		 */
		do_action( 'fincommerce_rest_set_order_item', $item, $posted );
			...
			if ( $changed_stock && ! is_wp_error( $changed_stock ) ) {
				$order->add_order_note(
					sprintf(
						...
						__( 'Adjusted stock: %s', 'fincommerce' ),
						sprintf(
							'%1$s (%2$s&rarr;%3$s)',
							$item->get_name(),
							$changed_stock['from'],
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'parent_id'            => array(
					'description' => __( 'Parent order ID.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'number'               => array(
					'description' => __( 'Order number.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				...
				'created_via'          => array(
					'description' => __( 'Shows where the order was created.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'version'              => array(
					'description' => __( 'Version of FinCommerce which last updated the order.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'status'               => array(
					'description' => __( 'Order status.', 'fincommerce' ),
					'type'        => 'string',
				...
				'currency'             => array(
					'description' => __( 'Currency the order was created with, in ISO format.', 'fincommerce' ),
					'type'        => 'string',
					'default'     => get_fincommerce_currency(),
					'enum'        => array_keys( get_fincommerce_currencies() ),
					'context'     => array( 'view', 'edit' ),
				),
				'date_created'         => array(
					'description' => __( "The date the order was created, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_created_gmt'     => array(
					'description' => __( 'The date the order was created, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_modified'        => array(
					'description' => __( "The date the order was last modified, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_modified_gmt'    => array(
					'description' => __( 'The date the order was last modified, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'discount_total'       => array(
					'description' => __( 'Total discount amount for the order.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'discount_tax'         => array(
					'description' => __( 'Total discount tax amount for the order.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'shipping_total'       => array(
					'description' => __( 'Total shipping amount for the order.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'shipping_tax'         => array(
					'description' => __( 'Total shipping tax amount for the order.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'cart_tax'             => array(
					'description' => __( 'Sum of line item taxes only.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'total'                => array(
					'description' => __( 'Grand total.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'total_tax'            => array(
					'description' => __( 'Sum of all taxes.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'prices_include_tax'   => array(
					'description' => __( 'True the prices included tax during checkout.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'customer_id'          => array(
					'description' => __( 'User ID who owns the order. 0 for guests.', 'fincommerce' ),
					'type'        => 'integer',
					'default'     => 0,
					'context'     => array( 'view', 'edit' ),
				),
				'customer_ip_address'  => array(
					'description' => __( "Customer's IP address.", 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'customer_user_agent'  => array(
					'description' => __( 'User agent of the customer.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'customer_note'        => array(
					'description' => __( 'Note left by customer during checkout.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'billing'              => array(
					'description' => __( 'Billing address.', 'fincommerce' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'first_name' => array(
						...
						'country'    => array(
							'description' => __( 'Country code in ISO 3166-1 alpha-2 format.', 'fincommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'email'      => array(
				...
				'shipping'             => array(
					'description' => __( 'Shipping address.', 'fincommerce' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'first_name' => array(
				...
				'payment_method'       => array(
					'description' => __( 'Payment method ID.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'payment_method_title' => array(
					'description' => __( 'Payment method title.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'arg_options' => array(
				...
				'transaction_id'       => array(
					'description' => __( 'Unique transaction ID.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'date_paid'            => array(
					'description' => __( "The date the order was paid, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_paid_gmt'        => array(
					'description' => __( 'The date the order was paid, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_completed'       => array(
					'description' => __( "The date the order was completed, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_completed_gmt'   => array(
					'description' => __( 'The date the order was completed, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'cart_hash'            => array(
					'description' => __( 'MD5 hash of cart items to ensure orders are not modified.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				...
				'line_items'           => array(
					'description' => __( 'Line items data.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'       => 'object',
						'properties' => array(
							'id'               => array(
								'description' => __( 'Item ID.', 'fincommerce' ),
								'type'        => 'integer',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							...
							'parent_name'      => array(
								'description' => __( 'Parent product name if the product is a variation.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'product_id'       => array(
								'description' => __( 'Product ID.', 'fincommerce' ),
								'type'        => 'mixed',
								'context'     => array( 'view', 'edit' ),
							),
							'variation_id'     => array(
								'description' => __( 'Variation ID, if applicable.', 'fincommerce' ),
								'type'        => 'integer',
								'context'     => array( 'view', 'edit' ),
							),
							'quantity'         => array(
								'description' => __( 'Quantity ordered.', 'fincommerce' ),
								'type'        => 'integer',
								'context'     => array( 'view', 'edit' ),
							),
							'tax_class'        => array(
								'description' => __( 'Tax class of product.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'subtotal'         => array(
								'description' => __( 'Line subtotal (before discounts).', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'subtotal_tax'     => array(
								'description' => __( 'Line subtotal tax (before discounts).', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'total'            => array(
								'description' => __( 'Line total (after discounts).', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'total_tax'        => array(
								'description' => __( 'Line total tax (after discounts).', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'taxes'            => array(
								'description' => __( 'Line taxes.', 'fincommerce' ),
								'type'        => 'array',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
								'items'       => array(
									...
									'properties' => array(
										'id'       => array(
											'description' => __( 'Tax rate ID.', 'fincommerce' ),
											'type'        => 'integer',
											'context'     => array( 'view', 'edit' ),
										),
										'total'    => array(
											'description' => __( 'Tax total.', 'fincommerce' ),
											'type'        => 'string',
											'context'     => array( 'view', 'edit' ),
										),
										'subtotal' => array(
											'description' => __( 'Tax subtotal.', 'fincommerce' ),
											'type'        => 'string',
											'context'     => array( 'view', 'edit' ),
										),
									),
							...
							'meta_data'        => array(
								...
								'items'       => array(
									...
									'properties' => array(
										...
										'display_key'   => array(
											'description' => __( 'Meta key for UI display.', 'fincommerce' ),
											'type'        => 'string',
											'context'     => array( 'view', 'edit' ),
										),
										'display_value' => array(
											'description' => __( 'Meta value for UI display.', 'fincommerce' ),
											'type'        => 'string',
											'context'     => array( 'view', 'edit' ),
										),
									),
							...
							'sku'              => array(
								'description' => __( 'Product SKU.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							...
							'price'            => array(
								'description' => __( 'Product price.', 'fincommerce' ),
								'type'        => 'number',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'image'            => array(
								'description' => __( 'Properties of the main product image.', 'fincommerce' ),
								'type'        => 'object',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
								'properties'  => array(
				...
				'tax_lines'            => array(
					'description' => __( 'Tax lines data.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
					'items'       => array(
						...
						'properties' => array(
							...
							'rate_code'          => array(
								'description' => __( 'Tax rate code.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							...
							'label'              => array(
								'description' => __( 'Tax rate label.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'compound'           => array(
								'description' => __( 'Show if is a compound tax rate.', 'fincommerce' ),
								'type'        => 'boolean',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'tax_total'          => array(
								'description' => __( 'Tax total (not including shipping taxes).', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'shipping_tax_total' => array(
								'description' => __( 'Shipping tax total.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
				...
				'shipping_lines'       => array(
					'description' => __( 'Shipping lines data.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'       => 'object',
						'properties' => array(
							...
							'method_title' => array(
								'description' => __( 'Shipping method name.', 'fincommerce' ),
								'type'        => 'mixed',
								'context'     => array( 'view', 'edit' ),
							),
							'method_id'    => array(
								'description' => __( 'Shipping method ID.', 'fincommerce' ),
								'type'        => 'mixed',
								'context'     => array( 'view', 'edit' ),
							),
							'instance_id'  => array(
								'description' => __( 'Shipping instance ID.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'total'        => array(
				...
				'fee_lines'            => array(
					'description' => __( 'Fee lines data.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'       => 'object',
						'properties' => array(
							...
							'name'       => array(
								'description' => __( 'Fee name.', 'fincommerce' ),
								'type'        => 'mixed',
								'context'     => array( 'view', 'edit' ),
							),
							'tax_class'  => array(
								'description' => __( 'Tax class of fee.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'tax_status' => array(
								'description' => __( 'Tax status of fee.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
				...
				'coupon_lines'         => array(
					'description' => __( 'Coupons line data.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'       => 'object',
						'properties' => array(
							...
							'code'           => array(
								'description' => __( 'Coupon code.', 'fincommerce' ),
								'type'        => 'mixed',
								'context'     => array( 'view', 'edit' ),
							),
							'discount'       => array(
								'description' => __( 'Discount total.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
							),
							'discount_tax'   => array(
								'description' => __( 'Discount total tax.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'discount_type'  => array(
								'description' => __( 'Discount type.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view' ),
								'readonly'    => true,
							),
							'nominal_amount' => array(
								'description' => __( 'Discount amount as defined in the coupon (absolute value or a percent, depending on the discount type).', 'fincommerce' ),
								'type'        => 'number',
								'context'     => array( 'view' ),
								'readonly'    => true,
							),
							'free_shipping'  => array(
								'description' => __( 'Whether the coupon grants free shipping or not.', 'fincommerce' ),
								'type'        => 'boolean',
								'context'     => array( 'view' ),
								'readonly'    => true,
							),
				...
				'refunds'              => array(
					'description' => __( 'List of refunds.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
					'items'       => array(
						...
						'properties' => array(
							'id'     => array(
								'description' => __( 'Refund ID.', 'fincommerce' ),
								'type'        => 'integer',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'reason' => array(
								'description' => __( 'Refund reason.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'total'  => array(
								'description' => __( 'Refund total.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
				...
				'payment_url'          => array(
					'description' => __( 'Order payment URL.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'set_paid'             => array(
					'description' => __( 'Define if the order is paid. It will set the status to processing and reduce stock items.', 'fincommerce' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'edit' ),
				),
				'is_editable'          => array(
					'description' => __( 'Whether an order can be edited.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'needs_payment'        => array(
					'description' => __( 'Whether an order needs payment, based on status and order total.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'needs_processing'     => array(
					'description' => __( 'Whether an order needs processing before it can be completed.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function get_collection_params() {
		...
		$params['status']                  = array(
			'default'           => 'any',
			'description'       => __( 'Limit result set to orders assigned a specific status.', 'fincommerce' ),
			'type'              => 'string',
		...
		$params['customer']                = array(
			'description'       => __( 'Limit result set to orders assigned a specific customer.', 'fincommerce' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['product']                 = array(
			'description'       => __( 'Limit result set to orders assigned a specific product.', 'fincommerce' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['dp']                      = array(
			...
			'description'       => __( 'Number of decimal points to use in each resource.', 'fincommerce' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['order_item_display_meta'] = array(
			'default'           => false,
			'description'       => __( 'Only show meta which is meant to be displayed for an order.', 'fincommerce' ),
			'type'              => 'boolean',
