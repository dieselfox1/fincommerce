...
class WC_REST_Payment_Gateways_V2_Controller extends WC_REST_Controller {
	...
	public function get_item( $request ) {
		...
		if ( is_null( $gateway ) ) {
			return new WP_Error( 'fincommerce_rest_payment_gateway_invalid', __( 'Resource does not exist.', 'fincommerce' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ) {
		...
		update_option( $gateway->get_option_key(), apply_filters( 'fincommerce_gateway_' . $gateway->id . '_settings_values', $settings, $gateway ) );

		// Update order.
		if ( isset( $request['order'] ) ) {
			$order                 = (array) get_option( 'fincommerce_gateway_order' );
			$order[ $gateway->id ] = absint( $request['order'] );
			update_option( 'fincommerce_gateway_order', $order );
	...
	public function prepare_item_for_response( $gateway, $request ) {
		 ...
		 * @param WP_REST_Request    $request  Request object.
		 */
		return apply_filters( 'fincommerce_rest_prepare_payment_gateway', $response, $gateway, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id'                 => array(
					'description' => __( 'Payment gateway ID.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'title'              => array(
					'description' => __( 'Payment gateway title on checkout.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'description'        => array(
					'description' => __( 'Payment gateway description on checkout.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'order'              => array(
					'description' => __( 'Payment gateway sort order.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'arg_options' => array(
				...
				'enabled'            => array(
					'description' => __( 'Payment gateway enabled status.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
				),
				'method_title'       => array(
					'description' => __( 'Payment gateway method title.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'method_description' => array(
					'description' => __( 'Payment gateway method description.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'settings'           => array(
					'description' => __( 'Payment gateway settings.', 'fincommerce' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'id'          => array(
