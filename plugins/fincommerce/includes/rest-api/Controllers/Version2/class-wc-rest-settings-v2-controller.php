...
class WC_REST_Settings_V2_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		$groups = apply_filters( 'fincommerce_settings_groups', array() );
		if ( empty( $groups ) ) {
			return new WP_Error( 'rest_setting_groups_empty', __( 'No setting groups have been registered.', 'fincommerce' ), array( 'status' => 500 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id'          => array(
					'description' => __( 'A unique identifier that can be used to link settings together.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				...
				'parent_id'   => array(
					'description' => __( 'ID of parent grouping.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'sub_groups'  => array(
					'description' => __( 'IDs for settings sub groups.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
