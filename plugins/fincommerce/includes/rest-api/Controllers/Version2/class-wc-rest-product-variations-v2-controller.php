...
class WC_REST_Product_Variations_V2_Controller extends WC_REST_Products_V2_Controller {
		...
		register_rest_route(
			$this->namespace, '/' . $this->rest_base . '/(?P<id>[\d]+)', array(
				'args'   => array(
					...
					'id'         => array(
						'description' => __( 'Unique identifier for the variation.', 'fincommerce' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function get_item_permissions_check( $request ) {
		if ( ! $this->check_variation_parent( (int) $request['id'], (int) $request['product_id'] ) ) {
			return new WP_Error( "fincommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'fincommerce' ), array( 'status' => 404 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $object || 0 === $object->get_id() ) {
			return new WP_Error(
				"fincommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'fincommerce' ), array(
					'status' => 404,
				)
			);
		}
		...
		if ( ! wc_rest_check_post_permissions( $this->post_type, 'delete', $object->get_id() ) ) {
			return new WP_Error(
				...
				"fincommerce_rest_user_cannot_delete_{$this->post_type}", sprintf( __( 'Sorry, you are not allowed to delete %s.', 'fincommerce' ), $this->post_type ), array(
		...
		if ( $force ) {
			...
			if ( ! $supports_trash ) {
				return new WP_Error(
					/* translators: %s: post type */
					'fincommerce_rest_trash_not_supported', sprintf( __( 'The %s does not support trashing.', 'fincommerce' ), $this->post_type ), array(
						'status' => 501,
					)
				);
			}
			...
			if ( is_callable( array( $object, 'get_status' ) ) ) {
				if ( 'trash' === $object->get_status() ) {
					return new WP_Error(
						/* translators: %s: post type */
						'fincommerce_rest_already_trashed', sprintf( __( 'The %s has already been deleted.', 'fincommerce' ), $this->post_type ), array(
							'status' => 410,
						)
					);
				}
		...
		if ( ! $result ) {
			return new WP_Error(
				/* translators: %s: post type */
				'fincommerce_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'fincommerce' ), $this->post_type ), array(
					'status' => 500,
				)
			);
		}
	...
	public function get_item_schema() {
		...
		$schema               = array(
			...
			'properties' => array(
				...
				'visible'               => array(
					'description' => __( "Define if the variation is visible on the product's page.", 'fincommerce' ),
					'type'        => 'boolean',
					'default'     => true,
					'context'     => array( 'view', 'edit' ),
				),
