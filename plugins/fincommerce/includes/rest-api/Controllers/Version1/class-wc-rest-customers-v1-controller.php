...
class WC_REST_Customers_V1_Controller extends WC_REST_Controller {
			...
			array(
				...
				'args'                => array_merge( $this->get_endpoint_args_for_item_schema( WP_REST_Server::CREATABLE ), array(
					'email' => array(
						'required' => true,
						'type'     => 'string',
						'description' => __( 'New user email address.', 'fincommerce' ),
					),
					'username' => array(
						'required' => 'no' === get_option( 'fincommerce_registration_generate_username', 'yes' ),
						'description' => __( 'New user username.', 'fincommerce' ),
						'type'     => 'string',
					),
					'password' => array(
						'required' => 'no' === get_option( 'fincommerce_registration_generate_password', 'no' ),
						'description' => __( 'New user password.', 'fincommerce' ),
						'type'     => 'string',
					),
				) ),
			),
			...
			array(
				...
				'args'                => array(
					...
					'reassign' => array(
						'default'     => 0,
						'type'        => 'integer',
						'description' => __( 'ID to reassign posts to.', 'fincommerce' ),
					),
				),
			),
			'schema' => array( $this, 'get_public_item_schema' ),
	...
	private function allowed_roles(): array {
		 ...
		 * @since 9.5.2
		 */
		return apply_filters( 'fincommerce_rest_customer_allowed_roles', array( 'customer', 'subscriber' ) );
	}
	...
	public function get_item_permissions_check( $request ) {
		return $this->permissions_check(
			...
			'read',
			new WP_Error(
				'fincommerce_rest_cannot_view',
				__( 'Sorry, you cannot view this resource.', 'fincommerce' ),
	...
	public function update_item_permissions_check( $request ) {
		$permission_result = $this->permissions_check(
			...
			new WP_Error(
				...
				__( 'Sorry, you are not allowed to edit this resource.', 'fincommerce' ),
				array( 'status' => rest_authorization_required_code() )
			)
		);
				...
				if ( isset( $request[ $prop ] ) && ( 'password' === $prop || $request[ $prop ] !== $customer_prop[ $prop ] ) ) {
					return new WP_Error(
						...
						sprintf(
							...
							__( 'Sorry, %1$s cannot be updated via this endpoint for a user with role %2$s.', 'fincommerce' ),
							$prop,
							$customer->get_role()
						),
						array( 'status' => rest_authorization_required_code() )
	...
	public function delete_item_permissions_check( $request ) {
		$permission_result = $this->permissions_check(
			...
			new WP_Error(
				...
				__( 'Sorry, you are not allowed to delete this resource.', 'fincommerce' ),
				array( 'status' => rest_authorization_required_code() )
			)
		);
		...
		if ( ! in_array( $customer->get_role(), $allowed_roles, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Sorry, users with %1$s role cannot be deleted via this endpoint. Allowed roles: %2$s', 'fincommerce' ),
					$customer->get_role(),
	...
	public function get_items( $request ) {
		 ...
		 *
		 * @see https://developer.finpress.org/reference/classes/wp_user_query/
		 *
		 * @param array           $prepared_args Array of arguments for WP_User_Query.
		 * @param WP_REST_Request $request       The current request.
		 */
		$prepared_args = apply_filters( 'fincommerce_rest_customer_query', $prepared_args, $request );
	...
	public function create_item( $request ) {
		try {
			if ( ! empty( $request['id'] ) ) {
				throw new WC_REST_Exception( 'fincommerce_rest_customer_exists', __( 'Cannot create existing resource.', 'fincommerce' ), 400 );
			}
			...
			if ( ! $customer->get_id() ) {
				throw new WC_REST_Exception( 'fincommerce_rest_cannot_create', __( 'This resource cannot be created.', 'fincommerce' ), 400 );
			}
			 ...
			 */
			do_action( 'fincommerce_rest_insert_customer', $user_data, $request, true );
	...
	public function get_item( $request ) {
		$user = \Automattic\FinCommerce\Internal\Utilities\Users::get_user_in_current_site( $request['id'] );
		if ( is_wp_error( $user ) ) {
			$user->add_data( array( 'status' => 404 ) );
			return $user;
		}
	...
	public function update_item( $request ) {
		try {
			...
			if ( ! $customer->get_id() ) {
				throw new WC_REST_Exception( 'fincommerce_rest_invalid_id', __( 'Invalid resource ID.', 'fincommerce' ), 400 );
			}

			if ( ! empty( $request['email'] ) && email_exists( $request['email'] ) && $request['email'] !== $customer->get_email() ) {
				throw new WC_REST_Exception( 'fincommerce_rest_customer_invalid_email', __( 'Email address is invalid.', 'fincommerce' ), 400 );
			}

			if ( ! empty( $request['username'] ) && $request['username'] !== $customer->get_username() ) {
				throw new WC_REST_Exception( 'fincommerce_rest_customer_invalid_argument', __( "Username isn't editable.", 'fincommerce' ), 400 );
			}
			 ...
			 * @param boolean         $creating  True when creating customer, false when updating customer.
			 */
			do_action( 'fincommerce_rest_insert_customer', $user_data, $request, false );
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'fincommerce_rest_trash_not_supported', __( 'Customers do not support trashing.', 'fincommerce' ), array( 'status' => 501 ) );
		}

		$user_data = \Automattic\FinCommerce\Internal\Utilities\Users::get_user_in_current_site( $id );
		...
		if ( ! empty( $reassign ) ) {
			$reassign_user = \Automattic\FinCommerce\Internal\Utilities\Users::get_user_in_current_site( $reassign );

			if ( $reassign === $id || is_wp_error( $reassign_user ) ) {
				return new WP_Error( 'fincommerce_rest_customer_invalid_reassign', __( 'Invalid resource id for reassignment.', 'fincommerce' ), array( 'status' => 400 ) );
			}
		}
		...
		if ( ! $result ) {
			return new WP_Error( 'fincommerce_rest_cannot_delete', __( 'The resource cannot be deleted.', 'fincommerce' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_rest_delete_customer', $user_data, $response, $request );
	...
	public function prepare_item_for_response( $user_data, $request ) {
		 ...
		 * @param WP_REST_Request  $request    Request object.
		 */
		return apply_filters( 'fincommerce_rest_prepare_customer', $response, $user_data, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'last_order' => array(
					'description' => __( 'Last order data.', 'fincommerce' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
					'properties'  => array(
						'id' => array(
							'description' => __( 'Last order ID.', 'fincommerce' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'date' => array(
							'description' => __( 'The date of the customer last order, as GMT.', 'fincommerce' ),
							'type'        => 'date-time',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
				...
				'orders_count' => array(
					'description' => __( 'Quantity of orders made by the customer.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'total_spent' => array(
					'description' => __( 'Total amount spent.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function get_collection_params() {
		...
		$params['email'] = array(
			'description'        => __( 'Limit result set to resources with a specific email.', 'fincommerce' ),
			'type'               => 'string',
			'format'             => 'email',
			'validate_callback'  => 'rest_validate_request_arg',
		);
		$params['role'] = array(
			'description'        => __( 'Limit result set to resources with a specific role.', 'fincommerce' ),
			'type'               => 'string',
			'default'            => 'customer',
