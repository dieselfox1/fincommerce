...
class WC_REST_Order_Notes_V1_Controller extends WC_REST_Controller {
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base, array(
			...
			array(
				...
				'args'                => array_merge( $this->get_endpoint_args_for_item_schema( WP_REST_Server::CREATABLE ), array(
					'note' => array(
						'type'        => 'string',
						'description' => __( 'Order note content.', 'fincommerce' ),
						'required'    => true,
					),
				) ),
			),
	...
	public function get_items( $request ) {
		...
		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new WP_Error( "fincommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid order ID.', 'fincommerce' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( ! $order || $this->post_type !== $order->get_type() ) {
			return new WP_Error( 'fincommerce_rest_order_invalid_id', __( 'Invalid order ID.', 'fincommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( ! $note_id ) {
			return new WP_Error( 'fincommerce_api_cannot_create_order_note', __( 'Cannot create order note, please try again.', 'fincommerce' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_rest_insert_order_note', $note, $request, true );
	...
	public function delete_item( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'fincommerce_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'fincommerce' ), 'order_note' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_rest_delete_order_note', $note, $response, $request );
	...
	public function prepare_item_for_response( $note, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( 'fincommerce_rest_prepare_order_note', $response, $note, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'date_created' => array(
					'description' => __( "The date the order note was created, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'note' => array(
					'description' => __( 'Order note.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'customer_note' => array(
					'description' => __( 'Shows/define if the note is only for reference or for the customer (the user will be notified).', 'fincommerce' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
