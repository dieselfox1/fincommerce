...
class WC_REST_Taxes_V1_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		 ...
		 */
		$prepared_args = apply_filters( 'fincommerce_rest_tax_query', $prepared_args, $request );
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new WP_Error( 'fincommerce_rest_tax_exists', __( 'Cannot create existing resource.', 'fincommerce' ), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_rest_insert_tax', $tax, $request, true );
	...
	public function get_item( $request ) {
		...
		if ( empty( $id ) || empty( $tax_obj ) ) {
			return new WP_Error( 'fincommerce_rest_invalid_id', __( 'Invalid resource ID.', 'fincommerce' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ) {
		 ...
		 * @param boolean         $creating  True when creating tax, false when updating tax.
		 */
		do_action( 'fincommerce_rest_insert_tax', $tax, $request, false );
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'fincommerce_rest_trash_not_supported', __( 'Taxes do not support trashing.', 'fincommerce' ), array( 'status' => 501 ) );
		}

		$tax = WC_Tax::_get_tax_rate( $id, OBJECT );

		if ( empty( $id ) || empty( $tax ) ) {
			return new WP_Error( 'fincommerce_rest_invalid_id', __( 'Invalid resource ID.', 'fincommerce' ), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_rest_delete_tax', $tax, $response, $request );
	...
	public function prepare_item_for_response( $tax, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( 'fincommerce_rest_prepare_tax', $response, $tax, $request );
	}
	...
	protected function add_tax_rate_locales( $data, $tax ) {
		...
		$locales = $wpdb->get_results(
			$wpdb->prepare(
				...
				FROM {$wpdb->prefix}fincommerce_tax_rate_locations
				WHERE tax_rate_id = %d
				",
				$tax->tax_rate_id
			)
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'country'  => array(
					'description' => __( 'Country ISO 3166 code.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'state'    => array(
					'description' => __( 'State code.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'postcode' => array(
					'description' => __( 'Postcode / ZIP.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'city'     => array(
				...
				'rate'     => array(
					'description' => __( 'Tax rate.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'name'     => array(
					'description' => __( 'Tax rate name.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'priority' => array(
					'description' => __( 'Tax priority.', 'fincommerce' ),
					'type'        => 'integer',
					'default'     => 1,
					'context'     => array( 'view', 'edit' ),
				),
				'compound' => array(
					'description' => __( 'Whether or not this is a compound rate.', 'fincommerce' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
				'shipping' => array(
					'description' => __( 'Whether or not this tax rate also gets applied to shipping.', 'fincommerce' ),
					'type'        => 'boolean',
					'default'     => true,
					'context'     => array( 'view', 'edit' ),
				),
				'order'    => array(
					'description' => __( 'Indicates the order that will appear in queries.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'class'    => array(
	...
	public function get_collection_params() {
		...
		$params['class']    = array(
			'description'       => __( 'Sort by tax class.', 'fincommerce' ),
			'enum'              => array_merge( array( 'standard' ), WC_Tax::get_tax_class_slugs() ),
