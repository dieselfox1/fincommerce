...
class WC_REST_Product_Variations_Controller extends WC_REST_Product_Variations_V2_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					'product_id'     => array(
						'description' => __( 'Unique identifier for the variable product.', 'fincommerce' ),
						'type'        => 'integer',
					),
					'delete'         => array(
						'description' => __( 'Deletes unused variations.', 'fincommerce' ),
						'type'        => 'boolean',
					),
					'default_values' => array(
						'description' => __( 'Default values for generated variations.', 'fincommerce' ),
						'type'        => 'object',
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		...
		if ( isset( $request['attributes'] ) ) {
			...
			if ( ! $parent ) {
				return new WP_Error(
					...
					"fincommerce_rest_{$this->post_type}_invalid_parent",
					__( 'Cannot set attributes due to invalid parent product.', 'fincommerce' ),
					array( 'status' => 404 )
				);
			}
		 ...
		 * @param bool            $creating  If is creating a new object.
		 */
		return apply_filters( "fincommerce_rest_pre_insert_{$this->post_type}_object", $variation, $request, $creating );
	}
	...
	protected function set_variation_image( $variation, $image ) {
		...
		if ( 0 === $attachment_id ) {
			if ( isset( $image['src'] ) ) {
				...
				if ( is_wp_error( $upload ) ) {
					 ...
					 */
					if ( ! apply_filters( 'fincommerce_rest_suppress_image_upload_error', false, $upload, $variation->get_id(), array( $image ) ) ) {
						throw new WC_REST_Exception( 'fincommerce_variation_image_upload_error', $upload->get_error_message(), 400 );
					}
				}
		...
		if ( ! wp_attachment_is_image( $attachment_id ) ) {
			...
			throw new WC_REST_Exception( 'fincommerce_variation_invalid_image_id', sprintf( __( '#%s is an invalid image ID.', 'fincommerce' ), $attachment_id ), 400 );
		}
	...
	public function get_item_schema() {
		...
		$schema               = array(
			...
			'properties' => array(
				...
				'description'           => array(
					'description' => __( 'Variation description.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'permalink'             => array(
				...
				'status'                => array(
					'description' => __( 'Variation status.', 'fincommerce' ),
					'type'        => 'string',
					// Not using ProductStatus constants here due the class not being loaded upon installation.
					// See: https://github.com/dieselfox1/fincommerce/issues/37464.
					'default'     => 'publish',
				...
				'low_stock_amount'      => array(
					'description' => __( 'Low Stock amount for the variation.', 'fincommerce' ),
	...
	public function get_collection_params() {
		...
		$params['has_price'] = array(
			'description'       => __( 'Limit result set to products with or without price.', 'fincommerce' ),
			'type'              => 'boolean',
		...
		$params['attributes'] = array(
			'description' => __( 'Limit result set to products with specified attributes.', 'fincommerce' ),
			'type'        => 'array',
			'items'       => array(
				'type'       => 'object',
				'properties' => array(
					'attribute' => array(
						'type'        => 'string',
						'description' => __( 'Attribute slug.', 'fincommerce' ),
					),
					'term'      => array(
						'type'        => 'string',
						'description' => __( 'Attribute term.', 'fincommerce' ),
					),
					'terms'     => array(
						'type'        => 'array',
						'description' => __( 'Attribute terms.', 'fincommerce' ),
					),
				),
			),
		);
		...
		$params['virtual'] = array(
			'description'       => __( 'Limit result set to virtual product variations.', 'fincommerce' ),
			'type'              => 'boolean',
		...
		$params['downloadable'] = array(
			'description'       => __( 'Limit result set to downloadable product variations.', 'fincommerce' ),
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		$params['include_status'] = array(
			'description'       => __( 'Limit result set to product variations with any of the statuses.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
		...
		$params['exclude_status'] = array(
			'description'       => __( 'Exclude product variations with any of the statuses from result set.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
