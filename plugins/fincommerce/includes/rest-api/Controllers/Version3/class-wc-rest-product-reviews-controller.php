...
class WC_REST_Product_Reviews_Controller extends WC_REST_Controller {
				...
				array(
					...
					'args'                => array_merge(
						$this->get_endpoint_args_for_item_schema( WP_REST_Server::CREATABLE ), array(
							'product_id'     => array(
								'required'    => true,
								'description' => __( 'Unique identifier for the product.', 'fincommerce' ),
								'type'        => 'integer',
							),
							'review'         => array(
								'required'    => true,
								'type'        => 'string',
								'description' => __( 'Review content.', 'fincommerce' ),
							),
							'reviewer'       => array(
								'required'    => true,
								'type'        => 'string',
								'description' => __( 'Name of the reviewer.', 'fincommerce' ),
							),
							'reviewer_email' => array(
								'required'    => true,
								'type'        => 'string',
								'description' => __( 'Email of the reviewer.', 'fincommerce' ),
							),
						)
					),
				),
	...
	public function create_item_permissions_check( $request ) {
		if ( ! wc_rest_check_product_reviews_permissions( 'create' ) ) {
			return new WP_Error( 'fincommerce_rest_cannot_create', __( 'Sorry, you are not allowed to create resources.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_product_reviews_permissions( 'edit', (int) $request['id'] ) ) {
			return new WP_Error( 'fincommerce_rest_cannot_edit', __( 'Sorry, you cannot edit this resource.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_product_reviews_permissions( 'delete', (int) $request['id'] ) ) {
			return new WP_Error( 'fincommerce_rest_cannot_delete', __( 'Sorry, you cannot delete this resource.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function batch_items_permissions_check( $request ) {
		if ( ! wc_rest_check_product_reviews_permissions( 'batch' ) ) {
			return new WP_Error( 'fincommerce_rest_cannot_batch', __( 'Sorry, you are not allowed to batch manipulate this resource.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_items( $request ) {
		 ...
		 */
		$prepared_args = apply_filters( 'fincommerce_rest_product_review_query', $prepared_args, $request );
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new WP_Error( 'fincommerce_rest_review_exists', __( 'Cannot create existing product review.', 'fincommerce' ), array( 'status' => 400 ) );
		}
		...
		if ( empty( $prepared_review['comment_content'] ) ) {
			return new WP_Error( 'fincommerce_rest_review_content_invalid', __( 'Invalid review content.', 'fincommerce' ), array( 'status' => 400 ) );
		}
		...
		if ( is_wp_error( $check_comment_lengths ) ) {
			...
			return new WP_Error( 'fincommerce_rest_' . $error_code, __( 'Product review field exceeds maximum length allowed.', 'fincommerce' ), array( 'status' => 400 ) );
		}
		...
		if ( is_wp_error( $prepared_review['comment_approved'] ) ) {
			...
			if ( 'comment_duplicate' === $error_code ) {
				return new WP_Error( 'fincommerce_rest_' . $error_code, $error_message, array( 'status' => 409 ) );
			}

			if ( 'comment_flood' === $error_code ) {
				return new WP_Error( 'fincommerce_rest_' . $error_code, $error_message, array( 'status' => 400 ) );
			}
		 ...
		 */
		$prepared_review = apply_filters( 'fincommerce_rest_pre_insert_product_review', $prepared_review, $request );
		if ( is_wp_error( $prepared_review ) ) {
			return $prepared_review;
		}
		...
		if ( ! $review_id ) {
			return new WP_Error( 'fincommerce_rest_review_failed_create', __( 'Creating product review failed.', 'fincommerce' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_rest_insert_product_review', $review, $request, true );
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['type'] ) && 'review' !== get_comment_type( $id ) ) {
			return new WP_Error( 'fincommerce_rest_review_invalid_type', __( 'Sorry, you are not allowed to change the comment type.', 'fincommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( empty( $prepared_args ) && isset( $request['status'] ) ) {
			...
			if ( ! $change ) {
				return new WP_Error( 'fincommerce_rest_review_failed_edit', __( 'Updating review status failed.', 'fincommerce' ), array( 'status' => 500 ) );
			}
			...
			if ( false === $updated ) {
				return new WP_Error( 'fincommerce_rest_comment_failed_edit', __( 'Updating review failed.', 'fincommerce' ), array( 'status' => 500 ) );
			}
		...
		do_action( 'fincommerce_rest_insert_product_review', $review, $request, false );
	...
	public function delete_item( $request ) {
		 ...
		 */
		$supports_trash = apply_filters( 'fincommerce_rest_product_review_trashable', ( EMPTY_TRASH_DAYS > 0 ), $review );
		...
		if ( $force ) {
			...
			if ( ! $supports_trash ) {
				...
				return new WP_Error( 'fincommerce_rest_trash_not_supported', sprintf( __( "The object does not support trashing. Set '%s' to delete.", 'fincommerce' ), 'force=true' ), array( 'status' => 501 ) );
			}

			if ( 'trash' === $review->comment_approved ) {
				return new WP_Error( 'fincommerce_rest_already_trashed', __( 'The object has already been trashed.', 'fincommerce' ), array( 'status' => 410 ) );
			}
		...
		if ( ! $result ) {
			return new WP_Error( 'fincommerce_rest_cannot_delete', __( 'The object cannot be deleted.', 'fincommerce' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_rest_delete_review', $review, $response, $request );
	...
	public function prepare_item_for_response( $review, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( 'fincommerce_rest_prepare_product_review', $response, $review, $request );
	}
	...
	protected function prepare_item_for_database( $request ) {
		 ...
		 * @param WP_REST_Request $request         The current request.
		 */
		return apply_filters( 'fincommerce_rest_preprocess_product_review', $prepared_review, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'date_created'     => array(
					'description' => __( "The date the review was created, in the site's timezone.", 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_created_gmt' => array(
					'description' => __( 'The date the review was created, as GMT.', 'fincommerce' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'product_id'       => array(
					'description' => __( 'Unique identifier for the product that the review belongs to.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'product_name'       => array(
				...
				'status'           => array(
					'description' => __( 'Status of the review.', 'fincommerce' ),
					'type'        => 'string',
					'default'     => 'approved',
				...
				'reviewer'         => array(
					'description' => __( 'Reviewer name.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'reviewer_email'   => array(
					'description' => __( 'Reviewer email.', 'fincommerce' ),
					'type'        => 'string',
					'format'      => 'email',
					'context'     => array( 'view', 'edit' ),
				),
				'review'           => array(
					'description' => __( 'The content of the review.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'arg_options' => array(
				...
				'rating'           => array(
					'description' => __( 'Review rating (0 to 5).', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'verified'         => array(
					'description' => __( 'Shows if the reviewer bought the product or not.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
		...
		if ( get_option( 'show_avatars' ) ) {
			...
			foreach ( $avatar_sizes as $size ) {
				$avatar_properties[ $size ] = array(
					...
					'description' => sprintf( __( 'Avatar URL with image size of %d pixels.', 'fincommerce' ), $size ),
					'type'        => 'string',
					'format'      => 'uri',
			...
			$schema['properties']['reviewer_avatar_urls'] = array(
				'description' => __( 'Avatar URLs for the object reviewer.', 'fincommerce' ),
				'type'        => 'object',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
	...
	public function get_collection_params() {
		...
		$params['after']            = array(
			'description' => __( 'Limit response to resources published after a given ISO8601 compliant date.', 'fincommerce' ),
			'type'        => 'string',
			'format'      => 'date-time',
		);
		$params['before']           = array(
			'description' => __( 'Limit response to reviews published before a given ISO8601 compliant date.', 'fincommerce' ),
			'type'        => 'string',
			'format'      => 'date-time',
		);
		$params['exclude']          = array(
			'description' => __( 'Ensure result set excludes specific IDs.', 'fincommerce' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$params['include']          = array(
			'description' => __( 'Limit result set to specific IDs.', 'fincommerce' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$params['offset']           = array(
			'description' => __( 'Offset the result set by a specific number of items.', 'fincommerce' ),
			'type'        => 'integer',
		);
		$params['order']            = array(
			'description' => __( 'Order sort attribute ascending or descending.', 'fincommerce' ),
			'type'        => 'string',
			'default'     => 'desc',
			'enum'        => array(
				'asc',
		...
		$params['orderby']          = array(
			'description' => __( 'Sort collection by object attribute.', 'fincommerce' ),
			'type'        => 'string',
			'default'     => 'date_gmt',
			'enum'        => array(
				'date',
		...
		$params['reviewer']         = array(
			'description' => __( 'Limit result set to reviews assigned to specific user IDs.', 'fincommerce' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$params['reviewer_exclude'] = array(
			'description' => __( 'Ensure result set excludes reviews assigned to specific user IDs.', 'fincommerce' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$params['reviewer_email']   = array(
			'default'     => null,
			'description' => __( 'Limit result set to that from a specific author email.', 'fincommerce' ),
			'format'      => 'email',
			'type'        => 'string',
		);
		$params['product']          = array(
			'default'     => array(),
			'description' => __( 'Limit result set to reviews assigned to specific product IDs.', 'fincommerce' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$params['status']           = array(
			'default'           => 'approved',
			'description'       => __( 'Limit result set to reviews assigned a specific status.', 'fincommerce' ),
		 ...
		 */
		return apply_filters( 'fincommerce_rest_product_review_collection_params', $params );
	}
	...
	protected function get_review( $id ) {
		$id    = (int) $id;
		$error = new WP_Error( 'fincommerce_rest_review_invalid_id', __( 'Invalid review ID.', 'fincommerce' ), array( 'status' => 404 ) );
