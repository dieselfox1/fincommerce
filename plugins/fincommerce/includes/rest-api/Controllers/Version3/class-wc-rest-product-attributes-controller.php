...
class WC_REST_Product_Attributes_Controller extends WC_REST_Product_Attributes_V2_Controller {
	...
	private function generate_unique_slug( $attribute_name ) {
		...
		$results = $wpdb->get_results(
			$wpdb->prepare( "SELECT attribute_name FROM {$wpdb->prefix}fincommerce_attribute_taxonomies WHERE attribute_name LIKE %s ORDER BY attribute_id DESC LIMIT 1", $root_slug . '%' )
		);
