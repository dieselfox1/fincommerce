...
use Automattic\FinCommerce\Internal\CostOfGoodsSold\CogsAwareRestControllerTrait;
...
class WC_REST_Products_Controller extends WC_REST_Products_V2_Controller {
	...
	public function duplicate_product( $request ) {
		...
		if ( ! $product ) {
			return new WP_Error( 'fincommerce_rest_product_invalid_id', __( 'Invalid product ID.', 'fincommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( is_wp_error( $duplicated_product ) ) {
			return new WP_Error( 'fincommerce_rest_product_duplicate_error', $duplicated_product->get_error_message(), array( 'status' => 400 ) );
		}
	...
	protected function get_images( $product ) {
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			$thumbnail = wp_get_attachment_image_src( $attachment_id, 'fincommerce_thumbnail' );
	...
	/**
	 * Make extra product orderby features supported by FinCommerce available to the WC API.
	...
	public function get_item_schema() {
		...
		$schema               = array(
			...
			'properties' => array(
				...
				'sku'                   => array(
					'description' => __( 'Stock Keeping Unit.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'global_unique_id'      => array(
					'description' => __( 'GTIN, UPC, EAN or ISBN.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'price'                 => array(
				...
				'stock_status'          => array(
					'description' => __( 'Controls the stock status of the product.', 'fincommerce' ),
					'type'        => 'string',
				...
				'low_stock_amount'      => array(
					'description' => __( 'Low Stock amount for the product.', 'fincommerce' ),
				...
				'post_password'         => array(
					'description' => __( 'Post password.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				...
				'brands'                => array(
					'description' => __( 'List of brands.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'       => 'object',
						'properties' => array(
							'id'   => array(
								'description' => __( 'Brand ID.', 'fincommerce' ),
								'type'        => 'integer',
								'context'     => array( 'view', 'edit' ),
							),
							'name' => array(
								'description' => __( 'Brand name.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'slug' => array(
								'description' => __( 'Brand slug.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
				...
				'has_options'           => array(
					'description' => __( 'Shows if the product needs to be configured before it can be bought.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
		...
		if ( is_post_type_viewable( $post_type_obj ) && $post_type_obj->public ) {
			$schema['properties']['permalink_template'] = array(
				'description' => __( 'Permalink template for the product.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'edit' ),
				'readonly'    => true,
			);
			...
			$schema['properties']['generated_slug'] = array(
				'description' => __( 'Slug automatically generated from the product name.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'edit' ),
				'readonly'    => true,
			);
		...
		if ( Features::is_enabled( 'experimental-wc-rest-api' ) ) {
			$schema['properties']['__experimental_min_price'] = array(
				'description' => __( 'Product minimum price.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'view', 'edit' ),
			);

			$schema['properties']['__experimental_max_price'] = array(
				'description' => __( 'Product maximum price.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'view', 'edit' ),
			);
		}
	...
	public function get_collection_params() {
		...
		$params['stock_status'] = array(
			'description'       => __( 'Limit result set to products with specified stock status.', 'fincommerce' ),
			'type'              => 'string',
			'enum'              => array_keys( wc_get_product_stock_status_options() ),
		...
		$params['search_sku'] = array(
			'description'       => __( "Limit results to those with a SKU that partial matches a string. This argument takes precedence over 'sku'.", 'fincommerce' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		$params['search_name_or_sku'] = array(
			'description'       => __( "Limit results to those with a name or SKU that partial matches a string. This argument takes precedence over 'search', 'sku' and 'search_sku'.", 'fincommerce' ),
			'type'              => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		$params['search_fields'] = array(
			'description'       => __( 'Limit search to specific fields when used with search parameter. Available fields: name, sku, global_unique_id, description, short_description. This argument takes precedence over all other search parameters.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
		...
		$params['include_status'] = array(
			'description'       => __( 'Limit result set to products with any of the statuses.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
		...
		$params['exclude_status'] = array(
			'description'       => __( 'Exclude products with any of the statuses from result set.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
		...
		$params['include_types'] = array(
			'description'       => __( 'Limit result set to products with any of the types.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
				'enum' => array_keys( wc_get_product_types() ),
		...
		$params['exclude_types'] = array(
			'description'       => __( 'Exclude products with any of the types from result set.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
				'enum' => array_keys( wc_get_product_types() ),
		...
		$params['downloadable'] = array(
			'description'       => __( 'Limit result set to downloadable products.', 'fincommerce' ),
			'type'              => 'boolean',
		...
		$params['virtual'] = array(
			'description'       => __( 'Limit result set to virtual products.', 'fincommerce' ),
			'type'              => 'boolean',
			'sanitize_callback' => 'rest_sanitize_boolean',
			'validate_callback' => 'rest_validate_request_arg',
		);
	...
	public function get_suggested_products_collection_params() {
		...
		$params['categories'] = array(
			'description'       => __( 'Limit result set to specific product categorie ids.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'integer',
			),
		...
		$params['tags'] = array(
			'description'       => __( 'Limit result set to specific product tag ids.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'integer',
			),
		...
		$params['limit'] = array(
			'description'       => __( 'Limit result set to specific amount of suggested products.', 'fincommerce' ),
			'type'              => 'integer',
			'default'           => 5,
			'validate_callback' => 'rest_validate_request_arg',
