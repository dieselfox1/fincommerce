...
class WC_REST_Orders_Controller extends WC_REST_Orders_V2_Controller {
	...
	protected function calculate_coupons( $request, $order ) {
		...
		foreach ( $request['coupon_lines'] as $item ) {
			if ( ! empty( $item['id'] ) ) {
				throw new WC_REST_Exception( 'fincommerce_rest_coupon_item_id_readonly', __( 'Coupon item ID is readonly.', 'fincommerce' ), 400 );
			}

			$coupon_code = ArrayUtil::get_value_or_default( $item, 'code' );
			if ( StringUtil::is_null_or_whitespace( $coupon_code ) ) {
				throw new WC_REST_Exception( 'fincommerce_rest_invalid_coupon', __( 'Coupon code is required.', 'fincommerce' ), 400 );
			}
			...
			if ( ! in_array( wc_strtolower( $coupon_code ), $current_order_coupon_codes, true ) ) {
				...
				if ( is_wp_error( $check_result ) ) {
					throw new WC_REST_Exception( 'fincommerce_rest_' . $check_result->get_error_code(), $check_result->get_error_message(), 400 );
				}
			}
		...
		foreach ( $coupon_codes as $new_coupon ) {
			...
			if ( is_wp_error( $results ) ) {
				throw new WC_REST_Exception( 'fincommerce_rest_' . $results->get_error_code(), $results->get_error_message(), 400 );
			}
		}
	...
	protected function remove_item( WC_Order $order, string $item_type, int $item_id ): void {
		...
		if ( ! $item ) {
			throw new WC_REST_Exception(
				'fincommerce_rest_invalid_item_id',
				esc_html__( 'Order item ID provided is not associated with order.', 'fincommerce' ),
				400
			);
		}
		 ...
		 * @since 9.3.0.
		 */
		do_action( 'fincommerce_rest_remove_order_item', $item );
	...
	public function get_item_schema() {
		...
		$schema['properties']['manual_update'] = array(
			'default'     => false,
			'description' => __( 'Set the action as manual so that the order note registers as "added by user".', 'fincommerce' ),
			'type'        => 'boolean',
			'context'     => array( 'edit' ),
		);
	...
	private function add_cogs_related_schema( array $schema ): array {
		$schema['properties']['cost_of_goods_sold'] = array(
			'description' => __( 'Cost of Goods Sold data.', 'fincommerce' ),
			'type'        => 'object',
			'context'     => array( 'view', 'edit' ),
			'properties'  => array(
				'total_value' => array(
					'description' => __( 'Total value of the Cost of Goods Sold for the order.', 'fincommerce' ),
					'type'        => 'number',
					'readonly'    => true,
					'context'     => array( 'view', 'edit' ),
				),
		...
		$schema['properties']['line_items']['items']['properties']['cost_of_goods_sold'] = array(
			'description' => __( 'Cost of Goods Sold data. Only present for product line items.', 'fincommerce' ),
			'type'        => 'object',
			'context'     => array( 'view', 'edit' ),
			'properties'  => array(
				'total_value' => array(
					'description' => __( 'Value of the Cost of Goods Sold for the order item.', 'fincommerce' ),
					'type'        => 'number',
					'readonly'    => true,
					'context'     => array( 'view', 'edit' ),
				),
	...
	public function get_collection_params() {
		...
		$params['status'] = array(
			'default'           => 'any',
			'description'       => __( 'Limit result set to orders which have specific statuses.', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
		...
		$params['created_via'] = array(
			'description'       => __( 'Limit result set to orders created via specific sources (e.g. checkout, admin).', 'fincommerce' ),
			'type'              => 'array',
			'items'             => array(
				'type' => 'string',
			),
