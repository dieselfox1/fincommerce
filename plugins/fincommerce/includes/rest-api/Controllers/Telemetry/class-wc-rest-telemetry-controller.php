...
class WC_REST_Telemetry_Controller extends WC_REST_Controller {
	...
	public function telemetry_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			return new WP_Error( 'fincommerce_rest_cannot_view', __( 'Sorry, you post telemetry data.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function record_usage_data( $request ) {
		...
		$data = get_option( 'fincommerce_mobile_app_usage' );
		if ( ! $data ) {
			$data = array();
		}
		...
		update_option( 'fincommerce_mobile_app_usage', $data );
	}
	...
	public function get_collection_params() {
		return array(
			'platform'          => array(
				'description'       => __( 'Platform to track.', 'fincommerce' ),
				'required'          => true,
				'type'              => 'string',
			...
			'version'           => array(
				'description'       => __( 'Platform version to track.', 'fincommerce' ),
				'required'          => true,
				'type'              => 'string',
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => 'rest_validate_request_arg',
			...
			'installation_date' => array(
				'description'       => __( 'Installation date of the FinCommerce mobile app.', 'fincommerce' ),
