 ...
 *
 * @package FinCommerce\Utilities
 */

namespace Automattic\FinCommerce\RestApi\Utilities;
...
class ImageAttachment {
	...
	public function upload_image_from_src( $src ) {
		...
		if ( is_wp_error( $upload ) ) {
			if ( ! apply_filters( 'fincommerce_rest_suppress_image_upload_error', false, $upload, $this->object_id, $images ) ) {
		...
		if ( ! wp_attachment_is_image( $this->id ) ) {
			...
			throw new \WC_REST_Exception( 'fincommerce_product_invalid_image_id', sprintf( __( '#%s is an invalid image ID.', 'fincommerce' ), $this->id ), 400 );
		}
	}
