 ...
 * Order Factory
 *
 * The FinCommerce order factory creating the right order objects.
 *
 * @version 3.0.0
...
class WC_Order_Factory {
	...
	public static function get_orders( $order_ids = array(), $skip_invalid = false ) {
		...
		foreach ( $order_id_classnames as $order_id => $classname ) {
			if ( ! $classname && ! $skip_invalid ) {
				...
				throw new \Exception( sprintf( __( 'Could not find classname for order ID %d', 'fincommerce' ), $order_id ) );
			}
	...
	public static function get_order_item( $item_id = 0 ) {
		...
		if ( $id && $item_type ) {
			...
			$classname = apply_filters( 'fincommerce_get_order_item_classname', $classname, $item_type, $id );
	...
	public static function get_class_names_for_order_ids( $order_ids ) {
		...
		foreach ( $order_types as $order_id => $order_type ) {
			 ...
			 * @since 3.0.0
			 */
			$order_class_names[ $order_id ] = apply_filters( 'fincommerce_order_class', $order_class_names[ $order_id ], $order_type, $order_id );
