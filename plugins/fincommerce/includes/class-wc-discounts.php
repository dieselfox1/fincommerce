...
class WC_Discounts {
	...
	public function get_items_to_validate() {
		return apply_filters( 'fincommerce_coupon_get_items_to_validate', $this->get_items(), $this );
	}
	...
	protected function get_items_to_apply_coupon( $coupon ) {
		 ...
		 */
		return apply_filters( 'fincommerce_coupon_get_items_to_apply', $items_to_apply, $coupon, $this );
	}
	...
	protected function apply_coupon_percent( $coupon, $items_to_apply ) {
		...
		foreach ( $items_to_apply as $item ) {
			...
			$price_to_discount = ( 'yes' === get_option( 'fincommerce_calc_discounts_sequentially', 'no' ) ) ? $discounted_price : NumberUtil::round( $item->price );
			...
			$apply_quantity    = max( 0, apply_filters( 'fincommerce_coupon_get_apply_quantity', $apply_quantity, $item, $coupon, $this ) );
			$price_to_discount = ( $price_to_discount / $item->quantity ) * $apply_quantity;

			// Run coupon calculations.
			$discount = floor( $price_to_discount * ( $coupon_amount / 100 ) );

			if ( is_a( $this->object, 'WC_Cart' ) && has_filter( 'fincommerce_coupon_get_discount_amount' ) ) {
				// Send through the legacy filter, but not as cents.
				$filtered_discount = wc_add_number_precision( apply_filters( 'fincommerce_coupon_get_discount_amount', wc_remove_number_precision( $discount ), wc_remove_number_precision( $price_to_discount ), $item->object, false, $coupon ) );
	...
	protected function apply_coupon_fixed_product( $coupon, $items_to_apply, $amount = null ) {
		...
		foreach ( $items_to_apply as $item ) {
			...
			// Get the price we actually want to discount, based on settings.
			$price_to_discount = ( 'yes' === get_option( 'fincommerce_calc_discounts_sequentially', 'no' ) ) ? $discounted_price : $item->price;
			...
			if ( $limit_usage_qty ) {
				...
				$apply_quantity = apply_filters( 'fincommerce_coupon_get_apply_quantity', $item->quantity, $item, $coupon, $this );
				$discount       = $amount * $apply_quantity;
			}

			if ( is_a( $this->object, 'WC_Cart' ) && has_filter( 'fincommerce_coupon_get_discount_amount' ) ) {
				// Send through the legacy filter, but not as cents.
				$discount = wc_add_number_precision( apply_filters( 'fincommerce_coupon_get_discount_amount', wc_remove_number_precision( $discount ), wc_remove_number_precision( $price_to_discount ), $item->object, false, $coupon ) );
			}
	...
	protected function apply_coupon_custom( $coupon, $items_to_apply ) {
		...
		foreach ( $items_to_apply as $item ) {
			...
			// Get the price we actually want to discount, based on settings.
			$price_to_discount = wc_remove_number_precision( ( 'yes' === get_option( 'fincommerce_calc_discounts_sequentially', 'no' ) ) ? $discounted_price : $item->price );
		...
		$this->discounts[ $coupon->get_code() ] = apply_filters( 'fincommerce_coupon_custom_discounts_array', $this->discounts[ $coupon->get_code() ], $coupon );
	...
	protected function validate_coupon_user_usage_limit( $coupon, $user_id = 0 ) {
		...
		if ( $coupon && $user_id && apply_filters( 'fincommerce_coupon_validate_user_usage_limit', $coupon->get_usage_limit_per_user() > 0, $user_id, $coupon, $this ) && $coupon->get_id() && $coupon->get_data_store() ) {
			$data_store  = $coupon->get_data_store();
	...
	protected function validate_coupon_expiry_date( $coupon ) {
		if ( $coupon->get_date_expires() && apply_filters( 'fincommerce_coupon_validate_expiry_date', time() > $coupon->get_date_expires()->getTimestamp(), $coupon, $this ) ) {
			throw new Exception(
				sprintf(
					/* translators: %s: coupon code */
	...
	protected function validate_coupon_minimum_amount( $coupon ) {
		...
		if ( $coupon->get_minimum_amount() > 0 && apply_filters( 'fincommerce_coupon_validate_minimum_amount', $coupon->get_minimum_amount() > $subtotal, $coupon, $subtotal ) ) {
			$allowed_tags = array(
				'span'  => array(
					'class' => true,
				),
	...
	protected function validate_coupon_maximum_amount( $coupon ) {
		...
		if ( $coupon->get_maximum_amount() > 0 && apply_filters( 'fincommerce_coupon_validate_maximum_amount', $coupon->get_maximum_amount() < $subtotal, $coupon ) ) {
			$allowed_tags = array(
				'span'  => array(
					'class' => true,
				),
		...
		if ( count( $coupon->get_product_ids() ) > 0 ) {
			...
			if ( ! $valid ) {
				throw new Exception(
					sprintf(
					/* translators: %s: coupon code */
						esc_html__( 'Sorry, coupon "%s" is not applicable to selected products.', 'fincommerce' ),
						esc_html( $coupon->get_code() )
					),
					109
				);
	...
	public function is_coupon_valid( $coupon ) {
		try {
			...
			if ( ! apply_filters( 'fincommerce_coupon_is_valid', true, $coupon, $this ) ) {
				throw new Exception( __( 'Coupon is not valid.', 'fincommerce' ), WC_Coupon::E_WC_COUPON_INVALID_FILTERED );
			}
		} catch ( Exception $e ) {
			/**
			 ...
			 * @param WC_Coupon $coupon        Coupon data.
			 */
			$message = apply_filters( 'fincommerce_coupon_error', is_numeric( $e->getMessage() ) ? $coupon->get_coupon_error( $e->getMessage() ) : $e->getMessage(), $e->getCode(), $coupon );
