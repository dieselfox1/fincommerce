...
class WC_Download_Handler {
	...
	public static function init() {
		...
		add_action( 'fincommerce_download_file_redirect', array( __CLASS__, 'download_file_redirect' ), 10, 2 );
		add_action( 'fincommerce_download_file_xsendfile', array( __CLASS__, 'download_file_xsendfile' ), 10, 2 );
		add_action( 'fincommerce_download_file_force', array( __CLASS__, 'download_file_force' ), 10, 2 );
	...
	public static function download_product() {
		...
		$file_path = apply_filters(
			'fincommerce_download_product_filepath',
	...
	private static function check_order_is_valid( $download ) {
		if ( $download->get_order_id() ) {
			...
			if ( $order && ! $order->is_download_permitted() ) {
				self::download_error( __( 'Invalid order.', 'fincommerce' ), '', 403 );
			}
		}
	}
	...
	private static function check_downloads_remaining( $download ) {
		if ( '' !== $download->get_downloads_remaining() && 0 >= $download->get_downloads_remaining() ) {
			self::download_error( __( 'Sorry, you have reached your download limit for this file', 'fincommerce' ), '', 403 );
		}
	}
	...
	private static function check_download_expiry( $download ) {
		if ( ! is_null( $download->get_access_expires() ) && $download->get_access_expires()->getTimestamp() < strtotime( 'midnight', time() ) ) {
			self::download_error( __( 'Sorry, this download has expired', 'fincommerce' ), '', 403 );
		}
	}
	...
	private static function check_download_login_required( $download ) {
		if ( $download->get_user_id() && 'yes' === get_option( 'fincommerce_downloads_require_login' ) ) {
			if ( ! is_user_logged_in() ) {
				if ( wc_get_page_id( 'myaccount' ) ) {
					wp_safe_redirect( add_query_arg( 'wc_error', rawurlencode( __( 'You must be logged in to download files.', 'fincommerce' ) ), wc_get_page_permalink( 'myaccount' ) ) );
					exit;
				} else {
					self::download_error( __( 'You must be logged in to download files.', 'fincommerce' ) . ' <a href="' . esc_url( wp_login_url( wc_get_page_permalink( 'myaccount' ) ) ) . '" class="wc-forward">' . __( 'Login', 'fincommerce' ) . '</a>', __( 'Log in to Download Files', 'fincommerce' ), 403 );
				}
			} elseif ( ! current_user_can( 'download_file', $download ) ) {
				self::download_error( __( 'This is not your download link.', 'fincommerce' ), '', 403 );
			}
		}
	}
	...
	public static function download( $file_path, $product_id ) {
		if ( ! $file_path ) {
			self::download_error( __( 'No file defined', 'fincommerce' ) );
		}
		...
		$filename = apply_filters( 'fincommerce_file_download_filename', $filename, $product_id );
		 ...
		 * @param string $file_path  URL to file.
		 */
		$file_download_method = apply_filters( 'fincommerce_file_download_method', get_option( 'fincommerce_file_download_method', 'force' ), $product_id, $file_path );

		// Add action to prevent issues in IE.
		add_action( 'nocache_headers', array( __CLASS__, 'ie_nocache_headers_fix' ) );

		// Trigger download via one of the methods.
		do_action( 'fincommerce_download_file_' . $file_download_method, $file_path, $filename );
	}
	...
	public static function parse_file_path( $file_path ) {
		...
		if ( '//' === substr( $file_path, 0, 2 ) ) {
			...
			return array(
				'remote_file' => true,
				'file_path'   => apply_filters( 'fincommerce_download_parse_remote_file_path', $file_path ),
			);
		}
		...
		*/
		return array(
			...
			'file_path'   => apply_filters( 'fincommerce_download_parse_file_path', $file_path, $remote_file ),
		);
	}
	...
	public static function download_file_xsendfile( $file_path, $filename ) {
		 ...
		 */
		if ( $parsed_file_path['remote_file'] && ! apply_filters( 'fincommerce_use_xsendfile_for_remote', false ) ) {
			do_action( 'fincommerce_download_file_force', $file_path, $filename );
			return;
		}

		if ( function_exists( 'apache_get_modules' ) && in_array( 'mod_xsendfile', apache_get_modules(), true ) ) {
			self::download_headers( $parsed_file_path['file_path'], $filename );
			$filepath = apply_filters( 'fincommerce_download_file_xsendfile_file_path', $parsed_file_path['file_path'], $file_path, $filename, $parsed_file_path );
			header( 'X-Sendfile: ' . $filepath );
			exit;
		} elseif ( stristr( getenv( 'SERVER_SOFTWARE' ), 'lighttpd' ) ) {
			self::download_headers( $parsed_file_path['file_path'], $filename );
			$filepath = apply_filters( 'fincommerce_download_file_xsendfile_lighttpd_file_path', $parsed_file_path['file_path'], $file_path, $filename, $parsed_file_path );
			...
			$xsendfile_path = apply_filters( 'fincommerce_download_file_xsendfile_x_accel_redirect_file_path', $xsendfile_path, $file_path, $filename, $parsed_file_path );
		...
		wc_get_logger()->warning(
			sprintf(
				...
				__( '%1$s could not be served using the X-Accel-Redirect/X-Sendfile method. A Force Download will be used instead.', 'fincommerce' ),
				$file_path
			)
		);
	...
	public static function download_file_force( $file_path, $filename ) {
		...
		if ( ! self::readfile_chunked( $parsed_file_path['file_path'], $start, $length ) ) {
			if ( $parsed_file_path['remote_file'] && 'yes' === get_option( 'fincommerce_downloads_redirect_fallback_allowed' ) ) {
				wc_get_logger()->warning(
					sprintf(
						/* translators: %1$s contains the filepath of the digital asset. */
						__( '%1$s could not be served using the Force Download method. A redirect will be used instead.', 'fincommerce' ),
						$file_path
					)
				);
				self::download_file_redirect( $file_path );
			} else {
				self::download_error( __( 'File not found', 'fincommerce' ) );
			}
		}
	 ...
	 *
	 * Defaults to attachment if `fincommerce_downloads_deliver_inline` setting is not selected.
	 *
	 * @return string Content disposition value.
	 */
	private static function get_content_disposition() : string {
		$disposition = 'attachment';
		if ( 'yes' === get_option( 'fincommerce_downloads_deliver_inline' ) ) {
			$disposition = 'inline';
		}
		return $disposition;
	}
	...
	private static function download_error( $message, $title = '', $status = 404 ) {
		 ...
		 */
		if ( headers_sent() ) {
			wc_get_logger()->log( 'warning', __( 'Headers already sent when generating download error message.', 'fincommerce' ) );
		} else {
		...
		if ( ! strstr( $message, '<a ' ) ) {
			$message .= ' <a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="wc-forward">' . esc_html__( 'Go to shop', 'fincommerce' ) . '</a>';
		}
	...
	public static function track_download( $download, $user_id = null, $user_ip_address = null, bool $defer = false ): void {
		try {
			...
			if ( get_option( 'fincommerce_downloads_count_partial', 'yes' ) !== 'yes' ) {
				return;
			}
			 ...
			 */
			$window = absint( apply_filters( 'fincommerce_partial_download_tracking_window', 30 * MINUTE_IN_SECONDS, $download->get_id() ) );
