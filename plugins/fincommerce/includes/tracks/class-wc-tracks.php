...
class WC_Tracks {
	...
	public static function get_role_details( $user ) {
		return array(
			...
			'can_manage_fincommerce' => $user->has_cap( 'manage_fincommerce' ),
		);
	}
	 ...
	 * fincommerce_allow_tracking option.
	 *
	 * @since x.x.x
	 *
	 ...
	 */
	public static function track_fincommerce_allow_tracking_toggled( $prev_value, $new_value, $context = 'settings' ) {
		if ( $new_value !== $prev_value ) {
			self::record_event(
				'fincommerce_allow_tracking_toggled',
				array(
	...
	public static function get_properties( $event_name, $event_properties ) {
		 ...
		 * @since 4.1.0
		 */
		$properties = apply_filters( 'fincommerce_tracks_event_properties', $event_properties, $event_name );
		$user       = wp_get_current_user();
