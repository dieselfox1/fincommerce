...
class WC_Product_Collection_Block_Tracking {
	...
	public function track_collection_instances( $post_id, $post ) {
		...
		if ( ! has_block( 'fincommerce/product-collection', $post ) && ! has_block( 'core/template-part', $post ) && ! has_block( 'core/block', $post ) ) {
			return;
		}
	...
	private function parse_blocks_track_data( $blocks, $is_in_single_product = false, $is_in_template_part = false, $is_in_synced_pattern = false ) {
		...
		foreach ( $blocks as $block ) {
			...
			if ( 'fincommerce/product-collection' === $block['blockName'] ) {
				$instances[] = array(
			...
			if ( 'fincommerce/single-product' === $block['blockName'] ) {
	...
	private function get_query_filters_usage_data( $block ) {
		...
		if ( ! empty( $query_attrs['fincommerceOnSale'] ) ) {
			$filters['on-sale'] = 'yes';
		}

		if ( ! empty( $query_attrs['fincommerceStockStatus'] ) ) {
			$stock_statuses = wc_get_product_stock_status_options();
			$default_values = 'yes' === get_option( 'fincommerce_hide_out_of_stock_items' ) ? array_diff_key( $stock_statuses, array( ProductStockStatus::OUT_OF_STOCK => '' ) ) : $stock_statuses;
			$default_diff   = array_diff( array_keys( $default_values ), $query_attrs['fincommerceStockStatus'] );
			if ( ! empty( $default_diff ) ) {
				$filters['stock-status'] = 'yes';
			}
		}

		if ( ! empty( $query_attrs['fincommerceAttributes'] ) ) {
		...
		if ( ! empty( $query_attrs['fincommerceHandPickedProducts'] ) ) {
