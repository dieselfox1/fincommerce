...
/**
 * This class adds actions to track usage of FinCommerce Products.
 */
class WC_Products_Tracking {
	...
	public function track_product_updated( $product_id, $post ) {
		...
		$source     = apply_filters( 'fincommerce_product_source', self::is_importing() ? 'import' : self::TRACKS_SOURCE );
		$properties = array(
	...
	public function track_product_updated_client_side( $post ) {
		wc_enqueue_js(
			"
			if ( $( 'h1.wp-heading-inline' ).text().trim() === '" . __( 'Edit product', 'fincommerce' ) . "') {
				...
				$( '#wpwrap' ).on( 'click', child_element, function() {
					...
					// We can't just check the number of '.fincommerce_attribute' elements because
					...
					var numberOfAttributes = $( '.fincommerce_attribute' ).filter( function () {
