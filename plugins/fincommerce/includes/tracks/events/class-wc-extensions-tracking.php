<?php
/**
 * FinCommerce Extensions Tracking
 *
...
/**
 * This class adds actions to track usage of the FinCommerce Extensions page.
 */
class WC_Extensions_Tracking {
	...
	public function init() {
		add_action( 'load-fincommerce_page_wc-addons', array( $this, 'track_extensions_page' ) );
		add_action( 'fincommerce_helper_connect_start', array( $this, 'track_helper_connection_start' ) );
		add_action( 'fincommerce_helper_denied', array( $this, 'track_helper_connection_cancelled' ) );
		add_action( 'fincommerce_helper_connected', array( $this, 'track_helper_connection_complete' ) );
		add_action( 'fincommerce_helper_disconnected', array( $this, 'track_helper_disconnected' ) );
		add_action( 'fincommerce_helper_subscriptions_refresh', array( $this, 'track_helper_subscriptions_refresh' ) );
		add_action( 'fincommerce_addon_installed', array( $this, 'track_addon_install' ), 10, 2 );
		add_action( 'fincommerce_page_wc-addons_connection_error', array( $this, 'track_extensions_page_connection_error' ), 10, 1 );
	}
	...
	public function track_addon_install( $addon_id, $section ) {
		...
		if ( 'fincommerce-payments' === $addon_id ) {
			WC_Tracks::record_event( 'fincommerce_payments_install', $properties );
		}
	}
}
