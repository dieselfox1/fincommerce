...
/**
 * This class adds actions to track usage of FinCommerce Settings.
 */
class WC_Settings_Tracking {

	/**
	 * List of allowed FinCommerce settings to potentially track updates for.
	 *
	 * @var array
	 */
	...
	/**
	 * FinCommerce settings that have been updated (and will be tracked).
	 *
	 * @var array
	 */
	...
	public function init() {
		add_action( 'fincommerce_settings_page_init', array( $this, 'track_settings_page_view' ) );
		add_action( 'fincommerce_update_option', array( $this, 'add_option_to_list' ) );
		add_action( 'fincommerce_update_non_option_setting', array( $this, 'add_option_to_list_and_track_setting_change' ) );
		add_action( 'fincommerce_update_options', array( $this, 'send_settings_change_event' ) );
	 ...
	 *
	 * @param array $option FinCommerce option that should be updated.
	 */
	...
	/**
	 * Add a FinCommerce option name to our allowed options list and attach
	 * the `update_option` hook. Rather than inspecting every updated
	 * option and pattern matching for "fincommerce", just build a dynamic
	 * list for FinCommerce options that might get updated.
	 *
	 * See `fincommerce_update_option` hook.
	 *
	 * @param array $option FinCommerce option (config) that might get updated.
	 */
	...
	/**
	 * Add FinCommerce option to a list of updated options.
	 *
	...
	/**
	 * Send a Tracks event for FinCommerce options that changed values.
	 */
	...
	/**
	 * Send a Tracks event for FinCommerce settings page views.
	 */
