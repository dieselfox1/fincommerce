...
/**
 * This class adds actions to track usage of FinCommerce Orders.
 */
class WC_Orders_Tracking {
	...
	public function init() {
		add_action( 'fincommerce_order_status_changed', array( $this, 'track_order_status_change' ), 10, 3 );
		// WC_Meta_Box_Order_Actions::save() hooks in at priority 50.
		add_action( 'fincommerce_process_shop_order_meta', array( $this, 'track_order_action' ), 51 );

		add_action( 'load-edit.php', array( $this, 'track_orders_view' ), 10 );
		add_action( 'load-fincommerce_page_wc-orders', array( $this, 'track_orders_view' ), 999 ); // HPOS.

		add_action( 'load-post-new.php', array( $this, 'track_add_order_from_edit' ), 10 );
		add_action( 'load-fincommerce_page_wc-orders', array( $this, 'track_add_order_from_edit' ), 999 ); // HPOS.

		add_action( 'fincommerce_process_shop_order_meta', array( $this, 'track_created_date_change' ), 10 );

		add_action( 'load-edit.php', array( $this, 'track_search_in_orders_list' ) );
		add_action( 'load-fincommerce_page_wc-orders', array( $this, 'track_search_in_orders_list' ), 999 ); // HPOS.
	...
	public function track_order_search( $order_ids, $term, $search_fields ) {
		...
		// Since `fincommerce_shop_order_search_results` can run in the front-end context, exit if get_current_screen isn't defined.
	 ...
	 * @param string $previous_status the old FinCommerce order status.
	 * @param string $next_status the new FinCommerce order status.
	 */
