...
/**
 * This class adds actions to track usage of FinCommerce.
 */
class WC_Site_Tracking {
	...
	public static function is_tracking_enabled() {
		 ...
		 * `fincommerce_apply_tracking` will be deprecated. Please use
		 * `fincommerce_apply_user_tracking` instead.
		 *
		 * @since 3.6.0
		 */
		if ( ! apply_filters( 'fincommerce_apply_user_tracking', true ) || ! apply_filters( 'fincommerce_apply_tracking', true ) ) {
			return false;
		}

		// Check if tracking is actively being opted into.
		$is_obw_opting_in = isset( $_POST['wc_tracker_checkbox'] ) && 'yes' === sanitize_text_field( $_POST['wc_tracker_checkbox'] ); // phpcs:ignore finpress.Security.NonceVerification.Missing, finpress.Security.ValidatedSanitizedInput

		/**
		 * Don't track users who haven't opted-in to tracking or aren't in
		 * the process of opting-in.
		 */
		if ( 'yes' !== get_option( 'fincommerce_allow_tracking' ) && ! $is_obw_opting_in ) {
			return false;
		}
	...
	public static function add_tracking_function() {
		 ...
		 * @since 6.5.0
		 */
		$filtered_properties = apply_filters( 'fincommerce_tracks_event_properties', $client_tracking_properties, false );
		...
		<script type="text/javascript">
			...
			window.wcTracks.recordEvent = function( name, properties ) {
				...
				if ( window.wp && window.wp.hooks && window.wp.hooks.applyFilters ) {
					eventProperties = window.wp.hooks.applyFilters( 'fincommerce_tracks_client_event_properties', eventProperties, eventName );
	 ...
	 * Note that this function does not update `fincommerce_allow_tracking` in the database
