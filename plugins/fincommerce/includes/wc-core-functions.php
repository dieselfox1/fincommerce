...
use Automattic\FinCommerce\Utilities\NumberUtil;
 ...
 */
add_filter( 'fincommerce_coupon_code', 'wc_sanitize_coupon_code' );
add_filter( 'fincommerce_coupon_code', 'wc_strtolower' );
add_filter( 'fincommerce_stock_amount', 'intval' ); // Stock amounts are integers by default.
add_filter( 'fincommerce_shipping_rate_label', 'sanitize_text_field' ); // Shipping rate label.
add_filter( 'fincommerce_attribute_label', 'wp_kses_post', 100 );

/**
 * Short Description (excerpt).
 */
if ( function_exists( 'do_blocks' ) ) {
	add_filter( 'fincommerce_short_description', 'do_blocks', 9 );
}
add_filter( 'fincommerce_short_description', 'wptexturize' );
add_filter( 'fincommerce_short_description', 'convert_smilies' );
add_filter( 'fincommerce_short_description', 'convert_chars' );
add_filter( 'fincommerce_short_description', 'wpautop' );
add_filter( 'fincommerce_short_description', 'shortcode_unautop' );
add_filter( 'fincommerce_short_description', 'prepend_attachment' );
add_filter( 'fincommerce_short_description', 'do_shortcode', 11 ); // After wpautop().
add_filter( 'fincommerce_short_description', 'wc_format_product_short_description', 9999999 );
add_filter( 'fincommerce_short_description', 'wc_do_oembeds' );
add_filter( 'fincommerce_short_description', array( $GLOBALS['wp_embed'], 'run_shortcode' ), 8 ); // Before wpautop().
...
function wc_create_order( $args = array() ) {
	...
	try {
		...
		if ( ! $args['order_id'] ) {
			$order->set_currency( get_fincommerce_currency() );
			$order->set_prices_include_tax( 'yes' === get_option( 'fincommerce_prices_include_tax' ) );
...
function wc_update_order( $args ) {
	if ( empty( $args['order_id'] ) ) {
		return new WP_Error( __( 'Invalid order ID.', 'fincommerce' ) );
	}
...
function wc_get_path_define_tokens() {
	...
	return apply_filters( 'fincommerce_get_path_define_tokens', $path_tokens );
}
...
function wc_get_template_part( $slug, $name = '' ) {
	...
	$template  = (string) wp_cache_get( $cache_key, 'fincommerce' );
	...
	if ( ! $template ) {
		...
		if ( ! $template ) {
			// If template file doesn't exist, look in yourtheme/slug.php and yourtheme/fincommerce/slug.php.
			$template = WC_TEMPLATE_DEBUG_MODE ? '' : locate_template(
				array(
					"{$slug}.php",
...
function wc_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
	...
	if ( $filter_template !== $template ) {
		if ( ! file_exists( $filter_template ) ) {
			...
			wc_doing_it_wrong( __FUNCTION__, sprintf( __( '%s does not exist.', 'fincommerce' ), '<code>' . $filter_template . '</code>' ), '2.1' );
			return;
		}
	...
	if ( ! empty( $args ) && is_array( $args ) ) {
		if ( isset( $args['action_args'] ) ) {
			wc_doing_it_wrong(
				__FUNCTION__,
				__( 'action_args should not be overwritten when calling wc_get_template.', 'fincommerce' ),
				'3.6.0'
			);
	...
	do_action( 'fincommerce_before_template_part', $action_args['template_name'], $action_args['template_path'], $action_args['located'], $action_args['args'] );

	include $action_args['located'];

	do_action( 'fincommerce_after_template_part', $action_args['template_name'], $action_args['template_path'], $action_args['located'], $action_args['args'] );
}
...
function wc_locate_template( $template_name, $template_path = '', $default_path = '' ) {
	...
	/**
	 * Filter to customize the path of a given FinCommerce template.
	 *
	 * Note: the $default_path argument was added in FinCommerce 9.5.0.
	 *
	 * @param string $template Full file path of the template.
	 * @param string $template_name Template name.
	 * @param string $template_path Template path.
	 * @param string $template_path Default FinCommerce templates path.
	 *
	 * @since 9.5.0 $default_path argument added.
	 */
	return apply_filters( 'fincommerce_locate_template', $template, $template_name, $template_path, $default_path );
}
...
function wc_set_template_cache( $cache_key, $template ) {
	wp_cache_set( $cache_key, $template, 'fincommerce' );

	$cached_templates = wp_cache_get( 'cached_templates', 'fincommerce' );
	...
	wp_cache_set( 'cached_templates', $cached_templates, 'fincommerce' );
}
...
function wc_clear_template_cache() {
	...
	if ( is_array( $cached_templates ) ) {
		foreach ( $cached_templates as $cache_key ) {
			wp_cache_delete( $cache_key, 'fincommerce' );
		}

		wp_cache_delete( 'cached_templates', 'fincommerce' );
	}
}
 ...
 * @return string
 */
function get_fincommerce_currency() {
	return apply_filters( 'fincommerce_currency', get_option( 'fincommerce_currency' ) );
}
 ...
 * @return array
 */
function get_fincommerce_currencies() {
	static $currencies;

	if ( ! isset( $currencies ) ) {
		$currencies = array_unique(
			apply_filters(
				'fincommerce_currencies',
				array(
					'AED' => __( 'United Arab Emirates dirham', 'fincommerce' ),
					'AFN' => __( 'Afghan afghani', 'fincommerce' ),
					'ALL' => __( 'Albanian lek', 'fincommerce' ),
					'AMD' => __( 'Armenian dram', 'fincommerce' ),
					'ANG' => __( 'Netherlands Antillean guilder', 'fincommerce' ),
					'AOA' => __( 'Angolan kwanza', 'fincommerce' ),
					'ARS' => __( 'Argentine peso', 'fincommerce' ),
					'AUD' => __( 'Australian dollar', 'fincommerce' ),
					'AWG' => __( 'Aruban florin', 'fincommerce' ),
					'AZN' => __( 'Azerbaijani manat', 'fincommerce' ),
					'BAM' => __( 'Bosnia and Herzegovina convertible mark', 'fincommerce' ),
					'BBD' => __( 'Barbadian dollar', 'fincommerce' ),
					'BDT' => __( 'Bangladeshi taka', 'fincommerce' ),
					'BGN' => __( 'Bulgarian lev', 'fincommerce' ),
					'BHD' => __( 'Bahraini dinar', 'fincommerce' ),
					'BIF' => __( 'Burundian franc', 'fincommerce' ),
					'BMD' => __( 'Bermudian dollar', 'fincommerce' ),
					'BND' => __( 'Brunei dollar', 'fincommerce' ),
					'BOB' => __( 'Bolivian boliviano', 'fincommerce' ),
					'BRL' => __( 'Brazilian real', 'fincommerce' ),
					'BSD' => __( 'Bahamian dollar', 'fincommerce' ),
					'BTC' => __( 'Bitcoin', 'fincommerce' ),
					'BTN' => __( 'Bhutanese ngultrum', 'fincommerce' ),
					'BWP' => __( 'Botswana pula', 'fincommerce' ),
					'BYR' => __( 'Belarusian ruble (old)', 'fincommerce' ),
					'BYN' => __( 'Belarusian ruble', 'fincommerce' ),
					'BZD' => __( 'Belize dollar', 'fincommerce' ),
					'CAD' => __( 'Canadian dollar', 'fincommerce' ),
					'CDF' => __( 'Congolese franc', 'fincommerce' ),
					'CHF' => __( 'Swiss franc', 'fincommerce' ),
					'CLP' => __( 'Chilean peso', 'fincommerce' ),
					'CNY' => __( 'Chinese yuan', 'fincommerce' ),
					'COP' => __( 'Colombian peso', 'fincommerce' ),
					'CRC' => __( 'Costa Rican col&oacute;n', 'fincommerce' ),
					'CUC' => __( 'Cuban convertible peso', 'fincommerce' ),
					'CUP' => __( 'Cuban peso', 'fincommerce' ),
					'CVE' => __( 'Cape Verdean escudo', 'fincommerce' ),
					'CZK' => __( 'Czech koruna', 'fincommerce' ),
					'DJF' => __( 'Djiboutian franc', 'fincommerce' ),
					'DKK' => __( 'Danish krone', 'fincommerce' ),
					'DOP' => __( 'Dominican peso', 'fincommerce' ),
					'DZD' => __( 'Algerian dinar', 'fincommerce' ),
					'EGP' => __( 'Egyptian pound', 'fincommerce' ),
					'ERN' => __( 'Eritrean nakfa', 'fincommerce' ),
					'ETB' => __( 'Ethiopian birr', 'fincommerce' ),
					'EUR' => __( 'Euro', 'fincommerce' ),
					'FJD' => __( 'Fijian dollar', 'fincommerce' ),
					'FKP' => __( 'Falkland Islands pound', 'fincommerce' ),
					'GBP' => __( 'Pound sterling', 'fincommerce' ),
					'GEL' => __( 'Georgian lari', 'fincommerce' ),
					'GGP' => __( 'Guernsey pound', 'fincommerce' ),
					'GHS' => __( 'Ghana cedi', 'fincommerce' ),
					'GIP' => __( 'Gibraltar pound', 'fincommerce' ),
					'GMD' => __( 'Gambian dalasi', 'fincommerce' ),
					'GNF' => __( 'Guinean franc', 'fincommerce' ),
					'GTQ' => __( 'Guatemalan quetzal', 'fincommerce' ),
					'GYD' => __( 'Guyanese dollar', 'fincommerce' ),
					'HKD' => __( 'Hong Kong dollar', 'fincommerce' ),
					'HNL' => __( 'Honduran lempira', 'fincommerce' ),
					'HRK' => __( 'Croatian kuna', 'fincommerce' ),
					'HTG' => __( 'Haitian gourde', 'fincommerce' ),
					'HUF' => __( 'Hungarian forint', 'fincommerce' ),
					'IDR' => __( 'Indonesian rupiah', 'fincommerce' ),
					'ILS' => __( 'Israeli new shekel', 'fincommerce' ),
					'IMP' => __( 'Manx pound', 'fincommerce' ),
					'INR' => __( 'Indian rupee', 'fincommerce' ),
					'IQD' => __( 'Iraqi dinar', 'fincommerce' ),
					'IRR' => __( 'Iranian rial', 'fincommerce' ),
					'IRT' => __( 'Iranian toman', 'fincommerce' ),
					'ISK' => __( 'Icelandic kr&oacute;na', 'fincommerce' ),
					'JEP' => __( 'Jersey pound', 'fincommerce' ),
					'JMD' => __( 'Jamaican dollar', 'fincommerce' ),
					'JOD' => __( 'Jordanian dinar', 'fincommerce' ),
					'JPY' => __( 'Japanese yen', 'fincommerce' ),
					'KES' => __( 'Kenyan shilling', 'fincommerce' ),
					'KGS' => __( 'Kyrgyzstani som', 'fincommerce' ),
					'KHR' => __( 'Cambodian riel', 'fincommerce' ),
					'KMF' => __( 'Comorian franc', 'fincommerce' ),
					'KPW' => __( 'North Korean won', 'fincommerce' ),
					'KRW' => __( 'South Korean won', 'fincommerce' ),
					'KWD' => __( 'Kuwaiti dinar', 'fincommerce' ),
					'KYD' => __( 'Cayman Islands dollar', 'fincommerce' ),
					'KZT' => __( 'Kazakhstani tenge', 'fincommerce' ),
					'LAK' => __( 'Lao kip', 'fincommerce' ),
					'LBP' => __( 'Lebanese pound', 'fincommerce' ),
					'LKR' => __( 'Sri Lankan rupee', 'fincommerce' ),
					'LRD' => __( 'Liberian dollar', 'fincommerce' ),
					'LSL' => __( 'Lesotho loti', 'fincommerce' ),
					'LYD' => __( 'Libyan dinar', 'fincommerce' ),
					'MAD' => __( 'Moroccan dirham', 'fincommerce' ),
					'MDL' => __( 'Moldovan leu', 'fincommerce' ),
					'MGA' => __( 'Malagasy ariary', 'fincommerce' ),
					'MKD' => __( 'Macedonian denar', 'fincommerce' ),
					'MMK' => __( 'Burmese kyat', 'fincommerce' ),
					'MNT' => __( 'Mongolian t&ouml;gr&ouml;g', 'fincommerce' ),
					'MOP' => __( 'Macanese pataca', 'fincommerce' ),
					'MRU' => __( 'Mauritanian ouguiya', 'fincommerce' ),
					'MUR' => __( 'Mauritian rupee', 'fincommerce' ),
					'MVR' => __( 'Maldivian rufiyaa', 'fincommerce' ),
					'MWK' => __( 'Malawian kwacha', 'fincommerce' ),
					'MXN' => __( 'Mexican peso', 'fincommerce' ),
					'MYR' => __( 'Malaysian ringgit', 'fincommerce' ),
					'MZN' => __( 'Mozambican metical', 'fincommerce' ),
					'NAD' => __( 'Namibian dollar', 'fincommerce' ),
					'NGN' => __( 'Nigerian naira', 'fincommerce' ),
					'NIO' => __( 'Nicaraguan c&oacute;rdoba', 'fincommerce' ),
					'NOK' => __( 'Norwegian krone', 'fincommerce' ),
					'NPR' => __( 'Nepalese rupee', 'fincommerce' ),
					'NZD' => __( 'New Zealand dollar', 'fincommerce' ),
					'OMR' => __( 'Omani rial', 'fincommerce' ),
					'PAB' => __( 'Panamanian balboa', 'fincommerce' ),
					'PEN' => __( 'Sol', 'fincommerce' ),
					'PGK' => __( 'Papua New Guinean kina', 'fincommerce' ),
					'PHP' => __( 'Philippine peso', 'fincommerce' ),
					'PKR' => __( 'Pakistani rupee', 'fincommerce' ),
					'PLN' => __( 'Polish z&#x142;oty', 'fincommerce' ),
					'PRB' => __( 'Transnistrian ruble', 'fincommerce' ),
					'PYG' => __( 'Paraguayan guaran&iacute;', 'fincommerce' ),
					'QAR' => __( 'Qatari riyal', 'fincommerce' ),
					'RON' => __( 'Romanian leu', 'fincommerce' ),
					'RSD' => __( 'Serbian dinar', 'fincommerce' ),
					'RUB' => __( 'Russian ruble', 'fincommerce' ),
					'RWF' => __( 'Rwandan franc', 'fincommerce' ),
					'SAR' => __( 'Saudi riyal', 'fincommerce' ),
					'SBD' => __( 'Solomon Islands dollar', 'fincommerce' ),
					'SCR' => __( 'Seychellois rupee', 'fincommerce' ),
					'SDG' => __( 'Sudanese pound', 'fincommerce' ),
					'SEK' => __( 'Swedish krona', 'fincommerce' ),
					'SGD' => __( 'Singapore dollar', 'fincommerce' ),
					'SHP' => __( 'Saint Helena pound', 'fincommerce' ),
					'SLL' => __( 'Sierra Leonean leone', 'fincommerce' ),
					'SOS' => __( 'Somali shilling', 'fincommerce' ),
					'SRD' => __( 'Surinamese dollar', 'fincommerce' ),
					'SSP' => __( 'South Sudanese pound', 'fincommerce' ),
					'STN' => __( 'S&atilde;o Tom&eacute; and Pr&iacute;ncipe dobra', 'fincommerce' ),
					'SYP' => __( 'Syrian pound', 'fincommerce' ),
					'SZL' => __( 'Swazi lilangeni', 'fincommerce' ),
					'THB' => __( 'Thai baht', 'fincommerce' ),
					'TJS' => __( 'Tajikistani somoni', 'fincommerce' ),
					'TMT' => __( 'Turkmenistan manat', 'fincommerce' ),
					'TND' => __( 'Tunisian dinar', 'fincommerce' ),
					'TOP' => __( 'Tongan pa&#x2bb;anga', 'fincommerce' ),
					'TRY' => __( 'Turkish lira', 'fincommerce' ),
					'TTD' => __( 'Trinidad and Tobago dollar', 'fincommerce' ),
					'TWD' => __( 'New Taiwan dollar', 'fincommerce' ),
					'TZS' => __( 'Tanzanian shilling', 'fincommerce' ),
					'UAH' => __( 'Ukrainian hryvnia', 'fincommerce' ),
					'UGX' => __( 'Ugandan shilling', 'fincommerce' ),
					'USD' => __( 'United States (US) dollar', 'fincommerce' ),
					'UYU' => __( 'Uruguayan peso', 'fincommerce' ),
					'UZS' => __( 'Uzbekistani som', 'fincommerce' ),
					'VEF' => __( 'Venezuelan bol&iacute;var (2008–2018)', 'fincommerce' ),
					'VES' => __( 'Venezuelan bol&iacute;var', 'fincommerce' ),
					'VND' => __( 'Vietnamese &#x111;&#x1ed3;ng', 'fincommerce' ),
					'VUV' => __( 'Vanuatu vatu', 'fincommerce' ),
					'WST' => __( 'Samoan t&#x101;l&#x101;', 'fincommerce' ),
					'XAF' => __( 'Central African CFA franc', 'fincommerce' ),
					'XCD' => __( 'East Caribbean dollar', 'fincommerce' ),
					'XOF' => __( 'West African CFA franc', 'fincommerce' ),
					'XPF' => __( 'CFP franc', 'fincommerce' ),
					'YER' => __( 'Yemeni rial', 'fincommerce' ),
					'ZAR' => __( 'South African rand', 'fincommerce' ),
					'ZMW' => __( 'Zambian kwacha', 'fincommerce' ),
				)
			)
		);
	}
 ...
 * @return array
 */
function get_fincommerce_currency_symbols() {

	$symbols = apply_filters(
		'fincommerce_currency_symbols',
		array(
			'AED' => '&#x62f;.&#x625;',
			'AFN' => '&#x60b;',
			'ALL' => 'L',
 ...
 * @return string
 */
function get_fincommerce_currency_symbol( $currency = '' ) {
	if ( ! $currency ) {
		$currency = get_fincommerce_currency();
	}

	$symbols = get_fincommerce_currency_symbols();

	$currency_symbol = isset( $symbols[ $currency ] ) ? $symbols[ $currency ] : '';

	return apply_filters( 'fincommerce_currency_symbol', $currency_symbol, $currency );
}

/**
 * Send HTML emails from FinCommerce.
 *
...
function wc_get_theme_support( $prop = '', $default = null ) {
	$theme_support = get_theme_support( 'fincommerce' );
 ...
 *
 * The returned variable is filtered by fincommerce_get_image_size_{image_size} filter to
...
function wc_get_image_size( $image_size ) {
	...
	$size      = ! is_customize_preview() ? wp_cache_get( $cache_key, 'fincommerce' ) : false;
	...
	if ( is_array( $image_size ) ) {
		...
		$image_size = str_replace( 'fincommerce_', '', $image_size );

		if ( 'single' === $image_size ) {
			$size['width']  = absint( wc_get_theme_support( 'single_image_width', get_option( 'fincommerce_single_image_width', 600 ) ) );
			$size['height'] = '';
			$size['crop']   = 0;
			...
			$size['width'] = absint( wc_get_theme_support( 'thumbnail_image_width', get_option( 'fincommerce_thumbnail_image_width', 300 ) ) );
			$cropping      = get_option( 'fincommerce_thumbnail_cropping', '1:1' );

			if ( 'uncropped' === $cropping ) {
				$size['height'] = '';
				$size['crop']   = 0;
			} elseif ( 'custom' === $cropping ) {
				$width          = max( 1, (float) get_option( 'fincommerce_thumbnail_cropping_custom_width', '4' ) );
				$height         = max( 1, (float) get_option( 'fincommerce_thumbnail_cropping_custom_height', '3' ) );
	...
	$size = apply_filters( 'fincommerce_get_image_size_' . $image_size, $size );

	if ( is_customize_preview() ) {
		wp_cache_delete( $cache_key, 'fincommerce' );
	} else {
		wp_cache_set( $cache_key, $size, 'fincommerce' );
	}
	return $size;
}
...
function wc_setcookie( $name, $value, $expire = 0, $secure = false, $httponly = false ) {
	 ...
	 */
	if ( ! apply_filters( 'fincommerce_set_cookie_enabled', true, $name, $value, $expire, $secure ) ) {
		return;
	}
	...
	if ( ! headers_sent() ) {
		...
		$options = apply_filters(
			'fincommerce_set_cookie_options',
			array(
				'expires'  => $expire,
				'secure'   => $secure,
				 ...
				 * @param bool   $secure   If the cookie should only be served over HTTPS.
				 */
				'httponly' => apply_filters( 'fincommerce_cookie_httponly', $httponly, $name, $value, $expire, $secure ),
			),
			$name,
			$value
		);
...
function wc_ms_protect_download_rewite_rules( $rewrite ) {
	if ( ! is_multisite() || 'redirect' === get_option( 'fincommerce_file_download_method' ) ) {
		return $rewrite;
	}

	$rule  = "\n# FinCommerce Rules - Protect Files from ms-files.php\n\n";
	$rule .= "<IfModule mod_rewrite.c>\n";
	$rule .= "RewriteEngine On\n";
	$rule .= "RewriteCond %{QUERY_STRING} file=fincommerce_uploads/ [NC]\n";
...
function wc_get_base_location() {
	$default = apply_filters( 'fincommerce_get_base_location', get_option( 'fincommerce_default_country', 'US:CA' ) );
...
function wc_get_customer_default_location() {
	$set_default_location_to = get_option( 'fincommerce_default_customer_address', 'base' );

	// Unless the location should be blank, use the base location as the default.
	if ( '' !== $set_default_location_to ) {
		$default_location_string = get_option( 'fincommerce_default_country', 'US:CA' );
	}
	...
	$default_location = wc_format_country_state_string(
		 ...
		 * @return string
		 */
		apply_filters( 'fincommerce_customer_default_location', $default_location_string ?? '' )
	);
	 ...
	 * @return array
	 */
	return apply_filters( 'fincommerce_customer_default_location_array', $default_location );
}
...
function wc_get_cart_url() {
	 ...
	 */
	return apply_filters( 'fincommerce_get_cart_url', $cart_url );
}
...
function wc_get_checkout_url() {
	...
	if ( $checkout_url ) {
		// Force SSL if needed.
		if ( is_ssl() || 'yes' === get_option( 'fincommerce_force_ssl_checkout' ) ) {
			$checkout_url = str_replace( 'http:', 'https:', $checkout_url );
		}
	}

	return apply_filters( 'fincommerce_get_checkout_url', $checkout_url );
}
 ...
 */
function fincommerce_register_shipping_method( $shipping_method ) {
...
function wc_get_credit_card_type_label( $type ) {
	...
	$labels = apply_filters(
		'fincommerce_credit_card_type_labels',
		array(
			'mastercard'       => _x( 'MasterCard', 'Name of credit card', 'fincommerce' ),
			'visa'             => _x( 'Visa', 'Name of credit card', 'fincommerce' ),
			'discover'         => _x( 'Discover', 'Name of credit card', 'fincommerce' ),
			'american express' => _x( 'American Express', 'Name of credit card', 'fincommerce' ),
			'cartes bancaires' => _x( 'Cartes Bancaires', 'Name of credit card', 'fincommerce' ),
			'diners'           => _x( 'Diners', 'Name of credit card', 'fincommerce' ),
			'jcb'              => _x( 'JCB', 'Name of credit card', 'fincommerce' ),
		)
	);
	 ...
	 * @since 8.9.0
	 */
	return apply_filters( 'fincommerce_get_credit_card_type_label', ( array_key_exists( $type, $labels ) ? $labels[ $type ] : ucwords( $type ) ) );
}
...
/**
 * Display a FinCommerce help tip.
 *
 * @since  2.5.0
 *
...
function wc_help_tip( $tip, $allow_html = false ) {
	 ...
	 * @return string
	 */
	return apply_filters( 'wc_help_tip', '<span class="fincommerce-help-tip" tabindex="0" aria-label="' . esc_attr( $aria_label ) . '" data-tip="' . $sanitized_tip . '"></span>', $sanitized_tip, $tip, $allow_html );
}
...
function wc_get_tax_rounding_mode() {
	...
	if ( 'auto' === $constant ) {
		return 'yes' === get_option( 'fincommerce_prices_include_tax', 'no' ) ? PHP_ROUND_HALF_DOWN : PHP_ROUND_HALF_UP;
	}
...
function wc_get_rounding_precision() {
	 ...
	 */
	return apply_filters( 'fincommerce_internal_rounding_precision', $precision );
}
 ...
 *
 * Use the fincommerce_logging_class filter to change the logging class. You may provide one of the following:
...
function wc_get_logger() {
	...
	$class = apply_filters( 'fincommerce_logging_class', 'WC_Logger' );
	...
	if ( is_array( $implements ) && in_array( 'WC_Logger_Interface', $implements, true ) ) {
		...
		wc_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: 1: class name 2: fincommerce_logging_class 3: WC_Logger_Interface */
				__( 'The class %1$s provided by %2$s filter must implement %3$s.', 'fincommerce' ),
				'<code>' . esc_html( is_object( $class ) ? get_class( $class ) : $class ) . '</code>',
				'<code>fincommerce_logging_class</code>',
...
add_action( 'fincommerce_cleanup_logs', 'wc_cleanup_logs' );
...
function wc_print_r( $expression, $return = false ) {
	...
	$alternatives = apply_filters( 'fincommerce_print_r_alternatives', $alternatives, $expression );
...
function wc_get_permalink_structure() {
	$saved_permalinks = (array) get_option( 'fincommerce_permalinks', array() );
	$permalinks       = wp_parse_args(
		array_filter( $saved_permalinks ),
		array(
			'product_base'           => _x( 'product', 'slug', 'fincommerce' ),
			'category_base'          => _x( 'product-category', 'slug', 'fincommerce' ),
			'tag_base'               => _x( 'product-tag', 'slug', 'fincommerce' ),
			'attribute_base'         => '',
...
/**
 * Switch FinCommerce to site language.
 *
 * @since 3.1.0
 */
...
/**
 * Switch FinCommerce language to original.
 *
 * @since 3.1.0
 */
...
/**
 * Read in FinCommerce headers when reading plugin headers.
 *
 * @since 3.2.0
...
function wc_enable_wc_plugin_headers( $headers ) {
	...
	// WC requires at least - allows developers to define which version of FinCommerce the plugin requires to run.
	$headers[] = WC_Plugin_Updates::VERSION_REQUIRED_HEADER;

	// WC tested up to - allows developers  to define which version of FinCommerce they have tested up to.
	$headers[] = WC_Plugin_Updates::VERSION_TESTED_HEADER;

	// Woo - This is used in FinCommerce extensions and is picked up by the helper.
	$headers[] = 'Woo';
...
/**
 * Prevent auto-updating the FinCommerce plugin on major releases if there are untested extensions active.
 *
 * @since 3.2.0
...
add_action( 'fincommerce_installed', 'wc_delete_expired_transients' );
...
function wc_cleanup_session_data() {
	$session_class = apply_filters( 'fincommerce_session_handler', 'WC_Session_Handler' );
...
add_action( 'fincommerce_cleanup_sessions', 'wc_cleanup_session_data' );
...
function wc_load_cart() {
	if ( ! did_action( 'before_fincommerce_init' ) || doing_action( 'before_fincommerce_init' ) ) {
		/* translators: 1: wc_load_cart 2: fincommerce_init */
		wc_doing_it_wrong( __FUNCTION__, sprintf( __( '%1$s should not be called before the %2$s action.', 'fincommerce' ), 'wc_load_cart', 'fincommerce_init' ), '3.7' );
		return;
	}
