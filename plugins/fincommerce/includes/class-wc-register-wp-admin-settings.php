...
class WC_Register_WP_Admin_Settings {
	...
	public function __construct( $object, $type ) {
		...
		if ( 'page' === $type ) {
			add_filter( 'fincommerce_settings_groups', array( $this, 'register_page_group' ) );
			add_filter( 'fincommerce_settings-' . $this->object->get_id(), array( $this, 'register_page_settings' ) );
		} elseif ( 'email' === $type ) {
			add_filter( 'fincommerce_settings_groups', array( $this, 'register_email_group' ) );
			add_filter( 'fincommerce_settings-email_' . $this->object->id, array( $this, 'register_email_settings' ) );
		}
	}
