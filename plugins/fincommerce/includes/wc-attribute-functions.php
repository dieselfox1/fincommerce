<?php
/**
 * FinCommerce Attribute Functions
 *
...
function wc_get_attribute_taxonomies() {
	$prefix      = WC_Cache_Helper::get_cache_prefix( 'fincommerce-attributes' );
	$cache_key   = $prefix . 'attributes';
	$cache_value = wp_cache_get( $cache_key, 'fincommerce-attributes' );
	...
	if ( false === $raw_attribute_taxonomies ) {
		...
		$raw_attribute_taxonomies = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}fincommerce_attribute_taxonomies WHERE attribute_name != '' ORDER BY attribute_name ASC;" );
	 ...
	 */
	$raw_attribute_taxonomies = (array) array_filter( apply_filters( 'fincommerce_attribute_taxonomies', $raw_attribute_taxonomies ) );
	...
	wp_cache_set( $cache_key, $attribute_taxonomies, 'fincommerce-attributes' );
...
function wc_get_attribute_taxonomy_ids() {
	...
	wp_cache_set( $cache_key, $taxonomy_ids, 'fincommerce-attributes' );
...
function wc_get_attribute_taxonomy_labels() {
	...
	wp_cache_set( $cache_key, $taxonomy_labels, 'fincommerce-attributes' );
...
function wc_attribute_label( $name, $product = '' ) {
	...
	return apply_filters( 'fincommerce_attribute_label', $label, $name, $product );
}
...
function wc_attribute_orderby( $name ) {
	...
	return apply_filters( 'fincommerce_attribute_orderby', isset( $taxonomies[ 'id:' . $id ] ) ? $taxonomies[ 'id:' . $id ]->attribute_orderby : 'menu_order', $name );
}
...
function wc_get_attribute_type_label( $type ) {
	...
	return isset( $types[ $type ] ) ? $types[ $type ] : __( 'Select', 'fincommerce' );
}
...
function wc_is_attribute_in_product_name( $attribute, $name ) {
	...
	return apply_filters( 'fincommerce_is_attribute_in_product_name', $is_in_name, $attribute, $name );
}
...
function wc_create_attribute( $args ) {
	...
	// Name is required.
	if ( empty( $args['name'] ) ) {
		return new WP_Error( 'missing_attribute_name', __( 'Please, provide an attribute name.', 'fincommerce' ), array( 'status' => 400 ) );
	}
	...
	if ( strlen( $slug ) > 28 ) {
		...
		return new WP_Error( 'invalid_product_attribute_slug_too_long', sprintf( __( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'fincommerce' ), $slug ), array( 'status' => 400 ) );
	} elseif ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
		/* translators: %s: attribute slug */
		return new WP_Error( 'invalid_product_attribute_slug_reserved_name', sprintf( __( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'fincommerce' ), $slug ), array( 'status' => 400 ) );
		...
		/* translators: %s: attribute slug */
		return new WP_Error( 'invalid_product_attribute_slug_already_exists', sprintf( __( 'Slug "%s" is already in use. Change it, please.', 'fincommerce' ), $slug ), array( 'status' => 400 ) );
	}
	...
	if ( 0 === $id ) {
		 ...
		 */
		do_action( 'fincommerce_attribute_added', $id, $data );
	} else {
		$results = $wpdb->update(
			$wpdb->prefix . 'fincommerce_attribute_taxonomies',
			$data,
		...
		if ( false === $results ) {
			return new WP_Error( 'cannot_update_attribute', __( 'Could not update the attribute.', 'fincommerce' ), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( 'fincommerce_attribute_updated', $id, $data, $old_slug );
...
function wc_delete_attribute( $id ) {
	 ...
	 */
	do_action( 'fincommerce_before_attribute_delete', $id, $name, $taxonomy );

	if ( $name && $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}fincommerce_attribute_taxonomies WHERE attribute_id = %d", $id ) ) ) {
		 ...
		 * @param string $taxonomy Attribute taxonomy name.
		 */
		do_action( 'fincommerce_attribute_deleted', $id, $name, $taxonomy );
		wp_schedule_single_event( time(), 'fincommerce_flush_rewrite_rules' );
		delete_transient( 'wc_attribute_taxonomies' );
		WC_Cache_Helper::invalidate_cache_group( 'fincommerce-attributes' );
...
function wc_attribute_taxonomy_slug( $attribute_name ) {
	...
	wp_cache_set( $cache_key, $attribute_slug, 'fincommerce-attributes' );
