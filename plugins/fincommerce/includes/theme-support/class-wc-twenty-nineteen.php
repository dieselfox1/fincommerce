...
class WC_Twenty_Nineteen {
	...
	public static function init() {
		...
		add_action( 'fincommerce_before_main_content', array( __CLASS__, 'output_content_wrapper' ), 10 );
		add_action( 'fincommerce_after_main_content', array( __CLASS__, 'output_content_wrapper_end' ), 10 );

		// This theme doesn't have a traditional sidebar.
		remove_action( 'fincommerce_sidebar', 'fincommerce_get_sidebar', 10 );

		// Enqueue theme compatibility styles.
		add_filter( 'fincommerce_enqueue_styles', array( __CLASS__, 'enqueue_styles' ) );
	...
	public static function enqueue_styles( $styles ) {
		unset( $styles['fincommerce-general'] );

		$styles['fincommerce-general'] = array(
		...
		return apply_filters( 'fincommerce_twenty_nineteen_styles', $styles );
	}
	...
	public static function custom_colors_css( $css, $primary_color, $saturation ) {
		...
		$css .= '
			.onsale,
			.fincommerce-info,
			.fincommerce-store-notice {
				background-color: hsl( ' . $primary_color . ', ' . $saturation . ', ' . $lightness . ' );
			}

			.fincommerce-tabs ul li.active a {
