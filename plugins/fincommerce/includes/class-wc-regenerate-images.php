...
class WC_Regenerate_Images {
	...
	public static function init() {
		...
		if ( apply_filters( 'fincommerce_background_image_regeneration', true ) ) {
			...
			add_action( 'fincommerce_hide_regenerating_thumbnails_notice', array( __CLASS__, 'dismiss_regenerating_notice' ) );
	...
	public static function filter_image_get_intermediate_size( $data, $attachment_id, $size ) {
		if ( ! is_string( $size ) || ! in_array( $size, apply_filters( 'fincommerce_image_sizes_to_resize', array( 'fincommerce_thumbnail', 'fincommerce_gallery_thumbnail', 'fincommerce_single' ) ), true ) ) {
			return $data;
		}
	...
	public static function add_uncropped_metadata( $meta_data ) {
		$size_data = wc_get_image_size( 'fincommerce_thumbnail' );
		if ( isset( $meta_data['sizes'], $meta_data['sizes']['fincommerce_thumbnail'] ) ) {
			$meta_data['sizes']['fincommerce_thumbnail']['uncropped'] = empty( $size_data['height'] );
		}
		return $meta_data;
	}
	...
	public static function dismiss_regenerating_notice() {
		if ( self::$background_process ) {
			...
			$log = wc_get_logger();
			$log->info(
				__( 'Cancelled product image regeneration job.', 'fincommerce' ),
				array(
	...
	public static function maybe_regenerate_images() {
		...
		if ( update_option( 'fincommerce_maybe_regenerate_images_hash', $size_hash ) ) {
	...
	public static function maybe_resize_image( $image, $attachment_id, $size, $icon ) {
		if ( ! apply_filters( 'fincommerce_resize_images', true ) ) {
			return $image;
		}

		// List of sizes we want to resize. Ignore others.
		if ( ! $image || ! in_array( $size, apply_filters( 'fincommerce_image_sizes_to_resize', array( 'fincommerce_thumbnail', 'fincommerce_gallery_thumbnail', 'fincommerce_single' ) ), true ) ) {
			return $image;
		}
