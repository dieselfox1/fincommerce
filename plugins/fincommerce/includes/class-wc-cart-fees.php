 ...
 *
 * We suggest using the action fincommerce_cart_calculate_fees hook for adding fees.
 *
...
final class WC_Cart_Fees {
	 ...
	 *
	 * @param null $deprecated Deprecated since FinCommerce 8.2.0.
	 *
	 * @since 3.2.0
	 */
	...
	public function add_fee( $args = array() ) {
		...
		$fee_props->name      = $fee_props->name ? $fee_props->name : __( 'Fee', 'fincommerce' );
		...
		if ( array_key_exists( $fee_props->id, $this->fees ) ) {
			return new WP_Error( 'fee_exists', __( 'Fee has already been added.', 'fincommerce' ) );
		}
	...
	protected function sort_fees_callback( $a, $b ) {
		 ...
		 * @param stdClass $b Fee object.
		 */
		return apply_filters( 'fincommerce_sort_fees_callback', $a->amount > $b->amount ? -1 : 1, $a, $b );
	}
