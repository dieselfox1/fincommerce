...
use Automattic\FinCommerce\Caching\CacheNameSpaceTrait;
...
class WC_Cache_Helper {
	...
	public static function geolocation_ajax_get_location_hash() {
		 ...
		 */
		return apply_filters( 'fincommerce_geolocation_ajax_get_location_hash', $location_hash, $location, $customer );
	}
	...
	public static function geolocation_ajax_redirect() {
		if ( 'geolocation_ajax' === get_option( 'fincommerce_default_customer_address' ) && ! is_checkout() && ! is_cart() && ! is_account_page() && ! wp_doing_ajax() && empty( $_POST ) ) { // WPCS: CSRF ok, input var ok.
	...
	/**
	 * Updates the `fincommerce_geo_hash` cookie, which is used to help ensure we display
	...
	public static function update_geolocation_hash() {
		if ( 'geolocation_ajax' === get_option( 'fincommerce_default_customer_address' ) ) {
			wc_setcookie( 'fincommerce_geo_hash', static::geolocation_ajax_get_location_hash(), time() + HOUR_IN_SECONDS );
		}
	}
	 ...
	 *
	 * Raised in issue https://github.com/dieselfox1/fincommerce/issues/5777.
	...
	public static function notices() {
		...
		if ( $enabled && ! in_array( '_wc_session_', $settings, true ) ) {
			...
			<div class="error">
				...
				echo wp_kses_post( sprintf( __( 'In order for <strong>database caching</strong> to work with FinCommerce you must add %1$s to the "Ignored Query Strings" option in <a href="%2$s">W3 Total Cache settings</a>.', 'fincommerce' ), '<code>_wc_session_</code>', esc_url( admin_url( 'admin.php?page=w3tc_dbcache' ) ) ) );
				?>
				</p>
			</div>
			<?php
	...
	/**
	 * Clean term caches added by FinCommerce.
	 *
	 * @since 3.3.4
	...
	public static function delete_version_transients( $version = '' ) {
		if ( ! wp_using_ext_object_cache() && ! empty( $version ) ) {
			...
			$limit = apply_filters( 'fincommerce_delete_version_transients_limit', 1000 );
