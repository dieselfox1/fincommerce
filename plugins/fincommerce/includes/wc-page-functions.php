...
function wc_page_endpoint_document_title_parts( $title ) {
	...
	if ( ! is_null( $wp_query ) && ! is_admin() && is_main_query() && is_page() && is_wc_endpoint_url() ) {
		$endpoint       = WC()->query->get_current_endpoint();
		$action         = isset( $_GET['action'] ) ? sanitize_text_field( wp_unslash( $_GET['action'] ) ) : ''; // phpcs:ignore finpress.Security.NonceVerification.Recommended
		$endpoint_title = WC()->query->get_endpoint_title( $endpoint, $action );
...
function wc_get_page_id( $page ) {
	...
	$page = apply_filters( 'fincommerce_get_' . $page . '_page_id', get_option( 'fincommerce_' . $page . '_page_id' ) );
...
function wc_get_page_permalink( $page, $fallback = null ) {
	...
	return apply_filters( 'fincommerce_get_' . $page . '_page_permalink', $permalink );
}
...
function wc_get_endpoint_url( $endpoint, $value = '', $permalink = '' ) {
	...
	$value      = ( get_option( 'fincommerce_myaccount_edit_address_endpoint', 'edit-address' ) === $endpoint ) ? wc_edit_address_i18n( $value ) : $value;
	...
	return apply_filters( 'fincommerce_get_endpoint_url', $url, $endpoint, $value, $permalink );
}
...
function wc_nav_menu_items( $items ) {
	$logout_endpoint = get_option( 'fincommerce_logout_endpoint', 'customer-logout' );
 ...
 *
 * See details in https://github.com/dieselfox1/fincommerce/issues/177.
 *
...
function wc_list_pages( $pages ) {
	...
	// Find shop_page_id through fincommerce options.
