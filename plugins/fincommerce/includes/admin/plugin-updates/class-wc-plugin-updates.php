...
class WC_Plugin_Updates {
	...
	/**
	 * The version for the update to FinCommerce.
	 *
	 * @var string
	 */
	...
	*/
	...
	protected function get_extensions_inline_warning_major() {
		...
		$message = sprintf( __( "<strong>Heads up!</strong> The versions of the following plugins you're running haven't been tested with FinCommerce %s. Please update them or confirm compatibility before updating FinCommerce, or you may experience issues:", 'fincommerce' ), $new_version );
	...
	*/
	 ...
	 *
	 * @param string $new_version FinCommerce version to test against.
	...
	public function get_untested_plugins( $new_version, $release ) {
		...
		$extensions        = array_merge( $this->get_plugins_with_header( self::VERSION_TESTED_HEADER ), $this->get_plugins_for_fincommerce() );
		$untested          = array();
		...
		foreach ( $extensions as $file => $plugin ) {
			if ( ! empty( $plugin[ self::VERSION_TESTED_HEADER ] ) ) {
				...
				$plugin[ self::VERSION_TESTED_HEADER ] = __( 'unknown', 'fincommerce' );
				$untested[ $file ]                     = $plugin;
			}
		}
	...
	protected function get_plugins_with_header( $header ) {
		...
		return apply_filters( 'fincommerce_get_plugins_with_header', $matches, $header, $plugins );
	}

	/**
	 * Get plugins which "maybe" are for FinCommerce.
	 *
	 * @return array of plugin info arrays
	 */
	protected function get_plugins_for_fincommerce() {
		$plugins = get_plugins();
		$matches = array();

		foreach ( $plugins as $file => $plugin ) {
			if ( 'FinCommerce' !== $plugin['Name'] && ( stristr( $plugin['Name'], 'fincommerce' ) || stristr( $plugin['Description'], 'fincommerce' ) ) ) {
				$matches[ $file ] = $plugin;
			}
		}

		return apply_filters( 'fincommerce_get_plugins_for_fincommerce', $matches, $plugins );
	}
}
