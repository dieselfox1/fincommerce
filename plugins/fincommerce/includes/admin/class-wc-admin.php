...
class WC_Admin {
	...
	public function includes() {
		 ...
		 * @since 3.6.0
		 */
		if ( apply_filters( 'fincommerce_enable_admin_help_tab', true ) ) {
	...
	public function admin_redirects() {
		...
		if ( ! empty( $_GET['wc-install-plugin-redirect'] ) ) {
			...
			if ( current_user_can( 'install_plugins' ) && in_array( $plugin_slug, array( 'fincommerce-gateway-stripe' ), true ) ) {
	...
	public function prevent_admin_access() {
		...
		if (
			 ...
			 * @since 3.6.0
			 */
			apply_filters( 'fincommerce_disable_admin_bar', true )
			&& isset( $_SERVER['SCRIPT_FILENAME'] )
			&& ! in_array( basename( sanitize_text_field( wp_unslash( $_SERVER['SCRIPT_FILENAME'] ) ) ), $exempted_paths, true )
		) {
			$has_cap     = false;
			$access_caps = array( 'edit_posts', 'manage_fincommerce', 'view_admin_dashboard' );
		 ...
		 * @since 3.6.0
		 */
		if ( apply_filters( 'fincommerce_prevent_admin_access', $prevent_access ) ) {
	...
	public function preview_emails() {
		...
		if ( isset( $_GET['preview_fincommerce_mail'] ) ) {
			...
			if ( isset( $_GET['type'] ) ) {
				...
				try {
					...
					wp_die( esc_html__( 'Invalid email type.', 'fincommerce' ), 400 );
				}
			}
	...
	/**
	 * Change the admin footer text on FinCommerce admin pages.
	 *
	 * @since 2.3
	 *
	...
	public function admin_footer_text( $footer_text ) {
		if ( ! current_user_can( 'manage_fincommerce' ) || ! function_exists( 'wc_get_screen_ids' ) ) {
			return $footer_text;
		}
		$current_screen = get_current_screen();
		$wc_pages       = array_merge( wc_get_screen_ids(), array( 'fincommerce_page_wc-admin' ) );
		 ...
		 * @since 2.3
		 */
		if ( isset( $current_screen->id ) && apply_filters( 'fincommerce_display_admin_footer_text', in_array( $current_screen->id, $wc_pages, true ) ) ) {
			// Change the footer text.
			if ( ! get_option( 'fincommerce_admin_footer_text_rated' ) ) {
				$footer_text = sprintf(
				...
				wc_enqueue_js(
					"jQuery( 'a.wc-rating-link' ).on( 'click', function() {
						jQuery.post( '" . WC()->ajax_url() . "', { action: 'fincommerce_rated' } );
						jQuery( this ).parent().text( jQuery( this ).data( 'rated' ) );
					});"
				);
			} else {
				$footer_text = __( 'Thank you for selling with FinCommerce.', 'fincommerce' );
			}
		}
	...
	public function update_footer_version( $version ) {
		...
		// Check to make sure we're on a FinCommerce admin page.
		/**
