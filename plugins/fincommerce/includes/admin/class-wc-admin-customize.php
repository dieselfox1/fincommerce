 ...
 *
 * @package FinCommerce\Admin\Customize
 * @version 3.1.0
 */
...
if ( ! class_exists( 'WC_Admin_Customize', false ) ) :
	...
	class WC_Admin_Customize {
		 ...
		 * This will register FinCommerce account endpoints as a nav menu item type.
		 *
		 * @since  3.1.0
		...
		public function register_customize_nav_menu_item_types( $item_types ) {
			$item_types[] = array(
				'title'      => __( 'FinCommerce Endpoints', 'fincommerce' ),
				'type_label' => __( 'FinCommerce Endpoint', 'fincommerce' ),
				'type'       => 'fincommerce_nav',
				'object'     => 'fincommerce_endpoint',
			);
		...
		public function register_customize_nav_menu_items( $items = array(), $type = '', $object = '', $page = 0 ) {
			if ( 'fincommerce_endpoint' !== $object ) {
				return $items;
			}
			...
			foreach ( $endpoints as $endpoint => $title ) {
				$items[] = array(
					'id'         => $endpoint,
					'title'      => $title,
					'type_label' => __( 'Custom Link', 'fincommerce' ),
