...
class WC_Admin_Duplicate_Product {
	...
	public function dupe_link( $actions, $post ) {
		...
		if ( ! current_user_can( apply_filters( 'fincommerce_duplicate_product_capability', 'manage_fincommerce' ) ) ) {
			return $actions;
		}
		...
		if ( $the_product && ProductStatus::PUBLISH === $the_product->get_status() && 0 < $the_product->get_total_sales() ) {
			$actions['trash'] = sprintf(
				...
				esc_attr( sprintf( __( 'Move &#8220;%s&#8221; to the Trash', 'fincommerce' ), $the_product->get_name() ) ),
				esc_html__( 'Trash', 'fincommerce' )
			);
		}

		$actions['duplicate'] = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=product&action=duplicate_product&amp;post=' . $post->ID ), 'fincommerce-duplicate-product_' . $post->ID ) . '" aria-label="' . esc_attr__( 'Make a duplicate from this product', 'fincommerce' )
			. '" rel="permalink">' . esc_html__( 'Duplicate', 'fincommerce' ) . '</a>';
	...
	public function dupe_button() {
		...
		$notify_url = wp_nonce_url( admin_url( 'edit.php?post_type=product&action=duplicate_product&post=' . absint( $post->ID ) ), 'fincommerce-duplicate-product_' . $post->ID );
		?>
		<div id="duplicate-action"><a class="submitduplicate duplication" href="<?php echo esc_url( $notify_url ); ?>"><?php esc_html_e( 'Copy to a new draft', 'fincommerce' ); ?></a></div>
		<?php
	}
	...
	public function duplicate_product_action() {
		if ( empty( $_REQUEST['post'] ) ) {
			wp_die( esc_html__( 'No product to duplicate has been supplied!', 'fincommerce' ) );
		}

		$product_id = isset( $_REQUEST['post'] ) ? absint( $_REQUEST['post'] ) : '';

		check_admin_referer( 'fincommerce-duplicate-product_' . $product_id );

		$product = wc_get_product( $product_id );

		if ( false === $product ) {
			/* translators: %s: product id */
			wp_die( sprintf( esc_html__( 'Product creation failed, could not find original product: %s', 'fincommerce' ), esc_html( $product_id ) ) );
		}

		$duplicate = $this->product_duplicate( $product );

		// Hook rename to match other fincommerce_product_* hooks, and to move away from depending on a response from the wp_posts table.
		do_action( 'fincommerce_product_duplicate', $duplicate, $product );
		wc_do_deprecated_action( 'fincommerce_duplicate_product', array( $duplicate->get_id(), $this->get_product_to_duplicate( $product_id ) ), '3.0', 'Use fincommerce_product_duplicate action instead.' );
	...
	public function product_duplicate( $product ) {
		...
		$meta_to_exclude = array_filter(
			apply_filters(
				'fincommerce_duplicate_product_exclude_meta',
				array(),
				array_map(
					function ( $datum ) {
						return $datum->key;
		...
		$duplicate->set_name( sprintf( esc_html__( '%s (Copy)', 'fincommerce' ), $duplicate->get_name() ) );
		 ...
		 * @since 3.0
		 */
		do_action( 'fincommerce_product_duplicate_before_save', $duplicate, $product );
		 ...
		 * @since 2.7
		 */
		if ( ! apply_filters( 'fincommerce_duplicate_product_exclude_children', false, $product ) && $product->is_type( ProductType::VARIABLE ) ) {
			foreach ( $product->get_children() as $child_id ) {
				 ...
				 * @since 3.0
				 */
				do_action( 'fincommerce_product_duplicate_before_save', $child_duplicate, $child );
