...
use Automattic\FinCommerce\Admin\RemoteInboxNotifications as PromotionRuleEngine;
use Automattic\FinCommerce\Admin\RemoteSpecs\RuleProcessors\RuleEvaluator;
...
class WC_Admin_Addons {
	...
	public static function fetch_featured() {
		...
		$url      = 'https://fincommerce.com/wp-json/wccom-extensions/4.0/featured';
	...
	public static function fetch_product_preview( int $product_id ) {
		$url = 'https://fincommerce.com/wp-json/wccom-extensions/1.0/product-previews?product_id=' . $product_id;
	...
	public static function get_sections() {
		...
		if ( false === ( $addon_sections ) ) {
			...
			$raw_sections     = wp_safe_remote_get(
				'https://fincommerce.com/wp-json/wccom-extensions/1.0/categories' . $parameter_string,
				array(
		...
		return apply_filters( 'fincommerce_addons_sections', $addon_sections );
	}
	 ...
	 * flow for obtaining and setting up FinCommerce extensons.
	 *
	...
	public static function output_button( $url, $text, $style, $plugin = '' ) {
		$style = __( 'Free', 'fincommerce' ) === $text ? 'addons-button-outline-purple' : $style;
		$style = is_plugin_active( $plugin ) ? 'addons-button-installed' : $style;
		$text  = is_plugin_active( $plugin ) ? __( 'Installed', 'fincommerce' ) : $text;
	...
	public static function handle_legacy_marketplace_redirects() {
		...
		if ( 'helper' === $section ) {
			...
			if ( isset( $_GET['connect'] ) ) { // phpcs:ignore finpress.Security.NonceVerification.Recommended
				$url .= '&connect';
			}
	...
	public static function filter_admin_body_classes( string $admin_body_class = '' ): string {
		if ( isset( $_GET['section'] ) && 'helper' === $_GET['section'] ) {
			return " $admin_body_class fincommerce-page-wc-subscriptions ";
		}

		return " $admin_body_class fincommerce-page-wc-marketplace ";
	}
	...
	/**
	 * Process API response from FinCommerce.com endpoints.
	 *
	...
	private static function process_api_response( $response, $context = 'api', $associative = false ) {
		if ( is_wp_error( $response ) ) {
			/**
			 * Hook fired when there is a connection error with FinCommerce.com.
			 *
			 * @since 6.1.0
			 * @param string $error_message The error message.
			 */
			do_action( 'fincommerce_page_wc_addons_connection_error', $response->get_error_message() );
		...
		if ( 200 !== $response_code ) {
			 ...
			 */
			do_action( 'fincommerce_page_wc_addons_connection_error', $response_code );

			$message = sprintf(
				/* translators: 1: Context (e.g. 'featured', 'product-preview') 2: HTTP error code */
				__( 'Our request to the %1$s API got error code %2$d.', 'fincommerce' ),
				$context,
				$response_code
			);
		...
		if ( empty( $data ) || ! is_array( $data ) ) {
			 ...
			 * @param string $error_message The error message.
			 */
			do_action( 'fincommerce_page_wc_addons_connection_error', 'Empty or malformed response' );

			$message = sprintf(
				/* translators: %s: Context (e.g. 'featured', 'product-preview') */
				__( 'Our request to the %s API got a malformed response.', 'fincommerce' ),
				$context
			);
	...
	/**
	 * Make wp_safe_remote_get request to FinCommerce.com endpoint.
