...
class WC_Admin_Log_Table_List extends WP_List_Table {
	 ...
	 * @const string
	 */
	public const PER_PAGE_USER_OPTION_KEY = 'fincommerce_status_log_items_per_page';
	 ...
	 * @const string
	 */
	public const SOURCE_CACHE_OPTION_KEY = 'fincommerce_status_log_db_sources';
	...
	public function level_dropdown() {
		...
		?>
			<label for="filter-by-level" class="screen-reader-text"><?php esc_html_e( 'Filter by level', 'fincommerce' ); ?></label>
			<select name="level" id="filter-by-level">
				<option<?php selected( $selected_level, '' ); ?> value=""><?php esc_html_e( 'All levels', 'fincommerce' ); ?></option>
				<?php
				foreach ( $levels as $l ) {
					printf(
	...
	protected function context_row( $log ) {
		...
		<tr id="log-context-<?php echo esc_attr( $log['log_id'] ); ?>" class="log-context">
			<td colspan="<?php echo esc_attr( $this->get_column_count() ); ?>">
				<p><strong><?php esc_html_e( 'Additional context', 'fincommerce' ); ?></strong></p>
	...
	public function get_columns() {
		return array(
			...
			'level'     => __( 'Level', 'fincommerce' ),
			'message'   => __( 'Message', 'fincommerce' ),
			'source'    => __( 'Source', 'fincommerce' ),
			'context'   => __( 'Context', 'fincommerce' ),
		);
	}
		...
		if ( ! empty( $log['context'] ) ) {
			...
			?>
				<button
					...
					data-label-show="<?php esc_attr_e( 'Show context', 'fincommerce' ); ?>"
					data-label-hide="<?php esc_attr_e( 'Hide context', 'fincommerce' ); ?>"
				>
					<span class="dashicons dashicons-arrow-down-alt2"></span>
					<span class="log-toggle-label screen-reader-text"><?php esc_html_e( 'Show context', 'fincommerce' ); ?></span>
				</button>
			<?php
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' === $which ) {
			echo '<div class="alignleft actions">';
				...
				submit_button( __( 'Filter', 'fincommerce' ), '', 'filter-action', false );
			echo '</div>';
		}
	}
	...
	protected function source_dropdown() {
		...
		if ( ! empty( $sources ) ) {
			...
			?>
				<label for="filter-by-source" class="screen-reader-text"><?php esc_html_e( 'Filter by source', 'fincommerce' ); ?></label>
				<select name="source" id="filter-by-source">
					<option<?php selected( $selected_source, '' ); ?> value=""><?php esc_html_e( 'All sources', 'fincommerce' ); ?></option>
					<?php
	...
	protected function get_sources() {
		...
		$sql = "
			...
			FROM {$wpdb->prefix}fincommerce_log
			WHERE source != ''
			ORDER BY source ASC
		";
