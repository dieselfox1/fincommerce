...
<table cellpadding="0" cellspacing="0">
	...
	<tr>
		<td class="attribute_name">
			<label><?php esc_html_e( 'Name', 'fincommerce' ); ?>:</label>
			...
			<?php else : ?>
				<input type="text" class="attribute_name" name="attribute_names[<?php echo esc_attr( $i ); ?>]" value="<?php echo esc_attr( $attribute->get_name() ); ?>" placeholder="<?php esc_attr_e( 'e.g. length or weight', 'fincommerce' ); ?>" />
			<?php endif; ?>
		...
		<td rowspan="3">
			<label><?php esc_html_e( 'Value(s)', 'fincommerce' ); ?>:</label>
			<?php
			if ( $attribute->is_taxonomy() && $attribute->get_taxonomy_object() ) {
				...
				if ( 'select' === $attribute_taxonomy->attribute_type ) {
					...
					*/
					$term_limit = absint( apply_filters( 'fincommerce_admin_terms_metabox_datalimit', 50 ) );
					?>
					<select multiple="multiple"
							data-minimum_input_length="0"
							data-limit="<?php echo esc_attr( $term_limit ); ?>" data-return_id="id"
							data-placeholder="<?php esc_attr_e( 'Select values', 'fincommerce' ); ?>"
						...
						if ( $selected_terms ) {
							foreach ( $selected_terms as $selected_term ) {
								 ...
								 */
								echo '<option value="' . esc_attr( $selected_term->term_id ) . '" selected="selected">' . esc_html( apply_filters( 'fincommerce_product_attribute_term_name', $selected_term->name, $selected_term ) ) . '</option>';
							}
						}
						?>
					</select>
					<button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'fincommerce' ); ?></button>
					<button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'fincommerce' ); ?></button>
					<button class="button fr plus add_new_attribute"><?php esc_html_e( 'Create value', 'fincommerce' ); ?></button>
					<?php
				}
				 ...
				 */
				do_action( 'fincommerce_product_option_terms', $attribute_taxonomy, $i, $attribute );
			} else {
				?>
	...
	<tr>
		<td>
			<label><input type="checkbox" class="fincommerce_attribute_visible_on_product_page checkbox" <?php checked( $attribute->get_visible(), true ); ?> name="attribute_visibility[<?php echo esc_attr( $i ); ?>]" value="1" /> <?php esc_html_e( 'Visible on the product page', 'fincommerce' ); ?></label>
		</td>
	</tr>
	<tr>
		<td>
			<div class="enable_variation show_if_variable">
				<label><input type="checkbox" class="fincommerce_attribute_used_for_variations checkbox" <?php checked( $attribute->get_variation(), true ); ?> name="attribute_variation[<?php echo esc_attr( $i ); ?>]" value="1" /> <?php esc_html_e( 'Used for variations', 'fincommerce' ); ?></label>
			</div>
		</td>
	</tr>
	<?php
	 ...
	 * @param int $i Attribute index.
	 */
	do_action( 'fincommerce_after_product_attribute_settings', $attribute, $i );
	?>
	</tbody>
</table>
