...
?>
<div id="inventory_product_data" class="panel fincommerce_options_panel hidden">
	<div class="options_group">
		...
		do_action( 'fincommerce_product_options_sku' );
		...
		do_action( 'fincommerce_product_options_global_unique_id' );

		?>
		<div class="inline notice fincommerce-message show_if_variable">
			<img class="info-icon" src="<?php echo esc_url( $info_img_url ); ?>" />
			<p>
				<?php echo esc_html_e( 'Settings below apply to all variations without manual stock management enabled. ', 'fincommerce' ); ?> <a target="_blank" href="https://fincommerce.com/document/variable-product/"><?php esc_html_e( 'Learn more', 'fincommerce' ); ?></a>
			</p>
		</div>
		<?php
		...
		if ( 'yes' === get_option( 'fincommerce_manage_stock' ) ) {
			...
			fincommerce_wp_checkbox(
				array(
					...
					'label'         => __( 'Stock management', 'fincommerce' ),
					'description'   => __( 'Track stock quantity for this product', 'fincommerce' ),
				)
			);

			do_action( 'fincommerce_product_options_stock' );
			...
			fincommerce_wp_text_input(
				array(
					...
					'label'             => __( 'Quantity', 'fincommerce' ),
					'desc_tip'          => true,
					'description'       => __( 'Stock quantity. If this is a variable product this value will be used to control stock for all variations, unless you define stock at variation level.', 'fincommerce' ),
					'type'              => 'number',
			 ...
			 */
			if ( apply_filters( 'fincommerce_product_allow_backorder_use_radio', true ) ) {
				fincommerce_wp_radio( $backorder_args );
			} else {
				fincommerce_wp_select( $backorder_args );
			}
			...
			fincommerce_wp_text_input(
				array(
					...
					'desc_tip'          => true,
					'description'       => __( 'When product stock reaches this amount you will be notified by email. It is possible to define different values for each variation individually. The shop default value can be set in Settings > Products > Inventory.', 'fincommerce' ),
					'type'              => 'number',
					'custom_attributes' => array(
						'step' => 'any',
					),
			...
			do_action( 'fincommerce_product_options_stock_fields' );
			...
			fincommerce_wp_note(
				array(
					...
					'label'            => __( 'Stock management', 'fincommerce' ),
					'label-aria-label' => __( 'Stock management disabled in store settings', 'fincommerce' ),
					'message'          => sprintf(
						/* translators: %s: url for store settings */
						__( 'Disabled in <a href="%s" aria-label="stock management store settings">store settings</a>.', 'fincommerce' ),
		 ...
		 */
		if ( apply_filters( 'fincommerce_product_stock_status_use_radio', $stock_status_count <= 3 && $stock_status_count >= 1 ) ) {
			fincommerce_wp_radio( $stock_status_args );
		} else {
			fincommerce_wp_select( $stock_status_args );
		}

		do_action( 'fincommerce_product_options_stock_status' );
		?>
	</div>
	...
	<div class="inventory_sold_individually options_group show_if_simple show_if_variable">
		...
		fincommerce_wp_checkbox(
			array(
				...
				'wrapper_class' => 'show_if_simple show_if_variable',
				'label'         => __( 'Sold individually', 'fincommerce' ),
				'description'   => __( 'Limit purchases to 1 item per order', 'fincommerce' ),
			)
		);

		echo wc_help_tip( __( 'Check to let customers to purchase only 1 item in a single order. This is particularly useful for items that have limited quantity, for example art or handmade goods.', 'fincommerce' ) );

		do_action( 'fincommerce_product_options_sold_individually' );
		?>
	</div>

	<?php do_action( 'fincommerce_product_options_inventory_product_data' ); ?>
</div>
