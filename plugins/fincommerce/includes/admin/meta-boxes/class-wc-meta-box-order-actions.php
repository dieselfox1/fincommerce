...
use Automattic\FinCommerce\Internal\Admin\Orders\PageController;
...
class WC_Meta_Box_Order_Actions {
	...
	public static function output( $post ) {
		...
		<ul class="order_actions submitbox">
			 ...
			 * @since 2.1.0
			 */
			do_action( 'fincommerce_order_actions_start', $order_id );
			?>
			...
			<li class="wide">
				<div id="delete-action">
					...
					if ( current_user_can( 'delete_post', $order_id ) ) {
						...
						if ( ! EMPTY_TRASH_DAYS ) {
							$delete_text = __( 'Delete permanently', 'fincommerce' );
						} else {
							$delete_text = __( 'Move to Trash', 'fincommerce' );
						}
						?>
				...
				<button type="submit" class="button save_order button-primary" name="save" value="<?php echo OrderStatus::AUTO_DRAFT === $order->get_status() ? esc_attr__( 'Create', 'fincommerce' ) : esc_attr__( 'Update', 'fincommerce' ); ?>"><?php echo OrderStatus::AUTO_DRAFT === $order->get_status() ? esc_html__( 'Create', 'fincommerce' ) : esc_html__( 'Update', 'fincommerce' ); ?></button>
			</li>
			 ...
			 * @since 2.1.0
			 */
			do_action( 'fincommerce_order_actions_end', $order_id );
			?>
	...
	public static function save( $post_id, $post ) {
		...
		if ( ! empty( $_POST['wc_order_action'] ) ) { // @codingStandardsIgnoreLine
			...
			if ( 'send_order_details' === $action ) {
				...
				// Note the event.
				$order->add_order_note( __( 'Order details manually sent to customer.', 'fincommerce' ), false, true );
				...
				do_action( 'fincommerce_before_resend_order_emails', $order, 'new_order' );

				WC()->payment_gateways();
				WC()->shipping();
				add_filter( 'fincommerce_new_order_email_allows_resend', '__return_true' );
				WC()->mailer()->emails['WC_Email_New_Order']->trigger( $order->get_id(), $order, true );
				remove_filter( 'fincommerce_new_order_email_allows_resend', '__return_true' );

				do_action( 'fincommerce_after_resend_order_email', $order, 'new_order' );
				...
				if ( ! did_action( 'fincommerce_order_action_' . sanitize_title( $action ) ) ) {
					do_action( 'fincommerce_order_action_' . sanitize_title( $action ), $order );
				}
			}
		}
	}
	...
	private static function get_available_order_actions_for_order( $order ) {
		$actions = array(
			'send_order_details'              => __( 'Send order details to customer', 'fincommerce' ),
			'send_order_details_admin'        => __( 'Resend new order notification', 'fincommerce' ),
			'regenerate_download_permissions' => __( 'Regenerate download permissions', 'fincommerce' ),
		);

		/**
		 * Filter: fincommerce_order_actions
		 ...
		 * @param WC_Order|null $order   The order object or null if no order is available.
		 */
		return apply_filters( 'fincommerce_order_actions', $actions, $order );
	}
}
