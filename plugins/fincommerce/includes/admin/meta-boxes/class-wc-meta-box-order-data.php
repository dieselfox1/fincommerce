...
class WC_Meta_Box_Order_Data {
	...
	protected static function get_billing_fields( $order = false, $context = 'edit' ) {
		...
		return apply_filters(
			'fincommerce_admin_billing_fields',
			array(
				'first_name' => array(
				...
				'address_1'  => array(
					'label' => __( 'Address line 1', 'fincommerce' ),
					'show'  => false,
				),
				'address_2'  => array(
				...
				'country'    => array(
					...
					'type'    => 'select',
					'options' => array( '' => __( 'Select a country / region&hellip;', 'fincommerce' ) ) + WC()->countries->get_countries(),
				),
				'state'      => array(
	...
	protected static function get_shipping_fields( $order = false, $context = 'edit' ) {
		 ...
		 */
		return apply_filters(
			'fincommerce_admin_shipping_fields',
			array(
				'first_name' => array(
					'label' => __( 'First name', 'fincommerce' ),
					'show'  => false,
	...
	public static function output( $post ) {
		...
		<div class="panel-wrap fincommerce">
			<input name="post_title" type="hidden" value="<?php echo esc_attr( empty( $order->get_title() ) ? __( 'Order', 'fincommerce' ) : $order->get_title() ); ?>" />
			<input name="post_status" type="hidden" value="<?php echo esc_attr( $order->get_status() ); ?>" />
			<div id="order_data" class="panel fincommerce-order-data">
				<div class="order_data_header">
					<div class="order_data_header_column">
						<h2 class="fincommerce-order-data__heading">
							<?php

							printf(
								/* translators: 1: order type 2: order number */
								esc_html__( '%1$s #%2$s details', 'fincommerce' ),
						...
						<p class="fincommerce-order-data__meta order_number">
							<?php
							...
							if ( $order->get_date_paid() ) {
								$meta_list[] = sprintf(
									...
									__( 'Paid on %1$s @ %2$s', 'fincommerce' ),
							...
							if ( $ip_address ) {
								$meta_list[] = sprintf(
									...
									__( 'Customer IP: %s', 'fincommerce' ),
									'<span class="fincommerce-Order-customerIP">' . esc_html( $ip_address ) . '</span>'
								);
							}
					...
					<div class="order_data_header_column">
						<?php
							 ...
							 * @since 9.9.0
							 */
							do_action( 'fincommerce_admin_order_data_header_right', $order );
						?>
					</div>
				</div>
				<?php
					 ...
					 * @since 7.9.0
					 */
					do_action( 'fincommerce_admin_order_data_after_payment_info', $order );
				?>
				<div class="order_data_column_container">
					<div class="order_data_column">
						<h3><?php esc_html_e( 'General', 'fincommerce' ); ?></h3>

						<p class="form-field form-field-wide">
							<?php
							$order_date_created_localised = ! is_null( $order->get_date_created() ) ? $order->get_date_created()->getOffsetTimestamp() : '';
							?>
							<label for="order_date"><?php esc_html_e( 'Date created:', 'fincommerce' ); ?></label>
							<input type="text" class="date-picker" name="order_date" maxlength="10" value="<?php echo esc_attr( date_i18n( 'Y-m-d', $order_date_created_localised ) ); ?>" pattern="<?php echo esc_attr( apply_filters( 'fincommerce_date_input_html_pattern', '[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])' ) ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment ?>" />@
							&lrm;
							<input type="number" class="hour" placeholder="<?php esc_attr_e( 'h', 'fincommerce' ); ?>" name="order_date_hour" min="0" max="23" step="1" value="<?php echo esc_attr( date_i18n( 'H', $order_date_created_localised ) ); ?>" pattern="([01]?[0-9]{1}|2[0-3]{1})" />:
							<input type="number" class="minute" placeholder="<?php esc_attr_e( 'm', 'fincommerce' ); ?>" name="order_date_minute" min="0" max="59" step="1" value="<?php echo esc_attr( date_i18n( 'i', $order_date_created_localised ) ); ?>" pattern="[0-5]{1}[0-9]{1}" />
						...
						<p class="form-field form-field-wide wc-order-status">
							<label for="order_status">
								<?php
								esc_html_e( 'Status:', 'fincommerce' );
								if ( $order->needs_payment() ) {
									printf(
										'<a href="%s">%s</a>',
										esc_url( $order->get_checkout_payment_url() ),
										esc_html__( 'Customer payment page &rarr;', 'fincommerce' )
									);
								}
								?>
							</label>
						...
						<p class="form-field form-field-wide wc-customer-user">
							...
							<label for="customer_user">
								<?php
								esc_html_e( 'Customer:', 'fincommerce' );
								if ( $order->get_user_id( 'edit' ) ) {
									...
									printf(
										...
										' ' . esc_html__( 'View other orders &rarr;', 'fincommerce' )
									);
									printf(
										'<a href="%s">%s</a>',
										esc_url( add_query_arg( 'user_id', $order->get_user_id( 'edit' ), admin_url( 'user-edit.php' ) ) ),
										' ' . esc_html__( 'Profile &rarr;', 'fincommerce' )
									);
								}
								?>
							</label>
							...
							if ( $order->get_user_id() ) {
								...
								if ( ! is_wp_error( $user ) ) {
									...
									$user_string = esc_html__( '(Not available)', 'fincommerce' );
								}
							}
							?>
							<select class="wc-customer-search" id="customer_user" name="customer_user" data-placeholder="<?php esc_attr_e( 'Guest', 'fincommerce' ); ?>" data-allow_clear="true">
								<?php
								 ...
								 */
								?>
								<option value="<?php echo esc_attr( $user_id ); ?>" selected="selected"><?php echo esc_html( htmlspecialchars( wp_kses_post( current( apply_filters( 'fincommerce_json_search_found_customers', array( $user_string ) ) ) ) ) ); ?></option>
								<?php // phpcs:enable FinCommerce.Commenting.CommentHooks.MissingHookComment ?>
							</select>
							<!--/email_off-->
						</p>
						<?php do_action( 'fincommerce_admin_order_data_after_order_details', $order ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment ?>
					</div>
					<div class="order_data_column">
						<h3>
							<?php esc_html_e( 'Billing', 'fincommerce' ); ?>
							<a href="#" class="edit_address"><?php esc_html_e( 'Edit', 'fincommerce' ); ?></a>
							<span>
								<a href="#" class="load_customer_billing" style="display:none;"><?php esc_html_e( 'Load billing address', 'fincommerce' ); ?></a>
							</span>
						</h3>
						<div class="address">
							<?php
							...
							$details_not_available_message = __( 'Details are not available for this customer as this user does not exist in the current site.', 'fincommerce' );
							...
							if ( $order->get_user_id() !== 0 && is_wp_error( $user ) ) {
								...
								if ( $order->get_formatted_billing_address() ) {
									...
									echo '<p class="none_set"><strong>' . esc_html__( 'Address:', 'fincommerce' ) . '</strong> ' . esc_html__( 'No billing address set.', 'fincommerce' ) . '</p>';
								}
						...
						<div class="edit_address">
							...
							foreach ( $billing_fields as $key => $field ) {
								...
								switch ( $field['type'] ) {
									case 'select':
										fincommerce_wp_select( $field, $order );
										break;
									case 'checkbox':
										fincommerce_wp_checkbox( $field, $order );
										break;
									default:
										fincommerce_wp_text_input( $field, $order );
										break;
								}
							}
							?>
							<p class="form-field form-field-wide">
								<label><?php esc_html_e( 'Payment method:', 'fincommerce' ); ?></label>
								<select name="_payment_method" id="_payment_method" class="first">
									<option value=""><?php esc_html_e( 'N/A', 'fincommerce' ); ?></option>
									<?php
									$found_method = false;
									...
									if ( ! $found_method && ! empty( $payment_method ) ) {
										echo '<option value="' . esc_attr( $payment_method ) . '" selected="selected">' . esc_html__( 'Other', 'fincommerce' ) . '</option>';
									} else {
										echo '<option value="other">' . esc_html__( 'Other', 'fincommerce' ) . '</option>';
									}
									?>
								</select>
							</p>
							...
							fincommerce_wp_text_input(
								array(
									'id'    => '_transaction_id',
									'label' => __( 'Transaction ID', 'fincommerce' ),
						...
						<?php do_action( 'fincommerce_admin_order_data_after_billing_address', $order ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment ?>
					</div>
					<div class="order_data_column">
						<h3>
							<?php esc_html_e( 'Shipping', 'fincommerce' ); ?>
							<a href="#" class="edit_address"><?php esc_html_e( 'Edit', 'fincommerce' ); ?></a>
							<span>
								<a href="#" class="load_customer_shipping" style="display:none;"><?php esc_html_e( 'Load shipping address', 'fincommerce' ); ?></a>
								<a href="#" class="billing-same-as-shipping" style="display:none;"><?php esc_html_e( 'Copy billing address', 'fincommerce' ); ?></a>
							</span>
						</h3>
						<div class="address">
							<?php
							...
							if ( $order->get_user_id() !== 0 && is_wp_error( $user ) ) {
								...
								if ( $order->get_formatted_shipping_address() ) {
									...
									echo '<p class="none_set"><strong>' . esc_html__( 'Address:', 'fincommerce' ) . '</strong> ' . esc_html__( 'No shipping address set.', 'fincommerce' ) . '</p>';
								}
								...
								if ( apply_filters( 'fincommerce_enable_order_notes_field', 'yes' === get_option( 'fincommerce_enable_order_comments', 'yes' ) ) && $order->get_customer_note() ) { // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
									echo '<p class="order_note"><strong>' . esc_html( __( 'Customer provided note:', 'fincommerce' ) ) . '</strong> ' . wp_kses( nl2br( esc_html( wc_wptexturize_order_note( $order->get_customer_note() ) ) ), array( 'br' => array() ) ) . '</p>';
								}
							}
							?>
						</div>
						<div class="edit_address">
							 ...
							 */
							if ( apply_filters( 'fincommerce_enable_order_notes_field', 'yes' === get_option( 'fincommerce_enable_order_comments', 'yes' ) ) ) :
								?>
								<p class="form-field form-field-wide">
									<label for="customer_note"><?php esc_html_e( 'Customer provided note', 'fincommerce' ); ?>:</label>
									<textarea rows="1" cols="40" name="customer_note" tabindex="6" id="excerpt" placeholder="<?php esc_attr_e( 'Customer notes about the order', 'fincommerce' ); ?>"><?php echo wp_kses( $order->get_customer_note(), array( 'br' => array() ) ); ?></textarea>
								</p>
							<?php endif; ?>
						</div>

						<?php do_action( 'fincommerce_admin_order_data_after_shipping_address', $order ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment ?>
					</div>
				</div>
				<div class="clear"></div>
			</div>
	...
	public static function save( $order_id ) {
		...
		if ( ! isset( $_POST['_payment_method'] ) ) {
			throw new Exception( __( 'Payment method is missing.', 'fincommerce' ), 400 );
		}
		...
		if ( $order->get_payment_method() !== wc_clean( wp_unslash( $_POST['_payment_method'] ) ) ) {
			...
			if ( 'other' === $payment_method ) {
				$payment_method_title = esc_html__( 'Other', 'fincommerce' );
			}
