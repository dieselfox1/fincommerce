...
class WC_Meta_Box_Order_Downloads {
	...
	public static function output( $post ) {
		...
		<div class="order_download_permissions wc-metaboxes-wrapper">
			...
			<div class="wc-metaboxes">
				...
				if ( $download_permissions && count( $download_permissions ) > 0 ) {
					foreach ( $download_permissions as $download ) {
						...
						$file_count = isset( $file['name'] ) ? $file['name'] : sprintf( __( 'File %d', 'fincommerce' ), $file_counter );
			...
			<div class="toolbar">
				<p class="buttons">
					<select id="grant_access_id" class="wc-product-search" name="grant_access_id[]" multiple="multiple" style="width: 400px;" data-placeholder="<?php esc_attr_e( 'Search for a downloadable product&hellip;', 'fincommerce' ); ?>" data-action="fincommerce_json_search_downloadable_products_and_variations"></select>
					<button type="button" class="button grant_access">
						<?php esc_html_e( 'Grant access', 'fincommerce' ); ?>
					</button>
				</p>
				<div class="clear"></div>
			</div>
