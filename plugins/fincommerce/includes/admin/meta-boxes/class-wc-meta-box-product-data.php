...
class WC_Meta_Box_Product_Data {
	...
	private static function get_product_data_tabs() {
		...
		$tabs = apply_filters(
			...
			array(
				...
				'inventory'      => array(
					'label'    => __( 'Inventory', 'fincommerce' ),
				...
				'linked_product' => array(
					'label'    => __( 'Linked Products', 'fincommerce' ),
					'target'   => 'linked_product_data',
					'class'    => array(),
					'priority' => 40,
				),
				'attribute'      => array(
					'label'    => __( 'Attributes', 'fincommerce' ),
	...
	public static function output_variations() {
		...
		$variations_count       = absint( apply_filters( 'fincommerce_admin_meta_boxes_variations_count', $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(ID) FROM $wpdb->posts WHERE post_parent = %d AND post_type = 'product_variation' AND post_status IN ('publish', 'private')", $post->ID ) ), $post->ID ) );
		$variations_per_page    = absint( apply_filters( 'fincommerce_admin_meta_boxes_variations_per_page', 15 ) );
		$variations_total_pages = ceil( $variations_count / $variations_per_page );
		$modal_title            = get_bloginfo( 'name' ) . __( ' says', 'fincommerce' );
		/* phpcs: enable */
	...
	public static function prepare_attributes( $data = false ) {
		...
		if ( isset( $data['attribute_names'], $data['attribute_values'] ) ) {
			...
			for ( $i = 0; $i <= $attribute_names_max_key; $i++ ) {
				...
				/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
				$attributes[] = apply_filters( 'fincommerce_admin_meta_boxes_prepare_attribute', $attribute, $data, $i );
				/* phpcs: enable */
			}
		}
		return $attributes;
	...
	public static function save( $post_id, $post ) {
		...
		if ( isset( $_POST['_stock'] ) ) {
			if ( isset( $_POST['_original_stock'] ) && wc_stock_amount( $product->get_stock_quantity( 'edit' ) ) !== wc_stock_amount( wp_unslash( $_POST['_original_stock'] ) ) ) {
				...
				WC_Admin_Meta_Boxes::add_error( sprintf( __( 'The stock has not been updated because the value has changed since editing. Product %1$d has %2$d units in stock.', 'fincommerce' ), $product->get_id(), $product->get_stock_quantity( 'edit' ) ) );
			} else {
		 ...
		 * @since 3.0.0
		 */
		do_action( 'fincommerce_admin_process_product_object', $product );
			...
			for ( $i = 0; $i <= $max_loop; $i++ ) {
				...
				if ( isset( $_POST['variable_stock'], $_POST['variable_stock'][ $i ] ) ) {
					if ( isset( $_POST['variable_original_stock'], $_POST['variable_original_stock'][ $i ] ) && wc_stock_amount( $variation->get_stock_quantity( 'edit' ) ) !== wc_stock_amount( wp_unslash( $_POST['variable_original_stock'][ $i ] ) ) ) {
						/* translators: 1: product ID 2: quantity in stock */
						WC_Admin_Meta_Boxes::add_error( sprintf( __( 'The stock has not been updated because the value has changed since editing. Product %1$d has %2$d units in stock.', 'fincommerce' ), $variation->get_id(), $variation->get_stock_quantity( 'edit' ) ) );
					} else {
				 ...
				 * @since 3.8.0
				 */
				do_action( 'fincommerce_admin_process_variation_object', $variation, $i );

				$variation->save();
				/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
				do_action( 'fincommerce_save_product_variation', $variation_id, $i );
				/* phpcs: enable */
			}
		}
