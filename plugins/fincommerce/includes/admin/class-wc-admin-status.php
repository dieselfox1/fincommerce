 ...
 * Debug/Status page
 *
 * @package FinCommerce\Admin\System Status
 * @version 2.2.0
 */
...
class WC_Admin_Status {
	...
	public static function status_tools() {
		...
		if ( ! empty( $_GET['action'] ) && ! empty( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( wp_unslash( $_REQUEST['_wpnonce'] ), 'debug_action' ) ) { // WPCS: input var ok, sanitization ok.
			...
			if ( array_key_exists( $action, $tools ) ) {
				...
				/**
				 * Fires after a FinCommerce system status tool has been executed.
				 *
				 * @param array  $tool  Details about the tool that has been executed.
				 */
				do_action( 'fincommerce_system_status_tool_executed', $tool );
			} else {
				$response = array(
					'success' => false,
					'message' => __( 'Tool does not exist.', 'fincommerce' ),
				);
			}
		...
		if ( isset( $_REQUEST['settings-updated'] ) ) { // WPCS: input var ok.
			echo '<div class="updated inline"><p>' . esc_html__( 'Your changes have been saved.', 'fincommerce' ) . '</p></div>';
		}
	...
	private static function flush_db_logs() {
		...
		if ( ! current_user_can( 'manage_fincommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to manage log entries.', 'fincommerce' ) );
		}
			...
			<?php
				echo esc_html(
					...
					// translators: Comma separated list of missing tables.
						__( 'Missing base tables: %s. Some FinCommerce functionality may not work as expected.', 'fincommerce' ),
	 ...
	 * @param array $untested_plugins List of plugins that haven't been tested with the current FinCommerce version.
	 * @return void
	 */
	private static function output_plugins_info( $plugins, $untested_plugins ) {
		...
		foreach ( $plugins as $plugin ) {
			if ( ! empty( $plugin['name'] ) ) {
				...
				if ( ! empty( $plugin['url'] ) ) {
					$plugin_name = '<a href="' . esc_url( $plugin['url'] ) . '" aria-label="' . esc_attr__( 'Visit plugin homepage', 'fincommerce' ) . '" target="_blank">' . $plugin_name . '</a>';
				}

				$has_newer_version = false;
				$version_string    = $plugin['version'];
				$network_string    = '';
				if ( strstr( $plugin['url'], 'woothemes.com' ) || strstr( $plugin['url'], 'fincommerce.com' ) || strstr( $plugin['url'], 'woo.com' ) ) {
					if ( ! empty( $plugin['version_latest'] ) && version_compare( $plugin['version_latest'], $plugin['version'], '>' ) ) {
						/* translators: 1: current version. 2: latest version */
						$version_string = sprintf( __( '%1$s (update to version %2$s is available)', 'fincommerce' ), $plugin['version'], $plugin['version_latest'] );
					}

					if ( false !== $plugin['network_activated'] ) {
						$network_string = ' &ndash; <strong style="color: black;">' . esc_html__( 'Network enabled', 'fincommerce' ) . '</strong>';
					}
				}
				$untested_string = '';
				if ( array_key_exists( $plugin['plugin'], $untested_plugins ) ) {
					...
					$untested_string .= esc_html( sprintf( __( 'Installed version not tested with active version of FinCommerce %s', 'fincommerce' ), $wc_version ) );
