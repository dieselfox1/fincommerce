...
class WC_Admin_Notices {
	...
	public static function init() {
		...
		self::set_notices( get_option( 'fincommerce_admin_notices', array() ) );

		add_action( 'switch_theme', array( __CLASS__, 'reset_admin_notices' ) );
		add_action( 'fincommerce_installed', array( __CLASS__, 'reset_admin_notices' ) );
	...
	public static function store_notices() {
		update_option( 'fincommerce_admin_notices', self::get_notices() );
	}
	...
	public static function get_notices() {
		...
		self::$notices[ $blog_id ] = get_option( 'fincommerce_admin_notices', array() );
		...
		if ( wc_get_container()->get( WebhookUtil::class )->get_legacy_webhooks_count() > 0 && ! WC()->legacy_rest_api_is_available() ) {
			self::add_custom_notice(
				...
				sprintf(
					...
					sprintf(
						'<h4>%s</h4>',
						esc_html__( 'FinCommerce webhooks that use the Legacy REST API are unsupported', 'fincommerce' )
					),
					sprintf(
	...
	public static function remove_notice( $name, $force_save = false ) {
		...
		delete_option( 'fincommerce_admin_notice_' . $name );
	...
	public static function hide_notices() {
		if ( isset( $_GET['wc-hide-notice'] ) && isset( $_GET['_wc_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( sanitize_key( wp_unslash( $_GET['_wc_notice_nonce'] ) ), 'fincommerce_hide_notices_nonce' ) ) {
			 ...
			 */
			$required_capability = apply_filters( 'fincommerce_dismiss_admin_notice_capability', 'manage_fincommerce', $notice_name );
	...
	private static function hide_notice( $name ) {
		...
		do_action( 'fincommerce_hide_' . $name . '_notice' );
	}
	...
	public static function add_notices() {
		...
		// Notices should only show on FinCommerce screens, the main dashboard, and on the plugins screen.
		if ( ! in_array( $screen_id, wc_get_screen_ids(), true ) && ! in_array( $screen_id, $show_on_screens, true ) ) {
			return;
		}

		wp_enqueue_style( 'fincommerce-activation', plugins_url( '/assets/css/activation.css', WC_PLUGIN_FILE ), array(), Constants::get_constant( 'WC_VERSION' ) );

		// Add RTL support.
		wp_style_add_data( 'fincommerce-activation', 'rtl', 'replace' );

		foreach ( $notices as $notice ) {
			if ( ! empty( self::$core_notices[ $notice ] ) && apply_filters( 'fincommerce_show_admin_notice', true, $notice ) ) {
	...
	public static function add_custom_notice( $name, $notice_html ) {
		...
		update_option( 'fincommerce_admin_notice_' . $name, wp_kses_post( $notice_html ) );
	}
		...
		if ( ! empty( $notices ) ) {
			foreach ( $notices as $notice ) {
				if ( empty( self::$core_notices[ $notice ] ) ) {
					$notice_html = get_option( 'fincommerce_admin_notice_' . $notice );
	...
	public static function install_notice() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', esc_html__( 'Onboarding is maintained in FinCommerce Admin.', 'fincommerce' ) );
	}
	...
	public static function legacy_shipping_notice() {
		...
		foreach ( $maybe_load_legacy_methods as $method ) {
			$options = get_option( 'fincommerce_' . $method . '_settings' );
	...
	public static function add_maxmind_missing_license_key_notice() {
		$default_address = get_option( 'fincommerce_default_customer_address' );

		if ( ! in_array( $default_address, array( 'geolocation', 'geolocation_ajax' ), true ) ) {
			return;
		}

		$integration_options = get_option( 'fincommerce_maxmind_geolocation_settings' );
	...
	public static function add_redirect_download_method_notice() {
		if ( 'redirect' === get_option( 'fincommerce_file_download_method' ) ) {
	...
	public static function download_directories_sync_complete() {
		$notice_dismissed = apply_filters(
			'fincommerce_hide_download_directories_sync_complete',
	...
	public static function maxmind_missing_license_key_notice() {
		...
		$filter_dismissed_notice = ! apply_filters( 'fincommerce_maxmind_geolocation_display_notices', true );
	...
	public static function redirect_download_method_notice() {
		if ( apply_filters( 'fincommerce_hide_redirect_method_nag', get_user_meta( get_current_user_id(), 'dismissed_redirect_download_method_notice', true ) ) ) {
			self::remove_notice( 'redirect_download_method' );
			return;
		}
	...
	public static function base_tables_missing_notice() {
		$notice_dismissed = apply_filters(
			'fincommerce_hide_base_tables_missing_nag',
	...
	protected static function is_uploads_directory_protected() {
		$cache_key = '_fincommerce_upload_directory_status';
		...
		// Check for the "uploads/fincommerce_uploads" directory.
		$response         = wp_safe_remote_get(
			esc_url_raw( $uploads['baseurl'] . '/fincommerce_uploads/' ),
			array(
				'redirection' => 0,
			)
		);
