...
use Automattic\FinCommerce\Internal\Admin\Orders\COTRedirectionController;
use Automattic\FinCommerce\Internal\Admin\Orders\PageController as Custom_Orders_PageController;
use Automattic\FinCommerce\Internal\Admin\Logging\PageController as LoggingPageController;
use Automattic\FinCommerce\Internal\Admin\Logging\FileV2\{ FileListTable, SearchListTable };
...
class WC_Admin_Menus {
	...
	public function __construct() {
		...
		/**
		 * Controls whether we add a submenu item for the FinCommerce Addons page.
		 ...
		 */
		if ( apply_filters( 'fincommerce_show_addons_page', true ) ) {
		...
		// Admin bar menus.
		if ( apply_filters( 'fincommerce_show_admin_bar_visit_store', true ) ) {
	...
	public function reports_menu() {
		if ( self::can_view_fincommerce_menu_item() ) {
			add_submenu_page( 'fincommerce', __( 'Reports', 'fincommerce' ), __( 'Reports', 'fincommerce' ), 'view_fincommerce_reports', 'wc-reports', array( $this, 'reports_page' ) );
		} else {
			add_menu_page( __( 'Sales reports', 'fincommerce' ), __( 'Sales reports', 'fincommerce' ), 'view_fincommerce_reports', 'wc-reports', array( $this, 'reports_page' ), 'dashicons-chart-bar', '55.6' );
		}
	}
	...
	public function settings_menu() {
		$settings_page = add_submenu_page(
			'fincommerce',
			__( 'FinCommerce settings', 'fincommerce' ),
	...
	/**
	 * Check if the user can access the top-level FinCommerce item.
	 */
	public static function can_view_fincommerce_menu_item() {
	...
	public function save_settings() {
		...
		if ( '' !== $current_section && apply_filters( "fincommerce_save_settings_{$current_tab}_{$current_section}", ! empty( $_POST['save'] ) ) ) { // WPCS: input var okay, CSRF ok.
			WC_Admin_Settings::save();
		} elseif ( '' === $current_section && apply_filters( "fincommerce_save_settings_{$current_tab}", ! empty( $_POST['save'] ) ) ) { // WPCS: input var okay, CSRF ok.
			WC_Admin_Settings::save();
		}
	}
	...
	public function status_menu() {
		$status_page = add_submenu_page( 'fincommerce', __( 'FinCommerce status', 'fincommerce' ), __( 'Status', 'fincommerce' ), 'manage_fincommerce', 'wc-status', array( $this, 'status_page' ) );
	...
	public function addons_menu() {
		...
		$menu_title = sprintf( __( 'Extensions %s', 'fincommerce' ), $count_html );
		add_submenu_page( 'fincommerce', __( 'FinCommerce extensions', 'fincommerce' ), $menu_title, 'manage_fincommerce', 'wc-addons', array( $this, 'addons_page' ) );
	}

	/**
	 * Registers the wc-addons page within the FinCommerce menu.
	 * Temporary measure till we convert the whole page to React.
	 *
	 * @return void
	 */
	public function addons_my_subscriptions() {
		add_submenu_page( 'fincommerce', __( 'FinCommerce extensions', 'fincommerce' ), null, 'manage_fincommerce', 'wc-addons', array( $this, 'addons_page' ) );
		// Temporarily hide the submenu item we've just added.
		$this->hide_submenu_page( 'fincommerce', 'wc-addons' );
	}
	...
	public function menu_highlight() {
		...
		switch ( $post_type ) {
			case 'shop_order':
			case 'shop_coupon':
				$parent_file = 'fincommerce'; // WPCS: override ok.
				break;
			case 'product':
	...
	public function menu_order_count() {
		...
		if ( isset( $submenu['fincommerce'] ) ) {
			// Remove 'FinCommerce' sub menu item.
			unset( $submenu['fincommerce'][0] );

			// Add count if user has access.
			if ( apply_filters( 'fincommerce_include_processing_order_count_in_menu', true ) && current_user_can( 'edit_others_shop_orders' ) ) {
				$order_count = apply_filters( 'fincommerce_menu_order_count', wc_processing_order_count() );
	...
	public function menu_order( $menu_order ) {
		...
		$fincommerce_menu_order = array();

		// Get the index of our custom separator.
		$fincommerce_separator = array_search( 'separator-fincommerce', $menu_order, true );

		// Get index of product menu.
		$fincommerce_product = array_search( 'edit.php?post_type=product', $menu_order, true );

		// Loop through menu order and do some rearranging.
		foreach ( $menu_order as $index => $item ) {

			if ( 'fincommerce' === $item ) {
				$fincommerce_menu_order[] = 'separator-fincommerce';
				$fincommerce_menu_order[] = $item;
				$fincommerce_menu_order[] = 'edit.php?post_type=product';
				unset( $menu_order[ $fincommerce_separator ] );
				unset( $menu_order[ $fincommerce_product ] );
			} elseif ( ! in_array( $item, array( 'separator-fincommerce' ), true ) ) {
				$fincommerce_menu_order[] = $item;
			}
		}

		// Return order.
		return $fincommerce_menu_order;
	}
	...
	public function custom_menu_order( $enabled ) {
		return $enabled || self::can_view_fincommerce_menu_item();
	}
	...
	public function set_screen_option( $status, $option, $value ) {
		$screen_options = array(
			'fincommerce_keys_per_page',
			'fincommerce_webhooks_per_page',
	...
	/**
	 * Link to the order admin list table from the main FinCommerce menu.
	 *
	 * @return void
	 */
	...
	public function nav_menu_links() {
		...
		$endpoints['lost-password'] = __( 'Lost password', 'fincommerce' );

		$endpoints = apply_filters( 'fincommerce_custom_nav_menu_items', $endpoints );

		?>
		<div id="posttype-fincommerce-endpoints" class="posttypediv">
			<div id="tabs-panel-fincommerce-endpoints" class="tabs-panel tabs-panel-active">
				<ul id="fincommerce-endpoints-checklist" class="categorychecklist form-no-clear">
					<?php
					$i = -1;
			...
			<p class="button-controls" data-items-type="posttype-fincommerce-endpoints">
				<span class="list-controls">
					<label>
						<input type="checkbox" class="select-all" />
						<?php esc_html_e( 'Select all', 'fincommerce' ); ?>
					</label>
				</span>
				<span class="add-to-menu">
					<button type="submit" class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to menu', 'fincommerce' ); ?>" name="add-post-type-menu-item" id="submit-posttype-fincommerce-endpoints"><?php esc_html_e( 'Add to menu', 'fincommerce' ); ?></button>
	...
	public function admin_bar_menus( $wp_admin_bar ) {
		...
		$wp_admin_bar->add_node(
			array(
				'parent' => 'site-name',
				'id'     => 'view-store',
				'title'  => __( 'Visit Store', 'fincommerce' ),
