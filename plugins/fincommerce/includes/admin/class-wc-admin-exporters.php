<?php
/**
 * Init FinCommerce data exporters.
 *
...
class WC_Admin_Exporters {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_fincommerce_do_ajax_product_export', array( $this, 'do_ajax_product_export' ) );

		// Register FinCommerce exporters.
		$this->exporters['product_exporter'] = array(
			'menu'       => 'edit.php?post_type=product',
			'name'       => __( 'Product Export', 'fincommerce' ),
			'capability' => 'export',
	...
	/**
	 * Return true if FinCommerce export is allowed for current user, false otherwise.
	 *
	...
	public function do_ajax_product_export() {
		...
		if ( ! $this->export_allowed() ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient privileges to export products.', 'fincommerce' ) ) );
		}
		...
		$query_args = apply_filters(
			'fincommerce_export_get_ajax_query_args',
			array(
	...
	public static function get_product_types() {
		...
		$product_types[ ProductType::VARIATION ] = __( 'Product variations', 'fincommerce' );
		 ...
		 * }
		 */
		return apply_filters( 'fincommerce_exporter_product_types', $product_types );
	}
}
