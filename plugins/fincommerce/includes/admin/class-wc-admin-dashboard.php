	...
	class WC_Admin_Dashboard {
		...
		public function init() {
			...
			if ( current_user_can( 'publish_shop_orders' ) && post_type_supports( 'product', 'comments' ) ) {
				wp_add_dashboard_widget( 'fincommerce_dashboard_recent_reviews', __( 'FinCommerce Recent Reviews', 'fincommerce' ), array( $this, 'recent_reviews' ) );
			}
			wp_add_dashboard_widget( 'fincommerce_dashboard_status', __( 'FinCommerce Status', 'fincommerce' ), array( $this, 'status_widget' ) );
		...
		public function register_network_order_widget() {
			wp_add_dashboard_widget( 'fincommerce_network_orders', __( 'FinCommerce Network Orders', 'fincommerce' ), array( $this, 'network_orders' ) );
		}
		...
		private function should_display_widget() {
			...
			$has_permission           = current_user_can( 'view_fincommerce_reports' ) || current_user_can( 'manage_fincommerce' ) || current_user_can( 'publish_shop_orders' );
			$task_completed_or_hidden = 'yes' === get_option( 'fincommerce_task_list_complete' ) || 'yes' === get_option( 'fincommerce_task_list_hidden' );
		...
		private function get_top_seller() {
			...
			$query['join']   = "INNER JOIN {$wpdb->prefix}fincommerce_order_items AS order_items ON orders.{$orders_column_id} = order_id ";
			$query['join']  .= "INNER JOIN {$wpdb->prefix}fincommerce_order_itemmeta AS order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id ";
			$query['join']  .= "INNER JOIN {$wpdb->prefix}fincommerce_order_itemmeta AS order_item_meta_2 ON order_items.order_item_id = order_item_meta_2.order_item_id ";
			 ...
			 */
			$order_statuses  = apply_filters( 'fincommerce_reports_order_statuses', array( OrderStatus::COMPLETED, OrderStatus::PROCESSING, OrderStatus::ON_HOLD ) );
		...
		public function status_widget() {
			...
			echo '<p>' . esc_html__( 'Loading status data...', 'fincommerce' ) . ' <span class="spinner is-active"></span></p>';
			echo '</div>';
		...
		public function status_widget_content() {
			//phpcs:ignore
			$is_wc_admin_disabled = apply_filters( 'fincommerce_admin_disabled', false ) || ! Features::is_enabled( 'analytics' );
			...
			if ( $is_wc_admin_disabled ) {
				 ...
				 * @since 9.5.0
				 */
				$status_widget_reports = apply_filters( 'fincommerce_dashboard_status_widget_reports', $status_widget_reports );
			} else {
			...
			if ( current_user_can( 'view_fincommerce_reports' ) ) {
			...
			if ( get_option( 'fincommerce_manage_stock' ) === 'yes' ) {
				$this->status_widget_stock_rows( $status_widget_reports['lowstock_link'], $status_widget_reports['outofstock_link'] );
			}

			/**
			 * Filter to change the first argument passed to the `fincommerce_after_dashboard_status_widget` action.
			 *
			 * Please note that this filter is mainly for backward compatibility with the legacy reports.
			 ...
			 * @since 9.5.0
			 */
			$reports = apply_filters( 'fincommerce_after_dashboard_status_widget_parameter', null );
			do_action( 'fincommerce_after_dashboard_status_widget', $reports );
			echo '</ul>';
		}
		...
		private function status_widget_stock_rows( $lowstock_link, $outofstock_link ) {
			...
			if ( version_compare( get_option( 'fincommerce_db_version', null ), '3.6', '<' ) ) {
				return;
			}

			$stock   = absint( max( get_option( 'fincommerce_notify_low_stock_amount' ), 1 ) );
			$nostock = absint( max( get_option( 'fincommerce_notify_no_stock_amount' ), 0 ) );
			...
			if ( false === $lowinstock_count ) {
				 ...
				 */
				$lowinstock_count = apply_filters( 'fincommerce_status_widget_low_in_stock_count_pre_query', null, $stock, $nostock );
			...
			if ( false === $outofstock_count ) {
				 ...
				 * @param int         $nostock          No stock amount
				 */
				$outofstock_count = apply_filters( 'fincommerce_status_widget_out_of_stock_count_pre_query', null, $nostock );
		...
		public function recent_reviews() {
			...
			$query_from = apply_filters(
				'fincommerce_report_recent_reviews_query_from',
			...
			if ( $comments ) {
				...
				foreach ( $comments as $comment ) {
					...
					echo '<div class="star-rating"><span style="width:' . esc_attr( $rating * 20 ) . '%">' . sprintf( esc_html__( '%s out of 5', 'fincommerce' ), esc_html( $rating ) ) . '</span></div>';

					/* translators: %s: review author */
					echo '<h4 class="meta"><a href="' . esc_url( get_permalink( $comment->ID ) ) . '#comment-' . esc_attr( absint( $comment->comment_ID ) ) . '">' . esc_html( apply_filters( 'fincommerce_admin_dashboard_recent_reviews', $comment->post_title, $comment ) ) . '</a> ' . sprintf( esc_html__( 'reviewed by %s', 'fincommerce' ), esc_html( $comment->comment_author ) ) . '</h4>';
					echo '<blockquote>' . wp_kses_data( $comment->comment_content ) . '</blockquote></li>';

				}
				echo '</ul>';
			} else {
				echo '<p>' . esc_html__( 'There are no product reviews yet.', 'fincommerce' ) . '</p>';
			}
		}
		...
		public function network_orders() {
			...
			<div id="fincommerce-network-order-table-loading" class="fincommerce-network-order-table-loading is-active">
				<p>
					<span class="spinner is-active"></span> <?php esc_html_e( 'Loading network orders', 'fincommerce' ); ?>
				</p>

			</div>
			<table id="fincommerce-network-order-table" class="fincommerce-network-order-table">
				<thead>
					<tr>
						<td><?php esc_html_e( 'Order', 'fincommerce' ); ?></td>
						<td><?php esc_html_e( 'Status', 'fincommerce' ); ?></td>
						<td><?php esc_html_e( 'Total', 'fincommerce' ); ?></td>
					</tr>
				</thead>
			...
			<div id="fincommerce-network-orders-no-orders" class="fincommerce-network-orders-no-orders">
				<p>
					<?php esc_html_e( 'No orders found', 'fincommerce' ); ?>
				</p>
			</div>
		...
		private function get_wc_admin_performance_data() {
			...
			if ( 200 !== $response->get_status() ) {
				return new \WP_Error( 'fincommerce_analytics_performance_indicators_result_failed', __( 'Sorry, fetching performance indicators failed.', 'fincommerce' ) );
			}
			$report_keys      = array(
		...
		private function sales_sparkline_markup( $type, $days, $total, $sparkline_data ) {
			if ( 'sales' === $type ) {
				...
				$tooltip = sprintf( __( 'Sold %1$s worth in the last %2$d days', 'fincommerce' ), strip_tags( wc_price( $total ) ), $days );
			} else {
				/* translators: 1: total items sold 2: days */
				$tooltip = sprintf( _n( 'Sold %1$d item in the last %2$d days', 'Sold %1$d items in the last %2$d days', $total, 'fincommerce' ), $total, $days );
			}
