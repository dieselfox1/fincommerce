<?php
/**
 * FinCommerce Admin Settings Class
 *
	...
	class WC_Admin_Settings {
		...
		public static function get_settings_pages() {
			if ( empty( self::$settings ) ) {
				...
				if ( \Automattic\FinCommerce\Admin\Features\Features::is_enabled( 'launch-your-store' ) ) {
				...
				self::$settings = apply_filters( 'fincommerce_get_settings_pages', $settings );
				add_action(
					'admin_head',
					function () {
						if ( function_exists( 'get_current_screen' ) ) {
							$screen = get_current_screen();
							if ( 'fincommerce_page_wc-settings' === $screen->id ) {
		...
		public static function save() {
			...
			check_admin_referer( 'fincommerce-settings' );

			// Trigger actions.
			do_action( 'fincommerce_settings_save_' . $current_tab );
			do_action( 'fincommerce_update_options_' . $current_tab );
			do_action( 'fincommerce_update_options' );

			self::add_message( __( 'Your settings have been saved.', 'fincommerce' ) );
			...
			do_action( 'fincommerce_settings_saved' );
		}
		 ...
		 * Settings page.
		 *
		 * Handles the display of the main fincommerce settings page in admin.
		 */
		public static function output() {
			...
			do_action( 'fincommerce_settings_start' );

			wp_enqueue_script( 'fincommerce_settings', WC()->plugin_url() . '/assets/js/admin/settings' . $suffix . '.js', array( 'jquery', 'wp-util', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'iris', 'selectWoo' ), WC()->version, true );

			wp_localize_script(
				'fincommerce_settings',
				'fincommerce_settings_params',
				array(
					'i18n_nav_warning'                    => __( 'The changes you made will be lost if you navigate away from this page.', 'fincommerce' ),
					'i18n_moved_up'                       => __( 'Item moved up', 'fincommerce' ),
					'i18n_moved_down'                     => __( 'Item moved down', 'fincommerce' ),
					'i18n_no_specific_countries_selected' => __( 'Selecting no country / region to sell to prevents from completing the checkout. Continue anyway?', 'fincommerce' ),
				)
			);

			// Get tabs for the settings page.
			$tabs = apply_filters( 'fincommerce_settings_tabs_array', array() );
		 ...
		 * Output admin fields.
		 *
		 * Loops through the fincommerce options array and outputs each field.
		 *
			...
			foreach ( $options as $value ) {
				...
				switch ( $value['type'] ) {
					...
					case 'title':
						...
						if ( ! empty( $value['id'] ) ) {
							do_action( 'fincommerce_settings_' . sanitize_title( $value['id'] ) );
						}
						break;
					...
					case 'sectionend':
						if ( ! empty( $value['id'] ) ) {
							do_action( 'fincommerce_settings_' . sanitize_title( $value['id'] ) . '_end' );
						}
						echo '</table>';
						if ( ! empty( $value['id'] ) ) {
							do_action( 'fincommerce_settings_' . sanitize_title( $value['id'] ) . '_after' );
						}
						break;
					...
					case 'image_width':
						...
						if ( has_filter( 'fincommerce_get_image_size_' . $image_size ) ) {
							$disabled_attr    = 'disabled="disabled"';
							$disabled_message = '<p><small>' . esc_html__( 'The settings of this image size have been disabled because its values are being overwritten by a filter.', 'fincommerce' ) . '</small></p>';
						}
						...
						<tr class="<?php echo esc_attr( $value['row_class'] ); ?>">
							...
							<td class="forminp image_width_settings">
								...
								<label><input name="<?php echo esc_attr( $value['field_name'] ); ?>[crop]" <?php echo $disabled_attr; // WPCS: XSS ok. ?> id="<?php echo esc_attr( $value['id'] ); ?>-crop" type="checkbox" value="1" <?php checked( 1, $crop ); ?> /> <?php esc_html_e( 'Hard crop?', 'fincommerce' ); ?></label>
					...
					case 'single_select_page':
						...
						<tr class="single_select_page <?php echo esc_attr( $value['row_class'] ); ?>">
							...
							<td class="forminp">
								<?php echo str_replace( ' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'fincommerce' ) . "' style='" . $value['css'] . "' class='" . $value['class'] . "' id=", wp_dropdown_pages( $args ) ); // WPCS: XSS ok. ?> <?php echo $description; // WPCS: XSS ok. ?>
							</td>
						</tr>
						<?php
						break;
					...
					case 'single_select_country':
						...
						<tr class="<?php echo esc_attr( $value['row_class'] ); ?>">
							...
							<td class="forminp"><select name="<?php echo esc_attr( $value['field_name'] ); ?>" id="<?php echo esc_attr( $value['id'] ); ?>" style="<?php echo esc_attr( $value['css'] ); ?>" data-placeholder="<?php esc_attr_e( 'Choose a country / region&hellip;', 'fincommerce' ); ?>" aria-label="<?php esc_attr_e( 'Country / Region', 'fincommerce' ); ?>" class="wc-enhanced-select">
						...
						<tr class="<?php echo esc_attr( $value['row_class'] ); ?>">
							...
							<td class="forminp">
								<select
									...
									id="<?php echo esc_attr( $value['id'] ); ?>"
									style="width:350px"
									data-placeholder="<?php esc_attr_e( 'Choose countries / regions&hellip;', 'fincommerce' ); ?>"
									aria-label="<?php esc_attr_e( 'Country / Region', 'fincommerce' ); ?>"
									...
									?>
								</select> <?php echo ( $description ) ? $description : ''; // WPCS: XSS ok. ?> <br /><a class="select_all button" href="#"><?php esc_html_e( 'Select all', 'fincommerce' ); ?></a> <a class="select_none button" href="#"><?php esc_html_e( 'Select none', 'fincommerce' ); ?></a>
							</td>
						</tr>
						<?php
						break;
					...
					default:
						do_action( 'fincommerce_admin_field_' . $value['type'], $value );
						break;
				}
			}
		}
		...
		public static function save_fields( $options, $data = null ) {
			...
			foreach ( $options as $option ) {
				 ...
				 */
				if ( has_action( 'fincommerce_update_option_' . sanitize_title( $option['type'] ) ) ) {
					wc_deprecated_function( 'The fincommerce_update_option_X action', '2.4.0', 'fincommerce_admin_settings_sanitize_option filter' );
					do_action( 'fincommerce_update_option_' . sanitize_title( $option['type'] ), $option );
					continue;
				}
				 ...
				 * @since 2.4.0
				 */
				$value = apply_filters( 'fincommerce_admin_settings_sanitize_option', $value, $option, $raw_value );
				 ...
				 * @since 2.4.0
				 */
				$value = apply_filters( "fincommerce_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );
		...
		public static function check_download_folder_protection() {
			...
			$downloads_path  = $upload_dir['basedir'] . '/fincommerce_uploads';
