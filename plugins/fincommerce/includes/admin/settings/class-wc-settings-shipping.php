...
class WC_Settings_Shipping extends WC_Settings_Page {
	...
	public function __construct() {
		$this->id    = 'shipping';
		$this->label = __( 'Shipping', 'fincommerce' );
	...
	protected function get_own_sections() {
		$sections = array(
			''        => __( 'Shipping zones', 'fincommerce' ),
			'options' => __( 'Shipping settings', 'fincommerce' ),
			'classes' => __( 'Classes', 'fincommerce' ),
		);
	...
	protected function get_settings_for_options_section() {
		$settings =
			array(
				array(
					'title' => __( 'Shipping settings', 'fincommerce' ),
					'type'  => 'title',
					'id'    => 'shipping_options',
				),

				array(
					'title'         => __( 'Calculations', 'fincommerce' ),
					'desc'          => __( 'Enable the shipping calculator on the cart page', 'fincommerce' ),
				...
				array(
					'desc'          => __( 'Hide shipping costs until an address is entered', 'fincommerce' ),
				...
				array(
					'desc'          => __( 'Hide shipping rates when free shipping is available', 'fincommerce' ),
					'id'            => 'fincommerce_shipping_hide_rates_when_free',
					'default'       => 'no',
					'type'          => 'checkbox',
					'autoload'      => false,
					'checkboxgroup' => 'end',
				...
				array(
					'title'           => __( 'Shipping destination', 'fincommerce' ),
					'desc'            => __( 'This controls which shipping address is used by default.', 'fincommerce' ),
					'id'              => 'fincommerce_ship_to_destination',
					'default'         => 'billing',
					'type'            => 'radio',
					'options'         => array(
						'shipping'     => __( 'Default to customer shipping address', 'fincommerce' ),
						'billing'      => __( 'Default to customer billing address', 'fincommerce' ),
						'billing_only' => __( 'Force shipping to the customer billing address', 'fincommerce' ),
					),
					'autoload'        => false,
					'desc_tip'        => true,
				...
				array(
					'title'    => __( 'Debug mode', 'fincommerce' ),
					'desc'     => __( 'Enable debug mode', 'fincommerce' ),
					'desc_tip' => __( 'Enable shipping debug mode to show matching shipping zones and to bypass shipping rate cache.', 'fincommerce' ),
		...
		return apply_filters( 'fincommerce_shipping_settings', $settings );
	}
	...
	protected function zone_methods_screen( $zone_id ) {
		...
		if ( ! $zone ) {
			wp_die( esc_html__( 'Zone does not exist!', 'fincommerce' ) );
		}
		...
		$localized_object = array(
			...
			'strings'                 => array(
				'unload_confirmation_msg'             => __( 'Your changed data will be lost if you leave this page without saving.', 'fincommerce' ),
				'save_changes_prompt'                 => __( 'Do you wish to save your changes first? Your changed data will be discarded if you choose to cancel.', 'fincommerce' ),
				'save_failed'                         => __( 'Your changes were not saved. Please retry.', 'fincommerce' ),
				'add_method_failed'                   => __( 'Shipping method could not be added. Please retry.', 'fincommerce' ),
				'remove_method_failed'                => __( 'Shipping method could not be removed. Please retry.', 'fincommerce' ),
				'yes'                                 => __( 'Yes', 'fincommerce' ),
				'no'                                  => __( 'No', 'fincommerce' ),
				'default_zone_name'                   => __( 'Zone', 'fincommerce' ),
				'delete_shipping_method_confirmation' => __( 'Are you sure you want to delete this shipping method?', 'fincommerce' ),
				'invalid_number_format'               => __( 'Please enter a valid number.', 'fincommerce' ),
			),
		);
	...
	protected function zones_screen() {
		...
		wp_localize_script(
			...
			array(
				...
				'strings'                 => array(
					'unload_confirmation_msg'     => __( 'Your changed data will be lost if you leave this page without saving.', 'fincommerce' ),
					'delete_confirmation_msg'     => __( 'Are you sure you want to delete this zone? This action cannot be undone.', 'fincommerce' ),
					'save_failed'                 => __( 'Your changes were not saved. Please retry.', 'fincommerce' ),
					'no_shipping_methods_offered' => __( 'No shipping methods offered to this zone.', 'fincommerce' ),
				),
			)
		);
	...
	protected function instance_settings_screen( $instance_id ) {
		...
		if ( ! $shipping_method ) {
			wp_die( esc_html__( 'Invalid shipping method!', 'fincommerce' ) );
		}
		if ( ! $zone ) {
			wp_die( esc_html__( 'Zone does not exist!', 'fincommerce' ) );
		}
		if ( ! $shipping_method->has_settings() ) {
			wp_die( esc_html__( 'This shipping method does not have any settings to configure.', 'fincommerce' ) );
		}
	...
	protected function output_shipping_class_screen() {
		...
		$shipping_class_columns = apply_filters(
			'fincommerce_shipping_classes_columns',
			array(
				'wc-shipping-class-name'        => __( 'Shipping class', 'fincommerce' ),
				'wc-shipping-class-slug'        => __( 'Slug', 'fincommerce' ),
				'wc-shipping-class-description' => __( 'Description', 'fincommerce' ),
				'wc-shipping-class-count'       => __( 'Product count', 'fincommerce' ),
			)
		);
