...
class WC_Settings_Payment_Gateways extends WC_Settings_Page {
	...
	public function __construct() {
		...
		$this->label = esc_html_x( 'Payments', 'Settings tab label', 'fincommerce' );
		...
		// on the FinCommerce Payments settings page and Reactified sections.
		add_filter( 'fincommerce_admin_features', array( $this, 'suppress_store_alerts' ), PHP_INT_MAX );
	...
	public function add_body_classes( $classes ) {
		...
		// If we are not on the FinCommerce Payments settings page, return the classes as they are.
		...
		if ( ! $this->should_render_react_section( $current_section ) ) {
			...
			$classes .= ' fincommerce-settings-payments-section_legacy';
			// Add a class to indicate that the current section is rendered in legacy mode.
			$classes .= ' fincommerce_page_wc-settings-checkout-section-' . esc_attr( $this->standardize_section_name( $current_section ) ) . '_legacy';
		}
	...
	private function get_reactified_sections(): array {
		 ...
		 */
		$optional_reactified_sections = apply_filters( 'experimental_fincommerce_admin_payment_reactify_render_sections', $optional_reactified_sections );
	 ...
	 * the `fincommerce_get_sections_checkout` filter.
	 *
	...
	public function get_sections() {
		...
		// We only want to prevent sections on the main FinCommerce Payments settings page and Reactified sections.
		...
		if ( self::MAIN_SECTION_NAME === $standardized_section ) {
			...
			foreach ( $wc_payment_gateways->payment_gateways() as $gateway ) {
				...
				if ( in_array( $standardized_section, array( $gateway->id, sanitize_title( get_class( $gateway ) ) ), true ) ) {
					 ...
					 */
					do_action( 'fincommerce_update_options_payment_gateways_' . $gateway->id );
					$wc_payment_gateways->init();
	...
	public function hide_help_tabs() {
		...
		if ( ! $screen instanceof WP_Screen || 'fincommerce_page_wc-settings' !== $screen->id ) {
			return;
		}

		// We only want to hide the help tabs on the main FinCommerce Payments settings page and Reactified sections.
		if ( self::TAB_NAME !== $current_tab ) {
			return;
		}
		if ( ! $this->should_render_react_section( $current_section ) ) {
	...
	/**
	 * Suppress WP admin notices on the FinCommerce Payments settings page.
	 */
	public function suppress_admin_notices() {
		...
		// We only want to suppress notices on the main FinCommerce Payments settings page and Reactified sections.
		if ( self::TAB_NAME !== $current_tab ) {
			return;
		}
		if ( ! $this->should_render_react_section( $current_section ) ) {
		...
		// FinCommerce uses the 'admin_notices' hook for its own notices.
		// We will only allow FinCommerce core notices to be displayed.
		...
		if ( empty( $wc_admin_notices ) ) {
			// If there are no FinCommerce core notices, we can remove all actions hooked into `admin_notices`.
			remove_all_actions( 'admin_notices' );
			return;
		}

		// Go through the callbacks hooked into `admin_notices` and
		// remove any that are NOT from the FinCommerce core (i.e. from the `WC_Admin_Notices` class).
		foreach ( $wp_admin_notices_hook->callbacks as $priority => $callbacks ) {
			...
			foreach ( $callbacks as $callback ) {
				...
				// FinCommerce doesn't use closures to handle notices.
				// FinCommerce core notices are handled by `WC_Admin_Notices` class methods.
				...
				// We need to allow Automattic\FinCommerce\Internal\Admin\Loader methods callbacks
				// because they are used to wrap notices.
				// @see Automattic\FinCommerce\Internal\Admin\Loader::inject_before_notices().
				// @see Automattic\FinCommerce\Internal\Admin\Loader::inject_after_notices().
				if (
					(
	...
	/**
	 * Suppress the store-alerts WCAdmin feature on the FinCommerce Payments settings page and Reactified sections.
	 *
