...
use Automattic\FinCommerce\Internal\Email\EmailColors;
use Automattic\FinCommerce\Internal\Email\EmailFont;
use Automattic\FinCommerce\Internal\Email\EmailStyleSync;
use Automattic\FinCommerce\Internal\EmailEditor\EmailTemplates\WooEmailTemplate;
use Automattic\FinCommerce\Internal\EmailEditor\WCTransactionalEmails\WCTransactionalEmailPostsManager;
...
class WC_Settings_Emails extends WC_Settings_Page {
	...
	public function __construct() {
		$this->id    = 'email';
		$this->label = __( 'Emails', 'fincommerce' );

		add_action( 'fincommerce_admin_field_email_notification', array( $this, 'email_notification_setting' ) );
		add_action( 'fincommerce_admin_field_email_notification_block_emails', array( $this, 'email_notification_setting_block_emails' ) );
		add_action( 'fincommerce_admin_field_email_preview', array( $this, 'email_preview' ) );
		add_action( 'fincommerce_admin_field_email_image_url', array( $this, 'email_image_url' ) );
		add_action( 'fincommerce_admin_field_email_font_family', array( $this, 'email_font_family' ) );
		add_action( 'fincommerce_admin_field_email_color_palette', array( $this, 'email_color_palette' ) );
		add_action( 'fincommerce_admin_field_previewing_new_templates', array( $this, 'previewing_new_templates' ) );
		add_action( 'fincommerce_admin_field_email_improvements_button', array( $this, 'email_improvements_button' ) );
		add_action( 'fincommerce_email_settings_after', array( $this, 'email_preview_single' ) );
		add_action( 'fincommerce_settings_saved', array( $this, 'enable_email_improvements_when_trying_new_templates' ), 999 );
		add_filter( 'fincommerce_admin_settings_sanitize_option_fincommerce_email_header_image', array( $this, 'sanitize_email_header_image' ), 10, 3 );
		add_filter( 'fincommerce_tracks_event_properties', array( $this, 'append_feature_email_improvements_to_tracks' ) );
	...
	protected function get_settings_for_default_section() {
		...
		if ( $block_email_editor_enabled ) {
			...
			$email_notifications_desc = sprintf( __( 'Email notifications sent from FinCommerce are listed below. Click on an email to configure it.<br>%s', 'fincommerce' ), $desc_help_text );
		}

		$settings =
			array(
				array(
					'title' => __( 'Email notifications', 'fincommerce' ),
				...
				array(
					'title' => __( 'Email sender options', 'fincommerce' ),
					'type'  => 'title',
					'desc'  => __( "Set the name and email address you'd like your outgoing emails to use.", 'fincommerce' ),
					'id'    => 'email_options',
				),

				array(
					'title'    => __( '"From" name', 'fincommerce' ),
					'desc'     => '',
					'id'       => 'fincommerce_email_from_name',
					'type'     => 'text',
					'css'      => 'min-width:400px;',
				...
				array(
					'title'             => __( '"From" address', 'fincommerce' ),
					'desc'              => '',
					'id'                => 'fincommerce_email_from_address',
					'type'              => 'email',
		...
		if ( ! $block_email_editor_enabled ) {
			$settings = array_merge(
				...
				array(
					array(
						'title' => __( 'Email template', 'fincommerce' ),
						'type'  => 'title',
						'desc'  => __( 'Customize your FinCommerce email template and preview it below.', 'fincommerce' ),
						'id'    => 'email_template_options',
					),

					array(
						'title' => __( 'Try new templates', 'fincommerce' ),
						'type'  => 'previewing_new_templates',
						'id'    => 'previewing_new_templates',
					),

					array(
						'title'       => __( 'Logo', 'fincommerce' ),
						'desc'        => __( 'Add your logo to each of your FinCommerce emails. If no logo is uploaded, your site title will be used instead.', 'fincommerce' ),
						'id'          => 'fincommerce_email_header_image',
					...
					array(
						'title'     => __( 'Logo width (px)', 'fincommerce' ),
						'id'        => 'fincommerce_email_header_image_width',
						'desc_tip'  => '',
						'default'   => '120',
						'type'      => 'number',
					...
					array(
						'title'     => __( 'Header alignment', 'fincommerce' ),
						'id'        => 'fincommerce_email_header_alignment',
						'desc_tip'  => '',
						'default'   => 'left',
						'type'      => 'select',
						'class'     => 'wc-enhanced-select',
						'options'   => array(
							'left'   => __( 'Left', 'fincommerce' ),
							'center' => __( 'Center', 'fincommerce' ),
							'right'  => __( 'Right', 'fincommerce' ),
						),
						'row_class' => $email_improvements_enabled ? '' : 'disabled',
					),

					array(
						'title'     => __( 'Font family', 'fincommerce' ),
						'id'        => 'fincommerce_email_font_family',
						'default'   => 'Helvetica',
					...
					array(
						'title'       => __( 'Footer text', 'fincommerce' ),
						/* translators: %s: Available placeholders for use */
						'desc'        => __( 'This text will appear in the footer of all of your FinCommerce emails.', 'fincommerce' ) . ' ' . sprintf( __( 'Available placeholders: %s', 'fincommerce' ), '{site_title} {site_url} {store_address} {store_email}' ),
						'id'          => 'fincommerce_email_footer_text',
					...
					array(
						'title' => __( 'Color palette', 'fincommerce' ),
						'type'  => 'email_color_palette',
						'id'    => 'email_color_palette',
					),

					array(
						'title'    => __( 'Accent', 'fincommerce' ),
						/* translators: %s: default color */
						'desc'     => sprintf( __( 'Customize the color of your buttons and links. Default %s.', 'fincommerce' ), '<code>' . $default_colors['base'] . '</code>' ),
						'id'       => 'fincommerce_email_base_color',
						'type'     => 'color',
						'css'      => 'width:6em;',
					...
					array(
						'title'    => __( 'Email background', 'fincommerce' ),
						/* translators: %s: default color */
						'desc'     => sprintf( __( 'Select a color for the background of your emails. Default %s.', 'fincommerce' ), '<code>' . $default_colors['bg'] . '</code>' ),
						'id'       => 'fincommerce_email_background_color',
						'type'     => 'color',
						'css'      => 'width:6em;',
					...
					array(
						'title'    => __( 'Content background', 'fincommerce' ),
						/* translators: %s: default color */
						'desc'     => sprintf( __( 'Choose a background color for the content area of your emails. Default %s.', 'fincommerce' ), '<code>' . $default_colors['body_bg'] . '</code>' ),
						'id'       => 'fincommerce_email_body_background_color',
						'type'     => 'color',
						'css'      => 'width:6em;',
					...
					array(
						'title'    => __( 'Heading & text', 'fincommerce' ),
						/* translators: %s: default color */
						'desc'     => sprintf( __( 'Set the color of your headings and text. Default %s.', 'fincommerce' ), '<code>' . $default_colors['body_text'] . '</code>' ),
						'id'       => 'fincommerce_email_text_color',
						'type'     => 'color',
						'css'      => 'width:6em;',
					...
					array(
						'title'    => __( 'Secondary text', 'fincommerce' ),
						/* translators: %s: footer default color */
						'desc'     => sprintf( __( 'Choose a color for your secondary text, such as your footer content. Default %s.', 'fincommerce' ), '<code>' . $default_colors['footer_text'] . '</code>' ),
						'id'       => 'fincommerce_email_footer_text_color',
						'type'     => 'color',
						'css'      => 'width:6em;',
					...
					array(
						'title'    => __( 'Auto-sync with theme', 'fincommerce' ),
						'desc'     => __( 'Automatically update email styles when theme styles change', 'fincommerce' ),
						'id'       => 'fincommerce_email_auto_sync_with_theme',
						'type'     => 'hidden',
						'default'  => 'no',
						'autoload' => false,
					),
					...
					array(
						'title' => __( 'Email improvements button', 'fincommerce' ),
		...
		return apply_filters( 'fincommerce_email_settings', $settings );
	}
				...
				<thead>
					<tr>
						...
						$columns = apply_filters(
							'fincommerce_email_setting_columns',
							array(
								'status'     => '',
								'name'       => __( 'Email', 'fincommerce' ),
								'email_type' => __( 'Content type', 'fincommerce' ),
								'recipient'  => __( 'Recipient(s)', 'fincommerce' ),
								'actions'    => '',
							)
						);
							...
							foreach ( $columns as $key => $column ) {
								...
								switch ( $key ) {
									case 'name':
										...
										break;
									case 'recipient':
										$to  = $email->is_customer_email() ? __( 'Customer', 'fincommerce' ) : $email->get_recipient();
										$cc  = false;
										$bcc = false;
									...
									case 'status':
										...
										if ( $email->is_manual() ) {
											echo '<span class="status-manual tips" data-tip="' . esc_attr__( 'Manually sent', 'fincommerce' ) . '">' . esc_html__( 'Manual', 'fincommerce' ) . '</span>';
										} elseif ( $email->is_enabled() ) {
											echo '<span class="status-enabled tips" data-tip="' . esc_attr__( 'Enabled', 'fincommerce' ) . '">' . esc_html__( 'Yes', 'fincommerce' ) . '</span>';
										} else {
											echo '<span class="status-disabled tips" data-tip="' . esc_attr__( 'Disabled', 'fincommerce' ) . '">-</span>';
										}
									...
									case 'actions':
										echo '<td class="wc-email-settings-table-' . esc_attr( $key ) . '">
										<a class="button alignright" href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=email&section=' . strtolower( $email_key ) ) ) . '">' . esc_html__( 'Manage', 'fincommerce' ) . '</a>
										</td>';
										break;
									default:
										do_action( 'fincommerce_email_setting_column_' . $key, $email );
										break;
								}
							}
	...
	public function email_notification_setting_block_emails() {
		...
		foreach ( $emails as $email_key => $email ) {
			...
			$email_types[] = array(
				...
				'recipients'  => array(
					'to'  => $email->is_customer_email() ? __( 'Customers', 'fincommerce' ) : $email->get_recipient(),
		...
		?>
		<div
			id="wc_settings_email_listing_slotfill" class="wc-settings-prevent-change-event fincommerce-email-listing-listview"
		...
		>
			...
			width: 100%;
			">
				<h3> <?php esc_html_e( 'Loading&hellip;', 'fincommerce' ); ?>  </h3>
			</div>
		</div>
		<div>
	...
	public function email_preview() {
		...
		<div
			...
			data-preview-url="<?php echo esc_url( wp_nonce_url( admin_url( '?preview_fincommerce_mail=true' ), 'preview-mail' ) ); ?>"
			data-email-types="<?php echo esc_attr( wp_json_encode( $email_types ) ); ?>"
	...
	public function email_preview_single( $email ) {
		...
		?>
		<h2><?php echo esc_html( __( 'Email preview', 'fincommerce' ) ); ?></h2>

		<p><?php echo esc_html( __( 'Preview your email template. You can also test on different devices and send yourself a test email.', 'fincommerce' ) ); ?></p>
		<div>
			<div
				id="wc_settings_email_preview_slotfill"
				data-preview-url="<?php echo esc_url( wp_nonce_url( admin_url( '?preview_fincommerce_mail=true' ), 'preview-mail' ) ); ?>"
			...
			></div>
			<input type="hidden" id="fincommerce_email_from_name" value="<?php echo esc_attr( get_option( 'fincommerce_email_from_name' ) ); ?>" />
			<input type="hidden" id="fincommerce_email_from_address" value="<?php echo esc_attr( get_option( 'fincommerce_email_from_address' ) ); ?>" />
		</div>
		<?php
	}
		...
		<tr class="<?php echo esc_attr( $value['row_class'] ); ?>">
			...
			<script type="text/javascript">
				...
				<select
					...
					id="<?php echo esc_attr( $value['id'] ); ?>"
					>
					<optgroup label="<?php echo esc_attr__( 'Standard fonts', 'fincommerce' ); ?>">
						<?php
					...
					<?php if ( $custom_fonts ) : ?>
						<optgroup label="<?php echo esc_attr__( 'Custom fonts', 'fincommerce' ); ?>">
							<?php
	...
	public function email_color_palette( $value ) {
		...
		<div class="wc-settings-email-color-palette-header">
			...
			<?php if ( $email_improvements_enabled ) : ?>
				...
				<input
					type="hidden"
					name="fincommerce_email_auto_sync_with_theme"
					id="fincommerce_email_auto_sync_with_theme"
					value="<?php echo esc_attr( $auto_sync ); ?>"
				/>
			<?php else : ?>
				<div class="wc-settings-email-color-palette-buttons">
					<button disabled type="button" class="components-button is-secondary"><?php esc_html_e( 'Sync with theme', 'fincommerce' ); ?></button>
				</div>
			<?php endif; ?>
		</div>
	...
	public function previewing_new_templates(): void {
		...
		update_option( 'fincommerce_admin_dismissed_try_email_improvements_modal', 'yes' );
		?>
		<div class="wc-settings-email-color-palette-header submit">
			<div class="notice notice-info inline">
				<p><?php esc_html_e( 'Previewing new template designs. You can customize and test your emails and switch to the new template whenever you are ready.', 'fincommerce' ); ?></p>
				<p>
					<button name="save" type="submit" class="components-button is-secondary" value="<?php esc_attr_e( 'Save changes', 'fincommerce' ); ?>">
						<?php esc_html_e( 'Switch to new template', 'fincommerce' ); ?>
					</button>
					<a href="?page=wc-settings&tab=email" class="components-button is-tertiary">
						<?php esc_html_e( 'Keep legacy template', 'fincommerce' ); ?>
					</a>
				</p>
			</div>
		</div>
	...
	public function email_improvements_button(): void {
		...
		if ( ! $is_feature_enabled && ! $trying_new_templates ) {
			...
			<a href="?page=wc-settings&tab=email&try-new-templates" class="components-button is-link">
				<?php esc_html_e( 'Try our new email templates!', 'fincommerce' ); ?>
			</a>
			<?php
			return;
		}

		$has_feature_enabled_since_installation = 'yes' === get_option( 'fincommerce_email_improvements_default_enabled', 'no' );
		if ( $is_feature_enabled && ! $has_feature_enabled_since_installation ) {
			...
			<a href="<?php echo esc_url( add_query_arg( $disable_feature_args ) ); ?>"
				...
				onclick="return confirm('<?php esc_attr_e( 'Are you sure want to revert to legacy? Doing so will erase any changes you’ve made to your new email templates, and will restore your previous email designs.', 'fincommerce' ); ?>');"
			>
				<?php esc_html_e( 'Revert to legacy template', 'fincommerce' ); ?>
			</a>
			<?php
			return;
		}
	...
	public function track_email_improvements_feature_change( $feature_id, $enabled ) {
		if ( 'email_improvements' === $feature_id ) {
			...
			if ( $enabled ) {
				$enabled_count = get_option( 'fincommerce_email_improvements_enabled_count', 0 );
				update_option( 'fincommerce_email_improvements_enabled_count', $enabled_count + 1 );
				add_option( 'fincommerce_email_improvements_first_enabled_at', $current_date );
				update_option( 'fincommerce_email_improvements_last_enabled_at', $current_date );
			} else {
				$disabled_count = get_option( 'fincommerce_email_improvements_disabled_count', 0 );
				update_option( 'fincommerce_email_improvements_disabled_count', $disabled_count + 1 );
				add_option( 'fincommerce_email_improvements_first_disabled_at', $current_date );
				update_option( 'fincommerce_email_improvements_last_disabled_at', $current_date );
			}
		}
	}
