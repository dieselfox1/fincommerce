...
class WC_Settings_Tax extends WC_Settings_Page {
	...
	public function __construct() {
		$this->id    = 'tax';
		$this->label = __( 'Tax', 'fincommerce' );

		add_filter( 'fincommerce_settings_tabs_array', array( $this, 'add_settings_page' ), 20 );
		add_action( 'fincommerce_admin_field_conflict_error', array( $this, 'conflict_error' ) );
		if ( wc_tax_enabled() ) {
	...
	public function conflict_error() {
		?>
		<tr valign="top">
							<th scope="row" class="titledesc fincommerce_admin_tax_settings_slotfill_th">
							</th>
							<td class="forminp forminp-text fincommerce_admin_tax_settings_slotfill_td">
	...
	protected function get_own_sections() {
		$sections = array(
			''         => __( 'Tax options', 'fincommerce' ),
			'standard' => __( 'Standard rates', 'fincommerce' ),
		);
		...
		foreach ( $tax_classes as $class ) {
			...
			$sections[ sanitize_title( $class ) ] = sprintf( __( '%s rates', 'fincommerce' ), $class );
		}
			...
			if ( is_wp_error( $tax_class ) ) {
				WC_Admin_Settings::add_error(
					sprintf(
						...
						esc_html__( 'Additional tax class "%1$s" couldn\'t be saved. %2$s.', 'fincommerce' ),
						esc_html( $name ),
	...
	public function output_tax_rates() {
		...
		wp_localize_script(
			...
			array(
				...
				'strings'       => array(
					'no_rows_selected'        => __( 'No row(s) selected', 'fincommerce' ),
					'unload_confirmation_msg' => __( 'Your changed data will be lost if you leave this page without saving.', 'fincommerce' ),
					'csv_data_cols'           => array(
						__( 'Country code', 'fincommerce' ),
						__( 'State code', 'fincommerce' ),
						__( 'Postcode / ZIP', 'fincommerce' ),
						__( 'City', 'fincommerce' ),
						__( 'Rate %', 'fincommerce' ),
						__( 'Tax name', 'fincommerce' ),
						__( 'Priority', 'fincommerce' ),
						__( 'Compound', 'fincommerce' ),
						__( 'Shipping', 'fincommerce' ),
						__( 'Tax class', 'fincommerce' ),
					),
				),
			)
		);
	...
	public function save_tax_rates() {
		...
		$tax_rate_order = absint( $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_order FROM {$wpdb->prefix}fincommerce_tax_rates WHERE tax_rate_id = %s", $first_tax_rate_id ) ) );
