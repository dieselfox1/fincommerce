...
if ( ! class_exists( 'WC_Settings_Page', false ) ) :
	...
	abstract class WC_Settings_Page {
		...
		public function __construct() {
			...
			add_action( 'fincommerce_sections_' . $this->id, array( $this, 'output_sections' ) );
			add_action( 'fincommerce_settings_' . $this->id, array( $this, 'output' ) );
			add_action( 'fincommerce_settings_save_' . $this->id, array( $this, 'save' ) );
			add_action( 'fincommerce_admin_field_add_settings_slot', array( $this, 'add_settings_slot' ) );
		}
		...
		public function add_settings_page_data( $pages ) {
			...
			$pages[ $this->id ]['start'] = $this->get_custom_view( 'fincommerce_before_settings_' . $this->id );
			$pages[ $this->id ]['end']   = $this->get_custom_view( 'fincommerce_after_settings_' . $this->id );
		...
		protected function get_section_settings_data( $section_id, $sections ) {
			...
			$custom_view = $this->get_custom_view( 'fincommerce_settings_' . $this->id, $section_id );
			...
			if ( $this->output_called ) {
				...
				foreach ( $section_settings as $section_setting ) {
					...
					if ( 'sectionend' === $section_setting['type'] && ! empty( $section_setting['id'] ) ) {
						$section_settings_data[] = $this->get_custom_view( 'fincommerce_settings_' . $section_setting['id'] . '_end' );
						$section_settings_data[] = $this->get_custom_view( 'fincommerce_settings_' . $section_setting['id'] . '_after' );
					}
					...
					if ( 'title' === $section_setting['type'] && ! empty( $section_setting['id'] ) ) {
						$section_settings_data[] = $this->get_custom_view( 'fincommerce_settings_' . $section_setting['id'] );
					}
				}
			}
		...
		protected function populate_setting_value( $section_setting ) {
			...
			if ( ! in_array( $type, $this->types, true ) ) {
				$section_setting = $this->get_custom_type_field( 'fincommerce_admin_field_' . $type, $section_setting );
			}
		 ...
		 *
		 * External settings classes (registered via 'fincommerce_get_settings_pages' filter)
		...
		final public function get_settings_for_section( $section_id ) {
			...
			return apply_filters( 'fincommerce_get_settings_' . $this->id, $settings, $section_id );
		}
		 ...
		 *
		 * When overriding, note that the 'fincommerce_get_settings_' filter must NOT be triggered,
		...
		public function get_sections() {
			 ...
			 */
			return (array) apply_filters( 'fincommerce_get_sections_' . $this->id, $sections );
		}
		...
		/**
		 * Trigger the 'fincommerce_update_options_'.id action.
		 *
		...
		protected function do_update_options_action( $section_id = null ) {
			...
			if ( $section_id ) {
				do_action( 'fincommerce_update_options_' . $this->id . '_' . $section_id );
			}
		}
	}
