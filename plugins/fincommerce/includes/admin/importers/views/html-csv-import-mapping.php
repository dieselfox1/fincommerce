...
?>
<form class="wc-progress-form-content fincommerce-importer" method="post" action="<?php echo esc_url( $this->get_next_step_link() ); ?>">
	<header>
		<h2><?php esc_html_e( 'Map CSV fields to products', 'fincommerce' ); ?></h2>
		<p><?php esc_html_e( 'Select fields from your CSV file to map against products fields, or to ignore during import.', 'fincommerce' ); ?></p>
	</header>
	<section class="wc-importer-mapping-table-wrapper">
		<table class="widefat wc-importer-mapping-table">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Column name', 'fincommerce' ); ?></th>
					<th><?php esc_html_e( 'Map to field', 'fincommerce' ); ?></th>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						<td class="wc-importer-mapping-table-name">
							...
							<?php if ( ! empty( $sample[ $index ] ) ) : ?>
								<span class="description"><?php esc_html_e( 'Sample:', 'fincommerce' ); ?> <code><?php echo esc_html( $sample[ $index ] ); ?></code></span>
							<?php endif; ?>
						</td>
						<td class="wc-importer-mapping-table-field">
							...
							<select name="map_to[<?php echo esc_attr( $index ); ?>]">
								<option value=""><?php esc_html_e( 'Do not import', 'fincommerce' ); ?></option>
								<option value="">--------------</option>
	...
	<div class="wc-actions">
		<button type="submit" class="button button-primary button-next" value="<?php esc_attr_e( 'Run the importer', 'fincommerce' ); ?>" name="save_step"><?php esc_html_e( 'Run the importer', 'fincommerce' ); ?></button>
