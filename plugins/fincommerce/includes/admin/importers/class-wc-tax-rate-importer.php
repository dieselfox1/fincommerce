...
/**
 * Tax Rates importer - import tax rates and local tax rates into FinCommerce.
 *
...
class WC_Tax_Rate_Importer extends WP_Importer {
	...
	public function __construct() {
		$this->import_page = 'fincommerce_tax_rate_csv';
	...
	public function import( $file ) {
		if ( ! is_file( $file ) ) {
			$this->import_error( __( 'The file does not exist, please try again.', 'fincommerce' ) );
		}
		...
		if ( false !== $handle ) {
			...
			if ( 10 === $count ) {
				...
				$this->import_error( __( 'The CSV is invalid.', 'fincommerce' ) );
			}
		...
		printf(
			...
			esc_html__( 'Import complete - imported %s tax rates.', 'fincommerce' ),
	...
	public function import_end() {
		echo '<p>' . esc_html__( 'All done!', 'fincommerce' ) . ' <a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=tax' ) ) . '">' . esc_html__( 'View tax rates', 'fincommerce' ) . '</a></p>';
	...
	public function handle_upload() {
		...
		if ( empty( $file_url ) ) {
			...
			if ( ! wc_is_file_valid_csv( $file['file'], false ) ) {
				...
				$this->set_import_error_message( __( 'Invalid file type. The importer supports CSV and TXT file formats.', 'fincommerce' ) );
	...
	public function header() {
		echo '<div class="wrap">';
		echo '<h1>' . esc_html__( 'Import tax rates', 'fincommerce' ) . '</h1>';
	}
	...
	public function greet() {
		...
		echo '<p>' . esc_html__( 'Hi there! Upload a CSV file containing tax rates to import the contents into your shop. Choose a .csv file to upload, then click "Upload file and import".', 'fincommerce' ) . '</p>';

		/* translators: 1: Link to tax rates sample file 2: Closing link. */
		echo '<p>' . sprintf( esc_html__( 'Your CSV needs to include columns in a specific order. %1$sClick here to download a sample%2$s.', 'fincommerce' ), '<a href="' . esc_url( WC()->plugin_url() ) . '/sample-data/sample_tax_rates.csv">', '</a>' ) . '</p>';

		$action = 'admin.php?import=fincommerce_tax_rate_csv&step=1';
			...
			<form enctype="multipart/form-data" id="import-upload-form" method="post" action="<?php echo esc_attr( wp_nonce_url( $action, 'import-upload' ) ); ?>">
				<table class="form-table">
					<tbody>
						<tr>
							<th>
								<label for="upload"><?php esc_html_e( 'Choose a file from your computer:', 'fincommerce' ); ?></label>
							</th>
							<td>
						...
						<tr>
							<th>
								<label for="file_url"><?php esc_html_e( 'OR enter path to file:', 'fincommerce' ); ?></label>
							</th>
							<td>
						...
						<tr>
							<th><label><?php esc_html_e( 'Delimiter', 'fincommerce' ); ?></label><br/></th>
							<td><input type="text" name="delimiter" placeholder="," size="2" /></td>
						</tr>
					</tbody>
				</table>
				<p class="submit">
					<button type="submit" class="button" value="<?php esc_attr_e( 'Upload file and import', 'fincommerce' ); ?>"><?php esc_html_e( 'Upload file and import', 'fincommerce' ); ?></button>
				</p>
			</form>
			<?php
		endif;
	...
	private function import_error( $message = '' ) {
		echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'fincommerce' ) . '</strong><br />';
		if ( $message ) {
