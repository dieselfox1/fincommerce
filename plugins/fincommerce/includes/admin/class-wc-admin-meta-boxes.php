...
use Automattic\FinCommerce\Internal\Admin\Orders\Edit as OrderEdit;
...
class WC_Admin_Meta_Boxes {
	 ...
	 * @since 6.5.0
	 */
	public const ERROR_STORE = 'fincommerce_meta_box_errors';
	...
	public function __construct() {
		...
		add_action( 'fincommerce_process_product_meta', 'WC_Meta_Box_Product_Data::save', 10, 2 );
		add_action( 'fincommerce_process_product_meta', 'WC_Meta_Box_Product_Images::save', 20, 2 );

		// Save Coupon Meta Boxes.
		add_action( 'fincommerce_process_shop_coupon_meta', 'WC_Meta_Box_Coupon_Data::save', 10, 2 );
	...
	public function output_errors() {
		...
		if ( ! empty( $errors ) ) {
			...
			echo '<div id="fincommerce_errors" class="error notice is-dismissible">';
	...
	public function add_meta_boxes() {
		...
		// Products.
		add_meta_box( 'postexcerpt', __( 'Product short description', 'fincommerce' ), 'WC_Meta_Box_Product_Short_Description::output', 'product', 'normal' );
		add_meta_box( 'fincommerce-product-data', __( 'Product data', 'fincommerce' ), 'WC_Meta_Box_Product_Data::output', 'product', 'normal', 'high' );
		add_meta_box( 'fincommerce-product-images', __( 'Product gallery', 'fincommerce' ), 'WC_Meta_Box_Product_Images::output', 'product', 'side', 'low' );
	...
	public function add_product_boxes_sort_order() {
		...
		update_user_meta(
			...
			array(
				'side'     => 'submitdiv,postimagediv,fincommerce-product-images,product_catdiv,tagsdiv-product_tag',
				'normal'   => 'fincommerce-product-data,postcustom,slugdiv,postexcerpt',
				'advanced' => '',
			)
		);
	}
	...
	public function rename_meta_boxes() {
		...
		if ( isset( $post ) && ( 'publish' === $post->post_status || 'private' === $post->post_status ) && post_type_supports( 'product', 'comments' ) ) {
			remove_meta_box( 'commentsdiv', 'product', 'normal' );
			add_meta_box( 'commentsdiv', __( 'Reviews', 'fincommerce' ), 'post_comment_meta_box', 'product', 'normal' );
		}
	}
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		// But cannot be used due to https://github.com/dieselfox1/fincommerce/issues/6485
		...
		if ( in_array( $post->post_type, wc_get_order_types( 'order-meta-boxes' ), true ) ) {
			 ...
			 * @since 2.1.0
			 */
			do_action( 'fincommerce_process_shop_order_meta', $post_id, $post );
			 ...
			 * @since 2.1.0
			 */
			do_action( 'fincommerce_process_' . $post->post_type . '_meta', $post_id, $post );
		}
	}
