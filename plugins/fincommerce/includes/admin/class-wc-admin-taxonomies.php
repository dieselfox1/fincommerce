...
class WC_Admin_Taxonomies {
	...
	public function add_category_fields() {
		...
		<div class="form-field term-display-type-wrap">
			<label for="display_type"><?php esc_html_e( 'Display type', 'fincommerce' ); ?></label>
			<select id="display_type" name="display_type" class="postform">
				<option value=""><?php esc_html_e( 'Default', 'fincommerce' ); ?></option>
				<option value="products"><?php esc_html_e( 'Products', 'fincommerce' ); ?></option>
				<option value="subcategories"><?php esc_html_e( 'Subcategories', 'fincommerce' ); ?></option>
				<option value="both"><?php esc_html_e( 'Both', 'fincommerce' ); ?></option>
			</select>
		</div>
			...
			<script type="text/javascript">
				...
				jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
					...
					file_frame = wp.media.frames.downloadable_file = wp.media({
						title: '<?php esc_html_e( 'Choose an image', 'fincommerce' ); ?>',
						button: {
							text: '<?php esc_html_e( 'Use image', 'fincommerce' ); ?>'
						},
						multiple: false
					});
	...
	public function edit_category_fields( $term ) {
		...
		<tr class="form-field term-display-type-wrap">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Display type', 'fincommerce' ); ?></label></th>
			<td>
				<select id="display_type" name="display_type" class="postform">
					<option value="" <?php selected( '', $display_type ); ?>><?php esc_html_e( 'Default', 'fincommerce' ); ?></option>
					<option value="products" <?php selected( 'products', $display_type ); ?>><?php esc_html_e( 'Products', 'fincommerce' ); ?></option>
					<option value="subcategories" <?php selected( 'subcategories', $display_type ); ?>><?php esc_html_e( 'Subcategories', 'fincommerce' ); ?></option>
					<option value="both" <?php selected( 'both', $display_type ); ?>><?php esc_html_e( 'Both', 'fincommerce' ); ?></option>
				</select>
			</td>
		</tr>
	...
	public function product_cat_description() {
		echo wp_kses(
			wpautop( __( 'Product categories for your store can be managed here. To change the order of categories on the front-end you can drag and drop to sort them. To see more categories listed click the "screen options" link at the top-right of this page.', 'fincommerce' ) ),
			array( 'p' => array() )
		);
	}
	...
	public function product_cat_notes() {
		...
		$category_name = ( ! $category || is_wp_error( $category ) ) ? _x( 'Uncategorized', 'Default category slug', 'fincommerce' ) : $category->name;
		?>
		<div class="form-wrap edit-term-notes">
			<p>
				<strong><?php esc_html_e( 'Note:', 'fincommerce' ); ?></strong><br>
				<?php
					printf(
						/* translators: %s: default category */
						esc_html__( 'Deleting a category does not delete the products in that category. Instead, products that were only assigned to the deleted category are set to the category %s.', 'fincommerce' ),
	...
	public function product_attribute_description() {
		echo wp_kses(
			wpautop( __( 'Attribute terms can be assigned to products and variations.<br/><br/><b>Note</b>: Deleting a term will remove it from all products and variations to which it has been assigned. Recreating a term will not automatically assign it back to products.', 'fincommerce' ) ),
			array( 'p' => array() )
		);
	}
	...
	public function product_cat_row_actions( $actions, $term ) {
		...
		if ( $default_category_id !== $term->term_id && current_user_can( 'edit_term', $term->term_id ) ) {
			$actions['make_default'] = sprintf(
				...
				esc_attr( sprintf( __( 'Make &#8220;%s&#8221; the default category', 'fincommerce' ), $term->name ) ),
				__( 'Make default', 'fincommerce' )
			);
		}
	...
	public function product_cat_column( $columns, $column, $id ) {
		if ( 'thumb' === $column ) {
			...
			if ( $default_category_id === $id ) {
				$columns .= wc_help_tip( __( 'This is the default category and it cannot be deleted. It will be automatically assigned to products with no category.', 'fincommerce' ) );
			}
