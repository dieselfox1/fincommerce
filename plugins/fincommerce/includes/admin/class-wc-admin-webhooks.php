<?php
/**
 * FinCommerce Admin Webhooks Class
 *
...
class WC_Admin_Webhooks {
	...
	public function __construct() {
		...
		add_action( 'fincommerce_settings_page_init', array( $this, 'screen_option' ) );
		add_filter( 'fincommerce_save_settings_advanced_webhooks', array( $this, 'allow_save_settings' ) );
	}
	...
	private function save() {
		...
		if ( ! current_user_can( 'manage_fincommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to update Webhooks', 'fincommerce' ) );
		}
		...
		if ( ! empty( $_POST['webhook_name'] ) ) {
			...
			$name = sprintf(
				/* translators: %s: date */
				__( 'Webhook created on %s', 'fincommerce' ),
				// @codingStandardsIgnoreStart
				(new DateTime('now'))->format( _x( 'M d, Y @ h:i A', 'Webhook created on date parsed by DateTime::format', 'fincommerce' ) )
		...
		if ( ! empty( $_POST['webhook_topic'] ) ) {
			...
			if ( wc_is_webhook_valid_topic( $topic ) ) {
				...
				$errors[] = __( 'Webhook topic unknown. Please select a valid topic.', 'fincommerce' );
			}
		}
		...
		// Run actions.
		do_action( 'fincommerce_webhook_options_save', $webhook->get_id() );
		if ( $errors ) {
	...
	public function screen_option() {
		...
		if ( ! isset( $_GET['edit-webhook'] ) && $this->is_webhook_settings_page() ) {
			...
			add_screen_option(
				...
				array(
					'default' => 10,
					'option'  => 'fincommerce_webhooks_per_page',
				)
			);
		}
	}
	...
	private static function table_list_output() {
		...
		echo '<h2 class="wc-table-list-header">' . esc_html__( 'Webhooks', 'fincommerce' ) . ' <a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=advanced&section=webhooks&edit-webhook=0' ) ) . '" class="page-title-action">' . esc_html__( 'Add webhook', 'fincommerce' ) . '</a></h2>';
		...
		if ( $count > 0 ) {
			...
			$webhooks_table_list->search_box( __( 'Search webhooks', 'fincommerce' ), 'webhook' );
			$webhooks_table_list->display();
		} else {
			echo '<div class="fincommerce-BlankState fincommerce-BlankState--webhooks">';
			?>
			<h2 class="fincommerce-BlankState-message"><?php esc_html_e( 'Webhooks are event notifications sent to URLs of your choice. They can be used to integrate with third-party services which support them.', 'fincommerce' ); ?></h2>
			<a class="fincommerce-BlankState-cta button-primary button" href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=advanced&section=webhooks&edit-webhook=0' ) ); ?>"><?php esc_html_e( 'Create a new webhook', 'fincommerce' ); ?></a>
	...
	private static function maybe_display_legacy_rest_api_warning() {
		...
		<div class='error inline'>
			<p><strong>
				<?php echo esc_html__( 'Incompatible webhooks warning', 'fincommerce' ); ?>
			</strong></p>
			<p>
				<?php
				echo wp_kses_data(
					sprintf(
						...
						_n(
							"There's %d webhook that is configured to be delivered using the Legacy REST API, which has been removed from FinCommerce. This webhook will fail to be sent.",
							'There are %d webhooks that are configured to be delivered using the Legacy REST API, which has been removed from FinCommerce. These webhooks will fail to be sent.',
			...
			<p>
				...
				echo wp_kses(
					sprintf(
						/* translators: %s = URL */
						_n(
							'This webhook has the ⚠️ symbol in front of its name in the list below. Please either edit the webhook to use a different delivery format, or install and activate <a href="%s" target="_blank">the FinCommerce Legacy REST API extension</a>.',
							'These webhooks have the ⚠️ symbol in front of their names in the list below. Please either edit the webhooks to use a different delivery format, or install and activate <a href="%s" target="_blank">the FinCommerce Legacy REST API extension</a>.',
							$legacy_api_webhooks_count,
							'fincommerce'
						),
			...
			<p><strong>
				...
				echo wp_kses_data(
					sprintf(
						...
						__( "<a href='%s'>More information</a>", 'fincommerce' ),
