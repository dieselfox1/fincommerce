...
class WC_Admin_Marketplace_Promotions {
	...
	const CRON_NAME           = 'fincommerce_marketplace_cron_fetch_promotions';
	const TRANSIENT_NAME      = 'fincommerce_marketplace_promotions_v2';
	const TRANSIENT_LIFE_SPAN = DAY_IN_SECONDS;
	const PROMOTIONS_API_URL  = 'https://fincommerce.com/wp-json/wccom-extensions/3.0/promotions';
	 ...
	 * Shows notice and adds menu badge to FinCommerce Extensions item
	...
	public static function init() {
		...
		add_action( 'fincommerce_marketplace_fetch_promotions', array( __CLASS__, 'clear_deprecated_action' ) );
		add_action(
			'fincommerce_marketplace_fetch_promotions_clear',
			array(
				__CLASS__,
	 ...
	 * Use `fincommerce_marketplace_suppress_promotions` filter to suppress promotions.
	 *
	 * @since 9.0
	 */
	public static function get_active_promotions() {
		 ...
		 * @since 8.8
		 */
		if ( apply_filters( 'fincommerce_marketplace_suppress_promotions', false ) ) {
			return array();
		}
	...
	private static function fetch_marketplace_promotions() {
		...
		if ( is_wp_error( $raw_promotions ) ) {
			 ...
			 * @since 8.7
			 */
			do_action( 'fincommerce_page_wc-addons_connection_error', $raw_promotions->get_error_message() );
		}
		...
		if ( 200 !== $response_code ) {
			 ...
			 * @since 8.7
			 */
			do_action( 'fincommerce_page_wc-addons_connection_error', $response_code );
		}
	 ...
	 * FinCommerce menu.
	 *
	 * Use `fincommerce_marketplace_suppress_promotions` filter to suppress the bubble.
	 *
	 * @return void
	...
	private static function maybe_show_bubble_promotions() {
		...
		foreach ( $bubble_promotions as $promotion ) {
			...
			if ( $now_date_time < $date_to_gmt ) {
				add_filter(
					'fincommerce_marketplace_menu_items',
	...
	/**
	 * Callback for the `fincommerce_marketplace_menu_items` filter
	 * in `Automattic\FinCommerce\Internal\Admin\Marketplace::get_marketplace_pages`.
	 ...
	 * @param ?array $promotion   Data about a promotion from the FinCommerce.com API.
	 *
	 * @return array
	 */
	public static function filter_marketplace_menu_items( $menu_items, $promotion = array() ): array {
		...
		foreach ( $menu_items as $index => $menu_item ) {
			if (
				'fincommerce' === $menu_item['parent']
				&& $promotion['menu_item_id'] === $menu_item['id']
			) {
				$bubble_text                   = $promotion['content'][ self::$locale ] ?? ( $promotion['content']['en_US'] ?? __( 'Sale', 'fincommerce' ) );
	...
	private static function append_bubble( string $menu_item_text, string $bubble_text ): string {
		...
		return $menu_item_text
			. '<span class="awaiting-mod update-plugins remaining-tasks-badge fincommerce-task-list-remaining-tasks-badge">'
			. esc_html( $bubble_text )
			. '</span>';
	}

	/**
	 * When FinCommerce is disabled, clear the WP Cron event we use to fetch promotions.
	 *
	 * @version 9.5.0
	 *
	 * @return void
	...
	/**
	 * Clear deprecated scheduled action that was used to fetch promotions in FinCommerce 8.8.
	 * Replaced with a transient in FinCommerce 9.0.
	 *
	 * @return void
	 */
	public static function clear_deprecated_scheduled_event() {
		if ( function_exists( 'as_unschedule_all_actions' ) ) {
			as_unschedule_all_actions( 'fincommerce_marketplace_fetch_promotions' );
		}
	}
	 ...
	 * `fincommerce_marketplace_fetch_promotions` action.
	 */
	public static function clear_deprecated_action() {
		if ( function_exists( 'as_schedule_single_action' ) ) {
			as_schedule_single_action( time(), 'fincommerce_marketplace_fetch_promotions_clear' );
		}
	}
}

// Fetch list of promotions from FinCommerce.com for FinCommerce admin UI.
