...
class WC_Admin_Post_Types {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_order_updated_messages', array( $this, 'order_updated_messages' ) );
		...
		add_action( 'fincommerce_product_bulk_and_quick_edit', array( $this, 'bulk_and_quick_edit_save_post' ), 10, 2 );
	}
	...
	public function post_updated_messages( $messages ) {
		...
		$messages['product'] = array(
			...
			1  => sprintf( __( 'Product updated. %1$sView Product%2$s', 'fincommerce' ), '<a id="fincommerce-product-updated-message-view-product__link" href="' . esc_url( get_permalink( $post->ID ) ) . '">', '</a>' ),
			2  => __( 'Custom field updated.', 'fincommerce' ),
			3  => __( 'Custom field deleted.', 'fincommerce' ),
			4  => __( 'Product updated.', 'fincommerce' ),
			5  => __( 'Revision restored.', 'fincommerce' ),
			/* translators: %1$s: Product link opening tag. %2$s: Product link closing tag.*/
			6  => sprintf( __( 'Product published. %1$sView Product%2$s', 'fincommerce' ), '<a id="fincommerce-product-updated-message-view-product__link" href="' . esc_url( get_permalink( $post->ID ) ) . '">', '</a>' ),
			7  => __( 'Product saved.', 'fincommerce' ),
			/* translators: %s: product url */
			8  => sprintf( __( 'Product submitted. <a target="_blank" href="%s">Preview product</a>', 'fincommerce' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
			9  => sprintf(
				/* translators: 1: date 2: product url */
				__( 'Product scheduled for: %1$s. <a target="_blank" href="%2$s">Preview product</a>', 'fincommerce' ),
				'<strong>' . date_i18n( __( 'M j, Y @ G:i', 'fincommerce' ), strtotime( $post->post_date ) ) . '</strong>',
				esc_url( get_permalink( $post->ID ) )
			),
			/* translators: %s: product url */
			10 => sprintf( __( 'Product draft updated. <a target="_blank" href="%s">Preview product</a>', 'fincommerce' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
		);
		...
		$messages['shop_coupon'] = array(
			0  => '', // Unused. Messages start at index 1.
			1  => __( 'Coupon updated.', 'fincommerce' ),
			2  => __( 'Custom field updated.', 'fincommerce' ),
			3  => __( 'Custom field deleted.', 'fincommerce' ),
			4  => __( 'Coupon updated.', 'fincommerce' ),
			5  => __( 'Revision restored.', 'fincommerce' ),
			6  => __( 'Coupon updated.', 'fincommerce' ),
			7  => __( 'Coupon saved.', 'fincommerce' ),
			8  => __( 'Coupon submitted.', 'fincommerce' ),
			9  => sprintf(
				/* translators: %s: date */
				__( 'Coupon scheduled for: %s.', 'fincommerce' ),
				'<strong>' . date_i18n( __( 'M j, Y @ G:i', 'fincommerce' ), strtotime( $post->post_date ) ) . '</strong>'
			),
			10 => __( 'Coupon draft updated.', 'fincommerce' ),
		);
	...
	public function order_updated_messages( array $messages ) {
		...
		if ( ! isset( $theorder ) || ! $theorder instanceof WC_Abstract_Order ) {
			if ( ! isset( $post ) || 'shop_order' !== $post->post_type ) {
				...
				\Automattic\FinCommerce\Utilities\OrderUtil::init_theorder_object( $post );
			}
		}

		$messages['shop_order'] = array(
			0  => '', // Unused. Messages start at index 1.
			1  => __( 'Order updated.', 'fincommerce' ),
			2  => __( 'Custom field updated.', 'fincommerce' ),
			3  => __( 'Custom field deleted.', 'fincommerce' ),
			4  => __( 'Order updated.', 'fincommerce' ),
			5  => __( 'Revision restored.', 'fincommerce' ),
			6  => __( 'Order updated.', 'fincommerce' ),
			7  => __( 'Order saved.', 'fincommerce' ),
			8  => __( 'Order submitted.', 'fincommerce' ),
			9  => sprintf(
			/* translators: %s: date */
				__( 'Order scheduled for: %s.', 'fincommerce' ),
				'<strong>' . date_i18n( __( 'M j, Y @ G:i', 'fincommerce' ), strtotime( $theorder->get_date_created() ?? $post->post_date ) ) . '</strong>'
			),
			10 => __( 'Order draft updated.', 'fincommerce' ),
			11 => __( 'Order updated and sent.', 'fincommerce' ),
		);
	...
	public function bulk_post_updated_messages( $bulk_messages, $bulk_counts ) {
		$bulk_messages['product'] = array(
			...
			'updated'   => _n( '%s product updated.', '%s products updated.', $bulk_counts['updated'], 'fincommerce' ),
			/* translators: %s: product count */
			'locked'    => _n( '%s product not updated, somebody is editing it.', '%s products not updated, somebody is editing them.', $bulk_counts['locked'], 'fincommerce' ),
			/* translators: %s: product count */
			'deleted'   => _n( '%s product permanently deleted.', '%s products permanently deleted.', $bulk_counts['deleted'], 'fincommerce' ),
			/* translators: %s: product count */
			'trashed'   => _n( '%s product moved to the Trash.', '%s products moved to the Trash.', $bulk_counts['trashed'], 'fincommerce' ),
			/* translators: %s: product count */
			'untrashed' => _n( '%s product restored from the Trash.', '%s products restored from the Trash.', $bulk_counts['untrashed'], 'fincommerce' ),
		);

		$bulk_messages['shop_order'] = array(
			/* translators: %s: order count */
			'updated'   => _n( '%s order updated.', '%s orders updated.', $bulk_counts['updated'], 'fincommerce' ),
			/* translators: %s: order count */
			'locked'    => _n( '%s order not updated, somebody is editing it.', '%s orders not updated, somebody is editing them.', $bulk_counts['locked'], 'fincommerce' ),
			/* translators: %s: order count */
			'deleted'   => _n( '%s order permanently deleted.', '%s orders permanently deleted.', $bulk_counts['deleted'], 'fincommerce' ),
			/* translators: %s: order count */
			'trashed'   => _n( '%s order moved to the Trash.', '%s orders moved to the Trash.', $bulk_counts['trashed'], 'fincommerce' ),
			/* translators: %s: order count */
			'untrashed' => _n( '%s order restored from the Trash.', '%s orders restored from the Trash.', $bulk_counts['untrashed'], 'fincommerce' ),
		);

		$bulk_messages['shop_coupon'] = array(
			/* translators: %s: coupon count */
			'updated'   => _n( '%s coupon updated.', '%s coupons updated.', $bulk_counts['updated'], 'fincommerce' ),
			/* translators: %s: coupon count */
			'locked'    => _n( '%s coupon not updated, somebody is editing it.', '%s coupons not updated, somebody is editing them.', $bulk_counts['locked'], 'fincommerce' ),
			/* translators: %s: coupon count */
			'deleted'   => _n( '%s coupon permanently deleted.', '%s coupons permanently deleted.', $bulk_counts['deleted'], 'fincommerce' ),
			/* translators: %s: coupon count */
			'trashed'   => _n( '%s coupon moved to the Trash.', '%s coupons moved to the Trash.', $bulk_counts['trashed'], 'fincommerce' ),
			/* translators: %s: coupon count */
			'untrashed' => _n( '%s coupon restored from the Trash.', '%s coupons restored from the Trash.', $bulk_counts['untrashed'], 'fincommerce' ),
		);
	...
	private function maybe_display_warning_for_password_protected_coupon() {
		...
		wp_admin_notice(
			__(
				'This coupon is password protected. FinCommerce does not support password protection for coupons. You can temporarily hide a coupon by making it private. Alternatively, usage limits and restrictions can be configured below.',
				'fincommerce'
			),
			array(
				'type'               => 'warning',
	...
	public function bulk_and_quick_edit_hook( $post_id, $post ) {
		...
		do_action( 'fincommerce_product_bulk_and_quick_edit', $post_id, $post );
		add_action( 'save_post', array( $this, 'bulk_and_quick_edit_hook' ), 10, 2 );
	}
	...
	public function bulk_and_quick_edit_save_post( $post_id, $post ) {
		...
		if ( ! empty( $request_data['fincommerce_quick_edit'] ) ) { // WPCS: input var ok.
	...
	private function quick_edit_save( $post_id, $product ) {
		...
		do_action( 'fincommerce_product_quick_edit_save', $product );
	}
	...
	public function bulk_edit_save( $post_id, $product ) {
		 ...
		 * @since 3.0.0
		 */
		$change_price_product_types    = apply_filters( 'fincommerce_bulk_edit_save_price_product_types', array( ProductType::SIMPLE, ProductType::EXTERNAL ) );
		...
		if ( 'yes' === get_option( 'fincommerce_manage_stock' ) ) {
			...
			// Reset values if FinCommerce Setting - Manage Stock status is disabled.
	...
	public function enter_title_here( $text, $post ) {
		switch ( $post->post_type ) {
			case 'product':
				$text = esc_html__( 'Product name', 'fincommerce' );
				break;
			case 'shop_coupon':
				$text = esc_html__( 'Coupon code', 'fincommerce' );
				break;
		}
		return $text;
	}
	...
	public function product_data_visibility() {
		...
		<div class="misc-pub-section" id="catalog-visibility">
			<?php esc_html_e( 'Catalog visibility:', 'fincommerce' ); ?>
			<strong id="catalog-visibility-display">
				...
				if ( 'yes' === $current_featured ) {
					echo ', ' . esc_html__( 'Featured', 'fincommerce' );
				}
				?>
			</strong>

			<a href="#catalog-visibility" class="edit-catalog-visibility hide-if-no-js"><?php esc_html_e( 'Edit', 'fincommerce' ); ?></a>
			...
			<div id="catalog-visibility-select" class="hide-if-js">
				...
				<?php
				echo '<p>' . esc_html__( 'This setting determines which shop pages products will be listed on.', 'fincommerce' ) . '</p>';

				foreach ( $visibility_options as $name => $label ) {
					echo '<input type="radio" name="_visibility" id="_visibility_' . esc_attr( $name ) . '" value="' . esc_attr( $name ) . '" ' . checked( $current_visibility, $name, false ) . ' data-label="' . esc_attr( $label ) . '" /> <label for="_visibility_' . esc_attr( $name ) . '" class="selectit">' . esc_html( $label ) . '</label><br />';
				}

				echo '<br /><input type="checkbox" name="_featured" id="_featured" ' . checked( $current_featured, 'yes', false ) . ' /> <label for="_featured">' . esc_html__( 'This is a featured product', 'fincommerce' ) . '</label><br />';
				?>
				<p>
					<a href="#catalog-visibility" class="save-post-visibility hide-if-no-js button"><?php esc_html_e( 'OK', 'fincommerce' ); ?></a>
					<a href="#catalog-visibility" class="cancel-post-visibility hide-if-no-js"><?php esc_html_e( 'Cancel', 'fincommerce' ); ?></a>
				</p>
			</div>
		</div>
		<?php
	...
	public function show_cpt_archive_notice( $post ) {
		...
		if ( $post && absint( $post->ID ) === $shop_page_id ) {
			...
			echo '<p>' . sprintf( wp_kses_post( __( 'This is the FinCommerce shop page. The shop page is a special archive that lists your products. <a href="%s">You can read more about this here</a>.', 'fincommerce' ) ), 'https://fincommerce.com/document/fincommerce-pages/#section-4' ) . '</p>';
			echo '</div>';
		}
	}
	...
	public function add_display_post_states( $post_states, $post ) {
		if ( wc_get_page_id( 'shop' ) === $post->ID ) {
			$post_states['wc_page_for_shop'] = __( 'Shop Page', 'fincommerce' );
		}

		if ( wc_get_page_id( 'cart' ) === $post->ID ) {
			$post_states['wc_page_for_cart'] = __( 'Cart Page', 'fincommerce' );
		}

		if ( wc_get_page_id( 'checkout' ) === $post->ID ) {
			$post_states['wc_page_for_checkout'] = __( 'Checkout Page', 'fincommerce' );
		}

		if ( wc_get_page_id( 'myaccount' ) === $post->ID ) {
			$post_states['wc_page_for_myaccount'] = __( 'My Account Page', 'fincommerce' );
		}

		if ( wc_get_page_id( 'terms' ) === $post->ID ) {
			$post_states['wc_page_for_terms'] = __( 'Terms and Conditions Page', 'fincommerce' );
		}
