...
class WC_Admin_Setup_Wizard {
	 ...
	 * and the store doesn't already have a FinCommerce theme.
	 *
	 * @deprecated 4.6.0
	 * @return boolean
	 */
	protected function should_show_theme() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		$support_fincommerce = current_theme_supports( 'fincommerce' ) && ! wc_is_wp_default_theme_active();
	...
	/**
	 * Is the FinCommerce Admin actively included in the FinCommerce core?
	...
	/**
	 * Should we show the new FinCommerce Admin onboarding experience?
	 *
	 * @deprecated 4.6.0
	 * @return boolean
	 */
	protected function should_show_wc_admin_onboarding() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		// As of FinCommerce 4.1, all new sites should use the latest OBW from wc-admin package.
		// This filter will allow for forcing the old wizard while we migrate e2e tests.
		return ! apply_filters( 'fincommerce_setup_wizard_force_legacy', false );
	}
	...
	public function setup_wizard() {
		...
		if ( 'virtual' === get_option( 'fincommerce_product_type' ) ) {
		...
		$this->steps = apply_filters( 'fincommerce_setup_wizard_steps', $default_steps );
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			<title><?php esc_html_e( 'FinCommerce &rsaquo; Setup Wizard', 'fincommerce' ); ?></title>
		...
		<h1 class="wc-logo"><a href="https://fincommerce.com/"><img src="<?php echo esc_url( WC()->plugin_url() ); ?>/assets/images/fincommerce_logo.png" alt="<?php esc_attr_e( 'FinCommerce', 'fincommerce' ); ?>" /></a></h1>
		<?php
	}
	...
	public function setup_wizard_footer() {
		...
		?>
			<?php if ( 'new_onboarding' === $current_step || 'store-setup' === $current_step ) : ?>
				<a class="wc-setup-footer-links" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Not right now', 'fincommerce' ); ?></a>
			<?php elseif ( 'recommended' === $current_step || 'activate' === $current_step ) : ?>
				<a class="wc-setup-footer-links" href="<?php echo esc_url( $this->get_next_step_link() ); ?>"><?php esc_html_e( 'Skip this step', 'fincommerce' ); ?></a>
			<?php endif; ?>
			<?php do_action( 'fincommerce_setup_footer' ); ?>
			</body>
		</html>
		<?php
	}
	...
	public function setup_wizard_steps() {
		...
		// Hide the activate step if Jetpack is already active, unless FinCommerce Services
	...
	/**
	 * Display's a prompt for users to try out the new improved FinCommerce onboarding experience in FinCommerce Admin.
	 *
	 * @deprecated 4.6.0
	 */
	public function wc_setup_new_onboarding() {
		...
		?>
			<div class="wc-setup-step__new_onboarding-wrapper">
				<p class="wc-setup-step__new_onboarding-welcome"><?php esc_html_e( 'Welcome to', 'fincommerce' ); ?></p>
				<h1 class="wc-logo"><a href="https://fincommerce.com/"><img src="<?php echo esc_url( WC()->plugin_url() ); ?>/assets/images/fincommerce_logo.png" alt="<?php esc_attr_e( 'FinCommerce', 'fincommerce' ); ?>" /></a></h1>
				<p><?php esc_html_e( 'Get your store up and running more quickly with our new and improved setup experience', 'fincommerce' ); ?></p>

				<form method="post" class="activate-new-onboarding">
					<?php wp_nonce_field( 'wc-setup' ); ?>
					<input type="hidden" name="save_step" value="new_onboarding" />
					<p class="wc-setup-actions step">
						<button class="button-primary button button-large" value="<?php esc_attr_e( 'Yes please', 'fincommerce' ); ?>" name="save_step"><?php esc_html_e( 'Yes please', 'fincommerce' ); ?></button>
					</p>
				</form>
				<?php if ( ! $this->is_wc_admin_active() ) : ?>
					<p class="wc-setup-step__new_onboarding-plugin-info"><?php esc_html_e( 'The "FinCommerce Admin" plugin will be installed and activated', 'fincommerce' ); ?></p>
				<?php endif; ?>
			</div>
		<?php
	}
	...
	/**
	 * Installs FinCommerce admin and redirects to the new onboarding experience.
	 *
	 * @deprecated 4.6.0
	 */
	...
	public function wc_setup_store_setup() {
		...
		$currency       = get_option( 'fincommerce_currency', 'USD' );
		$product_type   = get_option( 'fincommerce_product_type', 'both' );
		$sell_in_person = get_option( 'fincommerce_sell_in_person', 'none_selected' );
		...
		<form method="post" class="address-step">
			...
			<?php wp_nonce_field( 'wc-setup' ); ?>
			<p class="store-setup"><?php esc_html_e( 'The following wizard will help you configure your store and get you started quickly.', 'fincommerce' ); ?></p>

			<div class="store-address-container">

				<label for="store_country" class="location-prompt"><?php esc_html_e( 'Where is your store based?', 'fincommerce' ); ?></label>
				<select id="store_country" name="store_country" required data-placeholder="<?php esc_attr_e( 'Choose a country / region&hellip;', 'fincommerce' ); ?>" aria-label="<?php esc_attr_e( 'Country / Region', 'fincommerce' ); ?>" class="location-input wc-enhanced-select dropdown">
				...
				<label class="location-prompt" for="store_address"><?php esc_html_e( 'Address', 'fincommerce' ); ?></label>
				<input type="text" id="store_address" class="location-input" name="store_address" required value="<?php echo esc_attr( $address ); ?>" />

				<label class="location-prompt" for="store_address_2"><?php esc_html_e( 'Address line 2', 'fincommerce' ); ?></label>
				...
				<div class="city-and-postcode">
					<div>
						<label class="location-prompt" for="store_city"><?php esc_html_e( 'City', 'fincommerce' ); ?></label>
					...
					<div class="store-state-container hidden">
						<label for="store_state" class="location-prompt">
							<?php esc_html_e( 'State', 'fincommerce' ); ?>
						</label>
						<select id="store_state" name="store_state" data-placeholder="<?php esc_attr_e( 'Choose a state&hellip;', 'fincommerce' ); ?>" aria-label="<?php esc_attr_e( 'State', 'fincommerce' ); ?>" class="location-input wc-enhanced-select dropdown"></select>
					</div>
					<div>
						<label class="location-prompt" for="store_postcode"><?php esc_html_e( 'Postcode / ZIP', 'fincommerce' ); ?></label>
			...
			<label class="location-prompt" for="currency_code">
				<?php esc_html_e( 'What currency do you accept payments in?', 'fincommerce' ); ?>
			</label>
			<select
				id="currency_code"
				name="currency_code"
				required
				data-placeholder="<?php esc_attr_e( 'Choose a currency&hellip;', 'fincommerce' ); ?>"
				class="location-input wc-enhanced-select dropdown"
			>
				<option value=""><?php esc_html_e( 'Choose a currency&hellip;', 'fincommerce' ); ?></option>
				<?php foreach ( get_fincommerce_currencies() as $code => $name ) : ?>
					<option value="<?php echo esc_attr( $code ); ?>" <?php selected( $currency, $code ); ?>>
						<?php
						$symbol = get_fincommerce_currency_symbol( $code );

						if ( $symbol === $code ) {
							/* translators: 1: currency name 2: currency code */
							echo esc_html( sprintf( __( '%1$s (%2$s)', 'fincommerce' ), $name, $code ) );
						} else {
							/* translators: 1: currency name 2: currency symbol, 3: currency code */
							echo esc_html( sprintf( __( '%1$s (%2$s %3$s)', 'fincommerce' ), $name, get_fincommerce_currency_symbol( $code ), $code ) );
						}
						?>
					</option>
				<?php endforeach; ?>
			...
			<label class="location-prompt" for="product_type">
				<?php esc_html_e( 'What type of products do you plan to sell?', 'fincommerce' ); ?>
			</label>
			<select id="product_type" name="product_type" required class="location-input wc-enhanced-select dropdown">
				<option value="both" <?php selected( $product_type, 'both' ); ?>><?php esc_html_e( 'I plan to sell both physical and digital products', 'fincommerce' ); ?></option>
				<option value="physical" <?php selected( $product_type, 'physical' ); ?>><?php esc_html_e( 'I plan to sell physical products', 'fincommerce' ); ?></option>
				<option value="virtual" <?php selected( $product_type, 'virtual' ); ?>><?php esc_html_e( 'I plan to sell digital products', 'fincommerce' ); ?></option>
			</select>
			</div>
			...
			<input
				type="checkbox"
				id="fincommerce_sell_in_person"
				name="sell_in_person"
				value="yes"
				<?php checked( $sell_in_person, true ); ?>
			/>
			<label class="location-prompt" for="fincommerce_sell_in_person">
				<?php esc_html_e( 'I will also be selling products or services in person.', 'fincommerce' ); ?>
			</label>
			</div>

			<input type="checkbox" id="wc_tracker_checkbox" name="wc_tracker_checkbox" value="yes" <?php checked( 'yes', get_option( 'fincommerce_allow_tracking', 'no' ) ); ?> />

			<?php $this->tracking_modal(); ?>

			<p class="wc-setup-actions step">
				<button class="button-primary button button-large" value="<?php esc_attr_e( "Let's go!", 'fincommerce' ); ?>" name="save_step"><?php esc_html_e( "Let's go!", 'fincommerce' ); ?></button>
			</p>
		</form>
		<?php
	}
	...
	public function tracking_modal() {
		...
		<script type="text/template" id="tmpl-wc-modal-tracking-setup">
			<div class="wc-backbone-modal fincommerce-tracker">
				<div class="wc-backbone-modal-content">
					<section class="wc-backbone-modal-main" role="main">
						<header class="wc-backbone-modal-header">
							<h1><?php esc_html_e( 'Help improve FinCommerce with usage tracking', 'fincommerce' ); ?></h1>
						</header>
						<article>
							<p>
							<?php
								printf(
									wp_kses(
										...
										__( 'Learn more about how usage tracking works, and how you\'ll be helping in our <a href="%1$s" target="_blank">usage tracking documentation</a>.', 'fincommerce' ),
										array(
											'a' => array(
												'href'   => array(),
												'target' => array(),
									...
									'https://fincommerce.com/usage-tracking/'
								);
							?>
							</p>
							<p class="fincommerce-tracker-checkbox">
								<input type="checkbox" id="wc_tracker_checkbox_dialog" name="wc_tracker_checkbox_dialog" value="yes" <?php checked( 'yes', get_option( 'fincommerce_allow_tracking', 'no' ) ); ?> />
								<label for="wc_tracker_checkbox_dialog"><?php esc_html_e( 'Enable usage tracking and help improve FinCommerce', 'fincommerce' ); ?></label>
							</p>
						</article>
						<footer>
							<div class="inner">
								<button class="button button-primary button-large" id="wc_tracker_submit" aria-label="<?php esc_attr_e( 'Continue', 'fincommerce' ); ?>"><?php esc_html_e( 'Continue', 'fincommerce' ); ?></button>
							</div>
						</footer>
					</section>
				</div>
	...
	public function run_deferred_actions() {
		...
		foreach ( $this->deferred_actions as $action ) {
			...
			if (
				...
				delete_option( 'fincommerce_setup_background_installing_' . $action['args'][0] );
			}
		}
	}
	...
	protected function install_plugin( $plugin_id, $plugin_info ) {
		...
		if ( get_option( 'fincommerce_setup_background_installing_' . $plugin_id ) ) {
			return;
		}
		...
		update_option( 'fincommerce_setup_background_installing_' . $plugin_id, true );
	}
	...
	/**
	 * Helper method to install FinCommerce Services and its Jetpack dependency.
	 *
	 * @deprecated 4.6.0
	 */
	protected function install_fincommerce_services() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		$this->install_jetpack();
		$this->install_plugin(
			'fincommerce-services',
			array(
				'name'      => __( 'FinCommerce Services', 'fincommerce' ),
				'repo-slug' => 'fincommerce-services',
			)
		);
	}

	/**
	 * Retrieve info for missing FinCommerce Services and/or Jetpack plugin.
	 *
	 * @deprecated 4.6.0
	 * @return array
	 */
	protected function get_wcs_requisite_plugins() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		$plugins = array();
		if ( ! is_plugin_active( 'fincommerce-services/fincommerce-services.php' ) && ! get_option( 'fincommerce_setup_background_installing_fincommerce-services' ) ) {
			$plugins[] = array(
				'name' => __( 'FinCommerce Services', 'fincommerce' ),
				'slug' => 'fincommerce-services',
			);
		}
		if ( ! is_plugin_active( 'jetpack/jetpack.php' ) && ! get_option( 'fincommerce_setup_background_installing_jetpack' ) ) {
			$plugins[] = array(
				'name' => __( 'Jetpack', 'fincommerce' ),
				'slug' => 'jetpack',
			);
	...
	public function plugin_install_info() {
		...
		<span class="plugin-install-info">
			<span class="plugin-install-info-label"><?php esc_html_e( 'The following plugins will be installed and activated for you:', 'fincommerce' ); ?></span>
	...
	protected function get_wizard_shipping_methods( $country_code, $currency_code ) {
		...
		$shipping_methods = array(
			'flat_rate'     => array(
				'name'        => __( 'Flat Rate', 'fincommerce' ),
				'description' => __( 'Set a fixed price to cover shipping costs.', 'fincommerce' ),
				'settings'    => array(
					'cost' => array(
						'type'          => 'text',
						'default_value' => __( 'Cost', 'fincommerce' ),
						'description'   => __( 'What would you like to charge for flat rate shipping?', 'fincommerce' ),
						'required'      => true,
					),
				),
			),
			'free_shipping' => array(
				'name'        => __( 'Free Shipping', 'fincommerce' ),
				'description' => __( "Don't charge for shipping.", 'fincommerce' ),
			),
		);
	...
	protected function get_product_weight_selection() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		$weight_unit = get_option( 'fincommerce_weight_unit' );
		ob_start();
		?>
		<span class="wc-setup-shipping-unit">
			<select id="weight_unit" name="weight_unit" class="wc-enhanced-select">
				<option value="kg" <?php selected( $weight_unit, 'kg' ); ?>><?php esc_html_e( 'Kilograms', 'fincommerce' ); ?></option>
				<option value="g" <?php selected( $weight_unit, 'g' ); ?>><?php esc_html_e( 'Grams', 'fincommerce' ); ?></option>
				<option value="lbs" <?php selected( $weight_unit, 'lbs' ); ?>><?php esc_html_e( 'Pounds', 'fincommerce' ); ?></option>
				<option value="oz" <?php selected( $weight_unit, 'oz' ); ?>><?php esc_html_e( 'Ounces', 'fincommerce' ); ?></option>
			</select>
		</span>
		<?php
	...
	protected function get_product_dimension_selection() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		$dimension_unit = get_option( 'fincommerce_dimension_unit' );
		ob_start();
		?>
		<span class="wc-setup-shipping-unit">
			<select id="dimension_unit" name="dimension_unit" class="wc-enhanced-select">
				<option value="m" <?php selected( $dimension_unit, 'm' ); ?>><?php esc_html_e( 'Meters', 'fincommerce' ); ?></option>
				<option value="cm" <?php selected( $dimension_unit, 'cm' ); ?>><?php esc_html_e( 'Centimeters', 'fincommerce' ); ?></option>
				<option value="mm" <?php selected( $dimension_unit, 'mm' ); ?>><?php esc_html_e( 'Millimeters', 'fincommerce' ); ?></option>
				<option value="in" <?php selected( $dimension_unit, 'in' ); ?>><?php esc_html_e( 'Inches', 'fincommerce' ); ?></option>
				<option value="yd" <?php selected( $dimension_unit, 'yd' ); ?>><?php esc_html_e( 'Yards', 'fincommerce' ); ?></option>
			</select>
		</span>
		<?php
	...
	public function wc_setup_shipping() {
		...
		$currency_code         = get_fincommerce_currency();
		...
		if ( empty( $existing_zones ) ) {
			$intro_text = sprintf(
				...
				__( "We've created two Shipping Zones - for %s and for the rest of the world. Below you can set Flat Rate shipping costs for these Zones or offer Free Shipping.", 'fincommerce' ),
		...
		?>
		<h1><?php esc_html_e( 'Shipping', 'fincommerce' ); ?></h1>
		<?php if ( $intro_text ) : ?>
		...
		<form method="post">
			<?php if ( $is_wcs_labels_supported || $is_shipstation_supported ) : ?>
				...
				if ( $is_wcs_labels_supported ) :
					$this->display_recommended_item(
						array(
							'type'        => 'fincommerce_services',
							'title'       => __( 'Did you know you can print shipping labels at home?', 'fincommerce' ),
							'description' => __( 'Use FinCommerce Shipping (powered by FinCommerce Services & Jetpack) to save time at the post office by printing your shipping labels at home.', 'fincommerce' ),
							'img_url'     => WC()->plugin_url() . '/assets/images/obw-fincommerce-services-icon.png',
							'img_alt'     => __( 'FinCommerce Services icon', 'fincommerce' ),
				...
				elseif ( $is_shipstation_supported ) :
					$this->display_recommended_item(
						array(
							'type'        => 'shipstation',
							'title'       => __( 'Did you know you can print shipping labels at home?', 'fincommerce' ),
							'description' => __( 'We recommend using ShipStation to save time at the post office by printing your shipping labels at home. Try ShipStation free for 30 days.', 'fincommerce' ),
							'img_url'     => WC()->plugin_url() . '/assets/images/obw-shipstation-icon.png',
							'img_alt'     => __( 'ShipStation icon', 'fincommerce' ),
							'plugins'     => array(
								array(
									'name' => __( 'ShipStation', 'fincommerce' ),
				...
				<ul class="wc-wizard-services shipping">
					<li class="wc-wizard-service-item">
						<div class="wc-wizard-service-name">
							<p><?php echo esc_html_e( 'Shipping Zone', 'fincommerce' ); ?></p>
						</div>
						<div class="wc-wizard-service-description">
							<p><?php echo esc_html_e( 'Shipping Method', 'fincommerce' ); ?></p>
						</div>
					</li>
					<li class="wc-wizard-service-item">
						<div class="wc-wizard-service-name">
					...
					<li class="wc-wizard-service-item">
						<div class="wc-wizard-service-name">
							<p><?php echo esc_html_e( 'Locations not covered by your other zones', 'fincommerce' ); ?></p>
						</div>
						<div class="wc-wizard-service-description">
					...
					<li class="wc-wizard-service-info">
						...
						printf(
							wp_kses(
								...
								__( 'If you\'d like to offer <span class="help_tip" data-tip="%1$s">live rates</span> from a specific carrier (e.g. UPS) you can find a variety of extensions available for FinCommerce <a href="%2$s" target="_blank">here</a>.', 'fincommerce' ),
								array(
									'span' => array(
										'class'    => array(),
										'data-tip' => array(),
							...
							esc_attr__( 'A live rate is the exact cost to ship an order, quoted directly from the shipping carrier.', 'fincommerce' ),
							'https://fincommerce.com/product-category/fincommerce-extensions/shipping-methods/shipping-carriers/'
						);
						?>
						</p>
					</li>
					...
					<?php
						echo wp_kses(
							sprintf(
								...
								esc_html__( 'We\'ll use %1$s for product weight and %2$s for product dimensions.', 'fincommerce' ),
			...
			<p class="wc-setup-actions step">
				...
				<button class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'fincommerce' ); ?>" name="save_step"><?php esc_html_e( 'Continue', 'fincommerce' ); ?></button>
				<?php wp_nonce_field( 'wc-setup' ); ?>
			</p>
		</form>
		<?php
	...
	/**
	 * Is FinCommerce Services shipping label country supported
	 *
	 * @param string $country_code Country code.
	 *
	 * @deprecated 4.6.0
	...
	protected function get_wizard_available_in_cart_payment_gateways() {
		...
		$stripe_description = '<p>' . sprintf(
			/* translators: %s: URL */
			__( 'Accept debit and credit cards in 135+ currencies, methods such as Alipay, and one-touch checkout with Apple Pay. <a href="%s" target="_blank">Learn more</a>.', 'fincommerce' ),
			'https://fincommerce.com/products/stripe/'
		) . '</p>';
		$paypal_checkout_description = '<p>' . sprintf(
			/* translators: %s: URL */
			__( 'Safe and secure payments using credit cards or your customer\'s PayPal account. <a href="%s" target="_blank">Learn more</a>.', 'fincommerce' ),
			'https://fincommerce.com/products/fincommerce-gateway-paypal-checkout/'
		) . '</p>';
		$klarna_checkout_description = '<p>' . sprintf(
			/* translators: %s: URL */
			__( 'Full checkout experience with pay now, pay later and slice it. No credit card numbers, no passwords, no worries. <a href="%s" target="_blank">Learn more about Klarna</a>.', 'fincommerce' ),
			'https://fincommerce.com/products/klarna-checkout/'
		) . '</p>';
		$klarna_payments_description = '<p>' . sprintf(
			/* translators: %s: URL */
			__( 'Choose the payment that you want, pay now, pay later or slice it. No credit card numbers, no passwords, no worries. <a href="%s" target="_blank">Learn more about Klarna</a>.', 'fincommerce' ),
			'https://fincommerce.com/products/klarna-payments/ '
		) . '</p>';
		$square_description = '<p>' . sprintf(
			/* translators: %s: URL */
			__( 'Securely accept credit and debit cards with one low rate, no surprise fees (custom rates available). Sell online and in store and track sales and inventory in one place. <a href="%s" target="_blank">Learn more about Square</a>.', 'fincommerce' ),
			'https://fincommerce.com/products/square/'
		) . '</p>';

		return array(
			'stripe'              => array(
				'name'        => __( 'FinCommerce Stripe Gateway', 'fincommerce' ),
				'image'       => WC()->plugin_url() . '/assets/images/stripe.png',
				'description' => $stripe_description,
				'class'       => 'checked stripe-logo',
				'repo-slug'   => 'fincommerce-gateway-stripe',
				'settings'    => array(
					'create_account' => array(
						'label'       => __( 'Set up Stripe for me using this email:', 'fincommerce' ),
						'type'        => 'checkbox',
						'value'       => 'yes',
						'default'     => 'yes',
						'placeholder' => '',
					...
					'email'          => array(
						'label'       => __( 'Stripe email address:', 'fincommerce' ),
						'type'        => 'email',
						'value'       => $user_email,
						'placeholder' => __( 'Stripe email address', 'fincommerce' ),
						'required'    => true,
					),
				),
			),
			'ppec_paypal'         => array(
				'name'        => __( 'FinCommerce PayPal Checkout Gateway', 'fincommerce' ),
				'image'       => WC()->plugin_url() . '/assets/images/paypal.png',
				'description' => $paypal_checkout_description,
				'enabled'     => false,
				'class'       => 'checked paypal-logo',
				'repo-slug'   => 'fincommerce-gateway-paypal-express-checkout',
				'settings'    => array(
					'reroute_requests' => array(
						'label'       => __( 'Set up PayPal for me using this email:', 'fincommerce' ),
						'type'        => 'checkbox',
						'value'       => 'yes',
						'default'     => 'yes',
						'placeholder' => '',
					...
					'email'            => array(
						'label'       => __( 'Direct payments to email address:', 'fincommerce' ),
						'type'        => 'email',
						'value'       => $user_email,
						'placeholder' => __( 'Email address to receive payments', 'fincommerce' ),
						'required'    => true,
					),
				),
			),
			WC_Gateway_Paypal::ID => array(
				'name'        => __( 'PayPal Standard', 'fincommerce' ),
				'description' => __( 'Accept payments via PayPal using account balance or credit card.', 'fincommerce' ),
				'image'       => '',
				'settings'    => array(
					'email' => array(
						'label'       => __( 'PayPal email address:', 'fincommerce' ),
						'type'        => 'email',
						'value'       => $user_email,
						'placeholder' => __( 'PayPal email address', 'fincommerce' ),
						'required'    => true,
					),
				),
			),
			'klarna_checkout'     => array(
				'name'        => __( 'Klarna Checkout for FinCommerce', 'fincommerce' ),
				'description' => $klarna_checkout_description,
				'image'       => WC()->plugin_url() . '/assets/images/klarna-black.png',
				'enabled'     => true,
				'class'       => 'klarna-logo',
				'repo-slug'   => 'klarna-checkout-for-fincommerce',
			),
			'klarna_payments'     => array(
				'name'        => __( 'Klarna Payments for FinCommerce', 'fincommerce' ),
				'description' => $klarna_payments_description,
				'image'       => WC()->plugin_url() . '/assets/images/klarna-black.png',
				'enabled'     => true,
				'class'       => 'klarna-logo',
				'repo-slug'   => 'klarna-payments-for-fincommerce',
			),
			'square'              => array(
				'name'        => __( 'FinCommerce Square', 'fincommerce' ),
				'description' => $square_description,
				'image'       => WC()->plugin_url() . '/assets/images/square-black.png',
				'class'       => 'square-logo',
				'enabled'     => false,
				'repo-slug'   => 'fincommerce-square',
			),
			'eway'                => array(
				'name'        => __( 'FinCommerce eWAY Gateway', 'fincommerce' ),
				'description' => __( 'The eWAY extension for FinCommerce allows you to take credit card payments directly on your store without redirecting your customers to a third party site to make payment.', 'fincommerce' ),
				'image'       => WC()->plugin_url() . '/assets/images/eway-logo.jpg',
				'enabled'     => false,
				'class'       => 'eway-logo',
				'repo-slug'   => 'fincommerce-gateway-eway',
			),
			'payfast'             => array(
				'name'        => __( 'FinCommerce PayFast Gateway', 'fincommerce' ),
				'description' => __( 'The PayFast extension for FinCommerce enables you to accept payments by Credit Card and EFT via one of South Africa’s most popular payment gateways. No setup fees or monthly subscription costs.', 'fincommerce' ),
				'image'       => WC()->plugin_url() . '/assets/images/payfast.png',
				'class'       => 'payfast-logo',
				'enabled'     => false,
				'repo-slug'   => 'fincommerce-payfast-gateway',
	...
	public function get_wizard_in_cart_payment_gateways() {
		...
		$country  = WC()->countries->get_base_country();
		$currency = get_fincommerce_currency();
		...
		} elseif ( $this->is_square_supported_country( $country ) && get_option( 'fincommerce_sell_in_person' ) ) {
	...
	public function get_wizard_manual_payment_gateways() {
		...
		$gateways = array(
			WC_Gateway_Cheque::ID => array(
				'name'        => _x( 'Check payments', 'Check payment method', 'fincommerce' ),
				'description' => __( 'A simple offline gateway that lets you accept a check as method of payment.', 'fincommerce' ),
				'image'       => '',
				'class'       => '',
			),
			WC_Gateway_BACS::ID   => array(
				'name'        => __( 'Bank transfer (BACS) payments', 'fincommerce' ),
				'description' => __( 'A simple offline gateway that lets you accept BACS payment.', 'fincommerce' ),
				'image'       => '',
				'class'       => '',
			),
			WC_Gateway_COD::ID    => array(
				'name'        => __( 'Cash on delivery', 'fincommerce' ),
				'description' => __( 'A simple offline gateway that lets you accept cash on delivery.', 'fincommerce' ),
				'image'       => '',
				'class'       => '',
			),
		);
	...
	public function display_service_item( $item_id, $item_info ) {
		...
		$previously_saved_settings = get_option( 'fincommerce_' . $item_id . '_settings' );
	...
	public function wc_setup_payment() {
		...
		?>
		<h1><?php esc_html_e( 'Payment', 'fincommerce' ); ?></h1>
		<form method="post" class="wc-wizard-payment-gateway-form">
			<p>
				...
				printf(
					wp_kses(
						/* translators: %s: Link */
						__( 'FinCommerce can accept both online and offline payments. <a href="%s" target="_blank">Additional payment methods</a> can be installed later.', 'fincommerce' ),
						array(
							'a' => array(
								'href'   => array(),
								'target' => array(),
			...
			<ul class="wc-wizard-services manual">
				<li class="wc-wizard-services-list-toggle closed">
					<div class="wc-wizard-service-name">
						<?php esc_html_e( 'Offline Payments', 'fincommerce' ); ?>
					</div>
					<div class="wc-wizard-service-description">
						<?php esc_html_e( 'Collect payments from customers offline.', 'fincommerce' ); ?>
					</div>
			...
			<p class="wc-setup-actions step">
				<?php $this->plugin_install_info(); ?>
				<button type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'fincommerce' ); ?>" name="save_step"><?php esc_html_e( 'Continue', 'fincommerce' ); ?></button>
				<?php wp_nonce_field( 'wc-setup' ); ?>
			</p>
		</form>
		<?php
	...
	public function wc_setup_recommended() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		?>
		<h1><?php esc_html_e( 'Recommended for All FinCommerce Stores', 'fincommerce' ); ?></h1>
		<p>
			<?php esc_html_e( 'Enhance your store with these recommended free features.', 'fincommerce' ); ?>
		</p>
		<form method="post">
			<ul class="recommended-step">
				...
				if ( $this->should_show_theme() ) :
					...
					$this->display_recommended_item( array(
						...
						'title'       => __( 'Storefront Theme', 'fincommerce' ),
						'description' => sprintf(
							/* translators: %s: theme name. */
							__(
								'Design your store with deep FinCommerce integration. If toggled on, we’ll install <a href="https://fincommerce.com/storefront/" target="_blank" rel="noopener noreferrer">Storefront</a>, and your current theme <em>%s</em> will be deactivated.',
								'fincommerce'
							),
							$theme_name
						),
						'img_url'     => WC()->plugin_url() . '/assets/images/obw-storefront-icon.svg',
						'img_alt'     => __( 'Storefront icon', 'fincommerce' ),
					) );
				endif;
				...
				if ( $this->should_show_automated_tax() ) :
					$this->display_recommended_item( array(
						...
						'title'       => __( 'Automated Taxes', 'fincommerce' ),
						'description' => __( 'Save time and errors with automated tax calculation and collection at checkout. Powered by FinCommerce Services and Jetpack.', 'fincommerce' ),
						'img_url'     => WC()->plugin_url() . '/assets/images/obw-taxes-icon.svg',
						'img_alt'     => __( 'automated taxes icon', 'fincommerce' ),
						'plugins'     => $this->get_wcs_requisite_plugins(),
					) );
				endif;
				...
				if ( $this->should_show_wc_admin() ) :
					$this->display_recommended_item( array(
						'type'        => 'wc_admin',
						'title'       => __( 'FinCommerce Admin', 'fincommerce' ),
						'description' => __( 'Manage your store\'s reports and monitor key metrics with a new and improved interface and dashboard.', 'fincommerce' ),
						'img_url'     => WC()->plugin_url() . '/assets/images/obw-fincommerce-admin-icon.svg',
						'img_alt'     => __( 'FinCommerce Admin icon', 'fincommerce' ),
						'plugins'     => array( array( 'name' => __( 'FinCommerce Admin', 'fincommerce' ), 'slug' => 'fincommerce-admin' ) ),
					) );
				endif;
				...
				if ( $this->should_show_mailchimp() ) :
					$this->display_recommended_item( array(
						'type'        => 'mailchimp',
						'title'       => __( 'Mailchimp', 'fincommerce' ),
						'description' => __( 'Join the 16 million customers who use Mailchimp. Sync list and store data to send automated emails, and targeted campaigns.', 'fincommerce' ),
						'img_url'     => WC()->plugin_url() . '/assets/images/obw-mailchimp-icon.svg',
						'img_alt'     => __( 'Mailchimp icon', 'fincommerce' ),
						'plugins'     => array( array( 'name' => __( 'Mailchimp for FinCommerce', 'fincommerce' ), 'slug' => 'mailchimp-for-fincommerce' ) ),
					) );
				endif;
				...
				if ( $this->should_show_facebook() ) :
					$this->display_recommended_item( array(
						'type'        => 'facebook',
						'title'       => __( 'Facebook', 'fincommerce' ),
						'description' => __( 'Enjoy all Facebook products combined in one extension: pixel tracking, catalog sync, messenger chat, shop functionality and Instagram shopping (coming soon)!', 'fincommerce' ),
						'img_url'     => WC()->plugin_url() . '/assets/images/obw-facebook-icon.svg',
						'img_alt'     => __( 'Facebook icon', 'fincommerce' ),
						'plugins'     => array( array( 'name' => __( 'Facebook for FinCommerce', 'fincommerce' ), 'slug' => 'facebook-for-fincommerce' ) ),
					) );
				endif;
			?>
		</ul>
	...
	protected function wc_setup_activate_get_feature_list() {
		...
		$stripe_settings = get_option( 'fincommerce_stripe_settings', false );
		...
		$ppec_settings   = get_option( 'fincommerce_ppec_paypal_settings', false );
		...
		$features['taxes']   = (bool) get_option( 'fincommerce_setup_automated_taxes', false );
		$features['labels']  = (bool) get_option( 'fincommerce_setup_shipping_labels', false );
	...
	protected function wc_setup_activate_get_feature_list_str() {
		...
		if ( $features['payment'] && $features['taxes'] && $features['labels'] ) {
			return __( 'payment setup, automated taxes and discounted shipping labels', 'fincommerce' );
		} else if ( $features['payment'] && $features['taxes'] ) {
			return __( 'payment setup and automated taxes', 'fincommerce' );
		} else if ( $features['payment'] && $features['labels'] ) {
			return __( 'payment setup and discounted shipping labels', 'fincommerce' );
		} else if ( $features['payment'] ) {
			return __( 'payment setup', 'fincommerce' );
		} else if ( $features['taxes'] && $features['labels'] ) {
			return __( 'automated taxes and discounted shipping labels', 'fincommerce' );
		} else if ( $features['taxes'] ) {
			return __( 'automated taxes', 'fincommerce' );
		} else if ( $features['labels'] ) {
			return __( 'discounted shipping labels', 'fincommerce' );
		}
		return false;
	}
	...
	public function wc_setup_activate() {
		...
		if ( isset( $_GET['activate_error'] ) ) {
			...
			$title = __( "Sorry, we couldn't connect your store to Jetpack", 'fincommerce' );
			...
			if ( $feature_list ) {
				if ( ! $jetpack_connected ) {
					...
					$description_base = __( 'Your store is almost ready! To activate services like %s, just connect with Jetpack.', 'fincommerce' );
				} else {
					$description_base = __( 'Thanks for using Jetpack! Your store is almost ready: to activate services like %s, just connect your store.', 'fincommerce' );
				}
			...
			if ( ! $jetpack_connected ) {
				$title = $feature_list ?
					__( 'Connect your store to Jetpack', 'fincommerce' ) :
					__( 'Connect your store to Jetpack to enable extra features', 'fincommerce' );
				$button_text = __( 'Continue with Jetpack', 'fincommerce' );
			} elseif ( $feature_list ) {
				$title = __( 'Connect your store to activate FinCommerce Services', 'fincommerce' );
				$button_text = __( 'Continue with FinCommerce Services', 'fincommerce' );
			} else {
		...
		<?php if ( $jetpack_connected ) : ?>
			<div class="activate-splash">
				<img
					...
					alt="<?php esc_attr_e( 'Jetpack logo', 'fincommerce' ); ?>"
				/>
				<img
					class="wcs-notice"
		...
		<?php if ( $has_jetpack_error ) : ?>
			<p class="wc-setup-actions step">
				...
				>
					<?php esc_html_e( 'Finish setting up your store', 'fincommerce' ); ?>
				</a>
			</p>
		<?php else : ?>
				...
				<h3 class="jetpack-reasons">
					<?php
						echo esc_html( $description ?
							__( "Bonus reasons you'll love Jetpack", 'fincommerce' ) :
							__( "Reasons you'll love Jetpack", 'fincommerce' )
						);
					?>
				</h3>
				<ul class="wc-wizard-features">
					<li class="wc-wizard-feature-item">
						<p class="wc-wizard-feature-name">
							<strong><?php esc_html_e( 'Better security', 'fincommerce' ); ?></strong>
						</p>
						<p class="wc-wizard-feature-description">
							<?php esc_html_e( 'Protect your store from unauthorized access.', 'fincommerce' ); ?>
						</p>
					</li>
					<li class="wc-wizard-feature-item">
						<p class="wc-wizard-feature-name">
							<strong><?php esc_html_e( 'Store stats', 'fincommerce' ); ?></strong>
						</p>
						<p class="wc-wizard-feature-description">
							<?php esc_html_e( 'Get insights on how your store is doing, including total sales, top products, and more.', 'fincommerce' ); ?>
						</p>
					</li>
					<li class="wc-wizard-feature-item">
						<p class="wc-wizard-feature-name">
							<strong><?php esc_html_e( 'Store monitoring', 'fincommerce' ); ?></strong>
						</p>
						<p class="wc-wizard-feature-description">
							<?php esc_html_e( 'Get an alert if your store is down for even a few minutes.', 'fincommerce' ); ?>
						</p>
					</li>
					<li class="wc-wizard-feature-item">
						<p class="wc-wizard-feature-name">
							<strong><?php esc_html_e( 'Product promotion', 'fincommerce' ); ?></strong>
						</p>
						<p class="wc-wizard-feature-description">
							<?php esc_html_e( "Share new items on social media the moment they're live in your store.", 'fincommerce' ); ?>
						</p>
					</li>
				</ul>
			<?php endif; ?>
	...
	protected function get_all_activate_errors() {
		_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '4.6.0', 'Onboarding is maintained in FinCommerce Admin.' );
		return array(
			'default' => __( "Sorry! We tried, but we couldn't connect Jetpack just now 😭. Please go to the Plugins tab to connect Jetpack, so that you can finish setting up your store.", 'fincommerce' ),
			'jetpack_cant_be_installed' => __( "Sorry! We tried, but we couldn't install Jetpack for you 😭. Please go to the Plugins tab to install it, and finish setting up your store.", 'fincommerce' ),
			'register_http_request_failed' => __( "Sorry! We couldn't contact Jetpack just now 😭. Please make sure that your site is visible over the internet, and that it accepts incoming and outgoing requests via curl. You can also try to connect to Jetpack again, and if you run into any more issues, please contact support.", 'fincommerce' ),
			'siteurl_private_ip_dev' => __( "Your site might be on a private network. Jetpack can only connect to public sites. Please make sure your site is visible over the internet, and then try connecting again 🙏." , 'fincommerce' ),
		);
	}
	...
	public function wc_setup_ready() {
		...
		$user_email = $this->get_current_user_email();
		$docs_url   = 'https://fincommerce.com/documentation/plugins/fincommerce/getting-started/?utm_source=setupwizard&utm_medium=product&utm_content=docs&utm_campaign=fincommerceplugin';
		$help_text  = sprintf(
			/* translators: %1$s: link to docs */
			__( 'Visit FinCommerce.com to learn more about <a href="%1$s" target="_blank">getting started</a>.', 'fincommerce' ),
			$docs_url
		);
		?>
		<h1><?php esc_html_e( "You're ready to start selling!", 'fincommerce' ); ?></h1>

		<div class="fincommerce-message fincommerce-newsletter">
			<p><?php esc_html_e( "We're here for you — get tips, product updates, and inspiration straight to your mailbox.", 'fincommerce' ); ?></p>
			<form action="//fincommerce.us8.list-manage.com/subscribe/post?u=2c1434dc56f9506bf3c3ecd21&amp;id=13860df971&amp;SIGNUPPAGE=plugin" method="post" target="_blank" novalidate>
				<div class="newsletter-form-container">
					<input
						...
						name="EMAIL"
						placeholder="<?php esc_attr_e( 'Email address', 'fincommerce' ); ?>"
						required
					>
					<p class="wc-setup-actions step newsletter-form-button-container">
						<button
							type="submit"
							value="<?php esc_attr_e( 'Yes please!', 'fincommerce' ); ?>"
							name="subscribe"
							id="mc-embedded-subscribe"
							class="button-primary button newsletter-form-button"
						><?php esc_html_e( 'Yes please!', 'fincommerce' ); ?></button>
					</p>
				</div>
			</form>
		</div>
		...
		<ul class="wc-wizard-next-steps">
			<li class="wc-wizard-next-step-item">
				<div class="wc-wizard-next-step-description">
					<p class="next-step-heading"><?php esc_html_e( 'Next step', 'fincommerce' ); ?></p>
					<h3 class="next-step-description"><?php esc_html_e( 'Create some products', 'fincommerce' ); ?></h3>
					<p class="next-step-extra-info"><?php esc_html_e( "You're ready to add products to your store.", 'fincommerce' ); ?></p>
				</div>
				<div class="wc-wizard-next-step-action">
					<p class="wc-setup-actions step">
						<a class="button button-primary button-large" href="<?php echo esc_url( admin_url( 'post-new.php?post_type=product&tutorial=true' ) ); ?>">
							<?php esc_html_e( 'Create a product', 'fincommerce' ); ?>
						</a>
					</p>
				</div>
			</li>
			<li class="wc-wizard-next-step-item">
				<div class="wc-wizard-next-step-description">
					<p class="next-step-heading"><?php esc_html_e( 'Have an existing store?', 'fincommerce' ); ?></p>
					<h3 class="next-step-description"><?php esc_html_e( 'Import products', 'fincommerce' ); ?></h3>
					<p class="next-step-extra-info"><?php esc_html_e( 'Transfer existing products to your new store — just import a CSV file.', 'fincommerce' ); ?></p>
				</div>
				<div class="wc-wizard-next-step-action">
					<p class="wc-setup-actions step">
						<a class="button button-large" href="<?php echo esc_url( admin_url( 'edit.php?post_type=product&page=product_importer' ) ); ?>">
							<?php esc_html_e( 'Import products', 'fincommerce' ); ?>
						</a>
					</p>
				</div>
			</li>
			<li class="wc-wizard-additional-steps">
				<div class="wc-wizard-next-step-description">
					<p class="next-step-heading"><?php esc_html_e( 'You can also:', 'fincommerce' ); ?></p>
				</div>
				<div class="wc-wizard-next-step-action">
					<p class="wc-setup-actions step">
						<a class="button button-large" href="<?php echo esc_url( admin_url() ); ?>">
							<?php esc_html_e( 'Visit Dashboard', 'fincommerce' ); ?>
						</a>
						<a class="button button-large" href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings' ) ); ?>">
							<?php esc_html_e( 'Review Settings', 'fincommerce' ); ?>
						</a>
						<a class="button button-large" href="<?php echo esc_url( add_query_arg( array( 'autofocus' => array( 'panel' => 'fincommerce' ), 'url' => wc_get_page_permalink( 'shop' ) ), admin_url( 'customize.php' ) ) ); ?>">
							<?php esc_html_e( 'View &amp; Customize', 'fincommerce' ); ?>
						</a>
					</p>
				</div>
			</li>
