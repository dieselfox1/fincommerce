...
use Automattic\FinCommerce\Internal\Admin\Orders\ListTable;
...
class WC_Admin_List_Table_Orders extends WC_Admin_List_Table {
	...
	public function define_columns( $columns ) {
		...
		$show_columns['order_number']     = __( 'Order', 'fincommerce' );
		$show_columns['order_date']       = __( 'Date', 'fincommerce' );
		$show_columns['order_status']     = __( 'Status', 'fincommerce' );
		$show_columns['billing_address']  = __( 'Billing', 'fincommerce' );
		$show_columns['shipping_address'] = __( 'Ship to', 'fincommerce' );
		$show_columns['order_total']      = __( 'Total', 'fincommerce' );
		$show_columns['wc_actions']       = __( 'Actions', 'fincommerce' );
	...
	public function define_bulk_actions( $actions ) {
		...
		$actions['mark_processing'] = __( 'Change status to processing', 'fincommerce' );
		$actions['mark_on-hold']    = __( 'Change status to on-hold', 'fincommerce' );
		$actions['mark_completed']  = __( 'Change status to completed', 'fincommerce' );
		$actions['mark_cancelled']  = __( 'Change status to cancelled', 'fincommerce' );
	...
	public static function get_order_preview_item_html( $order ) {
		$hidden_order_itemmeta = apply_filters(
			'fincommerce_hidden_order_itemmeta',
			array(
				'_qty',
				'_tax_class',
				'_product_id',
		...
		$line_items = apply_filters( 'fincommerce_admin_order_preview_line_items', $order->get_items(), $order );
		$columns    = apply_filters(
			'fincommerce_admin_order_preview_line_item_columns',
			array(
				'product'  => __( 'Product', 'fincommerce' ),
				'quantity' => __( 'Quantity', 'fincommerce' ),
				'tax'      => __( 'Tax', 'fincommerce' ),
				'total'    => __( 'Total', 'fincommerce' ),
			),
			$order
		);
		...
		foreach ( $line_items as $item_id => $item ) {
			...
			$row_class      = apply_filters( 'fincommerce_admin_html_order_preview_item_class', '', $item, $order );
			...
			foreach ( $columns as $column => $label ) {
				...
				switch ( $column ) {
					...
					case 'total':
						...
						break;
					default:
						$html .= apply_filters( 'fincommerce_admin_order_preview_line_item_column_' . sanitize_key( $column ), '', $item, $item_id, $order );
						break;
				}
				$html .= '</td>';
			}
	...
	public static function get_order_preview_actions_html( $order ) {
		...
		if ( $order->has_status( array( OrderStatus::PENDING ) ) ) {
			$status_actions['on-hold'] = array(
				'url'    => wp_nonce_url( admin_url( 'admin-ajax.php?action=fincommerce_mark_order_status&status=on-hold&order_id=' . $order->get_id() ), 'fincommerce-mark-order-status' ),
				'name'   => __( 'On-hold', 'fincommerce' ),
				'title'  => __( 'Change order status to on-hold', 'fincommerce' ),
				'action' => 'on-hold',
			);
		}
		...
		if ( $order->has_status( array( OrderStatus::PENDING, OrderStatus::ON_HOLD ) ) ) {
			$status_actions['processing'] = array(
				...
				'title'  => __( 'Change order status to processing', 'fincommerce' ),
				'action' => 'processing',
			);
		}
		...
		if ( $order->has_status( array( OrderStatus::PENDING, OrderStatus::ON_HOLD, OrderStatus::PROCESSING ) ) ) {
			$status_actions['complete'] = array(
				...
				'name'   => __( 'Completed', 'fincommerce' ),
				'title'  => __( 'Change order status to completed', 'fincommerce' ),
				'action' => 'complete',
			);
		}
		...
		if ( $status_actions ) {
			$actions['status'] = array(
				'group'   => __( 'Change status: ', 'fincommerce' ),
				'actions' => $status_actions,
			);
		}

		return wc_render_action_buttons( apply_filters( 'fincommerce_admin_order_preview_actions', $actions, $order ) );
	}
	...
	public static function order_preview_get_order_details( $order ) {
		...
		/**
		 * Filter to customize the order details data that the fincommerce_get_order_details action will send.
		 *
		 * @param array $order_details Order details.
		 */
		$order_details = apply_filters(
			'fincommerce_admin_order_preview_get_order_details',
			array(
				'data'                       => $order->get_data(),
				...
				'formatted_billing_address'  => $billing_address ? $billing_address : __( 'N/A', 'fincommerce' ),
				'formatted_shipping_address' => $shipping_address ? $shipping_address : __( 'N/A', 'fincommerce' ),
	...
	public function handle_bulk_actions( $redirect_to, $action, $ids ) {
		$ids     = apply_filters( 'fincommerce_bulk_action_ids', array_reverse( array_map( 'absint', $ids ) ), $action, 'order' );
		$changed = 0;
		...
		if ( 'remove_personal_data' === $action ) {
			...
			foreach ( $ids as $id ) {
				...
				if ( $order ) {
					do_action( 'fincommerce_remove_order_personal_data', $order );
					++$changed;
				}
			}
			...
			if ( isset( $order_statuses[ 'wc-' . $new_status ] ) ) {
				...
				foreach ( $ids as $id ) {
					...
					do_action( 'fincommerce_order_edit_status', $id, $new_status );
					++$changed;
				}
			}
		}
