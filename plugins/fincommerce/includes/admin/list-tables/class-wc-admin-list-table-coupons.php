...
class WC_Admin_List_Table_Coupons extends WC_Admin_List_Table {
	...
	protected function render_blank_state() {
		...
		echo '<h2 class="fincommerce-BlankState-message">' . esc_html__( 'Coupons are a great way to offer discounts and rewards to your customers. They will appear here once created.', 'fincommerce' ) . '</h2>';
		echo '<a class="fincommerce-BlankState-cta button-primary button" href="' . esc_url( admin_url( 'post-new.php?post_type=shop_coupon' ) ) . '">' . esc_html__( 'Create your first coupon', 'fincommerce' ) . '</a>';
		echo '<a class="fincommerce-BlankState-cta button" target="_blank" href="https://fincommerce.com/document/coupon-management/?utm_source=blankslate&utm_medium=product&utm_content=couponsdoc&utm_campaign=fincommerceplugin">' . esc_html__( 'Learn more about coupons', 'fincommerce' ) . '</a>';
		echo '</div>';
	}
	...
	public function define_columns( $columns ) {
		...
		$show_columns['coupon_code'] = __( 'Code', 'fincommerce' );
		$show_columns['type']        = __( 'Coupon type', 'fincommerce' );
		$show_columns['amount']      = __( 'Coupon amount', 'fincommerce' );
		$show_columns['description'] = __( 'Description', 'fincommerce' );
		$show_columns['products']    = __( 'Product IDs', 'fincommerce' );
		$show_columns['usage']       = __( 'Usage / Limit', 'fincommerce' );
		$show_columns['expiry_date'] = __( 'Expiry date', 'fincommerce' );
	...
	protected function render_filters() {
		...
		<select name="coupon_type" id="dropdown_shop_coupon_type">
			<option value=""><?php esc_html_e( 'Show all types', 'fincommerce' ); ?></option>
			<?php
	...
	protected function query_filters( $query_vars ) {
		if ( ! empty( $_GET['coupon_type'] ) ) { // WPCS: input var ok, sanitization ok.
			$query_vars['meta_key']   = 'discount_type'; // phpcs:ignore finpress.DB.SlowDBQuery.slow_db_query_meta_key
			$query_vars['meta_value'] = wc_clean( wp_unslash( $_GET['coupon_type'] ) ); // phpcs:ignore finpress.DB.SlowDBQuery.slow_db_query_meta_value, finpress.VIP.SuperGlobalInputUsage.AccessDetected
		}
		return $query_vars;
	}
}
