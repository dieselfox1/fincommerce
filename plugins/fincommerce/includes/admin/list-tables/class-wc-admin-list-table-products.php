...
class WC_Admin_List_Table_Products extends WC_Admin_List_Table {
	...
	protected function render_blank_state() {
		echo '<div class="fincommerce-BlankState">';

		echo '<h2 class="fincommerce-BlankState-message">' . esc_html__( 'Ready to start selling something awesome?', 'fincommerce' ) . '</h2>';

		echo '<div class="fincommerce-BlankState-buttons">';

		echo '<a class="fincommerce-BlankState-cta button-primary button" href="' . esc_url( admin_url( 'post-new.php?post_type=product&tutorial=true' ) ) . '">' . esc_html__( 'Create Product', 'fincommerce' ) . '</a>';
		echo '<a class="fincommerce-BlankState-cta button" href="' . esc_url( admin_url( 'edit.php?post_type=product&page=product_importer' ) ) . '">' . esc_html__( 'Start Import', 'fincommerce' ) . '</a>';
	...
	protected function get_row_actions( $actions, $post ) {
		...
		return array_merge( array( 'id' => sprintf( __( 'ID: %d', 'fincommerce' ), $post->ID ) ), $actions );
	}
	...
	public function define_columns( $columns ) {
		...
		$show_columns['thumb'] = '<span class="wc-image tips" data-tip="' . esc_attr__( 'Image', 'fincommerce' ) . '">' . __( 'Image', 'fincommerce' ) . '</span>';
		$show_columns['name']  = __( 'Name', 'fincommerce' );

		if ( wc_product_sku_enabled() ) {
			$show_columns['sku'] = __( 'SKU', 'fincommerce' );
		}

		if ( 'yes' === get_option( 'fincommerce_manage_stock' ) ) {
			$show_columns['is_in_stock'] = __( 'Stock', 'fincommerce' );
		}

		$show_columns['price'] = __( 'Price', 'fincommerce' );
		if ( $this->cogs_is_enabled ) {
			$show_columns['cogs_value'] = __( 'Cost', 'fincommerce' );
		}
		$show_columns['product_cat'] = __( 'Categories', 'fincommerce' );
		$show_columns['product_tag'] = __( 'Tags', 'fincommerce' );
		$show_columns['featured']    = '<span class="wc-featured parent-tips" data-tip="' . esc_attr__( 'Featured', 'fincommerce' ) . '">' . __( 'Featured', 'fincommerce' ) . '</span>';
		$show_columns['date']        = __( 'Date', 'fincommerce' );
	...
	protected function render_product_cat_column() {
		...
		if ( ! $terms ) {
			...
			echo apply_filters( 'fincommerce_admin_product_term_list', implode( ', ', $termlist ), 'product_cat', $this->object->get_id(), $termlist, $terms ); // WPCS: XSS ok.
		}
	}
	...
	protected function render_product_tag_column() {
		...
		if ( ! $terms ) {
			...
			echo apply_filters( 'fincommerce_admin_product_term_list', implode( ', ', $termlist ), 'product_tag', $this->object->get_id(), $termlist, $terms ); // WPCS: XSS ok.
		}
	}
	...
	protected function render_featured_column() {
		$url = wp_nonce_url( admin_url( 'admin-ajax.php?action=fincommerce_feature_product&product_id=' . $this->object->get_id() ), 'fincommerce-feature-product' );
		echo '<a href="' . esc_url( $url ) . '" aria-label="' . esc_attr__( 'Toggle featured', 'fincommerce' ) . '">';
		if ( $this->object->is_featured() ) {
			echo '<span class="wc-featured tips" data-tip="' . esc_attr__( 'Yes', 'fincommerce' ) . '">' . esc_html__( 'Yes', 'fincommerce' ) . '</span>';
		} else {
			echo '<span class="wc-featured not-featured tips" data-tip="' . esc_attr__( 'No', 'fincommerce' ) . '">' . esc_html__( 'No', 'fincommerce' ) . '</span>';
		}
		echo '</a>';
	}
	...
	protected function render_is_in_stock_column() {
		if ( $this->object->is_on_backorder() ) {
			$stock_html = '<mark class="onbackorder">' . __( 'On backorder', 'fincommerce' ) . '</mark>';
		} elseif ( $this->object->is_in_stock() ) {
			$stock_html = '<mark class="instock">' . __( 'In stock', 'fincommerce' ) . '</mark>';
		} else {
			$stock_html = '<mark class="outofstock">' . __( 'Out of stock', 'fincommerce' ) . '</mark>';
		}
		...
		echo wp_kses_post( apply_filters( 'fincommerce_admin_stock_html', $stock_html, $this->object ) );
	}
	...
	protected function render_filters() {
		...
		echo apply_filters( 'fincommerce_product_filters', $output ); // WPCS: XSS ok.
	}
	...
	protected function render_products_category_filter() {
		...
		if ( $categories_count <= apply_filters( 'fincommerce_product_category_filter_threshold', 100 ) ) {
			wc_product_dropdown_categories(
				array(
					'option_select_text' => __( 'Filter by category', 'fincommerce' ),
					'hide_empty'         => 0,
				)
			);
		} else {
			...
			?>
			<select class="wc-category-search" name="product_cat" data-placeholder="<?php esc_attr_e( 'Filter by category', 'fincommerce' ); ?>" data-allow_clear="true">
	...
	protected function render_products_type_filter() {
		...
		$output               = '<select name="product_type" id="dropdown_product_type"><option value="">' . esc_html__( 'Filter by product type', 'fincommerce' ) . '</option>';
		...
		foreach ( wc_get_product_types() as $value => $label ) {
			...
			if ( ProductType::SIMPLE === $value ) {
				...
				$output .= '> ' . ( is_rtl() ? '&larr;' : '&rarr;' ) . ' ' . esc_html__( 'Downloadable', 'fincommerce' ) . '</option>';

				$output .= '<option value="virtual" ';
				$output .= selected( 'virtual', $current_product_type, false );
				$output .= '> ' . ( is_rtl() ? '&larr;' : '&rarr;' ) . ' ' . esc_html__( 'Virtual', 'fincommerce' ) . '</option>';
			}
		}
	...
	public function render_products_stock_status_filter() {
		...
		$output               = '<select name="stock_status"><option value="">' . esc_html__( 'Filter by stock status', 'fincommerce' ) . '</option>';
	...
	public function product_views( $views ) {
		...
		if ( current_user_can( 'edit_others_products' ) ) {
			...
			$views['byorder'] = '<a href="' . esc_url( $query_string ) . '" class="' . esc_attr( $class ) . '">' . __( 'Sorting', 'fincommerce' ) . '</a>';
		}
	...
	public function add_sample_product_badge( $column_name, $post_id ) {
		...
		if ( $is_sample_product && 'name' === $column_name ) {
			echo '<span class="sample-product-badge" style="margin-right: 6px;border-radius: 4px; background: #F6F7F7; padding: 4px; color: #3C434A;font-size: 12px;font-style: normal;font-weight: 400;line-height: 16px; height: 24px;">' . esc_html__( 'Sample', 'fincommerce' ) . '</span>';
		}
	}
}
