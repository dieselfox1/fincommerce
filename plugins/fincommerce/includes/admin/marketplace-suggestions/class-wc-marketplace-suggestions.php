...
class WC_Marketplace_Suggestions {
	...
	public static function init() {
		...
		add_action( 'fincommerce_product_data_tabs', array( __CLASS__, 'product_data_tabs' ) );
		add_action( 'fincommerce_product_data_panels', array( __CLASS__, 'product_data_panels' ) );

		// Register ajax api handlers.
		add_action( 'wp_ajax_fincommerce_add_dismissed_marketplace_suggestion', array( __CLASS__, 'post_add_dismissed_suggestion_handler' ) );
	...
	public static function product_data_tabs( $tabs ) {
		$tabs['marketplace-suggestions'] = array(
			'label'    => _x( 'Get more options', 'Marketplace suggestions', 'fincommerce' ),
	...
	public static function maybe_trigger_suggestions_fetch() {
		...
		if ( 'fincommerce_page_wc-admin' === $screen_id && self::allow_suggestions() ) {
	...
	public static function allow_suggestions() {
		...
		return apply_filters( 'fincommerce_allow_marketplace_suggestions', true );
	}
	...
	public static function get_suggestions_api_data() {
		$data = get_option( 'fincommerce_marketplace_suggestions', array() );
		...
		if ( empty( $data['updated'] ) || ( time() - WEEK_IN_SECONDS ) > $data['updated'] ) {
			$next = WC()->queue()->get_next( 'fincommerce_update_marketplace_suggestions' );
			if ( ! $next ) {
				WC()->queue()->cancel_all( 'fincommerce_update_marketplace_suggestions' );
				WC()->queue()->schedule_single( time(), 'fincommerce_update_marketplace_suggestions' );
			}
		}
