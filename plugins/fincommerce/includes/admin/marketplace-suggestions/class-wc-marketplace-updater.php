...
class WC_Marketplace_Updater {
	...
	public static function init() {
		add_action( 'fincommerce_update_marketplace_suggestions', array( __CLASS__, 'update_marketplace_suggestions' ) );
	}
	...
	public static function update_marketplace_suggestions() {
		$data = get_option(
			'fincommerce_marketplace_suggestions',
			array(
				'suggestions' => array(),
				'updated'     => time(),
			)
		...
		$url = 'https://fincommerce.com/wp-json/wccom/marketplace-suggestions/2.0/suggestions.json';
		...
		if ( is_wp_error( $request ) ) {
			self::retry();
			return update_option( 'fincommerce_marketplace_suggestions', $data, false );
		}
	...
	public static function retry() {
		...
		WC()->queue()->schedule_single( time() + DAY_IN_SECONDS, 'fincommerce_update_marketplace_suggestions' );
	}
