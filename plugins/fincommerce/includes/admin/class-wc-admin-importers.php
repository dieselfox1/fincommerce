...
/**
 * Init FinCommerce data importers.
 *
...
class WC_Admin_Importers {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_fincommerce_do_ajax_product_import', array( $this, 'do_ajax_product_import' ) );
	...
	/**
	 * Return true if FinCommerce imports are allowed for current user, false otherwise.
	 *
	...
	public function register_importers() {
		if ( Constants::is_defined( 'WP_LOAD_IMPORTERS' ) ) {
			register_importer( 'fincommerce_product_csv', __( 'FinCommerce products (CSV)', 'fincommerce' ), __( 'Import <strong>products</strong> to your store via a csv file.', 'fincommerce' ), array( $this, 'product_importer' ) );
			register_importer( 'fincommerce_tax_rate_csv', __( 'FinCommerce tax rates (CSV)', 'fincommerce' ), __( 'Import <strong>tax rates</strong> to your store via a csv file.', 'fincommerce' ), array( $this, 'tax_rates_importer' ) );
		}
	}
						...
						if ( strstr( $term['domain'], 'pa_' ) ) {
							if ( ! taxonomy_exists( $term['domain'] ) ) {
								...
								register_taxonomy(
									$term['domain'],
									// phpcs:ignore
									apply_filters( 'fincommerce_taxonomy_objects_' . $term['domain'], array( 'product' ) ),
									// phpcs:ignore
									apply_filters(
										'fincommerce_taxonomy_args_' . $term['domain'],
										array(
											'hierarchical' => true,
											'show_ui'      => false,
											'query_var'    => true,
	...
	public function do_ajax_product_import() {
		if ( ! $this->import_allowed() ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient privileges to import products.', 'fincommerce' ) ) );
		}
