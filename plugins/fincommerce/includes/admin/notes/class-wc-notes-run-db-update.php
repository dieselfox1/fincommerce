<?php
/**
 * FinCommerce: Db update note.
 *
 * Adds a note to complete the FinCommerce db update after the upgrade in the WC Admin context.
 *
 * @package FinCommerce
 */
...
class WC_Notes_Run_Db_Update {
	...
	public function __construct() {
		...
		add_action( 'fincommerce_hide_update_notice', array( __CLASS__, 'set_notice_actioned' ) );
	...
	private static function update_needed_notice( $note_id = null ) {
		...
		$note_actions = array(
			array(
				'name'         => 'update-db_run',
				'label'        => __( 'Update FinCommerce Database', 'fincommerce' ),
				'url'          => $update_url,
				'status'       => 'unactioned',
				'primary'      => true,
			...
			array(
				...
				'label'   => __( 'Learn more about updates', 'fincommerce' ),
				'url'     => 'https://fincommerce.com/document/how-to-update-fincommerce/',
				'status'  => 'unactioned',
				'primary' => false,
			),
		);
		...
		$note->set_title( __( 'FinCommerce database update required', 'fincommerce' ) );
		$note->set_content(
			__( 'FinCommerce has been updated! To keep things running smoothly, we have to update your database to the newest version.', 'fincommerce' )
			/* translators: %1$s: opening <a> tag %2$s: closing </a> tag*/
			. sprintf( ' ' . esc_html__( 'The database update process runs in the background and may take a little while, so please be patient. Advanced users can alternatively update via %1$sWP CLI%2$s.', 'fincommerce' ), '<a href="https://github.com/dieselfox1/fincommerce/wiki/Upgrading-the-database-using-WP-CLI">', '</a>' )
		);
		$note->set_type( Note::E_WC_ADMIN_NOTE_UPDATE );
		$note->set_name( self::NOTE_NAME );
		$note->set_content_data( (object) array() );
		$note->set_source( 'fincommerce-core' );
	...
	private static function update_in_progress_notice( $note_id ) {
		...
		$cron_cta            = $cron_disabled ? __( 'You can manually run queued updates here.', 'fincommerce' ) : __( 'View progress →', 'fincommerce' );

		$note = new Note( $note_id );
		$note->set_title( __( 'FinCommerce database update in progress', 'fincommerce' ) );
		$note->set_content( __( 'FinCommerce is updating the database in the background. The database update process may take a little while, so please be patient.', 'fincommerce' ) );
	...
	private static function update_done_notice( $note_id ) {
		$hide_notices_url = html_entity_decode( // to convert &amp;s to normal &, otherwise produces invalid link.
			add_query_arg(
				...
				wc_get_current_admin_url() ? remove_query_arg( 'do_update_fincommerce', wc_get_current_admin_url() ) : admin_url( 'admin.php?page=wc-settings' )
			)
		);
		...
		$note_actions = array(
			array(
				'name'         => 'update-db_done',
				'label'        => __( 'Thanks!', 'fincommerce' ),
				'url'          => $hide_notices_url,
				'status'       => 'actioned',
				'primary'      => true,
				'nonce_action' => 'fincommerce_hide_notices_nonce',
		...
		$note->set_title( __( 'FinCommerce database update done', 'fincommerce' ) );
		$note->set_content( __( 'FinCommerce database update complete. Thank you for updating to the latest version!', 'fincommerce' ) );
