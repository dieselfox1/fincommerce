...
class WC_Brands_Admin {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_sortable_taxonomies', array( $this, 'sort_brands' ) );
		...
		add_filter(
			'fincommerce_products_admin_list_table_filters',
			function ( $args ) {
		...
		if ( is_string( $setting_value ) ) {
			...
			add_action(
				'before_fincommerce_init',
				function () {
					$this->init_form_fields();
					$this->settings_tabs = array(
						'brands' => __( 'Brands', 'fincommerce' ),
					);
				}
			);
			add_action( 'fincommerce_get_sections_products', array( $this, 'add_settings_tab' ) );
			add_action( 'fincommerce_get_settings_products', array( $this, 'add_settings_section' ), null, 2 );
		}

		add_action( 'fincommerce_update_options_catalog', array( $this, 'save_admin_settings' ) );

		/* 2.1 */
		add_action( 'fincommerce_update_options_products', array( $this, 'save_admin_settings' ) );

		// Add brands filtering to the coupon creation screens.
		add_action( 'fincommerce_coupon_options_usage_restriction', array( $this, 'add_coupon_brands_fields' ) );
		add_action( 'fincommerce_coupon_options_save', array( $this, 'save_coupon_brands' ) );

		// Permalinks.
		add_filter( 'pre_update_option_fincommerce_permalinks', array( $this, 'validate_product_base' ) );

		add_action( 'current_screen', array( $this, 'add_brand_base_setting' ) );

		// CSV Import/Export Support.
		// https://github.com/dieselfox1/fincommerce/wiki/Product-CSV-Importer-&-Exporter
		// Import.
		add_filter( 'fincommerce_csv_product_import_mapping_options', array( $this, 'add_column_to_importer_exporter' ), 10 );
		add_filter( 'fincommerce_csv_product_import_mapping_default_columns', array( $this, 'add_default_column_mapping' ), 10 );
		add_filter( 'fincommerce_product_import_inserted_product_object', array( $this, 'process_import' ), 10, 2 );

		// Export.
		add_filter( 'fincommerce_product_export_column_names', array( $this, 'add_column_to_importer_exporter' ), 10 );
		add_filter( 'fincommerce_product_export_product_default_columns', array( $this, 'add_column_to_importer_exporter' ), 10 );
		add_filter( 'fincommerce_product_export_product_column_brand_ids', array( $this, 'get_column_value_brand_ids' ), 10, 2 );
	}
	...
	public function add_coupon_brands_fields() {
		...
		<p class="form-field"><label for="product_brands"><?php esc_html_e( 'Product brands', 'fincommerce' ); ?></label>
			<select id="product_brands" name="product_brands[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Any brand', 'fincommerce' ); ?>">
				<?php
			...
			<?php
				echo wc_help_tip( esc_html__( 'A product must be associated with this brand for the coupon to remain valid or, for "Product Discounts", products with these brands will be discounted.', 'fincommerce' ) );
				// Exclude Brands.
			?>
		<p class="form-field"><label for="exclude_product_brands"><?php esc_html_e( 'Exclude brands', 'fincommerce' ); ?></label>
			<select id="exclude_product_brands" name="exclude_product_brands[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="<?php esc_attr_e( 'No brands', 'fincommerce' ); ?>">
				<?php
			...
			<?php
				echo wc_help_tip( esc_html__( 'Product must not be associated with these brands for the coupon to remain valid or, for "Product Discounts", products associated with these brands will not be discounted.', 'fincommerce' ) );
			?>
		</div>
		<?php
	}
	...
	public function init_form_fields() {
		...
		$this->settings = apply_filters(
			'fincommerce_brands_settings_fields',
			array(
				array(
					'name' => __( 'Brands Archives', 'fincommerce' ),
					'type' => 'title',
					'desc' => '',
					'id'   => 'brands_archives',
				),
				array(
					'name' => __( 'Show description', 'fincommerce' ),
					'desc' => __( 'Choose to show the brand description on the archive page. Turn this off if you intend to use the description widget instead. Please note: this is only for themes that do not show the description.', 'fincommerce' ),
					'tip'  => '',
	...
	public function admin_settings() {
		fincommerce_admin_fields( $this->settings );
	}
	...
	public function add_thumbnail_field() {
		...
		<div class="form-field">
			<label><?php esc_html_e( 'Thumbnail', 'fincommerce' ); ?></label>
			...
			<div style="line-height:60px;">
				...
				<button type="button" class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'fincommerce' ); ?></button>
				<button type="button" class="remove_image_button button"><?php esc_html_e( 'Remove image', 'fincommerce' ); ?></button>
			</div>
				...
				jQuery(function(){
					...
					jQuery(document).on( 'click', '.upload_image_button', function( event ){
						...
						file_frame = wp.media.frames.downloadable_file = wp.media({
							title: '<?php echo esc_js( __( 'Choose an image', 'fincommerce' ) ); ?>',
							button: {
								text: '<?php echo esc_js( __( 'Use image', 'fincommerce' ) ); ?>',
							},
							multiple: false
						});
	...
	public function edit_thumbnail_field( $term ) {
		...
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'fincommerce' ); ?></label></th>
			<td>
	...
	public function taxonomy_description() {
		echo wp_kses_post( wpautop( __( 'Brands can be added and managed from this screen. You can optionally upload a brand image to display in brand widgets and on brand archives', 'fincommerce' ) ) );
	}
	...
	public function columns( $columns ) {
		...
		$new_columns['thumb'] = __( 'Image', 'fincommerce' );
		unset( $columns['cb'] );
	...
	public function render_product_brand_filter() {
		 ...
		 */
		if ( $brands_count <= apply_filters( 'fincommerce_product_brand_filter_threshold', 100 ) ) {
			wc_product_dropdown_categories(
				array(
					...
					'show_option_none'  => __( 'Filter by brand', 'fincommerce' ),
					'option_none_value' => '',
					'value_field'       => 'slug',
					'taxonomy'          => 'product_brand',
					'name'              => 'product_brand',
	...
	public function add_brand_base_setting() {
		...
		add_settings_field(
			'fincommerce_product_brand_slug',
			__( 'Product brand base', 'fincommerce' ),
	...
	public function product_brand_slug_input() {
		$permalink = get_option( 'fincommerce_brand_permalink', '' );
		?>
		<input name="fincommerce_product_brand_slug" type="text" class="regular-text code" value="<?php echo esc_attr( $permalink ); ?>" placeholder="<?php echo esc_attr_x( 'brand', 'slug', 'fincommerce' ); ?>" />
		<?php
	}
	...
	public function validate_product_base( $value ) {
		if ( '/%product_brand%/' === trailingslashit( $value['product_base'] ) ) {
			$value['product_base'] = '/' . _x( 'product', 'slug', 'fincommerce' ) . $value['product_base'];
		}
	...
	public function add_column_to_importer_exporter( $options ) {
		$options['brand_ids'] = __( 'Brands', 'fincommerce' );
		return $options;
	}
	...
	public function add_default_column_mapping( $mappings ) {
		$new_mapping = array( __( 'Brands', 'fincommerce' ) => 'brand_ids' );
