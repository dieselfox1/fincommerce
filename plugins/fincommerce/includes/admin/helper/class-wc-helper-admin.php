 ...
 * an account on FinCommerce.com.
 */
class WC_Helper_Admin {
	/**
	 * Clear cache tool identifier.
	 */
	const CACHE_TOOL_ID = 'clear_fincommerce_helper_cache';
	...
	public static function load() {
		if ( is_admin() ) {
			...
			if ( $is_wc_home_or_in_app_marketplace ) {
				add_filter( 'fincommerce_admin_shared_settings', array( __CLASS__, 'add_marketplace_settings' ) );
			}

			add_filter( 'fincommerce_debug_tools', array( __CLASS__, 'register_cache_clear_tool' ) );
		}
	...
	/**
	 * Pushes settings onto the FinCommerce Admin global settings object (wcSettings).
	 *
	...
	public static function add_marketplace_settings( $settings ) {
		...
		$settings['wccomHelper'] = array(
			...
			'trackingAllowed'            => 'yes' === get_option( 'fincommerce_allow_tracking' ),
		);
	...
	/**
	 * Generates the URL for connecting or disconnecting the store to/from FinCommerce.com.
	...
	public static function register_rest_routes() {
		/* Used by the FinCommerce > Extensions > Discover page. */
		register_rest_route(
			'wc/v3',
	...
	/**
	 * Fetch featured products from FinCommerce.com and serve them
	 * as JSON.
	 */
	...
	/**
	 * Fetch data for product previews from FinCommerce.com.
	 *
	...
	public static function get_product_preview( $request ) {
		...
		if ( ! $product_id ) {
			wp_send_json_error(
				array(
					'message' => __( 'Missing product ID', 'fincommerce' ),
				),
				400
			);
		}
		...
		if ( ! $product_preview ) {
			wp_send_json_error(
				array(
					'message' => __( 'We couldn\'t find a preview for this product.', 'fincommerce' ),
				),
				404
			);
		}
	...
	public static function register_cache_clear_tool( $debug_tools ) {
		$debug_tools[ self::CACHE_TOOL_ID ] = array(
			'name'     => __( 'Clear FinCommerce.com cache', 'fincommerce' ),
			'button'   => __( 'Clear', 'fincommerce' ),
			'desc'     => sprintf(
				__( 'This tool will empty the FinCommerce.com data cache, used in FinCommerce Extensions.', 'fincommerce' ),
			),
	...
	public static function run_clear_cache_tool() {
		...
		return __( 'Helper cache cleared.', 'fincommerce' );
	}
}
