<?php
/**
 * FinCommerce Admin Helper - React admin interface
 *
 ...
 * The Subscriptions API manages FinCommerce.com Subscriptions.
 */
class WC_Helper_Subscriptions_API {
	...
	/**
	 * The Extensions page can only be accessed by users with the manage_fincommerce
	...
	/**
	 * Fetch subscriptions from FinCommerce.com and serve them
	 * as JSON.
	 */
	...
	/**
	 * Refresh account and subscriptions from FinCommerce.com and serve subscriptions
	 * as JSON.
	 */
	...
	/**
	 * Connect a FinCommerce.com subscription.
	 *
	...
	public static function connect( $request ) {
		...
		if ( $success ) {
			wp_send_json_success(
				array(
					'message' => __( 'Your subscription has been connected.', 'fincommerce' ),
				)
			);
		} else {
			wp_send_json_error(
				array(
					'message' => __( 'There was an error connecting your subscription. Please try again.', 'fincommerce' ),
				),
				400
			);
		}
	...
	/**
	 * Activate a plugin for a FinCommerce.com subscription.
	 *
	 * @param WP_REST_Request $request Request object.
	 */
	public static function activate_plugin( $request ) {
		...
		if ( $success ) {
			wp_send_json_success(
				array(
					'message' => __( 'The plugin for your subscription has been activated.', 'fincommerce' ),
				)
			);
		} else {
			wp_send_json_error(
				array(
					'message' => __( 'The plugin for your subscription couldn\'t be activated.', 'fincommerce' ),
				),
				400
			);
		}
	...
	/**
	 * Disconnect a FinCommerce.com subscription.
	 *
	 * @param WP_REST_Request $request Request object.
	 */
	public static function disconnect( $request ) {
		...
		if ( $success ) {
			wp_send_json_success(
				array(
					'message' => __( 'Your subscription has been disconnected.', 'fincommerce' ),
				)
			);
		} else {
			wp_send_json_error(
				array(
					'message' => __( 'There was an error disconnecting your subscription. Please try again.', 'fincommerce' ),
				),
				400
			);
		}
	...
	/**
	 * Activate a FinCommerce.com product.
	...
	public static function activate( $request ) {
		...
		if ( ! $subscription ) {
			wp_send_json_error(
				array(
					'message' => __( 'We couldn\'t find a subscription for this product.', 'fincommerce' ),
				),
				400
			);
		}
		...
		if ( true !== $subscription['local']['installed'] || ! isset( $subscription['local']['active'] ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'This product is not installed.', 'fincommerce' ),
				),
				400
			);
		}
		...
		if ( true === $subscription['local']['active'] ) {
			wp_send_json_success(
				array(
					'message' => __( 'This product is already active.', 'fincommerce' ),
				),
			);
		}
		...
		if ( 'plugin' === $subscription['product_type'] ) {
			...
			if ( is_wp_error( $success ) ) {
				wp_send_json_error(
					array(
						'message' => __( 'There was an error activating this plugin.', 'fincommerce' ),
					),
					400
				);
			}
			...
			if ( $subscription['local']['slug'] !== $theme->get_stylesheet() ) {
				wp_send_json_error(
					array(
						'message' => __( 'There was an error activating this theme.', 'fincommerce' ),
					),
					400
				);
			}
		...
		wp_send_json_success(
			array(
				'message' => __( 'This product has been activated.', 'fincommerce' ),
			),
		);
	}

	/**
	 * Get the install URL for a FinCommerce.com product.
	 *
	 * @param WP_REST_Request $request Request object.
	 */
	public static function install_url( $request ) {
		...
		if ( true === $subscription['local']['installed'] ) {
			wp_send_json_success(
				array(
					'message' => __( 'This product is already installed.', 'fincommerce' ),
				),
			);
		}
		...
		if ( ! $install_url ) {
			wp_send_json_error(
				array(
					'message' => __( 'There was an error getting the install URL for this product.', 'fincommerce' ),
				),
				400
			);
		}
