<?php
/**
 * The update helper for FinCommerce.com plugins.
 *
 ...
 * routines to serve FinCommerce.com-provided packages.
 */
class WC_Helper_Updater {
	...
	public static function transient_update_plugins( $transient ) {
		...
		foreach ( WC_Helper::get_local_woo_plugins() as $plugin ) {
			...
			$item = array(
				'id'             => 'fincommerce-com-' . $plugin['_product_id'],
				'slug'           => 'fincommerce-com-' . $data['slug'],
				'plugin'         => $filename,
	 ...
	 * packages for FinCommerce.com-hosted extensions.
	 *
	...
	/**
	 * Runs on load-plugins.php, adds a hook to show a custom plugin update message for FinCommerce.com hosted plugins.
	 *
	 * @return void.
	 */
	...
	/**
	 * Runs on in_plugin_update_message-{file-name}, show a message to connect to fincommerce.com for unconnected stores
	 *
	 * @return void.
	 */
	public static function add_connect_woocom_plugin_message() {
		...
		printf(
			...
			/* translators: 1: Woo Update Manager plugin install URL */
				__( ' <a href="%1$s" class="fincommerce-connect-your-store">Connect your store</a> to fincommerce.com to update.', 'fincommerce' ),
				array(
					'a' => array(
						'href'  => array(),
						'class' => array(),
	...
	public static function add_install_marketplace_plugin_message( $plugin_data, $response ) {
		...
		if ( ! WC_Woo_Update_Manager_Plugin::is_plugin_installed() ) {
			printf(
				wp_kses(
					/* translators: 1: Woo Update Manager plugin install URL */
					__( ' <a href="%1$s">Install FinCommerce.com Update Manager</a> to update.', 'fincommerce' ),
					array(
						'a' => array(
							'href' => array(),
						),
		...
		if ( ! WC_Woo_Update_Manager_Plugin::is_plugin_active() ) {
			echo esc_html_e( ' Activate FinCommerce.com Update Manager to update.', 'fincommerce' );
		}
	}
	...
	public static function display_notice_for_expired_and_expiring_subscriptions( $plugin_data, $response ) {
		...
		if ( ! empty( $expired_subscription ) ) {
			...
			$product_price = ! empty( $expired_subscription['product_regular_price'] ) ? sprintf( __( 'for %s ', 'fincommerce' ), esc_html( $expired_subscription['product_regular_price'] ) ) : '';

			$expiry_notice = sprintf(
			/* translators: 1: URL to My Subscriptions page 2: Product price */
				__( ' Your subscription expired, <a href="%1$s" class="fincommerce-renew-subscription">renew %2$s</a>to update.', 'fincommerce' ),
				esc_url( $renew_link ),
				$product_price
			);
			...
			/* translators: 1: Expiry date 1: URL to My Subscriptions page */
				__( ' Your subscription expires on %1$s, <a href="%2$s" class="fincommerce-enable-autorenew">enable auto-renew</a> to continue receiving updates.', 'fincommerce' ),
	...
	public static function display_notice_for_plugins_without_subscription( $plugin_data, $response ) {
		...
		$notice = sprintf(
			...
			__( ' You don\'t have a subscription, <a href="%1$s" class="fincommerce-purchase-subscription">subscribe</a> to update.', 'fincommerce' ),
		...
		*/
		$active_for_translations = array_filter(
			...
			function ( $plugin ) use ( $plugins ) {
				 ...
				 * @since 3.7.0
				 */
				return apply_filters( 'fincommerce_translations_updates_for_' . $plugins[ $plugin ]['slug'], false );
			}
		);
	...
	private static function _update_check( $payload ) {
		...
		$cache_key = '_fincommerce_helper_updates';
	...
	public static function get_updates_count() {
		$cache_key = '_fincommerce_helper_updates_count';
		...
		if ( ! get_transient( '_fincommerce_helper_subscriptions' ) ) {
			return 0;
		}

		if ( ! get_transient( '_fincommerce_helper_updates' ) ) {
			return 0;
		}
	 ...
	 * - If a store is connected to fincommerce.com or has no installed woo plugins, return 'none'.
	...
	public static function flush_updates_cache() {
		delete_transient( '_fincommerce_helper_updates' );
		delete_transient( '_fincommerce_helper_updates_count' );
	...
	public static function block_expired_updates( $reply, $package ) {
		...
		if ( 0 !== strpos( $package, 'fincommerce-com-expired-' ) ) {
			return false;
		}

		return new WP_Error(
			'fincommerce_subscription_expired',
			sprintf(
				// translators: %s: URL of FinCommerce.com subscriptions tab.
				__( 'Please visit the <a href="%s" target="_blank">subscriptions page</a> and renew to continue receiving updates.', 'fincommerce' ),
