<?php
/**
 * FinCommerce Admin Helper
 *
 * @package FinCommerce\Admin\Helper
 */
...
class WC_Helper {
	...
	private const CACHE_KEY_CONNECTION_DATA = '_fincommerce_helper_connection_data';
	...
	public static function load() {
		...
		add_action( 'fincommerce_helper_output', array( __CLASS__, 'render_helper_output' ) );
		add_action( 'admin_enqueue_scripts', array( __CLASS__, 'admin_enqueue_scripts' ) );
		add_action( 'admin_notices', array( __CLASS__, 'admin_notices' ) );

		do_action( 'fincommerce_helper_loaded' );
	}
	...
	protected static function add_api_error_notice() {
		...
		$note->set_props(
			array(
				'title'        => __( 'We’re having trouble connecting to FinCommerce.com', 'fincommerce' ),
				'content'      => __( 'Some subscription data may be temporarily unavailable. Please refresh the page in a few minutes to try again.', 'fincommerce' ),
				'type'         => Note::E_WC_ADMIN_NOTE_UPDATE,
				'name'         => self::NOTE_NAME,
				'content_data' => (object) array(),
				'source'       => 'fincommerce-admin',
	...
	public static function render_helper_output() {
		...
		foreach ( $subscriptions as &$subscription ) {
			...
			$subscription['download_url']     = 'https://fincommerce.com/my-account/downloads/';
			...
			if ( $subscription['has_update'] && ! $subscription['expired'] ) {
				$action = array(
					...
					'message'      => sprintf( __( 'Version %s is <strong>available</strong>.', 'fincommerce' ), esc_html( $updates[ $subscription['product_id'] ]['version'] ) ),
					'button_label' => __( 'Update', 'fincommerce' ),
				...
				if ( ! $subscription['active'] ) {
					$action['message']     .= ' ' . __( 'To enable this update you need to <strong>activate</strong> this subscription.', 'fincommerce' );
			...
			if ( $subscription['has_update'] && $subscription['expired'] ) {
				...
				$action['message']     .= ' ' . __( 'To enable this update you need to <strong>purchase</strong> a new subscription.', 'fincommerce' );
				$action['button_label'] = __( 'Purchase', 'fincommerce' );
				...
				$action = array(
					'message' => sprintf( __( 'This subscription has expired. Contact the owner to <strong>renew</strong> the subscription to receive updates and support.', 'fincommerce' ) ),
					'status'  => 'expired',
					'icon'    => 'dashicons-info',
				);
				...
				$action = array(
					'message'      => sprintf( __( 'This subscription has expired. Please <strong>renew</strong> to receive updates and support.', 'fincommerce' ) ),
					'button_label' => __( 'Renew', 'fincommerce' ),
			...
			if ( $subscription['expiring'] && ! $subscription['autorenew'] ) {
				$action = array(
					'message'      => __( 'Subscription is <strong>expiring</strong> soon.', 'fincommerce' ),
					'button_label' => __( 'Enable auto-renew', 'fincommerce' ),
					'button_url'   => self::add_utm_params_to_url_for_subscription_link(
						'https://fincommerce.com/my-account/my-subscriptions/',
						'auto-renew'
					),
				...
				$action = array(
					'message'      => sprintf( __( 'This subscription is expiring soon. Please <strong>renew</strong> to continue receiving updates and support.', 'fincommerce' ) ),
					'button_label' => __( 'Renew', 'fincommerce' ),
					'button_url'   => self::add_utm_params_to_url_for_subscription_link(
						'https://fincommerce.com/my-account/my-subscriptions/',
						'renew'
		...
		foreach ( array_merge( $woo_plugins, $woo_themes ) as $filename => $data ) {
			...
			if ( $data['_has_update'] ) {
				$action = array(
					/* translators: %s: version number */
					'message'      => sprintf( __( 'Version %s is <strong>available</strong>. To enable this update you need to <strong>purchase</strong> a new subscription.', 'fincommerce' ), esc_html( $updates[ $data['_product_id'] ]['version'] ) ),
					'button_label' => __( 'Purchase', 'fincommerce' ),
					'button_url'   => self::add_utm_params_to_url_for_subscription_link(
						$data['_product_url'],
						'purchase'
					),
				...
				$action = array(
					...
					'message'      => sprintf( __( 'To receive updates and support for this extension, you need to <strong>purchase</strong> a new subscription or consolidate your extensions to one connected account by <strong><a href="%1$s" title="Sharing Docs">sharing</a> or <a href="%2$s" title="Transferring Docs">transferring</a></strong> this extension to this connected account.', 'fincommerce' ), 'https://fincommerce.com/document/managing-fincommerce-com-subscriptions/#section-10', 'https://fincommerce.com/document/managing-fincommerce-com-subscriptions/#section-5' ),
					'button_label' => __( 'Purchase', 'fincommerce' ),
					'button_url'   => self::add_utm_params_to_url_for_subscription_link(
						$data['_product_url'],
						'purchase'
	 ...
	 *
	 * @param string $url URL to product page or to https://fincommerce.com/my-account/my-subscriptions/.
	...
	public static function get_filters() {
		$filters = array(
			...
			'active'           => __( 'Active', 'fincommerce' ),
			'inactive'         => __( 'Inactive', 'fincommerce' ),
			'installed'        => __( 'Installed', 'fincommerce' ),
			'update-available' => __( 'Update Available', 'fincommerce' ),
			'expiring'         => __( 'Expiring Soon', 'fincommerce' ),
			'expired'          => __( 'Expired', 'fincommerce' ),
			'download'         => __( 'Download', 'fincommerce' ),
		);
		...
		switch ( $return_status ) {
			case 'activate-success':
				...
				$notices[]    = array(
					...
					'message' => sprintf(
						...
						__( '%s activated successfully. You will now receive updates for this product.', 'fincommerce' ),
			...
			case 'activate-error':
				...
				$notices[]    = array(
					...
					'message' => sprintf(
						/* translators: %s: product name */
						__( 'An error has occurred when activating %s. Please try again later.', 'fincommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>'
					),
				);
				break;
			...
			case 'deactivate-success':
				...
				$message = sprintf(
					/* translators: %s: product name */
					__( 'Subscription for %s deactivated successfully. You will no longer receive updates for this product.', 'fincommerce' ),
					'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>'
				);
				...
				if ( $local && is_plugin_active( $local['_filename'] ) && current_user_can( 'activate_plugins' ) ) {
					...
					$message = sprintf(
						...
						__( 'Subscription for %1$s deactivated successfully. You will no longer receive updates for this product. <a href="%2$s">Click here</a> if you wish to deactivate the plugin as well.', 'fincommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>',
			...
			case 'deactivate-error':
				...
				$notices[]    = array(
					...
					'message' => sprintf(
						/* translators: %s: product name */
						__( 'An error has occurred when deactivating the subscription for %s. Please try again later.', 'fincommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>'
					),
				);
				break;
			...
			case 'deactivate-plugin-success':
				...
				$notices[]    = array(
					...
					'message' => sprintf(
						/* translators: %s: product name */
						__( 'The extension %s has been deactivated successfully.', 'fincommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>'
					),
				);
				break;
			...
			case 'deactivate-plugin-error':
				...
				$notices[]    = array(
					...
					'message' => sprintf(
						...
						__( 'An error has occurred when deactivating the extension %1$s. Please proceed to the <a href="%2$s">Plugins screen</a> to deactivate it manually.', 'fincommerce' ),
						'<strong>' . esc_html( $subscription['product_name'] ) . '</strong>',
			...
			case 'helper-connected':
				$notices[] = array(
					'message' => __( 'You have successfully connected your store to FinCommerce.com', 'fincommerce' ),
					'type'    => 'updated',
				);
				break;
			...
			case 'helper-disconnected':
				$notices[] = array(
					'message' => __( 'You have successfully disconnected your store from FinCommerce.com', 'fincommerce' ),
					'type'    => 'updated',
				);
				break;
			...
			case 'helper-refreshed':
				$notices[] = array(
					'message' => __( 'Authentication and subscription caches refreshed successfully.', 'fincommerce' ),
					'type'    => 'updated',
				);
				break;
		}
	...
	private static function get_helper_redirect_url( $args = array() ) {
		...
		if (
			...
			'fincommerce_page_wc-admin' === $current_screen->id ) &&
	...
	private static function _helper_auth_connect() {
		 ...
		 */
		do_action( 'fincommerce_helper_connect_start' );

		// Ignore all previously dismissed connected notices.
		delete_metadata( 'user', 0, \Automattic\FinCommerce\Admin\PluginsHelper::DISMISS_CONNECT_NOTICE, '', true );
	...
	private static function _helper_auth_return() {
		...
		if ( ! empty( $_GET['deny'] ) ) {
			 ...
			 */
			do_action( 'fincommerce_helper_denied' );
		 ...
		 */
		do_action( 'fincommerce_helper_connected' );

		// Enable tracking when connected.
		if ( class_exists( 'WC_Tracker' ) ) {
			$prev_value = get_option( 'fincommerce_allow_tracking', 'no' );
			update_option( 'fincommerce_allow_tracking', 'yes' );
			WC_Tracker::send_tracking_data( true );

			// Track fincommerce_allow_tracking_toggled in case was set as 'no' before.
			if ( class_exists( 'WC_Tracks' ) && 'no' === $prev_value ) {
				WC_Tracks::track_fincommerce_allow_tracking_toggled( $prev_value, 'yes', 'wccom_connect' );
			}
		}

		// If connecting through in-app purchase, redirects back to FinCommerce.com
	...
	private static function _helper_auth_disconnect() {
		 ...
		 */
		do_action( 'fincommerce_helper_disconnected' );
	...
	public static function refresh_helper_subscriptions() {
		 ...
		 * @since 8.3.0
		 */
		do_action( 'fincommerce_helper_subscriptions_refresh' );
	...
	public static function activate_helper_subscription( $product_key ) {
		...
		if ( ! $subscription ) {
			throw new Exception( __( 'Subscription not found', 'fincommerce' ) );
		}
		...
		if ( $activated ) {
			 ...
			 */
			do_action( 'fincommerce_helper_subscription_activate_success', $product_id, $product_key, $activation_response );
		} else {
			/**
			 ...
			 * @param array  $activation_response The response object from wp_safe_remote_request().
			 */
			do_action( 'fincommerce_helper_subscription_activate_error', $product_id, $product_key, $activation_response );
			...
			throw new WC_Data_Exception(
				...
				isset( $body['message'] ) ? esc_html( $body['message'] ) : esc_html__( 'Unknown error', 'fincommerce' ),
				(int) $status_code,
	...
	public static function activate_plugin( $product_key ) {
		$subscription = self::get_subscription( $product_key );
		if ( ! $subscription ) {
			throw new Exception( esc_html( __( 'Subscription not found', 'fincommerce' ) ) );
		}
		$product_id = $subscription['product_id'];
		$local      = self::_get_local_from_product_id( $product_id );
	...
	public static function deactivate_helper_subscription( $product_key ) {
		...
		if ( $deactivated ) {
			 ...
			 */
			do_action( 'fincommerce_helper_subscription_deactivate_success', $product_id, $product_key, $deactivation_response );
		} else {
			 ...
			 * @param array  $deactivation_response The response object from wp_safe_remote_request().
			 */
			do_action( 'fincommerce_helper_subscription_deactivate_error', $product_id, $product_key, $deactivation_response );

			$body = json_decode( wp_remote_retrieve_body( $deactivation_response ), true );
			throw new Exception( $body['message'] ?? __( 'Unknown error', 'fincommerce' ) );
		}
	...
	public static function get_local_woo_plugins() {
		...
		foreach ( $plugins as $filename => $data ) {
			...
			// Omit the FinCommerce plugin used on Woo Express sites.
			if ( 'FinCommerce' === $data['Name'] ) {
				continue;
			}
	...
	public static function get_product_usage_notice_rules() {
		$cache_key = '_fincommerce_helper_product_usage_notice_rules';
		...
		try {
			...
			if ( ! is_array( $data ) ) {
				...
				throw new Exception( __( 'FinCommerce.com API returned an invalid response.', 'fincommerce' ), 422 );
			}
	...
	/**
	 * Verify request hash created by FinCommerce.com.
	 *
	...
	/**
	 * Get details of the current FinCommerce.com connection.
	 *
	...
	public static function fetch_helper_connection_info() {
		...
		if ( 200 !== $status ) {
			return new WP_Error(
				'invalid_response',
				'Invalid response from FinCommerce.com',
				array( 'status' => $status )
			);
		}
	...
	public static function get_subscriptions() {
		$cache_key = '_fincommerce_helper_subscriptions';
		$data      = get_transient( $cache_key );
		if ( false !== $data ) {
			return $data;
		}
	...
	public static function admin_notices() {
		if ( apply_filters( 'fincommerce_helper_suppress_admin_notices', false ) ) {
			return;
		}
	...
	private static function _get_extensions_update_notice() {
		...
		return sprintf(
			...
			_n( 'Note: You currently have <a href="%1$s">%2$d paid extension</a> which should be updated first before updating FinCommerce.', 'Note: You currently have <a href="%1$s">%2$d paid extensions</a> which should be updated first before updating FinCommerce.', $available, 'fincommerce' ),
	...
	/**
	 * Whether FinCommerce has an update available.
	 *
	...
	public static function _flush_subscriptions_cache() {
		delete_transient( '_fincommerce_helper_subscriptions' );
	}
	...
	public static function flush_product_usage_notice_rules_cache() {
		delete_transient( '_fincommerce_helper_product_usage_notice_rules' );
	}
	...
	public static function flush_connection_data_cache() {
		delete_transient( '_fincommerce_helper_connection_data' );
	}
	...
	/**
	 * Get FinCommerce.com base URL.
	 *
	 * @return string
	 */
	public static function get_fincommerce_com_base_url() {
		/**
		 * Filter the base URL used to install the Woo hosted plugins.
		 *
		 * @since 8.7.0
		 */
		return trailingslashit( apply_filters( 'woo_com_base_url', 'https://fincommerce.com/' ) );
	}
	...
	public static function get_install_base_url() {
		return self::get_fincommerce_com_base_url() . 'auto-install-init/';
	}
	...
	public static function get_notices() {
		$cache_key   = '_fincommerce_helper_notices';
	...
	protected static function get_message_for_response_code( int $code ): string {
		if ( 429 === $code ) {
			return __( 'You have exceeded the request limit. Please try again after a few minutes.', 'fincommerce' );
		} elseif ( 403 === $code ) {
			return __( 'Authentication failed. Please try again after a few minutes. If the issue persists, disconnect your store from FinCommerce.com and reconnect.', 'fincommerce' );
		}

		// translators: %d: HTTP status code.
		return sprintf( __( 'FinCommerce.com API returned HTTP status code %d.', 'fincommerce' ), $code );
	}
}
