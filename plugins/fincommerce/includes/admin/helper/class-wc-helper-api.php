 ...
 * WC_Helper_API Class
 *
 * Provides a communication interface with the FinCommerce.com Helper API.
 */
class WC_Helper_API {
	/**
	 ...
	 * Note that sslverify will be turned off for the fincommerce.dev + WP_DEBUG combination.
	 * The URL can be changed on plugins_loaded before priority 10.
	 */
	public static function load() {
		self::$api_base = apply_filters( 'fincommerce_helper_api_base', 'https://fincommerce.com/wp-json/helper/1.0' );
	}
	...
	public static function request( $endpoint, $args = array() ) {
		...
		if ( ! empty( $args['authenticated'] ) ) {
			if ( ! self::_authenticate( $url, $args ) ) {
				return new WP_Error( 'authentication', __( 'Authentication failed. Please try again after a few minutes. If the issue persists, disconnect your store from FinCommerce.com and reconnect.', 'fincommerce' ), 401 );
			}
		}

		if ( ! isset( $args['user-agent'] ) ) {
			$args['user-agent'] = 'FinCommerce/' . WC()->version . '; ' . get_bloginfo( 'url' );
		}
		 ...
		 */
		$args = apply_filters( 'fincommerce_helper_api_request_args', $args, $endpoint );
