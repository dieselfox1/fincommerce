...
class WC_Report_Customer_List extends WP_List_Table {
	...
	public function no_items() {
		esc_html_e( 'No customers found.', 'fincommerce' );
	}
	...
	public function output_report() {
		...
		if ( ! empty( $_GET['refresh'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'refresh' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
			...
			echo '<div class="updated"><p>' . sprintf( esc_html__( 'Refreshed stats for %s', 'fincommerce' ), esc_html( $user->display_name ) ) . '</p></div>';
		}

		echo '<form method="post" id="fincommerce_customers">';

		$this->search_box( __( 'Search customers', 'fincommerce' ), 'customer_search' );
		$this->display();
	...
	public function column_default( $user, $column_name ) {
		switch ( $column_name ) {
			...
			case 'last_order':
				...
				if ( ! empty( $orders ) ) {
					$order = $orders[0];
					return '<a href="' . admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) . '">' . _x( '#', 'hash before order number', 'fincommerce' ) . $order->get_order_number() . '</a> &ndash; ' . wc_format_datetime( $order->get_date_created() );
				} else {
					return '-';
				}
			...
			case 'wc_actions':
				...
				?><p>
					<?php
					do_action( 'fincommerce_admin_user_actions_start', $user );

					$actions = array();

					$actions['refresh'] = array(
						'url'    => wp_nonce_url( add_query_arg( 'refresh', $user->ID ), 'refresh' ),
						'name'   => __( 'Refresh stats', 'fincommerce' ),
						'action' => 'refresh',
					);

					$actions['edit'] = array(
						'url'    => admin_url( 'user-edit.php?user_id=' . $user->ID ),
						'name'   => __( 'Edit', 'fincommerce' ),
						'action' => 'edit',
					);

					$actions['view'] = array(
						'url'    => admin_url( 'edit.php?post_type=shop_order&_customer_user=' . $user->ID ),
						'name'   => __( 'View orders', 'fincommerce' ),
						'action' => 'view',
					);
					...
					if ( $orders ) {
						$actions['link'] = array(
							...
							'name'   => __( 'Link previous orders', 'fincommerce' ),
							'action' => 'link',
						);
					}

					$actions = apply_filters( 'fincommerce_admin_user_actions', $actions, $user );

					foreach ( $actions as $action ) {
						printf( '<a class="button tips %s" href="%s" data-tip="%s">%s</a>', esc_attr( $action['action'] ), esc_url( $action['url'] ), esc_attr( $action['name'] ), esc_attr( $action['name'] ) );
					}

					do_action( 'fincommerce_admin_user_actions_end', $user );
					?>
				</p>
				<?php
	...
	public function get_columns() {
		$columns = array(
			'customer_name' => __( 'Name (Last, First)', 'fincommerce' ),
			'username'      => __( 'Username', 'fincommerce' ),
			'email'         => __( 'Email', 'fincommerce' ),
			'location'      => __( 'Location', 'fincommerce' ),
			'orders'        => __( 'Orders', 'fincommerce' ),
			'spent'         => __( 'Money spent', 'fincommerce' ),
			'last_order'    => __( 'Last order', 'fincommerce' ),
			'wc_actions'    => __( 'Actions', 'fincommerce' ),
		);
	...
	public function prepare_items() {
		...
		$query = new WP_User_Query(
			apply_filters(
				'fincommerce_admin_report_customer_list_user_query_args',
				array(
