 ...
 * @author      WooThemes
 * @category    Admin
 * @package     FinCommerce\Admin\Reports
 * @version     3.3.0
 */
...
class WC_Report_Downloads extends WP_List_Table {
	...
	public function output_report() {
		...
		if ( ! empty( $_GET['permission_id'] ) ) { // WPCS: input var ok.
			...
			try {
				...
				wp_die( sprintf( esc_html__( 'Permission #%d not found.', 'fincommerce' ), esc_html( $permission_id ) ) );
			}
		}

		echo '<h1>' . esc_html__( 'Customer downloads', 'fincommerce' );

		$filters      = $this->get_filter_vars();
		$filter_list  = array();
		$filter_names = array(
			'product_id'      => __( 'Product', 'fincommerce' ),
			'download_id'     => __( 'File ID', 'fincommerce' ),
			'permission_id'   => __( 'Permission ID', 'fincommerce' ),
			'order_id'        => __( 'Order', 'fincommerce' ),
			'user_id'         => __( 'User', 'fincommerce' ),
			'user_ip_address' => __( 'IP address', 'fincommerce' ),
		);
		...
		foreach ( $filters as $key => $value ) {
			...
			switch ( $key ) {
				case 'order_id':
					...
					if ( $order ) {
						$display_value = _x( '#', 'hash before order number', 'fincommerce' ) . $order->get_order_number();
					} else {
						break 2;
					}
					break;
			...
			$filter_list[] = $filter_names[ $key ] . ' ' . $display_value . ' <a href="' . esc_url( remove_query_arg( $key ) ) . '" class="fincommerce-reports-remove-filter">&times;</a>';
		}

		echo '</h1>';

		echo '<div id="active-filters" class="fincommerce-reports-wide"><h2>';
		echo esc_html__( 'Active filters', 'fincommerce' ) . ': ';
		...
		switch ( $column_name ) {
			...
			case 'product':
				if ( ! empty( $product ) ) {
					...
					echo '<a href="' . esc_url( add_query_arg( 'product_id', $product->get_id() ) ) . '">' . esc_html__( 'Filter by product', 'fincommerce' ) . '</a>';
					echo '</div>';
				}
				break;
			case 'file':
				if ( ! empty( $permission ) && ! empty( $product ) ) {
					...
					if ( false === $file ) {
						echo esc_html__( 'File does not exist', 'fincommerce' );
					} else {
						echo esc_html( $file->get_name() . ' - ' . basename( $file->get_file() ) );

						echo '<div class="row-actions">';
						echo '<a href="' . esc_url( add_query_arg( 'download_id', $permission->get_download_id() ) ) . '">' . esc_html__( 'Filter by file', 'fincommerce' ) . '</a>';
						echo '</div>';
					}
				}
				break;
			case 'order':
				if ( ! empty( $permission ) && ( $order = wc_get_order( $permission->order_id ) ) ) {
					edit_post_link( esc_html( _x( '#', 'hash before order number', 'fincommerce' ) . $order->get_order_number() ), '', '', $permission->order_id, 'view-link' );

					echo '<div class="row-actions">';
					echo '<a href="' . esc_url( add_query_arg( 'order_id', $order->get_id() ) ) . '">' . esc_html__( 'Filter by order', 'fincommerce' ) . '</a>';
					echo '</div>';
				}
				break;
			case 'user':
				if ( $item->user_id > 0 ) {
					...
					if ( ! empty( $user ) ) {
						...
						echo '<div class="row-actions">';
						echo '<a href="' . esc_url( add_query_arg( 'user_id', $item->user_id ) ) . '">' . esc_html__( 'Filter by user', 'fincommerce' ) . '</a>';
						echo '</div>';
					}
				} else {
					esc_html_e( 'Guest', 'fincommerce' );
				}
				break;
			case 'user_ip_address':
				...
				echo '<div class="row-actions">';
				echo '<a href="' . esc_url( add_query_arg( 'user_ip_address', $item->user_ip_address ) ) . '">' . esc_html__( 'Filter by IP address', 'fincommerce' ) . '</a>';
				echo '</div>';
				break;
		}
	}
	...
	public function get_columns() {
		$columns = array(
			'timestamp'       => __( 'Timestamp', 'fincommerce' ),
			'product'         => __( 'Product', 'fincommerce' ),
			'file'            => __( 'File', 'fincommerce' ),
			'order'           => __( 'Order', 'fincommerce' ),
			'user'            => __( 'User', 'fincommerce' ),
			'user_ip_address' => __( 'IP address', 'fincommerce' ),
		);
	...
	public function prepare_items() {
		...
		$per_page = max( 1, apply_filters( 'fincommerce_admin_downloads_report_downloads_per_page', 20 ) );
	...
	public function no_items() {
		esc_html_e( 'No customer downloads found.', 'fincommerce' );
	}
	...
	public function get_items( $current_page, $per_page ) {
		...
		if ( ! is_null( $filters->product_id ) || ! is_null( $filters->download_id ) || ! is_null( $filters->order_id ) ) {
			$query_from .= " LEFT JOIN {$wpdb->prefix}fincommerce_downloadable_product_permissions as permissions on downloads.permission_id = permissions.permission_id ";
		}
		...
		$query_from  = apply_filters( 'fincommerce_report_downloads_query_from', $query_from );
