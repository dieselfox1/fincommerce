...
class WC_Report_Taxes_By_Code extends WC_Admin_Report {
	...
	public function get_main_chart() {
		...
		<table class="widefat">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Tax', 'fincommerce' ); ?></th>
					<th><?php esc_html_e( 'Rate', 'fincommerce' ); ?></th>
					<th class="total_row"><?php esc_html_e( 'Number of orders', 'fincommerce' ); ?></th>
					<th class="total_row"><?php esc_html_e( 'Tax amount', 'fincommerce' ); ?> <?php echo wc_help_tip( __( 'This is the sum of the "Tax rows" tax amount within your orders.', 'fincommerce' ) ); ?></th>
					<th class="total_row"><?php esc_html_e( 'Shipping tax amount', 'fincommerce' ); ?> <?php echo wc_help_tip( __( 'This is the sum of the "Tax rows" shipping tax amount within your orders.', 'fincommerce' ) ); ?></th>
					<th class="total_row"><?php esc_html_e( 'Total tax', 'fincommerce' ); ?> <?php echo wc_help_tip( __( 'This is the total tax for the rate (shipping tax + product tax).', 'fincommerce' ) ); ?></th>
				</tr>
			</thead>
			<?php if ( ! empty( $tax_rows ) ) : ?>
				<tbody>
					...
					foreach ( $tax_rows as $rate_id => $tax_row ) {
						$rate = $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate FROM {$wpdb->prefix}fincommerce_tax_rates WHERE tax_rate_id = %d;", $rate_id ) );
						?>
						<tr>
							<th scope="row"><?php echo wp_kses_post( apply_filters( 'fincommerce_reports_taxes_tax_rate', $tax_row->tax_rate, $rate_id, $tax_row ) ); ?></th>
							<td><?php echo wp_kses_post( apply_filters( 'fincommerce_reports_taxes_rate', $rate, $rate_id, $tax_row ) ); ?>%</td>
				...
				<tfoot>
					<tr>
						<th scope="row" colspan="3"><?php esc_html_e( 'Total', 'fincommerce' ); ?></th>
			...
			<?php else : ?>
				<tbody>
					<tr>
						<td><?php esc_html_e( 'No taxes found in this period', 'fincommerce' ); ?></td>
					</tr>
				</tbody>
			<?php endif; ?>
		</table>
