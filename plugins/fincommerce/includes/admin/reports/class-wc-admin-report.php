...
class WC_Admin_Report {
	...
	public function get_order_report_data( $args = array() ) {
		...
		$order_status = apply_filters( 'fincommerce_reports_order_statuses', $order_status );
		...
		foreach ( ( $data + $where ) as $raw_key => $value ) {
			...
			switch ( $type ) {
				...
				case 'parent_meta':
					...
					break;
				case 'order_item_meta':
					$joins['order_items'] = "{$join_type} JOIN {$wpdb->prefix}fincommerce_order_items AS order_items ON (posts.ID = order_items.order_id)";

					if ( ! empty( $value['order_item_type'] ) ) {
						$joins['order_items'] .= " AND (order_items.order_item_type = '{$value['order_item_type']}')";
					}

					$joins[ "order_item_meta_{$key}" ] = "{$join_type} JOIN {$wpdb->prefix}fincommerce_order_itemmeta AS order_item_meta_{$key} ON " .
		...
		if ( ! empty( $where_meta ) ) {
			foreach ( $where_meta as $value ) {
				...
				if ( 'order_item_meta' === $type ) {
					...
					$joins['order_items']              = "{$join_type} JOIN {$wpdb->prefix}fincommerce_order_items AS order_items ON posts.ID = order_items.order_id";
					$joins[ "order_item_meta_{$key}" ] = "{$join_type} JOIN {$wpdb->prefix}fincommerce_order_itemmeta AS order_item_meta_{$key} ON order_items.order_item_id = order_item_meta_{$key}.order_item_id";
		...
		$query = apply_filters( 'fincommerce_reports_get_order_report_query', $query );
		$query = implode( ' ', $query );
		...
		if ( $debug || $nocache ) {
			...
			$result = apply_filters( 'fincommerce_reports_get_order_report_data', $wpdb->$query_type( $query ), $data );
		} else {
	...
	public function sales_sparkline( $id = '', $days = 7, $type = 'sales' ) {
		...
		if ( 'sales' === $type ) {
			...
			$tooltip = sprintf( __( 'Sold %1$s worth in the last %2$d days', 'fincommerce' ), wp_strip_all_tags( wc_price( $total ) ), $days );
		} else {
	...
	/**
	 * Return currency tooltip JS based on FinCommerce currency position settings.
	 *
	 * @return string
	 */
	public function get_currency_tooltip() {
		switch ( get_option( 'fincommerce_currency_pos' ) ) {
			case 'right':
				$currency_tooltip = 'append_tooltip: "' . get_fincommerce_currency_symbol() . '"';
				break;
			case 'right_space':
				$currency_tooltip = 'append_tooltip: "&nbsp;' . get_fincommerce_currency_symbol() . '"';
				break;
			case 'left':
				$currency_tooltip = 'prepend_tooltip: "' . get_fincommerce_currency_symbol() . '"';
				break;
			case 'left_space':
			default:
				$currency_tooltip = 'prepend_tooltip: "' . get_fincommerce_currency_symbol() . '&nbsp;"';
