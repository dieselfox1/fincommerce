<?php
/**
 * FinCommerce Admin API Keys Class
 *
...
class WC_Admin_API_Keys {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_save_settings_advanced_keys', array( $this, 'allow_save_settings' ) );
	}
	...
	private function is_api_keys_settings_page() {
		return is_wc_admin_settings_page() && isset( $_GET['tab'], $_GET['section'] ) && 'advanced' === $_GET['tab'] && 'keys' === $_GET['section']; // phpcs:ignore finpress.Security.NonceVerification.Recommended
	}
		...
		if ( isset( $_GET['create-key'] ) || isset( $_GET['edit-key'] ) ) {
			...
			if ( $key_id && $user_id && ! current_user_can( 'edit_user', $user_id ) ) {
				if ( get_current_user_id() !== $user_id ) {
					wp_die( esc_html__( 'You do not have permission to edit this API Key', 'fincommerce' ) );
				}
			}
	...
	public function screen_option() {
		...
		if ( ! isset( $_GET['create-key'] ) && ! isset( $_GET['edit-key'] ) && $this->is_api_keys_settings_page() ) { // WPCS: input var okay, CSRF ok.
			...
			add_screen_option(
				...
				array(
					'default' => 10,
					'option'  => 'fincommerce_keys_per_page',
				)
			);
		}
	}
	...
	private static function table_list_output() {
		...
		echo '<h2 class="wc-table-list-header">' . esc_html__( 'REST API', 'fincommerce' ) . ' <a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=advanced&section=keys&create-key=1' ) ) . '" class="page-title-action">' . esc_html__( 'Add key', 'fincommerce' ) . '</a></h2>';

		// Get the API keys count.
		$count = $wpdb->get_var( "SELECT COUNT(key_id) FROM {$wpdb->prefix}fincommerce_api_keys WHERE 1 = 1;" );
		...
		if ( absint( $count ) && $count > 0 ) {
			...
			$keys_table_list->search_box( __( 'Search key', 'fincommerce' ), 'key' );
			$keys_table_list->display();
		} else {
			echo '<div class="fincommerce-BlankState fincommerce-BlankState--api">';
			?>
			<h2 class="fincommerce-BlankState-message"><?php esc_html_e( 'The FinCommerce REST API allows external apps to view and manage store data. Access is granted only to those with valid API keys.', 'fincommerce' ); ?></h2>
			<a class="fincommerce-BlankState-cta button-primary button" href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=advanced&section=keys&create-key=1' ) ); ?>"><?php esc_html_e( 'Create an API key', 'fincommerce' ); ?></a>
	...
	public static function notices() {
		if ( isset( $_GET['revoked'] ) ) { // WPCS: input var okay, CSRF ok.
			...
			/* translators: %d: count */
			WC_Admin_Settings::add_message( sprintf( _n( '%d API key permanently revoked.', '%d API keys permanently revoked.', $revoked, 'fincommerce' ), $revoked ) );
		}
	}
	...
	private function revoke_key() {
		...
		if ( isset( $_REQUEST['revoke-key'] ) ) { // WPCS: input var okay, CSRF ok.
			...
			$user_id = (int) $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM {$wpdb->prefix}fincommerce_api_keys WHERE key_id = %d", $key_id ) );

			if ( $key_id && $user_id && ( current_user_can( 'edit_user', $user_id ) || get_current_user_id() === $user_id ) ) {
				$this->remove_key( $key_id );
			} else {
				wp_die( esc_html__( 'You do not have permission to revoke this API Key', 'fincommerce' ) );
			}
		}
	...
	private function bulk_actions() {
		...
		if ( ! current_user_can( 'manage_fincommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to edit API Keys', 'fincommerce' ) );
		}
	...
	private function bulk_revoke_key( $keys ) {
		if ( ! current_user_can( 'remove_users' ) ) {
			wp_die( esc_html__( 'You do not have permission to revoke API Keys', 'fincommerce' ) );
		}
	...
	private function remove_key( $key_id ) {
		...
		$delete = $wpdb->delete( $wpdb->prefix . 'fincommerce_api_keys', array( 'key_id' => $key_id ), array( '%d' ) );
