...
if ( ! class_exists( 'WC_Admin_Profile', false ) ) :
	...
	class WC_Admin_Profile {
		 ...
		 *
		 * @return array Fields to display which are filtered through fincommerce_customer_meta_fields before being returned
		 */
		public function get_customer_meta_fields() {
			$show_fields = apply_filters(
				'fincommerce_customer_meta_fields',
				array(
					'billing'  => array(
						'title'  => __( 'Customer billing address', 'fincommerce' ),
						'fields' => array(
							...
							'billing_country'    => array(
								...
								'type'        => 'select',
								'options'     => array( '' => __( 'Select a country / region&hellip;', 'fincommerce' ) ) + WC()->countries->get_allowed_countries(),
							),
							'billing_state'      => array(
								'label'       => __( 'State / County', 'fincommerce' ),
								'description' => __( 'State / County or state code', 'fincommerce' ),
					...
					'shipping' => array(
						'title'  => __( 'Customer shipping address', 'fincommerce' ),
						'fields' => array(
							'copy_billing'        => array(
								'label'       => __( 'Copy from billing address', 'fincommerce' ),
								'description' => '',
								'class'       => 'js_copy-billing',
								'type'        => 'button',
								'text'        => __( 'Copy', 'fincommerce' ),
							),
		...
		public function add_customer_meta_fields( $user ) {
			if ( ! apply_filters( 'fincommerce_current_user_can_edit_customer_meta_fields', current_user_can( 'manage_fincommerce' ), $user->ID ) ) {
				return;
			}
		...
		public function save_customer_meta_fields( $user_id ) {
			if ( ! apply_filters( 'fincommerce_current_user_can_edit_customer_meta_fields', current_user_can( 'manage_fincommerce' ), $user_id ) ) {
				return;
			}
			...
			foreach ( $save_fields as $fieldset_type => $fieldset ) {
				 ...
				 */
				do_action( 'fincommerce_customer_save_address', $user_id, $address_type );
			}
		}
