<?php
/**
 * FinCommerce Webhooks Table List
 *
...
class WC_Admin_Webhooks_Table_List extends WP_List_Table {
	...
	public function no_items() {
		esc_html_e( 'No webhooks found.', 'fincommerce' );
	}
	...
	public function get_columns() {
		return array(
			...
			'topic'        => __( 'Topic', 'fincommerce' ),
			'delivery_url' => __( 'Delivery URL', 'fincommerce' ),
		);
	}
	...
	public function column_title( $webhook ) {
		...
		$warning_prefix =
			...
			sprintf(
				...
				esc_html__( 'This webhook is configured to be delivered using the Legacy REST API, but the Legacy REST API plugin is not installed on this site.', 'fincommerce' )
			) : '';
		...
		$actions = array(
			...
			'id'     => sprintf( __( 'ID: %d', 'fincommerce' ), $webhook->get_id() ),
			'edit'   => '<a href="' . esc_url( $edit_link ) . '">' . esc_html__( 'Edit', 'fincommerce' ) . '</a>',
			/* translators: %s: webhook name */
			'delete' => '<a class="submitdelete" aria-label="' . esc_attr( sprintf( __( 'Delete "%s" permanently', 'fincommerce' ), $webhook->get_name() ) ) . '" href="' . esc_url(
				wp_nonce_url(
					add_query_arg(
						array(
			...
			) . '">' . esc_html__( 'Delete permanently', 'fincommerce' ) . '</a>',
		);
	...
	protected function get_views() {
		...
		/* translators: %s: count */
		$status_links['all'] = "<a href='admin.php?page=wc-settings&amp;tab=advanced&amp;section=webhooks'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_webhooks, 'posts', 'fincommerce' ), number_format_i18n( $total_webhooks ) ) . '</a>';
		...
		if ( $legacy_webhooks_count > 0 ) {
			...
			$label = $this->get_status_label( __( 'Legacy', 'fincommerce' ), $legacy_webhooks_count );
	...
	public function process_bulk_action() {
		...
		if ( false !== $action ) {
			...
			if ( ! current_user_can( 'manage_fincommerce' ) ) {
				wp_die( esc_html__( 'You do not have permission to edit Webhooks', 'fincommerce' ) );
			}
	...
	public function prepare_items() {
		$per_page     = $this->get_items_per_page( 'fincommerce_webhooks_per_page' );
