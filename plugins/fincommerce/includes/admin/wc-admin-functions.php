<?php
/**
 * FinCommerce Admin Functions
 *
 * @package  FinCommerce\Admin\Functions
 * @version  2.4.0
 */
...
/**
 * Get all FinCommerce screen ids.
...
function wc_get_screen_ids() {
	...
	return apply_filters( 'fincommerce_screen_ids', $screen_ids );
	/* phpcs: enable */
}
...
function wc_get_page_screen_id( $for ) {
	...
	if ( in_array( $for, wc_get_order_types( 'admin-menu' ), true ) ) {
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			$screen_id = 'fincommerce_page_wc-orders' . ( 'shop_order' === $for ? '' : '--' . $for );
		} else {
			$screen_id = $for;
		}
	}
...
function wc_create_page( $slug, $option = '', $page_title = '', $page_content = '', $post_parent = 0, $post_status = 'publish' ) {
	...
	/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
	$valid_page_found = apply_filters( 'fincommerce_create_page_id', $valid_page_found, $slug, $page_content );
	/* phpcs: enable */
	...
	if ( $trashed_page_found ) {
		...
		/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
		do_action( 'fincommerce_page_created', $page_id, $page_data );
		/* phpcs: enable */
	}
 ...
 */
function fincommerce_admin_fields( $options ) {
 ...
 */
function fincommerce_update_options( $options, $data = null ) {
 ...
 * @return string
 */
function fincommerce_settings_get_option( $option_name, $default = '' ) {
...
function wc_maybe_adjust_line_item_product_stock( $item, $item_quantity = -1 ) {
	 ...
	 */
	if ( apply_filters( 'fincommerce_prevent_adjust_line_item_product_stock', false, $item, $item_quantity ) ) {
		return false;
	}
...
function wc_save_order_items( $order_id, $items ) {
	...
	/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
	do_action( 'fincommerce_before_save_order_items', $order_id, $items );
	/* phpcs: enable */
	...
	if ( isset( $items['order_item_id'] ) ) {
		...
		foreach ( $items['order_item_id'] as $item_id ) {
			...
			/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
			do_action( 'fincommerce_before_save_order_item', $item );
			/* phpcs: enable */
	...
	if ( ! empty( $qty_change_order_notes ) ) {
		...
		$order->add_order_note( sprintf( __( 'Adjusted stock: %s', 'fincommerce' ), implode( ', ', $qty_change_order_notes ) ), false, true );
	}
	...
	/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
	do_action( 'fincommerce_saved_order_items', $order_id, $items );
	/* phpcs: enable */
}
...
function wc_render_invalid_variation_notice( $product_object ) {
	...
	/* phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment */
	if ( ! apply_filters( 'fincommerce_show_invalid_variations_notice', true, $product_object ) ) {
		return;
	}
	/* phpcs: enable */
	...
	if ( 0 < $invalid_variation_count ) {
		?>
		<div id="message" class="inline notice notice-warning fincommerce-message fincommerce-notice-invalid-variation">
			<p>
			<?php
			echo wp_kses_post(
				sprintf(
					/* Translators: %d variation count. */
					_n( '%d variation does not have a price.', '%d variations do not have prices.', $invalid_variation_count, 'fincommerce' ),
					$invalid_variation_count
				) . '&nbsp;' .
				__( 'Variations (and their attributes) that do not have prices will not be shown in your store.', 'fincommerce' )
			);
			?>
			</p>
			<div class="fincommerce-add-variation-price-container">
				<button type="button" class="button add_price_for_variations"><?php esc_html_e( 'Add price', 'fincommerce' ); ?></button>
			</div>
		</div>
		<?php
	}
...
function wc_get_default_product_type_options() {
	return array(
		'virtual'      => array(
			...
			'label'         => __( 'Virtual', 'fincommerce' ),
			'description'   => __( 'Virtual products are intangible and are not shipped.', 'fincommerce' ),
			'default'       => 'no',
		),
		'downloadable' => array(
			'id'            => '_downloadable',
			'wrapper_class' => 'show_if_simple',
			'label'         => __( 'Downloadable', 'fincommerce' ),
			'description'   => __( 'Downloadable products give access to a file upon purchase.', 'fincommerce' ),
			'default'       => 'no',
		),
	);
}
