...
class WC_Webhook extends WC_Legacy_Webhook {
	...
	public function process( $arg ) {
		 ...
		 */
		do_action( 'fincommerce_webhook_process_delivery', $this, $arg );
	...
	private function should_deliver( $arg ) {
		 ...
		 * @param mixed      $arg First hook argument.
		 */
		return apply_filters( 'fincommerce_webhook_should_deliver', $should_deliver, $this, $arg );
	}
	...
	private function is_valid_action( $arg ) {
		...
		if ( 0 === strpos( $current_action, 'fincommerce_process_shop' ) || 0 === strpos( $current_action, 'fincommerce_process_product' ) ) {
	...
	private function is_valid_processing_action( $arg ) {
		// The `fincommerce_process_shop_*` and `fincommerce_process_product_*` hooks
	...
	public function deliver( $arg ) {
		...
		$http_args = apply_filters( 'fincommerce_webhook_http_args', $http_args, $arg, $this->get_id() );
		...
		do_action( 'fincommerce_webhook_delivery', $http_args, $response, $duration, $arg, $this->get_id() );
	}
	...
	public function build_payload( $resource_id ) {
		...
		if ( 'deleted' === $event ) {
			...
			if ( ! WC()->legacy_rest_api_is_available() ) {
				throw new \Exception( 'The Legacy REST API plugin is not installed on this site. More information: https://developer.fincommerce.com/2023/10/03/the-legacy-rest-api-will-move-to-a-dedicated-extension-in-fincommerce-9-0/ ' );
			}
		...
		return apply_filters( 'fincommerce_webhook_payload', $payload, $resource, $resource_id, $this->get_id() );
	}
	...
	public function log_delivery( $delivery_id, $request, $response, $duration ) {
		...
		$message = array(
			'Webhook Delivery' => array(
				...
				'Date'        => date_i18n( __( 'M j, Y @ G:i', 'fincommerce' ), strtotime( 'now' ), true ),
				'URL'         => $this->get_delivery_url(),
				'Duration'    => $duration,
				'Request'     => array(
	...
	private function failed_delivery() {
		...
		if ( $failures > apply_filters( 'fincommerce_max_webhook_delivery_failures', 5 ) ) {
			$this->set_status( 'disabled' );

			do_action( 'fincommerce_webhook_disabled_due_delivery_failures', $this->get_id() );
		} else {
	...
	public function deliver_ping() {
		...
		if ( is_wp_error( $test ) ) {
			...
			return new WP_Error( 'error', sprintf( __( 'Error: Delivery URL cannot be reached: %s', 'fincommerce' ), $test->get_error_message() ) );
		}

		if ( 200 !== $response_code ) {
			/* translators: error message */
			return new WP_Error( 'error', sprintf( __( 'Error: Delivery URL returned response code: %s', 'fincommerce' ), absint( $response_code ) ) );
		}
	...
	*/
	...
	public function get_name( $context = 'view' ) {
		return apply_filters( 'fincommerce_webhook_name', $this->get_prop( 'name', $context ), $this->get_id() );
	}
	...
	public function get_status( $context = 'view' ) {
		return apply_filters( 'fincommerce_webhook_status', $this->get_prop( 'status', $context ), $this->get_id() );
	}
	...
	public function get_secret( $context = 'view' ) {
		return apply_filters( 'fincommerce_webhook_secret', $this->get_prop( 'secret', $context ), $this->get_id() );
	}
	...
	public function get_topic( $context = 'view' ) {
		return apply_filters( 'fincommerce_webhook_topic', $this->get_prop( 'topic', $context ), $this->get_id() );
	}
	...
	public function get_delivery_url( $context = 'view' ) {
		return apply_filters( 'fincommerce_webhook_delivery_url', $this->get_prop( 'delivery_url', $context ), $this->get_id() );
	}
	...
	*/
	...
	private function get_topic_hooks( $topic ) {
		$topic_hooks = array(
			'coupon.created'   => array(
				'fincommerce_process_shop_coupon_meta',
				'fincommerce_new_coupon',
			),
			'coupon.updated'   => array(
				'fincommerce_process_shop_coupon_meta',
				'fincommerce_update_coupon',
			),
			...
			'customer.created' => array(
				...
				'fincommerce_new_customer',
			),
			'customer.updated' => array(
				'profile_update',
				'fincommerce_update_customer',
			),
			'customer.deleted' => array(
				'delete_user',
			),
			'order.created'    => array(
				'fincommerce_new_order',
			),
			'order.updated'    => array(
				'fincommerce_update_order',
				'fincommerce_order_refunded',
			),
			'order.deleted'    => array(
				'wp_trash_post',
				'fincommerce_trash_order',
			),
			'order.restored'   => array(
				'untrashed_post',
				'fincommerce_untrash_order',
			),
			'product.created'  => array(
				'fincommerce_process_product_meta',
				'fincommerce_new_product',
				'fincommerce_new_product_variation',
			),
			'product.updated'  => array(
				'fincommerce_process_product_meta',
				'fincommerce_update_product',
				'fincommerce_update_product_variation',
			),
		...
		$topic_hooks = apply_filters( 'fincommerce_webhook_topic_hooks', $topic_hooks, $this );
	...
	public function get_hooks() {
		...
		return apply_filters( 'fincommerce_webhook_hooks', $hooks, $this->get_id() );
	}
	...
	public function get_resource() {
		...
		return apply_filters( 'fincommerce_webhook_resource', $topic[0], $this->get_id() );
	}
	...
	public function get_event() {
		...
		return apply_filters( 'fincommerce_webhook_event', isset( $topic[1] ) ? $topic[1] : '', $this->get_id() );
	}
