<?php
/**
 * FinCommerce Template
 *
 * Functions for the templating system.
 *
 * @package  FinCommerce\Functions
 * @version  2.5.0
 */

use Automattic\Jetpack\Constants;
use Automattic\FinCommerce\Blocks\Utils\CartCheckoutUtils;
use Automattic\FinCommerce\Enums\OrderStatus;
use Automattic\FinCommerce\Enums\PaymentGatewayFeature;
use Automattic\FinCommerce\Enums\ProductType;
use Automattic\FinCommerce\Internal\DataStores\Fulfillments\FulfillmentsDataStore;
use Automattic\FinCommerce\Internal\Fulfillments\Fulfillment;
use Automattic\FinCommerce\Internal\Utilities\HtmlSanitizer;
...
function wc_template_redirect() {
	...
	if ( isset( $wp->query_vars['customer-logout'] ) ) {
		...
		wc_add_notice( sprintf( __( 'Are you sure you want to log out? <a href="%s">Confirm and log out</a>', 'fincommerce' ), wc_logout_url() ) );
		wp_safe_redirect( wc_get_page_permalink( 'myaccount' ) );
		exit;
	}

	// Trigger 404 if trying to access an endpoint on wrong page.
	if ( is_wc_endpoint_url() && ! is_account_page() && ! is_checkout() && apply_filters( 'fincommerce_account_endpoint_page_not_found', true ) ) {
		$wp_query->set_404();
		status_header( 404 );
	...
	if ( is_search() && is_post_type_archive( 'product' ) && apply_filters( 'fincommerce_redirect_single_search_result', true ) && 1 === absint( $wp_query->found_posts ) ) {
...
function wc_gallery_noscript() {
	?>
	<noscript><style>.fincommerce-product-gallery{ opacity: 1 !important; }</style></noscript>
	<?php
}
...
/**
 * Sets up the fincommerce_loop global from the passed args or from the main query.
 *
 * @since 3.3.0
...
function wc_setup_loop( $args = array() ) {
	...
	if ( isset( $GLOBALS['fincommerce_loop'] ) ) {
		$default_args = array_merge( $default_args, $GLOBALS['fincommerce_loop'] );
	}

	$GLOBALS['fincommerce_loop'] = wp_parse_args( $args, $default_args );
}
add_action( 'fincommerce_before_shop_loop', 'wc_setup_loop' );

/**
 * Resets the fincommerce_loop global.
 *
 * @since 3.3.0
 */
function wc_reset_loop() {
	unset( $GLOBALS['fincommerce_loop'] );
}
add_action( 'fincommerce_after_shop_loop', 'fincommerce_reset_loop', 999 );

/**
 * Gets a property from the fincommerce_loop global.
 *
 * @since 3.3.0
...
function wc_get_loop_prop( $prop, $default = '' ) {
	...
	return isset( $GLOBALS['fincommerce_loop'], $GLOBALS['fincommerce_loop'][ $prop ] ) ? $GLOBALS['fincommerce_loop'][ $prop ] : $default;
}

/**
 * Sets a property in the fincommerce_loop global.
 *
 * @since 3.3.0
...
function wc_set_loop_prop( $prop, $value = '' ) {
	...
	$GLOBALS['fincommerce_loop'][ $prop ] = $value;
}

/**
 * Set the current visibility for a product in the fincommerce_loop global.
 *
 * @since 4.4.0
...
/**
 * Gets the cached current visibility for a product from the fincommerce_loop global.
 *
 * @since 4.4.0
...
/**
 * Should the FinCommerce loop be displayed?
 *
 ...
 * @return bool
 */
function fincommerce_product_loop() {
	return have_posts() || 'products' !== fincommerce_get_loop_display_mode();
}
...
function wc_generator_tag( $gen, $type ) {
	...
	switch ( $type ) {
		case 'html':
			$gen .= "\n" . '<meta name="generator" content="FinCommerce ' . esc_attr( $version ) . '">';
			break;
		case 'xhtml':
			$gen .= "\n" . '<meta name="generator" content="FinCommerce ' . esc_attr( $version ) . '" />';
			break;
...
function wc_body_class( $classes ) {
	...
	if ( is_fincommerce() ) {
		...
		$classes[] = 'fincommerce';
		$classes[] = 'fincommerce-page';

	} elseif ( is_checkout() ) {

		$classes[] = 'fincommerce-checkout';
		$classes[] = 'fincommerce-page';

	} elseif ( is_cart() ) {

		$classes[] = 'fincommerce-cart';
		$classes[] = 'fincommerce-page';

	} elseif ( is_account_page() ) {

		$classes[] = 'fincommerce-account';
		$classes[] = 'fincommerce-page';

		$account_page_id = get_option( 'fincommerce_myaccount_page_id' );

		if ( ! empty( $account_page_id ) && ( get_post_field( 'post_name', $account_page_id ) === basename( $wp->request ) && is_user_logged_in() ) ) {
			$classes[] = 'fincommerce-dashboard';
		}
	}

	if ( is_store_notice_showing() ) {
		$classes[] = 'fincommerce-demo-store';
	}

	foreach ( WC()->query->get_query_vars() as $key => $value ) {
		if ( is_wc_endpoint_url( $key ) ) {
			$classes[] = 'fincommerce-' . sanitize_html_class( $key );
		}
	}

	if ( wp_is_block_theme() ) {

		$classes[] = 'fincommerce-uses-block-theme';

	}

	if ( wc_block_theme_has_styles_for_element( 'button' ) ) {

		$classes[] = 'fincommerce-block-theme-has-button-styles';

	}

	$classes[] = 'fincommerce-no-js';
...
function wc_get_default_products_per_row() {
	$columns      = get_option( 'fincommerce_catalog_columns', 4 );
	...
	if ( $min_columns && $columns < $min_columns ) {
		$columns = $min_columns;
		update_option( 'fincommerce_catalog_columns', $columns );
...
function wc_get_default_product_rows_per_page() {
	$rows         = absint( get_option( 'fincommerce_catalog_rows', 4 ) );
	$product_grid = wc_get_theme_support( 'product_grid' );
	...
	if ( $min_rows && $rows < $min_rows ) {
		$rows = $min_rows;
		update_option( 'fincommerce_catalog_rows', $rows );
...
function wc_reset_product_grid_settings() {
	...
	if ( ! empty( $product_grid['default_rows'] ) ) {
		update_option( 'fincommerce_catalog_rows', absint( $product_grid['default_rows'] ) );
	}

	if ( ! empty( $product_grid['default_columns'] ) ) {
		update_option( 'fincommerce_catalog_columns', absint( $product_grid['default_columns'] ) );
	}
...
/**
 * Get classname for fincommerce loops.
 *
 * @since 2.6.0
 * @return string
 */
...
function wc_get_product_class( $class = '', $product = null ) {
	...
	// Note, to change classes you will need to use the newer fincommerce_post_class filter.
	...
	if ( apply_filters( 'fincommerce_get_product_class_include_taxonomies', false ) ) {
	...
	/**
	 * FinCommerce Post Class filter.
	 *
	 * @since 3.6.2
	 * @param array      $classes Array of CSS classes.
	 * @param WC_Product $product Product object.
	 */
	$classes = apply_filters( 'fincommerce_post_class', $classes, $product );
...
function wc_terms_and_conditions_page_id() {
	...
	return apply_filters( 'fincommerce_terms_and_conditions_page_id', 0 < $page_id ? absint( $page_id ) : 0 );
}
...
function wc_privacy_policy_page_id() {
	...
	return apply_filters( 'fincommerce_privacy_policy_page_id', 0 < $page_id ? absint( $page_id ) : 0 );
}
...
function wc_get_terms_and_conditions_checkbox_text() {
	...
	return trim( apply_filters( 'fincommerce_get_terms_and_conditions_checkbox_text', get_option( 'fincommerce_checkout_terms_and_conditions_checkbox_text', sprintf( __( 'I have read and agree to the website %s', 'fincommerce' ), '[terms]' ) ) ) );
}
...
function wc_get_privacy_policy_text( $type = '' ) {
	...
	switch ( $type ) {
		case 'checkout':
			...
			$text = get_option( 'fincommerce_checkout_privacy_policy_text', sprintf( __( 'Your personal data will be used to process your order, support your experience throughout this website, and for other purposes described in our %s.', 'fincommerce' ), '[privacy_policy]' ) );
			break;
		case 'registration':
			/* translators: %s privacy policy page name and link */
			$text = get_option( 'fincommerce_registration_privacy_policy_text', sprintf( __( 'Your personal data will be used to support your experience throughout this website, to manage access to your account, and for other purposes described in our %s.', 'fincommerce' ), '[privacy_policy]' ) );
			break;
	}

	return trim( apply_filters( 'fincommerce_get_privacy_policy_text', $text, $type ) );
}
...
function wc_checkout_privacy_policy_text() {
	echo '<div class="fincommerce-privacy-policy-text">';
...
/**
 * Replaces placeholders with links to FinCommerce policy pages.
 *
 * @since 3.4.0
...
function wc_replace_policy_page_link_placeholders( $text ) {
	...
	$terms_page_id   = wc_terms_and_conditions_page_id();
	$privacy_link    = $privacy_page_id ? '<a href="' . esc_url( get_permalink( $privacy_page_id ) ) . '" class="fincommerce-privacy-policy-link" target="_blank">' . __( 'privacy policy', 'fincommerce' ) . '</a>' : __( 'privacy policy', 'fincommerce' );
	$terms_link      = $terms_page_id ? '<a href="' . esc_url( get_permalink( $terms_page_id ) ) . '" class="fincommerce-terms-and-conditions-link" target="_blank">' . __( 'terms and conditions', 'fincommerce' ) . '</a>' : __( 'terms and conditions', 'fincommerce' );
...
if ( ! function_exists( 'fincommerce_content' ) ) {

	/**
	 * Output FinCommerce content.
	 *
	 * This function is only used in the optional 'fincommerce.php' template.
	 * which people can add to their themes to add basic fincommerce support.
	 * without hooks or modifying core templates.
	 */
	function fincommerce_content() {
		...
		if ( is_singular( 'product' ) ) {
			...
			<?php if ( apply_filters( 'fincommerce_show_page_title', true ) ) : ?>

				<h1 class="page-title"><?php fincommerce_page_title(); ?></h1>

			<?php endif; ?>

			<?php do_action( 'fincommerce_archive_description' ); ?>

			<?php if ( fincommerce_product_loop() ) : ?>

				<?php do_action( 'fincommerce_before_shop_loop' ); ?>

				<?php fincommerce_product_loop_start(); ?>
				...
				<?php fincommerce_product_loop_end(); ?>

				<?php do_action( 'fincommerce_after_shop_loop' ); ?>
...
if ( ! function_exists( 'fincommerce_output_content_wrapper' ) ) {

	/**
	 * Output the start of the page wrapper.
	 */
	function fincommerce_output_content_wrapper() {
		wc_get_template( 'global/wrapper-start.php' );
	}
}
if ( ! function_exists( 'fincommerce_output_content_wrapper_end' ) ) {

	/**
	 * Output the end of the page wrapper.
	 */
	function fincommerce_output_content_wrapper_end() {
		wc_get_template( 'global/wrapper-end.php' );
	}
}

if ( ! function_exists( 'fincommerce_get_sidebar' ) ) {

	/**
	 * Get the shop sidebar template.
	 */
	function fincommerce_get_sidebar() {
		wc_get_template( 'global/sidebar.php' );
	}
}

if ( ! function_exists( 'fincommerce_demo_store' ) ) {

	/**
	 * Adds a demo store banner to the site if enabled.
	 */
	function fincommerce_demo_store() {
		if ( ! is_store_notice_showing() ) {
			return;
		}

		$notice = get_option( 'fincommerce_demo_store_notice' );

		if ( empty( $notice ) ) {
			$notice = __( 'This is a demo store for testing purposes &mdash; no orders shall be fulfilled.', 'fincommerce' );
		}
...
if ( ! function_exists( 'fincommerce_page_title' ) ) {
	 ...
	 * @return string
	 */
	function fincommerce_page_title( $echo = true ) {

		if ( is_search() ) {
			/* translators: %s: search query */
			$page_title = sprintf( __( 'Search results: &ldquo;%s&rdquo;', 'fincommerce' ), get_search_query() );

			if ( get_query_var( 'paged' ) ) {
				/* translators: %s: page number */
				$page_title .= sprintf( __( '&nbsp;&ndash; Page %s', 'fincommerce' ), get_query_var( 'paged' ) );
			}
		} elseif ( is_tax() ) {
...
if ( ! function_exists( 'fincommerce_product_loop_start' ) ) {
	 ...
	 * @return string
	 */
	function fincommerce_product_loop_start( $echo = true ) {
		ob_start();
...
if ( ! function_exists( 'fincommerce_product_loop_end' ) ) {
	 ...
	 * @return string
	 */
	function fincommerce_product_loop_end( $echo = true ) {
		ob_start();
...
if ( ! function_exists( 'fincommerce_template_loop_product_title' ) ) {
...
if ( ! function_exists( 'fincommerce_template_loop_category_title' ) ) {
	 ...
	 */
	function fincommerce_template_loop_category_title( $category ) {
		?>
		<h2 class="fincommerce-loop-category__title">
			<?php
...
if ( ! function_exists( 'fincommerce_template_loop_product_link_open' ) ) {
	/**
	 * Insert the opening anchor tag for products in the loop.
	 */
	function fincommerce_template_loop_product_link_open() {
		global $product;
		...
		$link = apply_filters( 'fincommerce_loop_product_link', get_the_permalink(), $product );

		echo '<a href="' . esc_url( $link ) . '" class="fincommerce-LoopProduct-link fincommerce-loop-product__link">';
	}
}

if ( ! function_exists( 'fincommerce_template_loop_product_link_close' ) ) {
	/**
	 * Insert the closing anchor tag for products in the loop.
	 */
	function fincommerce_template_loop_product_link_close() {
		echo '</a>';
	}
}

if ( ! function_exists( 'fincommerce_template_loop_category_link_open' ) ) {
	/**
	 * Insert the opening anchor tag for categories in the loop.
	 *
	 * @param int|object|string $category Category ID, Object or String.
	 */
	function fincommerce_template_loop_category_link_open( $category ) {
		$category_term = get_term( $category, 'product_cat' );
		$category_name = ( ! $category_term || is_wp_error( $category_term ) ) ? '' : $category_term->name;
		/* translators: %s: Category name */
		echo '<a aria-label="' . sprintf( esc_attr__( 'Visit product category %1$s', 'fincommerce' ), esc_attr( $category_name ) ) . '" href="' . esc_url( get_term_link( $category, 'product_cat' ) ) . '">';
	}
}

if ( ! function_exists( 'fincommerce_template_loop_category_link_close' ) ) {
	/**
	 * Insert the closing anchor tag for categories in the loop.
	 */
	function fincommerce_template_loop_category_link_close() {
		echo '</a>';
	}
}

if ( ! function_exists( 'fincommerce_product_taxonomy_archive_header' ) ) {
	/**
	 * Output the products header on taxonomy archives.
	 */
	function fincommerce_product_taxonomy_archive_header() {
		wc_get_template( 'loop/header.php' );
	}
}

if ( ! function_exists( 'fincommerce_taxonomy_archive_description' ) ) {
	/**
	 * Show an archive description on taxonomy archives.
	 */
	function fincommerce_taxonomy_archive_description() {
...
if ( ! function_exists( 'fincommerce_product_archive_description' ) ) {

	/**
	 * Show a shop page description on product archives.
	 */
	function fincommerce_product_archive_description() {
...
if ( ! function_exists( 'fincommerce_template_loop_add_to_cart' ) ) {
	 ...
	 */
	function fincommerce_template_loop_add_to_cart( $args = array() ) {
		global $product;
		 ...
		 * @since 2.4.11
		 */
		$args = apply_filters( 'fincommerce_loop_add_to_cart_args', wp_parse_args( $args, $defaults ), $product );
		...
		$cart_redirect_after_add  = get_option( 'fincommerce_cart_redirect_after_add' ) === 'yes';
		$ajax_add_to_cart_enabled = get_option( 'fincommerce_enable_ajax_add_to_cart' ) === 'yes';
		...
		// See https://github.com/dieselfox1/fincommerce/issues/59382.
		if (
...
if ( ! function_exists( 'fincommerce_template_loop_product_thumbnail' ) ) {
...
if ( ! function_exists( 'fincommerce_template_loop_price' ) ) {

	/**
	 * Get the product price for the loop.
	 */
	function fincommerce_template_loop_price() {
		wc_get_template( 'loop/price.php' );
	}
}
if ( ! function_exists( 'fincommerce_template_loop_rating' ) ) {

	/**
	 * Display the average rating in the loop.
	 */
	function fincommerce_template_loop_rating() {
		wc_get_template( 'loop/rating.php' );
	}
}
if ( ! function_exists( 'fincommerce_show_product_loop_sale_flash' ) ) {

	/**
	 * Get the sale flash for the loop.
	 */
	function fincommerce_show_product_loop_sale_flash() {
		wc_get_template( 'loop/sale-flash.php' );
	}
}

if ( ! function_exists( 'fincommerce_get_product_thumbnail' ) ) {

	/**
	 * Get the product thumbnail, or the placeholder if not set.
	 *
	 * @param string $size (default: 'fincommerce_thumbnail').
	 * @param  array  $attr Image attributes.
	 * @param  bool   $placeholder True to return $placeholder if no image is found, or false to return an empty string.
	 * @return string
	 */
	function fincommerce_get_product_thumbnail( $size = 'fincommerce_thumbnail', $attr = array(), $placeholder = true ) {
		global $product;
...
if ( ! function_exists( 'fincommerce_result_count' ) ) {

	/**
	 * Output the result count text (Showing x - x of x results).
	 */
	function fincommerce_result_count() {
		if ( ! wc_get_loop_prop( 'is_paginated' ) || ! fincommerce_products_will_display() ) {
			return;
		}
		...
		$catalog_orderedby_options = apply_filters(
			'fincommerce_catalog_orderedby',
			array(
				'menu_order' => __( 'Default sorting', 'fincommerce' ),
				'popularity' => __( 'Sorted by popularity', 'fincommerce' ),
				'rating'     => __( 'Sorted by average rating', 'fincommerce' ),
				'date'       => __( 'Sorted by latest', 'fincommerce' ),
				'price'      => __( 'Sorted by price: low to high', 'fincommerce' ),
				'price-desc' => __( 'Sorted by price: high to low', 'fincommerce' ),
			)
		);
...
if ( ! function_exists( 'fincommerce_catalog_ordering' ) ) {
	 ...
	 */
	function fincommerce_catalog_ordering( $attributes = null ) {
		if ( ! wc_get_loop_prop( 'is_paginated' ) || ! fincommerce_products_will_display() ) {
			return;
		}
		 ...
		 * @param string $default_orderby The default orderby option.
		 */
		$show_default_orderby = 'menu_order' === apply_filters( 'fincommerce_default_catalog_orderby', get_option( 'fincommerce_default_catalog_orderby', 'menu_order' ) );
		...
		if ( isset( $attributes ) && isset( $attributes['useLabel'] ) && $attributes['useLabel'] ) {
			...
			$catalog_orderby_options = apply_filters(
				'fincommerce_catalog_orderby',
				array(
					'menu_order' => __( 'Default', 'fincommerce' ),
					'popularity' => __( 'Popularity', 'fincommerce' ),
					'rating'     => __( 'Average rating', 'fincommerce' ),
					'date'       => __( 'Latest', 'fincommerce' ),
					'price'      => __( 'Price: low to high', 'fincommerce' ),
					'price-desc' => __( 'Price: high to low', 'fincommerce' ),
				)
			);
		} else {
			...
			$catalog_orderby_options = apply_filters(
				...
				array(
					'menu_order' => __( 'Default sorting', 'fincommerce' ),
					'popularity' => __( 'Sort by popularity', 'fincommerce' ),
					'rating'     => __( 'Sort by average rating', 'fincommerce' ),
					'date'       => __( 'Sort by latest', 'fincommerce' ),
					'price'      => __( 'Sort by price: low to high', 'fincommerce' ),
					'price-desc' => __( 'Sort by price: high to low', 'fincommerce' ),
				)
			);
		}
...
if ( ! function_exists( 'fincommerce_pagination' ) ) {

	/**
	 * Output the pagination.
	 */
	function fincommerce_pagination() {
		if ( ! wc_get_loop_prop( 'is_paginated' ) || ! fincommerce_products_will_display() ) {
			return;
		}
...
if ( ! function_exists( 'fincommerce_show_product_images' ) ) {

	/**
	 * Output the product image before the single product summary.
	 */
	function fincommerce_show_product_images() {
		wc_get_template( 'single-product/product-image.php' );
	}
}
if ( ! function_exists( 'fincommerce_show_product_thumbnails' ) ) {

	/**
	 * Output the product thumbnails.
	 */
	function fincommerce_show_product_thumbnails() {
 ...
 *
 * Hooks: fincommerce_gallery_thumbnail_size, fincommerce_gallery_image_size and fincommerce_gallery_full_size accept name based image sizes, or an array of width/height values.
 *
 * @since 3.3.2
...
function wc_get_gallery_image_html( $attachment_id, $main_image = false, $image_index = -1 ) {
	...
	$flexslider        = (bool) apply_filters( 'fincommerce_single_product_flexslider_enabled', get_theme_support( 'wc-product-gallery-slider' ) );
	$gallery_thumbnail = wc_get_image_size( 'gallery_thumbnail' );
	$thumbnail_size    = apply_filters( 'fincommerce_gallery_thumbnail_size', array( $gallery_thumbnail['width'], $gallery_thumbnail['height'] ) );
	$image_size        = apply_filters( 'fincommerce_gallery_image_size', $flexslider || $main_image ? 'fincommerce_single' : $thumbnail_size );
	$full_size         = apply_filters( 'fincommerce_gallery_full_size', apply_filters( 'fincommerce_product_thumbnails_large_size', 'full' ) );
	...
	$alt_text          = ( empty( $alt_text ) && ( $product instanceof WC_Product ) ) ? fincommerce_get_alt_from_product_title_and_position( $product->get_title(), $main_image, $image_index ) : $alt_text;
	...
	*/
	$image_params = apply_filters(
		'fincommerce_gallery_image_html_attachment_image_params',
		array(
	...
	return '<div data-thumb="' . esc_url( isset( $thumbnail_src[0] ) ? $thumbnail_src[0] : '' ) . '" data-thumb-alt="' . esc_attr( $alt_text ) . '" data-thumb-srcset="' . esc_attr( isset( $thumbnail_srcset ) ? $thumbnail_srcset : '' ) . '"  data-thumb-sizes="' . esc_attr( isset( $thumbnail_sizes ) ? $thumbnail_sizes : '' ) . '" class="fincommerce-product-gallery__image"><a href="' . esc_url( isset( $full_src[0] ) ? $full_src[0] : '' ) . '">' . $image . '</a></div>';
}

if ( ! function_exists( 'fincommerce_get_alt_from_product_title_and_position' ) ) {
	 ...
	 */
	function fincommerce_get_alt_from_product_title_and_position( $product_name, $main_image, $image_index ) {
		if ( -1 === $image_index ) {
			return $product_name;
		}
		...
		return sprintf( __( '%1$s - Image %2$s', 'fincommerce' ), $product_name, ( $image_index + $adder ) );
	}
}
if ( ! function_exists( 'fincommerce_output_product_data_tabs' ) ) {

	/**
	 * Output the product tabs.
	 */
	function fincommerce_output_product_data_tabs() {
		wc_get_template( 'single-product/tabs/tabs.php' );
	}
}
if ( ! function_exists( 'fincommerce_template_single_title' ) ) {

	/**
	 * Output the product title.
	 */
	function fincommerce_template_single_title() {
		wc_get_template( 'single-product/title.php' );
	}
}
if ( ! function_exists( 'fincommerce_template_single_rating' ) ) {

	/**
	 * Output the product rating.
	 */
	function fincommerce_template_single_rating() {
...
if ( ! function_exists( 'fincommerce_template_single_price' ) ) {

	/**
	 * Output the product price.
	 */
	function fincommerce_template_single_price() {
...
if ( ! function_exists( 'fincommerce_template_single_excerpt' ) ) {

	/**
	 * Output the product short description (excerpt).
	 */
	function fincommerce_template_single_excerpt() {
...
if ( ! function_exists( 'fincommerce_template_single_meta' ) ) {

	/**
	 * Output the product meta.
	 */
	function fincommerce_template_single_meta() {
		if ( ! is_a( $GLOBALS['product'] ?? null, \WC_Product::class ) ) {
			return;
		}
...
if ( ! function_exists( 'fincommerce_template_single_sharing' ) ) {

	/**
	 * Output the product sharing.
	 */
	function fincommerce_template_single_sharing() {
		wc_get_template( 'single-product/share.php' );
	}
}
if ( ! function_exists( 'fincommerce_show_product_sale_flash' ) ) {

	/**
	 * Output the product sale flash.
	 */
	function fincommerce_show_product_sale_flash() {
		wc_get_template( 'single-product/sale-flash.php' );
	}
}

if ( ! function_exists( 'fincommerce_template_single_add_to_cart' ) ) {

	/**
	 * Trigger the single product add to cart action.
	 */
	function fincommerce_template_single_add_to_cart() {
		global $product;
...
if ( ! function_exists( 'fincommerce_simple_add_to_cart' ) ) {

	/**
	 * Output the simple product add to cart area.
	 */
	function fincommerce_simple_add_to_cart() {
		wc_get_template( 'single-product/add-to-cart/simple.php' );
	}
}
if ( ! function_exists( 'fincommerce_grouped_add_to_cart' ) ) {

	/**
	 * Output the grouped product add to cart area.
	 */
	function fincommerce_grouped_add_to_cart() {
		global $product;
...
if ( ! function_exists( 'fincommerce_variable_add_to_cart' ) ) {

	/**
	 * Output the variable product add to cart area.
	 */
	function fincommerce_variable_add_to_cart() {
		global $product;
		...
		$get_variations = count( $product->get_children() ) <= apply_filters( 'fincommerce_ajax_variation_threshold', 30, $product );
...
if ( ! function_exists( 'fincommerce_external_add_to_cart' ) ) {

	/**
	 * Output the external product add to cart area.
	 */
	function fincommerce_external_add_to_cart() {
		global $product;
...
if ( ! function_exists( 'fincommerce_quantity_input' ) ) {
	 ...
	 * @return string
	 */
	function fincommerce_quantity_input( $args = array(), $product = null, $echo = true ) {
...
if ( ! function_exists( 'fincommerce_product_description_tab' ) ) {

	/**
	 * Output the description tab content.
	 */
	function fincommerce_product_description_tab() {
		wc_get_template( 'single-product/tabs/description.php' );
	}
}
if ( ! function_exists( 'fincommerce_product_additional_information_tab' ) ) {

	/**
	 * Output the attributes tab content.
	 */
	function fincommerce_product_additional_information_tab() {
		wc_get_template( 'single-product/tabs/additional-information.php' );
	}
}
if ( ! function_exists( 'fincommerce_default_product_tabs' ) ) {
	 ...
	 * @return array
	 */
	function fincommerce_default_product_tabs( $tabs = array() ) {
		global $product, $post;
		...
		if ( $post->post_content ) {
			$tabs['description'] = array(
				...
				'priority' => 10,
				'callback' => 'fincommerce_product_description_tab',
			);
		}
		...
		if ( $product->has_attributes() || apply_filters( 'wc_product_enable_dimensions_display', $product->has_weight() || $product->has_dimensions() ) ) {
			$tabs['additional_information'] = array(
				'title'    => __( 'Additional information', 'fincommerce' ),
				'priority' => 20,
				'callback' => 'fincommerce_product_additional_information_tab',
			);
		}
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				...
				'title'    => sprintf( __( 'Reviews (%d)', 'fincommerce' ), $product->get_review_count() ),
				'priority' => 30,
...
if ( ! function_exists( 'fincommerce_sort_product_tabs' ) ) {
	 ...
	 * @return array
	 */
	function fincommerce_sort_product_tabs( $tabs = array() ) {
...
if ( ! function_exists( 'fincommerce_comments' ) ) {
...
if ( ! function_exists( 'fincommerce_review_display_gravatar' ) ) {
	/**
	 ...
	 * @return void
	 */
	function fincommerce_review_display_gravatar( $comment ) {
		echo get_avatar( $comment, apply_filters( 'fincommerce_review_gravatar_size', '60' ), '' );
	}
}

if ( ! function_exists( 'fincommerce_review_display_rating' ) ) {
	/**
	 * Display the reviewers star rating
	 *
	 * @return void
	 */
	function fincommerce_review_display_rating() {
...
if ( ! function_exists( 'fincommerce_review_display_meta' ) ) {
	/**
	 * Display the review authors meta (name, verified owner, review date)
	 *
	 * @return void
	 */
	function fincommerce_review_display_meta() {
		wc_get_template( 'single-product/review-meta.php' );
	}
}

if ( ! function_exists( 'fincommerce_review_display_comment_text' ) ) {

	/**
	 * Display the review content.
	 */
	function fincommerce_review_display_comment_text() {
...
if ( ! function_exists( 'fincommerce_output_related_products' ) ) {

	/**
	 * Output the related products.
	 */
	function fincommerce_output_related_products() {
		...
		fincommerce_related_products( apply_filters( 'fincommerce_output_related_products_args', $args ) );
	}
}

if ( ! function_exists( 'fincommerce_related_products' ) ) {
	 ...
	 */
	function fincommerce_related_products( $args = array() ) {
		global $product;
		...
		wc_set_loop_prop( 'columns', apply_filters( 'fincommerce_related_products_columns', $args['columns'] ) );

		wc_get_template( 'single-product/related.php', $args );
	}
}

if ( ! function_exists( 'fincommerce_upsell_display' ) ) {
	 ...
	 */
	function fincommerce_upsell_display( $limit = -1, $columns = 4, $orderby = 'rand', $order = 'desc' ) {
		global $product;
		...
		$args = apply_filters(
			'fincommerce_upsell_display_args',
			array(
		...
		wc_set_loop_prop( 'columns', apply_filters( 'fincommerce_upsells_columns', isset( $args['columns'] ) ? $args['columns'] : $columns ) );

		$orderby = apply_filters( 'fincommerce_upsells_orderby', isset( $args['orderby'] ) ? $args['orderby'] : $orderby );
		$order   = apply_filters( 'fincommerce_upsells_order', isset( $args['order'] ) ? $args['order'] : $order );
		/**
		 ...
		 * @since 3.0.0
		 */
		$limit = intval( apply_filters( 'fincommerce_upsells_total', $args['posts_per_page'] ?? $limit ) );
...
if ( ! function_exists( 'fincommerce_shipping_calculator' ) ) {
	 ...
	 */
	function fincommerce_shipping_calculator( $button_text = '' ) {
		if ( 'no' === get_option( 'fincommerce_enable_shipping_calc' ) || ! WC()->cart->needs_shipping() ) {
			return;
		}
...
if ( ! function_exists( 'fincommerce_cart_totals' ) ) {

	/**
	 * Output the cart totals.
	 */
	function fincommerce_cart_totals() {
		if ( is_checkout() ) {
			return;
		}
...
if ( ! function_exists( 'fincommerce_cross_sell_display' ) ) {
	 ...
	 */
	function fincommerce_cross_sell_display( $limit = 2, $columns = 2, $orderby = 'rand', $order = 'desc' ) {
		if ( is_checkout() ) {
			return;
		}
		...
		wc_set_loop_prop( 'columns', apply_filters( 'fincommerce_cross_sells_columns', $columns ) );

		// Handle orderby and limit results.
		$orderby     = apply_filters( 'fincommerce_cross_sells_orderby', $orderby );
		$order       = apply_filters( 'fincommerce_cross_sells_order', $order );
		 ...
		 * @since 3.0.0
		 */
		$limit       = intval( apply_filters( 'fincommerce_cross_sells_total', $limit ) );
...
if ( ! function_exists( 'fincommerce_button_proceed_to_checkout' ) ) {

	/**
	 * Output the proceed to checkout button.
	 */
	function fincommerce_button_proceed_to_checkout() {
		wc_get_template( 'cart/proceed-to-checkout-button.php' );
	}
}

if ( ! function_exists( 'fincommerce_widget_shopping_cart_button_view_cart' ) ) {

	/**
	 * Output the view cart button.
	 */
	function fincommerce_widget_shopping_cart_button_view_cart() {
		$wp_button_class = wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '';
		if ( ! CartCheckoutUtils::has_cart_page() ) {
			echo '';
		}
		echo '<a href="' . esc_url( wc_get_cart_url() ) . '" class="button wc-forward' . esc_attr( $wp_button_class ) . '">' . esc_html__( 'View cart', 'fincommerce' ) . '</a>';
	}
}

if ( ! function_exists( 'fincommerce_widget_shopping_cart_proceed_to_checkout' ) ) {

	/**
	 * Output the proceed to checkout button.
	 */
	function fincommerce_widget_shopping_cart_proceed_to_checkout() {
		$wp_button_class = wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '';
		echo '<a href="' . esc_url( wc_get_checkout_url() ) . '" class="button checkout wc-forward' . esc_attr( $wp_button_class ) . '">' . esc_html__( 'Checkout', 'fincommerce' ) . '</a>';
	}
}

if ( ! function_exists( 'fincommerce_widget_shopping_cart_subtotal' ) ) {
	/**
...
if ( ! function_exists( 'fincommerce_mini_cart' ) ) {
	 ...
	 * @param array $args Arguments.
	 */
	function fincommerce_mini_cart( $args = array() ) {
...
if ( ! function_exists( 'fincommerce_login_form' ) ) {

	/**
	 * Output the FinCommerce Login Form.
	 *
	 * @param array $args Arguments.
	 */
	function fincommerce_login_form( $args = array() ) {
...
if ( ! function_exists( 'fincommerce_checkout_login_form' ) ) {

	/**
	 * Output the FinCommerce Checkout Login Form.
	 */
	function fincommerce_checkout_login_form() {
		wc_get_template(
...
if ( ! function_exists( 'fincommerce_breadcrumb' ) ) {

	/**
	 * Output the FinCommerce Breadcrumb.
	 *
	 * @param array $args Arguments.
	 */
	function fincommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args(
			$args,
			apply_filters(
				'fincommerce_breadcrumb_defaults',
				array(
					'delimiter'   => '&nbsp;&#47;&nbsp;',
					'wrap_before' => '<nav class="fincommerce-breadcrumb" aria-label="Breadcrumb">',
					'wrap_after'  => '</nav>',
					'before'      => '',
					'after'       => '',
					'home'        => _x( 'Home', 'breadcrumb', 'fincommerce' ),
				)
			)
		);
		...
		if ( ! empty( $args['home'] ) ) {
			$breadcrumbs->add_crumb( $args['home'], apply_filters( 'fincommerce_breadcrumb_home_url', home_url() ) );
		}

		$args['breadcrumb'] = $breadcrumbs->generate();

		/**
		 * FinCommerce Breadcrumb hook
		 *
		 * @hooked WC_Structured_Data::generate_breadcrumblist_data() - 10
		 */
		do_action( 'fincommerce_breadcrumb', $breadcrumbs, $args );

		wc_get_template( 'global/breadcrumb.php', $args );
	}
}

if ( ! function_exists( 'fincommerce_order_review' ) ) {
	 ...
	 */
	function fincommerce_order_review( $deprecated = false ) {
		wc_get_template(
...
if ( ! function_exists( 'fincommerce_checkout_payment' ) ) {

	/**
	 * Output the Payment Methods on the checkout.
	 */
	function fincommerce_checkout_payment() {
		if ( ! WC()->cart ) {
			wc_doing_it_wrong(
				__FUNCTION__,
				__( 'Cart is not available. This may indicate that the function is being called before fincommerce_init or in an admin context.', 'fincommerce' ),
				'9.8.0'
			);
			return;
		}
		...
		wc_get_template(
			...
			array(
				...
				'order_button_text'  => apply_filters( 'fincommerce_order_button_text', __( 'Place order', 'fincommerce' ) ),
			)
		);
	}
}

if ( ! function_exists( 'fincommerce_checkout_coupon_form' ) ) {

	/**
	 * Output the Coupon form for the checkout.
	 */
	function fincommerce_checkout_coupon_form() {
...
if ( ! function_exists( 'fincommerce_products_will_display' ) ) {
	 ...
	 * @return bool
	 */
	function fincommerce_products_will_display() {
		$display_type = fincommerce_get_loop_display_mode();

		return 0 < wc_get_loop_prop( 'total', 0 ) && 'subcategories' !== $display_type;
	}
}

if ( ! function_exists( 'fincommerce_get_loop_display_mode' ) ) {
	 ...
	 */
	function fincommerce_get_loop_display_mode() {
		...
		if ( is_shop() ) {
			$display_type = get_option( 'fincommerce_shop_page_display', '' );
		} elseif ( is_product_category() ) {
			$parent_id    = get_queried_object_id();
			$display_type = get_term_meta( $parent_id, 'display_type', true );
			$display_type = '' === $display_type ? get_option( 'fincommerce_category_archive_display', '' ) : $display_type;
		}
		...
		if ( in_array( $display_type, array( 'subcategories', 'both' ), true ) ) {
			$subcategories = fincommerce_get_product_subcategories( $parent_id );
...
if ( ! function_exists( 'fincommerce_maybe_show_product_subcategories' ) ) {
	 ...
	 * @return string
	 */
	function fincommerce_maybe_show_product_subcategories( $loop_html = '' ) {
		...
		if ( 'subcategories' === $display_type || 'both' === $display_type ) {
			ob_start();
			fincommerce_output_product_categories(
				array(
...
if ( ! function_exists( 'fincommerce_output_product_categories' ) ) {
	/**
	 * Display product sub categories as thumbnails.
	 *
	 * This is a replacement for fincommerce_product_subcategories which also does some logic
	 ...
	 * @return boolean
	 */
	function fincommerce_output_product_categories( $args = array() ) {
		$args = wp_parse_args(
			$args,
			array(
				'before'    => apply_filters( 'fincommerce_before_output_product_categories', '' ),
				'after'     => apply_filters( 'fincommerce_after_output_product_categories', '' ),
				'parent_id' => 0,
			)
		);

		$product_categories = fincommerce_get_product_subcategories( $args['parent_id'] );
...
if ( ! function_exists( 'fincommerce_get_product_subcategories' ) ) {
	/**
	 ...
	 * @return array
	 */
	function fincommerce_get_product_subcategories( $parent_id = 0 ) {
		$parent_id          = absint( $parent_id );
		$cache_key          = apply_filters( 'fincommerce_get_product_subcategories_cache_key', 'product-category-hierarchy-' . $parent_id, $parent_id );
		...
		if ( apply_filters( 'fincommerce_product_subcategories_hide_empty', true ) ) {
...
if ( ! function_exists( 'fincommerce_subcategory_thumbnail' ) ) {
	 ...
	 */
	function fincommerce_subcategory_thumbnail( $category ) {
		$small_thumbnail_size = apply_filters( 'subcategory_archive_thumbnail_size', 'fincommerce_thumbnail' );
...
if ( ! function_exists( 'fincommerce_order_details_table' ) ) {
	 ...
	 */
	function fincommerce_order_details_table( $order_id ) {
		if ( ! $order_id ) {
			return;
		}
		...
		wc_get_template(
			...
			array(
				 ...
				 */
				'show_downloads' => apply_filters( 'fincommerce_order_downloads_table_show_downloads', ( $order->has_downloadable_item() && $order->is_download_permitted() ), $order ),
			)
		);
	}
}

if ( ! function_exists( 'fincommerce_order_downloads_table' ) ) {
	 ...
	 */
	function fincommerce_order_downloads_table( $downloads ) {
		if ( ! $downloads ) {
			return;
		}
		wc_get_template(
...
if ( ! function_exists( 'fincommerce_order_again_button' ) ) {
	 ...
	 */
	function fincommerce_order_again_button( $order ) {
		/**
		 ...
		 */
		$statuses_for_reordering = apply_filters( 'fincommerce_valid_order_statuses_for_order_again', array( OrderStatus::COMPLETED ) );
		...
		wc_get_template(
			...
			array(
				'order'           => $order,
				'wp_button_class' => wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '',
				'order_again_url' => wp_nonce_url( add_query_arg( 'order_again', $order->get_id(), wc_get_cart_url() ), 'fincommerce-order_again' ),
			)
		);
	}
}
...
if ( ! function_exists( 'fincommerce_form_field' ) ) {
	 ...
	 * @return string
	 */
	function fincommerce_form_field( $key, $args, $value = null ) {
		$defaults = array(
			'type'              => 'text',
			'label'             => '',
			'description'       => '',
		...
		$args = wp_parse_args( $args, $defaults );
		$args = apply_filters( 'fincommerce_form_field_args', $args, $key, $value );
		...
		if ( $args['required'] ) {
			...
			$required_indicator = '&nbsp;<span class="optional">(' . esc_html__( 'optional', 'fincommerce' ) . ')</span>';
		}
		...
		switch ( $args['type'] ) {
			case 'country':
				...
				if ( 1 === count( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_attr__( 'Select a country / region&hellip;', 'fincommerce' ) ) . '" ' . $data_label . '><option value="">' . esc_html__( 'Select a country / region&hellip;', 'fincommerce' ) . '</option>';
					...
					$field .= '<noscript><button type="submit" name="fincommerce_checkout_update_totals" value="' . esc_attr__( 'Update country / region', 'fincommerce' ) . '">' . esc_html__( 'Update country / region', 'fincommerce' ) . '</button></noscript>';
			...
			case 'state':
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_html__( 'Select an option&hellip;', 'fincommerce' ) ) . '"  data-input-classes="' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . $data_label . '>
						<option value="">' . esc_html__( 'Select an option&hellip;', 'fincommerce' ) . '</option>';
					...
					foreach ( $args['options'] as $option_key => $option_text ) {
						if ( '' === $option_key ) {
							...
							if ( empty( $args['placeholder'] ) ) {
								$args['placeholder'] = $option_text ? $option_text : __( 'Choose an option', 'fincommerce' );
							}
		...
		if ( ! empty( $field ) ) {
			...
			$field_html .= '<span class="fincommerce-input-wrapper">' . $field;
		 ...
		 * Filter by type.
		 */
		$field = apply_filters( 'fincommerce_form_field_' . $args['type'], $field, $key, $args, $value );
		 ...
		 * @since 3.4.0
		 */
		$field = apply_filters( 'fincommerce_form_field', $field, $key, $args, $value );
...
if ( ! function_exists( 'fincommerce_output_auth_header' ) ) {

	/**
	 * Output the Auth header.
	 */
	function fincommerce_output_auth_header() {
		wc_get_template( 'auth/header.php' );
	}
}

if ( ! function_exists( 'fincommerce_output_auth_footer' ) ) {

	/**
	 * Output the Auth footer.
	 */
	function fincommerce_output_auth_footer() {
		wc_get_template( 'auth/footer.php' );
	}
}

if ( ! function_exists( 'fincommerce_single_variation' ) ) {

	/**
	 * Output placeholders for the single variation.
	 */
	function fincommerce_single_variation() {
		echo '<div class="fincommerce-variation single_variation" role="alert" aria-relevant="additions"></div>';
	}
}

if ( ! function_exists( 'fincommerce_single_variation_add_to_cart_button' ) ) {

	/**
	 * Output the add to cart button for variations.
	 */
	function fincommerce_single_variation_add_to_cart_button() {
...
if ( ! function_exists( 'wc_dropdown_variation_attribute_options' ) ) {
	...
	function wc_dropdown_variation_attribute_options( $args = array() ) {
		$args = wp_parse_args(
			apply_filters( 'fincommerce_dropdown_variation_attribute_options_args', $args ),
			array(
				'options'          => false,
				'attribute'        => false,
				'product'          => false,
				...
				'id'               => '',
				'class'            => '',
				'show_option_none' => __( 'Choose an option', 'fincommerce' ),
			)
		);
		...
		$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : __( 'Choose an option', 'fincommerce' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options, true ) ) {
						$html .= '<option value="' . esc_attr( $term->slug ) . '" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'fincommerce_variation_option_name', $term->name, $term, $attribute, $product ) ) . '</option>';
					}
				}
			} else {
				foreach ( $options as $option ) {
					...
					$html    .= '<option value="' . esc_attr( $option ) . '" ' . $selected . '>' . esc_html( apply_filters( 'fincommerce_variation_option_name', $option, null, $attribute, $product ) ) . '</option>';
				}
			}
		}
...
if ( ! function_exists( 'fincommerce_account_content' ) ) {

	/**
	 * My Account content output.
	 */
	function fincommerce_account_content() {
		global $wp;
		...
		if ( ! empty( $wp->query_vars ) ) {
			foreach ( $wp->query_vars as $key => $value ) {
				...
				if ( has_action( 'fincommerce_account_' . $key . '_endpoint' ) ) {
					do_action( 'fincommerce_account_' . $key . '_endpoint', $value );
					return;
				}
			}
		}
...
if ( ! function_exists( 'fincommerce_account_navigation' ) ) {

	/**
	 * My Account navigation template.
	 */
	function fincommerce_account_navigation() {
		wc_get_template( 'myaccount/navigation.php' );
	}
}

if ( ! function_exists( 'fincommerce_account_orders' ) ) {
	 ...
	 */
	function fincommerce_account_orders( $current_page ) {
		$current_page    = empty( $current_page ) ? 1 : absint( $current_page );
		$customer_orders = wc_get_orders(
			apply_filters(
				'fincommerce_my_account_my_orders_query',
				array(
...
if ( ! function_exists( 'fincommerce_account_view_order' ) ) {
	 ...
	 */
	function fincommerce_account_view_order( $order_id ) {
		WC_Shortcode_My_Account::view_order( absint( $order_id ) );
	}
}

if ( ! function_exists( 'fincommerce_account_downloads' ) ) {

	/**
	 * My Account > Downloads template.
	 */
	function fincommerce_account_downloads() {
		wc_get_template( 'myaccount/downloads.php' );
	}
}

if ( ! function_exists( 'fincommerce_account_edit_address' ) ) {
	 ...
	 */
	function fincommerce_account_edit_address( $type ) {
...
if ( ! function_exists( 'fincommerce_account_payment_methods' ) ) {

	/**
	 * My Account > Downloads template.
	 */
	function fincommerce_account_payment_methods() {
		wc_get_template( 'myaccount/payment-methods.php' );
	}
}

if ( ! function_exists( 'fincommerce_account_add_payment_method' ) ) {

	/**
	 * My Account > Add payment method template.
	 */
	function fincommerce_account_add_payment_method() {
		WC_Shortcode_My_Account::add_payment_method();
	}
}

if ( ! function_exists( 'fincommerce_account_edit_account' ) ) {

	/**
	 * My Account > Edit account template.
	 */
	function fincommerce_account_edit_account() {
...
if ( ! function_exists( 'wc_get_email_order_items' ) ) {
	...
	function wc_get_email_order_items( $order, $args = array() ) {
		...
		wc_get_template(
			$template,
			apply_filters(
				'fincommerce_email_order_items_args',
				array(
					'order'               => $order,
		...
		return apply_filters( 'fincommerce_email_order_items_table', ob_get_clean(), $order );
	}
}
...
if ( ! function_exists( 'wc_get_email_fulfillment_items' ) ) {
	...
	function wc_get_email_fulfillment_items( $order, $fulfillment, $args = array() ) {
		 ...
		 */
		return apply_filters( 'fincommerce_get_email_fulfillment_items_table', ob_get_clean(), $order, $fulfillment );
	}
}
		...
		if ( ! empty( $downloads ) ) {
			...
			foreach ( $downloads as $file ) {
				...
				if ( $args['show_url'] ) {
					...
					$prefix    = count( $downloads ) > 1 ? sprintf( __( 'Download %d', 'fincommerce' ), $i ) : __( 'Download', 'fincommerce' );
...
if ( ! function_exists( 'fincommerce_photoswipe' ) ) {

	/**
	 * Get the shop sidebar template.
	 */
	function fincommerce_photoswipe() {
...
function wc_display_product_attributes( $product ) {
	...
	if ( $display_dimensions && $product->has_dimensions() ) {
		$product_attributes['dimensions'] = array(
			'label' => __( 'Dimensions', 'fincommerce' ),
	...
	foreach ( $attributes as $attribute ) {
		...
		$product_attributes[ 'attribute_' . sanitize_title_with_dashes( $attribute->get_name() ) ] = array(
			...
			'value' => apply_filters( 'fincommerce_attribute', wpautop( wptexturize( implode( ', ', $values ) ) ), $attribute, $values ),
		);
	}

	/**
	 * Hook: fincommerce_display_product_attributes.
	 *
	 * @since 3.6.0.
	 ...
	 */
	$product_attributes = apply_filters( 'fincommerce_display_product_attributes', $product_attributes, $product );
...
function wc_get_stock_html( $product ) {
	...
	if ( has_filter( 'fincommerce_stock_html' ) ) {
		wc_deprecated_function( 'The fincommerce_stock_html filter', '', 'fincommerce_get_stock_html' );
		$html = apply_filters( 'fincommerce_stock_html', $html, $availability['availability'], $product );
	}

	return apply_filters( 'fincommerce_get_stock_html', $html, $product );
}
...
function wc_get_rating_html( $rating, $count = 0 ) {
	...
	if ( 0 < $rating ) {
		...
		$label = sprintf( __( 'Rated %s out of 5', 'fincommerce' ), $rating );
		$html  = '<div class="star-rating" role="img" aria-label="' . esc_attr( $label ) . '">' . wc_get_star_rating_html( $rating, $count ) . '</div>';
	}

	return apply_filters( 'fincommerce_product_get_rating_html', $html, $rating, $count );
}
...
function wc_get_star_rating_html( $rating, $count = 0 ) {
	...
	if ( 0 < $count ) {
		...
		$html .= sprintf( _n( 'Rated %1$s out of 5 based on %2$s customer rating', 'Rated %1$s out of 5 based on %2$s customer ratings', $count, 'fincommerce' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>', '<span class="rating">' . esc_html( $count ) . '</span>' );
	} else {
		/* translators: %s: rating */
		$html .= sprintf( esc_html__( 'Rated %s out of 5', 'fincommerce' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>' );
	}

	$html .= '</span>';

	return apply_filters( 'fincommerce_get_star_rating_html', $html, $rating, $count );
}
...
function wc_get_price_html_from_text() {
	return apply_filters( 'fincommerce_get_price_html_from_text', '<span class="from">' . _x( 'From:', 'min_price', 'fincommerce' ) . ' </span>' );
}
...
function wc_get_logout_redirect_url() {
	 ...
	 * @return string
	 */
	return apply_filters( 'fincommerce_logout_default_redirect_url', wc_get_page_permalink( 'myaccount' ) );
}
...
function wc_empty_cart_message() {
	$notice = wc_print_notice(
		wp_kses_post(
			 ...
			 * @return string
			 */
			apply_filters( 'wc_empty_cart_message', __( 'Your cart is currently empty.', 'fincommerce' ) )
		),
		'notice',
		array(),
		true
...
function wc_get_theme_slug_for_templates() {
	return apply_filters( 'fincommerce_theme_slug_for_templates', get_option( 'template' ) );
}
...
function wc_get_formatted_cart_item_data( $cart_item, $flat = false ) {
	...
	if ( $cart_item['data']->is_type( ProductType::VARIATION ) && is_array( $cart_item['variation'] ) ) {
		foreach ( $cart_item['variation'] as $name => $value ) {
			...
			if ( taxonomy_exists( $taxonomy ) ) {
				...
				$value = apply_filters( 'fincommerce_variation_option_name', $value, null, $taxonomy, $cart_item['data'] );
	...
	$item_data = apply_filters( 'fincommerce_get_item_data', $item_data, $cart_item );
...
function wc_get_cart_remove_url( $cart_item_key ) {
	...
	return apply_filters( 'fincommerce_get_remove_url', $cart_page_url ? wp_nonce_url( add_query_arg( 'remove_item', $cart_item_key, $cart_page_url ), 'fincommerce-cart' ) : '' );
}
...
function wc_get_cart_undo_url( $cart_item_key ) {
	...
	return apply_filters( 'fincommerce_get_undo_url', $cart_page_url ? wp_nonce_url( add_query_arg( $query_args, $cart_page_url ), 'fincommerce-cart' ) : '', $cart_item_key );
}
 ...
 * @since 3.5.0
 */
function fincommerce_output_all_notices() {
	...
	echo '<div class="fincommerce-notices-wrapper">';
	wc_print_notices();
	echo '</div>';
}
...
function wc_get_pay_buttons() {
	...
	echo '<div class="fincommerce-pay-buttons">';
	foreach ( $supported_gateways as $pay_button_id ) {
		printf( '<div class="fincommerce-pay-button__%1$s %1$s" id="%1$s"></div>', esc_attr( $pay_button_id ) );
	}
	echo '</div>';
}
...
function wc_set_hooked_blocks_version() {
	...
	$option_name = 'fincommerce_hooked_blocks_version';
...
function wc_update_store_notice_visible_on_theme_switch( $old_name, $old_theme ) {
	$enable_store_notice_in_classic_theme_option = 'fincommerce_enable_store_notice_in_classic_theme';
	$is_store_notice_active_option               = 'fincommerce_demo_store';
...
/**
 * If the user switches from a classic to a block theme and they haven't already got a fincommerce_hooked_blocks_version,
 ...
 * See @link https://github.com/dieselfox1/fincommerce/issues/30733.
 *
...
function wc_get_quantity_input_args( $args, $product = null ) {
	// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment, FinCommerce.Commenting.CommentHooks.HookCommentWrongStyle
	$defaults = array(
		'input_id'     => uniqid( 'quantity_' ),
		'input_name'   => 'quantity',
		'classes'      => apply_filters( 'fincommerce_quantity_input_classes', array( 'input-text', 'qty', 'text' ), $product ),
		'pattern'      => apply_filters( 'fincommerce_quantity_input_pattern', wc_is_stock_amount_integer() ? '[0-9]*' : '' ),
		'inputmode'    => apply_filters( 'fincommerce_quantity_input_inputmode', wc_is_stock_amount_integer() ? 'numeric' : 'decimal' ),
		'placeholder'  => apply_filters( 'fincommerce_quantity_input_placeholder', '', $product ),
		'autocomplete' => apply_filters( 'fincommerce_quantity_input_autocomplete', 'off', $product ),
		'readonly'     => false,
	);
	...
	if ( $product ) {
		...
		$defaults['min_value']    = apply_filters( 'fincommerce_quantity_input_min', 1, $product );
		$defaults['max_value']    = apply_filters( 'fincommerce_quantity_input_max', -1, $product );
		$defaults['step']         = apply_filters( 'fincommerce_quantity_input_step', 1, $product );
		$defaults['product_name'] = '';
	}

	// phpcs:enable FinCommerce.Commenting.CommentHooks.MissingHookComment, FinCommerce.Commenting.CommentHooks.HookCommentWrongStyle
	/**
	 ...
	 * @return array
	 */
	$args = apply_filters( 'fincommerce_quantity_input_args', wp_parse_args( $args, $defaults ), $product );
	 ...
	 */
	$args['type'] = apply_filters( 'fincommerce_quantity_input_type', $type );
