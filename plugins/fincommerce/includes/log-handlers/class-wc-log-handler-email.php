...
class WC_Log_Handler_Email extends WC_Log_Handler {
	...
	protected function get_subject() {
		...
		return sprintf(
			...
			_n(
				'[%1$s] %2$s: %3$s FinCommerce log message',
				'[%1$s] %2$s: %3$s FinCommerce log messages',
				$log_count,
				'fincommerce'
			),
			$site_name,
	...
	protected function get_body() {
		...
		$log_count = count( $this->logs );
		return _n(
			'You have received the following FinCommerce log message:',
			'You have received the following FinCommerce log messages:',
			$log_count,
			'fincommerce'
		) . PHP_EOL
			. PHP_EOL
			...
			. sprintf( __( 'Visit %s admin area:', 'fincommerce' ), $site_name )
			. PHP_EOL
			. admin_url();
	}
