...
class WC_Log_Handler_DB extends WC_Log_Handler {
	...
	protected static function add( $timestamp, $level, $message, $source, $context ) {
		...
		return false !== $wpdb->insert( "{$wpdb->prefix}fincommerce_log", $insert, $format );
	}
	...
	public static function flush() {
		...
		return $wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}fincommerce_log" );
	}
	...
	public function clear( $source ) {
		...
		return $wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}fincommerce_log WHERE source = %s",
				$source
			)
		);
	}
	...
	public static function delete_logs_before_timestamp( $timestamp = 0 ) {
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}fincommerce_log WHERE timestamp < %s",
