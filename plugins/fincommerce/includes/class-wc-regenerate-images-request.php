...
class WC_Regenerate_Images_Request extends WC_Background_Process {
	...
	protected function task( $item ) {
		...
		$log->info(
			sprintf(
				...
				__( 'Regenerating images for attachment ID: %s', 'fincommerce' ),
				$this->attachment_id
			),
			array(
	...
	public function adjust_intermediate_image_sizes( $sizes ) {
		...
		$unfiltered_sizes = array( 'fincommerce_thumbnail', 'fincommerce_gallery_thumbnail', 'fincommerce_single' );
		static $in_filter = false;
		if ( $in_filter ) {
			return $unfiltered_sizes;
		}
		$in_filter      = true;
		$filtered_sizes = apply_filters( 'fincommerce_regenerate_images_intermediate_image_sizes', $unfiltered_sizes );
		$in_filter      = false;
	...
	protected function complete() {
		...
		$log = wc_get_logger();
		$log->info(
			__( 'Completed product image regeneration job.', 'fincommerce' ),
			array(
				'source' => 'wc-image-regeneration',
			)
		);
