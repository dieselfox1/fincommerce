 ...
 *
 * @package FinCommerce\Classes\Data
 * @version 2.2.0
 */
...
class WC_Post_Data {
	...
	public static function init() {
		...
		add_action( 'fincommerce_product_set_stock_status', array( __CLASS__, 'delete_product_query_transients' ) );
		add_action( 'fincommerce_product_set_visibility', array( __CLASS__, 'delete_product_query_transients' ) );
		add_action( 'fincommerce_product_type_changed', array( __CLASS__, 'product_type_changed' ), 10, 3 );
		...
		add_action( 'fincommerce_before_delete_order', array( __CLASS__, 'before_delete_order' ) );
		...
		// Priority 50 to make sure this runs after FinCommerce attribute migrations.
		add_action( 'fincommerce_attribute_updated', array( __CLASS__, 'handle_global_attribute_updated' ), 50, 3 );
		add_action( 'fincommerce_attribute_deleted', array( __CLASS__, 'handle_global_attribute_updated' ), 10, 3 );
		...
		add_action( 'fincommerce_product_attributes_updated', array( __CLASS__, 'on_product_attributes_updated' ), 10, 1 );
	...
	public static function product_type_changed( $product, $from, $to ) {
		 ...
		 * @param string     $to      New type.
		 */
		if ( apply_filters( 'fincommerce_delete_variations_on_product_type_change', ProductType::VARIABLE === $from && ProductType::VARIABLE !== $to, $product, $from, $to ) ) {
	...
	public static function delete_order_items( $postid ) {
		...
		if ( OrderUtil::is_order( $postid, wc_get_order_types() ) ) {
			do_action( 'fincommerce_delete_order_items', $postid );

			$wpdb->query(
				"
				DELETE {$wpdb->prefix}fincommerce_order_items, {$wpdb->prefix}fincommerce_order_itemmeta
				FROM {$wpdb->prefix}fincommerce_order_items
				JOIN {$wpdb->prefix}fincommerce_order_itemmeta ON {$wpdb->prefix}fincommerce_order_items.order_item_id = {$wpdb->prefix}fincommerce_order_itemmeta.order_item_id
				WHERE {$wpdb->prefix}fincommerce_order_items.order_id = '{$postid}';
				"
			); // WPCS: unprepared SQL ok.

			do_action( 'fincommerce_deleted_order_items', $postid );
		}
	}
	...
	public static function delete_order_downloadable_permissions( $postid ) {
		if ( OrderUtil::is_order( $postid, wc_get_order_types() ) ) {
			do_action( 'fincommerce_delete_order_downloadable_permissions', $postid );

			$data_store = WC_Data_Store::load( 'customer-download' );
			$data_store->delete_by_order_id( $postid );

			do_action( 'fincommerce_deleted_order_downloadable_permissions', $postid );
		}
	}
	...
	public static function regenerate_variation_attribute_summary( $variation_id ) {
		...
		if ( $data_store->has_callable( 'get_attribute_summary' ) ) {
			...
			*/
			do_action( 'fincommerce_updated_product_attribute_summary', $variation_id );
		}
	}
	...
	public static function get_variation_summaries_sync_threshold() {
		 ...
		 */
		return absint( apply_filters( 'fincommerce_regenerate_variation_summaries_sync_threshold', 50 ) );
	}
	 ...
	 *
	 * This method uses the FinCommerce Action Scheduler to queue a single regeneration action
	 ...
	 *                                the scheduled action. Default 'fincommerce'.
	 *
	 * @return void
	 */
	private static function schedule_variation_summary_regeneration( $action_name, $args, $warning_message, $group = 'fincommerce' ) {
		if ( function_exists( 'as_schedule_single_action' ) ) {
			...
			wc_get_logger()->warning(
				...
				array( 'source' => 'fincommerce-variations' )
			);
		}
	}
