...
use Automattic\FinCommerce\Internal\AddressProvider\AddressProviderController;
use Automattic\FinCommerce\Internal\AssignDefaultCategory;
use Automattic\FinCommerce\Internal\BatchProcessing\BatchProcessingController;
use Automattic\FinCommerce\Internal\ComingSoon\ComingSoonAdminBarBadge;
use Automattic\FinCommerce\Internal\ComingSoon\ComingSoonCacheInvalidator;
use Automattic\FinCommerce\Internal\ComingSoon\ComingSoonRequestHandler;
use Automattic\FinCommerce\Internal\DataStores\Orders\CustomOrdersTableController;
use Automattic\FinCommerce\Internal\DownloadPermissionsAdjuster;
...
use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Register as ProductDownloadDirectories;
use Automattic\FinCommerce\Internal\ProductImage\MatchImageBySKU;
use Automattic\FinCommerce\Internal\RegisterHooksInterface;
use Automattic\FinCommerce\Internal\RestockRefundedItemsAdjuster;
use Automattic\FinCommerce\Internal\Settings\OptionSanitizer;
use Automattic\FinCommerce\Internal\Utilities\LegacyRestApiStub;
use Automattic\FinCommerce\Internal\Utilities\WebhookUtil;
use Automattic\FinCommerce\Internal\Admin\EmailImprovements\EmailImprovements;
use Automattic\FinCommerce\Internal\Admin\Marketplace;
use Automattic\FinCommerce\Proxies\LegacyProxy;
use Automattic\FinCommerce\Utilities\{LoggingUtil, RestApiUtil, TimeUtil};
use Automattic\FinCommerce\Internal\Logging\RemoteLogger;
use Automattic\FinCommerce\Caches\OrderCountCacheService;
use Automattic\FinCommerce\Internal\StockNotifications\StockNotifications;
use Automattic\Jetpack\Constants;

/**
 * Main FinCommerce Class.
 *
 * @class FinCommerce
 */
final class FinCommerce {
	 ...
	 * API instance
	 *
	 * @deprecated 9.0.0 The Legacy REST API has been removed from FinCommerce core. Now this property points to a RestApiUtil instance, unless the Legacy REST API plugin is installed.
	 *
	 * @var WC_API
	 */
	private $api;
	...
	/**
	 * Main FinCommerce Instance.
	 *
	 * Ensures only one instance of FinCommerce is loaded or can be loaded.
	 *
	 * @since 2.1
	 * @static
	 * @see WC()
	 * @return FinCommerce - Main instance.
	 */
	...
	public function __clone() {
		wc_doing_it_wrong( __FUNCTION__, __( 'Cloning is forbidden.', 'fincommerce' ), '2.1' );
	}
	...
	public function __wakeup() {
		wc_doing_it_wrong( __FUNCTION__, __( 'Unserializing instances of this class is forbidden.', 'fincommerce' ), '2.1' );
	}
	...
	public function __get( $key ) {
		if ( 'api' === $key ) {
			// The Legacy REST API was removed from FinCommerce core as of version 9.0 (moved to a dedicated plugin),
	...
	/**
	 * Get the FinCommerce version.
	 *
	 * @since 10.3.0
	 *
	 * @return string The FinCommerce version.
	 */
	...
	/**
	 * When WP has loaded all plugins, trigger the `fincommerce_loaded` hook.
	 *
	 * This ensures `fincommerce_loaded` is called only after all other plugins
	...
	public function on_plugins_loaded() {
		/**
		 * Action to signal that FinCommerce has finished loading.
		 *
		 * @since 3.6.0
		 */
		do_action( 'fincommerce_loaded' );
	}
	...
	public function init_jetpack_connection_config() {
		...
		$config->ensure(
			...
			array(
				...
				// See https://github.com/dieselfox1/fincommerce/pull/47113.
				'name' => 'FinCommerce',
			)
		);
	}
	...
	private function init_hooks() {
		...
		add_action( 'fincommerce_installed', array( $this, 'add_fincommerce_inbox_variant' ) );
		add_action( 'fincommerce_updated', array( $this, 'add_fincommerce_inbox_variant' ) );
		add_action( 'rest_api_init', array( $this, 'register_wp_admin_settings' ) );
		add_action( 'fincommerce_installed', array( $this, 'add_fincommerce_remote_variant' ) );
		add_action( 'fincommerce_updated', array( $this, 'add_fincommerce_remote_variant' ) );
		add_action( 'fincommerce_newly_installed', 'wc_set_hooked_blocks_version', 10 );
		add_action( 'update_option_fincommerce_allow_tracking', array( $this, 'get_tracking_history' ), 10, 2 );
		add_action( 'update_option_fincommerce_allow_tracking', array( $this, 'handle_tracking_setting_change' ), 10, 2 );
		...
		add_filter( 'wp_plugin_dependencies_slug', array( $this, 'convert_fincommerce_slug' ) );
		add_filter( 'fincommerce_register_log_handlers', array( $this, 'register_remote_log_handler' ) );
		...
		// Feature flags.
		if ( Constants::is_true( 'fincommerce_BIS_ALPHA_ENABLED' ) ) {
		 ...
		 */
		$container->get( Automattic\FinCommerce\Internal\Utilities\PluginInstaller::class )->register();
		$container->get( Automattic\FinCommerce\Internal\TransientFiles\TransientFilesEngine::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Orders\OrderAttributionController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Orders\OrderAttributionBlocksController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\CostOfGoodsSold\CostOfGoodsSoldController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Admin\Settings\PaymentsController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\WooPayments\WooPaymentsController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Utilities\LegacyRestApiStub::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Email\EmailStyleSync::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Fulfillments\FulfillmentsController::class )->register();

		// Classes inheriting from RestApiControllerBase.
		$container->get( Automattic\FinCommerce\Internal\ReceiptRendering\ReceiptRenderingRestController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Orders\OrderActionsRestController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Orders\OrderStatusRestController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Admin\Settings\PaymentsRestController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\WooPayments\WooPaymentsRestController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Admin\EmailPreview\EmailPreviewRestController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\Admin\Emails\EmailListingRestController::class )->register();

		$container->get( Automattic\FinCommerce\Internal\ProductFilters\MainQueryController::class )->register();
		$container->get( Automattic\FinCommerce\Internal\ProductFilters\CacheController::class )->register();
	}

	/**
	 * Add fincommerce_inbox_variant for the Remote Inbox Notification.
	 *
	 * P2 post can be found at https://wp.me/paJDYF-1uJ.
	 *
	 * This will no longer be used. The more flexible add_fincommerce_remote_variant
	 * below will be used instead.
	 */
	public function add_fincommerce_inbox_variant() {
		$config_name = 'fincommerce_inbox_variant_assignment';
	...
	/**
	 * Add fincommerce_remote_variant_assignment used to determine cohort
	 * or group assignment for Remote Spec Engines.
	 */
	public function add_fincommerce_remote_variant() {
		$config_name = 'fincommerce_remote_variant_assignment';
		if ( false === get_option( $config_name, false ) ) {
	...
	public function log_errors() {
		...
		if ( $error && in_array( $error['type'], array( E_ERROR, E_PARSE, E_COMPILE_ERROR, E_USER_ERROR, E_RECOVERABLE_ERROR ), true ) ) {
			 ...
			 * @since 3.2.0
			 */
			do_action( 'fincommerce_shutdown_error', $error );
		}
	}
	...
	private function define_constants() {
		...
		$this->define( 'fincommerce_VERSION', $this->version );
		$this->define( 'WC_ROUNDING_PRECISION', 6 );
		$this->define( 'WC_DISCOUNT_ROUNDING_MODE', 2 );
		$this->define( 'WC_TAX_ROUNDING_MODE', 'yes' === get_option( 'fincommerce_prices_include_tax', 'no' ) ? 2 : 1 );
		$this->define( 'WC_DELIMITER', '|' );
		$this->define( 'WC_SESSION_CACHE_GROUP', 'wc_session_id' );
		$this->define( 'WC_TEMPLATE_DEBUG_MODE', false );

		/**
		 * As of 8.8.0, it is preferable to use the `fincommerce_log_directory` filter hook to change the log
		 ...
		 *   - System Status Report ('Installed version not tested with active version of FinCommerce' warning)
	...
	private function define_tables() {
		...
		$tables = array(
			'payment_tokenmeta'      => 'fincommerce_payment_tokenmeta',
			'order_itemmeta'         => 'fincommerce_order_itemmeta',
	...
	public function is_rest_api_request() {
		 ...
		 * @since 3.6.0
		 */
		return apply_filters( 'fincommerce_is_rest_api_request', $is_rest_api_request );
	}
	...
	public function includes() {
		...
		if ( $this->is_request( 'admin' ) ) {
			...
			include_once WC_ABSPATH . 'includes/admin/fincommerce-legacy-reports.php';
		}
	...
	/**
	 * Function used to Init FinCommerce Template Functions - This makes them pluggable by plugins and themes.
	 */
	...
	public function init() {
		...
		/**
		 * Action triggered before FinCommerce initialization begins.
		 */
		do_action( 'before_fincommerce_init' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
		...
		/**
		 * Action triggered after FinCommerce initialization finishes.
		 */
		do_action( 'fincommerce_init' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
	}
	...
	public function load_plugin_textdomain() {
		...
		if ( is_readable( $custom_translation_path ) ) {
			unload_textdomain( 'fincommerce' );
			load_textdomain( 'fincommerce', $custom_translation_path );
			load_textdomain( 'fincommerce', $plugin_translation_path );
		}
	}
	 ...
	 *
	 * As of 3.3, image sizes can be registered via themes using add_theme_support for fincommerce
	 * and defining an array of args. If these are not defined, we will use defaults. This is
	 * handled in wc_get_image_size function.
	 *
	 * 3.3 sizes:
	 *
	 * fincommerce_thumbnail - Used in product listings. We assume these work for a 3 column grid layout.
	 * fincommerce_single - Used on single product pages for the main image.
	 *
	 * @since 2.3
	 */
	public function add_image_sizes() {
		...
		add_image_size( 'fincommerce_thumbnail', $thumbnail['width'], $thumbnail['height'], $thumbnail['crop'] );
		add_image_size( 'fincommerce_single', $single['width'], $single['height'], $single['crop'] );
		add_image_size( 'fincommerce_gallery_thumbnail', $gallery_thumbnail['width'], $gallery_thumbnail['height'], $gallery_thumbnail['crop'] );
	}
	...
	public function template_path() {
		 ...
		 */
		return apply_filters( 'fincommerce_template_path', 'fincommerce/' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
	}
	...
	public function api_request_url( $request, $ssl = null ) {
		 ...
		 */
		return esc_url_raw( apply_filters( 'fincommerce_api_request_url', $api_request_url, $request, $ssl ) );  // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
	}
	...
	private function load_webhooks() {
		...
		/**
		 * Hook: fincommerce_load_webhooks_limit.
		 *
		 * @since 3.6.0
		 * @param int $limit Used to limit how many webhooks are loaded. Default: no limit.
		 */
		$limit = apply_filters( 'fincommerce_load_webhooks_limit', null );
	...
	public function initialize_session() {
		 ...
		 */
		$session_class = apply_filters( 'fincommerce_session_handler', 'WC_Session_Handler' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
	...
	public function robots_txt( $output ) {
		...
		$above[] = "Disallow: $path/wp-content/uploads/fincommerce_transient_files/";
		$above[] = "Disallow: $path/wp-content/uploads/fincommerce_uploads/";
	...
	public function activated_plugin( $filename ) {
		...
		if ( '/fincommerce.php' === substr( $filename, -16 ) ) {
			set_transient( 'fincommerce_activated_plugin', $filename );
		}
	...
	public function build_dependencies_satisfied() {
		...
		if ( ! file_exists( WC()->plugin_path() . '/assets/js/admin/fincommerce_admin.min.js' ) ) {
			return false;
		}
	...
	/**
	 * Converts the FinCommerce slug to the correct slug for the current version.
	 ...
	 * @return string
	 */
	public function convert_fincommerce_slug( $slug ) {
		if ( 'fincommerce' === $slug ) {
	...
	/**
	 * Tracks the history FinCommerce Allow Tracking option.
	 * - When the field was first set to allow tracking
	 * - Last time the option was changed
	 *
	 * @param string $old_value The old value for the fincommerce_allow_tracking option.
	 * @param string $value The current value for the fincommerce_allow_tracking option.
	 * @since x.x.x
	 *
	 * @return void
	 */
	public function get_tracking_history( $old_value, $value ) {
		// If fincommerce_allow_tracking_first_optin is not set. It means is the first time it gets set.
		if ( ! get_option( 'fincommerce_allow_tracking_first_optin' ) && 'yes' === $value ) {
			update_option( 'fincommerce_allow_tracking_first_optin', time() );
		}

		// Always update the last change.
		update_option( 'fincommerce_allow_tracking_last_modified', time() );
	}
	...
	public function add_recurring_action_wrappers() {
		add_action( 'fincommerce_tracker_send_event_wrapper', array( $this, 'add_fincommerce_tracker_send_event_wrapper' ) );
		...
		add_action( 'fincommerce_cleanup_rate_limits_wrapper', array( $this, 'add_fincommerce_cleanup_rate_limits_wrapper' ) );
	}
	...
	public function unschedule_unwrapped_actions() {
		...
		as_unschedule_all_actions( 'fincommerce_tracker_send_event' );
		as_unschedule_all_actions( 'wc_admin_daily' );
		as_unschedule_all_actions( 'generate_category_lookup_table' );
		as_unschedule_all_actions( 'fincommerce_cleanup_rate_limits' );
	}

	/**
	 * Wrapper for the `fincommerce_tracker_send_event` action. This prevents the event failing when the class is not loaded.
	 * It loads the class if it exists, and then calls the actual action.
	 *
	 * @return void
	 * @internal For exclusive usage of FinCommerce core, backwards compatibility not guaranteed.
	 */
	public function add_fincommerce_tracker_send_event_wrapper() {
		if ( true !== wc_string_to_bool( get_option( 'fincommerce_allow_tracking', 'no' ) ) ) {
			return;
		}
		try {
			...
			wc_get_logger()->error( 'Error initializing WC_Tracker: ' . $e->getMessage(), array( 'source' => 'fincommerce-scheduled-actions' ) );
		}
		// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment
		do_action( 'fincommerce_tracker_send_event' );
	}
	...
	public function add_wc_admin_daily_wrapper() {
		try {
			if ( class_exists( \Automattic\FinCommerce\Internal\Admin\Events::class ) ) {
				\Automattic\FinCommerce\Internal\Admin\Events::instance();
			}
		} catch ( Throwable $e ) {
			wc_get_logger()->error( 'Error initializing wc_admin_daily: ' . $e->getMessage(), array( 'source' => 'fincommerce-scheduled-actions' ) );
		}
		// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment
	...
	public function add_generate_category_lookup_table_wrapper() {
		try {
			if ( class_exists( \Automattic\FinCommerce\Internal\Admin\CategoryLookup::class ) ) {
				\Automattic\FinCommerce\Internal\Admin\CategoryLookup::instance();
			}
		} catch ( Throwable $e ) {
			wc_get_logger()->error( 'Error in category lookup wrapper: ' . $e->getMessage(), array( 'source' => 'fincommerce-scheduled-actions' ) );
		}
		// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment
	...
	/**
	 * Wrapper for the `fincommerce_cleanup_rate_limits` action. This prevents the event failing when the class is not loaded.
	 * It loads the class if it exists, and then calls the actual action.
	 *
	 * @return void
	 * @internal For exclusive usage of FinCommerce core, backwards compatibility not guaranteed.
	 */
	public function add_fincommerce_cleanup_rate_limits_wrapper() {
		try {
			...
			wc_get_logger()->error( 'Error in rate limiter cleanup wrapper: ' . $e->getMessage(), array( 'source' => 'fincommerce-scheduled-actions' ) );
		}
		// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment
		do_action( 'fincommerce_cleanup_rate_limits' );
	}
	...
	public function register_recurring_actions() {
		...
		as_schedule_recurring_action( $scheduled_sales_time, DAY_IN_SECONDS, 'fincommerce_scheduled_sales', array(), 'fincommerce', true );

		$held_duration = get_option( 'fincommerce_hold_stock_minutes', '60' );
		...
		if ( '' !== $held_duration ) {
			 ...
			 * @since 5.1.0
			 */
			$cancel_unpaid_interval = apply_filters( 'fincommerce_cancel_unpaid_orders_interval_minutes', absint( $held_duration ) );

			as_schedule_single_action( time() + ( absint( $cancel_unpaid_interval ) * 60 ), 'fincommerce_cancel_unpaid_orders', array(), 'fincommerce', true );

		}

		// Delay the first run of `fincommerce_cleanup_personal_data` by 10 seconds
		// so it doesn't occur in the same request. FinCommerce Admin also schedules
		...
		as_schedule_recurring_action( time() + 10, DAY_IN_SECONDS, 'fincommerce_cleanup_personal_data', array(), 'fincommerce', true );

		// Schedule daily cleanup logs at 3 AM.

		as_schedule_recurring_action( time() + ( 3 * HOUR_IN_SECONDS ), DAY_IN_SECONDS, 'fincommerce_cleanup_logs', array(), 'fincommerce', true );

		// Schedule twice daily cleanup sessions at 6 AM and 6 PM.

		as_schedule_recurring_action( time() + ( 6 * HOUR_IN_SECONDS ), 12 * HOUR_IN_SECONDS, 'fincommerce_cleanup_sessions', array(), 'fincommerce', true );

		// Schedule geoip updater every 15 days.

		as_schedule_recurring_action( time() + MINUTE_IN_SECONDS, 15 * DAY_IN_SECONDS, 'fincommerce_geoip_updater', array(), 'fincommerce', true );
		...
		as_schedule_recurring_action( time() + ( 3 * HOUR_IN_SECONDS ), DAY_IN_SECONDS, 'fincommerce_cleanup_rate_limits_wrapper', array(), 'fincommerce', true );

		as_schedule_recurring_action( time(), DAY_IN_SECONDS, 'wc_admin_daily_wrapper', array(), 'fincommerce', true );

		// Note: this is potentially redundant when the core package exists.
		as_schedule_single_action( time() + 10, 'generate_category_lookup_table_wrapper', array(), 'fincommerce', true );
	}

	/**
	 * Schedule the action send tracking events if tracking is enabled, or unregister it if tracking is disabled.
	 * This will be called when the `fincommerce_allow_tracking` option is updated.
	 *
	 * @param string $old_value The old value of the `fincommerce_allow_tracking` option.
	 * @param string $value     The new value of the `fincommerce_allow_tracking` option.
	 *
	 * @return void
	 */
	public function handle_tracking_setting_change( $old_value, $value ) {
		...
		if ( false === wc_string_to_bool( $value ) ) {
			as_unschedule_all_actions( 'fincommerce_tracker_send_event_wrapper', array(), 'fincommerce' );
		} else {
			$this->schedule_tracking_action();
		}
	}
	...
	public function schedule_tracking_action() {
		if ( false === wc_string_to_bool( get_option( 'fincommerce_allow_tracking', 'no' ) ) ) {
			return;
		}
		 ...
		 * @since 2.3.0
		 */
		$tracker_recurrence = apply_filters( 'fincommerce_tracker_event_recurrence', 'daily' );
		$core_internals     = wp_get_schedules();
		as_schedule_recurring_action( time() + 10, $core_internals[ $tracker_recurrence ]['interval'], 'fincommerce_tracker_send_event_wrapper', array(), 'fincommerce', true );
	}

	/**
	 * Initialize the customizer on the plugins_loaded action.
	 * If FinCommerce is network activated, wp_is_block_theme() will be called too early,
	 ...
	 *
	 * @internal
	 * @see https://github.com/dieselfox1/fincommerce/issues/58364
	 */
