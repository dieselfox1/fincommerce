 ...
 *
 * FinCommerce Emails Class which handles the sending on transactional emails and email templates. This class loads in available emails.
 *
...
class WC_Emails {
	...
	public static function init_transactional_emails() {
		...
		$email_actions = apply_filters(
			'fincommerce_email_actions',
			array(
				'fincommerce_low_stock',
				'fincommerce_no_stock',
				'fincommerce_product_on_backorder',
				'fincommerce_order_status_pending_to_processing',
				'fincommerce_order_status_pending_to_completed',
				'fincommerce_order_status_processing_to_cancelled',
				'fincommerce_order_status_pending_to_failed',
				'fincommerce_order_status_pending_to_on-hold',
				'fincommerce_order_status_failed_to_processing',
				'fincommerce_order_status_failed_to_completed',
				'fincommerce_order_status_failed_to_on-hold',
				'fincommerce_order_status_cancelled_to_processing',
				'fincommerce_order_status_cancelled_to_completed',
				'fincommerce_order_status_cancelled_to_on-hold',
				'fincommerce_order_status_on-hold_to_processing',
				'fincommerce_order_status_on-hold_to_cancelled',
				'fincommerce_order_status_on-hold_to_failed',
				'fincommerce_order_status_completed',
				'fincommerce_order_status_failed',
				'fincommerce_order_fully_refunded',
				'fincommerce_order_partially_refunded',
		 ...
		 */
		if ( apply_filters( 'fincommerce_defer_transactional_emails', false ) ) {
	...
	public static function send_queued_transactional_email( $filter = '', $args = array() ) {
		 ...
		 */
		if ( apply_filters( 'fincommerce_allow_send_queued_transactional_email', true, $filter, $args ) ) {
	...
	public function __construct() {
		...
		add_action( 'fincommerce_email_header', array( $this, 'email_header' ) );
		add_action( 'fincommerce_email_footer', array( $this, 'email_footer' ) );
		add_action( 'fincommerce_email_order_details', array( $this, 'order_downloads' ), 10, 4 );
		add_action( 'fincommerce_email_order_details', array( $this, 'order_details' ), 10, 4 );
		add_action( 'fincommerce_email_order_meta', array( $this, 'order_meta' ), 10, 3 );
		add_action( 'fincommerce_email_customer_details', array( $this, 'customer_details' ), 10, 3 );
		add_action( 'fincommerce_email_customer_details', array( $this, 'email_addresses' ), 20, 3 );
		add_action( 'fincommerce_email_customer_details', array( $this, 'additional_checkout_fields' ), 30, 3 );
		add_action( 'fincommerce_email_customer_address_section', array( $this, 'additional_address_fields' ), 30, 4 );

		if ( FeaturesUtil::feature_is_enabled( 'fulfillments' ) ) {
			// Fulfillment details and meta.
			add_action( 'fincommerce_email_fulfillment_details', array( $this, 'fulfillment_details' ), 10, 5 );
			add_action( 'fincommerce_email_fulfillment_meta', array( $this, 'fulfillment_meta' ), 30, 4 );
		}

		// Hooks for sending emails during store events.
		add_action( 'fincommerce_low_stock_notification', array( $this, 'low_stock' ) );
		add_action( 'fincommerce_no_stock_notification', array( $this, 'no_stock' ) );
		add_action( 'fincommerce_product_on_backorder_notification', array( $this, 'backorder' ) );
		add_action( 'fincommerce_created_customer_notification', array( $this, 'customer_new_account' ), 10, 3 );

		// Hook for replacing {site_title} in email-footer.
		add_filter( 'fincommerce_email_footer_text', array( $this, 'replace_placeholders' ) );
		 ...
		 */
		do_action( 'fincommerce_email', $this );
	}
	...
	public function init() {
		 ...
		 */
		$this->emails = apply_filters( 'fincommerce_email_classes', $this->emails );
	}
	...
	public function get_from_name() {
		return wp_specialchars_decode( get_option( 'fincommerce_email_from_name' ), ENT_QUOTES );
	}
	...
	public function get_from_address() {
		return sanitize_email( get_option( 'fincommerce_email_from_address' ) );
	}
	...
	public function replace_placeholders( $text ) {
		...
		return str_replace(
			...
			array(
				...
				$domain,
				$domain,
				'<a href="https://fincommerce.com">FinCommerce</a>',
				'<a href="https://fincommerce.com">FinCommerce</a>',
	...
	/**
	 * Wraps a message in the fincommerce mail template.
	 *
	...
	public function wrap_message( $email_heading, $message, $deprecated = false ) {
		 ...
		 */
		do_action( 'fincommerce_email_header', $email_heading, null );
		 ...
		 * @param null $null Unused.
		 */
		do_action( 'fincommerce_email_footer', null );
	...
	public function order_downloads( $order, $sent_to_admin = false, $plain_text = false, $email = '' ) {
		...
		$columns = apply_filters(
			'fincommerce_email_downloads_columns',
			array(
				'download-product' => __( 'Product', 'fincommerce' ),
				'download-expires' => __( 'Expires', 'fincommerce' ),
				'download-file'    => __( 'Download', 'fincommerce' ),
			),
			$order
		);
	...
	public function order_meta( $order, $sent_to_admin = false, $plain_text = false ) {
		 ...
		 * @param WC_Order $order         Order instance.
		 */
		$fields = apply_filters( 'fincommerce_email_order_meta_fields', array(), $sent_to_admin, $order );

		/**
		 * Deprecated fincommerce_email_order_meta_keys filter.
		 *
		 * @since 2.3.0
		 * @param array    $fields        Array of meta fields.
		 * @param bool     $sent_to_admin If sent to admin.
		 */
		$_fields = apply_filters( 'fincommerce_email_order_meta_keys', array(), $sent_to_admin );
		...
		if ( 0 < count( $public_fields ) ) {
			...
			foreach ( $public_fields as $field ) {
				if ( isset( $field->key ) && isset( $field->value ) && $field->value ) {
					 ...
					 * @since 10.1.0
					 */
					$meta_key_translation = apply_filters( 'fincommerce_fulfillment_translate_meta_key', $field->key );
					if ( $plain_text ) {
	...
	public function customer_details( $order, $sent_to_admin = false, $plain_text = false ) {
		 ...
		 * @param WC_Order $order         Order instance.
		 */
		$fields = array_filter( apply_filters( 'fincommerce_email_customer_details_fields', array(), $sent_to_admin, $order ), array( $this, 'customer_detail_field_is_valid' ) );
	...
	public function additional_checkout_fields( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if ( $plain_text ) {
			echo "\n" . esc_html( wc_strtoupper( __( 'Additional information', 'fincommerce' ) ) ) . "\n\n";
			foreach ( $fields as $field ) {
	...
	public function get_store_address() {
		add_filter(
			'fincommerce_formatted_address_force_country_display',
	...
	public function low_stock( $product ) {
		if ( 'no' === get_option( 'fincommerce_notify_low_stock', 'yes' ) ) {
			return;
		}
		 ...
		 * @since 4.7.0
		 */
		if ( false === apply_filters( 'fincommerce_should_send_low_stock_notification', true, $product->get_id() ) ) {
			return;
		}
		...
		$subject = sprintf( '[%s] %s', $this->get_blogname(), __( 'Product low in stock', 'fincommerce' ) );
		$message = sprintf(
		/* translators: 1: product name 2: items in stock */
			__( '%1$s is low in stock. There are %2$d left.', 'fincommerce' ),
		 ...
		 * @param null $null Unused.
		 */
			apply_filters( 'fincommerce_email_recipient_low_stock', get_option( 'fincommerce_stock_email_recipient' ), $product, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_subject_low_stock', $subject, $product, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_content_low_stock', $message, $product ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_headers', '', 'low_stock', $product, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_attachments', array(), 'low_stock', $product, null )
		);
	...
	public function no_stock( $product ) {
		if ( 'no' === get_option( 'fincommerce_notify_no_stock', 'yes' ) ) {
			return;
		}
		 ...
		 * @since 4.6.0
		 */
		if ( false === apply_filters( 'fincommerce_should_send_no_stock_notification', true, $product->get_id() ) ) {
			return;
		}
		...
		$subject = sprintf( '[%s] %s', $this->get_blogname(), __( 'Product out of stock', 'fincommerce' ) );
		/* translators: %s: product name */
		$message = sprintf( __( '%s is out of stock.', 'fincommerce' ), html_entity_decode( wp_strip_all_tags( $product->get_formatted_name() ), ENT_QUOTES, get_bloginfo( 'charset' ) ) );
		 ...
		 * @param null $null Unused.
		 */
			apply_filters( 'fincommerce_email_recipient_no_stock', get_option( 'fincommerce_stock_email_recipient' ), $product, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_subject_no_stock', $subject, $product, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_content_no_stock', $message, $product ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_headers', '', 'no_stock', $product, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_attachments', array(), 'no_stock', $product, null )
		);
	...
	public function backorder( $args ) {
		...
		$subject = sprintf( '[%s] %s', $this->get_blogname(), __( 'Product backorder', 'fincommerce' ) );
		/* translators: 1: backordered quantity 2: product name 3: order number */
		$message = sprintf( __( '%1$s units of %2$s have been backordered in order #%3$s.', 'fincommerce' ), $backordered_quantity, html_entity_decode( wp_strip_all_tags( $args['product']->get_formatted_name() ), ENT_QUOTES, get_bloginfo( 'charset' ) ), $order->get_order_number() );
		 ...
		 * @param null $null Unused.
		 */
			apply_filters( 'fincommerce_email_recipient_backorder', get_option( 'fincommerce_stock_email_recipient' ), $args, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_subject_backorder', $subject, $args, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_content_backorder', $message, $args ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_headers', '', 'backorder', $args, null ),
			/**
			...
			* @param null $null Unused.
			*/
			apply_filters( 'fincommerce_email_attachments', array(), 'backorder', $args, null )
		);
