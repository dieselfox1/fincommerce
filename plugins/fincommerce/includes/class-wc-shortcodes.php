...
class WC_Shortcodes {
	...
	public static function init() {
		$shortcodes = array(
			...
			'fincommerce_order_tracking' => __CLASS__ . '::order_tracking',
			'fincommerce_cart'           => __CLASS__ . '::cart',
			'fincommerce_checkout'       => __CLASS__ . '::checkout',
			'fincommerce_my_account'     => __CLASS__ . '::my_account',
		);
		...
		add_shortcode( 'fincommerce_messages', __CLASS__ . '::shop_messages' );
	}
	...
	public static function product_categories( $atts ) {
		...
		return '<div class="fincommerce columns-' . $columns . '">' . ob_get_clean() . '</div>';
	}
	...
	public static function product_add_to_cart( $atts ) {
		...
		echo '<p class="product fincommerce add_to_cart_inline ' . esc_attr( $atts['class'] ) . '" style="' . ( empty( $atts['style'] ) ? '' : esc_attr( $atts['style'] ) ) . '">';
	...
	public static function product_page( $atts ) {
		 ...
		 * @return array
		 */
		$invalid_statuses = apply_filters( 'fincommerce_shortcode_product_page_invalid_statuses', array( ProductStatus::TRASH ), $product_id );
		 ...
		 * @return bool
		 */
		$force_rendering = apply_filters( 'fincommerce_shortcode_product_page_force_rendering', null, $product_id );
		...
		if ( isset( $atts['show_title'] ) && ! $atts['show_title'] ) {
			remove_action( 'fincommerce_single_product_summary', 'fincommerce_template_single_title', 5 );
		}

		// Change form action to avoid redirect.
		add_filter( 'fincommerce_add_to_cart_form_action', '__return_empty_string' );
		...
		remove_filter( 'fincommerce_add_to_cart_form_action', '__return_empty_string' );

		return '<div class="fincommerce">' . ob_get_clean() . '</div>';
	}
	...
	public static function shop_messages() {
		...
		return '<div class="fincommerce">' . wc_print_notices( true ) . '</div>';
	}
	...
	public static function related_products( $atts ) {
		...
		fincommerce_related_products( $atts );
