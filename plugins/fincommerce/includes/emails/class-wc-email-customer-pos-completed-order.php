...
if ( ! class_exists( 'WC_Email_Customer_POS_Completed_Order', false ) ) :
	...
	class WC_Email_Customer_POS_Completed_Order extends WC_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'POS completed order', 'fincommerce' );
			...
			$this->description = $this->email_improvements_enabled
				? __( 'Let customers know once their POS order is complete.', 'fincommerce' )
				: __( 'Order complete emails are sent to customers when their POS orders are marked completed.', 'fincommerce' );
		...
		public function get_default_subject() {
			...
			return sprintf( __( 'Your in-store purchase #%1$s at %2$s', 'fincommerce' ), '{order_number}', esc_html( $store_name ) );
		}
		...
		public function get_default_heading() {
			return __( 'Thank you for your in-store purchase', 'fincommerce' );
		}
		...
		private function enable_order_email_actions_for_pos_orders() {
			...
			add_action( 'fincommerce_rest_order_actions_email_send', array( $this, 'trigger' ), 10, 2 );
		}
		...
		public function order_item_totals( $total_rows, $order, $tax_display ) {
			...
			if ( '' !== $cash_payment_change_due_amount ) {
				...
				$total_rows['cash_payment_change_due_amount'] = array(
					...
					'label' => __( 'Change due:', 'fincommerce' ),
		...
		private function enable_email_template_for_pos_orders() {
			add_filter( 'fincommerce_rest_order_actions_email_valid_template_classes', array( $this, 'add_to_valid_template_classes' ), 10, 2 );
		}
