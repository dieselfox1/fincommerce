 ...
 * Class WC_Email file.
 *
 * @package FinCommerce\Emails
 */

use Automattic\FinCommerce\Internal\EmailEditor\BlockEmailRenderer;
use Automattic\FinCommerce\Internal\EmailEditor\TransactionalEmailPersonalizer;
 ...
 * Email Class
 *
 * FinCommerce Email Class which is extended by specific email template classes to add emails to FinCommerce
 *
 * @class       WC_Email
 * @version     2.5.0
 * @package     FinCommerce\Classes\Emails
...
class WC_Email extends WC_Settings_API {
	...
	public function __construct() {
		...
		add_action( 'fincommerce_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );
	...
	public function format_string( $string ) {
		...
		if ( has_filter( 'fincommerce_email_format_string_replace' ) || has_filter( 'fincommerce_email_format_string_find' ) ) {
			...
			$string = str_replace( apply_filters( 'fincommerce_email_format_string_find', $legacy_find, $this ), apply_filters( 'fincommerce_email_format_string_replace', $legacy_replace, $this ), $string );
		}
		 ...
		 * @since 3.2.0
		 */
		return apply_filters( 'fincommerce_email_format_string', str_replace( $find, $replace, $string ), $this );
	}
	...
	public function setup_locale() {
		 ...
		 */
		$switch_email_locale = apply_filters( 'fincommerce_allow_switching_email_locale', true, $this );

		if ( $switch_email_locale && $this->is_customer_email() && apply_filters( 'fincommerce_email_setup_locale', true ) ) {
	...
	public function restore_locale() {
		 ...
		 * @param WC_Email $this The WC_Email object.
		 */
		$restore_email_locale = apply_filters( 'fincommerce_allow_restoring_email_locale', true, $this );

		if ( $restore_email_locale && $this->is_customer_email() && apply_filters( 'fincommerce_email_restore_locale', true ) ) {
			wc_restore_locale();
		}
	}
	...
	public function get_email_groups() {
		$email_groups = array(
			'accounts'         => __( 'Accounts', 'fincommerce' ),
			'orders'           => __( 'Orders', 'fincommerce' ),
			'order-processing' => __( 'Order processing', 'fincommerce' ),
			'order-exceptions' => __( 'Order exceptions', 'fincommerce' ),
			'payments'         => __( 'Payments', 'fincommerce' ),
		);
		 ...
		 */
		return apply_filters( 'fincommerce_email_groups', $email_groups );
	}
	...
	public function get_email_group_title() {
		 ...
		 * @param array $email_groups Associative array of email group slugs => titles.
		 */
		return (string) apply_filters( 'fincommerce_email_group_title', $title, $this->email_group, $email_groups );
	}
	...
	public function get_additional_content() {
		 ...
		 */
		return apply_filters( 'fincommerce_email_additional_content_' . $this->id, $this->format_string( $this->get_option_or_transient( 'additional_content' ) ), $this->object, $this );
	}
	...
	public function get_subject() {
		 ...
		 * @param WC_Email    $email   WC_Email instance managing the email.
		 */
		$subject = apply_filters( 'fincommerce_email_subject_' . $this->id, $this->format_string( $this->get_option_or_transient( 'subject', $this->get_default_subject() ) ), $this->object, $this );
		if ( $this->block_email_editor_enabled ) {
	...
	public function get_preheader() {
		 ...
		 * @param WC_Email    $email   WC_Email instance managing the email.
		 */
		$preheader = apply_filters( 'fincommerce_email_preheader' . $this->id, $this->format_string( $this->get_option_or_transient( 'preheader', '' ) ), $this->object, $this );
		if ( $this->block_email_editor_enabled ) {
	...
	public function get_heading() {
		 ...
		 * @param WC_Email    $email   WC_Email instance managing the email.
		 */
		return apply_filters( 'fincommerce_email_heading_' . $this->id, $this->format_string( $this->get_option_or_transient( 'heading', $this->get_default_heading() ) ), $this->object, $this );
	}
	...
	public function get_recipient() {
		 ...
		 * @param WC_Email $email     WC_Email instance managing the email.
		 */
		$recipient  = apply_filters( 'fincommerce_email_recipient_' . $this->id, $this->recipient, $this->object, $this );
	...
	public function get_cc_recipient() {
		 ...
		 * @param WC_Email $email  WC_Email instance managing the email.
		 */
		$cc  = apply_filters( 'fincommerce_email_cc_recipient_' . $this->id, $this->cc, $this->object, $this );
	...
	public function get_bcc_recipient() {
		 ...
		 * @param WC_Email $email  WC_Email instance managing the email.
		 */
		$bcc  = apply_filters( 'fincommerce_email_bcc_recipient_' . $this->id, $this->bcc, $this->object, $this );
	...
	public function get_headers() {
		...
		return apply_filters( 'fincommerce_email_headers', $header, $this->id, $this->object, $this );
	}
	...
	public function get_attachments() {
		return apply_filters( 'fincommerce_email_attachments', array(), $this->id, $this->object, $this );
	}
	...
	public function get_email_type() {
		...
		if ( $is_email_preview ) {
			$transient  = get_transient( "fincommerce_{$this->id}_email_type" );
	...
	public function get_content_type( $default_content_type = '' ) {
		...
		return apply_filters( 'fincommerce_email_content_type', $content_type, $this, $default_content_type );
	}
	...
	public function get_title() {
		return apply_filters( 'fincommerce_email_title', $this->title, $this );
	}
	...
	public function get_description() {
		return apply_filters( 'fincommerce_email_description', $this->description, $this );
	}
	...
	public function get_option( $key, $empty_value = null ) {
		...
		return apply_filters( 'fincommerce_email_get_option', $value, $this, $value, $key, $empty_value );
	}
	...
	public function is_enabled() {
		return apply_filters( 'fincommerce_email_enabled_' . $this->id, 'yes' === $this->enabled, $this->object, $this );
	}
	...
	public function style_inline( $content ) {
		if ( in_array( $this->get_content_type(), array( 'text/html', 'multipart/alternative' ), true ) ) {
			 ...
			 * @param WC_Email $this The WC_Email object.
			 */
			$style_inline_callback = apply_filters( 'fincommerce_mail_style_inline_callback', array( $this, 'apply_inline_style' ), $content, $this );
	...
	private function apply_inline_style( $content ) {
		 ...
		 */
		$css = apply_filters( 'fincommerce_email_styles', $css, $this );
		...
		if ( $this->supports_emogrifier() && class_exists( $css_inliner_class ) ) {
			try {
				 ...
				 */
				do_action( 'fincommerce_emogrifier', $css_inliner, $this );
	...
	protected function get_must_use_css_styles(): string {
		...
		* See https://github.com/dieselfox1/fincommerce/pull/47738.
		*/
		.screen-reader-text {
			display: none;
		}
	...
	public function get_from_name( $from_name = '' ) {
		$from_name = apply_filters( 'fincommerce_email_from_name', get_option( 'fincommerce_email_from_name' ), $this, $from_name );
	...
	public function get_from_address( $from_email = '' ) {
		$from_email = apply_filters( 'fincommerce_email_from_address', get_option( 'fincommerce_email_from_address' ), $this, $from_email );
	...
	public function send( $to, $subject, $message, $headers, $attachments ) {
		...
		$message              = apply_filters( 'fincommerce_mail_content', $this->style_inline( $message ) );
		$mail_callback        = apply_filters( 'fincommerce_mail_callback', 'wp_mail', $this );
		$mail_callback_params = apply_filters( 'fincommerce_mail_callback_params', array( $to, wp_specialchars_decode( $subject ), $message, $headers, $attachments ), $this );
		 ...
		 * @param WC_Email $this   WC_Email instance.
		 */
		do_action( 'fincommerce_email_sent', $return, (string) $this->id, $this );
	...
	public function init_form_fields() {
		...
		$placeholder_text  = sprintf( __( 'Available placeholders: %s', 'fincommerce' ), '<code>' . esc_html( implode( '</code>, <code>', array_keys( $this->placeholders ) ) ) . '</code>' );
		$this->form_fields = array(
			'enabled'            => array(
				'title'   => __( 'Enable/Disable', 'fincommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable this email notification', 'fincommerce' ),
				'default' => 'yes',
			),
			'subject'            => array(
				'title'       => __( 'Subject', 'fincommerce' ),
				'type'        => 'text',
				'desc_tip'    => true,
			...
			'heading'            => array(
				'title'       => __( 'Email heading', 'fincommerce' ),
				'type'        => 'text',
				'desc_tip'    => true,
				'description' => $placeholder_text,
			...
			'additional_content' => array(
				'title'       => __( 'Additional content', 'fincommerce' ),
				'description' => __( 'Text to appear below the main email content.', 'fincommerce' ) . ' ' . $placeholder_text,
			...
			'email_type'         => array(
				'title'       => __( 'Email type', 'fincommerce' ),
				'type'        => 'select',
				'description' => __( 'Choose which format of email to send.', 'fincommerce' ),
				'default'     => 'html',
	...
	protected function get_cc_field() {
		return array(
			'title'       => __( 'Cc(s)', 'fincommerce' ),
			'type'        => 'text',
			/* translators: %s: admin email */
			'description' => __( 'Enter Cc recipients (comma-separated) for this email.', 'fincommerce' ),
			'placeholder' => '',
			'default'     => '',
			'desc_tip'    => true,
		);
	...
	protected function get_bcc_field() {
		return array(
			'title'       => __( 'Bcc(s)', 'fincommerce' ),
			'type'        => 'text',
			/* translators: %s: admin email */
			'description' => __( 'Enter Bcc recipients (comma-separated) for this email.', 'fincommerce' ),
			'placeholder' => '',
			'default'     => '',
			'desc_tip'    => true,
		);
	...
	public function get_email_type_options() {
		$types = array( 'plain' => __( 'Plain text', 'fincommerce' ) );

		if ( class_exists( 'DOMDocument' ) ) {
			$types['html']      = __( 'HTML', 'fincommerce' );
			$types['multipart'] = __( 'Multipart', 'fincommerce' );
		}
	...
	protected function save_template( $template_code, $template_path ) {
		if ( current_user_can( 'edit_themes' ) && ! empty( $template_code ) && ! empty( $template_path ) ) {
			...
			if ( ! $saved ) {
				$redirect = add_query_arg( 'wc_error', rawurlencode( __( 'Could not write to template file.', 'fincommerce' ) ) );
	...
	public function get_theme_template_file( $template ) {
		return get_stylesheet_directory() . '/' . apply_filters( 'fincommerce_template_directory', 'fincommerce', $template ) . '/' . $template;
	}
	...
	protected function move_template_action( $template_type ) {
		...
		if ( ! empty( $template ) ) {
			...
			if ( wp_mkdir_p( dirname( $theme_file ) ) && ! file_exists( $theme_file ) ) {
				...
				$template_file = apply_filters( 'fincommerce_locate_core_template', $core_file, $template, $this->template_base, $this->id );
				 ...
				 */
				do_action( 'fincommerce_copy_email_template', $template_type, $this );

				wc_clear_template_cache();
				?>
				<div class="updated">
					<p><?php echo esc_html__( 'Template file copied to theme.', 'fincommerce' ); ?></p>
				</div>
				<?php
			}
		}
		...
		if ( $template ) {
			if ( ! empty( $template ) ) {
				...
				if ( file_exists( $theme_file ) ) {
					 ...
					 * @param string $email The email object
					 */
					do_action( 'fincommerce_delete_email_template', $template_type, $this );

					wc_clear_template_cache();
					?>
					<div class="updated">
						<p><?php echo esc_html__( 'Template file deleted from theme.', 'fincommerce' ); ?></p>
					</div>
					<?php
				}
			}
	...
	protected function admin_actions() {
		...
		if (
			...
			if ( ! current_user_can( 'edit_themes' ) ) {
				wp_die( esc_html__( 'You don&#8217;t have permission to do this.', 'fincommerce' ) );
			}
	...
	public function admin_options() {
		 ...
		 * @param string $email The email object
		 */
		do_action( 'fincommerce_email_settings_before', $this );
		?>
		 ...
		 * @param string $email The email object
		 */
		do_action( 'fincommerce_email_settings_after', $this );
		?>
		...
		if ( current_user_can( 'edit_themes' ) && ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) ) {
			...
			<div id="template">
				<?php
				$templates = array(
					'template_html'  => __( 'HTML template', 'fincommerce' ),
					'template_plain' => __( 'Plain text template', 'fincommerce' ),
				);
				...
				foreach ( $templates as $template_type => $title ) :
					...
					$core_file     = $this->template_base . $template;
					$template_file = apply_filters( 'fincommerce_locate_core_template', $core_file, $template, $this->template_base, $this->id );
					$template_dir  = apply_filters( 'fincommerce_template_directory', 'fincommerce', $template );
					?>
					<div class="template <?php echo esc_attr( $template_type ); ?>">
						...
						<?php if ( file_exists( $local_file ) ) : ?>
							<p>
								...
								printf( esc_html__( 'This template has been overridden by your theme and can be found in: %s.', 'fincommerce' ), '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
						...
						<?php elseif ( file_exists( $template_file ) ) : ?>
							<p>
								...
								printf( esc_html__( 'To override and edit this email template copy %1$s to your theme folder: %2$s.', 'fincommerce' ), '<code>' . esc_html( plugin_basename( $template_file ) ) . '</code>', '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
			...
			wc_enqueue_js(
				...
				var view = '" . esc_js( __( 'View template', 'fincommerce' ) ) . "';
				var hide = '" . esc_js( __( 'Hide template', 'fincommerce' ) ) . "';
				...
				jQuery( 'a.delete_template' ).on( 'click', function() {
					if ( window.confirm('" . esc_js( __( 'Are you sure you want to delete this template file?', 'fincommerce' ) ) . "') ) {
						return true;
					}
