...
use Automattic\FinCommerce\Internal\Email\OrderPriceFormatter;
...
if ( ! class_exists( 'WC_Email_Customer_POS_Refunded_Order', false ) ) :
	...
	class WC_Email_Customer_POS_Refunded_Order extends WC_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'POS refunded order', 'fincommerce' );
			...
			$this->description = $this->email_improvements_enabled
				? __( 'Let customers know when a full or partial refund is on its way to them for their POS order.', 'fincommerce' )
				: __( 'Order refunded emails are sent to customers when their POS orders are refunded.', 'fincommerce' );
		...
		public function get_default_subject( $partial = false ) {
			...
			if ( $partial ) {
				...
				return sprintf( __( 'Your %1$s order #%2$s has been partially refunded', 'fincommerce' ), esc_html( $store_name ), '{order_number}' );
			} else {
				/* translators: %1$s: Store name, %2$s: Order number */
				return sprintf( __( 'Your %1$s order #%2$s has been refunded', 'fincommerce' ), esc_html( $store_name ), '{order_number}' );
			}
		}
		...
		public function get_default_heading( $partial = false ) {
			if ( $partial ) {
				return $this->email_improvements_enabled
					? __( 'Partial refund: Order {order_number}', 'fincommerce' )
					: __( 'Partial Refund: Order {order_number}', 'fincommerce' );
			} else {
				return $this->email_improvements_enabled
					? __( 'Order refunded: {order_number}', 'fincommerce' )
					: __( 'Order Refunded: {order_number}', 'fincommerce' );
			}
		}
		...
		public function get_subject() {
			 ...
			 * @since 3.7.0
			 */
			return apply_filters( 'fincommerce_email_subject_customer_refunded_order', $this->format_string( $subject ), $this->object, $this );
		}
		...
		public function get_heading() {
			 ...
			 * @since 3.7.0
			 */
			return apply_filters( 'fincommerce_email_heading_customer_refunded_order', $this->format_string( $heading ), $this->object, $this );
		}
		...
		public function get_content_html() {
			...
			add_action( 'fincommerce_pos_email_header', array( $this, 'email_header' ) );
			add_action( 'fincommerce_pos_email_footer', array( $this, 'email_footer' ) );
			...
			remove_action( 'fincommerce_pos_email_header', array( $this, 'email_header' ) );
			remove_action( 'fincommerce_pos_email_footer', array( $this, 'email_footer' ) );
			return $content;
		}
		...
		public function init_form_fields() {
			...
			$this->form_fields = array(
				...
				'subject_full'       => array(
					'title'       => __( 'Full refund subject', 'fincommerce' ),
					'type'        => 'text',
					'desc_tip'    => true,
				...
				'subject_partial'    => array(
					'title'       => __( 'Partial refund subject', 'fincommerce' ),
					'type'        => 'text',
					'desc_tip'    => true,
					'description' => $placeholder_text,
				...
				'heading_full'       => array(
					'title'       => __( 'Full refund email heading', 'fincommerce' ),
					'type'        => 'text',
					'desc_tip'    => true,
					'description' => $placeholder_text,
				...
				'heading_partial'    => array(
					'title'       => __( 'Partial refund email heading', 'fincommerce' ),
					'type'        => 'text',
					'desc_tip'    => true,
					'description' => $placeholder_text,
		...
		private function add_pos_customizations() {
			...
			add_action( 'fincommerce_order_item_meta_start', array( $this, 'add_unit_price' ), 10, 4 );
			// Add filter to include additional details in the order item totals table.
			add_filter( 'fincommerce_get_order_item_totals', array( $this, 'order_item_totals' ), 10, 3 );
			// Add filter for custom footer text with highest priority to run before the default footer text filtering in `WC_Emails`.
			add_filter( 'fincommerce_email_footer_text', array( $this, 'replace_footer_placeholders' ), 1, 2 );
		}
		...
		private function remove_pos_customizations() {
			...
			remove_action( 'fincommerce_order_item_meta_start', array( $this, 'add_unit_price' ), 10 );
			remove_filter( 'fincommerce_get_order_item_totals', array( $this, 'order_item_totals' ), 10 );
			remove_filter( 'fincommerce_email_footer_text', array( $this, 'replace_footer_placeholders' ), 1 );
		}
		...
		public function add_unit_price( $item_id, $item, $order ) {
			$unit_price = OrderPriceFormatter::get_formatted_item_subtotal( $order, $item, get_option( 'fincommerce_tax_display_cart' ) );
			echo wp_kses_post( '<br /><small>' . $unit_price . '</small>' );
		}

		/**
		 * Disable default FinCommerce refund emails for POS orders.
		 * The core refund email IDs are in WC_Email_Customer_Refunded_Order's trigger method.
		 *
		 * This method adds filters to prevent the default FinCommerce refund emails
		...
		private function disable_default_refund_emails_for_pos_orders() {
			add_filter( 'fincommerce_email_enabled_customer_partially_refunded_order', array( $this, 'disable_default_refund_email_for_pos_orders' ), 10, 3 );
			add_filter( 'fincommerce_email_enabled_customer_refunded_order', array( $this, 'disable_default_refund_email_for_pos_orders' ), 10, 3 );
		}

		/**
		 * Disable the default FinCommerce refund email for POS orders.
		 *
		...
		private function register_refund_email_triggers() {
			add_action( 'fincommerce_order_fully_refunded_notification', array( $this, 'trigger_full' ), 10, 2 );
			add_action( 'fincommerce_order_partially_refunded_notification', array( $this, 'trigger_partial' ), 10, 2 );
		}
		...
		public function order_item_totals( $total_rows, $order, $tax_display ) {
			...
			if ( ! empty( $auth_code ) ) {
				$total_rows['payment_auth_code'] = array(
					...
					'label' => __( 'Auth code:', 'fincommerce' ),
					'value' => $auth_code,
				);
			}
			...
			if ( $order->get_date_paid() !== null ) {
				$total_rows['date_paid'] = array(
					'type'  => 'date_paid',
					'label' => __( 'Time of payment:', 'fincommerce' ),
		...
		private function get_pos_store_name() {
			return $this->format_string(
				get_option( 'fincommerce_pos_store_name', PointOfSaleDefaultSettings::get_default_store_name() )
			);
		}
		...
		private function get_pos_store_email() {
			return $this->format_string(
				get_option( 'fincommerce_pos_store_email', PointOfSaleDefaultSettings::get_default_store_email() )
			);
		}
		...
		private function get_pos_store_phone_number() {
			return $this->format_string(
				get_option( 'fincommerce_pos_store_phone' )
			);
		}
		...
		private function get_pos_store_address() {
			return $this->format_string(
				get_option( 'fincommerce_pos_store_address', PointOfSaleDefaultSettings::get_default_store_address() )
			);
		}
		...
		private function get_pos_refund_returns_policy() {
			return $this->format_string(
				get_option( 'fincommerce_pos_refund_returns_policy' )
			);
		}
