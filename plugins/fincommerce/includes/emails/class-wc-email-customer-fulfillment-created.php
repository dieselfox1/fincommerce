...
if ( ! class_exists( 'WC_Email_Customer_Fulfillment_Created', false ) ) :
	...
	class WC_Email_Customer_Fulfillment_Created extends WC_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'Fulfillment created', 'fincommerce' );
			...
			add_action( 'fincommerce_fulfillment_created_notification', array( $this, 'trigger' ), 10, 3 );

			// Call parent constructor.
			parent::__construct();

			$this->description = __( 'Fulfillment created emails are sent to the customer when the merchant creates a fulfillment for the order, and marks it as fulfilled. The notification isn’t sent for draft fulfillments.', 'fincommerce' );
		}
		...
		public function get_default_subject() {
			return __( 'An item from {site_title} order {order_number} has been fulfilled!', 'fincommerce' );
		}
		...
		public function get_default_heading() {
			return __( 'Your item is on the way!', 'fincommerce' );
		}
		...
		public function get_default_additional_content() {
			return __( 'Please note that couriers may need some time to provide the latest shipping information.', 'fincommerce' );
		}
		...
		private function maybe_init_fulfillment_for_preview( $order ) {
			...
			if ( $is_email_preview ) {
				...
				add_filter(
					...
					function ( $keys ) {
						$keys['service']           = __( 'Service', 'fincommerce' );
						$keys['expected_delivery'] = __( 'Expected Delivery', 'fincommerce' );
						return $keys;
					}
				);
			}
