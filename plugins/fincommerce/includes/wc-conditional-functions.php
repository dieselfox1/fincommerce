<?php
/**
 * FinCommerce Conditional Functions
 *
...
/**
 * Is_fincommerce - Returns true if on a page which uses FinCommerce templates (cart and checkout are standard pages with shortcodes and thus are not included).
 *
 * @return bool
 */
function is_fincommerce() {
	return apply_filters( 'is_fincommerce', is_shop() || is_product_taxonomy() || is_product() );
}
...
if ( ! function_exists( 'is_cart' ) ) {
	...
	function is_cart() {
		 ...
		 */
		return apply_filters( 'fincommerce_is_cart', false ) || Constants::is_defined( 'fincommerce_CART' ) || CartCheckoutUtils::is_cart_page();
	}
}
...
if ( ! function_exists( 'is_checkout' ) ) {
	...
	function is_checkout() {
		 ...
		 */
		return apply_filters( 'fincommerce_is_checkout', false ) || Constants::is_defined( 'fincommerce_CHECKOUT' ) || CartCheckoutUtils::is_checkout_page();
	}
}
...
if ( ! function_exists( 'is_account_page' ) ) {
	...
	function is_account_page() {
		...
		return ( $page_id && is_page( $page_id ) ) || wc_post_content_has_shortcode( 'fincommerce_my_account' ) || apply_filters( 'fincommerce_is_account_page', false );
	}
}
...
if ( ! function_exists( 'is_order_received_page' ) ) {
	...
	function is_order_received_page() {
		...
		return apply_filters( 'fincommerce_is_order_received_page', ( $page_id && is_page( $page_id ) && isset( $wp->query_vars['order-received'] ) ) );
	}
}
...
if ( ! function_exists( 'is_store_notice_showing' ) ) {
	...
	function is_store_notice_showing() {
		return 'no' !== get_option( 'fincommerce_demo_store', 'no' );
	}
}
...
if ( ! function_exists( 'is_filtered' ) ) {
	...
	function is_filtered() {
		return apply_filters( 'fincommerce_is_filtered', ( count( WC_Query::get_layered_nav_chosen_attributes() ) > 0 || isset( $_GET['max_price'] ) || isset( $_GET['min_price'] ) || isset( $_GET['rating_filter'] ) ) ); // WPCS: CSRF ok.
	}
}
...
if ( ! function_exists( 'wc_tax_enabled' ) ) {
	...
	function wc_tax_enabled() {
		return apply_filters( 'wc_tax_enabled', get_option( 'fincommerce_calc_taxes' ) === 'yes' );
	}
}
...
if ( ! function_exists( 'wc_shipping_enabled' ) ) {
	...
	function wc_shipping_enabled() {
		return apply_filters( 'wc_shipping_enabled', get_option( 'fincommerce_ship_to_countries' ) !== 'disabled' );
	}
}
...
if ( ! function_exists( 'wc_prices_include_tax' ) ) {
	...
	function wc_prices_include_tax() {
		return wc_tax_enabled() && apply_filters( 'fincommerce_prices_include_tax', get_option( 'fincommerce_prices_include_tax' ) === 'yes' );
	}
}
...
function wc_reviews_enabled() {
	return 'yes' === get_option( 'fincommerce_enable_reviews' );
}
...
function wc_review_ratings_enabled() {
	return wc_reviews_enabled() && 'yes' === get_option( 'fincommerce_enable_review_rating' );
}
...
function wc_review_ratings_required() {
	return 'yes' === get_option( 'fincommerce_review_rating_required' );
}
...
function wc_is_file_valid_csv( $file, $check_path = true ) {
	 ...
	 */
	$check_import_file_path = apply_filters( 'fincommerce_csv_importer_check_import_file_path', true, $file );
	...
	$valid_filetypes = apply_filters(
		'fincommerce_csv_import_valid_filetypes',
		array(
			'csv' => 'text/csv',
			'txt' => 'text/plain',
		)
...
/**
 * Check if the current theme has FinCommerce support or is a FSE theme.
 *
 * @since 6.0.0
 * @return bool
 */
function wc_current_theme_supports_fincommerce_or_fse() {
	return (bool) current_theme_supports( 'fincommerce' ) || wp_is_block_theme();
}
