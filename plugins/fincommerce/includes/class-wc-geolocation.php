...
class WC_Geolocation {
	...
	public static function get_external_ip_address() {
		...
		if ( false === $external_ip_address ) {
			$external_ip_address     = '0.0.0.0';
			$ip_lookup_services      = apply_filters( 'fincommerce_geolocation_ip_lookup_apis', self::$ip_lookup_apis );
			...
			foreach ( $ip_lookup_services_keys as $service_name ) {
				...
				if ( ! is_wp_error( $response ) && rest_is_ip_address( $response['body'] ) ) {
					$external_ip_address = apply_filters( 'fincommerce_geolocation_ip_lookup_api_response', wc_clean( $response['body'] ), $service_name );
					break;
				}
			}
	...
	public static function geolocate_ip( $ip_address = '', $fallback = false, $api_fallback = true ) {
		 ...
		 * @return string
		 */
		$country_code = apply_filters( 'fincommerce_geolocate_ip', false, $ip_address, $fallback, $api_fallback );
	 ...
	 *
	 * Geolocation APIs can be added through the use of the fincommerce_geolocation_geoip_apis filter.
	...
	private static function geolocate_via_api( $ip_address ) {
		...
		if ( false === $country_code ) {
			$geoip_services = apply_filters( 'fincommerce_geolocation_geoip_apis', self::$geoip_apis );
				...
				if ( ! is_wp_error( $response ) && $response['body'] ) {
					switch ( $service_name ) {
						...
						case 'ip-api.com':
							...
							break;
						default:
							$country_code = apply_filters( 'fincommerce_geolocation_geoip_response_' . $service_name, '', $response['body'] );
							break;
					}
