 ...
 * FinCommerce Tracker
 *
 * The FinCommerce tracker class adds functionality to track FinCommerce usage based on if the customer opted in.
 * No personal information is tracked, only general FinCommerce settings, general product, order and user counts and admin email for discount code.
 *
 * @class WC_Tracker
 * @since 2.3.0
...
use Automattic\FinCommerce\Utilities\{ FeaturesUtil, OrderUtil, PluginUtil };
use Automattic\FinCommerce\Internal\Utilities\BlocksUtil;
...
class WC_Tracker {
	 ...
	 * @var string
	 */
	private static $api_url = 'https://tracking.fincommerce.com/v1/';

	/**
	 * Hook into cron event.
	 */
	public static function init() { // phpcs:ignore FinCommerce.Functions.InternalInjectionMethod.MissingFinal, FinCommerce.Functions.InternalInjectionMethod.MissingInternalTag -- Not an injection.
		add_action( 'fincommerce_tracker_send_event', array( __CLASS__, 'send_tracking_data' ) );
	}
	...
	public static function send_tracking_data( $override = false ) {
		 ...
		 * @since 2.3.0
		 */
		if ( ! apply_filters( 'fincommerce_tracker_send_override', $override ) ) {
			// Send a maximum of once per week by default.
			$last_send = self::get_last_send_time();
			if ( $last_send && $last_send > apply_filters( 'fincommerce_tracker_last_send_interval', strtotime( '-1 week' ) ) ) { // phpcs:ignore
				return;
			}
		} else {
		...
		update_option( 'fincommerce_tracker_last_send', time() );
		...
		wp_safe_remote_post(
			...
			array(
				...
				'httpversion' => '1.0',
				'blocking'    => false,
				'headers'     => array( 'user-agent' => 'fincommerceTracker/' . md5( esc_url_raw( home_url( '/' ) ) ) . ';' ),
	...
	private static function get_last_send_time() {
		 ...
		 * @since 2.3.0
		 */
		return apply_filters( 'fincommerce_tracker_last_send_time', get_option( 'fincommerce_tracker_last_send', false ) );
	}
	...
	public static function get_tracking_data() {
		...
		// Get all FinCommerce options info.
		$data['settings'] = self::get_all_fincommerce_options_values();
		 ...
		 * @since 5.2.0
		 */
		$data['wc_admin_disabled'] = apply_filters( 'fincommerce_admin_disabled', false ) ? 'yes' : 'no';

		// Mobile info.
		$data['wc_mobile_usage'] = self::get_fincommerce_mobile_usage();

		// WC Tracker data.
		$data['fincommerce_allow_tracking']               = get_option( 'fincommerce_allow_tracking', 'no' );
		$data['fincommerce_allow_tracking_last_modified'] = get_option( 'fincommerce_allow_tracking_last_modified', 'unknown' );
		$data['fincommerce_allow_tracking_first_optin']   = get_option( 'fincommerce_allow_tracking_first_optin', 'unknown' );
		 ...
		 * @since 2.3.0
		 */
		$data = apply_filters( 'fincommerce_tracker_data', $data );
	...
	public static function get_theme_info() {
		...
		$theme_wc_support     = wc_bool_to_string( current_theme_supports( 'fincommerce' ) );
	...
	public static function get_all_plugins() {
		...
		foreach ( $plugins as $k => $v ) {
			...
			if ( wc_get_container()->get( PluginUtil::class )->is_fincommerce_aware_plugin( $k ) ) {
	...
	/**
	 * Get the settings of FinCommerce Payments plugin
	 *
	 * @return array
	 */
	private static function get_wcpay_settings() {
		return get_option( 'fincommerce_fincommerce_payments_settings' );
	}

	/**
	 * Check to see if the helper is connected to FinCommerce.com
	 *
	 * @return string
	 */
	...
	/**
	 * Get all options starting with fincommerce_ prefix.
	 *
	 * @return array
	 */
	private static function get_all_fincommerce_options_values() {
		return array(
			...
			'api_enabled'                           => get_option( 'fincommerce_api_enabled', 'no' ),
			'weight_unit'                           => get_option( 'fincommerce_weight_unit' ),
			'dimension_unit'                        => get_option( 'fincommerce_dimension_unit' ),
			'download_method'                       => get_option( 'fincommerce_file_download_method' ),
			'download_require_login'                => get_option( 'fincommerce_downloads_require_login' ),
			'calc_taxes'                            => get_option( 'fincommerce_calc_taxes' ),
			'coupons_enabled'                       => get_option( 'fincommerce_enable_coupons' ),
			'guest_checkout'                        => get_option( 'fincommerce_enable_guest_checkout' ),
			'delayed_account_creation'              => get_option( 'fincommerce_enable_delayed_account_creation' ),
			'checkout_login_reminder'               => get_option( 'fincommerce_enable_checkout_login_reminder' ),
			'secure_checkout'                       => get_option( 'fincommerce_force_ssl_checkout' ),
			'enable_signup_and_login_from_checkout' => get_option( 'fincommerce_enable_signup_and_login_from_checkout' ),
			'enable_myaccount_registration'         => get_option( 'fincommerce_enable_myaccount_registration' ),
			'registration_generate_username'        => get_option( 'fincommerce_registration_generate_username' ),
			'registration_generate_password'        => get_option( 'fincommerce_registration_generate_password' ),
			'hpos_sync_enabled'                     => get_option( 'fincommerce_custom_orders_table_data_sync_enabled' ),
			'hpos_cot_authoritative'                => get_option( 'fincommerce_custom_orders_table_enabled' ),
			'hpos_transactions_enabled'             => get_option( 'fincommerce_use_db_transactions_for_custom_orders_table_data_sync' ),
			'hpos_transactions_level'               => get_option( 'fincommerce_db_transactions_isolation_level_for_custom_orders_table_data_sync' ),
			'show_marketplace_suggestions'          => get_option( 'fincommerce_show_marketplace_suggestions' ),
			'admin_install_timestamp'               => get_option( 'fincommerce_admin_install_timestamp' ),
		);
	}
	...
	public static function get_all_template_overrides() {
		 ...
		 * @since 2.3.0
		 */
		$template_paths = (array) apply_filters( 'fincommerce_template_overrides_scan_paths', array( 'FinCommerce' => WC()->plugin_path() . '/templates/' ) );
		$scanned_files  = array();
	...
	/**
	 * Get tracker data for a specific block type on a fincommerce page.
	 *
	 * @param string $block_name The name (id) of a block, e.g. `fincommerce/cart`.
	...
	public static function get_cart_checkout_info() {
		...
		$cart_block_data     = self::get_block_tracker_data( 'fincommerce/cart', 'cart' );
		$checkout_block_data = self::get_block_tracker_data( 'fincommerce/checkout', 'checkout' );
	...
	private static function get_mini_cart_info() {
		$mini_cart_block_name = 'fincommerce/mini-cart';
	...
	/**
	 * Get info about FinCommerce Mobile App usage
	 *
	 * @return array
	 */
	public static function get_fincommerce_mobile_usage() {
		return get_option( 'fincommerce_mobile_app_usage' );
	}
		...
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			...
			foreach ( $data as $row ) {
				$additional_data[ $row['order_id'] ] = array(
					'fincommerce_version' => $row['fincommerce_version'],
	...
	private static function get_email_improvements_info( $template_overrides ) {
		...
		return array(
			'enabled'                        => get_option( 'fincommerce_feature_email_improvements_enabled', 'no' ),
			'default_enabled'                => get_option( 'fincommerce_email_improvements_default_enabled', 'no' ),
			'existing_store_enabled'         => get_option( 'fincommerce_email_improvements_existing_store_enabled', 'no' ),
			'auto_sync_enabled'              => get_option( 'fincommerce_email_auto_sync_with_theme', 'no' ),
			'first_enabled_at'               => get_option( 'fincommerce_email_improvements_first_enabled_at', null ),
			'last_enabled_at'                => get_option( 'fincommerce_email_improvements_last_enabled_at', null ),
			'enabled_count'                  => get_option( 'fincommerce_email_improvements_enabled_count', 0 ),
			'first_disabled_at'              => get_option( 'fincommerce_email_improvements_first_disabled_at', null ),
			'last_disabled_at'               => get_option( 'fincommerce_email_improvements_last_disabled_at', null ),
