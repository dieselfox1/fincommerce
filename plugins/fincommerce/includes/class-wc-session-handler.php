 ...
 *
 * From 2.5 this uses a custom table for session storage. Based on https://github.com/kloon/fincommerce-large-sessions.
 *
...
class WC_Session_Handler extends WC_Session {
	...
	public function __construct() {
		 ...
		 */
		$this->_cookie = (string) apply_filters( 'fincommerce_cookie', 'wp_fincommerce_session_' . COOKIEHASH );
		$this->_table  = $GLOBALS['wpdb']->prefix . 'fincommerce_sessions';
	...
	protected function init_hooks() {
		add_action( 'fincommerce_set_cart_cookies', array( $this, 'set_customer_session_cookie' ), 10 );
	...
	private function init_session_from_request() {
		$session_token = wc_clean( wp_unslash( $_GET['session'] ?? '' ) ); // phpcs:ignore finpress.Security.NonceVerification.Recommended
	...
	public function init_session_cookie() {
		 ...
		 * however this caused a conflict with FinCommerce Payments session handler which overrides this class.
		 *
		 * Ref: https://github.com/dieselfox1/fincommerce/pull/57652
		 * See also: https://github.com/dieselfox1/fincommerce/pull/59530
		 */
	...
	private function migrate_guest_session_to_user_session() {
		 ...
		 */
		do_action( 'fincommerce_guest_session_to_user_id', $guest_session_id, $this->_customer_id );
	}
	...
	private function restore_session_data() {
		 ...
		 */
		$this->_data = (array) apply_filters( 'fincommerce_restored_session_data', $session_data );
	}
	...
	public function maybe_update_nonce_user_logged_out( $uid, $action ) {
		if ( is_string( $action ) && StringUtil::starts_with( $action, 'fincommerce' ) ) {
