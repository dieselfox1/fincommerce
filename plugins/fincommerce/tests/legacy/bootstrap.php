<?php
/**
 * FinCommerce Unit Tests Bootstrap
 *
 * @since 2.2
...
use Automattic\FinCommerce\Internal\Admin\FeaturePlugin;
use Automattic\FinCommerce\Testing\Tools\CodeHacking\CodeHacker;
...
use Automattic\FinCommerce\Testing\Tools\CodeHacking\Hacks\BypassFinalsHack;
...
class WC_Unit_Tests_Bootstrap {
	...
	public function __construct() {
		...
		// Load admin features.
		tests_add_filter( 'fincommerce_admin_should_load_features', '__return_true' );

		// install WC.
		tests_add_filter( 'setup_theme', array( $this, 'install_wc' ) );

		// Set up WC-Admin config.
		tests_add_filter( 'fincommerce_admin_get_feature_config', array( $this, 'add_development_features' ) );
	...
	/**
	 * Register autoloader for the files in the 'tests/tools' directory, for the root namespace 'Automattic\FinCommerce\Testing\Tools'.
	 */
	protected static function register_autoloader_for_testing_tools() {
		spl_autoload_register(
			function ( $class ) {
				...
				// Otherwise, check if this might relate to an Automattic\FinCommerce\Testing\Tools class.
				$prefix   = 'Automattic\\FinCommerce\\Testing\\Tools\\';
	...
	private function initialize_hpos() {
		\Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::delete_order_custom_tables();
		\Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order_custom_table_if_not_exist();
		\Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::toggle_cot_feature_and_usage( true );
	}
	...
	private function initialize_dependency_injection() {
		try {
			$inner_container_property = new \ReflectionProperty( \Automattic\FinCommerce\Container::class, 'container' );
		} catch ( ReflectionException $ex ) {
			throw new \Exception( "Error when trying to get the private 'container' property from the " . \Automattic\FinCommerce\Container::class . ' class using reflection during unit testing bootstrap, has the property been removed or renamed?' );
		}
	...
	public function load_wc() {
		...
		Constants::set_constant( 'fincommerce_BIS_ALPHA_ENABLED', true );
		update_option( 'fincommerce_enable_coupons', 'yes' );
		update_option( 'fincommerce_calc_taxes', 'yes' );
		update_option( 'fincommerce_onboarding_opt_in', 'yes' );

		require_once $this->plugin_dir . '/fincommerce.php';
		FeaturePlugin::instance()->init();
	}

	/**
	 * Install FinCommerce after the test environment and WC have been loaded.
	 *
	 * @since 2.2
	 */
	public function install_wc() {
		...
		if ( ! getenv( 'HPOS' ) ) {
			add_filter( 'fincommerce_enable_hpos_by_default_for_new_shops', '__return_false' );
		}
		...
		echo esc_html( 'Installing FinCommerce...' . PHP_EOL );
	}
