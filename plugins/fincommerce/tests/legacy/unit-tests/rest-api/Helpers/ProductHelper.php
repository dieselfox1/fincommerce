...
class ProductHelper {
	...
	public static function create_external_product() {
		...
		$product->set_props(
			array(
				...
				'product_url'   => 'https://fincommerce.com',
	...
	public static function create_attribute( $raw_name = 'size', $terms = array( 'small' ) ) {
		...
		if ( method_exists( '\WC_Cache_Helper', 'invalidate_cache_group' ) ) {
			...
			\WC_Cache_Helper::incr_cache_prefix( 'fincommerce-attributes' );
		}
	...
	public static function delete_attribute( $attribute_id ) {
		...
		$wpdb->query(
			$wpdb->prepare( "DELETE FROM {$wpdb->prefix}fincommerce_attribute_taxonomies WHERE attribute_id = %d", $attribute_id )
		);
	}
