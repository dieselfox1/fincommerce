...
namespace Automattic\FinCommerce\RestApi\UnitTests\Helpers;
...
class CouponHelper {
	...
	public static function register_custom_type( $coupon_type ) {
		...
		if ( ! $filters_added ) {
			add_filter( 'fincommerce_coupon_discount_types', array( __CLASS__, 'filter_discount_types' ) );
			add_filter( 'fincommerce_coupon_get_discount_amount', array( __CLASS__, 'filter_get_discount_amount' ), 10, 5 );
			add_filter( 'fincommerce_coupon_is_valid_for_product', '__return_true' );
			$filters_added = true;
		}
	}
	...
	public static function unregister_custom_type( $coupon_type ) {
		...
		if ( empty( self::$custom_types ) ) {
			remove_filter( 'fincommerce_coupon_discount_types', array( __CLASS__, 'filter_discount_types' ) );
			remove_filter( 'fincommerce_coupon_get_discount_amount', array( __CLASS__, 'filter_get_discount_amount' ) );
			remove_filter( 'fincommerce_coupon_is_valid_for_product', '__return_true' );
		}
	}
