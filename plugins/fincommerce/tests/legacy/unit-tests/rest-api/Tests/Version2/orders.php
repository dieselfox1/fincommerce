...
class WC_Tests_API_Orders_V2 extends WC_REST_Unit_Test_Case {
	...
	public function test_get_items() {
		...
		// Create 10 orders.
		for ( $i = 0; $i < 10; $i++ ) {
			$this->orders[] = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order( $this->user );
		}
	...
	public function test_get_items_without_permission() {
		...
		$this->orders[] = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order();
		$response       = $this->server->dispatch( new WP_REST_Request( 'GET', '/wc/v2/orders' ) );
	...
	public function test_get_item_with_line_items_meta_data() {
		...
		$order = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order();
	...
	public function test_create_order_with_invalid_product() {
		...
		$response = $this->server->dispatch( $request );
		$data     = $response->get_data();
		$this->assertEquals( 'fincommerce_rest_required_product_reference', $data['code'] );
		$this->assertEquals( 400, $response->get_status() );
	}
	...
	public function test_update_order_after_delete_product() {
		wp_set_current_user( $this->user );
		$product = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\ProductHelper::create_simple_product();
		$order   = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order( 1, $product );
		$product->delete( true );
	...
	public function test_update_order_add_coupons() {
		...
		$coupon     = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\CouponHelper::create_coupon( 'fake-coupon' );
		$coupon->set_amount( 5 );
		$coupon->save();
		$request = new WP_REST_Request( 'PUT', '/wc/v2/orders/' . $order->get_id() );
		$request->set_body_params(
	...
	public function test_orders_batch() {
		...
		$order1 = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order();
		$order2 = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order();
		$order3 = \Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper::create_order();
