 ...
 *
 * @package Automattic/FinCommerce/Tests
 */
...
class WC_Tests_REST_System_Status extends WC_REST_Unit_Test_Case {
	...
	public function test_get_system_status_info_database() {
		...
		$this->assertArrayHasKey( 'fincommerce', $database['database_tables'], wc_print_r( $database, true ) );
		$this->assertArrayHasKey( $wpdb->prefix . 'fincommerce_payment_tokens', $database['database_tables']['fincommerce'], wc_print_r( $database, true ) );
	}
	...
	public function test_get_system_status_info_settings() {
		...
		$this->assertEquals( ( 'yes' === get_option( 'fincommerce_api_enabled' ) ), $settings['api_enabled'] );
		$this->assertEquals( get_fincommerce_currency(), $settings['currency'] );
	...
	public function test_execute_system_tool() {
		...
		$this->assertEquals( 1, did_action( 'fincommerce_rest_insert_system_status_tool' ) );
	...
	protected function mock_http_responses( $request, $url ) {
		...
		if ( in_array( $url, array( 'https://www.paypal.com/cgi-bin/webscr', 'https://fincommerce.com/wc-api/product-key-api?request=ping&network=0' ), true ) ) {
