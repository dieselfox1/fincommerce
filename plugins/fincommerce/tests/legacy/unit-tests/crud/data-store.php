...
class WC_Tests_Data_Store extends WC_Unit_Test_Case {
	...
	public function test_store_swap() {
		...
		add_filter( 'fincommerce_dummy_data_store', array( $this, 'set_dummy_store' ) );

		$store = new WC_Data_Store( 'dummy' );
		$this->assertEquals( 'WC_Dummy_Data_Store_Custom_Table', $store->get_current_class_name() );

		add_filter( 'fincommerce_dummy_data_store', array( $this, 'set_default_dummy_store' ) );
	}
	...
	/**
	 * Loads two dummy data store classes that can be swapped out for each other. Adds to the `fincommerce_data_stores` filter.
	 *
	 * @since 3.0.0
	 */
	private function load_dummy_store() {
		include_once dirname( dirname( dirname( __FILE__ ) ) ) . '/framework/class-wc-dummy-data-store.php';
		add_filter( 'fincommerce_data_stores', array( $this, 'add_dummy_data_store' ) );
	}
