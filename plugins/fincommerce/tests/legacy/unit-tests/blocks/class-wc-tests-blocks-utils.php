 ...
 *
 * @package FinCommerce\Tests\Blocks
 */
...
class WC_Test_Blocks_Utils extends WC_Unit_Test_Case {
	...
	public function test_has_block_in_page_on_page_with_single_block() {
		$page = array(
			'name'    => 'blocks-page',
			'title'   => 'Checkout',
			'content' => '<!-- wp:fincommerce/checkout {"showOrderNotes":false} --> <div class="wp-block-fincommerce-checkout is-loading"></div> <!-- /wp:fincommerce/checkout -->',
		);

		$page_id = wc_create_page( $page['name'], '', $page['title'], $page['content'] );

		$this->assertTrue( WC_Blocks_Utils::has_block_in_page( $page_id, 'fincommerce/checkout' ) );
		$this->assertFalse( WC_Blocks_Utils::has_block_in_page( $page_id, 'fincommerce/cart' ) );
	}
	...
	public function test_has_block_in_page_on_page_with_no_blocks() {
		$page = array(
			'name'    => 'shortcode-page',
			'title'   => 'Checkout',
			'content' => '<!-- wp:shortcode --> [fincommerce_checkout] <!-- /wp:shortcode -->',
		);

		$page_id = wc_create_page( $page['name'], '', $page['title'], $page['content'] );

		$this->assertFalse( WC_Blocks_Utils::has_block_in_page( $page_id, 'fincommerce/checkout' ) );
		$this->assertFalse( WC_Blocks_Utils::has_block_in_page( $page_id, 'fincommerce/cart' ) );
	}
	...
	public function test_has_block_in_page_on_page_with_multiple_blocks() {
		$page = array(
			'name'    => 'shortcode-page',
			'title'   => 'Checkout',
			'content' => '<!-- wp:fincommerce/featured-product {"editMode":false,"productId":17} -->
		...
		$this->assertTrue( WC_Blocks_Utils::has_block_in_page( $page_id, 'fincommerce/featured-product' ) );
	...
	public function test_get_all_blocks_from_page() {
		...
		wc_create_page( $page['name'], 'fincommerce_cart_page_id', $page['title'], $page['content'] );
