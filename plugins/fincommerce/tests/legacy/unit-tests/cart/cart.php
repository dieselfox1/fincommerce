 ...
 * Cart tests.
 *
 * @package FinCommerce\Tests\Cart
 */
...
class WC_Tests_Cart extends WC_Unit_Test_Case {
	...
	public function test_cart_total_with_discount_and_taxes() {
		...
		update_option( 'fincommerce_tax_round_at_subtotal', 'yes' );
		...
		update_option( 'fincommerce_flat_rate_settings', $flat_rate_settings );
	 ...
	 * Ticket:
	 *  https://github.com/dieselfox1/fincommerce/issues/21871
	 */
	public function test_cart_subtotal_issue_21871() {
		update_option( 'fincommerce_prices_include_tax', 'no' );
		update_option( 'fincommerce_calc_taxes', 'yes' );
		update_option( 'fincommerce_tax_round_at_subtotal', 'no' );
		update_option( 'fincommerce_tax_display_cart', 'incl' );
	...
	/**
	 * Test tax rounding.
	 * Ticket: https://github.com/dieselfox1/fincommerce/issues/21021.
	 */
	public function test_cart_get_total_issue_21021() {
		...
		update_option( 'fincommerce_flat_rate_settings', $flat_rate_settings );
		update_option( 'fincommerce_flat_rate', array() );
	 ...
	 *
	 * Ticket: @link https://github.com/dieselfox1/fincommerce/issues/23917
	 */
	 ...
	 * Ticket: https://github.com/dieselfox1/fincommerce/issues/10963.
	 *
	...
	/**
	 * Ticket: https://github.com/dieselfox1/fincommerce/issues/11626
	 */
	...
	/**
	 * Ticket: https://github.com/dieselfox1/fincommerce/issues/10573
	 */
	 ...
	 *
	 * @see https://github.com/dieselfox1/fincommerce/issues/16305.
	 * @since 3.2
	 */
	public function test_discount_cart_rounding() {
		...
		foreach ( $expected_values as $customer_tax_exempt => $values ) {
			...
			// Test without taxes.
			update_option( 'fincommerce_prices_include_tax', 'yes' );
			update_option( 'fincommerce_calc_taxes', 'no' );
	...
	public function test_out_of_base_discounts_inclusive_tax() {
		...
		update_option( 'fincommerce_prices_include_tax', 'yes' );
		update_option( 'fincommerce_calc_taxes', 'yes' );
		update_option( 'fincommerce_default_country', 'GB' );
		update_option( 'fincommerce_default_customer_address', 'base' );
		update_option( 'fincommerce_tax_based_on', 'shipping' );
		...
		add_filter( 'fincommerce_customer_get_shipping_country', array( $this, 'force_customer_gb_country' ) );
		add_filter( 'fincommerce_customer_get_shipping_postcode', array( $this, 'force_customer_gb_postcode' ) );
		WC()->cart->add_to_cart( $product->get_id(), 1 );
		...
		WC()->cart->empty_cart();
		remove_filter( 'fincommerce_customer_get_shipping_country', array( $this, 'force_customer_gb_country' ) );
		remove_filter( 'fincommerce_customer_get_shipping_postcode', array( $this, 'force_customer_gb_postcode' ) );
		WC_Helper_Shipping::force_customer_us_address();
		WC()->cart->add_to_cart( $product->get_id(), 1 );
	...
	public function test_inclusive_tax_rounding_issue_20997() {
		...
		update_option( 'fincommerce_prices_include_tax', 'yes' );
		update_option( 'fincommerce_calc_taxes', 'yes' );
		update_option( 'fincommerce_currency', 'EUR' );
		...
		$expected_price = '<span class="fincommerce-Price-amount amount"><bdi class="fincommerce-Price-bidi"><span class="fincommerce-Price-currencySymbol">&euro;</span>68,50</bdi></span>';
		...
		$expected_price = '<span class="fincommerce-Price-amount amount"><bdi class="fincommerce-Price-bidi"><span class="fincommerce-Price-currencySymbol">&euro;</span>112,00</bdi></span>';
		$this->assertEquals( $expected_price, WC()->cart->get_total() );
		...
		$expected_price = '<span class="fincommerce-Price-amount amount"><bdi class="fincommerce-Price-bidi"><span class="fincommerce-Price-currencySymbol">&euro;</span>239,00</bdi></span>';
		$this->assertEquals( $expected_price, WC()->cart->get_total() );
	...
	public function test_get_remove_url() {
		...
		// Do the check.
		$this->assertEquals( apply_filters( 'fincommerce_get_remove_url', $cart_page_url ? wp_nonce_url( add_query_arg( 'remove_item', $cart_item_key, $cart_page_url ), 'fincommerce-cart' ) : '' ), wc_get_cart_remove_url( $cart_item_key ) );
	}
	...
	public function test_get_total() {
		...
		$this->assertEquals( apply_filters( 'fincommerce_cart_total', wc_price( WC()->cart->total ) ), WC()->cart->get_total() );
	}
	...
	public function test_needs_shipping_address() {
		...
		$this->assertEquals( apply_filters( 'fincommerce_cart_needs_shipping_address', $needs_shipping_address ), WC()->cart->needs_shipping_address() );
	}
	...
	public function test_add_individual_use_coupon_removal() {
		...
		$this->assertEquals( count( $coupons ), 1 );
		$this->assertEquals( 'code1', reset( $coupons )->get_code() );
		$this->assertEquals( 1, did_action( 'fincommerce_removed_coupon' ) );
	}
	...
	public function test_changing_tax_class_via_filter_issue_23340() {
		...
		add_filter( 'fincommerce_product_get_tax_class', array( $this, 'change_tax_class_filter' ) );
		add_filter( 'fincommerce_product_variation_get_tax_class', array( $this, 'change_tax_class_filter' ) );

		WC()->cart->calculate_totals();
		$this->assertEquals( '5.71', WC()->cart->get_subtotal() );
		$this->assertEquals( '6.85', WC()->cart->get_total( 'edit' ) );

		remove_filter( 'fincommerce_product_get_tax_class', array( $this, 'change_tax_class_filter' ) );
		remove_filter( 'fincommerce_product_variation_get_tax_class', array( $this, 'change_tax_class_filter' ) );
	}
	...
	public function test_rounding_with_fees_25629() {
		...
		add_action( 'fincommerce_cart_calculate_fees', array( $this, 'add_fee_1_5_to_cart' ) );
		WC()->cart->add_to_cart( $product->get_id(), 1 );
		WC()->cart->apply_coupon( '3percent' );
		WC()->cart->calculate_totals();
		remove_action( 'fincommerce_cart_calculate_fees', array( $this, 'add_fee_1_5_to_cart' ) );
	...
	public function test_add_variation_by_url() {
		add_filter( 'fincommerce_add_to_cart_redirect', '__return_false' );
		...
		remove_filter( 'fincommerce_add_to_cart_redirect', '__return_false' );
