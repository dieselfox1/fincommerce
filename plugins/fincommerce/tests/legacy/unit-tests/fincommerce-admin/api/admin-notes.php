...
class WC_Admin_Tests_API_Admin_Notes extends WC_REST_Unit_Test_Case {
	...
	public function test_nonce() {
		...
		$note->add_action( 'learn-more', __( 'Learn More', 'fincommerce' ), 'https://fincommerce.com/', 'unactioned' );
		...
		$expected_url = 'https://fincommerce.com/?bar=' . wp_create_nonce( 'foo' );
	...
	public function test_nonce_when_url_has_params() {
		...
		$note = new \Automattic\FinCommerce\Admin\Notes\Note();
		$note->set_name( 'nonce-note' );
		$note->add_action( 'learn-more', __( 'Learn More', 'fincommerce' ), 'https://example.com/?x=1&y=2', 'unactioned' );
		$note->add_nonce_to_action( 'learn-more', 'foo', 'bar' );
		$note->save();
