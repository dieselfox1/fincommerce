...
use Automattic\FinCommerce\Admin\API\OnboardingThemes;
...
class WC_Admin_Tests_API_Onboarding_Themes extends WC_REST_Unit_Test_Case {
	...
	public function test_install_invalid_theme() {
		...
		$this->assertEquals( 'fincommerce_rest_theme_install', $data['code'] );
	}
	...
	public function test_activate_invalid_theme() {
		...
		$this->assertEquals( 'fincommerce_rest_invalid_theme', $data['code'] );
	}
	...
	public function test_get_recommended_themes() {
		...
		add_filter(
			'pre_option_fincommerce_show_marketplace_suggestions',
			function () {
				return 'yes';
			}
		);
