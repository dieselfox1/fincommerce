...
class WC_Admin_Tests_Notes_Data_Store extends WC_Unit_Test_Case {
	...
	public function test_read_throws_exception() {
		...
		$mock_datastore = $this->getMockBuilder( \Automattic\FinCommerce\Admin\Notes\DataStore::class )
		...
		add_filter( 'fincommerce_admin-note_data_store', $filter_datastore );
	...
	public function test_lookup_notes_versus_get_notes() {
		...
		// Add filter for 'fincommerce_note_where_clauses' that should be called only once.
		$filter_hit_count = 0;
		$filter_callback  = function( $arg ) use ( &$filter_hit_count ) {
			$filter_hit_count++;
			return $arg;
		};
		add_filter( 'fincommerce_note_where_clauses', $filter_callback, 1 );
	...
	public function test_get_notes_context_param() {
		...
		// Add filter for 'fincommerce_note_where_clauses' that applies only in context.
		...
		add_filter( 'fincommerce_note_where_clauses', $context_filter_callback, 10, 3 );

		// Add filter for 'fincommerce_note_where_clauses' that applies in any context.
		...
		add_filter( 'fincommerce_note_where_clauses', $no_context_filter_callback, 10, 3 );
