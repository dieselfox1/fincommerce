 ...
 *
 * @package FinCommerce\Tests\Gateways\Paypal
 */
...
class WC_Tests_Paypal_Gateway_Request extends WC_Unit_Test_Case {
	...
	public function test_request_url() {
		...
		$correct_options = array(
			// fincommerce_calc_taxes, fincommerce_prices_include_tax, $shipping_tax_included values.
			array( 'no', 'no', false ),
			array( 'yes', 'no', false ),
		...
		foreach ( $correct_options as $values ) {
			update_option( 'fincommerce_calc_taxes', $values[0] );
			update_option( 'fincommerce_prices_include_tax', $values[1] );
