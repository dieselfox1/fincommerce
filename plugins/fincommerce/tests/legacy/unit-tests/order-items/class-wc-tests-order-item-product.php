 ...
 *
 * @package FinCommerce\Tests\Order_Items
 * @since 3.2.0
 */
...
class WC_Tests_Order_Item_Product extends WC_Unit_Test_Case {
	...
	public function test_get_item_download_url() {
		...
		$order = new WC_Order();
		$order->set_billing_email( 'test@fincommerce.com' );
		$order->save();
		...
		$expected_regex = '/download_file=.*&order=wc_order_.*&email=test%40fincommerce.com&key=100/';
