...
class WC_Tests_Deprecated_Hooks extends WC_Unit_Test_Case {
	...
	public function test_get_old_hooks() {
		$old_filters = $this->handlers['filters']->get_old_hooks( 'fincommerce_structured_data_order' );
		$old_actions = $this->handlers['actions']->get_old_hooks( 'fincommerce_new_order_item' );

		$this->assertContains( 'fincommerce_email_order_schema_markup', $old_filters );
		$this->assertContains( 'fincommerce_order_add_shipping', $old_actions );
	}
	...
	public function test_hook_in() {
		$this->assertTrue( (bool) has_filter( 'fincommerce_structured_data_order', array( $this->handlers['filters'], 'maybe_handle_deprecated_hook' ) ) );
		$this->assertTrue( (bool) has_action( 'fincommerce_new_order_item', array( $this->handlers['actions'], 'maybe_handle_deprecated_hook' ) ) );
	}
	...
	public function test_filter_handler() {
		...
		$this->setExpectedDeprecated( 'fincommerce_product_width' );
		add_filter( 'fincommerce_product_width', array( $this, 'toggle_value' ) );

		$new_width = apply_filters( 'fincommerce_product_get_width', $test_width );
	...
	public function test_action_handler() {
		...
		$this->setExpectedDeprecated( 'fincommerce_order_edit_product' );
		add_action( 'fincommerce_order_edit_product', array( $this, 'set_meta' ), 10, 2 );
		do_action( 'fincommerce_update_order_item', $test_item_id, $test_item, $test_order_id );
	...
	public function test_created_actions_deprecation() {
		add_filter( 'fincommerce_payment_token_created', '__return_true' );
		add_filter( 'fincommerce_create_product_variation', '__return_true' );

		$this->setExpectedDeprecated( 'fincommerce_payment_token_created' );
		$this->setExpectedDeprecated( 'fincommerce_create_product_variation' );
		...
		$this->assertEquals( 1, did_action( 'fincommerce_payment_token_created' ) );
		$this->assertEquals( 1, did_action( 'fincommerce_new_payment_token' ) );
		$this->assertEquals( 1, did_action( 'fincommerce_create_product_variation' ) );
		$this->assertEquals( 1, did_action( 'fincommerce_new_product_variation' ) );
	}
}
