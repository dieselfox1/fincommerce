 ...
 *
 * @package FinCommerce\Tests\Session
 */
...
class WC_Tests_Session_Handler extends WC_Unit_Test_Case {
	...
	public function test_delete_session() {
		...
		$session_id = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT `session_id` FROM {$wpdb->prefix}fincommerce_sessions WHERE session_key = %s",
				$this->session_key
			)
		);
	...
	public function test_update_session_timestamp() {
		...
		$session_expiry = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT session_expiry FROM {$wpdb->prefix}fincommerce_sessions WHERE session_key = %s",
				$this->session_key
			)
		);
	...
	/**
	 * @testdox Test that nonce of user logged out is only changed by FinCommerce.
	 */
	public function test_maybe_update_nonce_user_logged_out() {
		...
		$this->assertEquals( $this->handler->get_customer_unique_id(), $this->handler->maybe_update_nonce_user_logged_out( 1, 'fincommerce-something' ) );
	}
	...
	public function test_destroy_session_cookie_expired() {
		...
		add_filter( 'fincommerce_set_cookie_enabled', '__return_false' );

		$handler->init_session_cookie();

		remove_filter( 'fincommerce_set_cookie_enabled', '__return_false' );
	...
	public function test_destroy_session_data_is_empty() {
		$this->markTestSkipped( 'This change was reverted due to a conflict in https://github.com/dieselfox1/fincommerce/pull/59530' );
	...
	protected function get_session_from_db( $session_key ) {
		...
		$session_data = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}fincommerce_sessions WHERE `session_key` = %s",
				$session_key
			)
		);
