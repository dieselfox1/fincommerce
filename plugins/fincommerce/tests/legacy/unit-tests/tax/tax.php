...
class WC_Tests_Tax extends WC_Unit_Test_Case {
	...
	public function test__insert_tax_rate() {
		...
		$new_row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}fincommerce_tax_rates WHERE tax_rate_id = %d",
				$tax_rate_id
			)
		);
	...
	public function test__update_tax_rate_postcodes() {
		...
		$results = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT location_code FROM {$wpdb->prefix}fincommerce_tax_rate_locations WHERE tax_rate_id = %d ORDER BY location_code ASC",
				$tax_rate_id
			)
		);
