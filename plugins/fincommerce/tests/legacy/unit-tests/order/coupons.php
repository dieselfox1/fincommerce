 ...
 * Order coupon tests.
 *
 * @package FinCommerce\Tests\Orders
 */
...
class WC_Tests_Order_Coupons extends WC_Unit_Test_Case {
	...
	protected function init_test() {
		...
		if ( get_option( 'fincommerce_prices_include_tax', 'no' ) === 'yes' && get_option( 'fincommerce_calc_taxes', 'no' ) === 'yes' ) {
			...
			$coupon_item_1->set_props(
				array(
					'code'         => 'test-coupon-1',
					'discount'     => 1,
					'discount_tax' => get_option( 'fincommerce_calc_taxes', 'no' ) === 'yes' ? 0.1 : 0,
				)
			);
			$coupon_item_2->set_props(
				array(
					'code'         => 'this-is-a-virtal-coupon',
					'discount'     => 200,
					'discount_tax' => get_option( 'fincommerce_calc_taxes', 'no' ) === 'yes' ? 20 : 0,
				)
			);
		}
