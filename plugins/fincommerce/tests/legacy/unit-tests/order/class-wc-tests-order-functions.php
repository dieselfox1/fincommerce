 ...
 * Class Functions.
 *
 * @package FinCommerce\Tests\Order
 * @since 2.3
 */
class WC_Tests_Order_Functions extends WC_Unit_Test_Case {
	...
	public function test_wc_get_order_status_name() {
		$this->assertEquals( _x( 'Pending payment', 'Order status', 'fincommerce' ), wc_get_order_status_name( OrderInternalStatus::PENDING ) );
		$this->assertEquals( _x( 'Pending payment', 'Order status', 'fincommerce' ), wc_get_order_status_name( OrderStatus::PENDING ) );
	...
	public function test_wc_ship_to_billing_address_only() {
		...
		$default = get_option( 'fincommerce_ship_to_destination' );

		update_option( 'fincommerce_ship_to_destination', 'shipping' );
		$this->assertFalse( wc_ship_to_billing_address_only() );

		update_option( 'fincommerce_ship_to_destination', 'billing_only' );
		$this->assertTrue( wc_ship_to_billing_address_only() );

		update_option( 'fincommerce_ship_to_destination', $default );
	}
	...
	public function test_wc_order_get_payment_tokens() {
		if ( \Automattic\FinCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled() ) {
	 ...
	 *
	 * @link https://github.com/dieselfox1/fincommerce/issues/24238
	 */
	 ...
	 *
	 * @link https://github.com/dieselfox1/fincommerce/issues/28969
	 */
	...
	public function test_wc_update_usage_count_without_coupon_hold() {
		...
		add_filter(
			'fincommerce_coupon_hold_minutes',
			function () {
				return 0;
			}
		);
	...
	public function test_wc_generate_order_key() {
		...
		$expected  = 'wc_' . apply_filters( 'fincommerce_generate_order_key', 'order_' . $key );
		$this->assertEquals( $expected, $order_key );

		// Test default key.
		$order_key = wc_generate_order_key();
		$prefix    = 'wc_' . apply_filters( 'fincommerce_generate_order_key', 'order_' );
