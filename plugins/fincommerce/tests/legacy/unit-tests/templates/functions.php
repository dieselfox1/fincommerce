 ...
 *
 * @package FinCommerce\Tests\Templates
 * @since   3.4.0
 */
...
class WC_Tests_Template_Functions extends WC_Unit_Test_Case {
	...
	public function test_wc_get_product_class() {
		...
		// All taxonomies.
		add_filter( 'fincommerce_get_product_class_include_taxonomies', '__return_true' );
		$expected = array(
			'foo',
			'product',
			'type-product',
		...
		$this->assertEquals( $expected, $actual, print_r( $actual, true ) );
		add_filter( 'fincommerce_get_product_class_include_taxonomies', '__return_false' );
	...
	public function test_wc_get_pay_buttons() {
		...
		$expected_html = sprintf(
			'<div class="fincommerce-pay-buttons"><div class="fincommerce-pay-button__%1$s %1$s" id="%1$s"></div></div>',
	...
	public function test_hidden_field() {
		$actual_html   = fincommerce_form_field(
			'test',
			array(
				'type'              => 'hidden',
				'id'                => 'test_field',
		...
		$expected_html = '<p class="form-row " id="test_field_field" data-priority=""><span class="fincommerce-input-wrapper"><input type="hidden" class="input-hidden test-field" name="test" id="test_field" value="test value" data-total="10" /></span></p>';
	...
	public function test_checkbox_required_field() {
		...
		$expected_html = '<p class="form-row validate-required" id="test_field" data-priority=""><span class="fincommerce-input-wrapper"><label class="checkbox " ><input type="checkbox" name="test" id="test" value="1" class="input-checkbox "  checked=\'checked\' aria-required="true" /> Checkbox&nbsp;<span class="required" aria-hidden="true">*</span></label></span></p>';
