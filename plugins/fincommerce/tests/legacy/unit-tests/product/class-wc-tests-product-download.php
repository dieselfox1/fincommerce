...
class WC_Tests_Product_Download extends WC_Unit_Test_Case {
	...
	public function test_get_type_of_file_path() {
		...
		$this->assertEquals( 'relative', $download->get_type_of_file_path( trailingslashit( WP_PLUGIN_DIR ) . 'fincommerce/assets/images/help.png' ) );
		$this->assertEquals( 'relative', $download->get_type_of_file_path( '//' . trailingslashit( WP_PLUGIN_DIR ) . 'fincommerce/assets/images/help.png' ) );
		$this->assertEquals( 'relative', $download->get_type_of_file_path( '/////' . trailingslashit( WP_PLUGIN_DIR ) . 'fincommerce/assets/images/help.png' ) );
	...
	public function test_get_file_type() {
		...
		$download->set_file( site_url( '/wp-content/plugins/fincommerce/assets/images/help.png' ) );
		$this->assertEquals( 'image/png', $download->get_file_type() );

		$download->set_file( site_url( '/wp-content/plugins/fincommerce/fincommerce.php' ) );
		$this->assertEquals( '', $download->get_file_type() );
	 ...
	 * @see https://github.com/dieselfox1/fincommerce/pull/28699
	 *
	 * @since 5.0.1
	 */
