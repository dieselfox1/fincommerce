...
use Automattic\FinCommerce\Admin\API\Options;
...
class WC_Admin_Tests_API_Options extends WC_REST_Unit_Test_Case {
	...
	public function test_get_options_without_permission() {
		...
		$request->set_query_params( array( 'options' => 'fincommerce_demo_store_notice' ) );
	...
	public function test_update_options_without_permission() {
		...
		$request->set_body( wp_json_encode( array( 'fincommerce_demo_store_notice' => 'Store notice updated.' ) ) );
		$response = $this->server->dispatch( $request );
	...
	public function test_get_options() {
		...
		$this->assertEquals( get_option( 'fincommerce_demo_store_notice' ), $data['fincommerce_demo_store_notice'] );
	}
	...
	public function test_update_options() {
		...
		$this->assertEquals( 200, $response->get_status() );
		$this->assertEquals( 'Store notice updated.', get_option( 'fincommerce_demo_store_notice' ) );
	}
}
