...
class WC_Admin_Tests_Order_Milestones extends WC_Unit_Test_Case {
	...
	/**
	 * Tests can_be_added method return false when fincommerce_admin_order_milestones_enabled is false.
	 */
	public function test_can_be_added_when_milestones_disabled() {
		add_filter(
			'fincommerce_admin_order_milestones_enabled',
			function ( $enabled ) {
				return false;
			}
		);
	...
	public function test_get_note_by_milestone_when_milestone_is_10() {
		...
		$this->assertEquals( $note->get_content(), "You've hit the 10 orders milestone! Look at you go. Browse some FinCommerce success stories for inspiration." );
