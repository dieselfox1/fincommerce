...
class WC_Admin_Tests_Marketing_Notes extends WC_Unit_Test_Case {
	...
	public function test_add_remove_marketing_note() {
		...
		// Opt out of FinCommerce marketing.
	...
	public function test_prevent_add_marketing_note() {
		...
		update_option( 'fincommerce_default_country', 'US:GA' );
		update_option( 'fincommerce_admin_install_timestamp', time() - WEEK_IN_SECONDS );
		// Set user preferences to disallow marketing suggestions.
		update_option( 'fincommerce_show_marketplace_suggestions', 'no' );

		fincommercePayments::possibly_add_note();
