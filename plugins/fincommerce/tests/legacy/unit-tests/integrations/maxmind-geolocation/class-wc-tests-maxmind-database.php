 ...
 * Class Functions.
 *
 * @package FinCommerce\Tests\Integrations
 */
...
class WC_Tests_MaxMind_Database extends WC_Unit_Test_Case {
	 ...
	 *
	 * @expectedDeprecated fincommerce_geolocation_local_database_path
	 */
	public function test_database_path_filters() {
		...
		$this->assertEquals( WP_CONTENT_DIR . '/uploads/fincommerce_uploads/' . WC_Integration_MaxMind_Database_Service::DATABASE . WC_Integration_MaxMind_Database_Service::DATABASE_EXTENSION, $path );

		add_filter( 'fincommerce_geolocation_local_database_path', array( $this, 'filter_database_path_deprecated' ), 1, 2 );
		$path = $database_service->get_database_path();
		remove_filter( 'fincommerce_geolocation_local_database_path', array( $this, 'filter_database_path_deprecated' ), 1 );

		$this->assertEquals( '/deprecated_filter', $path );

		add_filter( 'fincommerce_geolocation_local_database_path', array( $this, 'filter_database_path' ) );
		$path = $database_service->get_database_path();
		remove_filter( 'fincommerce_geolocation_local_database_path', array( $this, 'filter_database_path' ) );
		...
		$path = $database_service->get_database_path();
		$this->assertEquals( WP_CONTENT_DIR . '/uploads/fincommerce_uploads/testing-' . WC_Integration_MaxMind_Database_Service::DATABASE . WC_Integration_MaxMind_Database_Service::DATABASE_EXTENSION, $path );
	}
	...
	public function test_download_database_wraps_errors() {
		...
		$this->assertEquals( 'fincommerce_maxmind_geolocation_database_license_key', $result->get_error_code() );

		$result = $database_service->download_database( 'generic_error' );

		$this->assertWPError( $result );
		$this->assertEquals( 'fincommerce_maxmind_geolocation_database_download', $result->get_error_code() );

		$result = $database_service->download_database( 'archive_error' );

		$this->assertWPError( $result );
		$this->assertEquals( 'fincommerce_maxmind_geolocation_database_archive', $result->get_error_code() );
	}
