...
class WC_Template_Cache extends WC_Unit_Test_Case {
	...
	public function test_wc_get_template_part() {
		...
		$this->assertEquals( array_keys( $templates ), wp_cache_get( 'cached_templates', 'fincommerce' ) );
		...
		foreach ( $templates as $cache_key => $template ) {
			...
			$from_cache = wc_untokenize_path( wp_cache_get( $cache_key, 'fincommerce' ), wc_get_path_define_tokens() );
	...
	protected function clear_template_cache() {
		...
		if ( $cached_templates ) {
			foreach ( (array) $cached_templates as $template ) {
				$this->assertEmpty( wp_cache_get( $template, 'fincommerce' ) );
			}
		}
		$this->assertEmpty( wp_cache_get( 'cached_templates', 'fincommerce' ) );
	}
}
