 ...
 *
 * @package FinCommerce\Tests\Core
 */
...
class WC_Tests_Cancel_Unpaid_Orders extends WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		add_filter( 'fincommerce_cancel_unpaid_order', '__return_false', 10, 0 );
		$this->original_hold_stock_minutes  = get_option( 'fincommerce_hold_stock_minutes', 0 );
		$this->original_manage_stock_option = get_option( 'fincommerce_manage_stock', 'yes' );
	}
	...
	public function test_cancel_unpaid_orders_reschedules_itself() {
		...
		update_option( 'fincommerce_hold_stock_minutes', 1 );
		update_option( 'fincommerce_manage_stock', 'yes' );
		...
		$this->assertFalse(
			as_next_scheduled_action( 'fincommerce_cancel_unpaid_orders', array(), 'fincommerce' ),
		...
		$next_scheduled = as_next_scheduled_action( 'fincommerce_cancel_unpaid_orders', array(), 'fincommerce' );
	...
	public function test_cancel_unpaid_orders_clears_existing_actions() {
		...
		update_option( 'fincommerce_hold_stock_minutes', 60 );
		update_option( 'fincommerce_manage_stock', 'yes' );

		// Manually schedule an action first, in the past so it is "past-due" - this prevents race conditions in the test.
		as_schedule_single_action( time() - 3600, 'fincommerce_cancel_unpaid_orders', array(), 'fincommerce' );

		// Verify action is scheduled.
		$old_scheduled = as_next_scheduled_action( 'fincommerce_cancel_unpaid_orders', array(), 'fincommerce' );
		...
		$new_scheduled = as_next_scheduled_action( 'fincommerce_cancel_unpaid_orders', array(), 'fincommerce' );
	...
	/**
	 * Test that the fincommerce_cancel_unpaid_orders_interval_minutes filter works.
	 */
	public function test_cancel_unpaid_orders_respects_interval_filter() {
		...
		add_filter(
			'fincommerce_cancel_unpaid_orders_interval_minutes',
	...
	public function test_cancel_unpaid_orders_no_reschedule_when_hold_stock_zero() {
		...
		update_option( 'fincommerce_hold_stock_minutes', 0 );
		update_option( 'fincommerce_manage_stock', 'yes' );
	...
	public function test_cancel_unpaid_orders_no_reschedule_when_stock_management_disabled() {
		...
		update_option( 'fincommerce_hold_stock_minutes', 60 );
		update_option( 'fincommerce_manage_stock', 'no' );
	...
	public function test_cancel_unpaid_orders_clears_both_as_and_wp_cron() {
		...
		wp_schedule_single_event( time() + 7200, 'fincommerce_cancel_unpaid_orders' );
	...
	public function tearDown(): void {
		...
		update_option( 'fincommerce_hold_stock_minutes', $this->original_hold_stock_minutes );
		update_option( 'fincommerce_manage_stock', $this->original_manage_stock_option );

		// Remove any filters that might have been added.
		remove_all_filters( 'fincommerce_cancel_unpaid_orders_interval_minutes' );
		remove_all_filters( 'fincommerce_cancel_unpaid_orders' );
		remove_all_filters( 'fincommerce_cancel_unpaid_order' );
	}
}
