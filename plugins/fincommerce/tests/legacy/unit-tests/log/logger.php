...
class WC_Tests_Logger extends WC_Unit_Test_Case {
	...
	/**
	 * Test 'fincommerce_register_log_handlers' filter.
	 *
	 * @since 3.0.0
	 */
	public function test_fincommerce_register_log_handlers_filter() {
		add_filter( 'fincommerce_register_log_handlers', array( $this, 'return_assertion_handlers' ) );
		$log = new WC_Logger();
		$log->debug( 'debug' );
		$log->info( 'info' );
		$log->notice( 'notice' );
		...
		$log->alert( 'alert' );
		$log->emergency( 'emergency' );
		remove_filter( 'fincommerce_register_log_handlers', array( $this, 'return_assertion_handlers' ) );
	}
	...
	public function test_threshold_defaults() {
		...
		// Test no filtering by default.
		delete_option( 'fincommerce_log_threshold' );
		$handler = $this
			->getMockBuilder( 'WC_Log_Handler_Interface' )
			->setMethods( array( 'handle' ) )
			->getMock();
	...
	/**
	 * Helper for 'fincommerce_register_log_handlers' filter test.
	 *
