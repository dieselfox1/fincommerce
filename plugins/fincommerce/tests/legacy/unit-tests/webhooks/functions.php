 ...
 * @package FinCommerce\Tests\Webhook
 */
...
class WC_Tests_Webhook_Functions extends WC_Unit_Test_Case {
	...
	public function data_provider_test_wc_is_webhook_valid_topic() {
		return array(
			array( true, wc_is_webhook_valid_topic( 'action.fincommerce_add_to_cart' ) ),
			...
			array( false, wc_is_webhook_valid_topic( 'action.fincommerce_login_credentials' ) ),
		);
	}
	...
	public function test_wc_load_webhooks_status( $status ) {
		...
		$webhook = $this->create_webhook( 'action.fincommerce_some_action', $status );
	...
	public function test_wc_load_webhooks_limit() {
		...
		$webhook_one = $this->create_webhook( 'action.fincommerce_one_test' );
		$webhook_two = $this->create_webhook( 'action.fincommerce_two_test' );

		$this->assertFalse( wc_load_webhooks( '', 0 ) );
		$this->assertFalse( isset( $wp_filter['fincommerce_one_test'] ) );
		$this->assertFalse( isset( $wp_filter['fincommerce_two_test'] ) );

		$this->assertTrue( wc_load_webhooks( '', 1 ) );
		$this->assertFalse( isset( $wp_filter['fincommerce_one_test'] ) );
		$this->assertTrue( isset( $wp_filter['fincommerce_two_test'] ) );

		$webhook_two->delete( true );

		$this->assertTrue( wc_load_webhooks( '', 1 ) );
		$this->assertTrue( isset( $wp_filter['fincommerce_one_test'] ) );
	...
	public function test_wc_load_webhooks_status_and_limit( $status ) {
		...
		$action_one  = 'fincommerce_one_test_status_' . $status;
		$webhook_one = $this->create_webhook( 'action.' . $action_one, $status );
		$action_two  = 'fincommerce_two_test_status_' . $status;
	 ...
	 */
	public function test_fincommerce_webhook_is_delivered_only_once() {
		...
		add_action( 'fincommerce_webhook_process_delivery', array( $this, 'fincommerce_webhook_process_delivery' ), 1, 2 );
		...
		remove_action( 'fincommerce_webhook_process_delivery', array( $this, 'fincommerce_webhook_process_delivery' ), 1, 2 );
	}
	 ...
	 */
	public function fincommerce_webhook_process_delivery( $webhook, $arg ) {
	 ...
	 */
	public function create_webhook( $topic = 'action.fincommerce_some_action', $status = 'active' ) {
