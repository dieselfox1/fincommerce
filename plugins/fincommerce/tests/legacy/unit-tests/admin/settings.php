...
class WC_Tests_Admin_Settings extends WC_Unit_Test_Case {
	...
	public function test_check_download_folder_protection() {
		$default    = get_option( 'fincommerce_file_download_method' );
		$upload_dir = wp_get_upload_dir();
		$file_path  = $upload_dir['basedir'] . '/fincommerce_uploads/.htaccess';

		// Test with "force" downloads method.
		update_option( 'fincommerce_file_download_method', 'force' );
		...
		update_option( 'fincommerce_file_download_method', 'redirect' );
		WC_Admin_Settings::check_download_folder_protection();
		$file_content = @file_get_contents( $file_path );
		$this->assertEquals( 'Options -Indexes', $file_content );

		update_option( 'fincommerce_file_download_method', $default );
	}
}
