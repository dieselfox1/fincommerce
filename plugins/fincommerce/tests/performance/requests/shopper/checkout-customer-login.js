...
export function checkoutCustomerLogin() {
	let fincommerce_process_checkout_nonce_customer;
	let fincommerce_login_nonce;
	...
	group( 'Proceed to checkout', function () {
		...
		checkResponse( response, 200, {
			...
			body: 'class="checkout fincommerce-checkout"',
			footer: FOOTER_TEXT,
		} );

		// Correlate nonce values for use in subsequent requests.
		fincommerce_login_nonce = response
			.html()
			.find( 'input[name=fincommerce-login-nonce]' )
			.first()
			.attr( 'value' );
	...
	group( 'Login on checkout', function () {
		...
		const response = http.post(
			`${ base_url }/checkout`,
			{
				...
				'fincommerce-login-nonce': `${ fincommerce_login_nonce }`,
		...
		// Correlate nonce values for use in subsequent requests.
		fincommerce_process_checkout_nonce_customer = response
			.html()
			.find( 'input[name=fincommerce-process-checkout-nonce]' )
			.first()
			.attr( 'value' );
	...
	group( 'Place Order', function () {
		...
		const response = http.post(
			`${ base_url }/?wc-ajax=checkout`,
			{
				...
				order_comments: '',
				payment_method: `${ payment_method }`,
				'fincommerce-process-checkout-nonce': `${ fincommerce_process_checkout_nonce_customer }`,
