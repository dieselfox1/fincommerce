...
export function cartApplyCoupon() {
	...
	group( 'View Cart', function () {
		...
		fincommerce_cart_nonce = response
			.html()
			.find( 'input[name=fincommerce-cart-nonce]' )
			.first()
			.attr( 'value' );
	} );
	...
	group( 'Apply Coupon', function () {
		...
		const cartResponse = http.post(
			`${ base_url }/cart`,
			{
				...
				coupon_code: '',
				'fincommerce-cart-nonce': `${ fincommerce_cart_nonce }`,
			},
			{
				headers: requestheaders,
		...
		check( cartResponse, {
			'is status 200': ( r ) => r.status === 200,
			"body contains: 'fincommerce-remove-coupon' class": ( r ) =>
				r.body.includes( 'class="fincommerce-remove-coupon"' ),
		} );
	} );
