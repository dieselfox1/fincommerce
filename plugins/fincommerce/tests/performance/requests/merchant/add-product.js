...
export function addProduct( includeTests = {} ) {
	...
	group( 'Add New Product', function () {
		...
		sample_permalink_nonce = response
			...
			.first()
			.attr( 'value' );
		fincommerce_meta_nonce = response
			.html()
			.find( 'input[id=fincommerce_meta_nonce]' )
			.first()
			.attr( 'value' );
	...
	if ( includedTests.other ) {
		group( 'Add New Product - Other Requests', function () {
			...
			response = http.get(
				`${ base_url }/wp-json/wc-admin/options?options=fincommerce_ces_tracks_queue&_locale=user`,
				{
					headers: requestHeaders,
					tags: {
						name: 'Merchant - wc-admin/options?options=fincommerce_ces_tracks_queue',
					},
				}
			);
			check( response, {
	...
	if ( includedTests.update ) {
		group( 'Update New Product', function () {
			...
			const productParams = new URLSearchParams( [
				...
				[ 'user_ID', '1' ],
				[ 'visibility', 'public' ],
				[ 'fincommerce_meta_nonce', `${ fincommerce_meta_nonce }` ],
				[ 'wp-preview', '' ],
			] );
