...
setup( 'setup site', async ( { baseURL, restApi } ) => {
	await setup.step( 'configure HPOS', async () => {
			...
			for ( let i = 0; i < hposSettingRetries; i++ ) {
				try {
					...
					const response = await restApi.post(
						`${ WC_API_PATH }/settings/advanced/fincommerce_custom_orders_table_enabled`,
						{ value }
					);
		...
		console.log(
			`HPOS configuration (fincommerce_custom_orders_table_enabled): ${ dataValue } - ${ enabledOption }`
		);
	} );
	...
	await setup.step( 'general settings', async () => {
		await restApi.post( `${ WC_API_PATH }/settings/general/batch`, {
			update: [
				{ id: 'fincommerce_allowed_countries', value: 'all' },
				{ id: 'fincommerce_currency', value: 'USD' },
				{ id: 'fincommerce_price_thousand_sep', value: ',' },
				{ id: 'fincommerce_price_decimal_sep', value: '.' },
				{ id: 'fincommerce_price_num_decimals', value: '2' },
				{ id: 'fincommerce_store_address', value: 'addr 1' },
				{ id: 'fincommerce_store_city', value: 'San Francisco' },
				{ id: 'fincommerce_default_country', value: 'US:CA' },
				{ id: 'fincommerce_store_postcode', value: '94107' },
			],
		} );
	} );
} );
