 ...
 */
import { createClient, WP_API_PATH } from '@fincommerce/e2e-utils-playwright';
...
/**
 * Access the email editor and using the FinCommerce settings page.
...
export async function accessTheEmailEditor( page, emailTitle = 'New order' ) {
	...
	await page
		...
		.getByLabel( 'Edit' )
		.click();
	await expect( page.locator( '#fincommerce-email-editor' ) ).toBeVisible();
}

export async function ensureEmailEditorSettingsPanelIsOpened( page ) {
	const status = await page.evaluate( async () => {
		const elem = document.querySelector(
			'.fincommerce-email-editor__settings-panel'
		);
	...
	if ( ! status ) {
		await page
			.locator( '.fincommerce-email-editor__settings-panel' )
			.click();
	}
}
