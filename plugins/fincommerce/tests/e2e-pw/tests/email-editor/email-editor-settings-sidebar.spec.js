...
test.describe( 'FinCommerce Email Editor Settings Sidebar Integration', () => {
	...
	test( 'Can update email subject and preview text', async ( { page } ) => {
		...
		await expect(
			page
				.locator( '.fincommerce-settings-panel-subject-text span' )
		...
		await page
			.locator(
				'.fincommerce-settings-panel-subject-text button[title="Personalization Tags"]'
			)
			.first()
		...
		await page
			...
			.filter( {
				hasText:
					/^Customer Email\[fincommerce\/customer-email\]Insert$/,
			} )
			.getByRole( 'button' )
			.click();
		...
		await page
			.locator(
				'.fincommerce-settings-panel-preheader-text button[title="Personalization Tags"]'
			)
			.first()
			.click(); // open personalization tags modal.
		await expect(
		...
		await page
			...
			.filter( {
				hasText:
					/^Customer First Name\[fincommerce\/customer-first-name\]Insert$/,
			} )
			.getByRole( 'button' )
			.click();
		await page.getByRole( 'button', { name: 'Save', exact: true } ).click();
		...
		).toContainText( `${ subject } [fincommerce/customer-email]` );
		await expect(
			page.locator( '[data-automation-id="email_preheader"]' )
		).toContainText( `${ preheader } [fincommerce/customer-first-name]` );
	} );
	...
	test( 'Can update email recipients', async ( { page } ) => {
		...
		await ensureEmailEditorSettingsPanelIsOpened( page );
		await expect(
			page.locator( '[for="fincommerce-email-editor-recipients"]' )
		).toBeVisible();
