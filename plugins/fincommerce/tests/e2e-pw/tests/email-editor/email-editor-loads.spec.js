...
test.describe( 'FinCommerce Email Editor Core', () => {
	...
	test( 'Can enable the email editor', async ( { page } ) => {
		...
		await expect(
			...
			'Manage email notifications sent from FinCommerce below'
		);
	} );
	...
	test( 'Can access the email editor', async ( { page } ) => {
		...
		).toContainText( `New order: #[fincommerce/order-number],` );
	} );
	...
	test( 'Can send test email', async ( { page } ) => {
		...
		await expect(
			page.locator( '.fincommerce-send-preview-modal-notice-error' )
	...
	test( 'Can edit and save content', async ( { page } ) => {
		...
		await page
			...
			.getByText( 'You’ve received a new' )
			.fill(
				'Hello world from Woo plugin\nYou’ve received a new order from [fincommerce/customer-full-name]'
			);
		await expect(
