	...
	() => {
		...
		test(
			...
			async ( { page, restApi } ) => {
				...
				// @see https://github.com/dieselfox1/fincommerce/issues/33077
				await restApi.put(
					`${ WC_API_PATH }/settings/general/fincommerce_default_customer_address`,
					{
						value: 'geolocation_ajax',
					}
				);
				await restApi.put(
					`${ WC_API_PATH }/settings/products/fincommerce_enable_ajax_add_to_cart`,
					{
						value: 'no',
					}
				);
