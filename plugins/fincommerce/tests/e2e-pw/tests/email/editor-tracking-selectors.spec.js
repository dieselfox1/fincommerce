...
const setFeatureFlag = async ( baseURL, value ) =>
	await setOption(
		request,
		baseURL,
		'fincommerce_feature_block_email_editor_enabled',
		value
	);
 ...
 */
test.describe( 'FinCommerce Email Editor Tracking Selectors', () => {
	...
	test( 'Check selectors for tracking events', async ( {
		...
		// Navigate to FinCommerce Email Settings page to generate email posts
		...
		const editorLocator = page.locator( '#fincommerce-email-editor' );
