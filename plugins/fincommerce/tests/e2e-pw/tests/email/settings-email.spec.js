...
test.describe( 'FinCommerce Email Settings', () => {
	test.use( { storageState: ADMIN_STATE_PATH } );

	const storeName = 'FinCommerce Core E2E Test Suite';
	...
	test(
		...
		async ( { page, baseURL } ) => {
			...
			const fromNameElement = '#fincommerce_email_from_name';
			const fromAddressElement = '#fincommerce_email_from_address';
	...
	test(
		...
		async ( { page, baseURL } ) => {
			...
			const baseColorId = 'fincommerce_email_base_color';
	...
	test(
		...
		async ( { page } ) => {
			...
			const subjectId = 'fincommerce_customer_processing_order_subject';
	...
	test( 'See updated footer text field', async ( { page } ) => {
		...
		const footerTextLabel = page.locator(
			'css=label[for="fincommerce_email_footer_text"]'
		);
		await expect( footerTextLabel ).toBeVisible();

		const tooltip = footerTextLabel.locator( 'span.fincommerce-help-tip' );
	...
	test( 'Reset color palette with a feature flag', async ( {
		...
		await page.fill( '#fincommerce_email_base_color', dummyColor );
		await page.fill( '#fincommerce_email_background_color', dummyColor );
		await page.fill(
			'#fincommerce_email_body_background_color',
			dummyColor
		);
		await page.fill( '#fincommerce_email_text_color', dummyColor );
		await page.fill( '#fincommerce_email_footer_text_color', dummyColor );
		...
		await expect(
			page.locator( '#fincommerce_email_base_color' )
		).not.toHaveValue( dummyColor );
		await expect(
			page.locator( '#fincommerce_email_background_color' )
		).not.toHaveValue( dummyColor );
		await expect(
			page.locator( '#fincommerce_email_body_background_color' )
		).not.toHaveValue( dummyColor );
		await expect(
			page.locator( '#fincommerce_email_text_color' )
		).not.toHaveValue( dummyColor );
		await expect(
			page.locator( '#fincommerce_email_footer_text_color' )
		).not.toHaveValue( dummyColor );
