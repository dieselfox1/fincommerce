...
test.describe( 'FinCommerce Email Settings List View', () => {
	...
	test( 'Email settings list view renders correctly and allows to edit email status and search', async ( {
		...
		await setFeatureFlag(
			baseURL,
			'fincommerce_feature_point_of_sale_enabled',
			'no'
		);

		// Navigate to FinCommerce Email Settings page
		await page.goto( 'wp-admin/admin.php?page=wc-settings&tab=email' );

		// Check that the ListView component is present
		const listViewLocator = page.locator(
			'.fincommerce-email-listing-listview'
		);
