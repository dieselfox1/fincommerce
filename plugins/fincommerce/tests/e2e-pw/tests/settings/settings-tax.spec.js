...
test.describe( 'FinCommerce Tax Settings > enable', () => {
	...
	test( 'can enable tax calculation', async ( { page } ) => {
		...
		await page.locator( '#fincommerce_calc_taxes' ).check();
		...
		await expect( page.locator( '#fincommerce_calc_taxes' ) ).toBeChecked();
...
test.describe.serial( 'FinCommerce Tax Settings', () => {
	test.use( { storageState: ADMIN_STATE_PATH } );

	test.beforeEach( async ( { restApi } ) => {
		await restApi.put(
			`${ WC_API_PATH }/settings/general/fincommerce_calc_taxes`,
			{
				value: 'yes',
			}
		);
	...
	test( 'can set tax options', async ( { page } ) => {
		...
		await page
			.locator( '#fincommerce_tax_based_on' )
			.selectOption( 'shipping' );
		// Standard tax class for shipping
		await page.locator( '#fincommerce_shipping_tax_class' ).selectOption( {
			label: 'Standard',
		} );
		// Leave rounding unchecked
		// Display prices excluding tax
		await page
			.locator( '#fincommerce_tax_display_shop' )
			.selectOption( 'excl' );
		// Display prices including tax in cart and at checkout
		await page
			.locator( '#fincommerce_tax_display_cart' )
			.selectOption( 'incl' );
		// Display a single tax total
		await page
			.locator( '#fincommerce_tax_total_display' )
			.selectOption( 'single' );
		await page.locator( 'text=Save changes' ).click();
		...
		await expect( page.locator( '#fincommerce_tax_based_on' ) ).toHaveValue(
			'shipping'
		);
		await expect(
			page.locator( '#fincommerce_shipping_tax_class' )
		).toContainText( 'Standard' );
		await expect(
			page.locator( '#fincommerce_tax_display_shop' )
		).toHaveValue( 'excl' );
		await expect(
			page.locator( '#fincommerce_tax_display_cart' )
		).toHaveValue( 'incl' );
		await expect(
			page.locator( '#fincommerce_tax_total_display' )
		).toHaveValue( 'single' );
	} );
	...
	test( 'can add tax classes', async ( { page } ) => {
		...
		await page.locator( '#fincommerce_tax_classes' ).fill( 'Fancy' );
		await page.locator( 'text=Save changes' ).click();
	...
	test( 'can remove tax classes', async ( { page } ) => {
		...
		await page.locator( '#fincommerce_tax_classes' ).fill( '' );
		await page.locator( 'text=Save changes' ).click();
		await expect( page.locator( '.blockOverlay' ) ).toBeHidden();
		...
		await expect( page.locator( '#fincommerce_tax_classes' ) ).toHaveValue(
			''
		);
		await expect(
