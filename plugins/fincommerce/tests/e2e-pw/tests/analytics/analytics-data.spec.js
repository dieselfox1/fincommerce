...
const test = baseTest.extend( {
	...
	page: async ( { page, restApi }, use ) => {
		...
		await restApi.post( `${ WC_ADMIN_API_PATH }/options`, {
			fincommerce_task_list_reminder_bar_hidden: 'yes',
		} );

		// Disable the orders report date tour
		await restApi.post( `${ WC_ADMIN_API_PATH }/options`, {
			fincommerce_orders_report_date_tour_shown: 'yes',
		} );

		// Disable the revenue report date tour
		await restApi.post( `${ WC_ADMIN_API_PATH }/options`, {
			fincommerce_revenue_report_date_tour_shown: 'yes',
		} );
...
test.beforeAll( async ( { browser, restApi } ) => {
	...
	await restApi
		.post(
			'wc-analytics/settings/wc_admin/fincommerce_excluded_report_order_statuses',
			{
	...
	await restApi
		.post(
			'wc-analytics/settings/wc_admin/fincommerce_actionable_order_statuses',
			{
	...
	await restApi
		.post(
			'wc-analytics/settings/wc_admin/fincommerce_default_date_range',
			{
