...
const test = baseTest.extend( {
	page: async ( { page, restApi }, use ) => {
		...
		const calcTaxesState = await updateIfNeeded(
			'general/fincommerce_calc_taxes',
			'yes'
		);

		const loginAtCheckoutState = await updateIfNeeded(
			'account/fincommerce_enable_checkout_login_reminder',
			'yes'
		);

		const signUpAtCheckoutState = await updateIfNeeded(
			'account/fincommerce_enable_signup_and_login_from_checkout',
			'yes'
		);
		...
		await resetValue( 'general/fincommerce_calc_taxes', calcTaxesState );
...
checkoutPages.forEach( ( { name, slug } ) => {
	test(
		...
		async ( { page, product, tax, customer } ) => {
			...
			// Login from classic checkout redirects to default checkout page: https://github.com/dieselfox1/fincommerce/issues/56205
