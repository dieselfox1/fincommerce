	...
	() => {
		...
		test(
			...
			async ( { pageObject, baseURL } ) => {
				...
				const categories = assembler.locator(
					'.fincommerce-customize-store__sidebar-homepage-content .components-item-group'
				);
		...
		test( 'Clicking on "Design your homepage" should open the Intro sidebar by default', async ( {
			...
			await expect(
				await assembler
					.locator(
						'.fincommerce-customize-store-edit-site-layout__sidebar-extra__pattern__header'
					)
					.textContent()
			).toContain( 'Intro' );
		} );
		...
		test( 'Clicking on a category should open the sidebar for it', async ( {
			...
			const categories = await assembler
				.locator(
					'.fincommerce-edit-site-sidebar-navigation-screen-patterns__group-homepage-label-container > span:first-child'
				)
				.all();
			...
			for ( const category of categories ) {
				...
				const sidebar = assembler.locator(
					'.fincommerce-customize-store-edit-site-layout__sidebar-extra'
				);
