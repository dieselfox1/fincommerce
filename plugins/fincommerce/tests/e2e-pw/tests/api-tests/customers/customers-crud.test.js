...
test.describe( 'Customers API tests: CRUD', () => {
		...
		if ( ! subscriberUserId ) {
			...
			const userResponse = await request.post( './wp-json/wp/v2/users', {
				data: {
					...
					email: `customer${ now }@fincommercecoretestsuite.com`,
					first_name: 'Jane',
					last_name: 'Smith',
					roles: [ 'subscriber' ],
					password: 'password',
	...
	test.describe( 'Retrieve after env setup', () => {
		 ...
		 * (https://github.com/dieselfox1/fincommerce/tree/trunk/plugins/fincommerce/tests/e2e-pw#fincommerce-playwright-end-to-end-tests),
