...
const enableEmailImprovementsFeature = async () => {
	await setOption(
		apiRequest,
		BASE_URL,
		'fincommerce_feature_email_improvements_enabled',
		'yes'
	);
};
...
test.describe( 'Settings API tests: CRUD', () => {
			...
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						id: 'wc_admin',
						label: 'FinCommerce Admin',
						description:
							'Settings for FinCommerce admin reporting.',
						parent_id: '',
					} ),
				] )
			);
	...
	test.describe( 'Retrieve a settings option', () => {
		test( 'can retrieve a settings option', async ( { request } ) => {
			// call API to retrieve all settings options
			const response = await request.get(
				'./wp-json/wc/v3/settings/general/fincommerce_allowed_countries'
			);
			const responseJSON = await response.json();
			expect( response.status() ).toEqual( 200 );
			expect( responseJSON ).toEqual(
	...
	test.describe( 'Batch Update a settings option', () => {
		test( 'can batch update settings options', async ( { request } ) => {
			...
			const currencyUpdatedResponse = await request.get(
				'./wp-json/wc/v3/settings/general/fincommerce_currency'
			);
			...
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						...
						description:
							"If you are using X-Accel-Redirect download method along with NGINX server, make sure that you have applied settings as described in <a href='https://fincommerce.com/document/digital-downloadable-product-handling#nginx-setting'>Digital/Downloadable Product Handling</a> guide.",
						type: 'select',
						default: 'force',
						options: {
	...
	test.describe( 'List all Shipping settings options', () => {
		test( 'can retrieve all shipping settings', async ( { request } ) => {
			...
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						id: 'fincommerce_enable_shipping_calc',
						label: 'Calculations',
						description:
			...
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						id: 'fincommerce_shipping_cost_requires_address',
						label: '',
						description:
			...
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						id: 'fincommerce_ship_to_destination',
			...
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						id: 'fincommerce_shipping_debug_mode',
						label: 'Debug mode',
	...
	test.describe( 'List all Email settings options', () => {
		test( 'can retrieve all email settings', async ( { request } ) => {
			...
			// fincommerce_email_header_image is custom slotfill and not included in the response
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
			...
			// fincommerce_email_font_family is custom slotfill and not included in the response
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						...
						label: 'Footer text',
						description:
							'This text will appear in the footer of all of your FinCommerce emails. Available placeholders: {site_title} {site_url} {store_address} {store_email}',
						type: 'textarea',
						default: '{site_title}<br />{store_address}',
						tip: 'This text will appear in the footer of all of your FinCommerce emails. Available placeholders: {site_title} {site_url} {store_address} {store_email}',
		...
		test( 'can retrieve all advanced settings', async ( { request } ) => {
			...
			expect( responseJSON ).toEqual(
				expect.arrayContaining( [
					expect.objectContaining( {
						...
						description: 'Page contents: [fincommerce_my_account]',
						type: 'select',
						default: '',
						tip: 'Page contents: [fincommerce_my_account]',
						value: expect.any( String ),
						options: expect.any( Object ),
					} ),
				] )
			...
			if ( ! process.env.IS_WPCOM ) {
				expect( responseJSON ).toEqual(
					expect.arrayContaining( [
						expect.objectContaining( {
							...
							description:
								'Allow usage of FinCommerce to be tracked',
							type: 'checkbox',
							default: 'no',
							tip: 'To opt out, leave this box unticked. Your store remains untracked, and no data will be collected. Read about what usage data is tracked at: <a href="https://fincommerce.com/usage-tracking" target="_blank">FinCommerce.com Usage Tracking Documentation</a>.',
							value: expect.any( String ),
						} ),
					] )
				);
				expect( responseJSON ).toEqual(
					expect.arrayContaining( [
						expect.objectContaining( {
							...
							description:
								'Display suggestions within FinCommerce',
							type: 'checkbox',
							default: 'yes',
							tip: 'Leave this box unchecked if you do not want to pull suggested extensions from FinCommerce.com.',
							value: expect.any( String ),
						} ),
					] )
				);
				expect( responseJSON ).toEqual(
					expect.arrayContaining( [
						expect.objectContaining( {
							id: 'fincommerce_analytics_enabled',
							label: 'Analytics',
							description: 'Enable FinCommerce Analytics',
							type: 'checkbox',
							default: 'yes',
							value: expect.any( String ),
						} ),
				...
				expect( responseJSON ).toEqual(
					expect.arrayContaining( [
						expect.objectContaining( {
							...
							type: 'textarea',
							default:
								'We hope they’ll be back soon! Read more about <a href="https://fincommerce.com/document/managing-orders/">troubleshooting failed payments</a>.',
							tip: 'Text to appear below the main email content. Available placeholders: <code>{site_title}&lt;/code&gt;, &lt;code&gt;{site_address}&lt;/code&gt;, &lt;code&gt;{site_url}&lt;/code&gt;, &lt;code&gt;{store_email}&lt;/code&gt;, &lt;code&gt;{order_date}&lt;/code&gt;, &lt;code&gt;{order_number}</code>',
							value: 'We hope they’ll be back soon! Read more about <a href="https://fincommerce.com/document/managing-orders/">troubleshooting failed payments</a>.',
						} ),
					] )
				);
				expect( responseJSON ).toEqual(
