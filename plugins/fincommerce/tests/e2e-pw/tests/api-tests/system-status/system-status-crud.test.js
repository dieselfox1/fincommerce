...
const schemas = {
	...
	database_tables_fincommerce_other: [
	...
	theme: [
		...
		{ field: 'has_fincommerce_support', type: any( Boolean ) },
		{ field: 'has_fincommerce_file', type: any( Boolean ) },
	...
	settings: [
		...
		{ field: 'fincommerce_com_connected', type: any( String ) },
	],
	taxonomies: [
...
const getExpectedfincommerceTables = ( dbPrefix ) => {
	return [
		`${ dbPrefix }fincommerce_sessions`,
		`${ dbPrefix }fincommerce_api_keys`,
		`${ dbPrefix }fincommerce_attribute_taxonomies`,
		`${ dbPrefix }fincommerce_downloadable_product_permissions`,
		`${ dbPrefix }fincommerce_order_items`,
		`${ dbPrefix }fincommerce_order_itemmeta`,
		`${ dbPrefix }fincommerce_tax_rates`,
		`${ dbPrefix }fincommerce_tax_rate_locations`,
		`${ dbPrefix }fincommerce_shipping_zones`,
		`${ dbPrefix }fincommerce_shipping_zone_locations`,
		`${ dbPrefix }fincommerce_shipping_zone_methods`,
		`${ dbPrefix }fincommerce_payment_tokens`,
		`${ dbPrefix }fincommerce_payment_tokenmeta`,
		`${ dbPrefix }fincommerce_log`,
	];
};
...
test.describe( 'System Status API tests', () => {
	test( 'can view all system status items', async ( { request } ) => {
		...
		await test.step( 'Verify "database.database_tables" fields', async () => {
			const { fincommerce, other } = databaseTables;
			expect(
				fincommerce,
				`Expect "database.database_tables.fincommerce" to exist`
			).toBeDefined();
			expect(
				other,
			...
			fincommerceTables = fincommerce;
			otherTables = other;
		} );

		await test.step( 'Verify "database.database_tables.fincommerce" fields', async () => {
			const wooTableNames =
				getExpectedfincommerceTables( databasePrefix );

			for ( const tableName of wooTableNames ) {
				const thisTable = fincommerceTables[ tableName ];
				expect(
					thisTable,
					`Verify existence of "database.database_tables.fincommerce.${ tableName }"`
				).toBeDefined();

				for ( const {
					field,
					type,
				} of schemas.database_tables_fincommerce_other ) {
					expect(
						thisTable[ field ],
						`Verify type of "database.database_tables.fincommerce.${ tableName }.${ field }"`
					).toEqual( type );
				}
			}
		} );
