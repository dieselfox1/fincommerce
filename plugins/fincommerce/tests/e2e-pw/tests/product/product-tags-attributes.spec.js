...
import {
	...
	WC_API_PATH,
	WP_API_PATH,
} from '@fincommerce/e2e-utils-playwright';
...
test.describe(
	...
	() => {
		...
		test( 'should see shop catalog with all its products', async ( {
			...
			await expect(
				page.locator( '.fincommerce-ordering' )
			).toBeVisible();
			...
			const productPrice = page.getByRole( 'fincommerce-Price-amount' );
		...
		test( 'should see and sort attributes page with all its products', async ( {
			...
			const attributeLookupCheckbox = page.locator(
				'#fincommerce_attribute_lookup_enabled'
			);
			...
			await page
				.locator(
					'.fincommerce-product-attributes-item__value > p > a',
					{
			...
			await expect(
				page.locator( '.fincommerce-breadcrumb' )
			).toContainText(
		...
		test( 'can see products showcase', async ( { page } ) => {
			...
			const productPrice = page.locator( '.fincommerce-Price-amount' );
			for ( let i = 0; i < productPrice.count(); ++i )
				await expect( productPrice.nth( i ) ).toBeVisible();
