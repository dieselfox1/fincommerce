...
test.describe( 'General tab', { tag: tags.GUTENBERG }, () => {
	test.describe( 'Linked product', () => {
		...
		test( 'can create a product with linked products', async ( {
			...
			await page
				.locator(
					'.wp-block-fincommerce-product-linked-list-field__form-group-content'
				)
				.first()
				.getByRole( 'combobox' )
				.fill( productName );
			...
			const upsellsRows = page.locator(
				'div.fincommerce-product-list div[role="table"] div[role="rowgroup"] div[role="row"]'
			);
			...
			const title = page.locator( '.fincommerce-product-header__title' );
