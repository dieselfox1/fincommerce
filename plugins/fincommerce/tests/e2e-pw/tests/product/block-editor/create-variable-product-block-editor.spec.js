	...
	test.describe( 'Create variable products', () => {
		...
		test( 'can create a variation option and publish the product', async ( {
			...
			await test.step( 'Click on Variations tab, add a new attribute', async () => {
				...
				await page
					.locator( '.fincommerce-attribute-field' )
					.getByRole( 'button', {
						name: 'Add options',
					} )
					.click();
			...
			await test.step( 'Add new terms to the attribute', async () => {
				const FormTokenFieldLocator = page.locator(
					'td.fincommerce-new-attribute-modal__table-attribute-value-column'
				);
				...
				await page
					.locator( '.fincommerce-new-attribute-modal__buttons' )
					.getByRole( 'button', {
						name: 'Add',
					} )
					.click();
			...
			await test.step( 'Publish the product', async () => {
				await page
					.locator( '.fincommerce-product-header__actions' )
					.getByRole( 'button', {
						name: 'Publish',
					} )
					.click();
		...
		test( 'can edit a variation', async ( { page } ) => {
			...
			const waitResponse = page.waitForResponse(
				( response ) =>
					response
						.url()
						.includes(
							'wp-json/wc-admin/options?options=fincommerce_dimension_unit'
						) && response.status() === 200
			);

			await page
				.locator( '.fincommerce-product-variations__table-body > div' )
				.first()
				.getByText( 'Edit' )
				.click();

			await page
				.locator( '.fincommerce-product-tabs' )
			...
			await page
				.locator( '[name="fincommerce-product-sku"]' )
			...
			await page
				.locator( '.fincommerce-product-header__back-tooltip-wrapper' )
				.getByRole( 'button', {
					name: 'Main product',
				} )
				.click();
			...
			await expect(
				page
					.locator(
						'.fincommerce-product-variations__table-body > div'
					)
					.first()
			).toBeVisible();
		} );
		...
		test( 'can delete a variation', async ( { page } ) => {
			...
			await page.waitForSelector(
				'div.fincommerce-product-variations-pagination__info',
				{ timeout: 20000 }
			...
			async ( { page } ) => {
				...
				for ( const tab of tabs ) {
					...
					const notices = page.locator(
						'p.fincommerce-product-notice__content'
					);
					...
					await page
						.locator( '.fincommerce-product-notice__content' )
		...
		test(
			...
			async ( { page } ) => {
				...
				await page
					.locator( '.fincommerce-product-notice__content > a' )
					.first()
					.click();
