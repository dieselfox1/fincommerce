...
test.describe( 'Add variations', { tag: tags.GUTENBERG }, () => {
	...
	test( 'can generate variations from product attributes', async ( {
		...
		await test.step( `Expect the number of variations to be ${ expectedGeneratedVariations.length }`, async () => {
			const variations = page.locator( '.fincommerce_variation' );
	...
	test( 'can manually add a variation', async ( { page } ) => {
		...
		// hook up the fincommerce_variations_added jQuery trigger so we can check if it's fired
		await test.step( 'Hook up the fincommerce_variations_added jQuery trigger', async () => {
			await page.evaluate( () => {
				window.fincommerceVariationsAddedFunctionCalls = [];

				window
					.jQuery( '#variable_product_options' )
					.on( 'fincommerce_variations_added', ( event, data ) => {
						window.fincommerceVariationsAddedFunctionCalls.push( [
							event,
							data,
						] );
					} );
		...
		await test.step( `Manually add ${ variationsToManuallyCreate.length } variations`, async () => {
			...
			for ( const variationToCreate of variationsToManuallyCreate ) {
				await test.step( 'Click "Add manually"', async () => {
					...
					// verify that the fincommerce_variations_added jQuery trigger was fired
					const fincommerceVariationsAddedFunctionCalls =
						await page.evaluate(
							() => window.fincommerceVariationsAddedFunctionCalls
						);
					expect(
						fincommerceVariationsAddedFunctionCalls.length
					).toEqual(
