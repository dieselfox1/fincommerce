	...
	productWithImage: async ( { restApi, product }, use ) => {
		...
		await restApi
			.put( `${ WC_API_PATH }/products/${ product.id }`, {
				images: [
					{
						src: 'http://demo.woothemes.com/fincommerce/wp-content/uploads/sites/56/2013/06/T_2_front.jpg',
					},
				],
			} )
			.then( ( response ) => {
		...
		await restApi
			.put( `${ WC_API_PATH }/products/${ product.id }`, {
				images: [
					...
					{
						src: 'http://demo.woothemes.com/fincommerce/wp-content/uploads/sites/56/2013/06/T_2_back.jpg',
					},
					{
						src: 'http://demo.woothemes.com/fincommerce/wp-content/uploads/sites/56/2013/06/T_3_front.jpg',
					},
				],
			} )
			.then( ( response ) => {
...
test.describe( 'Products > Product Images', () => {
	...
	test( 'can create a product gallery', async ( {
		...
		await test.step( 'Verify product gallery', async () => {
			...
			await expect(
				page
					.locator( `.fincommerce-product-gallery ol img` )
					.nth( images.length ),
	...
	test( 'can update a product gallery', async ( {
		...
		await test.step( 'Verify product gallery', async () => {
			...
			const selector = `.fincommerce-product-gallery ol img`;
			await expect(
