...
test.describe( 'Product > Export Selected Products', () => {
	test( 'should allow exporting a single selected simple product', async ( {
		...
		await test.step( 'Navigate to export page and verify UI elements', async () => {
			...
			await expect( page.locator( '.wrap.fincommerce h1' ) ).toHaveText(
				'Export Products'
			);
			await expect(
			...
			await expect(
				page.locator( 'label[for="fincommerce-exporter-types"]' )
			).toBeHidden();
			await expect(
				page.locator( 'label[for="fincommerce-exporter-category"]' )
			).toBeHidden();
		} );
	} );
