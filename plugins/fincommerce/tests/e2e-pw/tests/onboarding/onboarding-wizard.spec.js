...
const getPluginLocator = ( page, slug ) => {
	return page.locator(
		`.fincommerce-profiler-plugins-plugin-card[data-slug="${ slug }"]`
	);
};
	...
	() => {
		...
		test( 'Can complete the core profiler skipping extension install', async ( {
			...
			test.skip(
				...
				'Test not working on a multisite setup, see https://github.com/dieselfox1/fincommerce/issues/55066'
			);
			await page.goto(
			...
			await test.step( 'Business Information', async () => {
				...
				).toHaveValue( 'FinCommerce Core E2E Test Suite' );
				await page
					.locator(
						'form.fincommerce-profiler-business-information-form > div > div > div > div > input'
					)
					.first()
					.click();
			...
			await test.step( 'Confirm that core profiler was completed and no extensions installed', async () => {
				...
				await expect(
					page.locator(
						'.fincommerce-onboarding-progress-bar__filler'
					)
				).toBeVisible();
				// dashboard shown
				await expect(
					page.getByRole( 'heading', {
						name: 'Welcome to FinCommerce Core E2E Test Suite',
					} )
				).toBeVisible();
				...
				await expect(
					page.getByText( 'MailPoet for FinCommerce', {
						exact: true,
					} )
				).toBeHidden();
				await expect(
					page.getByText( 'Pinterest for FinCommerce', {
						exact: true,
					} )
				).toBeHidden();
				await expect(
					page.getByText( 'Google for FinCommerce', { exact: true } )
				).toBeHidden();
			} );
		...
		test( 'Can complete the core profiler installing default extensions', async ( {
			...
			await test.step( 'Extensions -- install some suggested extensions', async () => {
				...
				try {
					await getPluginLocator( page, 'pinterest-for-fincommerce' )
						.getByRole( 'checkbox' )
						.check( { timeout: 2000 } );
				} catch ( e ) {
				...
				try {
					await getPluginLocator( page, 'mailchimp-for-fincommerce' )
						.getByRole( 'checkbox' )
						.uncheck( { timeout: 2000 } );
				} catch ( e ) {
			...
			await test.step( 'Confirm that core profiler was completed and a couple of default extensions installed', async () => {
				...
				try {
					await expect(
						page.locator(
							`[data-slug="pinterest-for-fincommerce"]`
						)
					).toBeVisible();
				} catch {
					console.log(
				...
				try {
					...
					console.log(
						`Google for FinCommerce is not found or not visible on the page`
					);
				}
				...
				try {
					await expect(
						page.locator(
							`[data-slug="mailchimp-for-fincommerce"]`
						)
					).toBeHidden();
				} catch {
