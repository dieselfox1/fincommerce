# FinCommerce Playwright End-to-End Tests
...
- Go through
  the [FinCommerce Monorepo prerequisites](https://github.com/dieselfox1/fincommerce/blob/trunk/README.md#prerequisites)
...
- `pnpm --filter='@fincommerce/plugin-fincommerce' build` (builds FinCommerce locally)
- `cd plugins/fincommerce` (changes into the FinCommerce plugin folder)
...
Other ways of running tests (make sure you are in the `plugins/fincommerce` folder):
...
To see all the Playwright options, make sure you are in the `plugins/fincommerce` folder and
...
The default environment configuration can be found in the `.wp-env.json` file in the `plugins/fincommerce`
folder.
...
you're at the root of the FinCommerce monorepo, and that you did not specify a custom location for the report, you would
...
- You're at the root of the FinCommerce monorepo
- You want to generate the `allure-report` folder in `plugins/fincommerce/tests/e2e-pw/test-results`

Then you would need to use the following commands:

```bash
pnpm exec allure generate --clean plugins/fincommerce/tests/e2e-pw/test-results/allure-results --output plugins/fincommerce/tests/e2e-pw/test-results/allure-report
pnpm exec allure open plugins/fincommerce/tests/e2e-pw/test-results/allure-report
```
