...
wp-env run tests-cli wp user create customer customer@fincommercecoree2etestsuite.com \
	--user_pass=password \
	--role=customer \
	--first_name='Jane' \
	--last_name='Smith' \
...
wp-env run tests-cli wp option update blogname 'FinCommerce Core E2E Test Suite'

echo -e 'Preparing Test Files \n'
wp-env run tests-cli sudo cp /var/www/html/wp-content/plugins/fincommerce/tests/legacy/unit-tests/importer/sample.csv /var/www/sample.csv

ENABLE_TRACKING="${ENABLE_TRACKING:-0}"

if [ $ENABLE_TRACKING == 1 ]; then
	echo -e 'Enable tracking\n'
	wp-env run tests-cli wp option update fincommerce_allow_tracking 'yes'
fi
