<?php
/**
 * Plugin Name: FinCommerce Cleanup
 * Description: Resets FinCommerce site to testing start state.
 * Version: 1.5.1
 * Author: Solaris Team
 * Requires at least: 6.7
 * Requires PHP: 7.4
 * @package fincommerceCleanup
 *
 * This file contains the main functionality for the FinCommerce Cleanup plugin.
 * It provides functions to reset the FinCommerce site to a clean testing state.
 */
...
/**
 * Remove all FinCommerce taxes and tax classes.
 */
function wc_cleanup_taxes() {
	global $wpdb;

	// Remove all tax rates.
	$wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}fincommerce_tax_rates" );
	$wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}fincommerce_tax_rate_locations" );

	// Remove all tax classes except the default ones.
	$wpdb->query( "DELETE FROM {$wpdb->prefix}fincommerce_tax_classes WHERE slug NOT IN ('standard', 'reduced-rate', 'zero-rate')" );

	// Reset tax options.
	update_option( 'fincommerce_tax_classes', 'Standard\nReduced rate\nZero rate' );
	update_option( 'fincommerce_tax_display_shop', 'excl' );
	update_option( 'fincommerce_tax_display_cart', 'excl' );
	update_option( 'fincommerce_tax_total_display', 'itemized' );
...
/**
 * Truncate a FinCommerce analytics table.
 *
...
function wc_cleanup_reset_site() {
	...
	// Remove all orders.
	if ( wc_get_container()->get( Automattic\FinCommerce\Internal\DataStores\Orders\CustomOrdersTableController::class )->custom_orders_table_usage_is_enabled() ) {
		// HPOS is enabled.
		global $wpdb;
	...
	// Remove all non-FinCommerce pages.
	$pages         = get_posts(
		array(
			'post_type'   => 'page',
			'numberposts' => -1,
	...
	foreach ( $pages as $page ) {
		if ( ! has_shortcode( $page->post_content, 'fincommerce' ) && ! in_array( $page->post_title, $ignore_titles, true ) ) {
	...
	// Clean up the FinCommerce analytics tables.
	...
	// Optionally, you can also clear FinCommerce transients.
	wc_delete_product_transients();

	// Set the site title.
	update_option( 'blogname', 'FinCommerce Core E2E Test Suite' );

	// Set the FinCommerce default country/state.
	update_option( 'fincommerce_default_country', 'US:CA' );

	// Set the FinCommerce selling locations to "Sell to all countries".
	update_option( 'fincommerce_allowed_countries', 'all' );

	// Set the FinCommerce shipping locations to "Ship to all countries you sell to".
	update_option( 'fincommerce_ship_to_countries', 'all' );

	// Disable taxes.
	update_option( 'fincommerce_calc_taxes', 'no' );

	// Enable coupons.
	update_option( 'fincommerce_enable_coupons', 'yes' );

	// Disable sequential coupon discounts.
	update_option( 'fincommerce_calc_discounts_sequentially', 'no' );

	// Set the FinCommerce currency to "United States (US) dollar".
	update_option( 'fincommerce_currency', 'USD' );

	// Set the FinCommerce currency position to "left".
	update_option( 'fincommerce_currency_pos', 'left' );

	// Set the FinCommerce thousand separator to a comma.
	update_option( 'fincommerce_price_thousand_sep', ',' );

	// Set the FinCommerce decimal separator to a period.
	update_option( 'fincommerce_price_decimal_sep', '.' );

	// Set the FinCommerce number of decimals to 2.
	...
	foreach ( $payment_gateways as $gateway ) {
		update_option( 'fincommerce_' . $gateway->id . '_settings', array_merge( get_option( 'fincommerce_' . $gateway->id . '_settings', array() ), array( 'enabled' => 'no' ) ) );
	}

	// Enable guest checkout.
	update_option( 'fincommerce_enable_guest_checkout', 'yes' );

	// Set the FinCommerce "From" email name.
	...
	update_option( 'fincommerce_allowed_countries', 'all' );
	update_option( 'fincommerce_ship_to_countries', '' );
	...
	update_option( 'fincommerce_store_address_2', '' );
	update_option( 'fincommerce_store_city', '' );
	update_option( 'fincommerce_store_postcode', '' );

	// Set FinCommerce measurement units to US standard (lbs, in).
	update_option( 'fincommerce_weight_unit', 'lbs' );
	update_option( 'fincommerce_dimension_unit', 'in' );
			...
			function () {
				if ( isset( $_POST['wc_cleanup_reset'] ) ) {
					...
					if ( check_admin_referer( 'wc_cleanup_reset_action', 'wc_cleanup_reset_nonce' ) ) {
						...
						echo '<div class="updated"><p>FinCommerce site has been reset.</p></div>';
					} else {
				...
				?>
				<div class="wrap">
					<h1>FinCommerce Cleanup</h1>
					<form method="post">
						<?php wp_nonce_field( 'wc_cleanup_reset_action', 'wc_cleanup_reset_nonce' ); ?>
						<p>Click the button below to reset the FinCommerce site to a clean testing state.</p>
						<p><input type="submit" name="wc_cleanup_reset" class="button button-primary" value="Reset FinCommerce Site"></p>
					</form>
				</div>
				<?php
			}
...
/**
 * Reset the FinCommerce site via API.
 *
 * @return WP_REST_Response
 */
function wc_cleanup_reset_site_via_api() {
	wc_cleanup_reset_site();
	return new WP_REST_Response( 'FinCommerce site has been reset.', 200 );
}
...
function wc_cleanup_reset_customer_email() {
	...
	if ( $customer ) {
		wp_update_user(
			array(
				'ID'         => $customer->ID,
				'user_email' => 'customer@fincommercecoree2etestsuite.com',
			)
		);
	}
}
