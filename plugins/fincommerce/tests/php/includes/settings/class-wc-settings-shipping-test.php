...
class WC_Settings_Shipping_Test extends WC_Settings_Unit_Test_Case {
	...
	public function test_get_options_settings_returns_all_settings() {
		...
		$expected = array(
			...
			'fincommerce_enable_shipping_calc'           => 'checkbox',
			'fincommerce_shipping_cost_requires_address' => 'checkbox',
			'fincommerce_shipping_hide_rates_when_free'  => 'checkbox',
			'fincommerce_ship_to_destination'            => 'radio',
			'fincommerce_shipping_debug_mode'            => 'checkbox',
		);
	 ...
	 * @param bool   $expect_update_options_action_invoked Is the 'fincommerce_update_options_' action expected to be triggered?.
	 */
	public function test_save_predefined_section( $section_name, $expect_save_settings_for_current_section_invoked, $expect_update_options_action_invoked ) {
		...
		$this->assertEquals( $expect_update_options_action_invoked ? 1 : 0, did_action( 'fincommerce_update_options_shipping_' . $section_name ) );
	}

	/**
	 * @testDox 'save' triggers 'fincommerce_update_options_shipping_{method_id}' when the current section is the id of a shipping method with settings.
	 */
	public function test_save_for_shipping_method_triggers_appropriate_action() {
		...
		$this->assertEquals( 1, did_action( 'fincommerce_update_options_shipping_method_id' ) );
	}
}
