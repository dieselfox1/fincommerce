...
class WC_Settings_Page_Test extends WC_Unit_Test_Case {
	...
	public function tearDown(): void {
		remove_filter( 'fincommerce_admin_features', array( $this, 'enable_modern_settings' ) );
		parent::tearDown();
	}
	...
	public function test_constructor() {
		remove_all_filters( 'fincommerce_settings_tabs_array' );
		remove_all_filters( 'fincommerce_sections_example' );
		remove_all_filters( 'fincommerce_settings_example' );
		remove_all_filters( 'fincommerce_settings_save_example' );

		$sut = new WC_Settings_Example();

		$this->assertTrue( has_filter( 'fincommerce_settings_tabs_array' ) );
		$this->assertTrue( has_filter( 'fincommerce_sections_example' ) );
		$this->assertTrue( has_filter( 'fincommerce_settings_example' ) );
		$this->assertTrue( has_filter( 'fincommerce_settings_save_example' ) );
	}
	...
	/**
	 * Test for get_settings (triggers fincommerce_get_settings_X filter).
	 */
	public function test_get_settings__get_settings_filter() {
		...
		add_filter(
			'fincommerce_get_settings_example',
		...
		remove_all_filters( 'fincommerce_get_settings_example' );
	...
	/**
	 * Test for get_section (triggers the fincommerce_get_sections_X filter).
	 */
	public function test_get_sections__get_sections_filter() {
		$actual_sections = null;

		add_filter(
			'fincommerce_get_sections_example',
		...
		$sut = new WC_Settings_Example();
		$sut->get_sections();
		remove_all_filters( 'fincommerce_get_sections_example' );
...
HTML;
	...
	/**
	 * Test for save (named section, triggers the fincommerce_update_options_example_foobar action).
	 */
	public function test_save__does_update_options_action__named_section() {
		global $current_section;

		$current_section = 'foobar';
		remove_all_filters( 'fincommerce_update_options_example_foobar' );

		$sut = new WC_Settings_Example();
		$sut->save();

		$this->assertEquals( 1, did_action( 'fincommerce_update_options_example_foobar' ) );
	}

	/**
	 * Test for save (default section, doesn't trigger any fincommerce_update_options_ action).
	 */
	public function test_save__does_update_options_action__default_section() {
		global $current_section;

		$current_section = '';
		remove_all_filters( 'fincommerce_update_options_example_' );

		$sut = new WC_Settings_Example();
		$sut->save();

		$this->assertEquals( 0, did_action( 'fincommerce_update_options_example_' ) );
	}
	...
	public function test_add_settings_page_data() {
		add_filter( 'fincommerce_admin_features', array( $this, 'enable_modern_settings' ) );
