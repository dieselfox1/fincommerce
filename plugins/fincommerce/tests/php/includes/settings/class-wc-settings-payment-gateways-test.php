...
class WC_Settings_Payment_Gateways_Test extends WC_Settings_Unit_Test_Case {
	 ...
	 *
	 * @testWith ["fincommerce_com", "fincommerce_get_settings_checkout"]
	 *
	...
	public function test_save_triggers_appropriate_gateway_methods_and_actions( $section_name, $expect_to_run_process_admin_options ) {
		...
		$this->assertEquals( '' === $section_name ? 0 : 1, did_action( 'fincommerce_update_options_payment_gateways_bacs' ) );
		$this->assertEquals( '' === $section_name ? 0 : 1, did_action( 'fincommerce_update_options_checkout_' . $section_name ) );
	}
}
