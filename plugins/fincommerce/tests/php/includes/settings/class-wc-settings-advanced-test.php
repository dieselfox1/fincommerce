...
class WC_Settings_Advanced_Test extends WC_Settings_Unit_Test_Case {
	...
	public function test_get_sections() {
		...
		if ( FeaturesUtil::feature_is_enabled( 'blueprint' ) ) {
			$position = array_search( 'fincommerce_com', $expected, true ) + 1;
	 ...
	 *
	 * @testWith ["", "fincommerce_settings_pages"]
	 *           ["fincommerce_com", "fincommerce_com_integration_settings"]
	 *           ["legacy_api", "fincommerce_settings_rest_api"]
	 *
	...
	public function test_get_default_settings_returns_all_settings( $site_is_https ) {
		...
		$expected = array(
			...
			'fincommerce_cart_page_id'                     => 'single_select_page_with_search',
			'fincommerce_checkout_page_id'                 => 'single_select_page_with_search',
			'fincommerce_myaccount_page_id'                => 'single_select_page_with_search',
			'fincommerce_terms_page_id'                    => 'single_select_page_with_search',
			'checkout_process_options'                     => array( 'title', 'sectionend' ),
			'fincommerce_force_ssl_checkout'               => 'checkbox',
			'fincommerce_unforce_ssl_checkout'             => 'checkbox',
			'checkout_endpoint_options'                    => array( 'title', 'sectionend' ),
			'fincommerce_checkout_pay_endpoint'            => 'text',
			'fincommerce_checkout_order_received_endpoint' => 'text',
			'fincommerce_myaccount_add_payment_method_endpoint' => 'text',
			'fincommerce_myaccount_delete_payment_method_endpoint' => 'text',
			'fincommerce_myaccount_set_default_payment_method_endpoint' => 'text',
			'account_endpoint_options'                     => array( 'title', 'sectionend' ),
			'fincommerce_myaccount_orders_endpoint'        => 'text',
			'fincommerce_myaccount_view_order_endpoint'    => 'text',
			'fincommerce_myaccount_downloads_endpoint'     => 'text',
			'fincommerce_myaccount_edit_account_endpoint'  => 'text',
			'fincommerce_myaccount_edit_address_endpoint'  => 'text',
			'fincommerce_myaccount_payment_methods_endpoint' => 'text',
			'fincommerce_myaccount_lost_password_endpoint' => 'text',
			'fincommerce_logout_endpoint'                  => 'text',
		);
	...
	/**
	 * @testdox get_settings('fincommerce_com') should return all the settings for the fincommerce_com section.
	 */
	public function test_get_fincommerce_com_settings_returns_all_settings() {
		$sut = new WC_Settings_Advanced();

		$expected = array(
			'tracking_options'                         => array( 'title', 'sectionend' ),
			'fincommerce_allow_tracking'               => 'checkbox',
			'marketplace_suggestions'                  => array( 'title', 'sectionend' ),
			'fincommerce_show_marketplace_suggestions' => 'checkbox',
		);

		$settings              = $sut->get_settings_for_section( 'fincommerce_com' );
		$setting_ids_and_types = $this->get_ids_and_types( $settings );
	...
	public function test_get_legacy_api_settings_returns_all_settings() {
		...
		$expected = array(
			...
			'fincommerce_api_enabled' => 'checkbox',
		);
	...
	/**
	 * @testdox save method should use the fincommerce_rest_api_valid_to_save filter to check if it's ok to save.
	 *
	 * @testWith ["keys", false]
	 *           ["webhooks", false]
	 *           ["foobar", true]
	...
	public function test_save_uses_filter_to_check_if_valid_to_save( $current_section_to_use, $expected_filter_supplied_value ) {
		...
		add_filter(
			'fincommerce_rest_api_valid_to_save',
		...
		remove_all_filters( 'fincommerce_rest_api_valid_to_save' );

		$this->assertEquals( $expected_filter_supplied_value, $actual_filter_supplied_value );
	}

	/**
	 * @testdox save method should save data only if the fincommerce_rest_api_valid_to_save filter returns true.
	 *
	 * @testWith [true]
	 *           [false]
	 *
	 * @param bool $is_valid_to_save Return value of the fincommerce_rest_api_valid_to_save filter.
	 */
	...
	/**
	 * @testdox save method should trigger the appropriate fincommerce_update_options action only if it's ok to save.
	 *
	 * @testWith [true]
	 *           [false]
	 *
	...
	public function test_save_does_updated_options_action_if_valid_to_save( $is_valid_to_save ) {
		...
		remove_all_filters( 'fincommerce_update_options_advanced_foobar' );
		...
		$this->assertEquals( $did_action, did_action( 'fincommerce_update_options_advanced_foobar' ) );
	}
	...
	public function test_save_prevents_the_terms_and_checkout_pages_from_being_the_same( $terms_page_id, $checkout_page_id, $expected_new_terms_page_id ) {
		$_POST['fincommerce_terms_page_id']    = $terms_page_id;
		$_POST['fincommerce_checkout_page_id'] = $checkout_page_id;
	...
	public function test_save_prevents_the_cart_checkout_and_my_account_pages_from_being_the_same( $cart_page_id, $checkout_page_id, $my_account_page_id, $expected_new_checkout_page_id, $expected_new_my_account_page_id ) {
		$_POST['fincommerce_cart_page_id']      = $cart_page_id;
		$_POST['fincommerce_checkout_page_id']  = $checkout_page_id;
		$_POST['fincommerce_myaccount_page_id'] = $my_account_page_id;
