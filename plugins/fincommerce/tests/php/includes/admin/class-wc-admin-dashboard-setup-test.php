 ...
 *
 * @package FinCommerce\Tests\Admin
 */
...
class WC_Admin_Dashboard_Setup_Test extends WC_Unit_Test_Case {
	...
	public function setUp(): void {
		// Set default country to non-US so that 'payments' task gets added but 'fincommerce-payments' doesn't,
		// by default it won't be considered completed but we can manually change that as needed.
		update_option( 'fincommerce_default_country', 'JP' );
	...
	public function tearDown(): void {
		remove_all_filters( 'fincommerce_available_payment_gateways' );
	...
	public function get_widget_output() {
		update_option( 'fincommerce_task_list_hidden', 'no' );
	...
	public function test_widget_render() {
		...
		add_filter(
			'fincommerce_available_payment_gateways',
			function () {
				return array();
			}
		);
	...
	/**
	 * Tests widget does not display when user cannot manage fincommerce.
	 */
	...
	public function test_get_button_link_redirects_to_core_profiler_when_needed() {
		...
		update_option( 'fincommerce_onboarding_profile', array( 'completed' => false ) );
		...
		delete_option( 'fincommerce_onboarding_profile' );
	}
	...
	public function test_get_button_link_returns_normal_url_when_profiler_completed() {
		...
		update_option( 'fincommerce_onboarding_profile', array( 'completed' => true ) );
	...
	public function test_get_button_link_returns_normal_url_when_profiler_skipped() {
		...
		update_option( 'fincommerce_onboarding_profile', array( 'skipped' => true ) );
