...
class WC_Install_Test extends \WC_Unit_Test_Case {
	...
	public function test_is_new_install(): void {
		...
		add_filter( 'option_fincommerce_version', $supply_version );
		add_filter( 'fincommerce_get_shop_page_id', $supply_shop_id );
		...
		// Cleanup.
		remove_filter( 'option_fincommerce_db_version', $supply_version );
		remove_filter( 'fincommerce_get_shop_page_id', $supply_shop_id );
	...
	public function test_db_auto_updates( ?bool $auto_update = null ): void {
		$options = array( 'fincommerce_db_version', 'fincommerce_version' );

		if ( ! is_null( $auto_update ) ) {
			add_filter( 'fincommerce_enable_auto_update_db', fn() => $auto_update );
		}
		...
		$update_scheduled = ! is_null( WC()->queue()->get_next( 'fincommerce_run_update_callback', null, 'fincommerce-db-updates' ) );
