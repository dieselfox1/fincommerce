...
class WC_Shipping_Test extends WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		update_option( 'fincommerce_shipping_debug_mode', 'yes' );
	}
	...
	public function tearDown(): void {
		...
		update_option( 'fincommerce_shipping_debug_mode', 'no' );
		update_option( 'fincommerce_shipping_hide_rates_when_free', 'no' );
	}

	/**
	 * @testdox shipping methods are hidden based on `fincommerce_shipping_hide_rates_when_free` option.
	 *
	 * @dataProvider provide_test_calculate_shipping_for_hide_rates_when_free
	 *
	 * @param string $option_value Option value for fincommerce_shipping_hide_rates_when_free.
	...
	public function test_calculate_shipping_for_hide_rates_when_free( string $option_value, array $shipping_methods, array $expected_rates ) {
		update_option( 'fincommerce_shipping_hide_rates_when_free', $option_value );

		$shipping_methods_hook = fn () => $shipping_methods;

		add_action( 'fincommerce_shipping_methods', $shipping_methods_hook );
		...
		remove_action( 'fincommerce_shipping_methods', $shipping_methods_hook );
	}
	...
	public function test_package_rates_filter_error_handling( callable $filter_callback, string $description ) {
		...
		add_action( 'fincommerce_shipping_methods', $shipping_methods_hook );
		add_filter( 'fincommerce_package_rates', $filter_callback, 10, 2 );
		...
		remove_filter( 'fincommerce_package_rates', $filter_callback, 10 );
		remove_action( 'fincommerce_shipping_methods', $shipping_methods_hook );
	}
