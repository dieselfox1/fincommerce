...
class WC_Core_Functions_Test extends \WC_Unit_Test_Case {
	...
	public function test_wc_help_tip_strips_html() {
		$expected = '<span class="fincommerce-help-tip" tabindex="0" aria-label="Strong text regular text" data-tip="&lt;strong&gt;Strong text&lt;/strong&gt; regular text"></span>';
	...
	public function test_wc_get_customer_default_location() {
		 ...
		 *
		 * fincommerce_default_country is set to 'US:CA' by default unless it was defined during setup.
		 */
		delete_option( 'fincommerce_default_customer_address' );
		delete_option( 'fincommerce_default_country' );
		delete_option( 'fincommerce_allowed_countries' );
		delete_option( 'fincommerce_specific_allowed_countries' );
		...
		update_option( 'fincommerce_default_country', 'DE:LS' );
		$result = wc_get_customer_default_location();
		...
		update_option( 'fincommerce_specific_allowed_countries', array( 'DE', 'AT', 'CH' ) );
		$result = wc_get_customer_default_location();
		$this->assertEquals( 'DE', $result['country'] );
		$this->assertEquals( 'LS', $result['state'] );
		...
		update_option( 'fincommerce_default_country', 'DE:LS' );
		update_option( 'fincommerce_allowed_countries', 'specific' );
		update_option( 'fincommerce_specific_allowed_countries', array( 'GB' ) );
		$result = wc_get_customer_default_location();
		$this->assertEquals( '', $result['country'] );
		$this->assertEquals( '', $result['state'] );

		// Test with no default address.
		update_option( 'fincommerce_default_customer_address', '' );
		update_option( 'fincommerce_default_country', 'GB' );
		$result = wc_get_customer_default_location();
		$this->assertEquals( '', $result['country'] );
		$this->assertEquals( '', $result['state'] );
		...
		update_option( 'fincommerce_default_customer_address', 'geolocation' );
		update_option( 'fincommerce_default_country', 'GB' );
		delete_option( 'fincommerce_allowed_countries' );
		delete_option( 'fincommerce_specific_allowed_countries' );
		add_filter(
			'fincommerce_geolocate_ip',
			function () {
				return 'FR';
			},
			10
		...
		$this->assertEquals( '', $result['state'] );
		remove_all_filters( 'fincommerce_geolocate_ip' );
