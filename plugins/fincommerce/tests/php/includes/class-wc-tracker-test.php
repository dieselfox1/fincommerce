 ...
 *
 * @package FinCommerce\Tests\WC_Tracker.
 */
...
class WC_Tracker_Test extends \WC_Unit_Test_Case {
	...
	public function test_wc_admin_disabled_get_tracking_data() {
		...
		// Test the case for fincommerce_admin_disabled filter returning true.
		add_filter(
			'fincommerce_admin_disabled',
			function( $default ) {
				return true;
			}
		);
		...
		// Test the default case of no filter for set for fincommerce_admin_disabled.
	...
	public function test_wc_admin_not_disabled_get_tracking_data() {
		...
		update_option( 'fincommerce_tracker_last_send', strtotime( '-2 weeks' ) );
	...
	public function test_get_tracking_data_plugin_feature_compatibility() {
		...
		$pluginutil_mock = new class() extends PluginUtil {
			...
			public function is_fincommerce_aware_plugin( $plugin ): bool {
				if ( 'plugin1' === $plugin ) {
					return false;
				}
	...
	public function test_get_tracking_data_order_snapshot() {
		...
		foreach ( $order_snapshot['first_20_orders'] as $order_details ) {
			...
			$this->assertEquals( $order_details['fincommerce_version'], fincommerce_VERSION );
		}
	...
	/**
	 * @testDox Test fincommerce_install_admin_timestamp is included in tracking data.
	 */
	public function test_get_tracking_data_admin_install_timestamp() {
		$time = time();
		update_option( 'fincommerce_admin_install_timestamp', $time );
		$tracking_data = WC_Tracker::get_tracking_data();
		...
		delete_option( 'fincommerce_admin_install_timestamp' );
	}
	...
	/**
	 * @testDox Test fincommerce_allow_tracking related data is included in tracking snapshot.
	 */
	public function test_tracking_data_fincommerce_allow_tracking() {
		$current_fincommerce_allow_tracking = get_option( 'fincommerce_allow_tracking', 'no' );

		// Clear everything.
		update_option( 'fincommerce_allow_tracking', 'no' );
		delete_option( 'fincommerce_allow_tracking_last_modified' );
		delete_option( 'fincommerce_allow_tracking_first_optin' );

		$tracking_data = WC_Tracker::get_tracking_data();
		$this->assertArrayHasKey( 'fincommerce_allow_tracking', $tracking_data );
		$this->assertArrayHasKey( 'fincommerce_allow_tracking_last_modified', $tracking_data );
		$this->assertArrayHasKey( 'fincommerce_allow_tracking_first_optin', $tracking_data );

		$this->assertEquals( $tracking_data['fincommerce_allow_tracking'], 'no' );
		$this->assertEquals( $tracking_data['fincommerce_allow_tracking_last_modified'], 'unknown' );
		$this->assertEquals( $tracking_data['fincommerce_allow_tracking_first_optin'], 'unknown' );

		$time_one = time();
		update_option( 'fincommerce_allow_tracking', 'yes' );
		$tracking_data = WC_Tracker::get_tracking_data();
		$this->assertEquals( $tracking_data['fincommerce_allow_tracking'], 'yes' );
		$this->assertTrue( $tracking_data['fincommerce_allow_tracking_last_modified'] >= $time_one );
		$this->assertTrue( $tracking_data['fincommerce_allow_tracking_first_optin'] >= $time_one );
		...
		$this->assertEquals( $tracking_data['fincommerce_allow_tracking'], 'no' );
		$this->assertTrue( $tracking_data['fincommerce_allow_tracking_last_modified'] >= $time_two );
		$this->assertTrue( $tracking_data['fincommerce_allow_tracking_first_optin'] >= $time_one && $tracking_data['fincommerce_allow_tracking_first_optin'] < $time_two );
		...
		$tracking_data = WC_Tracker::get_tracking_data();
		$this->assertEquals( $tracking_data['fincommerce_allow_tracking'], 'yes' );
		$this->assertTrue( $tracking_data['fincommerce_allow_tracking_last_modified'] >= $time_three );
		$this->assertTrue( $tracking_data['fincommerce_allow_tracking_first_optin'] >= $time_one && $tracking_data['fincommerce_allow_tracking_first_optin'] < $time_two );

		// Restore everything as it was.
		update_option( 'fincommerce_allow_tracking', $current_fincommerce_allow_tracking );
		delete_option( 'fincommerce_allow_tracking_last_modified' );
		delete_option( 'fincommerce_allow_tracking_first_optin' );
	}
}
