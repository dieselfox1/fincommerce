 ...
 *
 * @package FinCommerce\Tests\Abstracts
 */
...
class WC_Abstract_Order_Test extends WC_Unit_Test_Case {
	...
	public function test_apply_coupon_for_correct_location_taxes() {
		update_option( 'fincommerce_tax_round_at_subtotal', 'yes' );
		update_option( 'fincommerce_prices_include_tax', 'yes' );
		update_option( 'fincommerce_tax_based_on', 'billing' );
		update_option( 'fincommerce_calc_taxes', 'yes' );
		...
		update_option( 'fincommerce_default_country', 'IN:AP' );
	 ...
	 * See: https://github.com/dieselfox1/fincommerce/issues/28166.
	 */
	 ...
	 *
	 * Issue :https://github.com/dieselfox1/fincommerce/issues/36794
	 */
	...
	public function test_cache_does_not_interferes_with_order_object() {
		...
		$this->assertInstanceOf( Automattic\FinCommerce\Admin\Overrides\Order::class, $order );
	}
	...
	/**
	 * @testdox The calculated value for Cost of Goods Sold can be modified using the 'fincommerce_calculated_order_cogs_value' filter.
	 */
	public function test_filter_can_be_used_to_alter_calculated_cogs_value() {
		...
		add_filter(
			'fincommerce_calculated_order_cogs_value',
	...
	public function test_status_transition_hooks_shouldnot_be_called_when_order_save_canceled() {
		...
		add_action( 'fincommerce_before_order_object_save', $callback );
		...
		add_action( 'fincommerce_order_status_' . $refunded_status, $should_not_be_called_callback );
		...
		$this->assertFalse(
			$triggered,
			'"fincommerce_order_status_' . $refunded_status . '" action hook has been triggered but shouldn\'t have been'
		);

		remove_action( 'fincommerce_before_order_object_save', $callback );
		remove_action( 'fincommerce_order_status_' . $refunded_status, $should_not_be_called_callback );
	}
	...
	public function test_status_transition_hooks_shouldnot_be_called_when_order_save_errored_on_item_save() {
		...
		add_action( 'fincommerce_before_order_item_object_save', $callback, 10, 0 );
		...
		remove_action( 'fincommerce_before_order_item_object_save', $callback );
		remove_action( 'fincommerce_order_status_' . $refunded_status, $should_not_be_called_callback );
	}
}
