...
/**
 * Unit tests for FinCommerce recurring actions.
 *
...
class WC_Recurring_Actions_Test extends WC_Unit_Test_Case {
	...
	public function test_recurring_actions_are_enqueued() {
		$this->assertTrue(
			...
			'Action Scheduler must support ensure_recurring_actions_hook for FinCommerce recurring actions to work properly'
		);
		...
		$this->assertTrue(
			as_has_scheduled_action( 'fincommerce_tracker_send_event_wrapper' ),
		...
		$this->assertTrue(
			as_has_scheduled_action( 'fincommerce_cleanup_rate_limits_wrapper' ),
			'Rate limits cleanup wrapper should be scheduled'
		);

		$this->assertTrue(
			as_has_scheduled_action( 'fincommerce_scheduled_sales' ),
			'Scheduled sales should be scheduled'
		);

		$this->assertTrue(
			as_has_scheduled_action( 'fincommerce_cleanup_personal_data' ),
			'Personal data cleanup should be scheduled'
		);

		$this->assertTrue(
			as_has_scheduled_action( 'fincommerce_cleanup_logs' ),
			'Log cleanup should be scheduled'
		);

		$this->assertTrue(
			as_has_scheduled_action( 'fincommerce_cleanup_sessions' ),
			'Session cleanup should be scheduled'
		);

		$this->assertTrue(
			as_has_scheduled_action( 'fincommerce_geoip_updater' ),
	...
	private function clear_scheduled_actions() {
		$actions = array(
			'fincommerce_tracker_send_event_wrapper',
			'wc_admin_daily_wrapper',
			'generate_category_lookup_table_wrapper',
			'fincommerce_cleanup_rate_limits_wrapper',
			'fincommerce_scheduled_sales',
			'fincommerce_cleanup_personal_data',
			'fincommerce_cleanup_logs',
			'fincommerce_cleanup_sessions',
			'fincommerce_geoip_updater',
		);
