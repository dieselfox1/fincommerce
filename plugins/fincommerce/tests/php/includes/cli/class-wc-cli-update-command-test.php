...
class WC_CLI_Update_Command_Test extends WC_Unit_Test_Case {
	...
	/**
	 * @testdox After `wp wc update` has run, the `fincommerce_db_option` should be left at the expected value.
	 */
	public function test_db_version_is_updated_if_have_callbacks() {
		...
		update_option( 'fincommerce_db_version', '4.0.0' );
		$sut = new WC_CLI_Update_Command();
		$sut->update();

		$this->assertTrue(
			version_compare( get_option( 'fincommerce_db_version' ), WC()->version, '>=' ),
			'After applying updates via WP CLI, the `fincommerce_db_version` option should match the last db update version in code.'
		);
	}

	/**
	 * @testdox After `wp wc update` has run, the `fincommerce_db_option` should be left at the expected value (even if no update callbacks were executed)
	 */
