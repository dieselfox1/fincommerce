...
class WC_Emails_Tests extends \WC_Unit_Test_Case {
	...
	public function test_email_header_is_compatible_with_legacy_do_action() {
		...
		$this->assertEquals( 10, has_action( 'fincommerce_email_header', array( $email_object, 'email_header' ) ) );
		ob_start();
		// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		do_action( 'fincommerce_email_header', 'header' );
	...
	public function test_email_footer_is_compatible_with_legacy_do_action() {
		$email_object = new WC_Emails();
		// 10 is expected priority of the hook.
		$this->assertEquals( 10, has_action( 'fincommerce_email_footer', array( $email_object, 'email_footer' ) ) );
		ob_start();
		// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		do_action( 'fincommerce_email_footer' );
		$content = ob_get_contents();
		ob_end_clean();
		$this->assertFalse( empty( $content ) );
	}
	...
	public function test_order_meta() {
		add_filter(
			'fincommerce_email_order_meta_keys',
			function () {
	...
	public function test_fulfillment_meta() {
		...
		add_filter(
			...
			function ( $translations ) {
				$translations['test_meta_key'] = __( 'Test meta key', 'fincommerce' );
				return $translations;
			}
		);
