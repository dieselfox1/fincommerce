 ...
 *
 * @package FinCommerce\Tests\WC_AJAX.
 */
...
class WC_AJAX_Test extends \WP_Ajax_UnitTestCase {
	...
	public function test_create_api_key_long_description_failure() {
		...
		try {
			$this->_handleAjax( 'fincommerce_update_api_key' );
	 ...
	 *
	 * @throws Automattic\FinCommerce\Internal\DependencyManagement\ContainerException If the LegacyProxy cannot be retrieved.
	 */
	public function test_json_search_customers(): void {
		...
		$response = $this->do_ajax( 'fincommerce_json_search_customers' );
		$this->assertEquals(
			$customer_id,
			key( $response ),
	...
	public function test_get_customer_details(): void {
		...
		$response = $this->do_ajax( 'fincommerce_get_customer_details' );
		$this->assertIsArray(
			$response,
