...
class WC_Email_Customer_POS_Completed_Order_Test extends \WC_Unit_Test_Case {
	...
	public function test_pos_email_includes_additional_rows_in_order_totals_while_regular_email_does_not() {
		...
		// Then POS email should include additional rows.
		$this->assertStringContainsString( __( 'Change due:', 'fincommerce' ), $pos_plain_text );
		$this->assertStringContainsString( __( 'Auth code:', 'fincommerce' ), $pos_plain_text );
		$this->assertStringContainsString( __( 'Time of payment:', 'fincommerce' ), $pos_plain_text );

		// And regular email should not include these rows.
		$this->assertStringNotContainsString( __( 'Change due:', 'fincommerce' ), $regular_plain_text );
		$this->assertStringNotContainsString( __( 'Auth code:', 'fincommerce' ), $regular_plain_text );
		$this->assertStringNotContainsString( __( 'Time of payment:', 'fincommerce' ), $regular_plain_text );
	}
	...
	public function test_pos_email_includes_pos_store_name_in_email_header_html_while_regular_email_includes_blog_name() {
		...
		update_option( 'fincommerce_pos_store_name', 'Physical Store' );
	...
	public function test_pos_email_replaces_placeholders_with_pos_store_details_in_email_footer_while_regular_email_does_not() {
		...
		update_option( 'fincommerce_pos_store_name', 'Physical Store' );
		update_option( 'fincommerce_pos_store_email', 'pos@example.com' );
		update_option( 'fincommerce_pos_store_address', '134 Main St, Anytown, USA' );
		// Placeholders that are set in the regular email.
		update_option( 'fincommerce_store_address', '606 2nd St, Anytown, USA' );
		update_option( 'fincommerce_email_from_address', 'online@example.com' );
		update_option( 'blogname', 'Online Store' );

		// Footer text that includes placeholders that can be replaced with POS store details.
		update_option( 'fincommerce_email_footer_text', 'footer_title: {site_title}; footer_address: {store_address}; footer_email: {store_email}' );
	...
	public function test_pos_email_header_html_includes_blog_name_when_pos_store_name_is_not_set() {
		...
		delete_option( 'fincommerce_pos_store_name' );
		update_option( 'blogname', 'Online Store' );
	...
	public function test_get_default_subject_includes_pos_store_name_when_option_is_set() {
		...
		update_option( 'fincommerce_pos_store_name', $store_name );
	...
	public function test_email_content_includes_store_details_when_options_are_set() {
		...
		update_option( 'fincommerce_pos_store_name', 'POS Store' );
		update_option( 'fincommerce_pos_store_email', 'pos@example.com' );
		update_option( 'fincommerce_pos_store_phone', '1234567890' );
		update_option( 'fincommerce_pos_store_address', '134 Main St, Anytown, USA' );
	...
	public function test_email_content_does_not_include_store_details_when_options_are_not_set() {
		...
		delete_option( 'fincommerce_pos_store_name' );
		delete_option( 'fincommerce_pos_store_email' );
		delete_option( 'fincommerce_pos_store_phone' );
		delete_option( 'fincommerce_pos_store_address' );
	...
	public function test_email_content_includes_refund_returns_policy_when_option_is_set() {
		// Given option is set.
		update_option( 'fincommerce_pos_refund_returns_policy', 'Accepted within 30 days of purchase.' );
		...
		// Then POS email should include store details.
		$this->assertStringContainsString( esc_html__( 'Refund & Returns Policy', 'fincommerce' ), $html_content );
		$this->assertStringContainsString( 'Accepted within 30 days of purchase.', $html_content );

		// And plain text email should include store details.
		$this->assertStringContainsString( esc_html__( 'Refund & Returns Policy', 'fincommerce' ), $plain_text_content );
	...
	public function test_email_content_does_not_include_refund_returns_policy_when_option_is_not_set() {
		...
		delete_option( 'fincommerce_pos_refund_returns_policy' );
		...
		$this->assertStringNotContainsString( esc_html__( 'Refund & Returns Policy', 'fincommerce' ), $html_content );

		// And plain text email should not include refund & returns policy.
		$this->assertStringNotContainsString( esc_html__( 'Refund & Returns Policy', 'fincommerce' ), $plain_text_content );
	}
}
