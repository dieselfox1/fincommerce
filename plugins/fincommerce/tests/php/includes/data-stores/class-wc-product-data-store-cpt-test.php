...
class WC_Product_Data_Store_CPT_Test extends WC_Unit_Test_Case {
	...
	public static function setUpBeforeClass(): void {
		...
		self::$default_uri = isset( $_SERVER['REQUEST_URI'] ) ? $_SERVER['REQUEST_URI'] : ''; // phpcs:ignore finpress.Security.ValidatedSanitizedInput.MissingUnslash, finpress.Security.ValidatedSanitizedInput.InputNotSanitized
	}
	...
	public function tearDown(): void {
		...
		remove_all_filters( 'fincommerce_load_cogs_value' );
		remove_all_filters( 'fincommerce_save_cogs_value' );
	}
	...
	/**
	 * @testdox Loaded Cost of Goods Sold information can be modified using the fincommerce_load_cogs_value filter.
	 */
	public function test_cogs_loaded_value_can_be_altered_via_filter() {
		...
		add_filter( 'fincommerce_load_product_cogs_value', fn( $value, $product ) => $value + $product->get_id(), 10, 2 );
	...
	/**
	 * @testdox Saved Cost of Goods Sold information can be modified using the fincommerce_save_cogs_value filter.
	 */
	public function test_cogs_saved_value_can_be_altered_via_filter() {
		$this->enable_cogs_feature();

		add_filter( 'fincommerce_save_product_cogs_value', fn( $value, $product ) => $value + $product->get_id(), 10, 2 );
	...
	/**
	 * @testdox Saving of the Cost of Goods Sold information can be suppressed using the fincommerce_save_cogs_value filter with a return value of false.
	 */
	public function test_cogs_saved_value_saving_can_be_suppressed_via_filter() {
		...
		add_filter( 'fincommerce_save_product_cogs_value', fn( $value, $product ) => false, 10, 2 );
