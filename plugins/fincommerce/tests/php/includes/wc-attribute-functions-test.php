...
class WC_Attribute_Functions_Test extends \WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		add_filter( 'fincommerce_attribute_taxonomies', $filter_mock );
	...
	public function tearDown(): void {
		remove_all_filters( 'fincommerce_attribute_taxonomies' );
	...
	public function test_wc_get_attribute_taxonomy_ids() {
		...
		$this->assertEquals(
			...
			'Filter `fincommerce_attribute_taxonomies` should have been triggered once after fetching all attribute taxonomies.'
		);
		$ids = wc_get_attribute_taxonomy_ids();
		$this->assertEquals( [], $ids );
		$this->assertEquals(
			1,
			$this->filter_recorder->getInvocationCount(),
			'Filter `fincommerce_attribute_taxonomies` should not be triggered a second time because the results should be loaded from the cache.'
		);
	}
