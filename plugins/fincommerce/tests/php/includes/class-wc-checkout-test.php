 ...
 *
 * @package FinCommerce\Tests\Checkout.
 */
...
class WC_Checkout_Test extends \WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		add_filter( 'fincommerce_checkout_registration_enabled', '__return_true' );
	}
	...
	public function tearDown(): void {
		remove_filter( 'fincommerce_checkout_registration_enabled', '__return_true' );
	}
	...
	public function test_validate_posted_data_adds_error_for_non_existing_country( $ship_to_different_address, $expect_error_message_for_shipping_country ) {
		...
		add_filter(
			'fincommerce_cart_needs_shipping_address',
			function () {
				return true;
			}
		);
	...
	public function test_validate_checkout_adds_we_dont_ship_error_only_if_country_exists( $country, $expect_we_dont_ship_error ) {
		add_filter(
			'fincommerce_countries_allowed_countries',
			function () {
				return array( 'ES' );
			}
		);
		...
		add_filter(
			'fincommerce_cart_needs_shipping',
			function () {
				return true;
			}
		);
		...
		remove_all_filters( 'fincommerce_countries_allowed_countries' );
	}

	/**
	 * @testdox If the FinCommerce class's customer object is null (like if WC has not been fully initialized yet),
	...
	public function test_checkout_page_contains_login_form_for_guests() {
		...
		ob_start();
		echo do_shortcode( '[fincommerce_checkout]' );
		$output = ob_get_clean();

		// Assert that the login form is present.
		$this->assertStringContainsString( 'fincommerce-form-login', $output );
	}
}
