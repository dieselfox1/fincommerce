...
use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Register as Approved_Directories;
...
class WC_Download_Handler_Tests extends \WC_Unit_Test_Case {
	...
	public function test_inactive_downloads_will_not_be_served() {
		...
		add_action( 'fincommerce_download_file_force', $download_counter );
	...
	public function test_downloads_remaining_count(): void {
		...
		list( $product, $order ) = $this->build_downloadable_product_and_order_one(
			array(
				array(
					'name' => 'Supersheet 123',
					'file' => content_url( 'uploads/fincommerce_uploads/supersheet-123.ods' ),
				),
			)
		);
	...
	private static function remove_download_handlers() {
		remove_action( 'fincommerce_download_file_xsendfile', array( WC_Download_Handler::class, 'download_file_xsendfile' ) );
		remove_action( 'fincommerce_download_file_redirect', array( WC_Download_Handler::class, 'download_file_redirect' ) );
		remove_action( 'fincommerce_download_file_force', array( WC_Download_Handler::class, 'download_file_force' ) );
	}
	...
	private static function restore_download_handlers() {
		add_action( 'fincommerce_download_file_redirect', array( WC_Download_Handler::class, 'download_file_redirect' ), 10, 2 );
		add_action( 'fincommerce_download_file_xsendfile', array( WC_Download_Handler::class, 'download_file_xsendfile' ), 10, 2 );
		add_action( 'fincommerce_download_file_force', array( WC_Download_Handler::class, 'download_file_force' ), 10, 2 );
	}
}
