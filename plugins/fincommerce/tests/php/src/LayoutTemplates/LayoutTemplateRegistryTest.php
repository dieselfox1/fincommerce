...
namespace Automattic\FinCommerce\Tests\LayoutTemplates;
...
class LayoutTemplateRegistryTest extends WC_Unit_Test_Case {
	...
	public function test_instantiation_actions() {
		...
		try {
			add_action( 'fincommerce_layout_template_after_instantiation', $after_instantiation_hook, 10, 3 );

			add_action( 'fincommerce_block_template_register', $deprecated_register_hook );
			...
			$this->assertTrue(
				...
				'fincommerce_layout_template_after_instantiation hook was not called.'
			);

			$this->assertTrue(
				$deprecated_register_hook_called,
				'fincommerce_block_template_register hook was not called.'
			);
		} finally {
			remove_action( 'fincommerce_layout_template_after_instantiation', $after_instantiation_hook );

			remove_action( 'fincommerce_block_template_register', $deprecated_register_hook );
		}
	}
}
