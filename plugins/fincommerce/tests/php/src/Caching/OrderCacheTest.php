...
class OrderCacheTest extends \WC_Unit_Test_Case {
	...
	public function test_meta_is_not_duplicated_when_cached() {
		...
		$query             = $wpdb->prepare( "SELECT id FROM $orders_meta_table WHERE order_id = %d AND meta_key = %s", $order->get_id(), 'test' ); // phpcs:ignore finpress.DB.PreparedSQL.InterpolatedNotPrepared
		$this->assertEquals( 1, count( $wpdb->get_col( $query ) ) ); // phpcs:ignore finpress.DB.PreparedSQL.NotPrepared -- Already prepared query.
	}
