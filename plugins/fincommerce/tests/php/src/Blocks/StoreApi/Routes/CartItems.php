...
use Automattic\FinCommerce\Tests\Blocks\StoreApi\Routes\ControllerTestCase;
use Automattic\FinCommerce\Tests\Blocks\Helpers\FixtureData;
use Automattic\FinCommerce\Tests\Blocks\Helpers\ValidateSchema;
use WC_Logger;
...
class CartItems extends ControllerTestCase {
	...
	public function test_prepare_item() {
		$routes     = new \Automattic\FinCommerce\StoreApi\RoutesController( new \Automattic\FinCommerce\StoreApi\SchemaController( $this->mock_extend ) );
	...
	public function test_cart_item_image_filtering_logging() {
		...
		$this->mock_logger
			->expects( $this->at( 0 ) )
			->method( 'warning' )
			->with( sprintf( 'After passing through fincommerce_cart_item_images filter, image with id %s did not have a valid src property.', $this->products[0]->get_image_id() ) );
		...
		$this->mock_logger
			->expects( $this->at( 0 ) )
			->method( 'warning' )
			->with( sprintf( 'After passing through fincommerce_cart_item_images filter, image with id %s did not have a valid thumbnail property.', $this->products[0]->get_image_id() ) );
