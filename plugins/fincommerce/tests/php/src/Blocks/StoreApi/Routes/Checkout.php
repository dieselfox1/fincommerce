...
namespace Automattic\FinCommerce\Tests\Blocks\StoreApi\Routes;
...
use Automattic\FinCommerce\StoreApi\Schemas\V1\CheckoutSchema;
use Automattic\FinCommerce\Tests\Blocks\Helpers\FixtureData;
use Automattic\FinCommerce\StoreApi\Routes\V1\Checkout as CheckoutRoute;
...
class Checkout extends MockeryTestCase {
	...
	protected function setUp(): void {
		...
		add_filter( 'fincommerce_set_cookie_enabled', array( $this, 'filter_fincommerce_set_cookie_enabled' ), 10, 4 );

		update_option( 'fincommerce_enable_guest_checkout', 'yes' );
		update_option( 'fincommerce_enable_signup_and_login_from_checkout', 'yes' );
	...
	protected function tearDown(): void {
		...
		remove_filter( 'fincommerce_set_cookie_enabled', array( $this, 'filter_fincommerce_set_cookie_enabled' ) );

		remove_all_filters( 'fincommerce_get_country_locale' );
		remove_all_filters( 'fincommerce_register_shop_order_post_statuses' );
		remove_all_filters( 'wc_order_statuses' );
		remove_all_actions( 'fincommerce_checkout_validate_order_before_payment' );
		remove_all_actions( 'fincommerce_store_api_checkout_order_processed' );
		remove_all_actions( 'fincommerce_valid_order_statuses_for_payment' );
	 ...
	 * @return false
	 */
	public function filter_fincommerce_set_cookie_enabled( $enabled, $name, $value, $expire ) {
		if ( $expire < time() ) {
			unset( $_COOKIE[ $name ] );
		} else {
			$_COOKIE[ $name ] = $value;
	...
	public function test_locale_required_filtering_post_data() {
		...
		// Test that a country that usually requires state can be overridden with fincommerce_get_country_locale filter.
		$request->set_body_params(
			array(
				'billing_address'  => (object) array(
					'first_name' => 'test',
	...
	public function test_post_extension_data() {
		...
		add_action( 'fincommerce_store_api_checkout_update_order_from_request', array( $action_callback, 'do_callback' ), 10, 2 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertEquals( 200, $response->get_status() );
		remove_action( 'fincommerce_store_api_checkout_update_order_from_request', array( $action_callback, 'do_callback' ), 10, 2 );
	}
	...
	public function test_checkout_force_create_account() {
		...
		update_option( 'fincommerce_enable_guest_checkout', 'no' );
		update_option( 'fincommerce_enable_signup_and_login_from_checkout', 'yes' );
	...
	public function test_checkout_invalid_shipping_method() {
		...
		foreach ( $shipping_methods as $shipping_method ) {
			$wpdb->update( "{$wpdb->prefix}fincommerce_shipping_zone_methods", array( 'is_enabled' => '0' ), array( 'instance_id' => absint( $shipping_method->instance_id ) ) );
		}
		$fixtures = new FixtureData();
		$fixtures->shipping_remove_pickup_location();
		...
		$data     = $response->get_data();
		$this->assertEquals( 400, $status, print_r( $data, true ) );
		$this->assertEquals( 'fincommerce_rest_invalid_shipping_option', $data['code'], print_r( $data, true ) );
	...
	public function test_checkout_deny_guest_checkout() {
		...
		update_option( 'fincommerce_enable_guest_checkout', 'no' );
		update_option( 'fincommerce_enable_signup_and_login_from_checkout', 'no' );
		...
		$this->assertEquals( 'fincommerce_rest_guest_checkout_disabled', $data['code'], print_r( $data, true ) );
	...
	public function test_put_order_update() {
		$fields = array(
			...
			array(
				...
				'label'    => __( 'Please leave my package on the porch if I\'m not home', 'fincommerce' ),
				'location' => 'order',
				'type'     => 'checkbox',
			),
		);
		array_map( 'fincommerce_register_additional_checkout_field', $fields );
	...
	public function test_perform_custom_order_validation() {
		$order_controller = new \Automattic\FinCommerce\StoreApi\Utilities\OrderController();
		$order            = new \WC_Order();

		// Set up a test action to add a custom validation error.
		add_action(
			'fincommerce_checkout_validate_order_before_payment',
		...
		$this->expectException( \Automattic\FinCommerce\StoreApi\Exceptions\RouteException::class );
	...
	public function test_local_pickup_country_validation() {
		...
		update_option( 'fincommerce_ship_to_countries', 'specific' );
		update_option( 'fincommerce_specific_ship_to_countries', array( 'GB' ) );
	...
	public function test_local_pickup_invalid_billing_country() {
		...
		update_option( 'fincommerce_allowed_countries', 'specific' );
		update_option( 'fincommerce_specific_allowed_countries', array( 'US' ) );
		...
		$response = rest_get_server()->dispatch( $request );
		$this->assertEquals( 400, $response->get_status() );
		$this->assertEquals( 'fincommerce_rest_invalid_address_country', $response->get_data()['code'] );
	...
	private function register_custom_order_status( $status_name, $add_to_payment_statuses = false ) {
		...
		if ( $add_to_payment_statuses ) {
			add_filter(
				'fincommerce_valid_order_statuses_for_payment',
