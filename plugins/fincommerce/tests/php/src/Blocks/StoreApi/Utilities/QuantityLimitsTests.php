...
namespace Automattic\FinCommerce\Tests\Blocks\StoreApi\Utilities;
...
class QuantityLimitsTests extends TestCase {
	...
	public function setUp(): void {
		$this->manage_stock = get_option( 'fincommerce_manage_stock' );
		parent::setUp();
	}
	...
	public function tearDown(): void {
		update_option( 'fincommerce_manage_stock', $this->manage_stock );
		// Clean up custom filters.
		remove_all_filters( 'fincommerce_store_api_product_quantity_multiple_of' );
		remove_all_filters( 'fincommerce_store_api_product_quantity_maximum' );
		remove_all_filters( 'fincommerce_store_api_product_quantity_minimum' );
		remove_all_filters( 'fincommerce_quantity_input_args' );
		parent::tearDown();
	}
	...
	private function enable_float_support() {
		...
		remove_all_filters( 'fincommerce_stock_amount' );
		// Add only floatval.
		add_filter( 'fincommerce_stock_amount', 'floatval' );
	}
	...
	private function disable_float_support() {
		...
		remove_all_filters( 'fincommerce_stock_amount' );
		// Add only intval.
		add_filter( 'fincommerce_stock_amount', 'intval' );
	}
	...
	public function test_quantity_limits_with_classic_filters() {
		...
		add_filter(
			'fincommerce_quantity_input_args',
	...
	public function test_quantity_limit_when_stock_management_disabled_but_product_has_manage_stock_enabled() {
		 ...
		 * @return mixed
		 */
		$expected_max = apply_filters( 'fincommerce_store_api_product_quantity_maximum', 9999, $product );
	...
	public function test_quantity_limits_with_float_support() {
		...
		add_filter(
			'fincommerce_store_api_product_quantity_multiple_of',
			function () {
				return 0.5;
			},
			10
