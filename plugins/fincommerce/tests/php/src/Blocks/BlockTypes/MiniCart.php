...
use Automattic\FinCommerce\Blocks\BlockTypes\MiniCart as MiniCartBlock;
...
use Automattic\FinCommerce\Tests\Blocks\Mocks\MiniCartMock;
...
class MiniCart extends \WP_UnitTestCase {
			...
			<!-- wp:fincommerce/mini-cart-contents -->
				<!-- wp:fincommerce/filled-mini-cart-contents-block -->
					<!-- wp:fincommerce/mini-cart-title-block -->
						<!-- wp:fincommerce/mini-cart-title-label-block -->
					<!-- /wp:fincommerce/mini-cart-title-label-block -->
					<!-- wp:fincommerce/mini-cart-title-items-counter-block -->
					<!-- /wp:fincommerce/mini-cart-title-items-counter-block -->
				<!-- /wp:fincommerce/mini-cart-title-block -->
				...
				<!-- wp:fincommerce/mini-cart-footer-block -->
					<!-- wp:fincommerce/mini-cart-cart-button-block -->
					<!-- /wp:fincommerce/mini-cart-cart-button-block -->
					<!-- wp:fincommerce/mini-cart-checkout-button-block -->
					<!-- /wp:fincommerce/mini-cart-checkout-button-block -->
				<!-- /wp:fincommerce/mini-cart-footer-block -->
			<!-- /wp:fincommerce/filled-mini-cart-contents-block -->
	...
	<!-- wp:fincommerce/mini-cart-contents -->
		<div class="wp-block-fincommerce-mini-cart-contents">
			...
			<div class="wp-block-fincommerce-filled-mini-cart-contents-block">
				<!-- wp:fincommerce/mini-cart-title-block -->
				<div class="wp-block-fincommerce-mini-cart-title-block">
					<!-- wp:fincommerce/mini-cart-title-label-block -->
					<div class="wp-block-fincommerce-mini-cart-title-label-block">
					</div>
					<!-- /wp:fincommerce/mini-cart-title-label-block -->
					...
					<!-- wp:group -->
						<div class="wp-block-group">
							<!-- wp:fincommerce/mini-cart-title-items-counter-block -->
							<div class="wp-block-fincommerce-mini-cart-title-items-counter-block">
							</div>
							<!-- /wp:fincommerce/mini-cart-title-items-counter-block -->
						</div>
					<!-- /wp:group -->
	...
	public function setUp(): void {
		...
		if ( $registry->is_registered( 'fincommerce/mini-cart' ) ) {
			$this->original_block_type = $registry->get_registered( 'fincommerce/mini-cart' );
			$registry->unregister( 'fincommerce/mini-cart' );
		}
		...
		WC()->cart->empty_cart();
		add_filter( 'fincommerce_is_rest_api_request', '__return_false', 1 );
	}
	...
	public function tearDown(): void {
		parent::tearDown();
		WC()->cart->empty_cart();
		remove_filter( 'fincommerce_is_rest_api_request', '__return_false', 1 );
	...
	public function test_product_count_visibility_with_empty_cart() {
		...
		$block  = parse_blocks( '<!-- wp:fincommerce/mini-cart {"productCountVisibility":"always"} /-->' );
		...
		$block  = parse_blocks( '<!-- wp:fincommerce/mini-cart {"productCountVisibility":"greater_than_zero"} /-->' );
		$output = render_block( $block[0] );
		$this->assertStringContainsString( '<span class="wc-block-mini-cart__badge"', $output );

		// Tests badge is not shown when "never" is selected.
		$block  = parse_blocks( '<!-- wp:fincommerce/mini-cart {"productCountVisibility":"never"} /-->' );
		$output = render_block( $block[0] );
