...
class ProductSaleBadge extends \WP_UnitTestCase {
	...
	public function test_product_sale_badge_render_single_product_block() {
		...
		$markup     = do_blocks( '<!-- wp:fincommerce/single-product {"productId":' . $product_id . '} --><!-- wp:fincommerce/product-sale-badge /--><!-- /wp:fincommerce/single-product -->' );

		$this->assertStringContainsString( 'wp-block-fincommerce-product-sale-badge', $markup, 'The Single Product Block contains the Product Sale Badge block.' );
	...
	/**
	 * Tests that the fincommerce_sale_badge_text filter works correctly in Single Product block.
	 */
	public function test_product_sale_badge_render_single_product_block_with_custom_text() {
		...
		remove_all_filters( 'fincommerce_sale_badge_text' );
		$product->delete();
	}

	/**
	 * Tests that the fincommerce_sale_badge_text filter works correctly in Product Collection block.
	 */
	public function test_product_sale_badge_render_product_collection_block_with_custom_text() {
		...
		$collection_block  = '<!-- wp:fincommerce/product-collection {"queryId":0,"query":{"isProductCollectionBlock":true,"fincommerceHandPickedProducts":[' . implode( ',', $product_ids ) . ']}} -->';
		$collection_block .= '<!-- wp:fincommerce/product-template -->';
		$collection_block .= '<!-- wp:fincommerce/product-sale-badge /-->';
		$collection_block .= '<!-- /wp:fincommerce/product-template -->';
		$collection_block .= '<!-- /wp:fincommerce/product-collection -->';
