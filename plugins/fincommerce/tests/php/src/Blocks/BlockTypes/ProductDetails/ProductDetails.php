...
namespace Automattic\FinCommerce\Tests\Blocks\BlockTypes\ProductDetails;

use WC_Helper_Product;
use Automattic\FinCommerce\Tests\Blocks\Mocks\ProductDetailsNoRegisterMock;
...
class ProductDetails extends \WP_UnitTestCase {
	...
	/**
	 * Test Product Details render function when `fincommerce_product_tabs` hook isn't used
	...
	/**
	 * Test Product Details render function when `fincommerce_product_tabs` hook is used.
	 * IMPORTANT: The current test doesn't validate the entire HTML, but only the text content inside the HTML.
	 * This is because some ids are generated dynamically via wp_unique_id that it is not straightforward to mock.
	 */
	...
	/**
	 * Test the `fincommerce_product_details_hooked_blocks` hook. This hook allows developers to
	...
	public function test_hooked_blocks() {
		...
		add_filter(
			'fincommerce_product_details_hooked_blocks',
		...
		// We pretend that we're in the `last_child` position of the `fincommerce/accordion-group` block.

		// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment -- test code.
		$hooked_block_types = apply_filters( 'hooked_block_types', array(), 'last_child', 'fincommerce/accordion-group', null );
		...
		$hooked_block_custom_info = apply_filters(
			...
			'last_child',
			array(
				'blockName'    => 'fincommerce/accordion-group',
				'attrs'        => array(
					'metadata' => array(
		...
		$this->assertSame( 'fincommerce/accordion-item', $hooked_block_custom_info['blockName'] );
		$this->assertCount( 2, $hooked_block_custom_info['innerBlocks'] );

		$this->assertSame( 'fincommerce/accordion-header', $hooked_block_custom_info['innerBlocks'][0]['blockName'] );
		$this->assertStringContainsString( $test_block['title'], $hooked_block_custom_info['innerBlocks'][0]['innerHTML'] );

		$this->assertSame( 'fincommerce/accordion-panel', $hooked_block_custom_info['innerBlocks'][1]['blockName'] );
