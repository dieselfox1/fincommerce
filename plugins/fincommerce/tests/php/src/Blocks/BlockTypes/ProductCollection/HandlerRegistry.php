...
namespace Automattic\FinCommerce\Tests\Blocks\BlockTypes\ProductCollection;

use Automattic\FinCommerce\Tests\Blocks\BlockTypes\ProductCollection\Utils;
use Automattic\FinCommerce\Tests\Blocks\Mocks\ProductCollectionMock;
...
class HandlerRegistry extends \WP_UnitTestCase {
	...
	public function test_collection_related_products() {
		...
		add_filter( 'fincommerce_product_related_posts_force_display', '__return_true', 0 );
		$related_filter->expects( $this->exactly( 2 ) )
			->method( '__invoke' )
			->with( array(), 1 )
			->willReturn( $expected_product_ids );
		add_filter( 'fincommerce_related_products', array( $related_filter, '__invoke' ), 10, 2 );

		// Frontend.
		$parsed_block                                       = Utils::get_base_parsed_block();
		$parsed_block['attrs']['collection']                = 'fincommerce/product-collection/related';
		...
		remove_filter( 'fincommerce_product_related_posts_force_display', '__return_true', 0 );
		remove_filter( 'fincommerce_related_products', array( $related_filter, '__invoke' ) );
	...
	public function test_collection_upsells() {
		...
		// Frontend.
		$parsed_block                                       = Utils::get_base_parsed_block();
		$parsed_block['attrs']['collection']                = 'fincommerce/product-collection/upsells';
		...
		$request->set_param(
			'productCollectionQueryContext',
			array(
				'collection' => 'fincommerce/product-collection/upsells',
			)
		);
		$result_editor = $this->block_instance->update_rest_query_in_editor( array(), $request );
	...
	public function test_collection_cross_sells() {
		...
		// Frontend.
		$parsed_block                                       = Utils::get_base_parsed_block();
		$parsed_block['attrs']['collection']                = 'fincommerce/product-collection/cross-sells';
		$parsed_block['attrs']['query']['productReference'] = $test_product->get_id();
		$result_frontend                                    = Utils::initialize_merged_query( $this->block_instance, $parsed_block );
	...
	public function test_collection_hand_picked_empty() {
		// Frontend.
		$parsed_block                        = Utils::get_base_parsed_block();
		$parsed_block['attrs']['collection'] = 'fincommerce/product-collection/hand-picked';
		$parsed_block['attrs']['query']['fincommerceHandPickedProducts'] = array();
		$result_frontend = Utils::initialize_merged_query( $this->block_instance, $parsed_block );

		// Editor.
		$request = Utils::build_request(
			array( 'fincommerceHandPickedProducts' => array() )
		);
		$request->set_param(
			'productCollectionQueryContext',
			array(
				'collection' => 'fincommerce/product-collection/hand-picked',
			)
		);
		$result_editor = $this->block_instance->update_rest_query_in_editor( array(), $request );
	...
	public function test_collection_hand_picked_order() {
		...
		$parsed_block                        = Utils::get_base_parsed_block();
		$parsed_block['attrs']['collection'] = 'fincommerce/product-collection/hand-picked';
		$parsed_block['attrs']['query']['fincommerceHandPickedProducts'] = $product_ids;
		$result_frontend = Utils::initialize_merged_query( $this->block_instance, $parsed_block );

		// Editor.
		$request = Utils::build_request(
			array( 'fincommerceHandPickedProducts' => $product_ids )
		);
		$request->set_param(
			'productCollectionQueryContext',
			array(
