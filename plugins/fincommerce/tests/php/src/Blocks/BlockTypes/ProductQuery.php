...
use Automattic\FinCommerce\Tests\Blocks\Mocks\ProductQueryMock;
...
class ProductQuery extends \WP_UnitTestCase {
		...
		return array(
			...
			'attrs'     => array(
				...
				'query'     => array(
					...
					'sticky'                   => '',
					'inherit'                  => false,
					'__fincommerceAttributes'  => array(),
					'__fincommerceStockStatus' => array(
	 ...
	 *
	 * @param bool  $fincommerce_on_sale FinCommerce on sale.
	 * @param array $fincommerce_attributes FinCommerce attributes.
	 * @param array $fincommerce_stock_status FinCommerce stock status.
	 * @return WP_REST_Request
	 */
	private function build_request( $fincommerce_on_sale = 'false', $fincommerce_attributes = array(), $fincommerce_stock_status = array() ) {
		$request = new \WP_REST_Request( 'GET', '/wp/v2/product' );
		$request->set_param( '__fincommerceOnSale', $fincommerce_on_sale );
		$request->set_param( '__fincommerceAttributes', $fincommerce_attributes );
		$request->set_param( '__fincommerceStockStatus', $fincommerce_stock_status );
	...
	public function test_merging_on_sale_queries() {
		...
		$parsed_block = $this->get_base_parsed_block();
		$parsed_block['attrs']['query']['__fincommerceOnSale'] = true;
	...
	public function test_merging_stock_status_queries() {
		$parsed_block = $this->get_base_parsed_block();
		$parsed_block['attrs']['query']['__fincommerceStockStatus'] = array(
			ProductStockStatus::OUT_OF_STOCK,
			ProductStockStatus::ON_BACKORDER,
		);
	...
	public function test_merging_attribute_queries() {
		$parsed_block = $this->get_base_parsed_block();
		$parsed_block['attrs']['query']['__fincommerceAttributes'] = array(
			array(
				'taxonomy' => 'pa_test',
				'termId'   => 1,
			),
	...
	public function test_product_visibility_query_exist_in_merged_query() {
		...
		// Test with hide out of stock items option enabled.
		add_filter(
			'pre_option_fincommerce_hide_out_of_stock_items',
			$fn
		);
