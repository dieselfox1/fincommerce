...
namespace Automattic\FinCommerce\Tests\Blocks\Assets;
...
class AssetDataRegistry extends \WP_UnitTestCase {
	...
	/**
	 * This tests the 'fincommerce_shared_settings' filter.
	 */
	public function test_fincommerce_filter_with_protected_data() {
		$this->registry->initialize_core_data();
		$original_data = $this->registry->get();
		add_filter( 'fincommerce_shared_settings', [ self::class, 'pdatcallback' ] );
		$data = $this->registry->get();
		$this->registry->initialize_core_data();
		$this->assertEquals( $original_data, $data );
		remove_filter( 'fincommerce_shared_settings', [ self::class, 'pdatcallback' ] );
	}
	...
	public function test_fincommerce_filter_with_new_data() {
		$this->registry->initialize_core_data();
		$original_data = $this->registry->get();
		add_filter( 'fincommerce_shared_settings', [ self::class, 'ndcallback' ] );
		$this->registry->initialize_core_data();
		$data = $this->registry->get();
		$original_data['cheeseburger'] = 'fries';
		$this->assertEquals( $original_data, $data );
		remove_filter( 'fincommerce_shared_settings', [ self::class, 'ndcallback' ] );
	}
}
