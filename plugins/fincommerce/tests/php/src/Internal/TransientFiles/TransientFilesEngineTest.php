...
namespace Automattic\FinCommerce\Tests\Internal\TransientFiles;
...
class TransientFilesEngineTest extends \WC_REST_Unit_Test_Case {
	...
	public static function setUpBeforeClass(): void {
		...
		self::$transient_files_dir      = self::$base_transient_files_dir . '/fincommerce_transient_files';
	...
	/**
	 * @testdox The base directory for the transient files can be modified with the fincommerce_transient_files_directory hook.
	 */
	public function test_transient_files_directory_can_be_changed_via_hook() {
		$original_directory = null;

		add_filter(
			'fincommerce_transient_files_directory',
	...
	public function test_get_transient_files_directory_throws_if_filter_is_used_and_directory_does_not_exist() {
		add_filter( 'fincommerce_transient_files_directory', fn( $default_dir) => 'foobar_dir' );
	...
	/**
	 * @testdox get_transient_files_directory creates the default base directory if it doesn't exist and the fincommerce_transient_files_directory filter is not used.
	 */
	...
	public function test_cleanup_scheduled_action_reschedules_for_one_day_later_if_no_files_are_deleted() {
		...
		try {
			...
			do_action( 'fincommerce_expired_transient_files_cleanup' );
		} finally {
			remove_all_filters( 'fincommerce_expired_transient_files_cleanup' );
		}
	...
	public function test_cleanup_scheduled_action_interval_can_be_modified_via_hook() {
		...
		add_filter( 'fincommerce_delete_expired_transient_files_interval', fn( $interval ) => HOUR_IN_SECONDS );
		...
		try {
			...
			remove_all_filters( 'fincommerce_expired_transient_files_cleanup' );
			remove_all_filters( 'fincommerce_delete_expired_transient_files_interval' );
		}
	...
	/**
	 * @testdox The interval for the next expired files cleanup scheduled action can be modified via the fincommerce_expired_transient_files_cleanup hook.
	 */
