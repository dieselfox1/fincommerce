...
use Automattic\FinCommerce\Internal\Orders\OrderActionsRestController;
use WC_Helper_Order;
...
class OrderActionsRestControllerTest extends WC_REST_Unit_Test_Case {
	...
	public function test_send_order_details_with_non_existent_order() {
		...
		$data = $response->get_data();
		$this->assertEquals( 'fincommerce_rest_not_found', $data['code'] );
	...
	public function test_send_order_details_without_order_email() {
		...
		$data = $response->get_data();
		$this->assertEquals( 'fincommerce_rest_missing_email', $data['code'] );
