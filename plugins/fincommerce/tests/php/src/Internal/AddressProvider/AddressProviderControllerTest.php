...
namespace Automattic\FinCommerce\Tests\Internal\AddressProvider;
...
class AddressProviderControllerTest extends MockeryTestCase {
	...
	protected function tearDown(): void {
		parent::tearDown();
		remove_all_filters( 'fincommerce_address_providers' );
	...
	public function test_add_address_autocomplete_settings_no_providers() {
		...
		foreach ( $settings as $setting ) {
			if ( isset( $setting['id'] ) && 'fincommerce_address_autocomplete_enabled' === $setting['id'] ) {
				$autocomplete_enabled_setting = $setting;
				break;
			}
			if ( isset( $setting['id'] ) && 'fincommerce_address_autocomplete_provider' === $setting['id'] ) {
	...
	public function test_add_address_autocomplete_settings_position() {
		...
		foreach ( $settings as $index => $setting ) {
			if ( isset( $setting['id'] ) ) {
				if ( 'fincommerce_default_customer_address' === $setting['id'] ) {
					$default_address_pos = $index;
				} elseif ( 'fincommerce_address_autocomplete_enabled' === $setting['id'] ) {
	...
	public function test_get_preferred_provider_from_options() {
		...
		update_option( 'fincommerce_address_autocomplete_provider', 'provider-2' );
	...
	public function test_get_preferred_provider_fallback() {
		...
		delete_option( 'fincommerce_address_autocomplete_provider' );
	...
	public function test_get_preferred_provider_unavailable() {
		...
		update_option( 'fincommerce_address_autocomplete_provider', 'provider-3' );
	...
	public function test_logs_error_for_non_array_filter_return() {
		...
		$this->mock_logger
			...
			->method( 'error' )
			->with(
				'Invalid return value for fincommerce_address_providers, expected an array of class names or instances.',
	...
	public function test_order_preserved_when_preferred_provider_not_found() {
		...
		update_option( 'fincommerce_address_autocomplete_provider', 'provider-nonexistent' );
	...
	public function test_order_preserved_when_first_provider_already_preferred() {
		...
		update_option( 'fincommerce_address_autocomplete_provider', 'provider-1' );
