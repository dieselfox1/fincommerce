...
namespace Automattic\FinCommerce\Tests\Internal\Fulfillments;

use Automattic\FinCommerce\Internal\Fulfillments\FulfillmentsManager;
use Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper;
use Automattic\FinCommerce\Tests\Internal\Fulfillments\Helpers\FulfillmentsHelper;
use Automattic\FinCommerce\Testing\Tools\TestingContainer;
use Automattic\FinCommerce\Tests\Internal\Fulfillments\Helpers\ShippingProviderMock;
use WC_Order;
...
class FulfillmentsManagerTest extends \WC_Unit_Test_Case {
	...
	public static function setUpBeforeClass(): void {
		...
		update_option( 'fincommerce_feature_fulfillments_enabled', 'yes' );
		$controller = wc_get_container()->get( \Automattic\FinCommerce\Internal\Fulfillments\FulfillmentsController::class );
		$controller->register();
	...
	public static function tearDownAfterClass(): void {
		update_option( 'fincommerce_feature_fulfillments_enabled', 'no' );
	...
	public function test_hooks() {
		$this->assertNotFalse( has_filter( 'fincommerce_fulfillment_translate_meta_key', array( $this->manager, 'translate_fulfillment_meta_key' ) ) );
	}
	...
	public function test_translate_fulfillment_meta_key() {
		...
		$this->assertEquals( __( 'Fulfillment Status', 'fincommerce' ), $translated_key );
	...
	public function test_extend_translate_fulfillment_meta_key() {
		...
		add_filter(
			...
			function ( $translations ) {
				$translations['custom_meta_key'] = __( 'Custom Meta Key', 'fincommerce' );
				return $translations;
			}
		);
		...
		$this->assertEquals( __( 'Custom Meta Key', 'fincommerce' ), $translated_key );
	}
	...
	public function test_translate_fulfillment_meta_key_with_filter() {
		 ...
		 * @since 10.1.0
		 */
		$translated_key = apply_filters( 'fincommerce_fulfillment_translate_meta_key', 'custom_meta_key' );
		$this->assertEquals( __( 'Custom Meta Key', 'fincommerce' ), $translated_key );
	}
	...
	public function test_get_initial_shipping_providers() {
		 ...
		 * @since 10.1.0
		 */
		$shipping_providers = apply_filters( 'fincommerce_fulfillment_shipping_providers', array() );
	...
	public function test_extend_initial_shipping_providers() {
		...
		add_filter(
			...
			function ( $providers ) {
				$providers['custom_provider'] = array(
					'label' => __( 'Custom Provider', 'fincommerce' ),
					'icon'  => 'custom-icon',
		...
		$this->assertEquals( __( 'Custom Provider', 'fincommerce' ), $shipping_providers['custom_provider']['label'] );
	}
	...
	public function test_init_fulfillment_status_hooks() {
		$this->assertNotFalse( has_action( 'fincommerce_fulfillment_after_create', array( $this->manager, 'update_order_fulfillment_status_on_fulfillment_update' ) ) );
		$this->assertNotFalse( has_action( 'fincommerce_fulfillment_after_update', array( $this->manager, 'update_order_fulfillment_status_on_fulfillment_update' ) ) );
		$this->assertNotFalse( has_action( 'fincommerce_fulfillment_after_delete', array( $this->manager, 'update_order_fulfillment_status_on_fulfillment_update' ) ) );
	}
	...
	public function test_update_order_fulfillment_status_on_fulfillment_updates() {
		...
		$this->assertTrue( did_action( 'fincommerce_fulfillment_after_create' ) > 0 );
		...
		$this->assertTrue( did_action( 'fincommerce_fulfillment_after_update' ) > 0 );
		$order = wc_get_order( $order->get_id() );
		$this->assertEquals( 'partially_fulfilled', $order->get_meta( '_fulfillment_status' ) );

		$fulfillments[0]->delete();
		$this->assertTrue( did_action( 'fincommerce_fulfillment_after_delete' ) > 0 );
		$order = wc_get_order( $order->get_id() );
