...
use Automattic\FinCommerce\Internal\Fulfillments\FulfillmentsSettings;
use WC_Order;
...
class FulfillmentsSettingsTest extends WC_Unit_Test_Case {
	...
	public function test_hooks_added() {
		...
		$this->assertNotFalse( has_action( 'fincommerce_order_status_processing', array( $fulfillments_settings, 'auto_fulfill_items_on_processing' ) ) > 0 );
		$this->assertNotFalse( has_action( 'fincommerce_order_status_completed', array( $fulfillments_settings, 'auto_fulfill_items_on_completed' ) ) > 0 );
	}
	...
	public function test_auto_fulfill_items_on_completed_calls_hook_with_item_and_setting_combinations() {
		$mock_sut = $this->getMockBuilder( \Automattic\FinCommerce\Internal\Fulfillments\FulfillmentsSettings::class )
