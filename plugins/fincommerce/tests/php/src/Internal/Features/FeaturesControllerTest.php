...
namespace Automattic\FinCommerce\Tests\Internal\Features;
...
class FeaturesControllerTest extends \WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		add_action(
			'fincommerce_register_feature_definitions',
		...
		delete_option( 'fincommerce_feature_mature1_enabled' );
		delete_option( 'fincommerce_feature_mature2_enabled' );
		delete_option( 'fincommerce_feature_experimental1_enabled' );
		delete_option( 'fincommerce_feature_experimental2_enabled' );
	...
	private function set_up_plugins(): void {
		...
		$this->fake_plugin_util = new class() extends PluginUtil {
			...
			public function get_fincommerce_aware_plugins( bool $active_only = false ): array {
	...
	public function test_get_features_including_experimental_and_values() {
		update_option( 'fincommerce_feature_mature1_enabled', 'yes' );
		update_option( 'fincommerce_feature_mature2_enabled', 'no' );
		update_option( 'fincommerce_feature_experimental1_enabled', 'yes' );
	...
	public function test_change_feature_enable( $previous_value, $enable, $expected_result, $expected_previous_enabled, $expected_new_enabled ) {
		if ( $previous_value ) {
			update_option( 'fincommerce_feature_mature1_enabled', $previous_value );
		}
	...
	/**
	 * @testdox 'declare_compatibility' fails when invoked from outside the 'before_fincommerce_init' action.
	 */
	public function test_declare_compatibility_outside_before_fincommerce_init_hook() {
		$function = null;
		$message  = null;
		$version  = null;
		...
		$this->assertEquals( 'FeaturesController::declare_compatibility should be called inside the before_fincommerce_init action.', $message );
	...
	public function test_declare_compatibility_for_non_existing_feature() {
		$this->simulate_inside_before_fincommerce_init_hook();
	...
	/**
	 * @testdox 'get_compatible_features_for_plugin' fails when invoked before the 'fincommerce_init' hook.
	 */
	public function test_get_compatible_features_for_plugin_before_fincommerce_init_hook() {
		$function = null;
		$message  = null;
		$version  = null;
		...
		$this->register_legacy_proxy_function_mocks(
			array(
				'did_action'        => function ( $action_name ) {
					return 'fincommerce_init' === $action_name ? false : did_action( $action_name );
				},
				'wc_doing_it_wrong' => function ( $f, $m, $v ) use ( &$function, &$message, &$version ) {
					$function = $f;
					$message  = $m;
		...
		$this->assertEquals( 'FeaturesController::get_compatible_features_for_plugin should not be called before the fincommerce_init action.', $message );
		$this->assertEquals( '7.0', $version );
	}
	...
	public function test_get_compatible_features_for_unregistered_plugin() {
		$this->simulate_after_fincommerce_init_hook();
	...
	public function test_get_compatible_enabled_features_for_registered_plugin() {
		...
		update_option( 'fincommerce_feature_mature1_enabled', 'yes' );
		update_option( 'fincommerce_feature_mature2_enabled', 'no' );
		update_option( 'fincommerce_feature_mature3_enabled', 'yes' );
		update_option( 'fincommerce_feature_experimental1_enabled', 'no' );
		update_option( 'fincommerce_feature_experimental2_enabled', 'yes' );
		update_option( 'fincommerce_feature_experimental3_enabled', 'no' );
	...
	/**
	 * @testdox 'get_compatible_plugins_for_feature' fails when invoked before the 'fincommerce_init' hook.
	 */
	public function test_get_compatible_plugins_for_feature_before_fincommerce_init_hook() {
		$function = null;
		$message  = null;
		$version  = null;
		...
		$this->assertEquals( 'FeaturesController::get_compatible_plugins_for_feature should not be called before the fincommerce_init action.', $message );
		$this->assertEquals( '7.0', $version );
	}
	...
	public function test_get_compatible_plugins_for_feature( bool $active_only ) {
		...
		// phpcs:enable FinCommerce.Commenting.CommentHooks.MissingHookComment
	...
	/**
	 * Simulates that the code is running inside the 'before_fincommerce_init' action.
	 */
	private function simulate_inside_before_fincommerce_init_hook() {
		$this->register_legacy_proxy_function_mocks(
			array(
				'doing_action' => function ( $action_name ) {
					return 'before_fincommerce_init' === $action_name || doing_action( $action_name );
				},
			)
		);
	}
	...
	/**
	 * Simulates that the code is running after the 'fincommerce_init' action has been fired.
	 */
	private function simulate_after_fincommerce_init_hook() {
		$this->register_legacy_proxy_function_mocks(
			array(
				'did_action' => function ( $action_name ) {
					return 'fincommerce_init' === $action_name || did_action( $action_name );
				},
			)
		);
	}
			...
			function ( $features_controller ) {
				$features = array(
					'custom_order_tables'  => array(
						'name'               => __( 'High-Performance order storage', 'fincommerce' ),
						'is_experimental'    => true,
	...
	public function test_lazy_declaration_and_processing() {
		...
		$plugin_util_mock = $this->getMockBuilder( PluginUtil::class )
			...
			->onlyMethods( array( 'get_wp_plugin_id', 'get_fincommerce_aware_plugins' ) )
			->getMock();
		...
		$plugin_util_mock->method( 'get_fincommerce_aware_plugins' )
	...
	public function test_deactivation_after_lazy_processing() {
		...
		// Goal: Replace $this->sut's ->plugin_util with a mocked version that
		// doesn't scan the disk, but resolves fake paths for our non-existant plugin.php.
		// Also replace get_fincommerce_aware_plugins to simulate deactivation.
		$plugin_util_mock = $this->getMockBuilder( PluginUtil::class )
			->disableOriginalConstructor()
			->onlyMethods( array( 'get_wp_plugin_id', 'get_fincommerce_aware_plugins' ) )
			->getMock();
		...
		// Control get_fincommerce_aware_plugins to simulate before/after deactivation.
		...
		$deactivated = true;
		do_action( 'deactivated_plugin', 'plugin/plugin.php' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
