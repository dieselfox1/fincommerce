...
namespace Automattic\FinCommerce\Tests\Internal\ProductDownloads\ApprovedDirectories;
...
class SynchronizeTest extends WC_Unit_Test_Case {
	...
	public function test_sync_process() {
		...
		add_filter( 'fincommerce_logger_log_message', $log_watcher );

		$this->sut->start();
		$this->sut->run();

		remove_filter( 'fincommerce_logger_log_message', $log_watcher );
