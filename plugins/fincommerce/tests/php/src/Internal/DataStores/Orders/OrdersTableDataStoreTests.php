...
namespace Automattic\FinCommerce\Tests\Internal\DataStores\Orders;

use Automattic\FinCommerce\Caches\OrderCache;
use Automattic\FinCommerce\Database\Migrations\CustomOrderTable\PostsToOrdersMigrationController;
use Automattic\FinCommerce\Enums\OrderStatus;
use Automattic\FinCommerce\Enums\OrderInternalStatus;
use Automattic\FinCommerce\Internal\CostOfGoodsSold\CogsAwareUnitTestSuiteTrait;
...
use Automattic\FinCommerce\Internal\DataStores\Orders\OrdersTableDataStoreMeta;
use Automattic\FinCommerce\Internal\DataStores\Orders\OrdersTableQuery;
use Automattic\FinCommerce\RestApi\UnitTests\Helpers\OrderHelper;
use Automattic\FinCommerce\RestApi\UnitTests\HPOSToggleTrait;
...
class OrdersTableDataStoreTests extends \HposTestCase {
	...
	public function tearDown(): void {
		...
		remove_all_filters( 'fincommerce_load_order_cogs_value' );
		remove_all_filters( 'fincommerce_save_order_cogs_value' );
		wc()->cart->empty_cart();
		parent::tearDown();
	}
	...
	public function test_cot_datastore_delete() {
		...
		$this->assertEmpty( $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}fincommerce_order_items WHERE order_id = %d", $order_id ) ) );
	...
	/**
	 * @testDox A 'suppress_filters' argument can be passed to 'delete', if true no 'fincommerce_(before_)trash/delete_order' actions will be fired.
	 *
	 * @testWith [null, true]
	 *           [true, true]
	 *           [false, true]
	...
	public function test_filters_can_be_suppressed_when_trashing_or_deleting_an_order( ?bool $suppress, bool $force_delete ) {
		...
		add_action(
			"fincommerce_before_{$trash_or_delete}_order",
		...
		add_action(
			"fincommerce_{$trash_or_delete}_order",
	...
	public function test_order_hooks_vs_cpt( $cot_is_authoritative = true ) {
		...
		$callback = function ( $order_id ) use ( &$new_count, &$update_count ) {
			$new_count    += 'fincommerce_new_order' === current_action() ? 1 : 0;
			$update_count += 'fincommerce_update_order' === current_action() ? 1 : 0;
		};

		add_action( 'fincommerce_new_order', $callback );
		add_action( 'fincommerce_update_order', $callback );

		// Creating a new order should trigger 'fincommerce_new_order' but not 'fincommerce_update_order'.
		$order = new WC_Order();
		$order->save();
		$this->assertEquals( 1, $new_count );
		$this->assertEquals( 0, $update_count );

		// Saving an order again (with no changes) should still trigger 'fincommerce_update_order'.
		$order->save();
		$this->assertEquals( 1, $new_count );
		$this->assertEquals( 1, $update_count );

		// An update to the order should only trigger 'fincommerce_update_order'.
		...
		// An auto-draft order should not trigger 'fincommerce_new_order' until first saved with a valid status.
		if ( $cot_is_authoritative ) {
			$order = new WC_Order();
		...
		remove_action( 'fincommerce_new_order', $callback );
		remove_action( 'fincommerce_update_order', $callback );
	}
	...
	private function reset_order_data_store_state( $sut ) {
		...
		wc_get_container()->get( \Automattic\FinCommerce\Caches\OrderCache::class )->flush();
	}
	...
	/**
	 * @testDox Hooking into fincommerce_delete_shop_order_transients does not cause data loss.
	 */
	public function test_data_retained_when_hooked_in_cache_filter() {
		...
		add_action(
			'fincommerce_delete_shop_order_transients',
			function ( $order_id ) {
		...
		remove_all_actions( 'fincommerce_delete_shop_order_transients' );
	}
	...
	public function test_order_save_is_not_called_frequently_on_meta_save() {
		...
		add_filter( 'fincommerce_order_data_store', $orders_table_data_store, 1000, 0 );
		...
		$count = 0;
		add_filter(
			'fincommerce_before_order_object_save',
			function () use ( &$count ) {
				$count++;
			}
		);
		...
		remove_filter( 'fincommerce_order_data_store', $orders_table_data_store, 1000 );
		remove_all_actions( 'fincommerce_before_order_object_save' );
	}
	...
	public function test_order_updated_webhook_delivered_once() {
		...
		add_action( 'fincommerce_webhook_process_delivery', array( $webhook_tests, 'fincommerce_webhook_process_delivery' ), 1, 2 );
		add_filter( 'fincommerce_webhook_should_deliver', '__return_true' );
		$call_private = function ( $order ) {
			return $this->should_save_after_meta_change( $order );
		};
		$this->assertTrue( $call_private->call( $this->sut, $order ) );
		...
		remove_all_actions( 'fincommerce_webhook_process_delivery' );
		remove_all_actions( 'fincommerce_webhook_should_deliver' );
	}
	...
	/**
	 * @testDox Creating an order with a draft status should not trigger the "fincommerce_new_order" action.
	 */
	...
	/**
	 * @testDox Updating an order status correctly triggers the "fincommerce_new_order" action.
	 */
	...
	/**
	 * @testDox Create a new order with processing status without saving and updating it should trigger the "fincommerce_new_order" action.
	 */
	...
	/**
	 * @testdox It's possible to modify the Cost of Goods Sold value that gets loaded from the database for an order using the 'fincommerce_load_order_cogs_value' filter.
	 */
	public function test_loaded_cogs_value_can_be_modified_via_filter() {
		...
		add_filter(
			'fincommerce_load_order_cogs_value',
	...
	/**
	 * @testdox It's possible to modify the Cost of Goods Sold value that gets persisted for an order using the 'fincommerce_save_order_cogs_value' filter, returning null suppresses the saving.
	 *
	...
	public function test_saved_cogs_value_can_be_altered_via_filter_with_null_meaning_dont_save( $filter_return_value, string $expected_saved_value ) {
		...
		add_filter(
			'fincommerce_save_order_cogs_value',
