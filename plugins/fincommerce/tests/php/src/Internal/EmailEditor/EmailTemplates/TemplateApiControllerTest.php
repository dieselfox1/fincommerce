...
namespace Automattic\FinCommerce\Tests\Internal\EmailEditor\EmailTemplates;
...
class TemplateApiControllerTest extends \WC_Unit_Test_Case {
	...
	/**
	 * Test that getTemplateData returns empty array for non-FinCommerce templates.
	 */
	...
	/**
	 * Test that getTemplateData returns sender settings for FinCommerce templates.
	 */
	public function testItGetsTemplateDataForWooTemplate(): void {
		$from_name    = 'Test Store';
		$from_address = 'test@example.com';

		update_option( 'fincommerce_email_from_name', $from_name );
		update_option( 'fincommerce_email_from_address', $from_address );
	...
	/**
	 * Test that saveData updates FinCommerce email settings.
	 */
	public function testItSavesTemplateDataForWooTemplate(): void {
		...
		$this->assertEquals( $new_from_name, get_option( 'fincommerce_email_from_name' ) );
		$this->assertEquals( $new_from_address, get_option( 'fincommerce_email_from_address' ) );
	}

	/**
	 * Test that saveTemplateData does not update settings for non-FinCommerce templates.
	 */
	public function testItDoesNotSaveTemplateDataForNonWooTemplate(): void {
		...
		update_option( 'fincommerce_email_from_name', $original_from_name );
		update_option( 'fincommerce_email_from_address', $original_from_address );
		...
		$this->assertEquals( $original_from_name, get_option( 'fincommerce_email_from_name' ) );
		$this->assertEquals( $original_from_address, get_option( 'fincommerce_email_from_address' ) );
	}
