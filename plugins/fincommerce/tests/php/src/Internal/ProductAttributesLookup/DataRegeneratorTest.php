...
namespace Automattic\FinCommerce\Tests\Internal\ProductAttributesLookup;
...
use Automattic\FinCommerce\Testing\Tools\FakeQueue;

/**
 * Tests for the DataRegenerator class.
 * @package Automattic\FinCommerce\Tests\Internal\ProductAttributesLookup
 */
class DataRegeneratorTest extends \WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		remove_all_actions( 'fincommerce_run_product_attribute_lookup_regeneration_callback' );
	...
	public function test_initiate_regeneration_initializes_temporary_options_and_enqueues_regeneration_step() {
		...
		$this->assertEquals( 100, get_option( 'fincommerce_attribute_lookup_last_product_id_to_process' ) );
		$this->assertEquals( 0, get_option( 'fincommerce_attribute_lookup_processed_count' ) );
		$this->assertFalse( get_option( 'fincommerce_attribute_lookup_enabled' ) );
	...
	public function test_initiate_regeneration_does_not_enqueues_regeneration_step_when_no_products( $get_products_result ) {
		...
		$this->assertFalse( get_option( 'fincommerce_attribute_lookup_last_product_id_to_process' ) );
		$this->assertFalse( get_option( 'fincommerce_attribute_lookup_processed_count' ) );
		$this->assertEquals( 'yes', get_option( 'fincommerce_attribute_lookup_enabled' ) );
	...
	public function test_initiate_regeneration_correctly_processes_ids_and_enqueues_next_step() {
		...
		update_option( 'fincommerce_attribute_lookup_processed_count', 7 );

		//phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		do_action( 'fincommerce_run_product_attribute_lookup_regeneration_callback' );

		$this->assertEquals( array( 1, 2, 3 ), $this->lookup_data_store->passed_products );
		$this->assertEquals( array( 7 ), $requested_products_offsets );
		$this->assertEquals( 7 + count( $this->lookup_data_store->passed_products ), get_option( 'fincommerce_attribute_lookup_processed_count' ) );
	...
	/**
	 * @testDox Products are processed in groups of whatever the 'fincommerce_attribute_lookup_regeneration_step_size' filter returns, defaulting to PRODUCTS_PER_GENERATION_STEP
	 *
	 * @testWith [true]
	 *           [false]
	 *
	 ...
	 */
	public function test_regeneration_uses_the_fincommerce_attribute_lookup_regeneration_step_size_filter( bool $set_filter ) {
		...
		if ( $set_filter ) {
			\add_filter(
				'fincommerce_attribute_lookup_regeneration_step_size',
		...
		remove_all_filters( ' fincommerce_attribute_lookup_regeneration_step_size' );
	...
	/**
	 * @testdox After FinCommerce is installed the table usage is enabled only if it hadn't been explicitly disabled by an admin.
	 *
	 * @testWith [null, "yes"]
	 *           ["yes", "yes"]
	 *           ["no", "no"]
	...
	public function test_after_install_table_usage_is_enabled_if_it_wasnt_disabled( ?string $previous_option_value, string $expected_final_option_value ) {
		if ( null === $previous_option_value ) {
			...
			update_option( 'fincommerce_attribute_lookup_enabled', $previous_option_value );
		}

		//phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		do_action( 'fincommerce_installed' );

		$actual_final_option_value = get_option( 'fincommerce_attribute_lookup_enabled' );
