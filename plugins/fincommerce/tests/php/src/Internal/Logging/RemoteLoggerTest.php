...
namespace Automattic\FinCommerce\Tests\Internal\Logging {
	...
	class RemoteLoggerTest extends \WC_Unit_Test_Case {
		...
		public function tearDown(): void {
			...
			delete_option( 'fincommerce_feature_remote_logging_enabled' );
		...
		private function cleanup_filters() {
			$filters = array(
				'option_fincommerce_admin_remote_feature_enabled',
				'option_fincommerce_allow_tracking',
				'option_fincommerce_version',
				'plugins_api',
				'pre_http_request',
				'fincommerce_remote_logger_formatted_log_data',
				'pre_site_transient_update_plugins',
				'fincommerce_remote_logger_request_uri_whitelist',
			);
		...
		public function remote_logging_disallowed_provider() {
			return array(
				'feature flag disabled' => array(
					...
					'setup'     => fn() => update_option( 'fincommerce_feature_remote_logging_enabled', 'no' ),
				),
				'tracking opted out'    => array(
					'condition' => 'tracking opted out',
					'setup'     => fn() => add_filter( 'option_fincommerce_allow_tracking', fn() => 'no' ),
				),
		 ...
		 *
		 * @param string $current_version The current FinCommerce version.
		 * @param string $new_version The new FinCommerce version.
		...
		/**
		 * @testdox fetch_new_fincommerce_version method returns correct version
		 */
		public function test_fetch_new_fincommerce_version() {
			$this->setup_mock_plugin_updates( '9.3.0' );

			$result = $this->invoke_private_method( $this->sut, 'fetch_new_fincommerce_version', array() );
			$this->assertEquals( '9.3.0', $result, 'The result should be the latest version when an update is available.' );
		}

		/**
		 * @testdox fetch_new_fincommerce_version method returns null when no update is available
		 */
		public function test_fetch_new_fincommerce_version_no_update() {
			...
			return array(
				'basic log data'            => array(
					...
					array(
						...
						'tags'     => array( 'fincommerce', 'php', 'tag1', 'tag2' ),
					),
				),
				'log with backtrace'        => array(
					'error',
					'Test error message',
					array( 'backtrace' => ABSPATH . 'wp-content/plugins/fincommerce/file.php' ),
					array( 'trace' => './fincommerce/file.php' ),
				),
				...
				'log with error file'       => array(
					...
					array(
						'file' => './fincommerce/includes/class-wc-test.php',
					),
				),
			);
		}
		...
		public function test_handle_filtered_log_null() {
			...
			add_filter( 'fincommerce_remote_logger_formatted_log_data', fn() => null, 10, 4 );
		...
		public function is_third_party_error_provider() {
			...
			return array(
				'FinCommerce error message' => array(
		...
		public function test_sanitize() {
			...
			$expected = './fincommerce/includes/class-wc-test.php on line 123';
		...
		public function test_sanitize_trace() {
			...
			$expected = "./fincommerce/includes/class-wc-test.php:123\n./wp-includes/plugin.php:456";
		...
		public function test_sanitize_with_custom_filter() {
			add_filter(
				'fincommerce_remote_logger_sanitized_content',
				function ( $sanitized ) {
			...
			$message  = WC_ABSPATH . 'includes/class-wc-test.php on line 123';
			$expected = './fincommerce/includes/class-wc-filtered.php on line 123';
			$result   = $this->invoke_private_method( $this->sut, 'sanitize', array( $message ) );
			$this->assertEquals( $expected, $result );
		}
		...
		private function setup_remote_logging_conditions( $enabled = true ) {
			update_option( 'fincommerce_feature_remote_logging_enabled', $enabled ? 'yes' : 'no' );
			add_filter( 'option_fincommerce_allow_tracking', fn() => 'yes' );
			 ...
			 *
			 * @param string $new_version The new version of FinCommerce to simulate.
			 */
 ...
 */
namespace Automattic\FinCommerce\Internal\Logging {
	/**
