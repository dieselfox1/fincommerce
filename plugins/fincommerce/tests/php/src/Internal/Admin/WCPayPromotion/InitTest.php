...
namespace Automattic\FinCommerce\Tests\Internal\Admin\WCPayPromotion;
...
use Automattic\FinCommerce\Internal\Admin\WCPayPromotion\DefaultPromotions;
use Automattic\FinCommerce\Internal\Admin\WCPayPromotion\Init as WCPayPromotion;
use Automattic\FinCommerce\Internal\Admin\WCPayPromotion\WCPayPromotionDataSourcePoller;
use WC_Unit_Test_Case;

/**
 * class WC_Admin_Tests_WCPayPromotion_Init
 *
 * @covers \Automattic\FinCommerce\Internal\Admin\WCPayPromotion\Init
 */
class InitTest extends WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		add_filter(
			'transient_fincommerce_admin_' . WCPayPromotionDataSourcePoller::ID . '_specs',
			function ( $value ) {
				if ( $value ) {
					return $value;
				}
	...
	public function tearDown(): void {
		...
		remove_all_filters( 'transient_fincommerce_admin_' . WCPayPromotionDataSourcePoller::ID . '_specs' );
	...
	public function test_use_default_specs_when_marketplace_suggestions_off() {
		// Arrange.
		update_option( 'fincommerce_show_marketplace_suggestions', 'no' );
	...
	public function test_specs_transient() {
		...
		set_transient(
			'fincommerce_admin_' . WCPayPromotionDataSourcePoller::ID . '_specs',
			array(
	...
	public function test_non_matching_promotions() {
		...
		update_option( 'fincommerce_default_country', 'XX' );
		set_transient(
			'fincommerce_admin_' . WCPayPromotionDataSourcePoller::ID . '_specs',
			array(
				'en_US' => array(
	...
	public function test_empty_remote_promotions_fallback_to_defaults() {
		// Arrange.
		update_option( 'fincommerce_default_country', 'US' );
		update_option( 'fincommerce_show_marketplace_suggestions', 'yes' );
		...
		// Act.
		$promotions                = WCPayPromotion::get_promotions();
		$stored_specs_in_transient = get_transient( 'fincommerce_admin_' . WCPayPromotionDataSourcePoller::ID . '_specs' );
		...
		$expires = (int) get_transient( '_transient_timeout_fincommerce_admin_' . WCPayPromotionDataSourcePoller::ID . '_specs' );
		...
		remove_all_filters( 'fincommerce_admin_payment_gateway_suggestion_specs' );
	}
}
