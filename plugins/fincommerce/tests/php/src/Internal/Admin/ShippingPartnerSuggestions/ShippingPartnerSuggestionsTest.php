...
namespace Automattic\FinCommerce\Tests\Internal\Admin\ShippingPartnerSuggestions;
...
use Automattic\FinCommerce\Admin\Features\ShippingPartnerSuggestions\DefaultShippingPartners;
use Automattic\FinCommerce\Admin\Features\ShippingPartnerSuggestions\ShippingPartnerSuggestions;
use Automattic\FinCommerce\Admin\Features\ShippingPartnerSuggestions\ShippingPartnerSuggestionsDataSourcePoller;
use Automattic\FinCommerce\Admin\Features\PaymentGatewaySuggestions\EvaluateSuggestion;
use WC_Unit_Test_Case;

/**
 * class WC_Admin_Tests_ShippingPartnerSuggestions_Init
 *
 * @covers \Automattic\FinCommerce\Admin\Features\ShippingPartnerSuggestions\ShippingPartnerSuggestions
 */
class ShippingPartnerSuggestionsTest extends WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		delete_option( 'fincommerce_show_marketplace_suggestions' );
		add_filter(
			'transient_fincommerce_admin_' . ShippingPartnerSuggestionsDataSourcePoller::ID . '_specs',
			function ( $value ) {
				if ( $value ) {
					return $value;
				}
		...
		add_filter( 'fincommerce_logging_class', array( $this, 'override_wc_logger' ) );
	...
	public function tearDown(): void {
		...
		remove_all_filters( 'transient_fincommerce_admin_' . ShippingPartnerSuggestionsDataSourcePoller::ID . '_specs' );
		update_option( 'fincommerce_default_country', 'US' );
	...
	public function test_specs_transient() {
		set_transient(
			'fincommerce_admin_' . ShippingPartnerSuggestionsDataSourcePoller::ID . '_specs',
			array(
				'en_US' => array(
					array(
						'id' => 'mock1',
	...
	public function test_matching_suggestions() {
		update_option( 'fincommerce_default_country', 'ZA' );
		$suggestions = ShippingPartnerSuggestions::get_suggestions();
	...
	public function test_suggestion_evaluations_are_logged() {
		add_filter( 'fincommerce_admin_remote_specs_evaluator_should_log', '__return_true' );
		...
		remove_filter( 'fincommerce_admin_remote_specs_evaluator_should_log', '__return_true' );
	}
