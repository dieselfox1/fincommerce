...
namespace Automattic\FinCommerce\Tests\Internal\Admin\EmailPreview;
 ...
 *
 * @covers \Automattic\FinCommerce\Internal\Admin\EmailPreview\EmailPreview
 */
class EmailPreviewTest extends WC_Unit_Test_Case {
	 ...
	 * @var string
	 */
	const DEFAULT_EMAIL_TYPE_KEY = 'fincommerce_' . EmailPreview::DEFAULT_EMAIL_ID . '_email_type';
	...
	public function setUp(): void {
		parent::setUp();
		update_option( 'fincommerce_feature_email_improvements_enabled', 'yes' );
	...
	public function tearDown(): void {
		parent::tearDown();
		update_option( 'fincommerce_feature_email_improvements_enabled', 'no' );
	}
	...
	/**
	 * Test dummy product filter - fincommerce_email_preview_dummy_product
	 */
	public function test_dummy_product_filter() {
		...
		add_filter( 'fincommerce_email_preview_dummy_product', $product_filter, 10, 1 );
		...
		remove_filter( 'fincommerce_email_preview_dummy_product', $product_filter, 10 );
	}

	/**
	 * Test dummy order filter - fincommerce_email_preview_dummy_order
	 */
	public function test_dummy_order_filter() {
		...
		add_filter( 'fincommerce_email_preview_dummy_order', $order_filter, 10, 1 );

		$content = $this->sut->render();
		$this->assertStringContainsString( '500.00', $content );
		$this->assertStringNotContainsString( '100.00', $content );

		remove_filter( 'fincommerce_email_preview_dummy_order', $order_filter, 10 );
	}

	/**
	 * Test dummy address filter - fincommerce_email_preview_dummy_address
	 */
	public function test_dummy_address_filter() {
		...
		add_filter( 'fincommerce_email_preview_dummy_address', $address_filter, 10, 1 );

		$content = $this->sut->render();
		$this->assertStringContainsString( 'Jane Smith', $content );
		$this->assertStringNotContainsString( 'John Doe', $content );

		remove_filter( 'fincommerce_email_preview_dummy_address', $address_filter, 10 );
	}

	/**
	 * Test that placeholders can be modified via `fincommerce_email_preview_placeholders`.
	 */
	public function test_placeholder_filter() {
		...
		add_filter( 'fincommerce_email_preview_placeholders', $placeholders_filter, 10, 1 );
		...
		remove_filter( 'fincommerce_email_preview_placeholders', $placeholders_filter, 10 );
	}

	/**
	 * Test that the `fincommerce_prepare_email_for_preview` filter is applied.
	 */
	public function test_prepare_email_for_preview_filter() {
		...
		add_filter( 'fincommerce_prepare_email_for_preview', $email_filter, 10, 1 );
		...
		remove_filter( 'fincommerce_prepare_email_for_preview', $email_filter, 10 );
	}
	...
	public function test_transient_values_in_subject() {
		...
		$key      = "fincommerce_{$email_id}_subject";
	...
	public function test_transient_values_in_email_content() {
		$email_id         = EmailPreview::DEFAULT_EMAIL_ID;
		$heading_key      = "fincommerce_{$email_id}_heading";
		$additional_key   = "fincommerce_{$email_id}_additional_content";
