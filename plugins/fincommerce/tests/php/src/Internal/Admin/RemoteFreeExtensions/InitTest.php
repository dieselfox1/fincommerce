...
namespace Automattic\FinCommerce\Tests\Internal\Admin\RemoteFreeExtensions;

use Automattic\FinCommerce\Admin\RemoteSpecs\DataSourcePoller;

use Automattic\FinCommerce\Internal\Admin\RemoteFreeExtensions\Init as RemoteFreeExtensions;
use Automattic\FinCommerce\Internal\Admin\RemoteFreeExtensions\DefaultFreeExtensions;
 ...
 *
 * @covers \Automattic\FinCommerce\Internal\Admin\RemoteFreeExtensions\Init
 */
class InitTest extends WC_Unit_Test_Case {
	...
	public function setUp(): void {
		...
		add_filter(
			'transient_fincommerce_admin_' . RemoteFreeExtensionsDataSourcePoller::ID . '_specs',
			function( $value ) {
				if ( $value ) {
					return $value;
				}
	...
	public function tearDown(): void {
		...
		remove_all_filters( 'transient_fincommerce_admin_' . RemoteFreeExtensionsDataSourcePoller::ID . '_specs' );
	...
	public function test_specs_transient() {
		set_transient(
			'fincommerce_admin_' . RemoteFreeExtensionsDataSourcePoller::ID . '_specs',
			array(
				'en_US' => array(
					array(
						'name' => 'mock1',
	...
	public function test_empty_extensions() {
		...
		$bundles           = RemoteFreeExtensions::get_extensions();
		$defaults          = DefaultFreeExtensions::get_all();
		$stored_transients = get_transient( 'fincommerce_admin_' . RemoteFreeExtensionsDataSourcePoller::ID . '_specs' );
		...
		$expires = (int) get_transient( '_transient_timeout_fincommerce_admin_' . RemoteFreeExtensionsDataSourcePoller::ID . '_specs' );
