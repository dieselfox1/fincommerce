...
use Automattic\FinCommerce\Admin\RemoteSpecs\RuleProcessors\EvaluateOverrides;
...
class EvaluateOverridesTest extends WC_Unit_Test_Case {
	...
	public function test_overrides_can_use_existing_rules() {
		$current_wc_country = get_option( 'fincommerce_default_country' );
		$evaluator          = new EvaluateOverrides();

		$extensions = $this->get_extensions();
		// Set default country to "CA".
		update_option( 'fincommerce_default_country', 'CA' );
		...
		update_option( 'fincommerce_default_country', $current_wc_country );
	}
