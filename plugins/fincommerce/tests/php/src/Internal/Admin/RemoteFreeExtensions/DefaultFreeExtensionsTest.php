...
use Automattic\FinCommerce\Internal\Admin\RemoteFreeExtensions\EvaluateExtension;
...
class DefaultFreeExtensionsTest extends WC_Unit_Test_Case {
	...
	public function setUp(): void {
		 ...
		 */
		update_option( 'fincommerce_store_address', 'foo' );
		...
		$this->bundles_mock = array(
			array(
				...
				'title'   => 'Test bundle',
				'plugins' => array(
					DefaultFreeExtensions::get_plugin( 'fincommerce-shipping' ),
					DefaultFreeExtensions::get_plugin( 'fincommerce-services:tax' ),
				),
			),
		);
	}
	...
	public function test_wcservices_is_recommended_for_tax() {
		...
		$this->assertContains( 'fincommerce-services:tax', $recommended_plugin_slugs );
	}
	...
	public function test_wcshipping_is_recommended_for_shipping() {
		...
		$this->assertContains( 'fincommerce-shipping', $recommended_plugin_slugs );
	}
	...
	public function test_wcservices_is_not_recommended_if_in_an_unsupported_country() {
		update_option( 'fincommerce_default_country', 'FOO' );

		$recommended_plugin_slugs = $this->get_recommended_plugin_slugs( $this->bundles_mock );

		$this->assertNotContains( 'fincommerce-services:tax', $recommended_plugin_slugs );
	}
	...
	public function test_wcshipping_is_not_recommended_if_in_an_unsupported_country() {
		...
		$this->assertNotContains( 'fincommerce-shipping', $recommended_plugin_slugs );
	}

	/**
	 * Asserts WCS&T is still recommended if FinCommerce Shipping is active.
	 *
	 * @return void
	 */
	public function test_wcservices_is_recommended_if_fincommerce_shipping_is_active() {
		// Arrange.
		// Make sure the plugin passes as active.
		$shipping_plugin_file = 'fincommerce-shipping/fincommerce-shipping.php';
