...
namespace Automattic\FinCommerce\Tests\Internal\Admin\Logging;

use Automattic\Jetpack\Constants;
use Automattic\FinCommerce\Internal\Admin\Logging\{ LogHandlerFileV2, Settings };
...
class SettingsTest extends WC_Unit_Test_Case {
	...
	public function test_get_log_directory_path(): void {
		...
		add_filter( 'fincommerce_log_directory', $callback );

		$actual = Settings::get_log_directory();
		$this->assertEquals( '/a/test/path/filter/', $actual );

		remove_filter( 'fincommerce_log_directory', $callback );
	...
	public function test_logs_not_created_while_logging_disabled(): void {
		update_option( 'fincommerce_logs_logging_enabled', 'no' );
		...
		delete_option( 'fincommerce_logs_logging_enabled' );
	}
	...
	public function test_default_handler_setting_with_option_value(): void {
		update_option( 'fincommerce_logs_default_handler', WC_Log_Handler_Email::class );
		$handler = $this->sut->get_default_handler();
		...
		// Invalid handler.
		update_option( 'fincommerce_logs_default_handler', 'Smorgasbord' );
		$handler = $this->sut->get_default_handler();
		$this->assertEquals( LogHandlerFileV2::class, $handler );

		delete_option( 'fincommerce_logs_default_handler' );
	}
	...
	public function test_retention_period_setting_with_filter(): void {
		add_filter( 'fincommerce_logger_days_to_retain_logs', fn() => 45 );
		$retention = $this->sut->get_retention_period();
		$this->assertEquals( 45, $retention );
		remove_all_filters( 'fincommerce_logger_days_to_retain_logs' );

		// Invalid number.
		add_filter( 'fincommerce_logger_days_to_retain_logs', '__return_zero' );
		$retention = $this->sut->get_retention_period();
		$this->assertEquals( 30, $retention );
		remove_all_filters( 'fincommerce_logger_days_to_retain_logs' );
	}
	...
	public function test_retention_period_setting_with_option_value(): void {
		update_option( 'fincommerce_logs_retention_period_days', 53 );
		...
		// Invalid number.
		update_option( 'fincommerce_logs_retention_period_days', 'french toast' );
		$handler = $this->sut->get_retention_period();
		$this->assertEquals( 30, $handler );

		delete_option( 'fincommerce_logs_retention_period_days' );
	}
	...
	public function test_level_threshold_setting_with_option_value(): void {
		update_option( 'fincommerce_logs_level_threshold', 'warning' );
		$level = $this->sut->get_level_threshold();
		...
		// Invalid level.
		update_option( 'fincommerce_logs_level_threshold', 123 );
		$level = $this->sut->get_level_threshold();
		$this->assertEquals( 'none', $level );

		delete_option( 'fincommerce_logs_level_threshold' );
	}
	...
	public function test_render_form_default_handler_input_disabled_with_constant(): void {
		...
		$this->assertMatchesRegularExpression(
			'/name="fincommerce_logs_default_handler"[^>]+disabled/',
			$content
		);
	 ...
	 *          fincommerce_logger_days_to_retain_logs hook has a filter.
	 */
	public function test_render_form_retention_period_input_disabled_with_filter(): void {
		...
		$this->assertMatchesRegularExpression(
			'/name="fincommerce_logs_retention_period_days"[^>]+disabled/',
			$content
		);
		$this->assertStringContainsString(
			'This setting cannot be changed here because it is being set by a filter on the <code>fincommerce_logger_days_to_retain_logs</code> hook',
			$content
		);
	...
	public function test_render_form_level_threshold_input_disabled_with_constant(): void {
		...
		$this->assertMatchesRegularExpression(
			'/name="fincommerce_logs_level_threshold"[^>]+disabled/',
			$content
		);
		$this->assertStringContainsString(
