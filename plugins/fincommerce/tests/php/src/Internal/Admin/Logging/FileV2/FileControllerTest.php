...
namespace Automattic\FinCommerce\Tests\Internal\Admin\Logging\FileV2;
...
class FileControllerTest extends WC_Unit_Test_Case {
	...
	public function test_write_to_file_needs_rotation() {
		...
		add_filter( 'fincommerce_log_file_size_limit', $filter_callback );
		...
		remove_filter( 'fincommerce_log_file_size_limit', $filter_callback );
	}
	...
	public function test_get_files_with_files(): void {
		...
		$this->assertInstanceOf( 'Automattic\\FinCommerce\\Internal\\Admin\\Logging\\FileV2\\File', $first_file );
		$second_file = array_shift( $files );
		$this->assertInstanceOf( 'Automattic\\FinCommerce\\Internal\\Admin\\Logging\\FileV2\\File', $second_file );
		...
		$first_file = array_shift( $files );
		$this->assertEquals( 'plugin-fincommerce', $first_file->get_source() );
		$second_file = array_shift( $files );
	...
	public function test_get_file_by_id() {
		...
		$this->assertInstanceOf( 'Automattic\\FinCommerce\\Internal\\Admin\\Logging\\FileV2\\File', $file );
	...
	public function test_get_file_sources(): void {
		$this->handler->handle( time(), 'debug', '1', array() ); // No source defaults to "plugin-fincommerce" as source.
		$this->handler->handle( time(), 'debug', '2', array( 'source' => 'unit-testing' ) );
		$this->handler->handle( time(), 'debug', '3', array( 'source' => 'unit-testing' ) );

		$sources = $this->sut->get_file_sources();
		$this->assertCount( 2, $sources );
		$this->assertContains( 'plugin-fincommerce', $sources );
	...
	public function test_delete_files(): void {
		...
		$files   = $this->sut->get_files( array( 'source' => 'plugin-fincommerce' ) );
