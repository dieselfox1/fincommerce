...
namespace Automattic\FinCommerce\Tests\Internal\Admin\Settings\PaymentsProviders;
...
use Automattic\FinCommerce\Internal\Admin\Suggestions\PaymentsExtensionSuggestions;
use Automattic\FinCommerce\Tests\Internal\Admin\Settings\Mocks\FakePaymentGateway;
use stdClass;
...
class PaymentGatewayTest extends WC_Unit_Test_Case {
	...
	public function test_get_details() {
		...
		$fake_gateway = new FakePaymentGateway(
			...
			array(
				...
				'plugin_slug'                 => 'fincommerce-payments',
				'plugin_file'                 => 'fincommerce-payments/fincommerce-payments.php',
		...
		$this->assertEquals(
			array(
				...
				'plugin'      => array(
					...
					'file'   => 'fincommerce-payments/fincommerce-payments',
	...
	public function test_get_title() {
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'method_title' => 'WooPayments' ) );
		$this->assertEquals( 'WooPayments', $this->sut->get_title( $fake_gateway ) );

		// Test title with HTML tags.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'method_title' => '<h1><a href="#">WooPayments</a></h1> <a href="#">Some link</a> ' ) );
		$this->assertEquals( 'WooPayments Some link', $this->sut->get_title( $fake_gateway ) );

		// Test title with encoded HTML entities.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'method_title' => htmlentities( '<h1><a href="#">WooPayments</a></h1> <a href="#">Some link</a> ' ) ) );
		$this->assertEquals( 'WooPayments Some link', $this->sut->get_title( $fake_gateway ) );
	...
	public function test_get_description() {
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'method_description' => 'Accept payments with WooPayments.' ) );
		$this->assertEquals( 'Accept payments with WooPayments.', $this->sut->get_description( $fake_gateway ) );

		// Test description with HTML tags.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'method_description' => '<a href="#">Accept</a> <b>payments</b> <strong><span>with</span> WooPayments. </strong><h1></h1> ' ) );
		$this->assertEquals( 'Accept payments with WooPayments.', $this->sut->get_description( $fake_gateway ) );

		// Test description with encoded HTML entities.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'method_description' => htmlentities( '<a href="#">Accept</a> <b>payments</b> <strong><span>with</span> WooPayments. </strong><h1></h1> ' ) ) );
		$this->assertEquals( 'Accept payments with WooPayments.', $this->sut->get_description( $fake_gateway ) );
	...
	public function test_get_icon() {
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'icon' => 'https://example.com/icon.png' ) );
		$this->assertEquals( 'https://example.com/icon.png', $this->sut->get_icon( $fake_gateway ) );

		// Test invalid URL falls back to default icon.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'icon' => 'not_good_url/icon.svg' ) );
		$this->assertStringContainsString( 'wp-content/plugins/fincommerce/assets/images/icons/default-payments.svg', $this->sut->get_icon( $fake_gateway ) );

		// Test empty icon falls back to default icon.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'icon' => '' ) );
		$this->assertStringContainsString( 'wp-content/plugins/fincommerce/assets/images/icons/default-payments.svg', $this->sut->get_icon( $fake_gateway ) );

		// Test wrong type icon falls back to default icon.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'icon' => true ) );
		$this->assertStringContainsString( 'wp-content/plugins/fincommerce/assets/images/icons/default-payments.svg', $this->sut->get_icon( $fake_gateway ) );
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'icon' => array( 'some-icon' ) ) );
		$this->assertStringContainsString( 'wp-content/plugins/fincommerce/assets/images/icons/default-payments.svg', $this->sut->get_icon( $fake_gateway ) );

		// Test missing icon falls back to default icon.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array() );
		$this->assertStringContainsString( 'wp-content/plugins/fincommerce/assets/images/icons/default-payments.svg', $this->sut->get_icon( $fake_gateway ) );

		// Test icon with img tag falls back to default icon.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'icon' => '<img src="https://example.com/icon.png" />' ) );
		$this->assertStringContainsString( 'wp-content/plugins/fincommerce/assets/images/icons/default-payments.svg', $this->sut->get_icon( $fake_gateway ) );

		// Test icon with list of images falls back to default icon.
		$fake_gateway = new FakePaymentGateway( 'fincommerce_payments', array( 'icon' => '<img src="https://example.com/icon.png" /><img src="https://example.com/icon2.png" />' ) );
		$this->assertStringContainsString( 'wp-content/plugins/fincommerce/assets/images/icons/default-payments.svg', $this->sut->get_icon( $fake_gateway ) );
	}
	...
	public function test_get_plugin_details() {
		...
		$fake_gateway = new FakePaymentGateway(
			...
			array(
				...
				'plugin_slug'    => null,
				'plugin_file'    => 'fincommerce-payments/fincommerce-payments.php',
				'class_filename' => trailingslashit( WP_PLUGIN_DIR ) . 'fincommerce-payments/some-dir/gateways/class-fake-gateway.php',
			)
		);
		$this->assertEquals(
			array(
		...
		$fake_gateway = new FakePaymentGateway(
			...
			array(
				...
				'plugin_slug'    => null,
				'plugin_file'    => 'fincommerce-payments/fincommerce-payments.php',
				'class_filename' => trailingslashit( WPMU_PLUGIN_DIR ) . 'fincommerce-payments/some-dir/gateways/class-fake-gateway.php',
			)
		);
		$this->assertEquals(
			array(
	...
	public function test_get_plugin_slug() {
		$fake_gateway = new FakePaymentGateway( 'gateway1', array( 'plugin_slug' => 'fincommerce-payments' ) );
		$this->assertEquals( 'fincommerce-payments', $this->sut->get_plugin_slug( $fake_gateway ) );
		...
		$fake_gateway = new FakePaymentGateway(
			...
			array(
				'plugin_slug'    => null,
				'class_filename' => trailingslashit( WP_PLUGIN_DIR ) . 'fincommerce-payments/class-fake-gateway.php',
			)
		);
		$this->assertEquals( 'fincommerce-payments', $this->sut->get_plugin_slug( $fake_gateway ) );
		...
		$fake_gateway = new FakePaymentGateway(
			...
			array(
				'plugin_slug'    => null,
				'class_filename' => trailingslashit( WPMU_PLUGIN_DIR ) . 'fincommerce-payments/class-fake-gateway.php',
			)
		);
		$this->assertEquals( 'fincommerce-payments', $this->sut->get_plugin_slug( $fake_gateway ) );
		...
		$fake_gateway = new FakePaymentGateway(
			...
			array(
				'plugin_slug'    => null,
				'class_filename' => '/var/www/something/fincommerce-payments/some-dir/gateways/class-fake-gateway.php',
			)
		);
		$this->assertEquals( '', $this->sut->get_plugin_slug( $fake_gateway ) );
	}
	...
	public function test_get_plugin_file() {
		$fake_gateway = new FakePaymentGateway( 'gateway1', array( 'plugin_file' => 'fincommerce-payments/fincommerce-payments.php' ) );
		$this->assertEquals( 'fincommerce-payments/fincommerce-payments', $this->sut->get_plugin_file( $fake_gateway ) );
