...
class PaymentsProvidersTest extends WC_Unit_Test_Case {
	...
	public function test_get_payment_gateway_base_details() {
		...
		$this->assertSame( 'fincommerce-payments', $gateway_details['plugin']['slug'] );
		$this->assertArrayHasKey( 'file', $gateway_details['plugin'], 'Gateway `plugin[file]` entry is missing' );
		$this->assertSame( 'fincommerce-payments/fincommerce-payments', $gateway_details['plugin']['file'] ); // No more .php extension.
	...
	public function test_get_payment_gateway_base_details_with_custom_provider_wildcard_mapping() {
		...
		update_option( 'mollie-payments-for-fincommerce_test_mode_enabled', 'yes' );
		update_option( 'mollie-payments-for-fincommerce_test_api_key', 'bogus_key' );
		...
		// Clean up.
		delete_option( 'mollie-payments-for-fincommerce_test_mode_enabled' );
		delete_option( 'mollie-payments-for-fincommerce_test_api_key' );
	}
	...
	public function test_get_payment_gateway_plugin_slug() {
		...
		// Assert.
		// The PayPal gateway is a core gateway, so the slug is 'fincommerce'.
		$this->assertSame( 'fincommerce', $slug );
	}
	...
	public function test_get_extension_suggestion_by_id() {
		...
		$suggestion_details = array(
			...
			'plugin'            => array(
				'_type' => ExtensionSuggestions::PLUGIN_TYPE_WPORG,
				'slug'  => 'fincommerce', // Use FinCommerce because it is an installed plugin, obviously.
			),
			'image'             => 'http://example.com/image1.png',
			'icon'              => 'http://example.com/icon1.png',
			'short_description' => null,
		...
		$expected_suggestion['plugin']['file']   = 'fincommerce/fincommerce'; // Ensure the file is set correctly.
	...
	public function test_get_extension_suggestion_by_plugin_slug() {
		// Arrange.
		$slug               = 'fincommerce'; // Use FinCommerce because it is an active plugin.
		$suggestion_details = array(
			'id'                => 'suggestion1',
			'_priority'         => 1,
			'_type'             => ExtensionSuggestions::TYPE_PSP,
	...
	public function test_attach_extension_suggestion_paypal( string $suggestion_id, string $received_id, string $type ) {
		...
		// Assert.
		$this->assertTrue( $result );
		$branded_option = get_option( 'fincommerce_paypal_branded' );
		...
		// Clean up.
		delete_option( 'fincommerce_paypal_branded' );
		delete_option( Payments::PAYMENTS_NOX_PROFILE_KEY );
	}
