...
class PaymentsTest extends WC_Unit_Test_Case {
	...
	public function test_get_payment_providers_only_gateways_no_suggestions() {
		...
		// All are FinCommerce core gateways.
		$gateways = array(
			new FakePaymentGateway( WC_Gateway_Paypal::ID, array( 'plugin_slug' => 'fincommerce' ) ),
			// The offline PMs.
			new FakePaymentGateway( WC_Gateway_BACS::ID, array( 'plugin_slug' => 'fincommerce' ) ),
			new FakePaymentGateway( WC_Gateway_Cheque::ID, array( 'plugin_slug' => 'fincommerce' ) ),
			new FakePaymentGateway( WC_Gateway_COD::ID, array( 'plugin_slug' => 'fincommerce' ) ),
		);
		$this->mock_providers
			->expects( $this->atLeastOnce() )
			->method( 'get_payment_gateways' )
		...
		$this->assertSame( 'fincommerce', $gateway['plugin']['slug'] );
		...
		$this->assertSame( 'fincommerce', $group['plugin']['slug'] );
	...
	public function test_get_country_with_fallback() {
		...
		update_option( 'fincommerce_default_country', $country );
		...
		// Clean up.
		delete_user_meta( $this->store_admin_id, Payments::PAYMENTS_NOX_PROFILE_KEY );
		update_option( 'fincommerce_default_country', $initial_country );
	}
