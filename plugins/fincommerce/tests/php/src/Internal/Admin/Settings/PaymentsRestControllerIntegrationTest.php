...
namespace Automattic\FinCommerce\Tests\Internal\Admin\Settings;
...
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsRestController;
use Automattic\FinCommerce\Internal\Admin\Suggestions\Incentives\Incentive;
...
class PaymentsRestControllerIntegrationTest extends WC_REST_Unit_Test_Case {
	...
	public static function wpSetUpBeforeClass(): void {
		...
		self::$initial_currency = get_fincommerce_currency();
	}
	...
	public static function wpTearDownAfterClass(): void {
		update_option( 'fincommerce_default_country', self::$initial_country );
		update_option( 'fincommerce_currency', self::$initial_currency );

		delete_option( 'fincommerce_paypal_settings' );
	}
					...
					'body'     => wp_json_encode(
						array(
							array(
								...
								'type'                => 'welcome_page',
								'cta_label'           => 'Install',
								'tc_url'              => 'https://fincommerce.com/terms-conditions',
	...
	public function tearDown(): void {
		...
		delete_option( 'fincommerce_gateway_order' );
	}
	...
	public function test_get_payment_providers_with_no_location() {
		...
		update_option( 'fincommerce_default_country', 'LI' ); // Liechtenstein.
	...
	public function test_get_payment_providers_has_all_the_data() {
		// Arrange.
		// Reset the FinCommerce gateway order.
		delete_option( 'fincommerce_gateway_order' );
		// phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		$filter_callback = fn( $caps ) => array(
			'manage_fincommerce' => true,
		...
		$this->assertSame(
			array(
				...
				'fincommerce_payments', // The fake WooPayments gateway.
		...
		$this->assertSame( 'fincommerce-payments', $suggestion['plugin']['slug'] );
		...
		$this->assertSame( 'fincommerce-payments', $provider['plugin']['slug'] );
		$this->assertArrayHasKey( 'file', $provider['plugin'], 'Provider (gateway) `plugin[file]` entry is missing' );
		$this->assertSame( 'fincommerce-payments/fincommerce-payments', $provider['plugin']['file'] ); // Skips the .php extension.
		...
		$this->assertSame( 'fincommerce', $offline_pms_group['plugin']['slug'] );
		...
		$this->assertSame( 'fincommerce', $offline_pm['plugin']['slug'] );
		$this->assertArrayHasKey( 'file', $offline_pm['plugin'], 'Offline payment method `plugin[file]` entry is missing' );
		$this->assertSame( 'fincommerce/fincommerce', $offline_pm['plugin']['file'] ); // Skips the .php extension.
