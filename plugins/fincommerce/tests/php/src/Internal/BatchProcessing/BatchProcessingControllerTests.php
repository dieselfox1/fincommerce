...
namespace Automattic\FinCommerce\Tests\Internal\BatchProcessing;
...
class BatchProcessingControllerTests extends \WC_Unit_Test_Case {
	...
	public function test_process_single_update_unfinished() {
		...
		add_filter(
			'fincommerce_get_batch_processor',
			function() use ( $test_process_mock ) {
				return $test_process_mock;
			}
		);
		$this->sut->enqueue_processor( get_class( $this->test_process ) );
		do_action( $this->sut::PROCESS_SINGLE_BATCH_ACTION_NAME, get_class( $this->test_process ) ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
