...
namespace Automattic\FinCommerce\Tests\Admin\API;

use Automattic\FinCommerce\Admin\Features\PaymentGatewaySuggestions\Init;
...
class PaymentGatewaySuggestionsTest extends WC_REST_Unit_Test_Case {
	...
	public function test_it_clears_cache_when_the_base_country_gets_updated() {
		...
		$response_mock_ref = function( $preempt, $parsed_args, $url ) {
			if ( str_contains( $url, 'https://fincommerce.com/wp-json/wccom/payment-gateway-suggestions/2.0/suggestions.json' ) ) {
				return array(
					'success' => true,
					'body'    => wp_json_encode(
						array(
		...
		update_option( 'fincommerce_default_country', 'CA:ON' );
		...
		update_option( 'fincommerce_default_country', $existing_base_country['country'] . ':' . $existing_base_country['state'] );
	}
}
