...
namespace Automattic\FinCommerce\Tests\Admin\Features\Blueprint\Exporters;
...
class ExportWCSettingsTaxTest extends WC_Unit_Test_Case {
	...
	public function test_custom_tax_classes_are_exported() {
		...
		$setting_options_mock = $this->getMockBuilder( \Automattic\FinCommerce\Admin\Features\Blueprint\SettingOptions::class )
		...
		foreach ( $steps as $step ) {
			if ( $step instanceof RunSql ) {
				...
				if ( strpos( $sql_content, 'fincommerce_tax_rates' ) !== false && strpos( $sql_content, 'TVA Réduite' ) !== false ) {
	...
	public function test_export_order_tax_classes_before_rates() {
		...
		foreach ( $steps as $index => $step ) {
			if ( $step instanceof RunSql ) {
				...
				if ( false !== strpos( $sql_content, 'fincommerce_tax_rates' ) && -1 === $tax_rate_position ) {
	...
	public function test_export_includes_all_components() {
		...
		foreach ( $steps as $step ) {
			if ( $step instanceof SetSiteOptions ) {
				...
				if ( strpos( $sql_content, 'fincommerce_tax_rates' ) !== false ) {
					$has_tax_rates = true;
				}
				if ( strpos( $sql_content, 'fincommerce_tax_rate_locations' ) !== false ) {
