...
namespace Automattic\FinCommerce\Tests\Admin\Features\Blueprint;

use Automattic\FinCommerce\Admin\Features\Blueprint\RestApi;
use WP_REST_Request;
...
class RestApiTest extends WP_Test_REST_TestCase {
			...
			array(
				'steps' => array(
					array(
						'step'     => 'setWCSettings',
						'settings' => array(
							'fincommerce_store_address' => '123 Test St',
							'fincommerce_store_city'    => 'Test City',
						),
					),
				),
			)
	...
	public function tearDown(): void {
		...
		remove_all_filters( 'pre_option_fincommerce_coming_soon' );
	}
	...
	public function test_cannot_import_blueprint_in_live_mode() {
		add_filter(
			'pre_option_fincommerce_coming_soon',
			function () {
				return 'no';
			}
		);
	...
	public function test_import_blueprint() {
		...
		$this->assertStringContainsString( 'fincommerce_store_address has been updated', $response_data['messages'][0]['message'] );
	}
}
