...
namespace Automattic\FinCommerce\Tests\Utilities;
...
class PluginUtilTests extends \WC_Unit_Test_Case {
	...
	/**
	 * @testdox 'get_fincommerce_aware_plugins' properly gets the names of all the existing FinCommerce aware plugins.
	 */
	public function test_get_all_woo_aware_plugins() {
		$result = $this->sut->get_fincommerce_aware_plugins( false );
	...
	/**
	 * @testdox 'get_fincommerce_aware_plugins' properly gets the names of the active FinCommerce aware plugins.
	 */
	public function test_get_active_woo_aware_plugins() {
		$result = $this->sut->get_fincommerce_aware_plugins( true );
	...
	public function test_get_plugin_name_with_name() {
		...
		$this->assertEquals( 'The FinCommerce aware plugin #1', $result );
	}
	...
	/**
	 * @testDox 'is_fincommerce_aware_plugin' works as expected when a plugin id (path/file.php) is passed.
	 *
	 ...
	 */
	public function test_is_fincommerce_aware_plugin_by_plugin_file( string $plugin_file, bool $expected_result ) {
		$result = $this->sut->is_fincommerce_aware_plugin( $plugin_file );
		$this->assertEquals( $expected_result, $result );
	}

	/**
	 * Data provider for test_is_fincommerce_aware_plugin_by_plugin_data.
	 *
	 * @return array[]
	 */
	public function data_provider_for_test_is_fincommerce_aware_plugin_by_plugin_data() {
		return array(
	...
	/**
	 * @testDox 'is_fincommerce_aware_plugin' works as expected when a an array of plugin data is passed.
	 *
	 * @dataProvider data_provider_for_test_is_fincommerce_aware_plugin_by_plugin_data
	 *
	 * @param array $plugin_data The plugin data to test.
	 * @param bool  $expected_result The expected result from the method.
	 */
	public function test_get_is_fincommerce_aware_plugin_by_plugin_data( array $plugin_data, bool $expected_result ) {
		$result = $this->sut->is_fincommerce_aware_plugin( $plugin_data );
		$this->assertEquals( $expected_result, $result );
	}
		...
		$this->register_legacy_proxy_function_mocks(
			array(
				'get_plugins' => function () {
					return array(
						'fincommerce/fincommerce.php' => array( 'WC tested up to' => '1.0' ),
			...
			array(
				...
				'get_plugin_data'  => function ( $plugin_name ) {
					return StringUtil::ends_with( $plugin_name, 'woo_aware_1' ) ?
						array(
							'WC tested up to' => '1.0',
							'Name'            => 'The FinCommerce aware plugin #1',
						) :
