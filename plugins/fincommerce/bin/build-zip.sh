...
# Possible slugs: fincommerce-dev (live branches) and fincommerce (plugin build)
...
pnpm --filter='@fincommerce/plugin-fincommerce' build || exit "$?"
echo "Cleaning up PHP dependencies..."
composer install --no-dev --quiet --optimize-autoloader || exit "$?"
echo "Run makepot..."
pnpm --filter=@fincommerce/plugin-fincommerce makepot || exit "$?"
echo "Syncing files..."
