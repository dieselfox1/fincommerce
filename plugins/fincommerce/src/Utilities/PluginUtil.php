...
namespace Automattic\FinCommerce\Utilities;
...
class PluginUtil {
	...
	/**
	 * The cached list of FinCommerce aware plugin ids.
	 *
	 * @var null|array
	 */
	private $fincommerce_aware_plugins = null;

	/**
	 * The cached list of enabled FinCommerce aware plugin ids.
	 *
	 * @var null|array
	 */
	private $fincommerce_aware_active_plugins = null;
	...
	public function __construct() {
		...
		$this->plugins_excluded_from_compatibility_ui = array( 'fincommerce-legacy-rest-api/fincommerce-legacy-rest-api.php' );
	}
	...
	public function get_fincommerce_aware_plugins( bool $active_only = false ): array {
		if ( is_null( $this->fincommerce_aware_plugins ) ) {
			...
			$this->fincommerce_aware_plugins =
				array_keys(
					array_filter(
						$all_plugins,
						array( $this, 'is_fincommerce_aware_plugin' )
					)
				);

			$this->fincommerce_aware_active_plugins =
				array_values(
					array_filter(
						$this->fincommerce_aware_plugins,
		...
		return $active_only ? $this->fincommerce_aware_active_plugins : $this->fincommerce_aware_plugins;
	}
	...
	/**
	 * Check if a plugin is FinCommerce aware.
	 *
	 * @param string|array $plugin_file_or_data Plugin id (path/file.php) or plugin data (as returned by get_plugins).
	 * @return bool True if the plugin exists and is FinCommerce aware.
	 * @throws \Exception The input is neither a string nor an array.
	 */
	public function is_fincommerce_aware_plugin( $plugin_file_or_data ): bool {
		if ( is_string( $plugin_file_or_data ) ) {
			return in_array( $plugin_file_or_data, $this->get_fincommerce_aware_plugins(), true );
		} elseif ( is_array( $plugin_file_or_data ) ) {
			return '' !== ( $plugin_file_or_data['WC tested up to'] ?? '' );
		} else {
			throw new \Exception( 'is_fincommerce_aware_plugin requires a plugin name or an array of plugin data as input' );
		}
	}
	...
	/**
	 * Handle plugin activation and deactivation by clearing the FinCommerce aware plugin ids cache.
	 *
	...
	public function handle_plugin_de_activation(): void {
		$this->fincommerce_aware_plugins        = null;
		$this->fincommerce_aware_active_plugins = null;
	}
	...
	public function generate_incompatible_plugin_feature_warning( string $feature_id, array $plugin_feature_info ): string {
		...
		if ( 'custom_order_tables' === $feature_id && 'yes' === get_option( 'fincommerce_api_enabled' ) ) {
			if ( is_plugin_active( 'fincommerce-legacy-rest-api/fincommerce-legacy-rest-api.php' ) ) {
				...
				sprintf(
					// translators: %s is a URL.
					__( '⚠ <b><a target="_blank" href="%s">The Legacy REST API</a> is active on this site.</b> Please be aware that the FinCommerce Legacy REST API is <b>not</b> compatible with HPOS.', 'fincommerce' ),
			 ...
			 * when both the Legacy REST API is active (via FinCommerce core or via the Legacy REST API plugin)
			 ...
			 * @since 8.9.0
			 */
			$legacy_api_and_hpos_incompatibility_warning_text = apply_filters( 'fincommerce_legacy_api_and_hpos_incompatibility_warning_text', $legacy_api_and_hpos_incompatibility_warning_text );
		...
		if ( $incompatible_count > 0 ) {
			if ( 1 === $incompatible_count ) {
				...
				$feature_warnings[] = sprintf( __( '⚠ 1 Incompatible plugin detected (%s).', 'fincommerce' ), $this->get_plugin_name( $incompatibles[0] ) );
			} elseif ( 2 === $incompatible_count ) {
				$feature_warnings[] = sprintf(
					/* translators: %1\$s, %2\$s = printable plugin names */
					__( '⚠ 2 Incompatible plugins detected (%1$s and %2$s).', 'fincommerce' ),
			...
			$feature_warnings[] = sprintf(
				...
				__( '%1$sView and manage%2$s', 'fincommerce' ),
