...
final class NumberUtil {
	 ...
	 * but in PHP 8 it throws an error. Specifically, in FinCommerce we have a few places where
	...
	public static function sanitize_cost_in_current_locale( $value ): string {
		...
		$currency_symbol_encoded    = get_fincommerce_currency_symbol();
			...
			throw new \InvalidArgumentException(
				esc_html(
					sprintf(
						...
						__( '%1$s is not a valid numeric value. Allowed characters are numbers, the thousand (%2$s), and decimal (%3$s) separators.', 'fincommerce' ),
						$value,
						wc_get_price_thousand_separator(),
						wc_get_price_decimal_separator()
					)
			...
			throw new \InvalidArgumentException(
				esc_html(
					sprintf(
						...
						__( '%s is not a valid numeric value: there should be one decimal separator and it has to be after the thousands separator.', 'fincommerce' ),
						$value
					)
				)
			);
		...
		if ( $value && ! is_numeric( $value ) ) {
			...
			throw new \InvalidArgumentException(
				esc_html(
					sprintf(
						/* translators: %s: Invalid value that was input by the user */
						__( '%s is not a valid numeric value.', 'fincommerce' ),
						$value
					)
				)
			);
