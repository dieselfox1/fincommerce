<?php
/**
 * Loads FinCommerce packages from the /packages directory. These are packages developed outside of core.
 */

declare( strict_types=1 );

namespace Automattic\FinCommerce;
...
class Packages {
	...
	/**
	 * Array of package names and their main package classes. Once a package has been merged into FinCommerce
	 * directly it should be removed from here and added to the merged packages array.
	 *
	 * @var array Key is the package name/directory, value is the main package class which handles init.
	 */
	protected static $packages = array(
		'email-editor' => '\\Automattic\\FinCommerce\\Internal\\EmailEditor\\Package',
	);

	/**
	 * Array of package names and their main package classes.
	 *
	 * One a package has been merged into FinCommerce Core it should be moved from the package list and placed in
	 ...
	 *
	 * Once a package has been merged into FinCommerce Core it should have its slug added here. This will ensure
	 ...
	 *
	 * The packages included in this array cannot be deactivated and will always load with FinCommerce core.
	 *
	 * @var array Key is the package name/directory, value is the main package class which handles init.
	 */
	protected static $base_packages = array(
		'fincommerce-admin'                    => '\\Automattic\\FinCommerce\\Admin\\Composer\\Package',
		'fincommerce-gutenberg-products-block' => '\\Automattic\\FinCommerce\\Blocks\\Package',
	);

	/**
	 * Similar to $base_packages, but
	 * the packages included in this array can be deactivated via the 'fincommerce_merged_packages' filter.
	 *
	 * @var array Key is the package name/directory, value is the main package class which handles init.
	 */
	protected static $merged_packages = array(
		'fincommerce-brands' => '\\Automattic\\FinCommerce\\Internal\\Brands',
	);
	...
	public static function init() {
		...
		// Prevent plugins already merged into FinCommerce core from getting activated as standalone plugins.
		add_action( 'activate_plugin', array( __CLASS__, 'deactivate_merged_plugins' ) );

		// Display a notice in the Plugins tab next to plugins already merged into FinCommerce core.
	...
	public static function should_load_class( $class_name ) {
		...
		foreach ( self::$merged_packages as $merged_package_name => $merged_package_class ) {
			if ( str_replace( 'fincommerce-', 'wc_', $merged_package_name ) === $class_name ) {
				return true;
			}
		}
	 ...
	 *
	 * Once a feature plugin is merged into FinCommerce Core it should be deactivated. This method will
	 ...
	 * because it will be deactivated on the request that updates or activates FinCommerce.
	 */
			...
			add_action(
				...
				function () use ( $plugin_data ) {
					...
					/* translators: %s: is referring to the plugin's name. */
						esc_html__( 'The %1$s plugin has been deactivated as the latest improvements are now included with the %2$s plugin.', 'fincommerce' ),
						'<code>' . esc_html( $plugin_data['Name'] ) . '</code>',
						'<code>FinCommerce</code>'
					);
					echo '</p></div>';
				}
			);
	...
	public static function deactivate_merged_plugins( $plugin ) {
		...
		if ( self::is_package_enabled( $plugin_dir ) ) {
			...
			wp_die(
				esc_html__( 'This plugin cannot be activated because its functionality is now included in FinCommerce core.', 'fincommerce' ),
				esc_html__( 'Plugin Activation Error', 'fincommerce' ),
				array(
					'link_url'  => esc_url( $plugins_url ),
					'link_text' => esc_html__( 'Return to the Plugins page', 'fincommerce' ),
				),
			);
		}
	}
	 ...
	 * that the plugin functionality is now offered by FinCommerce core.
	 *
	...
	public static function display_notice_for_merged_plugins( $plugin_file ) {
		...
		$notice        = __( 'This plugin can no longer be activated because its functionality is now included in <strong>FinCommerce</strong>. It is recommended to <strong>delete</strong> it.', 'fincommerce' );
	...
	protected static function initialize_packages() {
		...
		// https://github.com/dieselfox1/fincommerce/issues/28697.
		if ( is_admin() ) {
			$activated_plugin = get_transient( 'fincommerce_activated_plugin' );
			if ( $activated_plugin ) {
				delete_transient( 'fincommerce_activated_plugin' );

				/**
				 * FinCommerce is activated hook.
				 *
				 * @since 5.0.0
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log(  // phpcs:ignore
				sprintf(
					...
					esc_html__( 'Missing the FinCommerce %s package', 'fincommerce' ),
					'<code>' . esc_html( $package ) . '</code>'
				) . ' - ' . esc_html__( 'Your installation of FinCommerce is incomplete. If you installed FinCommerce from GitHub, please refer to this document to set up your development environment: https://github.com/dieselfox1/fincommerce/wiki/How-to-set-up-FinCommerce-development-environment', 'fincommerce' )
			);
		}
		add_action(
			'admin_notices',
				...
				<div class="notice notice-error">
					<p>
						...
						printf(
							...
							esc_html__( 'Your installation of FinCommerce is incomplete. If you installed FinCommerce from GitHub, %1$splease refer to this document%2$s to set up your development environment.', 'fincommerce' ),
							'<a href="' . esc_url( 'https://github.com/dieselfox1/fincommerce/wiki/How-to-set-up-FinCommerce-development-environment' ) . '" target="_blank" rel="noopener noreferrer">',
							'</a>'
						);
						?>
					</p>
