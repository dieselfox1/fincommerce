...
final class QuantityLimits {
	...
	public function get_add_to_cart_limits( \WC_Product $product, $cart_item = null ) {
		// Compatibility with the fincommerce_quantity_input_args filter. Gets initial values to match classic quantity input.
	...
	public function validate_cart_item_quantity( $quantity, $cart_item ) {
		...
		if ( ! $limits['editable'] && $quantity > $limits['maximum'] ) {
			...
			return new \WP_Error( 'readonly_quantity', sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be changed', 'fincommerce' ), $product->get_name() ) );
		}

		if ( $quantity < $limits['minimum'] ) {
			/* translators: 1: product name 2: minimum quantity */
			return new \WP_Error( 'invalid_quantity', sprintf( __( 'The minimum quantity of &quot;%1$s&quot; allowed in the cart is %2$s', 'fincommerce' ), $product->get_name(), $limits['minimum'] ) );
		}

		if ( $quantity > $limits['maximum'] ) {
			/* translators: 1: product name 2: maximum quantity */
			return new \WP_Error( 'invalid_quantity', sprintf( __( 'The maximum quantity of &quot;%1$s&quot; allowed in the cart is %2$s', 'fincommerce' ), $product->get_name(), $limits['maximum'] ) );
		}

		if ( ! $this->is_multiple_of( $quantity, NumberUtil::normalize( $limits['multiple_of'] ) ) ) {
			/* translators: 1: product name 2: multiple of */
			return new \WP_Error( 'invalid_quantity', sprintf( __( 'The quantity of &quot;%1$s&quot; must be a multiple of %2$s', 'fincommerce' ), $product->get_name(), $limits['multiple_of'] ) );
		}
	...
	protected function filter_numeric_value( $value, string $value_type, \WC_Product $product, $cart_item = null ) {
		 ...
		 * @return mixed
		 */
		$filtered_value = apply_filters( 'fincommerce_store_api_product_quantity_' . $value_type, $value, $product, $cart_item );
