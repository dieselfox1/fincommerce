...
class PaymentUtils {
	...
	/**
	 * Callback for fincommerce_payment_methods_list_item filter to add token id
	...
	public static function include_token_id_with_payment_methods( $list_item, $token ) {
		...
		$brand                = ! empty( $list_item['method']['brand'] ) ?
			...
			'';
		if ( ! empty( $brand ) && esc_html__( 'Credit card', 'fincommerce' ) !== $brand ) {
	...
	public static function get_saved_payment_methods() {
		...
		add_filter( 'fincommerce_payment_methods_list_item', [ self::class, 'include_token_id_with_payment_methods' ], 10, 2 );
		...
		remove_filter( 'fincommerce_payment_methods_list_item', [ self::class, 'include_token_id_with_payment_methods' ], 10, 2 );
