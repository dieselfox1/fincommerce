<?php
namespace Automattic\FinCommerce\StoreApi\Utilities;

use Automattic\FinCommerce\Checkout\Helpers\ReserveStock;
use Automattic\FinCommerce\Enums\ProductStatus;
use Automattic\FinCommerce\Enums\ProductType;
use Automattic\FinCommerce\StoreApi\Exceptions\InvalidCartException;
use Automattic\FinCommerce\StoreApi\Exceptions\NotPurchasableException;
use Automattic\FinCommerce\StoreApi\Exceptions\OutOfStockException;
use Automattic\FinCommerce\StoreApi\Exceptions\PartialOutOfStockException;
use Automattic\FinCommerce\StoreApi\Exceptions\RouteException;
use Automattic\FinCommerce\StoreApi\Exceptions\TooManyInCartException;
use Automattic\FinCommerce\StoreApi\Utilities\ArrayUtils;
use Automattic\FinCommerce\StoreApi\Utilities\DraftOrderTrait;
use Automattic\FinCommerce\StoreApi\Utilities\NoticeHandler;
use Automattic\FinCommerce\StoreApi\Utilities\QuantityLimits;
use WP_Error;
...
class CartController {
	...
	public function get_cart_for_response() {
		return did_action( 'fincommerce_after_calculate_totals' ) ? $this->get_cart_instance() : $this->calculate_totals();
	}
	...
	public function add_to_cart( $request ) {
		 ...
		 * @since 2.5.0
		 *
		 * @internal Matches filter name in FinCommerce core.
		 *
		 ...
		 */
		$cart->cart_contents = apply_filters( 'fincommerce_cart_contents_changed', $cart->cart_contents );

		/**
		 * Fires when an item is added to the cart.
		 *
		 * This hook fires when an item is added to the cart. This is triggered from the Store API in this context, but
		 * FinCommerce core add to cart events trigger the same hook.
		 *
		 * @since 2.5.0
		 *
		 * @internal Matches action name in FinCommerce core.
		 *
		 * @param string $cart_id ID of the item in the cart.
	...
	public function set_cart_item_quantity( $item_id, $quantity = 1 ) {
		...
		if ( empty( $cart_item ) ) {
			throw new RouteException( 'fincommerce_rest_cart_invalid_key', esc_html__( 'Cart item does not exist.', 'fincommerce' ), 409 );
		}

		$product = $cart_item['data'] ?? false;

		if ( ! $product instanceof \WC_Product ) {
			throw new RouteException( 'fincommerce_rest_cart_invalid_product', esc_html__( 'Cart item is invalid.', 'fincommerce' ), 404 );
		}
	...
	public function validate_add_to_cart( \WC_Product $product, $request ) {
		...
		if ( floatval( $request['quantity'] ) <= 0 ) {
			throw new RouteException(
				'fincommerce_rest_product_invalid_quantity',
				sprintf(
					/* translators: %s: product name */
					esc_html__( 'You cannot add &quot;%s&quot; with a quantity less than or equal to 0 to the cart.', 'fincommerce' ),
		...
		if ( ! $product->is_in_stock() ) {
			throw new RouteException(
				'fincommerce_rest_product_out_of_stock',
				sprintf(
					/* translators: %s: product name */
					esc_html__( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'fincommerce' ),
					$product->get_name()
				),
				400
			);
		...
		if ( $product->managing_stock() && ! $product->backorders_allowed() ) {
			...
			if ( $qty_remaining < $qty_in_cart + $request_quantity ) {
				throw new RouteException(
					'fincommerce_rest_product_partially_out_of_stock',
					sprintf(
						/* translators: 1: product name 2: quantity in stock */
						esc_html__( 'You cannot add that amount of &quot;%1$s&quot; to the cart because there is not enough stock (%2$s remaining).', 'fincommerce' ),
						$product->get_name(),
		...
		if ( ! $passed_validation ) {
			...
			NoticeHandler::convert_notices_to_exceptions( 'fincommerce_rest_add_to_cart_error' );
		 ...
		 * @deprecated 7.1.0 Use fincommerce_store_api_validate_add_to_cart instead.
		 */
		wc_do_deprecated_action(
			...
			'This action was deprecated in FinCommerce Blocks version 7.1.0. Please use fincommerce_store_api_validate_add_to_cart instead.'
		);
		 ...
		 * @param array       $request Add to cart request params including id, quantity, and variation attributes.
		 */
		do_action( 'fincommerce_store_api_validate_add_to_cart', $product, $request );
	}
	...
	private function get_error_message_for_stock_exception_type( $exception_type, $singular_or_plural ) {
		...
		return esc_html__( 'There was an error with an item in your cart.', 'fincommerce' );
	}
	...
	public function validate_cart() {
		 ...
		 */
		do_action( 'fincommerce_store_api_cart_errors', $cart_errors, $cart );
		...
		// Before running the fincommerce_check_cart_items hook, unhook validation from the core cart.
		remove_action( 'fincommerce_check_cart_items', array( $cart, 'check_cart_items' ), 1 );
		remove_action( 'fincommerce_check_cart_items', array( $cart, 'check_cart_coupons' ), 1 );
		...
		$cart_errors = NoticeHandler::convert_notices_to_wp_errors( 'fincommerce_rest_cart_item_error' );
	...
	public function validate_cart_items() {
		...
		if ( $error->has_errors() ) {
			...
			throw new InvalidCartException(
				'fincommerce_stock_availability_error',
				$error,
				409
			);
		}
	...
	private function stock_exceptions_to_wp_errors( $too_many_in_cart_products, $not_purchasable_products, $partial_out_of_stock_products, $out_of_stock_products ) {
		...
		if ( count( $not_purchasable_products ) > 0 ) {
			...
			$error->add(
				'fincommerce_rest_product_not_purchasable',
		...
		if ( count( $too_many_in_cart_products ) > 0 ) {
			...
			$error->add(
				'fincommerce_rest_product_too_many_in_cart',
	...
	public function validate_cart_item( $cart_item ) {
		 ...
		 * @param \WC_Product $product Product object being added to the cart.
		 * @param array       $cart_item Cart item array.
		 * @deprecated 7.1.0 Use fincommerce_store_api_validate_cart_item instead.
		 */
		wc_do_deprecated_action(
			...
			'7.1.0',
			'fincommerce_store_api_validate_cart_item',
			'This action was deprecated in FinCommerce Blocks version 7.1.0. Please use fincommerce_store_api_validate_cart_item instead.'
		);
		 ...
		 * @param array       $cart_item Cart item array.
		 */
		do_action( 'fincommerce_store_api_validate_cart_item', $product, $cart_item );
	}
	...
	public function get_cart_instance() {
		...
		if ( ! $cart || ! $cart instanceof \WC_Cart ) {
			throw new RouteException( 'fincommerce_rest_cart_error', esc_html__( 'Unable to retrieve cart.', 'fincommerce' ), 500 );
		}
	 ...
	 *
	 * @todo this can be refactored once https://github.com/dieselfox1/fincommerce/pull/26101 lands.
	 *
	 ...
	 *
	 * @param array $package Shipping package from FinCommerce.
		...
		return apply_filters(
			...
			$index > 1 ?
				sprintf(
					...
					_x( 'Shipment %d', 'shipping packages', 'fincommerce' ),
					$index
				) :
				_x( 'Shipment 1', 'shipping packages', 'fincommerce' ),
			$package['package_id'],
			$package
		);
	}
	...
	public function apply_coupon( $coupon_code ) {
		...
		if ( ! wc_is_same_coupon( $coupon->get_code(), $coupon_code ) ) {
			throw new RouteException(
				...
				sprintf(
					...
					esc_html__( '"%s" is an invalid coupon code.', 'fincommerce' ),
		...
		if ( $this->has_coupon( $coupon_code ) ) {
			throw new RouteException(
				...
				sprintf(
					/* translators: %s coupon code */
					esc_html__( 'Coupon code "%s" has already been applied.', 'fincommerce' ),
		...
		foreach ( $individual_use_coupons as $code ) {
			 ...
			 * @return boolean
			 */
			if ( false === apply_filters( 'fincommerce_apply_with_individual_use_coupon', false, $coupon, $individual_use_coupon, $applied_coupons ) ) {
				throw new RouteException(
					'fincommerce_rest_cart_coupon_error',
					sprintf(
						/* translators: %s: coupon code */
						esc_html__( '"%s" has already been applied and cannot be used in conjunction with other coupons.', 'fincommerce' ),
		...
		if ( $coupon->get_individual_use() ) {
			 ...
			 * @return array
			 */
			$coupons_to_remove = array_diff( $applied_coupons, apply_filters( 'fincommerce_apply_individual_use_coupon', array(), $coupon, $applied_coupons ) );
		...
		if ( ! $coupon->is_valid() ) {
			...
			throw new RouteException(
				...
				sprintf(
					...
					esc_html__( 'The "%1$s" coupon has been removed from your cart: %2$s', 'fincommerce' ),
					$coupon->get_code(),
	...
	protected function get_product_for_cart( $request ) {
		...
		if ( ! $product || ProductStatus::TRASH === $product->get_status() ) {
			throw new RouteException(
				'fincommerce_rest_cart_invalid_product',
				sprintf(
					/* translators: %s: product ID */
					esc_html__( 'Product with ID "%s" was not found and cannot be added to the cart.', 'fincommerce' ),
					esc_html( $request['id'] )
				),
				400
			);
	 ...
	 *
	 * @throws RouteException Exception with code fincommerce_rest_product_not_purchasable.
	 *
	 * @param \WC_Product $product Product object associated with the cart item.
	 */
	protected function throw_default_product_exception( \WC_Product $product ) {
		throw new RouteException(
			...
			sprintf(
				/* translators: %s: product name */
				esc_html__( '&quot;%s&quot; is not available for purchase.', 'fincommerce' ),
				$product->get_name()
			),
			400
		);
	...
	protected function filter_request_data( $request ) {
		...
		if ( $product->is_sold_individually() ) {
			 ...
			 * @return integer
			 */
			$request['quantity'] = apply_filters( 'fincommerce_add_to_cart_sold_individually_quantity', 1, $request['quantity'], $product_id, $variation_id, $request['cart_item_data'] );
		}
	...
	protected function parse_variation_data( $request ) {
		...
		foreach ( $variable_product_attributes as $attribute ) {
			...
			if ( isset( $request['variation'][ wc_variation_attribute_name( $attribute['name'] ) ] ) ) {
				...
				throw new RouteException(
					'fincommerce_rest_invalid_variation_data',
					sprintf(
						/* translators: %1$s: Attribute name, %2$s: Allowed values. */
						esc_html__( 'Invalid value posted for %1$s. Allowed values: %2$s', 'fincommerce' ),
		...
		if ( ! empty( $missing_attributes ) ) {
			throw new RouteException(
				'fincommerce_rest_missing_variation_data',
				esc_html__( 'Missing variation data for variable product.', 'fincommerce' ) . ' ' .
				esc_html(
					sprintf(
	...
	protected function get_variation_id_from_variation_data( $request, $product ) {
		...
		if ( empty( $variation_id ) ) {
			...
			if ( count( $selected_attributes ) < count( $required_attributes ) ) {
				throw new RouteException(
					'fincommerce_rest_missing_attributes',
					esc_html__( 'Missing attributes for variable product.', 'fincommerce' ),
					400
				);
			}
			throw new RouteException(
				'fincommerce_rest_variation_id_from_variation_data',
				esc_html__( 'No matching variation found.', 'fincommerce' ),
				400
			);
		}
	...
	protected function get_variable_product_attributes( $product ) {
		...
		if ( ! $product || ProductStatus::TRASH === $product->get_status() ) {
			throw new RouteException(
				'fincommerce_rest_cart_invalid_parent_product',
				esc_html__( 'This product cannot be added to the cart.', 'fincommerce' ),
				400
			);
		}
