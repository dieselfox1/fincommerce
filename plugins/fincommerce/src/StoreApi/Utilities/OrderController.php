...
class OrderController {
	...
	public function create_order_from_cart() {
		if ( wc()->cart->is_empty() ) {
			throw new RouteException(
				'fincommerce_rest_cart_empty',
				__( 'Cannot create order from empty cart.', 'fincommerce' ),
				400
			);
		}

		add_filter( 'fincommerce_default_order_status', array( $this, 'default_order_status' ) );
		...
		remove_filter( 'fincommerce_default_order_status', array( $this, 'default_order_status' ) );
	...
	public function update_order_from_cart( \WC_Order $order, $update_totals = true ) {
		 ...
		 *
		 * -    The method `$customer->get_taxable_address()` runs the filter `fincommerce_customer_taxable_address`.
		 ...
		 *
		 * The alternative would be to instead use `fincommerce_order_get_tax_location` to return the pickup location
		 ...
		 */
		add_filter(
			'fincommerce_order_get_tax_location',
			function ( $location ) {
	...
	/**
	 * Perform custom order validation via FinCommerce hooks.
	 *
	...
	protected function perform_custom_order_validation( \WC_Order $order ) {
		 ...
		 * @since 9.9.0
		 */
		do_action( 'fincommerce_checkout_validate_order_before_payment', $order, $validation_errors );

		// Check if there are any errors after custom validation.
		if ( $validation_errors->has_errors() ) {
			throw new RouteException(
				'fincommerce_rest_checkout_custom_validation_error',
	...
	protected function validate_coupons( \WC_Order $order, bool $use_order_data = false ) {
		...
		if ( $coupon_errors ) {
			...
			if ( $use_order_data ) {
				$error_code = 'fincommerce_rest_order_coupon_errors';
				...
				$error_code = 'fincommerce_rest_cart_coupon_errors';
			...
			if ( 1 === count( $coupon_errors ) && $use_order_data ) {
				$error_message = sprintf(
					...
					__( '"%1$s" was removed from the order. %2$s', 'fincommerce' ),
				...
				$error_message = sprintf(
					/* translators: %1$s Coupon codes, %2$s Reason */
					__( '"%1$s" was removed from the cart. %2$s', 'fincommerce' ),
					array_keys( $coupon_errors )[0],
					array_values( $coupon_errors )[0],
				);
				...
				$error_message = sprintf(
					...
					__( 'Invalid coupons were removed from the order: "%s"', 'fincommerce' ),
					implode( '", "', array_keys( $coupon_errors ) )
				);
			} else {
				$error_message = sprintf(
					/* translators: %s Coupon codes. */
					__( 'Invalid coupons were removed from the cart: "%s"', 'fincommerce' ),
					implode( '", "', array_keys( $coupon_errors ) )
				);
			}
	...
	protected function validate_email( \WC_Order $order ) {
		...
		if ( empty( $email ) ) {
			throw new RouteException(
				'fincommerce_rest_missing_email_address',
				__( 'A valid email address is required', 'fincommerce' ),
				400
			);
		}
		...
		if ( ! is_email( $email ) ) {
			throw new RouteException(
				'fincommerce_rest_invalid_email_address',
				sprintf(
					/* translators: %s provided email. */
					__( 'The provided email address (%s) is not valid—please provide a valid email address', 'fincommerce' ),
					esc_html( $email )
				),
				400
			);
	...
	protected function validate_addresses( \WC_Order $order, bool $needs_shipping ) {
		...
		if ( $needs_shipping ) {
			...
			if ( ! $selected_shipping_rates_are_all_local_pickup && ! $this->validate_allowed_country( $shipping_country, (array) wc()->countries->get_shipping_countries() ) ) {
				throw new RouteException(
					'fincommerce_rest_invalid_address_country',
					sprintf(
						/* translators: %s country code. */
						esc_html__( 'Sorry, we do not ship orders to the provided country (%s)', 'fincommerce' ),
		...
		if ( ! $this->validate_allowed_country( $billing_country, (array) wc()->countries->get_allowed_countries() ) ) {
			throw new RouteException(
				...
				sprintf(
					/* translators: %s country code. */
					esc_html__( 'Sorry, we do not allow orders from the provided country (%s)', 'fincommerce' ),
		...
		foreach ( $errors_by_code as $code => $error_messages ) {
			throw new RouteException(
				'fincommerce_rest_invalid_address',
				sprintf(
					/* translators: %s Address type. */
					esc_html__( 'There was a problem with the provided %s:', 'fincommerce' ) . ' ' . esc_html( implode( ', ', $error_messages ) ),
					'shipping' === $code ? esc_html__( 'shipping address', 'fincommerce' ) : esc_html__( 'billing address', 'fincommerce' )
				),
				400,
				array(
	...
	protected function validate_address_fields( \WC_Order $order, $address_type, \WP_Error $errors ) {
		...
		foreach ( $current_locale as $address_field_key => $address_field ) {
			...
			if ( $is_empty ) {
				...
				$errors->add( $address_type, sprintf( __( '%s is required', 'fincommerce' ), $address_field['label'] ), $address_field_key );
			}
		}
	...
	public function validate_selected_shipping_methods( $needs_shipping, $chosen_shipping_methods = array() ) {
		...
		$exception = new RouteException(
			'fincommerce_rest_invalid_shipping_option',
			__( 'Sorry, this order requires a shipping option.', 'fincommerce' ),
			400,
			array()
		);
	...
	public function validate_order_key( $order_id, $order_key ) {
		...
		if ( ! $order || ! $order_key || $order->get_id() !== $order_id || ! hash_equals( $order->get_order_key(), $order_key ) ) {
			throw new RouteException( 'fincommerce_rest_invalid_order', __( 'Invalid order ID or key provided.', 'fincommerce' ), 401 );
		}
	}
			...
			if ( ! $order->get_data_store()->get_stock_reduced( $order->get_id() ) ) {
				foreach ( $order->get_items() as $item_key => $item ) {
					if ( $item && is_callable( array( $item, 'get_product' ) ) ) {
						 ...
						 * @since 9.8.0-dev
						 */
						if ( ! apply_filters( 'fincommerce_pay_order_product_in_stock', $product->is_in_stock(), $product, $order ) ) {
							return array(
								'code'    => 'fincommerce_rest_out_of_stock',
								/* translators: %s: product name */
								'message' => sprintf( __( 'Sorry, "%s" is no longer in stock so this order cannot be paid for. We apologize for any inconvenience caused.', 'fincommerce' ), $product->get_name() ),
							);
						}
						 ...
						 * @since 9.8.0-dev
						 */
						if ( ! apply_filters( 'fincommerce_pay_order_product_has_enough_stock', ( $product->get_stock_quantity() >= ( $held_stock + $required_stock ) ), $product, $order ) ) {
							/* translators: 1: product name 2: quantity in stock */
							return array(
								'code'    => 'fincommerce_rest_out_of_stock',
								/* translators: %s: product name */
								'message' => sprintf( __( 'Sorry, we do not have enough "%1$s" in stock to fulfill your order (%2$s available). We apologize for any inconvenience caused.', 'fincommerce' ), $product->get_name(), wc_format_stock_quantity_for_display( $product->get_stock_quantity() - $held_stock, $product ) ),
							);
						}
					}
				}
