...
use Automattic\FinCommerce\StoreApi\Payments\PaymentContext;
use Automattic\FinCommerce\StoreApi\Payments\PaymentResult;
use Automattic\FinCommerce\Blocks\Domain\Services\CheckoutFieldsSchema\DocumentObject;
...
trait CheckoutTrait {
	...
	private function process_payment( \WP_REST_Request $request, PaymentResult $payment_result ) {
		try {
			 ...
			 *
			 * @hook fincommerce_rest_checkout_process_payment_with_context
			 *
			 ...
			 */
			do_action_ref_array( 'fincommerce_rest_checkout_process_payment_with_context', [ $context, &$payment_result ] );

			if ( ! $payment_result instanceof PaymentResult ) {
				throw new RouteException( 'fincommerce_rest_checkout_invalid_payment_result', __( 'Invalid payment result received from payment method.', 'fincommerce' ), 500 );
			}
		} catch ( \Exception $e ) {
			$additional_data = [];
			 ...
			 */
			if ( apply_filters( 'fincommerce_return_previous_exceptions', Constants::is_true( 'WP_DEBUG' ) ) && $e->getPrevious() ) {
				$additional_data = [
					'previous' => get_class( $e->getPrevious() ),
				];
			}

			throw new RouteException( 'fincommerce_rest_checkout_process_payment_error', esc_html( $e->getMessage() ), 400, array_map( 'esc_attr', $additional_data ) );
		}
	}
	...
	private function update_order_from_request( \WP_REST_Request $request ) {
		...
		wc_do_deprecated_action(
			'__experimental_fincommerce_blocks_checkout_update_order_from_request',
			array(
				$this->order,
				$request,
			),
			...
			'This action was deprecated in FinCommerce Blocks version 6.3.0. Please use fincommerce_store_api_checkout_update_order_from_request instead.'
		);

		wc_do_deprecated_action(
			'fincommerce_blocks_checkout_update_order_from_request',
			array(
				$this->order,
				$request,
			),
			'7.2.0',
			'fincommerce_store_api_checkout_update_order_from_request',
			'This action was deprecated in FinCommerce Blocks version 7.2.0. Please use fincommerce_store_api_checkout_update_order_from_request instead.'
		);
		 ...
		 * @param \WP_REST_Request $request Full details about the request.
		 */
		do_action( 'fincommerce_store_api_checkout_update_order_from_request', $this->order, $request );
