...
class CartAddItem extends AbstractCartRoute {
	...
	public function get_args() {
		return [
			[
				...
				'args'                => [
					'id'        => [
						'description'       => __( 'The cart item product or variation ID.', 'fincommerce' ),
						'type'              => 'integer',
						'context'           => [ 'view', 'edit' ],
						'sanitize_callback' => 'absint',
					],
					'quantity'  => [
						'description' => __( 'Quantity of this item to add to the cart.', 'fincommerce' ),
						'type'        => 'number',
						'context'     => [ 'view', 'edit' ],
						'arg_options' => [
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		...
		if ( ! empty( $request['key'] ) ) {
			throw new RouteException( 'fincommerce_rest_cart_item_exists', esc_html__( 'Cannot create an existing cart item.', 'fincommerce' ), 400 );
		}
		 ...
		 * fincommerce_add_cart_item, but this does not have access to the original Store API request like this hook does.
		 *
		 * @since 8.8.0
		 *
