...
class ProductBrandsById extends AbstractRoute {
	...
	protected function get_route_response( \WP_REST_Request $request ) {
		...
		if ( ! $object ) {
			if ( isset( $request['identifier'] ) && is_numeric( $request['identifier'] ) ) {
				throw new RouteException( 'fincommerce_rest_brand_invalid_id', esc_html__( 'Invalid brand ID.', 'fincommerce' ), 404 );
			} else {
				throw new RouteException( 'fincommerce_rest_brand_invalid_slug', esc_html__( 'Invalid brand slug.', 'fincommerce' ), 404 );
			}
		}
