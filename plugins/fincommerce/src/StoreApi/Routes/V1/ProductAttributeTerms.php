...
class ProductAttributeTerms extends AbstractTermsRoute {
	...
	public function get_args() {
		return [
			'args'   => array(
				'attribute_id' => array(
					'description' => __( 'Unique identifier for the attribute.', 'fincommerce' ),
					'type'        => 'integer',
				),
			),
			[
	...
	protected function get_route_response( \WP_REST_Request $request ) {
		...
		if ( ! $attribute || ! taxonomy_exists( $attribute->slug ) ) {
			throw new RouteException( 'fincommerce_rest_taxonomy_invalid', __( 'Attribute does not exist.', 'fincommerce' ), 404 );
		}
