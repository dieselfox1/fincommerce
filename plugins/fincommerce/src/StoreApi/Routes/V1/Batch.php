					...
					'items'    => array(
						...
						'properties' => array(
							'method'  => array(
								 ...
								 */
								'enum'    => apply_filters( '__experimental_fincommerce_store_api_batch_request_methods', array( 'POST', 'PUT', 'PATCH', 'DELETE' ) ),
								'default' => 'POST',
							),
							'path'    => array(
								'type'     => 'string',
		...
		try {
			foreach ( $request['requests'] as $args ) {
				if ( ! stristr( $args['path'], 'wc/store' ) ) {
					throw new RouteException( 'fincommerce_rest_invalid_path', __( 'Invalid path provided.', 'fincommerce' ), 400 );
				}
			}
