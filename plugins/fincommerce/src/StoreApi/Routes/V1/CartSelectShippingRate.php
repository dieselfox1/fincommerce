...
class CartSelectShippingRate extends AbstractCartRoute {
	...
	public function get_args() {
		return [
			[
				...
				'args'                => [
					'package_id' => array(
						'description' => __( 'The ID of the package being shipped. Leave blank to apply to all packages.', 'fincommerce' ),
						'type'        => [ 'integer', 'string', 'null' ],
						'required'    => false,
					),
					'rate_id'    => [
						'description' => __( 'The chosen rate ID for the package.', 'fincommerce' ),
						'type'        => 'string',
						'required'    => true,
					],
				],
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		if ( ! wc_shipping_enabled() ) {
			throw new RouteException( 'fincommerce_rest_shipping_disabled', esc_html__( 'Shipping is disabled.', 'fincommerce' ), 404 );
		}

		if ( ! isset( $request['rate_id'] ) ) {
			throw new RouteException( 'fincommerce_rest_cart_missing_rate_id', esc_html__( 'Invalid Rate ID.', 'fincommerce' ), 400 );
		}
		 ...
		 */
		do_action( 'fincommerce_store_api_cart_select_shipping_rate', $package_id, $rate_id, $request );
