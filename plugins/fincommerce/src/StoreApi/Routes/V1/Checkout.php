...
namespace Automattic\FinCommerce\StoreApi\Routes\V1;

use Automattic\FinCommerce\StoreApi\Payments\PaymentResult;
...
use Automattic\FinCommerce\Checkout\Helpers\ReserveStockException;
use Automattic\FinCommerce\StoreApi\Utilities\CheckoutTrait;
...
class Checkout extends AbstractCartRoute {
		...
		return [
			...
			[
				...
				'args'                => array_merge(
					[
						'payment_data'      => [
							'description' => __( 'Data to pass through to the payment method when processing payment.', 'fincommerce' ),
							'type'        => 'array',
							'items'       => [
								'type'       => 'object',
								'properties' => [
						...
						'customer_password' => [
							'description' => __( 'Customer password for new accounts, if applicable.', 'fincommerce' ),
							'type'        => 'string',
						],
					],
			...
			[
				...
				'args'                => array_merge(
					[
						'additional_fields' => [
							'description' => __( 'Additional fields related to the order.', 'fincommerce' ),
							'type'        => 'object',
						],
						'payment_method'    => [
							'description' => __( 'Selected payment method for the order.', 'fincommerce' ),
							'type'        => 'string',
						],
						'order_notes'       => [
							'description' => __( 'Order notes.', 'fincommerce' ),
							'type'        => 'string',
						],
					],
	...
	public function get_response( \WP_REST_Request $request ) {
		...
		if ( ! $response ) {
			try {
				...
				$response = $this->get_route_error_response( 'fincommerce_rest_unknown_server_error', $error->getMessage(), 500 );
			}
		}
	...
	public function validate_callback( $request ) {
			...
			foreach ( $additional_fields as $field_key => $field ) {
				...
				if ( empty( $field_value ) ) {
					if ( true === $field['required'] ) {
						...
						$error_message = sprintf( __( '%s is required', 'fincommerce' ), $field['label'] );
						if ( 'shipping_address' === $context ) {
							/* translators: %s: is the field error message */
							$error_message = sprintf( __( 'There was a problem with the provided shipping address: %s', 'fincommerce' ), $error_message );
						} elseif ( 'billing_address' === $context ) {
							/* translators: %s: is the field error message */
							$error_message = sprintf( __( 'There was a problem with the provided billing address: %s', 'fincommerce' ), $error_message );
						}
						$errors->add( 'fincommerce_required_checkout_field', $error_message, [ 'key' => $field_key ] );
					}
					continue;
				}
		...
		if ( $invalid_groups ) {
			return new \WP_Error(
				...
				esc_html( sprintf( __( 'Invalid parameter(s): %s', 'fincommerce' ), implode( ', ', array_keys( $invalid_groups ) ) ) ),
				array(
					'status'  => 400,
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		...
		try {
			...
			throw new RouteException(
				'fincommerce_rest_coupon_reserve_failed',
		 ...
		 * `fincommerce_checkout_order_created` action. This in turn would trigger the `wc_reserve_stock_for_order`
		 ...
		 *
		 * @since 9.2 Stock is no longer held for all draft orders, nor on non-POST requests. See https://github.com/dieselfox1/fincommerce/issues/44231
		...
		wc_do_deprecated_action(
			'__experimental_fincommerce_blocks_checkout_order_processed',
			array(
				$this->order,
			),
			'6.3.0',
			'fincommerce_store_api_checkout_order_processed',
			'This action was deprecated in FinCommerce Blocks version 6.3.0. Please use fincommerce_store_api_checkout_order_processed instead.'
		);

		wc_do_deprecated_action(
			'fincommerce_blocks_checkout_order_processed',
			array(
				$this->order,
			),
			'7.2.0',
			'fincommerce_store_api_checkout_order_processed',
			'This action was deprecated in FinCommerce Blocks version 7.2.0. Please use fincommerce_store_api_checkout_order_processed instead.'
		);

		// Set the order status to 'pending' as an initial step.
		// This allows the order to proceed towards completion. The hook
		// 'fincommerce_store_api_checkout_order_processed' (fired below) can be used
		 ...
		 *
		 * This is similar to existing core hook fincommerce_checkout_order_processed. We're using a new action:
		 * - To keep the interface focused (only pass $order, not passing request data).
		 * - This also explicitly indicates these orders are from checkout block/StoreAPI.
		 *
		 * @since 7.2.0
		 *
		 * @see https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/3238
		 * @example See docs/examples/checkout-order-processed.md

		 * @param \WC_Order $order Order object.
		 */
		do_action( 'fincommerce_store_api_checkout_order_processed', $this->order );
	...
	private function create_or_update_draft_order( \WP_REST_Request $request ) {
		...
		wc_do_deprecated_action(
			'__experimental_fincommerce_blocks_checkout_update_order_meta',
			array(
				$this->order,
			),
			'6.3.0',
			'fincommerce_store_api_checkout_update_order_meta',
			'This action was deprecated in FinCommerce Blocks version 6.3.0. Please use fincommerce_store_api_checkout_update_order_meta instead.'
		);

		wc_do_deprecated_action(
			'fincommerce_blocks_checkout_update_order_meta',
			array(
				$this->order,
			),
			'7.2.0',
			'fincommerce_store_api_checkout_update_order_meta',
			'This action was deprecated in FinCommerce Blocks version 7.2.0. Please use fincommerce_store_api_checkout_update_order_meta instead.'
		);
		 ...
		 *
		 * This is similar to existing core hook fincommerce_checkout_update_order_meta.
		 ...
		 * @since 7.2.0
		 *
		 * @see https://github.com/dieselfox1/fincommerce-gutenberg-products-block/pull/3686
		 *
		 * @param \WC_Order $order Order object.
		 */
		do_action( 'fincommerce_store_api_checkout_update_order_meta', $this->order );

		// Confirm order is valid before proceeding further.
		if ( ! $this->order instanceof \WC_Order ) {
			throw new RouteException(
				'fincommerce_rest_checkout_missing_order',
				esc_html__( 'Unable to create order', 'fincommerce' ),
				500
			);
		}
	...
	private function update_customer_from_request( \WP_REST_Request $request ) {
		 ...
		 * @param \WP_REST_Request $request Full details about the request.
		 */
		do_action( 'fincommerce_store_api_checkout_update_customer_from_request', $customer, $request );
	...
	private function get_request_payment_method( \WP_REST_Request $request ) {
		...
		if ( empty( $request_payment_method ) ) {
			if ( $requires_payment_method ) {
				throw new RouteException(
					'fincommerce_rest_checkout_missing_payment_method',
					esc_html__( 'No payment method provided.', 'fincommerce' ),
					400
				);
			}
			return null;
		...
		if ( ! isset( $available_gateways[ $request_payment_method ] ) ) {
			...
			throw new RouteException(
				'fincommerce_rest_checkout_payment_method_disabled',
				sprintf(
					// Translators: %s Payment method ID.
					esc_html__( '%s is not available for this order—please choose a different payment method', 'fincommerce' ),
	...
	/**
	 * This validates if the order can be placed regarding settings in FinCommerce > Settings > Accounts & Privacy
		...
		if (
			...
			throw new RouteException(
				...
				esc_html(
					 ...
					 */
					apply_filters(
						'fincommerce_checkout_must_be_logged_in_message',
						__( 'You must be logged in to checkout.', 'fincommerce' )
					)
				),
				403
			);
