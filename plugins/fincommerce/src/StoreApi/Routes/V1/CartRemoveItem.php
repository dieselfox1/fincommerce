...
class CartRemoveItem extends AbstractCartRoute {
	...
	public function get_args() {
		return [
			[
				...
				'args'                => [
					'key' => [
						'description' => __( 'Unique identifier (key) for the cart item.', 'fincommerce' ),
						'type'        => 'string',
					],
				],
			],
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		...
		if ( empty( $cart_item ) ) {
			throw new RouteException( 'fincommerce_rest_cart_invalid_key', __( 'Cart item no longer exists or is invalid.', 'fincommerce' ), 409 );
		}
