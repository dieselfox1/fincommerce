...
use Automattic\FinCommerce\StoreApi\Exceptions\InvalidStockLevelsInCartException;
use Automattic\FinCommerce\StoreApi\Exceptions\RouteException;
use Automattic\FinCommerce\StoreApi\Utilities\OrderAuthorizationTrait;
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		...
		if ( ! $this->order || ! $this->order->needs_payment() ) {
			return new \WP_Error(
				...
				__( 'This order cannot be paid for.', 'fincommerce' )
			);
		}
		...
		if ( empty( $request_payment_method ) ) {
			if ( $requires_payment_method ) {
				throw new RouteException(
					...
					__( 'No payment method provided.', 'fincommerce' ),
					400
				);
			}
			return null;
		...
		if ( ! isset( $available_gateways[ $request_payment_method ] ) ) {
			throw new RouteException(
				...
				sprintf(
					...
					__( 'The %s payment gateway is not available.', 'fincommerce' ),
