...
use Automattic\FinCommerce\StoreApi\Utilities\CartTokenUtils;
...
abstract class AbstractCartRoute extends AbstractRoute {
	...
	protected function cart_updated( \WP_REST_Request $request ) {
		...
		if ( $draft_order ) {
			...
			wc_do_deprecated_action(
				'fincommerce_blocks_cart_update_order_from_request',
				array(
					$draft_order,
					$request,
				),
				...
				'This action was deprecated in FinCommerce Blocks version 7.2.0. Please use fincommerce_store_api_cart_update_order_from_request instead.'
			);
			 ...
			 */
			do_action( 'fincommerce_store_api_cart_update_order_from_request', $draft_order, $request );
		}
	}
	...
	protected function check_nonce( \WP_REST_Request $request ) {
		 ...
		 * @return boolean
		 */
		if ( apply_filters( 'fincommerce_store_api_disable_nonce_check', false ) ) {
			return true;
		}

		if ( null === $nonce ) {
			return $this->get_route_error_response( 'fincommerce_rest_missing_nonce', __( 'Missing the Nonce header. This endpoint requires a valid nonce.', 'fincommerce' ), 401 );
		}

		if ( ! wp_verify_nonce( $nonce, 'wc_store_api' ) ) {
			return $this->get_route_error_response( 'fincommerce_rest_invalid_nonce', __( 'Nonce is invalid.', 'fincommerce' ), 403 );
		}
