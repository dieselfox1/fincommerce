...
class CartUpdateCustomer extends AbstractCartRoute {
	...
	protected function validate_address_params( $request, $billing, $shipping ) {
		...
		if ( $posted_billing ) {
			...
			if ( false === $billing_validation_check ) {
				$invalid_params['billing_address'] = __( 'Invalid parameter.', 'fincommerce' );
		...
		if ( $posted_shipping ) {
			...
			if ( false === $shipping_validation_check ) {
				$invalid_params['shipping_address'] = __( 'Invalid parameter.', 'fincommerce' );
		...
		if ( $invalid_params ) {
			return new \WP_Error(
				...
				sprintf( __( 'Invalid parameter(s): %s', 'fincommerce' ), implode( ', ', array_keys( $invalid_params ) ) ),
				[
					'status'  => 400,
	...
	protected function get_route_post_response( \WP_REST_Request $request ) {
		...
		wc_do_deprecated_action(
			'fincommerce_blocks_cart_update_customer_from_request',
			array(
				$customer,
				$request,
			),
			...
			'This action was deprecated in FinCommerce Blocks version 7.2.0. Please use fincommerce_store_api_cart_update_customer_from_request instead.'
		);
		 ...
		 */
		do_action( 'fincommerce_store_api_cart_update_customer_from_request', $customer, $request );
