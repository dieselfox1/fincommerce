...
use Automattic\FinCommerce\StoreApi\Utilities\ProductItemTrait;
...
class CartItemSchema extends ItemSchema {
	...
	/**
	 * Convert a FinCommerce cart item to an object suitable for the response.
	 *
	...
	public function get_item_response( $cart_item ) {
		 ...
		 */
		$product_permalink = apply_filters( 'fincommerce_cart_item_permalink', $product->get_permalink(), $cart_item, $cart_item['key'] );
		...
		return [
			...
			'prices'               => (object) $this->prepare_product_price_response( $product, get_option( 'fincommerce_tax_display_cart' ) ),
	...
	protected function get_cart_images( \WC_Product $product, array $cart_item, string $cart_item_key ) {
		 ...
		 * @since 9.6.0
		 */
		$filtered_images = apply_filters( 'fincommerce_store_api_cart_item_images', $product_images, $cart_item, $cart_item_key );
		...
		foreach ( $filtered_images as $image ) {
			...
			if ( ! isset( $image->id ) ) {
				$logger->warning( 'After passing through fincommerce_cart_item_images filter, one of the images did not have an id property.' );
				continue;
			}

			// Check if thumbnail is a valid url.
			if ( empty( $image->thumbnail ) || ! filter_var( $image->thumbnail, FILTER_VALIDATE_URL ) ) {
				$logger->warning( sprintf( 'After passing through fincommerce_cart_item_images filter, image with id %s did not have a valid thumbnail property.', $image->id ) );
				continue;
			}

			// Check if src is a valid url.
			if ( empty( $image->src ) || ! filter_var( $image->src, FILTER_VALIDATE_URL ) ) {
				$logger->warning( sprintf( 'After passing through fincommerce_cart_item_images filter, image with id %s did not have a valid src property.', $image->id ) );
				continue;
			}
	...
	protected function get_item_data( $cart_item ) {
		 ...
		 * @return array
		 */
		$item_data       = apply_filters( 'fincommerce_get_item_data', array(), $cart_item );
		$clean_item_data = [];
	...
	/**
	 * Remove HTML tags from cart item data and set the `hidden` property to `__experimental_fincommerce_blocks_hidden`.
	 *
	...
	protected function format_item_data_element( $item_data_element ) {
		if ( array_key_exists( '__experimental_fincommerce_blocks_hidden', $item_data_element ) ) {
			$item_data_element['hidden'] = $item_data_element['__experimental_fincommerce_blocks_hidden'];
		}
