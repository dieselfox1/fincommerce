...
class BillingAddressSchema extends AbstractAddressSchema {
	...
	public function get_properties() {
		...
		return array_merge(
			...
			[
				'email' => [
					'description' => __( 'Email', 'fincommerce' ),
					'type'        => 'string',
					'context'     => [ 'view', 'edit' ],
					'required'    => true,
				],
	...
	public function validate_callback( $address, $request, $param ) {
		...
		if ( ! empty( $address['email'] ) && ! is_email( $address['email'] ) ) {
			$errors->add(
				'invalid_email',
				__( 'The provided email address is not valid', 'fincommerce' )
			);
		}
	...
	public function get_item_response( $address ) {
		...
		throw new RouteException(
			...
			sprintf(
				...
				__( '%1$s requires an instance of %2$s or %3$s for the address', 'fincommerce' ),
