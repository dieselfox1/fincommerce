...
class ProductCategorySchema extends TermSchema {
	...
	public function get_properties() {
		...
		$schema['image']        = [
			'description' => __( 'Category image.', 'fincommerce' ),
			'type'        => 'object',
		...
		$schema['review_count'] = [
			'description' => __( 'Number of reviews for products in this category.', 'fincommerce' ),
			'type'        => 'integer',
			'context'     => [ 'view', 'edit' ],
			'readonly'    => true,
		];
		$schema['permalink']    = [
			'description' => __( 'Category URL.', 'fincommerce' ),
			'type'        => 'string',
			'format'      => 'uri',
			'context'     => [ 'view', 'edit', 'embed' ],
			'readonly'    => true,
	...
	protected function get_category_review_count( $term ) {
		...
		$review_count = $wpdb->get_var( $products_of_category_sql ); // phpcs:ignore finpress.DB.PreparedSQL.NotPrepared
