...
use Automattic\FinCommerce\StoreApi\Utilities\ValidationUtils;
...
abstract class AbstractAddressSchema extends AbstractSchema {
	...
	public function get_properties() {
		return array_merge(
			[
				...
				'company'    => [
					'description' => __( 'Company', 'fincommerce' ),
					'type'        => 'string',
					'context'     => [ 'view', 'edit' ],
					'required'    => true,
				],
				'address_1'  => [
					'description' => __( 'Address', 'fincommerce' ),
					'type'        => 'string',
					'context'     => [ 'view', 'edit' ],
					'required'    => true,
				],
				'address_2'  => [
					'description' => __( 'Apartment, suite, etc.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => [ 'view', 'edit' ],
					'required'    => true,
				],
				...
				'state'      => [
					'description' => __( 'State/County code, or name of the state, county, province, or district.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => [ 'view', 'edit' ],
					'required'    => true,
				],
				...
				'country'    => [
					'description' => __( 'Country/Region code in ISO 3166-1 alpha-2 format.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => [ 'view', 'edit' ],
					'required'    => true,
				],
				'phone'      => [
					'description' => __( 'Phone', 'fincommerce' ),
					'type'        => 'string',
					'context'     => [ 'view', 'edit' ],
					'required'    => true,
				],
	...
	public function validate_callback( $address, $request, $param ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $value, $schema[ $key ], $key ) ) ) {
				$errors->add(
					...
					sprintf(
						...
						__( 'Invalid %s provided.', 'fincommerce' ),
						$key
					)
				);
			}
		...
		if ( ! empty( $address['country'] ) && ! in_array( $address['country'], array_keys( wc()->countries->get_countries() ), true ) ) {
			$errors->add(
				...
				sprintf(
					...
					__( 'Invalid country code provided. Must be one of: %s', 'fincommerce' ),
		...
		if ( ! empty( $address['state'] ) && ! $validation_util->validate_state( $address['state'], $address['country'] ) ) {
			$errors->add(
				...
				sprintf(
					...
					__( 'The provided state (%1$s) is not valid. Must be one of: %2$s', 'fincommerce' ),
		...
		if ( ! empty( $address['postcode'] ) && ! \WC_Validation::is_postcode( $address['postcode'], $address['country'] ) ) {
			$errors->add(
				'invalid_postcode',
				__( 'The provided postcode / ZIP is not valid', 'fincommerce' )
			);
		}
		...
		if ( ! empty( $address['phone'] ) ) {
			...
			if ( ! \WC_Validation::is_phone( $address['phone'] ) ) {
				$errors->add(
					'invalid_phone',
					__( 'The provided phone number is not valid', 'fincommerce' )
				);
			}
		}
