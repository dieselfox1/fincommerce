<?php
namespace Automattic\FinCommerce\StoreApi\Schemas;
...
use Automattic\FinCommerce\StoreApi\Schemas\V1\ProductSchema;
 ...
 *
 * @see fincommerce_store_api_register_endpoint_data()
 * @see fincommerce_store_api_register_update_callback()
 * @see fincommerce_store_api_register_payment_requirements()
 * @see fincommerce_store_api_get_formatter()
 */
final class ExtendSchema {
	...
	private function throw_exception( $exception_or_error ) {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && current_user_can( 'manage_fincommerce' ) ) {
			throw $exception;
		}
	}
	...
	private function format_extensions_properties( $namespace, $schema, $schema_type ) {
		if ( ARRAY_N === $schema_type ) {
			return [
				...
				'description' => sprintf( __( 'Extension data registered by %s', 'fincommerce' ), $namespace ),
				'type'        => [ 'array', 'null' ],
				'context'     => [ 'view', 'edit' ],
				'items'       => $schema,
			];
