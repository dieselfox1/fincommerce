<?php
namespace Automattic\FinCommerce\StoreApi;
...
class Legacy {
	...
	public function init() {
		add_action( 'fincommerce_rest_checkout_process_payment_with_context', array( $this, 'process_legacy_payment' ), 999, 2 );
	}

	/**
	 * Attempt to process a payment for the checkout API if no payment methods support the
	 * fincommerce_rest_checkout_process_payment_with_context action.
	 *
	...
	public function process_legacy_payment( PaymentContext $context, PaymentResult &$result ) {
		...
		NoticeHandler::convert_notices_to_exceptions( 'fincommerce_rest_payment_error' );
		...
		if ( isset( $gateway_result['result'] ) && 'failure' === $gateway_result['result'] ) {
			if ( isset( $gateway_result['message'] ) ) {
				throw new RouteException( 'fincommerce_rest_payment_error', esc_html( wp_strip_all_tags( $gateway_result['message'] ) ), 400 );
			} else {
				NoticeHandler::convert_notices_to_exceptions( 'fincommerce_rest_payment_error' );
			}
		}
