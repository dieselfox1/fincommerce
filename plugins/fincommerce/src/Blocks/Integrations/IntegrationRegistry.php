<?php
namespace Automattic\FinCommerce\Blocks\Integrations;
...
class IntegrationRegistry {
	...
	public function initialize( $registry_identifier = '' ) {
		...
		if ( empty( $this->registry_identifier ) ) {
			_doing_it_wrong( __METHOD__, esc_html__( 'Integration registry requires an identifier.', 'fincommerce' ), '4.6.0' );
			return false;
		}
		 ...
		 */
		do_action( 'fincommerce_blocks_' . $this->registry_identifier . '_registration', $this );
	...
	public function register( IntegrationInterface $integration ) {
		...
		if ( $this->is_registered( $name ) ) {
			...
			_doing_it_wrong( __METHOD__, esc_html( sprintf( __( '"%s" is already registered.', 'fincommerce' ), $name ) ), '4.6.0' );
			return false;
		}
	...
	public function unregister( $name ) {
		...
		if ( ! $this->is_registered( $name ) ) {
			/* translators: %s: Integration name. */
			_doing_it_wrong( __METHOD__, esc_html( sprintf( __( 'Integration "%s" is not registered.', 'fincommerce' ), $name ) ), '4.6.0' );
			return false;
		}
