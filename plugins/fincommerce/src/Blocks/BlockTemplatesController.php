...
class BlockTemplatesController {
	...
	public function add_plugin_templates_parts_support( $settings, $metadata ) {
		if (
			...
			$settings['render_callback'] = array( $this, 'render_fincommerce_template_part' );
		}
		return $settings;
	}
	...
	public function prevent_shortcodes_html_breakage( $settings, $metadata ) {
		if (
			...
			$settings['render_callback']          = function ( $attributes, $content ) use ( $settings ) {
				...
				if ( strstr( $content, 'fincommerce-cart-form' ) || strstr( $content, 'wc-empty-cart-message' ) || strstr( $content, 'fincommerce-checkout-form' ) ) {
	...
	/**
	 * This function checks if there's a block template file in `fincommerce/templates/templates/`
	...
	public function get_block_file_template( $template, $id, $template_type ) {
		...
		// If we are not dealing with a FinCommerce template let's return early and let it continue through the process.
	...
	/**
	 * Add the template title and description to FinCommerce templates.
	 *
	...
	/**
	 * Add the block template objects currently saved in the database with the FinCommerce slug.
	 * That is, templates that have been customised before FinCommerce started to use the
	...
	public function add_db_templates_with_woo_slug( $query_result, $query, $template_type ) {
		...
		if ( count( $new_templates ) > 0 ) {
			...
			// See: https://github.com/dieselfox1/fincommerce/issues/42220.
	...
	/**
	 * When creating a template from the WP suggestion, don't load the templates with the FinCommerce slug.
	...
	/**
	 * Gets the templates from the FinCommerce blocks directory, skipping those for which a template already exists
	 ...
	 * @param string   $template_type wp_template or wp_template_part.
	 *
	 * @return array Templates from the FinCommerce blocks plugin directory.
	 */
	public function get_block_templates_from_fincommerce( $slugs, $already_found_templates, $template_type = 'wp_template' ) {
	...
	public function get_block_templates( $slugs = array(), $template_type = 'wp_template' ) {
		...
		$templates_from_woo = $this->get_block_templates_from_fincommerce( $slugs, $templates_from_db, $template_type );
