...
class BlockPatterns {
	...
	public function register_block_patterns() {
		...
		foreach ( $patterns as $pattern ) {
			 ...
			 *
			 * Remove the backward compatibility logic in the FinCommerce 10.1 lifecycle: https://github.com/dieselfox1/fincommerce/issues/57354.
			 */
	...
	private function get_pattern_cache() {
		$pattern_data = get_site_transient( 'fincommerce_blocks_patterns' );

		if ( is_array( $pattern_data ) && fincommerce_VERSION === $pattern_data['version'] ) {
	...
	private function set_pattern_cache( array $patterns ) {
		...
		set_site_transient( 'fincommerce_blocks_patterns', $pattern_data, MONTH_IN_SECONDS );
	}
		...
		if ( empty( $patterns ) || ! is_array( $patterns ) ) {
			...
			if ( ! get_transient( $transient_key ) && ! call_user_func( $has_scheduled_action, 'fetch_patterns' ) ) {
				wc_get_logger()->warning(
					__( 'Empty patterns received from the PTK Pattern Store', 'fincommerce' ),
				);
	...
	/**
	 * Parse prefixed categories from the PTK patterns into the actual FinCommerce categories.
	 *
