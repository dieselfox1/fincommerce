...
class Installer {
	...
	public function init() {
		...
		add_filter( 'fincommerce_create_pages', array( $this, 'create_pages' ) );
	}
	 ...
	 * We check for shortcode as default because after FinCommerce 8.3, block-based checkout is used by default.
	 * This only runs on Tools > Create Pages as the filter is not applied on FinCommerce plugin activation.
	 *
	...
	public function create_pages( $pages ) {
		...
		if ( '<!-- wp:shortcode -->[fincommerce_cart]<!-- /wp:shortcode -->' === ( $pages['cart']['content'] ?? null ) ) {
			$pages['cart']['content'] = '<!-- wp:fincommerce/classic-shortcode {"shortcode":"cart"} /-->';
		}

		if ( '<!-- wp:shortcode -->[fincommerce_checkout]<!-- /wp:shortcode -->' === ( $pages['checkout']['content'] ?? null ) ) {
			$pages['checkout']['content'] = '<!-- wp:fincommerce/classic-shortcode {"shortcode":"checkout"} /-->';
		}
		...
		add_action(
			...
			function() use ( $table_name ) {
				...
				printf(
					...
					esc_html__( 'FinCommerce %1$s table creation failed. Does the %2$s user have CREATE privileges on the %3$s database?', 'fincommerce' ),
