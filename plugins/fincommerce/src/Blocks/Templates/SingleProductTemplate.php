...
use Automattic\FinCommerce\Blocks\Utils\ProductDataUtils;
...
class SingleProductTemplate extends AbstractTemplate {
	...
	public function get_template_title() {
		return _x( 'Single Product', 'Template name', 'fincommerce' );
	}
	...
	public function get_template_description() {
		return __( 'Displays a single product.', 'fincommerce' );
	}
	...
	public function render_block_template() {
		if ( ! is_embed() && is_singular( 'product' ) ) {
			...
			if ( isset( $template ) && BlockTemplateUtils::template_has_legacy_template_block( $template ) ) {
				add_filter( 'fincommerce_disable_compatibility_layer', '__return_true' );
			}
		...
		$query_result = array_map(
			function ( $template ) {
				if ( str_contains( $template->slug, self::SLUG ) ) {
					...
					// More details: https://github.com/dieselfox1/fincommerce-blocks/issues/9662.
	...
	private static function replace_first_single_product_template_block_with_password_form( $parsed_blocks, $is_already_replaced ) {
		...
		$single_product_template_blocks = array(
			'fincommerce/product-image-gallery',
		...
		return array_reduce(
			...
			function ( $carry, $block ) use ( $single_product_template_blocks ) {
				if ( in_array( $block['blockName'], $single_product_template_blocks, true ) || ( 'core/pattern' === $block['blockName'] && isset( $block['attrs']['slug'] ) && 'fincommerce-blocks/related-products' === $block['attrs']['slug'] ) ) {
