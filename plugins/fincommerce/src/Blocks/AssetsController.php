...
final class AssetsController {
	...
	private function get_prefetch_resource_hints() {
		...
		$has_block_cart     = $cart_page_id && has_block( 'fincommerce/cart', $cart_page_id );
		$has_block_checkout = $checkout_page_id && has_block( 'fincommerce/checkout', $checkout_page_id );

		// Checks the current page to see if it contains the named block.
		$is_block_cart     = has_block( 'fincommerce/cart' );
		$is_block_checkout = has_block( 'fincommerce/checkout' );
	...
	private function get_block_asset_resource_hints_cache() {
		...
		$cache = get_site_transient( 'fincommerce_block_asset_resource_hints' );
	...
	private function set_block_asset_resource_hints_cache( $filename, $data ) {
		...
		set_site_transient( 'fincommerce_block_asset_resource_hints', $updated, WEEK_IN_SECONDS );
	}
	...
	protected function get_file_version( $file ) {
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG && file_exists( \Automattic\FinCommerce\Blocks\Package::get_path() . $file ) ) {
			return filemtime( \Automattic\FinCommerce\Blocks\Package::get_path() . $file );
		}
	...
	public function update_block_style_dependencies() {
		...
		// In WC < 5.5, `fincommerce-general` is not registered in block editor
		// screens, so we don't add it as a dependency if it's not registered.
		// In WC >= 5.5, `fincommerce-general` is registered on `admin_enqueue_scripts`,
		// so we need to check if it's registered here instead of on `init`.
		if (
			wp_style_is( 'fincommerce-general', 'registered' ) &&
			! in_array( 'fincommerce-general', $style->deps, true )
		) {
			$style->deps[] = 'fincommerce-general';
		}
	}
	 ...
	 *
	 * @see https://github.com/dieselfox1/fincommerce-gutenberg-products-block/issues/5052
	 */
