...
class ProductQuery extends AbstractBlock {
	 ...
	 * https://github.com/dieselfox1/fincommerce-blocks/pull/7382
	 *
	 * @var boolean
	 */
	 ...
	 * @return boolean
	 */
	public static function is_fincommerce_variation( $parsed_block ) {
		return isset( $parsed_block['attrs']['namespace'] )
		&& substr( $parsed_block['attrs']['namespace'], 0, 11 ) === 'fincommerce';
	}
	...
	public function enqueue_styles( string $block_content, array $block ) {
		if ( 'core/query' === $block['blockName'] && self::is_fincommerce_variation( $block ) ) {
	...
	public function update_query( $pre_render, $parsed_block ) {
		...
		if ( self::is_fincommerce_variation( $parsed_block ) ) {
	...
	public function update_rest_query( $args, $request ): array {
		$woo_attributes      = $request->get_param( '__fincommerceAttributes' );
		$is_valid_attributes = is_array( $woo_attributes );
		$orderby             = $request->get_param( 'orderby' );
		$woo_stock_status    = $request->get_param( '__fincommerceStockStatus' );
		$on_sale             = $request->get_param( '__fincommerceOnSale' ) === 'true';
	...
	public function build_query( $query, $block = null ) {
		...
		if ( ! $this->is_fincommerce_variation( $parsed_block ) || $is_product_collection_block ) {
			return $query;
		}
	...
	private function get_product_visibility_query( $stock_query ) {
		...
		if ( empty( $stock_query ) && 'yes' === get_option( 'fincommerce_hide_out_of_stock_items' ) ) {
	...
	private function get_queries_by_custom_attributes( $parsed_block ) {
		...
		$on_sale_enabled  = isset( $query['__fincommerceOnSale'] ) && true === $query['__fincommerceOnSale'];
		$attributes_query = isset( $query['__fincommerceAttributes'] ) ? $this->get_product_attributes_query( $query['__fincommerceAttributes'] ) : array();
		$stock_query      = isset( $query['__fincommerceStockStatus'] ) ? $this->get_stock_status_query( $query['__fincommerceStockStatus'] ) : array();
	...
	private function get_global_query( $parsed_block ) {
		...
		$inherit_enabled = isset( $parsed_block['attrs']['query']['__fincommerceInherit'] ) && true === $parsed_block['attrs']['query']['__fincommerceInherit'];
