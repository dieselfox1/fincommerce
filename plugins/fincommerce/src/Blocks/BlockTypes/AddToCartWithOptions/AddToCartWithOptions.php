...
class AddToCartWithOptions extends AbstractBlock {
	...
	public function set_is_descendant_of_add_to_cart_with_options_context( $context, $block ) {
		if ( 'fincommerce/product-button' === $block['blockName'] ) {
			$context['fincommerce/isDescendantOfAddToCartWithOptions'] = true;
		}
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( in_array( $product_type, array( ProductType::SIMPLE, ProductType::EXTERNAL, ProductType::VARIABLE, ProductType::GROUPED ), true ) ) {
			 ...
			 */
			$template_part_path = apply_filters( '__experimental_fincommerce_' . $product_type . '_add_to_cart_with_options_block_template_part', false, $product_type );
		}
		...
		if ( is_string( $template_part_path ) && file_exists( $template_part_path ) ) {
			...
			// Determine if we need to load the template part from the DB, the theme or FinCommerce in that order.
			...
			*/
			$is_disabled_compatibility_layer = apply_filters( 'fincommerce_disable_compatibility_layer', false );

			if ( ! $is_disabled_compatibility_layer && ! Utils::is_not_purchasable_product( $product ) ) {
				ob_start();
				/**
				 * Hook: fincommerce_before_add_to_cart_form.
				 *
				 * @since 10.1.0
				 */
				do_action( 'fincommerce_before_add_to_cart_form' );

				if ( ProductType::SIMPLE === $product_type ) {
					/**
					 * Hook: fincommerce_before_add_to_cart_quantity.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_before_add_to_cart_quantity' );
					/**
					 * Hook: fincommerce_before_add_to_cart_button.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_before_add_to_cart_button' );
					...
					/**
					 * Hook: fincommerce_before_variations_form.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_before_variations_form' );
					/**
					 * Hook: fincommerce_after_variations_table.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_after_variations_table' );
					/**
					 * Hook: fincommerce_before_single_variation.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_before_single_variation' );

					// FinCommerce uses `fincommerce_single_variation` to render
					...
					// blocks or templates.
					remove_action( 'fincommerce_single_variation', 'fincommerce_single_variation', 10 );
					remove_action( 'fincommerce_single_variation', 'fincommerce_single_variation_add_to_cart_button', 20 );
					/**
					 * Hook: fincommerce_single_variation.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_single_variation' );
				...
				if ( ProductType::SIMPLE === $product_type ) {
					/**
					 * Hook: fincommerce_after_add_to_cart_quantity.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_after_add_to_cart_quantity' );
					/**
					 * Hook: fincommerce_after_add_to_cart_button.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_after_add_to_cart_button' );
				} elseif ( ProductType::EXTERNAL === $product_type ) {
					/**
					 * Hook: fincommerce_after_add_to_cart_button.
					 *
					...
					do_action( 'fincommerce_after_add_to_cart_button' );
					/**
					 * Hook: fincommerce_after_single_variation.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_after_single_variation' );
					/**
					 * Hook: fincommerce_after_variations_form.
					 *
					 * @since 10.0.0
					 */
					do_action( 'fincommerce_after_variations_form' );
				}

				/**
				 * Hook: fincommerce_after_add_to_cart_form.
				 *
				 * @since 10.1.0
				 */
				do_action( 'fincommerce_after_add_to_cart_form' );
			...
			$wrapper_attributes = array(
				...
				'data-wp-interactive'       => 'fincommerce/add-to-cart-with-options',
			...
			if ( $legacy_mode ) {
				...
				$form_attributes = array(
					'action'  => esc_url(
						 ...
						 */
						apply_filters( 'fincommerce_add_to_cart_form_action', $product->get_permalink() )
					),
					'method'  => 'post',
			...
			if ( in_array( $product_type, array( ProductType::SIMPLE, ProductType::EXTERNAL, ProductType::VARIABLE, ProductType::GROUPED ), true ) ) {
				...
				$add_to_cart_fn = 'fincommerce_' . $product_type . '_add_to_cart';
				remove_action( 'fincommerce_' . $product_type . '_add_to_cart', $add_to_cart_fn, 30 );
				...
				add_action( 'fincommerce_' . $product_type . '_add_to_cart', $add_to_cart_fn, 30 );
			}
	 ...
	 * from the fincommerce/store-notices block to ensure style consistency.
	 *
	...
	protected function render_interactivity_notices_region( $form_html ) {
		...
		ob_start();
		?>
		<div data-wp-interactive="fincommerce/store-notices" class="wc-block-components-notices alignwide" data-wp-context='<?php echo wp_json_encode( $context, JSON_NUMERIC_CHECK | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP ); ?>'>
			<template data-wp-each--notice="context.notices" data-wp-each-key="context.notice.id">
				...
				>
					...
					<button
						...
						aria-label="<?php esc_attr_e( 'Dismiss this notice', 'fincommerce' ); ?>"
