...
use Automattic\FinCommerce\Blocks\BlockTypes\AddToCartWithOptions\Utils as AddToCartWithOptionsUtils;
use WP_Block;
...
class GroupedProductItemSelector extends AbstractBlock {
	...
	private function get_quantity_selector_markup( $product ) {
		...
		if ( $min_value === $max_value && $min_value > 0 ) {
			add_filter( 'fincommerce_quantity_input_type', array( $this, 'set_quantity_input_type' ) );
		}
		...
		fincommerce_quantity_input(
			array(
				 ...
				 */
				'placeholder' => apply_filters( 'fincommerce_quantity_input_placeholder', 0, $product ),
			)
		);

		if ( $min_value === $max_value && $min_value > 0 ) {
			remove_filter( 'fincommerce_quantity_input_type', array( $this, 'set_quantity_input_type' ) );
		}
