	...
	private function get_reviews_title( $attributes, $product ) {
		...
		if ( $show_reviews_count && $show_product_title ) {
			return 1 === $reviews_count
				...
				? sprintf( __( 'One review for %s', 'fincommerce' ), $product->get_title() )
				: sprintf(
		...
		if ( ! $show_reviews_count && $show_product_title ) {
			return 1 === $reviews_count
				/* translators: %s: Product title. */
				? sprintf( __( 'Review for %s', 'fincommerce' ), $product->get_title() )
				: sprintf(
					/* translators: %s: Product title. */
		...
		if ( $show_reviews_count && ! $show_product_title ) {
			return 1 === $reviews_count
				...
				: sprintf(
					/* translators: %s: Number of reviews. */
					_n( '%s review', '%s reviews', $reviews_count, 'fincommerce' ),
					number_format_i18n( $reviews_count )
				);
		}
