...
namespace Automattic\FinCommerce\Blocks\BlockTypes\Reviews;
...
class ProductReviewForm extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( get_option( 'fincommerce_review_rating_verification_required' ) !== 'no' && ! wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) {
			return '<p class="fincommerce-verification-required">' . esc_html__( 'Only logged in customers who have purchased this product may leave a review.', 'fincommerce' ) . '</p>';
		}
		...
		if ( wc_review_ratings_enabled() ) {
			...
			$interactivy_config['i18nRequiredRatingText'] = esc_attr__( 'Please select a rating', 'fincommerce' );
			...
			$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating-selector" id="comment-form-rating-label">' .
				esc_html__( 'Your rating', 'fincommerce' ) . ( wc_review_ratings_required() ? '&nbsp;<span class="required">*</span>' : '' ) .
		...
		ob_start();
		echo '<div id="review_form_wrapper" data-wp-interactive="fincommerce/product-reviews"><div id="review_form">';
		/**
			...
			*/
		comment_form( apply_filters( 'fincommerce_product_review_comment_form_args', $comment_form ), $block->context['postId'] );
		echo '</div></div>';
		$form = ob_get_clean();
		...
		if ( ! empty( $interactivy_state ) ) {
			wp_interactivity_state( 'fincommerce/product-reviews', $interactivy_state );
		}
		if ( ! empty( $interactivy_config ) ) {
			wp_interactivity_config( 'fincommerce/product-reviews', $interactivy_config );
		}
	...
	private function render_stars() {
		...
		for ( $i = 1; $i < 6; $i++ ) {
			...
			<button
				...
				aria-label='<?php echo esc_attr( sprintf( _n( '%d of 5 star', '%d of 5 stars', $i, 'fincommerce' ), $i ) ); ?>'
