...
class MiniCartProductsTableBlock extends AbstractInnerBlock {
	...
	protected function render_experimental_iapi_markup( $attributes, $content, $block ) {
		$screen_reader_text = __( 'Products in cart', 'fincommerce' );
		$remove_item_label  = __( 'Remove item', 'fincommerce' );
		$head_product_label = __( 'Product', 'fincommerce' );
		$head_details_label = __( 'Details', 'fincommerce' );
		$head_total_label   = __( 'Total', 'fincommerce' );

		// translators: %s is the name of the product in cart.
		$reduce_quantity_label = __( 'Reduce quantity of %s', 'fincommerce' );

		// translators: %s is the name of the product in cart.
		$increase_quantity_label = __( 'Increase quantity of %s', 'fincommerce' );

		// translators: %s is the name of the product in cart.
		$quantity_description_label = __( 'Quantity of %s in your cart', 'fincommerce' );

		// translators: %s is the name of the product in cart.
		$remove_from_cart_label = __( 'Remove %s from cart', 'fincommerce' );

		// translators: Save as in "Save $x".
		$save_label = __( 'Save', 'fincommerce' );

		$available_on_backorder_label = __( 'Available on backorder', 'fincommerce' );

		/* translators: %d stock amount (number of items in stock for product) */
		$low_in_stock_label = __( '%d left in stock', 'fincommerce' );
		...
		<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			<table class="wc-block-cart-items wc-block-mini-cart-items" tabindex="-1">
				...
				<tbody>
					<template
						data-wp-each--cart-item="fincommerce::state.cart.items"
									...
									<div class="wc-block-cart-item__prices">
										...
										<span data-wp-bind--hidden="!state.cartItemHasDiscount" class="price wc-block-components-product-price">
											<span class="screen-reader-text">
												<?php esc_html_e( 'Previous price:', 'fincommerce' ); ?>
											</span>
											<del data-wp-text="state.priceWithoutDiscount" class="wc-block-components-product-price__regular"></del>
											<span class="screen-reader-text">
												<?php esc_html_e( 'Discounted price:', 'fincommerce' ); ?>
											</span>
