...
final class ProductFilterStatus extends AbstractBlock {
	...
	public function prepare_selected_filters( $items, $params ) {
		...
		foreach ( $active_statuses as $status ) {
			$items[] = array(
				...
				'activeLabel' => sprintf( __( 'Status: %s', 'fincommerce' ), $status_options[ $status ] ),
			);
		}
	...
	protected function enqueue_data( array $stock_statuses = array() ) {
		...
		$this->asset_data_registry->add( 'hideOutOfStockItems', 'yes' === get_option( 'fincommerce_hide_out_of_stock_items' ) );
	}
	...
	protected function render( $attributes, $content, $block ) {
		...
		$filter_context = array(
			...
			'groupLabel' => __( 'Status', 'fincommerce' ),
		);
		...
		$wrapper_attributes = array(
			...
			'data-wp-context'     => wp_json_encode(
				array(
					...
					'activeLabelTemplate' => __( 'Status: {{label}}', 'fincommerce' ),
					'filterType'          => 'status',
				),
