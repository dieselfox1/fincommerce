...
class ComingSoon extends AbstractBlock {
	...
	protected function enqueue_assets( array $attributes, $content, $block ) {
		...
		if ( isset( $attributes['style']['color']['background'] ) ) {
			wp_add_inline_style(
				'wc-blocks-style',
				':root{--fincommerce-coming-soon-color: ' . esc_html( $attributes['style']['color']['background'] ) . '}'
			);
		} elseif ( isset( $attributes['color'] ) ) {
			// Deprecated: To support coming soon templates created before FinCommerce 9.8.0.
			wp_add_inline_style(
				'wc-blocks-style',
				':root{--fincommerce-coming-soon-color: ' . esc_html( $attributes['color'] ) . '}'
			);
			wp_enqueue_style(
				'fincommerce-coming-soon',
	 ...
	 * coming soon templates created before FinCommerce 9.8.0.
	 */
	public function enqueue_block_assets() {
		...
		if ( $block_template ) {
			...
			foreach ( $parsed_blocks as $block ) {
				if ( isset( $block['blockName'] ) && 'fincommerce/coming-soon' === $block['blockName'] ) {
					// Color attribute is deprecated in FinCommerce 9.8.0.
