...
use Automattic\FinCommerce\Blocks\Utils\ProductGalleryUtils;
use WP_Block;
...
class ProductGalleryLargeImage extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		$has_product_image_block = ! empty(
			array_filter(
				...
				function ( $inner_block ) {
					return 'fincommerce/product-image' === $inner_block->name;
				}
			)
		);
		...
		foreach ( $block->inner_blocks as $inner_block ) {
			if ( 'fincommerce/product-image' === $inner_block->name ) {
				...
				if ( 'fincommerce/product-gallery-large-image-next-previous' === $inner_block->name ) {
				...
				$inner_block_html = (
					new WP_Block(
						...
						array_merge(
							(array) $block->context,
							array( 'iapi/provider' => 'fincommerce/product-gallery' )
						),
					)
	...
	private function update_single_image( $image_html, $context, $index ) {
		...
		$img_classes = 'wc-block-fincommerce-product-gallery-large-image__image';

		if ( ! empty( $context['fullScreenOnClick'] ) ) {
			$img_classes .= ' wc-block-fincommerce-product-gallery-large-image__image--full-screen-on-click';

			$p->set_attribute( 'data-wp-on--click', 'actions.openDialog' );
		}
		if ( ! empty( $context['hoverZoom'] ) ) {
			$img_classes .= ' wc-block-fincommerce-product-gallery-large-image__image--hoverZoom';
	...
	private function get_main_images_html( $context, $product, $inner_block ) {
		$image_data = ProductGalleryUtils::get_product_gallery_image_data( $product, 'fincommerce_single' );

		ob_start();
		?>
			<ul
				class="wc-block-product-gallery-large-image__container"
				data-wp-interactive="fincommerce/product-gallery"
				data-wp-on--keydown="actions.onSelectedLargeImageKeyDown"
				aria-label="<?php esc_attr_e( 'Product gallery', 'fincommerce' ); ?>"
				tabindex="0"
	...
	private function legacy_get_main_images_html( $context, $product ) {
		$image_data   = ProductGalleryUtils::get_product_gallery_image_data( $product, 'fincommerce_single' );
		$base_classes = 'wc-block-fincommerce-product-gallery-large-image__image wc-block-fincommerce-product-gallery-large-image__image--legacy';

		if ( ! empty( $context['fullScreenOnClick'] ) ) {
			$base_classes .= ' wc-block-fincommerce-product-gallery-large-image__image--full-screen-on-click';
		}
		if ( ! empty( $context['hoverZoom'] ) ) {
			$base_classes .= ' wc-block-fincommerce-product-gallery-large-image__image--hoverZoom';
		}
	...
	public function add_block_type_metadata_settings( $settings, $metadata ) {
		if ( ! empty( $metadata['name'] ) && 'fincommerce/product-gallery-large-image' === $metadata['name'] ) {
