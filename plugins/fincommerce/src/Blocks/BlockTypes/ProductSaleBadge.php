...
class ProductSaleBadge extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		 ...
		 *
		 * @hook fincommerce_sale_badge_text
		 * @since 10.0.0
		 *
		 ...
		 */
		$sale_text = apply_filters( 'fincommerce_sale_badge_text', __( 'Sale', 'fincommerce' ), $product );

		$output  = '<div class="wp-block-fincommerce-product-sale-badge ' . esc_attr( $classname ) . '">';
		...
		$output .= '<span class="screen-reader-text">'
						. __( 'Product on sale', 'fincommerce' )
					. '</span>';
		$output .= '</div></div>';
