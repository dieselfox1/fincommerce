...
use Automattic\FinCommerce\Blocks\BlockTypes\AttributeFilter;
use Automattic\FinCommerce\Blocks\BlockTypes\PriceFilter;
use Automattic\FinCommerce\Blocks\BlockTypes\RatingFilter;
use Automattic\FinCommerce\Blocks\BlockTypes\StockFilter;
use WP_Query;
use WC_Tax;
...
class QueryBuilder {
	...
	public function get_final_frontend_query( $collection_args, $query, $page = 1, $is_exclude_applied_filters = false ) {
		...
		$is_on_sale          = $query['fincommerceOnSale'] ?? false;
		$order_by            = $query['orderBy'] ?? '';
		$stock_status        = $query['fincommerceStockStatus'] ?? array_keys( wc_get_product_stock_status_options() );
		$product_attributes  = $query['fincommerceAttributes'] ?? array();
		$taxonomies_query    = $this->get_filter_by_taxonomies_query( $query['tax_query'] ?? array() );
		$handpicked_products = $query['fincommerceHandPickedProducts'] ?? array();
	...
	protected function adjust_price_filter_for_tax_class( $price_filter, $tax_class ) {
		$tax_display    = get_option( 'fincommerce_tax_display_shop' );
		...
		if ( 'incl' === $tax_display ) {
			 ...
			 *
			 * The fincommerce_adjust_non_base_location_prices filter can stop base taxes being taken off when dealing
			 ...
			 * @return boolean
			 */
			$taxes = apply_filters( 'fincommerce_adjust_non_base_location_prices', true ) ? WC_Tax::calc_tax( $price_filter, $base_tax_rates, true ) : WC_Tax::calc_tax( $price_filter, $tax_rates, true );
	...
	private function should_adjust_price_range_for_taxes() {
		$display_setting      = get_option( 'fincommerce_tax_display_shop' ); // Tax display setting ('incl' or 'excl').
