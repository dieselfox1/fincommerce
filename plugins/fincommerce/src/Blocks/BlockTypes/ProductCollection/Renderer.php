...
namespace Automattic\FinCommerce\Blocks\BlockTypes\ProductCollection;

use Automattic\FinCommerce\Blocks\BlockTypes\ProductCollection\Utils as ProductCollectionUtils;
...
class Renderer {
	...
	public function __construct() {
		...
		add_filter( 'render_block_fincommerce/product-collection', array( $this, 'handle_rendering' ), 10, 2 );

		// Disable block render if the ProductTemplate block is empty.
		add_filter(
			'render_block_fincommerce/product-template',
			function ( $html ) {
		...
		add_filter(
			'render_block_fincommerce/product-collection-no-results',
			function ( $html ) {
	...
	public function enhance_product_collection_with_interactivity( $block_content, $block ) {
		...
		if ( $is_product_collection_block ) {
			wp_enqueue_script_module( 'fincommerce/product-collection' );
			...
			$context                        = array(
				...
				'ariaLabelPrevious'       => __( 'Scroll products left', 'fincommerce' ),
				'ariaLabelNext'           => __( 'Scroll products right', 'fincommerce' ),
			);
			...
			if ( $p->next_tag( array( 'class_name' => 'wp-block-fincommerce-product-collection' ) ) ) {
				$p->set_attribute( 'data-wp-interactive', 'fincommerce/product-collection' );
	...
	protected function render_interactivity_notices_region() {
		...
		ob_start();
		?>
		<div data-wp-interactive="fincommerce/store-notices" class="wc-block-components-notices alignwide">
		...
		if ( $is_product_collection_block && $is_enhanced_pagination_enabled && $query_id === $parsed_query_id ) {
			...
			while ( $p->next_tag( 'A' ) ) {
				if ( $p->has_class( 'wp-block-query-pagination-next' ) || $p->has_class( 'wp-block-query-pagination-previous' ) ) {
					$p->set_attribute( 'data-wp-on--click', 'fincommerce/product-collection::actions.navigate' );
					$p->set_attribute(
						'data-wp-key',
					...
					$p->set_attribute( 'data-wp-watch', 'fincommerce/product-collection::callbacks.prefetch' );
					$p->set_attribute( 'data-wp-on--mouseenter', 'fincommerce/product-collection::actions.prefetchOnHover' );
	...
	public function extend_context_for_inner_blocks( $context ) {
		...
		// This is needed to avoid SSR renders while in editor. @see https://github.com/dieselfox1/fincommerce/issues/45181.
		if ( is_admin() || \WC()->is_rest_api_request() ) {
			return $context;
		}

		// Add iapi/provider to inner blocks so they can run this store's Interactivity API actions.
		$context['iapi/provider'] = 'fincommerce/product-collection';
