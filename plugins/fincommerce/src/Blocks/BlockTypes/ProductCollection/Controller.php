...
class Controller extends AbstractBlock {
	...
	public function add_product_title_click_event_directives( $block_content, $block, $instance ) {
		$namespace              = $instance->attributes['__fincommerceNamespace'] ?? '';
		$is_product_title_block = 'fincommerce/product-collection/product-title' === $namespace;
		...
		if ( $is_product_title_block && $is_link ) {
			...
			if ( $is_anchor ) {
				$p->set_attribute( 'data-wp-on--click', 'fincommerce/product-collection::actions.viewProduct' );
	...
	public function disable_enhanced_pagination( $parsed_block ) {
		...
		if (
			...
			'fincommerce/product-collection' === $block_name &&
			! $force_page_reload_global
		) {
			...
			if ( ! isset( $render_product_collection_callback ) ) {
				...
				$render_product_collection_callback = static function ( $content, $block ) use ( &$enhanced_query_stack, &$dirty_enhanced_queries, &$render_product_collection_callback ) {
					...
					if ( empty( $enhanced_query_stack ) ) {
						remove_filter( 'render_block_fincommerce/product-collection', $render_product_collection_callback, 5 );
				...
				add_filter( 'render_block_fincommerce/product-collection', $render_product_collection_callback, 5, 2 );
			}
		} elseif (
	...
	public function register_settings() {
		register_setting(
			...
			array(
				...
				'show_in_rest' => array(
					'name'   => 'fincommerce_default_catalog_orderby',
					'schema' => array(
						'type' => 'string',
	...
	public function update_rest_query_in_editor( $query, $request ): array {
		...
		$on_sale                        = $request->get_param( 'fincommerceOnSale' ) === 'true';
		$stock_status                   = $request->get_param( 'fincommerceStockStatus' );
		$product_attributes             = $request->get_param( 'fincommerceAttributes' );
		$handpicked_products            = $request->get_param( 'fincommerceHandPickedProducts' );
