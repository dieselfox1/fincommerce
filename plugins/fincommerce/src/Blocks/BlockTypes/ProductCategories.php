...
class ProductCategories extends AbstractDynamicBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		$output  = '<div class="wp-block-fincommerce-product-categories ' . esc_attr( $classes ) . '" style="' . esc_attr( $styles ) . '">';
	...
	protected function renderDropdown( $categories, $attributes, $uid ) {
		$aria_label = empty( $attributes['hasCount'] ) ?
			__( 'List of categories', 'fincommerce' ) :
			__( 'List of categories with their product counts', 'fincommerce' );
		...
		$output = '
			<div class="wc-block-product-categories__dropdown">
				...
				>
					' . esc_html__( 'Select a category', 'fincommerce' ) . '
				</label>
			...
			<button
				...
				aria-label="' . esc_html__( 'Go to category', 'fincommerce' ) . '"
	 ...
	 * @param array    $attributes Block attributes. Default empty array.
	 * @param string   $size Image size, defaults to 'fincommerce_thumbnail'.
	 * @return string
	 */
	public function get_image_html( $category, $attributes, $size = 'fincommerce_thumbnail' ) {
		if ( empty( $attributes['hasImage'] ) ) {
			return '';
		}
		...
		if ( ! $image_id ) {
			return '<span class="wc-block-product-categories-list-item__image wc-block-product-categories-list-item__image--placeholder">' . wc_placeholder_img( 'fincommerce_thumbnail' ) . '</span>';
		}

		return '<span class="wc-block-product-categories-list-item__image">' . wp_get_attachment_image( $image_id, 'fincommerce_thumbnail' ) . '</span>';
	}
	...
	protected function getCount( $category, $attributes ) {
		...
		$screen_reader_text = sprintf(
			...
			_n( '%d product', '%d products', absint( $category->count ), 'fincommerce' ),
