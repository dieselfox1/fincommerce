...
use Automattic\FinCommerce\Blocks\BlockTypes\AddToCartWithOptions\Utils;
...
class ProductButton extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		$is_descendant_of_add_to_cart_form = isset( $block->context['fincommerce/isDescendantOfAddToCartWithOptions'] ) ? $block->context['fincommerce/isDescendantOfAddToCartWithOptions'] : false;
		...
		if ( ! $is_descendant_of_add_to_cart_form ) {
			 ...
			 */
			$default_quantity = apply_filters( 'fincommerce_add_to_cart_quantity', $default_quantity, $product->get_id() );
		}

		$add_to_cart_text = null !== $product->add_to_cart_text() ? $product->add_to_cart_text() : __( 'Add to cart', 'fincommerce' );
		...
		$div_directives = '
			data-wp-interactive="fincommerce/product-button"
		...
		$button_directives = $is_descendant_of_add_to_cart_form ?
			'data-wp-class--disabled="fincommerce/add-to-cart-with-options::!state.isFormValid" data-wp-on--click="actions.handlePressedState"' :
			'data-wp-on--click="actions.addCartItem"';
		$anchor_directive  = $is_descendant_of_add_to_cart_form ? '' : 'data-wp-on--click="fincommerce/product-collection::actions.viewProduct"';
	...
	private function get_in_the_cart_text( $product ) {
		if ( $product->is_type( ProductType::GROUPED ) ) {
			return __( 'Added to cart', 'fincommerce' );
		}
	...
	private function get_view_cart_html() {
		return sprintf(
			...
			esc_attr__( 'View cart', 'fincommerce' ),
			esc_html__( 'View cart', 'fincommerce' )
		);
	}
}
