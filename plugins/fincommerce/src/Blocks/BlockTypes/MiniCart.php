...
use Automattic\FinCommerce\Blocks\Utils\MiniCartUtils;
use Automattic\FinCommerce\Blocks\Utils\BlockHooksTrait;
use Automattic\FinCommerce\Admin\Features\Features;
use Automattic\FinCommerce\Blocks\Utils\BlocksSharedState;
...
class MiniCart extends AbstractBlock {
	...
	/**
	 * FinCommerce mini-cart template blocks.
	 *
	 * @var array
	 */
	const MINI_CART_TEMPLATE_BLOCKS = array(
		'fincommerce/mini-cart-contents',
		'fincommerce/filled-mini-cart-contents-block',
		'fincommerce/mini-cart-title-block',
		'fincommerce/mini-cart-title-label-block',
		'fincommerce/mini-cart-title-items-counter-block',
		'fincommerce/mini-cart-items-block',
		'fincommerce/mini-cart-products-table-block',
		...
		'fincommerce/mini-cart-shopping-button-block',
	);
	...
	protected function initialize() {
		...
		add_filter( 'hooked_block_fincommerce/mini-cart', array( $this, 'modify_hooked_block_attributes' ), 10, 5 );
		add_filter( 'hooked_block_types', array( $this, 'register_hooked_block' ), 9, 4 );

		// Priority 20 ensures this runs after FinCommerce block registration (priority 10)
	...
	public function enable_interactivity_support() {
		if ( Features::is_enabled( 'experimental-iapi-mini-cart' ) ) {
			$block_type = \WP_Block_Type_Registry::get_instance()->get_registered( 'fincommerce/mini-cart' );
	...
	public function modify_hooked_block_attributes( $parsed_hooked_block, $hooked_block_type, $relative_position, $parsed_anchor_block, $context ) {
		$mini_cart_block_font_size = wp_get_global_styles( array( 'blocks', 'fincommerce/mini-cart', 'typography', 'fontSize' ) );
	...
	protected function render_experimental_iapi_mini_cart( $attributes, $content, $block ) {
		...
		$this->register_cart_interactivity( 'I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of FinCommerce' );
		$this->initialize_shared_config( 'I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of FinCommerce' );
		$this->placeholder_image( 'I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of FinCommerce' );
		...
		if ( $cart ) {
			...
			$wrapper_classes                  = sprintf( 'wc-block-mini-cart wp-block-fincommerce-mini-cart %s', $classes_styles['classes'] );
			...
			$display_cart_price_including_tax = get_option( 'fincommerce_tax_display_cart' ) === 'incl';
			$cart_item_count                  = $cart ? $cart->get_cart_contents_count() : 0;
			...
			$button_aria_label_template = isset( $attributes['hasHiddenPrice'] ) && false !== $attributes['hasHiddenPrice']
				...
				: __( 'Number of items in the cart: %1$d. Total price of %2$s', 'fincommerce' );
	...
	public function render_experimental_iapi_mini_cart_overlay() {
		...
		<div
			data-wp-interactive="fincommerce/mini-cart"
			data-wp-router-region='{ "id": "fincommerce/mini-cart-overlay", "attachTo": "body" }'
	...
	protected function process_template_contents( $template_contents ) {
		...
		$is_old_template = $p->next_tag(
			array(
				'tag_name'   => 'div',
				'class_name' => 'wp-block-fincommerce-mini-cart-contents',
			)
		);
	...
	protected function get_markup( $attributes ) {
		...
		if ( is_cart() || is_checkout() ) {
			...
			return '<div class="' . esc_attr( $wrapper_classes ) . '" style="visibility:hidden" aria-hidden="true">
				<button class="wc-block-mini-cart__button" disabled aria-label="' . __( 'Cart', 'fincommerce' ) . '">' . $button_html . '</button>
			</div>';
		}
		...
		return '<div class="' . esc_attr( $wrapper_classes ) . '" style="' . esc_attr( $wrapper_styles ) . '">
			<button class="wc-block-mini-cart__button" aria-label="' . __( 'Cart', 'fincommerce' ) . '">' . $button_html . '</button>
	 ...
	 * https://github.com/dieselfox1/fincommerce/blob/e730f7463c25b50258e97bf56e31e9d7d3bc7ae7/includes/class-wc-cart.php#L1582
	 *
	 * @return array;
	 */
	...
	public function register_empty_cart_message_block_pattern() {
		register_block_pattern(
			...
			array(
				'title'    => __( 'Empty Mini-Cart Message', 'fincommerce' ),
				'inserter' => false,
				'content'  => '<!-- wp:paragraph {"align":"center"} --><p class="has-text-align-center"><strong>' . __( 'Your cart is currently empty!', 'fincommerce' ) . '</strong></p><!-- /wp:paragraph -->',
			)
		);
	}
