 ...
 *
 * @package Automattic\FinCommerce\Blocks\BlockTypes
 */
final class ProductFilterRating extends AbstractBlock {
	...
	public function prepare_selected_filters( $items, $params ) {
		...
		foreach ( $active_ratings as $rating ) {
			$items[] = array(
				...
				'activeLabel' => sprintf( __( 'Rating: Rated %d out of 5', 'fincommerce' ), $rating ),
			);
		}
	...
	protected function render( $attributes, $content, $block ) {
		...
		$filter_options = array_map(
			function ( $rating ) use ( $selected_rating ) {
				$aria_label = sprintf(
					...
					__( 'Rated %1$d out of 5', 'fincommerce' ),
					$rating['rating'],
				);
		...
		$filter_context = array(
			...
			'groupLabel' => __( 'Rating', 'fincommerce' ),
		);
		...
		$wrapper_attributes = array(
			...
			'data-wp-context'     => wp_json_encode(
				array(
					...
					'activeLabelTemplate' => __( 'Rating: {{label}}', 'fincommerce' ),
					'filterType'          => 'rating',
				),
