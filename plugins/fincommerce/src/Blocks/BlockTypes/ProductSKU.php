...
class ProductSKU extends AbstractBlock {
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( $is_interactive ) {
			...
			wp_enqueue_script_module( 'fincommerce/product-elements' );
		}

		$styles_and_classes = StyleAttributesUtils::get_classes_and_styles_by_attributes( $attributes );

		$prefix = isset( $attributes['prefix'] ) ? wp_kses_post( ( $attributes['prefix'] ) ) : __( 'SKU: ', 'fincommerce' );
		if ( ! empty( $prefix ) ) {
		...
		$interactive_attributes = $is_interactive ? 'data-wp-interactive="fincommerce/product-elements" data-wp-text="state.productData.sku"' : '';

		return sprintf(
			'<div class="wc-block-components-product-sku wc-block-grid__product-sku wp-block-fincommerce-product-sku product_meta wp-block-post-terms %1$s" style="%2$s">
				%3$s
