...
use Automattic\FinCommerce\Blocks\Templates\ProductAttributeTemplate;
use Automattic\FinCommerce\Blocks\Templates\ProductCatalogTemplate;
use Automattic\FinCommerce\Blocks\Templates\ProductCategoryTemplate;
use Automattic\FinCommerce\Blocks\Templates\ProductTagTemplate;
use Automattic\FinCommerce\Blocks\Templates\ProductSearchResultsTemplate;
use Automattic\FinCommerce\Blocks\Templates\OrderConfirmationTemplate;
...
class ClassicTemplate extends AbstractDynamicBlock {
	 ...
	 *
	 * @see https://github.com/dieselfox1/fincommerce/pull/60223
	 */
	public function enqueue_legacy_assets() {
		...
		add_filter( 'fincommerce_single_product_zoom_enabled', '__return_true' );
		add_filter( 'fincommerce_single_product_photoswipe_enabled', '__return_true' );
		add_filter( 'fincommerce_single_product_flexslider_enabled', '__return_true' );
	}
	...
	protected function render( $attributes, $content, $block ) {
		 ...
		 *
		 * @see https://github.com/dieselfox1/fincommerce-gutenberg-products-block/issues/5328#issuecomment-989013447
		 */
	...
	protected function render_single_product() {
		 ...
		 *
		 * @see fincommerce_output_content_wrapper() Outputs opening DIV for the content (priority 10)
		 * @see fincommerce_breadcrumb() Outputs breadcrumb trail to the current product (priority 20)
		 ...
		 *
		 * @see fincommerce_output_content_wrapper_end() Outputs closing DIV for the content (priority 10)
		 *
		 * @since 6.3.0
		 */
	...
	protected function render_archive_product() {
		...
		<header class="fincommerce-products-header">
			...
			if ( apply_filters( 'fincommerce_show_page_title', true ) ) {
				?>
				<h1 class="fincommerce-products-header__title page-title">
					<?php
						fincommerce_page_title();
					?>
				</h1>
				<?php
			}
			 ...
			 *
			 * @see fincommerce_taxonomy_archive_description() Renders the taxonomy archive description (priority 10)
			 * @see fincommerce_product_archive_description() Renders the product archive description (priority 10)
			 *
			 * @since 6.3.0
			 */
		...
		if ( fincommerce_product_loop() ) {
			 ...
			 *
			 * @see fincommerce_output_all_notices() Render error notices (priority 10)
			 * @see fincommerce_result_count() Show number of results found (priority 20)
			 * @see fincommerce_catalog_ordering() Show form to control sort order (priority 30)
			 *
			 * @since 6.3.0
			 */
			 ...
			 *
			 * @see fincommerce_pagination() Renders pagination (priority 10)
			 *
			 * @since 6.3.0
			 */
	...
	public function add_alignment_class_to_wrapper( string $content, array $block ) {
		if ( ( 'fincommerce/' . $this->block_name ) !== $block['blockName'] ) {
			return $content;
		}
