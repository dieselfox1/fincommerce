...
class AddToCartForm extends AbstractBlock {
	...
	private function add_steppers( $product_html, $product_name ) {
		...
		$minus_button = '$1<button aria-label="' . esc_attr( sprintf( __( 'Reduce quantity of %s', 'fincommerce' ), $product_name ) ) . '" type="button" data-wp-on--click="actions.removeQuantity" class="wc-block-components-quantity-selector__button wc-block-components-quantity-selector__button--minus">−</button>';
		// Replacement string to add button AFTER the matched <input> element.
		/* translators: %s refers to the item name in the cart. */
		$plus_button = '$1<button aria-label="' . esc_attr( sprintf( __( 'Increase quantity of %s', 'fincommerce' ), $product_name ) ) . '" type="button" data-wp-on--click="actions.addQuantity" class="wc-block-components-quantity-selector__button wc-block-components-quantity-selector__button--plus">+</button>';
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( $is_descendent_of_single_product_block ) {
			add_filter( 'fincommerce_add_to_cart_form_action', array( $this, 'add_to_cart_form_action' ), 10 );
		}
		 ...
		 */
		add_action( 'fincommerce_variation_add_to_cart', 'fincommerce_simple_add_to_cart', 10 );
		 ...
		 */
		remove_action( 'fincommerce_variation_add_to_cart', 'fincommerce_simple_add_to_cart', 10 );

		$product_html = ob_get_clean();

		if ( $is_descendent_of_single_product_block ) {
			remove_filter( 'fincommerce_add_to_cart_form_action', array( $this, 'add_to_cart_form_action' ), 10 );
		}
		...
		$form = sprintf(
			'<div %1$s %2$s>%3$s</div>',
			$wrapper_attributes,
			$is_stepper_style ? 'data-wp-interactive="fincommerce/add-to-cart-form"' : '',
			$product_html
		);
