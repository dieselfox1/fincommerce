...
use Automattic\FinCommerce\StoreApi\Utilities\PaymentUtils;
use Automattic\FinCommerce\Blocks\Domain\Services\CheckoutFieldsSchema\Validation;
...
class Checkout extends AbstractBlock {
	 ...
	 * @return void
	 */
	public function dequeue_fincommerce_core_scripts() {
	...
	public function register_settings() {
		register_setting(
			...
			array(
				'type'         => 'object',
				'description'  => __( 'Controls the display of the phone field in checkout.', 'fincommerce' ),
				'label'        => __( 'Phone number', 'fincommerce' ),
				'show_in_rest' => array(
					'name'   => 'fincommerce_checkout_phone_field',
					'schema' => array(
						'type' => 'string',
		...
		register_setting(
			...
			array(
				'type'         => 'object',
				'description'  => __( 'Controls the display of the company field in checkout.', 'fincommerce' ),
				'label'        => __( 'Company', 'fincommerce' ),
				'show_in_rest' => array(
					'name'   => 'fincommerce_checkout_company_field',
					'schema' => array(
						'type' => 'string',
						'enum' => array( 'optional', 'required', 'hidden' ),
					),
		...
		register_setting(
			...
			array(
				'type'         => 'object',
				'description'  => __( 'Controls the display of the apartment (address_2) field in checkout.', 'fincommerce' ),
				'label'        => __( 'Address Line 2', 'fincommerce' ),
				'show_in_rest' => array(
					'name'   => 'fincommerce_checkout_address_2_field',
					'schema' => array(
						'type' => 'string',
						'enum' => array( 'optional', 'required', 'hidden' ),
					),
	...
	public function register_patterns() {
		register_block_pattern(
			'fincommerce/checkout-heading',
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"align":"wide", "level":1} --><h1 class="wp-block-heading alignwide">' . esc_html__( 'Checkout', 'fincommerce' ) . '</h1><!-- /wp:heading -->',
			)
		);
	}
	...
	protected function get_block_type_script( $key = null ) {
		...
		if ( ! is_user_logged_in() && 'no' === get_option( 'fincommerce_registration_generate_password' ) ) {
	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( $this->is_checkout_endpoint() ) {
			...
			return wp_is_block_theme() ? do_shortcode( '[fincommerce_checkout]' ) : '[fincommerce_checkout]';
		}

		// Dequeue the core scripts when rendering this block.
		add_action( 'wp_enqueue_scripts', array( $this, 'dequeue_fincommerce_core_scripts' ), 20 );
		 ...
		 */
		$regex_for_empty_block = '/<div class="[a-zA-Z0-9_\- ]*wp-block-fincommerce-checkout[a-zA-Z0-9_\- ]*"><\/div>/mi';
		...
		if ( $has_i1_template ) {
			...
			$inner_blocks_html = '
				<div data-block-name="fincommerce/checkout-fields-block" class="wp-block-fincommerce-checkout-fields-block">
					<div data-block-name="fincommerce/checkout-express-payment-block" class="wp-block-fincommerce-checkout-express-payment-block"></div>
					<div data-block-name="fincommerce/checkout-contact-information-block" class="wp-block-fincommerce-checkout-contact-information-block"></div>
					<div data-block-name="fincommerce/checkout-shipping-address-block" class="wp-block-fincommerce-checkout-shipping-address-block"></div>
					<div data-block-name="fincommerce/checkout-billing-address-block" class="wp-block-fincommerce-checkout-billing-address-block"></div>
					<div data-block-name="fincommerce/checkout-shipping-methods-block" class="wp-block-fincommerce-checkout-shipping-methods-block"></div>
					<div data-block-name="fincommerce/checkout-payment-block" class="wp-block-fincommerce-checkout-payment-block"></div>
					<div data-block-name="fincommerce/checkout-additional-information-block" class="wp-block-fincommerce-checkout-additional-information-block"></div>' .
					( isset( $attributes['showOrderNotes'] ) && false === $attributes['showOrderNotes'] ? '' : '<div data-block-name="fincommerce/checkout-order-note-block" class="wp-block-fincommerce-checkout-order-note-block"></div>' ) .
					( isset( $attributes['showPolicyLinks'] ) && false === $attributes['showPolicyLinks'] ? '' : '<div data-block-name="fincommerce/checkout-terms-block" class="wp-block-fincommerce-checkout-terms-block"></div>' ) .
					'<div data-block-name="fincommerce/checkout-actions-block" class="wp-block-fincommerce-checkout-actions-block"></div>
				</div>
				<div data-block-name="fincommerce/checkout-totals-block" class="wp-block-fincommerce-checkout-totals-block">
					<div data-block-name="fincommerce/checkout-order-summary-block" class="wp-block-fincommerce-checkout-order-summary-block"></div>
				</div>
			';
		...
		$order_summary_with_inner_blocks = '$0
			<div data-block-name="fincommerce/checkout-order-summary-cart-items-block" class="wp-block-fincommerce-checkout-order-summary-cart-items-block"></div>
			<div data-block-name="fincommerce/checkout-order-summary-subtotal-block" class="wp-block-fincommerce-checkout-order-summary-subtotal-block"></div>
			<div data-block-name="fincommerce/checkout-order-summary-fee-block" class="wp-block-fincommerce-checkout-order-summary-fee-block"></div>
			<div data-block-name="fincommerce/checkout-order-summary-discount-block" class="wp-block-fincommerce-checkout-order-summary-discount-block"></div>
			<div data-block-name="fincommerce/checkout-order-summary-coupon-form-block" class="wp-block-fincommerce-checkout-order-summary-coupon-form-block"></div>
			<div data-block-name="fincommerce/checkout-order-summary-shipping-block" class="wp-block-fincommerce-checkout-order-summary-shipping-block"></div>
			<div data-block-name="fincommerce/checkout-order-summary-taxes-block" class="wp-block-fincommerce-checkout-order-summary-taxes-block"></div>
		';
		// Order summary subtotal block was added in i3, so we search for it to see if we have a Checkout i2 template.
		$regex_for_order_summary_subtotal = '/<div[^<]*?data-block-name="fincommerce\/checkout-order-summary-subtotal-block"[^>]*?>/mi';
		$regex_for_order_summary          = '/<div[^<]*?data-block-name="fincommerce\/checkout-order-summary-block"[^>]*?>/mi';
		 ...
		 */
		$local_pickup_inner_blocks = '<div data-block-name="fincommerce/checkout-shipping-method-block" class="wp-block-fincommerce-checkout-shipping-method-block"></div>' . PHP_EOL . PHP_EOL . '<div data-block-name="fincommerce/checkout-pickup-options-block" class="wp-block-fincommerce-checkout-pickup-options-block"></div>' . PHP_EOL . PHP_EOL . '$0';
		$has_local_pickup_regex    = '/<div[^<]*?data-block-name="fincommerce\/checkout-shipping-method-block"[^>]*?>/mi';
		$has_local_pickup          = preg_match( $has_local_pickup_regex, $content );

		if ( ! $has_local_pickup ) {
			$shipping_address_block_regex = '/<div[^<]*?data-block-name="fincommerce\/checkout-shipping-address-block" class="wp-block-fincommerce-checkout-shipping-address-block"[^>]*?><\/div>/mi';
		 ...
		 */
		$additional_information_inner_blocks = '$0' . PHP_EOL . PHP_EOL . '<div data-block-name="fincommerce/checkout-additional-information-block" class="wp-block-fincommerce-checkout-additional-information-block"></div>' . PHP_EOL . PHP_EOL;
		$has_additional_information_regex    = '/<div[^<]*?data-block-name="fincommerce\/checkout-additional-information-block"[^>]*?>/mi';
		$has_additional_information_block    = preg_match( $has_additional_information_regex, $content );

		if ( ! $has_additional_information_block ) {
			$payment_block_regex = '/<div[^<]*?data-block-name="fincommerce\/checkout-payment-block" class="wp-block-fincommerce-checkout-payment-block"[^>]*?><\/div>/mi';
	...
	/**
	 * Update the local pickup title in FinCommerce Settings when the checkout page containing a Checkout block is saved.
	 *
	...
	public function update_local_pickup_title( $post_id, $post ) {
		...
		if ( ! empty( $post->post_type ) && 'wp_template' !== $post->post_type && ( false === has_block( 'fincommerce/checkout', $post ) || (string) get_option( 'fincommerce_checkout_page_id' ) !== (string) $post_id ) ) {
			return;
		}

		if ( ( ! empty( $post->post_type ) && ! empty( $post->post_name ) && 'page-checkout' !== $post->post_name && 'wp_template' === $post->post_type ) || false === has_block( 'fincommerce/checkout', $post ) ) {
			return;
		}
		...
		update_option( 'fincommerce_pickup_location_settings', $pickup_location_settings );
	}
	...
	private function find_local_pickup_text_in_checkout_block( $blocks ) {
		...
		foreach ( $blocks as $block ) {
			if ( ! empty( $block['blockName'] ) && 'fincommerce/checkout-shipping-method-block' === $block['blockName'] ) {
	...
	protected function enqueue_data( array $attributes = [] ) {
		...
		$this->asset_data_registry->add( 'checkoutShowLoginReminder', filter_var( get_option( 'fincommerce_enable_checkout_login_reminder' ), FILTER_VALIDATE_BOOLEAN ) );
		$this->asset_data_registry->add( 'displayCartPricesIncludingTax', 'incl' === get_option( 'fincommerce_tax_display_cart' ) );
		$this->asset_data_registry->add( 'displayItemizedTaxes', 'itemized' === get_option( 'fincommerce_tax_total_display' ) );
		$this->asset_data_registry->add( 'forcedBillingAddress', 'billing_only' === get_option( 'fincommerce_ship_to_destination' ) );
		$this->asset_data_registry->add( 'generatePassword', filter_var( get_option( 'fincommerce_registration_generate_password' ), FILTER_VALIDATE_BOOLEAN ) );
		...
		if ( $is_block_editor && ! $this->asset_data_registry->exists( 'incompatibleExtensions' ) ) {
			if ( ! class_exists( '\Automattic\FinCommerce\Utilities\FeaturesUtil' ) || ! function_exists( 'get_plugins' ) ) {
				return;
			}

			$declared_extensions     = \Automattic\FinCommerce\Utilities\FeaturesUtil::get_compatible_plugins_for_feature( 'cart_checkout_blocks' );
