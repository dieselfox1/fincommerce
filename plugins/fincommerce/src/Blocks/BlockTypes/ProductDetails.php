...
class ProductDetails extends AbstractBlock {
	...
	protected function initialize() {
		 ...
		 *
		 * @hook fincommerce_product_details_hooked_blocks
		 *
		 * @since 10.0.0
		 ...
		 */
		$hooked_blocks = apply_filters( 'fincommerce_product_details_hooked_blocks', [] );
	...
	protected function render( $attributes, $content, $block ) {
		 ...
		 * @since 7.6.0
		 */
		if ( ! apply_filters( 'fincommerce_disable_compatibility_layer', false ) ) {
	...
	private function inject_compatible_tabs( $parsed_block ) {
		...
		$product_tabs = apply_filters(
			'fincommerce_product_tabs',
			array()
		);

		$default_tabs_callbacks = array(
			'fincommerce_product_description_tab',
			'fincommerce_product_additional_information_tab',
			'comments_template',
		);
	...
	private function create_accordion_item_block( $title, $content ) {
		$template = '<!-- wp:fincommerce/accordion-item -->
			<div class="wp-block-fincommerce-accordion-item"><!-- wp:fincommerce/accordion-header -->
			<h3 class="wp-block-fincommerce-accordion-header accordion-item__heading">
			...
			<!-- /wp:fincommerce/accordion-header -->

			<!-- wp:fincommerce/accordion-panel -->
			<div class="wp-block-fincommerce-accordion-panel"><div class="accordion-content__wrapper">
			%2$s
			</div></div>
			<!-- /wp:fincommerce/accordion-panel --></div>
			<!-- /wp:fincommerce/accordion-item -->';
	...
	private function inject_parsed_accordion_blocks( $parsed_block, $accordion_blocks ) {
		if ( 'fincommerce/accordion-group' === $parsed_block['blockName'] ) {
	...
	private function register_hooked_block( $slug, $block ) {
		add_filter(
			...
			function ( $hooked_block_types, $relative_position, $anchor_block_type ) use ( $slug ) {
				if (
					'fincommerce/accordion-group' === $anchor_block_type &&
		...
		add_filter(
			...
			function ( $parsed_hooked_block, $hooked_block_type, $relative_position, $parsed_anchor_block ) use ( $block ) {
				if (
					...
					'fincommerce/accordion-group' !== $parsed_anchor_block['blockName'] ||
					'last_child' !== $relative_position ||
	 ...
	 * method here.
	 *
	 * @see https://github.com/dieselfox1/fincommerce/pull/59005
	 *
	 * @param array    $attributes Block attributes.
	 * @param string   $content Block content.
	 * @param WP_Block $block Block instance.
	...
	protected function render_legacy_block( $attributes, $content, $block ) {
		...
		if ( $hide_tab_title ) {
			add_filter( 'fincommerce_product_description_heading', '__return_empty_string' );
			add_filter( 'fincommerce_product_additional_information_heading', '__return_empty_string' );
			add_filter( 'fincommerce_reviews_title', '__return_empty_string' );
		}

		$tabs = $this->render_tabs();

		if ( $hide_tab_title ) {
			remove_filter( 'fincommerce_product_description_heading', '__return_empty_string' );
			remove_filter( 'fincommerce_product_additional_information_heading', '__return_empty_string' );
			remove_filter( 'fincommerce_reviews_title', '__return_empty_string' );
		...
		return sprintf(
			'<div class="wp-block-fincommerce-product-details %1$s">
				<div style="%2$s">
					%3$s
				</div>
			</div>',
	...
	protected function render_tabs() {
		...
		while ( have_posts() ) {
			the_post();
			fincommerce_output_product_data_tabs();
		}
