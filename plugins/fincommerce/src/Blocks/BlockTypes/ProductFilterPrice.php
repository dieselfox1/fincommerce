...
final class ProductFilterPrice extends AbstractBlock {
	...
	public function prepare_selected_filters( $items, $params ) {
		...
		if ( ! $formatted_min_price ) {
			...
			$item['activeLabel'] = sprintf( __( 'Price: Up to %s', 'fincommerce' ), $formatted_max_price );
			$item['value']       = "|{$max_price}";
		}

		if ( ! $formatted_max_price ) {
			/* translators: %s is the formatted minimum price. */
			$item['activeLabel'] = sprintf( __( 'Price: From %s', 'fincommerce' ), $formatted_min_price );
	...
	protected function render( $attributes, $content, $block ) {
		...
		$filter_context = array(
			...
			'groupLabel' => __( 'Price', 'fincommerce' ),
		);
