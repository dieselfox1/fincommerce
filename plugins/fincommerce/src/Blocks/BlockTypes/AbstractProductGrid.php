...
use Automattic\FinCommerce\Blocks\Utils\BlocksWpQuery;
...
abstract class AbstractProductGrid extends AbstractDynamicBlock {
	...
	protected function render( $attributes = array(), $content = '', $block = null ) {
		 ...
		 *
		 * @see https://github.com/dieselfox1/fincommerce-blocks/pull/6849
		 */
		 ...
		 *
		 * Fires a WP Hook named `experimental__fincommerce_blocks-product-list-render` on render so that the client
	...
	protected function get_products() {
		 ...
		 * @since 2.5.0
		 */
		$is_cacheable      = (bool) apply_filters( 'fincommerce_blocks_product_grid_is_cacheable', true, $this->query_args );
	...
	protected function get_container_classes() {
		$classes = array(
			...
			"wp-block-fincommerce-{$this->block_name}",
	...
	protected function get_sale_badge_html( $product ) {
		...
		return '<div class="wc-block-grid__product-onsale">
			<span aria-hidden="true">' . esc_html__( 'Sale', 'fincommerce' ) . '</span>
			<span class="screen-reader-text">' . esc_html__( 'Product on sale', 'fincommerce' ) . '</span>
		</div>';
	}
	...
	protected function get_add_to_cart( $product ) {
		 ...
		 */
		$attributes = apply_filters( 'fincommerce_blocks_product_grid_add_to_cart_attributes', $attributes, $product );
