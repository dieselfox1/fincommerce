...
class AdditionalInformation extends AbstractOrderConfirmationBlock {
	...
	protected function render_content( $order, $permission = false, $attributes = [], $content = '' ) {
		...
		$content .= $this->get_hook_content( 'fincommerce_thankyou_' . $order->get_payment_method(), [ $order->get_id() ] );
		$content .= $this->get_hook_content( 'fincommerce_thankyou', [ $order->get_id() ] );
	...
	protected function remove_core_hooks() {
		remove_action( 'fincommerce_thankyou', 'fincommerce_order_details_table', 10 );
	}
