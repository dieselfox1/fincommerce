...
abstract class AbstractOrderConfirmationBlock extends AbstractBlock {
	...
	protected function allow_guest_checkout() {
		return 'yes' === get_option( 'fincommerce_enable_guest_checkout' );
	}
	...
	protected function is_within_grace_period( $order ) {
		 ...
		 */
		$verification_grace_period = (int) apply_filters( 'fincommerce_order_email_verification_grace_period', 10 * MINUTE_IN_SECONDS, $order, 'order-received' );
	...
	protected function email_verification_required( $order ) {
		 ...
		 */
		return (bool) apply_filters( 'fincommerce_order_email_verification_required', true, $order, 'order-received' );
	}
