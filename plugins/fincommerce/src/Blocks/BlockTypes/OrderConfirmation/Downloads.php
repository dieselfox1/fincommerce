	...
	protected function render_order_download_row( $download ) {
		...
		foreach ( wc_get_account_downloads_columns() as $column_id => $column_name ) {
			...
			if ( has_action( 'fincommerce_account_downloads_column_' . $column_id ) ) {
				$return .= $this->get_hook_content( 'fincommerce_account_downloads_column_' . $column_id, [ $download ] );
			} else {
				switch ( $column_id ) {
					case 'download-product':
						...
						break;
					case 'download-file':
						$return .= '<a href="' . esc_url( $download['download_url'] ) . '" class="fincommerce-MyAccount-downloads-file button alt">' . esc_html( $download['download_name'] ) . '</a>';
						break;
					case 'download-remaining':
						$return .= is_numeric( $download['downloads_remaining'] ) ? esc_html( $download['downloads_remaining'] ) : esc_html__( '&infin;', 'fincommerce' );
						break;
					case 'download-expires':
						if ( ! empty( $download['access_expires'] ) ) {
							...
							$return .= esc_html__( 'Never', 'fincommerce' );
						}
						break;
				}
			}
