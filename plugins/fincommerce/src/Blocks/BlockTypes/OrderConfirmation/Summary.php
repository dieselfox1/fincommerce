...
class Summary extends AbstractOrderConfirmationBlock {
	...
	protected function render_content( $order, $permission = false, $attributes = [], $content = '' ) {
		...
		$content .= $this->render_summary_row( __( 'Order #:', 'fincommerce' ), $order->get_order_number() );
		$content .= $this->render_summary_row( __( 'Date:', 'fincommerce' ), wc_format_datetime( $order->get_date_created() ) );
		$content .= $this->render_summary_row( __( 'Total:', 'fincommerce' ), $order->get_formatted_order_total() );
		$content .= $this->render_summary_row( __( 'Email:', 'fincommerce' ), $order->get_billing_email() );
		$content .= $this->render_summary_row( __( 'Payment:', 'fincommerce' ), $order->get_payment_method_title() );
		$content .= '</ul>';
