...
namespace Automattic\FinCommerce\Blocks\BlockTypes\OrderConfirmation;
...
class Status extends AbstractOrderConfirmationBlock {
	...
	protected function render_content( $order, $permission = false, $attributes = [], $content = '' ) {
		if ( ! $permission ) {
			...
			return '<p>' . wp_kses_post( apply_filters( 'fincommerce_thankyou_order_received_text', esc_html__( 'Thank you. Your order has been received.', 'fincommerce' ), null ) ) . '</p>';
		}

		$content = $this->get_hook_content( 'fincommerce_before_thankyou', [ $order->get_id() ] );
		...
		switch ( $status ) {
			case 'cancelled':
				$content .= '<h1>' . wp_kses_post(
					 ...
					 */
					apply_filters(
						'fincommerce_thankyou_order_received_title',
						esc_html__( 'Order cancelled', 'fincommerce' ),
						$order
					)
				) . '</h1>';
				$content .= '<p>' . wp_kses_post(
						// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
					apply_filters(
						'fincommerce_thankyou_order_received_text',
						esc_html__( 'Your order has been cancelled.', 'fincommerce' ),
						$order
					)
				) . '</p>';
				break;
			case 'refunded':
					$content .= '<h1>' . wp_kses_post(
						...
						apply_filters(
							'fincommerce_thankyou_order_received_title',
							esc_html__( 'Order refunded', 'fincommerce' ),
							$order
						)
					) . '</h1>';
					$content .= '<p>' . wp_kses_post(
						sprintf(
							...
							apply_filters(
								...
								esc_html__( 'Your order was refunded %s.', 'fincommerce' ),
								$order
							),
			...
			case 'completed':
				$content .= '<h1>' . wp_kses_post(
					...
					apply_filters(
						'fincommerce_thankyou_order_received_title',
						esc_html__( 'Order completed', 'fincommerce' ),
						$order
					)
				) . '</h1>';
				$content .= '<p>' . wp_kses_post(
					...
					apply_filters(
						'fincommerce_thankyou_order_received_text',
						esc_html__( 'Thank you. Your order has been fulfilled.', 'fincommerce' ),
						$order
					)
				) . '</p>';
				break;
			case 'failed':
				// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
				$order_received_text = apply_filters( 'fincommerce_thankyou_order_received_text', esc_html__( 'Your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'fincommerce' ), null );
				$actions             = '<a href="' . esc_url( $order->get_checkout_payment_url() ) . '" class="button">' . esc_html__( 'Try again', 'fincommerce' ) . '</a> ';

				if ( wc_get_page_permalink( 'myaccount' ) ) {
					$actions .= '<a href="' . esc_url( wc_get_page_permalink( 'myaccount' ) ) . '" class="button">' . esc_html__( 'My account', 'fincommerce' ) . '</a> ';
				}
				$content .= '<h1>' . wp_kses_post(
					// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
					apply_filters(
						'fincommerce_thankyou_order_received_title',
						esc_html__( 'Order failed', 'fincommerce' ),
						$order
					)
				) . '</h1>';
				$content .= '
			...
			default:
				$content .= '<h1>' . wp_kses_post(
					...
					apply_filters(
						'fincommerce_thankyou_order_received_title',
						esc_html__( 'Order received', 'fincommerce' ),
						$order
					)
				) . '</h1>';
				$content .= '<p>' . wp_kses_post(
					...
					apply_filters(
						'fincommerce_thankyou_order_received_text',
						esc_html__( 'Thank you. Your order has been received.', 'fincommerce' ),
						$order
					)
				) . '</p>';
				break;
	...
	protected function render_content_fallback() {
		// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		return '<p>' . esc_html__( 'Please check your email for the order confirmation.', 'fincommerce' ) . '</p>';
	}
	...
	protected function render_confirmation_notice( $order = null ) {
		if ( ! $order ) {
			$content = '<p>' . esc_html__( 'If you\'ve just placed an order, give your email a quick check for the confirmation.', 'fincommerce' );

			if ( wc_get_page_permalink( 'myaccount' ) ) {
				$content .= ' ' . sprintf(
					/* translators: 1: opening a link tag 2: closing a link tag */
					esc_html__( 'Have an account with us? %1$sLog in here to view your order details%2$s.', 'fincommerce' ),
		...
		$content  = '<p>';
		$content .= esc_html__( 'Great news! Your order has been received, and a confirmation will be sent to your email address.', 'fincommerce' );
		$content .= $my_account_page ? ' ' . sprintf(
			/* translators: 1: opening a link tag 2: closing a link tag */
			esc_html__( 'Have an account with us? %1$sLog in here%2$s to view your order.', 'fincommerce' ),
			'<a href="' . esc_url( $my_account_page ) . '" class="button">',
			'</a>'
		) : '';

		if ( $verification_required && $verification_permitted ) {
			$content .= ' ' . esc_html__( 'Alternatively, confirm the email address linked to the order below.', 'fincommerce' );
		}
	...
	protected function render_verification_form() {
		...
		return '<form method="post" class="fincommerce-form fincommerce-verify-email">' .
			...
			sprintf(
				...
				esc_html__( 'Email address', 'fincommerce' ) . '&nbsp;<span class="required">*</span>'
			) .
			sprintf(
				...
				esc_html__( 'Confirm email and view order', 'fincommerce' ),
