...
use Automattic\FinCommerce\StoreApi\Utilities\OrderController;
...
class CreateAccount extends AbstractOrderConfirmationBlock {
	 ...
	 * Initialize hooks.
	 *
	 * @see https://developer.finpress.org/reference/hooks/hooked_block/
	 */
	protected function initialize_hooks() {
		...
		add_filter(
			...
			function ( $hooked_block_types, $relative_position, $anchor_block_type, $context ) {
				if ( 'after' !== $relative_position || 'fincommerce/order-confirmation-summary' !== $anchor_block_type || ! $context instanceof \WP_Block_Template ) {
		...
		add_filter(
			'hooked_block_fincommerce/order-confirmation-create-account',
			function ( $parsed_hooked_block, $hooked_block_type, $relative_position ) {
				...
				$site_title_heading                  = sprintf( __( 'Create an account with %s', 'fincommerce' ), wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ) );
				$parsed_hooked_block['innerContent'] = array(
					'<div class="wp-block-fincommerce-order-confirmation-create-account alignwide">
					<!-- wp:heading {"level":3} -->
                    ...
                    <li>' . esc_html__( 'Faster future purchases', 'fincommerce' ) . '</li>
                    <!-- /wp:list-item -->
                    <!-- wp:list-item -->
                    <li>' . esc_html__( 'Securely save payment info', 'fincommerce' ) . '</li>
                    <!-- /wp:list-item -->
                    <!-- wp:list-item -->
                    <li>' . esc_html__( 'Track orders &amp; view shopping history', 'fincommerce' ) . '</li>
                    <!-- /wp:list-item --></ul>
                    <!-- /wp:list -->
                    </div>',
				);
	...
	protected function is_feature_enabled() {
		return get_option( 'fincommerce_enable_delayed_account_creation', 'yes' ) === 'yes';
	}
	...
	protected function process_form_post( $order ) {
		...
		if ( ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ?? '' ) ), 'wc_create_account' ) ) {
			return new \WP_Error( 'invalid_nonce', __( 'Unable to create account. Please try again.', 'fincommerce' ) );
		}
		...
		if ( $order->get_customer_id() ) {
			return new \WP_Error( 'order_already_has_user', __( 'This order is already linked to a user account.', 'fincommerce' ) );
		}

		// Check given details match the current viewed order.
		if ( $order->get_billing_email() !== $user_email ) {
			return new \WP_Error( 'email_mismatch', __( 'The email address provided does not match the email address on this order.', 'fincommerce' ) );
		}

		$generate_password = filter_var( get_option( 'fincommerce_registration_generate_password', 'no' ), FILTER_VALIDATE_BOOLEAN );

		if ( $generate_password ) {
			$password = ''; // Will be generated by wc_create_new_customer.
		} else {
			$password = wp_unslash( $_POST['password'] ?? '' ); // phpcs:ignore finpress.Security.ValidatedSanitizedInput.InputNotSanitized

			if ( empty( $password ) || strlen( $password ) < 8 ) {
				return new \WP_Error( 'password_too_short', __( 'Password must be at least 8 characters.', 'fincommerce' ) );
			}
		}
	...
	protected function render_content( $order, $permission = false, $attributes = [], $content = '' ) {
		...
		if ( ! $processor->next_tag( array( 'class_name' => 'wp-block-fincommerce-order-confirmation-create-account' ) ) ) {
			return $content;
		}
	...
	protected function render_confirmation() {
		...
		$content .= '<h3>' . esc_html__( 'Your account has been successfully created', 'fincommerce' ) . '</h3>';
		$content .= '<p>' . sprintf(
			/* translators: 1: link to my account page, 2: link to shipping and billing addresses, 3: link to account details, 4: closing tag */
			esc_html__( 'You can now %1$sview your recent orders%4$s, manage your %2$sshipping and billing addresses%4$s, and edit your %3$spassword and account details%4$s.', 'fincommerce' ),
	...
	protected function enqueue_data( array $attributes = [] ) {
		...
		$this->asset_data_registry->add( 'registrationGeneratePassword', filter_var( get_option( 'fincommerce_registration_generate_password' ), FILTER_VALIDATE_BOOLEAN ) );
	}
}
