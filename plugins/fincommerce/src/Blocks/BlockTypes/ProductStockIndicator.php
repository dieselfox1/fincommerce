	...
	protected function render( $attributes, $content, $block ) {
		...
		if ( empty( $content ) && $is_backorder_notification_visible && $total_stock > 0 ) {
			$low_stock_text = sprintf(
				...
				__( '%d left in stock', 'fincommerce' ),
				$total_stock
			);
		}
		...
		if ( $is_interactive && 'out-of-stock' !== $availability['class'] ) {
			...
			$wrapper_attributes['data-wp-interactive'] = 'fincommerce/product-elements';
		...
		$output  = '';
		$output .= '<div class="wc-block-components-product-stock-indicator wp-block-fincommerce-product-stock-indicator ' . esc_attr( $classnames ) . '"';
