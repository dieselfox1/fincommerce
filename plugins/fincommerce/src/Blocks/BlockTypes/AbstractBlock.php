...
abstract class AbstractBlock {
	 ...
	 * @var string
	 */
	protected $namespace = 'fincommerce';
	...
	protected function initialize() {
		if ( empty( $this->block_name ) ) {
			_doing_it_wrong( __METHOD__, esc_html__( 'Block name is required.', 'fincommerce' ), '4.5.0' );
			return false;
		}
	...
	protected function get_chunks_paths( $chunks_folder ) {
		$build_path = \Automattic\FinCommerce\Blocks\Package::get_path() . 'assets/client/blocks/';
		$blocks     = [];
	...
	protected function enqueue_data( array $attributes = [] ) {
		...
		if ( ! $this->asset_data_registry->exists( 'wcBlocksConfig' ) ) {
			$wc_blocks_config = [
				 ...
				 */
				'wordCountType' => _x( 'words', 'Word count type. Do not translate!', 'fincommerce' ),
			];
			if ( is_admin() && ! WC()->is_rest_api_request() ) {
	...
	protected function get_routes_from_namespace( $namespace ) {
		...
		$routes = apply_filters(
			'fincommerce_blocks_pre_get_routes_from_namespace',
			array(),
			$namespace,
			'view'
		);
