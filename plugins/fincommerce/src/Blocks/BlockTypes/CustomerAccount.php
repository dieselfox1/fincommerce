...
class CustomerAccount extends AbstractBlock {
	...
	protected function initialize() {
		...
		if ( version_compare( get_bloginfo( 'version' ), '6.5', '>=' ) ) {
			add_filter( 'hooked_block_fincommerce/customer-account', array( $this, 'modify_hooked_block_attributes' ), 10, 5 );
	...
	public function modify_hooked_block_attributes( $parsed_hooked_block, $hooked_block_type, $relative_position, $parsed_anchor_block, $context ) {
		...
		$customer_account_block_font_size = wp_get_global_styles( array( 'blocks', 'fincommerce/customer-account', 'typography', 'fontSize' ) );
	...
	protected function render( $attributes, $content, $block ) {
		...
		$account_link = get_option( 'fincommerce_myaccount_page_id' ) ? wc_get_account_endpoint_url( 'dashboard' ) : wp_login_url();
		...
		return "<div class='wp-block-fincommerce-customer-account " . esc_attr( $classes_and_styles['classes'] ) . "' style='" . esc_attr( $classes_and_styles['styles'] ) . "'>
