<?php
namespace Automattic\FinCommerce\Blocks\Shipping;
...
class ShippingController {
	...
	public function init() {
		...
		$this->asset_data_registry->add( 'shippingCostRequiresAddress', get_option( 'fincommerce_shipping_cost_requires_address', false ) === 'yes' );
		add_action( 'rest_api_init', array( $this, 'register_settings' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'admin_scripts' ) );
		add_action( 'admin_footer', array( $this, 'hydrate_client_settings' ), 0 );
		add_action( 'fincommerce_load_shipping_methods', array( $this, 'register_local_pickup' ) );
		add_filter( 'fincommerce_local_pickup_methods', array( $this, 'register_local_pickup_method' ) );
		add_filter( 'fincommerce_order_hide_shipping_address', array( $this, 'hide_shipping_address_for_local_pickup' ), 10 );
		add_filter( 'fincommerce_customer_taxable_address', array( $this, 'filter_taxable_address' ) );
		add_filter( 'fincommerce_shipping_settings', array( $this, 'remove_shipping_settings' ) );
		add_filter( 'fincommerce_shipping_packages', array( $this, 'filter_shipping_packages' ) );
		add_filter( 'pre_update_option_fincommerce_pickup_location_settings', array( $this, 'flush_cache' ) );
		add_filter( 'pre_update_option_pickup_location_pickup_locations', array( $this, 'flush_cache' ) );
		add_filter( 'fincommerce_shipping_packages', array( $this, 'remove_shipping_if_no_address' ), 11 );
		add_filter( 'fincommerce_order_shipping_to_display', array( $this, 'show_local_pickup_details' ), 10, 2 );
	...
	public function show_local_pickup_details( $return_value, $order ) {
		...
		if ( $location ) {
			$lines[] = sprintf(
				...
				__( 'Collection from <strong>%s</strong>:', 'fincommerce' ),
				$location
			);
		}
		...
		if ( $cost > 0 ) {
			$tax_display = get_option( 'fincommerce_tax_display_cart' );
			...
			if ( 'excl' === $tax_display ) {
				...
				if ( (float) $tax > 0 && $order->get_prices_include_tax() ) {
					...
					$formatted_cost .= apply_filters(
						'fincommerce_order_shipping_to_display_tax_label',
	...
	public function remove_shipping_settings( $settings ) {
		if ( CartCheckoutUtils::is_cart_block_default() ) {
			foreach ( $settings as $index => $setting ) {
				if ( 'fincommerce_enable_shipping_calc' === $setting['id'] ) {
					$settings[ $index ]['desc_tip'] = sprintf(
					/* translators: %s: URL to the documentation. */
						__( 'This feature is not available when using the <a href="%s">Cart and checkout blocks</a>. Shipping will be calculated at checkout.', 'fincommerce' ),
						'https://fincommerce.com/document/fincommerce-store-editing/customizing-cart-and-checkout/'
					);
	...
	public function register_settings() {
		register_setting(
			'options',
			'fincommerce_pickup_location_settings',
			array(
				'type'         => 'object',
				'description'  => 'FinCommerce Local Pickup Method Settings',
				'default'      => array(),
				'show_in_rest' => array(
					...
					'schema' => array(
						...
						'properties' => array(
							'enabled'    => array(
								'description' => __( 'If enabled, this method will appear on the block based checkout.', 'fincommerce' ),
								'type'        => 'string',
								'enum'        => array( 'yes', 'no' ),
							),
							'title'      => array(
							...
							'tax_status' => array(
								'description' => __( 'If a cost is defined, this controls if taxes are applied to that cost.', 'fincommerce' ),
								'type'        => 'string',
								'enum'        => array( ProductTaxStatus::TAXABLE, ProductTaxStatus::NONE ),
							),
							'cost'       => array(
								'description' => __( 'Optional cost to charge for local pickup.', 'fincommerce' ),
								'type'        => 'string',
							),
						),
					),
		...
		register_setting(
			...
			array(
				'type'         => 'array',
				'description'  => 'FinCommerce Local Pickup Locations',
				'default'      => array(),
				'show_in_rest' => array(
	...
	/**
	 * Declares the Pickup Location shipping method as a Local Pickup method for FinCommerce.
	 *
	...
	public function filter_taxable_address( $address ) {
		...
		if ( $chosen_method_id && true === apply_filters( 'fincommerce_apply_base_tax_for_local_pickup', true ) && in_array( $chosen_method_id, LocalPickupUtils::get_local_pickup_method_ids(), true ) ) {
	...
	public function remove_shipping_if_no_address( $packages ) {
		...
		$shipping_cost_requires_address = wc_string_to_bool( get_option( 'fincommerce_shipping_cost_requires_address', 'no' ) );
	...
	public function track_local_pickup( $served, $result, $request ) {
		...
		$data = array(
			...
			'title'                    => __( 'Pickup', 'fincommerce' ) === $settings['title'],
