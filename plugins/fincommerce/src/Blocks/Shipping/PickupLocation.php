...
class PickupLocation extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'Local pickup', 'fincommerce' );
		$this->method_description = __( 'Allow customers to choose a local pickup location during checkout.', 'fincommerce' );
		$this->init();
	}
	...
	public function init() {
		...
		$this->title            = $this->get_option( 'title', __( 'Pickup', 'fincommerce' ) );
		$this->tax_status       = $this->get_option( 'tax_status' );
		$this->cost             = $this->get_option( 'cost' );
		$this->supports         = [ 'settings', 'local-pickup' ];
		$this->pickup_locations = get_option( $this->id . '_pickup_locations', [] );
		add_filter( 'fincommerce_attribute_label', array( $this, 'translate_meta_data' ), 10, 3 );
	}
	...
	public function is_available( $package ) {
		...
		return apply_filters( 'fincommerce_shipping_' . $this->id . '_is_available', 'yes' === $this->enabled, $package, $this );
	}
	...
	public function translate_meta_data( $label, $name, $product ) {
		...
		switch ( $name ) {
			case 'pickup_location':
				return __( 'Pickup location', 'fincommerce' );
			case 'pickup_address':
				return __( 'Pickup address', 'fincommerce' );
			case 'pickup_details':
				return __( 'Pickup details', 'fincommerce' );
		}
		return $label;
	}
	...
	public function admin_options() {
		...
		echo '<h2>' . esc_html__( 'Local pickup', 'fincommerce' ) . '</h2>';
