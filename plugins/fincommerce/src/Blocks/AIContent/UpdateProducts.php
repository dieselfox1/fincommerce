...
namespace Automattic\FinCommerce\Blocks\AIContent;
...
class UpdateProducts {
	...
	public function fetch_dummy_products_to_update() {
		...
		if ( ! is_array( $dummy_products_ids ) ) {
			return new \WP_Error( 'failed_to_fetch_dummy_products', __( 'Failed to fetch dummy products.', 'fincommerce' ) );
		}
	...
	private function product_update( $product, $product_image_id, $product_title, $product_description, $product_price ) {
		if ( ! $product instanceof \WC_Product ) {
			return new WP_Error( 'invalid_product', __( 'Invalid product.', 'fincommerce' ) );
		}
		...
		if ( ! is_wp_error( $product_image_id ) ) {
			...
			wc_get_logger()->warning(
				sprintf(
					...
					__( 'The image upload failed: "%s", creating the product without image', 'fincommerce' ),
