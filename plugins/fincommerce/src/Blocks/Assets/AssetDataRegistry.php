<?php
namespace Automattic\FinCommerce\Blocks\Assets;
...
class AssetDataRegistry {
	...
	protected function get_core_data() {
		return [
			...
			'currentUserIsAdmin'     => current_user_can( 'manage_fincommerce' ),
	...
	protected function get_currency_data() {
		...
		return [
			...
			'symbol'            => html_entity_decode( get_fincommerce_currency_symbol( $currency ) ),
	...
	protected function get_products_settings() {
		return [
			'cartRedirectAfterAdd' => get_option( 'fincommerce_cart_redirect_after_add' ) === 'yes',
		];
	}
	...
	protected function initialize_core_data() {
		 ...
		 * @return array
		 */
		$settings = apply_filters( 'fincommerce_shared_settings', $this->data );
	...
	public function add( $key, $data, $check_key_exists = false ) {
		if ( $check_key_exists ) {
			wc_deprecated_argument( 'Automattic\FinCommerce\Blocks\Assets\AssetDataRegistry::add()', '8.9', 'The $check_key_exists parameter is no longer used: all duplicate data will be ignored if the key exists by default' );
		}
