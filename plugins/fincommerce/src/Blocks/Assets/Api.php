...
class Api {
	 ...
	 * @var string
	 */
	private $script_data_transient_key = 'fincommerce_blocks_asset_api_script_data';
	...
	public function register_script( $handle, $relative_src, $dependencies = [], $has_i18n = true ) {
				...
				$dependencies = array_diff( $script_data['dependencies'], [ $handle ] );
					add_action(
						...
						function () use ( $handle ) {
								...
								printf( esc_html__( 'Script with handle %s had a dependency on itself which has been removed. This is an indicator that your JS code has a circular dependency that can cause bugs.', 'fincommerce' ), esc_html( $handle ) );
								echo '</p></div>';
						}
					);
			} else {
		...
		if ( $has_i18n && function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( $handle, 'fincommerce', $this->package->get_path( 'languages' ) );
			wp_set_script_translations( $handle, 'fincommerce', $this->package->get_path( 'i18n/languages' ) );
		}
	}
