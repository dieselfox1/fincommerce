...
namespace Automattic\FinCommerce\Blocks\Utils;
...
class CartCheckoutUtils {
	...
	private static function is_page_type( string $page_type ): ?bool {
		...
		if ( $post instanceof \WP_Post ) {
			return wc_post_content_has_shortcode( 'cart' === $page_type ? 'fincommerce_cart' : 'fincommerce_checkout' ) || self::has_block_variation( 'fincommerce/classic-shortcode', 'shortcode', $page_type, $post->post_content );
		}
	...
	public static function has_block_variation( $block_id, $attribute, $value, $post_content ) {
		...
		if ( has_block( $block_id, $post_content ) ) {
			...
			foreach ( $blocks as $block ) {
				...
				// `Cart` is default for `fincommerce/classic-shortcode` so it will be empty in the block attributes.
				if ( 'fincommerce/classic-shortcode' === $block_id && 'shortcode' === $attribute && 'cart' === $value && ! isset( $block['attrs']['shortcode'] ) ) {
					return true;
				}
			}
		}
	...
	public static function is_cart_block_default() {
		if ( wp_is_block_theme() ) {
			...
			foreach ( $templates_from_db as $template ) {
				if ( has_block( 'fincommerce/cart', $template->content ) ) {
					return true;
				}
			}
		}
		$cart_page_id = wc_get_page_id( 'cart' );
		return $cart_page_id && has_block( 'fincommerce/cart', $cart_page_id );
	}
	...
	public static function is_checkout_block_default() {
		if ( wp_is_block_theme() ) {
			...
			foreach ( $templates_from_db as $template ) {
				if ( has_block( 'fincommerce/checkout', $template->content ) ) {
					return true;
				}
			}
		}
		$checkout_page_id = wc_get_page_id( 'checkout' );
		return $checkout_page_id && has_block( 'fincommerce/checkout', $checkout_page_id );
	}

	/**
	 * Migrate checkout block field visibility attributes to settings when using the checkout block.
	 *
	 * This migration routine is called if the options (fincommerce_checkout_phone_field, fincommerce_checkout_company_field,
	 * fincommerce_checkout_address_2_field) are not set. They are not set by default; they were orignally set by the
	...
	protected static function migrate_checkout_block_field_visibility_attributes() {
		...
		update_option( 'fincommerce_checkout_phone_field', 'optional' );
		update_option( 'fincommerce_checkout_company_field', 'hidden' );
		update_option( 'fincommerce_checkout_address_2_field', 'optional' );

		// Parse the block from the checkout page.
		$checkout_blocks = \WC_Blocks_Utils::get_blocks_from_page( 'fincommerce/checkout', 'checkout' );
		...
		if ( $block_attributes['showPhoneField'] ) {
			update_option( 'fincommerce_checkout_phone_field', $block_attributes['requirePhoneField'] ? 'required' : 'optional' );
		} else {
			update_option( 'fincommerce_checkout_phone_field', 'hidden' );
		}

		if ( $block_attributes['showCompanyField'] ) {
			update_option( 'fincommerce_checkout_company_field', $block_attributes['requireCompanyField'] ? 'required' : 'optional' );
		} else {
			update_option( 'fincommerce_checkout_company_field', 'hidden' );
		}

		if ( $block_attributes['showApartmentField'] ) {
			update_option( 'fincommerce_checkout_address_2_field', $block_attributes['requireApartmentField'] ? 'required' : 'optional' );
		} else {
			update_option( 'fincommerce_checkout_address_2_field', 'hidden' );
		}
	}
	...
	public static function get_company_field_visibility() {
		$option_value = get_option( 'fincommerce_checkout_company_field' );
		...
		if ( self::is_checkout_block_default() ) {
			...
			return get_option( 'fincommerce_checkout_company_field', 'hidden' );
		}
	...
	public static function get_address_2_field_visibility() {
		$option_value = get_option( 'fincommerce_checkout_address_2_field' );
		...
		if ( self::is_checkout_block_default() ) {
			self::migrate_checkout_block_field_visibility_attributes();
			return get_option( 'fincommerce_checkout_address_2_field', 'optional' );
		}
	...
	public static function get_phone_field_visibility() {
		$option_value = get_option( 'fincommerce_checkout_phone_field' );
		...
		if ( self::is_checkout_block_default() ) {
			self::migrate_checkout_block_field_visibility_attributes();
			return get_option( 'fincommerce_checkout_phone_field', 'optional' );
		}
	...
	public static function is_overriden_by_custom_template_content( string $block ): bool {
		...
		$block = str_replace( 'fincommerce/', '', $block );

		if ( wp_is_block_theme() ) {
			$templates_from_db = BlockTemplateUtils::get_block_templates_from_db( array( 'page-' . $block ) );
			foreach ( $templates_from_db as $template ) {
				if ( ! has_block( 'fincommerce/page-content-wrapper', $template->content ) ) {
					// Return true if the template does not load the page content via the  fincommerce/page-content-wrapper block.
					return true;
				}
			}
		}
	...
	/**
	 * Retrieves formatted shipping zones from FinCommerce.
	 *
	...
	public static function get_shipping_zones() {
		...
		$formatted_shipping_zones[] = [
			'id'          => 0,
			'title'       => __( 'International', 'fincommerce' ),
			'description' => __( 'Locations outside all other zones', 'fincommerce' ),
		];
	...
	public static function find_express_checkout_attributes( $blocks, $cart_or_checkout ) {
		$express_block_name = 'fincommerce/' . $cart_or_checkout . '-express-payment-block';
		foreach ( $blocks as $block ) {
