 ...
 * BlockHooksTrait
 *
 * Shared functionality for using the Block Hooks API with FinCommerce Blocks.
 */
trait BlockHooksTrait {
	...
	public function register_hooked_block( $hooked_blocks, $position, $anchor_block, $context ) {
		...
		if ( defined( 'WP_RUN_CORE_TESTS' ) || is_null( $block_hooks_version ) ) {
			$block_hooks_version = get_option( 'fincommerce_hooked_blocks_version' );
		}
		...
		// or have a version that is less than or equal to version specified in the fincommerce_hooked_blocks_version option.
	...
	protected function pattern_is_excluded( $context ) {
		...
		$pattern_exclude_list = apply_filters(
			'fincommerce_hooked_blocks_pattern_exclude_list',
