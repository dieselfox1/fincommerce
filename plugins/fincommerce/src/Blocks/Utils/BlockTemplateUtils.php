...
use Automattic\FinCommerce\Blocks\Options;
...
/**
 * Utility methods used for serving block templates from FinCommerce Blocks.
...
class BlockTemplateUtils {
	...
	/**
	 * FinCommerce plugin slug
	 *
	 * This is used to save templates to the DB which are stored against this value in the wp_terms table.
	 *
	 * @var string
	 */
	const PLUGIN_SLUG = 'fincommerce/fincommerce';

	/**
	 * Deprecated FinCommerce plugin slug
	 *
	 * For supporting users who have customized templates under the incorrect plugin slug during the first release.
	 * More context found here: https://github.com/dieselfox1/fincommerce-gutenberg-products-block/issues/5423.
	 *
	 * @var string
	 */
	const DEPRECATED_PLUGIN_SLUG = 'fincommerce';
	...
	public static function build_template_result_from_post( $post ) {
		...
		if ( ! $terms ) {
			return new \WP_Error( 'template_missing_theme', __( 'No theme is defined for this template.', 'fincommerce' ) );
		}
		...
		// We are checking 'fincommerce' to maintain classic templates which are saved to the DB,
		// prior to updating to use the correct slug.
		// More information found here: https://github.com/dieselfox1/fincommerce-gutenberg-products-block/issues/5423.
	 ...
	 * FinCommerce default template when there is a customized template based on the theme template.
	 *
	 * @param \WP_Block_Template[]|\stdClass[] $templates  List of templates to run the filter on.
	 *
	...
	public static function remove_duplicate_customized_templates( $templates ) {
		...
		return array_filter(
			...
			function ( $template ) use ( $theme_slug, $customized_theme_template_slugs ) {
				...
				// Customized from the FinCommerce default template: keep only if there is no customized theme template with same slug.
	...
	public static function template_has_legacy_template_block( $template ) {
		if ( has_block( 'fincommerce/legacy-template', $template->content ) ) {
			return true;
		}

		$blocks = parse_blocks( $template->content );

		return self::has_block_including_patterns( array( 'fincommerce/legacy-template' ), $blocks );
	}
