...
use Automattic\FinCommerce\Blocks\BlockTypes\Cart;
use Automattic\FinCommerce\Blocks\BlockTypes\Checkout;
use Automattic\FinCommerce\Blocks\BlockTypes\MiniCartContents;
...
final class BlockTypesController {
	...
	/**
	 * Holds the registered blocks that have FinCommerce blocks as their parents.
	 *
	 * @var array List of registered blocks.
	 */
	private $registered_blocks_with_fincommerce_parents;
	 ...
	 */
	protected function init() { // phpcs:ignore FinCommerce.Functions.InternalInjectionMethod.MissingPublic
		...
		add_action( 'fincommerce_login_form_end', array( $this, 'redirect_to_field' ) );
		add_filter( 'widget_types_to_hide_from_legacy_widget_block', array( $this, 'hide_legacy_widgets_with_block_equivalent' ) );
		add_action( 'fincommerce_delete_product_transients', array( $this, 'delete_product_transients' ) );
		add_filter( 'register_block_type_args', array( $this, 'enqueue_block_style_for_classic_themes' ), 10, 2 );
	}

	/**
	 * Get registered blocks that have FinCommerce blocks as their parents. Adds the value to the
	 * `registered_blocks_with_fincommerce_parents` cache if `init` has been fired.
	 *
	 * @return array Registered blocks with FinCommerce blocks as parents.
	 */
	public function get_registered_blocks_with_fincommerce_parent() {
		// If init has run and the cache is already set, return it.
		if ( did_action( 'init' ) && ! empty( $this->registered_blocks_with_fincommerce_parents ) ) {
			return $this->registered_blocks_with_fincommerce_parents;
		}
		...
		$this->registered_blocks_with_fincommerce_parents = array_filter(
			$registered_blocks,
			function ( $block ) {
				...
				$fincommerce_blocks = array_filter(
					$block->parent,
					function ( $parent_block_name ) {
						return 'fincommerce' === strtok( $parent_block_name, '/' );
					}
				);
				return ! empty( $fincommerce_blocks );
			}
		);
		return $this->registered_blocks_with_fincommerce_parents;
	}
	...
	public function register_block_patterns() {
		register_block_pattern(
			'fincommerce/order-confirmation-totals-heading',
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"level":2,"style":{"typography":{"fontSize":"24px"}}} --><h2 class="wp-block-heading" style="font-size:24px">' . esc_html__( 'Order details', 'fincommerce' ) . '</h2><!-- /wp:heading -->',
			)
		);
		register_block_pattern(
			'fincommerce/order-confirmation-downloads-heading',
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"level":2,"style":{"typography":{"fontSize":"24px"}}} --><h2 class="wp-block-heading" style="font-size:24px">' . esc_html__( 'Downloads', 'fincommerce' ) . '</h2><!-- /wp:heading -->',
			)
		);
		register_block_pattern(
			'fincommerce/order-confirmation-shipping-heading',
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"level":2,"style":{"typography":{"fontSize":"24px"}}} --><h2 class="wp-block-heading" style="font-size:24px">' . esc_html__( 'Shipping address', 'fincommerce' ) . '</h2><!-- /wp:heading -->',
			)
		);
		register_block_pattern(
			'fincommerce/order-confirmation-billing-heading',
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"level":2,"style":{"typography":{"fontSize":"24px"}}} --><h2 class="wp-block-heading" style="font-size:24px">' . esc_html__( 'Billing address', 'fincommerce' ) . '</h2><!-- /wp:heading -->',
			)
		);
		register_block_pattern(
			'fincommerce/order-confirmation-additional-fields-heading',
			array(
				'title'    => '',
				'inserter' => false,
				'content'  => '<!-- wp:heading {"level":2,"style":{"typography":{"fontSize":"24px"}}} --><h2 class="wp-block-heading" style="font-size:24px">' . esc_html__( 'Additional information', 'fincommerce' ) . '</h2><!-- /wp:heading -->',
			)
		);
	}
	 ...
	 * FinCommerce Blocks related categories to the Gutenberg editor.
	 *
	 * @param array $categories The array of already registered categories.
	 */
	public function register_block_categories( $categories ) {
		$fincommerce_block_categories = array(
			array(
				'slug'  => 'fincommerce',
				'title' => __( 'FinCommerce', 'fincommerce' ),
			),
			array(
				'slug'  => 'fincommerce-product-elements',
				'title' => __( 'FinCommerce Product Elements', 'fincommerce' ),
			),
		);

		return array_merge( $categories, $fincommerce_block_categories );
	}

	/**
	 * Check if a block should have data attributes appended on render. If it's in an allowed namespace, or the block
	 * has explicitly been added to the allowed block list, or if one of the block's parents is in the FinCommerce
	...
	public function block_should_have_data_attributes( $block_name ) {
		 ...
		 */
		$allowed_namespaces = array_merge( array( 'fincommerce', 'fincommerce-checkout' ), (array) apply_filters( '__experimental_fincommerce_blocks_add_data_attributes_to_namespace', array() ) );
		 ...
		 * @param array $allowed_namespaces List of namespaces.
		 */
		$allowed_blocks = (array) apply_filters( '__experimental_fincommerce_blocks_add_data_attributes_to_block', array() );

		$blocks_with_woo_parents   = $this->get_registered_blocks_with_fincommerce_parent();
	...
	/**
	 * Add data- attributes to blocks when rendered if the block is under the fincommerce/ namespace.
	 *
	 ...
	 * @return array $widget_types An array including the FinCommerce widgets to hide.
	 */
	public function hide_legacy_widgets_with_block_equivalent( $widget_types ) {
		array_push(
			$widget_types,
			'fincommerce_product_search',
			'fincommerce_product_categories',
			'fincommerce_recent_reviews',
			'fincommerce_product_tag_cloud',
			'fincommerce_price_filter',
			'fincommerce_layered_nav',
			'fincommerce_layered_nav_filters',
			'fincommerce_rating_filter'
		);
	...
	protected function get_block_types() {
		 ...
		 */
		return apply_filters( 'fincommerce_get_block_types', $block_types );
	}

	/**
	 * By default, when the classic theme is used, block style is always
	 * enqueued even if the block is not used on the page. We want FinCommerce
	...
	public function enqueue_block_style_for_classic_themes( $args, $block_name ) {
		if (
			...
			false === strpos( $block_name, 'fincommerce/' ) ||
