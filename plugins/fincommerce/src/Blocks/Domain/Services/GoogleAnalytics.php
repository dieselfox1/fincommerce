...
class GoogleAnalytics {
	...
	public function enqueue_scripts() {
		 ...
		 */
		if ( apply_filters( 'fincommerce_ga_disable_tracking', ! wc_string_to_bool( $settings['ga_event_tracking_enabled'] ) ) ) {
			return;
		}
	...
	private function get_google_analytics_settings() {
		return wp_parse_args(
			get_option( 'fincommerce_google_analytics_settings' ),
			[
				'ga_id'                     => '',
	...
	public function async_script_loader_tags( $tag, $handle, $src ) {
		...
		if ( did_action( 'fincommerce_gtag_snippet' ) ) {
			return '';
		}
