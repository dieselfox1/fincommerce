...
class CheckoutLink {
	...
	public function handle_checkout_link_endpoint() {
		...
		if ( ! $this->validate_checkout_link() ) {
			$redirect = add_query_arg( 'wc_error', rawurlencode( __( 'The provided checkout link was out of date or invalid. No products were added to the cart.', 'fincommerce' ) ), wc_get_cart_url() );
		} else {
			wc()->cart->empty_cart();
	...
	protected function get_checkout_link() {
		...
		if ( wc()->cart->is_empty() ) {
			$errors->add( 'error', __( 'The provided checkout link was out of date or invalid. No products were added to the cart.', 'fincommerce' ) );
