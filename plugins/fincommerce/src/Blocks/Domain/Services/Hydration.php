...
class Hydration {
	...
	private function get_response_from_controller( $controller_class, $path ) {
		 ...
		 */
		$hydration_result = apply_filters( 'fincommerce_hydration_dispatch_request', null, $request, $path, $handler );
		 ...
		 * @param WP_REST_Request                                  $request  Request used to generate the response.
		 */
		$response = apply_filters( 'fincommerce_hydration_request_after_callbacks', $response, $handler, $request );
	...
	protected function disable_nonce_check() {
		add_filter( 'fincommerce_store_api_disable_nonce_check', array( $this, 'disable_nonce_check_callback' ) );
	}
	...
	protected function restore_nonce_check() {
		remove_filter( 'fincommerce_store_api_disable_nonce_check', array( $this, 'disable_nonce_check_callback' ) );
	}
	...
	protected function cache_store_notices() {
		if ( ! did_action( 'fincommerce_init' ) || null === WC()->session ) {
			return;
		}
