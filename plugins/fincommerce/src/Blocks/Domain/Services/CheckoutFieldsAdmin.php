...
class CheckoutFieldsAdmin {
	...
	public function init() {
		add_filter( 'fincommerce_admin_billing_fields', array( $this, 'admin_address_fields' ), 10, 3 );
		add_filter( 'fincommerce_admin_billing_fields', array( $this, 'admin_contact_fields' ), 10, 3 );
		add_filter( 'fincommerce_admin_shipping_fields', array( $this, 'admin_address_fields' ), 10, 3 );
		add_filter( 'fincommerce_admin_shipping_fields', array( $this, 'admin_order_fields' ), 10, 3 );
	}

	/**
	 * Converts the shape of a checkout field to match whats needed in the FinCommerce meta boxes.
	 *
	...
	public function admin_address_fields( $fields, $order = null, $context = 'edit' ) {
		...
		$group_name        = doing_action( 'fincommerce_admin_billing_fields' ) ? 'billing' : 'shipping';
