...
namespace Automattic\FinCommerce\Blocks\Domain\Services;

use Automattic\FinCommerce\Blocks\Utils\CartCheckoutUtils;
use Automattic\FinCommerce\Blocks\Assets\AssetDataRegistry;
use Automattic\FinCommerce\Blocks\Domain\Services\CheckoutFieldsSchema\{
...
class CheckoutFields {
	...
	public function init() {
		add_filter( 'fincommerce_get_country_locale_default', array( $this, 'update_default_locale_with_fields' ) );
		add_action( 'fincommerce_blocks_checkout_enqueue_data', array( $this, 'add_fields_data' ) );
		add_action( 'fincommerce_blocks_cart_enqueue_data', array( $this, 'add_fields_data' ) );
		add_filter( 'fincommerce_customer_allowed_session_meta_keys', array( $this, 'add_session_meta_keys' ) );
	}
	...
	public function default_validate_callback( $value, $field ) {
		if ( true === $field['required'] && empty( $value ) ) {
			return new WP_Error(
				'fincommerce_required_checkout_field',
				sprintf(
					// translators: %s is field key.
					__( 'The field %s is required.', 'fincommerce' ),
					$field['id']
				)
			);
		}
	...
	public function register_checkout_field( $options ) {
		...
		$field_data = wp_parse_args(
			...
			[
				...
				'optionalLabel'              => sprintf( __( '%s (optional)', 'fincommerce' ), $options['label'] ),
				'location'                   => '',
				'type'                       => 'text',
				'hidden'                     => false,
				'required'                   => false,
		...
		if ( $document_object && $this->contains_valid_rules( $field['validation'] ) ) {
			return function ( $field_value, $field ) use ( $document_object ) {
				...
				if ( is_wp_error( $validate_result ) ) {
					...
					$error_message = sprintf( __( 'Please provide a valid %s', 'fincommerce' ), $field['label'] );
					$error_code    = 'fincommerce_invalid_checkout_field';
	...
	private function validate_options( &$options ) {
		if ( empty( $options['id'] ) ) {
			_doing_it_wrong( 'fincommerce_register_additional_checkout_field', 'A checkout field cannot be registered without an id.', '8.6.0' );
			return false;
		}
		...
		if ( count( explode( '/', $options['id'] ) ) < 2 ) {
			...
			_doing_it_wrong( 'fincommerce_register_additional_checkout_field', esc_html( $message ), '8.6.0' );
			return false;
		}
	...
	private function process_checkbox_field( $field_data, $options ) {
		...
		if ( false === $field_data['required'] && ! empty( $options['error_message'] ) ) {
			...
			_doing_it_wrong( 'fincommerce_register_additional_checkout_field', esc_html( $message ), '9.8.0' );
	...
	public function get_core_fields() {
		return [
			...
			'address_2'  => [
				'label'          => __( 'Apartment, suite, etc.', 'fincommerce' ),
				'optionalLabel'  => __(
	...
	public function sanitize_field( $field_key, $field_value ) {
		try {
			 ...
			 *
			 * @since 8.6.0
			 * @deprecated 8.7.0 Use fincommerce_sanitize_additional_field instead.
			 */
			$field_value = apply_filters_deprecated( '__experimental_fincommerce_blocks_sanitize_additional_field', array( $field_value, $field_key ), '8.7.0', 'fincommerce_sanitize_additional_field', 'This action has been graduated, use fincommerce_sanitize_additional_field instead.' );
			 ...
			 * @since 8.7.0
			 */
			return apply_filters( 'fincommerce_sanitize_additional_field', $field_value, $field_key );
		...
		try {
			...
			if ( ! empty( $field['validate_callback'] ) && is_callable( $field['validate_callback'] ) ) {
				...
				if ( is_wp_error( $validate_callback_result ) ) {
					...
					/* translators: %s: is the field label */
					$error_message = sprintf( __( 'Please provide a valid %s', 'fincommerce' ), $field['label'] );
					$errors->add( 'fincommerce_invalid_checkout_field', $error_message );
				}
			}

			wc_do_deprecated_action( '__experimental_fincommerce_blocks_validate_additional_field', array( $errors, $field['id'], $field_value ), '8.7.0', 'fincommerce_validate_additional_field', 'This action has been graduated, use fincommerce_validate_additional_field instead.' );
			 ...
			 * @since 8.7.0
			 */
			do_action( 'fincommerce_validate_additional_field', $errors, $field['id'], $field_value );
	...
	public function validate_fields_for_location( $fields, $location, $group = 'other' ) {
		...
		try {
			wc_do_deprecated_action( '__experimental_fincommerce_blocks_validate_location_' . $location . '_fields', array( $errors, $fields, $group ), '8.9.0', 'fincommerce_blocks_validate_location_' . $location . '_fields', 'This action has been graduated, use fincommerce_blocks_validate_location_' . $location . '_fields instead.' );
			 ...
			 * @since 8.7.0
			 */
			do_action( 'fincommerce_blocks_validate_location_' . $location . '_fields', $errors, $fields, $group );
			...
			trigger_error(
				sprintf(
					...
					esc_html( 'fincommerce_blocks_validate_' . $location . '_fields' ),
					esc_html( $location ),
					esc_html( $e->getMessage() )
				),
				E_USER_WARNING
	...
	public function validate_field_for_location( $key, $value, $location ) {
		...
		if ( ! $this->is_field( $key ) ) {
			return new WP_Error(
				'fincommerce_invalid_checkout_field',
				\sprintf(
				// translators: % is field key.
					__( 'The field %s is invalid.', 'fincommerce' ),
					$key
				)
			);
		}
		...
		if ( ! in_array( $key, $this->fields_locations[ $location ], true ) ) {
			return new WP_Error(
				'fincommerce_invalid_checkout_field_location',
				\sprintf(
				// translators: %1$s is field key, %2$s location.
					__( 'The field %1$s is invalid for the location %2$s.', 'fincommerce' ),
					$key,
					$location
				)
			);
	...
	private function set_array_meta( string $key, $value, WC_Data $wc_object, string $group ) {
		 ...
		 * @since 8.9.0
		 */
		do_action( 'fincommerce_set_additional_field_value', $key, $value, $group, $wc_object );
	...
	public function get_field_from_object( string $key, WC_Data $wc_object, string $group = 'other' ) {
		...
		if ( ! $value && '0' !== $value ) {
			 ...
			 * @since 8.9.0
			 */
			$value = apply_filters( "fincommerce_get_default_value_for_{$key}", null, $group, $wc_object );
		}
	...
	public function get_all_fields_from_object( WC_Data $wc_object, string $group = 'other', bool $all = false ) {
		...
		foreach ( $missing_fields as $missing_field ) {
				 ...
				 * @since 8.9.0
				 */
				$value = apply_filters( "fincommerce_get_default_value_for_{$missing_field}", null, $group, $wc_object );
	...
	public function filter_fields_for_order_confirmation( $fields, $context = array() ) {
		return array_filter(
			...
			function ( $field ) use ( $fields, $context ) {
				 ...
				 * @since 10.1.0
				 */
				return apply_filters( 'fincommerce_filter_fields_for_order_confirmation', ! empty( $field['show_in_order_confirmation'] ), $field, $fields, $context, $this );
			}
		);
	}
	...
	public function format_additional_field_value( $value, $field ) {
		if ( 'checkbox' === $field['type'] ) {
			$value = $value ? __( 'Yes', 'fincommerce' ) : __( 'No', 'fincommerce' );
		}
