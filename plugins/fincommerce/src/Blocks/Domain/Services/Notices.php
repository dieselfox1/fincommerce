...
/**
 * Service class for adding new-style Notices to FinCommerce core.
 *
 * @internal
 */
class Notices {
	...
	public function init() {
		add_action(
			...
			function () {
				 ...
				 * @return bool
				 */
				if ( wp_is_block_theme() || apply_filters( 'fincommerce_use_block_notices_in_classic_theme', false ) ) {
					add_filter( 'wc_get_template', [ $this, 'get_notices_template' ], 10, 5 );
				}
			}
		);

		add_filter( 'fincommerce_kses_notice_allowed_tags', [ $this, 'add_kses_notice_allowed_tags' ] );
	 ...
	 * 2. The block notices use different CSS classes to the core notices. Core uses `fincommerce-message`, `is-info`
	...
	public function get_notices_template( $template, $template_name, $args, $template_path, $default_path ) {
		if ( in_array( $template_name, $this->notice_templates, true ) ) {
			...
			$file      = $directory . '/fincommerce/' . $template_name;
