...
namespace Automattic\FinCommerce\Blocks\Domain\Services\CheckoutFieldsSchema;
...
class Validation {
	...
	public static function validate_document_object( DocumentObject $document_object, $rules ) {
		...
		try {
			...
			return new WP_Error( 'fincommerce_rest_checkout_validation_failed', __( 'Validation failed.', 'fincommerce' ) );
		}

		// Return generic error message.
		return new WP_Error( 'fincommerce_rest_checkout_invalid_field', __( 'Invalid field.', 'fincommerce' ) );
	}
	...
	public static function is_valid_schema( $rules ) {
		if ( ! is_array( $rules ) ) {
			return new WP_Error( 'fincommerce_rest_checkout_invalid_field_schema', 'Rules must be defined as an array.' );
		}
		...
		if ( $result->hasError() ) {
			return new WP_Error( 'fincommerce_rest_checkout_invalid_field_schema', esc_html( (string) $result->error() ) );
		}
