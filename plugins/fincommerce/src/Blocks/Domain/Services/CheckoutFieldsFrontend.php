...
class CheckoutFieldsFrontend {
	...
	public function init() {
		...
		add_action( 'fincommerce_order_details_after_customer_address', array( $this, 'render_order_address_fields' ), 10, 2 );
		add_action( 'fincommerce_order_details_after_customer_details', array( $this, 'render_order_other_fields' ), 10 );

		// Show custom checkout fields on the My Account page.
		add_action( 'fincommerce_my_account_after_my_address', array( $this, 'render_address_fields' ), 10, 1 );

		// Edit account form under my account (for contact details).
		add_filter( 'fincommerce_edit_account_form_fields', array( $this, 'edit_account_form_fields' ), 10, 1 );
		add_action( 'fincommerce_save_account_details', array( $this, 'save_account_form_fields' ), 10, 1 );

		// Edit address form under my account.
		add_filter( 'fincommerce_address_to_edit', array( $this, 'edit_address_fields' ), 10, 2 );
		add_action( 'fincommerce_customer_save_address', array( $this, 'save_address_fields' ), 10, 4 );
	}
	...
	public function edit_account_form_fields() {
		...
		foreach ( $fields as $key => $field ) {
			...
			fincommerce_form_field( $field_key, $form_field, wc_get_post_data_by_key( $key, $form_field['value'] ) );
		}
	}
	...
	public function save_address_fields( $user_id, $address_type, $address = [], $customer = null ) {
		try {
			...
			wc_add_notice(
				sprintf(
					/* translators: %s: Error message. */
					__( 'An error occurred while saving address details: %s', 'fincommerce' ),
					esc_html( $e->getMessage() )
				),
				'error'
			);
			...
			if ( empty( $field_value ) ) {
				if ( true === $field['required'] ) {
					$errors->add(
						...
						sprintf( __( '%s is required', 'fincommerce' ), '<strong>' . $field['label'] . '</strong>' )
					);
					continue;
				}
