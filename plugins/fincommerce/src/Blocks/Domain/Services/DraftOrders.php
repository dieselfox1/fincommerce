...
/**
 * Service class for adding DraftOrder functionality to FinCommerce core.
 *
...
class DraftOrders {
	...
	const DRAFT_CLEANUP_EVENT_HOOK = 'fincommerce_cleanup_draft_orders';
	...
	public function init() {
		...
		add_filter( 'fincommerce_register_shop_order_post_statuses', [ $this, 'register_draft_order_post_status' ] );
		add_filter( 'fincommerce_analytics_excluded_order_statuses', [ $this, 'append_draft_order_post_status' ] );
		add_filter( 'fincommerce_valid_order_statuses_for_payment', [ $this, 'append_draft_order_post_status' ], 999 );
		add_filter( 'fincommerce_valid_order_statuses_for_payment_complete', [ $this, 'append_draft_order_post_status' ], 999 );
		// Hook into the query to retrieve My Account orders so draft status is excluded.
		add_action( 'fincommerce_my_account_my_orders_query', [ $this, 'delete_draft_order_post_status_from_args' ] );
	...
	public function register_draft_order_status( array $statuses ) {
		$statuses[ self::DB_STATUS ] = _x( 'Draft', 'Order status', 'fincommerce' );
		return $statuses;
	}
	...
	private function get_post_status_properties() {
		return [
			'label'                     => _x( 'Draft', 'Order status', 'fincommerce' ),
			'public'                    => false,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => false,
			'show_in_admin_status_list' => true,
			/* translators: %s: number of orders */
			'label_count'               => _n_noop( 'Drafts <span class="count">(%s)</span>', 'Drafts <span class="count">(%s)</span>', 'fincommerce' ),
		];
	}
