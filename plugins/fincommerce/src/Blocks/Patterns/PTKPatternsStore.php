...
class PTKPatternsStore {
	...
	public function __construct( PTKClient $ptk_client ) {
		...
		if ( Features::is_enabled( 'pattern-toolkit-full-composability' ) ) {
			...
			// - The FinCommerce plugin is deactivated.
			// - The `fincommerce_allow_tracking` option is disabled.
			//
			// We also want to re-fetch the patterns and update the cache when:
			// - The `fincommerce_allow_tracking` option changes to enabled.
			// - The FinCommerce plugin is activated (if `fincommerce_allow_tracking` is enabled).
			// - The FinCommerce plugin is updated.

			add_action( 'fincommerce_activated_plugin', array( $this, 'flush_or_fetch_patterns' ), 10, 2 );
			add_action( 'update_option_fincommerce_allow_tracking', array( $this, 'flush_or_fetch_patterns' ), 10, 2 );
	...
	/**
	 * Resets the cached patterns when the `fincommerce_allow_tracking` option is disabled.
	...
	/**
	 * Re-fetch the patterns when the FinCommerce plugin is updated.
	 *
	...
	public function fetch_patterns_on_plugin_update( $upgrader_object, $options ) {
		if ( 'update' === $options['action'] && 'plugin' === $options['type'] && isset( $options['plugins'] ) ) {
			foreach ( $options['plugins'] as $plugin ) {
				if ( str_contains( $plugin, 'fincommerce.php' ) ) {
					$this->schedule_fetch_patterns();
				}
			}
		}
		...
		if ( is_wp_error( $patterns ) ) {
			wc_get_logger()->warning(
				...
				// translators: %s is a generated error message.
					__( 'Failed to get FinCommerce patterns from the PTK: "%s"', 'fincommerce' ),
	...
	private function allowed_tracking_is_enabled(): bool {
		return 'yes' === get_option( 'fincommerce_allow_tracking' );
	}
