...
class PTKClient {
	...
	/**
	 * Fetch the FinCommerce patterns from the Patterns Toolkit (PTK) API.
	 *
	...
	public function fetch_patterns( array $options = array() ) {
		...
		if ( is_wp_error( $patterns ) || 200 !== wp_remote_retrieve_response_code( $patterns ) ) {
			return new WP_Error(
				...
				__( 'Failed to connect with the Patterns Toolkit API: try again later.', 'fincommerce' )
			);
		}
		...
		if ( empty( $body ) ) {
			return new WP_Error(
				'patterns_toolkit_api_error',
				__( 'Empty response received from the Patterns Toolkit API.', 'fincommerce' )
			);
		}
		...
		if ( ! $is_pattern_payload_valid ) {
			return new WP_Error(
				'patterns_toolkit_api_error',
				__( 'Wrong response received from the Patterns Toolkit API: try again later.', 'fincommerce' )
			);
		}
