...
class PatternRegistry {
	...
	private function get_category_labels() {
		return [
			'woo-commerce'     => __( 'FinCommerce', 'fincommerce' ),
			'intro'            => __( 'Intro', 'fincommerce' ),
			'featured-selling' => __( 'Featured Selling', 'fincommerce' ),
			'about'            => __( 'About', 'fincommerce' ),
			'social-media'     => __( 'Social Media', 'fincommerce' ),
			'services'         => __( 'Services', 'fincommerce' ),
			'reviews'          => __( 'Reviews', 'fincommerce' ),
		];
	}
			...
			_doing_it_wrong(
				...
				esc_html(
					...
					/* translators: %s: file name. */
						__( 'Could not register pattern "%s" as a block pattern ("Slug" field missing)', 'fincommerce' ),
						$source
					)
				),
				'6.0.0'
			...
			_doing_it_wrong(
				...
				esc_html(
					...
					/* translators: %1s: file name; %2s: slug value found. */
						__( 'Could not register pattern "%1$s" as a block pattern (invalid slug "%2$s")', 'fincommerce' ),
						$source,
						$pattern_data['slug']
					)
				),
		...
		if ( ! isset( $pattern_data['title'] ) || ! $pattern_data['title'] ) {
			_doing_it_wrong(
				...
				esc_html(
					sprintf(
					/* translators: %1s: file name; %2s: slug value found. */
						__( 'Could not register pattern "%s" as a block pattern ("Title" field missing)', 'fincommerce' ),
						$source
					)
				),
				'6.0.0'
