...
namespace Automattic\FinCommerce\Checkout\Helpers;
...
final class ReserveStock {
	...
	public function __construct() {
		...
		$this->enabled = get_option( 'fincommerce_schema_version', 0 ) >= 430;
	}
	 ...
	 * @param int       $minutes How long to reserve stock in minutes. Defaults to fincommerce_hold_stock_minutes.
	 */
	public function reserve_stock_for_order( $order, $minutes = 0 ) {
		$minutes = $minutes ? $minutes : (int) get_option( 'fincommerce_hold_stock_minutes', 60 );
		/**
		 ...
		 * @since 8.8.0
		 *
		 * @param int       $minutes How long to reserve stock for the order in minutes. Defaults to fincommerce_hold_stock_minutes or 10 if block checkout entry.
		 * @param \WC_Order $order Order object.
		 */
		$minutes = (int) apply_filters( 'fincommerce_order_hold_stock_minutes', $minutes, $order );
			...
			foreach ( $items as $item ) {
				...
				if ( ! $product->is_in_stock() ) {
					throw new ReserveStockException(
						'fincommerce_product_out_of_stock',
						sprintf(
							/* translators: %s: product name */
							__( '&quot;%s&quot; is out of stock and cannot be purchased.', 'fincommerce' ),
							$product->get_name()
						),
						403
					);
				 ...
				 */
				$item_quantity = apply_filters( 'fincommerce_order_item_quantity', $item->get_quantity(), $order, $item );
				...
				if ( count( $held_stock_notes ) < 5 ) {
					...
					$held_stock_notes[] = sprintf( _x( '- %1$s &times; %2$d', 'held stock note', 'fincommerce' ), $product->get_formatted_name(), $rows[ $managed_by_id ] );
				}
			}
		...
		if ( ! empty( $held_stock_notes ) ) {
			...
			if ( $remaining_count > 0 ) {
				$held_stock_notes[] = sprintf(
					...
					_nx( '- ...and %d more item.', '- ... and %d more items.', $remaining_count, 'held stock note', 'fincommerce' ),
					$remaining_count
				);
			}
			...
			$order->add_order_note(
				sprintf(
					...
					_x( 'Stock hold of %1$s minutes applied to: %2$s', 'held stock note', 'fincommerce' ),
					$minutes,
	...
	private function reserve_stock_for_product( $product_id, $stock_quantity, $order, $minutes ) {
		...
		if ( ! $result ) {
			...
			throw new ReserveStockException(
				'fincommerce_product_not_enough_stock',
				sprintf(
					/* translators: %s: product name */
					__( 'Not enough units of %s are available in stock to fulfil this order.', 'fincommerce' ),
	...
	private function get_query_for_reserved_stock( $product_id, $exclude_order_id = 0 ) {
		 ...
		 */
		return apply_filters( 'fincommerce_query_for_reserved_stock', $query, $product_id, $exclude_order_id );
	}
}
