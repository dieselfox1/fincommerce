...
namespace Automattic\FinCommerce\Admin\Schedulers;
...
trait SchedulerTraits {
	...
	public static function get_batch_size( $action ) {
		 ...
		 */
		return apply_filters( 'fincommerce_analytics_regenerate_batch_size', $batch_size, static::$name, $action );
	}
	...
	public static function schedule_action( $action_name, $args = array() ) {
		...
		if (
			...
			apply_filters( 'fincommerce_analytics_disable_action_scheduling', false )
		) {
			call_user_func_array( array( static::class, $action_name ), $args );
			return;
		}
