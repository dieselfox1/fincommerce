<?php
/**
 * FinCommerce Async Product Editor Category Field.
 */

namespace Automattic\FinCommerce\Admin\Features\AsyncProductEditorCategoryField;
...
class Init {
	...
	public function __construct() {
		if ( Features::is_enabled( self::FEATURE_ID ) ) {
			...
			add_filter( 'fincommerce_taxonomy_args_product_cat', array( $this, 'add_metabox_args' ) );
		}
	}
	...
	public function enqueue_styles() {
		...
		wp_register_style(
			'fincommerce_admin_product_category_metabox_styles',
			WCAdminAssets::get_url( 'product-category-metabox/style', 'css' ),
			array(),
			$version
		);
		wp_style_add_data( 'fincommerce_admin_product_category_metabox_styles', 'rtl', 'replace' );

		wp_enqueue_style( 'fincommerce_admin_product_category_metabox_styles' );
	}
