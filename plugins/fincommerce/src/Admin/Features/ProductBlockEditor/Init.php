<?php
/**
 * FinCommerce Product Block Editor
 */

declare(strict_types = 1);

namespace Automattic\FinCommerce\Admin\Features\ProductBlockEditor;

use Automattic\FinCommerce\Admin\Features\Features;
use Automattic\FinCommerce\Admin\Features\ProductBlockEditor\ProductTemplate;
use Automattic\FinCommerce\Admin\PageController;
use Automattic\FinCommerce\Enums\ProductType;
use Automattic\FinCommerce\LayoutTemplates\LayoutTemplateRegistry;

use Automattic\FinCommerce\Internal\Features\ProductBlockEditor\ProductTemplates\SimpleProductTemplate;
use Automattic\FinCommerce\Internal\Features\ProductBlockEditor\ProductTemplates\ProductVariationTemplate;
use WC_Meta_Data;
...
class Init {
	 ...
	 */
	const EDITOR_CONTEXT_NAME = 'fincommerce/edit-product';
	...
	public function __construct() {
		...
		if ( \Automattic\FinCommerce\Utilities\FeaturesUtil::feature_is_enabled( 'product_block_editor' ) ) {
			...
			add_filter( 'fincommerce_register_post_type_product_variation', array( $this, 'enable_rest_api_for_product_variation' ) );
			...
			add_filter( 'fincommerce_get_block_types', array( $this, 'get_block_types' ), 999, 1 );

			add_filter( 'fincommerce_rest_prepare_product_object', array( $this, 'possibly_add_template_id' ), 10, 2 );
			add_filter( 'fincommerce_rest_prepare_product_variation_object', array( $this, 'possibly_add_template_id' ), 10, 2 );
	...
	public function possibly_add_template_id( $response, $product ) {
		...
		if ( ! $product->meta_exists( '_product_template_id' ) ) {
			 ...
			 * @since 9.1.0
			 */
			$product_template_id = apply_filters( 'experimental_fincommerce_product_editor_product_template_id_for_product', '', $product );
	...
	public function dequeue_conflicting_styles() {
		...
		// Dequeuing this to avoid conflicts, until we remove the 'fincommerce-page' class.
		wp_dequeue_style( 'fincommerce-blocktheme' );
	}
	...
	private function get_default_product_templates() {
		...
		$templates[] = new ProductTemplate(
			array(
				...
				'title'              => __( 'Standard product', 'fincommerce' ),
				'description'        => __( 'A single physical or virtual product, e.g. a t-shirt or an eBook.', 'fincommerce' ),
				'order'              => 10,
				'icon'               => 'shipping',
		...
		$templates[] = new ProductTemplate(
			array(
				...
				'title'              => __( 'Grouped product', 'fincommerce' ),
				'description'        => __( 'A set of products that go well together, e.g. camera kit.', 'fincommerce' ),
				'order'              => 20,
				'icon'               => 'group',
				'layout_template_id' => 'simple-product',
				'product_data'       => array(
		...
		$templates[] = new ProductTemplate(
			array(
				...
				'title'              => __( 'Affiliate product', 'fincommerce' ),
				'description'        => __( 'A link to a product sold on a different website, e.g. brand collab.', 'fincommerce' ),
				'order'              => 30,
				'icon'               => 'link',
				'layout_template_id' => 'simple-product',
				'product_data'       => array(
	...
	public function register_product_templates() {
		 ...
		 * @since 8.5.0
		 */
		$this->product_templates = apply_filters( 'fincommerce_product_editor_product_templates', $this->get_default_product_templates() );
	...
	public function register_user_metas() {
		register_rest_field(
			...
			array(
				...
				'schema'          => array(
					'type'        => 'array',
					'description' => __( 'The metaboxhidden_product meta from the user metas.', 'fincommerce' ),
					'items'       => array(
						'type' => 'string',
					),
					'arg_options' => array(
	 ...
	 *
	 * @see https://github.com/finpress/gutenberg/blob/6aaa3686ae67adc1a6a6b08096d3312859733e1b/lib/compat/finpress-6.5/blocks.php#L27-L47
	...
	/**
	 * Filters fincommerce block types.
	 *
	 * @param string[] $block_types Array of fincommerce block types.
	 * @return array
	 */
	public function get_block_types( $block_types ) {
		if ( PageController::is_admin_page() ) {
			// Ignore all fincommerce blocks.
			return array();
		}
