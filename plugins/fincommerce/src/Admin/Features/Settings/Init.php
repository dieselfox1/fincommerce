...
namespace Automattic\FinCommerce\Admin\Features\Settings;
...
class Init {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_admin_shared_settings', array( __CLASS__, 'add_component_settings' ) );
	...
	/**
	 * Check if the current screen is the FinCommerce settings page.
	 *
	 * @return bool
	 */
	public function is_settings_page() {
		$screen = get_current_screen();
		return $screen && 'fincommerce_page_wc-settings' === $screen->id;
	}
	...
	public function enqueue_settings_editor_styles() {
		...
		wp_register_style(
			...
			// See plugins/fincommerce/assets/client/admin/settings-editor/style.asset.php. Should be: `isset( $style_assets['dependencies'] ) ? $style_assets['dependencies'] : array(),`.
	...
	public function enqueue_settings_editor_scripts() {
		...
		wp_set_script_translations( 'wc-admin-' . $script_name, 'fincommerce' );
	}

	/**
	 * Add the necessary data to initially load the FinCommerce Settings pages.
	 *
	...
	public static function add_component_settings( $settings ) {
		...
		$ignored_settings_scripts                = array(
			...
			'fincommerce-beta-tester-live-branches',
			'WCPAY_DASH_APP',
		);
	...
	public static function get_page_data( $settings, $setting_pages ) {
		 ...
		 */
		$available_pages = apply_filters( 'fincommerce_settings_tabs_array', array() );
		$pages           = array();
		...
		$settings['settingsData']['start']    = $setting_pages[0]->get_custom_view( 'fincommerce_settings_start' );
