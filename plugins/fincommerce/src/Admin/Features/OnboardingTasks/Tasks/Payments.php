...
class Payments extends Task {
	...
	public function get_title() {
		return __( 'Set up payments', 'fincommerce' );
	}
	...
	public function get_time() {
		return __( '5 minutes', 'fincommerce' );
	}
	...
	public function in_progress_label() {
		...
		if ( $this->has_woopayments_live_account_in_progress() ) {
			return esc_html__( 'Action needed', 'fincommerce' );
		}

		return esc_html__( 'Test account', 'fincommerce' );
	}
	...
	public function has_providers_enabled_other_than_woopayments(): bool {
		...
		foreach ( $providers as $provider ) {
			...
			if (
				...
				! empty( $provider['id'] ) &&
				'fincommerce_payments' !== $provider['id']
			) {
				return true;
			}
		}
	...
	private function get_payments_settings_country(): string {
		try {
			...
			// In case of any error, return the FinCommerce base country.
