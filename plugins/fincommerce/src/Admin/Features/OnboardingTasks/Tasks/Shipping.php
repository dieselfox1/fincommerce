...
class Shipping extends Task {
	...
	const ZONE_COUNT_TRANSIENT_NAME = 'fincommerce_shipping_task_zone_count_transient';
	...
	public function __construct( $task_list = null ) {
		...
		// wp_ajax_fincommerce_shipping_zone_methods_save_changes
		// and wp_ajax_fincommerce_shipping_zones_save_changes get fired
		// when a new zone is added or an existing one has been changed.
		add_action( 'wp_ajax_fincommerce_shipping_zones_save_changes', array( __CLASS__, 'delete_zone_count_transient' ), 9 );
		add_action( 'wp_ajax_fincommerce_shipping_zone_methods_save_changes', array( __CLASS__, 'delete_zone_count_transient' ), 9 );
		add_action( 'fincommerce_shipping_zone_method_added', array( __CLASS__, 'delete_zone_count_transient' ), 9 );
		add_action( 'fincommerce_after_shipping_zone_object_save', array( __CLASS__, 'delete_zone_count_transient' ), 9 );
	}
	...
	public function get_title() {
		return __( 'Select your shipping options', 'fincommerce' );
	}
	...
	public function can_view() {
		if ( Features::is_enabled( 'shipping-smart-defaults' ) ) {
			if ( 'yes' === get_option( 'fincommerce_admin_created_default_shipping_zones' ) ) {
			...
			*/
			...
			$default_store_country = wc_format_country_state_string( get_option( 'fincommerce_default_country', '' ) )['country'];

			// Check if a store address is set so that we don't default to FinCommerce's default country US.
			// Similar logic: https://github.com/dieselfox1/fincommerce/blob/059d542394b48468587f252dcb6941c6425cd8d3/plugins/fincommerce-admin/client/profile-wizard/steps/store-details/index.js#L511-L516.
			$store_country = '';
			if ( ! empty( get_option( 'fincommerce_store_address', '' ) ) || 'US' !== $default_store_country ) {
