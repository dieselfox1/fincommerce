...
class TaskList {
	 ...
	 */
	const HIDDEN_OPTION = 'fincommerce_task_list_hidden_lists';

	/**
	 * Option name of completed task lists.
	 */
	const COMPLETED_OPTION = 'fincommerce_task_list_completed_lists';

	/**
	 * Option name of hidden reminder bar.
	 */
	const REMINDER_BAR_HIDDEN_OPTION = 'fincommerce_task_list_reminder_bar_hidden';
	...
	public function __construct( $data = array() ) {
		...
		foreach ( $data['tasks'] as $task_name ) {
			$class = 'Automattic\FinCommerce\Admin\Features\OnboardingTasks\Tasks\\' . $task_name;
			$task  = new $class( $this );
			$this->add_task( $task );
		}
	...
	public function maybe_set_default_layout( $completed_or_hidden_tasklist_ids ) {
		if ( in_array( 'setup', $completed_or_hidden_tasklist_ids, true ) ) {
			update_option( 'fincommerce_default_homepage_layout', 'two_columns' );
		}
	}
	...
	public function add_task( $task ) {
		if ( ! is_subclass_of( $task, 'Automattic\FinCommerce\Admin\Features\OnboardingTasks\Task' ) ) {
			return new \WP_Error(
				'fincommerce_task_list_invalid_task',
				__( 'Task is not a subclass of `Task`', 'fincommerce' )
			);
		}
	...
	public function get_keep_completed_task_list() {
		return get_option( 'fincommerce_task_list_keep_completed', 'no' );
	}
