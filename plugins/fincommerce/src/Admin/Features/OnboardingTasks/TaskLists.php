...
namespace Automattic\FinCommerce\Admin\Features\OnboardingTasks;

use Automattic\FinCommerce\Admin\Features\Features;
use Automattic\FinCommerce\Admin\Features\OnboardingTasks\Tasks\ReviewShippingOptions;
...
class TaskLists {
	...
	public static function init() {
		...
		add_filter( 'fincommerce_admin_shared_settings', array( __CLASS__, 'task_list_preloaded_settings' ), 20 );
	}
	...
	public static function init_default_lists() {
		...
		self::add_list(
			array(
				'id'                      => 'setup',
				'title'                   => __( 'Get ready to start selling', 'fincommerce' ),
				'tasks'                   => $tasks,
		...
		self::add_list(
			array(
				'id'      => 'extended',
				'title'   => __( 'Things to do next', 'fincommerce' ),
				'sort_by' => array(
					array(
						'key'   => 'is_complete',
						'order' => 'asc',
		...
		if ( has_filter( 'fincommerce_admin_experimental_onboarding_tasklists' ) ) {
			/**
			 ...
			 */
			self::$lists = apply_filters( 'fincommerce_admin_experimental_onboarding_tasklists', self::$lists );
		}
	}
	...
	public static function init_tasks() {
		foreach ( self::DEFAULT_TASKS as $task ) {
			$class = 'Automattic\FinCommerce\Admin\Features\OnboardingTasks\Tasks\\' . $task;
	...
	public static function set_active_task() {
		if ( ! isset( $_GET[ Task::ACTIVE_TASK_TRANSIENT ] ) || ! current_user_can( 'manage_fincommerce' ) ) { // phpcs:ignore csrf ok.
			return;
		}
	...
	public static function add_list( $args ) {
		if ( isset( self::$lists[ $args['id'] ] ) ) {
			return new \WP_Error(
				'fincommerce_task_list_exists',
				__( 'Task list ID already exists', 'fincommerce' )
			);
		}
	...
	public static function add_task( $list_id, $task ) {
		if ( ! isset( self::$lists[ $list_id ] ) ) {
			return new \WP_Error(
				'fincommerce_task_list_invalid_list',
				__( 'Task list ID does not exist', 'fincommerce' )
			);
		}
	...
	public static function menu_task_count() {
		...
		if ( ! $tasks_count || ! isset( $submenu['fincommerce'] ) ) {
			return;
		}
