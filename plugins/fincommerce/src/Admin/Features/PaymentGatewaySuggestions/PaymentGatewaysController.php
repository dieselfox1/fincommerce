...
use Automattic\FinCommerce\Admin\Features\TransientNotices;
...
class PaymentGatewaysController {
	...
	public static function init() {
		add_filter( 'fincommerce_rest_prepare_payment_gateway', array( __CLASS__, 'extend_response' ), 10, 3 );
		add_filter( 'admin_init', array( __CLASS__, 'possibly_do_connection_return_action' ) );
		add_action( 'fincommerce_admin_payment_gateway_connection_return', array( __CLASS__, 'handle_successfull_connection' ) );
	}
	...
	public static function possibly_do_connection_return_action() {
		...
		do_action( 'fincommerce_admin_payment_gateway_connection_return', $gateway_id );
	}
		...
		TransientNotices::add(
			array(
				...
				'content' => sprintf(
					...
					__( '%s connected successfully', 'fincommerce' ),
