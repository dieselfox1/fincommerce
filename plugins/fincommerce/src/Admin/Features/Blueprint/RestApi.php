...
use Automattic\FinCommerce\Internal\ComingSoon\ComingSoonHelper;
use WP_Error;

/**
 * Class RestApi
 *
 * This class handles the REST API endpoints for importing and exporting FinCommerce Blueprints.
 *
 * @package Automattic\FinCommerce\Admin\Features\Blueprint
 */
class RestApi {
	/**
	...
	protected function get_max_file_size() {
		 ...
		 */
		return apply_filters( 'fincommerce_blueprint_upload_max_file_size', self::MAX_FILE_SIZE );
	}
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'steps' => array(
							'description' => __( 'A list of plugins to install', 'fincommerce' ),
							'type'        => 'object',
							'properties'  => array(
								'settings' => array(
									'type'  => 'array',
				...
				array(
					...
					'args'                => array(
						'step_definition' => array(
							'description' => __( 'The step definition to import', 'fincommerce' ),
							'type'        => 'object',
							'required'    => true,
						),
					),
	...
	public function check_export_permission() {
		if ( ! current_user_can( 'manage_fincommerce' ) ) {
			return new \WP_Error( 'fincommerce_rest_cannot_view', __( 'Sorry, you cannot export FinCommerce Blueprints.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function check_import_permission() {
		if (
			...
			return new \WP_Error( 'fincommerce_rest_cannot_view', __( 'Sorry, you cannot import FinCommerce Blueprints.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function import_step( \WP_REST_Request $request ) {
		...
		if ( ! $this->can_import_blueprint( $session_token ) ) {
			return array(
				...
				'messages' => array(
					array(
						'message' => __( 'Blueprint imports are disabled', 'fincommerce' ),
						'type'    => 'error',
					),
				),
			);
				...
				'messages' => array(
					array(
						'message' => sprintf(
							...
							__( 'Blueprint step definition size exceeds maximum limit of %s MB', 'fincommerce' ),
		...
		return array(
			...
			'properties' => array(
				'import_allowed' => array(
					'description' => __( 'Whether blueprint imports are currently allowed', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
