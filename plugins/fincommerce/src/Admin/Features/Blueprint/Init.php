...
namespace Automattic\FinCommerce\Admin\Features\Blueprint;

use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCPaymentGateways;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsAccount;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsAdvanced;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsEmails;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsGeneral;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsTax;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsIntegrations;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsProducts;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsSiteVisibility;
use Automattic\FinCommerce\Admin\Features\Blueprint\Exporters\ExportWCSettingsShipping;
 ...
 * Class Init
 *
 * This class initializes the Blueprint feature for FinCommerce.
 */
class Init {
	use UseWPFunctions;

	const INSTALLED_WP_ORG_PLUGINS_TRANSIENT = 'fincommerce_blueprint_installed_wp_org_plugins';
	const INSTALLED_WP_ORG_THEMES_TRANSIENT  = 'fincommerce_blueprint_installed_wp_org_themes';
	/**
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_admin_shared_settings', array( $this, 'add_js_vars' ) );
	...
	public function get_step_groups_for_js() {
		return array(
			array(
				'id'          => 'settings',
				'description' => __( 'Includes all the items featured in FinCommerce | Settings.', 'fincommerce' ),
				'label'       => __( 'FinCommerce Settings', 'fincommerce' ),
				'icon'        => 'settings',
				'items'       => array_map(
					function ( $exporter ) {
						return array(
			...
			array(
				'id'          => 'plugins',
				'description' => __( 'Includes all the installed plugins.', 'fincommerce' ),
				'label'       => __( 'Plugins', 'fincommerce' ),
				'icon'        => 'plugins',
				'items'       => $this->get_plugins_for_export_group(),
			),
			array(
				'id'          => 'themes',
				'description' => __( 'Includes all the installed themes.', 'fincommerce' ),
	...
	public function add_js_vars( $settings ) {
		...
		if ( 'fincommerce_page_wc-settings-advanced-blueprint' === PageController::get_instance()->get_current_screen_id() ) {
	...
	private function get_installed_wp_org_plugins() {
		...
		foreach ( $all_plugins as $key => $plugin ) {
			 ...
			 */
			$slug = apply_filters( 'wp_plugin_dependencies_slug', $slug ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
