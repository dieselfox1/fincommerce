...
class ExportWCSettingsShipping extends ExportWCSettings {
	...
	public function get_label(): string {
		return __( 'Shipping', 'fincommerce' );
	}
	...
	public function get_description(): string {
		return __( 'Includes all settings in FinCommerce | Settings | Shipping.', 'fincommerce' );
	}
	...
	private function get_steps_for_zones(): array {
		...
		return array_map(
			fn( $zone ) => new RunSql( Util::array_to_insert_sql( $zone, $wpdb->prefix . 'fincommerce_shipping_zones', 'replace into' ) ),
			$wpdb->get_results( "SELECT * FROM {$wpdb->prefix}fincommerce_shipping_zones", ARRAY_A )
		);
	}
	...
	private function get_steps_for_locations(): array {
		...
		return array_map(
			fn( $location ) => new RunSql( Util::array_to_insert_sql( $location, $wpdb->prefix . 'fincommerce_shipping_zone_locations', 'replace into' ) ),
			$wpdb->get_results( "SELECT * FROM {$wpdb->prefix}fincommerce_shipping_zone_locations", ARRAY_A )
		);
	}
	...
	private function get_steps_for_methods_and_options(): array {
		...
		$methods        = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}fincommerce_shipping_zone_methods", ARRAY_A );
		$method_options = $wpdb->get_results(
			"SELECT * FROM {$wpdb->prefix}options WHERE option_name LIKE 'fincommerce_flat_rate_%_settings'
            OR option_name LIKE 'fincommerce_free_shipping_%_settings'",
			ARRAY_A
		);

		return array_merge(
			array_map(
				fn( $method ) => new RunSql( Util::array_to_insert_sql( $method, $wpdb->prefix . 'fincommerce_shipping_zone_methods', 'replace into' ) ),
				$methods
			),
			array_map(
	...
	private function get_step_for_local_pickup(): SetSiteOptions {
		return new SetSiteOptions(
			array(
				'fincommerce_pickup_location_settings' => get_option( 'fincommerce_pickup_location_settings', array() ),
