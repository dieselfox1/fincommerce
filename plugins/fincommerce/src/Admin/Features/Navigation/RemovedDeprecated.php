...
namespace Automattic\FinCommerce\Admin\Features\Navigation;
...
class RemovedDeprecated {
	...
	private static function handle_deprecated_method_call( $name ) {
		...
		if ( $logger ) {
			$logger->warning(
				"The FinCommerce Admin Navigation feature and its classes (Screen, Menu, CoreMenu) are deprecated since 9.3 with no alternative. Please remove the call to $name."
			);
		}
