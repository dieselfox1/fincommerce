...
use Automattic\FinCommerce\Internal\Admin\WCAdminUser;
...
class LaunchYourStore {
	...
	public function __construct() {
		add_action( 'fincommerce_update_options_site-visibility', array( $this, 'save_site_visibility_options' ) );
		add_filter( 'fincommerce_admin_shared_settings', array( $this, 'preload_settings' ) );
		...
		add_filter( 'fincommerce_tracks_event_properties', array( $this, 'append_coming_soon_global_tracks' ), 10, 2 );
		add_action( 'wp_login', array( $this, 'reset_fincommerce_coming_soon_banner_dismissed' ), 10, 2 );
	...
	/**
	 * Save values submitted from FinCommerce -> Settings -> General.
	 *
	 * @return void
	 */
	public function save_site_visibility_options() {
		...
		$nonce_string = Features::is_enabled( 'settings' ) ? 'wp_rest' : 'fincommerce-settings';
		...
		$options = array(
			'fincommerce_coming_soon'      => array( 'yes', 'no' ),
			'fincommerce_store_pages_only' => array( 'yes', 'no' ),
			'fincommerce_private_link'     => array( 'yes', 'no' ),
		);
	...
	public function append_coming_soon_global_tracks( $event_properties ) {
		if ( is_array( $event_properties ) ) {
			$coming_soon = 'no';
			if ( 'yes' === get_option( 'fincommerce_coming_soon', 'no' ) ) {
				if ( 'yes' === get_option( 'fincommerce_store_pages_only', 'no' ) ) {
					$coming_soon = 'store';
				} else {
					$coming_soon = 'site';
				}
	...
	public function preload_settings( $settings ) {
		...
		if ( $is_setting_page || $is_woopayments_connect ) {
			...
			add_option( 'fincommerce_share_key', wp_generate_password( 32, false ) );

			$settings['siteVisibilitySettings'] = array(
				'shop_permalink'               => get_permalink( wc_get_page_id( 'shop' ) ),
				'fincommerce_coming_soon'      => get_option( 'fincommerce_coming_soon' ),
				'fincommerce_store_pages_only' => get_option( 'fincommerce_store_pages_only' ),
				'fincommerce_private_link'     => get_option( 'fincommerce_private_link' ),
				'fincommerce_share_key'        => get_option( 'fincommerce_share_key' ),
			);
		}
	 ...
	 * - 'fincommerce_coming_soon' option value must be 'yes'
	...
	public function maybe_add_coming_soon_banner_on_frontend() {
		...
		$has_dismissed_banner = WCAdminUser::get_user_data_field( $current_user_id, self::BANNER_DISMISS_USER_META_KEY )
				...
				|| get_user_meta( $current_user_id, 'fincommerce_' . self::BANNER_DISMISS_USER_META_KEY, true ) === 'yes';
		...
		// 'fincommerce_coming_soon' must be 'yes'
		if ( get_option( 'fincommerce_coming_soon', 'no' ) !== 'yes' ) {
			return false;
		}

		$store_pages_only = get_option( 'fincommerce_store_pages_only' ) === 'yes';
	...
	/**
	 * Reset 'fincommerce_coming_soon_banner_dismissed' user meta to 'no'.
	 *
	 ...
	 */
	public function reset_fincommerce_coming_soon_banner_dismissed( $user_login, $user ) {
	...
	public function maybe_track_template_change( $post_id, $post, $update ) {
		...
		// Check multiple fields to avoid false matches with non-FinCommerce templates.
