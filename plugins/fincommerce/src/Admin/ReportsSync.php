...
use Automattic\FinCommerce\Internal\Admin\Schedulers\CustomersScheduler;
use Automattic\FinCommerce\Internal\Admin\Schedulers\OrdersScheduler;
use Automattic\FinCommerce\Internal\Admin\Schedulers\ImportScheduler;
...
class ReportsSync {
	...
	public static function init() {
		...
		add_action( 'fincommerce_update_product', array( __CLASS__, 'clear_stock_count_cache' ) );
		add_action( 'fincommerce_new_product', array( __CLASS__, 'clear_stock_count_cache' ) );
		add_action( 'update_option_fincommerce_notify_low_stock_amount', array( __CLASS__, 'clear_stock_count_cache' ) );
		add_action( 'update_option_fincommerce_notify_no_stock_amount', array( __CLASS__, 'clear_stock_count_cache' ) );
	}
	...
	public static function get_schedulers() {
		$schedulers = apply_filters(
			'fincommerce_analytics_report_schedulers',
			array(
		...
		foreach ( $schedulers as $scheduler ) {
			if ( ! is_subclass_of( $scheduler, 'Automattic\FinCommerce\Internal\Admin\Schedulers\ImportScheduler' ) ) {
				throw new \Exception( __( 'Report sync schedulers should be derived from the Automattic\FinCommerce\Internal\Admin\Schedulers\ImportScheduler class.', 'fincommerce' ) );
			}
		}
	...
	public static function regenerate_report_data( $days, $skip_existing ) {
		if ( self::is_importing() ) {
			return new \WP_Error( 'wc_admin_import_in_progress', __( 'An import is already in progress. Please allow the previous import to complete before beginning a new one.', 'fincommerce' ) );
		}
		 ...
		 * @param bool     $skip_existing Skip existing records.
		 */
		do_action( 'fincommerce_analytics_regenerate_init', $days, $skip_existing );

		return __( 'Report table data is being rebuilt. Please allow some time for data to fully populate.', 'fincommerce' );
	}
	...
	public static function delete_report_data() {
		...
		return __( 'Report table data is being deleted.', 'fincommerce' );
	}
