...
class PageController {
	...
	/**
	 * Registered pages
	 * Contains information (breadcrumbs, menu info) about JS powered pages and classic FinCommerce pages.
	 *
	 * @var array
	 */
	private $pages = array();
	...
	public function connect_page( $options ) {
		 ...
		 * }
		 */
		$options = apply_filters( 'fincommerce_navigation_connect_page_options', $options );
	...
	public function get_breadcrumbs() {
		...
		if ( false === $current_page ) {
			...
			return apply_filters( 'fincommerce_navigation_get_breadcrumbs', array( '' ), $current_page );
		}
		...
		$fincommerce_breadcrumb = array( 'admin.php?page=' . self::PAGE_ROOT, __( 'FinCommerce', 'fincommerce' ) );

		array_unshift( $breadcrumbs, $fincommerce_breadcrumb );
		 ...
		 */
		return apply_filters( 'fincommerce_navigation_get_breadcrumbs', $breadcrumbs, $current_page );
	}
	...
	public function get_current_page() {
		...
		if ( ! did_action( 'current_screen' ) ) {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Current page retrieval should be called on or after the `current_screen` hook.', 'fincommerce' ), '0.16.0' );
		}
	...
	public function get_current_screen_id() {
		...
		if ( wp_is_serving_rest_request() ) {
			 ...
			 */
			return apply_filters( 'fincommerce_navigation_current_screen_id', false, null );
		}

		$current_screen = get_current_screen();
		if ( ! $current_screen ) {
			// Filter documentation below.
			return apply_filters( 'fincommerce_navigation_current_screen_id', false, $current_screen );
		}
		...
		$pages_with_tabs = apply_filters(
			'fincommerce_navigation_pages_with_tabs',
			array(
				'wc-reports'  => 'orders',
				'wc-settings' => 'general',
				'wc-status'   => 'status',
		...
		$tabs_with_sections = apply_filters(
			'fincommerce_navigation_page_tab_sections',
			array(
		 ...
		 *
		 * Used for identifying pages to render the FinCommerce Admin header.
		 *
		 * @param string|boolean $screen_id The screen id or false if not identified.
		 * @param WP_Screen      $current_screen The current WP_Screen.
		 */
		return apply_filters( 'fincommerce_navigation_current_screen_id', implode( '-', $screen_pieces ), $current_screen );
	}
	...
	public function is_connected_page() {
		 ...
		 *
		 * Used to determine if the FinCommerce Admin header should be rendered.
		 *
		 * @param boolean       $is_connected_page True if the current page is connected.
		 * @param array|boolean $current_page The connected page data or false if not identified.
		 */
		return apply_filters( 'fincommerce_navigation_is_connected_page', $is_connected_page, $current_page );
	}
	...
	public function is_registered_page() {
		 ...
		 *
		 * Used to determine if this is a JS-powered FinCommerce Admin page.
		 *
		 ...
		 */
		return apply_filters( 'fincommerce_navigation_is_registered_page', $is_registered_page, $current_page );
	}
	...
	public function register_page( $options ) {
		$defaults = array(
			...
			'title'      => '',
			'page_title' => '',
			'capability' => 'view_fincommerce_reports',
			'path'       => '',
			'icon'       => '',
			'position'   => null,
			'js_page'    => true,
	...
	/**
	 * Connects existing FinCommerce pages.
	 *
	...
	public function register_store_details_page() {
		wc_admin_register_page(
			array(
				'title'  => __( 'Setup Wizard', 'fincommerce' ),
				'parent' => '',
				'path'   => '/setup-wizard',
			)
		);
