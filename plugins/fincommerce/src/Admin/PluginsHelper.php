...
namespace Automattic\FinCommerce\Admin;

use ActionScheduler;
use ActionScheduler_DBStore;
use ActionScheduler_QueueRunner;
use Automatic_Upgrader_Skin;
use Automattic\FinCommerce\Admin\PluginsInstallLoggers\AsyncPluginsInstallLogger;
use Automattic\FinCommerce\Admin\PluginsInstallLoggers\PluginsInstallLogger;
...
class PluginsHelper {
	...
	/**
	 * The URL for the FinCommerce subscription page.
	 */
	const WOO_SUBSCRIPTION_PAGE_URL = 'https://fincommerce.com/my-account/my-subscriptions/';

	/**
	 * The URL for the FinCommerce.com cart page.
	 */
	const WOO_CART_PAGE_URL = 'https://fincommerce.com/cart/';

	/**
	 * The URL for the FinCommerce.com add payment method page.
	 */
	const WOO_ADD_PAYMENT_METHOD_URL = 'https://fincommerce.com/my-account/add-payment-method/';
	...
	public static function init() {
		add_action( 'fincommerce_plugins_install_callback', array( __CLASS__, 'install_plugins' ), 10, 2 );
		add_action( 'fincommerce_plugins_install_and_activate_async_callback', array( __CLASS__, 'install_and_activate_plugins_async_callback' ), 10, 3 );
		add_action( 'fincommerce_plugins_activate_callback', array( __CLASS__, 'activate_plugins' ), 10, 2 );
	...
	public static function install_plugins( $plugins, ?PluginsInstallLogger $logger = null, ?string $source = null ) {
		 ...
		 * @since 6.4.0
		 */
		$plugins = apply_filters( 'fincommerce_admin_plugins_pre_install', $plugins );

		if ( empty( $plugins ) || ! is_array( $plugins ) ) {
			return new WP_Error(
				'fincommerce_plugins_invalid_plugins',
				__( 'Plugins must be a non-empty array.', 'fincommerce' )
			);
		}
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( is_wp_error( $api ) ) {
				$properties = array(
					'error_message'     => sprintf(
						// translators: %s: plugin slug (example: fincommerce-services).
						__(
				 ...
				 * @since 6.4.0
				 */
				do_action( 'fincommerce_plugins_install_api_error', $slug, $api );

				$error_message = sprintf(
				/* translators: %s: plugin slug (example: fincommerce-services) */
					__( 'The requested plugin `%s` could not be installed. Plugin API call failed.', 'fincommerce' ),
					$slug
				);
			 ...
			 * @since 9.8
			 */
			do_action( 'fincommerce_plugins_install_before', $slug, $source );
			...
			if ( is_wp_error( $result ) || is_null( $result ) ) {
				 ...
				 * @since 6.4.0
				 */
				do_action( 'fincommerce_plugins_install_error', $slug, $api, $result, $upgrader );

				$install_error_message = sprintf(
				/* translators: %s: plugin slug (example: fincommerce-services) */
					__( 'The requested plugin `%s` could not be installed. Upgrader install failed.', 'fincommerce' ),
					$slug
				);
				$errors->add(
					$plugin,
			 ...
			 * @since 9.8
			 */
			do_action( 'fincommerce_plugins_install_after', $slug, $source );
		}
	...
	public static function install_and_activate_plugins_async_callback( array $plugins, string $job_id, ?string $source = null ) {
		$option_name = 'fincommerce_onboarding_plugins_install_and_activate_async_' . $job_id;
	...
	public static function schedule_install_plugins( $plugins ) {
		...
		$job_id = uniqid();
		WC()->queue()->schedule_single( time() + 5, 'fincommerce_plugins_install_callback', array( $plugins ) );
	...
	public static function activate_plugins( $plugins, ?PluginsInstallLogger $logger = null ) {
		...
		// the mollie-payments-for-fincommerce plugin calls `WP_Filesystem()` during it's activation hook, which crashes without this include.
		 ...
		 * @since 6.4.0
		 */
		$plugins = apply_filters( 'fincommerce_admin_plugins_pre_activate', $plugins );
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( ! $path ) {
				/* translators: %s: plugin slug (example: fincommerce-services) */
				$message = sprintf( __( 'The requested plugin `%s`. is not yet installed.', 'fincommerce' ), $slug );
				$errors->add(
					$plugin,
					$message
				);
			...
			if ( ! is_plugin_active( $path ) ) {
				 ...
				 * @since 6.4.0
				 */
				do_action( 'fincommerce_plugins_activate_error', $slug, $result );

				/* translators: %s: plugin slug (example: fincommerce-services) */
				$message = sprintf( __( 'The requested plugin `%s` could not be activated.', 'fincommerce' ), $slug );
				$errors->add(
					$plugin,
					$message
				);
	...
	public static function schedule_activate_plugins( $plugins ) {
		...
		WC()->queue()->schedule_single(
			time() + 5,
			'fincommerce_plugins_activate_callback',
	...
	public static function get_installation_status( $job_id = null ) {
		$actions = WC()->queue()->search(
			array(
				'hook'    => 'fincommerce_plugins_install_callback',
				'search'  => $job_id,
				'orderby' => 'date',
				'order'   => 'DESC',
			)
	...
	public static function get_activation_status( $job_id = null ) {
		$actions = WC()->queue()->search(
			array(
				'hook'    => 'fincommerce_plugins_activate_callback',
				'search'  => $job_id,
				'orderby' => 'date',
				'order'   => 'DESC',
			)
	...
	/**
	 * Show notices to connect to fincommerce.com for unconnected store in the plugin list.
	 *
	 * @return void
	 */
	public static function maybe_show_connect_notice_in_plugin_list() {
		if ( 'fincommerce_page_wc-settings' !== get_current_screen()->id ) {
			return;
		}
		...
		if ( 'long' === $notice_type ) {
			$notice_string .= __( 'Your store might be at risk as you are running old versions of FinCommerce plugins.', 'fincommerce' );
			$notice_string .= ' ';
		}
		...
		$notice_string .= sprintf(
			...
			__( '<a id="woo-connect-notice-url" href="%s">Connect your store</a> to FinCommerce.com to get updates and streamlined support for your subscriptions.', 'fincommerce' ),
	...
	/**
	 * Enqueue scripts for connect notice in FinCommerce settings page.
	 *
	 * @return void
	 */
	...
	public static function get_subscriptions_notice_data( array $all_subs, array $subs_to_show, int $total, array $messages, string $type ) {
		...
		$renew_string     = __( 'Renew', 'fincommerce' );
		$subscribe_string = __( 'Subscribe', 'fincommerce' );
		if ( isset( $subscription['product_regular_price'] ) ) {
			/* translators: 1: Product price */
			$renew_string = sprintf( __( 'Renew for %1$s', 'fincommerce' ), $subscription['product_regular_price'] );
		}
	...
	public static function get_expiring_subscription_notice( $allowed_link = true ) {
		...
		// When payment method is missing on FinCommerce.com.
		...
		$notice_data = self::get_subscriptions_notice_data(
			...
			array(
				...
				'single_manage'           => __( 'Your subscription for <strong>%1$s</strong> expires on %2$s. <a href="%3$s">Enable auto-renewal</a> to continue receiving updates and streamlined support.', 'fincommerce' ),
				/* translators: 1) product name 2) expiry date 3) URL to My Subscriptions page */
				'multiple_manage'         => __( 'One of your subscriptions for <strong>%1$s</strong> expires on %2$s. <a href="%3$s">Enable auto-renewal</a> to continue receiving updates and streamlined support.', 'fincommerce' ),
				/* translators: 1) total expiring subscriptions 2) URL to My Subscriptions page */
				'different_subscriptions' => __( 'You have <strong>%1$s Woo extension subscriptions</strong> expiring soon. <a href="%2$s">Enable auto-renewal</a> to continue receiving updates and streamlined support.', 'fincommerce' ),
			),
			'expiring',
		);
		...
		return array(
			...
			'button_text' => __( 'Enable auto-renewal', 'fincommerce' ),
	...
	public static function get_expired_subscription_notice( $allowed_link = true ) {
		...
		$notice_data = self::get_subscriptions_notice_data(
			...
			array(
				...
				'single_manage'           => __( 'Your subscription for <strong>%1$s</strong> expired. <a href="%3$s">%4$s</a> to continue receiving updates and streamlined support.', 'fincommerce' ),
				/* translators: 1) product name 3) URL to My Subscriptions page 4) Renew product price string */
				'multiple_manage'         => __( 'One of your subscriptions for <strong>%1$s</strong> has expired. <a href="%3$s">%4$s</a> to continue receiving updates and streamlined support.', 'fincommerce' ),
				/* translators: 1) total expired subscriptions 2) URL to My Subscriptions page */
				'different_subscriptions' => __( 'You have <strong>%1$s Woo extension subscriptions</strong> that expired. <a href="%2$s">Renew</a> to continue receiving updates and streamlined support.', 'fincommerce' ),
			),
			'expired',
		);
		...
		return array(
			'description' => $allowed_link ? $notice_data['parsed_message'] : preg_replace( '#<a.*?>(.*?)</a>#i', '\1', $notice_data['parsed_message'] ),
			'button_text' => __( 'Renew', 'fincommerce' ),
			'button_link' => $button_link,
		);
	}
	...
	public static function get_missing_subscription_notice() {
		...
		$notice_data = self::get_subscriptions_notice_data(
			...
			array(
				...
				'single_manage'           => __( 'You don\'t have a subscription for <strong>%1$s</strong>. Subscribe to receive updates and streamlined support.', 'fincommerce' ),
				/* translators: 1) total expired subscriptions */
				'different_subscriptions' => __( 'You don\'t have subscriptions for <strong>%1$s Woo extensions</strong>. Subscribe to receive updates and streamlined support.', 'fincommerce' ),
			),
			'missing',
		);
		...
		$button_text = __( 'Subscribe', 'fincommerce' );
	...
	public static function get_wccom_disconnected_notice() {
		...
		return sprintf(
			...
			__( 'Successfully disconnected from <b>%1$s</b>.', 'fincommerce' ),
			$user_email
		);
	}
	...
	public static function get_wccom_connected_notice( $user_email ) {
		...
		return sprintf(
		/* translators: 1: Disconnected user email */
			__( 'Successfully connected to <b>%s</b>.', 'fincommerce' ),
			$user_email
		);
	}
	...
	public static function get_missing_payment_method_notice( $allowed_link = true, $total_expiring_subscriptions = 1 ) {
		...
		$description             = $allowed_link
			? sprintf(
			/* translators: %s: FinCommerce.com URL to add payment method */
				_n(
					'Your FinCommerce extension subscription is missing a payment method for renewal. <a href="%s">Add a payment method</a> to ensure you continue receiving updates and streamlined support.',
					'Your FinCommerce extension subscriptions are missing a payment method for renewal. <a href="%s">Add a payment method</a> to ensure you continue receiving updates and streamlined support.',
					$total_expiring_subscriptions,
					'fincommerce'
				),
			...
			: _n(
				'Your FinCommerce extension subscription is missing a payment method for renewal. Add a payment method to ensure you continue receiving updates and streamlined support.',
				'Your FinCommerce extension subscriptions are missing a payment method for renewal. Add a payment method to ensure you continue receiving updates and streamlined support.',
				$total_expiring_subscriptions,
				'fincommerce'
			);
		...
		return array(
			...
			'button_text' => __( 'Add payment method', 'fincommerce' ),
