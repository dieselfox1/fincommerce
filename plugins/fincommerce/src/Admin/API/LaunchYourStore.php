...
class LaunchYourStore {
	...
	public function must_be_shop_manager_or_admin() {
		// phpcs:ignore
		if ( ! current_user_can( 'manage_fincommerce' ) && ! current_user_can( 'administrator' ) ) {
			return false;
		}
		return true;
	}
	...
	public function initialize_coming_soon() {
		...
		update_option( 'fincommerce_coming_soon', $coming_soon );
		update_option( 'fincommerce_store_pages_only', $store_pages_only );
		add_option( 'fincommerce_private_link', $private_link );
		add_option( 'fincommerce_share_key', $share_key );
	...
	/**
	 * Update fincommerce_admin_launch_your_store_survey_completed to yes or no
	 *
	...
	public function update_survey_status( \WP_REST_Request $request ) {
		update_option( 'fincommerce_admin_launch_your_store_survey_completed', $request->get_param( 'status' ) );
		return new \WP_REST_Response();
	}

	/**
	 * Return fincommerce_admin_launch_your_store_survey_completed option.
	 *
	 * @return \WP_REST_Response
	 */
	public function has_survey_completed() {
		return new \WP_REST_Response( get_option( 'fincommerce_admin_launch_your_store_survey_completed', 'no' ) );
	}
}
