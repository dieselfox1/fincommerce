...
use Automattic\FinCommerce\Admin\Features\Settings\Init;
...
class Settings extends \WC_REST_Data_Controller {
	...
	public function save_settings( $request ) {
		...
		if ( ! check_ajax_referer( 'wp_rest', false, false ) ) {
			return new \WP_Error(
				'fincommerce_settings_invalid_nonce',
		...
		try {
			...
			"fincommerce_save_settings_{$current_tab}" :
			"fincommerce_save_settings_{$current_tab}_{$current_section}";
			...
			return new \WP_Error(
				'fincommerce_settings_save_error',
				// translators: %s: error message.
				sprintf( __( 'Failed to save settings: %s', 'fincommerce' ), $e->getMessage() ),
				array( 'status' => 500 )
			);
		}
	}
	...
	public function save_items_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'tab'     => array(
					'type'        => 'string',
					'description' => __( 'Settings tab.', 'fincommerce' ),
					'context'     => array( 'view', 'edit' ),
					'default'     => 'general',
				),
				'section' => array(
					'type'        => 'string',
					'description' => __( 'Settings section.', 'fincommerce' ),
					'context'     => array( 'view', 'edit' ),
					'default'     => '',
				),
			),
