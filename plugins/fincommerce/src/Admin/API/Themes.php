...
use Automattic\FinCommerce\Admin\Overrides\ThemeUpgrader;
use Automattic\FinCommerce\Admin\Overrides\ThemeUpgraderSkin;
...
class Themes extends \WC_REST_Data_Controller {
	...
	public function upload_theme_permissions_check( $request ) {
		if ( ! current_user_can( 'upload_themes' ) ) {
			return new \WP_Error( 'fincommerce_rest_cannot_view', __( 'Sorry, you are not allowed to install themes on this site.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function upload_theme( $request ) {
		...
		if ( ! is_wp_error( $install ) && isset( $install['destination_name'] ) ) {
			 ...
			 */
			do_action( 'fincommerce_theme_installed', $theme );
		} else {
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 */
		return apply_filters( 'fincommerce_rest_prepare_themes', $response, $item, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'status'  => array(
					'description' => __( 'Theme installation status.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'message' => array(
					'description' => __( 'Theme installation message.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'theme'   => array(
					'description' => __( 'Uploaded theme.', 'fincommerce' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function get_collection_params() {
		...
		$params['pluginzip'] = array(
			'description'       => __( 'A zip file of the theme to be uploaded.', 'fincommerce' ),
			'type'              => 'file',
			'validate_callback' => 'rest_validate_request_arg',
		);

		return apply_filters( 'fincommerce_rest_themes_collection_params', $params );
	}
}
