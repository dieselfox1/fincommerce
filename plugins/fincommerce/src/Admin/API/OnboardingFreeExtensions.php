...
class OnboardingFreeExtensions extends WC_REST_Data_Controller {
	...
	public function get_available_extensions( $request ) {
		...
		/**
		* Allows removing Jetpack suggestions from FinCommerce Admin when false.
		 *
		 * In this instance it is removed from the list of extensions suggested in the Onboarding Profiler. This list is first retrieved from the FinCommerce.com API, then if a plugin with the 'jetpack' slug is found, it is removed.
		 *
		 * @since 7.8
		*/
		if ( false === apply_filters( 'fincommerce_suggest_jetpack', true ) ) {
