...
use Automattic\FinCommerce\Internal\Admin\Onboarding\OnboardingProfile as Profile;
use Automattic\FinCommerce\Internal\Admin\Onboarding\OnboardingProducts;
...
class OnboardingProfile extends \WC_REST_Data_Controller {
				...
				array(
					...
					'args'                => array(
						'step' => array(
							'required'    => true,
							'type'        => 'string',
							'description' => __( 'The Core Profiler step to mark as complete.', 'fincommerce' ),
							'enum'        => array(
								'intro-opt-in',
								'skip-guided-setup',
								'user-profile',
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'country_code' => array(
							'description' => __( 'Country code.', 'fincommerce' ),
							'type'        => 'string',
							'required'    => true,
						),
					),
	...
	public function update_items_permissions_check( $request ) {
		if ( ! wc_rest_check_manager_permissions( 'settings', 'edit' ) ) {
			return new \WP_Error( 'fincommerce_rest_cannot_view', __( 'Sorry, you cannot edit this resource.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_items( $request ) {
		 ...
		 */
		do_action( 'fincommerce_onboarding_profile_data_updated', $onboarding_data, $query_args );

		$result = array(
			'status'  => 'success',
			'message' => __( 'Onboarding profile data has been updated.', 'fincommerce' ),
		);
	...
	public function core_profiler_step_complete( $request ) {
		 ...
		 */
		do_action( 'fincommerce_core_profiler_step_complete', $step );
	...
	public function update_store_currency_and_measurement_units( WP_REST_Request $request ) {
		...
		if ( empty( $country_code ) || ! isset( $locale_info[ $country_code ] ) ) {
			return new WP_Error(
				'fincommerce_rest_invalid_country_code',
				__( 'Invalid country code.', 'fincommerce' ),
				array( 'status' => 400 )
			);
		}
		...
		$currency_settings = array(
			'fincommerce_currency'           => $country_info['currency_code'],
			'fincommerce_currency_pos'       => $country_info['currency_pos'],
			'fincommerce_price_thousand_sep' => $country_info['thousand_sep'],
			'fincommerce_price_decimal_sep'  => $country_info['decimal_sep'],
			'fincommerce_price_num_decimals' => $country_info['num_decimals'],
			'fincommerce_weight_unit'        => $country_info['weight_unit'],
			'fincommerce_dimension_unit'     => $country_info['dimension_unit'],
		);
	...
	protected function prepare_objects_query( $params ) {
		 ...
		 * @param array $params The params sent in the request.
		 */
		$args = apply_filters( 'fincommerce_rest_onboarding_profile_object_query', $args, $params );
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 */
		return apply_filters( 'fincommerce_rest_onboarding_prepare_profile', $response, $item, $request );
	}
	...
	public static function get_profile_properties() {
		$properties = array(
			'completed'               => array(
				'type'              => 'boolean',
				'description'       => __( 'Whether or not the profile was completed.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
			...
			'skipped'                 => array(
				'type'              => 'boolean',
				'description'       => __( 'Whether or not the profile was skipped.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
				'validate_callback' => 'rest_validate_request_arg',
			),
			'industry'                => array(
				'type'              => 'array',
				'description'       => __( 'Industry.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
				'nullable'          => true,
				'validate_callback' => 'rest_validate_request_arg',
			...
			'business_extensions'     => array(
				'type'              => 'array',
				'description'       => __( 'Extra business extensions to install.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
			...
			'is_agree_marketing'      => array(
				'type'              => 'boolean',
				'description'       => __( 'Whether or not this store agreed to receiving marketing contents from FinCommerce.com.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
				'validate_callback' => 'rest_validate_request_arg',
			),
			'store_email'             => array(
				'type'              => 'string',
				'description'       => __( 'Store email address.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
				'nullable'          => true,
			...
			'is_store_country_set'    => array(
				'type'              => 'boolean',
				'description'       => __( 'Whether or not this store country is set via onboarding profiler.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
				'validate_callback' => 'rest_validate_request_arg',
			),
			'is_plugins_page_skipped' => array(
				'type'              => 'boolean',
				'description'       => __( 'Whether or not plugins step in core profiler was skipped.', 'fincommerce' ),
				'context'           => array( 'view' ),
				'readonly'          => true,
				'validate_callback' => 'rest_validate_request_arg',
			),
			'business_choice'         => array(
				'type'        => 'string',
				'description' => __( 'Business choice.', 'fincommerce' ),
				'context'     => array( 'view' ),
				'readonly'    => true,
				'nullable'    => true,
			),
			'selling_online_answer'   => array(
				'type'        => 'string',
				'description' => __( 'Selling online answer.', 'fincommerce' ),
				'context'     => array( 'view' ),
				'readonly'    => true,
				'nullable'    => true,
			),
			'selling_platforms'       => array(
				...
				'description' => __( 'Selling platforms.', 'fincommerce' ),
				'context'     => array( 'view' ),
				'readonly'    => true,
				'nullable'    => true,
				'items'       => array(
		 ...
		 */
		return apply_filters( 'fincommerce_rest_onboarding_profile_properties', $properties );
	}
	...
	public static function rest_validate_marketing_email( $value, $request, $param ) {
		...
		if (
			...
			! is_email( $value ) ) {
			return new \WP_Error( 'rest_invalid_email', __( 'Invalid email address', 'fincommerce' ) );
		}
		return true;
	}
	...
	public function get_collection_params() {
		 ...
		 */
		return apply_filters( 'fincommerce_rest_onboarding_profile_collection_params', $params );
	}
}
