...
use Automattic\FinCommerce\Admin\Notes\Notes as NotesRepository;
...
class Notes extends \WC_REST_CRUD_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					'id' => array(
						'description' => __( 'Unique ID for the resource.', 'fincommerce' ),
						'type'        => 'integer',
					),
				),
				array(
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'status' => array(
							'description'       => __( 'Status of note.', 'fincommerce' ),
							'type'              => 'array',
	...
	public function get_item( $request ) {
		...
		if ( ! $note ) {
			return new \WP_Error(
				'fincommerce_note_invalid_id',
				__( 'Sorry, there is no resource with that ID.', 'fincommerce' ),
				array( 'status' => 404 )
			);
		}
	...
	private function is_tasklist_experiment_assigned_treatment() {
		...
		$allow_tracking = 'yes' === get_option( 'fincommerce_allow_tracking' );
		$abtest         = new \FinCommerce\Admin\Experimental_Abtest(
			$anon_id,
			'fincommerce',
			$allow_tracking
		);
		...
		$experiment_name = sprintf(
			'fincommerce_tasklist_progression_headercard_%s_%s',
			$date->format( 'Y' ),
			$date->format( 'm' )
		);

		$experiment_name_2col = sprintf(
			'fincommerce_tasklist_progression_headercard_2col_%s_%s',
			$date->format( 'Y' ),
			$date->format( 'm' )
		);
	...
	protected function prepare_objects_query( $request ) {
		 ...
		 * @since 3.9.0
		 */
		$args = apply_filters( 'fincommerce_rest_notes_object_query', $args, $request );
	...
	public function delete_item( $request ) {
		...
		if ( ! $note ) {
			return new \WP_Error(
				'fincommerce_note_invalid_id',
				__( 'Sorry, there is no note with that ID.', 'fincommerce' ),
				array( 'status' => 404 )
			);
		}
	...
	public function batch_update_items( $request ) {
		...
		if ( ! isset( $note_ids ) || ! is_array( $note_ids ) ) {
			return new \WP_Error(
				'fincommerce_note_invalid_ids',
				__( 'Please provide an array of IDs through the noteIds param.', 'fincommerce' ),
				array( 'status' => 422 )
			);
		}
	...
	public function activate_promo_note( $request ) {
		 ...
		 * @since 7.8.0
		 */
		$allowed_promo_notes = apply_filters( 'fincommerce_admin_allowed_promo_notes', [] );

		$promo_note_name = $request->get_param( 'promo_note_name' );

		if ( ! in_array( $promo_note_name, $allowed_promo_notes, true ) ) {
			return new \WP_Error(
				'fincommerce_note_invalid_promo_note_name',
				__( 'Please provide a valid promo note name.', 'fincommerce' ),
				array( 'status' => 422 )
			);
		}
	...
	public function prepare_item_for_response( $data, $request ) {
		 ...
		 * @since 3.9.0
		 */
		return apply_filters( 'fincommerce_rest_prepare_note', $response, $data, $request );
	}
	...
	public function get_collection_params() {
		...
		$params['type']     = array(
			'description'       => __( 'Type of note.', 'fincommerce' ),
			'type'              => 'array',
			'sanitize_callback' => 'wp_parse_slug_list',
			'validate_callback' => 'rest_validate_request_arg',
			'items'             => array(
		...
		$params['source']   = array(
			'description'       => __( 'Source of note.', 'fincommerce' ),
			'type'              => 'array',
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id'                => array(
					'description' => __( 'ID of the note record.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'name'              => array(
					'description' => __( 'Name of the note.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'type'              => array(
					'description' => __( 'The type of the note (e.g. error, warning, etc.).', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'locale'            => array(
					'description' => __( 'Locale used for the note title and content.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'title'             => array(
					'description' => __( 'Title of the note.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'content'           => array(
					'description' => __( 'Content of the note.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'content_data'      => array(
					'description' => __( 'Content data for the note. JSON string. Available for re-localization.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'status'            => array(
					'description' => __( 'The status of the note (e.g. unactioned, actioned).', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'source'            => array(
					'description' => __( 'Source of the note.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_created'      => array(
					'description' => __( 'Date the note was created.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_created_gmt'  => array(
					'description' => __( 'Date the note was created (GMT).', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_reminder'     => array(
					'description' => __( 'Date after which the user should be reminded of the note, if any.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true, // @todo Allow date_reminder to be updated.
				),
				'date_reminder_gmt' => array(
					'description' => __( 'Date after which the user should be reminded of the note, if any (GMT).', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'is_snoozable'      => array(
					'description' => __( 'Whether or not a user can request to be reminded about the note.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'actions'           => array(
					'description' => __( 'An array of actions, if any, for the note.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'layout'            => array(
					'description' => __( 'The layout of the note (e.g. banner, thumbnail, plain).', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'image'             => array(
					'description' => __( 'The image of the note, if any.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'is_deleted'        => array(
					'description' => __( 'Registers whether the note is deleted or not', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'is_read'           => array(
					'description' => __( 'Registers whether the note is read or not', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
