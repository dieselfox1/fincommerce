...
class DataDownloadIPs extends \WC_REST_Data_Controller {
	...
	public function get_items( $request ) {
		...
		if ( isset( $request['match'] ) ) {
			...
			return new \WP_Error( 'fincommerce_rest_data_download_ips_invalid_request', __( 'Invalid request. Please pass the match parameter.', 'fincommerce' ), array( 'status' => 400 ) );
		}
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 */
		return apply_filters( 'fincommerce_rest_prepare_data_download_ip', $response, $item, $request );
	}
	...
	public function get_collection_params() {
		...
		$params['match']   = array(
			'description'       => __( 'A partial IP address can be passed and matching results will be returned.', 'fincommerce' ),
			'type'              => 'string',
		...
		$schema = array(
			...
			'properties' => array(
				'user_ip_address' => array(
					'type'        => 'string',
					'description' => __( 'IP address.', 'fincommerce' ),
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
			),
