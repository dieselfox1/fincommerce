...
class GenericQuery extends \WC_Object_Query {
	 ...
	 * and to call `fincommerce_analytics_{snake_case(name)}_*` filters.
	 *
	 * @var string
	 */
	protected $name;
	 ...
	 * Filters query vars through `fincommerce_analytics_{snake_case(name)}_query_args` filter.
	 * Filters results through `fincommerce_analytics_{snake_case(name)}_select_query` filter.
	 *
	...
	public function get_data() {
		 ...
		 */
		$args = apply_filters( "fincommerce_analytics_{$snake_name}_query_args", $this->get_query_vars() );
		 ...
		 */
		return apply_filters( "fincommerce_analytics_{$snake_name}_select_query", $results, $args );
	}
}
