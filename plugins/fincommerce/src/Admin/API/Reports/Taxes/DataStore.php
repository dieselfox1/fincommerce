...
namespace Automattic\FinCommerce\Admin\API\Reports\Taxes;

defined( 'ABSPATH' ) || exit;

use Automattic\FinCommerce\Admin\API\Reports\DataStore as ReportsDataStore;
use Automattic\FinCommerce\Admin\API\Reports\DataStoreInterface;
use Automattic\FinCommerce\Admin\API\Reports\TimeInterval;
use Automattic\FinCommerce\Admin\API\Reports\SqlQuery;
use Automattic\FinCommerce\Admin\API\Reports\Cache as ReportsCache;
...
class DataStore extends ReportsDataStore implements DataStoreInterface {
	...
	protected function assign_report_columns() {
		...
		// Using wp_fincommerce_tax_rates table limits the result to only the existing tax rates and
		// omits the historical records which differs from the purpose of wp_wc_order_tax_lookup table.
		// So in order to get the same data present in wp_fincommerce_tax_rates without breaking the
		// API contract the values are now retrieved from wp_fincommerce_order_items and wp_fincommerce_order_itemmeta.
		// And given that country, state and priority are not separate columns within the fincommerce_order_items,
		...
		$this->report_columns = array(
			...
			'name'         => "SUBSTRING_INDEX(SUBSTRING_INDEX({$wpdb->prefix}fincommerce_order_items.order_item_name,'-',-2), '-', 1) as name",
			'tax_rate'     => "CAST({$wpdb->prefix}fincommerce_order_itemmeta.meta_value AS DECIMAL(7,4)) as tax_rate",
			'country'      => "SUBSTRING_INDEX({$wpdb->prefix}fincommerce_order_items.order_item_name,'-',1) as country",
			'state'        => "SUBSTRING_INDEX(SUBSTRING_INDEX({$wpdb->prefix}fincommerce_order_items.order_item_name,'-',-3), '-', 1) as state",
			'priority'     => "SUBSTRING_INDEX({$wpdb->prefix}fincommerce_order_items.order_item_name,'-',-1) as priority",
	...
	public static function init() {
		add_action( 'fincommerce_analytics_delete_order_stats', array( __CLASS__, 'sync_on_order_delete' ), 15 );
	}
	...
	protected function add_from_sql_params( $query_args, $order_status_filter ) {
		...
		$this->subquery->add_sql_clause( 'join', "JOIN {$wpdb->prefix}fincommerce_order_items ON {$table_name}.order_id = {$wpdb->prefix}fincommerce_order_items.order_id AND {$wpdb->prefix}fincommerce_order_items.order_item_type = 'tax'" );
		$this->subquery->add_sql_clause( 'join', "JOIN {$wpdb->prefix}fincommerce_order_itemmeta ON {$wpdb->prefix}fincommerce_order_itemmeta.order_item_id = {$wpdb->prefix}fincommerce_order_items.order_item_id AND {$wpdb->prefix}fincommerce_order_itemmeta.meta_key = 'rate_percent'" );
	}

	/**
	 * Updates the database query with parameters used for Taxes report: categories and order status.
	 *
	 * @see Automattic\FinCommerce\Admin\API\Reports\Taxes\Stats\DataStore::update_sql_query_params()
	 * @param array $query_args Query arguments supplied by the user.
	 */
	...
	protected function normalize_order_by( $order_by ) {
		...
		if ( 'tax_code' === $order_by ) {
			return "{$wpdb->prefix}fincommerce_order_items.order_item_name";
	...
	public static function sync_order_taxes( $order_id ) {
		...
		foreach ( $tax_items as $tax_item ) {
			 ...
			 * @param int $order_id    Order ID.
			 */
			do_action( 'fincommerce_analytics_update_tax', $tax_item->get_rate_id(), $order->get_id() );
	...
	public static function sync_on_order_delete( $order_id ) {
		 ...
		 * @param int $order_id    Order ID.
		 */
		do_action( 'fincommerce_analytics_delete_tax', 0, $order_id );
	...
	protected function initialize_queries() {
		...
		$this->subquery->add_sql_clause( 'group_by', ", {$wpdb->prefix}fincommerce_order_items.order_item_name, {$wpdb->prefix}fincommerce_order_itemmeta.meta_value" );
	}
}
