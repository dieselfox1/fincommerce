	...
	public static function rest_validate_between_numeric_arg( $value, $request, $param ) {
		if ( ! wp_is_numeric_array( $value ) ) {
			return new \WP_Error(
				...
				sprintf( __( '%1$s is not a numerically indexed array.', 'fincommerce' ), $param )
			);
		}
		...
		if (
			...
			return new \WP_Error(
				...
				sprintf( __( '%s must contain 2 numbers.', 'fincommerce' ), $param )
			);
		}
	...
	public static function rest_validate_between_date_arg( $value, $request, $param ) {
		...
		if (
			...
			return new \WP_Error(
				'rest_invalid_param',
				/* translators: %s: parameter name */
				sprintf( __( '%s must contain 2 valid dates.', 'fincommerce' ), $param )
			);
		}
