...
class DataStore extends ReportsDataStore implements DataStoreInterface {
	...
	public static function init() {
		add_action( 'fincommerce_new_customer', array( __CLASS__, 'update_registered_customer' ) );

		add_action( 'fincommerce_update_customer', array( __CLASS__, 'update_registered_customer' ) );
		...
		add_action( 'fincommerce_privacy_remove_order_personal_data', array( __CLASS__, 'anonymize_customer' ) );

		add_action( 'fincommerce_analytics_delete_order_stats', array( __CLASS__, 'sync_on_order_delete' ), 15, 2 );
	}
	...
	public static function sync_order_customer( $post_id ) {
		 ...
		 * @since 4.0.0
		 */
		do_action( 'fincommerce_analytics_update_customer', $customer_id );
	...
	public static function get_or_create_customer_from_order( $order ) {
		 ...
		 * @since 4.0.0
		 */
		do_action( 'fincommerce_analytics_new_customer', $customer_id );
	...
	protected static function is_valid_customer( $user_id ) {
		 ...
		 * @since 4.0.0
		 */
		$customer_roles = (array) apply_filters( 'fincommerce_analytics_customer_roles', array( 'customer' ) );
	...
	public static function delete_customer( $customer_id ) {
		...
		if ( $num_deleted ) {
			 ...
			 * @since 4.0.0
			 */
			do_action( 'fincommerce_analytics_delete_customer', $customer_id );
	...
	public static function anonymize_customer( $order ) {
		...
		$deleted_text = __( '[deleted]', 'fincommerce' );
		$updated      = $wpdb->query(
			$wpdb->prepare(
