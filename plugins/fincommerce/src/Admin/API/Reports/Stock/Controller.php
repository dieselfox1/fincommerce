...
namespace Automattic\FinCommerce\Admin\API\Reports\Stock;
...
class Controller extends GenericController implements ExportableInterface {
	...
	public static function add_wp_query_filter( $where, $wp_query ) {
		...
		if ( $wp_query->get( 'low_in_stock' ) ) {
			...
			$no_stock_amount  = absint( max( get_option( 'fincommerce_notify_no_stock_amount' ), 0 ) );
			$low_stock_amount = absint( max( get_option( 'fincommerce_notify_low_stock_amount' ), 1 ) );
			$where           .= "
	...
	public function prepare_item_for_response( $product, $request ) {
		...
		if ( '' === $data['low_stock_amount'] ) {
			$data['low_stock_amount'] = absint( max( get_option( 'fincommerce_notify_low_stock_amount' ), 1 ) );
		}
		 ...
		 */
		return apply_filters( 'fincommerce_rest_prepare_report_stock', $response, $product, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'stock_status'   => array(
					'description' => __( 'Stock status.', 'fincommerce' ),
					'type'        => 'string',
				...
				'manage_stock'   => array(
					'description' => __( 'Manage stock.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function get_collection_params() {
		...
		$params['type']               = array(
			'description' => __( 'Limit result set to items assigned a stock report type.', 'fincommerce' ),
			'type'        => 'string',
			'default'     => 'all',
	...
	public function get_export_columns() {
		$export_columns = array(
			'title'          => __( 'Product / Variation', 'fincommerce' ),
			'sku'            => __( 'SKU', 'fincommerce' ),
			'stock_status'   => __( 'Status', 'fincommerce' ),
			'stock_quantity' => __( 'Stock', 'fincommerce' ),
		);
		...
		return apply_filters(
			'fincommerce_report_stock_export_columns',
			$export_columns
		);
	}
	...
	public function prepare_item_for_export( $item ) {
		 ...
		 */
		return apply_filters(
			'fincommerce_report_stock_prepare_export_item',
			$export_item,
			$item
		);
	}
