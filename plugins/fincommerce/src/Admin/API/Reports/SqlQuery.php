...
class SqlQuery {
	...
	protected function get_sql_clause( $type, $handling = 'unfiltered' ) {
		 ...
		 */
		$clauses = apply_filters( "fincommerce_analytics_clauses_{$type}", $clauses, $this->context );
		/**
		 * Filter SQL clauses by type and context.
		 *
		 * @param array  $clauses The original arguments for the request.
		 */
		$clauses = apply_filters( "fincommerce_analytics_clauses_{$type}_{$this->context}", $clauses );
