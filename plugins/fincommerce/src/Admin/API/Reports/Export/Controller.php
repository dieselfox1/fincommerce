...
namespace Automattic\FinCommerce\Admin\API\Reports\Export;
 ...
 *
 * @internal
 * @extends \Automattic\FinCommerce\Admin\API\Reports\Controller
 */
class Controller extends \Automattic\FinCommerce\Admin\API\Reports\Controller {
	...
	protected function get_export_collection_params() {
		...
		$params['report_args'] = array(
			'description'       => __( 'Parameters to pass on to the exported report.', 'fincommerce' ),
			'type'              => 'object',
			'validate_callback' => 'rest_validate_request_arg', // @todo: use each controller's schema?
		);
		$params['email']       = array(
			'description'       => __( 'When true, email a link to download the export to the requesting user.', 'fincommerce' ),
			'type'              => 'boolean',
	...
	public function get_export_public_schema() {
		$schema = array(
			...
			'properties' => array(
				'status'    => array(
					'description' => __( 'Export status.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'message'   => array(
					'description' => __( 'Export status message.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'export_id' => array(
					'description' => __( 'Export ID.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
		...
		$schema = array(
			...
			'properties' => array(
				'percent_complete' => array(
					'description' => __( 'Percentage complete.', 'fincommerce' ),
					'type'        => 'int',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'download_url'     => array(
					'description' => __( 'Export download URL.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function export_items( $request ) {
		...
		$export_id         = apply_filters( 'fincommerce_admin_export_id', $default_export_id );
		...
		if ( 0 === $total_rows ) {
			return rest_ensure_response(
				array(
					'message' => __( 'There is no data to export for the given request.', 'fincommerce' ),
				)
			);
		}
		...
		$response = rest_ensure_response(
			array(
				'message'   => __( 'Your report file is being generated.', 'fincommerce' ),
				'export_id' => $export_id,
			)
		);
	...
	public function export_status( $request ) {
		...
		if ( false === $percentage ) {
			return new \WP_Error(
				'fincommerce_admin_reports_export_invalid_id',
				__( 'Sorry, there is no export with that ID.', 'fincommerce' ),
				array( 'status' => 404 )
			);
		}
