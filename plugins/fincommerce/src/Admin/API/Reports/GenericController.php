 ...
 *             ),
 *         );
 *         // Add additional fields from `get_additional_fields` method and apply `fincommerce_rest_' . $schema['title'] . '_schema` filter.
 ...
 *
 * To use the controller, please register it with the filter `fincommerce_admin_rest_controllers` filter.
 *
...
abstract class GenericController extends \WC_REST_Reports_Controller {
	...
	public function get_items( $request ) {
		...
		if ( ! isset( $report_data->data ) || ! isset( $report_data->page_no ) || ! isset( $report_data->pages ) ) {
			return new \WP_Error( 'fincommerce_rest_reports_invalid_response', __( 'Invalid response from data store.', 'fincommerce' ), array( 'status' => 500 ) );
		}
	...
	protected function apply_custom_orderby_filters( $orderby_enum ) {
		 ...
		 */
		return apply_filters( "fincommerce_analytics_orderby_enum_{$this->rest_base}", $orderby_enum );
	}
}
