...
use Automattic\FinCommerce\Admin\API\Reports\Segmenter as ReportsSegmenter;
use Automattic\FinCommerce\Admin\API\Reports\ParameterException;
...
class Segmenter extends ReportsSegmenter {
	 ...
	 *
	 * @return array
	 * @throws \Automattic\FinCommerce\Admin\API\Reports\ParameterException In case of segmenting by variations, when no parent product is specified.
	 */
	protected function get_segments( $type, $query_params, $table_name ) {
		...
		if ( 'product' === $this->query_args['segmentby'] ) {
			...
			if ( ! isset( $this->query_args['product_includes'] ) || ! is_array( $this->query_args['product_includes'] ) || count( $this->query_args['product_includes'] ) !== 1 ) {
				throw new ParameterException( 'wc_admin_reports_invalid_segmenting_variation', __( 'product_includes parameter need to specify exactly one product when segmenting by variation.', 'fincommerce' ) );
			}
