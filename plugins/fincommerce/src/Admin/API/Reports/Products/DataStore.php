...
class DataStore extends ReportsDataStore implements DataStoreInterface {
	...
	public static function init() {
		add_action( 'fincommerce_analytics_delete_order_stats', array( __CLASS__, 'sync_on_order_delete' ), 10 );
		add_action( 'fincommerce_order_partially_refunded', array( __CLASS__, 'add_partial_refund_type_meta' ), 10, 2 );
		add_action( 'fincommerce_order_fully_refunded', array( __CLASS__, 'add_full_refund_type_meta' ), 10, 2 );
	}
		...
		foreach ( $products_data as $key => $product_data ) {
			...
			if ( $query_args['extended_info'] ) {
				...
				if ( ! $product ) {
					if ( ! isset( $product_names[ $product_id ] ) ) {
						$product_names[ $product_id ] = $wpdb->get_var(
							$wpdb->prepare(
								...
								JOIN {$wpdb->prefix}fincommerce_order_items i ON i.order_item_id = l.order_item_id
					...
					$products_data[ $key ]['extended_info']['name'] = $product_names[ $product_id ] ? sprintf( __( '%s (Deleted)', 'fincommerce' ), $product_names[ $product_id ] ) : __( '(Deleted)', 'fincommerce' );
					continue;
				}

				$extended_attributes = apply_filters( 'fincommerce_rest_reports_products_extended_attributes', $this->extended_attributes, $product_data );
				...
				if ( '' === $extended_info['low_stock_amount'] ) {
					$extended_info['low_stock_amount'] = absint( max( get_option( 'fincommerce_notify_low_stock_amount' ), 1 ) );
				}
	...
	public static function sync_order_products( $order_id ) {
		...
		$round_tax      = 'no' === get_option( 'fincommerce_tax_round_at_subtotal' );
		...
		foreach ( $order_items as $order_item ) {
			 ...
			 * @param int $order_id      Order ID.
			 */
			do_action( 'fincommerce_analytics_update_product', $order_item_id, $order->get_id() );
	...
	public static function sync_on_order_delete( $order_id ) {
		 ...
		 * @param int $order_id   Order ID.
		 */
		do_action( 'fincommerce_analytics_delete_product', 0, $order_id );
