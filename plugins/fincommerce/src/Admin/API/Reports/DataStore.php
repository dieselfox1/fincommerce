 ...
 * </code></pre>
 *
 * Please use the `fincommerce_data_stores` filter to add your custom data store to the list of available ones.
 ...
 * <pre><code class="language-php">add_filter( 'fincommerce_data_stores', function( $stores ) {
 ...
 *
 * Note that this class is NOT {@link https://developer.fincommerce.com/docs/how-to-manage-fincommerce-data-stores/ a CRUD data store}.
...
class DataStore extends SqlQuery implements DataStoreInterface {
	...
	public function __construct() {
		...
		if ( $this->report_columns ) {
			$this->report_columns = apply_filters(
				'fincommerce_admin_report_columns',
				$this->report_columns,
				$this->context,
	...
	protected function should_use_cache() {
		 ...
		 */
		return (bool) apply_filters( 'fincommerce_analytics_report_should_use_cache', true, $this->cache_key );
	}
	...
	protected function cast_numbers( $array ) {
		$retyped_array = array();
		$column_types  = apply_filters( 'fincommerce_rest_reports_column_types', $this->column_types, $array );
	...
	protected static function get_excluded_report_order_statuses() {
		$excluded_statuses = \WC_Admin_Settings::get_option( 'fincommerce_excluded_report_order_statuses', array( 'pending', 'failed', 'cancelled' ) );
		$excluded_statuses = array_merge( array( 'auto-draft', 'trash' ), array_map( 'esc_sql', $excluded_statuses ) );
		return apply_filters( 'fincommerce_analytics_excluded_order_statuses', $excluded_statuses );
	}
		...
		foreach ( $post_meta_comparators as $comparator => $arg ) {
			...
			foreach ( $query_args[ $arg ] as $attribute_term ) {
				...
				if ( empty( $sql_clauses['join'] ) ) {
					$sql_clauses['join'][] = "JOIN {$wpdb->prefix}fincommerce_order_items orderitems ON orderitems.order_id = {$table_to_join_on}.order_id";
				}
				...
				if ( 'AND' === $match_operator || 1 === count( $sql_clauses['join'] ) ) {
					...
					$sql_clauses['join'][] = "JOIN {$wpdb->prefix}fincommerce_order_itemmeta as {$join_alias} ON {$join_alias}.order_item_id = {$table_to_join_on}.order_item_id";
				}
	...
	protected function get_filtered_ids( $query_args, $field, $separator = ',' ) {
		 ...
		 */
		$ids = apply_filters( 'fincommerce_analytics_' . $field, $ids, $query_args, $field, $this->context );
