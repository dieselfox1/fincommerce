...
namespace Automattic\FinCommerce\Admin\API\Reports\PerformanceIndicators;
...
class Controller extends GenericController {
	...
	public function __construct() {
		add_filter( 'fincommerce_rest_performance_indicators_data_value', array( $this, 'format_data_value' ), 10, 5 );
	}
	...
	private function get_jetpack_modules_data() {
		...
		$items = apply_filters(
			'fincommerce_rest_performance_indicators_jetpack_items',
			array(
				'stats/visitors' => array(
					'label'      => __( 'Visitors', 'fincommerce' ),
					'permission' => 'view_stats',
					'format'     => 'number',
					'module'     => 'stats',
				),
				'stats/views'    => array(
					'label'      => __( 'Views', 'fincommerce' ),
					'permission' => 'view_stats',
					'format'     => 'number',
					'module'     => 'stats',
				),
	 ...
	 *
	 * @internal
	 * @see https://github.com/dieselfox1/fincommerce-admin/issues/1282
	...
	public function sort( $a, $b ) {
		...
		$stat_order = apply_filters(
			'fincommerce_rest_report_sort_performance_indicators',
			array(
				'revenue/total_sales',
				'revenue/net_revenue',
				'orders/orders_count',
	...
	public function get_items( $request ) {
		...
		if ( empty( $query_args['stats'] ) ) {
			return new \WP_Error( 'fincommerce_analytics_performance_indicators_empty_query', __( 'A list of stats to query must be provided.', 'fincommerce' ), 400 );
		}
		...
		foreach ( $query_args['stats'] as $stat ) {
			...
			$stats[] = (object) array(
				...
				'label'  => $label,
				'format' => $format,
				'value'  => apply_filters( 'fincommerce_rest_performance_indicators_data_value', $data, $stat, $report, $chart, $query_args ),
			);
		}
	...
	public function prepare_item_for_response( $stat_data, $request ) {
		 ...
		 */
		return apply_filters( 'fincommerce_rest_prepare_report_performance_indicators', $response, $stat_data, $request );
	}
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				...
				'chart'  => array(
					'description' => __( 'The specific chart this stat referrers to.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'label'  => array(
					'description' => __( 'Human readable label for the stat.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'format' => array(
					'description' => __( 'Format of the stat.', 'fincommerce' ),
					'type'        => 'number',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				...
				'value'  => array(
					'description' => __( 'Value of the stat. Returns null if the stat does not exist or cannot be loaded.', 'fincommerce' ),
					'type'        => 'number',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function get_collection_params() {
		$indicator_data = $this->get_indicator_data();
		if ( is_wp_error( $indicator_data ) ) {
			$allowed_stats = __( 'There was an issue loading the report endpoints', 'fincommerce' );
		} else {
		...
		$params['stats']   = array(
			'description'       => sprintf(
				...
				__( 'Limit response to specific report stats. Allowed values: %s.', 'fincommerce' ),
				$allowed_stats
			),
			'type'              => 'array',
