...
use Automattic\FinCommerce\Admin\API\Reports\StatsDataStoreTrait;
...
class DataStore extends ReportsDataStore implements DataStoreInterface {
	...
	public function __construct() {
		$this->date_column_name = get_option( 'fincommerce_date_type', 'date_paid' );
		parent::__construct();
	}
	...
	public static function init() {
		add_action( 'fincommerce_before_delete_order', array( __CLASS__, 'delete_order' ) );
	...
	public function get_noncached_stats_data( $query_args, $params, &$data, $expected_interval_count ) {
		...
		if ( null === $totals ) {
			return new \WP_Error( 'fincommerce_analytics_revenue_result_failed', __( 'Sorry, fetching revenue data failed.', 'fincommerce' ) );
		}
	...
	public static function update( $order ) {
		 ...
		 */
		$data = apply_filters(
			'fincommerce_analytics_update_order_stats_data',
			array(
		 ...
		 * @since 4.0.0.
		 */
		do_action( 'fincommerce_analytics_update_order_stats', $order->get_id() );
	...
	public static function delete_order( $post_id ) {
		 ...
		 * @since 4.0.0
		 */
		do_action( 'fincommerce_analytics_delete_order_stats', $order_id, $customer_id );
	...
	public static function is_returning_customer( $order, $customer_id = null ) {
		if ( is_null( $customer_id ) ) {
			$customer_id = \Automattic\FinCommerce\Admin\API\Reports\Customers\DataStore::get_existing_customer_id_from_order( $order );
		}
		...
		$oldest_orders = \Automattic\FinCommerce\Admin\API\Reports\Customers\DataStore::get_oldest_orders( $customer_id );
