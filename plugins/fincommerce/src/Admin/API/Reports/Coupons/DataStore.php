...
class DataStore extends ReportsDataStore implements DataStoreInterface {
	...
	public static function init() {
		add_action( 'fincommerce_analytics_delete_order_stats', array( __CLASS__, 'sync_on_order_delete' ), 5 );
	}

	// phpcs:enable FinCommerce.Functions.InternalInjectionMethod.MissingFinal
		...
		foreach ( $coupon_data as $idx => $coupon_datum ) {
			...
			if ( $query_args['extended_info'] ) {
				...
				if ( 0 === $coupon->get_id() ) {
					...
					$extended_info = array(
						'code'             => __( '(Deleted)', 'fincommerce' ),
						'date_created'     => '',
						'date_created_gmt' => '',
						'date_expires'     => '',
						'date_expires_gmt' => '',
						'discount_type'    => __( 'N/A', 'fincommerce' ),
					);
				} else {
	...
	public static function get_coupon_id( \WC_Order_Item_Coupon $coupon_item ) {
		...
		// See: https://github.com/dieselfox1/fincommerce/blob/3dc7df7af9f7ca0c0aa34ede74493e856f276abe/includes/abstracts/abstract-wc-order.php#L1206.
	...
	public static function sync_order_coupons( $order_id ) {
		...
		foreach ( $coupon_items as $coupon_item ) {
			 ...
			 * @param int $order_id  Order ID.
			 */
			do_action( 'fincommerce_analytics_update_coupon', $coupon_id, $order_id );
	...
	public static function sync_on_order_delete( $order_id ) {
		 ...
		 * @param int $order_id  Order ID.
		 */
		do_action( 'fincommerce_analytics_delete_coupon', 0, $order_id );
