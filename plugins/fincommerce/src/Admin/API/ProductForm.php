...
use Automattic\FinCommerce\Internal\Admin\ProductForm\FormFactory;
...
class ProductForm extends \WC_REST_Data_Controller {
	...
	public function get_product_form_permission_check( $request ) {
		if ( ! current_user_can( 'manage_fincommerce' ) ) {
			return new \WP_Error( 'fincommerce_rest_cannot_create', __( 'Sorry, you are not allowed to retrieve product form data.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
