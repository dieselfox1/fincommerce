...
use Automattic\FinCommerce\Admin\Features\PaymentGatewaySuggestions\Init as Suggestions;
...
class PaymentGatewaySuggestions extends \WC_REST_Data_Controller {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => array( $this, 'user_can_manage_fincommerce' ),
					'args'                => array(
						'force_default_suggestions' => array(
							'type'        => 'boolean',
							'description' => __( 'Return the default payment suggestions when fincommerce_show_marketplace_suggestions and fincommerce_setting_payments_recommendations_hidden options are set to no', 'fincommerce' ),
						),
					),
				),
	 ...
	 * @return \WP_Error|boolean
	 */
	public function user_can_manage_fincommerce() {
		if ( current_user_can( 'manage_fincommerce' ) ) {
			return true;
		}

		return new \WP_Error( 'fincommerce_rest_invalid_user', __( 'You are not allowed to make this request.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'content'                 => array(
					'description' => __( 'Suggestion description.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'id'                      => array(
					'description' => __( 'Suggestion ID.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'image'                   => array(
					'description' => __( 'Gateway image.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'is_visible'              => array(
					'description' => __( 'Suggestion visibility.', 'fincommerce' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'plugins'                 => array(
					'description' => __( 'Array of plugin slugs.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'recommendation_priority' => array(
					'description' => __( 'Priority of recommendation.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'title'                   => array(
					'description' => __( 'Gateway title.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'transaction_processors'  => array(
					'description'         => __( 'Array of transaction processors and their images.', 'fincommerce' ),
					'type'                => 'object',
