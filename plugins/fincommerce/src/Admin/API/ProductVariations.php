...
class ProductVariations extends \WC_REST_Product_Variations_Controller {
	...
	public function get_collection_params() {
		...
		$params['search'] = array(
			'description'       => __( 'Search by similar product name, sku, or attribute value.', 'fincommerce' ),
			'type'              => 'string',
	...
	public function get_items( $request ) {
		...
		add_filter( 'posts_groupby', array( 'Automattic\FinCommerce\Admin\API\Products', 'add_wp_query_group_by' ), 10, 2 );
		$response = parent::get_items( $request );
		remove_filter( 'posts_where', array( __CLASS__, 'add_wp_query_filter' ), 10 );
		remove_filter( 'posts_join', array( __CLASS__, 'add_wp_query_join' ), 10 );
		remove_filter( 'posts_groupby', array( 'Automattic\FinCommerce\Admin\API\Products', 'add_wp_query_group_by' ), 10 );
		return $response;
	}
	...
	public function get_item_schema() {
		...
		$schema['properties']['name']      = array(
			'description' => __( 'Product parent name.', 'fincommerce' ),
			'type'        => 'string',
