...
use Automattic\FinCommerce\Admin\Notes\Notes as NotesFactory;
...
class NoteActions extends Notes {
	...
	public function register_routes() {
		register_rest_route(
			...
			array(
				'args'   => array(
					'note_id'   => array(
						'description' => __( 'Unique ID for the Note.', 'fincommerce' ),
						'type'        => 'integer',
					),
					'action_id' => array(
						'description' => __( 'Unique ID for the Note Action.', 'fincommerce' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	public function trigger_note_action( $request ) {
		...
		if ( ! $triggered_action ) {
			return new \WP_Error(
				'fincommerce_note_action_invalid_id',
				__( 'Sorry, there is no resource with that ID.', 'fincommerce' ),
				array( 'status' => 404 )
			);
		}
