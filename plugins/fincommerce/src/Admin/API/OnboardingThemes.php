...
class OnboardingThemes extends \WC_REST_Data_Controller {
	...
	public function update_item_permissions_check( $request ) {
		if ( ! current_user_can( 'switch_themes' ) ) {
			return new \WP_Error( 'fincommerce_rest_cannot_update', __( 'Sorry, you cannot manage themes.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function install_theme( $request ) {
		...
		if ( is_wp_error( $api ) ) {
			return new \WP_Error(
				'fincommerce_rest_theme_install',
				sprintf(
				/* translators: %s: theme slug (example: fincommerce-services) */
					__( 'The requested theme `%s` could not be installed. Theme API call failed.', 'fincommerce' ),
					$theme
				),
				500
			);
		...
		if ( is_wp_error( $result ) || is_null( $result ) ) {
			return new \WP_Error(
				...
				sprintf(
				/* translators: %s: theme slug (example: fincommerce-services) */
					__( 'The requested theme `%s` could not be installed.', 'fincommerce' ),
					$theme
				),
				500
			);
	...
	public function activate_theme( $request ) {
		...
		if ( ! in_array( $theme, array_keys( $installed_themes ), true ) ) {
			/* translators: %s: theme slug (example: fincommerce-services) */
			return new \WP_Error( 'fincommerce_rest_invalid_theme', sprintf( __( 'Invalid theme %s.', 'fincommerce' ), $theme ), 404 );
		}

		$result = switch_theme( $theme );
		if ( ! is_null( $result ) ) {
			return new \WP_Error( 'fincommerce_rest_invalid_theme', sprintf( __( 'The requested theme could not be activated.', 'fincommerce' ), $theme ), 500 );
		}
	...
	public function get_recommended_themes( $request ) {
		...
		if (
			 ...
			 * @since 8.3.0
			 */
			! apply_filters( 'fincommerce_allow_marketplace_suggestions', true ) ||
			get_option( 'fincommerce_show_marketplace_suggestions', 'yes' ) === 'no'
		) {
			...
			return apply_filters(
				'__experimental_fincommerce_rest_get_recommended_themes',
				array(
					'themes' => array(),
					'_links' => array(
						'browse_all' => array(
		...
		$core_themes = array(
			...
			array(
				'name'           => 'Highline',
				/* translators: %d: price */
				'price'          => sprintf( __( '$%d/year', 'fincommerce' ), 79 ),
				'is_free'        => false,
				'color_palettes' => array(
					array(
						'title'     => 'Primary',
				...
				'total_palettes' => 9,
				'slug'           => 'highline',
				'thumbnail_url'  => 'https://fincommerce.com/wp-content/uploads/2023/12/Featured-image-538x403-1.png',
				'link_url'       => add_query_arg( $in_app_purchase_params, 'https://fincommerce.com/products/highline/' ),
			),
			array(
				'name'           => 'Luminate',
				/* translators: %d: price */
				...
				'total_palettes' => 5,
				'slug'           => 'luminate',
				'thumbnail_url'  => 'https://fincommerce.com/wp-content/uploads/2022/07/Featured-image-538x403-2.png',
				'link_url'       => add_query_arg( $in_app_purchase_params, 'https://fincommerce.com/products/luminate/' ),
			),
			array(
				'name'           => 'Gizmo',
				/* translators: %d: price */
				...
				'total_palettes' => 10,
				'slug'           => 'gizmo',
				'thumbnail_url'  => 'https://fincommerce.com/wp-content/uploads/2022/11/gizmo-regular-card-product-logo.jpg?w=900',
				'link_url'       => add_query_arg( $in_app_purchase_params, 'https://fincommerce.com/products/gizmo/' ),
			),
		);
		...
		$ai_connection_enabled = get_option( 'fincommerce_blocks_allow_ai_connection' );
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'slug'   => array(
					'description' => __( 'Theme slug.', 'fincommerce' ),
					'type'        => 'string',
				...
				'status' => array(
					'description' => __( 'Theme status.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
