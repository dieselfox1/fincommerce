...
use Automattic\FinCommerce\Admin\API\Reports\Categories\DataStore as CategoriesDataStore;
use Automattic\FinCommerce\Admin\API\Reports\Coupons\DataStore as CouponsDataStore;
use Automattic\FinCommerce\Admin\API\Reports\Customers\DataStore as CustomersDataStore;
use Automattic\FinCommerce\Admin\API\Reports\Products\DataStore as ProductsDataStore;
...
class Leaderboards extends \WC_REST_Data_Controller {
	...
	protected function get_coupons_leaderboard( $per_page, $after, $before, $persisted_query ) {
		...
		$coupons_data       = $per_page > 0 ? $coupons_data_store->get_data(
			apply_filters(
				'fincommerce_analytics_coupons_query_args',
				array(
		...
		return array(
			'id'      => 'coupons',
			'label'   => __( 'Top Coupons - Number of Orders', 'fincommerce' ),
			'headers' => array(
				array(
					'label' => __( 'Coupon code', 'fincommerce' ),
				),
				array(
					'label' => __( 'Orders', 'fincommerce' ),
				),
				array(
					'label' => __( 'Amount discounted', 'fincommerce' ),
				),
			),
			'rows'    => $rows,
		);
	...
	protected function get_categories_leaderboard( $per_page, $after, $before, $persisted_query ) {
		...
		$categories_data       = $per_page > 0 ? $categories_data_store->get_data(
			apply_filters(
				'fincommerce_analytics_categories_query_args',
				array(
					'orderby'       => 'items_sold',
					'order'         => 'desc',
					'after'         => $after,
		...
		return array(
			'id'      => 'categories',
			'label'   => __( 'Top categories - Items sold', 'fincommerce' ),
			'headers' => array(
				array(
					'label' => __( 'Category', 'fincommerce' ),
				),
				array(
	...
	protected function get_customers_leaderboard( $per_page, $after, $before, $persisted_query ) {
		...
		$customers_data       = $per_page > 0 ? $customers_data_store->get_data(
			apply_filters(
				'fincommerce_analytics_customers_query_args',
				array(
					'orderby'      => 'total_spend',
					'order'        => 'desc',
					'order_after'  => $after,
		...
		return array(
			'id'      => 'customers',
			'label'   => __( 'Top Customers - Total Spend', 'fincommerce' ),
			'headers' => array(
				array(
					'label' => __( 'Customer Name', 'fincommerce' ),
				),
				array(
					'label' => __( 'Orders', 'fincommerce' ),
				),
	...
	protected function get_products_leaderboard( $per_page, $after, $before, $persisted_query ) {
		...
		$products_data       = $per_page > 0 ? $products_data_store->get_data(
			apply_filters(
				'fincommerce_analytics_products_query_args',
				array(
					'orderby'       => 'items_sold',
					'order'         => 'desc',
					'after'         => $after,
		...
		return array(
			'id'      => 'products',
			'label'   => __( 'Top products - Items sold', 'fincommerce' ),
			'headers' => array(
				array(
	...
	public function get_leaderboards( $per_page, $after, $before, $persisted_query ) {
		...
		return apply_filters( 'fincommerce_leaderboards', $leaderboards, $per_page, $after, $before, $persisted_query );
	}
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 */
		return apply_filters( 'fincommerce_rest_prepare_leaderboard', $response, $item, $request );
	}
	...
	public function get_collection_params() {
		...
		$params['persisted_query'] = array(
			'description'       => __( 'URL query to persist across links.', 'fincommerce' ),
			'type'              => 'string',
			'validate_callback' => 'rest_validate_request_arg',
		);
		return $params;
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id'      => array(
					'type'        => 'string',
					'description' => __( 'Leaderboard ID.', 'fincommerce' ),
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'label'   => array(
					'type'        => 'string',
					'description' => __( 'Displayed title for the leaderboard.', 'fincommerce' ),
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'headers' => array(
					'type'        => 'array',
					'description' => __( 'Table headers.', 'fincommerce' ),
					'context'     => array( 'view' ),
					'readonly'    => true,
					'items'       => array(
						'type'       => 'array',
						'properties' => array(
							'label' => array(
								'description' => __( 'Table column header.', 'fincommerce' ),
								'type'        => 'string',
				...
				'rows'    => array(
					'type'        => 'array',
					'description' => __( 'Table rows.', 'fincommerce' ),
					'context'     => array( 'view' ),
					'readonly'    => true,
					'items'       => array(
						'type'       => 'array',
						'properties' => array(
							'display' => array(
								'description' => __( 'Table cell display.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'value'   => array(
								'description' => __( 'Table cell value.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
							'format'  => array(
								'description' => __( 'Table cell format.', 'fincommerce' ),
								'type'        => 'string',
								'context'     => array( 'view' ),
