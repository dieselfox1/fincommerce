...
class Plugins extends \WC_REST_Data_Controller {
	...
	public function update_item_permissions_check( $request ) {
		if ( ! current_user_can( 'install_plugins' ) ) {
			return new \WP_Error( 'fincommerce_rest_cannot_update', __( 'Sorry, you cannot manage plugins.', 'fincommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function install_plugin( $request ) {
		wc_deprecated_function( 'install_plugin', '4.3', '\Automattic\FinCommerce\Admin\API\Plugins()->install_plugins' );
	...
	public function install_plugins( $request ) {
		...
		if ( empty( $request['plugins'] ) || ! is_array( $plugins ) ) {
			return new \WP_Error( 'fincommerce_rest_invalid_plugins', __( 'Plugins must be a non-empty array.', 'fincommerce' ), 404 );
		}
		...
		if ( isset( $request['async'] ) && $request['async'] ) {
			...
			return array(
				...
				'message' => __( 'Plugin installation has been scheduled.', 'fincommerce' ),
			);
		}
		...
		return array(
			...
			'message' => count( $data['errors']->errors ) === 0
				? __( 'Plugins were successfully installed.', 'fincommerce' )
				: __( 'There was a problem installing some of the requested plugins.', 'fincommerce' ),
		);
	}
	...
	public function activate_plugins( $request ) {
		...
		if ( isset( $request['async'] ) && $request['async'] ) {
			...
			return array(
				...
				'message' => __( 'Plugin activation has been scheduled.', 'fincommerce' ),
			);
		}
		...
		return ( array(
			...
			'success' => count( $data['errors']->errors ) === 0,
			'message' => count( $data['errors']->errors ) === 0
				? __( 'Plugins were successfully activated.', 'fincommerce' )
				: __( 'There was a problem activating some of the requested plugins.', 'fincommerce' ),
		) );
	}
	...
	public function connect_jetpack( $request ) {
		if ( ! class_exists( '\Jetpack' ) ) {
			return new \WP_Error( 'fincommerce_rest_jetpack_not_active', __( 'Jetpack is not installed or active.', 'fincommerce' ), 404 );
		}

		// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingHookComment
		$redirect_url = apply_filters( 'fincommerce_admin_onboarding_jetpack_connect_redirect_url', esc_url_raw( $request['redirect_url'] ) );
		$connect_url  = \Jetpack::init()->build_connect_url( true, $redirect_url, 'fincommerce-onboarding' );

		$calypso_env = defined( 'fincommerce_CALYPSO_ENVIRONMENT' ) && in_array( fincommerce_CALYPSO_ENVIRONMENT, array( 'development', 'wpcalypso', 'horizon', 'stage' ), true ) ? fincommerce_CALYPSO_ENVIRONMENT : 'production';
	 ...
	 *
	 * @return \WP_Error|array Connection URL for FinCommerce.com
	 */
	public function request_wccom_connect() {
		...
		if ( ! class_exists( 'WC_Helper_API' ) ) {
			return new \WP_Error( 'fincommerce_rest_helper_not_active', __( 'There was an error loading the FinCommerce.com Helper API.', 'fincommerce' ), 404 );
		}
		...
		if ( 200 !== $code ) {
			return new \WP_Error( 'fincommerce_rest_helper_connect', __( 'There was an error connecting to FinCommerce.com. Please try again.', 'fincommerce' ), 500 );
		}
		...
		if ( defined( 'fincommerce_CALYPSO_ENVIRONMENT' ) && in_array( fincommerce_CALYPSO_ENVIRONMENT, array( 'development', 'wpcalypso', 'horizon', 'stage' ), true ) ) {
			$connect_url = add_query_arg(
				array(
					'calypso_env' => fincommerce_CALYPSO_ENVIRONMENT,
				),
				$connect_url
			);
		}
	...
	/**
	 * Finishes connecting to FinCommerce.com.
	 *
	...
	public function connect_square() {
		if ( ! class_exists( '\FinCommerce\Square\Handlers\Connection' ) ) {
			return new \WP_Error( 'fincommerce_rest_helper_connect', __( 'There was an error connecting to Square.', 'fincommerce' ), 500 );
		}
		...
		if ( $has_cbd_industry ) {
			$url = 'https://squareup.com/t/f_partnerships/d_referrals/p_fincommerce/c_general/o_none/l_us/dt_alldevice/pr_payments/?route=/solutions/cbd';
		} else {
			$url = \FinCommerce\Square\Handlers\Connection::CONNECT_URL_PRODUCTION;
		}
	...
	public function connect_wcpay() {
		if ( ! class_exists( 'WC_Payments' ) ) {
			return new \WP_Error( 'fincommerce_rest_helper_connect', __( 'There was an error communicating with the WooPayments plugin.', 'fincommerce' ), 500 );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'slug'   => array(
					'description' => __( 'Plugin slug.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'name'   => array(
					'description' => __( 'Plugin name.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'status' => array(
					'description' => __( 'Plugin status.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	public function get_connect_schema() {
		...
		$schema['properties']['connectAction'] = array(
			'description' => __( 'Action that should be completed to connect Jetpack.', 'fincommerce' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
			'readonly'    => true,
		);
