...
class OnboardingPlugins extends WC_REST_Data_Controller {
				...
				array(
					...
					'args'                => array(
						...
						'from'         => array(
							...
							'sanitize_callback' => 'sanitize_text_field',
							'required'          => false,
							'default'           => 'fincommerce-onboarding',
						),
					),
				),
			)
		);
		add_action( 'fincommerce_plugins_install_error', array( $this, 'log_plugins_install_error' ), 10, 4 );
		add_action( 'fincommerce_plugins_install_api_error', array( $this, 'log_plugins_install_api_error' ), 10, 2 );
	}
	...
	public function install_and_activate_async( WP_REST_Request $request ) {
		...
		WC()->queue()->add( 'fincommerce_plugins_install_and_activate_async_callback', array( $plugins, $job_id, $source ) );
	...
	public function get_scheduled_installs( WP_REST_Request $request ) {
		...
		$actions = WC()->queue()->search(
			array(
				'hook'    => 'fincommerce_plugins_install_and_activate_async_callback',
				'search'  => $job_id,
				'orderby' => 'date',
				'order'   => 'DESC',
			)
		...
		$option = get_option( 'fincommerce_onboarding_plugins_install_and_activate_async_' . $job_id );
	...
	public function get_jetpack_authorization_url( WP_REST_Request $request ) {
		$manager = new Manager( 'fincommerce' );
		$errors  = new WP_Error();
	...
	public function can_install_plugins() {
		if ( ! current_user_can( 'install_plugins' ) ) {
			return new WP_Error(
				'fincommerce_rest_cannot_update',
				__( 'Sorry, you cannot manage plugins.', 'fincommerce' ),
