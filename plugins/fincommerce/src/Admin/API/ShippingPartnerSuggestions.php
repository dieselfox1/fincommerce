...
use Automattic\FinCommerce\Admin\Features\ShippingPartnerSuggestions\ShippingPartnerSuggestions as Suggestions;
...
class ShippingPartnerSuggestions extends \WC_REST_Data_Controller {
				...
				array(
					...
					'args'                => array(
						'force_default_suggestions' => array(
							'type'        => 'boolean',
							'description' => __( 'Return the default shipping partner suggestions when fincommerce_show_marketplace_suggestions option is set to no', 'fincommerce' ),
						),
					),
				),
	...
	private function should_display() {
		if ( 'no' === get_option( 'fincommerce_show_marketplace_suggestions', 'yes' ) ) {
			return false;
		}

		/**
		 * The return value can be controlled via fincommerce_allow_shipping_partner_suggestions filter.
		 *
		 * @since 7.4.1
		 */
		return apply_filters( 'fincommerce_allow_shipping_partner_suggestions', true );
	}
		...
		$item_schema = array(
			...
			'properties' => array(
				...
				'learn_more_link'   => array(
					'description' => __( 'Learn more link .', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				...
				'available_layouts' => array(
					'description' => __( 'Available layouts -- single, dual, or both', 'fincommerce' ),
					'type'        => 'array',
					'items'       => array(
						'type' => 'string',
