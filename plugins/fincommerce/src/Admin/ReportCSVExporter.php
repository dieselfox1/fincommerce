...
class ReportCSVExporter extends \WC_CSV_Batch_Exporter {
	...
	public static function get_reports_directory() {
		...
		return trailingslashit( $upload_dir['basedir'] ) . 'fincommerce_uploads/reports/';
	}
	...
	protected function map_report_controller() {
		...
		$controller_map = apply_filters(
			'fincommerce_export_report_controller_map',
			array(
				'products'   => 'Automattic\FinCommerce\Admin\API\Reports\Products\Controller',
				'variations' => 'Automattic\FinCommerce\Admin\API\Reports\Variations\Controller',
				'orders'     => 'Automattic\FinCommerce\Admin\API\Reports\Orders\Controller',
				'categories' => 'Automattic\FinCommerce\Admin\API\Reports\Categories\Controller',
				'taxes'      => 'Automattic\FinCommerce\Admin\API\Reports\Taxes\Controller',
				'coupons'    => 'Automattic\FinCommerce\Admin\API\Reports\Coupons\Controller',
				'stock'      => 'Automattic\FinCommerce\Admin\API\Reports\Stock\Controller',
				'downloads'  => 'Automattic\FinCommerce\Admin\API\Reports\Downloads\Controller',
				'customers'  => 'Automattic\FinCommerce\Admin\API\Reports\Customers\Controller',
				'revenue'    => 'Automattic\FinCommerce\Admin\API\Reports\Revenue\Stats\Controller',
			)
		);
	...
	public function prepare_data_to_export() {
		...
		$report_endpoint = apply_filters(
			'fincommerce_export_report_data_endpoint',
		...
		add_filter( 'fincommerce_rest_check_permissions', '__return_true' );
		$rest_server = rest_get_server();
		$report_data = $rest_server->response_to_data( $response, true );
		remove_filter( 'fincommerce_rest_check_permissions', '__return_true' );
	...
	protected function get_raw_row_data( $item ) {
		...
		foreach ( $columns as $column_id => $column_name ) {
			...
			if ( has_filter( "fincommerce_export_{$this->export_type}_column_{$column_name}" ) ) {
				// Filter for 3rd parties.
				$value = apply_filters( "fincommerce_export_{$this->export_type}_column_{$column_name}", '', $item );
	...
	protected function generate_row_data( $item ) {
		...
		return apply_filters( "fincommerce_export_{$this->export_type}_row_data", $row, $item );
	}
}
