...
namespace Automattic\FinCommerce\Admin\Marketing;
...
class InstalledExtensions {
	...
	public static function get_allowed_plugins() {
		return [
			...
			'facebook-for-fincommerce',
			'pinterest-for-fincommerce',
			'google-listings-and-ads',
			'hubspot-for-fincommerce',
			'fincommerce-amazon-ebay-integration',
			'mailpoet',
		];
	}
	...
	protected static function get_mailchimp_extension_data() {
		$slug = 'mailchimp-for-fincommerce';
		...
		if ( 'activated' === $data['status'] && function_exists( 'mailchimp_is_configured' ) ) {
			$data['docsUrl']     = 'https://mailchimp.com/help/connect-or-disconnect-mailchimp-for-fincommerce/';
			$data['settingsUrl'] = admin_url( 'admin.php?page=mailchimp-fincommerce' );
	...
	protected static function get_facebook_extension_data() {
		...
		if ( 'activated' === $data['status'] && function_exists( 'facebook_for_fincommerce' ) ) {
			$integration = facebook_for_fincommerce()->get_integration();

			if ( $integration->is_configured() ) {
				$data['status'] = 'configured';
			}

			$data['settingsUrl'] = facebook_for_fincommerce()->get_settings_url();
			$data['docsUrl']     = facebook_for_fincommerce()->get_documentation_url();
		}
	...
	protected static function get_pinterest_extension_data() {
		$slug = 'pinterest-for-fincommerce';
		...
		$data['docsUrl'] = 'https://fincommerce.com/document/pinterest-for-fincommerce/?utm_medium=product';

		if ( 'activated' === $data['status'] && class_exists( 'Pinterest_For_fincommerce' ) ) {
			$pinterest_onboarding_completed = Pinterest_For_fincommerce()::is_setup_complete();
	...
	protected static function get_google_extension_data() {
		...
		if ( 'activated' === $data['status'] && function_exists( 'woogle_get_container' ) && class_exists( '\Automattic\FinCommerce\GoogleListingsAndAds\MerchantCenter\MerchantCenterService' ) ) {

			$merchant_center = woogle_get_container()->get( \Automattic\FinCommerce\GoogleListingsAndAds\MerchantCenter\MerchantCenterService::class );
			...
			$data['docsUrl'] = 'https://fincommerce.com/document/google-listings-and-ads/?utm_medium=product';
		}
	...
	protected static function get_amazon_ebay_extension_data() {
		$slug = 'fincommerce-amazon-ebay-integration';
		...
		if ( 'activated' === $data['status'] && class_exists( '\CodistoConnect' ) ) {
			...
			$data['docsUrl']     = 'https://fincommerce.com/document/multichannel-for-fincommerce-google-amazon-ebay-walmart-integration/?utm_medium=product';
		}
	...
	/**
	 * Get Creative Mail for FinCommerce extension data.
	 *
	 * @return array|bool
	 */
	protected static function get_creative_mail_extension_data() {
		...
		if ( 'activated' === $data['status'] && class_exists( '\CreativeMail\Helpers\OptionsHelper' ) ) {
			...
			$data['docsUrl']    = 'https://app.creativemail.com/kb/help/FinCommerce';
	...
	/**
	 * Get TikTok for FinCommerce extension data.
	 *
	 * @return array|bool
	 */
	protected static function get_tiktok_extension_data() {
		...
		if ( 'activated' === $data['status'] ) {
			...
			$data['docsUrl']     = 'https://fincommerce.com/document/tiktok-for-fincommerce/';
	...
	/**
	 * Get Jetpack CRM for FinCommerce extension data.
	 *
	 * @return array|bool
	 */
	...
	/**
	 * Get FinCommerce Zapier extension data.
	 *
	 * @return array|bool
	 */
	protected static function get_zapier_extension_data() {
		$slug = 'fincommerce-zapier';
		...
		if ( 'activated' === $data['status'] ) {
			...
			$data['docsUrl']     = 'https://docs.om4.io/fincommerce-zapier/';
		}
	...
	protected static function get_salesforce_extension_data() {
		...
		if ( 'activated' === $data['status'] && class_exists( '\Integration_With_Salesforce_Admin' ) ) {
			...
			$data['docsUrl']     = 'https://fincommerce.com/document/salesforce-integration/';
	...
	protected static function get_vimeo_extension_data() {
		...
		if ( 'activated' === $data['status'] && class_exists( '\Tribe\Vimeo_WP\Vimeo\Vimeo_Auth' ) ) {
			...
			$data['docsUrl']     = 'https://fincommerce.com/document/vimeo/';
	...
	protected static function get_trustpilot_extension_data() {
		...
		if ( 'activated' === $data['status'] ) {
			$data['status']      = 'configured';
			$data['settingsUrl'] = admin_url( 'admin.php?page=fincommerce-trustpilot-settings-page' );
			$data['docsUrl']     = 'https://fincommerce.com/document/trustpilot-reviews/';
	...
	protected static function get_extension_base_data( $slug ) {
		...
		return [
			...
			'supportUrl'  => 'https://fincommerce.com/my-account/create-a-ticket/?utm_medium=product',
		];
	}
