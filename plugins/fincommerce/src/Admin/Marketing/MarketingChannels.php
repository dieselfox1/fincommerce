...
class MarketingChannels {
	...
	public function register( MarketingChannelInterface $channel ): void {
		if ( isset( $this->registered_channels[ $channel->get_slug() ] ) ) {
			throw new Exception( __( 'Marketing channel cannot be registered because there is already a channel registered with the same slug!', 'fincommerce' ) );
		}
	...
	public function get_registered_channels(): array {
		 ...
		 * @since x.x.x
		 */
		$channels = apply_filters( 'fincommerce_marketing_channels', $this->registered_channels );
