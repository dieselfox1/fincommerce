...
class Notes {
	...
	public static function init() {
		...
		add_action( 'update_option_fincommerce_show_marketplace_suggestions', array( __CLASS__, 'possibly_delete_marketing_notes' ), 10, 2 );
	...
	public static function get_notes( $context = 'edit', $args = array() ) {
		...
		foreach ( (array) $raw_notes as $raw_note ) {
			try {
				 ...
				 */
				$note                               = apply_filters( 'fincommerce_get_note_from_db', $note );
				$note_id                            = $note->get_id();
	...
	public static function trigger_note_action( $note, $triggered_action ) {
		 ...
		 */
		do_action( 'fincommerce_note_action', $triggered_action->name, $note );
		 ...
		 * @param Note $note The corresponding Note.
		 */
		do_action( 'fincommerce_note_action_' . $triggered_action->name, $note );
	...
	public static function load_data_store() {
		try {
			...
			throw new NotesUnavailableException(
				'fincommerce_admin_notes_unavailable',
				__( 'Notes are unavailable because the "admin-note" data store cannot be loaded.', 'fincommerce' )
			);
		}
	}
}
