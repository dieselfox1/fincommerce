...
namespace Automattic\FinCommerce\Admin\RemoteInboxNotifications;

defined( 'ABSPATH' ) || exit;

use Automattic\FinCommerce\Admin\Features\Features;
use Automattic\FinCommerce\Admin\Notes\Notes;
use Automattic\FinCommerce\Admin\PluginsProvider\PluginsProvider;
use Automattic\FinCommerce\Internal\Admin\Onboarding\OnboardingProfile;
use Automattic\FinCommerce\Admin\Notes\Note;
use Automattic\FinCommerce\Admin\RemoteSpecs\RemoteSpecsEngine;
use Automattic\FinCommerce\Admin\RemoteSpecs\RuleProcessors\StoredStateSetupForProducts;
...
class RemoteInboxNotificationsEngine extends RemoteSpecsEngine {
	 ...
	 * phpcs:disable FinCommerce.Functions.InternalInjectionMethod.MissingInternalTag
	 */
	public static function init() {
		...
		add_action(
			'fincommerce_run_on_fincommerce_admin_updated',
			array( __CLASS__, 'run_on_fincommerce_admin_updated' )
		);
		add_action(
			'fincommerce_updated',
			function () {
				$next_hook = WC()->queue()->get_next(
					'fincommerce_run_on_fincommerce_admin_updated',
					array(),
					'fincommerce-remote-inbox-engine'
				);
				if ( null === $next_hook ) {
		...
		add_filter( 'fincommerce_get_note_from_db', array( __CLASS__, 'get_note_from_db' ), 10, 1 );
		add_filter( 'fincommerce_debug_tools', array( __CLASS__, 'add_debug_tools' ) );
		add_action(
			'wp_ajax_fincommerce_json_inbox_notifications_search',
	...
	/**
	 * Set an option indicating that FinCommerce Admin has just been updated,
	 * run the specs, then clear that option. This lets the
	 * fincommerceAdminUpdatedRuleProcessor trigger on WCA update.
	 */
	public static function run_on_fincommerce_admin_updated() {
	...
	/**
	 * Add the debug tools to the FinCommerce debug tools (FinCommerce > Status > Tools).
	 *
	...
	public static function add_debug_tools( $tools ) {
		...
		$tools['refresh_remote_inbox_notifications'] = array(
			'name'     => __( 'Refresh Remote Inbox Notifications', 'fincommerce' ),
			'button'   => __( 'Refresh', 'fincommerce' ),
			'desc'     => __( 'This will refresh the remote inbox notifications', 'fincommerce' ),
			'callback' => function () {
				RemoteInboxNotificationsDataSourcePoller::get_instance()->read_specs_from_data_sources();
				RemoteInboxNotificationsEngine::run();

				return __( 'Remote inbox notifications have been refreshed', 'fincommerce' );
			},
		);

		$tools['delete_inbox_notification'] = array(
			'name'     => __( 'Delete an Inbox Notification', 'fincommerce' ),
			'button'   => __( 'Delete', 'fincommerce' ),
			'desc'     => __( 'This will delete an inbox notification by slug', 'fincommerce' ),
			'selector' => array(
				'description'   => __( 'Select an inbox notification to delete:', 'fincommerce' ),
				'class'         => 'wc-product-search',
				'search_action' => 'fincommerce_json_inbox_notifications_search',
				'name'          => 'delete_inbox_notification_note_id',
				'placeholder'   => esc_attr__( 'Search for an inbox notification&hellip;', 'fincommerce' ),
			),
			'callback' => function () {
				check_ajax_referer( 'debug_action', '_wpnonce' );

				if ( ! isset( $_GET['delete_inbox_notification_note_id'] ) ) {
					return __( 'No inbox notification selected', 'fincommerce' );
				}
				$note_id = wc_clean( sanitize_text_field( wp_unslash( $_GET['delete_inbox_notification_note_id'] ) ) );
				$note = Notes::get_note( $note_id );

				if ( ! $note ) {
					return __( 'Inbox notification not found', 'fincommerce' );
				}

				$note->delete( true );
				return __( 'Inbox notification has been deleted', 'fincommerce' );
			},
		);
