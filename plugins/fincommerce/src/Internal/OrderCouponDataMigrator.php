...
namespace Automattic\FinCommerce\Internal;

use Automattic\FinCommerce\Internal\BatchProcessing\BatchProcessingController;
use Automattic\FinCommerce\Internal\BatchProcessing\BatchProcessorInterface;
...
class OrderCouponDataMigrator implements BatchProcessorInterface, RegisterHooksInterface {
	...
	public function register() {
		add_filter( 'fincommerce_debug_tools', array( $this, 'handle_fincommerce_debug_tools' ), 999, 1 );
	}
	...
	public function get_total_pending_count(): int {
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}fincommerce_order_itemmeta WHERE meta_key=%s",
				'coupon_data'
			)
		);
	}
	 ...
	 * A batch in this context is a list of 'meta_id' values from the wp_fincommerce_order_itemmeta table.
	 *
	...
	public function get_next_batch_to_process( int $size ): array {
		...
		$meta_ids = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT meta_id FROM {$wpdb->prefix}fincommerce_order_itemmeta WHERE meta_key=%s ORDER BY meta_id ASC LIMIT %d",
				'coupon_data',
				$size
			)
		);
	 ...
	 */
	public function handle_fincommerce_debug_tools( array $tools ): array {
		...
		if ( 0 === $pending_count ) {
			$tools['start_convert_order_coupon_data'] = array(
				'name'     => __( 'Start converting order coupon data to the simplified format', 'fincommerce' ),
				'button'   => __( 'Start converting', 'fincommerce' ),
				'disabled' => true,
				'desc'     => __( 'This will convert <code>coupon_data</code> order item meta entries to simplified <code>coupon_info</code> entries. The conversion will happen overtime in the background (via Action Scheduler). There are currently no entries to convert.', 'fincommerce' ),
			);
		} elseif ( $batch_processor->is_enqueued( self::class ) ) {
			$tools['stop_convert_order_coupon_data'] = array(
				'name'     => __( 'Stop converting order coupon data to the simplified format', 'fincommerce' ),
				'button'   => __( 'Stop converting', 'fincommerce' ),
				'desc'     =>
					/* translators: %d=count of entries pending conversion */
					sprintf( __( 'This will stop the background process that converts <code>coupon_data</code> order item meta entries to simplified <code>coupon_info</code> entries. There are currently %d entries that can be converted.', 'fincommerce' ), $pending_count ),
				'callback' => array( $this, 'dequeue' ),
			);
		} else {
			$tools['start_converting_order_coupon_data'] = array(
				'name'     => __( 'Convert order coupon data to the simplified format', 'fincommerce' ),
				'button'   => __( 'Start converting', 'fincommerce' ),
				'desc'     =>
					/* translators: %d=count of entries pending conversion */
					sprintf( __( 'This will convert <code>coupon_data</code> order item meta entries to simplified <code>coupon_info</code> entries. The conversion will happen overtime in the background (via Action Scheduler). There are currently %d entries that can be converted.', 'fincommerce' ), $pending_count ),
	...
	public function enqueue(): string {
		...
		if ( $batch_processor->is_enqueued( self::class ) ) {
			return __( 'Background process for coupon meta conversion already started, nothing done.', 'fincommerce' );
		}

		$batch_processor->enqueue_processor( self::class );
		return __( 'Background process for coupon meta conversion started', 'fincommerce' );
	}
	...
	public function dequeue(): string {
		$batch_processor = wc_get_container()->get( BatchProcessingController::class );
		if ( ! $batch_processor->is_enqueued( self::class ) ) {
			return __( 'Background process for coupon meta conversion not started, nothing done.', 'fincommerce' );
		}

		$batch_processor->remove_processor( self::class );
		return __( 'Background process for coupon meta conversion stopped', 'fincommerce' );
	}
}
