...
namespace Automattic\FinCommerce\Internal\Features;

use Automattic\FinCommerce\Internal\Admin\EmailPreview\EmailPreview;
use WC_Tracks;
...
use Automattic\FinCommerce\Utilities\PluginUtil;

defined( 'ABSPATH' ) || exit;

/**
 * Class to define the FinCommerce features that can be enabled and disabled by admin users,
 * provides also a mechanism for FinCommerce plugins to declare that they are compatible
 * (or incompatible) with a given feature.
 *
 * Note: the 'fincommerce_register_feature_definitions' hook allows registering new features
...
class FeaturesController {
	...
	public const FEATURE_ENABLED_CHANGED_ACTION = 'fincommerce_feature_enabled_changed';

	public const PLUGINS_COMPATIBLE_BY_DEFAULT_OPTION = 'fincommerce_plugins_are_compatible_with_features_by_default';
	...
	/**
	 * The registered compatibility info for FinCommerce plugins, with plugin names as keys.
	 *
	 * @var array
	 */
	...
	/**
	 * The registered compatibility info for FinCommerce plugins, with feature ids as keys.
	 *
	 * @var array
	 */
	 ...
	 * via fincommerce_register_feature_definitions action.
	 *
	 * @var bool
	 */
	...
	public function __construct() {
		// In principle, register_additional_features is triggered manually from within class-fincommerce
		// right before before_fincommerce_init is fired (this is needed for the features to be visible
		...
		if ( ! $this->registered_additional_features_via_action ) {
			if ( did_action( 'before_fincommerce_init' ) ) {
				// Needed for unit tests, where 'before_fincommerce_init' will have been fired already at this point.
				$this->register_additional_features();
			} else {
				// This needs to have a higher $priority than the 'before_fincommerce_init' hooked by plugins that declare compatibility.
				add_filter( 'before_fincommerce_init', array( $this, 'register_additional_features' ), -9999, 0 );
			}
		}
		...
		add_filter( 'fincommerce_get_sections_advanced', array( $this, 'add_features_section' ), 10, 1 );
		add_filter( 'fincommerce_get_settings_advanced', array( $this, 'add_feature_settings' ), 10, 2 );
		...
		add_filter( 'fincommerce_admin_shared_settings', array( $this, 'set_change_feature_enable_nonce' ), 20, 1 );
		...
		add_filter( 'fincommerce_admin_features', array( $this, 'sync_iapi_mini_cart_feature' ) );
	}
	...
	public function sync_iapi_mini_cart_feature( $features ) {
		$option_name = 'fincommerce_feature_experimental-iapi-mini-cart_enabled';
	 ...
	 * Register a feature.
	 *
	 * This used to be called during the `fincommerce_register_feature_definitions` action hook,
	...
	private function get_feature_definitions() {
		...
		if ( ! $this->registered_additional_features_via_class_calls ) {
			...
			// These used to be tied to the now deprecated fincommerce_register_feature_definitions action,
	 ...
	 * - Features that get initialized via the (deprecated) fincommerce_register_feature_definitions.
	...
	private function init_feature_definitions(): void {
		$alpha_feature_testing_is_enabled = Constants::is_true( 'fincommerce_ENABLE_ALPHA_FEATURE_TESTING' );
		$tracking_enabled                 = WC_Site_Tracking::is_tracking_enabled();

		$legacy_features = array(
			'analytics'                   => array(
				'name'               => __( 'Analytics', 'fincommerce' ),
				'description'        => __( 'Enable FinCommerce Analytics', 'fincommerce' ),
			...
			'product_block_editor'        => array(
				'name'            => __( 'New product editor', 'fincommerce' ),
				'description'     => __( 'Try the new product editor (Beta)', 'fincommerce' ),
				'is_experimental' => true,
				'disable_ui'      => false,
				'is_legacy'       => true,
				'disabled'        => function () {
			...
			'cart_checkout_blocks'        => array(
				'name'            => __( 'Cart & Checkout Blocks', 'fincommerce' ),
				'description'     => __( 'Optimize for faster checkout', 'fincommerce' ),
				'is_experimental' => false,
				'disable_ui'      => true,
			),
			'rate_limit_checkout'         => array(
				'name'               => __( 'Rate limit Checkout', 'fincommerce' ),
				'description'        => sprintf(
					// translators: %s is the URL to the rate limiting documentation.
					__( 'Enables rate limiting for Checkout place order and Store API /checkout endpoint. To further control this, refer to <a href="%s" target="_blank">rate limiting documentation</a>.', 'fincommerce' ),
					'https://developer.fincommerce.com/docs/apis/store-api/rate-limiting/'
				),
				'is_experimental'    => false,
				'disable_ui'         => false,
				'enabled_by_default' => false,
			...
			'marketplace'                 => array(
				'name'               => __( 'Marketplace', 'fincommerce' ),
				'description'        => __(
					'New, faster way to find extensions and themes for your FinCommerce store',
					'fincommerce'
				),
				'is_experimental'    => false,
				'enabled_by_default' => true,
			...
			// https://github.com/dieselfox1/fincommerce/pull/39701#discussion_r1376976959.
			'order_attribution'           => array(
				'name'               => __( 'Order Attribution', 'fincommerce' ),
				'description'        => __(
			...
			'hpos_fts_indexes'            => array(
				'name'               => __( 'HPOS Full text search indexes', 'fincommerce' ),
				'description'        => __(
			...
			'hpos_datastore_caching'      => array(
				'name'               => __( 'HPOS Data Caching', 'fincommerce' ),
				'description'        => __(
			...
			'remote_logging'              => array(
				'name'               => __( 'Remote Logging', 'fincommerce' ),
				'description'        => sprintf(
					/* translators: %1$s: opening link tag, %2$s: closing link tag */
					__( 'Allow FinCommerce to send error logs and non-sensitive diagnostic data to help improve FinCommerce. This feature requires %1$susage tracking%2$s to be enabled.', 'fincommerce' ),
					'<a href="' . admin_url( 'admin.php?page=wc-settings&tab=advanced&section=fincommerce_com' ) . '">',
					'</a>'
				),
				'enabled_by_default' => true,
				'disable_ui'         => false,
				 ...
				 * feature.
				 *
				 * @see https://github.com/dieselfox1/fincommerce/pull/39701#discussion_r1376976959
				 */
				'is_legacy'          => true,
				'is_experimental'    => false,
				'setting'            => array(
					...
					'desc_tip' => function () use ( $tracking_enabled ) {
						if ( ! $tracking_enabled ) {
							return __( '⚠ Usage tracking must be enabled to use remote logging.', 'fincommerce' );
						}
						return '';
					},
				),
			...
			'email_improvements'          => array(
				'name'            => __( 'Email improvements', 'fincommerce' ),
				'description'     => __(
				 ...
				 *
				 * @see https://github.com/dieselfox1/fincommerce/issues/39147
				 * @see https://github.com/dieselfox1/fincommerce/issues/55540
				 */
				'is_legacy'       => true,
				'is_experimental' => false,
			),
			'blueprint'                   => array(
				'name'               => __( 'Blueprint (beta)', 'fincommerce' ),
				'description'        => __(
			...
			'block_email_editor'          => array(
				'name'               => __( 'Block Email Editor (alpha)', 'fincommerce' ),
				'description'        => __(
					'Enable the block-based email editor for transactional emails. <a href="https://github.com/dieselfox1/fincommerce/discussions/52897#discussioncomment-11630256" target="_blank">Learn more</a>',
					'fincommerce'
				),
			...
			'point_of_sale'               => array(
				'name'               => __( 'Point of Sale', 'fincommerce' ),
				'description'        => __(
					'Enable Point of Sale functionality in the FinCommerce mobile apps.',
					'fincommerce'
				),
				'enabled_by_default' => true,
				'disable_ui'         => false,
			...
			'fulfillments'                => array(
				'name'               => __( 'Order Fulfillments', 'fincommerce' ),
				'description'        => __(
			...
			'experimental-iapi-mini-cart' => array(
				'name'            => __( 'Interactivity API powered Mini Cart', 'fincommerce' ),
				'description'     => __( 'Enable the new version of the Mini Cart that uses the Interactivity API instead of React in the frontend.', 'fincommerce' ),
				'is_experimental' => true,
			),
		);
	...
	/**
	 * Function to trigger the (now deprecated) 'fincommerce_register_feature_definitions' hook.
	 *
	 * This function must execute immediately before the 'before_fincommerce_init'
	...
	public function register_additional_features() {
		 ...
		 */
		do_action( 'fincommerce_register_feature_definitions', $this );
	...
	/**
	 * Get all the existing FinCommerce features.
	 *
	...
	public function get_plugins_are_incompatible_by_default( string $feature_id ): bool {
		...
		if ( is_null( $feature_definition ) ) {
			throw new \InvalidArgumentException( esc_html( "The FinCommerce feature '$feature_id' doesn't exist" ) );
		}
		 ...
		 * @since 9.2.0
		 */
		return (bool) apply_filters( 'fincommerce_plugins_are_incompatible_with_feature_by_default', $incompatible_by_default, $feature_id );
	}
	 ...
	 *
	 * This method MUST be executed from inside a handler for the 'before_fincommerce_init' hook.
	 *
	...
	public function declare_compatibility( string $feature_id, string $plugin_file, bool $positive_compatibility = true ): bool {
		if ( ! $this->proxy->call_function( 'doing_action', 'before_fincommerce_init' ) ) {
			$class_and_method = ( new \ReflectionClass( $this ) )->getShortName() . '::' . __FUNCTION__;
			/* translators: 1: class::method 2: before_fincommerce_init */
			$this->proxy->call_function( 'wc_doing_it_wrong', $class_and_method, sprintf( __( '%1$s should be called inside the %2$s action.', 'fincommerce' ), $class_and_method, 'before_fincommerce_init' ), '7.0' );
			return false;
		}
		if ( ! $this->feature_exists( $feature_id ) ) {
			return false;
	 ...
	 *
	 * @internal For usage by FinCommerce core only. Backwards compatibility not guaranteed.
	 * @since 10.1.0
	 *
	 * @param string $feature_id Unique feature ID.
	 ...
	 *
	 * This method can't be called before the 'fincommerce_init' hook is fired.
	 *
	...
	public function get_compatible_features_for_plugin( string $plugin_name, bool $enabled_features_only = false ): array {
		...
		$this->verify_did_fincommerce_init( __FUNCTION__ );
	...
	public function get_compatible_plugins_for_feature( string $feature_id, bool $active_only = false ): array {
		...
		$woo_aware_plugins = $this->plugin_util->get_fincommerce_aware_plugins( $active_only );
		if ( ! $this->feature_exists( $feature_id ) ) {
			return array(
				'compatible'   => array(),
				'incompatible' => array(),
	...
	/**
	 * Check if the 'fincommerce_init' has run or is running, do a 'wc_doing_it_wrong' if not.
	 *
	 * @param string|null $function_name Name of the invoking method, if not null, 'wc_doing_it_wrong' will be invoked if 'fincommerce_init' has not run and is not running.
	 *
	 * @return bool True if 'fincommerce_init' has run or is running, false otherwise.
	 */
	private function verify_did_fincommerce_init( ?string $function_name = null ): bool {
		if ( ! $this->proxy->call_function( 'did_action', 'fincommerce_init' ) &&
			! $this->proxy->call_function( 'doing_action', 'fincommerce_init' ) ) {
			if ( ! is_null( $function_name ) ) {
				$class_and_method = ( new \ReflectionClass( $this ) )->getShortName() . '::' . $function_name;
				/* translators: 1: class::method 2: plugins_loaded */
				$this->proxy->call_function( 'wc_doing_it_wrong', $class_and_method, sprintf( __( '%1$s should not be called before the %2$s action.', 'fincommerce' ), $class_and_method, 'fincommerce_init' ), '7.0' );
			}
			return false;
		}
	 ...
	 * defaults to "fincommerce_feature_{$feature_id}_enabled" if a different
	...
	public function feature_enable_option_name( string $feature_id ): string {
		...
		return "fincommerce_feature_{$feature_id}_enabled";
	}
	 ...
	 * from the FinCommerce feature settings page.
	 */
	...
	public function process_updated_option( string $option, $old_value, $value ) {
		$matches                   = array();
		$is_default_key            = preg_match( '/^fincommerce_feature_([a-zA-Z0-9_]+)_enabled$/', $option, $matches );
	...
	/**
	 * Handler for the 'fincommerce_get_sections_advanced' hook,
	...
	public function add_features_section( $sections ) {
		if ( ! isset( $sections['features'] ) ) {
			$sections['features'] = __( 'Features', 'fincommerce' );
		}
		return $sections;
	}

	/**
	 * Handler for the 'fincommerce_get_settings_advanced' hook,
	 * it adds the settings UI for all the existing features.
	 *
	 * Note that the settings added via the 'fincommerce_settings_features' hook will be
	...
	public function add_feature_settings( $settings, $current_section ): array {
		...
		$feature_settings = array(
			array(
				'title' => __( 'Features', 'fincommerce' ),
				'type'  => 'title',
				'desc'  => __( 'Start using new features that are being progressively rolled out to improve the store management experience.', 'fincommerce' ),
				'id'    => 'features_options',
			),
		);
		...
		foreach ( $feature_ids as $id ) {
			if ( 'mature_features_end' === $id ) {
				...
				/**
				 * Filter allowing to add additional settings to the FinCommerce Advanced - Features settings page.
				 *
				 * @param bool $disabled False.
				 */
				$feature_settings = apply_filters( 'fincommerce_settings_features', $feature_settings );
				...
				if ( ! empty( $experimental_feature_ids ) ) {
					...
					$feature_settings[] = array(
						'title' => __( 'Experimental features', 'fincommerce' ),
						'type'  => 'title',
						'desc'  => __( 'These features are either experimental or incomplete, enable them at your own risk!', 'fincommerce' ),
		...
		if ( $this->verify_did_fincommerce_init() ) {
	...
	private function get_setting_for_feature( string $feature_id, array $feature ): array {
		...
		// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingSinceComment
		/**
		 * Filter allowing FinCommerce Admin to be disabled.
		 *
		 * @param bool $disabled False.
		 */
		...
		if ( ( 'analytics' === $feature_id || 'new_navigation' === $feature_id ) && $admin_features_disabled ) {
			$disabled = true;
			$desc_tip = __( 'FinCommerce Admin has been disabled', 'fincommerce' );
		...
		if ( ! $this->is_legacy_feature( $feature_id ) && ! $disabled && $this->verify_did_fincommerce_init() ) {
		 ...
		 */
		$desc_tip = apply_filters( 'fincommerce_feature_description_tip', $desc_tip, $feature_id, $disabled );
		 ...
		 * @since 8.0.0
		 */
		return apply_filters( 'fincommerce_feature_setting', $feature_setting, $feature_id );
	}
	...
	private function maybe_display_feature_incompatibility_warning(): void {
		...
		$relevant_plugins     = array_diff( $this->plugin_util->get_fincommerce_aware_plugins( true ), $this->plugins_excluded_from_compatibility_ui );
	...
	private function maybe_display_current_feature_filter_description(): bool {
		...
		$message =
			'all' === $feature_id
			? __( 'You are viewing active plugins that are incompatible with currently enabled FinCommerce features.', 'fincommerce' )
			: sprintf(
				/* translators: %s is a feature name. */
				__( "You are viewing the active plugins that are incompatible with the '%s' feature.", 'fincommerce' ),
				$features[ $feature_id ]['name']
			);

		$message .= '<br />';
		$message .= sprintf(
			__( "<a href='%1\$s'>View all plugins</a> - <a href='%2\$s'>Manage FinCommerce features</a>", 'fincommerce' ),
			$plugins_page_url,
			$features_page_url
		);
	 ...
	 *
	 * @see https://github.com/dieselfox1/fincommerce/issues/37343
	 *
	 * @return void
	 *
	 * @internal For exclusive usage of FinCommerce core, backwards compatibility not guaranteed.
	...
	public function handle_plugin_list_rows( $plugin_file, $plugin_data ) {
		...
		if ( is_null( $wp_list_table ) || ! $this->plugin_util->is_fincommerce_aware_plugin( $plugin_data ) ) {
			return;
		}
		...
		if ( $incompatible_features_count > 0 ) {
			...
			if ( 1 === $incompatible_features_count ) {
				$message = sprintf(
					...
					__( "⚠ This plugin is incompatible with the enabled FinCommerce feature '%s', it shouldn't be activated.", 'fincommerce' ),
					$features[ $incompatible_features[0] ]['name']
				);
			} elseif ( 2 === $incompatible_features_count ) {
				/* translators: %1\$s, %2\$s = printable plugin names */
				$message = sprintf(
					__( "⚠ This plugin is incompatible with the enabled FinCommerce features '%1\$s' and '%2\$s', it shouldn't be activated.", 'fincommerce' ),
					$features[ $incompatible_features[0] ]['name'],
				...
				$message = sprintf(
					__( "⚠ This plugin is incompatible with the enabled FinCommerce features '%1\$s', '%2\$s' and %3\$d more, it shouldn't be activated.", 'fincommerce' ),
					$features[ $incompatible_features[0] ]['name'],
					$features[ $incompatible_features[1] ]['name'],
	...
	public function handle_plugins_page_views_list( $views ): array {
		...
		$incompatible_text          =
			'all' === $feature_id
			? __( 'Incompatible with FinCommerce features', 'fincommerce' )
			/* translators: %s = name of a FinCommerce feature */
			: sprintf( __( "Incompatible with '%s'", 'fincommerce' ), $features[ $feature_id ]['name'] );
	...
	private function is_preview_email_improvements_enabled( string $feature_id ): bool {
		 ...
		 */
		$is_email_preview = apply_filters( 'fincommerce_is_email_preview', false );
		if ( $is_email_preview ) {
