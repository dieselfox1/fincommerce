<?php
/**
 * FinCommerce Section Block class.
 */
...
use Automattic\FinCommerce\Admin\Features\ProductBlockEditor\ProductTemplates\SectionInterface;
use Automattic\FinCommerce\Admin\Features\ProductBlockEditor\ProductTemplates\SubsectionInterface;
...
class Section extends ProductBlock implements SectionInterface {
	...
	public function __construct( array $config, BlockTemplateInterface &$root_template, ?ContainerInterface &$parent = null ) { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.parentFound
		if ( ! empty( $config['blockName'] ) ) {
			throw new \InvalidArgumentException( 'Unexpected key "blockName", this defaults to "fincommerce/product-section".' );
		}
		parent::__construct( array_merge( array( 'blockName' => 'fincommerce/product-section' ), $config ), $root_template, $parent );
	}
