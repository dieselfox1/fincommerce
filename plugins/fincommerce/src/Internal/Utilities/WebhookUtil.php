	...
	public function maybe_render_user_with_webhooks_warning( \WP_User $current_user, array $userids ): void {
		...
		foreach ( $userids as $user_id ) {
			...
			$text = sprintf(
				...
				_nx(
					'User #%1$s %2$s has created %3$d FinCommerce webhook.',
					'User #%1$s %2$s has created %3$d FinCommerce webhooks.',
					$webhooks_count,
					'user webhook count',
					'fincommerce'
				),
		...
		if ( $users_have_content ) {
			...
			$text = __( 'The affected FinCommerce webhooks will <b>not</b> be deleted and will be attributed to user id 0.<br/>', 'fincommerce' );
		}

		$text .= sprintf(
			/* translators: 1 = url of the FinCommerce webhooks settings page */
			__( 'After that they can be reassigned to the logged-in user by going to the <a href="%1$s">FinCommerce webhooks settings page</a> and re-saving them.', 'fincommerce' ),
