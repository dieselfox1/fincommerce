...
namespace Automattic\FinCommerce\Internal\Utilities;

use Automattic\FinCommerce\Internal\RegisterHooksInterface;
use Automattic\FinCommerce\Utilities\RestApiUtil;

/**
 * The Legacy REST API was removed in FinCommerce 9.0 and is now available as a dedicated extension.
 * A stub is kept in FinCommerce core that acts when the extension is not installed and has two purposes:
 *
 * 1. Return a "The FinCommerce API is disabled on this site" error for any request to the Legacy REST API endpoints.
 *
 * 2. Provide the not-endpoint related utility methods that were previously supplied by the WC_API class,
 *    this is achieved by setting the value of FinCommerce::api (typically accessed via 'WC()->api') to an instance of this class.
 *
 ...
 *
 * See: https://developer.fincommerce.com/2023/10/03/the-legacy-rest-api-will-move-to-a-dedicated-extension-in-fincommerce-9-0/
 */
class LegacyRestApiStub implements RegisterHooksInterface {
	 ...
	 * Otherwise it returns a "The FinCommerce API is disabled on this site" error,
	 * unless the request contains a "wc-api" variable and the appropriate
	 * "fincommerce_api_*" hook is set.
	 *
	 * @internal For exclusive usage of FinCommerce core, backwards compatibility not guaranteed.
	 */
	public static function parse_legacy_rest_api_request() {
		...
		if ( ! empty( $wp->query_vars['wc-api-version'] ) && ! empty( $wp->query_vars['wc-api-route'] ) ) {
			...
			echo wp_json_encode(
				array(
					'errors' => array(
						'code'    => 'fincommerce_api_disabled',
						'message' => 'The FinCommerce API is disabled on this site',
					),
				)
			);
			exit;
	...
	private static function maybe_process_wc_api_query_var() {
		...
		if ( ! empty( $wp->query_vars['wc-api'] ) ) {
			...
			// phpcs:disable FinCommerce.Commenting.CommentHooks.HookCommentWrongStyle

			// Trigger generic action before request hook.
			do_action( 'fincommerce_api_request', $api_request );

			// Is there actually something hooked into this API request? If not trigger 400 - Bad request.
			status_header( has_action( 'fincommerce_api_' . $api_request ) ? 200 : 400 );

			// Trigger an action which plugins can hook into to fulfill the request.
			do_action( 'fincommerce_api_' . $api_request );

			// phpcs:enable FinCommerce.Commenting.CommentHooks.HookCommentWrongStyle
	...
	/**
	 * Get data from a FinCommerce API endpoint.
	 * This method used to be part of the FinCommerce Legacy REST API.
	 *
	 * @since 9.1.0
	 *
	...
	public function get_endpoint_data( $endpoint, $params = array() ) {
		wc_doing_it_wrong(
			'get_endpoint_data',
			"'WC()->api->get_endpoint_data' is deprecated, please use the following instead: wc_get_container()->get(Automattic\FinCommerce\Utilities\RestApiUtil::class)->get_endpoint_data",
			'9.1.0'
		);
