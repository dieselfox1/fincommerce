...
class Users {
	 ...
	 * (fincommerce_network_wide_customers) is enabled.
	 *
	...
	public static function get_user_in_current_site( $user_id, ?int $requesting_user_id = null ) {
		...
		$error              = new WP_Error( 'wc_user_invalid_id', __( 'Invalid user ID.', 'fincommerce' ) );
		...
		if (
			...
			&& get_site_option( 'fincommerce_network_wide_customers', 'no' ) !== 'yes'
		) {
			return $error;
		}
	...
	public static function should_user_verify_order_email( $order_id, $supplied_email = null, $context = 'view' ) {
		 ...
		 */
		$verification_grace_period = (int) apply_filters( 'fincommerce_order_email_verification_grace_period', 10 * MINUTE_IN_SECONDS, $order, $context );
		 ...
		 */
		return (bool) apply_filters( 'fincommerce_order_email_verification_required', $email_verification_required, $order, $context );
	}
