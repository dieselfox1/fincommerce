...
use Automattic\FinCommerce\Utilities\{ PluginUtil, StringUtil };

/**
 * This class allows installing a plugin programmatically.
 *
 * Information about plugins installed in that way will be stored in a 'fincommerce_autoinstalled_plugins' option,
 * and a notice will be shown under the plugin name in the plugins list indicating that it was automatically
 * installed (these notices can be disabled with the 'fincommerce_show_autoinstalled_plugin_notices' hook).
 *
...
class PluginInstaller implements RegisterHooksInterface {
	 ...
	 * in the plugins list:
	 *
	 * - 'installed_by': defaults to 'FinCommerce' if not present.
	 * - 'info_link': if present, a "More information" link will be included in the notice.
	 *
	 * If 'installed_by' is supplied and it's not 'FinCommerce' (case-insensitive), an exception will be thrown
	 * if the code calling this method is not in a FinCommerce core file (in 'includes' or in 'src').
	 *
	 * Information about plugins successfully installed with this method will be kept in an option named
	 * 'fincommerce_autoinstalled_plugins'. Keys will be the plugin name and values will be associative arrays
	...
	public function install_plugin( string $plugin_url, array $metadata = array() ): array {
		...
		$plugins_being_installed = get_site_option( 'fincommerce_autoinstalling_plugins', array() );
		...
		update_site_option( 'fincommerce_autoinstalling_plugins', $plugins_being_installed );
		...
		try {
			...
			if ( empty( $plugins_being_installed ) ) {
				delete_site_option( 'fincommerce_autoinstalling_plugins' );
			} else {
				update_site_option( 'fincommerce_autoinstalling_plugins', $plugins_being_installed );
			}
	...
	private function install_plugin_core( string $plugin_url, array $metadata ): array {
		...
		if ( $install_ok ) {
			...
			$auto_installed_plugins                 = get_site_option( 'fincommerce_autoinstalled_plugins', array() );
			$auto_installed_plugins[ $plugin_name ] = $plugin_data;
			update_site_option( 'fincommerce_autoinstalled_plugins', $auto_installed_plugins );

			$auto_installed_plugins_history = get_site_option( 'fincommerce_history_of_autoinstalled_plugins', array() );
			if ( ! isset( $auto_installed_plugins_history[ $plugin_name ] ) ) {
				$auto_installed_plugins_history[ $plugin_name ] = $plugin_data;
				update_site_option( 'fincommerce_history_of_autoinstalled_plugins', $auto_installed_plugins_history );
			}
		...
		if ( is_multisite() ) {
			// We log the install in the main site, unless the main site doesn't have FinCommerce installed;
			// in that case we fallback to logging in the current site.
			switch_to_blog( get_main_site_id() );
			if ( self::fincommerce_is_active_in_current_site() ) {
				$post_install();
	...
	/**
	 * Check if FinCommerce is installed and active in the current blog.
	 * This is useful for multisite installs when a blog other than the one running this code is selected with 'switch_to_blog'.
	 *
	 * @return bool True if FinCommerce is installed and active in the current blog, false otherwise.
	 */
	private static function fincommerce_is_active_in_current_site(): bool {
		$active_valid_plugins = wc_get_container()->get( PluginUtil::class )->get_all_active_valid_plugins();

		return ! empty(
			array_filter(
				$active_valid_plugins,
				fn( $plugin ) => substr_compare( $plugin, '/fincommerce.php', -strlen( '/fincommerce.php' ) ) === 0
			)
		);
	}
	 ...
	 * that have been installed using this class (unless the 'fincommerce_show_autoinstalled_plugin_notices' filter
	...
	public function handle_plugin_list_rows( $plugin_file, $plugin_data ) {
		 ...
		 * @returns bool
		 */
		if ( ! apply_filters( 'fincommerce_show_autoinstalled_plugin_notices', '__return_true' ) ) {
			return;
		}

		$auto_installed_plugins_info = get_site_option( 'fincommerce_autoinstalled_plugins', array() );
		...
		$installed_by = $current_plugin_info['metadata']['installed_by'] ?? 'FinCommerce';
		...
		if ( $info_link ) {
			...
			$message = sprintf( __( 'Plugin installed by %1$s on %2$s. <a target="_blank" href="%3$s">More information</a>', 'fincommerce' ), $installed_by, $current_plugin_info['date'], $info_link );
		} else {
			/* translators: 1 = who installed the plugin, 2 = ISO-formatted date and time */
			$message = sprintf( __( 'Plugin installed by %1$s on %2$s.', 'fincommerce' ), $installed_by, $current_plugin_info['date'] );
		}
	...
	public function handle_upgrader_process_complete( \WP_Upgrader $upgrader, array $hook_extra ) {
		...
		$auto_installed_plugins = get_site_option( 'fincommerce_autoinstalled_plugins' );
		...
		if ( empty( $new_auto_installed_plugins ) ) {
			delete_site_option( 'fincommerce_autoinstalled_plugins' );
		} else {
			update_site_option( 'fincommerce_autoinstalled_plugins', $new_auto_installed_plugins );
		}
	}
}
