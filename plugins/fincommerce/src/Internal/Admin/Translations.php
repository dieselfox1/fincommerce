<?php
/**
 * Register the scripts, and handles items needed for managing translations within FinCommerce Admin.
 */
...
class Translations {
	 ...
	 * @var string
	 */
	private static $plugin_domain = 'fincommerce';
	...
	public function __construct() {
		...
		// Handler for FinCommerce and FinCommerce Admin plugin activation.
		add_action( 'fincommerce_activated_plugin', array( $this, 'potentially_generate_translation_strings' ) );
	...
	private function combine_user_translation_chunks( $json_i18n_filenames ) {
		...
		foreach ( $json_i18n_filenames as $json_filename ) {
			...
			if ( empty( $combined_translation_data ) ) {
				...
				// Combine all messages from all chunk files.
				$combined_translation_data['locale_data']['fincommerce'] = array_merge(
					$combined_translation_data['locale_data']['fincommerce'],
					$chunk_data['locale_data']['fincommerce']
				);
			}
		}
	...
	/**
	 * Run when plugin is activated (can be FinCommerce or FinCommerce Admin).
	 *
