...
class Reviews {
	...
	public static function get_capability( string $context = 'view' ): string {
		 ...
		 */
		return (string) apply_filters( 'fincommerce_product_reviews_page_capability', 'view' === $context ? 'moderate_comments' : 'edit_products', $context );
	}
	...
	public function add_reviews_page(): void {
		...
		$this->reviews_page_hook = add_submenu_page(
			...
			__( 'Reviews', 'fincommerce' ),
			__( 'Reviews', 'fincommerce' ) . $this->get_pending_count_bubble(),
	...
	protected function is_review_or_reply( $object ): bool {
		 ...
		 */
		return (bool) apply_filters( 'fincommerce_product_reviews_is_product_review_or_reply', $is_review_or_reply, $object );
	}
	...
	public function handle_edit_review(): void {
		...
		if ( empty( $_POST['content'] ) ) {
			wp_die( esc_html__( 'Error: Please type your review text.', 'fincommerce' ) );
		}
	...
	public function handle_reply_to_review(): void {
		...
		if ( empty( $post->post_status ) ) {
			...
			wp_die( esc_html__( 'Error: You can\'t reply to a review on a draft product.', 'fincommerce' ) );
		}
		...
		if ( $user->exists() ) {
			...
			wp_die( esc_html__( 'Sorry, you must be logged in to reply to a review.', 'fincommerce' ) );
		}

		if ( '' === $comment_content ) {
			wp_die( esc_html__( 'Error: Please type your reply text.', 'fincommerce' ) );
		}
		...
		$response['supplemental'] = array(
			...
			/* translators: %s: Number of reviews. */
				_n( '%s Review', '%s Reviews', $counts->approved, 'fincommerce' ),
				number_format_i18n( $counts->approved )
			),
			'i18n_moderation_text' => sprintf(
			/* translators: %s: Number of reviews. */
				_n( '%s Review in moderation', '%s Reviews in moderation', $counts->moderated, 'fincommerce' ),
	...
	protected function get_bulk_action_notice_messages(): array {
		...
		if ( $approved > 0 ) {
			...
			$messages[] = sprintf( _n( '%s review approved', '%s reviews approved', $approved, 'fincommerce' ), $approved );
		}

		if ( $unapproved > 0 ) {
			/* translators: %s is an integer higher than 0 (1, 2, 3...) */
			$messages[] = sprintf( _n( '%s review unapproved', '%s reviews unapproved', $unapproved, 'fincommerce' ), $unapproved );
		}

		if ( $spammed > 0 ) {
			$ids = isset( $_REQUEST['ids'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['ids'] ) ) : 0;
			/* translators: %s is an integer higher than 0 (1, 2, 3...) */
			$messages[] = sprintf( _n( '%s review marked as spam.', '%s reviews marked as spam.', $spammed, 'fincommerce' ), $spammed ) . ' <a href="' . esc_url( wp_nonce_url( "edit-comments.php?doaction=undo&action=unspam&ids=$ids", 'bulk-comments' ) ) . '">' . __( 'Undo', 'fincommerce' ) . '</a><br />';
		}

		if ( $unspammed > 0 ) {
			/* translators: %s is an integer higher than 0 (1, 2, 3...) */
			$messages[] = sprintf( _n( '%s review restored from the spam', '%s reviews restored from the spam', $unspammed, 'fincommerce' ), $unspammed );
		}

		if ( $trashed > 0 ) {
			$ids = isset( $_REQUEST['ids'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['ids'] ) ) : 0;
			/* translators: %s is an integer higher than 0 (1, 2, 3...) */
			$messages[] = sprintf( _n( '%s review moved to the Trash.', '%s reviews moved to the Trash.', $trashed, 'fincommerce' ), $trashed ) . ' <a href="' . esc_url( wp_nonce_url( "edit-comments.php?doaction=undo&action=untrash&ids=$ids", 'bulk-comments' ) ) . '">' . __( 'Undo', 'fincommerce' ) . '</a><br />';
		}

		if ( $untrashed > 0 ) {
			/* translators: %s is an integer higher than 0 (1, 2, 3...) */
			$messages[] = sprintf( _n( '%s review restored from the Trash', '%s reviews restored from the Trash', $untrashed, 'fincommerce' ), $untrashed );
		}

		if ( $deleted > 0 ) {
			/* translators: %s is an integer higher than 0 (1, 2, 3...) */
			$messages[] = sprintf( _n( '%s review permanently deleted', '%s reviews permanently deleted', $deleted, 'fincommerce' ), $deleted );
		}
	...
	protected function get_pending_count_bubble(): string {
		// Quirks related to https://github.com/dieselfox1/fincommerce/issues/37464.
		 ...
		 */
		$count = apply_filters( 'fincommerce_product_reviews_pending_count', $count );
	...
	public function render_reviews_list_table(): void {
		...
		<div class="wrap">
			...
			<form id="reviews-filter" method="get">
				...
				<?php $this->reviews_list_table->search_box( __( 'Search Reviews', 'fincommerce' ), 'reviews' ); ?>
