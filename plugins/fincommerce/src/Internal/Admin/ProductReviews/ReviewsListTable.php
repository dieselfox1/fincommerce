...
namespace Automattic\FinCommerce\Internal\Admin\ProductReviews;
...
class ReviewsListTable extends WP_List_Table {
	...
	public function prepare_items() : void {
		 ...
		 */
		$args     = (array) apply_filters( 'fincommerce_product_reviews_list_table_prepare_items_args', $args );
	...
	protected function handle_row_actions( $item, $column_name, $primary ) : string {
		...
		if ( $comment_status && 'all' !== $comment_status ) {
			if ( 'approved' === $review_status ) {
				$actions['unapprove'] = sprintf(
					...
					esc_attr__( 'Unapprove this review', 'fincommerce' ),
					esc_html__( 'Unapprove', 'fincommerce' )
				);
				...
				$actions['approve'] = sprintf(
					...
					esc_attr__( 'Approve this review', 'fincommerce' ),
					esc_html__( 'Approve', 'fincommerce' )
				);
			}
		} else {
			$actions['approve'] = sprintf(
		...
		if ( 'spam' !== $review_status ) {
			$actions['spam'] = sprintf(
				...
				esc_attr__( 'Mark this review as spam', 'fincommerce' ),
				/* translators: "Mark as spam" link. */
				esc_html_x( 'Spam', 'verb', 'fincommerce' )
			);
		} else {
			$actions['unspam'] = sprintf(
				...
				esc_attr__( 'Restore this review from the spam', 'fincommerce' ),
				esc_html_x( 'Not Spam', 'review', 'fincommerce' )
			);
		}
		...
		if ( 'trash' === $review_status ) {
			$actions['untrash'] = sprintf(
				...
				esc_attr__( 'Restore this review from the Trash', 'fincommerce' ),
				esc_html__( 'Restore', 'fincommerce' )
			);
		}
		...
		if ( 'spam' === $review_status || 'trash' === $review_status || ! EMPTY_TRASH_DAYS ) {
			$actions['delete'] = sprintf(
				...
				esc_attr__( 'Delete this review permanently', 'fincommerce' ),
				esc_html__( 'Delete Permanently', 'fincommerce' )
			);
		} else {
			$actions['trash'] = sprintf(
				...
				esc_attr__( 'Move this review to the Trash', 'fincommerce' ),
				esc_html_x( 'Trash', 'verb', 'fincommerce' )
			);
		}
		...
		if ( 'spam' !== $review_status && 'trash' !== $review_status ) {
			$actions['edit'] = sprintf(
				...
				esc_attr__( 'Edit this review', 'fincommerce' ),
				esc_html__( 'Edit', 'fincommerce' )
			);
			...
			$actions['quickedit'] = sprintf(
				...
				esc_attr__( 'Quick edit this review inline', 'fincommerce' ),
				esc_html__( 'Quick Edit', 'fincommerce' )
			);
			...
			$actions['reply'] = sprintf(
				...
				esc_attr__( 'Reply to this review', 'fincommerce' ),
				esc_html__( 'Reply', 'fincommerce' )
			);
		}
		...
		$output .= '</div>';
		$output .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . esc_html__( 'Show more details', 'fincommerce' ) . '</span></button>';
	...
	public function get_columns() : array {
		 ...
		 * @param array $columns
		 */
		return (array) apply_filters( 'fincommerce_product_reviews_table_columns', $columns );
	}
	...
	protected function get_bulk_actions() : array {
		...
		if ( in_array( $comment_status, [ 'all', 'approved' ], true ) ) {
			$actions['unapprove'] = __( 'Unapprove', 'fincommerce' );
		}

		if ( in_array( $comment_status, [ 'all', 'moderated' ], true ) ) {
			$actions['approve'] = __( 'Approve', 'fincommerce' );
		}

		if ( in_array( $comment_status, [ 'all', 'moderated', 'approved', 'trash' ], true ) ) {
			$actions['spam'] = _x( 'Mark as spam', 'review', 'fincommerce' );
		}

		if ( 'trash' === $comment_status ) {
			$actions['untrash'] = __( 'Restore', 'fincommerce' );
		} elseif ( 'spam' === $comment_status ) {
			$actions['unspam'] = _x( 'Not spam', 'review', 'fincommerce' );
		}

		if ( in_array( $comment_status, [ 'trash', 'spam' ], true ) || ! EMPTY_TRASH_DAYS ) {
			$actions['delete'] = __( 'Delete permanently', 'fincommerce' );
		} else {
			$actions['trash'] = __( 'Move to Trash', 'fincommerce' );
		}
	...
	public function no_items() : void {
		...
		if ( 'moderated' === $comment_status ) {
			esc_html_e( 'No reviews awaiting moderation.', 'fincommerce' );
		} else {
			esc_html_e( 'No reviews found.', 'fincommerce' );
		}
	}
	...
	protected function column_cb( $item ) : void {
		...
		if ( $this->current_user_can_edit_review ) {
			?>
			<label class="screen-reader-text" for="cb-select-<?php echo esc_attr( $item->comment_ID ); ?>"><?php esc_html_e( 'Select review', 'fincommerce' ); ?></label>
			<input
	...
	private function get_in_reply_to_review_text( $reply ) : string {
		...
		return sprintf(
			...
			ent2ncr( __( 'In reply to %s.', 'fincommerce' ) ),
	...
	protected function filter_column_output( $column_name, $output, $item ) : string {
		 ...
		 */
		return (string) apply_filters( 'fincommerce_product_reviews_table_column_' . $column_name . '_content', $output, $item );
	}
	...
	protected function extra_tablenav( $which ) : void {
		...
		if ( ( 'spam' === $comment_status || 'trash' === $comment_status ) && $this->has_items() && $this->current_user_can_moderate_reviews ) {
			...
			$title = 'spam' === $comment_status
				? esc_attr__( 'Empty Spam', 'fincommerce' )
				: esc_attr__( 'Empty Trash', 'fincommerce' );
	...
	protected function review_type_dropdown( $current_type ) : void {
		...
		$item_types = apply_filters(
			'fincommerce_product_reviews_list_table_item_types',
			array(
				'all'     => __( 'All types', 'fincommerce' ),
				'comment' => __( 'Replies', 'fincommerce' ),
				'review'  => __( 'Reviews', 'fincommerce' ),
			)
		);

		?>
		<label class="screen-reader-text" for="filter-by-review-type"><?php esc_html_e( 'Filter by review type', 'fincommerce' ); ?></label>
	...
	public function review_rating_dropdown( $current_rating ) : void {
		...
		?>
		<label class="screen-reader-text" for="filter-by-review-rating"><?php esc_html_e( 'Filter by review rating', 'fincommerce' ); ?></label>
	...
	protected function product_search( ?WC_Product $current_product ) : void {
		?>
		<label class="screen-reader-text" for="filter-by-product"><?php esc_html_e( 'Filter by product', 'fincommerce' ); ?></label>
		<select
			id="filter-by-product"
			class="wc-product-search"
			name="product_id"
			style="width: 200px;"
			data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'fincommerce' ); ?>"
			data-action="fincommerce_json_search_products"
	...
	protected function comments_bubble( $post_id, $pending_comments ) : void {
		...
		$approved_only_phrase = sprintf(
			/* translators: %s: Number of reviews. */
			_n( '%s review', '%s reviews', $approved_review_count, 'fincommerce' ),
			$approved_reviews_number
		);

		$approved_phrase = sprintf(
			/* translators: %s: Number of reviews. */
			_n( '%s approved review', '%s approved reviews', $approved_review_count, 'fincommerce' ),
			$approved_reviews_number
		);

		$pending_phrase = sprintf(
			/* translators: %s: Number of reviews. */
			_n( '%s pending review', '%s pending reviews', $pending_comments, 'fincommerce' ),
		...
		if ( ! $approved_review_count && ! $pending_comments ) {
			...
			printf(
				...
				esc_html__( 'No reviews', 'fincommerce' )
			);
			...
			printf(
				...
				esc_html( $approved_reviews_number ),
				$pending_comments ? esc_html__( 'No approved reviews', 'fincommerce' ) : esc_html__( 'No reviews', 'fincommerce' )
			);
		}
		...
		if ( $pending_comments ) {
			...
			printf(
				...
				esc_html( $pending_reviews_number ),
				$approved_review_count ? esc_html__( 'No pending reviews', 'fincommerce' ) : esc_html__( 'No reviews', 'fincommerce' )
			);
		}
	}
