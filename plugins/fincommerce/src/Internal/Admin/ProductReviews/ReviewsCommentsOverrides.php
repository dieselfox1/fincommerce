...
class ReviewsCommentsOverrides {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_dismiss_admin_notice_capability', array( $this, 'get_dismiss_capability' ), 10, 2 );
	...
	protected function display_reviews_moved_notice() : void {
		...
		<div class="notice notice-info is-dismissible">
			<p><strong><?php esc_html_e( 'Product reviews have moved!', 'fincommerce' ); ?></strong></p>
			<p><?php esc_html_e( 'Product reviews can now be managed from Products > Reviews.', 'fincommerce' ); ?></p>
			<p class="submit">
				<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=product&page=product-reviews' ) ); ?>" class="button-primary"><?php esc_html_e( 'Visit new location', 'fincommerce' ); ?></a>
			</p>
			...
			<form action="<?php echo esc_url( admin_url( 'edit-comments.php' ) ); ?>" method="get">
				...
				<?php wp_nonce_field( 'fincommerce_hide_notices_nonce', '_wc_notice_nonce' ); ?>

				<button type="submit" class="notice-dismiss">
					<span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'fincommerce' ); ?></span>
				</button>
			</form>
		</div>
		<?php
	 ...
	 *
	 * This is required so that users who do not have the manage_fincommerce capability (e.g. Editors) can still dismiss
