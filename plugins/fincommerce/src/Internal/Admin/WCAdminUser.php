...
class WCAdminUser {
	...
	public function update_user_data_values( $values, $user, $field_id ) {
		if ( empty( $values ) || ! is_array( $values ) || 'fincommerce_meta' !== $field_id ) {
			return;
		}
	...
	/**
	 * We store some FinCommerce specific user meta attached to users endpoint,
	...
	public function get_user_data_fields() {
		 ...
		 */
		return apply_filters( 'fincommerce_admin_get_user_data_fields', array( 'variable_product_tour_shown' ) );
	}
	...
	public static function update_user_data_field( $user_id, $field, $value ) {
		update_user_meta( $user_id, 'fincommerce_admin_' . $field, $value );
	}
	...
	public static function get_user_data_field( $user_id, $field ) {
		$meta_value = get_user_meta( $user_id, 'fincommerce_admin_' . $field, true );

		// Migrate old meta values (prefix changed from `wc_admin_` to `fincommerce_admin_`).
		if ( '' === $meta_value ) {
