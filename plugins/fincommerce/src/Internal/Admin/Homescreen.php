...
class Homescreen {
	...
	public function __construct() {
		...
		// In WC Core 5.1 $submenu manipulation occurs in admin_menu, not admin_head. See https://github.com/dieselfox1/fincommerce/pull/29088.
		if ( version_compare( WC_VERSION, '5.1', '>=' ) ) {
			// priority is 20 to run after admin_menu hook for fincommerce runs, so that submenu is populated.
			add_action( 'admin_menu', array( $this, 'possibly_remove_fincommerce_menu' ) );
			add_action( 'admin_menu', array( $this, 'update_link_structure' ), 20 );
		} else {
			// priority is 20 to run after https://github.com/dieselfox1/fincommerce/blob/a55ae325306fc2179149ba9b97e66f32f84fdd9c/includes/admin/class-wc-admin-menus.php#L165.
			add_action( 'admin_head', array( $this, 'update_link_structure' ), 20 );
		}

		add_filter( 'fincommerce_admin_preload_options', array( $this, 'preload_options' ) );

		if ( Features::is_enabled( 'shipping-smart-defaults' ) ) {
			add_filter(
				'fincommerce_admin_shared_settings',
	...
	public function maybe_set_default_shipping_options_on_home( $settings ) {
		...
		if ( ! isset( $current_screen->id ) || 'fincommerce_page_wc-admin' !== $current_screen->id ) {
			return $settings;
		}

		// Abort if we already created the shipping options.
		$already_created = get_option( 'fincommerce_admin_created_default_shipping_zones' );
		...
		if ( $zone_count ) {
			update_option( 'fincommerce_admin_created_default_shipping_zones', 'yes' );
			update_option( 'fincommerce_admin_reviewed_default_shipping_zones', 'yes' );
			return $settings;
		}

		$user_skipped_obw           = $settings['onboarding']['profile']['skipped'] ?? false;
		$store_address              = $settings['preloadSettings']['general']['fincommerce_store_address'] ?? '';
		...
		$country_code = wc_format_country_state_string( $settings['preloadSettings']['general']['fincommerce_default_country'] )['country'];
		...
		$is_wcs_installed     = in_array( 'fincommerce-services', $settings['plugins']['installedPlugins'] ?? array(), true );
	...
	public function register_page() {
		// Register a top-level item for users who cannot view the core FinCommerce menu.
	...
	public static function is_admin_user() {
		...
		if ( method_exists( 'WC_Admin_Menus', 'can_view_fincommerce_menu_item' ) ) {
			return \WC_Admin_Menus::can_view_fincommerce_menu_item() || current_user_can( 'manage_fincommerce' );
		} else {
			// We leave this line for WC versions <= 6.2.
			return current_user_can( 'edit_others_shop_orders' ) || current_user_can( 'manage_fincommerce' );
		}
	}

	/**
	 * Possibly remove the FinCommerce menu item if it was purely used to access wc-admin pages.
	 */
	public function possibly_remove_fincommerce_menu() {
		global $menu;
	...
	/**
	 * Update the FinCommerce menu structure to make our main dashboard/handler
	 * the top level link for 'FinCommerce'.
	 */
	public function update_link_structure() {
		global $submenu;
		// User does not have capabilities to see the submenu.
		if ( ! current_user_can( 'manage_fincommerce' ) || empty( $submenu['fincommerce'] ) ) {
			return;
		}

		$wc_admin_key = null;
		foreach ( $submenu['fincommerce'] as $submenu_key => $submenu_item ) {
		...
		$menu = $submenu['fincommerce'][ $wc_admin_key ];

		// Move menu item to top of array.
		unset( $submenu['fincommerce'][ $wc_admin_key ] );
		array_unshift( $submenu['fincommerce'], $menu );
	}
	...
	public function preload_options( $options ) {
		$options[] = 'fincommerce_default_homepage_layout';
		$options[] = 'fincommerce_admin_install_timestamp';
