...
namespace Automattic\FinCommerce\Internal\Admin\ImportExport;
...
class CSVUploadHelper {
	...
	public function handle_csv_upload( string $import_type, string $files_index = 'import', ?array $allowed_mime_types = null ): array {
		...
		remove_filter( 'wp_check_filetype_and_ext', array( $this, 'filter_fincommerce_check_filetype_for_csv' ), 10 );
		...
		if ( ! wc_is_file_valid_csv( $upload['file'], false ) ) {
			...
			throw new \Exception( esc_html__( 'Invalid file type for a CSV import.', 'fincommerce' ) );
		}
	 ...
	 */
	public function filter_fincommerce_check_filetype_for_csv( $data, $file, $filename, $mimes, $real_mime ) {
