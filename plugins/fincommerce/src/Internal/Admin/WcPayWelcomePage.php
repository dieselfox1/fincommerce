...
use Automattic\FinCommerce\Internal\Admin\Suggestions\PaymentsExtensionSuggestionIncentives;
 ...
 *
 * @deprecated 9.9.0 The WooPayments welcome page is deprecated and will be removed in a future version of FinCommerce.
 */
class WcPayWelcomePage {
	/**
	...
	public function has_incentive( bool $skip_wcpay_active = false ): bool {
		...
		if ( get_option( 'fincommerce_show_marketplace_suggestions', 'yes' ) === 'no' ) {
			return false;
		}
		 ...
		 * @since 3.6.0
		 */
		if ( ! apply_filters( 'fincommerce_allow_marketplace_suggestions', true ) ) {
			return false;
		}
	 ...
	 *
	 * It can be either 'fincommerce-payments' or 'payments'.
	 *
	 * @return string Either 'fincommerce-payments' or 'payments'. Empty string if no task is found.
	 */
	private function get_active_payments_task_slug(): string {
		...
		if ( ! empty( $setup_task_list ) && $setup_task_list->is_visible() ) {
			$payments_task = $setup_task_list->get_task( 'fincommerce-payments' );
			if ( ! empty( $payments_task ) && $payments_task->can_view() ) {
	...
	/**
	 * Get the FinCommerce setup task list Payments task instance.
	 *
	...
	/**
	 * Determine if the FinCommerce setup task list Payments task is complete.
	 *
