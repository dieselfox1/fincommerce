...
namespace Automattic\FinCommerce\Internal\Admin\EmailPreview;

use Automattic\FinCommerce\Internal\EmailEditor\WooContentProcessor;
use Throwable;
use WC_Email;
use WC_Order;
use WC_Product;
...
class EmailPreview {
	...
	const TRANSIENT_PREVIEW_EMAIL_IMPROVEMENTS = 'fincommerce_preview_email_improvements';
	...
	private static array $email_style_setting_ids = array(
		'fincommerce_email_background_color',
		'fincommerce_email_base_color',
		'fincommerce_email_body_background_color',
		'fincommerce_email_font_family',
		'fincommerce_email_footer_text',
		'fincommerce_email_footer_text_color',
		'fincommerce_email_header_alignment',
		'fincommerce_email_header_image',
		'fincommerce_email_header_image_width',
		'fincommerce_email_text_color',
	);
	...
	public static function get_email_style_setting_ids() {
		 ...
		 * @since 9.8.0
		 */
		return apply_filters( 'fincommerce_email_preview_email_style_setting_ids', self::$email_style_setting_ids );
	}
	...
	public static function get_email_content_setting_ids( ?string $email_id ) {
		...
		$setting_ids = array(
			"fincommerce_{$email_id}_subject",
			"fincommerce_{$email_id}_heading",
			"fincommerce_{$email_id}_additional_content",
			"fincommerce_{$email_id}_email_type",
		);
		 ...
		 * @since 9.8.0
		 */
		return apply_filters( 'fincommerce_email_preview_email_content_setting_ids', $setting_ids, $email_id );
	}
	...
	public function set_email_type( string $email_type ) {
		 ...
		 * @since 9.6.0
		 */
		$this->email = apply_filters( 'fincommerce_prepare_email_for_preview', $this->email );
	...
	/**
	 * Ensure links open in new tab. User in FinCommerce Settings,
	...
	private function render_preview_email() {
		...
		return apply_filters( 'fincommerce_mail_content', $inlined ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
	}
	...
	private function get_dummy_order() {
		...
		$order->set_total( 65 );
		$order->set_payment_method_title( __( 'Direct bank transfer', 'fincommerce' ) );
		$order->set_transaction_id( '999999999' );
		$order->set_customer_note( __( "This is a customer note. Customers can add a note to their order on checkout.\n\nIt can be multiple lines. If there's no note, this section is hidden.", 'fincommerce' ) );

		// Add shipping method.
		$shipping_item = new WC_Order_Item_Shipping();
		$shipping_item->set_props(
			array(
				'method_title' => __( 'Flat rate', 'fincommerce' ),
				'method_id'    => 'flat_rate',
				'total'        => '5.00',
			)
		);
		 ...
		 * @since 9.6.0
		 */
		return apply_filters( 'fincommerce_email_preview_dummy_order', $order, $this->email_type );
	}

	/**
	 * Get a dummy product. Also used with `fincommerce_order_item_product` filter
	...
	private function get_dummy_product() {
		...
		$product->set_name( __( 'Dummy Product', 'fincommerce' ) );
		 ...
		 * @since 9.6.0
		 */
		return apply_filters( 'fincommerce_email_preview_dummy_product', $product, $this->email_type );
	}
	...
	private function get_dummy_product_variation() {
		...
		$variation->set_name( __( 'Dummy Product Variation', 'fincommerce' ) );
		$variation->set_price( 20 );
		$variation->set_attributes(
			array(
				__( 'Color', 'fincommerce' ) => __( 'Red', 'fincommerce' ),
				__( 'Size', 'fincommerce' )  => __( 'Small', 'fincommerce' ),
			)
		);
		 ...
		 * @since 9.7.0
		 */
		return apply_filters( 'fincommerce_email_preview_dummy_product_variation', $variation, $this->email_type );
	}
	...
	private function get_dummy_address() {
		 ...
		 * @since 9.6.0
		 */
		return apply_filters( 'fincommerce_email_preview_dummy_address', $address, $this->email_type );
	}
	...
	private function get_placeholders( $email_object ) {
		 ...
		 * @since 9.6.0
		 */
		return apply_filters( 'fincommerce_email_preview_placeholders', $placeholders, $this->email_type, $email_object );
	}
	...
	public function set_up_filters() {
		...
		add_filter( 'fincommerce_order_needs_shipping_address', array( $this, 'enable_shipping_address' ) );
		...
		add_filter( 'fincommerce_order_item_product', array( $this, 'get_dummy_product_when_not_set' ), 10, 1 );
		// Enable email preview mode - this way transient values are fetched for live preview.
		add_filter( 'fincommerce_is_email_preview', array( $this, 'enable_preview_mode' ) );
		// Use placeholder image included in FinCommerce files.
		add_filter( 'fincommerce_order_item_thumbnail', array( $this, 'get_placeholder_image' ) );
	}
	...
	public function clean_up_filters() {
		remove_filter( 'fincommerce_order_needs_shipping_address', array( $this, 'enable_shipping_address' ) );
		remove_filter( 'fincommerce_order_item_product', array( $this, 'get_dummy_product_when_not_set' ), 10 );
		remove_filter( 'fincommerce_is_email_preview', array( $this, 'enable_preview_mode' ) );
		remove_filter( 'fincommerce_order_item_thumbnail', array( $this, 'get_placeholder_image' ) );
		$this->restore_locale();
	}
	...
	public function generate_placeholder_content( string $email_type_class_name ): string {
		...
		$generate_content_closure = function () use ( $woo_content_processor ) {
			// Note: If 'fincommerce_email_styles' filter was intentional and `prepare_css` isn't
			// the intended callback, adjust accordingly. This assumes `prepare_css` applies styles
			// needed for the Woo content block.
			add_filter( 'fincommerce_email_styles', array( $woo_content_processor, 'prepare_css' ), 10, 2 );
		...
		try {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				...
				try {
					...
					throw new \RuntimeException( esc_html__( 'There was an error rendering the email editor placeholder content.', 'fincommerce' ), 0, $e );
				}
				ob_end_clean();
			}
		} finally {
