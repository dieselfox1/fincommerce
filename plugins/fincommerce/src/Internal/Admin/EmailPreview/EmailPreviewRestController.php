...
class EmailPreviewRestController extends RestApiControllerBase {
	...
	private function get_args_for_send_preview() {
		return array(
			'type'  => array(
				'description'       => __( 'The email type to preview.', 'fincommerce' ),
				'type'              => 'string',
				'required'          => true,
			...
			'email' => array(
				'description'       => __( 'Email address to send the email preview to.', 'fincommerce' ),
				'type'              => 'string',
				'format'            => 'email',
				'required'          => true,
	...
	private function get_args_for_preview_subject() {
		return array(
			'type' => array(
				'description'       => __( 'The email type to get subject for.', 'fincommerce' ),
				'type'              => 'string',
				'required'          => true,
				'validate_callback' => fn( $key ) => $this->validate_email_type( $key ),
				'sanitize_callback' => 'sanitize_text_field',
		...
		return array(
			'key'   => array(
				...
				'validate_callback' => function ( $key ) {
					if ( ! in_array( $key, EmailPreview::get_all_email_setting_ids(), true ) ) {
						return new \WP_Error(
							'fincommerce_rest_not_allowed_key',
			...
			'value' => array(
				...
				'sanitize_callback' => function ( $value, $request ) {
					...
					if (
						'fincommerce_email_footer_text' === $key
	...
	private function get_schema_for_preview_subject() {
		return array(
			...
			'properties' => array(
				'subject' => array(
					'description' => __( 'A subject for provided email type after filters are applied and placeholders replaced.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
	...
	private function validate_email_type( string $email_type ) {
		try {
			...
			return new WP_Error(
				'fincommerce_rest_invalid_email_type',
				__( 'Invalid email type.', 'fincommerce' ),
				array( 'status' => 400 ),
			);
		}
		return true;
	...
	private function check_permissions( WP_REST_Request $request ) {
		...
		if ( ! wp_verify_nonce( $nonce, self::NONCE_KEY ) ) {
			return new WP_Error(
				'invalid_nonce',
				__( 'Invalid nonce.', 'fincommerce' ),
				array( 'status' => 403 ),
			);
		}
		return $this->check_permission( $request, 'manage_fincommerce' );
	}
	...
	public function send_email_preview( WP_REST_Request $request ) {
		...
		try {
			...
			ob_end_clean();
			return new WP_Error(
				'fincommerce_rest_email_preview_not_rendered',
				__( 'There was an error rendering an email preview.', 'fincommerce' ),
				array( 'status' => 500 )
			);
		}
		ob_end_clean();
		...
		if ( $sent ) {
			return array(
				...
				'message' => sprintf( __( 'Test email sent to %s.', 'fincommerce' ), $email_address ),
			);
		}
		return new WP_Error(
			'fincommerce_rest_email_preview_not_sent',
			__( 'Error sending test email. Please try again.', 'fincommerce' ),
			array( 'status' => 500 )
		);
	}
	...
	public function save_transient( WP_REST_Request $request ) {
		...
		if ( ! $is_set ) {
			return new WP_Error(
				'fincommerce_rest_transient_not_set',
				__( 'Error saving transient. Please try again.', 'fincommerce' ),
				array( 'status' => 500 )
			);
		}
		return array(
			// translators: %s: Email settings color key, e.g., "fincommerce_email_base_color".
			'message' => sprintf( __( 'Transient saved for key %s.', 'fincommerce' ), $key ),
		);
	}
}
