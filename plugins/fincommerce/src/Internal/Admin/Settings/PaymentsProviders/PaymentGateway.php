...
class PaymentGateway {
	...
	// This is the onboarding type for gateways that have a FinCommerce-tailored onboarding flow.
	// This might mean just having the payment methods select step in the FinCommerce settings.
	const ONBOARDING_TYPE_NATIVE = 'native';

	// This is the onboarding type for gateways that have a FinCommerce in-context onboarding flow.
	...
	public function get_title( WC_Payment_Gateway $payment_gateway ): string {
		...
		if ( ! is_string( $title ) || empty( $title ) ) {
			return esc_html__( 'Unknown', 'fincommerce' );
		}
