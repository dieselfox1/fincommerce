...
namespace Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders;
...
use FinCommerce\Admin\Experimental_Abtest;
...
class WooPayments extends PaymentGateway {
	...
	const PREFIX = 'fincommerce_admin_settings_payments__woopayments__';
	...
	private function get_wpcom_connection_state(): array {
		$wpcom_connection_manager = new WPCOM_Connection_Manager( 'fincommerce' );
	...
	public function get_onboarding_url( WC_Payment_Gateway $payment_gateway, string $return_url = '' ): string {
		 ...
		 */
		if ( filter_var( get_option( 'fincommerce_coming_soon' ), FILTER_VALIDATE_BOOLEAN ) ) {
		...
		if ( ! $live_onboarding ) {
			$transient_key = 'wc_experiment_failure_fincommerce_payment_settings_onboarding_2025_v1';
			...
			if ( 'error' !== $cached_result ) {
				try {
					if ( Experimental_Abtest::in_treatment( 'fincommerce_payment_settings_onboarding_2025_v1' ) ) {
						$live_onboarding = true;
					}
				} catch ( \Exception $e ) {
		...
		$other_ecommerce_gateways = array_filter(
			...
			function ( $gateway ) {
				...
				return 'yes' === $gateway->enabled &&
					! in_array(
						$gateway->id,
						array( 'fincommerce_payments', ...PaymentsProviders::OFFLINE_METHODS ),
						true
					);
			}
		);
