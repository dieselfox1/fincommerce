...
class Mollie extends PaymentGateway {
	...
	public function is_account_connected( WC_Payment_Gateway $payment_gateway ): bool {
		try {
			...
			if ( true === $sandbox_mode ) {
				...
				return ! empty( get_option( 'mollie-payments-for-fincommerce_test_api_key', '' ) );
			} elseif ( false === $sandbox_mode ) {
				// In production mode, we check the live API key.
				return ! empty( get_option( 'mollie-payments-for-fincommerce_live_api_key', '' ) );
			}
		} catch ( Throwable $e ) {
	...
	private function is_mollie_in_sandbox_mode( WC_Payment_Gateway $payment_gateway ): ?bool {
		try {
			...
			return filter_var( get_option( 'mollie-payments-for-fincommerce_test_mode_enabled', 'yes' ), FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE );
		} catch ( \Throwable $e ) {
			// Do nothing but log so we can investigate.
			SafeGlobalFunctionProxy::wc_get_logger()->debug(
