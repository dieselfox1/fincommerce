...
class PayPal extends PaymentGateway {
	...
	private function is_paypal_in_sandbox_mode( WC_Payment_Gateway $payment_gateway ): ?bool {
		if ( class_exists( '\FinCommerce\PayPalCommerce\PPCP' ) &&
			is_callable( '\FinCommerce\PayPalCommerce\PPCP::container' ) ) {
			try {
				$container = \FinCommerce\PayPalCommerce\PPCP::container();
				...
				if ( $container->has( 'onboarding.environment' ) &&
					defined( '\FinCommerce\PayPalCommerce\Onboarding\Environment::SANDBOX' ) ) {
					$environment         = $container->get( 'onboarding.environment' );
					$current_environment = $environment->current_environment();

					return \FinCommerce\PayPalCommerce\Onboarding\Environment::SANDBOX === $current_environment;
				}
			} catch ( \Throwable $e ) {
		...
		if ( class_exists( '\FinCommerce\PayPalCommerce\PPCP' ) &&
			...
			try {
				...
				if ( $container->has( 'onboarding.state' ) &&
					defined( '\FinCommerce\PayPalCommerce\Onboarding\State::STATE_ONBOARDED' ) ) {
					$state = $container->get( 'onboarding.state' );

					return $state->current_state() >= \FinCommerce\PayPalCommerce\Onboarding\State::STATE_ONBOARDED;
				}
			} catch ( \Throwable $e ) {
				// Do nothing but log so we can investigate.
				SafeGlobalFunctionProxy::wc_get_logger()->debug(
