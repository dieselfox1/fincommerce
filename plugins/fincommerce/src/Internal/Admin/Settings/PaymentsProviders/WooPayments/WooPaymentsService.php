...
use Automattic\FinCommerce\Internal\Admin\Settings\Exceptions\ApiArgumentException;
use Automattic\FinCommerce\Internal\Admin\Settings\Exceptions\ApiException;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders;
use Automattic\FinCommerce\Internal\Admin\Settings\Utils;
...
class WooPaymentsService {
	...
	const GATEWAY_ID = 'fincommerce_payments';
	...
	const NOX_PROFILE_OPTION_KEY    = 'fincommerce_woopayments_nox_profile';
	const NOX_ONBOARDING_LOCKED_KEY = 'fincommerce_woopayments_nox_onboarding_locked';
	...
	final public function init( PaymentsProviders $payment_providers, LegacyProxy $proxy ): void {
		...
		$this->wpcom_connection_manager = $this->proxy->get_instance_of( WPCOM_Connection_Manager::class, 'fincommerce' );
	...
	public function get_onboarding_step_status( string $step_id, string $location ): string {
		if ( ! $this->is_valid_onboarding_step_id( $step_id ) ) {
			throw new ApiArgumentException(
				'fincommerce_woopayments_onboarding_invalid_step_id',
				esc_html__( 'Invalid onboarding step ID.', 'fincommerce' ),
	...
	public function onboarding_step_save( string $step_id, string $location, array $request_data ): bool {
		...
		if ( ! $this->is_valid_onboarding_step_data( $step_id, $request_data ) ) {
			throw new ApiArgumentException(
				'fincommerce_woopayments_onboarding_invalid_step_data',
				esc_html__( 'Invalid onboarding step data.', 'fincommerce' ),
				(int) WP_Http::BAD_REQUEST
			);
		}
		...
		switch ( $step_id ) {
			...
			default:
				throw new ApiException(
					'fincommerce_woopayments_onboarding_step_action_not_supported',
					esc_html__( 'Save action not supported for the onboarding step ID.', 'fincommerce' ),
	...
	public function onboarding_test_account_init( string $location, ?string $source = self::SESSION_ENTRY_DEFAULT ): array {
		...
		if ( $this->has_test_account() ) {
			throw new ApiException(
				'fincommerce_woopayments_test_account_already_exists',
				esc_html__( 'A test account is already set up.', 'fincommerce' ),
		...
		if ( $this->has_account() ) {
			...
			throw new ApiException(
				'fincommerce_woopayments_onboarding_action_error',
				esc_html__( 'An account is already set up. Reset the onboarding first.', 'fincommerce' ),
				(int) WP_Http::FORBIDDEN
			);
		}
		...
		try {
			...
			$response = new WP_Error(
				'fincommerce_woopayments_onboarding_client_api_exception',
				esc_html__( 'An unexpected error happened while initializing the test account.', 'fincommerce' ),
				array(
					'code'    => $e->getCode(),
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new ApiException(
				'fincommerce_woopayments_onboarding_client_api_error',
		...
		if ( ! is_array( $response ) || empty( $response['success'] ) ) {
			...
			$this->mark_onboarding_step_failed(
				...
				array(
					...
					'message' => esc_html__( 'Received an unexpected response from the platform.', 'fincommerce' ),
					'context' => array(
						'response' => $response,
					),
				)
			...
			throw new ApiException(
				'fincommerce_woopayments_onboarding_client_api_error',
				esc_html__( 'Failed to initialize the test account.', 'fincommerce' ),
				(int) WP_Http::FAILED_DEPENDENCY
			);
		}
	...
	public function get_onboarding_kyc_session( string $location, array $self_assessment = array(), ?string $source = self::SESSION_ENTRY_DEFAULT ): array {
		...
		try {
			...
			$response = new WP_Error(
				'fincommerce_woopayments_onboarding_client_api_exception',
				esc_html__( 'An unexpected error happened while creating the KYC session.', 'fincommerce' ),
				array(
					'code'    => $e->getCode(),
					'message' => $e->getMessage(),
					'trace'   => $e->getTrace(),
		...
		if ( ! is_array( $response ) ) {
			...
			throw new ApiException(
				'fincommerce_woopayments_onboarding_client_api_error',
				esc_html__( 'Failed to get the KYC session data.', 'fincommerce' ),
				(int) WP_Http::FAILED_DEPENDENCY
			);
		}
	...
	public function finish_onboarding_kyc_session( string $location, ?string $source = self::SESSION_ENTRY_DEFAULT ): array {
		...
		try {
			...
			$response = new WP_Error(
				'fincommerce_woopayments_onboarding_client_api_exception',
				esc_html__( 'An unexpected error happened while finalizing the KYC session.', 'fincommerce' ),
				array(
					'code'    => $e->getCode(),
					'message' => $e->getMessage(),
					'trace'   => $e->getTrace(),
		...
		if ( ! is_array( $response ) ) {
			...
			throw new ApiException(
				'fincommerce_woopayments_onboarding_client_api_error',
				esc_html__( 'Failed to finish the KYC session.', 'fincommerce' ),
				(int) WP_Http::FAILED_DEPENDENCY
			);
		}
	...
	public function onboarding_preload(): array {
		...
		if ( $this->is_onboarding_locked() ) {
			throw new ApiException(
				'fincommerce_woopayments_onboarding_locked',
				esc_html__( 'Another onboarding action is already in progress. Please wait for it to finish.', 'fincommerce' ),
	...
	public function reset_onboarding( string $location, string $from = '', ?string $source = self::SESSION_ENTRY_DEFAULT ): array {
		...
		if ( $this->has_account() ) {
			try {
				...
				$response = new WP_Error(
					'fincommerce_woopayments_onboarding_client_api_exception',
					esc_html__( 'An unexpected error happened while resetting onboarding.', 'fincommerce' ),
					array(
						'code'    => $e->getCode(),
						'message' => $e->getMessage(),
						'trace'   => $e->getTrace(),
		...
		if ( ! is_array( $response ) || empty( $response['success'] ) ) {
			throw new ApiException(
				'fincommerce_woopayments_onboarding_client_api_error',
				esc_html__( 'Failed to reset onboarding.', 'fincommerce' ),
				(int) WP_Http::FAILED_DEPENDENCY
			);
		}
	...
	public function disable_test_account( string $location, string $from = '', ?string $source = self::SESSION_ENTRY_DEFAULT ): array {
		...
		if ( $has_test_account ) {
			try {
				...
				$response = new WP_Error(
					'fincommerce_woopayments_onboarding_client_api_exception',
					esc_html__( 'An unexpected error happened while disabling the test account.', 'fincommerce' ),
					array(
						'code'    => $e->getCode(),
						'message' => $e->getMessage(),
						'trace'   => $e->getTrace(),
		...
		if ( ! is_array( $response ) || empty( $response['success'] ) ) {
			throw new ApiException(
				'fincommerce_woopayments_onboarding_client_api_error',
				esc_html__( 'Failed to disable the test account.', 'fincommerce' ),
				(int) WP_Http::FAILED_DEPENDENCY
			);
		}
	 ...
	 * @param string $business_country  The business registration country code as set in the FinCommerce Payments settings.
	 *                                  This is an ISO 3166-1 alpha-2 country code.
	...
	private function check_if_onboarding_action_is_acceptable() {
		...
		if ( ! $this->is_extension_active() ) {
			throw new ApiException(
				'fincommerce_woopayments_onboarding_extension_not_active',
				/* translators: %s: WooPayments. */
				sprintf( esc_html__( 'The %s extension is not active.', 'fincommerce' ), 'WooPayments' ),
				(int) WP_Http::FORBIDDEN
			);
		}
		...
		if ( Constants::is_defined( 'WCPAY_VERSION_NUMBER' ) &&
			...
			throw new ApiException(
				'fincommerce_woopayments_onboarding_extension_version',
				/* translators: %s: WooPayments. */
				sprintf( esc_html__( 'The %s extension is not up-to-date. Please update to the latest version and try again.', 'fincommerce' ), 'WooPayments' ),
				(int) WP_Http::FORBIDDEN
			);
		}
	...
	private function check_if_onboarding_step_action_is_acceptable( string $step_id, string $location ): void {
		...
		if ( ! $this->check_onboarding_step_requirements( $step_id, $location ) ) {
			throw new ApiException(
				'fincommerce_woopayments_onboarding_step_requirements_not_met',
				esc_html__( 'Onboarding step requirements are not met.', 'fincommerce' ),
				(int) WP_Http::FORBIDDEN
			);
		}
		if ( $this->is_onboarding_step_blocked( $step_id, $location ) ) {
			throw new ApiException(
				'fincommerce_woopayments_onboarding_step_blocked',
				esc_html__( 'There are environment or store setup issues which are blocking progress. Please resolve them to proceed.', 'fincommerce' ),
				(int) WP_Http::FORBIDDEN,
				array(
	...
	private function standardize_onboarding_step_details( array $step_details, string $location, string $rest_path ): array {
		...
		if ( ! isset( $step_details['id'] ) ) {
			...
			throw new Exception( sprintf( esc_html__( 'The onboarding step is missing required entries: %s', 'fincommerce' ), 'id' ) );
		}
		// Validate the step ID.
		if ( ! $this->is_valid_onboarding_step_id( $step_details['id'] ) ) {
			/* translators: %s: The invalid step ID. */
			throw new Exception( sprintf( esc_html__( 'The onboarding step ID is invalid: %s', 'fincommerce' ), esc_attr( $step_details['id'] ) ) );
		}
	...
	private function get_onboarding_kyc_fields( string $location ): array {
		...
		if ( ! is_array( $response ) || ! isset( $response['data'] ) ) {
			throw new Exception( esc_html__( 'Failed to get onboarding fields data.', 'fincommerce' ) );
		}
