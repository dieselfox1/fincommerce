...
class Payments {
	...
	const PAYMENTS_NOX_PROFILE_KEY              = 'fincommerce_payments_nox_profile';
	const PAYMENTS_PROVIDER_STATE_SNAPSHOTS_KEY = 'fincommerce_payments_provider_state_snapshots';
	...
	public function get_payment_providers( string $location, bool $for_display = true, bool $remove_shells = false ): array {
		...
		if ( in_array( PaymentsProviders::TYPE_OFFLINE_PM, array_column( $payment_providers, '_type' ), true ) ) {
			...
			$payment_providers[] = array(
				...
				'title'       => esc_html__( 'Take offline payments', 'fincommerce' ),
				'description' => esc_html__( 'Accept payments offline using multiple different methods. These can also be used to test purchases.', 'fincommerce' ),
				'icon'        => plugins_url( 'assets/images/payment_methods/cod.svg', WC_PLUGIN_FILE ),
				// The offline PMs (and their group) are obviously from FinCommerce, and WC is always active.
				'plugin'      => array(
					'_type'  => 'wporg',
					'slug'   => 'fincommerce',
