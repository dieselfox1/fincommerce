...
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Affirm;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\AfterpayClearpay;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Airwallex;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\AmazonPay;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Antom;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\GoCardless;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\HelioPay;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Klarna;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\KlarnaCheckout;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\MercadoPago;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Mollie;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Monei;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Payfast;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\PaymentGateway;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Paymob;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Payoneer;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\PayPal;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Paystack;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Paytrail;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\PayUIndia;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Razorpay;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Stripe;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Tilopay;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\Vivacom;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\WCCore;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\WooPayments;
use Automattic\FinCommerce\Internal\Admin\Settings\PaymentsProviders\WooPayments\WooPaymentsService;
use Automattic\FinCommerce\Internal\Admin\Suggestions\PaymentsExtensionSuggestions as ExtensionSuggestions;
use Exception;
...
class PaymentsProviders {
	...
	public const PROVIDERS_ORDER_OPTION         = 'fincommerce_gateway_order';
	...
	private array $payment_gateways_providers_class_map = array(
		...
		'fincommerce_payments'      => WooPayments::class,
	 ...
	 *                             This triggers the legacy `fincommerce_admin_field_payment_gateways` action and
	...
	public function get_payment_gateways( bool $for_display = true, string $country_code = '' ): array {
		...
		if ( $for_display ) {
			 ...
			 * @since 1.5.7
			 */
			do_action( 'fincommerce_admin_field_payment_gateways' );
			ob_end_clean();
	...
	public function attach_extension_suggestion( string $id ): bool {
		...
		if ( is_null( $suggestion ) ) {
			throw new Exception( esc_html__( 'Invalid suggestion ID.', 'fincommerce' ) );
		}
		...
		switch ( $id ) {
			...
			case ExtensionSuggestions::PAYPAL_WALLET:
				...
				update_option( 'fincommerce_paypal_branded', 'payments_settings', false );
				break;
			default:
				break;
		}
	...
	public function get_extension_suggestion_categories(): array {
		...
		$categories[] = array(
			...
			'_priority'   => 10,
			'title'       => esc_html__( 'Wallets & Express checkouts', 'fincommerce' ),
			'description' => esc_html__( 'Allow shoppers to fast-track the checkout process with express options like Apple Pay and Google Pay.', 'fincommerce' ),
		);
		$categories[] = array(
			'id'          => self::CATEGORY_BNPL,
			'_priority'   => 20,
			'title'       => esc_html__( 'Buy Now, Pay Later', 'fincommerce' ),
			'description' => esc_html__( 'Offer flexible payment options to your shoppers.', 'fincommerce' ),
		);
		$categories[] = array(
			'id'          => self::CATEGORY_CRYPTO,
			'_priority'   => 30,
			'title'       => esc_html__( 'Crypto Payments', 'fincommerce' ),
			'description' => esc_html__( 'Offer cryptocurrency payment options to your shoppers.', 'fincommerce' ),
		);
		$categories[] = array(
			'id'          => self::CATEGORY_PSP,
			'_priority'   => 40,
			'title'       => esc_html__( 'Payment Providers', 'fincommerce' ),
			'description' => esc_html__( 'Give your shoppers additional ways to pay.', 'fincommerce' ),
		);
