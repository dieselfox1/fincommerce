...
use Automattic\FinCommerce\Internal\Logging\SafeGlobalFunctionProxy;
use Throwable;
use WC_Gateway_BACS;
...
class PaymentsController {
	...
	const TRANSIENT_HAS_PROVIDERS_WITH_INCENTIVE_KEY = 'fincommerce_admin_settings_payments_has_providers_with_incentive';
	...
	public function register() {
		...
		add_filter( 'fincommerce_admin_allowed_promo_notes', array( $this, 'add_allowed_promo_notes' ) );
		add_filter( 'fincommerce_get_sections_checkout', array( $this, 'handle_sections' ), 20 );
		add_action( 'fincommerce_admin_payments_extension_suggestion_incentive_dismissed', array( $this, 'handle_incentive_dismissed' ) );
	}
	...
	public function add_menu() {
		...
		$menu_title = esc_html__( 'Payments', 'fincommerce' );
		...
		add_menu_page(
			$menu_title,
			$menu_title,
			'manage_fincommerce', // Capability required to see the menu item.
			$menu_path,
			null,
			$menu_icon,
			56, // Position after FinCommerce Product menu item.
		);
	...
	public function add_body_classes( $classes = '' ) {
		...
		if ( 'checkout' === $current_tab && ! str_contains( 'fincommerce-settings-payments-tab', $classes ) ) {
			$classes = "$classes fincommerce-settings-payments-tab";
		}
