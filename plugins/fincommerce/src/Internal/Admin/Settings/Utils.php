...
use Automattic\FinCommerce\Admin\API\OnboardingPlugins;
use WP_REST_Request;
...
class Utils {
	...
	/**
	 * Retrieves a URL to relative path inside FinCommerce admin Payments settings with
	...
	public static function rest_endpoint_get_request( string $endpoint, array $params = array() ) {
		...
		if ( 200 !== $response->get_status() ) {
			return new \WP_Error(
				'fincommerce_settings_payments_rest_error',
				sprintf(
					/* translators: 1: the endpoint relative URL, 2: error code, 3: error message */
					esc_html__( 'REST request GET %1$s failed with: (%2$s) %3$s', 'fincommerce' ),
					$endpoint,
					$response_data['code'] ?? 'unknown_error',
					$response_data['message'] ?? esc_html__( 'Unknown error', 'fincommerce' )
				),
				$response_data
			);
		}
	...
	/**
	 * Post data to a FinCommerce API endpoint and return the response data.
	 *
	 * @param string $endpoint Endpoint.
	...
	public static function rest_endpoint_post_request( string $endpoint, array $params = array() ) {
		...
		if ( 200 !== $response->get_status() ) {
			return new \WP_Error(
				...
				sprintf(
				/* translators: 1: the endpoint relative URL, 2: error code, 3: error message */
					esc_html__( 'REST request POST %1$s failed with: (%2$s) %3$s', 'fincommerce' ),
					$endpoint,
					$response_data['code'] ?? 'unknown_error',
					$response_data['message'] ?? esc_html__( 'Unknown error', 'fincommerce' )
				),
		...
		if ( ! empty( $result['url'] ) ) {
			$result['url'] = add_query_arg(
				array(
					...
					'plugin_name'  => 'fincommerce-payments',
