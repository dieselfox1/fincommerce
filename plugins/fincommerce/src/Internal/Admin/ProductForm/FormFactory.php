<?php
/**
 * FinCommerce Product Form Factory
 *
 * @package Fincommerce ProductForm
 */

namespace Automattic\FinCommerce\Internal\Admin\ProductForm;

use WP_Error;

/**
 * Factory that contains logic for the FinCommerce Product Form.
 */
class FormFactory {
	/**
	 * Class instance.
	...
	private static function get_items( $type, $class_name, $sort_by = array(
		...
		$class     = 'Automattic\\FinCommerce\\Internal\\Admin\\ProductForm\\' . $class_name;
		...
		if ( ! class_exists( $class ) ) {
			return new WP_Error(
				...
				/* translators: 1: missing class name. */
					esc_html__( '%1$s class does not exist.', 'fincommerce' ),
					$class
				)
			);
		}
		if ( isset( $item_list[ $id ] ) ) {
			return new WP_Error(
				...
				/* translators: 1: Item type 2: Duplicate registered item id. */
					esc_html__( 'You have attempted to register a duplicate form %1$s with FinCommerce Form: %2$s', 'fincommerce' ),
					$type,
					'`' . $id . '`'
				)
			);
