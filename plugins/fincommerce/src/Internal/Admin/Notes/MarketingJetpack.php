<?php
/**
 * FinCommerce Admin Jetpack Marketing Note Provider.
 *
 ...
 *
 * @see  https://developer.fincommerce.com/2020/10/16/using-the-admin-notes-inbox-in-fincommerce/
 */
class MarketingJetpack {
	...
	public static function get_note() {
		$note = new Note();
		$note->set_title( __( 'Protect your FinCommerce Store with Jetpack Backup.', 'fincommerce' ) );
		$note->set_content( __( 'Store downtime means lost sales. One-click restores get you back online quickly if something goes wrong.', 'fincommerce' ) );
		...
		$note->set_source( 'fincommerce-admin-notes' );
		$note->add_action(
			'jetpack-backup-fincommerce',
			__( 'Get backups', 'fincommerce' ),
			esc_url( 'https://jetpack.com/upgrade/backup-fincommerce/?utm_source=inbox&utm_medium=automattic_referred&utm_campaign=jp_backup_to_woo' ),
