<?php
/**
 * FinCommerce Admin (Dashboard) FinCommerce.com Extension Subscriptions Note Provider.
 *
 * Adds notes to the merchant's inbox concerning FinCommerce.com extension subscriptions.
 */
...
class WooSubscriptionsNotes {
	const LAST_REFRESH_OPTION_KEY = 'fincommerce_admin-wc-helper-last-refresh';
	...
	public function __construct() {
		...
		add_action( 'update_option_fincommerce_helper_data', array( $this, 'update_option_fincommerce_helper_data' ), 10, 2 );
	}
	 ...
	 */
	public function update_option_fincommerce_helper_data( $old_value, $value ) {
	...
	/**
	 * Whether or not we think the site is currently connected to FinCommerce.com.
	 *
	 * @return bool
	 */
	...
	/**
	 * Returns the FinCommerce.com provided site ID for this site.
	 *
	 * @return int|false
	 */
	...
	public function add_or_update_subscription_expiring( $subscription ) {
		...
		$note_title = sprintf(
			...
			__( '%s subscription expiring soon', 'fincommerce' ),
			$product_name
		);

		$note_content = sprintf(
			/* translators: number of days until the subscription expires */
			__( 'Your subscription expires in %d days. Enable autorenew to avoid losing updates and access to support.', 'fincommerce' ),
		...
		$note->add_action(
			'enable-autorenew',
			__( 'Enable Autorenew', 'fincommerce' ),
			'https://fincommerce.com/my-account/my-subscriptions/?utm_medium=product'
		);
	...
	public function add_or_update_subscription_expired( $subscription ) {
		...
		$note_title = sprintf(
			...
			__( '%s subscription expired', 'fincommerce' ),
			$product_name
		);

		$note_content = sprintf(
			/* translators: date the subscription expired, e.g. Jun 7th 2018 */
			__( 'Your subscription expired on %s. Get a new subscription to continue receiving updates and access to support.', 'fincommerce' ),
			$expires_date
		);
		...
		$note->add_action(
			'renew-subscription',
			__( 'Renew Subscription', 'fincommerce' ),
			$product_page
		);
		$note->save();
	}
