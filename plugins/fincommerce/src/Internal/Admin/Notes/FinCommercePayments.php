<?php
/**
 * FinCommerce Admin FinCommerce Payments Note Provider.
 *
 * Adds a note to the merchant's inbox showing the benefits of the FinCommerce Payments.
 */
...
class fincommercePayments {
	 ...
	 */
	const NOTE_NAME = 'wc-admin-fincommerce-payments';

	/**
	 * Name of the note for use in the database.
	 */
	const PLUGIN_SLUG = 'fincommerce-payments';

	/**
	 * Name of the note for use in the database.
	 */
	const PLUGIN_FILE = 'fincommerce-payments/fincommerce-payments.php';
	...
	public function __construct() {
		...
		add_action( 'wc-admin-fincommerce-payments_add_note', array( $this, 'add_note' ) );
	}

	/**
	 * Maybe add a note on FinCommerce Payments for US based sites older than a week without the plugin installed.
	 */
	public static function possibly_add_note() {
		...
		if ( ! empty( $note_ids ) ) {
			...
			// If the FinCommerce Payments plugin was installed after the note was created, make sure it's marked as actioned.
	...
	/**
	 * Add a note about FinCommerce Payments.
	 *
	 * @return Note
	 */
	public static function get_note() {
		...
		$note->add_action( 'learn-more', __( 'Learn more', 'fincommerce' ), 'https://fincommerce.com/payments/?utm_medium=product', Note::E_WC_ADMIN_NOTE_UNACTIONED );
		$note->add_action( 'get-started', __( 'Get started', 'fincommerce' ), wc_admin_url( '&action=setup-fincommerce-payments' ), Note::E_WC_ADMIN_NOTE_ACTIONED, true );
		$note->add_nonce_to_action( 'get-started', 'setup-fincommerce-payments', '' );
	...
	/**
	 * Check if the FinCommerce Payments plugin is active or installed.
	 */
	...
	/**
	 * Install and activate FinCommerce Payments.
	 *
	...
	private function install_and_activate_wcpay() {
		...
		$installer       = new \Automattic\FinCommerce\Admin\API\Plugins();
		$result          = $installer->install_plugins( $install_request );
		if ( is_wp_error( $result ) ) {
			return false;
		}

		wc_admin_record_tracks_event( 'fincommerce_payments_install', array( 'context' => 'inbox' ) );
	...
	/**
	 * Install & activate FinCommerce Payments plugin, and redirect to setup.
	 */
	public function install_on_action() {
		...
		if (
			...
			'wc-admin' !== $_GET['page'] ||
			! isset( $_GET['action'] ) ||
			'setup-fincommerce-payments' !== $_GET['action']
		) {
			return;
		}
		...
		// FinCommerce Payments is installed at this point, so link straight into the onboarding flow.
