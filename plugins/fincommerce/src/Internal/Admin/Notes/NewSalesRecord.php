<?php
/**
 * FinCommerce Admin (Dashboard) New Sales Record Note Provider.
 *
...
class NewSalesRecord {
	 ...
	 */
	const RECORD_DATE_OPTION_KEY = 'fincommerce_sales_record_date';

	/**
	 * Option name for the sales record amount.
	 */
	const RECORD_AMOUNT_OPTION_KEY = 'fincommerce_sales_record_amount';
	...
	public static function possibly_add_note() {
		 ...
		 */
		$sales_record_notes_enabled = apply_filters( 'fincommerce_admin_sales_record_milestone_enabled', true );
	...
	public static function get_note_with_record_data( $record_date, $record_amt, $yesterday, $total ) {
		...
		$content = sprintf(
			...
			__( 'Woohoo, %1$s was your record day for sales! Net sales was %2$s beating the previous record of %3$s set on %4$s.', 'fincommerce' ),
			$formatted_yesterday,
			$formatted_total,
		...
		$note = new Note();
		$note->set_title( __( 'New sales record!', 'fincommerce' ) );
		$note->set_content( $content );
		$note->set_content_data( $content_data );
		$note->set_type( Note::E_WC_ADMIN_NOTE_INFORMATIONAL );
		$note->set_name( self::NOTE_NAME );
		$note->set_source( 'fincommerce-admin' );
		$note->add_action( 'view-report', __( 'View report', 'fincommerce' ), $report_url );
