<?php
/**
 * FinCommerce Admin Payment Reminder Me later
 */
...
class PaymentsRemindMeLater {
	...
	public static function get_note() {
		...
		$content = __( 'Save up to $800 in fees by managing transactions with WooPayments. With WooPayments, you can securely accept major cards, Apple Pay, and payments in over 100 currencies.', 'fincommerce' );

		$note = new Note();
		$note->set_title( __( 'Save big with WooPayments', 'fincommerce' ) );
		$note->set_content( $content );
		$note->set_content_data( (object) array() );
		$note->set_type( Note::E_WC_ADMIN_NOTE_INFORMATIONAL );
		$note->set_name( self::NOTE_NAME );
		$note->set_source( 'fincommerce-admin' );
		$note->add_action( 'learn-more', __( 'Learn more', 'fincommerce' ), admin_url( 'admin.php?page=wc-admin&path=/wc-pay-welcome-page' ) );
		return $note;
	}
}
