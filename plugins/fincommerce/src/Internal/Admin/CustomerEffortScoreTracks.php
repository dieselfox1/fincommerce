<?php
/**
 * FinCommerce Customer effort score tracks
 *
 * @package FinCommerce\Admin\Features
 */
...
class CustomerEffortScoreTracks {
	 ...
	 */
	const CES_TRACKS_QUEUE_OPTION_NAME = 'fincommerce_ces_tracks_queue';
	 ...
	 */
	const SHOWN_FOR_ACTIONS_OPTION_NAME = 'fincommerce_ces_shown_for_actions';
	...
	/**
	 * Constructor. Sets up filters to hook into FinCommerce.
	 */
	...
	/**
	 * Add actions that require fincommerce_allow_tracking.
	 */
	private function enable_survey_enqueing_if_tracking_is_enabled() {
		...
		$allow_tracking = 'yes' === get_option( 'fincommerce_allow_tracking', 'no' );
		if ( ! $allow_tracking ) {
			return;
		}

		add_action( 'admin_init', array( $this, 'maybe_clear_ces_tracks_queue' ) );
		add_action( 'fincommerce_update_options', array( $this, 'run_on_update_options' ), 10, 3 );
		...
		add_action( 'fincommerce_attribute_added', array( $this, 'run_on_add_product_attributes' ), 10, 3 );
		...
		$this->onsubmit_label = __( 'Thank you for your feedback!', 'fincommerce' );
	}
	...
	public function enqueue_ces_survey_for_search( $search_area, $page_now, $admin_page ) {
		...
		$this->enqueue_to_ces_tracks(
			array(
				...
				'firstQuestion'  => __(
					'The search feature in FinCommerce is easy to use.',
					'fincommerce'
				),
				'secondQuestion' => __(
	...
	public function add_script_track_product_categories() {
		...
		wc_enqueue_js(
			$this->get_script_track_edit_php(
				...
				__( 'How easy was it to add product category?', 'fincommerce' ),
				__( 'The product category details screen is easy to use.', 'fincommerce' ),
				__( "The product category details screen's functionality meets my needs.", 'fincommerce' )
			)
		);
	}
	...
	public function add_script_track_product_tags() {
		...
		wc_enqueue_js(
			$this->get_script_track_edit_php(
				...
				__( 'How easy was it to add a product tag?', 'fincommerce' ),
				__( 'The product tag details screen is easy to use.', 'fincommerce' ),
				__( "The product tag details screen's functionality meets my needs.", 'fincommerce' )
			)
		);
	}
	...
	public function run_on_update_options() {
		...
		$this->enqueue_to_ces_tracks(
			array(
				...
				'onsubmit_label' => $this->onsubmit_label,
				'pagenow'        => 'fincommerce_page_wc-settings',
				'adminpage'      => 'fincommerce_page_wc-settings',
				'props'          => (object) $props,
			)
		);
	}
