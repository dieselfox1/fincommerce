<?php
/**
 * FinCommerce Admin: Feature plugin main class.
 */
...
use Automattic\FinCommerce\Admin\API;
...
use Automattic\FinCommerce\Admin\PluginsInstaller;
use Automattic\FinCommerce\Admin\ReportExporter;
use Automattic\FinCommerce\Admin\ReportsSync;
use Automattic\FinCommerce\Internal\Admin\CategoryLookup;
use Automattic\FinCommerce\Internal\Admin\Events;
use Automattic\FinCommerce\Internal\Admin\Onboarding\Onboarding;
...
class FeaturePlugin {
	...
	/**
	 * Init the feature plugin, only if we can detect both Gutenberg and FinCommerce.
	 */
	public function init() {
		...
		// Load the page controller functions file first to prevent fatal errors when disabling FinCommerce Admin.
		...
		if ( did_action( 'plugins_loaded' ) ) {
			...
			// Make sure we hook into `plugins_loaded` before core's Automattic\FinCommerce\Package::init().
			// If core is network activated but we aren't, the packaged version of FinCommerce Admin will
			// attempt to use a data store that hasn't been loaded yet - because we've defined our constants here.
			// See: https://github.com/dieselfox1/fincommerce-admin/issues/3869.
	...
	protected function hooks() {
		add_filter( 'fincommerce_admin_features', array( $this, 'replace_supported_features' ), 0 );
	...
	public function replace_supported_features( $features ) {
		 ...
		 * @since 6.5.0
		 */
		$feature_config = apply_filters( 'fincommerce_admin_get_feature_config', wc_admin_get_feature_config() );
