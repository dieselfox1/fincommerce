 ...
 *
 * @deprecated 9.9.0 The WooPayments promotion engine is deprecated and will be removed in a future version of FinCommerce.
 */
class Init extends RemoteSpecsEngine {
	...
	public static function possibly_register_pre_install_wc_pay_promotion_gateway( $gateways ) {
		if ( self::can_show_promotion() && ! WCPaymentGatewayPreInstallWCPayPromotion::is_dismissed() ) {
			$gateways[] = 'Automattic\FinCommerce\Internal\Admin\WCPayPromotion\WCPaymentGatewayPreInstallWCPayPromotion';
		}
		return $gateways;
	}
	...
	public static function get_wc_pay_promotion_spec( $fetch_from_remote = true ) {
		...
		$wc_pay_promotion_spec = array_values(
			array_filter(
				$promotions,
				function ( $promotion ) {
					return isset( $promotion->plugins ) && in_array( 'fincommerce-payments', $promotion->plugins, true );
				}
			)
		);
	...
	public static function is_woopay_eligible() {
		...
		return $wcpay_promotion && 'fincommerce_payments:woopay' === $wcpay_promotion->id;
	}
