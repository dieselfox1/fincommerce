...
use Automattic\FinCommerce\Admin\API\Plugins;
...
/**
 * Contains logic in regards to FinCommerce Admin Settings.
 */
class Settings {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_components_settings', array( $this, 'add_component_settings' ) );
		// New settings injection.
		add_filter( 'fincommerce_admin_shared_settings', array( $this, 'add_component_settings' ) );
		add_filter( 'fincommerce_settings_groups', array( $this, 'add_settings_group' ) );
		add_filter( 'fincommerce_settings-wc_admin', array( $this, 'add_settings' ) );
	}
	...
	public static function get_currency_settings() {
		...
		return apply_filters(
			...
			array(
				...
				'symbol'            => html_entity_decode( get_fincommerce_currency_symbol( $code ) ),
				'symbolPosition'    => get_option( 'fincommerce_currency_pos' ),
				'decimalSeparator'  => wc_get_price_decimal_separator(),
				'thousandSeparator' => wc_get_price_thousand_separator(),
				'priceFormat'       => html_entity_decode( get_fincommerce_price_format() ),
			)
		);
	}
	...
	public function add_settings_group( $groups ) {
		$groups[] = array(
			'id'          => 'wc_admin',
			'label'       => __( 'FinCommerce Admin', 'fincommerce' ),
			'description' => __( 'Settings for FinCommerce admin reporting.', 'fincommerce' ),
		);
		return $groups;
	}
	...
	public function add_settings( $settings ) {
		...
		$settings[] = array(
			'id'          => 'fincommerce_excluded_report_order_statuses',
			'option_key'  => 'fincommerce_excluded_report_order_statuses',
			'label'       => __( 'Excluded report order statuses', 'fincommerce' ),
			'description' => __( 'Statuses that should not be included when calculating report totals.', 'fincommerce' ),
			'default'     => array( 'pending', 'cancelled', 'failed' ),
			'type'        => 'multiselect',
			'options'     => $all_statuses,
		);
		$settings[] = array(
			'id'          => 'fincommerce_actionable_order_statuses',
			'option_key'  => 'fincommerce_actionable_order_statuses',
			'label'       => __( 'Actionable order statuses', 'fincommerce' ),
			'description' => __( 'Statuses that require extra action on behalf of the store admin.', 'fincommerce' ),
			'default'     => array( 'processing', 'on-hold' ),
			'type'        => 'multiselect',
			'options'     => $all_statuses,
		);
		$settings[] = array(
			'id'          => 'fincommerce_default_date_range',
			'option_key'  => 'fincommerce_default_date_range',
			'label'       => __( 'Default Date Range', 'fincommerce' ),
			'description' => __( 'Default Date Range', 'fincommerce' ),
			'default'     => 'period=month&compare=previous_year',
			'type'        => 'text',
		);
		$settings[] = array(
			'id'          => 'fincommerce_date_type',
			'option_key'  => 'fincommerce_date_type',
			'label'       => __( 'Date Type', 'fincommerce' ),
			'description' => __( 'Database date field considered for Revenue and Orders reports', 'fincommerce' ),
			'type'        => 'select',
			'options'     => array(
