...
use Automattic\FinCommerce\Internal\Admin\RemoteFreeExtensions\Init;
use Automattic\FinCommerce\Internal\Admin\RemoteFreeExtensions\ProcessCoreProfilerPluginInstallOptions;
...
class OnboardingSetupWizard {
	...
	public function init() {
		...
		add_action(
			'fincommerce_plugins_install_before',
		...
		// Run after Automattic\FinCommerce\Internal\Admin\Loader.
		add_filter( 'fincommerce_components_settings', array( $this, 'component_settings' ), 20 );
		// New settings injection.
		add_filter( 'fincommerce_admin_shared_settings', array( $this, 'component_settings' ), 20 );
		add_filter( 'fincommerce_admin_preload_settings', array( $this, 'preload_settings' ) );
		...
		add_filter( 'fincommerce_show_admin_notice', array( $this, 'remove_old_install_notice' ), 10, 2 );
	 ...
	 * @return bool
	 */
	private function is_fincommerce_page() {
	...
	public function component_settings( $settings ) {
		...
		// or the current page is one of the FinCommerce Admin pages.
		if (
		...
		$settings['onboarding']['currencySymbols'] = get_fincommerce_currency_symbols();
		...
		// phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		return apply_filters( 'fincommerce_admin_onboarding_preloaded_data', $settings );
	}
