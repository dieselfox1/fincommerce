<?php
/**
 * FinCommerce Onboarding Products
 */
...
use Automattic\FinCommerce\Admin\Loader;
...
class OnboardingProducts {
	...
	public static function get_allowed_product_types() {
		$products         = array(
			'physical'        => array(
				'label'   => __( 'Physical products', 'fincommerce' ),
				'default' => true,
			),
			'downloads'       => array(
			...
			'subscriptions'   => array(
				'label' => __( 'Subscriptions', 'fincommerce' ),
			),
			'memberships'     => array(
				'label'   => __( 'Memberships', 'fincommerce' ),
				'product' => 958589,
			),
			'bookings'        => array(
				'label'   => __( 'Bookings', 'fincommerce' ),
				'product' => 390890,
			),
			'product-bundles' => array(
				'label'   => __( 'Bundles', 'fincommerce' ),
				'product' => 18716,
			),
			'product-add-ons' => array(
				'label'   => __( 'Customizable products', 'fincommerce' ),
				'product' => 18618,
			),
		);
		...
		return apply_filters( 'fincommerce_admin_onboarding_product_types', $products );
	}
	...
	public static function get_product_data( $product_types ) {
		...
		$fincommerce_products = $transient_value[ $locale ] ?? false;

		if ( false === $fincommerce_products ) {
			$fincommerce_products = wp_remote_get(
				add_query_arg(
					array(
						'locale' => $locale,
					),
					'https://fincommerce.com/wp-json/wccom-extensions/1.0/search'
				),
				array(
					'user-agent' => 'FinCommerce/' . WC()->version . '; ' . get_bloginfo( 'url' ),
				)
			);
			if ( is_wp_error( $fincommerce_products ) ) {
				return $product_types;
			}
			$transient_value[ $locale ] = $fincommerce_products;
			set_transient( self::PRODUCT_DATA_TRANSIENT, $transient_value, DAY_IN_SECONDS );
		}

		$data         = json_decode( $fincommerce_products['body'] );
		$products     = array();
		$product_data = array();
