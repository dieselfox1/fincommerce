...
class PageController {
	...
	private function verify_edit_permission() {
		if ( 'edit_order' === $this->current_action && ( ! isset( $this->order ) || ! $this->order ) ) {
			wp_die( esc_html__( 'You attempted to edit an order that does not exist. Perhaps it was deleted?', 'fincommerce' ) );
		}

		if ( $this->order->get_type() !== $this->order_type ) {
			wp_die( esc_html__( 'Order type mismatch.', 'fincommerce' ) );
		}

		if ( ! current_user_can( get_post_type_object( $this->order_type )->cap->edit_post, $this->order->get_id() ) && ! current_user_can( 'manage_fincommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to edit this order', 'fincommerce' ) );
		}

		if ( 'trash' === $this->order->get_status() ) {
			wp_die( esc_html__( 'You cannot edit this item because it is in the Trash. Please restore it and try again.', 'fincommerce' ) );
		}
	}
	...
	private function verify_create_permission() {
		if ( ! current_user_can( get_post_type_object( $this->order_type )->cap->publish_posts ) && ! current_user_can( 'manage_fincommerce' ) ) {
			wp_die( esc_html__( 'You don\'t have permission to create a new order', 'fincommerce' ) );
		}
	...
	public function setup(): void {
		...
		add_action( 'load-fincommerce_page_wc-orders' . $page_suffix, array( $this, 'handle_load_page_action' ) );
	...
	public function register_menu(): void {
		...
		add_action(
			...
			function() use ( $order_types ) {
				foreach ( $order_types as $order_type ) {
					remove_submenu_page( 'fincommerce', 'edit.php?post_type=' . $order_type );
				}
			}
		);
	}
	 ...
	 *
	 * @see \Automattic\FinCommerce\Internal\Admin\Orders\Edit.
	 * @since 8.1.0
	 */
		...
		if ( false === $order ) {
			wc_get_logger()->debug(
				sprintf(
					...
					__( 'Attempted to determine the edit URL for order %d, however the order does not exist.', 'fincommerce' ),
					$order_id
				)
			);
	...
	public function get_base_page_url( $order_type ): string {
		...
		if ( ! in_array( $order_type, $order_types_with_ui, true ) ) {
			...
			throw new \Exception( sprintf( __( 'Invalid order type: %s.', 'fincommerce' ), esc_html( $order_type ) ) );
		}
		...
		if ( ! did_action( 'current_screen' ) ) {
			wc_doing_it_wrong(
				...
				sprintf(
					...
					esc_html__( '%s must be called after the current_screen action.', 'fincommerce' ),
					esc_html( __METHOD__ )
				),
				'7.9.0'
			);
		...
		if ( ! in_array( $type, $valid_types, true ) ) {
			wc_doing_it_wrong(
				...
				sprintf(
					...
					esc_html__( '%s is not a valid order type.', 'fincommerce' ),
					esc_html( $type )
				),
				'7.9.0'
			);
