...
class EditLock {
	...
	public function get_lock( \WC_Order $order ) {
		...
		$time_window = apply_filters( 'wp_check_post_lock_window', 150 ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
	...
	public function refresh_lock_ajax( $response, $data ) {
		...
		if ( ! $order || ! is_a( $order, \WC_Order::class ) || ( ! current_user_can( get_post_type_object( $order->get_type() )->cap->edit_post, $order->get_id() ) && ! current_user_can( 'manage_fincommerce' ) ) ) {
			return $response;
		}
		...
		if ( ! $this->is_locked_by_another_user( $order ) ) {
			...
			$response['wc-refresh-order-lock']['error'] = array(
				...
				'message'            => sprintf( __( '%s has taken over and is currently editing.', 'fincommerce' ), $user->display_name ),
	...
	public function check_locked_orders_ajax( $response, $data ) {
		...
		foreach ( $order_ids as $order_id ) {
			...
			if ( ! $this->is_locked_by_another_user( $order ) || ( ! current_user_can( get_post_type_object( $order->get_type() )->cap->edit_post, $order->get_id() ) && ! current_user_can( 'manage_fincommerce' ) ) ) {
				continue;
			}
	...
	public function render_dialog( $order ) {
		...
		$edit_url = wc_get_container()->get( \Automattic\FinCommerce\Internal\Admin\Orders\PageController::class )->get_edit_url( $order->get_id() );

		$sendback_url = wp_get_referer();
		if ( ! $sendback_url ) {
			$sendback_url = wc_get_container()->get( \Automattic\FinCommerce\Internal\Admin\Orders\PageController::class )->get_base_page_url( $order->get_type() );
		}

		$sendback_text = __( 'Go back', 'fincommerce' );
		?>
		<div id="post-lock-dialog" class="notification-dialog-wrap <?php echo $locked ? '' : 'hidden'; ?> order-lock-dialog">
			...
			<div class="post-locked-message">
				...
				<?php
				// translators: %s is a user's name.
				echo esc_html( sprintf( __( '%s is currently editing this order. Do you want to take over?', 'fincommerce' ), esc_html( $user->display_name ) ) );
				?>
				</p>
				<p>
					<a class="button" href="<?php echo esc_url( $sendback_url ); ?>"><?php echo esc_html( $sendback_text ); ?></a>
					<a class="button button-primary wp-tab-last" href="<?php echo esc_url( add_query_arg( 'claim-lock', '1', wp_nonce_url( $edit_url, 'claim-lock-' . $order->get_id() ) ) ); ?>"><?php esc_html_e( 'Take over', 'fincommerce' ); ?></a>
				</p>
			</div>
			<?php else : ?>
