...
namespace Automattic\FinCommerce\Internal\Admin\Orders\MetaBoxes;

use Automattic\FinCommerce\Internal\DataStores\CustomMetaDataStore;
...
class CustomMetaBox {
	...
	public function render_meta_form( \WC_Order $order ) : void {
		?>
		<p><strong><?php esc_html_e( 'Add New Custom Field:', 'fincommerce' ); ?></strong></p>
		<table id="newmeta">
			<thead>
			<tr>
				<th class="left"><label for="metakeyselect"><?php esc_html_e( 'Name', 'fincommerce' ); ?></label></th>
				<th><label for="metavalue"><?php esc_html_e( 'Value', 'fincommerce' ); ?></label></th>
			</tr>
			</thead>
			...
			<tr>
				<td id="newmetaleft" class="left">
					...
					<select id="metakeyselect" name="metakeyselect" class="wc-order-metakey-search" data-placeholder="<?php esc_attr_e( 'Add existing', 'fincommerce' ); ?>" data-minimum-input-length="0" data-order_id="<?php echo esc_attr( $order->get_id() ); ?>">
					</select>
					</span>
					<input class="hidden" type="text" id="metakeyinput" name="metakeyinput" value="" aria-label="<?php esc_attr_e( 'New custom field name', 'fincommerce' ); ?>" />
					...
					<span id="enternew"><?php esc_html_e( 'Enter new', 'fincommerce' ); ?></span>
					<span id="cancelnew" class="hidden"><?php esc_html_e( 'Cancel', 'fincommerce' ); ?></span>
				</td>
		...
		<div class="submit add-custom-field">
			<?php
			submit_button(
				__( 'Add Custom Field', 'fincommerce' ),
				'',
				'addmeta',
				false,
				array(
	...
	private function verify_order_edit_permission_for_ajax( int $order_id ): ?WC_Order {
		if ( ! current_user_can( 'manage_fincommerce' ) || ! current_user_can( 'edit_others_shop_orders' ) ) {
	...
	private function list_meta_row( array $entry, int &$count ) : string {
		...
		$r .= "\n\t\t<td class='left'><label class='screen-reader-text' for='meta-{$entry['meta_id']}-key'>" . __( 'Key', 'fincommerce' ) . "</label><input name='meta[{$entry['meta_id']}][key]' id='meta-{$entry['meta_id']}-key' type='text' size='20' value='{$entry['meta_key']}' />";

		$r .= "\n\t\t<div class='submit'>";
		$r .= get_submit_button( __( 'Delete', 'fincommerce' ), 'deletemeta small', "deletemeta[{$entry['meta_id']}]", false, array( 'data-wp-lists' => "delete:the-list:meta-{$entry['meta_id']}::_ajax_nonce:$delete_nonce" ) );
		$r .= "\n\t\t";
		$r .= get_submit_button( __( 'Update', 'fincommerce' ), 'updatemeta small', "meta-{$entry['meta_id']}-submit", false, array( 'data-wp-lists' => "add:the-list:meta-{$entry['meta_id']}::_ajax_nonce-add-meta={$this->update_nonce}" ) );
		$r .= '</div>';
		$r .= wp_nonce_field( 'change-meta', '_ajax_nonce', false, false );
		$r .= '</td>';

		$r .= "\n\t\t<td><label class='screen-reader-text' for='meta-{$entry['meta_id']}-value'>" . __( 'Value', 'fincommerce' ) . "</label><textarea name='meta[{$entry['meta_id']}][value]' id='meta-{$entry['meta_id']}-value' rows='2' cols='30'>{$entry['meta_value']}</textarea></td>\n\t</tr>";
		return $r;
	}
