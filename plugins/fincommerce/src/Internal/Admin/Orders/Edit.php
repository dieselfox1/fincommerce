...
use Automattic\FinCommerce\Internal\Admin\Orders\MetaBoxes\CustomerHistory;
use Automattic\FinCommerce\Internal\Admin\Orders\MetaBoxes\CustomMetaBox;
use Automattic\FinCommerce\Internal\Admin\Orders\MetaBoxes\OrderAttribution;
use Automattic\FinCommerce\Internal\Admin\Orders\MetaBoxes\TaxonomiesMetaBox;
...
class Edit {
	...
	public static function add_order_meta_boxes( string $screen_id, string $title ) {
		...
		add_meta_box( 'fincommerce-order-data', sprintf( __( '%s data', 'fincommerce' ), $title ), 'WC_Meta_Box_Order_Data::output', $screen_id, 'normal', 'high' );
		add_meta_box( 'fincommerce-order-items', __( 'Items', 'fincommerce' ), 'WC_Meta_Box_Order_Items::output', $screen_id, 'normal', 'high' );
		/* Translators: %s order type name. */
		add_meta_box( 'fincommerce-order-notes', sprintf( __( '%s notes', 'fincommerce' ), $title ), 'WC_Meta_Box_Order_Notes::output', $screen_id, 'side', 'default' );
		add_meta_box( 'fincommerce-order-downloads', __( 'Downloadable product permissions', 'fincommerce' ) . wc_help_tip( __( 'Note: Permissions for order items will automatically be granted when the order status changes to processing/completed.', 'fincommerce' ) ), 'WC_Meta_Box_Order_Downloads::output', $screen_id, 'normal', 'default' );
		/* Translators: %s order type name. */
		add_meta_box( 'fincommerce-order-actions', sprintf( __( '%s actions', 'fincommerce' ), $title ), 'WC_Meta_Box_Order_Actions::output', $screen_id, 'side', 'high' );
	...
	public static function add_save_meta_boxes() {
		 ...
		 */
		add_action( 'fincommerce_process_shop_order_meta', 'WC_Meta_Box_Order_Items::save', 10 );
		add_action( 'fincommerce_process_shop_order_meta', 'WC_Meta_Box_Order_Downloads::save', 30, 2 );
		add_action( 'fincommerce_process_shop_order_meta', 'WC_Meta_Box_Order_Data::save', 40 );
		add_action( 'fincommerce_process_shop_order_meta', 'WC_Meta_Box_Order_Actions::save', 50, 2 );
	}
	...
	public function setup( \WC_Order $order ) {
		...
		$this->add_order_meta_boxes( $this->screen_id, __( 'Order', 'fincommerce' ) );
	...
	private static function maybe_register_order_attribution( string $screen_id, string $title ) {
		...
		add_meta_box(
			'fincommerce-order-source-data',
			/* Translators: %s order type name. */
			sprintf( __( '%s attribution', 'fincommerce' ), $title ),
		...
		if ( 'yes' !== get_option( 'fincommerce_analytics_enabled' ) ) {
			return;
		}
		...
		add_meta_box(
			'fincommerce-customer-history',
			__( 'Customer history', 'fincommerce' ),
	...
	public function handle_order_update() {
		 ...
		 * @since 2.1.0
		 */
		do_action( 'fincommerce_process_shop_order_meta', $this->order->get_id(), $this->order );
	...
	private function redirect_order( \WC_Order $order ) {
		...
		wp_safe_redirect(
			 ...
			 */
			apply_filters(
				'fincommerce_redirect_order_location',
				$redirect_to,
				$order->get_id(),
				$order
			)
	...
	public function display() {
		 ...
		 * @since 7.4.0.
		 */
		$messages = apply_filters( 'fincommerce_order_updated_messages', array() );
