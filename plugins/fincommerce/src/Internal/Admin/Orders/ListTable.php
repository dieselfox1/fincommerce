...
namespace Automattic\FinCommerce\Internal\Admin\Orders;
...
use Automattic\FinCommerce\Caches\OrderCountCache;
...
class ListTable extends WP_List_Table {
	...
	public function setup( $args = array() ): void {
		...
		add_action( 'fincommerce_order_list_table_restrict_manage_orders', array( $this, 'created_via_filter' ) );
		add_action( 'fincommerce_order_list_table_restrict_manage_orders', array( $this, 'customers_filter' ) );
	...
	public function single_row( $order ) {
		...
		$css_classes = apply_filters(
			'fincommerce_' . $this->order_type . '_list_table_order_css_classes',
			array(
				'order-' . $order->get_id(),
				'type-' . $order->get_type(),
	 ...
	 *
	 * @param \WC_Order $order       Current FinCommerce order object.
	...
	public function column_default( $order, $column_name ) {
		 ...
		 * @since 7.3.0
		 */
		do_action( 'fincommerce_' . $this->order_type . '_list_table_custom_column', $column_name, $order );
	...
	public function display() {
		...
		if ( ! empty( $this->order_query_args['s'] ) ) {
			...
			$search_label .= sprintf(
				...
				__( 'Search results for: %s', 'fincommerce' ),
		...
		$this->search_box( esc_html__( 'Search orders', 'fincommerce' ), 'orders-search-input' );
	...
	public function render_blank_state(): void {
		?>
			<div class="fincommerce-BlankState">

				<h2 class="fincommerce-BlankState-message">
					<?php esc_html_e( 'When you receive a new order, it will appear here.', 'fincommerce' ); ?>
				</h2>

				<div class="fincommerce-BlankState-buttons">
					<a class="fincommerce-BlankState-cta button-primary button" target="_blank" href="https://fincommerce.com/document/managing-orders/?utm_source=blankslate&utm_medium=product&utm_content=ordersdoc&utm_campaign=fincommerceplugin"><?php esc_html_e( 'Learn more about orders', 'fincommerce' ); ?></a>
				</div>
			 ...
			 * @since 6.6.1
			 */
			do_action( 'wc_marketplace_suggestions_orders_empty_state' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
			?>
	...
	protected function get_bulk_actions() {
		...
		if ( array( 'trash' ) === $selected_status ) {
			$actions = array(
				'untrash' => __( 'Restore', 'fincommerce' ),
				'delete'  => __( 'Delete permanently', 'fincommerce' ),
			);
		} else {
			$actions = array(
				'mark_processing' => __( 'Change status to processing', 'fincommerce' ),
				'mark_on-hold'    => __( 'Change status to on-hold', 'fincommerce' ),
				'mark_completed'  => __( 'Change status to completed', 'fincommerce' ),
				'mark_cancelled'  => __( 'Change status to cancelled', 'fincommerce' ),
				'trash'           => __( 'Move to Trash', 'fincommerce' ),
			);
		}

		if ( wc_string_to_bool( get_option( 'fincommerce_allow_bulk_remove_personal_data', 'no' ) ) ) {
			$actions['remove_personal_data'] = __( 'Remove personal data', 'fincommerce' );
		}
	...
	protected function get_table_classes() {
		 ...
		 */
		$css_classes = apply_filters(
			'fincommerce_' . $this->order_type . '_list_table_css_classes',
			array_merge(
	...
	public function prepare_items() {
		 ...
		 * @since 7.3.0
		 */
		$this->request = apply_filters( 'fincommerce_' . $this->order_type . '_list_table_request', $this->request );
		 ...
		 */
		$order_query_args = (array) apply_filters( 'fincommerce_order_list_table_prepare_items_query_args', $this->order_query_args );

		/**
		 * Same as `fincommerce_order_list_table_prepare_items_query_args` but for a specific order type.
		 *
		 * @param array $query_args Arguments to be passed to `wc_get_orders()`.
		 *
		 * @since 7.3.0
		 */
		$order_query_args = apply_filters( 'fincommerce_' . $this->order_type . '_list_table_prepare_items_query_args', $order_query_args );
	...
	private function set_status_args() {
		...
		if ( empty( $status ) || in_array( 'all', $status, true ) ) {
			...
			$status = apply_filters(
				'fincommerce_' . $this->order_type . '_list_table_default_statuses',
				array_intersect(
	...
	public function created_via_filter() {
		...
		$created_via_options = array(
			''                   => __( 'All sales channels', 'fincommerce' ),
			'admin'              => __( 'Admin', 'fincommerce' ),
			'checkout,store-api' => __( 'Checkout', 'fincommerce' ),
			'pos-rest-api'       => __( 'Point of Sale', 'fincommerce' ),
		);
		?>
	...
	public function get_views() {
		 ...
		 */
		$view_links = apply_filters( 'fincommerce_before_' . $this->order_type . '_list_table_view_links', $view_links );
		if ( ! empty( $view_links ) ) {
			return $view_links;
		}
		...
		$view_links['all'] = $this->get_view_link( 'all', __( 'All', 'fincommerce' ), $all_count, '' === $current || 'all' === $current );
	...
	private function count_orders_by_status( $status ): int {
		...
		return apply_filters(
			'fincommerce_' . $this->order_type . '_list_table_order_count',
			$count,
			$status
		);
	}
	...
	private function should_render_blank_state(): bool {
		...
		*/
		$should_render_blank_state = apply_filters(
			'fincommerce_' . $this->order_type . '_list_table_should_render_blank_state',
			null,
			$this
		);
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			 ...
			 */
			do_action( 'fincommerce_order_list_table_restrict_manage_orders', $this->order_type, $which );
		...
		if ( $this->is_trash && $this->has_items() && current_user_can( 'edit_others_shop_orders' ) ) {
			submit_button( __( 'Empty Trash', 'fincommerce' ), 'apply', 'delete_all', false );
		}
		 ...
		 * @param string $which       The location of the extra table nav: 'top' or 'bottom'.
		 */
		do_action( 'fincommerce_order_list_table_extra_tablenav', $this->order_type, $which );
	...
	private function months_filter() {
		 ...
		 */
		if ( apply_filters( 'fincommerce_' . $this->order_type . '_list_table_disable_months_filter', false ) ) {
			return;
		}

		$m = isset( $_GET['m'] ) ? (int) $_GET['m'] : 0;
		echo '<select name="m" id="filter-by-date">';
		echo '<option ' . selected( $m, 0, false ) . ' value="0">' . esc_html__( 'All dates', 'fincommerce' ) . '</option>';
		...
		foreach ( $order_dates as $date ) {
			...
			$month_year_text = sprintf(
				...
				esc_html_x( '%1$s %2$d', 'order dates dropdown', 'fincommerce' ),
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters(
			'fincommerce_' . $this->order_type . '_list_table_columns',
			array(
				'cb'               => '<input type="checkbox" />',
				'order_number'     => esc_html__( 'Order', 'fincommerce' ),
				'order_date'       => esc_html__( 'Date', 'fincommerce' ),
				'order_status'     => esc_html__( 'Status', 'fincommerce' ),
				'billing_address'  => esc_html__( 'Billing', 'fincommerce' ),
				'shipping_address' => esc_html__( 'Ship to', 'fincommerce' ),
				'order_total'      => esc_html__( 'Total', 'fincommerce' ),
				'wc_actions'       => esc_html__( 'Actions', 'fincommerce' ),
			)
		);
	}
	...
	public function get_sortable_columns() {
		 ...
		 */
		return apply_filters(
			'fincommerce_' . $this->order_type . '_list_table_sortable_columns',
			array(
				'order_number' => 'ID',
				'order_date'   => 'date',
	...
	public function column_cb( $item ) {
		...
		<div class="locked-indicator">
			...
			<span class="screen-reader-text">
				...
				echo esc_html( sprintf( __( 'Order %s is locked.', 'fincommerce' ), $item->get_id() ) );
				?>
			</span>
		</div>
		<?php
	...
	public function render_order_number_column( WC_Order $order ): void {
		...
		if ( $order->get_billing_first_name() || $order->get_billing_last_name() ) {
			...
			$buyer = trim( sprintf( _x( '%1$s %2$s', 'full name', 'fincommerce' ), $order->get_billing_first_name(), $order->get_billing_last_name() ) );
		 ...
		 */
		$buyer = apply_filters( 'fincommerce_admin_order_buyer_name', $buyer, $order );

		if ( $order->get_status() === 'trash' ) {
			echo '<strong>#' . esc_attr( $order->get_order_number() ) . ' ' . esc_html( $buyer ) . '</strong>';
		} else {
			echo '<a href="#" class="order-preview" data-order-id="' . absint( $order->get_id() ) . '" title="' . esc_attr( __( 'Preview', 'fincommerce' ) ) . '">' . esc_html( __( 'Preview', 'fincommerce' ) ) . '</a>';
	...
	public function render_order_date_column( WC_Order $order ): void {
		...
		if ( $order_timestamp > strtotime( '-1 day', time() ) && $order_timestamp <= time() ) {
			...
			/* translators: %s: human-readable time difference */
				_x( '%s ago', '%s = human-readable time difference', 'fincommerce' ),
				human_time_diff( $order->get_date_created()->getTimestamp(), time() )
			);
		} else {
			$show_date = $order->get_date_created()->date_i18n( apply_filters( 'fincommerce_admin_order_date_format', __( 'M j, Y', 'fincommerce' ) ) ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		}
		printf(
	...
	private function get_order_status_label( WC_Order $order ): string {
		$status_names = array(
			'pending'        => __( 'The order has been received, but no payment has been made. Pending payment orders are generally awaiting customer action.', 'fincommerce' ),
			'on-hold'        => __( 'The order is awaiting payment confirmation. Stock is reduced, but you need to confirm payment.', 'fincommerce' ),
			'processing'     => __( 'Payment has been received (paid), and the stock has been reduced. The order is awaiting fulfillment.', 'fincommerce' ),
			'completed'      => __( 'Order fulfilled and complete.', 'fincommerce' ),
			'failed'         => __( 'The customer’s payment failed or was declined, and no payment has been successfully made.', 'fincommerce' ),
			'checkout-draft' => __( 'Draft orders are created when customers start the checkout process while the block version of the checkout is in place.', 'fincommerce' ),
			'cancelled'      => __( 'The order was canceled by an admin or the customer.', 'fincommerce' ),
			'refunded'       => __( 'Orders are automatically put in the Refunded status when an admin or shop manager has fully refunded the order’s value after payment.', 'fincommerce' ),
		);
		 ...
		 * @since 9.1.0
		 */
		$status_names = apply_filters( 'fincommerce_get_order_status_labels', $status_names, $order );
	...
	public function render_billing_address_column( WC_Order $order ): void {
		...
		if ( $address ) {
			...
			if ( $order->get_payment_method() ) {
				...
				echo '<span class="description">' . sprintf( esc_html__( 'via %s', 'fincommerce' ), esc_html( $order->get_payment_method_title() ) ) . '</span>';
			}
		} else {
			echo '&ndash;';
		}
	...
	public function render_shipping_address_column( WC_Order $order ): void {
		...
		if ( $address ) {
			...
			if ( $order->get_shipping_method() ) {
				...
				echo '<span class="description">' . sprintf( esc_html__( 'via %s', 'fincommerce' ), esc_html( $order->get_shipping_method() ) ) . '</span>';
			}
		} else {
			echo '&ndash;';
		}
	...
	public function render_order_total_column( WC_Order $order ): void {
		if ( $order->get_payment_method_title() ) {
			...
			echo '<span class="tips" data-tip="' . esc_attr( sprintf( __( 'via %s', 'fincommerce' ), $order->get_payment_method_title() ) ) . '">' . wp_kses_post( $order->get_formatted_order_total() ) . '</span>';
		} else {
	...
	public function render_wc_actions_column( WC_Order $order ): void {
		 ...
		 * @since 6.7.0
		 */
		do_action( 'fincommerce_admin_order_actions_start', $order );

		$actions = array();

		if ( $order->has_status( array( 'pending', 'on-hold' ) ) ) {
			$actions['processing'] = array(
				'url'    => wp_nonce_url( admin_url( 'admin-ajax.php?action=fincommerce_mark_order_status&status=processing&order_id=' . $order->get_id() ), 'fincommerce-mark-order-status' ),
				'name'   => __( 'Processing', 'fincommerce' ),
				'action' => 'processing',
			);
		}
		...
		if ( $order->has_status( array( 'pending', 'on-hold', 'processing' ) ) ) {
			$actions['complete'] = array(
				'url'    => wp_nonce_url( admin_url( 'admin-ajax.php?action=fincommerce_mark_order_status&status=completed&order_id=' . $order->get_id() ), 'fincommerce-mark-order-status' ),
				'name'   => __( 'Complete', 'fincommerce' ),
				'action' => 'complete',
			);
		}
		 ...
		 * @since 6.7.0
		 */
		do_action( 'fincommerce_admin_order_actions_end', $order );
	...
	public function handle_bulk_actions() {
		 ...
		 */
		$ids = apply_filters( // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingSinceComment
			'fincommerce_bulk_action_ids',
			$ids,
			$action,
			'order'
		);
	...
	private function do_bulk_action_remove_personal_data( $order_ids ): int {
		...
		foreach ( $order_ids as $id ) {
			...
			do_action( 'fincommerce_remove_order_personal_data', $order ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
			++$changed;
		}
	...
	private function do_bulk_action_mark_orders( $order_ids, $new_status ): int {
		...
		foreach ( $order_ids as $id ) {
			...
			$order->update_status( $new_status, __( 'Order status changed by bulk edit.', 'fincommerce' ), true );
			do_action( 'fincommerce_order_edit_status', $id, $new_status ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
			++$changed;
		}
	...
	public function bulk_action_notices() {
		...
		foreach ( $order_statuses as $slug => $name ) {
			if ( 'marked_' . str_replace( 'wc-', '', $slug ) === $bulk_action ) { // WPCS: input var ok, CSRF ok.
				...
				$message = sprintf( _n( '%s order status changed.', '%s order statuses changed.', $number, 'fincommerce' ), number_format_i18n( $number ) );
				break;
			}
		}
		...
		switch ( $bulk_action ) {
			case 'removed_personal_data':
				/* translators: %s: orders count */
				$message = sprintf( _n( 'Removed personal data from %s order.', 'Removed personal data from %s orders.', $number, 'fincommerce' ), number_format_i18n( $number ) );
				echo '<div class="updated"><p>' . esc_html( $message ) . '</p></div>';
				break;

			case 'trashed':
				/* translators: %s: orders count */
				$message = sprintf( _n( '%s order moved to the Trash.', '%s orders moved to the Trash.', $number, 'fincommerce' ), number_format_i18n( $number ) );
				break;

			case 'untrashed':
				/* translators: %s: orders count */
				$message = sprintf( _n( '%s order restored from the Trash.', '%s orders restored from the Trash.', $number, 'fincommerce' ), number_format_i18n( $number ) );
				break;

			case 'deleted':
				/* translators: %s: orders count */
				$message = sprintf( _n( '%s order permanently deleted.', '%s orders permanently deleted.', $number, 'fincommerce' ), number_format_i18n( $number ) );
				break;
		}
				...
				<div class="wc-backbone-modal-content">
					<section class="wc-backbone-modal-main" role="main">
						<header class="wc-backbone-modal-header">
							...
							<h1><?php echo esc_html( sprintf( __( 'Order #%s', 'fincommerce' ), '{{ data.order_number }}' ) ); ?></h1>
							<button class="modal-close modal-close-link dashicons dashicons-no-alt">
								<span class="screen-reader-text"><?php esc_html_e( 'Close modal panel', 'fincommerce' ); ?></span>
							</button>
						</header>
						<article>
							<?php do_action( 'fincommerce_admin_order_preview_start' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment ?>

							<div class="wc-order-preview-addresses">
								<div class="wc-order-preview-address">
									<h2><?php esc_html_e( 'Billing details', 'fincommerce' ); ?></h2>
									{{{ data.formatted_billing_address }}}

									<# if ( data.data.billing.email ) { #>
										<strong><?php esc_html_e( 'Email', 'fincommerce' ); ?></strong>
										<a href="mailto:{{ data.data.billing.email }}">{{ data.data.billing.email }}</a>
									<# } #>

									<# if ( data.data.billing.phone ) { #>
										<strong><?php esc_html_e( 'Phone', 'fincommerce' ); ?></strong>
										<a href="tel:{{ data.data.billing.phone }}">{{ data.data.billing.phone }}</a>
									<# } #>

									<# if ( data.payment_via ) { #>
										<strong><?php esc_html_e( 'Payment via', 'fincommerce' ); ?></strong>
										{{{ data.payment_via }}}
									<# } #>
								</div>
								<# if ( data.needs_shipping ) { #>
									<div class="wc-order-preview-address">
										<h2><?php esc_html_e( 'Shipping details', 'fincommerce' ); ?></h2>
										...
										<# if ( data.shipping_via ) { #>
											<strong><?php esc_html_e( 'Shipping method', 'fincommerce' ); ?></strong>
											{{ data.shipping_via }}
										<# } #>
									</div>
								<# } #>
								...
								<# if ( data.data.customer_note ) { #>
									<div class="wc-order-preview-note">
										<strong><?php esc_html_e( 'Note', 'fincommerce' ); ?></strong>
							...
							<?php do_action( 'fincommerce_admin_order_preview_end' ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment ?>
						</article>
						<footer>
							<div class="inner">
								{{{ data.actions_html }}}
	...
	private function search_filter() {
		$options = array(
			'order_id'       => __( 'Order ID', 'fincommerce' ),
			'customer_email' => __( 'Customer Email', 'fincommerce' ),
			'customers'      => __( 'Customers', 'fincommerce' ),
			'products'       => __( 'Products', 'fincommerce' ),
			'all'            => __( 'All', 'fincommerce' ),
		);

		/**
		 * Filters the search filters available in the admin order search. Can be used to add new or remove existing filters.
		 * When adding new filters, `fincommerce_hpos_generate_where_for_search_filter` should also be used to generate the WHERE clause for the new filter
		 *
		 * @since 8.9.0.
		 *
		 * @param $options array List of available filters.
		 */
		$options       = apply_filters( 'fincommerce_hpos_admin_search_filters', $options );
