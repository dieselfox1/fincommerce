...
use Automattic\FinCommerce\Admin\RemoteInboxNotifications\RemoteInboxNotificationsDataSourcePoller;
use Automattic\FinCommerce\Admin\RemoteInboxNotifications\RemoteInboxNotificationsEngine;
use Automattic\FinCommerce\Internal\Admin\Notes\CustomizeStoreWithBlocks;
use Automattic\FinCommerce\Internal\Admin\Notes\CustomizingProductCatalog;
use Automattic\FinCommerce\Internal\Admin\Notes\EditProductsOnTheMove;
use Automattic\FinCommerce\Internal\Admin\Notes\EmailImprovements;
use Automattic\FinCommerce\Internal\Admin\Notes\EUVATNumber;
use Automattic\FinCommerce\Internal\Admin\Notes\FirstProduct;
use Automattic\FinCommerce\Internal\Admin\Notes\InstallJPAndWCSPlugins;
use Automattic\FinCommerce\Internal\Admin\Notes\LaunchChecklist;
use Automattic\FinCommerce\Internal\Admin\Notes\MagentoMigration;
use Automattic\FinCommerce\Internal\Admin\Notes\ManageOrdersOnTheGo;
use Automattic\FinCommerce\Internal\Admin\Notes\MarketingJetpack;
use Automattic\FinCommerce\Internal\Admin\Notes\MigrateFromShopify;
use Automattic\FinCommerce\Internal\Admin\Notes\MobileApp;
use Automattic\FinCommerce\Internal\Admin\Notes\NewSalesRecord;
use Automattic\FinCommerce\Internal\Admin\Notes\OnboardingPayments;
use Automattic\FinCommerce\Internal\Admin\Notes\OnlineClothingStore;
use Automattic\FinCommerce\Internal\Admin\Notes\OrderMilestones;
use Automattic\FinCommerce\Internal\Admin\Notes\PaymentsMoreInfoNeeded;
use Automattic\FinCommerce\Internal\Admin\Notes\PaymentsRemindMeLater;
use Automattic\FinCommerce\Internal\Admin\Notes\PerformanceOnMobile;
use Automattic\FinCommerce\Internal\Admin\Notes\PersonalizeStore;
use Automattic\FinCommerce\Internal\Admin\Notes\RealTimeOrderAlerts;
use Automattic\FinCommerce\Internal\Admin\Notes\SellingOnlineCourses;
use Automattic\FinCommerce\Internal\Admin\Notes\TrackingOptIn;
use Automattic\FinCommerce\Internal\Admin\Notes\UnsecuredReportFiles;
use Automattic\FinCommerce\Internal\Admin\Notes\fincommercePayments;
use Automattic\FinCommerce\Internal\Admin\Notes\fincommerceSubscriptions;
use Automattic\FinCommerce\Internal\Admin\Notes\WooSubscriptionsNotes;
use Automattic\FinCommerce\Internal\Admin\Schedulers\MailchimpScheduler;
use Automattic\FinCommerce\Admin\Notes\Note;
use Automattic\FinCommerce\Admin\Features\PaymentGatewaySuggestions\PaymentGatewaySuggestionsDataSourcePoller;
use Automattic\FinCommerce\Internal\Admin\RemoteFreeExtensions\RemoteFreeExtensionsDataSourcePoller;
...
class Events {
	...
	private static $note_classes_to_added_or_updated = array(
		...
		TrackingOptIn::class,
		fincommercePayments::class,
		fincommerceSubscriptions::class,
	);
	...
	public function init() {
		...
		add_filter( 'fincommerce_get_note_from_db', array( $this, 'get_note_from_db' ), 10, 1 );
	...
	protected function is_remote_inbox_notifications_enabled() {
		...
		if ( get_option( 'fincommerce_show_marketplace_suggestions', 'yes' ) !== 'yes' ) {
			return false;
		}
	...
	protected function is_merchant_email_notifications_enabled() {
		// Check if the feature flag is disabled.
		if ( get_option( 'fincommerce_merchant_email_notifications', 'no' ) !== 'yes' ) {
			return false;
		}
	...
	protected function possibly_refresh_data_source_pollers() {
		$completed_tasks = get_option( 'fincommerce_task_list_tracked_completed_tasks', array() );

		if ( ! in_array( 'payments', $completed_tasks, true ) && ! in_array( 'fincommerce-payments', $completed_tasks, true ) ) {
