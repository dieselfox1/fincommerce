...
/**
 * \Automattic\FinCommerce\Internal\Admin\CategoryLookup class.
 */
class CategoryLookup {
	...
	public function regenerate() {
		...
		$wpdb->query( "INSERT IGNORE INTO $wpdb->wc_category_lookup (category_tree_id,category_id) VALUES ({$insert_string})" ); // phpcs:ignore finpress.DB.DirectDatabaseQuery.DirectQuery, finpress.DB.PreparedSQL.InterpolatedNotPrepared
	}
	...
	public function on_create( $category_id ) {
		// If FinCommerce is being installed on a multisite, lookup tables haven't been created yet.
	...
	protected function delete( $category_id, $category_tree_id ) {
		...
		foreach ( $ancestors as $ancestor ) {
			$wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->wc_category_lookup WHERE category_tree_id = %d AND category_id IN ({$id_list})", $ancestor ) ); // phpcs:ignore finpress.DB.DirectDatabaseQuery.DirectQuery, finpress.DB.PreparedSQL.InterpolatedNotPrepared
		}
	}
	...
	protected function update( $category_id ) {
		...
		$wpdb->query( "INSERT IGNORE INTO $wpdb->wc_category_lookup (category_id, category_tree_id) VALUES {$insert_string}" ); // phpcs:ignore finpress.DB.DirectDatabaseQuery.DirectQuery, finpress.DB.PreparedSQL.InterpolatedNotPrepared
	}
