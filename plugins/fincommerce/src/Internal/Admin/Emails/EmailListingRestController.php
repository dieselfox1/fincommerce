...
namespace Automattic\FinCommerce\Internal\Admin\Emails;
...
use Automattic\FinCommerce\Internal\EmailEditor\WCTransactionalEmails\WCTransactionalEmailPostsGenerator;
use WP_Error;
use WP_REST_Request;
...
class EmailListingRestController extends RestApiControllerBase {
	...
	private function get_args_for_recreate_email_post() {
		return array(
			'email_id' => array(
				'description'       => __( 'The email ID to recreate the post for.', 'fincommerce' ),
				'type'              => 'string',
				'required'          => true,
	...
	private function get_schema_with_message() {
		return array(
			...
			'properties' => array(
				...
				'post_id' => array(
					'description' => __( 'The post ID of the generated email post.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
	...
	private function validate_email_id( string $email_id ) {
		if ( ! in_array( $email_id, WCTransactionalEmails::get_transactional_emails(), true ) ) {
			return new \WP_Error(
				'fincommerce_rest_not_allowed_email_id',
	...
	public function recreate_email_post( WP_REST_Request $request ) {
		...
		try {
			...
			return new WP_Error(
				'fincommerce_rest_email_post_generation_failed',
				// translators: %s: Error message.
				sprintf( __( 'Error generating email post. Error: %s.', 'fincommerce' ), $e->getMessage() ),
				array( 'status' => 500 )
			);
		}
		...
		if ( $generated_post_id ) {
			return array(
				// translators: %s: FinCommerce transactional email ID.
				'message' => sprintf( __( 'Email post generated for %s.', 'fincommerce' ), $email_id ),
				'post_id' => (string) $generated_post_id,
			);
		}
		return new WP_Error(
			'fincommerce_rest_email_post_generation_error',
			__( 'Error unable to generate email post.', 'fincommerce' ),
			array( 'status' => 500 )
		);
	}
}
