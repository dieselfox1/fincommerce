<?php
/**
 * Register the scripts, and styles used within FinCommerce Admin.
 */
...
class WCAdminAssets {
	 ...
	 * or the asset version (file content hash) if exists, or the FinCommerce version.
	 *
	 * @param string      $ext File extension.
	...
	private function modify_script_dependencies( $dependencies, $script ) {
		switch ( $script ) {
			...
			case 'wc-product-editor':
				...
				$is_product_data_view_page = \Automattic\FinCommerce\Admin\Features\ProductDataViews\Init::is_product_data_view_page();
	...
	public function register_scripts() {
		...
		$scripts = array(
			...
			// NOTE: This should be removed when Gutenberg is updated and the notices package is removed from FinCommerce Admin.
			'wc-notices',
			'wc-number',
			'wc-tracks',
			'wc-date',
		...
		foreach ( $scripts as $script ) {
			...
			try {
				...
				if ( in_array( $script, $translated_scripts, true ) ) {
					wp_set_script_translations( $script, 'fincommerce' );
				}
	...
	public function inject_wc_settings_dependencies() {
		...
		if ( wp_script_is( 'wc-settings', 'registered' ) ) {
			$handles_for_injection = array(
				...
				// the notices package is removed from FinCommerce Admin.
				'wc-notices',
				'wc-number',
				'wc-date',
				'wc-components',
