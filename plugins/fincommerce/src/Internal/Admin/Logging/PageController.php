...
use Automattic\FinCommerce\Internal\Admin\Logging\FileV2\{ File, FileController, FileListTable, SearchListTable };
use WC_Admin_Status;
...
class PageController {
	...
	private function init_hooks(): void {
		add_action( 'load-fincommerce_page_wc-status', array( $this, 'maybe_do_logs_tab_action' ), 2 );
					...
					<div class="notice notice-warning">
						<p>
							...
							printf(
								...
								wp_kses_post( __( 'Logging is disabled. It can be enabled in <a href="%s">Logs Settings</a>.', 'fincommerce' ) ),
		...
		<ul class="subsubsub">
			...
			<li>
				...
				printf(
					...
					'settings' === $params['view'] ? ' class="current"' : '',
					esc_html__( 'Settings', 'fincommerce' )
				);
				?>
			</li>
		</ul>
	...
	private function render_list_files_view(): void {
		...
		<header id="logs-header" class="wc-logs-header">
			<h2>
				<?php esc_html_e( 'Browse log files', 'fincommerce' ); ?>
			</h2>
	...
	private function render_single_file_view(): void {
		...
		if ( is_wp_error( $file ) ) {
			...
			<div class="notice notice-error notice-inline">
				...
				printf(
					...
					esc_html__( 'Return to the file list.', 'fincommerce' )
				);
				?>
			</div>
			<?php
		...
		$delete_confirmation_js = sprintf(
			...
			esc_js( __( 'Delete this log file permanently?', 'fincommerce' ) )
		);
		...
		<header id="logs-header" class="wc-logs-header">
			<h2>
				...
				printf(
					...
					esc_html__( 'Viewing log file %s', 'fincommerce' ),
					sprintf(
			...
			<?php if ( count( $rotations ) > 1 ) : ?>
				<nav class="wc-logs-single-file-rotations">
					<h3><?php esc_html_e( 'File rotations:', 'fincommerce' ); ?></h3>
					<ul class="wc-logs-rotation-links">
						<?php if ( isset( $rotations['current'] ) ) : ?>
							...
							printf(
								...
								esc_html__( 'Current', 'fincommerce' )
							);
			...
			<div class="wc-logs-single-file-actions">
				...
				printf(
					...
					esc_html__( 'Download', 'fincommerce' )
				);
				?>
				<?php
				// Delete button.
	...
	private function render_search_results_view(): void {
		...
		?>
		<header id="logs-header" class="wc-logs-header">
			<h2><?php esc_html_e( 'Search results', 'fincommerce' ); ?></h2>
			<?php $this->render_search_field(); ?>
		</header>
		<?php $list_table->display(); ?>
		<?php
	...
	public function handle_list_table_bulk_actions( string $view ): void {
		...
		if ( $action ) {
			...
			if ( ! current_user_can( 'manage_fincommerce' ) ) {
				wp_die( esc_html__( 'You do not have permission to manage log files.', 'fincommerce' ) );
			}
					...
					<div class="notice notice-info is-dismissible">
						<p>
							...
							// translators: %s is a number of files.
								esc_html( _n( '%s log file deleted.', '%s log files deleted.', $deleted, 'fincommerce' ) ),
			...
			if ( isset( $message_chunks[1] ) ) {
				try {
					...
					$message_chunks[1] = sprintf(
						...
						esc_html__( 'Additional context', 'fincommerce' ),
			...
			<form id="logs-search" class="wc-logs-search" method="get">
				<fieldset class="wc-logs-search-fieldset">
					...
					<label for="logs-search-field">
						<?php esc_html_e( 'Search within these files', 'fincommerce' ); ?>
						<input
					...
					<?php submit_button( __( 'Search', 'fincommerce' ), 'secondary', null, false ); ?>
				</fieldset>
