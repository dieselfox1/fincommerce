...
namespace Automattic\FinCommerce\Internal\Admin\Logging;

use Automattic\Jetpack\Constants;
use Automattic\FinCommerce\Internal\Admin\Logging\FileV2\File;
use Automattic\FinCommerce\Internal\Admin\Logging\LogHandlerFileV2;
use Automattic\FinCommerce\Internal\Admin\Logging\FileV2\FileController;
...
class Settings {
	 ...
	 * @const string
	 */
	private const PREFIX = 'fincommerce_logs_';
	...
	public static function get_log_directory( bool $create_dir = true ): string {
		if ( true === Constants::get_constant( 'WC_LOG_DIR_CUSTOM' ) ) {
			...
			/**
			 * Filter to change the directory for storing FinCommerce's log files.
			 *
			 * @param string $dir The full directory path, with trailing slash.
			 *
			 * @since 8.8.0
			 */
			$dir = apply_filters( 'fincommerce_log_directory', $upload_dir['basedir'] . '/wc-logs/' );
		}
	...
	private function get_settings_definitions(): array {
		$settings = array(
			'start'                 => array(
				'title' => __( 'Logs settings', 'fincommerce' ),
				'id'    => self::PREFIX . 'settings',
				'type'  => 'title',
			),
			'logging_enabled'       => array(
				'title'    => __( 'Logger', 'fincommerce' ),
				'desc'     => __( 'Enable logging', 'fincommerce' ),
	...
	private function get_default_handler_setting_definition(): array {
		$handler_options = array(
			LogHandlerFileV2::class  => __( 'File system (default)', 'fincommerce' ),
			WC_Log_Handler_DB::class => __( 'Database (not recommended on live sites)', 'fincommerce' ),
		);
		 ...
		 * @since 8.6.0
		 */
		$handler_options = apply_filters( 'fincommerce_logger_handler_options', $handler_options );
		...
		$desc[] = __( 'Note that if this setting is changed, any log entries that have already been recorded will remain stored in their current location, but will not migrate.', 'fincommerce' );

		$hardcoded = ! is_null( Constants::get_constant( 'WC_LOG_HANDLER' ) );
		if ( $hardcoded ) {
			$desc[] = sprintf(
				// translators: %s is the name of a code variable.
				__( 'This setting cannot be changed here because it is defined in the %s constant.', 'fincommerce' ),
				'<code>WC_LOG_HANDLER</code>'
			);
		}

		return array(
			'title'       => __( 'Log storage', 'fincommerce' ),
			'desc_tip'    => __( 'This determines where log entries are saved.', 'fincommerce' ),
	...
	private function get_retention_period_days_setting_definition(): array {
		...
		$hardcoded = has_filter( 'fincommerce_logger_days_to_retain_logs' );
		if ( $hardcoded ) {
			$custom_attributes['disabled'] = 'true';

			$desc[] = sprintf(
				// translators: %s is the name of a filter hook.
				__( 'This setting cannot be changed here because it is being set by a filter on the %s hook.', 'fincommerce' ),
				'<code>fincommerce_logger_days_to_retain_logs</code>'
			);
		}

		$file_delete_has_filter = LogHandlerFileV2::class === $this->get_default_handler() && has_filter( 'fincommerce_logger_delete_expired_file' );
		if ( $file_delete_has_filter ) {
			$desc[] = sprintf(
				// translators: %s is the name of a filter hook.
				__( 'The %s hook has a filter set, so some log files may have different retention settings.', 'fincommerce' ),
				'<code>fincommerce_logger_delete_expired_file</code>'
			);
		}

		return array(
			'title'             => __( 'Retention period', 'fincommerce' ),
			'desc_tip'          => __( 'This sets how many days log entries will be kept before being auto-deleted.', 'fincommerce' ),
	...
	private function get_level_threshold_setting_definition(): array {
		...
		if ( $hardcoded ) {
			$desc = sprintf(
				...
				__( 'This setting cannot be changed here because it is defined in the %1$s constant, probably in your %2$s file.', 'fincommerce' ),
		...
		$labels['none'] = __( 'None', 'fincommerce' );
		...
		return array(
			'title'             => __( 'Level threshold', 'fincommerce' ),
			'desc_tip'          => __( 'This sets the minimum severity level of logs that will be stored. Lower severity levels will be ignored. "None" means all logs will be stored.', 'fincommerce' ),
	...
	private function get_filesystem_settings_definitions(): array {
		...
		try {
			...
			if ( $filesystem instanceof WP_Filesystem_Direct ) {
				$status_info[] = __( '✅ Ready', 'fincommerce' );
			} else {
				$status_info[] = __( '⚠️ The file system is not configured for direct writes. This could cause problems for the logger.', 'fincommerce' );
				$status_info[] = __( 'You may want to switch to the database for log storage.', 'fincommerce' );
			}
		} catch ( Exception $exception ) {
			$status_info[] = __( '⚠️ The file system connection could not be initialized.', 'fincommerce' );
			$status_info[] = __( 'You may want to switch to the database for log storage.', 'fincommerce' );
		}

		$location_info[] = sprintf(
			// translators: %s is a location in the filesystem.
			__( 'Log files are stored in this directory: %s', 'fincommerce' ),
			sprintf(
				'<code>%s</code>',
				esc_html( $directory )
			)
		...
		if ( ! wp_is_writable( $directory ) ) {
			$location_info[] = __( '⚠️ This directory does not appear to be writable.', 'fincommerce' );
		}

		$location_info[] = sprintf(
			// translators: %s is an amount of computer disk space, e.g. 5 KB.
			__( 'Directory size: %s', 'fincommerce' ),
			size_format( wc_get_container()->get( FileController::class )->get_log_directory_size() )
		);

		return array(
			'file_start'    => array(
				'title' => __( 'File system settings', 'fincommerce' ),
				'id'    => self::PREFIX . 'settings',
				'type'  => 'title',
			),
			'file_status'   => array(
	...
	private function get_database_settings_definitions(): array {
		global $wpdb;
		$table = "{$wpdb->prefix}fincommerce_log";

		$location_info = sprintf(
			// translators: %s is the name of a table in the database.
			__( 'Log entries are stored in this database table: %s', 'fincommerce' ),
			"<code>$table</code>"
		);

		return array(
			'file_start'     => array(
				'title' => __( 'Database settings', 'fincommerce' ),
				'id'    => self::PREFIX . 'settings',
				'type'  => 'title',
			),
	...
	public function save_settings( string $view ): void {
		...
		if ( $is_saving ) {
			...
			if ( ! current_user_can( 'manage_fincommerce' ) ) {
				wp_die( esc_html__( 'You do not have permission to manage logging settings.', 'fincommerce' ) );
			}
	...
	public function render_form(): void {
		...
		<form id="mainform" class="wc-logs-settings" method="post">
			...
			<?php submit_button( __( 'Save changes', 'fincommerce' ), 'primary', 'save_settings' ); ?>
		</form>
		<?php
	}
	...
	public function get_retention_period(): int {
		...
		if ( has_filter( 'fincommerce_logger_days_to_retain_logs' ) ) {
			/**
			 ...
			 * @since 3.4.0
			 */
			$retention_period = apply_filters( 'fincommerce_logger_days_to_retain_logs', $retention_period );
		} else {
