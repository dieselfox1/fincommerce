...
class FileController {
	...
	private function get_file_size_limit(): int {
		 ...
		 */
		$file_size_limit = apply_filters( 'fincommerce_log_file_size_limit', $default );
	...
	public function get_files( array $args = array(), bool $count_only = false ) {
		...
		if ( false === $paths ) {
			return new WP_Error(
				...
				__( 'Could not access the log file directory.', 'fincommerce' )
			);
		}
	...
	public function get_file_by_id( string $file_id ) {
		...
		if ( count( $result ) < 1 ) {
			return new WP_Error(
				'wc_log_file_error',
				esc_html__( 'This file does not exist.', 'fincommerce' )
			);
		}
		...
		if ( count( $result ) > 1 ) {
			return new WP_Error(
				'wc_log_file_error',
				esc_html__( 'Multiple files match this ID.', 'fincommerce' )
			);
		}
	...
	public function export_multiple_files( array $file_ids ) {
		...
		if ( count( $files ) < 1 ) {
			return new WP_Error(
				...
				__( 'Could not access the specified files.', 'fincommerce' )
			);
		}
		...
		if ( ! is_dir( $temp_dir ) || ! wp_is_writable( $temp_dir ) ) {
			return new WP_Error(
				...
				__( 'Could not write to the temp directory. Try downloading files one at a time instead.', 'fincommerce' )
			);
		}

		require_once ABSPATH . 'wp-admin/includes/class-pclzip.php';

		$path       = trailingslashit( $temp_dir ) . 'fincommerce_logs_' . gmdate( 'Y-m-d_H-i-s' ) . '.zip';
