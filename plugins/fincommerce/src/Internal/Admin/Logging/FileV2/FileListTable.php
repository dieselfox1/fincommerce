...
namespace Automattic\FinCommerce\Internal\Admin\Logging\FileV2;

use Automattic\FinCommerce\Internal\Admin\Logging\PageController;
...
class FileListTable extends WP_List_Table {
	 ...
	 * @const string
	 */
	public const PER_PAGE_USER_OPTION_KEY = 'fincommerce_logging_file_list_per_page';
	...
	public function no_items(): void {
		esc_html_e( 'No log files found.', 'fincommerce' );
	}
	...
	protected function get_bulk_actions(): array {
		return array(
			'export' => esc_html__( 'Download', 'fincommerce' ),
			'delete' => esc_html__( 'Delete permanently', 'fincommerce' ),
		);
	}
	...
	protected function extra_tablenav( $which ): void {
		...
		<div class="alignleft actions">
			<?php if ( 'top' === $which ) : ?>
				<label for="filter-by-source" class="screen-reader-text"><?php esc_html_e( 'Filter by log source', 'fincommerce' ); ?></label>
				<select name="source" id="filter-by-source">
					<option<?php selected( $current_source, '' ); ?> value=""><?php esc_html_e( 'All sources', 'fincommerce' ); ?></option>
	...
	public function get_columns(): array {
		$columns = array(
			...
			'source'   => esc_html__( 'Source', 'fincommerce' ),
			'created'  => esc_html__( 'Date created', 'fincommerce' ),
			'modified' => esc_html__( 'Date modified', 'fincommerce' ),
			'size'     => esc_html__( 'File size', 'fincommerce' ),
		);
		...
		<label for="cb-select-<?php echo esc_attr( $item->get_file_id() ); ?>">
			<span class="screen-reader-text">
				...
				printf(
					...
					esc_html__( 'Select the %1$s log file for %2$s', 'fincommerce' ),
