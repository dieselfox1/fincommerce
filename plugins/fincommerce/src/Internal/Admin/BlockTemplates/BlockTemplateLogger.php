...
class BlockTemplateLogger {
	...
	protected function __construct() {
		...
		$threshold = get_option( 'fincommerce_block_template_logging_threshold', \WC_Log_Levels::WARNING );
		...
		add_action(
			'fincommerce_block_template_after_add_hide_condition',
			function ( BlockInterface $block ) {
				$this->log(
					$block->is_detached()
		...
		add_action(
			'fincommerce_block_template_after_remove_hide_condition',
			function ( BlockInterface $block ) {
				$this->log(
	...
	private function log( string $event_type, BlockInterface $block, $additional_info = array() ) {
		if ( ! isset( self::$event_types[ $event_type ] ) ) {
			...
			wc_doing_it_wrong( __METHOD__, sprintf( __( '%1$s was called with an invalid event type "%2$s".', 'fincommerce' ), '<code>BlockTemplateLogger::log</code>', $event_type ), '8.4' );
		}
