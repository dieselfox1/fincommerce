...
trait BlockContainerTrait {
	...
	/**
	 * Do the `fincommerce_block_template_after_add_block` action.
	...
	private function do_after_add_block_action( BlockInterface $block ) {
		try {
			 ...
			 * @since 8.2.0
			 */
			do_action( 'fincommerce_block_template_after_add_block', $block );
		} catch ( \Exception $e ) {
			$this->do_after_add_block_error_action( $block, 'fincommerce_block_template_after_add_block', $e );
		}
	}

	/**
	 * Do the `fincommerce_block_template_area_{template_area}_after_add_block_{block_id}` action.
	 * Handle exceptions thrown by the action.
	 *
	 * @param BlockInterface $block The block.
	 */
	private function do_after_add_specific_block_action( BlockInterface $block ) {
		try {
			 ...
			 * @since 8.2.0
			 */
			do_action( "fincommerce_block_template_area_{$this->get_root_template()->get_area()}_after_add_block_{$block->get_id()}", $block );
		} catch ( \Exception $e ) {
			$this->do_after_add_block_error_action( $block, "fincommerce_block_template_area_{$this->get_root_template()->get_area()}_after_add_block_{$block->get_id()}", $e );
		}
	}

	/**
	 * Do the `fincommerce_block_after_add_block_error` action.
	 *
	 * @param BlockInterface $block The block.
	...
	private function do_after_add_block_error_action( BlockInterface $block, string $action, \Exception $e ) {
		/**
		 * Action called after an exception is thrown by a `fincommerce_block_template_after_add_block` action hook.
		 *
		 * @param BlockInterface $block The block.
		 * @param string         $action The action that threw the exception.
		 ...
		 */
		do_action(
			'fincommerce_block_template_after_add_block_error',
			$block,
			$action,
			$e,
		);
	...
	/**
	 * Do the `fincommerce_block_template_after_remove_block` action.
	 * Handle exceptions thrown by the action.
	 *
	 * @param BlockInterface $block The block.
	 */
	private function do_after_remove_block_action( BlockInterface $block ) {
		try {
			 ...
			 * @since 8.2.0
			 */
			do_action( 'fincommerce_block_template_after_remove_block', $block );
		} catch ( \Exception $e ) {
			$this->do_after_remove_block_error_action( $block, 'fincommerce_block_template_after_remove_block', $e );
		}
	}

	/**
	 * Do the `fincommerce_block_template_area_{template_area}_after_remove_block_{block_id}` action.
	 * Handle exceptions thrown by the action.
	 *
	 * @param BlockInterface $block The block.
	 */
	private function do_after_remove_specific_block_action( BlockInterface $block ) {
		try {
			 ...
			 * @since 8.2.0
			 */
			do_action( "fincommerce_block_template_area_{$this->get_root_template()->get_area()}_after_remove_block_{$block->get_id()}", $block );
		} catch ( \Exception $e ) {
			$this->do_after_remove_block_error_action( $block, "fincommerce_block_template_area_{$this->get_root_template()->get_area()}_after_remove_block_{$block->get_id()}", $e );
		}
	}

	/**
	 * Do the `fincommerce_block_after_remove_block_error` action.
	 *
	 * @param BlockInterface $block The block.
	 * @param string         $action The action that threw the exception.
	 * @param \Exception     $e The exception.
	...
	private function do_after_remove_block_error_action( BlockInterface $block, string $action, \Exception $e ) {
		/**
		 * Action called after an exception is thrown by a `fincommerce_block_template_after_remove_block` action hook.
		 *
		 * @param BlockInterface $block The block.
		 * @param string         $action The action that threw the exception.
		 * @param \Exception     $exception The exception.
		 ...
		 */
		do_action(
			'fincommerce_block_template_after_remove_block_error',
			$block,
			$action,
			$e,
		);
