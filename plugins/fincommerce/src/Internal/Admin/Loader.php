<?php
/**
 * Register the scripts, styles, and includes needed for pieces of the FinCommerce Admin experience.
 */

namespace Automattic\FinCommerce\Internal\Admin;

use Automattic\FinCommerce\Admin\API\Reports\Orders\DataStore as OrdersDataStore;
...
use Automattic\FinCommerce\Internal\Admin\ProductReviews\Reviews;
use Automattic\FinCommerce\Internal\Admin\ProductReviews\ReviewsCommentsOverrides;
...
class Loader {
	...
	public function __construct() {
		...
		// Added this hook to delete the field fincommerce_onboarding_homepage_post_id when deleting the homepage.
	...
	public static function embed_page_header() {
		...
		?>
		<div id="fincommerce-embedded-root" class="is-embed-loading">
			<div class="fincommerce-layout">
				<div class="fincommerce-layout__header is-embed-loading">
					<h1 class="fincommerce-layout__header-heading">
	...
	public static function add_admin_body_classes( $admin_body_class = '' ) {
		...
		$classes[] = 'fincommerce-admin-page';
		if ( PageController::is_embed_page() ) {
			$classes[] = 'fincommerce-embed-page';
		}
		 ...
		 * Setting `fincommerce_admin_is_loading` to true allows us to premeptively hide these
		 * elements while the JS app loads.
		 * This class needs to be removed by those feature components (like <ProfileWizard />).
		 *
		 * @param bool $is_loading If FinCommerce Admin is loading a fullscreen view.
		 * @since 6.5.0
		 */
		$is_loading = apply_filters( 'fincommerce_admin_is_loading', false );

		if ( PageController::is_admin_page() && $is_loading ) {
			$classes[] = 'fincommerce-admin-is-loading';
		}
	...
	public static function update_admin_title( $admin_title ) {
		...
		return sprintf( __( '%1$s &lsaquo; %2$s', 'fincommerce' ), $title, get_bloginfo( 'name' ) );
	}
	...
	/**
	 * Hooks extra necessary data into the component settings array already set in FinCommerce core.
	 *
	...
	public static function add_component_settings( $settings ) {
		...
		/**
		 * The fincommerce_component_settings_preload_endpoints filter
		 *
		 * @since 6.5.0
		 */
		$preload_data_endpoints = apply_filters( 'fincommerce_component_settings_preload_endpoints', array() );
		...
		/**
		 * The fincommerce_admin_preload_options filter
		 *
		 * @since 6.5.0
		 */
		$preload_options = apply_filters( 'fincommerce_admin_preload_options', array() );
		...
		/**
		 * The fincommerce_admin_preload_settings filter
		 *
		 * @since 6.5.0
		 */
		$preload_settings = apply_filters( 'fincommerce_admin_preload_settings', array() );
		...
		$settings['reviewsEnabled']       = get_option( 'fincommerce_enable_reviews' );
		$settings['manageStock']          = get_option( 'fincommerce_manage_stock' );
		$settings['commentModeration']    = get_option( 'comment_moderation' );
		$settings['notifyLowStockAmount'] = get_option( 'fincommerce_notify_low_stock_amount' );
		// @todo On merge, once plugin images are added to core FinCommerce, `wcAdminAssetUrl` can be retired,
		...
		// FinCommerce Branding is an example of this - so pass through the translation of
		// 'FinCommerce' to wcSettings.
		$settings['fincommerceTranslation'] = __( 'FinCommerce', 'fincommerce' );
		...
		/* phpcs:ignore */
		$settings['variationTitleAttributesSeparator'] = apply_filters( 'fincommerce_product_variation_title_attributes_separator', ' - ', new \WC_Product() );
	 ...
	 *
	 * @deprecated migrate to \Automattic\FinCommerce\Internal\Admin\Settings instead.
	 */
	public static function get_order_statuses( $statuses ) {
		wc_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '9.9.0', '\Automattic\FinCommerce\Internal\Admin\Settings::get_order_statuses' );
	...
	public static function add_settings_group( $groups ) {
		wc_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '9.9.0', '\Automattic\FinCommerce\Internal\Admin\Settings::add_settings_group' );
	...
	public static function add_settings( $settings ) {
		wc_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '9.9.0', '\Automattic\FinCommerce\Internal\Admin\Settings::add_settings' );
	...
	public static function get_currency_settings() {
		wc_deprecated_function( __CLASS__ . '::' . __FUNCTION__, '9.9.0', '\Automattic\FinCommerce\Internal\Admin\Settings::get_currency_settings' );

		return Settings::get_currency_settings();
	}

	/**
	 * Delete fincommerce_onboarding_homepage_post_id field when the homepage is deleted
	 *
	...
	public static function delete_homepage( $post_id ) {
		...
		$homepage_id = intval( get_option( 'fincommerce_onboarding_homepage_post_id', false ) );
		if ( $homepage_id === $post_id ) {
			delete_option( 'fincommerce_onboarding_homepage_post_id' );
		}
	}
