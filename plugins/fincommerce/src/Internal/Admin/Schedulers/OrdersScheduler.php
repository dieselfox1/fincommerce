...
namespace Automattic\FinCommerce\Internal\Admin\Schedulers;
...
use Automattic\FinCommerce\Admin\API\Reports\Orders\DataStore as OrderDataStore;
...
use Automattic\FinCommerce\Admin\API\Reports\Taxes\DataStore as TaxesDataStore;
...
class OrdersScheduler extends ImportScheduler {
	...
	public static function init() {
		...
		\Automattic\FinCommerce\Admin\Overrides\Order::add_filters();
		\Automattic\FinCommerce\Admin\Overrides\OrderRefund::add_filters();

		// Order and refund data must be run on these hooks to ensure meta data is set.
		add_action( 'fincommerce_update_order', array( __CLASS__, 'possibly_schedule_import' ) );
		add_filter( 'fincommerce_create_order', array( __CLASS__, 'possibly_schedule_import' ) );
		add_action( 'fincommerce_refund_created', array( __CLASS__, 'possibly_schedule_import' ) );
		add_action( 'fincommerce_schedule_import', array( __CLASS__, 'possibly_schedule_import' ) );
	...
	public static function get_dependencies() {
		return array(
			'import_batch_init' => \Automattic\FinCommerce\Internal\Admin\Schedulers\CustomersScheduler::get_action( 'import_batch_init' ),
		);
	}
...
"
	...
	public static function possibly_schedule_import( $order_id ) {
		if ( ! OrderUtil::is_order( $order_id, array( 'shop_order' ) ) && 'fincommerce_refund_created' !== current_filter() && 'fincommerce_schedule_import' !== current_filter() ) {
			return $order_id;
		}
