...
trait CogsAwareRestControllerTrait {
	...
	private function add_cogs_related_product_schema( array $schema, bool $for_variations_controller ): array {
		$schema['properties']['cost_of_goods_sold'] = array(
			...
			'properties'  => array(
				'values'                    => array(
					'description' => __( 'Cost of Goods Sold values for the product.', 'fincommerce' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'items'       => array(
						'type'       => 'object',
						'properties' => array(
							'defined_value'   => array(
								'description' => __( 'Defined cost value.', 'fincommerce' ),
								'type'        => 'number',
								'context'     => array( 'view', 'edit' ),
							),
							'effective_value' => array(
								'description' => __( 'Effective monetary cost value.', 'fincommerce' ),
								'type'        => 'number',
								'context'     => array( 'view', 'edit' ),
								'readonly'    => true,
							),
				...
				'defined_value_is_additive' => array(
					'description' => __( 'Applies to variations only. If true, the effective value is the base value from the parent product plus the defined value; if false, the defined value is the final effective value.', 'fincommerce' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
				'total_value'               => array(
					'description' => __( 'Total monetary value of the Cost of Goods Sold for the product (sum of all the effective values).', 'fincommerce' ),
					'type'        => 'number',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
		...
		if ( $for_variations_controller ) {
			$schema['properties']['cost_of_goods_sold']['properties']['defined_value_is_additive']['description'] =
				__( 'If true, the effective value is the base value from the parent product plus the defined value; if false, the defined value is the final effective value.', 'fincommerce' );
		}
