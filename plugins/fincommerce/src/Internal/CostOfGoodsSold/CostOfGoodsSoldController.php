...
namespace Automattic\FinCommerce\Internal\CostOfGoodsSold;
...
class CostOfGoodsSoldController implements RegisterHooksInterface {
	...
	public function register() {
		add_filter( 'fincommerce_debug_tools', array( $this, 'add_debug_tools_entry' ), 999, 1 );
	}
	...
	public function add_feature_definition( $features_controller ) {
		$definition = array(
			'description'        => __( 'Allows entering cost of goods sold information for products.', 'fincommerce' ),
		...
		$features_controller->add_feature_definition(
			'cost_of_goods_sold',
			__( 'Cost of Goods Sold', 'fincommerce' ),
			$definition
		);
	}

	/**
	 * Add the entry for "add/remove COGS value column to/from the product meta lookup table" to the FinCommerce admin tools.
	 *
	...
	public function add_debug_tools_entry( array $tools_array ): array {
		...
		$tools_array['generate_cogs_value_meta_column'] = array(
			'name'     => $column_exists ?
				__( 'Remove COGS columns from the product meta lookup table', 'fincommerce' ) :
				__( 'Create COGS columns in the product meta lookup table', 'fincommerce' ),
			'button'   => $column_exists ?
				__( 'Remove columns', 'fincommerce' ) :
				__( 'Create columns', 'fincommerce' ),
			'desc'     =>
				$column_exists ?
				__( 'This tool will remove the Cost of Goods Sold (COGS) related columns from the product meta lookup table. COGS will continue working (if the feature is enabled) but some functionality will not be available.', 'fincommerce' ) :
				__( 'This tool will generate the necessary Cost of Goods Sold (COGS) related columns in the product meta lookup table, and populate them from existing product data.', 'fincommerce' ),
			'callback' =>
	...
	public function get_general_cost_edit_field_tooltip( bool $for_variable_products ) {
		return $for_variable_products ?
			__( 'Add the amount it costs you to buy or make this product. This will be applied as the default value for variations.', 'fincommerce' ) :
			__( 'Add the amount it costs you to buy or make this product.', 'fincommerce' );
	}
}
