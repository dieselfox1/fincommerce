...
use Automattic\FinCommerce\Internal\McStats;
use Jetpack_Options;
use WC_Rate_Limiter;
use WC_Log_Levels;
...
/**
 * FinCommerce Remote Logger
 *
 * The FinCommerce remote logger class adds functionality to log FinCommerce errors remotely based on if the customer opted in and several other conditions.
 *
...
class RemoteLogger extends \WC_Log_Handler {
	...
	public function get_formatted_log( $level, $message, $context = array() ) {
		 ...
		 */
		return apply_filters( 'fincommerce_remote_logger_formatted_log_data', $log_data, $level, $message, $context );
	}
	 ...
	 * 4. The current FinCommerce version is the latest so we don't log errors that might have been fixed in a newer version.
	 *
	 * @return bool
	 */
	...
	/**
	 * Check if the current FinCommerce version is the latest.
	 *
	 * @return bool
	 */
	private function should_current_version_be_logged() {
		...
		if ( false === $new_version ) {
			$new_version = $this->fetch_new_fincommerce_version();
	...
	/**
	 * Get the current FinCommerce version reliably through a series of fallbacks
	 *
	 * @return string The current FinCommerce version.
	 */
	...
	protected function is_third_party_error( string $message, array $context ): bool {
		...
		// Check if the error message contains the FinCommerce plugin directory.
		...
		if ( isset( $context['backtrace'] ) && is_array( $context['backtrace'] ) ) {
			...
			// Check if the relevant frame is from FinCommerce.
		 ...
		 * @param array  $context              The error context.
		 */
		return apply_filters( 'fincommerce_remote_logging_is_third_party_error', true, $message, $context );
	}
	 ...
	 */
	private function fetch_new_fincommerce_version() {
		$plugin_updates = SafeGlobalFunctionProxy::get_plugin_updates();

		// Check if FinCommerce plugin update information is available.
	 ...
	 *
	 * /var/www/html/wp-content/plugins/fincommerce/includes/class-wc-remote-logger.php on line 123
	 * will be sanitized to: **\/fincommerce/includes/class-wc-remote-logger.php on line 123
	 *
	...
	private function sanitize( $content ) {
		 ...
		 */
		return apply_filters( 'fincommerce_remote_logger_sanitized_content', $sanitized, $content );
	}
	...
	private function sanitize_request_uri( $request_uri ) {
		 ...
		 */
		$whitelist = apply_filters( 'fincommerce_remote_logger_request_uri_whitelist', $default_whitelist );
