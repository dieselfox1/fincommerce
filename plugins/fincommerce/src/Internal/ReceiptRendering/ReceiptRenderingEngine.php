...
namespace Automattic\FinCommerce\Internal\ReceiptRendering;
...
class ReceiptRenderingEngine {
	...
	public function generate_receipt( $order, $expiration_date = null, bool $force_new = false ): ?string {
		 ...
		 * The formatted line items aren't included, use the fincommerce_printable_order_receipt_formatted_line_item
		 ...
		 * @since 9.0.0
		 */
		$data = apply_filters( 'fincommerce_printable_order_receipt_data', $this->get_order_data( $order ), $order );
		...
		foreach ( $data['line_items'] as $line_item_data ) {
			 ...
			 * @since 9.0.0
			 */
			$line_item_display_data = apply_filters( 'fincommerce_printable_order_receipt_line_item_display_data', $line_item_display_data, $line_item_data, $order );
			$attributes             = '';
		 ...
		 * @since 9.0.0
		 */
		$data['css'] = apply_filters( 'fincommerce_printable_order_receipt_css', $css, $order );
	...
	private function get_order_data( WC_Abstract_Order $order ): array {
		...
		if ( $store_name ) {
			...
			$receipt_title = sprintf( __( 'Receipt from %s', 'fincommerce' ), $store_name );
		} else {
			$receipt_title = __( 'Receipt', 'fincommerce' );
		}

		$order_id = $order->get_id();
		if ( $order_id ) {
			/* translators: %d = order id */
			$summary_title = sprintf( __( 'Summary: Order #%d', 'fincommerce' ), $order->get_id() );
		} else {
			$summary_title = __( 'Summary', 'fincommerce' );
		}
		...
		if ( ! empty( $coupon_names ) ) {
			$line_items_info[] = array(
				...
				'title'  => sprintf( __( 'Discount (%s)', 'fincommerce' ), join( ', ', $coupon_names ) ),
		...
		foreach ( $order->get_fees() as $fee ) {
			...
			$line_items_info[] = array(
				'type'   => 'fee',
				'title'  => '' === $name ? __( 'Fee', 'fincommerce' ) : $name,
		...
		$line_items_info[] = array(
			'type'   => 'amount_paid',
			'title'  => $is_order_failed ? __( 'Amount', 'fincommerce' ) : __( 'Amount Paid', 'fincommerce' ),
		...
		return array(
			...
			'texts'                     => array(
				...
				'amount_paid_section_title'    => $is_order_failed ? __( 'Order Total', 'fincommerce' ) : __( 'Amount Paid', 'fincommerce' ),
				'date_paid_section_title'      => $is_order_failed ? __( 'Order Date', 'fincommerce' ) : __( 'Date Paid', 'fincommerce' ),
				'payment_method_section_title' => __( 'Payment method', 'fincommerce' ),
				'payment_status_section_title' => __( 'Payment status', 'fincommerce' ),
				'payment_status'               => $is_order_failed ? __( 'Failed', 'fincommerce' ) : __( 'Success', 'fincommerce' ),
				'summary_section_title'        => $summary_title,
				'order_notes_section_title'    => __( 'Notes', 'fincommerce' ),
				'app_name'                     => __( 'Application Name', 'fincommerce' ),
				'aid'                          => __( 'AID', 'fincommerce' ),
				'account_type'                 => __( 'Account Type', 'fincommerce' ),
			),
	...
	/**
	 * Get the order data related to FinCommerce Payments.
	 *
	 * It will return null if any of these is true:
	 *
	 * - Payment method is not "fincommerce_payments".
	 * - FinCommerce Payments is not installed.
