...
class ReceiptRenderingRestController extends RestApiControllerBase {
	...
	public function get_order_receipt( WP_REST_Request $request ) {
		...
		return is_null( $filename ) ?
			new WP_Error( 'fincommerce_rest_not_found', __( 'Receipt not found', 'fincommerce' ), array( 'status' => 404 ) ) :
	...
	public function create_order_receipt( WP_REST_Request $request ) {
		...
		return is_null( $filename ) ?
			new WP_Error( 'fincommerce_rest_not_found', __( 'Order not found', 'fincommerce' ), array( 'status' => 404 ) ) :
			$this->get_response_for_file( $filename );
	}
	...
	private function get_schema_for_get_and_post_order_receipt(): array {
		...
		$schema['properties'] = array(
			'receipt_url'     => array(
				'description' => __( 'Public url of the receipt.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			),
			'expiration_date' => array(
				'description' => __( 'Expiration date of the receipt, formatted as yyyy-mm-dd.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			),
	...
	private function get_args_for_create_order_receipt(): array {
		return array(
			...
			'expiration_date' => array(
				'description' => __( 'Expiration date formatted as yyyy-mm-dd.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
				'default'     => null,
			...
			'expiration_days' => array(
				'description' => __( 'Number of days to be added to the current date to get the expiration date.', 'fincommerce' ),
				'type'        => 'integer',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
				'default'     => 1,
			...
			'force_new'       => array(
				'description' => __( 'True to force the creation of a new receipt even if one already exists and has not expired yet.', 'fincommerce' ),
				'type'        => 'boolean',
				'required'    => false,
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
