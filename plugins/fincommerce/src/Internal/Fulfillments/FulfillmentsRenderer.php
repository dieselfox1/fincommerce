<?php
/**
 * FinCommerce order fulfillments renderer script.
 */
...
class FulfillmentsRenderer {
	...
	public function register() {
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			...
			add_filter( 'manage_fincommerce_page_wc-orders_columns', array( $this, 'add_fulfillment_columns' ) );
			// Hook into the column rendering and render the new fulfillment columns.
			add_action( 'manage_fincommerce_page_wc-orders_custom_column', array( $this, 'render_fulfillment_column_row_data' ), 10, 2 );
		} else {
		...
		add_action( 'fincommerce_admin_order_data_header_right', array( $this, 'render_order_details_badges' ) );
		// Hook into the order details before order table to render the fulfillment customer details.
		add_action( 'fincommerce_order_details_before_order_table', array( $this, 'render_fulfillment_customer_details' ) );
		...
		add_filter( 'fincommerce_order_details_status', array( $this, 'render_fulfillment_status_text' ), 10, 2 );
		add_filter( 'fincommerce_order_tracking_status', array( $this, 'render_fulfillment_status_text' ), 10, 2 );
	}
	...
	public function init_admin_hooks() {
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			...
			add_filter( 'bulk_actions-fincommerce_page_wc-orders', array( $this, 'define_fulfillment_bulk_actions' ) );
			add_filter( 'handle_bulk_actions-fincommerce_page_wc-orders', array( $this, 'handle_fulfillment_bulk_actions' ), 10, 3 );
			// For custom orders table, we need to filter the query to include fulfillment status.
			add_action( 'fincommerce_order_list_table_restrict_manage_orders', array( $this, 'render_fulfillment_filters' ) );
			add_filter( 'fincommerce_order_query_args', array( $this, 'filter_orders_list_table_query' ), 10, 1 );
		} else {
	...
	public function add_fulfillment_columns( $columns ) {
		...
		foreach ( $columns as $column_name => $column_info ) {
			...
			if ( 'order_status' === $column_name ) {
				...
				$new_columns['fulfillment_status'] = __( 'Fulfillment Status', 'fincommerce' );
				$new_columns['shipment_tracking']  = __( 'Shipment Tracking', 'fincommerce' );
				$new_columns['shipment_provider']  = __( 'Shipment Provider', 'fincommerce' );
			}
		}
		return $new_columns;
	}
	...
	private function render_order_fulfillment_status_badge( $order, string $order_fulfillment_status ) {
		...
		echo "<a href='#' class='fulfillments-trigger' data-order-id='" . esc_attr( $order->get_id() ) . "' title='" . esc_attr__( 'View Fulfillments', 'fincommerce' ) . "'>
	...
	private function render_shipment_provider_column_row_data( WC_Order $order, array $fulfillments ) {
		...
		if ( count( $providers ) > 1 ) {
			echo '<span>' . esc_html__( 'Multiple providers', 'fincommerce' ) . '</span>';
	...
	private function render_shipment_tracking_column_row_data( WC_Order $order, array $fulfillments ) {
		...
		if ( count( $tracking ) > 1 ) {
			echo '<span>' . esc_html__( 'Multiple trackings', 'fincommerce' ) . '</span>';
	...
	public function define_fulfillment_bulk_actions( $actions ) {
		$actions['fulfill'] = __( 'Mark as fulfilled', 'fincommerce' );
	...
	public function render_fulfillment_customer_details( WC_Order $order ) {
		...
		if ( ! empty( $fulfillments ) ) {
			?>
<section class="fincommerce-order-details">
	<table class="fincommerce-table fincommerce-table--order-details shop_table order_details">
		<thead>
			<?php
			foreach ( $fulfillments as $index => $fulfillment ) {
				...
				?>
			<tr>
				<th class="fincommerce-table__shipment-info shipment-info" style="font-weight: normal;">
				<?php
				printf(
					/* translators: %1$s is the shipment index, %2$s is the shipment date */
					wp_kses( __( '<b>Shipment %1$s</b> was shipped on <b>%2$s</b>', 'fincommerce' ), 'b' ),
					intval( $index ) + 1,
					esc_html(
						gmdate(
							'F j, Y',
				...
				<th class="fincommerce-table__shipment-tracking shipment-tracking" style="font-weight: normal;">
	...
	protected function load_fulfillments_js_settings() {
		$fulfillment_settings = array(
			...
			'currency_symbols'           => get_fincommerce_currency_symbols(),
	...
	protected function should_render_fulfillment_drawer(): bool {
		...
		return 'fincommerce_page_wc-orders' === $current_screen->id // HPOS screen.
