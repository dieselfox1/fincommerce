...
namespace Automattic\FinCommerce\Internal\Fulfillments;
 ...
 *
 * @package Automattic\FinCommerce\Internal\Fulfillments
 */
class OrderFulfillmentsRestController extends RestApiControllerBase {
	...
	protected function check_permission_for_fulfillments( WP_REST_Request $request ) {
		...
		if ( $request->has_param( 'order_id' ) ) {
			...
			if ( ! $order ) {
				return new \WP_Error(
					'fincommerce_rest_order_invalid_id',
					esc_html__( 'Invalid order ID.', 'fincommerce' ),
	...
	public function create_fulfillment( WP_REST_Request $request ) {
		...
		try {
			...
			if ( $fulfillment->get_is_fulfilled() && $notify_customer ) {
				 ...
				 * @since 10.1.0
				 */
				do_action( 'fincommerce_fulfillment_created_notification', $order_id, $fulfillment, wc_get_order( $order_id ) );
			}
	...
	public function get_fulfillment( WP_REST_Request $request ): WP_REST_Response {
		...
		try {
			...
			if ( $fulfillment->get_date_deleted() ) {
				throw new \Exception(
					esc_html__( 'Fulfillment not found.', 'fincommerce' ),
					WP_Http::NOT_FOUND
				);
			}
		} catch ( \Exception $e ) {
		...
		try {
			...
			if ( $notify_customer ) {
				if ( ! $previous_state && $next_state ) {
					 ...
					 * @since 10.1.0
					 */
					do_action( 'fincommerce_fulfillment_updated_notification', $order_id, $fulfillment, wc_get_order( $order_id ) );
				}
			}
		} catch ( ApiException $ex ) {
			return $this->prepare_error_response(
	...
	public function delete_fulfillment( WP_REST_Request $request ) {
		...
		if ( $fulfillment->get_is_fulfilled() && $notify_customer ) {
			 ...
			 * @since 10.1.0
			 */
			do_action( 'fincommerce_fulfillment_deleted_notification', $order_id, $fulfillment, wc_get_order( $order_id ) );
		}
		return new WP_REST_Response(
			array(
				'message' => __( 'Fulfillment deleted successfully.', 'fincommerce' ),
			),
			WP_Http::OK
		);
	}
	...
	public function get_tracking_number_details( WP_REST_Request $request ) {
		...
		if ( empty( $tracking_number ) ) {
			return $this->prepare_error_response(
				'fincommerce_rest_tracking_number_missing',
				__( 'Tracking number is required.', 'fincommerce' ),
		...
		if ( ! $order_id ) {
			return $this->prepare_error_response(
				'fincommerce_rest_order_id_missing',
				__( 'Order ID is required.', 'fincommerce' ),
				array( 'status' => WP_Http::BAD_REQUEST )
			);
		}
		...
		if ( ! $order || ! $order instanceof WC_Order ) {
			return $this->prepare_error_response(
				'fincommerce_rest_order_invalid_id',
				__( 'Invalid order ID.', 'fincommerce' ),
		...
		$tracking_number_parse_result = apply_filters(
			'fincommerce_fulfillment_parse_tracking_number',
			$tracking_number,
	...
	private function get_args_for_get_fulfillments(): array {
		return array(
			'order_id' => array(
				'description' => __( 'Unique identifier for the order.', 'fincommerce' ),
				'type'        => 'integer',
				'required'    => true,
	...
	private function get_schema_for_get_fulfillments(): array {
		...
		$schema['title']      = __( 'Get fulfillments response.', 'fincommerce' );
		$schema['properties'] = array(
			'fulfillment' => array(
				'description' => __( 'The fulfillment object.', 'fincommerce' ),
				'type'        => 'array',
				'required'    => true,
	...
	private function get_schema_for_create_fulfillment(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'Create fulfillment response.', 'fincommerce' );
		$schema['properties'] = array(
			'fulfillment' => array(
				'description' => __( 'The created fulfillment object.', 'fincommerce' ),
				'type'        => 'object',
				'required'    => true,
				'schema'      => $this->get_read_schema_for_fulfillment(),
			),
	...
	private function get_args_for_get_fulfillment(): array {
		return array(
			...
			'fulfillment_id' => array(
				'description' => __( 'Unique identifier for the fulfillment.', 'fincommerce' ),
				'type'        => 'integer',
				'context'     => array( 'view', 'edit' ),
				'required'    => true,
			),
	...
	private function get_schema_for_get_fulfillment(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'Get fulfillment response.', 'fincommerce' );
		$schema['properties'] = array(
			'fulfillment' => array(
				'description' => __( 'The fulfillment object.', 'fincommerce' ),
				'type'        => 'object',
	...
	private function get_schema_for_update_fulfillment(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'Update fulfillment response.', 'fincommerce' );
		$schema['properties'] = array(
			'fulfillment' => array(
				'description' => __( 'The fulfillment object.', 'fincommerce' ),
				'type'        => 'object',
	...
	private function get_args_for_delete_fulfillment(): array {
		return array(
			...
			'notify_customer' => array(
				'description' => __( 'Whether to notify the customer about the fulfillment update.', 'fincommerce' ),
				'type'        => 'boolean',
				'default'     => false,
				'required'    => false,
				'context'     => array( 'view', 'edit' ),
	...
	private function get_schema_for_delete_fulfillment(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'Delete fulfillment response.', 'fincommerce' );
		$schema['properties'] = array(
			'message' => array(
				'description' => __( 'The response message.', 'fincommerce' ),
				'type'        => 'string',
				'required'    => true,
			),
		);
	...
	private function get_schema_for_get_fulfillment_meta(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'Get fulfillment meta data response.', 'fincommerce' );
		$schema['properties'] = array(
			'meta_data' => array(
				'description' => __( 'The meta data array.', 'fincommerce' ),
				'type'        => 'array',
				'required'    => true,
				'items'       => array(
					'description' => __( 'The meta data object.', 'fincommerce' ),
					'type'        => 'object',
	...
	private function get_schema_for_update_fulfillment_meta(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'Update fulfillment meta data response.', 'fincommerce' );
		$schema['properties'] = array(
			'meta_data' => array(
				'description' => __( 'The meta data array.', 'fincommerce' ),
				'type'        => 'array',
	...
	private function get_schema_for_delete_fulfillment_meta(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'Delete fulfillment meta data response.', 'fincommerce' );
		$schema['properties'] = array(
			'meta_data' => array(
				'description' => __( 'The meta data array.', 'fincommerce' ),
				'type'        => 'array',
	...
	private function get_args_for_get_tracking_number_details(): array {
		return array(
			...
			'tracking_number' => array(
				'description' => __( 'The tracking number to look up.', 'fincommerce' ),
				'type'        => 'string',
				'required'    => true,
				'context'     => array( 'view', 'edit' ),
			),
	...
	private function get_schema_for_get_tracking_number_details(): array {
		$schema               = $this->get_base_schema();
		$schema['title']      = __( 'The tracking number details response.', 'fincommerce' );
		$schema['properties'] = array(
			'tracking_number_details' => array(
				'description' => __( 'The tracking number details.', 'fincommerce' ),
				'type'        => 'object',
				'required'    => true,
				'properties'  => array(
					'tracking_number'   => array(
						'description' => __( 'The tracking number.', 'fincommerce' ),
						'type'        => 'string',
						'required'    => true,
					),
					'shipping_provider' => array(
						'description' => __( 'The shipping provider.', 'fincommerce' ),
						'type'        => 'string',
						'required'    => true,
					),
					'tracking_url'      => array(
						'description' => __( 'The tracking URL.', 'fincommerce' ),
						'type'        => 'string',
						'required'    => true,
					),
				),
	...
	private function get_read_schema_for_fulfillment() {
		return array(
			...
			'entity_type'    => array(
				'description' => __( 'The type of entity for which the fulfillment is created.', 'fincommerce' ),
				'type'        => 'string',
				'required'    => true,
				'context'     => array( 'view', 'edit' ),
			),
			'entity_id'      => array(
				'description' => __( 'Unique identifier for the entity.', 'fincommerce' ),
				'type'        => 'string',
				'required'    => true,
				'context'     => array( 'view', 'edit' ),
			),
			'status'         => array(
				'description' => __( 'The status of the fulfillment.', 'fincommerce' ),
				'type'        => 'string',
				'default'     => 'unfulfilled',
				'required'    => true,
				'context'     => array( 'view', 'edit' ),
			...
			'is_fulfilled'   => array(
				'description' => __( 'Whether the fulfillment is fulfilled.', 'fincommerce' ),
				'type'        => 'boolean',
				'default'     => false,
				'required'    => true,
				'context'     => array( 'view', 'edit' ),
			...
			'date_updated'   => array(
				'description' => __( 'The date the fulfillment was created.', 'fincommerce' ),
				'type'        => 'string',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
				'required'    => true,
			...
			'date_deleted'   => array(
				'description' => __( 'The date the fulfillment was deleted.', 'fincommerce' ),
				'type'        => 'string',
				'default'     => null,
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
			...
			'meta_data'      => array(
				'description' => __( 'Meta data for the fulfillment.', 'fincommerce' ),
				'type'        => 'array',
				'required'    => true,
				'schema'      => $this->get_schema_for_meta_data(),
			),
	...
	private function get_schema_for_meta_data(): array {
		return array(
			...
			'properties' => array(
				'id'    => array(
					'description' => __( 'The unique identifier for the meta data. Set `0` for new records.', 'fincommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'key'   => array(
					'description' => __( 'The key of the meta data.', 'fincommerce' ),
					'type'        => 'string',
					'required'    => true,
					'context'     => array( 'view', 'edit' ),
				),
				'value' => array(
					'description' => __( 'The value of the meta data.', 'fincommerce' ),
					'type'        => 'string',
					'required'    => true,
					'context'     => array( 'view', 'edit' ),
				),
	...
	private function validate_fulfillment( Fulfillment $fulfillment, int $fulfillment_id, int $order_id ) {
		if ( $fulfillment->get_id() !== $fulfillment_id || $fulfillment->get_entity_type() !== WC_Order::class || $fulfillment->get_entity_id() !== "$order_id" ) {
			throw new \Exception( esc_html__( 'Invalid fulfillment ID.', 'fincommerce' ) );
		}
	}
}
