...
use Automattic\FinCommerce\Internal\Fulfillments\Providers\AbstractShippingProvider;
use WC_Order;
use WC_Order_Refund;
 ...
 *
 * This class is responsible for adding hooks related to fulfillments in FinCommerce.
 *
 * @since 10.1.0
 * @package FinCommerce\Internal\Fulfillments
 */
class FulfillmentsManager {
	...
	public function register() {
		add_filter( 'fincommerce_fulfillment_shipping_providers', array( $this, 'get_initial_shipping_providers' ), 10, 1 );
		add_filter( 'fincommerce_fulfillment_translate_meta_key', array( $this, 'translate_fulfillment_meta_key' ), 10, 1 );
		add_filter( 'fincommerce_fulfillment_parse_tracking_number', array( $this, 'try_parse_tracking_number' ), 10, 3 );
	...
	private function init_fulfillment_status_hooks() {
		...
		add_action( 'fincommerce_fulfillment_after_create', array( $this, 'update_order_fulfillment_status_on_fulfillment_update' ), 10, 1 );
		add_action( 'fincommerce_fulfillment_after_update', array( $this, 'update_order_fulfillment_status_on_fulfillment_update' ), 10, 1 );
		add_action( 'fincommerce_fulfillment_after_delete', array( $this, 'update_order_fulfillment_status_on_fulfillment_update' ), 10, 1 );
	}
	...
	private function init_refund_hooks() {
		add_action( 'fincommerce_refund_created', array( $this, 'update_fulfillments_after_refund' ), 10, 1 );
		add_action( 'fincommerce_delete_order_refund', array( $this, 'update_fulfillment_status_after_refund_deleted' ), 10, 1 );
	}
	...
	public function translate_fulfillment_meta_key( $meta_key ) {
		...
		$meta_key_translations = apply_filters(
			...
			array(
				'fulfillment_status' => __( 'Fulfillment Status', 'fincommerce' ),
				'shipment_tracking'  => __( 'Shipment Tracking', 'fincommerce' ),
				'shipment_provider'  => __( 'Shipment Provider', 'fincommerce' ),
			)
		);
	 ...
	 *
	 * This method provides the initial shipping providers that feeds the `fincommerce_fulfillment_shipping_providers` filter,
			...
			if ( class_exists( $provider ) && is_subclass_of( $provider, AbstractShippingProvider::class ) ) {
				try {
					...
					$logger->error(
						...
						array( 'source' => 'fincommerce-fulfillments' )
					);
