...
class FulfillmentUtils {
	...
	public static function get_tracking_info_html( Fulfillment $fulfillment ): string {
		...
		if ( ! empty( $tracking_url ) && ! empty( $tracking_number ) ) {
			...
			$tracking_html .= '<span class="no-tracking">' . esc_html__( 'No tracking number available', 'fincommerce' ) . '</span>';
		}
	...
	public static function get_order_fulfillment_status_text( WC_Order $order ): string {
		...
		switch ( $fulfillment_status ) {
			case 'fulfilled':
				$fulfillment_status_text = ' ' . __( 'It has been <mark class="fulfillment-status">Fulfilled</mark>.', 'fincommerce' );
				break;
			case 'partially_fulfilled':
				$fulfillment_status_text = ' ' . __( 'It has been <mark class="fulfillment-status">Partially fulfilled</mark>.', 'fincommerce' );
				break;
			case 'unfulfilled':
				$fulfillment_status_text = ' ' . __( 'It is currently <mark class="fulfillment-status">Unfulfilled</mark>.', 'fincommerce' );
				break;
			case 'no_fulfillments':
				$fulfillment_status_text = ' ' . __( 'It has <mark class="fulfillment-status">no fulfillments</mark> yet.', 'fincommerce' );
				break;
		}
	 ...
	 * in the FinCommerce Fulfillments system. It can be filtered using the
	 * `fincommerce_fulfillment_order_fulfillment_statuses` filter.
	 *
	...
	public static function get_order_fulfillment_statuses(): array {
		 ...
		 * FinCommerce Fulfillments system.
		 *
		 * @since 10.1.0
		 *
	 ...
	 * in the FinCommerce Fulfillments system. It can be filtered using the
	 * `fincommerce_fulfillment_fulfillment_statuses` filter.
	 *
	 ...
	 *
	 * This method retrieves the shipping providers registered in the FinCommerce Fulfillments system.
	 * It can be filtered using the `fincommerce_fulfillment_shipping_providers` filter.
	 *
	...
	protected static function get_default_order_fulfillment_statuses(): array {
		return array(
			'fulfilled'           => array(
				'label'            => __( 'Fulfilled', 'fincommerce' ),
				'background_color' => '#C6E1C6',
				'text_color'       => '#13550F',
			),
			'partially_fulfilled' => array(
				'label'            => __( 'Partially fulfilled', 'fincommerce' ),
				'background_color' => '#C8D7E1',
				'text_color'       => '#003D66',
			),
			'unfulfilled'         => array(
				'label'            => __( 'Unfulfilled', 'fincommerce' ),
				'background_color' => '#FBE5E5',
				'text_color'       => '#CC1818',
			),
			'no_fulfillments'     => array(
				'label'            => __( 'No fulfillments', 'fincommerce' ),
