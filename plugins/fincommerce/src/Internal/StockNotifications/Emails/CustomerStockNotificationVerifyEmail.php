...
class CustomerStockNotificationVerifyEmail extends WC_Email {
	...
	public function __construct() {
		...
		$this->title       = __( 'Back in stock sign-up verification', 'fincommerce' );
		$this->description = __( 'Verification e-mail sent to customers, as part of the double opt-in sign-up process.', 'fincommerce' );
		...
		add_action( 'fincommerce_email_stock_notification_verify_notification', array( $this, 'trigger' ), 10, 1 );
	...
	public function get_default_subject() {
		return __( 'Join the "{product_name}" waitlist.', 'fincommerce' );
	}
	...
	public function get_default_heading() {
		return __( 'Confirm sign-up', 'fincommerce' );
	}
	...
	public function get_default_intro_content() {
		return __( 'Please follow the link below to complete the sign-up process and join the "{product_name}" waitlist.', 'fincommerce' );
	}
	...
	public function get_default_additional_content() {
		return __( 'Thanks for shopping with us.', 'fincommerce' );
	}
	...
	public function get_intro_content() {
		 ...
		 * @return string
		 */
		return apply_filters( 'fincommerce_email_stock_notification_intro_content', $this->format_string( $this->get_option_or_transient( 'intro_content', $this->get_default_intro_content() ) ), $this->object, $this );
	}
	...
	private function get_additional_template_args(): array {
		...
		$verification_button_text  = apply_filters(
			'fincommerce_email_stock_notification_verify_button_text',
			_x( 'Confirm', 'Stock Notification confirm notification', 'fincommerce' ),
			$notification,
			$product
		);
		...
		$expiration_threshold_text = sprintf(
			...
			_n( '%s minute', '%s minutes', $expiration_threshold / 60, 'fincommerce' ),
	...
	public function init_form_fields() {
		...
		$intro_content_field = array(
			'title'       => __( 'Email content', 'fincommerce' ),
			'description' => __( 'Text to appear below the main e-mail header.', 'fincommerce' ) . ' ' . $placeholder_text,
