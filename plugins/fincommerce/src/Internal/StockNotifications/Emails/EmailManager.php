...
namespace Automattic\FinCommerce\Internal\StockNotifications\Emails;
...
use Automattic\FinCommerce\Internal\StockNotifications\Emails\CustomerStockNotificationEmail;
use Automattic\FinCommerce\Internal\StockNotifications\Emails\CustomerStockNotificationVerifyEmail;
use Automattic\FinCommerce\Internal\StockNotifications\Emails\CustomerStockNotificationVerifiedEmail;
use Automattic\FinCommerce\Internal\StockNotifications\Emails\EmailTemplatesController;
/**
 * Emails manager.
 */
class EmailManager {
	...
	final public function init() {
		...
		add_filter( 'fincommerce_email_classes', array( $this, 'email_classes' ) );

		// Add "transactional" emails.
		add_action( 'fincommerce_email_actions', array( $this, 'add_transactional_emails' ) );

		// Setup styles.
		add_filter( 'fincommerce_email_styles', array( $this, 'add_stylesheets' ), 10, 2 );

		// Preview.
		add_filter( 'fincommerce_prepare_email_for_preview', array( $this, 'prepare_email_for_preview' ) );
		add_filter( 'fincommerce_email_preview_email_content_setting_ids', array( $this, 'add_intro_content_to_preview_settings' ), 10, 2 );

		// Restore customer's context while rendering the emails.
		add_action( 'fincommerce_email_stock_notification_product', array( $this, 'maybe_restore_customer_tax_location_data' ), 9 );
	 ...
	 *
	 * @hook fincommerce_defer_transactional_emails
	 *
	...
	public function add_transactional_emails( $actions ) {
		...
		$actions[] = 'fincommerce_customer_stock_notification_verify';
		$actions[] = 'fincommerce_customer_stock_notification_verified';
	...
	public function maybe_restore_customer_tax_location_data( $notification ) {
		...
		if ( 'incl' !== get_option( 'fincommerce_tax_display_shop' ) ) {
			return;
		}
	...
	public function add_stylesheets( $css, $email = null ) {
		...
		/**
		 * `fincommerce_email_stock_notification_emails_to_style` filter.
		 *
		 * @since  10.2.0
		 *
		 * @return array
		 */
		if ( ( is_null( $email ) || ! in_array( $email->id, (array) apply_filters( 'fincommerce_email_stock_notification_emails_to_style', self::$email_ids ), true ) ) ) {
			return $css;
		}

		// General text.
		$text = get_option( 'fincommerce_email_text_color' );

		// Primary color.
		$base = get_option( 'fincommerce_email_base_color' );

		/**
		 * `fincommerce_email_stock_notification_base_text_color` filter.
		 *
		 * @since  10.2.0
		 *
		 * @return string
		 */
		$base_text = (string) apply_filters( 'fincommerce_email_stock_notification_base_text_color', wc_light_or_dark( $base, '#202020', '#ffffff' ), $email );
	...
	/**
	 * Register intro_content email fields to be watched by FinCommerce's live email preview.
	 *
	...
	public function add_intro_content_to_preview_settings( $setting_ids, $email_id ) {
		...
		if ( in_array( $email_id, self::$email_ids, true ) ) {
			$setting_ids[] = "fincommerce_{$email_id}_intro_content";
		}
