...
class EmailTemplatesController {
	...
	public function register_template_hooks() {
		add_action( 'fincommerce_email_stock_notification_product', array( $this, 'email_product_image' ), 10, 3 );
		add_action( 'fincommerce_email_stock_notification_product', array( $this, 'email_product_title' ), 20, 3 );
		add_action( 'fincommerce_email_stock_notification_product', array( $this, 'email_product_attributes' ), 30, 3 );
		add_action( 'fincommerce_email_stock_notification_product', array( $this, 'email_product_price' ), 40, 3 );
	}
	...
	public function email_product_image( $product, $notification, $plain_text = false ) {
		...
		$image     = wp_get_attachment_image_src( $product->get_image_id(), 'fincommerce_thumbnail' );
