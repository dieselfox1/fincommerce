...
namespace Automattic\FinCommerce\Internal\StockNotifications\Utilities;
...
use Automattic\FinCommerce\Internal\StockNotifications\Utilities\StockManagementHelper;
...
class EligibilityService {
	...
	public function is_product_eligible( ?WC_Product $product ): bool {
		...
		/**
		 * Filter: fincommerce_customer_stock_notifications_product_is_valid
		 ...
		 */
		return (bool) apply_filters( 'fincommerce_customer_stock_notifications_product_is_valid', true, $product );
	}
	...
	public function should_skip_notification( Notification $notification, WC_Product $product ): bool {
		...
		if ( $should_skip ) {
			...
			if ( $user_id ) {
				...
				if ( $user && ( user_can( $user, 'manage_fincommerce' ) || user_can( $user, 'manage_options' ) ) ) {
					$should_skip = false;
				}
			}
		}
		...
		/**
		 * Filter: fincommerce_customer_stock_notification_should_skip_sending
		 *
		 * @since 10.2.0
		 *
		 ...
		 * @return bool
		 */
		return (bool) apply_filters( 'fincommerce_customer_stock_notification_should_skip_sending', $should_skip, $notification->get_id() );
	}
	...
	private function is_notification_throttled( Notification $notification ): bool {
		...
		/**
		 * Filter: fincommerce_customer_stock_notification_throttle_threshold
		 *
		 * @since 10.2.0
		 *
		 * @param int $threshold Throttle time in seconds should pass from the last notification delivery time.
		 */
		$threshold = (int) apply_filters( 'fincommerce_customer_stock_notification_throttle_threshold', self::SPAM_THRESHOLD );
		if ( $threshold <= 0 ) {
			return false;
		}
