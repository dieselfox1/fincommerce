...
class StockSyncController {
	...
	public function __construct() {
		// Event handlers.
		add_action( 'fincommerce_product_set_stock_status', array( $this, 'handle_product_stock_status_change' ), 100, 3 );
		add_action( 'fincommerce_variation_set_stock_status', array( $this, 'handle_product_stock_status_change' ), 100, 3 );
	...
	public function process_queue(): void {
		 ...
		 */
		do_action( 'fincommerce_customer_stock_notifications_product_sync', $product_ids );
		$this->queue = array();
	}
	...
	private function store_admin_notice( $product_id ): void {
		...
		$notice_message = sprintf( __( 'Back-in-stock notifications for this product are now being processed. Subscribed customers will receive these emails over the next few minutes. You can monitor or manage individual subscriptions on the <a href="%s">Stock Notifications page</a>.', 'fincommerce' ), sprintf( admin_url( 'admin.php?page=wc-customer-stock-notifications&customer_stock_notifications_product_filter=%d&status=active_customer_stock_notifications&filter_action=Filter' ), $product_id ) );
	...
	public function output_admin_notice(): void {
		...
		\wp_admin_notice(
			...
			array(
				'type'        => 'info',
				'id'          => 'fincommerce_customer_stock_notifications_product_sync_notice',
				'dismissible' => false,
			)
		);
