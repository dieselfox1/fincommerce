...
class NotificationCreatePage {
	...
	public function process_create_form() {
		...
		check_admin_referer( 'fincommerce-customer-stock-notification-create', 'customer_stock_notification_create_security' );
		...
		if ( ! isset( $_POST['product_id'] ) || empty( $_POST['product_id'] ) ) {
			NotificationsPage::add_notice( __( 'Please select a product.', 'fincommerce' ), 'error' );
			return;
		}

		if ( empty( $_POST['user_id'] ) && empty( $_POST['user_email'] ) ) {
			NotificationsPage::add_notice( __( 'Please select a customer.', 'fincommerce' ), 'error' );
			return;
		}
		...
		if ( isset( $_POST['user_id'] ) && ! empty( $_POST['user_id'] ) ) {
			...
			if ( ! filter_var( $posted_data['user_email'], FILTER_VALIDATE_EMAIL ) ) {
				NotificationsPage::add_notice( __( 'Please enter a valid email address.', 'fincommerce' ), 'error' );
				return;
			}
		...
		if ( is_wp_error( $result ) ) {
			...
			$notice_message = __( 'Notification created.', 'fincommerce' );
