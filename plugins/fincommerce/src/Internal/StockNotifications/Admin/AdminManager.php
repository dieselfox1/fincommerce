...
use Automattic\FinCommerce\Internal\StockNotifications\Admin\MenusController;
use Automattic\FinCommerce\Internal\StockNotifications\Admin\SettingsController;
...
class AdminManager {
	...
	public static function admin_resources() {
		...
		$params = array(
			'i18n_wc_delete_notification_warning'       => __( 'Delete this notification permanently?', 'fincommerce' ),
			'i18n_wc_bulk_delete_notifications_warning' => __( 'Delete the selected notifications permanently?', 'fincommerce' ),
		);
		...
		if (
			! in_array(
				$screen_id,
				array( 'fincommerce_page_wc-customer-stock-notifications', 'fincommerce_page_wc-settings' ),
				true
			)
		) {
			return;
