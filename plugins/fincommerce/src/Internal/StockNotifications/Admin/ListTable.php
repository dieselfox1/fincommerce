...
use Automattic\FinCommerce\Internal\StockNotifications\Notification;
use Automattic\FinCommerce\Internal\StockNotifications\Factory;
use Automattic\FinCommerce\Internal\StockNotifications\Admin\NotificationsPage;
use Automattic\FinCommerce\Internal\StockNotifications\Utilities\EligibilityService;
...
class ListTable extends \WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct(
			array(
				'singular' => 'fincommerce_stock_notification',
				'plural'   => 'fincommerce_stock_notifications',
			)
		);
	}
	...
	public function column_cb( $notification ) {
		...
		<?php
			...
			printf( esc_html__( 'Select %s', 'fincommerce' ), esc_html( $notification->get_id() ) );
		?>
		</label>
	...
	public function column_id( $notification ) {
		$actions = array(
			'edit'   => sprintf( '<a href="' . admin_url( NotificationsPage::PAGE_URL . '&notification_action=edit&notification_id=%d' ) . '">%s</a>', $notification->get_id(), __( 'Edit', 'fincommerce' ) ),
			'delete' => sprintf( '<a href="' . wp_nonce_url( admin_url( NotificationsPage::PAGE_URL . '&notification_action=delete&notification_id=%d' ), 'delete_customer_stock_notification' ) . '">%s</a>', $notification->get_id(), __( 'Delete', 'fincommerce' ) ),
		);
		...
		printf(
			...
			/* translators: %s: Notification code */
			sprintf( esc_attr__( '&#8220;%s&#8221; (Edit)', 'fincommerce' ), esc_attr( $title ) ),
			esc_html( $title ),
	...
	public function no_items() {
		?>
		<p class="main">
			<?php esc_html_e( 'No Notifications found', 'fincommerce' ); ?>
		</p>
		<?php
	}
	...
	public function get_columns() {
		...
		$columns['id']               = _x( 'Notification', 'column_name', 'fincommerce' );
		$columns['status']           = _x( 'Status', 'column_name', 'fincommerce' );
		$columns['user']             = _x( 'User/Email', 'column_name', 'fincommerce' );
		$columns['product']          = _x( 'Product', 'column_name', 'fincommerce' );
		$columns['sku']              = _x( 'SKU', 'column_name', 'fincommerce' );
		$columns['date_created_gmt'] = _x( 'Signed Up', 'column_name', 'fincommerce' );
	...
	protected function get_bulk_actions() {
		$actions           = array();
		$actions['enable'] = __( 'Activate', 'fincommerce' );
		$actions['cancel'] = __( 'Cancel', 'fincommerce' );
	...
	public function prepare_items() {
		...
		if ( ! empty( $_GET['customer_stock_notifications_product_filter'] ) ) { // phpcs:ignore finpress.Security.NonceVerification.Recommended
			...
			if ( $product instanceof \WC_Product ) {
				...
				NotificationsPage::add_notice( __( 'Invalid product selected.', 'fincommerce' ), 'error' );
			}
		}
	...
	public function extra_tablenav( $which ) {
		if ( 'top' === $which && ! is_singular() ) {
			...
			<div class="alignleft actions">
				<?php
				$this->render_filters();
				submit_button( __( 'Filter', 'fincommerce' ), '', 'filter_action', false, array( 'id' => 'post-query-submit' ) );
				?>
			</div>
			<?php
		}
	...
	protected function display_product_dropdown() {
		...
		if ( ! empty( $_GET['customer_stock_notifications_product_filter'] ) ) { // phpcs:ignore finpress.Security.NonceVerification.Recommended
			...
			if ( $product ) {
				$product_string = sprintf(
					...
					esc_html__( '%1$s (#%2$s)', 'fincommerce' ),
		...
		?>
		<select class="wc-product-search" name="customer_stock_notifications_product_filter" data-placeholder="<?php esc_attr_e( 'Select product&hellip;', 'fincommerce' ); ?>" data-allow_clear="true" id="customer_stock_notifications_product_filter">
	...
	protected function display_customer_dropdown() {
		...
		?>
		<select class="wc-customer-search" name="customer_stock_notifications_customer_filter" data-placeholder="<?php esc_attr_e( 'Select customer&hellip;', 'fincommerce' ); ?>" data-allow_clear="true" id="customer_stock_notifications_customer_filter">
	...
	protected function display_months_dropdown() {
		...
		<select name="m" id="filter-by-date">
			<option<?php selected( $m, 0 ); ?> value="0"><?php esc_html_e( 'All dates', 'fincommerce' ); ?></option>
			<?php
			foreach ( $months as $arc_row ) {
				...
				printf(
					...
					sprintf( esc_html__( '%1$s %2$d', 'fincommerce' ), esc_html( $wp_locale->get_month( $month ) ), esc_html( $year ) )
				);
			}
			?>
		</select>
	...
	public function process_delete_action(): void {
		...
		try {
			...
			$notice_message = __( 'Notification deleted.', 'fincommerce' );
			NotificationsPage::add_notice( $notice_message, 'success' );

		} catch ( \Exception $e ) {

			$notice_message = __( 'Notification not found.', 'fincommerce' );
