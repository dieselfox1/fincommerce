...
use Automattic\FinCommerce\Internal\StockNotifications\Admin\ListTable;
...
class NotificationEditPage {
	...
	public function process_edit_form( Notification $notification ) {
		...
		check_admin_referer( 'fincommerce-customer-stock-notification-edit', 'customer_stock_notification_edit_security' );
		...
		switch ( $action ) {
			case 'activate_notification':
				...
				if ( is_wp_error( $result ) ) {
					...
					$notice_message = __( 'Notification updated.', 'fincommerce' );
			...
			case 'send_notification':
				...
				if ( ! $product || ! $product->is_in_stock() ) {
					$notice_message = __( 'Failed to send notification. Please make sure that the listed product is available.', 'fincommerce' );
					NotificationsPage::add_notice( $notice_message, 'error' );
				} else {
					...
					$notice_message = sprintf( __( 'Notification sent to "%s".', 'fincommerce' ), $notification->get_user_email() );
					NotificationsPage::add_notice( $notice_message, 'success' );
				}
				break;
			case 'send_verification_email':
				// translators: %s user email.
				$notice_message = sprintf( __( 'Verification email sent to "%s".', 'fincommerce' ), $notification->get_user_email() );
				NotificationsPage::add_notice( $notice_message, 'success' );
				break;
		}
