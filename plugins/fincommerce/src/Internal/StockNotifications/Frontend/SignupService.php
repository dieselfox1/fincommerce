...
namespace Automattic\FinCommerce\Internal\StockNotifications\Frontend;
...
use Automattic\FinCommerce\Internal\StockNotifications\NotificationQuery;
...
class SignupService {
	...
	public function signup( int $product_id, int $user_id, string $user_email, array $posted_attributes = array() ) {
		...
		if ( $notification instanceof Notification ) {
			...
			if ( NotificationStatus::PENDING === $notification->get_status() ) {
				...
				/**
				 * Action: fincommerce_customer_stock_notifications_signup
				 *
				 * @since 10.2.0
				 *
				 * @param Notification $notification The notification.
				 */
				do_action( 'fincommerce_customer_stock_notifications_signup', $notification );
	...
	private function create_customer( string $user_email ) {
		...
		try {
			...
			$password = 'yes' === get_option( 'fincommerce_registration_generate_password' ) ? '' : wp_generate_password();
	...
	public function get_error_message( string $error_code ): string {
		switch ( $error_code ) {
			case self::ERROR_INVALID_PRODUCT:
				return wp_kses_post( __( 'Invalid product.', 'fincommerce' ) );
			case self::ERROR_INVALID_USER:
				return wp_kses_post( __( 'Invalid user.', 'fincommerce' ) );
			case self::ERROR_INVALID_EMAIL:
				return wp_kses_post( __( 'Invalid email address.', 'fincommerce' ) );
			case self::ERROR_INVALID_OPT_IN:
				return wp_kses_post( __( 'To proceed, please consent to the creation of a new account with your e-mail.', 'fincommerce' ) );
			case self::ERROR_RATE_LIMITED:
				return wp_kses_post( __( 'You have already signed up too many times. Please try again later.', 'fincommerce' ) );
			default:
				return wp_kses_post( __( 'Failed to sign up. Please try again.', 'fincommerce' ) );
		}
	}
	...
	public function get_signup_user_message( string $signup_code, Notification $notification ): string {
		...
		switch ( $signup_code ) {
			...
			case self::SIGNUP_SUCCESS:
				...
				$message = sprintf( esc_html__( 'You have successfully signed up! You will be notified when "%s" is back in stock.', 'fincommerce' ), $notification->get_product_name() );
				break;

			case self::SIGNUP_SUCCESS_DOUBLE_OPT_IN:
				$message = esc_html__( 'Thanks for signing up! Please complete the sign-up process by following the verification link sent to your e-mail.', 'fincommerce' );
				break;

			case self::SIGNUP_SUCCESS_ACCOUNT_CREATED:
				/* translators: Product name */
				$message = sprintf( esc_html__( 'You have successfully signed up and will be notified when "%s" is back in stock! Note that a new account has been created for you; please check your e-mail for details.', 'fincommerce' ), $notification->get_product_name() );
				break;

			case self::SIGNUP_SUCCESS_ACCOUNT_CREATED_DOUBLE_OPT_IN:
				$message = esc_html__( 'Thanks for signing up! An account has been created for you. Please complete the sign-up process by following the verification link sent to your e-mail.', 'fincommerce' );
				break;

			case self::SIGNUP_ALREADY_JOINED:
				$message = esc_html__( 'You have already joined this waitlist.', 'fincommerce' );
				break;

			case self::SIGNUP_ALREADY_JOINED_DOUBLE_OPT_IN:
				$notice_text     = esc_html__( 'You have already joined this waitlist. Please complete the sign-up process by following the verification link sent to your e-mail.', 'fincommerce' );
				...
				$message         = sprintf(
					...
					$url,
					$wp_button_class,
					esc_html_x( 'Resend verification', 'notice action', 'fincommerce' ),
					$notice_text
				);
		...
		if ( is_user_logged_in() && ! $has_action_button ) {
			$button_class    = \wc_wp_theme_get_element_class_name( 'button' );
			$wp_button_class = $button_class ? ' ' . $button_class : '';
			$message         = sprintf( '<a href="%s" class="button wc-forward%s">%s</a> %s', \wc_get_account_endpoint_url( 'stock-notifications' ), $wp_button_class, esc_html_x( 'Manage notifications', 'notice action', 'fincommerce' ), $message );
		}
