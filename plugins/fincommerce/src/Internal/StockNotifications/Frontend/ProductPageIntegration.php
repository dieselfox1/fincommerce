...
use Automattic\FinCommerce\Internal\StockNotifications\Frontend\SignupService;
...
class ProductPageIntegration {
	...
	public function __construct() {
		add_action( 'fincommerce_simple_add_to_cart', array( $this, 'maybe_render_form' ), 30 );
		add_action( 'fincommerce_after_add_to_cart_form', array( $this, 'maybe_render_form' ), 30 );
	}
	...
	public function display_account_required( WC_Product $product ): void {
		 ...
		 */
		$pre = apply_filters( 'fincommerce_customer_stock_notifications_account_required_message_html', null, $product );
		if ( ! is_null( $pre ) ) {
			echo wp_kses_post( $pre );
			return;
		}

		$text = __( 'Please {login_link} to sign up for stock notifications.', 'fincommerce' );
		$text = str_replace( '{login_link}', '<a href="' . wc_get_account_endpoint_url( 'my-account' ) . '">' . _x( 'log in', 'back in stock form', 'fincommerce' ) . '</a>', $text );
	...
	public function display_already_signed_up( WC_Product $product, Notification $notification ): void {
		 ...
		 * @return string|null The message.
		 */
		$pre = apply_filters( 'fincommerce_customer_stock_notifications_already_signed_up_message_html', null, $product, $notification );
		if ( ! is_null( $pre ) ) {
			echo wp_kses_post( $pre );
			return;
		}

		$text = __( 'You have already joined the waitlist! Click {manage_account_link} to manage your notifications.', 'fincommerce' );
		$text = str_replace( '{manage_account_link}', '<a href="' . wc_get_account_endpoint_url( 'stock-notifications' ) . '">' . _x( 'here', 'back in stock form', 'fincommerce' ) . '</a>', $text );
		wc_print_notice( $text, 'notice' );
	}
	...
	public function display_form( WC_Product $product ): void {
		...
		$is_visible = ! $product->is_type( 'variable' ) || ( 'yes' === get_option( 'fincommerce_hide_out_of_stock_items' ) && ! $product->has_purchasable_variations() );
	...
	public static function is_personalization_enabled(): bool {
		 ...
		 * @return bool
		 */
		return (bool) apply_filters( 'fincommerce_customer_stock_notifications_personalization_enabled', false );
	}
}
