...
namespace Automattic\FinCommerce\Internal\StockNotifications\AsyncTasks;
...
class JobManager {
	...
	public function schedule_initial_job_for_product( int $product_id ): bool {
		...
		try {
			...
			/**
			 * Filter: fincommerce_customer_stock_notifications_first_batch_delay
			 *
			 * @since 10.2.0
			 *
			 ...
			 */
			$delay = (int) apply_filters( 'fincommerce_customer_stock_notifications_first_batch_delay', MINUTE_IN_SECONDS, $product_id );
			$delay = max( 0, $delay );
	...
	public function schedule_next_batch_for_product( int $product_id ): bool {
		...
		/**
		 * Filter: fincommerce_customer_stock_notifications_next_batch_delay
		 *
		 * @since 10.2.0
		 *
		 ...
		 * @param int   $product_id  Product ID being scheduled.
		 */
		$delay = (int) apply_filters( 'fincommerce_customer_stock_notifications_next_batch_delay', 0, $product_id );
		$delay = max( 0, $delay );
