...
namespace Automattic\FinCommerce\Internal\ProductAttributesLookup;
 ...
 * regular FinCommerce data regenerations mechanism.
 *
 ...
 *
 * Additionally, after the regeneration is completed a 'fincommerce_attribute_lookup_enabled' option
 ...
 * 'fincommerce_attribute_lookup_regeneration_in_progress' option) the option will be set to 'no'.
 *
...
class DataRegenerator {
	...
	public function __construct() {
		...
		add_filter( 'fincommerce_debug_tools', array( $this, 'add_initiate_regeneration_entry_to_tools_array' ), 1, 999 );
		add_action( 'fincommerce_run_product_attribute_lookup_regeneration_callback', array( $this, 'run_regeneration_step_callback' ) );
		add_action( 'fincommerce_installed', array( $this, 'run_fincommerce_installed_callback' ) );
	}
	...
	private function delete_all_attributes_lookup_data( bool $truncate_table ) {
		...
		delete_option( 'fincommerce_attribute_lookup_enabled' );
		delete_option( 'fincommerce_attribute_lookup_last_product_id_to_process' );
		delete_option( 'fincommerce_attribute_lookup_processed_count' );
	...
	private function initialize_table_and_data(): int {
		...
		update_option( 'fincommerce_attribute_lookup_last_product_id_to_process', $last_existing_product_id );
		update_option( 'fincommerce_attribute_lookup_processed_count', 0 );
	...
	public function run_regeneration_step_callback() {
		if ( ! $this->data_store->regeneration_is_in_progress() ) {
			...
			// was manually aborted via deleting the 'fincommerce_attribute_lookup_regeneration_in_progress' option.
	...
	private function enqueue_regeneration_step_run() {
		...
		$queue->schedule_single(
			...
			'fincommerce_run_product_attribute_lookup_regeneration_callback',
			array(),
	...
	public function do_regeneration_step( ?int $step_size = null, bool $use_optimized_db_access = false ) {
		 ...
		 */
		$products_per_generation_step = apply_filters( 'fincommerce_attribute_lookup_regeneration_step_size', $step_size ?? self::PRODUCTS_PER_GENERATION_STEP );

		$products_already_processed = get_option( 'fincommerce_attribute_lookup_processed_count', 0 );
		...
		update_option( 'fincommerce_attribute_lookup_processed_count', $products_already_processed );

		$last_product_id_to_process = get_option( 'fincommerce_attribute_lookup_last_product_id_to_process', PHP_INT_MAX );
	...
	public function finalize_regeneration( bool $enable_usage ) {
		...
		update_option( 'fincommerce_attribute_lookup_enabled', $enable_usage ? 'yes' : 'no' );
	}

	/**
	 * Add a 'Regenerate product attributes lookup table' entry to the Status - Tools page.
	 *
	 * @param array $tools_array The tool definitions array that is passed ro the fincommerce_debug_tools filter.
	...
	public function add_initiate_regeneration_entry_to_tools_array( array $tools_array ) {
		...
		$entry = array(
			'name'             => __( 'Regenerate the product attributes lookup table', 'fincommerce' ),
			'desc'             => __( 'This tool will regenerate the product attributes lookup table data from existing product(s) data. This process may take a while.', 'fincommerce' ),
			'requires_refresh' => true,
			'callback'         => function () {
				$this->initiate_regeneration_from_tools_page();
				return __( 'Product attributes lookup table data is regenerating', 'fincommerce' );
			},
			'selector'         => array(
				'description'   => __( 'Select a product to regenerate the data for, or leave empty for a full table regeneration:', 'fincommerce' ),
				'class'         => 'wc-product-search',
				'search_action' => 'fincommerce_json_search_products',
				'name'          => 'regenerate_product_attribute_lookup_data_product_id',
				'placeholder'   => esc_attr__( 'Search for a product&hellip;', 'fincommerce' ),
			),
		);
		...
		if ( $generation_is_in_progress ) {
			$entry['button'] = sprintf(
				...
				__( 'Filling in progress (%d)', 'fincommerce' ),
				get_option( 'fincommerce_attribute_lookup_processed_count', 0 )
			);
			$entry['disabled'] = true;
		} else {
			$entry['button'] = __( 'Regenerate', 'fincommerce' );
		}
		...
		if ( $generation_is_in_progress ) {
			$entry = array(
				'name'             => __( 'Abort the product attributes lookup table regeneration', 'fincommerce' ),
				'desc'             => __( 'This tool will abort the regenerate product attributes lookup table regeneration. After this is done the process can be either started over, or resumed to continue where it stopped.', 'fincommerce' ),
				'requires_refresh' => true,
				'callback'         => function () {
					$this->abort_regeneration( true );
					return __( 'Product attributes lookup table regeneration process has been aborted.', 'fincommerce' );
				},
				'button'           => __( 'Abort', 'fincommerce' ),
			);
			$tools_array['abort_product_attributes_lookup_table_regeneration'] = $entry;
		} elseif ( $generation_was_aborted ) {
			$processed_count = get_option( 'fincommerce_attribute_lookup_processed_count', 0 );
			$entry           = array(
				'name'             => __( 'Resume the product attributes lookup table regeneration', 'fincommerce' ),
				'desc'             =>
					sprintf(
						/* translators: %1$s = count of products already processed. */
						__( 'This tool will resume the product attributes lookup table regeneration at the point in which it was aborted (%1$s products were already processed).', 'fincommerce' ),
						$processed_count
					),
				'requires_refresh' => true,
				'callback'         => function () {
					$this->resume_regeneration( true );
					return __( 'Product attributes lookup table regeneration process has been resumed.', 'fincommerce' );
				},
				'button'           => __( 'Resume', 'fincommerce' ),
			);
	...
	private function enable_or_disable_lookup_table_usage( $enable ) {
		...
		update_option( 'fincommerce_attribute_lookup_enabled', $enable ? 'yes' : 'no' );
	}
	...
	public function cancel_regeneration_scheduled_action() {
		$queue = WC()->get_instance_of( \WC_Queue::class );
		$queue->cancel_all( 'fincommerce_run_product_attribute_lookup_regeneration_callback' );
	}
	...
	public function has_scheduled_action_for_regeneration_step(): bool {
		...
		$actions = $queue->search(
			array(
				'hook'   => 'fincommerce_run_product_attribute_lookup_regeneration_callback',
 ...
 PRIMARY KEY  ( `product_or_parent_id`, `term_id`, `product_id`, `taxonomy` )
	...
	/**
	 * Run additional setup needed after a FinCommerce install or update finishes.
	 *
	 * @internal For exclusive usage of FinCommerce core, backwards compatibility not guaranteed.
	 */
	public function run_fincommerce_installed_callback() {
		...
		if ( $this->data_store->lookup_table_has_data() || ! $this->get_last_existing_product_id() ) {
			$must_enable = get_option( 'fincommerce_attribute_lookup_enabled' ) !== 'no';
			$this->delete_all_attributes_lookup_data( false );
			update_option( 'fincommerce_attribute_lookup_enabled', $must_enable ? 'yes' : 'no' );
		} else {
			$this->initiate_regeneration();
		}
	}
