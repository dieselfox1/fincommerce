...
class CLIRunner {
	...
	private function enable_core( array $args, array $assoc_args ) {
		...
		if ( 'yes' === get_option( 'fincommerce_attribute_lookup_enabled' ) ) {
		...
		update_option( 'fincommerce_attribute_lookup_enabled', 'yes' );
		$table_name = $this->lookup_data_store->get_lookup_table_name();
	...
	private function disable_core( array $args, array $assoc_args ) {
		...
		update_option( 'fincommerce_attribute_lookup_enabled', 'no' );
		$table_name = $this->lookup_data_store->get_lookup_table_name();
	...
	private function regenerate_for_product_core( array $args = array(), array $assoc_args = array() ) {
		...
		if ( $this->lookup_data_store->get_last_create_operation_failed() ) {
			$this->error( "Lookup data regeneration failed.\nSee the FinCommerce logs (source is %9palt-updates%n) for details." );
		} else {
	...
	private function info_core( array $args, array $assoc_args ) {
		...
		$enabled = 'yes' === get_option( 'fincommerce_attribute_lookup_enabled' );
		...
		if ( $this->lookup_data_store->regeneration_is_in_progress() ) {
			$max_product_id_to_process = get_option( 'fincommerce_attribute_lookup_last_product_id_to_process', '???' );
			WP_CLI::log( '' );
	...
	private function regenerate_core( array $args = array(), array $assoc_args = array() ) {
		...
		$was_enabled = 'yes' === get_option( 'fincommerce_attribute_lookup_enabled' );
		...
		if ( ! $this->lookup_data_store->regeneration_is_in_progress() || array_key_exists( 'from-scratch', $assoc_args ) ) {
			...
			$last_product_id = get_option( 'fincommerce_attribute_lookup_last_product_id_to_process' );
			if ( false === $last_product_id ) {
				WP_CLI::error( 'Regeneration seems to be already in progress, but the fincommerce_attribute_lookup_last_product_id_to_process option isn\'t there. Try %9wp cli palt cleanup_regeneration_progress%n first." );' );
				return 1;
			}
