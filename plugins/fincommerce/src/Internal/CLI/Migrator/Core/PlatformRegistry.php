...
namespace Automattic\FinCommerce\Internal\CLI\Migrator\Core;

use InvalidArgumentException;
use Automattic\FinCommerce\Internal\CLI\Migrator\Interfaces\PlatformFetcherInterface;
use Automattic\FinCommerce\Internal\CLI\Migrator\Interfaces\PlatformMapperInterface;
use WP_CLI;
...
class PlatformRegistry {
	...
	private function load_platforms(): void {
		 ...
		 * @since 1.0.0
		 */
		$platforms = apply_filters( 'fincommerce_migrator_platforms', array() );
	...
	public function get_fetcher( string $platform_id ): PlatformFetcherInterface {
		...
		if ( ! $platform ) {
			throw new InvalidArgumentException(
				sprintf(
					...
					esc_html__( 'Platform %s not found.', 'fincommerce' ),
		...
		if ( ! is_string( $fetcher_class ) || empty( $fetcher_class ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: Platform ID */
					esc_html__( 'Invalid fetcher class for platform %s. Fetcher must be a non-empty string.', 'fincommerce' ),
					esc_html( $platform_id )
				)
			);
		}
		...
		if ( ! class_exists( $fetcher_class ) ) {
			throw new InvalidArgumentException(
				sprintf(
					...
					esc_html__( 'Invalid fetcher class for platform %1$s. Class %2$s does not exist.', 'fincommerce' ),
					esc_html( $platform_id ),
		...
		if ( ! in_array( PlatformFetcherInterface::class, class_implements( $fetcher_class ), true ) ) {
			throw new InvalidArgumentException(
				sprintf(
					...
					esc_html__( 'Invalid fetcher class for platform %1$s. Class %2$s does not implement %3$s.', 'fincommerce' ),
					esc_html( $platform_id ),
					esc_html( $fetcher_class ),
		...
		// Use the FinCommerce DI container to properly inject dependencies.
	...
	public function get_mapper( string $platform_id, array $args = array() ): PlatformMapperInterface {
		...
		if ( ! is_string( $mapper_class ) || empty( $mapper_class ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: Platform ID */
					esc_html__( 'Invalid mapper class for platform %s. Mapper must be a non-empty string.', 'fincommerce' ),
					esc_html( $platform_id )
				)
			);
		}
		...
		if ( ! class_exists( $mapper_class ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %1$s: Platform ID, %2$s: Class name */
					esc_html__( 'Invalid mapper class for platform %1$s. Class %2$s does not exist.', 'fincommerce' ),
					esc_html( $platform_id ),
		...
		if ( ! in_array( PlatformMapperInterface::class, class_implements( $mapper_class ), true ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %1$s: Platform ID, %2$s: Class name, %3$s: Interface name */
					esc_html__( 'Invalid mapper class for platform %1$s. Class %2$s does not implement %3$s.', 'fincommerce' ),
					esc_html( $platform_id ),
					esc_html( $mapper_class ),
		...
		// Otherwise, use the FinCommerce DI container for dependency injection.
		if ( ! empty( $args ) ) {
