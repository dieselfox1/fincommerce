...
use Automattic\FinCommerce\EmailEditor\Engine\Renderer\Renderer as EmailRenderer;
...
class BlockEmailRenderer {
	...
	/**
	 * Service for extracting FinCommerce content from WC_Email object.
	 *
	...
	/**
	 * FinCommerce Email Template Manager instance.
	 *
	 ...
	 *
	 * @param WooContentProcessor $woo_content_processor Service for extracting FinCommerce content from WC_Email object.
	 * @internal
	 */
	final public function init( WooContentProcessor $woo_content_processor ): void {
		...
		add_action( 'fincommerce_email_blocks_renderer_initialized', array( $this, 'register_block_renderers' ) );
	}

	/**
	 * Maybe render block-based email content.
	 *
	 * @param \WC_Email $wc_email FinCommerce email.
	 ...
	 * @param string    $woo_content FinCommerce email content.
	 * @param \WC_Email $wc_email FinCommerce email.
	...
	private function render_block_email( \WP_Post $email_post, string $woo_content, \WC_Email $wc_email ): ?string {
		try {
			...
			add_filter( 'fincommerce_email_styles', array( $this->woo_content_processor, 'prepare_css' ), 10, 2 );
	 ...
	 *
	 * @param \WC_Email $email FinCommerce email.
	 * @return \WP_Post|null
	 */
