...
namespace Automattic\FinCommerce\Internal\EmailEditor;

use Automattic\FinCommerce\EmailEditor\Email_Editor_Container;
use Automattic\FinCommerce\EmailEditor\Engine\Dependency_Check;
use Automattic\FinCommerce\Internal\Admin\EmailPreview\EmailPreview;
use Automattic\FinCommerce\Internal\EmailEditor\EmailPatterns\PatternsController;
use Automattic\FinCommerce\Internal\EmailEditor\EmailTemplates\TemplatesController;
use Automattic\FinCommerce\Internal\EmailEditor\Renderer\Blocks\WooContent;
use Automattic\FinCommerce\Internal\EmailEditor\WCTransactionalEmails\WCTransactionalEmails;
use Automattic\FinCommerce\Internal\EmailEditor\WCTransactionalEmails\WCTransactionalEmailPostsManager;
use Automattic\FinCommerce\Internal\EmailEditor\EmailTemplates\TemplateApiController;
use Automattic\FinCommerce\EmailEditor\Engine\Logger\Email_Editor_Logger;
use WP_Post;
...
class Integration {
	...
	final public function init(): void {
		...
		add_action( 'fincommerce_init', array( $this, 'initialize' ) );
	}
	...
	public function init_logger() {
		...
		// Register the FinCommerce logger with the email editor package.
	...
	public function register_hooks() {
		add_filter( 'fincommerce_email_editor_post_types', array( $this, 'add_email_post_type' ) );
		add_filter( 'fincommerce_is_email_editor_page', array( $this, 'is_editor_page' ), 10, 1 );
		...
		add_filter( 'fincommerce_email_editor_send_preview_email_rendered_data', array( $this, 'update_send_preview_email_rendered_data' ) );
		add_filter( 'fincommerce_email_editor_send_preview_email_personalizer_context', array( $this, 'update_send_preview_email_personalizer_context' ) );
		add_filter( 'fincommerce_email_editor_preview_post_template_html', array( $this, 'update_preview_post_template_html_data' ), 100, 1 );
	}

	/**
	 * Add FinCommerce email post type to the list of supported post types.
	 *
	...
	public function add_email_post_type( array $post_types ): array {
		$post_types[] = array(
			...
			'args' => array(
				'labels'   => array(
					'name'          => __( 'Emails', 'fincommerce' ),
					'singular_name' => __( 'Email', 'fincommerce' ),
					'add_new_item'  => __( 'Add Email', 'fincommerce' ),
					'edit_item'     => __( 'Edit Email', 'fincommerce' ),
					'new_item'      => __( 'New Email', 'fincommerce' ),
					'view_item'     => __( 'View Email', 'fincommerce' ),
					'search_items'  => __( 'Search Emails', 'fincommerce' ),
				),
	...
	/**
	 * Extend the post API for the wp_template post type to add and save the fincommerce_data field.
	 */
	...
	/**
	 * Extend the post API for the woo_email post type to add and save the fincommerce_data field.
	 */
