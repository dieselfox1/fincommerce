...
use Automattic\FinCommerce\EmailEditor\Validator\Builder;
...
/**
 * API Controller for managing FinCommerce email templates via extending the post type API.
 *
 * @internal
 */
class EmailApiController {
	/**
	 * A list of FinCommerce emails.
	 *
	 * @var \WC_Email[]
	 */
	...
	/**
	 * The FinCommerce transactional email post manager.
	 *
	...
	/**
	 * Update FinCommerce specific option data by post name.
	 *
	...
	private function validate_email_data( array $data ) {
		...
		if ( ! empty( $invalid_recipients ) ) {
			$error_message = sprintf(
				...
				__( 'One or more Recipient email addresses are invalid: “%s”. Please enter valid email addresses separated by commas.', 'fincommerce' ),
		...
		if ( ! empty( $invalid_cc ) ) {
			$error_message = sprintf(
				// translators: %s will be replaced by comma-separated email addresses. For example, "invalidemail1@example.com,invalidemail2@example.com".
				__( 'One or more CC email addresses are invalid: “%s”. Please enter valid email addresses separated by commas.', 'fincommerce' ),
				implode( ',', $invalid_cc )
			);
		...
		if ( ! empty( $invalid_bcc ) ) {
			$error_message = sprintf(
				// translators: %s will be replaced by comma-separated email addresses. For example, "invalidemail1@example.com,invalidemail2@example.com".
				__( 'One or more BCC email addresses are invalid: “%s”. Please enter valid email addresses separated by commas.', 'fincommerce' ),
				implode( ',', $invalid_bcc )
			);
	...
	/**
	 * Get the schema for the FinCommerce email post data.
	 *
	 * @return array
	 */
