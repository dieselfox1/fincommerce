...
namespace Automattic\FinCommerce\Internal\EmailEditor\EmailTemplates;
	...
	public function get_template_data( $template_data ): array {
		...
		return array(
			'sender_settings' => array(
				'from_name'    => get_option( 'fincommerce_email_from_name' ),
				'from_address' => get_option( 'fincommerce_email_from_address' ),
			),
		);
	}

	/**
	 * Update FinCommerce specific data we store with Template.
	 *
	...
	public function save_template_data( array $data, \WP_Block_Template $template_post ): ?\WP_Error {
		if ( WooEmailTemplate::TEMPLATE_SLUG === $template_post->slug && isset( $data['sender_settings'] ) ) {
			...
			if ( null !== $new_from_name ) {
				update_option( 'fincommerce_email_from_name', $new_from_name );
			}
			...
			if ( null === $new_from_address || ! filter_var( $new_from_address, FILTER_VALIDATE_EMAIL ) ) {
				return new \WP_Error( 'invalid_email_address', __( 'Invalid email address provided for sender settings', 'fincommerce' ), array( 'status' => 400 ) );
			}

			update_option( 'fincommerce_email_from_address', $new_from_address );
		}
