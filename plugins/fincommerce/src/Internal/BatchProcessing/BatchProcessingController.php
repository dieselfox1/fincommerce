 ...
 *    or an instance must be provided via the 'fincommerce_get_batch_processor' filter.
...
namespace Automattic\FinCommerce\Internal\BatchProcessing;

/**
 * Class BatchProcessingController
 *
 * @package Automattic\FinCommerce\Internal\BatchProcessing.
 */
class BatchProcessingController {
	/*
	...
	private function schedule_watchdog_action( bool $with_delay = false, bool $unique = false ): void {
		...
		if ( $with_delay ) {
			 ...
			 */
			$time += apply_filters( 'fincommerce_batch_processor_watchdog_delay_seconds', HOUR_IN_SECONDS );
		}
	...
	private function get_processor_instance( string $processor_class_name ): BatchProcessorInterface {
		 ...
		 * @since 6.8.0.
		 */
		$processor = apply_filters( 'fincommerce_get_batch_processor', $processor, $processor_class_name );
