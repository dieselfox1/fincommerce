...
namespace Automattic\FinCommerce\Internal\TransientFiles;
...
use Automattic\FinCommerce\Utilities\TimeUtil;
 ...
 * wp-content/uploads/fincommerce_transient_files/yyyy-mm-dd, where "yyyy-mm-dd" is the expiration date
 ...
 * available in the FinCommerce tools page. The action runs once per day but this can be customized
 * via a dedicated hook.
 */
class TransientFilesEngine implements RegisterHooksInterface {

	private const CLEANUP_ACTION_NAME  = 'fincommerce_expired_transient_files_cleanup';
	...
	public function register() {
		...
		add_filter( 'fincommerce_debug_tools', array( $this, 'add_debug_tools_entries' ), 999, 1 );
	...
	public function get_transient_files_directory(): string {
		...
		$default_transient_files_directory = untrailingslashit( $upload_dir_info['basedir'] ) . '/fincommerce_transient_files';
		 ...
		 * @since 8.5.0
		 */
		$transient_files_directory = apply_filters( 'fincommerce_transient_files_directory', $default_transient_files_directory );
	 ...
	 * (but this can be changed via the 'fincommerce_delete_expired_transient_files_interval' filter).
	 *
	...
	public function handle_expired_files_cleanup_action(): void {
		...
		try {
			...
			if ( is_null( $new_interval ) ) {
				 ...
				 * @since 8.5.0
				 */
				$new_interval = apply_filters( 'fincommerce_delete_expired_transient_files_interval', DAY_IN_SECONDS );
			}
	...
	/**
	 * Add the tools to (re)schedule and un-schedule the expired files cleanup actions in the FinCommerce debug tools page.
	 *
	...
	public function add_debug_tools_entries( array $tools_array ): array {
		...
		$tools_array['schedule_expired_transient_files_cleanup'] = array(
			'name'             => $cleanup_is_scheduled ?
				__( 'Re-schedule expired transient files cleanup', 'fincommerce' ) :
				__( 'Schedule expired transient files cleanup', 'fincommerce' ),
			'desc'             => $cleanup_is_scheduled ?
				__( 'Remove the currently scheduled action to delete expired transient files, then schedule it again for running immediately. Subsequent actions will run once every 24h.', 'fincommerce' ) :
				__( 'Schedule the action to delete expired transient files for running immediately. Subsequent actions will run once every 24h.', 'fincommerce' ),
			'button'           => $cleanup_is_scheduled ?
				__( 'Re-schedule', 'fincommerce' ) :
		...
		if ( $cleanup_is_scheduled ) {
			$tools_array['unschedule_expired_transient_files_cleanup'] = array(
				'name'             => __( 'Un-schedule expired transient files cleanup', 'fincommerce' ),
				'desc'             => __( "Remove the currently scheduled action to delete expired transient files. Expired files won't be automatically deleted until the 'Schedule expired transient files cleanup' tool is run again.", 'fincommerce' ),
				'button'           => __( 'Un-schedule', 'fincommerce' ),
				'requires_refresh' => true,
	...
	private function serve_file_contents( string $file_name ) {
		...
		try {
			 ...
			 * @since 8.5.0
			 */
			do_action( 'fincommerce_transient_file_contents_served', $file_name );
		} catch ( Exception $e ) {
