...
namespace Automattic\FinCommerce\Internal\Traits;

use Automattic\FinCommerce\Vendor\Detection\MobileDetect;
use Exception;
use WC_Meta_Data;
use WC_Order;
use WP_Post;
...
trait OrderAttributionMeta {
	...
	private function get_hpos_order_object( $post_or_order ) {
		...
		if ( ! $post_or_order instanceof WC_Order ) {
			throw new Exception( __( 'Order not found.', 'fincommerce' ) );
		}
		...
		switch ( $source_type ) {
			case 'utm':
				$label = $translated ?
					...
					__( 'Source: %s', 'fincommerce' )
					: 'Source: %s';
				break;
			case 'organic':
				$label = $translated ?
					/* translators: %s is the source value */
					__( 'Organic: %s', 'fincommerce' )
					: 'Organic: %s';
				break;
			case 'referral':
				$label = $translated ?
					/* translators: %s is the source value */
					__( 'Referral: %s', 'fincommerce' )
					: 'Referral: %s';
				break;
			case 'typein':
				$label  = '';
				$source = $translated ?
					__( 'Direct', 'fincommerce' )
					: 'Direct';
				break;
			case 'mobile_app':
				$label  = '';
				$source = $translated ?
					__( 'Mobile app', 'fincommerce' )
					: 'Mobile app';
				break;
			case 'admin':
				$label  = '';
				$source = $translated ?
					__( 'Web admin', 'fincommerce' )
					: 'Web admin';
				break;
			case 'pos':
				$label  = '';
				$source = $translated ?
					__( 'Point of Sale', 'fincommerce' )
					: 'Point of Sale';
				break;
	...
	private function get_field_description( string $field_name ): string {
		...
		$description = sprintf( __( 'Order attribution field: %s', 'fincommerce' ), $field_name );
