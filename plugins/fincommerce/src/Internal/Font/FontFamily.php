	...
	private static function validate_font_family( $font_family ) {
		...
		if ( empty( $font_family['fontFamily'] ) ) {
			return new \WP_Error(
				...
				__( 'The font family name is required.', 'fincommerce' ),
			);
		}
		...
		if ( empty( $font_family['preview'] ) ) {
			return new \WP_Error(
				...
				__( 'The font family preview is required.', 'fincommerce' ),
			);
		}
	}
	...
	public static function insert_font_family( array $font_family_settings ) {
		...
		if ( ! empty( $query->get_posts() ) ) {
			return new \WP_Error(
				...
				sprintf( __( 'A font family with slug "%s" already exists.', 'fincommerce' ), $font_family['slug'] )
			);
		}
