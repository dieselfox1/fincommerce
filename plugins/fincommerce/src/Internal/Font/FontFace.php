...
class FontFace {
	...
	private static function handle_font_file_upload_error( $file, $message ) {
		...
		if ( __( 'Sorry, you are not allowed to upload this file type.', 'fincommerce' ) === $message ) {
			$status = 400;
	...
	public static function insert_font_face( array $font_face, int $parent_font_family_id ) {
		...
		if ( ! empty( $query->get_posts() ) ) {
			return new \WP_Error(
				...
				sprintf( __( 'A font face with slug "%s" already exists.', 'fincommerce' ), $slug ),
			);
		}
	...
	private static function validate_font_face( $font_face ) {
		...
		if ( empty( $font_face['fontFamily'] ) ) {
			return new \WP_Error(
				...
				__( 'The font face family name is required.', 'fincommerce' ),
			);
		}
		...
		if ( empty( $font_face['fontStyle'] ) ) {
			return new \WP_Error(
				...
				__( 'The font face font style is required.', 'fincommerce' ),
			);
		}
		...
		if ( empty( $font_face['fontWeight'] ) ) {
			return new \WP_Error(
				...
				__( 'The font face weight is required.', 'fincommerce' ),
			);
		}
		...
		if ( empty( $font_face['src'] ) ) {
			return new \WP_Error(
				...
				__( 'The font face src is required.', 'fincommerce' ),
			);
		}
	}
