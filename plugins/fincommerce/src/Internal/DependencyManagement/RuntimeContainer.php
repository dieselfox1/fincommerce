...
namespace Automattic\FinCommerce\Internal\DependencyManagement;
 ...
 * Instead, all the classes in the Automattic\FinCommerce namespace can be resolved
...
class RuntimeContainer {
	/**
	 * The root namespace of all FinCommerce classes in the `src` directory.
	 *
	 * @var string
	 */
	const fincommerce_NAMESPACE = 'Automattic\\FinCommerce\\';
	 ...
	 *
	 * - $class_name is outside the FinCommerce root namespace (and wasn't included in the initial resolve cache).
	...
	protected function get_core( string $class_name, array &$resolve_chain ) {
		...
		if ( ! $this->is_class_allowed( $class_name ) ) {
			throw new ContainerException( "Attempt to get an instance of class '$class_name', which is not in the " . self::fincommerce_NAMESPACE . ' namespace. Did you forget to add a namespace import?' );
		}
		...
		if ( StringUtil::starts_with( $class_name, 'Automattic\FinCommerce\StoreApi\\' ) ) {
			return StoreApi::container()->get( $class_name );
		}
		if ( StringUtil::starts_with( $class_name, 'Automattic\FinCommerce\Blocks\\' ) ) {
	...
	protected function is_class_allowed( string $class_name ): bool {
		return StringUtil::starts_with( $class_name, self::fincommerce_NAMESPACE, false );
	}
}
