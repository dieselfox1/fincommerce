...
use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Admin\SyncUI;
use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Admin\UI;
use Automattic\FinCommerce\Internal\Utilities\URL;
use Automattic\FinCommerce\Internal\Utilities\URLException;
...
class Register {
	...
	public function add_approved_directory( string $url, bool $enabled = true ): int {
		...
		throw new ApprovedDirectoriesException( __( 'URL could not be added (probable database error).', 'fincommerce' ), ApprovedDirectoriesException::DB_ERROR );
	}
	...
	public function update_approved_directory( int $id, string $url, bool $enabled = true ): bool {
		...
		if ( false === $wpdb->update( $this->get_table(), $fields, array( 'url_id' => $id ) ) ) {
			throw new ApprovedDirectoriesException( __( 'URL could not be updated (probable database error).', 'fincommerce' ), ApprovedDirectoriesException::DB_ERROR );
		}
	...
	private function prepare_url_for_upsert( string $url ): string {
		...
		if ( mb_strlen( $url ) > 256 ) {
			throw new ApprovedDirectoriesException( __( 'Approved directory URLs cannot be longer than 256 characters.', 'fincommerce' ), ApprovedDirectoriesException::INVALID_URL );
		}
