...
namespace Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Admin;

use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Register;
...
class UI {
	...
	final public function init_hooks() {
		...
		add_filter( 'fincommerce_get_sections_products', array( $this, 'add_section' ) );
		add_action( 'load-fincommerce_page_wc-settings', array( $this, 'setup' ) );
		add_action( 'fincommerce_settings_products', array( $this, 'render' ) );
	}
	...
	public function add_section( array $sections ): array {
		$sections['download_urls'] = __( 'Approved download directories', 'fincommerce' );
		return $sections;
	}
	...
	public function render() {
		...
		$this->table->search_box( _x( 'Search', 'Approved Directory URLs', 'fincommerce' ), 'download_url_search' );
		$this->table->display();
	}
	...
	private function display_title() {
		$turn_on_off = $this->register->get_mode() === Register::MODE_ENABLED
			? '<a href="' . esc_url( $this->table->get_action_url( 'turn-off', 0 ) ) . '" class="page-title-action">' . esc_html_x( 'Stop Enforcing Rules', 'Approved product download directories', 'fincommerce' ) . '</a>'
			: '<a href="' . esc_url( $this->table->get_action_url( 'turn-on', 0 ) ) . '" class="page-title-action">' . esc_html_x( 'Start Enforcing Rules', 'Approved product download directories', 'fincommerce' ) . '</a>';
	...
	private function edit_screen( int $url_id ) {
		...
		if ( 0 !== $url_id && ! $existing ) {
			WC_Admin_Settings::add_error( _x( 'The provided ID was invalid.', 'Approved product download directories', 'fincommerce' ) );
		...
		?>
			<h2 class='wc-table-list-header'>
				...
				<?php if ( $existing ) : ?>
					<a href="<?php echo esc_url( $this->table->get_action_url( 'edit', 0 ) ); ?>" class="page-title-action"><?php esc_html_e( 'Add New', 'fincommerce' ); ?></a>
				<?php endif; ?>
				<a href="<?php echo esc_url( $this->table->get_base_url() ); ?>" class="page-title-action"><?php esc_html_e( 'Cancel', 'fincommerce' ); ?></a>
			</h2>
			<table class='form-table'>
				<tbody>
					<tr valign='top'>
						<th scope='row' class='titledesc'>
							<label for='approved_directory_url'><?php echo esc_html_x( 'Directory URL', 'Approved product download directories', 'fincommerce' ); ?></label>
						</th>
						<td class='forminp'>
					...
					<tr valign='top'>
						<th scope='row' class='titledesc'>
							<label for='approved_directory_enabled'><?php echo esc_html_x( 'Enabled', 'Approved product download directories', 'fincommerce' ); ?></label>
						</th>
						<td class='forminp'>
	...
	private function admin_notices() {
		...
		if ( $successfully_deleted ) {
			WC_Admin_Settings::add_message(
				sprintf(
					/* translators: %d: count */
					_n( '%d approved directory URL deleted.', '%d approved directory URLs deleted.', $successfully_deleted, 'fincommerce' ),
			...
			WC_Admin_Settings::add_message(
				sprintf(
				/* translators: %d: count */
					_n( '%d approved directory URL enabled.', '%d approved directory URLs enabled.', $successfully_enabled, 'fincommerce' ),
			...
			WC_Admin_Settings::add_message(
				sprintf(
				/* translators: %d: count */
					_n( '%d approved directory URL disabled.', '%d approved directory URLs disabled.', $successfully_disabled, 'fincommerce' ),
		...
		if ( $failed_updates ) {
			WC_Admin_Settings::add_error(
				sprintf(
					/* translators: %d: count */
					_n( '%d URL could not be updated.', '%d URLs could not be updated.', $failed_updates, 'fincommerce' ),
					$failed_updates
				)
			);
		}
		...
		if ( 'added' === $edit_status ) {
			WC_Admin_Settings::add_message( __( 'URL was successfully added.', 'fincommerce' ) );
		}

		if ( 'updated' === $edit_status ) {
			WC_Admin_Settings::add_message( __( 'URL was successfully updated.', 'fincommerce' ) );
		}
		...
		if ( 'failure' === $edit_status && ! empty( $edit_url ) ) {
			WC_Admin_Settings::add_error(
				sprintf(
					...
					__( '"%s" could not be saved. Please review, ensure it is a valid URL and try again.', 'fincommerce' ),
					$edit_url
				)
			);
		}
	...
	private function security_check() {
		if ( ! Users::is_site_administrator() || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['check'] ?? '' ) ), 'modify_approved_directories' ) ) {
			wp_die( esc_html__( 'You do not have permission to modify the list of approved directories for product downloads.', 'fincommerce' ) );
		}
	}
}
