...
use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\Synchronize;
...
class SyncUI {
	...
	final public function init_hooks() {
		...
		add_filter( 'fincommerce_debug_tools', array( $this, 'add_tools' ) );
	}
	...
	public function add_tools( array $tools ): array {
		...
		if ( ! $sync->in_progress() ) {
			...
			$tools['approved_directories_sync'] = array(
				'name'             => __( 'Synchronize approved download directories', 'fincommerce' ),
				'desc'             => __( 'Updates the list of Approved Product Download Directories. Note that triggering this tool does not impact whether the Approved Download Directories list is enabled or not.', 'fincommerce' ),
				'button'           => __( 'Update', 'fincommerce' ),
				'callback'         => array( $this, 'trigger_sync' ),
				'requires_refresh' => true,
			);

			$tools['approved_directories_clear'] = array(
				'name'             => __( 'Empty the approved download directories list', 'fincommerce' ),
				'desc'             => __( 'Removes all existing entries from the Approved Product Download Directories list.', 'fincommerce' ),
			...
			$tools['cancel_directories_scan'] = array(
				'name'     => __( 'Cancel synchronization of approved directories', 'fincommerce' ),
				'desc'     => sprintf(
				/* translators: %d is an integer between 0-100 representing the percentage complete of the current scan. */
					__( 'The Approved Product Download Directories list is currently being synchronized with the product catalog (%d%% complete). If you need to, you can cancel it.', 'fincommerce' ),
					$sync->get_progress()
				),
				'button'   => __( 'Cancel', 'fincommerce' ),
	...
	public function cancel_sync() {
		$this->security_check();
		wc_get_logger()->log( 'info', __( 'Approved Download Directories sync: scan has been cancelled.', 'fincommerce' ) );
