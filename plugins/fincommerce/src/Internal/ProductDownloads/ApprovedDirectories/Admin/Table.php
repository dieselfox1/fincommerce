...
use Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories\StoredUrl;
use WP_List_Table;
use WP_Screen;
...
class Table extends WP_List_Table {
	...
	public function __construct() {
		...
		add_filter( 'manage_fincommerce_page_wc-settings_columns', array( $this, 'get_columns' ) );
		$this->items_per_page();
		set_screen_options();
	}
	...
	public function no_items() {
		esc_html_e( 'No approved directory URLs found.', 'fincommerce' );
	}
	...
	public function get_columns() {
		return array(
			...
			'title' => _x( 'URL', 'Approved product download directories', 'fincommerce' ),
			'enabled' => _x( 'Enabled', 'Approved product download directories', 'fincommerce' ),
		);
	}
	...
	public function column_title( $item ) {
		...
		$enable_disable_text = esc_html( $enabled ? __( 'Disable', 'fincommerce' ) : __( 'Enable', 'fincommerce' ) );
		$delete_url          = esc_url( $this->get_action_url( 'delete', $id ) );
		$edit_link           = "<a href='{$edit_url}'>" . esc_html_x( 'Edit', 'Product downloads list', 'fincommerce' ) . '</a>';
		$enable_disable_link = "<a href='{$enable_disable_url}'>{$enable_disable_text}</a>";
		$delete_link         = "<a href='{$delete_url}' class='submitdelete wc-confirm-delete'>" . esc_html_x( 'Delete permanently', 'Product downloads list', 'fincommerce' ) . '</a>';
	...
	public function column_enabled( StoredUrl $item ): string {
		return $item->is_enabled()
			? '<mark class="yes" title="' . esc_html__( 'Enabled', 'fincommerce' ) . '"><span class="dashicons dashicons-yes"></span></mark>'
			: '<mark class="no" title="' . esc_html__( 'Disabled', 'fincommerce' ) . '">&ndash;</mark>';
	}
	...
	protected function get_bulk_actions() {
		return array(
			'enable'  => __( 'Enable rule', 'fincommerce' ),
			'disable' => __( 'Disable rule', 'fincommerce' ),
	...
	protected function display_tablenav( $which ) {
		...
		if ( $this->has_items() ) {
			...
			if ( $directories->count( false ) > 0 ) {
				echo '<a href="' . esc_url( $this->get_action_url( 'enable-all', 0 ) ) . '" class="wp-core-ui button">' . esc_html_x( 'Enable All', 'Approved product download directories', 'fincommerce' ) . '</a> ';
			}

			if ( $directories->count( true ) > 0 ) {
				echo '<a href="' . esc_url( $this->get_action_url( 'disable-all', 0 ) ) . '" class="wp-core-ui button">' . esc_html_x( 'Disable All', 'Approved product download directories', 'fincommerce' ) . '</a>';
			}
