...
namespace Automattic\FinCommerce\Internal\ProductDownloads\ApprovedDirectories;
...
class Synchronize {
	 ...
	 */
	public const SYNC_TASK = 'fincommerce_download_dir_sync';

	/**
	 * The group under which synchronization tasks run (our standard 'fincommerce-db-updates' group).
	 */
	public const SYNC_TASK_GROUP = 'fincommerce-db-updates';
	...
	public function init_feature( bool $synchronize = true, bool $enable_feature = true ) {
		try {
			...
			wc_get_logger()->log( 'warning', __( 'It was not possible to synchronize download directories following the most recent update.', 'fincommerce' ) );
		}
	...
	/**
	 * By default we add the fincommerce_uploads directory (file path plus web URL) to the list
	...
	public function add_default_directories() {
		...
		$this->register->add_approved_directory( $upload_dir['basedir'] . '/fincommerce_uploads' );
		$this->register->add_approved_directory( $upload_dir['baseurl'] . '/fincommerce_uploads' );
	}
	...
	public function start(): bool {
		if ( null !== $this->queue->get_next( self::SYNC_TASK ) ) {
			wc_get_logger()->log( 'warning', __( 'Synchronization of approved product download directories is already in progress.', 'fincommerce' ) );
			return false;
		}

		update_option( self::SYNC_TASK_PAGE, 1 );
		$this->queue->schedule_single( time(), self::SYNC_TASK, array(), self::SYNC_TASK_GROUP );
		wc_get_logger()->log( 'info', __( 'Approved Download Directories sync: new scan scheduled.', 'fincommerce' ) );
		return true;
	}
	...
	public function run() {
		...
		if ( count( $products ) < self::SYNC_TASK_BATCH_SIZE ) {
			wc_get_logger()->log( 'info', __( 'Approved Download Directories sync: scan is complete!', 'fincommerce' ) );
			$this->stop();
		} else {
			wc_get_logger()->log(
				'info',
				...
				/* translators: %1$d is the current batch in the synchronization task, %2$d is the percent complete. */
					__( 'Approved Download Directories sync: completed batch %1$d (%2$d%% complete).', 'fincommerce' ),
	...
	private function get_next_set_of_downloadable_products(): array {
		...
		add_filter( 'fincommerce_product_data_store_cpt_get_products_query', $query_filter );
		...
		remove_filter( 'fincommerce_product_data_store_cpt_get_products_query', $query_filter );
	...
	private function process_product( WC_Product $product ) {
		...
		foreach ( $downloads as $downloadable ) {
			$parent_url = _x( 'invalid URL', 'Approved product download URLs migration', 'fincommerce' );
			...
			try {
				 ...
				 */
				if ( apply_filters( 'fincommerce_product_downloads_approved_directory_validation_for_shortcodes', true ) && 'shortcode' === $downloadable->get_type_of_file_path() ) {
				...
				wc_get_logger()->log(
					...
					/* translators: %s is a URL, %d is a product ID. */
						__( 'Product download migration: %1$s (for product %1$d) could not be added to the list of approved download directories.', 'fincommerce' ),
						$parent_url,
						$product->get_id()
					)
				);
