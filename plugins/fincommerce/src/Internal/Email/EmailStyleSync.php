...
namespace Automattic\FinCommerce\Internal\Email;
...
class EmailStyleSync implements RegisterHooksInterface {
	 ...
	 */
	const AUTO_SYNC_OPTION = 'fincommerce_email_auto_sync_with_theme';
	...
	protected function update_email_colors() {
		...
		if ( ! empty( $colors['base'] ) ) {
			update_option( 'fincommerce_email_base_color', $colors['base'] );
		}

		if ( ! empty( $colors['bg'] ) ) {
			update_option( 'fincommerce_email_background_color', $colors['bg'] );
		}

		if ( ! empty( $colors['body_bg'] ) ) {
			update_option( 'fincommerce_email_body_background_color', $colors['body_bg'] );
		}

		if ( ! empty( $colors['body_text'] ) ) {
			update_option( 'fincommerce_email_text_color', $colors['body_text'] );
		}

		if ( ! empty( $colors['footer_text'] ) ) {
			update_option( 'fincommerce_email_footer_text_color', $colors['footer_text'] );
		}
	}
}
