...
namespace Automattic\FinCommerce\Internal\DataStores\Orders;

use Automattic\Jetpack\Constants;
use Automattic\FinCommerce\Caches\OrderCache;
use Automattic\FinCommerce\Caching\WPCacheEngine;
use Automattic\FinCommerce\Enums\OrderInternalStatus;
use Automattic\FinCommerce\Internal\Admin\Orders\EditLock;
...
class OrdersTableDataStore extends \Abstract_WC_Order_Data_Store_CPT implements \WC_Object_Data_Store_Interface, \WC_Order_Data_Store_Interface {
	...
	/**
	 * Order IDs for which we are checking sync on read in the current request. In FinCommerce, using wc_get_order is a very common pattern, to avoid performance issues, we only sync on read once per request per order. This works because we consider out of sync orders to be an anomaly, so we don't recommend running HPOS with incompatible plugins.
	 *
	 * @var array
	 */
	...
	public static function get_all_table_names_with_id() {
		...
		return array(
			...
			'items'            => $wpdb->prefix . 'fincommerce_order_items',
		);
	}
	...
	public function backfill_post_record( $order ) {
		...
		if ( $order->get_id() && is_null( get_post( $order->get_id() ) ) ) {
			if ( ! $this->maybe_create_backup_post( $order, 'backfill' ) ) {
				...
				$this->error_logger->warning( sprintf( __( 'Unable to create backup post for order %d.', 'fincommerce' ), $order->get_id() ) );
				return;
			}
		}
		 ...
		 */
		do_action( 'fincommerce_hpos_post_record_backfilled', $order );
	}
	...
	public function update_order_from_object( $order ) {
		...
		add_filter( 'fincommerce_orders_table_datastore_should_save_after_meta_change', '__return_false' );
		$hpos_order->save_meta_data();
		remove_filter( 'fincommerce_orders_table_datastore_should_save_after_meta_change', '__return_false' );
...
",
	...
	public function get_total_tax_refunded( $order ) {
		...
		$total = $wpdb->get_var(
			...
			$wpdb->prepare(
				...
				FROM {$wpdb->prefix}fincommerce_order_itemmeta AS order_itemmeta
				INNER JOIN $order_table AS orders ON ( orders.type = 'shop_order_refund' AND orders.parent_order_id = %d )
				INNER JOIN {$wpdb->prefix}fincommerce_order_items AS order_items ON ( order_items.order_id = orders.id AND order_items.order_item_type = 'tax' )
	...
	public function get_total_shipping_refunded( $order ) {
		...
		$total = $wpdb->get_var(
			...
			$wpdb->prepare(
				...
				FROM {$wpdb->prefix}fincommerce_order_itemmeta AS order_itemmeta
				INNER JOIN $order_table AS orders ON ( orders.type = 'shop_order_refund' AND orders.parent_order_id = %d )
				INNER JOIN {$wpdb->prefix}fincommerce_order_items AS order_items ON ( order_items.order_id = orders.id AND order_items.order_item_type = 'shipping' )
				WHERE order_itemmeta.order_item_id = order_items.order_item_id
	...
	public function get_order_count( $status ) {
		...
		return absint( $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$orders_table} WHERE type = %s AND status = %s", 'shop_order', $status ) ) ); // phpcs:ignore finpress.DB.PreparedSQL.InterpolatedNotPrepared
	}
	 ...
	 *
	 * @param  int $date This timestamp is expected in the timezone in finpress settings for legacy reason, even though it's not a good practice.
	 *
	...
	public function search_orders( $term ) {
		 ...
		 * is `fincommerce_shop_order_search_results`.
		 *
		 * @since 7.0.0
		 *
		 ...
		 */
		return array_map( 'intval', (array) apply_filters( 'fincommerce_cot_shop_order_search_results', $order_ids, $term ) );
	}
	...
	public function read_multiple( &$orders ) {
		...
		if ( count( $data ) !== count( $order_ids ) ) {
			throw new \Exception( esc_html__( 'Invalid order IDs in call to read_multiple()', 'fincommerce' ) );
		}
		...
		if ( $data_sync_enabled ) {
			 ...
			 * @since 8.1.0
			 */
			$data_sync_enabled = apply_filters( 'fincommerce_hpos_enable_sync_on_read', $data_sync_enabled );
		}
	...
	private function read_cogs_data( WC_Abstract_Order $order ) {
		 ...
		 */
		$cogs_value = apply_filters( 'fincommerce_load_order_cogs_value', $cogs_value, $order );
				...
				try {
					...
					$this->error_logger->warning(
						sprintf(
							...
							__( 'Unable to load the post record for order %1$d', 'fincommerce' ),
							$order_id
						),
						array(
	...
	private function migrate_post_record( \WC_Abstract_Order &$order, \WC_Abstract_Order $post_order ): void {
		 ...
		 */
		do_action( 'fincommerce_hpos_post_record_migrated_on_read', $order, $diff );
	}
				...
				try {
					...
					$this->error_logger->warning(
						...
						/* translators: %1$d = peoperty name, %2$d = order ID, %3$s = error message. */
							__( 'Error when setting property \'%1$s\' for order %2$d: %3$s', 'fincommerce' ),
							$prop_details['name'],
							$order_id,
							$e->getMessage()
						),
	...
	private function get_order_data_for_ids_from_db( array $ids ): array {
		...
		// phpcs:disable finpress.DB.PreparedSQL.InterpolatedNotPrepared, finpress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- $order_table_query is autogenerated and should already be prepared.
...
FROM $order_meta_table
	...
	private function join_address_table_order_query( $address_type, $order_table_alias, $address_table_alias ) {
		...
		// phpcs:disable finpress.DB.PreparedSQL.InterpolatedNotPrepared -- $clauses['join'] and $address_table_alias are hardcoded.
	...
	protected function persist_order_to_db( &$order, bool $force_all_fields = false ) {
		...
		if ( 'create' === $context ) {
			...
			if ( ! $post_id ) {
				throw new \Exception( esc_html__( 'Could not create order in posts table.', 'fincommerce' ) );
			}
		...
		foreach ( $db_updates as $update ) {
			...
			if ( false === $result ) {
				...
				throw new \Exception( esc_html( sprintf( __( 'Could not persist order to database table "%s".', 'fincommerce' ), $update['table'] ) ) );
			}
		}
	...
	private function save_cogs_data( WC_Abstract_Order $order ) {
		 ...
		 */
		$cogs_value = apply_filters( 'fincommerce_save_order_cogs_value', $cogs_value, $order );
	...
	public function set_custom_taxonomies( \WC_Abstract_Order $order, array $sanitized_tax_input ) {
		...
		foreach ( $sanitized_tax_input as $taxonomy => $tags ) {
			...
			if ( ! $taxonomy_obj ) {
				...
				_doing_it_wrong( __FUNCTION__, esc_html( sprintf( __( 'Invalid taxonomy: %s.', 'fincommerce' ), $taxonomy ) ), '7.9.0' );
				continue;
			}
	...
	protected function get_db_rows_for_order( \WC_Abstract_Order $order, string $context = 'create', bool $only_changes = false ): array {
		 ...
		 */
		$ext_rows = apply_filters( 'fincommerce_orders_table_datastore_extra_db_rows_for_order', array(), $order, $context );

		/**
		 * Filters the rows that are going to be inserted or updated during an order save.
		 *
		 * @since 8.8.0
		 * @internal Use 'fincommerce_orders_table_datastore_extra_db_rows_for_order' for adding rows to the database save.
		 *
		 * @param array     $rows    Array of rows to be inserted/updated. See 'fincommerce_orders_table_datastore_extra_db_rows_for_order' for exact format.
		 * @param \WC_Order $order   The order object.
		 * @param string    $context The context of the operation: 'create' or 'update'.
		 */
		$result = apply_filters(
			'fincommerce_orders_table_datastore_db_rows_for_order',
	...
	public function delete( &$order, $args = array() ) {
		...
		if ( $args['force_delete'] ) {
			...
			if ( $do_filters ) {
				 ...
				 */
				do_action( 'fincommerce_before_delete_order', $order_id, $order );
			}
			...
			if ( $do_filters ) {
				 ...
				 */
				do_action( 'fincommerce_delete_order', $order_id ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
			}
		} else {
			if ( $do_filters ) {
				/**
				 ...
				 */
				do_action( 'fincommerce_before_trash_order', $order_id, $order );
			}
			...
			if ( $do_filters ) {
				 ...
				 */
				do_action( 'fincommerce_trash_order', $order_id ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
			}
		}
	}
	...
	public function untrash_order( WC_Order $order ): bool {
		...
		if ( 'trash' !== $status ) {
			wc_get_logger()->warning(
				sprintf(
					...
					__( 'Order %1$d cannot be restored from the trash: it has already been restored to status "%2$s".', 'fincommerce' ),
					$id,
					$status
				)
			);
		...
		if ( $previous_state_is_invalid && $pending_is_valid_status ) {
			...
			wc_get_logger()->warning(
				sprintf(
					/* translators: 1: order ID, 2: order status */
					__( 'The previous status of order %1$d ("%2$s") is invalid. It has been restored to "pending" status instead.', 'fincommerce' ),
					$id,
					$previous_status
				)
			);
			...
			wc_get_logger()->warning(
				sprintf(
					/* translators: 1: order ID, 2: order status */
					__( 'The previous status of order %1$d ("%2$s") is invalid. It could not be restored.', 'fincommerce' ),
					$id,
					$previous_status
				)
			);
		 ...
		 */
		do_action( 'fincommerce_untrash_order', $order->get_id(), $previous_status );
		...
		wc_get_logger()->warning(
			sprintf(
				/* translators: 1: order ID, 2: order status */
				__( 'Something went wrong when trying to restore order %d from the trash. It could not be restored.', 'fincommerce' ),
				$id
			)
		);
	...
	public function create( &$order ) {
		...
		// Do not fire 'fincommerce_new_order' for draft statuses for backwards compatibility.
		 ...
		 */
		do_action( 'fincommerce_new_order', $order->get_id(), $order );
	}
	...
	protected function persist_save( &$order, bool $force_all_fields = false, $backfill = true ) {
		...
		$order->set_currency( $order->get_currency() ? $order->get_currency() : get_fincommerce_currency() );
	...
	public function update( &$order ) {
		...
		if (
			...
			&& $order->has_status( apply_filters( 'fincommerce_payment_complete_order_status', $order->needs_processing() ? 'processing' : 'completed', $order->get_id(), $order ) ) // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
		) {
		...
		if (
			...
			do_action( 'fincommerce_new_order', $order->get_id(), $order ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
			return;
		}
		...
		do_action( 'fincommerce_update_order', $order->get_id(), $order ); // phpcs:ignore FinCommerce.Commenting.CommentHooks.MissingHookComment
	}
	...
	public function query( $query_vars ) {
		...
		if ( isset( $query_vars['anonymized'] ) ) {
			$query_vars['meta_query'] = $query_vars['meta_query'] ?? array(); // phpcs:ignore finpress.DB.SlowDBQuery.slow_db_query_meta_query
...
CREATE TABLE $operational_data_table_name (
	...
	fincommerce_version varchar(20) NULL,
...
";
	...
	public function delete_meta( &$object, $meta ) { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.objectFound
		...
		if ( ! $changes_applied && $object instanceof WC_Abstract_Order && $this->should_backfill_post_record() && isset( $meta->key ) ) {
			...
			if ( is_object( $meta->value ) && '__PHP_Incomplete_Class' === get_class( $meta->value ) ) {
				...
				$logger->warning( sprintf( 'encountered an order meta value of type __PHP_Incomplete_Class during `delete_meta` in order with ID %d: "%s"', $object->get_id(), var_export( $meta_value, true ) ) ); // phpcs:ignore finpress.PHP.DevelopmentFunctions.error_log_var_export
			} else {
	...
	private function should_save_after_meta_change( $order, $meta = null ) {
		 ...
		 */
		return apply_filters( 'fincommerce_orders_table_datastore_should_save_after_meta_change', $should_save );
	}
}
