...
use Automattic\FinCommerce\Internal\BatchProcessing\{ BatchProcessingController, BatchProcessorInterface };
...
class DataSynchronizer implements BatchProcessorInterface {
	...
	public const ORDERS_DATA_SYNC_ENABLED_OPTION = 'fincommerce_custom_orders_table_data_sync_enabled';
	...
	public const ORDERS_TABLE_CREATED = 'fincommerce_custom_orders_table_created';
	...
	public const BACKGROUND_SYNC_MODE_OPTION     = 'fincommerce_custom_orders_table_background_sync_mode';
	public const BACKGROUND_SYNC_INTERVAL_OPTION = 'fincommerce_custom_orders_table_background_sync_interval';
	...
	public const BACKGROUND_SYNC_EVENT_HOOK      = 'fincommerce_custom_orders_table_background_sync';
	...
	public function __construct() {
		...
		add_action( 'fincommerce_new_order', array( $this, 'handle_updated_order' ), 100 );
		add_action( 'fincommerce_refund_created', array( $this, 'handle_updated_order' ), 100 );
		add_action( 'fincommerce_update_order', array( $this, 'handle_updated_order' ), 100 );
	...
	public function get_ignored_order_props() {
		 ...
		 * @since 8.6.0
		 */
		$ignored_props = apply_filters( 'fincommerce_hpos_sync_ignored_order_props', array() );
	...
	public function get_current_orders_pending_sync_count( $use_cache = false ): int {
		if ( $use_cache ) {
			$pending_count = wp_cache_get( 'fincommerce_hpos_pending_sync_count', 'counts' );
		...
		wp_cache_set( 'fincommerce_hpos_pending_sync_count', $pending_count, 'counts' );
	...
	public function has_orders_pending_sync( $use_cache = false ) {
		if ( $use_cache ) {
			$has_pending_sync = wp_cache_get( 'fincommerce_hpos_has_orders_pending_sync', 'counts' );
		...
		wp_cache_set( 'fincommerce_hpos_has_orders_pending_sync', $has_pending_sync, 'counts' );
		...
		if ( empty( $order_post_types ) ) {
			$this->error_logger->debug(
				sprintf(
					...
					esc_html__( '%1$s was called but no order types were registered: it may have been called too early.', 'fincommerce' ),
					__METHOD__
				)
			);
...
",
	...
	public function cleanup_synchronization_state() {
		delete_option( 'fincommerce_initial_orders_pending_sync_count' );
	}
	...
	public function get_default_batch_size(): int {
		 ...
		 */
		return apply_filters( 'fincommerce_orders_cot_and_posts_sync_step_size', $batch_size );
	}
	...
	/**
	 * Handle the 'fincommerce_update_order' action.
	 *
	...
	public function delete_auto_draft_orders() {
		 ...
		 * @since 7.7.0
		 */
		do_action( 'fincommerce_scheduled_auto_draft_delete' );
	}
