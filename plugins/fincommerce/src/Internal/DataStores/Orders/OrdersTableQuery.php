...
class OrdersTableQuery {
	...
	/**
	 * Lets the `fincommerce_hpos_pre_query` filter override the query.
	 *
	...
	private function maybe_override_query(): bool {
		 ...
		 */
		$pre_query = apply_filters( 'fincommerce_hpos_pre_query', null, $this, $this->sql );
	...
	private function maybe_remap_args(): void {
		$mapping = array(
			...
			'order_version'       => 'fincommerce_version',
	...
	private function date_to_date_query_arg( $date ): array {
		...
		if ( is_numeric( $date ) ) {
			...
			// For backwards compat (see https://github.com/dieselfox1/fincommerce/wiki/wc_get_orders-and-WC_Order_Query#date)
	...
	private function local_time_to_gmt_date_query( $dates_raw, $operator ) {
		...
		// Convert YYYY-MM-DD to UTC timestamp. Per https://github.com/dieselfox1/fincommerce/wiki/wc_get_orders-and-WC_Order_Query#date only date is relevant (time is ignored).
		...
		foreach ( $date_keys as $date_key ) {
			...
			if ( $is_local ) {
				...
				if ( ! is_numeric( $dates_raw[0] ) && ( ! isset( $dates_raw[1] ) || ! is_numeric( $dates_raw[1] ) ) ) {
					// Only non-numeric args can be considered local time. Timestamps are assumed to be UTC per https://github.com/dieselfox1/fincommerce/wiki/wc_get_orders-and-WC_Order_Query#date.
	...
	private function build_query(): void {
		...
		if ( ! $this->suppress_filters ) {
			 ...
			 */
			$clauses = (array) apply_filters_ref_array( 'fincommerce_orders_table_query_clauses', array( $pieces, &$this, $this->args ) );
		...
		if ( ! $this->suppress_filters ) {
			 ...
			 * @param array            $args  Query args.
			 */
			$this->sql = apply_filters_ref_array( 'fincommerce_orders_table_query_sql', array( $this->sql, &$this, $this->args ) );
		}
	...
	private function build_count_query( $fields, $join, $where, $groupby ) {
		...
		if ( ! $this->suppress_filters ) {
			 ...
			 * @param string           $groupby Prepared GROUP BY clause.
			 */
			$this->count_sql = apply_filters_ref_array( 'fincommerce_orders_table_query_count_sql', array( $this->count_sql, &$this, $this->args, $fields, $join, $where, $groupby ) );
		}
	}
	...
	private function join( string $table, string $alias = '', string $on = '', string $join_type = 'inner', bool $alias_once = false ) {
		...
		if ( $this->tables['orders'] === $alias ) {
			...
			throw new \Exception( sprintf( __( '%s can not be used as a table alias in OrdersTableQuery', 'fincommerce' ), $alias ) );
		}
		...
		if ( isset( $this->join[ $alias ] ) ) {
			if ( ! $alias_once ) {
				// translators: %s is a table name.
				throw new \Exception( sprintf( __( 'Can not re-use table alias "%s" in OrdersTableQuery.', 'fincommerce' ), $alias ) );
			}
	...
	public function get_table_name( string $table_id = '' ): string {
		if ( ! isset( $this->tables[ $table_id ] ) ) {
			...
			throw new \Exception( sprintf( __( 'Invalid table id: %s.', 'fincommerce' ), $table_id ) );
		}
