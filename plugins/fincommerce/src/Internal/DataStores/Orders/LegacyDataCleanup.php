...
class LegacyDataCleanup implements BatchProcessorInterface {
	 ...
	 * @deprecated 9.1.0
	 */
	public const OPTION_NAME = 'fincommerce_hpos_legacy_data_cleanup_in_progress';
	...
	public function process_batch( array $batch ): void {
			...
			try {
				...
				$this->error_logger->error(
					sprintf(
						...
						__( 'Order %1$d legacy data could not be cleaned up during batch process. Error: %2$s', 'fincommerce' ),
						$order_id,
						$e->getMessage()
					)
				);
		...
		if ( $batch_failed ) {
			$this->error_logger->error( __( 'Order legacy cleanup failed for an entire batch of orders. Aborting cleanup.', 'fincommerce' ) );
		}
	...
	/**
	 * Returns an array in format required by 'fincommerce_debug_tools' to register the cleanup tool in WC.
	 *
	...
	public function get_tools_entries() {
		...
		$entry                    = array(
			'name'             => __( 'Clean up order data from legacy tables', 'fincommerce' ),
			'desc'             => __( 'This tool will clear the data from legacy order tables in FinCommerce.', 'fincommerce' ),
			'requires_refresh' => true,
			'button'           => __( 'Clear data', 'fincommerce' ),
		...
		if ( ! $this->can_run() ) {
			...
			$entry['desc'] .= sprintf(
				...
				__( 'Note:', 'fincommerce' ),
				__( 'Only available when HPOS is authoritative and compatibility mode is disabled.', 'fincommerce' )
			);
		} else {
			if ( $this->is_flag_set() ) {
				$entry['status_text'] = sprintf(
					...
					__( 'Clearing data...', 'fincommerce' )
				);
				$entry['button']      = __( 'Cancel', 'fincommerce' );
				$entry['callback']    = function() {
					$this->toggle_flag( false );
					return __( 'Order legacy data cleanup has been canceled.', 'fincommerce' );
				};
			} elseif ( ! $orders_for_cleanup_exist ) {
				$entry['button'] = __( 'No orders in need of cleanup', 'fincommerce' );
			} else {
				$entry['callback'] = function() {
					$this->toggle_flag( true );
					return __( 'Order legacy data cleanup process has been started.', 'fincommerce' );
				};
			}
		}
