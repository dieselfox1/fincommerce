...
class OrdersTableSearchQuery {
	...
	private function generate_join_for_search_filter( $search_filter ): string {
		 ...
		 * Provide a JOIN clause for a new search filter. This should be used along with `fincommerce_hpos_admin_search_filters`
		 * to declare a new custom filter, and `fincommerce_hpos_generate_where_for_search_filter` to generate the WHERE
		 * clause.
		 *
		...
		return apply_filters(
			'fincommerce_hpos_generate_join_for_search_filter',
			$join,
			$this->search_term,
			$search_filter,
			$this->query
	...
	private function maybe_get_join_for_customers(): string {
		...
		$maybe_phone_field = '';
		if ( version_compare( get_option( 'fincommerce_db_version' ), '9.4.0', '>=' ) ) {
	...
	private function generate_where_for_search_filter( string $search_filter ): string {
		 ...
		 * `fincommerce_hpos_admin_search_filters` to declare a new custom filter, and optionally also with the
		 * `fincommerce_hpos_generate_join_for_search_filter` filter if a join is also needed.
		 *
		 ...
		 */
		return apply_filters(
			'fincommerce_hpos_generate_where_for_search_filter',
			'',
			$this->search_term,
			$search_filter,
			$this->query
...
",
	...
	private function get_meta_fields_to_be_searched(): string {
		 ...
		 * is 'fincommerce_shop_order_search_fields'.
		 *
		 * @since 7.0.0
		 *
		 * @param array
		...
		$meta_keys = apply_filters(
			'fincommerce_order_table_search_query_meta_keys',
