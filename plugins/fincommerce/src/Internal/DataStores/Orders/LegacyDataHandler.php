...
class LegacyDataHandler {
	...
	public function cleanup_post_data( int $order_id, bool $skip_checks = false ): void {
		...
		if ( ! in_array( $post_type, array_merge( wc_get_order_types( 'cot-migration' ), array( $this->data_synchronizer::PLACEHOLDER_ORDER_POST_TYPE ) ), true ) ) {
			...
			throw new \Exception( esc_html( sprintf( __( '%d is not of a valid order type.', 'fincommerce' ), $order_id ) ) );
		}
		...
		if ( $order_exists ) {
			...
			if ( ! $order ) {
				// translators: %d is an order ID.
				throw new \Exception( esc_html( sprintf( __( '%d is not a valid order ID.', 'fincommerce' ), $order_id ) ) );
			}

			if ( ! $skip_checks && ! $this->is_order_newer_than_post( $order ) ) {
				// translators: %1 is an order ID.
				throw new \Exception( esc_html( sprintf( __( 'Data in posts table appears to be more recent than in HPOS tables. Compare order data with `wp wc hpos diff %1$d` and use `wp wc hpos backfill %1$d --from=posts --to=hpos` to fix.', 'fincommerce' ), $order_id ) ) );
			}
		}
	...
	private function is_order_newer_than_post( \WC_Abstract_Order $order ): bool {
		if ( ! is_a( $order->get_data_store()->get_current_class_name(), OrdersTableDataStore::class, true ) ) {
			throw new \Exception( esc_html__( 'Order is not an HPOS order.', 'fincommerce' ) );
		}
	...
	public function get_diff_for_order( int $order_id ): array {
		...
		foreach ( $all_keys as $key ) {
			...
			// Workaround for https://github.com/dieselfox1/fincommerce/issues/43126.
	...
	public function get_order_from_datastore( int $order_id, string $data_store_id = 'hpos' ) {
		...
		if ( ! $order_type ) {
			// translators: %d is an order ID.
			throw new \Exception( esc_html( sprintf( __( '%d is not an order or has an invalid order type.', 'fincommerce' ), $order_id ) ) );
		}
		...
		add_filter( 'fincommerce_hpos_enable_sync_on_read', $prevent_sync_on_read, 999 );
		try {
			$data_store->read( $order );
		} finally {
			remove_filter( 'fincommerce_hpos_enable_sync_on_read', $prevent_sync_on_read, 999 );
		}
	...
	public function backfill_order_to_datastore( int $order_id, string $source_data_store, string $destination_data_store, array $fields = array() ) {
		...
		if ( ! $datastore || ! method_exists( $datastore, 'update_order_from_object' ) ) {
			throw new \Exception( esc_html__( 'The backup datastore does not support updating orders.', 'fincommerce' ) );
		}
		...
		if ( ! empty( $fields['props'] ) ) {
			...
			if ( 'hpos' === $destination_data_store ) {
				...
				add_filter( 'fincommerce_orders_table_datastore_db_rows_for_order', $limit_cb, 10, 2 );
			}
		}
		...
		if ( 'hpos' === $destination_data_store && isset( $limit_cb ) ) {
			remove_filter( 'fincommerce_orders_table_datastore_db_rows_for_order', $limit_cb );
		}
	}
	 ...
	 * Called via the `fincommerce_orders_table_datastore_db_rows_for_order` filter in `backfill_order_to_datastore`.
	 *
