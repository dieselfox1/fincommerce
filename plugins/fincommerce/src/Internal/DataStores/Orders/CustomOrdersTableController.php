...
use Automattic\FinCommerce\Caches\OrderCacheController;
 ...
 * - Providing the proper data store for orders via 'fincommerce_order_data_store' hook
 *
...
class CustomOrdersTableController {
	 ...
	 */
	public const CUSTOM_ORDERS_TABLE_USAGE_ENABLED_OPTION = 'fincommerce_custom_orders_table_enabled';

	/**
	 * The name of the option that tells whether database transactions are to be used or not for data synchronization.
	 */
	public const USE_DB_TRANSACTIONS_OPTION = 'fincommerce_use_db_transactions_for_custom_orders_table_data_sync';

	/**
	 * The name of the option to store the transaction isolation level to use when database transactions are enabled.
	 */
	public const DB_TRANSACTIONS_ISOLATION_LEVEL_OPTION = 'fincommerce_db_transactions_isolation_level_for_custom_orders_table_data_sync';

	public const DEFAULT_DB_TRANSACTIONS_ISOLATION_LEVEL = 'READ UNCOMMITTED';

	public const HPOS_FTS_INDEX_OPTION = 'fincommerce_hpos_fts_index_enabled';

	public const HPOS_FTS_ADDRESS_INDEX_CREATED_OPTION = 'fincommerce_hpos_address_fts_index_created';

	public const HPOS_FTS_ORDER_ITEM_INDEX_CREATED_OPTION = 'fincommerce_hpos_order_item_fts_index_created';

	public const HPOS_DATASTORE_CACHING_ENABLED_OPTION = 'fincommerce_hpos_datastore_caching_enabled';
	...
	private function init_hooks() {
		add_filter( 'fincommerce_order_data_store', array( $this, 'get_orders_data_store' ), 999, 1 );
		add_filter( 'fincommerce_order-refund_data_store', array( $this, 'get_refunds_data_store' ), 999, 1 );
		add_filter( 'fincommerce_debug_tools', array( $this, 'add_hpos_tools' ), 999 );
		...
		add_action( 'fincommerce_after_register_post_type', array( $this, 'register_post_type_for_order_placeholders' ), 10, 0 );
		add_action( 'fincommerce_sections_advanced', array( $this, 'sync_now' ) );
		...
		add_action( 'before_fincommerce_init', array( $this, 'maybe_set_order_cache_group_as_non_persistent' ) );
	}
	 ...
	 *
	 * @param \WC_Object_Data_Store_Interface|string $default_data_store The default data store (as received via the fincommerce_order_data_store hook).
	 *
	 ...
	 *
	 * @param \WC_Object_Data_Store_Interface|string $default_data_store The default data store (as received via the fincommerce_order-refund_data_store hook).
	 *
	 * @return \WC_Object_Data_Store_Interface|string The actual data store to use.
	 *
	 * @internal For exclusive usage of FinCommerce core, backwards compatibility not guaranteed.
	...
	public function add_hpos_tools( array $tools_array ): array {
		...
		if ( $this->custom_orders_table_usage_is_enabled() || $this->data_synchronizer->data_sync_is_enabled() || $this->batch_processing_controller->is_enqueued( get_class( $this->data_synchronizer ) ) ) {
			$disabled = true;
			$message  = __( 'This will delete the custom orders tables. The tables can be deleted only if the "High-Performance order storage" is not authoritative and sync is disabled (via Settings > Advanced > Features).', 'fincommerce' );
		} else {
			$disabled = false;
			$message  = __( 'This will delete the custom orders tables. To create them again enable the "High-Performance order storage" feature (via Settings > Advanced > Features).', 'fincommerce' );
		}

		$tools_array['delete_custom_orders_table'] = array(
			'name'             => __( 'Delete the custom orders tables', 'fincommerce' ),
			'desc'             => sprintf(
			...
			'callback'         => function () use ( $disabled ) {
				...
				return __( 'Custom orders tables have been deleted.', 'fincommerce' );
			},
			'button'           => __( 'Delete', 'fincommerce' ),
			'disabled'         => $disabled,
		);
	...
	public function process_updated_option_fts_index( $option, $old_value, $value ) {
		...
		if ( ! $this->custom_orders_table_usage_is_enabled() ) {
			...
			if ( class_exists( 'WC_Admin_Settings' ) ) {
				WC_Admin_Settings::add_error( __( 'Failed to create FTS index on orders table. This feature is only available when High-performance order storage is enabled.', 'fincommerce' ) );
			}
			return;
		}
		...
		if ( $this->db_util->fts_index_on_order_address_table_exists() ) {
			...
			if ( class_exists( 'WC_Admin_Settings ' ) ) {
				WC_Admin_Settings::add_error( __( 'Failed to create FTS index on address table', 'fincommerce' ) );
			}
		}
		...
		if ( $this->db_util->fts_index_on_order_item_table_exists() ) {
			...
			if ( class_exists( 'WC_Admin_Settings ' ) ) {
				WC_Admin_Settings::add_error( __( 'Failed to create FTS index on order item table', 'fincommerce' ) );
			}
		}
	}
	...
	public function recreate_order_address_fts_index(): array {
		...
		if ( $this->db_util->fts_index_on_order_address_table_exists() ) {
			return array(
				'status'  => false,
				'message' => __( 'Failed to modify existing FTS index. Please go to FinCommerce > Status > Tools and run the "Re-create Order Address FTS index" tool.', 'fincommerce' ),
			);
		} else {
			update_option( self::HPOS_FTS_ADDRESS_INDEX_CREATED_OPTION, 'no', false );
		}
		...
		if ( ! $this->db_util->fts_index_on_order_address_table_exists() ) {
			return array(
				'status'  => false,
				'message' => __( 'Failed to create FTS index on order address table. Please go to FinCommerce > Status > Tools and run the "Re-create Order Address FTS index" tool.', 'fincommerce' ),
			);
		} else {
			update_option( self::HPOS_FTS_ADDRESS_INDEX_CREATED_OPTION, 'yes', false );
			return array(
				'status'  => true,
				'message' => __( 'FTS index recreated.', 'fincommerce' ),
			);
		}
	}
	...
	public function sync_now() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ?? '' ) ), "hpos-{$action}" ) ) {
			WC_Admin_Settings::add_error(
				'sync-now' === $action ?
					esc_html__( 'Unable to start synchronization. The link you followed may have expired.', 'fincommerce' )
					: esc_html__( 'Unable to stop synchronization. The link you followed may have expired.', 'fincommerce' )
			);
			return;
		}
		...
		if ( 'sync-now' === $action ) {
			if ( ! $this->data_synchronizer->check_orders_table_exists() && ! $this->data_synchronizer->create_database_tables() ) {
				WC_Admin_Settings::add_error(
					__( 'Unable to create HPOS tables for synchronization.', 'fincommerce' )
				);
				return;
			}
	...
	/**
	 * Handler for the fincommerce_after_register_post_type post,
	...
	public function add_feature_definition( $features_controller ) {
		...
		$features_controller->add_feature_definition(
			'custom_order_tables',
			__( 'High-Performance order storage', 'fincommerce' ),
			$definition
		);
	}
	...
	private function get_hpos_setting_for_feature() {
		...
		$get_disabled = function () {
			...
			$disabled           = array();
			// Changing something here? You might also want to look at `enable|disable` functions in Automattic\FinCommerce\Database\Migrations\CustomOrderTable\CLIRunner.
		...
		$get_sync_message = function () {
			...
			if ( $is_dangerous ) {
				$sync_message[] = wp_kses_data(
					__( "There are orders pending sync.", 'fincommerce' )
					. '<strong>'
					. __( 'Switching data storage while sync is incomplete is dangerous and can lead to order data corruption or loss!', 'fincommerce' )
					. '</strong>'
				);
			}

			if ( ! $sync_enabled && $this->data_synchronizer->background_sync_is_enabled() ) {
				$sync_message[] = __( 'Background sync is enabled.', 'fincommerce' );
			}
			...
			if ( $sync_in_progress && $sync_is_pending ) {
				...
				$sync_message[] = sprintf(
					...
					__( 'Currently syncing orders... %s pending', 'fincommerce' ),
				...
				if ( ! $is_dangerous ) {
					$sync_message[] = wp_kses_data(
						__( "You can switch order data storage <strong>only when the posts and orders tables are in sync</strong>. There are currently orders out of sync.", 'fincommerce' ),
					);
				}
				...
				$sync_message[] = sprintf(
					...
					__( 'Sync orders now', 'fincommerce' )
				);
			}
