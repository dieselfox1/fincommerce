...
use Automattic\FinCommerce\Internal\Fulfillments\FulfillmentUtils;
use WC_Meta_Data;
...
class FulfillmentsDataStore extends \WC_Data_Store_WP implements \WC_Object_Data_Store_Interface, FulfillmentsDataStoreInterface {
	...
	public function create( &$data ): void {
		...
		if ( ! $data->get_entity_type() ) {
			throw new \Exception( esc_html__( 'Invalid entity type.', 'fincommerce' ) );
		}
		if ( ! $data->get_entity_id() ) {
			throw new \Exception( esc_html__( 'Invalid entity ID.', 'fincommerce' ) );
		}
		if ( ! FulfillmentUtils::is_valid_fulfillment_status( $data->get_status() ) ) {
			throw new \Exception( esc_html__( 'Invalid fulfillment status.', 'fincommerce' ) );
		}
		 ...
		 * @since 10.1.0
		 */
		$data = apply_filters( 'fincommerce_fulfillment_before_create', $data );
		...
		if ( $is_fulfill_action ) {
			 ...
			 */
			$data = apply_filters(
				'fincommerce_fulfillment_before_fulfill',
				$data
			);
		}
		...
		if ( false === $rows_inserted ) {
			throw new \Exception( esc_html__( 'Failed to insert fulfillment.', 'fincommerce' ) );
		}
		...
		if ( ! doing_action( 'fincommerce_fulfillment_after_create' ) ) {
			/**
			...
			* @since 10.1.0
			*/
			do_action( 'fincommerce_fulfillment_after_create', $data );
		}

		if ( $is_fulfill_action && ! doing_action( 'fincommerce_fulfillment_after_fulfill' ) ) {
			/**
			 * Action to perform after a fulfillment is fulfilled.
			 *
			 * @since 10.1.0
			 */
			do_action( 'fincommerce_fulfillment_after_fulfill', $data );
		}
	}
	...
	public function read( &$data ): void {
		...
		if ( empty( $fulfillment_data ) ) {
			throw new \Exception( esc_html__( 'Fulfillment not found.', 'fincommerce' ) );
		}
	...
	public function update( &$data ): void {
		 ...
		 * @since 10.1.0
		 */
		$data = apply_filters( 'fincommerce_fulfillment_before_update', $data );
		...
		// Check for errors.
		if ( $wpdb->last_error ) {
			throw new \Exception( esc_html__( 'Failed to update fulfillment.', 'fincommerce' ) );
		}
		...
		if ( ! doing_action( 'fincommerce_fulfillment_after_update' ) ) {
			/**
			 ...
			 * @since 10.1.0
			 */
			do_action( 'fincommerce_fulfillment_after_update', $data );
		}
	...
	public function delete( &$data, $args = array() ): void {
		 ...
		 * @since 10.1.0
		 */
		$data = apply_filters( 'fincommerce_fulfillment_before_delete', $data );
		...
		// Check for errors.
		if ( $wpdb->last_error ) {
			throw new \Exception( esc_html__( 'Failed to delete fulfillment.', 'fincommerce' ) );
		}
		...
		if ( ! doing_action( 'fincommerce_fulfillment_after_delete' ) ) {
			/**
			 * Action to perform after a fulfillment is deleted.
			 *
			 * @since 10.1.0
			 */
			do_action( 'fincommerce_fulfillment_after_delete', $data );
		}
	...
	public function read_meta( &$data ): array {
		if ( ! $data->get_id() ) {
			throw new \Exception( esc_html__( 'Invalid fulfillment.', 'fincommerce' ) );
		}
	...
	public function delete_meta( &$data, $meta ): void {
		...
		if ( $data->get_date_deleted() ) {
			throw new \Exception( esc_html__( 'Cannot delete meta from a deleted fulfillment.', 'fincommerce' ) );
		}

		$meta_id = $meta->id;
		if ( ! is_numeric( $data_id ) || $data_id <= 0 || ! is_numeric( $meta_id ) || $meta_id <= 0 ) {
			throw new \Exception( esc_html__( 'Invalid fulfillment or meta.', 'fincommerce' ) );
		}
	...
	public function add_meta( &$data, $meta ): int {
		...
		if ( $data->get_date_deleted() ) {
			throw new \Exception( esc_html__( 'Cannot add meta to a deleted fulfillment.', 'fincommerce' ) );
		}
		...
		if ( $wpdb->last_error ) {
			throw new \Exception( esc_html__( 'Failed to insert fulfillment meta.', 'fincommerce' ) );
		}
	...
	public function update_meta( &$data, $meta ): int {
		...
		if ( $data->get_date_deleted() ) {
			throw new \Exception( esc_html__( 'Cannot update meta for a deleted fulfillment.', 'fincommerce' ) );
		}
		...
		// Check for errors.
		if ( $wpdb->last_error ) {
			throw new \Exception( esc_html__( 'Failed to update fulfillment meta.', 'fincommerce' ) );
		}
	...
	public function read_fulfillments( string $entity_type, string $entity_id, bool $with_deleted = false ): array {
		...
		if ( is_wp_error( $fulfillment_data ) ) {
			throw new \Exception( esc_html__( 'Failed to read fulfillment data.', 'fincommerce' ) );
		}
	...
	private function validate_items( Fulfillment $data ): void {
		...
		if ( empty( $items ) ) {
			throw new \Exception( esc_html__( 'The fulfillment should contain at least one item.', 'fincommerce' ) );
		}

		if ( ! is_array( $items ) ) {
			throw new \Exception( esc_html__( 'The fulfillment items should be an array.', 'fincommerce' ) );
		}
		...
		foreach ( $data->get_items() as $item ) {
			if ( ! isset( $item['item_id'] )
				...
				throw new \Exception( esc_html__( 'Invalid item.', 'fincommerce' ) );
			}
		}
	}
}
