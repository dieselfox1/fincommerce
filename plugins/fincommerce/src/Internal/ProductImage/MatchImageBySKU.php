...
namespace Automattic\FinCommerce\Internal\ProductImage;
...
class MatchImageBySKU {
	 ...
	 * @var string
	 */
	private $setting_name = 'fincommerce_product_match_featured_image_by_sku';
	...
	private function init_hooks() {
		add_filter( 'fincommerce_get_settings_products', array( $this, 'add_product_image_sku_setting' ), 110, 2 );
	}
	...
	/**
	 * Handler for 'fincommerce_get_settings_products', adds the settings related to the product image SKU matching table.
	 *
	...
	public function add_product_image_sku_setting( array $settings, string $section_id ): array {
		...
		$settings[] = array(
			'title' => __( 'Product image matching by SKU', 'fincommerce' ),
			'type'  => 'title',
		);

		$settings[] = array(
			'title'         => __( 'Match images', 'fincommerce' ),
			'desc'          => __( 'Set product featured image when uploaded image file name matches product SKU.', 'fincommerce' ),
