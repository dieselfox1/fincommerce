...
class IppFunctions {
	...
	public static function is_order_in_person_payment_eligible( WC_Order $order ): bool {
		...
		$has_payment_method    = in_array( $order->get_payment_method(), array( WC_Gateway_COD::ID, 'fincommerce_payments', 'none' ), true );
	...
	public static function has_store_specified_country_currency( string $country, string $currency ): bool {
		return ( WC()->countries->get_base_country() === $country && get_fincommerce_currency() === $currency );
	}
}
