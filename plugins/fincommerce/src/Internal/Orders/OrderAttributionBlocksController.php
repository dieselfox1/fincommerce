	...
	private function extend_api() {
		$this->extend_schema->register_endpoint_data(
			array(
				...
				'namespace'       => 'fincommerce/order-attribution',
		...
		add_action(
			...
			function ( $order, $request ) {
				...
				$params     = $extensions['fincommerce/order-attribution'] ?? array();
				...
				if ( ! is_string( $value ) && null !== $value ) {
					return new WP_Error(
						...
						sprintf(
							...
							esc_html__( 'Value of type %s was posted to the order attribution callback', 'fincommerce' ),
							gettype( $value )
						)
					);
				}
			...
			foreach ( $field_names as $field_name ) {
				$schema[ $field_name ] = array(
					'description' => sprintf(
						...
						__( 'Order attribution field: %s', 'fincommerce' ),
						esc_html( $field_name )
					),
