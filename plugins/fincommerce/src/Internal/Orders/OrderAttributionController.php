...
use Automattic\FinCommerce\Internal\Integrations\WPConsentAPI;
use Automattic\FinCommerce\Internal\RegisterHooksInterface;
use Automattic\FinCommerce\Internal\Traits\ScriptDebug;
use Automattic\FinCommerce\Internal\Traits\OrderAttributionMeta;
...
class OrderAttributionController implements RegisterHooksInterface {
	...
	/**
	 * FinCommerce logger class instance.
	 *
	 ...
	 *
	 * Takes the place of the constructor within FinCommerce Dependency injection.
	 *
	 * @internal
	 *
	...
	public function on_init() {
		...
		$stamp_checkout_html_actions = apply_filters(
			...
			array(
				'fincommerce_checkout_billing',
				'fincommerce_after_checkout_billing_form',
				'fincommerce_checkout_shipping',
				'fincommerce_after_order_notes',
				'fincommerce_checkout_after_customer_details',
			)
		);
		...
		add_action( 'fincommerce_register_form', array( $this, 'stamp_html_element' ) );

		// Update order based on submitted fields.
		add_action(
			'fincommerce_checkout_order_created',
			function ( $order ) {
				 ...
				 */
				do_action( 'fincommerce_order_save_attribution_data', $order, $params );
			}
		);

		add_action(
			'fincommerce_order_save_attribution_data',
			function ( $order, $data ) {
	...
	private function log( string $message, string $method, string $level = WC_Log_Levels::DEBUG ) {
		...
		$this->logger->log(
			...
			array( 'source' => 'fincommerce-order-attribution' )
		);
	}
	...
	private function register_order_origin_column() {
		...
		$add_column = function ( $columns ) {
			$columns['origin'] = esc_html__( 'Origin', 'fincommerce' );
