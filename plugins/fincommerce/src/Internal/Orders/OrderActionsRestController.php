...
namespace Automattic\FinCommerce\Internal\Orders;
...
class OrderActionsRestController extends RestApiControllerBase {
	...
	public function register_routes(): void {
		register_rest_route(
			...
			array(
				'args'   => array(
					'id' => array(
						'description' => __( 'Unique identifier of the order.', 'fincommerce' ),
						'type'        => 'integer',
					),
				),
				array(
	...
	private function validate_order_id( WP_REST_Request $request ) {
		...
		if ( ! $order ) {
			return new WP_Error( 'fincommerce_rest_not_found', __( 'Order not found', 'fincommerce' ), array( 'status' => 404 ) );
		}
	...
	private function get_args_for_order_actions( string $action_slug ): array {
		$args = array(
			'email'              => array(
				'description'       => __( 'Email address to send the order details to.', 'fincommerce' ),
				'type'              => 'string',
				'format'            => 'email',
				'context'           => array( 'edit' ),
				'required'          => false,
			...
			'force_email_update' => array(
				'description'       => __( 'Whether to update the billing email of the order, even if it already has one.', 'fincommerce' ),
				'type'              => 'boolean',
				'context'           => array( 'edit' ),
				'required'          => false,
		...
		if ( 'send_email' === $action_slug ) {
			$args['template_id'] = array(
				'description'       => __( 'The ID of the template to use for sending the email.', 'fincommerce' ),
				'type'              => 'string',
	...
	public function get_schema_for_email_templates(): array {
		$schema = array(
			...
			'properties' => array(
				'id'          => array(
					'description' => __( 'A unique ID string for the email template.', 'fincommerce' ),
					'type'        => 'string',
					'enum'        => $this->get_template_id_enum(),
					'context'     => array( 'view', 'embed' ),
				),
				'title'       => array(
					'description' => __( 'The display name of the email template.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
				),
				'description' => array(
					'description' => __( 'A description of the purpose of the email template.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
				),
			),
	...
	public function get_schema_for_order_actions(): array {
		$schema = array(
			'$schema'    => 'http://json-schema.org/draft-04/schema#',
			'title'      => __( 'Order Actions', 'fincommerce' ),
			'type'       => 'object',
			'properties' => array(
				'message' => array(
					'description' => __( 'A message indicating that the action completed successfully.', 'fincommerce' ),
					'type'        => 'string',
					'context'     => array( 'edit' ),
					'readonly'    => true,
				),
	...
	private function get_available_email_templates( WC_Order $order ): array {
		 ...
		 * of the email. See the `fincommerce_rest_order_actions_email_send` action hook.
		 *
		 * @since 9.8.0
		 *
		...
		$valid_template_classes = apply_filters(
			'fincommerce_rest_order_actions_email_valid_template_classes',
	...
	protected function send_email( WP_REST_Request $request ) {
		...
		if ( ! is_email( $order->get_billing_email() ) ) {
			return new WP_Error(
				'fincommerce_rest_missing_email',
				__( 'Order does not have an email address.', 'fincommerce' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( is_null( $template ) ) {
			return new WP_Error(
				'fincommerce_rest_invalid_email_template',
				sprintf(
					// translators: %s is a string ID for an email template.
					__( '%s is not a valid template for this order.', 'fincommerce' ),
		...
		switch ( $template_id ) {
			...
			case 'customer_completed_order':
				...
				do_action( 'fincommerce_order_status_completed_notification', $order->get_id(), $order );
				break;
			case 'customer_failed_order':
				/** This action is documented in includes/class-wc-emails.php */
				do_action( 'fincommerce_order_status_failed_notification', $order->get_id(), $order );
				break;
			...
			case 'customer_processing_order':
				/** This action is documented in includes/class-wc-emails.php */
				do_action( 'fincommerce_order_status_pending_to_processing_notification', $order->get_id(), $order );
				break;
			case 'customer_refunded_order':
				if ( $this->order_is_partially_refunded( $order ) ) {
					/** This action is documented in includes/class-wc-emails.php */
					do_action( 'fincommerce_order_partially_refunded_notification', $order->get_id() );
				} else {
					/** This action is documented in includes/class-wc-emails.php */
					do_action( 'fincommerce_order_fully_refunded_notification', $order->get_id() );
				}
				break;
			...
			default:
				 ...
				 * See the `fincommerce_rest_order_actions_email_valid_template_classes` filter hook.
				 *
				 * @since 9.8.0
				 *
				 ...
				 */
				do_action( 'fincommerce_rest_order_actions_email_send', $order->get_id(), $template_id );
				break;
		}
		...
		$messages[] = sprintf(
			...
			esc_html__( 'Email template "%1$s" sent to %2$s, via %3$s.', 'fincommerce' ),
	...
	protected function send_order_details( WP_REST_Request $request ) {
		...
		do_action( 'fincommerce_before_resend_order_emails', $order, 'customer_invoice' );
		...
		$messages[] = sprintf(
			...
			esc_html__( 'Order details sent to %1$s, via %2$s.', 'fincommerce' ),
			esc_html( $order->get_billing_email() ),
			$user_agent ? $user_agent : 'REST API'
		);
		...
		// phpcs:disable FinCommerce.Commenting.CommentHooks.MissingSinceComment
		/** This action is documented in includes/admin/meta-boxes/class-wc-meta-box-order-actions.php */
		do_action( 'fincommerce_after_resend_order_email', $order, 'customer_invoice' );
	...
	private function maybe_update_billing_email( WC_Order $order, string $email, ?bool $force = false ) {
		...
		if ( $existing_email && true !== $force ) {
			return new WP_Error(
				'fincommerce_rest_order_billing_email_exists',
				__( 'Order already has a billing email.', 'fincommerce' ),
				array( 'status' => 400 )
			);
		}
		...
		return sprintf(
			...
			__( 'Billing email updated to %s.', 'fincommerce' ),
			esc_html( $email )
		);
	}
