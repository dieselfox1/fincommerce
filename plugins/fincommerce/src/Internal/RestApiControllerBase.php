 ...
 *
 * The following must be added at the end of the 'init_hooks' method in the 'FinCommerce' class,
 ...
 *   return array(
 *     'id' => array(
 *       'description' => __( 'Unique identifier of the foobar.', 'fincommerce' ),
 *       'type'        => 'integer',
 ...
 *     'message'     => array(
 *       'description' => __( 'A message.', 'fincommerce' ),
 *       'type'        => 'string',
 *       'context'     => array( 'view', 'edit' ),
 *       'readonly'    => true,
 *     ),
...
abstract class RestApiControllerBase implements RegisterHooksInterface {
	...
	public function register() {
		add_filter( 'fincommerce_rest_api_get_rest_namespaces', array( $this, 'handle_fincommerce_rest_api_get_rest_namespaces' ) );
	}

	/**
	 * Handle the fincommerce_rest_api_get_rest_namespaces filter
	 * to add ourselves to the list of REST API controllers registered by FinCommerce.
	 *
	 * @param array $namespaces The original list of FinCommerce REST API namespaces/controllers.
	 * @return array The updated list of FinCommerce REST API namespaces/controllers.
	 *
	 * @internal For exclusive usage of FinCommerce core, backwards compatibility not guaranteed.
	 */
	public function handle_fincommerce_rest_api_get_rest_namespaces( array $namespaces ): array {
		$namespaces['wc/v3'][ $this->get_rest_api_namespace() ] = static::class;
		return $namespaces;
	}

	/**
	 * Get the FinCommerce REST API namespace for the class. It must be unique across all other derived classes
	 ...
	 * if the user has the 'manage_fincommerce' capability.
	 *
	...
	protected function run( WP_REST_Request $request, string $method_name ) {
		try {
			...
			return new WP_Error( 'fincommerce_rest_invalid_argument', $message ? $message : __( 'Internal server error', 'fincommerce' ), array( 'status' => 400 ) );
		} catch ( Exception $ex ) {
	...
	protected function internal_wp_error( Exception $exception ): WP_Error {
		...
		return new WP_Error( 'fincommerce_rest_internal_error', __( 'Internal server error', 'fincommerce' ), $data );
	}
	...
	protected function get_authentication_error_by_method( string $method ) {
		$errors = array(
			'GET'    => array(
				'code'    => 'fincommerce_rest_cannot_view',
				'message' => __( 'Sorry, you cannot view resources.', 'fincommerce' ),
			),
			'POST'   => array(
				'code'    => 'fincommerce_rest_cannot_create',
				'message' => __( 'Sorry, you cannot create resources.', 'fincommerce' ),
			),
			'DELETE' => array(
				'code'    => 'fincommerce_rest_cannot_delete',
				'message' => __( 'Sorry, you cannot delete resources.', 'fincommerce' ),
			),
		);
