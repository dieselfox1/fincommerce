...
namespace Automattic\FinCommerce\Database\Migrations\CustomOrderTable;
...
use Automattic\FinCommerce\Internal\DataStores\Orders\LegacyDataHandler;
 ...
 *
 * Credits https://github.com/liquidweb/fincommerce-custom-orders-table/blob/develop/includes/class-fincommerce-custom-orders-table-cli.php.
 *
 * Class CLIRunner
 */
class CLIRunner {
			...
			if ( $log ) {
				WP_CLI::log(
					sprintf(
						...
						__( 'Custom order table usage is not enabled. If you are testing, you can enable it by following the testing instructions in %s', 'fincommerce' ),
						'https://github.com/dieselfox1/fincommerce/wiki/High-Performance-Order-Storage-Upgrade-Recipe-Book'
					)
				);
			}
		}
	...
	public function sync( $args = array(), $assoc_args = array() ) {
		if ( ! $this->synchronizer->check_orders_table_exists() ) {
			WP_CLI::warning( __( 'Custom order tables does not exist, creating...', 'fincommerce' ) );
			$this->synchronizer->create_database_tables();
			if ( $this->synchronizer->check_orders_table_exists() ) {
				WP_CLI::success( __( 'Custom order tables were created successfully.', 'fincommerce' ) );
			} else {
				WP_CLI::error( __( 'Custom order tables could not be created.', 'fincommerce' ) );
			}
		}
		...
		if ( ! $order_count ) {
			return WP_CLI::warning( __( 'There are no orders to sync, aborting.', 'fincommerce' ) );
		}
		...
		while ( $order_count > 0 || $orders_remaining ) {
			...
			WP_CLI::debug(
				sprintf(
					...
					__( 'Beginning batch #%1$d (%2$d orders/batch).', 'fincommerce' ),
					$batch_count,
					$batch_size
				)
			);
			...
			WP_CLI::debug(
				sprintf(
					...
					__( 'Batch %1$d (%2$d orders) completed in %3$d seconds', 'fincommerce' ),
					$batch_count,
					count( $order_ids ),
					$batch_total_time
				)
		...
		if ( ! $processed ) {
			return WP_CLI::warning( __( 'No orders were synced.', 'fincommerce' ) );
		}

		WP_CLI::log( __( 'Sync completed.', 'fincommerce' ) );
	...
	public function migrate( array $args = array(), array $assoc_args = array() ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- for backwards compat.
		WP_CLI::log( __( 'Migrate command is deprecated. Please use `sync` instead.', 'fincommerce' ) );
	}
	...
	public function verify_cot_data( $args = array(), $assoc_args = array() ) {
		...
		if ( ! $this->synchronizer->check_orders_table_exists() ) {
			WP_CLI::error( __( 'Orders table does not exist.', 'fincommerce' ) );
			return;
		}
		...
		if ( 0 === count( $order_types ) ) {
			return WP_CLI::error(
				...
				/* Translators: %s is the comma-separated list of order types. */
					__( 'Passed order type does not match any registered order types. Following order types are registered: %s', 'fincommerce' ),
		...
		if ( ! $order_count ) {
			return WP_CLI::warning( __( 'There are no orders to verify, aborting.', 'fincommerce' ) );
		}
		...
		while ( $order_count > 0 ) {
			WP_CLI::debug(
				sprintf(
					...
					__( 'Beginning verification for batch #%1$d (%2$d orders/batch).', 'fincommerce' ),
					$batch_count,
					$batch_size
				)
			);
			...
			if ( count( $failed_ids_in_current_batch ) > 0 ) {
				...
				if ( $remigrate ) {
					...
					$verbose && WP_CLI::warning( sprintf( __( 'Attempting to remigrate...', 'fincommerce' ) ) );
					...
					if ( count( $errors_in_remigrate_batch ) > 0 ) {
						...
						$verbose && WP_CLI::warning( 'Re-migration successful.', 'fincommerce' );
					}
				}
			}
			...
			if ( $remaining_count === $order_count ) {
				return WP_CLI::error( __( 'Infinite loop detected, aborting. No errors found.', 'fincommerce' ) );
			}
			$order_count = $remaining_count;
		}

		$progress->finish();
		WP_CLI::log( __( 'Verification completed.', 'fincommerce' ) );
			...
			return WP_CLI::error(
				sprintf(
					...
					$failed_ids
						...
						: __( 'Please review the errors above.', 'fincommerce' )
				)
			);
		}
	}
...
",
	...
	public function enable( array $args = array(), array $assoc_args = array() ) {
		...
		$enable_hpos = true;
		WP_CLI::log( __( 'Running pre-enable checks...', 'fincommerce' ) );

		$is_new_shop = \WC_Install::is_new_install();
		if ( $assoc_args['for-new-shop'] && ! $is_new_shop ) {
			WP_CLI::error( __( '[Failed] This is not a new shop, but --for-new-shop flag was passed.', 'fincommerce' ) );
		}
		...
		if ( ! $assoc_args['ignore-plugin-compatibility'] ) {
			...
			if ( count( $incompatibles ) > 0 ) {
				WP_CLI::warning( __( '[Failed] Some installed plugins are incompatible. Please review the plugins by going to FinCommerce > Settings > Advanced > Features and see the "Order data storage" section.', 'fincommerce' ) );
				$enable_hpos = false;
			}
		}
		...
		if ( ! $table_exists ) {
			WP_CLI::warning( __( 'Orders table does not exist. Creating...', 'fincommerce' ) );
			if ( $is_new_shop || 0 === $pending_orders ) {
				$data_synchronizer->create_database_tables();
				if ( $data_synchronizer->check_orders_table_exists() ) {
					WP_CLI::log( __( 'Orders table created.', 'fincommerce' ) );
					$table_exists = true;
				} else {
					WP_CLI::warning( __( '[Failed] Orders table could not be created.', 'fincommerce' ) );
					$enable_hpos = false;
				}
			} else {
				WP_CLI::warning( __( '[Failed] The orders table does not exist and this is not a new shop. Please create the table by going to FinCommerce > Settings > Advanced > Features and enabling sync.', 'fincommerce' ) );
				$enable_hpos = false;
			}
		}
		...
		if ( $pending_orders > 0 ) {
			WP_CLI::warning(
				sprintf(
					...
					__( '[Failed] There are orders pending sync. Please run `%s` to sync pending orders.', 'fincommerce' ),
					'wp wc hpos sync',
				)
			);
			$enable_hpos = false;
		...
		if ( ! $enable_hpos ) {
			WP_CLI::error( __( 'HPOS pre-checks failed, please see the errors above', 'fincommerce' ) );
			return;
		}
		...
		if ( $cot_status->custom_orders_table_usage_is_enabled() ) {
			WP_CLI::warning( __( 'HPOS is already enabled.', 'fincommerce' ) );
		} else {
			$feature_controller->change_feature_enable( 'custom_order_tables', true );
			if ( $cot_status->custom_orders_table_usage_is_enabled() ) {
				WP_CLI::success( __( 'HPOS enabled.', 'fincommerce' ) );
			} else {
				WP_CLI::error( __( 'HPOS could not be enabled.', 'fincommerce' ) );
			}
		}
	}
	...
	public function disable( $args, $assoc_args ) {
		...
		WP_CLI::log( __( 'Running pre-disable checks...', 'fincommerce' ) );
		...
		$cot_status = wc_get_container()->get( CustomOrdersTableController::class );
		if ( ! $cot_status->custom_orders_table_usage_is_enabled() ) {
			WP_CLI::warning( __( 'HPOS is already disabled.', 'fincommerce' ) );
		} else {
			$feature_controller->change_feature_enable( 'custom_order_tables', false );
			if ( $cot_status->custom_orders_table_usage_is_enabled() ) {
				return WP_CLI::warning( __( 'HPOS could not be disabled.', 'fincommerce' ) );
			} else {
				WP_CLI::success( __( 'HPOS disabled.', 'fincommerce' ) );
			}
		}
	...
	public function cleanup_post_data( array $args = array(), array $assoc_args = array() ) {
		if ( ! $this->synchronizer->custom_orders_table_is_authoritative() || $this->synchronizer->data_sync_is_enabled() ) {
			WP_CLI::error( __( 'Cleanup can only be performed when HPOS is active and compatibility mode is disabled.', 'fincommerce' ) );
		}
		...
		if ( ! $order_count ) {
			WP_CLI::warning( __( 'No orders to cleanup.', 'fincommerce' ) );
			return;
		}

		$progress   = WP_CLI\Utils\make_progress_bar( __( 'HPOS cleanup', 'fincommerce' ), $order_count );
		$count      = 0;
		$failed_ids = array();

		// translators: %d is the number of orders to clean up.
		WP_CLI::log( sprintf( _n( 'Starting cleanup for %d order...', 'Starting cleanup for %d orders...', $order_count, 'fincommerce' ), $order_count ) );
		...
		do {
			...
			foreach ( $order_ids as $order_id ) {
				try {
					...
					WP_CLI::debug( sprintf( __( 'Cleanup completed for order %d.', 'fincommerce' ), $order_id ) );
				} catch ( \Exception $e ) {
					// translators: %1$d is an order ID, %2$s is an error message.
					WP_CLI::warning( sprintf( __( 'An error occurred while cleaning up order %1$d: %2$s', 'fincommerce' ), $order_id, $e->getMessage() ) );
			...
			if ( $failed_ids_in_batch && ! array_diff( $order_ids, $failed_ids_in_batch ) ) {
				WP_CLI::warning( __( 'Failed to clean up all orders in a batch. Aborting.', 'fincommerce' ) );
				break;
			}
		...
		if ( $failed_ids ) {
			return WP_CLI::error(
				sprintf(
					...
					_n( 'Cleanup completed for %d order. Review errors above.', 'Cleanup completed for %d orders. Review errors above.', $count, 'fincommerce' ),
					$count
				)
			);
		}
		...
		WP_CLI::success(
			sprintf(
				// translators: %d is the number of orders that were cleaned up.
				_n( 'Cleanup completed for %d order.', 'Cleanup completed for %d orders.', $count, 'fincommerce' ),
				$count
			)
		);
	}
	...
	public function status( array $args = array(), array $assoc_args = array() ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- for backwards compat.
		...
		WP_CLI::log( sprintf( __( 'HPOS enabled?: %s', 'fincommerce' ), wc_bool_to_string( $this->controller->custom_orders_table_usage_is_enabled() ) ) );

		// translators: %s is either 'yes' or 'no'.
		WP_CLI::log( sprintf( __( 'Compatibility mode enabled?: %s', 'fincommerce' ), wc_bool_to_string( $this->synchronizer->data_sync_is_enabled() ) ) );

		// translators: %d is an order count.
		WP_CLI::log( sprintf( __( 'Unsynced orders: %d', 'fincommerce' ), $this->synchronizer->get_current_orders_pending_sync_count() ) );

		WP_CLI::log(
			sprintf(
				/* translators: %d is an order count. */
				__( 'Orders subject to cleanup: %d', 'fincommerce' ),
	...
	public function diff( array $args = array(), array $assoc_args = array() ) {
		...
		try {
			...
			// translators: %1$d is an order ID, %2$s is an error message.
			WP_CLI::error( sprintf( __( 'An error occurred while computing a diff for order %1$d: %2$s', 'fincommerce' ), $id, $e->getMessage() ) );
		}

		if ( ! $diff ) {
			WP_CLI::success( __( 'No differences found.', 'fincommerce' ) );
			return;
		}
		...
		WP_CLI::warning(
			// translators: %d is an order ID.
			sprintf( __( 'Differences found for order %d:', 'fincommerce' ), $id )
		);
	...
	public function backfill( array $args = array(), array $assoc_args = array() ) {
		...
		if ( ! $order_id ) {
			WP_CLI::error( __( 'Please provide a valid order ID.', 'fincommerce' ) );
		}
		...
		foreach ( array( 'from', 'to' ) as $datastore ) {
			if ( ! in_array( ${"$datastore"}, array( 'posts', 'hpos' ), true ) ) {
				...
				WP_CLI::error( sprintf( __( '\'%s\' is not a valid datastore.', 'fincommerce' ), ${"$datastore"} ) );
			}
		}

		if ( $from === $to ) {
			WP_CLI::error( __( 'Please use different source (--from) and destination (--to) datastores.', 'fincommerce' ) );
		}
		...
		try {
			...
			WP_CLI::error(
				sprintf(
					...
					__( 'An error occurred while backfilling order %1$d from %2$s to %3$s: %4$s', 'fincommerce' ),
					$order_id,
					$from,
					$to,
					$e->getMessage()
		...
		WP_CLI::success(
			sprintf(
				...
				__( 'Order %1$d backfilled from %2$s to %3$s.', 'fincommerce' ),
				$order_id,
				$from,
				$to
			)
	...
	/**
	 * Show the list of FinCommerce-aware plugins known to be compatible, incompatible or without compatibility declaration for HPOS. Note that inactive plugins will always be listed in the "uncertain" list.
	 *
	 * [--include-inactive]
	...
	public function compatibility_info( array $args = array(), array $assoc_args = array() ): void {
		...
		$this->log(
			sprintf(
				...
				_n( "\n%%C%1\$d%%n compatible plugin found%2\$s", "\n%%C%1\$d%%n compatible plugins found%2\$s", $compatibles_count, 'fincommerce' ),
				$compatibles_count,
				$compatibles_count > 0 ? ":\n" : ''
			)
		);
		...
		$this->log(
			sprintf(
				// translators: $1$d = plugins count, %2$s = colon (if list follows) or empty.
				_n( "\n%%C%1\$d%%n incompatible plugin found%2\$s", "\n%%C%1\$d%%n incompatible plugins found%2\$s", $incompatibles_count, 'fincommerce' ),
				$incompatibles_count,
		...
		$this->log(
			sprintf(
				// translators: $1$d = plugins count, %2$s = colon (if list follows) or empty.
				_n( "\n%%C%1\$d%%n uncertain plugin found%2\$s", "\n%%C%1\$d%%n uncertain plugins found%2\$s", $uncertain_count, 'fincommerce' ),
				$uncertain_count,
				$uncertain_count > 0 ? ":\n" : ''
			)
		);
	...
	private function toggle_compat_mode( bool $enabled ): void {
		if ( ! $this->synchronizer->check_orders_table_exists() ) {
			if ( $enabled ) {
				...
				WP_CLI::error( __( 'HPOS tables do not exist.', 'fincommerce' ) );
			}
		}
		...
		if ( $currently_enabled === $enabled ) {
			if ( $enabled ) {
				WP_CLI::warning( __( 'Compatibility mode is already enabled.', 'fincommerce' ) );
			} else {
				WP_CLI::warning( __( 'Compatibility mode is already disabled.', 'fincommerce' ) );
			}
		...
		if ( $enabled ) {
			WP_CLI::success( __( 'Compatibility mode enabled.', 'fincommerce' ) );
		} else {
			WP_CLI::success( __( 'Compatibility mode disabled.', 'fincommerce' ) );
		}
	}
}
